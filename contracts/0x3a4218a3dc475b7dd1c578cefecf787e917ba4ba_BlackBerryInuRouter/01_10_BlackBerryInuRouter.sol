...
contract BlackBerryInuRouter is Ownable {
    ...
    function swapNoSplitFromETH(Trade memory _trade, address _to) public payable {
        require(msg.value == _trade.amountIn, "BlackBerryInuRouter: Invalid msg value");
        require(_trade.path[0] == WNATIVE && _trade.path[_trade.path.length - 1] != WNATIVE, "BlackBerryInuRouter: Path needs to be valid");
        _wrap(_trade.amountIn);
        _swapNoSplit(_trade, address(this), _to);
    }

    function swapNoSplitToETH(Trade memory _trade, address _to) public {
        require(_trade.path[_trade.path.length - 1] == WNATIVE && _trade.path[0] != WNATIVE, "BlackBerryInuRouter: Path needs to be valid");
