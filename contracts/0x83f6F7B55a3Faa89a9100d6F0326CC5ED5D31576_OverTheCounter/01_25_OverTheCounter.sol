...
contract OverTheCounter is IERC721Receiver, Ownable, ReentrancyGuard {
  ...
  function poolUpdate(
    ...
    // refund any excess ETH provided by user
    uint256 _refundAmount = _pool.token0 == address(0)
      ? msg.value - _amount0ToAdd
      : msg.value;
    if (_refundAmount > 0) {
  ...
  function swapPool(
    ...
    // refund any excess ETH provided by swapper
    uint256 _refundAmount = _pool.token1 == address(0)
      ? msg.value - _amount1Provided
      : msg.value;
    if (_refundAmount > 0) {
      _sendETHOrERC20(address(this), msg.sender, address(0), _refundAmount);
    }
