...
/*
   ...
   __  _____              _ _    __    _            _ _ _          ____  _ _ _ 
  / / |  __ \            ( ) |   \ \  | |          | (_) |        |  _ \(_) | |
 | |  | |  | | ___  _ __ |/| |_   | | | |__   ___  | |_| | _____  | |_) |_| | |
 | |  | |  | |/ _ \| '_ \  | __|  | | | '_ \ / _ \ | | | |/ / _ \ |  _ <| | | |
 | |  | |__| | (_) | | | | | |_   | | | |_) |  __/ | | |   <  __/ | |_) | | | |
 | |  |_____/ \___/|_| |_|  \__|  | | |_.__/ \___| |_|_|_|\_\___| |____/|_|_|_|
  \_\                            /_/                                           
...
TG: https://t.me/billtokeneth
Twitter: @_be_like_bill
...
contract BILL is Context, Ownable, IERC20, ERC20Detailed {
  ...
  using SafeMath for uint256;
    ...
    address payable public marketingwallet = payable(0xBe2c5Ed7680Aff33F10C93594156C7dBa2Fd071b);
    
    bool inSwapAndLiquify;
    bool public swapAndLiquifyEnabled = true;
   
    uint256 private numTokensSellToAddToLiquidity = 420069 * 10**18;
    uint256 public maxWalletToken = 8401381 * (10**18);
    uint256 public _maxTxAmount = 8401381 * 10**18;
    ...
    constructor () ERC20Detailed("Bill", "BILL", 18) {
      _owner = msg.sender ;
    ...
    function setMarketingFeePercent(uint256 updatedMarketingFee) external onlyOwner() {
        ...
        require(marketingFee <= 30, "Fee limit reached");
    }
    ...
    {
        require(_numTokensSellToAddToLiquidity >= 4000 * (10**18), "Swap amount cannot be lower than 0.01% total supply.");
        require(_numTokensSellToAddToLiquidity <= 200000 * (10**18), "Swap amount cannot be higher than 0.5% total supply.");
    ...
    function setMaxWalletTokens(uint256 maxToken) external onlyOwner {
        require(maxToken >= 400000 * (10**18),"Max Walllet Tokens cannot be lesser than 1% of total supply" );
  	    maxWalletToken = maxToken ;
  	}

     function setMaxTx(uint256 maxTx) external onlyOwner() {
        require(maxTx >= 200000 * (10**18),"Max Transaction cannot be lesser than 0.5% of total supply" );
            _maxTxAmount = maxTx;
    }
