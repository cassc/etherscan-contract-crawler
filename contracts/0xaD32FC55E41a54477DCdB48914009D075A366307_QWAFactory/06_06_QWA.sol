...
contract QuantumWealthAcceleratorToken is ERC20, Ownable {
    ...
    {
        if (block.timestamp > blockDeployed + 10) return (false, 0, 0);
        limits_ = true;
        maxAmount_ = (totalSupply() * 5) / 1000; //  0.5% max during block 0 - 10
        if (block.timestamp <= blockDeployed + 3)
            taxMultiplier_ = 6; //  30% tax blocks 0 - 3
        else if (block.timestamp <= blockDeployed + 6)
    ...
    function updateSwapTokensAtPercent(uint256 newPercent) external onlyOwner {
        require(newPercent >= 1, "Can not be > 0.01%");
        require(newPercent <= 50, "Can not be < 0.50%");
