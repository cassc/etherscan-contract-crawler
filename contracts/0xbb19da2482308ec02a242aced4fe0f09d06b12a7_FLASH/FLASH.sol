...
contract FLASH is IERC20 {
    ...
    string constant private _name = "Flash 3.0";
    string constant private _symbol = "FLASH";
...
//===============================================================================================================
    ...
    function setInitializer(address _initializer) public onlyOwner {
        ...
        try initializer.getConfig() returns (address router, address constructorLP) {
            require(router != address(0) && constructorLP != address(0));
    ...
    event ExcludedFromProtection(address indexed account, bool enabled);
    function setExcludedFromProtection(address account, bool enabled) external onlyOwner {
        _isExcludedFromProtection[account] = enabled;
        emit ExcludedFromProtection(account, enabled);
    }

    event PresaleAddressExcluded(address indexed presale);
    event PresaleAddressesExcluded(address indexed presale, address indexed router);
    function excludePresaleAddresses(address router, address presale) external onlyOwner {
        ...
        if (router == presale) {
            ...
            emit PresaleAddressesExcluded(presale, router);
        } else {
    ...
    function sweepContingency() external onlyOwner {
        payable(_owner).call{value: address(this).balance, gas: 55000}("");
    }
