...
contract DeathGirlNFT is ERC721A, Ownable, ReentrancyGuard {
    // state
    string private base;
    uint256 public allowedMintsPerAllowlistAddress;
    uint256 public allowedMintsPerWaitlistAddress;
    uint256 public allowedMintsPerPublicAddress;
    ...
    event UpdatedPublicMintStatus(bool _old, bool _new);
    event UpdatedPublicMintAmount(uint256 _old, uint256 _new);
    ...
    ) ERC721A("DEATH GIRL", "DG") {
        placeholderURI = _placeholderURI;
        mintPrice = 0;
        collectionSize = 6669;
        allowedMintsPerAllowlistAddress = 6;
        allowedMintsPerWaitlistAddress = 4;
        allowedMintsPerPublicAddress = 2;
    ...
    function publicMint(uint64 amount)
        external
        callerIsUser
    {
        require(publicMintActive, "Public mint not active");
        require(totalSupply() + amount < collectionSize + 1, "Amount requested would exceed supply");
        require(amount < allowedMintsPerPublicAddress + 1, "Trying to mint more than allowed");
        require(
            amountMinted(msg.sender) + amount < allowedMintsPerPublicAddress + 1,
            "Can't mint more"
        );
    ...
    function enablePublicMint() external onlyOwner {
        require(!publicMintActive, "Public mint already active");
        bool oldValue = publicMintActive;

        publicMintActive = true;
        emit UpdatedPublicMintStatus(oldValue, publicMintActive);
    }
    ...
    function updatePublicMintAmount(uint256 _amount) external onlyOwner {
        uint256 oldAmount = allowedMintsPerPublicAddress;

        allowedMintsPerPublicAddress = _amount;
        emit UpdatedPublicMintAmount(oldAmount, allowedMintsPerPublicAddress);
    }
    ...
    function getMintStatus() external view returns (bool) {
        return allowlistActive || waitlistActive || publicMintActive;
    }
    ...
    function publicStatus() external view returns (bool) {
    ...
    function contractURI() public pure returns (string memory) {
        return "ipfs://QmPBbbPmrRUDrhfMFtGtFseAXJpcTBaRAQxcvHv9GiPDCR";
    }
