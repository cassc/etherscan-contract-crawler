...
contract sBlackDAO is IsBLKD, ERC20Permit {
    /* ========== DEPENDENCIES ========== */
    ...
    IgBLKD public gBLKD; // additional staked supply (governance token)
    ...
    constructor() ERC20("Staked BLKD", "sBLKD", 9) ERC20Permit("Staked BLKD") {
    ...
    function setgBLKD(address _gBLKD) external {
        require(msg.sender == initializer, "Initializer:  caller is not initializer");
        require(address(gBLKD) == address(0), "gBLKD:  gBLKD already set");
        require(_gBLKD != address(0), "gBLKD:  gBLKD is not a valid contract");
        gBLKD = IgBLKD(_gBLKD);
    }
    ...
    /**
        @notice increases rBLKD supply to increase staking balances relative to profit_
    ...
    // this function is called by the treasury, and informs sBLKD of changes to debt.
    // note that addresses with debt balances cannot transfer collateralized sBLKD
    ...
    function changeDebt(
        ...
        require(debtBalances[debtor] <= balanceOf(debtor), "sBLKD: insufficient balance");
    }
    ...
    // toG converts an sBLKD balance to gBLKD terms. gBLKD is an 18 decimal token. balance given is in 18 decimal format.
    function toG(uint256 amount) external view override returns (uint256) {
        return gBLKD.balanceTo(amount);
    }

    // fromG converts a gBLKD balance to sBLKD terms. sBLKD is a 9 decimal token. balance given is in 9 decimal format.
    function fromG(uint256 amount) external view override returns (uint256) {
        return gBLKD.balanceFrom(amount);
    }

    // Staking contract holds excess sBLKD
    function circulatingSupply() public view override returns (uint256) {
        return
            _totalSupply.sub(balanceOf(stakingContract)).add(gBLKD.balanceFrom(IERC20(address(gBLKD)).totalSupply())).add(
