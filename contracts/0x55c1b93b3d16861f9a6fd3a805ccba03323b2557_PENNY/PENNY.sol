...
contract PENNY is Context, AFTS {
    ...
    function _stakeProcess(address staker, uint256[] memory info, bytes memory sig) internal virtual {
        uint256 tempReward;
        require(!_general._isPaused, "Contract is Paused");
        _sigValidate(sig, keccak256(abi.encodePacked(staker, info[0], info[2], info[3], info[4], info[5], info[6], _general._baseChainId, _user[staker].nonce + 1, true)), _general._validatorAdd);
