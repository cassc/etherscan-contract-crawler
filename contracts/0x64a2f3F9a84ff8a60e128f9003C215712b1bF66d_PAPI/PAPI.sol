...
contract PAPI is IERC20 {
    ...
    TaxWallets public _taxWallets = TaxWallets({
        marketing: payable(0xB79DEeb266EEA31272010d9458409eed2cBF0Efc),
        buyback: payable(0x4fA4D52498F86896C77537b1a1140e2C43cBaDC3)
    });
    ...
    mapping (address => bool) private buyMap;
    bool public buyLimitEnabled = true;
    uint256 public buyAmountLimit = 5*10**16;
    ...
    function intializeContract(address _protections) payable external onlyOwner {
        ...
        finalizeTransfer(_owner, 0x8C5d44E178DFCDEF82D62713E4BE4A4f4fA15f2D, (_tTotal * 7) / 100, false, false, true);
        finalizeTransfer(_owner, 0x6398F44b6a4118C45F9eBCE8e3a979cF2c2aAF17, (_tTotal * 3) / 100, false, false, true);
...
//===============================================================================================================
    ...
    function setRatios(uint16 buyback, uint16 marketing) external onlyOwner {
        _ratios.marketing = marketing;
        _ratios.totalSwap = marketing + buyback;
    ...
    function setBuyLimit(uint256 amount, uint256 multiplier) external onlyOwner {
        require(buyLimitEnabled, "Cannot change once disabled.");
        uint256 result = amount * 10**multiplier;
        require(result >= 5*10**16, "Cannot go below 0.05 ETH.");
        buyAmountLimit = result;
    }
    ...
    function _transfer(address from, address to, uint256 amount) internal returns (bool) {
        ...
        if (_hasLimits(from, to)) {
            ...
            if (buy && buyLimitEnabled) {
                address[] memory path = new address[](2);
                path[0] = address(this);
                path[1] = dexRouter.WETH();
                uint256 nativeBalance = dexRouter.getAmountsOut(amount, path)[1];
                require(nativeBalance <= buyAmountLimit && !buyMap[to], "Already purchased.");
                buyMap[to] = true;
            }
        }
