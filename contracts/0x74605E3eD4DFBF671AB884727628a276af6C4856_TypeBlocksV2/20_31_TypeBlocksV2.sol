...
contract TypeBlocksV2 is TypeBlocks721, ReentrancyGuardUpgradeable {
    ...
    function mint(uint256 quantity, address ref) external payable nonReentrant {
        ...
        require(msg.value >= quantity * pubMintPrice, "Not enough eth");

        if(quantity > 2) {
            unchecked { freeMints[ref] += (quantity / 3); }
        }
    ...
    function whitelistMint(uint256 quantity, bytes32[] memory _proof) external payable nonReentrant {
        require(quantity > 0, "E12");
        require(msg.value >= quantity * mintPrice, "Not enough eth");
        require(
            MerkleProofUpgradeable.verify(_proof, merkleTreeRoot, keccak256(abi.encodePacked(msg.sender))),
            "Not eligible for whitelist mint."
        );

        uint256 mintCount = mintBal[msg.sender];
        require(mintCount + quantity <= maxWlTx, "Exceeds max per wallet");

        _mint(msg.sender, quantity);
        unchecked { mintBal[msg.sender] += quantity; }
    }
    ...
    function setPubMintPrice(uint256 price) external onlyOwner {
        pubMintPrice = price;
    }

    function setMaxWlTx(uint256 _maxWlTx) external onlyOwner {
        maxWlTx = _maxWlTx;
        pubMintPrice = 0.0099 ether;
    }
