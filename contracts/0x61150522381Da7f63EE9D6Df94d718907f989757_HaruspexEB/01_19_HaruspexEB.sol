...
contract HaruspexEB is TTTERC721Base {
        ...
        returns (string memory)
    {
        require(
            _exists(tokenId),
            "HaruspexEB: URI query for nonexistent token"
        );
    ...
    function updateTokenURI(uint256 tokenId, string calldata uri)
        ...
        onlyOwner
    {
        require(bytes(uri).length > 0, "HaruspexEB: no URI set for token");
    ...
    function mint(address, uint256)
        ...
        onlyUnsealed
    {
        require(false, "HaruspexEB: must use the mintWithUri to mint");
    }
