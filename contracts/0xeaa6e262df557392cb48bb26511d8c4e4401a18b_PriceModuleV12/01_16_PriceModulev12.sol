...
contract PriceModuleV12 is ChainlinkService, Initializable {
    ...
    function getUSDPrice(address _tokenAddress) public view returns (uint256) {
        ...
        uint256 usdtbal=(IConvex(tokens[_tokenAddress].feedAddress)
                .balances(0)).mul(1e12);                              
        uint256 wbtcbal=(IConvex(tokens[_tokenAddress].feedAddress)
                 .balances(1)).mul(1e10);
        uint256 wethbal=(IConvex(tokens[_tokenAddress].feedAddress)
                .balances(2));
        ...
        return(sum/IConvex(_tokenAddress).totalSupply());
        }
