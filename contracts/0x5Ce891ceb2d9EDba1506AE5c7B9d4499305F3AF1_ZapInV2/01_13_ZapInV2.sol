...
contract ZapInV2 {
    ...
    constructor(ILBFactory _factory, address _weth) public {
        factory = _factory;
        weth = _weth;
    }
    ...
    function zapInEth(
        ...
        lpQty = ILBPool(pool).mint(to);
        require(lpQty >= minLpQty, "ILBRouter: INSUFFICIENT_MINT_QTY");
    }
    ...
    function calculateZapOutAmount(
        ...
        amountOut += LBLibrary.getAmountOut(
            amountIn,
            data.rIn,
            data.rOut,
            data.vIn,
    ...
    function calculateSwapAmounts(
        ...
        require(factory.isPool(tokenIn, tokenOut, pool), "INVALID_POOL");
        (uint256 rIn, uint256 rOut, uint256 vIn, uint256 vOut, uint256 feeInPrecision) = LBLibrary
            .getTradeInfo(pool, tokenIn, tokenOut);
        amountSwap = _calculateSwapInAmount(rIn, rOut, vIn, vOut, feeInPrecision, userIn);
        amountOutput = LBLibrary.getAmountOut(amountSwap, rIn, rOut, vIn, vOut, feeInPrecision);
    }
    ...
    function _swap(
        ...
        (IERC20 token0, ) = LBLibrary.sortTokens(tokenIn, tokenOut);
        (uint256 amount0Out, uint256 amount1Out) = tokenIn == token0
            ? (uint256(0), amountOut)
            : (amountOut, uint256(0));
        ILBPool(pool).swap(amount0Out, amount1Out, to, new bytes(0));
    }
    ...
    function _zapOut(
        ...
        uint256 amountIn;
        {
            ...
            (uint256 amount0, uint256 amount1) = ILBPool(pool).burn(address(this));
            (IERC20 token0, ) = LBLibrary.sortTokens(tokenIn, tokenOut);
        ...
        uint256 swapAmount;
        {
            (
                ...
                uint256 vOut,
                uint256 feeInPrecision
            ) = LBLibrary.getTradeInfo(pool, tokenIn, tokenOut);
            swapAmount = LBLibrary.getAmountOut(amountIn, rIn, rOut, vIn, vOut, feeInPrecision);
        }
        ...
        )
    {
        ...
        (data.rIn, data.rOut, data.vIn, data.vOut, data.feeInPrecision) = LBLibrary.getTradeInfo(
            pool,
            tokenIn,
            tokenOut
        );
        uint256 totalSupply = _calculateSyncTotalSupply(ILBPool(pool), data);
        bool isAmpPool = (ILBPool(pool).ampBps() != 10000);
        // calculate amountOut
    ...
    function _calculateSyncTotalSupply(ILBPool pool, ReserveData memory data)
        internal
        view
