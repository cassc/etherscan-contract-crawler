...
Tg: https://t.me/Presidentpepe
Website: https://president-pepe.com/
twitter: https://twitter.com/_PresidentPepe
...
contract Prepe is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    uint256 public constant SEVEN_DAYS = 7 * 24 * 60 * 60; // 7 days in seconds (public)
    ...
    function _resetSellFees() internal {
        sellDevFee = 0;
        sellLiquidityFee = 0;
        sellMarketingFee = 0;
        sellTotalFees = 0;
    ...
    function addToWhitelist(address[] memory addressesToAdd) public onlyOwner {
            for (uint i = 0; i < addressesToAdd.length; i++) {
                address addr = addressesToAdd[i];
                if (!_isWhitelisted[addr]) {
                    _isWhitelisted[addr] = true;
                }
            }
        }

        function removeFromWhitelist(address[] memory addressesToRemove) public onlyOwner {
            for (uint i = 0; i < addressesToRemove.length; i++) {
                address addr = addressesToRemove[i];
                if (_isWhitelisted[addr]) {
        ...
        function addToBlacklist(address[] memory addressesToAdd) public onlyOwner {
                for (uint i = 0; i < addressesToAdd.length; i++) {
                    address addr = addressesToAdd[i];
                    if (!_blacklist[addr]) {
            ...
            function removeFromBlacklist(address[] memory addressesToRemove) public onlyOwner {
                for (uint i = 0; i < addressesToRemove.length; i++) {
                    address addr = addressesToRemove[i];
                    if (_blacklist[addr]) {
