...
contract ScalarBoosterPools is Ownable, ReentrancyGuard {
    ...
    function withdraw(uint256 _amount, uint8 _stakeType) external payable nonReentrant {
        ...
        for(uint256 j = 0; j < stakes.length; j++) {
            ...
            pending = pending + _pending;
            if(stake.end < block.timestamp || rewardsEndBlock < block.number) {
    ...
    function insufficientRewards() external view returns (uint256) {
        ...
        for(uint i = 0; i < lockups.length; i++) {
            if(startBlock == 0) {
                ...
                uint256 remainBlocks = _getMultiplier(lockups[i].lastRewardBlock, rewardsEndBlock);
    ...
    function userInfo(uint8 _stakeType, address _account) external view returns (uint256 amount, uint256 available, uint256 locked) {
        ...
        for(uint256 i = 0; i < stakes.length; i++) {
            ...
            if(block.timestamp > stake.end || rewardsEndBlock < block.number) {
    ...
    *************************/
    ...
    function increaseEmissionRate(uint8 _stakeType, uint256 _amount) external onlyOwner {
        ...
        require(rewardsEndBlock > block.number, "pool was already finished");
        require(_amount > 0, "invalid amount");
        ...
        if(remainRewards > shouldTotalPaid) {
            ...
            uint256 remainBlocks = rewardsEndBlock - block.number;
    ...
    function startReward() external onlyOwner {
        ...
        rewardsEndBlock = startBlock + duration * 6426;
        for(uint256 i = 0; i < lockups.length; i++) {
            lockups[i].lastRewardBlock = startBlock;
        }
        
        emit NewStartAndEndBlocks(startBlock, rewardsEndBlock);
    }
    ...
    function _getMultiplier(uint256 _from, uint256 _to)
        ...
        returns (uint256)
    {
        if (_to <= rewardsEndBlock) {
            return _to - _from;
        } else if (_from >= rewardsEndBlock) {
            return 0;
        } else {
            return rewardsEndBlock - _from;
        }
    }
