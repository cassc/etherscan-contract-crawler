...
/// @title StakeOshikuru
/// @notice The stake Oshikuru contract for staking and unstaking Oshikuru
contract StakeOshikuru is Ownable {
    using SafeERC20 for IERC20;
    using SafeMath for uint256;

    // Oshikuru Contract Address
    // solhint-disable-next-line var-name-mixedcase
    IERC20 public OshikuruContract;
    ...
    uint256 public totalOshikuruStaked;
    ...
    constructor(IERC20 oshikuruContractAddress) {
        require(address(oshikuruContractAddress) != address(0), "Cannot be address zero");
        OshikuruContract = oshikuruContractAddress;
    }
     ...
     * - Oshikuru balance of Staker must be greater than or equal to amount
     */
    function claimRewards() external {
        ...
        require(
            OshikuruContract.balanceOf(address(this)) >= rewardsEarned,
            "claimRewards: Insufficient contract balance"
        );

        OshikuruContract.safeTransfer(msg.sender, rewardsEarned);
    ...
    /**
     * @dev To stake Oshikuru in the contract
     *
     * Requirements:
     * - amount must be greater than 0
     * - Oshikuru balance of Staker must be greater than or equal to amount
    ...
    function stake(uint256 amount) external {
        ...
        uint256 stakerOshikuruBalance = OshikuruContract.balanceOf(msg.sender);
        require(stakerOshikuruBalance >= amount, "stake: Insufficient user balance");
        ...
        OshikuruContract.safeTransferFrom(msg.sender, address(this), amount);

        totalOshikuruStaked = totalOshikuruStaked.add(amount);
    ...
    /**
     * @dev To Unstake Oshikuru from the contract
     *
     * Requirements:
     * - Caller's staked Oshikuru must be greater than 0
     * - Unstake amount must be less than or equal to the staked Oshikuru
     * - Contract's Oshikuru balance must be greater than staked Oshikuru
     */
    function unstake(uint256 amount) external {
        ...
        totalOshikuruStaked = totalOshikuruStaked.sub(amount);
    ...
    function withdraw() external {
        ...
        require(
            OshikuruContract.balanceOf(address(this)) >= unstakeAmount,
            "withdraw: Insufficient contract balance"
        );

        OshikuruContract.safeTransfer(msg.sender, unstakeAmount);
    ...
    /**
     * @dev Returns the number of hours for which Oshikuru was staked
     */
