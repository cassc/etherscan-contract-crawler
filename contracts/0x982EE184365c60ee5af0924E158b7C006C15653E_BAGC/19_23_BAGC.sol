...
error BAGC_NOT_USER_TOKEN_ID(uint256 tokenId);
...
contract BAGC is ERC721URIStorageUpgradeable, ERC721EnumerableUpgradeable, BAGCCore {
    ...
    function ownerMint(address to, uint256 tokenId) public onlyOwner {
        require(!isUserToken(tokenId), "BAGC: it's user tokenId");
    ...
    function userMint(
        ...
        uint256 tokenId = getRandomAvailableTokenId(msg.sender);

        if (!isUserToken(tokenId)) {
            revert BAGC_NOT_USER_TOKEN_ID(tokenId);
        }
