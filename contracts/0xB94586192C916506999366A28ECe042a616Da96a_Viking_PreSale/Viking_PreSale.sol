...
contract Viking_PreSale is ReentrancyGuard, Ownable, Pausable {
    ...
    constructor() {
        ...
        baseDecimals = (10**18);
        salePrice = 90 * (10**10); // 10% bonus
        hardcapSize = 5_000_000;
        totalTokensForPresale = 28_800_000_000_000;
        minimumBuyAmount = 0;
        inSale = totalTokensForPresale;
        startTime = block.timestamp;
        endTime = block.timestamp + 30 days;
        dataOracle = 0x5f4eC3Df9cbd43714FE2740f5E3616155c5b8419;
        dAddress = 0x772910E887d69D9618BDA146212C8D924ff107eE;
    ...
    function startStep(uint256 stepIndex) internal {
        ...
        if (stepIndex == 1) {
            salePrice = 90 * (10**10); //10% Bonus $0.0000010 USD
            nextPrice = 12 * (10**11); //0.0000012 USD
            hardcapsizeUSD = 3000000;
            totalUsdValueForPresale = 3000000;
            inSaleUSDvalue = 3000000000000000000000000;
        } else if (stepIndex == 2) {
            salePrice = 12 * (10**11); //0.0000012 USD
            nextPrice = 15 * (10**11); //0.0000015 USD
            hardcapsizeUSD = 3500000;
            totalUsdValueForPresale = 3500000;
            inSaleUSDvalue = 3500000000000000000000000;
        } else if (stepIndex == 3){
            salePrice = 15 * (10**11); //0.0000015 USD
            nextPrice = 20 * (10**15); //0.0000020 USD
        ...
        whenNotPaused
    {
        uint256 usdPrice = calculatePrice(amount);
        require(!(usdPrice == 0 && currentStep == 3),"Presale it's over, sorry!");
        if (purchaseToken == 0 || purchaseToken == 1) usdPrice = usdPrice; //USDT have 6 decimals
        ...
        returns (uint256 usdPrice)
    {
        ...
        if (purchaseToken == 0 || purchaseToken == 1)
            usdPrice = usdPrice / (10**12); //USDT have 6 decimals
    }
