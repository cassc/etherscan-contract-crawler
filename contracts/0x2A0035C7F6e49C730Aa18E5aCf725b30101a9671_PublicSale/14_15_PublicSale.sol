...
contract PublicSale is Validatable, ReentrancyGuardUpgradeable {
     ...
     * emit {Bought} events
     */
    function buy(uint256 id, uint256 times) external payable nonReentrant notZero(times) validPublicSaleId(id) {
        PublicSaleInfo storage publicSaleInfo = publicSales[id];
        require(publicSaleInfo.status, "Public sale was cancel");
        ...
        IGenesis(publicSaleInfo.collection).mintBatch(_msgSender(), times);
