...
// File: contracts/Fiverr/Golden Access/trippybeasts.sol
...
contract TrippyBeasts is ERC721A, DefaultOperatorFilterer, Ownable, ERC2981 {
    uint256 private mintPrice;
    uint256 public regularMintPrice = 0.09 ether;
    uint256 public whiteListedMintPrice = 0.0675 ether;
    ...
    address payable public withdrawWallet = payable(0x3D5f623009C91725364D8cF094a606aaF08937a5);
    ...
    constructor(bytes32 _root, uint256 _mintingStartTime, string memory _preRevealUri, string memory _baseUri) ERC721A("Trippy Beasts", "TB") {
        root = _root;
    ...
    function setWhiteListMintPrice(uint256 _whiteListMintPrice ) public onlyOwner{
        whiteListedMintPrice = _whiteListMintPrice;
    }

    function setMintingStartTime(uint256 _time ) public onlyOwner{
        mintingStartTime = _time;
    }
    ...
    //mint function
    function mint(uint256 _quantity, bytes32[] memory proof) external payable {
        if(isWhitelisted(proof, keccak256(abi.encodePacked(msg.sender)))){
            mintPrice = whiteListedMintPrice;
            startTime = mintingStartTime - 1 hours;
        }else{
            mintPrice = regularMintPrice;
        ...
        require(block.timestamp >= startTime, "Minting will start soon");
