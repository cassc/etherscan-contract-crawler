...
contract AstralDungeon is Ownable, ERC721A, PaymentSplitter {
    ...
    uint private constant MAX_GIFT = 10000;
    uint private constant MAX_WHITELIST = 10000;
    uint private constant MAX_PUBLIC = 0;
    uint private constant MAX_SUPPLY_MINUS_GIFT = MAX_SUPPLY - MAX_GIFT;

    uint public wlSalePrice = 0.00 ether;
    uint public publicSalePrice = 0.50 ether;

    uint public saleStartTime = 1684598400;
    ...
    address[] private _team = [
        0x40a3aaBB22Ebb6c0024277A2143d07888C142F54
    ];
    ...
    **/
    function whitelistMint(address _account, uint _quantity, bytes32[] calldata _proof) external payable callerIsUser{
        ...
        require(currentTime() < saleStartTime +  100 hours, "Sale is finished");
    ...
    **/
    function publicMint(address _account, uint _quantity) external payable callerIsUser {
        require(!isPaused, "Contract is Paused");
        require(currentTime() >= saleStartTime + 100 hours, "Public sale has not started yet");
        require(currentTime() < saleStartTime + 200 hours, "Public Sale is finished");
        uint price = publicSalePrice;
        require(price != 0, "Price is 0");
        require(sellingStep == Step.PublicSale, "Public sale is not activated");
        require(amountNFTperWalletPublicSale[msg.sender] + _quantity <= maxPerAddressDuringPublicMint, "You can only get 1 NFTs on the Whitelist Sale");
