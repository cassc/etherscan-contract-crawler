...
contract Staking is Ownable {
    ...
    function setStandardRewardInterval(uint256 interval) public onlyOwner {
        require(interval > 0, "Staking: interval must be greater than 0");
    ...
    function injectRewards() public payable onlyCaller {
        if (msg.value > 0) {
            _injectRewardsWithTime(msg.value, standardRewardInterval);
        }
    }
    ...
    function withdraw(uint256 _pid, uint256 _amount) public {
        ...
        require(
            user.amount >= _amount,
            "Staking: Withdraw with insufficient balance"
        );
    ...
    function emergencyRewardWithdraw(uint256 _amount) public onlyOwner {
        require(_amount <= address(this).balance, "Staking: not enough token");
    ...
    function addCaller(address val) public onlyOwner {
        require(val != address(0), "Staking: val is the zero address");
        _callers.add(val);
    }

    function delCaller(address caller) public onlyOwner returns (bool) {
        require(caller != address(0), "Staking: caller is the zero address");
