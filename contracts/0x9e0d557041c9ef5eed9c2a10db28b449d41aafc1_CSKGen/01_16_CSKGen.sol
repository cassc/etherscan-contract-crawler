...
contract CSKGen is EIP712, AccessControl, ReentrancyGuard {
    ...
    uint256 public constant MAXIMUM_MINT_PRICE = 0.01 ether;
    uint256 public WHITELIST_PRICE = 0 ether;
    ...
    function getTotalMinted() public view returns (uint8, uint8, uint8) {
        return (
            mintTotalCount[msg.sender].Codelist,
            mintTotalCount[msg.sender].Waitlist,
            mintTotalCount[msg.sender].Public
        );
    }
        ...
        return
            _hashTypedDataV4(
                keccak256(
                    abi.encode(
                        keccak256(
                            "MintInfo(address minter,uint256 timestamp,uint256 mintType,uint256 metadata,uint256 metadata2,bool double,uint256 nonce)"
                        ),
                        info.minter,
                        info.timestamp,
                        info.mintType,
    ...
    function genToken(MintInfo calldata info) external payable nonReentrant {
        uint256 ethAmount;
        MintType mintType;
        require(block.timestamp <= info.timestamp + 1 minutes, "Times out");
        ...
        if (info.mintType == 0) {
            if(info.double) {
                revert ("Over Codelist Limit");
            } else {
                require(mintTotalCount[msg.sender].Codelist < 1, "Over Codelist Limit");
            }
            ...
            if(info.double) {
                require((mintTotalCount[msg.sender].Waitlist + 1) < 2, "Over Waitlist Limit");
                ethAmount = WAITLIST_PRICE * 2;
            } else {
                require(mintTotalCount[msg.sender].Waitlist < 2, "Over Waitlist Limit");
            ...
            if(info.double) {
                require((mintTotalCount[msg.sender].Public + 1) < 2, "Over Public Limit");
                ethAmount = MINT_PRICE * 2;
            } else {
                require(mintTotalCount[msg.sender].Public < 2, "Over Public Limit");
        ...
        if(info.double) {
            ...
            loopGenToken(mintType, info.metadata2);
        } else {
            loopGenToken(mintType, info.metadata);
        }
