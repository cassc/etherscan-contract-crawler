...
contract UnitLondonMarketplace is Vault, IOwnable {
  ...
  function onlyPlatform() internal view returns (bool) {
    return msg.sender == platform;
  }
  ...
  function onlyArtist(address artist) internal view returns (bool) {
    ...
    return msg.sender == platform || msg.sender == artist;
  }
  ...
  function updateCollectionMintFeePlatform(address collection, uint256 newMintFeePlatform) external {
    CollectionData storage data = collections[collection];

    require(onlyPlatform(), "Invalid permission");
  ...
  function addToken(
    ...
    require(onlyPlatform() || collections[collection].artist == msg.sender, "Invalid artist");
