...
contract ZombieHunter is ERC721AQueryable, Ownable, ReentrancyGuard {
 ...
 function massMint(address[] calldata _receivers) external onlyOwner nonReentrant {
    uint ctr = 0;
    uint _total = _receivers.length;
    require(totalSupply() + _total <= maxSupply, 'Max Supply exceeded!');
    while(ctr < _total) {
        nftClaimed[_msgSender()] = nftClaimed[_msgSender()] + 1;
        _safeMint(_receivers[ctr], 1);
        ctr = ctr + 1;
    }
  }
