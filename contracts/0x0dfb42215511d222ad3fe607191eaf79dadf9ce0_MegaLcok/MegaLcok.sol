...
contract MegaLcok is Ownable, ReentrancyGuard {
  ...
  mapping (address => uint256) private partnersAmount;
  ...
  function partnerAmount(address _address) public view returns (uint256) {
    return partnersAmount[_address];
  } 
   ...
   */
  function setPartnerTokenAndHold(address partners_, uint256 _hold) public onlyOwner {
    partners[partners_] = true;
    partnersAmount[partners_] = _hold;
  }

  function removePartner(address partners_) public onlyOwner {
      partners[partners_] = true;
  }
  
  function setFees(uint256 _partnerDiscount, uint256 _ethFee,uint256 _liquidityFee) public onlyOwner {
    gFees.referralDiscount = _partnerDiscount;
   ...
   * _partner the referrer address if any or address(0) for none
   * _fee_in_eth fees can be paid in eth 
   * _withdrawer the user who can withdraw liquidity once the lock expires.
   */
  function lockLPToken (address _lpToken, uint256 _amount, uint256 _unlock_date, address payable _partner, bool _fee_in_eth, address payable _withdrawer) external payable nonReentrant {
    ...
    if (_partner != address(0) && address(_partner) != address(0)) {
      require(isPartner(_partner), 'This address is not a partner');
      require(IERC(_partner).balanceOf(msg.sender) >= partnerAmount(_partner), 'INADEQUATE BALANCE');
    }
    // flatrate fees
