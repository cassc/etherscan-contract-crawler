...
contract MANEDAO is Ownable, IERC20 {
    ...
    constructor(
        ...
        excludeFromMaxTx(address(treasuryWallet), true);
        excludeFromMaxTx(address(marketingWallet), true);

        excludeFromMaxWallet(owner(), true);
        excludeFromMaxWallet(address(this), true);
        excludeFromMaxWallet(address(dividendTracker), true);
        excludeFromMaxWallet(address(treasuryWallet), true);
        excludeFromMaxWallet(address(marketingWallet), true);
        _mint(marketingWallet, 20_000_000_000 * (10**18));
        _mint(owner(), 80_000_000_000 * (10**18));
    }
        ...
        returns (bool)
    {
        ...
        require(
            ...
            "MANE: decreased allowance below zero"
        );
        _approve(_msgSender(), spender, currentAllowance - subtractedValue);
        return true;
    }
    ...
    function transferFrom(
        ...
        require(
            ...
            "MANE: transfer amount exceeds allowance"
        );
        _approve(sender, _msgSender(), currentAllowance - amount);
        return true;
    }
    ...
    function _transfer(
        ...
        require(!isBlacklisted[sender], "MANE: Sender is blacklisted");
        require(!isBlacklisted[recipient], "MANE: Recipient is blacklisted");

        require(sender != address(0), "MANE: transfer from the zero address");
        require(recipient != address(0), "MANE: transfer to the zero address");
        ...
        require(
            ...
            "MANE: transfer amount exceeds balance"
        );
    ...
    function _approve(
        ...
        require(owner != address(0), "MANE: approve from the zero address");
        require(spender != address(0), "MANE: approve to the zero address");
        _allowances[owner][spender] = amount;
        emit Approval(owner, spender, amount);
    }

    function _mint(address account, uint256 amount) private {
        require(account != address(0), "MANE: mint to the zero address");
        _totalSupply += amount;
        _balances[account] += amount;
        emit Transfer(address(0), account, amount);
    }
    ...
    function _burn(address account, uint256 amount) private {
        require(account != address(0), "MANE: burn from the zero address");
        uint256 accountBalance = _balances[account];
        require(accountBalance >= amount, "MANE: burn amount exceeds balance");
        _balances[account] = accountBalance - amount;
        _totalSupply -= amount;
        emit Transfer(account, address(0), amount);
    }
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(
            ...
            "MANE: account is already set to requested state"
        );
        _isExcludedFromFees[account] = excluded;
    ...
    function setAutomatedMarketMakerPair(address pair, bool value)
        ...
        onlyOwner
    {
        require(pair != uniswapV2Pair, "MANE: DEX pair can not be removed");
    ...
    function setFee(
        ...
        totalFeeBPS = _treasuryFee + _liquidityFee + _dividendFee + _marketingFee;

        treasurySellFeeBPS = _sellTreasuryFee;
        marketingSellFeeBPS = _sellMarketingFee;
        liquiditySellFeeBPS = _sellLiquidityFee;
        dividendSellFeeBPS = _sellDividendFee;
        totalSellFeeBPS = _sellTreasuryFee + _sellLiquidityFee + _sellDividendFee + _sellMarketingFee;
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(
            automatedMarketMakerPairs[pair] != value,
            "MANE: automated market maker pair is already set to that value"
        );
        automatedMarketMakerPairs[pair] = value;
        if (value) {
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(
            ...
            "MANE: the router is already set to the new address"
        );
    ...
    function compound() public {
        require(compoundingEnabled, "MANE: compounding is not enabled");
...
contract DividendTracker is Ownable, IERC20 {
    ...
    string private _name = "MANEDAO_DividendTracker";
    string private _symbol = "MANEDAO_DividendTracker";
    ...
    constructor(address _tokenAddress, address _uniswapRouter) {
        minTokenBalanceForDividends = 10_000_000 * (10**18);
        ...
        onlyOwner
    {
        require(
            ...
            "MANEDAO_DividendTracker: account already set to requested state"
        );
        excludedFromDividends[account] = excluded;
        if (excluded) {
            _setBalance(account, 0);
    ...
    function _mint(address account, uint256 amount) private {
        require(
            account != address(0),
            "MANEDAO_DividendTracker: mint to the zero address"
        );
        _totalSupply += amount;
        _balances[account] += amount;
        emit Transfer(address(0), account, amount);
    ...
    function _burn(address account, uint256 amount) private {
        require(
            account != address(0),
            "MANEDAO_DividendTracker: burn from the zero address"
        );
        uint256 accountBalance = _balances[account];
        require(
            accountBalance >= amount,
            "MANEDAO_DividendTracker: burn amount exceeds balance"
        );
        _balances[account] = accountBalance - amount;
        _totalSupply -= amount;
        emit Transfer(account, address(0), amount);
    ...
    function transfer(address, uint256) public pure override returns (bool) {
        revert("MANEDAO_DividendTracker: method not implemented");
    }
