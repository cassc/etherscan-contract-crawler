...
import "../found/DailyMint.sol";
import "./EncoderV1.sol";

contract CoinV1 is EncoderV1 {
    ...
    function ownerOf(uint tokenId) external view returns (address) {
        return _note.data().ownerOf(tokenId);
    }
    ...
    function coinStats() external view returns (string memory) {
        ...
        return string(
            abi.encodePacked(
                ...
                ',"totalSupply":', _encodeDecimals(_mint.totalSupply()),
    ...
    function coinBalances(address addr) external view returns (string memory) {
        ...
        for (uint coinId = 1; coinId <= total; coinId += 1) {
            ...
            if (coinId == total) {
                balances = abi.encodePacked(balances, _encodeDecimals(balance));
            } else {
                balances = abi.encodePacked(balances, _encodeDecimals(balance), ',');
            }
        }
    ...
    function tokenStats(uint coinId) public view returns (bytes memory) {
        uint artId = _note.coinToArt(coinId);
        return abi.encodePacked(
            '{"coinId":', coinId.toString(),
            ',"artId":', artId.toString(),
            ',"dailyBonus":', _encodeDecimals(_note.dailyBonus()),
            ',"address":"', uint(uint160(address(_mint.addressOf(coinId)))).toHexString(),
...
return abi.encodePacked(
            '","votes":', _encodeDecimals(_note.votesOnArt(coinId, artId)),
            ',"claim":', _encodeDecimals(_note.getClaims(coinId)),
            ',"supply":', _encodeDecimals(_mint.totalSupplyOf(coinId)),
            ',"rate":', _encodeDecimals(_mint.convertCoin(coinId, 1 ether))
        );
    }
    ...
    constructor(ArtData data_, FoundNote note_, DailyMint mint_) EncoderV1(data_) {
        _note = note_;
        _mint = mint_;
    }
}