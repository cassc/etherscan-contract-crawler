...
contract Bidder is Ownable {
     ...
     */
    function withdraw(uint256 _tId) external {
        uint256 startGas = gasleft();

        LastBid storage lb = lastBidForToken[_tId];
        if (lb.bidder == address(0)) {
            revert NoBidFoundForToken(_tId);
        }
        ...
        token.safeTransferFrom(address(this), config.receiver, _tId);
    ...
    function getLastBid(uint256 _tId) external view returns (LastBid memory) {
        return lastBidForToken[_tId];
    }
