...
https://0xpixel.guru
https://t.me/OxPixel
https://twitter.com/0xPixel_ERC20
*/
...
contract OxPixel is IERC20, Ownable { 
    string private constant _name = "0xPixel";
    ...
    uint256 private constant contractSwapLimit = 25_000_001 * 10**_decimals;
    ...
    address payable private constant OxPixel = payable(0x902e7177FED11fEA8f42a1eBc7a51FD0dab796aB);
    ...
    modifier tradingLock(address sender) {
        require(tradingOpen || sender == deployerAddress || sender == OxPixel); 
        _;
    }
    ...
    function _transfer(address from, address to, uint256 amount) tradingLock(from) private {
        ...
        if (from != address(this) && from != OxPixel && to != OxPixel && to != deployerAddress) {  
    ...
    function swapback(uint256 tokenAmount) private swapLock {
        ...
        if(contractETHBalance > 0) {
            OxPixel.transfer(contractETHBalance); 
        }
    }
