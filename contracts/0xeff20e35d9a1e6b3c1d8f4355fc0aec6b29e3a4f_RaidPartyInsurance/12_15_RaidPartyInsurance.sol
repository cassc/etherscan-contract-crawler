...
contract RaidPartyInsurance is RaidPartyInsuranceHelper {
    ...
    function buyInsuranceFighterBulk(
        ...
        uint256[] calldata _fighterPosition
    )
        external
    {
    ...
    function activeRemainingConfettiReserves()
        external
        view
        returns (uint256)
    {
        ...
        return confettiReserves
            - confettiReservesPerBatch[batch]
            - confettiReservesPerBatch[batch + 1];
    }
    ...
    function getFighterPosByTokenIDAndUser(
        address _user,
        uint256 _tokenID
    )
        external
        ...
        returns (uint256)
    {
        ...
        for (uint256 i = 0; i < length; i++) {
            currentID = mainGame.getUserFighters(_user)[i];
            if (currentID == _tokenID) return i;
        }

        revert(
            "RaidPartyInsurance: FIGHTER_NOT_EQUIPPED_BY_USER"
        );
    }
