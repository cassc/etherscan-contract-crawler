...
// File contracts/common/UniversalUpgradeable.sol
...
// File contracts/common/ERC721BaseV2.sol
...
contract AccessControlEnumerableV1Storage is AccessControlUpgradeable {
    mapping(bytes32 => EnumerableSetUpgradeable.AddressSet) private _roleMembers;
    uint256[49] private __gap;
}

contract ERC721EnumerableV1Storage is Initializable, ERC721Upgradeable {
...
contract ERC721BurnableV1Storage is Initializable, ContextUpgradeable, ERC721Upgradeable {
    uint256[50] private __gap;
}

contract ERC721BaseV2 is
    AccessControlEnumerableV1Storage,
    ERC721EnumerableV1Storage,
    ERC721BurnableV1Storage,
    ERC721RoyaltyUpgradeable,
    UniversalUpgradeable
{
...
// File contracts/pokavz/PFPNFTV2.sol

// License-Identifier: MIT
pragma solidity ^0.8.0;


contract PFPNFTV2 is ERC721BaseV2 {
    // errors
    error ZeroQuantity();
    ...
    bool public merkleMintEnabled;
    bool public publicMintEnabled;

    uint256 public maxPublicMintsPerWallet;

    // Mapping to keep track of the number of mints a given wallet has done on public mint.
    ...
    function initialize(
        ...
        __ERC721Base_init(owner, name, symbol, baseTokenURI, contractURI, feeReceiver, feeNumerator);
    }
    ...
    function getPublicMints(address account) public view virtual returns (uint256) {
        return _publicMints[account];
    }
    ...
    /**
     * @dev Mint token(s) for whitelist
     */
    function merkleMint(uint256 index, address account, uint256 quantity, bytes32[] calldata merkleProof) external {
        if (!merkleMintEnabled) revert MintNotEnabled();
        if (isClaimed(index)) revert AlreadyClaimed();

        uint256 currentTokenId = _validQuantityToMint(quantity);
    ...
    function publicMint(uint256 quantity) external {
        ...
        unchecked {
            ...
            if (maxPublicMintsPerWallet > 0) {
                ...
                if (mintsPerWallet + quantity > maxPublicMintsPerWallet) {
                    quantity = maxPublicMintsPerWallet - mintsPerWallet;
                }
            }

            mintsPerWallet += quantity;
            _publicMints[_msgSender()] = mintsPerWallet;
        }

        uint256 currentTokenId = _validQuantityToMint(quantity);

        _mintTokens(currentTokenId, _msgSender(), quantity);
    }

    // ----------------------------admin call------------------------------------

    /**
     * @dev Mint token(s) for reserve
     */
    function reserveMint(address to, uint256 quantity) external onlyOwner {
        uint256 currentTokenId = _validQuantityToMint(quantity);

        _mintTokens(currentTokenId, to, quantity);
    }
    ...
    function setMerkleMintEnabled(bool enabled) external onlyOwner {
        merkleMintEnabled = enabled;
    }
    ...
    function setMaxPublicMintsPerWallet(uint256 maxMintsPerWallet) external onlyOwner {
        maxPublicMintsPerWallet = maxMintsPerWallet;
    }

    function initializeSettings(
        uint256 _maxSupply,
        uint256 _maxPublicMintsPerWallet,
        bytes32 _merkleRoot
    ) external onlyOwner {
        ...
        merkleMintEnabled = true;
    }

    function setPhasePublicMint() external onlyOwner {
        merkleMintEnabled = false;
    ...
    /**
     * @dev Check max supply limit and return current token count
     */
    function _validQuantityToMint(uint256 quantityToMint) internal returns (uint256) {
        uint256 currentTokenCount = _tokenCount;
        if (_tokenCount + quantityToMint > maxSupply) revert MaxSupplyLimit();
        unchecked {
            _tokenCount += quantityToMint;
        }
        return currentTokenCount;
    }

    function _mintTokens(uint256 currentTokenId, address to, uint256 quantity) internal {
