...
abstract contract MarketplaceCore is ReentrancyGuard, IMarketplaceCore {
     ...
     */
    function _setFees(uint16 feeBPS, uint16 referrerBPS) internal {
        require(feeBPS + referrerBPS < 10000, "Invalid fee config");
        _feeBPS = feeBPS;
        _referrerBPS = referrerBPS;
    ...
    function createListing(MarketplaceLib.ListingDetails calldata listingDetails, MarketplaceLib.TokenDetails calldata tokenDetails, MarketplaceLib.DeliveryFees calldata deliveryFees, MarketplaceLib.ListingReceiver[] calldata listingReceivers, bool enableReferrer) external virtual override returns (uint256) {
        ...
        listing.marketplaceBPS = _feeBPS;
        if (enableReferrer) {
            listing.referrerBPS = _referrerBPS;
        }
     ...
     */
    function collect(uint40 listingId) external virtual override nonReentrant {
        require(listingId <= _listingCounter, "Listing not found");
        MarketplaceLib.Listing storage listing = _listings[listingId];
     ...
     */
    function _cancel(uint40 listingId, uint16 holdbackBPS, bool isAdmin) internal virtual nonReentrant {
        require(listingId <= _listingCounter, "Listing not found");
        MarketplaceLib.Listing storage listing = _listings[listingId];
        require((listing.flags & MarketplaceLib.FLAG_MASK_FINALIZED) == 0, "Listing not found");
