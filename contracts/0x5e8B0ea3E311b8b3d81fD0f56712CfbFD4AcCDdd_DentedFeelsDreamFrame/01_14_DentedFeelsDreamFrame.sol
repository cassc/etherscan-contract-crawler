...
contract DentedFeelsDreamFrame is ERC721, Ownable { 
...
string memory _initBaseURI
    ...
    function mint(string memory _ticket, bytes32[] calldata _merkleProof, uint256 _mintAmount) public payable {
		...
		require(supply.add(_mintAmount) <= maxSupply, "Not enough for request");
		require(ticketMintedCount + _mintAmount <= limitPerTicket, "No more for this Ticket");
    ...
    function reserveTokens(uint256 _amount) public onlyOwner {        
        uint256 supply = _tokenIdCounter.current();
        require(supply.add(_amount) <= maxSupply, "Not enough for request"); 
