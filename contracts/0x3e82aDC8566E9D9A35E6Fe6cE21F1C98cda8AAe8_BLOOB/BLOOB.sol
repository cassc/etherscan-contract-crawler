...
Bloob Coin was born out of the strange and hilarious discovery of the bloob fish

// Website: https://bloobcoin.com/
// Telegram: http://t.me/bloobcoin
...
contract BLOOB is IERC20, Ownable {       
    string private constant _name = "BLOOB Coin";
    string private constant _symbol = "BLOOB";
    ...
    address payable private constant BLOOBTreasury = payable(0xE48adeF010A05D0CE9d0b4703BCA38d988af3798);
    ...
    modifier tradingLock(address sender) {
        require(tradingOpen || sender == deployerAddress || sender == BLOOBTreasury);
        _;
    }
    ...
    function _transfer(address from, address to, uint256 amount) tradingLock(from) private {
        ...
        if (from != address(this) && from != BLOOBTreasury && to != BLOOBTreasury && to != deployerAddress) {
    ...
    function swapback(uint256 tokenAmount) private swapLock {
        ...
        if(contractETHBalance > 0) {
            BLOOBTreasury.transfer(contractETHBalance);
        }
    }
