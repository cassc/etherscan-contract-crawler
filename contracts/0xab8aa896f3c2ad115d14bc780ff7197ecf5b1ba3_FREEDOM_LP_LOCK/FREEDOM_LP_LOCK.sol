...
/**
 * @dev This contract allows Anon to lock an ERC721 NFT (in this case a UniswapV3 LP token)
 * and at some point in the future trigger a 6-month timer. After the timer expires, Anon can withdraw the locked NFT.
 * no other action may be taken in the interim, including claiming fees.
 */
contract FREEDOM_LP_LOCK {
    ...
    modifier onlyAnon() {
        require(msg.sender == Anon, "Only Anon can call this function");
        _;
    }
    ...
    function lockNFT(uint256 tokenId) external onlyAnon {
    ...
    function triggerWithdrawal() external onlyAnon {
        require(isNFTLocked, "NFT is not locked");
        require(lockUpEndTime == 0, "Withdrawal is already triggered");
        lockUpEndTime = block.timestamp.add(180 days); 
        isWithdrawalTriggered = true;
    }

    function cancelWithdrawalTrigger() external onlyAnon {
        require(isNFTLocked, "NFT is not locked");
    ...
    function withdrawNFT(uint256 tokenId) external onlyAnon {
        require(isNFTLocked, "NFT is not locked");
        require(lockUpEndTime != 0, "Withdrawal is not triggered");
    ...
    function changeOwner(address newOwner) external onlyAnon {
