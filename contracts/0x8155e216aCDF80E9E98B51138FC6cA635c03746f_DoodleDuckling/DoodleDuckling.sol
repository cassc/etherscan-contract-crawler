...
// File contracts/meme/DoodleDuckling.sol


pragma solidity ^0.8.0;
contract DoodleDuckling is ERC721Upgradeable, OwnableUpgradeable
{
    ...
    mapping(address => bool) public secondFreeMintedList;

    function initialize(string memory name, string memory symbol, address payable payee_, string memory baseURI_) public initializer {
    ...
    function freeMint() public {
        ...
        require(!secondFreeMintedList[_msgSender()],"You have minted all");
        uint num = 5;
        if(freeMintedList[_msgSender()]){
            num = 4;
        }
        require(_tokenId.current() + num <= MAX_AMOUNT, "No more Ducklings");
        for(uint i=0;i<num;i++){
            _tokenId.increment();
            _mint(_msgSender(), _tokenId.current());
        }
        secondFreeMintedList[_msgSender()] = true;
    }
