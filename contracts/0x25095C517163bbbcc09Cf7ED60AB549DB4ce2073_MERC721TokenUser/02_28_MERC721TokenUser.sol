...
contract MERC721TokenUser is MTokenUser, ERC721("MERC721","MERC721"), MERC721UserInterface {
     ...
     * If mToken is the anchorToken then a collection bid is placed and no instant sale is performed.
    ...
    function addAuctionBid(uint240 mToken) external payable nonReentrant2 {
        if (mToken == mtroller.getAnchorToken(address(this))) {
            require(tokenAuction.addOfferETH.value(msg.value)(mToken, msg.sender, address(0), 0) == 0, "IIS");
        }
        else {
