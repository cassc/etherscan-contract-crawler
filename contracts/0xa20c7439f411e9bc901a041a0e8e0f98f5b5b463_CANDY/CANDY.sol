...
/** 
    ...
    https://candycrush.site
    https://twitter.com/candycrusheth
    https://t.me/candycrushportal
...
// File @openzeppelin/contracts/token/ERC20/extensions/IERC20Meta[email protected]
...
contract CANDY is ERC20, Ownable {
    string private _name = unicode"Candy Crush";
    string private _symbol = unicode"CANDY";

    address private taxAddy = 0xF74956582D4525B4eB229647A0Aae9D09028E825;
    ...
    uint256 private constant _supply        = 1e9 ether;
    uint256 public maxTxAmount     = _supply * 25 / 1000;
    uint256 public maxWalletAmount = _supply * 25 / 1000;
    uint256 public swapThreshold = _supply * 2 / 10000;

    uint256 public buyTaxGlobal = 1;
    uint256 public sellTaxGlobal = 1;
    uint256 private sniperTax = 30;
    address private developmentAddy = 0x76FCEAf3D7C75A8e4504709bc0e9864F9F820a3C;
    ...
    function _transfer(
        ...
        if (!isFeeExempt[from] && !isFeeExempt[to]) {
            if ((from == uniswapV2Pair || to == uniswapV2Pair)) 
            {
                ...
                if (
                    uniswapV2Pair == from && !isTxLimitExempt[to] && from != address(this)
                ) {
                ...
                if (
                    uniswapV2Pair == to && !isTxLimitExempt[from] &&  to != address(this) && _swapEnabled && !progress_swap
                ) {
                ...
                if (
                    sniperWallets[from] == true && uniswapV2Pair != to
                ) {
