...
// by Ryo Sato @Ryo1Sato on Twitter
...
contract SmartMonkeys is ERC721A, ReentrancyGuard, Ownable, DefaultOperatorFilterer, OnChainMetadata {

    uint256 public maxSupply = 1024;

    mapping(uint256 => string) public promotionURLs;
    ...
    mapping(uint256 => uint256) public allTokensWhoLinkedEver;
    mapping(uint256 => string) public allLinksEver;
    mapping(uint256 => uint256) public tokenMinBlock;
    uint256 public mintPrice = 0.01 ether;
    uint256 public linksEverCount;
 
 
    function setPromoURI(string memory promoURL, uint256 tokenId, uint256 otherId) external payable {
        require(
                    ( (ownerOf(tokenId) == msg.sender) && (msg.value >= (mintPrice/10) ) ) || 
                    ( (tokenLevel[otherId]>tokenLevel[tokenId]) && (msg.value >= ((mintPrice/10) * tokenLevel[tokenId] )) && (ownerOf(otherId) == msg.sender) ) 
            );
        linksEverCount = linksEverCount + 1;
        allTokensWhoLinkedEver[linksEverCount] = tokenId;
        if (ownerOf(tokenId) == msg.sender) {
            require(block.number > tokenMinBlock[tokenId]);
            tokenLevel[tokenId] = tokenLevel[tokenId] + 1;
            promotionURLs[tokenId] = promoURL; 
            allLinksEver[linksEverCount] = promoURL;
        }
        else {
            tokenLevel[tokenId] = tokenLevel[tokenId]/2; 
            allLinksEver[linksEverCount] = string.concat("overridden by ",Strings.toString(otherId)); 
            promotionURLs[tokenId] = string.concat("overridden by ",Strings.toString(otherId));  
            tokenMinBlock[tokenId] = block.number + (tokenLevel[otherId] * 5000);      
        }
    }
    ...
    function setColor(uint256 tokenId,uint256 hue,uint256 sat,uint256 lum) external payable {
        require(ownerOf(tokenId) == msg.sender && msg.value >= (mintPrice/10) );
        // require(hue < 361);
        // require(sat < 101);
        // require(lum < 101);
        tokenLevel[tokenId] = tokenLevel[tokenId] + 1;
        tokenHue[tokenId] = hue;
        tokenSat[tokenId] = sat;
        tokenLum[tokenId] = lum;
    }
    ...
    constructor() ERC721A("Smart NFT Monkeys", "SMART_MONKEY") {
    ...
    function _buildTokenURI(uint256 id) internal view returns (string memory) {
        ...
        bytes memory image = abi.encodePacked(
            ...
            Base64.encode(
                bytes(
                    abi.encodePacked(
                        '<?xml version="1.0" encoding="UTF-8"?><svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" version="1.1" viewBox="0 0 400 400" preserveAspectRatio="xMidYMid meet"><style type="text/css"><![CDATA[text { font-family: monospace; font-size: 9px;} .st0{fill-rule:evenodd;clip-rule:evenodd;} .h1 {font-size: 40px; font-weight: 600;}]]></style><text class="h1" x="50" y="250" >Smart NFT</text><text class="h1" x="50" y="275" >Monkey</text><text x="25" y="300"> Owner Promoted Link</text><text x="25" y="325"><a href="',
                        promotionURLs[id],
                        '" target="_blank">',
                        promotionURLs[id],
                        '</a></text><g fill="hsl(',
            ...
            string(
                abi.encodePacked(
                    ...
                    Base64.encode(
                        bytes(
                            abi.encodePacked(
                                '{"name":"Smart NFT", "image":"',
                                image,
                                '", "description": "A Smart NFT Monkey which can promote a link, change colors, and gain levels by owner on chain actions","attributes": [{"trait_type": "Metadata","value": "OnChain"},{"trait_type": "Utility","value": "ColorSwap/LinkPromo"},{"trait_type": "Token Level","value":',
