...
contract StakingPool is Ownable, ReentrancyGuard {
    ...
    receive() external payable {
        assert(address(msg.sender) == address(WETH)); // only accept ETH via fallback from the WETH contract
    }    
    ...
    function _withdrawStakingToken(uint256 _pid, uint256 _amount) internal {
        ...
        if(address(stakingToken[_pid]) == address(WETH)){
            ...
            safeTransferETH(msg.sender, _amount-fee);
            safeTransferETH(treasury, fee);
        }else {
            stakingToken[_pid].safeTransfer(msg.sender, _amount-fee);
            stakingToken[_pid].safeTransfer(treasury, fee);
        }
    }
