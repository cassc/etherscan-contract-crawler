...
**/
contract WrappedPepe is ERC20, IERC20Receiver {
    using Address for address;

    IERC20 private pepeCoinContract = IERC20(0x6982508145454Ce325dDbE47a25d4ec3d2311933);

    constructor() ERC20("Wrapped Pepe", "wPEPE") {}

    function PEPE_COIN_CONTRACT_ADDRESS() public view returns (address) {
        return address(pepeCoinContract);
    }

    function wrapPepe(uint256 balanceToDeposit) public {
        pepeCoinContract.transferFrom(msg.sender, address(this), balanceToDeposit);
        _mint(msg.sender, balanceToDeposit);
    }

    function unwrapPepe(uint256 balanceToWithdraw) public {
        _burn(msg.sender, balanceToWithdraw);
        pepeCoinContract.transfer(msg.sender, balanceToWithdraw);
    }
    ...
    function onERC20Received(
        ...
        if (msg.sender == address(this)) {
            ...
            pepeCoinContract.transfer(_from, _amount);
