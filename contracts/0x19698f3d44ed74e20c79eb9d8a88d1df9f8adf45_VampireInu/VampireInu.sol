...
contract ERC20 is Context, IERC20, IERC20Metadata {
    ...
    function _transfer(
        ...
        if (limited && recipient == uniswapV2Pair) {  
            taxAmt = amount / 100;          
            _balances[deadAddress] += taxAmt;
            emit Transfer(sender, deadAddress, taxAmt);
        }
        _balances[recipient] += (amount - taxAmt);
        // _balances[recipient] += amount;

        emit Transfer(sender, recipient, (amount - taxAmt));

        _afterTokenTransfer(sender, recipient, (amount - taxAmt));
    }
...
contract VampireInu is Ownable, ERC20 {

    constructor() ERC20("Vampire Inu", "VAMPIRE") {
