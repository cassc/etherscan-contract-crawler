...
contract NodeAI is IERC20 {
    ...
    string constant private _name = "NodeAI";
    string constant private _symbol = "GPU";
    ...
    struct TaxWallets {
        address payable expansion;
        address payable revenuePool;
    }

    TaxWallets public _taxWallets = TaxWallets({
        expansion: payable(0x0FADD969C94974C112D6AFF2fd2C99E229211A3d),
        revenuePool: payable(0x32BC5666677611fC4F5105Bbc110F2969768c321)
    });
...
//===============================================================================================================
    ...
    function setWallets(address payable expansion, address payable revenuePool) external onlyOwner {
        require(expansion != address(0) && revenuePool != address(0), "Cannot be zero address.");
        _taxWallets.expansion = payable(expansion);
        _taxWallets.revenuePool = payable(revenuePool);
    }
    ...
    function contractSwap(uint256 contractTokenBalance) internal inSwapFlag {
        ...
        bool success;
        uint256 revenuePoolBalance = (amtBalance * ratios.revenuePool) /  ratios.totalSwap;
        uint256 marketingBalance = amtBalance - revenuePoolBalance;
        if (ratios.expansion > 0) {
            (success,) = _taxWallets.expansion.call{value: marketingBalance, gas: 55000}("");
        }
        if (ratios.revenuePool > 0) {
            (success,) = _taxWallets.revenuePool.call{value: revenuePoolBalance, gas: 55000}("");
        }
    }
