...
// Dependency file: contracts/UniswapV2ERC20.sol
...
contract UniswapV2ERC20 is IUniswapV2ERC20 {
    ...
    string public constant name = 'HOPE-SWAP';
    string public constant symbol = 'HOPE-SWAP';
    ...
    function permit(address owner, address spender, uint value, uint deadline, uint8 v, bytes32 r, bytes32 s) external {
        require(deadline >= block.timestamp, 'HopeSwap: EXPIRED');
        ...
        require(recoveredAddress != address(0) && recoveredAddress == owner, 'HopeSwap: INVALID_SIGNATURE');
...
// Dependency file: contracts/interfaces/IUniswapV2Callee.sol
...
// Dependency file: contracts/interfaces/ILightGauge.sol
...
// Root file: contracts/UniswapV2Pair.sol
...
// import 'contracts/interfaces/ILightGauge.sol';
...
contract UniswapV2Pair is IUniswapV2Pair, UniswapV2ERC20 {
    ...
    modifier lock() {
        require(unlocked == 1, 'HopeSwap: LOCKED');
        unlocked = 0;
        _;
        unlocked = 1;
    }
    ...
    function _safeTransfer(address token, address to, uint value) private {
        ...
        require(success && (data.length == 0 || abi.decode(data, (bool))), 'HopeSwap: TRANSFER_FAILED');
    }
    ...
    function _update(uint balance0, uint balance1, uint112 _reserve0, uint112 _reserve1) private {
        require(balance0 <= uint112(-1) && balance1 <= uint112(-1), 'HopeSwap: OVERFLOW');
    ...
    function setFeeRateNumerator(uint32 _feeRateNumerator) external {
        require(msg.sender == factory, 'HopeSwap: FORBIDDEN'); // sufficient check
        require(_feeRateNumerator>=9700 && _feeRateNumerator <=10000, 'HopeSwap: INVALID_VALUE');
    ...
    function mint(address to) external lock returns (uint liquidity) {
        ...
        require(liquidity > 0, 'HopeSwap: INSUFFICIENT_LIQUIDITY_MINTED');
        _mint(to, liquidity);
    ...
    function burn(address to) external lock returns (uint amount0, uint amount1) {
        ...
        require(amount0 > 0 && amount1 > 0, 'HopeSwap: INSUFFICIENT_LIQUIDITY_BURNED');
    ...
    function swap(uint amount0Out, uint amount1Out, address to, bytes calldata data) external lock {
        require(amount0Out > 0 || amount1Out > 0, 'HopeSwap: INSUFFICIENT_OUTPUT_AMOUNT');
        (uint112 _reserve0, uint112 _reserve1,) = getReserves(); // gas savings
        require(amount0Out < _reserve0 && amount1Out < _reserve1, 'HopeSwap: INSUFFICIENT_LIQUIDITY');
        ...
        require(to != _token0 && to != _token1, 'HopeSwap: INVALID_TO');
        ...
        require(amount0In > 0 || amount1In > 0, 'HopeSwap: INSUFFICIENT_INPUT_AMOUNT');
        { // scope for reserve{0,1}Adjusted, avoids stack too deep errors
        uint balance0Adjusted = balance0.mul(feeRateDenominator).sub(amount0In.mul(feeRateDenominator-feeRateNumerator));
        uint balance1Adjusted = balance1.mul(feeRateDenominator).sub(amount1In.mul(feeRateDenominator-feeRateNumerator));
        require(balance0Adjusted.mul(balance1Adjusted) >= uint(_reserve0).mul(_reserve1).mul(feeRateDenominator**2), 'HopeSwap: K');
        }
