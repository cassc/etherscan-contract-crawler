...
contract ThePortalClub is ERC721A, Ownable, ReentrancyGuard {
    ...
    constructor() ERC721A("ThePortalClub", "The Portal Club",69,6969)
    {
        baseURI = "ipfs://QmPfFTQAgH68VSHNHVDdprT6CyVc9bx8iinyWj6GTjHguc/";
        MintStartEnabled = true;
    }

    function mint(uint256 qty) external payable
    {
        require(MintStartEnabled , "Portal Info:  Minting Public Pause");
        require(qty <= maxMintPerTx, "Portal Info:  Limit Per Transaction");
        require(totalSupply() + qty <= maxSupply,"Portal Info:  Soldout");
        _safemint(qty);
    }
    ...
    function _safemint(uint256 qty) internal
    {
        if(WalletMint[msg.sender] < freeMint) 
        {
           ...
           require(msg.value >= (qty - freeMint) * MintPrice,"Portal Info:  Fund not enough");
            WalletMint[msg.sender] += qty;
           _safeMint(msg.sender, qty);
        }
        else
        {
           require(msg.value >= qty * MintPrice,"Portal Info:  Fund not enough");
            WalletMint[msg.sender] += qty;
           _safeMint(msg.sender, qty);
        }
    }
