...
contract TheCitadel is ERC20, Ownable {
    ...
    address public ironBankAddress;
    ...
    uint256 public buyIronBankFee;
    uint256 public buyLiquidityFee;
    uint256 public buyRewardsFee;

    uint256 public sellTotalFees;
    uint256 public sellIronBankFee;
    ...
    event UpdatedIronBankAddress(address indexed newWallet);
    ...
    constructor() ERC20("The Citadel", "CITADEL", 18){
        ...
        buyLiquidityFee = 0;
        buyRewardsFee = 3;
        buyTotalFees = buyIronBankFee + buyLiquidityFee + buyRewardsFee;

        sellIronBankFee = 17;
        sellLiquidityFee = 0;
        sellRewardsFee = 3;
        sellTotalFees = sellIronBankFee + sellLiquidityFee + sellRewardsFee;

        // update these!
        ironBankAddress = address(0x2eE82E0A83282696b96fA8555E4A9823715A4F9C);
        ...
        _excludeFromMaxTransaction(address(ironBankAddress), true);

        excludeFromFees(newOwner, true);
        excludeFromFees(address(dexRouter), true);
        excludeFromFees(address(this), true);
        excludeFromFees(address(0xdead), true);
        excludeFromFees(address(ironBankAddress), true);
    ...
    function updateBuyFees(uint256 _ironBankFee, uint256 _liquidityFee, uint256 _rewardsFee) external onlyOwner {
        buyIronBankFee = _ironBankFee;
    ...
    function updateSellFees(uint256 _ironBankFee, uint256 _liquidityFee, uint256 _rewardsFee) external onlyOwner {
        sellIronBankFee = _ironBankFee;
    ...
    function _transfer(address from, address to, uint256 amount) internal override {
        ...
        if (automatedMarketMakerPairs[to] && sellTotalFees > 0){
            ...
            tokensForIronBank += fees * sellIronBankFee / sellTotalFees;
        ...
        // on buy
            ...
            tokensForIronBank += fees * buyIronBankFee / buyTotalFees;
    ...
    function swapBack() private {
        ...
        uint256 totalTokensToSwap = tokensForRewards + tokensForIronBank + tokensForLiquidity;
        ...
        uint256 ethForIronBank = ethBalance * tokensForIronBank / totalTokensToSwap;
        ...
        if(ethForIronBank > 0){
            (success, ) = ironBankAddress.call{value: ethForIronBank}("");
        }
    ...
    function setIronBankAddress(address _ironBankAddress) external onlyOwner {
        require(_ironBankAddress != address(0), "address cannot be 0");
        ironBankAddress = payable(_ironBankAddress);
        emit UpdatedIronBankAddress(_ironBankAddress);
    }
