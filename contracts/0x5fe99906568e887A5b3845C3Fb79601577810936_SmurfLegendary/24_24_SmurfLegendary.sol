...
contract SmurfLegendary is Initializable, ERC721Upgradeable, ERC721EnumerableUpgradeable, PausableUpgradeable, AccessControlUpgradeable, ERC721BurnableUpgradeable, ERC2981Upgradeable, EIP712Upgradeable, SmurfConstants, ISmurfMint {
    ...
    event CrystalMinted(address indexed to, uint indexed token, uint phase);
    ...
    modifier onlyWhenMintingOpen() {
        require(__isCrystalMintingOpen, "Smurf: Not allowed to mint");
        _;
    }

    // Minting logic
	function mintCrystalSmurf(uint _crystalId, bytes memory _signature) public payable onlyWhenMintingOpen whenNotPaused {
		checkApproval(msg.sender,_crystalId, _signature);
        uint price = getPrice(_crystalId);

		require(msg.value == price, "Smurf: Incorrect price");
    ...
    function mintSmurf(uint _qty, address _to, uint _phaseId) internal whenNotPaused {
        ...
        require(__givenSmurfsForPhase[_phaseId] + _qty <= __phaseQuantities[_phaseId]);
    ...
    function _safeSmurfMint(address _to, uint _tokenId, uint _phaseId) internal  {
        ...
        emit CrystalMinted(_to, _tokenId, _phaseId);
    }
