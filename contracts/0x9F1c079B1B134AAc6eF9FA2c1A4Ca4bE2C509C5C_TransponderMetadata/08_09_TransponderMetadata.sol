...
contract TransponderMetadata {
    ...
    function uploadModels (uint48 count, bytes memory data) public onlyAuthorized {
    ...
    function rawToTransmission (bool throwErrorOnInvalidMessage, string memory message, string memory handle) internal pure returns (Transmission memory) {
        uint messageLength = measureString(message);
        if (messageLength == 0) {
            if (throwErrorOnInvalidMessage) revert("invalid message");
            message = "Err";
            messageLength = 3;
        }
        ...
        if (handleLength > 0) {
            ...
            handle = string(new bytes(0));
        }
        return Transmission(length, message, handle);
    }

    function signalToTransmission (bytes[37] memory bs, string memory handle, uint handleLength) internal pure returns (Transmission memory) {
        uint messageLength = 0;
        string memory message;
        for (; messageLength < 37; messageLength++) {
            if (uint8(bytes1(bs[messageLength])) == 0) {
    ...
    Transmission[] Propaganda;

    function uploadPropaganda (string[] calldata messages, string[] calldata handles) public onlyAuthorized {
        require (messages.length == handles.length, "mismatched length");
        for (uint i = 0; i < messages.length; i++) {
            Propaganda.push(rawToTransmission(true, messages[i], handles[i]));
        }
    }

    function updatePropaganda (uint[] calldata ids, string[] calldata messages, string[] calldata handles) public onlyAuthorized {
        require (messages.length == handles.length && handles.length == ids.length, "mismatched length");
        for (uint i = 0; i < ids.length; i++) {
            Propaganda[ids[i]] = rawToTransmission(true, messages[i], handles[i]);
        }
    }

    uint constant PRIME = 72729269248899238429;

    uint public propagandaDuration = 75;

    function randomPropaganda (uint modelId, uint seed) internal view returns (Transmission memory) {
        uint b = block.number - (block.number % propagandaDuration);
        uint val = uint32(uint256(keccak256(abi.encodePacked(modelId, seed, blockhash(b - 2)))));
        return Propaganda[(val * PRIME) % Propaganda.length];
    }
    ...
    function transponderImage (bool active, bytes memory duration, bytes memory boxKeyTimes, uint totalSegments, uint modelId, bytes memory chroma) internal view returns (bytes memory) {
        bytes memory broadcasting;
        if (active) {
            broadcasting = disappearGroup(duration, boxKeyTimes, totalSegments,
    ...
    bytes12[6] shift = [bytes12(0x1c65252798363acf4c53ff68),  // Emerald
                        bytes12(0x277e9535badc5acce6b7f2ff),  // Sapphire
                        bytes12(0xcf8404f7a708fdd912fff527),  // Amber
                        bytes12(0x651b1ca31f1ced2929ff5353),  // Ruby
                        bytes12(0x2a1c6a4918987d3acf9c49ff),  // Amethyst
                        bytes12(0xff3fd4ff5bf3ff9affffcaff)]; // Ponderware
    ...
    function assembleSVG (bool active, uint modelId, uint chromaId, Transmission[] memory ts) internal view returns (string memory) {
        ...
        bytes memory img = transponderImage(active, duration, boxKeyTimes, ts.length, modelId, chroma);
    ...
    function assembleSVGURI  (bool active, uint modelId, uint chromaId, Transmission[] memory ts) internal view returns (string memory) {
        return string(abi.encodePacked("data:image/svg+xml;base64,", Base64.encode(bytes(assembleSVG(active, modelId, chromaId, ts)))));
    }
    ...
    function encodeNumericAttribute (string memory key, uint256 value) internal pure returns (bytes memory) {
    ...
    function propagandaMetadata (uint modelId) public onlyAuthorized view returns (string memory) {
        Transmission[] memory ts = new Transmission[](3);
        ts[0] = randomPropaganda(modelId, 1);
        ts[1] = randomPropaganda(modelId, 2);
        ts[2] = randomPropaganda(modelId, 3);

        bytes memory description = abi.encodePacked("Stay tuned as required per code.",
                                                    Strings.toString(block.number % 7923),
                                                    " | Your compliance is compulsory and appreciated.");

        bytes memory attributes = abi.encodePacked(encodeStringAttribute("source", "[REDACTED]"));

        return toJSON(ModelDetails[modelId * 2],
                         description,
                         attributes,
                         assembleSVGURI(false, modelId, 0, ts));

    }

    function scramble (string memory raw, uint offset) internal view returns (bytes memory res) {
        res = bytes(raw);
        uint seed = uint256(blockhash(block.number - 1)) >> offset;
        for (uint i = 0; i < res.length - 2; i++) {
            if (seed & 7 == 0) {
                res[i] = hex'e2';
                res[i + 1] = hex'96';
                res[i + 2] = hex'88';
    ...
    function broadcastMetadata (bool signalling, uint peer, uint modelId, uint startBlock, string memory message, string memory handle) public onlyAuthorized view returns (string memory) {
        Transmission[] memory ts = new Transmission[](1);
        ts[0] = rawToTransmission(false, message, handle);

        bytes memory description = abi.encodePacked(scramble("Stay tuned as required per code.", 0),
                                                    unicode"█",
                                                    scramble(" | Your compliance is compulsory and appreciated.", 96));

        bytes memory attributes =
            abi.encodePacked(signalling
                             ? abi.encodePacked(encodeStringAttribute("peer", Strings.toString(peer)),
                                                ",",
                                                encodeStringAttribute("request", "GNU")) // Terry Pratchet
                             : encodeStringAttribute("err","override"),
                             ",",
                             encodeStringAttribute("source", ts[0].handle), ",",
                             encodeStringAttribute("plaintext", ts[0].message), ",",
                             encodeStringAttribute("timestamp", string(abi.encodePacked("cycle.", Strings.toString(startBlock / 7200)))));
        string memory name = signalling ?
            string(abi.encodePacked("peer ", Strings.toString(peer), " - ", ModelDetails[modelId * 2]))
            : ModelDetails[modelId * 2];
        return toJSON(name,
                      description,
                      attributes,
                      assembleSVGURI(true, modelId, 5, ts));
    }
    ...
    function signalMetadata(uint peer, Signal memory local, Signal memory peer1, Signal memory peer2) public onlyAuthorized view returns (string memory) {
        ...
        string memory handle;
        {
            ...
            ts[0] = signalToTransmission(local.message, handle, handleLength);
        }
        {
            (string memory peerHandle, uint handleLength) = getHandle(peer1.sender);
            ts[1] = signalToTransmission(peer1.message, peerHandle, handleLength);
        }
        {
            (string memory peerHandle, uint handleLength) = getHandle(peer2.sender);
            ts[2] = signalToTransmission(peer2.message, peerHandle, handleLength);
        }
