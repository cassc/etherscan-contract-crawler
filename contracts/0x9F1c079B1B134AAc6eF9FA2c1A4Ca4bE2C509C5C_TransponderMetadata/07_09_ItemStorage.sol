...
library ItemStorage {
    ...
    /*
    function old_get (Store storage store, uint id) internal view returns (bytes memory) {
        require(id < total(store), "Out of bounds");
        uint counter = 0;
        uint index = 0;
        address pointer;
        for (uint b = 0; b < store.blocks.length; b++) {
            counter += store.blocks[b].count;
            if (id < counter) {
                index = store.blocks[b].count - (counter - id);
                pointer = store.blocks[b].pointer;
                break;
            }
        }
        uint pos = index * 2;
