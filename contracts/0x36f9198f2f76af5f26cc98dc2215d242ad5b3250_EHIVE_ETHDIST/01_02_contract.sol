...
contract EHIVE is ERC20, Ownable {
    ...
    uint256 private _liquidityFeePercentage = 16; // circa 1% of the total fee
    uint256 private _tokensForContract;
    ...
    */
    function updateFees(uint256 swapFee) external teamOROwner {
        require(swapFee <= 6);
    ...
    */
    ...
    function _transfer(
        ...
        if (takeFee) {
            ...
            _tokensForLiquidity += fees.mul(_liquidityFeePercentage).div(100);
            _tokensForContract = fees.sub(_tokensForLiquidity);
    ...
    function swapBack() internal {
        ...
        uint256 totalTokensToSwap = _tokensForLiquidity + _tokensForContract;
        ...
        uint256 ethForContract = ethBalance.mul(_tokensForContract).div(totalTokensToSwap);
        uint256 ethForLiquidity = ethBalance - ethForContract;

        _tokensForContract = 0;
        _tokensForLiquidity = 0;

        payable(_swapFeeReceiver).transfer(ethForContract);
