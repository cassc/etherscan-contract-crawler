...
contract ZipFi is Ownable, ERC20 {

	IRouter public uniswapV2Router;
	address public immutable uniswapV2Pair;

	string private constant _name =  "ZipFi";
	string private constant _symbol = "ZIP";
	uint8 private constant _decimals = 18;

	ZipTokenDividendTracker public dividendTracker;
	...
	uint256 public maxWalletAmount = 12000000000000 * (10**18);

	uint256 public maxTxAmount = 6000000000000 * (10**18);
	...
	// Initial Base taxes
	CustomTaxPeriod private _base = CustomTaxPeriod("base",0,0,1,0,4,10,0,2,0,2,1,1);
	...
	modifier hasBridgeSupplyPermission {
		require(msg.sender == bridge, "only bridge contract can mint");
		_;
	}

	constructor() ERC20(_name, _symbol) {
		dividendTracker = new ZipTokenDividendTracker();
		...
		marketingWallet = address(0xCF94Efc6d69E8AC55242810dc48c7eE0264AA6a3);
		buyBackWallet = address(0x008B3B7EC7aF8c7A61F322a2e95e21609209613D);
	...
	function _setAutomatedMarketMakerPair(address pair, bool value) private {
		require(automatedMarketMakerPairs[pair] != value, "ZipToken: Automated market maker pair is already set to that value");
	...
	function excludeFromFees(address account, bool excluded) external onlyOwner {
		require(_isExcludedFromFee[account] != excluded, "ZipToken: Account is already the value of 'excluded'");
	...
	function excludeFromMaxTransactionLimit(address account, bool excluded) external onlyOwner {
		require(_isExcludedFromMaxTransactionLimit[account] != excluded, "ZipToken: Account is already the value of 'excluded'");
		_isExcludedFromMaxTransactionLimit[account] = excluded;
		emit ExcludeFromMaxTransferChange(account, excluded);
	}
	function excludeFromMaxWalletLimit(address account, bool excluded) external onlyOwner {
		require(_isExcludedFromMaxWalletLimit[account] != excluded, "ZipToken: Account is already the value of 'excluded'");
	...
	function setWallets(address newLiquidityWallet, address newMarketingWallet, address newBuyBackWallet) external onlyOwner {
		if(liquidityWallet != newLiquidityWallet) {
			require(newLiquidityWallet != address(0), "ZipToken: The liquidityWallet cannot be 0");
			emit WalletChange("liquidityWallet", newLiquidityWallet, liquidityWallet);
			liquidityWallet = newLiquidityWallet;
		}
		if(marketingWallet != newMarketingWallet) {
			require(newMarketingWallet != address(0), "ZipToken: The marketingWallet cannot be 0");
			emit WalletChange("marketingWallet", newMarketingWallet, marketingWallet);
			marketingWallet = newMarketingWallet;
		}
		if(buyBackWallet != newBuyBackWallet) {
			require(newBuyBackWallet != address(0), "ZipToken: The buyBackWallet cannot be 0");
	...
	function setBaseFeesOnBuy(uint8 _liquidityFeeOnBuy, uint8 _marketingFeeOnBuy, uint8 _buyBackFeeOnBuy, uint8 _burnFeeOnBuy, uint8 _holdersFeeOnBuy) external onlyOwner {
		   require(8 > _liquidityFeeOnBuy + _marketingFeeOnBuy + _buyBackFeeOnBuy + _burnFeeOnBuy + _holdersFeeOnBuy, "ZipToken: buy fee must be fair!!!");
	...
	function setBaseFeesOnSell(uint8 _liquidityFeeOnSell, uint8 _marketingFeeOnSell, uint8 _buyBackFeeOnSell, uint8 _burnFeeOnSell, uint8 _holdersFeeOnSell) external onlyOwner {
		   require( 10 > _liquidityFeeOnSell + _marketingFeeOnSell + _buyBackFeeOnSell + _burnFeeOnSell + _holdersFeeOnSell, "ZipToken: sell fee must be fair!!!");
	...
	function setUniswapRouter(address newAddress) external onlyOwner {
		require(newAddress != address(uniswapV2Router), "ZipToken: The router already has that address");
		emit UniswapV2RouterChange(newAddress, address(uniswapV2Router));
		uniswapV2Router = IRouter(newAddress);
		dividendTracker.setUniswapRouter(newAddress);
	}
	function setMaxTransactionAmount(uint256 newValue) external onlyOwner {
		require(newValue != maxTxAmount, "ZipToken: Cannot update maxTxAmount to same value");
		emit MaxTransactionAmountChange(newValue, maxTxAmount);
		maxTxAmount = newValue;
	}
	function setMaxWalletAmount(uint256 newValue) external onlyOwner {
		require(newValue != maxWalletAmount, "ZipToken: Cannot update maxWalletAmount to same value");
		emit MaxWalletAmountChange(newValue, maxWalletAmount);
		maxWalletAmount = newValue;
	}
	function setMinimumTokensBeforeSwap(uint256 newValue) external onlyOwner {
		require(newValue != minimumTokensBeforeSwap, "ZipToken: Cannot update minimumTokensBeforeSwap to same value");
	...
	function claimETHOverflow(uint256 amount) external onlyOwner {
	    require(amount < address(this).balance, "ZipToken: Cannot send more than contract balance");
        (bool success,) = address(owner()).call{value : amount}("");
        if (success){
            emit ClaimETHOverflow(amount);
        }
	}
	function bridgeSupply(address account, uint256 value) external hasBridgeSupplyPermission returns(bool) {
		_mint(account, value);
		return true;
	}
	...
	function _transfer(
		...
		uint256 amount
			...
			if(!_isAllowedToTradeWhenDisabled[from] && !_isAllowedToTradeWhenDisabled[to]) {
				require(isTradingEnabled, "ZipToken: Trading is currently disabled.");
				if (!_isExcludedFromMaxTransactionLimit[to] && !_isExcludedFromMaxTransactionLimit[from]) {
					require(amount <= maxTxAmount, "ZipToken: Buy amount exceeds the maxTxBuyAmount.");
				}
				if (!_isExcludedFromMaxWalletLimit[to]) {
					require((balanceOf(to) + amount) <= maxWalletAmount, "ZipToken: Expected wallet amount exceeds the maxWalletAmount.");
				}
			}
