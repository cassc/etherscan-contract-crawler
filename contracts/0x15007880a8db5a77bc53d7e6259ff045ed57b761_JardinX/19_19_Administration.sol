 ...
 * the owner.
 */
abstract contract Administration is Initializable, ContextUpgradeable {
    address private _owner;
    mapping(address => MapAdmin) internal admin;
    ...
    function deleteAdmin(address _addr) public onlySuperAdmin {
        require(admin[_addr].exist == 1, "Admin not found.");
        ...
        for (uint256 i = 0; i < adminArr.length; i++) {
            if (adminArr[i] == _addr) {
                delete adminArr[i];
                adminArr[i] = adminArr[adminArr.length - 1];
                adminArr.pop();
                break;
            }
        }
