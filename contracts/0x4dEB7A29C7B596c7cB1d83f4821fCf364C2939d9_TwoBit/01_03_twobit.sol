...
contract ERC721A is IERC721A, ERC2981 {
    // Bypass for a `--via-ir` bug (https://github.com/chiru-labs/ERC2981/pull/364).
...
contract TwoBit is ERC721A, OperatorFilterer {
    ...
    mapping(uint256 => Detail) private _tokenIdToDetails;
    ...
    string[3] private _species = ["Jimm", "Ricks", "Rongs"];
    ...
    constructor() ERC721A("2 Chain Bit ", "2bit") {
        owner = msg.sender;
        _seed = uint256(
        keccak256(
            abi.encodePacked(
    ...
    function mint(uint256 count) payable public {
        ...
        require(_total + count < maxSupply + 1, "Sold out!");
        ...
        for (uint256 i = _total; i < _total + count; i++) {
            _seed = uint256(
                keccak256(
                    abi.encodePacked(
                        _seed >> 1,
            ...
            _tokenIdToDetails[i] = _createDetailFromRandom(_seed, i);
        }
    }
        ...
        returns (Detail memory detail)
    {
        ...
        detail = _tokenIdToDetails[tokenId];
    }
        ...
        returns (string memory)
    {
        ...
        return _tokenUriForDetail(_tokenIdToDetails[tokenId], tokenId);
    }
