 ...
 *
 * NOTE: This is a duplicated version of the BondController with some minor changes
 *       for AMPL. (or other non-dilutive rebasing tokens)
 *
 * Tranche amounts are scaled by the bond's CDR. Previously they were scaled by
 * a factor of (totalDebt/collateralBalance). However, This can be manipulated by
 * transferring more collateral tokens into the bond resulting in grief from tranche amount mismatch.
 *
 * For tokens like AMPL we can compute this factor using collateral's `totalSupply`.
 *
 * Invariants:
 *  - `totalDebt` should always equal the sum of all tranche tokens' `totalSupply()`
 */
contract AmpleBondController is IBondController, OwnableUpgradeable {
    ...
    // we enforce the debt to be at least MINIMUM_DEBT, from the first deposit until maturity
    uint256 private constant MINIMUM_DEBT = 10e9;
    ...
    uint256 public initCollateralSupply;
    ...
    function deposit(uint256 amount) external override {
        ...
        uint256 currentCollateralSupply = IERC20(collateralToken).totalSupply();
        // recording the AMPL supply on the first deposit.
        if (_totalDebt == 0) {
            initCollateralSupply = currentCollateralSupply;
        }
        ...
        for (uint256 i = 0; i < _tranches.length; i++) {
            ...
            // In the case of AMPL we infer it based on the ratio between the recorded collateral supply
            // during the first deposit, and the supply now.
            trancheValue = (trancheValue * initCollateralSupply) / currentCollateralSupply;
    ...
    function _enforceTotalDebt() internal {
        require(totalDebt >= MINIMUM_DEBT, "BondController: Expected minimum valid debt");
    }
}