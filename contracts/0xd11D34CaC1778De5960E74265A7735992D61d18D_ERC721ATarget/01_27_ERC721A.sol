    ...
    IRoyaltySplitter
{
    ...
    /// @notice configure drop
    /// @param amount amount by which to increase supply
    function increaseSupply(uint256 amount) public onlyRole(MANAGER_ROLE) {
        _maxSupply += amount;
    }

    /// @notice configure drop
    /// @param amount amount by which to reduce supply
    function decreaseSupply(uint256 amount) public onlyRole(MANAGER_ROLE) {
        _maxSupply -= amount;
    }
    ...
    function setUpRecipients(uint16[] memory _royaltySplits, address payable[] memory _royaltyRecipients) public onlyRole(MANAGER_ROLE) {
        if (_royaltySplits.length != _royaltyRecipients.length) {
            revert RoyaltiesMismatch(_royaltySplits.length, _royaltyRecipients.length);
        }

        uint32 mBpTotal = 0;
        for (uint i = 0; i < _royaltyRecipients.length; i++) {
    ...
    function setRecipients(Recipient[] calldata _recipients) public onlyRole(MANAGER_ROLE) {
        ...
        address payable[] memory _royaltyRecipients = new address payable[](_recipients.length);
    ...
    function getRecipients() external view returns (Recipient[] memory) {
        Recipient[] memory mRecipients = new Recipient[](royaltyRecipients.length);

        for (uint i = 0; i < royaltyRecipients.length; i++) {
            Recipient memory recipient = Recipient(royaltyRecipients[i], royaltySplits[i]);
