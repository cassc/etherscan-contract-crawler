...
// OPTION 1: Standard, hardcoded. Note that min/max tax are hardcoded and immutable.
	...
	constructor() {
        ...
        _maxtax = 500; // Maximum tax hardcoded to 15.00% = 1500 points.
...
contract PotHead is ERC20, Pausable, Ownable, Taxable {
    uint256 public maxHoldingCap = 1;

    constructor() ERC20("PotHead", "POT") {
    ...
    function updateTax(uint newtax) public onlyOwner {
        _updatetax(newtax);
    }
        ...
        override
    {
        if(to != owner() && from != owner())
            require(balanceOf(to)+amount <= _getMaxHolding(), "Maximal holding cap reached.");
    ...
    function _getMaxHolding() public view virtual returns (uint256) {
        return  (super.totalSupply()*maxHoldingCap)/100;
    }

    function setMaxHoldingCap(uint256 maxCap) public onlyOwner {
        maxHoldingCap = maxCap;
    }
}