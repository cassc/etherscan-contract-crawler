...
contract BrokePets is ERC721A, Ownable, ReentrancyGuard {
    using Strings for uint256;

    uint256 public maxTokensPerClaim = 50;
    uint256 public maxTokensPerMint = 50;
    ...
    uint256 public marketingReserved = 500;

    address public claimingToken = 0xf8Ef0631aA8150aEE8cFb4Db3Fa27e81D78df620;
    uint256 public claimingCoef = 1;
    mapping(uint256 => bool) public usedTokens;

    uint256 public tokensToBuyAmount = 20000 - marketingReserved;

    constructor() ERC721A("BrokePets", "BBP") {
        _safeMint(marketingWallet, marketingReserved);
    }
    ...
    function setMaxTokensPerClaim(uint256 val) external onlyOwner {
        maxTokensPerClaim = val;
    }
    ...
    function getAvailableTokensForClaim(address target) external view returns (uint256[] memory) {
        ...
        uint256 balance = IERC721(claimingToken).balanceOf(target);
        for (uint256 i = 0; i < balance; i++) {
            uint256 id = IERC721Enumerable(claimingToken).tokenOfOwnerByIndex(target, i);
    ...
    function claim(uint256[] calldata ids) external nonReentrant returns (uint256[] memory) {
        require(tokensToBuyAmount > 0, "No more tokens left to claim!");
        require(hasClaimingStarted, "Cannot claim before sale start");
        //require(ids.length * claimingCoef <= tokensToBuyAmount, "Cannot claim this many tokens. No tokens left");
        require(ids.length * claimingCoef <= maxTokensPerClaim, "Cannot claim this many tokens per transaction");

        ERC721Enumerable claimer = ERC721Enumerable(claimingToken);
    ...
    function checkToken(uint256 tokenId, address target) external view returns (bool isUsed, bool isOwned) {
        isUsed = usedTokens[tokenId];
        isOwned = IERC721(claimingToken).ownerOf(tokenId) == target;
    }

    function mint(uint256 amount) external payable nonReentrant {
        require(msg.value >= tokenPrice * amount, "Incorrect ETH");
        require(hasSaleStarted, "Cannot mint before start of sale");
        require(amount <= tokensToBuyAmount, "Cannot mint. No enough tokens left");
    ...
    function getContractInfo() external view returns (
        ...
        bool _hasClaimingStarted,
        bool _isBurnEnabled,
        uint256 _maxTokensPerClaim,
        uint256 _maxTokensPerMint,
        uint256 _tokenPrice,
        uint256 _tokensToBuyAmount
    ) {
        _hasSaleStarted = hasSaleStarted;
        _hasClaimingStarted = hasClaimingStarted;
        _isBurnEnabled = isBurnEnabled;
        _maxTokensPerClaim = maxTokensPerClaim;
