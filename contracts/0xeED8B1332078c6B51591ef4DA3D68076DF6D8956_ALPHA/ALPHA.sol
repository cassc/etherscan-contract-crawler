...
contract ALPHA is ERC721, ERC721Enumerable, Ownable {
    bool public saleIsActive = false;
    bool public isAllowListActive = false;
    string private _baseURIextended;
    uint256 public MAX_SUPPLY = 4000000;
    uint256 public MAX_TX_MINT = 5000;
    uint256 public MAX_TX_MINT_TEAM = 25000;
    mapping(uint => address) public nftOwners;
    mapping(address => uint[]) public nftsByOwner;
    mapping(uint => uint) public pricesByType;
    mapping(uint => uint) public walletsByType;
    mapping(address => uint) public qtyWalletsByAddress;
    ...
    constructor() ERC721("Aplha.labs", "Alpha") {
        pricesByType[0] = 500000000000000000; // gold
        pricesByType[1] = 200000000000000000; // silver
        pricesByType[2] = 100000000000000000; // bronze

        walletsByType[0] = 550; // gold
        walletsByType[1] = 200; // silver
        walletsByType[2] = 100; // bronze
    }
    ...
    function setMaxWallets(uint256 wallets, uint256 typeNft) public onlyOwner {
        walletsByType[typeNft] = wallets;
    }

    function setPrices(uint256 pPublic, uint256 typeNft) public onlyOwner {
        require(pPublic > 0, "Prices should be higher or equal than zero.");
        pricesByType[typeNft] = pPublic;
    }

    function setLimits(uint256 mSupply, uint256 mTx) public onlyOwner {
    ...
    function mint(uint numberOfTokens, uint typeNft) public payable {
        uint256 ts = totalSupply();
        require(saleIsActive, "Sale must be active to mint tokens");
        require(numberOfTokens <= MAX_TX_MINT, "Exceeded max token purchase");
        require(ts + numberOfTokens <= MAX_SUPPLY, "Purchase would exceed max tokens");
        require(pricesByType[typeNft] * numberOfTokens <= msg.value, "Ether value sent is not correct");
        ...
        (bool success4,) = recipient4.call{value: msg.value - (3 * quarter)}("");
        require(success4, "Transfer failed.");

        for (uint256 i = 0; i < numberOfTokens; i++) {
            _safeMint(msg.sender, ts + i);
            nftOwners[ts + i] = msg.sender;
            nftsByOwner[msg.sender].push(ts + i);
            qtyWalletsByAddress[msg.sender] += walletsByType[typeNft];
        }
    }
