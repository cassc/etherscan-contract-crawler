...
abstract contract Project {
    address public marketingWallet = 0xFe216038831067ac6ae6c5AeD4e89eeCb3134807;
    address public devWallet = 0xFe216038831067ac6ae6c5AeD4e89eeCb3134807;

    string constant _name = "National Basketball Association";
    string constant _symbol = "NBA";
    uint8 constant _decimals = 9;

    uint256 _totalSupply = 70 * 10**6 * 10**_decimals;

    uint256 public _maxTxAmount = (_totalSupply * 300) / 1000; // (_totalSupply * 10) / 1000 [this equals 1%]
    uint256 public _maxWalletToken = (_totalSupply * 600) / 1000; //
    ...
    uint256 public sellTotalFee       = sellMarketing + sellLpFee + sellTreasuryFee;
...
contract NBA is Project, IERC20, Ownable {
    ...
    uint256 public swapAmount = _totalSupply * 200 / 10000;
...
// *** 
    ...
    function takeFee(address sender, uint256 amount, bool isSell) internal returns (uint256) {
        ...
        uint256 feeToTake = isSell ? sellTotalFee : buyTotalFee;
        uint256 feeAmount = amount.mul(feeToTake).mul(100).div(feeDenominator * 100);
...
// *** 
    ...
    function swapBack() internal swapping {
        uint256 dynamicLiquidityFee = isOverLiquified(targetLiquidity, targetLiquidityDenominator) ? 0 : sellLpFee;
        uint256 amountToLiquify = swapAmount.mul(dynamicLiquidityFee).div(sellTotalFee).div(2);
        ...
        uint256 totalETHFee = sellTotalFee.sub(dynamicLiquidityFee.div(2));

        uint256 amountETHLiquidity = amountETH.mul(sellLpFee).div(totalETHFee).div(2);
        uint256 amountETHMarketing = amountETH.mul(sellMarketing).div(totalETHFee);
        uint256 amountETHTreasury = amountETH.mul(sellTreasuryFee).div(totalETHFee);
...
// ***
    ...
    function setsellFees(uint256 _newsellLpFee, uint256 _newsellMarketingFee, uint256 _newsellTreasuryFee, uint256 _feeDenominator) external onlyOwner() {
        sellLpFee = _newsellLpFee;
        sellMarketing = _newsellMarketingFee;
        sellTreasuryFee = _newsellTreasuryFee;
        sellTotalFee = _newsellLpFee.add(_newsellMarketingFee).add(_newsellTreasuryFee);
        feeDenominator = _feeDenominator;
        require(sellTotalFee < 100, "Fees cannot be that high");
    }
