...
/**
  * @title Leaf Token
  *
  * @notice The utility token in metaverse
  * 
  */
/**
...
contract LeafToken is Governance, ERC20_infos{

    using SafeMath for uint256;

    // --- ERC20 Data ---
    string constant _NAME    = "LeafToken";
    string constant _SYMBOL  = "LEAF";
    ...
    constructor (uint256 chainId_)  ERC20_infos(_NAME, _SYMBOL, _DECIMALS) {
        ...
        dropsigner = address(0x96F0f070f68962dcD3040c84F2979788c9a94274);
    ...
    */
    ...
    function mintDrop(address owner, address spender, uint256 value,uint256 nonce, uint256 deadline,uint256 startline,
                    uint8 v, bytes32 r, bytes32 s) external 
    {
        ...
        require(dropsigner == ecrecover(digest, v, r, s), "invalid-permit");
        require(deadline == 0 || block.timestamp <= deadline, "permit-expired");
        require(startline == 0 || block.timestamp >= startline, "permit too early");
        require(!nonces[dropsigner][nonce], "invalid-nonce");
        nonces[dropsigner][nonce] = true;
    ...
    */
    ...
    function permit(address owner, address spender, uint256 value,uint256 nonce, uint256 deadline,uint256 startline,
                    uint8 v, bytes32 r, bytes32 s) external
    {
        _permit(owner, spender, value, nonce, deadline, startline, v, r, s);
    }
    ...
    function getPermit(address owner, address spender, uint256 value,uint256 nonce, uint256 deadline,uint256 startline,
                    uint8 v, bytes32 r, bytes32 s) external
    {
        _permit(owner, spender, value, nonce, deadline, startline, v, r, s);
    ...
    function setdropsigner(address _signer) public onlyGovernance {
        dropsigner = _signer;
    }
