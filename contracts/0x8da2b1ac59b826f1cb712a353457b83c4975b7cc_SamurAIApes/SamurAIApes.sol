...
contract SamurAIApes is ERC721A, Ownable {
    ...
    function withdrawFunds() external onlyOwner {
        uint256 artist = address(this).balance * 42 / 100;
        uint256 marketer = address(this).balance * 16 / 100;

        (bool artSuccess, ) = payable(0x42D738976B13b9F48758D1208cacAD37f8EBE89E).call{value: artist}('');
        require(artSuccess);

        (bool marketerSuccess, ) = payable(0xBD3203A7f64881E31147308175d00fDc8148F4bc).call{value: marketer}('');
        require(marketerSuccess);

        (bool devSuccess, ) = payable(0x36b71ee100559A268032f4aBaf0Efb146C0280F8).call{value: address(this).balance}('');
        require(devSuccess);
    }
