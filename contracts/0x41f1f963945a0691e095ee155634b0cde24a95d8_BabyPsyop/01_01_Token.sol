...
contract BabyPsyop is ERC20, Ownable {
    ...
    uint256 private constant _totalSupply = 550e9 * 1e18;
    ...
    //Wallets
    address public marketingWallet = 0x99A95A52953F1336f378047F00878Ac0e100765f;
    ...
    constructor() ERC20("BabyPsyop", "$BABYPSYOP") {
        address newOwner = 0xB5C562F175d6Df133fe49b34837784936512B617;
        uniswapRouter = DexRouter(0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D);
        pairAddress = DexFactory(uniswapRouter.factory()).createPair(address(this), USDT);
    ...
    function updateTax(uint256 marketingFeeOnBuy, uint256 makretingFeeOnSell) public onlyOwner {
        require(marketingFeeOnBuy <= 10, "Can't set marketing fee on buy more than 10%");
        require(makretingFeeOnSell <= 10, "Can't set marketing fee on buy more than 10%");
        buyTaxes.marketingTax = marketingFeeOnBuy;
        sellTaxes.marketingTax = makretingFeeOnSell;
    }
    ...
    function internalSwap(uint256 amountTokens) internal {
        if (amountTokens == 0) {
            return;
        }
        swapToUSDTandSendToMarketing(amountTokens);
    }

    function swapToUSDTandSendToMarketing(uint256 _amount) internal {
        address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = USDT;
        _approve(address(this), address(uniswapRouter), _amount);
        uniswapRouter.swapExactTokensForTokensSupportingFeeOnTransferTokens(
            _amount, 0, path, marketingWallet, block.timestamp
        );
    }
    ...
    function withdrawStuckTokens(address ERC20_token) external onlyOwner {
        IERC20(ERC20_token).transfer(msg.sender, IERC20(ERC20_token).balanceOf(address(this)));
    }
