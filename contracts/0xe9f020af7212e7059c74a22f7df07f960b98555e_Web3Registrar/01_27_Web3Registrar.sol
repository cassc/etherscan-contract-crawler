...
contract Web3Registrar is ERC721Enumerable, Web3RegistrarVerifier {
    ...
    ) ERC721("Web3ite Pass", "WEB3") Web3RegistrarVerifier(verifierAddress) {
        registry = _registry;
    ...
    function register(LibWeb3Domain.Order memory order, bytes memory signature)
        external
        payable
    {
        ...
        registry.setSubnodeOwner(baseNode, bytes32(order.tokenId), order.owner);
