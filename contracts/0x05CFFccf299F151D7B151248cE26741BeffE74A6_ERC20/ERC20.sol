...
contract CogniCents is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    CogniCentsDividendTracker public dividendTracker;
    ...
    constructor() ERC20("CogniCents", "COC") {
        marketingWallet = payable(0xdAF7386d0544b3B9c1fB3E99EaB8341D57C89904);
        devWallet = payable(0xdAF7386d0544b3B9c1fB3E99EaB8341D57C89904);
        ...
        dividendTracker = new CogniCentsDividendTracker(
            payable(this),
            router,
            0xdAF7386d0544b3B9c1fB3E99EaB8341D57C89904,
            "CogniCentsTRACKER",
            "COCTRACKER"
        );
        ...
        uint256 totalTokenSupply = (12_000_000_000) * (10**18);
        _mint(owner(), totalTokenSupply); // only time internal mint function is ever called is to create supply
        maxWallet = totalTokenSupply / 200000000; // 5%
        swapTokensAtAmount = totalTokenSupply / 200000000; // 5%;
...
contract CogniCentsDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    CogniCents public CogniCentsContract;
    ...
    constructor(
        ...
        defaultToken = token;
        CogniCentsContract = CogniCents(mainContract);
    ...
    function _transfer(
        ...
        require(false, "CogniCents_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "CogniCents_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main CogniCents contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(CogniCentsContract);

            uint256 prevBalance = CogniCentsContract.balanceOf(address(this));
            ...
            {
                uint256 received = CogniCentsContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    CogniCentsContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
