...
contract PropsERC721AInteractive is
  Initializable,
  IOwnable,
  IAllowlist,
  IConfig,
  ...
  ERC2981
{
  ...
  bytes32 private constant MODULE_TYPE = bytes32("PropsERC721AInteractive");
  ...
  function tokenURI(uint256 _tokenId) public view virtual override returns (string memory) {
      ...
      return string(abi.encodePacked(baseURI_, _tokenId.toString(), ".json?seed=", StringsUpgradeable.toHexString(uint160(tokenMinter[_tokenId]), 20)));
  }
  ...
  function mint(
    ...
    unchecked {
        for (uint256 i = nextTokenId; i < nextTokenId + _quantity; i++) {
            ...
            console.log('for', msg.sender);
            tokenMinter[i] = msg.sender;
            console.log('tokenMinter[i] ', tokenMinter[i]);
        }
