...
contract InedibleXV1Pair is IInedibleXV1Pair, InedibleXV1ERC20 {
    ...
    // first liquidity lock duration
    uint40 public initialLockDuration;
    // last trade timestamp to handle MEV logic
    uint40 private lastTrade;
    ...
    function swap(
        ...
        {
            // Mev protection added by Inedible
            // only allows trade in one direction after first trade
            uint8 currentSwapType = _getSwapType(amount0In, amount1In);
            if (lastTrade < block.timestamp) {
                lastTrade = uint40(block.timestamp);
            } else if (lastTrade == block.timestamp && currentSwapType == 1) {
                lastTrade = uint40(block.timestamp + 1);
            } else if (lastTrade == block.timestamp && currentSwapType == 2) {
                lastTrade = uint40(block.timestamp + 2);
            } else if (lastTrade == block.timestamp && currentSwapType == 0) {
                lastTrade = uint40(block.timestamp + 3);
            } else if (lastTrade == block.timestamp + 1) {
                require(currentSwapType == 1, "MEV detected 1");
            } else if (lastTrade == block.timestamp + 2) {
                require(currentSwapType == 2, "MEV detected 2");
            } else {
                // revert if last swapType == 0
                revert("MEV detected 0");
            }
        }
    ...
    /**
     * @dev used to get current swap type
     * @param token0In amount of token0In
     * @param token1In amount of token1In
     * @return uint representing swap type 0 = none, 1 = buy, 2 = sell
     **/
...
// File contracts/InedibleXV1Factory.sol
