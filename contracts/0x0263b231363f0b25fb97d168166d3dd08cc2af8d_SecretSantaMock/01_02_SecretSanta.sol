...
contract SecretSanta is ERC721TokenReceiver {
    ...
    function draw(uint256 jollyId) external inState(State.WITHDRAW) nonReentrant() returns (address nft_, uint256 id_) {
        ...
        uint256 nftId = nft.id_ == type(uint88).max ? bigIds[list[index]] : nft.id_;
        ...
        // Transfer the NFT
        IERC721Like(nft.address_).transferFrom(address(this), msg.sender, nftId);
    }
