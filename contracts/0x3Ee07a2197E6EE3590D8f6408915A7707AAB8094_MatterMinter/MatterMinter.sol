...
contract MatterToken is ERC20, ERC20Detailed, ERC20Mintable, ERC20Capped, Ownable {
	/// @notice The daily allowed MTR to be minted
	uint256 public mintableDaily;

	constructor() public ERC20Detailed("Matter", "MTR", 18) ERC20Capped(433494437 * 1e18) {
		mintableDaily = 1000 * 1e18;
	}
...
contract MatterMinter {
	...
	/// @notice The MTR token address
	address public mtrAddress;

	/// @notice The DEN Multisig
	...
	constructor(
		...
		mtrAddress = _mtrAddress;
		mintAddress = _mintAddress;
		denMultiSig = msg.sender;
		minMintTimeIntervalSec = 24 hours;
		mintWindowOffsetSec = 0; // 12 AM UTC mint
		mintWindowLengthSec = 3 * 60 * 60; // 180 minutes
	}
	 ...
	 * @notice must be den multisig
	 * @param _address The new mint address where freshly minted MTR will be sent
	 */
	function setMintAddress(address _address) public onlyDenMultiSig {
	...
	function _inMintWindow() internal view {
		require(now.mod(minMintTimeIntervalSec) >= mintWindowOffsetSec, "too early");
		require(now.mod(minMintTimeIntervalSec) < (mintWindowOffsetSec.add(mintWindowLengthSec)), "too late");
	}
	...
	function mint() public {
		...
		uint256 previousMintTimestampSec = lastMintTimestampSec;
		// Snap the mint time to the start of this window.
		lastMintTimestampSec = now.sub(now.mod(minMintTimeIntervalSec)).add(mintWindowOffsetSec);

		uint256 mintMultiplier = lastMintTimestampSec.sub(previousMintTimestampSec) / 1 days;
		epoch = epoch.add(mintMultiplier);

		MatterToken mtr = MatterToken(mtrAddress);
		uint256 mintAmount = mtr.mintableDaily().mul(mintMultiplier);
		bool minted = mtr.mint(mintAddress, mintAmount);
		require(minted, "error occurred while minting");
		emit Mint(epoch, mintAmount);
	}
