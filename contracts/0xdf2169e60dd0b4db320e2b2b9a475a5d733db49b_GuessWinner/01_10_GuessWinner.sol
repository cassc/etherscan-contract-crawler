...
contract GuessWinner is ReentrancyGuard, Ownable {
    ...
    function deposit(
        ...
        require(
            (usersAmount[msg.sender].add(amount)) <= amountMax,
            "Amount limit"
        );

        uint256 fee = getUserFee(amount);
        uint256 reward = getRelationReward(amount);
        uint256 sa = amount.sub(fee.add(reward));

        teamAmount[teamId].add(sa);
        totalAmount.add(sa);

        teamAddress[teamId][msg.sender].add(sa);

        usersAmount[msg.sender].add(amount);
