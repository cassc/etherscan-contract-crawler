...
contract CTokenStorage {
    ...
    /**
     * @notice Address of migrator contract
     */
    address payable public migrator;
}
...
abstract contract CTokenInterface is CTokenStorage {
    ...
    /**
     * @notice Event emitted when tokens are minted to
     */
    event MintTo(address minter, address receiver, uint mintAmount, uint mintTokens);
...
abstract contract CErc20AuraInterface is CErc20Interface {

    /*** User Interface ***/

    function mintTo(uint mintAmount, address user) external virtual returns (uint);
    function redeemAndUnwrap(uint redeemTokens, bool restake) external virtual returns (uint);
}

abstract contract CErc20YearnInterface is CErc20Interface {

    /*** User Interface ***/

    function mintTo(uint mintAmount, address user) external virtual returns (uint);
    function redeemAndUnwrap(uint redeemTokens) external virtual returns (uint);
    function borrowUnderlying(uint borrowAmount) external virtual returns (uint);
}
