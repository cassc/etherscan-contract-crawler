...
contract SwapFacet is Signers, VerifySigEIP712 {
    ...
    function swapRouter(SwapData calldata _swap, bytes calldata _plexusData) external payable {
        ...
        emit LibData.Swap(_swap.user, _swap.srcToken, _swap.dstToken, _swap.amount, dstAmount, _plexusData);
    }
    ...
    function relaySwapRouter(SwapData calldata _swap, Input calldata _sigCollect, bytes[] calldata signature, bytes calldata _plexusData) external {
        SigData storage s = sigDataStorage();
        ...
        s.txHashCheck[_sigCollect.txHash] = true; // add
        ...
        emit LibData.Relayswap(_sigCollect.userAddress, _sigCollect.toTokenAddress, dstAmount, _plexusData);
    }
}