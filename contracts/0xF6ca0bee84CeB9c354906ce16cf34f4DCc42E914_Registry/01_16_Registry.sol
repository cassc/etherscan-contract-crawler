...
contract Registry is Initializable, OwnableUpgradeable, UUPSUpgradeable {
    ...
    event DomainRegisterPublicWithID(address _user, string _domain, uint _ethBalance, uint _nftId, uint _time);
    event ReserveDomainRegisterWithID(
        address _user,
        string _domain,
        uint _ethBalance,
        uint _nftId,
    ...
    event ReserveDomainRegisterOwnerWithID(address _user, string _domain, uint _nftId, uint _time);
    ...
    function buyReserveDomainByOwner(string memory _domain) public onlyOwner {
        ...
        uint nftsMinted = modsMNSNFT.totalSupply();
        modsMNSNFT.mintTokens(msg.sender);

        emit ReserveDomainRegisterOwnerWithID(msg.sender, _domain, nftsMinted, block.timestamp);
    }
    ...
    function buyReserveDomain(string memory _domain) public payable {
        ...
        if (getModsPriceInUsdc() <= modsBalance && modsDiscountAmount != 0 && modsDiscountAmount != 1) {
            _ethPrice = (_ethPrice -
                (_ethPrice * modsHoldersDiscountPercent) /
                100);
        } else if(modsDiscountAmount == 1) {
            _ethPrice = (_ethPrice -
                (_ethPrice * modsHoldersDiscountPercent) /
                100);
        }
        ...
        emit ReserveDomainRegisterWithID(msg.sender, _domain, _ethPrice, nftsMinted, block.timestamp);
    }
    ...
    function registerDomainPublic(string memory _domain) public payable {
        ...
        emit DomainRegisterPublicWithID(msg.sender, _domain, _ethPrice, nftsMinted, block.timestamp);
    }
