...
contract TORUSMintingRebalancingRewardsHandler is ITORUSMintingRebalancingRewardsHandler, Ownable, BaseMinter {
    ...
    uint256 internal constant _INITIAL_REBALANCING_REWARD_PER_DOLLAR_PER_SECOND = 1e18 / uint256(3600 * 1 * 10_000 * 10);
    ...
    modifier onlyInflationManager() {
        require(msg.sender == address(controller.inflationManager()), "only InflationManager can call this function");
        _;
    }

    constructor(IController _controller, ITORUSToken _torus, address emergencyMinter)
    ...
    function setTorusRebalancingRewardPerDollarPerSecond(uint256 _torusRebalancingRewardPerDollarPerSecond)
        external
        override
        onlyOwner
    {
    ...
    function poolTORUSRebalancingRewardPerSecond(address pool) public view override returns (uint256) {
        (uint256 poolWeight, uint256 totalUSDValue) = controller.inflationManager().computePoolWeight(pool);
    ...
    function handleRebalancingRewards(
        ...
        uint256 torusRewardAmount = computeRebalancingRewards(address(torusPool), deviationBefore, deviationAfter);
    ...
    function computeRebalancingRewards(address torusPool, uint256 deviationBefore, uint256 deviationAfter)
        public
        view
        override
        returns (uint256)
    {
        ...
        uint256 deviationImprovementRatio =
            deviationDelta.divDown(ITorusPool(torusPool).totalDeviationAfterWeightUpdate());
