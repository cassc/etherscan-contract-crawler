...
contract WUSETHer is IERC721Receiver, ReentrancyGuard
{
  ...
  constructor ()
  {
    IERC20(_WUSD).safeApprove(address(_NFPM), type(uint128).max);
    IERC20(_WETH).safeApprove(address(_NFPM), type(uint128).max);
  }
  ...
  {
    return Math.max(1e6, (_percent(amount, 1_00) * 1e6) / 1e18);
  }
  ...
  function _return (Returnance memory returnance, uint256 nft) internal
  {
    ...
    {
      IERC20(_WUSD).safeTransfer(msg.sender, returnance.amount0);


      if (returnance.creditized)
      {
        uint256 multiplier = Math.min((returnance.amount0 * 100_00).div(100_000e18), 100_00);
    ...
    {
      IERC20(_WETH).safeTransfer(msg.sender, returnance.amount1);
    }
  }
  ...
  function provide (uint256 addable0, uint256 addable1, address referrer) external nonReentrant
  {
    ...
    IERC20(_WUSD).safeTransferFrom(msg.sender, address(this), addable0);
    IERC20(_WETH).safeTransferFrom(msg.sender, address(this), addable1);
    ...
    IERC20(_USDT).safeTransferFrom(msg.sender, _REGISTRY.collector(), _fee(added0));
    ...
    emit Provide(msg.sender, nft, added0, referrer);
  }
  ...
  function _decrease (uint256 nft, uint256 percentage) internal
  {
    ...
    ({
      tokenId: nft,
      liquidity: uint128(_percent(_provision[nft].liquidity, percentage)),
      amount0Min: 0,
      amount1Min: 0,
      deadline: block.timestamp
    }));
  ...
  function decrease (uint256 percentage) external nonReentrant
  {
    ...
    require(percentage > 0 && percentage < 100_00, "!valid %");
