...
contract LockableToken is BurnableToken {
  ...
  function getLockedAmount(address granted) public view returns(uint) {
    ...
    uint len = grantedLocks[granted].length;
    for (uint i = 0; i < len; i++) {
      if (block.timestamp < grantedLocks[granted][i].expiresAt) {
...
contract OrchardbitToken is LockableToken {
  ...
  function transferWithLocks(address granted, uint amount, uint[] memory lockAmounts, uint[] memory lockTimes) public onlyOwnerOrOperator {
    require(lockAmounts.length == lockTimes.length, "Array length are not equal");
    uint totalLockAmount = 0;
    for(uint i = 0; i < lockAmounts.length; i++){
        totalLockAmount = totalLockAmount + lockAmounts[i];
    }
    require(amount >= totalLockAmount, "Total lock amount can not be greater than amount");
    
    for(uint i = 0; i < lockAmounts.length; i++){
        addLock(granted, lockAmounts[i], lockTimes[i]);
    }
    transfer(granted, amount);
  }
}