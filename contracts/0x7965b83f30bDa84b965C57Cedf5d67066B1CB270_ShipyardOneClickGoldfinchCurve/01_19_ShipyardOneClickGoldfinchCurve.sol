...
contract ShipyardOneClickGoldfinchCurve is Ownable, SlippageManager, ReentrancyGuard {
    ...
    function withdraw(address _shipyardVaultAddress, address _requestedTokenAddress, uint256 _withdrawAmountInShipToken) external nonReentrant {
        ...
        if (isUnderlyingToken) {
            ICurveSwap(poolAddress).safeRemoveLiquidityOneCoin(
                slippage,
                poolTokenBalance,
                strategy.underlyingTokenIndex(_requestedTokenAddress)
            );
        ...
        ICurveSwap(poolAddress).safeRemoveLiquidityOneCoin(
            slippage,
            poolTokenBalance,
            strategy.underlyingTokenIndex(preferredTokenAddress)
        );
