...
contract FlowerPower is ERC721A, Ownable {
  ...
  uint256 public MAX_PRESALE_MINTS = 3;
  uint256 public MAX_PUBLIC_MINTS = 1;
  uint256 public PUBLIC_PRICE = 0.0 ether;
  uint256 public PRESALE_PRICE = 0.0 ether;
  ...
  mapping(address => uint256) private _allowedPublic;

  string private _baseURIextended;
  string public notRevealedUri;

  address[] private mintPayees = [
    0x8C2D15ef108c60FC2a749f30F24b20e4b4Ed1F89
  ];
    ...
    payable
  {
    ...
    require(_allowed[msg.sender] + nMints <= MAX_PRESALE_MINTS, "Exceeds mint limit");
  ...
  function mint(uint256 nMints) external payable {
    ...
    require(_allowedPublic[msg.sender] + nMints <= MAX_PUBLIC_MINTS, "Exceeds mint limit");

    // Keep track of mints for each address
    if (_allowedPublic[msg.sender] > 0) {
      _allowedPublic[msg.sender] = _allowedPublic[msg.sender] + nMints;
    } else {
      _allowedPublic[msg.sender] = nMints;
    }

    _safeMint(msg.sender, nMints);
  }

   function setPrice(uint _publicPrice, uint _presalePrice) external onlyOwner {
        PUBLIC_PRICE = _publicPrice;
        PRESALE_PRICE = _presalePrice;  
    }

    function setMaxMint(uint _maxPresaleMint, uint _maxPublicMint) external onlyOwner {
        MAX_PRESALE_MINTS = _maxPresaleMint;
        MAX_PUBLIC_MINTS = _maxPublicMint;  
    }
