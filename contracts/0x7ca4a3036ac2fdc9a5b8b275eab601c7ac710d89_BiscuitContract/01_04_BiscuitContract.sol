...
contract BiscuitContract is Ownable {
    ...
    function endGame(
        ...
        require(foundLoser, "Invalid loser address");
        ...
        uint256 revenueShare = (g.betAmounts[loserIndex] * revenueBps) / 10000;

        uint256 tokensPerWinner = (totalBets +
            g.betAmounts[loserIndex] -
            revenueShare) / (g.players.length - 1);

        for (uint16 i = 0; i < g.players.length; i++) {
            address winner = g.players[i];
            if (i != loserIndex) {
                chosenToken.transfer(winner, tokensPerWinner);
                emit TokensAwarded(_tgChatId, winner, tokensPerWinner);
            }
        }
