...
library AspenERC1155DropLogic {
    ...
    function setClaimConditions(
        ...
        if ((claimData.nextTokenIdToMint <= _tokenId)) revert IDropErrorsV3.InvalidTokenId(_tokenId);
        ...
        for (uint256 i = 0; i < _phases.length; i++) {
            ...
            if (_isOutOfLimits(_phases[i].maxClaimableSupply, supplyClaimedAlready))
                revert IDropErrorsV3.CrossedLimitMaxClaimableSupplySft(
                    _tokenId,
    ...
    function executeClaim(
        ...
        if ((claimData.nextTokenIdToMint <= claimExecutionData.tokenId))
            revert IDropErrorsV3.InvalidTokenId(claimExecutionData.tokenId);
    ...
    function verifyClaimQuantity(
        ...
        if (!(_quantity > 0 && (_quantity <= currentClaimPhase.quantityLimitPerTransaction))) {
            revert IDropErrorsV3.CrossedLimitQuantityPerTransactionSft(
                _tokenId,
        ...
        if (_isOutOfLimits(claimData.maxTotalSupply[_tokenId], claimData.totalSupply[_tokenId] + _quantity)) {
            revert IDropErrorsV3.CrossedLimitMaxTotalSupplySft(
                _tokenId,
                claimData.maxTotalSupply[_tokenId] - claimData.totalSupply[_tokenId]
            );
        }
        if (
            _isOutOfLimits(
            ...
            revert IDropErrorsV3.CrossedLimitMaxWalletClaimCountSft(
                _tokenId,
                claimData.maxWalletClaimCount[_tokenId] - claimData.walletClaimCount[_tokenId][_claimer]
            );
        }
    }
    ...
    function verifyBatch(
        ...
        if (_isOutOfLimits(claimData.maxTotalSupply[_tokenId], claimData.totalSupply[_tokenId] + totalQuantity)) {
            revert IDropErrorsV3.CrossedLimitMaxTotalSupplySft(
                _tokenId,
                claimData.maxTotalSupply[_tokenId] - claimData.totalSupply[_tokenId]
            );
