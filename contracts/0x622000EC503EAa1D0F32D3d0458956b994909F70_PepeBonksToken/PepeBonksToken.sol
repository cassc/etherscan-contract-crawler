...
contract PepeBonksToken is IERC20, Auth {
    string private constant _name         = "Pepe Bonks";
    string private constant _symbol       = "PEPEB";
    uint8 private constant _decimals      = 9;
    uint256 private constant _totalSupply = 10_000_000_000_000 * (10**_decimals);
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;

    uint32 private _tradeCount;

    address payable private constant _walletMarketing = payable(0xF67eF1106f0810c6d99c3316Ce5755E58320f44c);
    uint256 private constant _taxSwapMin = _totalSupply / 2000000;
    uint256 private constant _taxSwapMax = _totalSupply / 10000;
    ...
    function tax() external pure returns (uint32 taxNumerator, uint32 taxDenominator) {
        (uint32 numerator, uint32 denominator) = _getTaxPercentages();
        return (numerator, denominator);
    }

    function _getTaxPercentages() private pure returns (uint32 numerator, uint32 denominator) {
        return (50000, 1_000_000);
    }
