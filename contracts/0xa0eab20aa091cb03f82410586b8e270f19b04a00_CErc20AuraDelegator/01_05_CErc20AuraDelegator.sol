...
/**
 * @title Compound's CErc20AuraDelegator Contract
 * @notice CTokens which wrap an EIP-20 underlying and delegate to an implementation
 * @author Compound
 */
contract CErc20AuraDelegator is CTokenInterface, CErc20AuraInterface, CDelegatorInterface {
    /**
     ...
     * @dev Accrues interest whether or not the operation succeeds, unless reverted
     * @param redeemTokens The number of cTokens to redeem into underlying
     * @param restake restake
     * @return uint 0=success, otherwise a failure (see ErrorReporter.sol for details)
     */
    function redeemAndUnwrap(uint redeemTokens, bool restake) override external returns (uint) {
        bytes memory data = delegateToImplementation(abi.encodeWithSignature("redeemAndUnwrap(uint256,bool)", redeemTokens, restake));
        return abi.decode(data, (uint));
    }
