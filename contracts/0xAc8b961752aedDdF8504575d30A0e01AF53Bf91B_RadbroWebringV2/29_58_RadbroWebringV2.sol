    ...
    ERC721Holder
{
    ...
    uint256 public constant V1_UPRGADE_MINTS = 1632;
    ...
    /// @notice Upgrade radbros from v1. Public function for users to upgrade from v1 contract.
    /// @dev 5% chance of winning a radbro
    /// @dev 10% chance of winning 100 $RAD
    function upgradeFromV1(
        ...
        for (uint256 i = 0; i < radbroIds.length; i++) {
            ...
            radbroV1.safeTransferFrom(_msgSender(), address(this), radbroId);
            this.safeTransferFrom(address(this), _msgSender(), radbroId);
        }

        // 5% chance of winning a radbro
        // 10% chance of winning 100 $RAD
        // chance multiplied by the # of radbros being upgraded.
        uint256 roll = (randProvider.getRandSeed(1000) * radbroIds.length);
        if (roll > 950 && mintedPrizeRadbros < MAX_PRIZE_RADBROS) {
        ...
        // mint unclaiimed v1 radcoin as v2 radcoin
        if (claimRadcoin) {
            uint256 radcoinBalance = radcoinV1.getClaimRewards(radbroIds);

            if (radcoinBalance > 0) {
                radcoinV2.mintForRadbros(_msgSender(), radcoinBalance);
            }
        }

        if (radcoinToUpgrade > 0) {
            ERC20(address(radcoinV1)).safeTransferFrom(_msgSender(), address(this), radcoinToUpgrade);
            radcoinV2.mintForRadbros(_msgSender(), radcoinToUpgrade);
        }

        emit RadbroUpgraded(_msgSender(), uint32(radbroIds.length));
    }
