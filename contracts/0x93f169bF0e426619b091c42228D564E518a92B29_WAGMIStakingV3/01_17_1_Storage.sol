...
contract WAGMIStakingV3 is UUPSUpgradeable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function availableRewards(address _staker) public view returns (uint256) {
        ...
        uint256 rewardsV3 = calculateRewards(_staker) + stakers[_staker].unclaimedRewards;
        return rewardsV1 + rewardsV2 + rewardsV3;
    }

    function getStakedTokens(address _user) public view returns (StakedToken[] memory) {
        IWAGMIStakingV1.StakedToken[] memory tokensV1;
        try stakingV1.getStakedTokens(_user) returns (IWAGMIStakingV1.StakedToken[] memory result) {
            tokensV1 = result;
        } catch {
            tokensV1 = new IWAGMIStakingV1.StakedToken[](0);
        }
        
        IWAGMIStakingV2.StakedToken[] memory tokensV2;
        try stakingV2.getStakedTokens(_user) returns (IWAGMIStakingV2.StakedToken[] memory result) {
            tokensV2 = result;
        } catch {
            tokensV2 = new IWAGMIStakingV2.StakedToken[](0);
        }
        ...
        for (; i < tokensV1.length; i++) {
            IWAGMIStakingV1.StakedToken memory token = tokensV1[i];
            tokens[i] = StakedToken(token.tokenId, token.tokenStoreTime, 1, vipTokenIds[token.tokenId]);
        }

        uint j=0;
        while (j < tokensV2.length) {
            IWAGMIStakingV2.StakedToken memory token = tokensV2[j++];
            tokens[i++] = StakedToken(token.tokenId, token.tokenStoreTime, 2, vipTokenIds[token.tokenId]);
        }

        uint k=0;
        while (k < tokensV3.length) {
            uint256 tokenId = tokensV3[k++];
            tokens[j++] = StakedToken(tokenId, tokenStakeTime[tokenId], 3, vipTokenIds[tokenId]);
        }
