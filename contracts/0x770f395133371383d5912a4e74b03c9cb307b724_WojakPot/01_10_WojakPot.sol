...
contract WojakPot is ERC20, Ownable, VRFConsumerBaseV2, ReentrancyGuard {
    ...
    function updateSellFees(
        ...
        totalSellFee = marketingAndTeamFeeSell + rewardFeeSell;
        require(totalSellFee <= 50, "Sell tax too high");
    }
    ...
    ///@notice As divisor is 1000 for calculation, so 5 means 0.5% and 10 means 1%
     function updateBuyFees(
        ...
        totalBuyFee = marketingAndTeamFeeBuy + rewardFeeBuy;
        require(totalBuyFee <= 50, "Buy tax too high");
    }
    ...
    function setMaxTxLimit (uint256 amount) external onlyOwner {
        maxTx = amount;
        require (maxTx >= totalSupply()/100, "Max Tx can't be lower than 1% of the supply");
    }
