...
// File src/contracts/shared/BaseTokenURI.sol
...
// File src/contracts/interfaces/ISummitWhitelabelNftFactory.sol

pragma solidity >=0.5.0;

interface ISummitWhitelabelNftFactory {
  function withdrawFee() external view returns (uint256);
}

// File src/contracts/SummitWhitelabelNft.sol
...
struct TokenInfo {
  ...
  uint256 whitelistMintPerWallet; // 0 means no limit
  uint256 publicMintPerWallet; // 0 means no limit
  Phase phase;
  bool isReveal;
}

contract SummitWhitelabelNft is DefaultOperatorFilterer, ERC721AQueryable, ERC2981, BaseTokenURI {
  ...
  function mint(uint256 _mintAmount) external payable {
    ...
    require(
      tokenInfo.publicMintPerWallet == 0 || publicMinted[_msgSender()] + _mintAmount <= tokenInfo.publicMintPerWallet,
  ...
  function mint(uint256 _mintAmount, bytes memory _signature) external payable {
    ...
    require(
      tokenInfo.whitelistMintPerWallet == 0 ||
        whitelistMinted[_msgSender()] + _mintAmount <= tokenInfo.whitelistMintPerWallet,
  ...
  function setMintPerWallet(uint256 _whitelistMintPerWallet, uint256 _publicMintPerWallet) external onlyOwner {
    tokenInfo.whitelistMintPerWallet = _whitelistMintPerWallet;
    tokenInfo.publicMintPerWallet = _publicMintPerWallet;
  ...
  function revealMetadata(string memory _baseUri) external onlyOwner {
    ...
    BaseTokenURI.setBaseTokenURI(_baseUri);
