...
contract Borex_Poissons is ERC20, ERC20Burnable, Ownable, Mintable, Pausable, TokenRecover, DividendTrackerFunctions {
    
    mapping (address => bool) public blacklisted;

    address public bppcAddress;
    uint16[3] public bppcFees;

    uint256 public swapThreshold;
    
    uint256 private _bppcPending;
    ...
    event bppcAddressUpdated(address bppcAddress);
    event bppcFeesUpdated(uint16 buyFee, uint16 sellFee, uint16 transferFee);
    event bppcFeeSent(address recipient, uint256 amount);
    ...
    constructor()
        ERC20("Borex Poissons", "PBPC") 
        Mintable(20000000)
    {
        address supplyRecipient = 0xaE67F61F47Df15C9337d105463228e264423b763;
        
        bppcAddressSetup(0x9c49169A613FF0946aBB450D3b9B584aCb144237);
        bppcFeesSetup(100, 500, 1000);

        updateSwapThreshold(5000 * (10 ** decimals()));

        _deployDividendTracker(7200, 1 * (10 ** decimals()), address(this));

        gasForProcessingSetup(300000);
        rewardsFeesSetup(100, 500, 1000);
        ...
        _transferOwnership(0xaE67F61F47Df15C9337d105463228e264423b763);
    }
    ...
    function bppcAddressSetup(address _newAddress) public onlyOwner {
        bppcAddress = _newAddress;

        excludeFromFees(_newAddress, true);

        emit bppcAddressUpdated(_newAddress);
    }

    function bppcFeesSetup(uint16 _buyFee, uint16 _sellFee, uint16 _transferFee) public onlyOwner {
        bppcFees = [_buyFee, _sellFee, _transferFee];

        totalFees[0] = 0 + bppcFees[0] + rewardsFees[0];
        totalFees[1] = 0 + bppcFees[1] + rewardsFees[1];
        totalFees[2] = 0 + bppcFees[2] + rewardsFees[2];
        require(totalFees[0] <= 2500 && totalFees[1] <= 2500 && totalFees[2] <= 2500, "TaxesDefaultRouter: Cannot exceed max total fee of 25%");

        emit bppcFeesUpdated(_buyFee, _sellFee, _transferFee);
    }
    ...
    function _transfer(
        ...
        if (!_swapping && amount > 0 && to != address(uniswapV2Router) && !isExcludedFromFees[from] && !isExcludedFromFees[to]) {
            ...
            if (txType < 3) {
                ...
                if (bppcFees[txType] > 0) {
                    bppcPortion = fees * bppcFees[txType] / totalFees[txType];
                    _sendInTokens(from, bppcAddress, bppcPortion);
                    emit bppcFeeSent(bppcAddress, bppcPortion);
                }

                _rewardsPending += fees * rewardsFees[txType] / totalFees[txType];

                fees = fees - bppcPortion;
            }
