...
contract StakeDoge4 is IStakingRewards, RewardsDistributionRecipient, ReentrancyGuard, Pausable {
    ...
    uint256 public feeScale = 50000;

    //NOTE:modify me before mainnet
    address public feeCollector = 0x7A7947985bEFA26fD273f5E3490b6CbbF1e25732;
    ...
    function withdrawableAmount(address account)public view returns(uint256){
        ...
        for (uint256 index = 0; index < _timedStake.stakeTimes.length; index++) {
...
function stake(uint256 amount) external nonReentrant notPaused updateReward(msg.sender) {
        ...
        // Add the staked amount for the same timestamp instead of overwriting it
        _timedStake.stakes[now] = _timedStake.stakes[now].add(amount);
        // Only push the timestamp if it's a new stake time
        if (_timedStake.stakes[now] == amount) {
