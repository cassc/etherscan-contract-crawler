...
contract MetadataTraits is IMetadataTraits, Ownable {
    ...
    function getTraits(BaseAttributes memory atts, IArtData artData) external virtual override view returns (string memory)
    {
        ...
        return string.concat(
            '[{ "trait_type": "Sky Color", "value": "',
            artData.getSkyName(atts.skyCol),
            '" }, { "trait_type": "Proximity", "value": "',
            artData.getProximityName(atts.proximity),
            '" }, { "trait_type": "Num Planes", "value": "',
            atts.numPlanes.toString(),
            '" }, { "trait_type": "Lattice", "value": "',
            atts.extraParams[uint(EP.NumAngles)].toString(),
            '" }, { "trait_type": "Trails Palette", "value": "',
            artData.getColorPaletteName(atts.palette),
            '" }',
    ...
    function appendFlockTypeString(string memory attsString, CalcInformation memory c, IArtData artData)
        internal view virtual returns (string memory)
    {
        //        uint8 flockTypeIdx = uint8((biggestFlockSize-1) + (art.matchingSpeed ? artData.getMaxNumPlanes() : 0));
        return string.concat( attsString,
