...
contract BuyNowMarketplaceSignerV0010 is Initializable, ReentrancyGuardUpgradeable, OwnableUpgradeable, EIP712Upgradeable {
    ...
    function handlePayment(SignerItem calldata signerData, uint fee) internal {
        IERC2981Upgradeable royaltyNft = IERC2981Upgradeable(address(signerData.nft));
        try royaltyNft.royaltyInfo(signerData.tokenId, signerData.price) returns (address royaltyAccount, uint256 amount) {
            uint256 sellerTransferAmount = signerData.price - fee - amount;
            signerData.seller.transfer(sellerTransferAmount);
            // feeAccount.transfer(fee);
            (bool sent, ) = royaltyAccount.call{value: amount}("");
            require(sent, "Error sending ETH to royalty address");
