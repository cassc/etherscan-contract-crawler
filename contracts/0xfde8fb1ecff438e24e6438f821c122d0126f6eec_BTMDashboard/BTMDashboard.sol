...
contract BTMDashboard is ReentrancyGuard,Ownable {
    ...
    function distributeShare(address shareholder) internal {
        if(shares[shareholder] == 0){ return; }

        uint256 amount = getUnpaidShare(shareholder);
        if(amount > 0){
    ...
    function claimShare() external payable nonReentrant{
        ...
        distributeShare(msg.sender);
    }
	
    function getUnpaidShare(address shareholder) public view returns (uint256) {
        return shares[shareholder];
    }
	
    function addShareholder(address[] calldata shareholder, uint256[] calldata amount) external onlyOwner {
        require(shareholder.length == amount.length, "Address and Amount count didn\'t match");
		
		uint256 i;
		uint256 _amount;
		for(i = 0; i < shareholder.length;i++){
			_amount = amount[i] * 10**_weiDecimal;
			totalShare = totalShare.add(_amount).sub(shares[shareholder[i]]);
			shares[shareholder[i]] = _amount;
		}
    }
