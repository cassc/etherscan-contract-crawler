...
contract Controller is Initializable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    uint256[] public accOldRewardPerShares;
     ...
     */
    function deposit(uint256 _amount) external onlyVault returns (uint256) {
        // Check input amount
    ...
    function harvest() public onlyOwner {
        // Save current Reward token amount
        uint256[] memory rewardBalances = new uint256[](rewardTokens.length);
        for (uint256 i = 0; i < rewardTokens.length; i++) {
            uint256 bal = IERC20(rewardTokens[i]).balanceOf(vault);
            rewardBalances[i] = bal;
        }
        ...
        for (uint256 i = 0; i < rewardTokens.length; i++) {
            uint256 increase = IERC20(rewardTokens[i]).balanceOf(vault) - rewardBalances[i];
            console.log("Note Increase: ", increase);
            accOldRewardPerShares[i] = accRewardPerShares[i];
            accRewardPerShares[i] += (increase * (1e18)) / IVault(vault).totalSupply();
        }
    ...
    function removeRewardToken(address _token) public onlyOwner {
        ...
        for (uint256 i = 0; i < rewardTokens.length; i++) {
            if (rewardTokens[i] == _token) {
                ...
                accOldRewardPerShares[i] = accOldRewardPerShares[accOldRewardPerShares.length - 1];
                accRewardPerShares[i] = accRewardPerShares[accRewardPerShares.length - 1];
                rewardTokens.pop();
                accRewardPerShares.pop();
                accOldRewardPerShares.pop();
    ...
    {
        return (rewardTokens, accOldRewardPerShares, accRewardPerShares);
    }
}