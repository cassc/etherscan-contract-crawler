...
contract PresaleV1 is Initializable, ReentrancyGuardUpgradeable, OwnableUpgradeable, PausableUpgradeable {
   ...
   */
  function claim(uint256 _amount, bytes32[] memory _proof) external whenNotPaused returns (bool) {
    ...
    require(MerkleProofUpgradeable.verify(_proof, claimMerkleRoot, leaf), 'Invalid proof');
    hasClaimed[_msgSender()] = true;
    bool success = IERC20Upgradeable(saleToken).transfer(_msgSender(), _amount);
    require(success, 'Token transfer failed');
    emit TokensClaimed(_msgSender(), _amount, block.timestamp);
    return true;
  }
