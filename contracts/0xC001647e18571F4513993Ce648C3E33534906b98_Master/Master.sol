...
contract Master is Ownable {
    ...
    // Market oracle provides the MAU/USD exchange rate as an 18 decimal fixed point number.
    ...
    constructor(address _mau) public {
        deviationThreshold = 5 * 10 ** (DECIMALS-2);
        // Start at .000000000051 cents 
        targetRate = (5508 * 10 ** (DECIMALS-14));
        negDamp = 2;
        posDamp = 20;

        rebaseCooldown = 8 hours; 
        lastRebaseTimestampSec = block.timestamp + rebaseCooldown;
    ...
    function rebase() external {
        ...
        uint256 supplyAfterRebase = mau.rebase(epoch, supplyDelta);
    ...
    function incrementTargetRate() internal {
        targetRate = targetRate.mul(6).div(5);
    }
    
    // Final rate of $.0001 per coin
    function finalRate() internal {
        targetRate = ( 1 ** (DECIMALS-4));
    }
    ...
    function getRebaseValues() public view returns (uint256, int256) {
        ...
        if (supplyDelta > 0 && mau.totalSupply().add(uint256(supplyDelta)) > MAX_SUPPLY) {
            supplyDelta = (MAX_SUPPLY.sub(mau.totalSupply())).toInt256Safe();
        }
        ...
        returns (int256)
    {
        ...
        return mau.totalSupply().toInt256Safe()
        ...
        onlyOwner
    {
        ...
        require(transactions.length > 0, "transactions list is empty");
        transactions[transactions.length - 1] = Transaction(false, address(0), "");
    }
