...
// File: contracts/interfaces/IRiverland.sol


pragma solidity ^0.8.9;

interface IRiverland{}
...
// File: contracts/RiverlandImpl.sol
...
contract RiverlandImpl is Initializable, ERC721Upgradeable,ERC721EnumerableUpgradeable, PausableUpgradeable, OwnableUpgradeable, ERC721BurnableUpgradeable, UUPSUpgradeable,AccessControlUpgradeable,ReentrancyGuardUpgradeable {
    using StringsUpgradeable for uint256;
    ...
    function getLands(uint256[] memory tokenIds) public view returns (Land[] memory) {
        Land[] memory lands = new Land[](tokenIds.length);
        for (uint256 index = 0; index < tokenIds.length; index++) {
            lands[index] = _landMapping[tokenIds[index]];
        }
        return lands;
    }

    function whiteListLevel1(bytes32[] calldata merkleProof, address addr) public view returns (bool){
        bytes32 leaf = keccak256(abi.encode(addr));
        return MerkleProof.verify(merkleProof, merkleRootLevel1, leaf);
    }

    function whiteListLevel2(bytes32[] calldata merkleProof, address addr) public view returns (bool){
        bytes32 leaf = keccak256(abi.encode(addr));
        return MerkleProof.verify(merkleProof, merkleRootLevel2, leaf);
    }

    function getBuyNumLevel1(address addr) public view returns (uint8,uint8){
        return ( maxNumLevel1,buyNumLevel1[addr]);
    }

    function getBuyNumLevel2(address addr) public view returns (uint8,uint8){
        return ( maxNumLevel2,buyNumLevel2[addr]);
    }

    function getLandNumLevel1() public view returns (uint16,uint16){
        return (sellNumLevel1, sellNumLevel1 - landNumLevel1);
    }

    function getLandNumLevel2() public view returns (uint16,uint16){
        uint16 num = sellNumLevel1 - landNumLevel1 + sellNumLevel2;
        return (num, num - landNumLevel2);
    }

    function buyLandLevel1(bytes32[] calldata merkleProof, uint8 buyNum) external payable whenNotPaused nonReentrant {
        _notContract();
        require(buyNum > 0,"buyNum error");
        require(msg.value == landPrice * buyNum,"price error");
        require(getStage() == BuyStage.Level1_STARTED,"stage error");
        require(whiteListLevel1(merkleProof,_msgSender()),"white list error");
        require(buyNumLevel1[_msgSender()] + buyNum <= maxNumLevel1,"buy num error");
        require(sellNumLevel1 - landNumLevel1 >= buyNum,"land num error");
        uint256[] memory tokenIds = new uint256[](buyNum);
        for (uint256 index = 0; index < buyNum; index++) {
            tokenIds[index] = _awardLandItem(_msgSender());
        }
        buyNumLevel1[_msgSender()] += buyNum;
        landNumLevel1 += buyNum;
        emit LandAwarded(_msgSender(), tokenIds, block.timestamp, 1 );
    }

    function buyLandLevel2(bytes32[] calldata merkleProof, uint8 buyNum) external payable whenNotPaused nonReentrant {
        _notContract();
        require(buyNum > 0,"buyNum error");
        require(msg.value == landPrice * buyNum,"price error");
        require(getStage() == BuyStage.Level2_STARTED,"stage error");
        require(whiteListLevel2(merkleProof,_msgSender()),"white list error");
        require(buyNumLevel2[_msgSender()] + buyNum <= maxNumLevel2,"buy num error");
        uint landNum;
        (,landNum) = getLandNumLevel2();
        require(landNum >= buyNum,"land num error");
        uint256[] memory tokenIds = new uint256[](buyNum);
        for (uint256 index = 0; index < buyNum; index++) {
            tokenIds[index] = _awardLandItem(_msgSender());
        }
        buyNumLevel2[_msgSender()] += buyNum;
        landNumLevel2 += buyNum;
        emit LandAwarded(_msgSender(), tokenIds, block.timestamp, 2 );
    }
    ...
    function setMerkleRootLevel1(bytes32 merkleRoot) external {
        require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "require admin permission");
        merkleRootLevel1 = merkleRoot;
    }

    function setMerkleRootLevel2(bytes32 merkleRoot) external {
        require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "require admin permission");
        merkleRootLevel2 = merkleRoot;
    }

    function setTimes(uint32 _level1StartTime,uint32 _level2StartTime,uint32 _level2EndTime) external {
        require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "require admin permission");
        if(_level1StartTime > 0){
            level1StartTime = _level1StartTime;
        }
        if(_level2StartTime > 0){
            level2StartTime = _level2StartTime;
        }
        if(_level2EndTime > 0){
            level2EndTime = _level2EndTime;
        }
    }

    function setMaxNumLevel1(uint8 num) external {
        require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "require admin permission");
        maxNumLevel1 = num;
    }

    function setMaxNumLevel2(uint8 num) external {
        require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "require admin permission");
        maxNumLevel2 = num;
    }

    function setLandNumLevel1(uint16 num) external {
        require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "require admin permission");
        landNumLevel1 = num;
    }

    function setLandNumLevel2(uint16 num) external {
        require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "require admin permission");
        landNumLevel2 = num;
    }

    function setSellNumLevel1(uint16 num) external {
        require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "require admin permission");
        sellNumLevel1 = num;
    }

    function setSellNumLevel2(uint16 num) external {
        require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "require admin permission");
        sellNumLevel2 = num;
    }
