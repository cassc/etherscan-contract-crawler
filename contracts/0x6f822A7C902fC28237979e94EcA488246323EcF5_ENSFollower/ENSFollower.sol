...
// File: contracts/ENSFollower.sol
...
contract ENSFollower {
    ...
    function follow(bytes32 domain, bytes32 domainToFollow) external {
        if (followingBitMap[domain][domainToFollow] == true) {
            revert("Domain is already followed");
        }

        if (registry.owner(domain) != msg.sender) {
        ...
        emit Followed(domain, domainToFollow);
    }

    function unfollow(bytes32 domain, bytes32 domainToUnfollow) external {
        ...
        emit Unfollowed(domain, domainToUnfollow);
    }

    // This event is triggered whenever a call to #follow succeeds.
    event Followed(bytes32 domain, bytes32 domainToFollow);

    // This event is triggered whenever a call to #unfollow succeeds.
    event Unfollowed(bytes32 domain, bytes32 domainToFollow);
}