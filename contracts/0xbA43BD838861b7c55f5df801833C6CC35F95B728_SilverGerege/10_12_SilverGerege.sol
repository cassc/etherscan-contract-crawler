...
contract SilverGerege is ERC721A, ERC2981, Ownable, ReentrancyGuard {
    ...
    /**
     * @notice Sets the mint whitelist status for an address
     * @dev Allows the contract owner to add or remove an address from the mint whitelist
     * @param _mintAddress The address to update the whitelist status
     * @param _isWhitelist The new whitelist status
     */
    function setMintWhitelist(address _mintAddress, bool _isWhitelist) external onlyOwner {
        require(_mintAddress != address(0), "Zero address");
        mintWhitelist[_mintAddress] = _isWhitelist;
    }

    /**
     * @notice Sets the manager status for an address
     * @dev Allows the contract owner to add or remove an address from the list of managers
     * @param _manager The address to update the manager status
     * @param _isWhitelist The new manager status
     */
    function setManager(address _manager, bool _isWhitelist) external onlyOwner {
