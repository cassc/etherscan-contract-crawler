...
abstract contract TradeV4 is ReentrancyGuardUpgradeable, OwnableUpgradeable, TradeSignaturesVerifier {
    ...
    function tradeNFT(Order memory order) internal virtual {
        safeTransferFrom(order.nftType, order.seller, order.buyer, order.nftAddress, order.tokenId, order.quantity);
    }
    ...
    function buyAssetWithETH(
        ...
        require((feeDistributionData.fees.price >= order.unitPrice * order.quantity), "Paid invalid amount");
        ...
        emit BuyAsset(order.seller, order.tokenId, order.quantity, msg.sender);
