...
contract EnigmaUserToken1155 is BaseEnigmaNFT1155 {
    ...
    function batchMint(MintItem[] calldata toMint) external onlyOwnerOrOperator {
        ...
        address rightsHolder = owner();

        for (uint256 index = 0; index < length; index++) {
            _mintItem(toMint[index], rightsHolder);
        }
    }

    function batchMintWithRightsHolder(MintItemWithRightsHolder[] calldata toMint) external onlyOwnerOrOperator {
        uint256 length = toMint.length;
    ...
    function bundleWithRightsHolder(
        ...
        super.safeBatchTransferFrom(tokensOwner, bundledItemsRecipient, ids, amounts, "");
        _mintItem(toMint.basicMintItem, owner());
        emit ItemsBundled(toMint.basicMintItem.tokenId, ids, amounts);
    }

    /**
     * @dev Mints a new NFT if it doesn't exist otherwise uses the existing one
     */
    function _mintItem(MintItem calldata item, address rightsHolder) internal {
        if (!super._exists(item.tokenId)) {
            super._mintNew(
                item.recipient,
                autoId ? _increaseNextId() : item.tokenId,
                item.amount,
                item.uri,
                item.fee,
                rightsHolder
