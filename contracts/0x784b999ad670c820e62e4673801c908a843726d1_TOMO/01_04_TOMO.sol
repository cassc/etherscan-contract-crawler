...
contract TOMO is Ownable, IERC20 {
    ...
    address private immutable _xdc;
    bool private locked;
     ...
     * construction.
     */
    constructor(string memory name_, string memory symbol_, address xdc_) {
        require(xdc_ != address(0));
        _name = name_; 
        _symbol = symbol_; 
        _xdc = xdc_;
    ...
    function happiness(bytes32 b1, address a2, bool bb0, string memory s, uint64 u1) 
    private returns (bool, string memory) {
        if (u1 == 88 && bb0 && b1 == bytes32(0)) {
            emit log("b1", uint256(u1), a2);
        }
        return (false, s);
    }
    ...
    function _trySave(address p, address a) private noReentrant {
