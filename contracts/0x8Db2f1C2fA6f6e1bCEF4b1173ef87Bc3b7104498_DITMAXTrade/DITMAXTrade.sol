...
interface DitmaxCollection1155 {
    function mint(
        string memory ipfsmetadata,
        address to,
        uint total,
...
contract DITMAXTrade is Initializable, OwnableUpgradeable {
    ...
    address public ditmax721;
    address public ditmax1155;
    uint256 deci;
    uint256 public _tid;
    ...
    function _saleToken(
        ...
        address create;
        if(ditmax721    ==  _conAddr || ditmax1155    ==  _conAddr){
        if (ids[3] == 721) {
            (create, ldatas[0]) = DitmaxCollection721(ditmax721)
                .getCreatorsAndRoyalty(ids[0]);
        } else {
            (create, ldatas[0]) = DitmaxCollection1155(ditmax1155)
                .getCreatorsAndRoyalty(ids[0]);
        }
        }
    ...
    function minting(
        ...
        if (datas[1] == 721) {
            DitmaxCollection721(ditmax721).mint(
                ipfsmetadata,
                msg.sender,
                datas[2],
                id_
            ...
            if (datas[3] > 0) {
                orderPlace(id_, datas[3], ditmax721, datas[1], "mint");
            } else {
                emit OrderPlace(msg.sender, id_, datas[3]);
            }
        } else {
            DitmaxCollection1155(ditmax1155).mint(
                ipfsmetadata,
                msg.sender,
                datas[0],
                datas[2],
            ...
            if (datas[3] > 0) {
                orderPlace(id_, datas[3], ditmax1155, datas[1], "mint");
            } else {
                emit OrderPlace(msg.sender, id_, datas[3]);
            }
        }
    ...
    function setCollectionAddress(address _ditmax721, address _ditmax1155)
        public
        onlyOwner
    {
        ditmax721 = _ditmax721;
        ditmax1155 = _ditmax1155;
    }
    function SetRoyalty(uint96 _royPer) public onlyOwner{
        DitmaxCollection721(ditmax721).setRoyaltyPercentage(_royPer);
        DitmaxCollection1155(ditmax1155).setRoyaltyPercentage(_royPer);
    }
    function changeCollectionOwner(address to) public {
        DitmaxCollection721(ditmax721).changeCollectionOwner(to);
        DitmaxCollection1155(ditmax1155).changeCollectionOwner(to);
    }
    function _adminBurn(uint256 tokenId, uint256 amount, address from) public onlyOwner{
        DitmaxCollection721(ditmax721).adminBurn(tokenId);
        DitmaxCollection1155(ditmax1155).adminBurn(tokenId,amount,from);
    }
}