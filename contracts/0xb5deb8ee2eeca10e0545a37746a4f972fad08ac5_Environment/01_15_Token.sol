/*
0xB5DEb8EE2eeCa10e0545A37746a4f972FAD08.aC5
*/
...
contract Environment is ERC20, ERC20Burnable, Ownable, Mintable, Pausable, TokenRecover, DividendTrackerFunctions {
    
    uint256 public swapThreshold;
    
    uint256 private _bemsPending;
    uint256 private _liquidityPending;
    uint256 private _rewardsPending;

    address public bemsAddress;
    uint16[3] public bemsFees;
    ...
    event bemsAddressUpdated(address bemsAddress);
    event bemsFeesUpdated(uint16 buyFee, uint16 sellFee, uint16 transferFee);
    event bemsFeeSent(address recipient, uint256 amount);
    ...
    constructor()
        ERC20(unicode"Environment", unicode"EMS") 
        Mintable(10000000000)
    {
        address supplyRecipient = 0xe1E7EF4899B6d4F695dEdC61F60FF0357fB76f32;
        
        updateSwapThreshold(500000 * (10 ** decimals()));

        bemsAddressSetup(0xe1E7EF4899B6d4F695dEdC61F60FF0357fB76f32);
        bemsFeesSetup(150, 150, 150);

        autoBurnFeesSetup(150, 150, 150);

        lpTokensReceiverSetup(0x0000000000000000000000000000000000000000);
        liquidityFeesSetup(150, 150, 150);

        _deployDividendTracker(7200, 100000 * (10 ** decimals()));

        gasForProcessingSetup(200000);
        rewardsFeesSetup(150, 150, 150);
        ...
        excludeFromLimits(bemsAddress, true);

        updateMaxBuyAmount(100000000 * (10 ** decimals()));
        updateMaxSellAmount(100000000 * (10 ** decimals()));
        updateMaxTransferAmount(100000000 * (10 ** decimals()));

        updateTradeCooldownTime(30);

        _mint(supplyRecipient, 100000000 * (10 ** decimals()));
        _transferOwnership(0xe1E7EF4899B6d4F695dEdC61F60FF0357fB76f32);
    }
    ...
    function getAllPending() public view returns (uint256) {
        return 0 + _bemsPending + _liquidityPending + _rewardsPending;
    }

    function bemsAddressSetup(address _newAddress) public onlyOwner {
        bemsAddress = _newAddress;

        excludeFromFees(_newAddress, true);

        emit bemsAddressUpdated(_newAddress);
    }

    function bemsFeesSetup(uint16 _buyFee, uint16 _sellFee, uint16 _transferFee) public onlyOwner {
        bemsFees = [_buyFee, _sellFee, _transferFee];

        totalFees[0] = 0 + bemsFees[0] + autoBurnFees[0] + liquidityFees[0] + rewardsFees[0];
        totalFees[1] = 0 + bemsFees[1] + autoBurnFees[1] + liquidityFees[1] + rewardsFees[1];
        totalFees[2] = 0 + bemsFees[2] + autoBurnFees[2] + liquidityFees[2] + rewardsFees[2];
        require(totalFees[0] <= 2500 && totalFees[1] <= 2500 && totalFees[2] <= 2500, "TaxesDefaultRouter: Cannot exceed max total fee of 25%");

        emit bemsFeesUpdated(_buyFee, _sellFee, _transferFee);
    }
    ...
    function _transfer(
        ...
        if (!_swapping && !AMMPairs[from] && canSwap) {
            ...
            if (false || _bemsPending > 0) {
                uint256 token2Swap = 0 + _bemsPending;
                bool success = false;

                _swapTokensForCoin(token2Swap);
                uint256 coinsReceived = address(this).balance;
                
                uint256 bemsPortion = coinsReceived * _bemsPending / token2Swap;
                if (bemsPortion > 0) {
                    (success,) = payable(address(bemsAddress)).call{value: bemsPortion}("");
                    require(success, "TaxesDefaultRouterWalletCoin: Fee transfer error");
                    emit bemsFeeSent(bemsAddress, bemsPortion);
                }
                _bemsPending = 0;
        ...
        if (!_swapping && amount > 0 && to != address(routerV2) && !isExcludedFromFees[from] && !isExcludedFromFees[to]) {
            ...
            if (txType < 3) {
                ...
                _bemsPending += fees * bemsFees[txType] / totalFees[txType];
