...
library ConfigHelper {
  ...
  function managedPortfolioAddress(AlloyxConfig config) internal view returns (address) {
    return config.getAddress(uint256(ConfigOptions.Addresses.ManagedPortfolio));
  }

  function poolAddress(AlloyxConfig config) internal view returns (address) {
  ...
  function getManagedPortfolio(AlloyxConfig config) internal view returns (IManagedPortfolio) {
    return IManagedPortfolio(managedPortfolioAddress(config));
  }

  function getPool(AlloyxConfig config) internal view returns (IPool) {
