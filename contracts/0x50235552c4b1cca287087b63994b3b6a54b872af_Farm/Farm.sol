...
// File contracts/interface/IUniPositionManager.sol
...
contract Farm is OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    struct PoolInfo {
        ...
        uint256 _type; // 1 single coin, 2 lp v2, 3 uniswap lp v3
        uint256 _totalStake;
        address _token0;
        address _token1;
    }
    ...
    function pendingToken(
        ...
        if (
            block.timestamp > _pool._lastRewardTimestamp &&
            _pool._totalStake != 0
        ) {
            ...
            _accTokenPerShare += (_tokenReward * 1e12) / _pool._totalStake;
        }

        return
            (_userInfo._amount * _accTokenPerShare) /
            1e12 -
    ...
    function getStakeTokenDecimals(
        PoolInfo memory _pool
    ) public view returns (uint256) {
        if (_pool._type == 3) {
            return 18;
        } else {
            return IERC20MetadataUpgradeable(_pool._stakeToken).decimals();
        }
    }
    ...
    function apr(
        ...
        uint256 _totalStakeToken = _pool._totalStake;
        if (_totalStakeToken == 0) {
            _totalStakeToken = 10 ** getStakeTokenDecimals(_pool);
        }
        ...
        uint256 _totalValueInPool = (_stakeTokenPrice * _totalStakeToken) /
            10 ** getStakeTokenDecimals(_pool);
    ...
    function _harvest(address _to, uint256 _pid) internal {
        ...
        uint256 _totalRewards = (_userInfo._amount * _pool._accTokenPerShare) /
            1e12;
        ...
        require(
            _pending <= rewardToken.balanceOf(address(this)),
            "Farm::_harvest: not enough rewardToken"
        );
        _safeTokenTransfer(_to, _pending);
    ...
    function _safeTokenTransfer(address _to, uint256 _amount) internal {
        ...
        if (_amount > _tokenBal) {
            rewardToken.transfer(_to, _tokenBal);
        } else {
    ...
    function deposit(uint256 _pid, uint256 _amount) external nonReentrant {
        PoolInfo storage _pool = poolInfo[_pid];
        require(_pool._type != 3, "Pool::deposit: not should be v3 pool");
        UserInfo storage _userInfo = userInfo[_pid][msg.sender];
        require(
            _pool._stakeToken != address(0),
            "Pool::deposit:: not accept deposit rewardToken"
        );
        updatePool(_pid);
        if (_userInfo._amount > 0) {
            _harvest(msg.sender, _pid);
        }
        _pool._totalStake += _amount;
        ...
        _userInfo._rewardDebt =
            (_userInfo._amount * _pool._accTokenPerShare) /
            1e12;
        emit Deposit(msg.sender, _pid, _amount);
    }

    function depositV3(uint256 _pid, uint256 _tokenId) external nonReentrant {
        PoolInfo storage _pool = poolInfo[_pid];
        require(_pool._type == 3, "Pool::deposit: should be v3 pool");
        UserInfo storage _userInfo = userInfo[_pid][msg.sender];
        require(
            _pool._stakeToken != address(0),
            "Pool::deposit:: not accept deposit rewardToken"
        );
        require(_userInfo._tokenId == 0, "Pool::deposit:: staked already");
        updatePool(_pid);
        if (_userInfo._amount > 0) {
            _harvest(msg.sender, _pid);
        }
        ...
        uint256 _amount = 0;
        require(
            IERC721MetadataUpgradeable(_pool._stakeToken).ownerOf(_tokenId) ==
                msg.sender,
            "Pool::deposit:: not your token"
        );
        (
            ,
            ,
        ...
        ) = IUniPositionManager(_pool._stakeToken).positions(_tokenId);
        require(_token0 == _pool._token0, "Pool::deposit:: liquidity error");
        require(_token1 == _pool._token1, "Pool::deposit:: liquidity error");
        _amount = _liquidity;

        require(_amount > 0, "Pool::deposit: no liquidity in token id");

        _pool._totalStake += _amount;
        IERC721MetadataUpgradeable(_pool._stakeToken).safeTransferFrom(
            msg.sender,
            address(this),
            _tokenId
        );
        _userInfo._tokenId = _tokenId;
        _userInfo._amount += _amount;
        _userInfo._rewardDebt =
            (_userInfo._amount * _pool._accTokenPerShare) /
            1e12;
    ...
    function withdrawV3(uint256 _pid) external nonReentrant {
        PoolInfo storage _pool = poolInfo[_pid];
        require(_pool._type == 3, "Pool::deposit: should be v3 pool");
        UserInfo storage _userInfo = userInfo[_pid][msg.sender];
        require(
            _userInfo._amount >= 0 && _userInfo._tokenId != 0,
        ...
        if (_pool._stakeToken != address(0)) {
            _pool._totalStake -= _userInfo._amount;
            IERC721MetadataUpgradeable(_pool._stakeToken).safeTransferFrom(
                address(this),
                msg.sender,
                _userInfo._tokenId
            );
            _userInfo._tokenId = 0;
        }

        emit Withdraw(msg.sender, _pid, _userInfo._amount);
    }
    ...
    function harvest(uint256 _pid) external nonReentrant {
        ...
        UserInfo storage _userInfo = userInfo[_pid][msg.sender];
        _userInfo._rewardDebt =
            (_userInfo._amount * poolInfo[_pid]._accTokenPerShare) /
            1e12;
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 _lpSupply = _pool._totalStake;
        if (_lpSupply == 0) {
