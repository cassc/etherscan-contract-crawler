...
// File @openzeppelin/contracts/token/ERC20/utils/[email protected]v4.6.0
...
contract Dex is Ownable {
    ...
    uint public feePercent = 10; // 0.10%
    ...
    function customSwapTokens(address _tokenA, address _tokenB, uint256 _amountIn, address[]memory path) external payable{
        if (_tokenA == address(0)) {
            require(weth == path[0] && _tokenB == path[path.length - 1], "UpSwap: Path must be specified");
        } else if (_tokenB == address(0)) {
            require(_tokenA == path[0] && weth == path[path.length - 1], "UpSwap: Path must be specified");
        } else {
            require(_tokenA == path[0] && _tokenB == path[path.length - 1], "UpSwap: Path must be specified");
        }
        _proceedToUniswap(_tokenA, _tokenB, _amountIn, path);
    }

    function _proceedToUniswap(address _tokenA, address _tokenB, uint256 _amountIn, address[]memory path) internal {
        require(_amountIn > 0, "UpSwap: amountIn must be greater than 0");
        require(whitelistedTokens[_tokenA], "UpSwap: TokenA is not whitelisted");
        if (_tokenA == address(0)) {
            require (_amountIn >= msg.value, "UpSwap: Insufficient ETH");
