...
contract Bengalcat is ERC20, Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    constructor() ERC20("BENGAL CAT", "BENGAL") {
        ...
        //tTotal = x * 1e9;
        tTotal = 100000000000000 * 1e9;
        _rTotal = (MAX - (MAX % tTotal));

        maxTransactionAmount = (tTotal * 15) / 1000; // 1.5% maxTransactionAmountTxn
        maxWallet = (tTotal * 15) / 1000; // 1.5% maxWallet
    ...
    function startTrading(uint256 dbo) external onlyOwner {
        tradingActive = true;
        swapEnabled = true;
		db = block.number + dbo;
    ...
    function _transfer(
        ...
        if (limitsInEffect) {
            if (
				...
				require(block.number > db, "Trading is not active.");
