...
import "./MMSCRewardsMadMonkeySeaClub.sol";
import "./MadMonkeySeaClub.sol";
import '@openzeppelin/contracts/access/Ownable.sol';
import '@openzeppelin/contracts/token/ERC721/IERC721Receiver.sol';

contract NFTStakingMMSC is Ownable, IERC721Receiver {
  ...
  MadMonkeySeaClub nft;
  MMSCRewardsMadMonkeySeaClub token;

  // maps tokenId to stake
  mapping(uint256 => Stake) public vault; 

   constructor(MadMonkeySeaClub _nft, MMSCRewardsMadMonkeySeaClub _token) { 
    nft = _nft;
    token = _token;
  }
 ...
 function getTotalRewardEarned(address user)
        ...
        returns (uint256 rewardEarned)
    {
        ...
        for (uint256 i; i < len; ) {
            uint256 _stakedAt = vault[tokens[i]].timestamp;
            
            uint256 stakingPeriod = block.timestamp - _stakedAt;
            earned += 100000 ether * (block.timestamp - stakingPeriod) / 1 days;
            unchecked {
                ++i;
            }
        }
        rewardEarned = earned / 100000;
