...
contract AlphaiStaking is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;

    struct UserInfo {
        uint256 stakedAmount; // How many ALPHAI tokens the user has provided.
        ...
        updateReward(msg.sender)
    {
        ...
        IERC20(alphaToken).safeTransferFrom(msg.sender, address(this), _amount);
  ...
  **/
        ...
        updateReward(msg.sender)
    {
        ...
        if (isEmergency) {
            ...
            IERC20(alphaToken).safeTransfer(msg.sender, outAmount);
    ...
    function claim() external nonReentrant {
        ...
        require(
            IERC20(alphaToken).balanceOf(address(this)) >= receiveAmount,
            "staking contract has not enough ALPHAI token"
        );

        IERC20(alphaToken).safeTransfer(msg.sender, receiveAmount);
        totalStakedAmount = IERC20(alphaToken).balanceOf(address(this)).sub(
            receiveAmount
        );
