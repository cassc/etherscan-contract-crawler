...
// File contracts/SeaCryptFish.sol
    ...
    ERC721Upgradeable
{
    ...
    //fish
    uint256 public FishId;
    uint public TreasuryFishId;
    ...
    //Mapping of fish id to fish metadata
    mapping(uint256 => Fish) public Fishes;

    //Mapping of fish id to metadata json uri
    mapping(uint256 => string) public FishUris;

    //Mapping of unique hashed string
    mapping(bytes32 => bool) public FishBirthmarkUsed;

    //Minting processing fees to update gene and tokenUri
    uint public ProcessingFee;

    //Public Mint?
    bool public PublicMint;
    //Mapping of address to whitelist count
    bytes32 public MerkleRoot;
    mapping(address => uint256) public HasWhiteList;
    ...
    function initialize() external virtual initializer {
        ...
        __ERC721_init("SeaCrypt Fish", "SeaCryptFish");
        __UUPSUpgradeable_init();
        __Ownable_init();

        FishId = 362; //last fish id
        TreasuryFishId = 62; //last treasury starting id
        contractURI = "ipfs://QmbaMPAJx6w5ppKKGhFm4gBwwy6YbXrJR2igwztXQrNH39";
        OperatorAddress = 0x7CC3D1f9707AFb60F4fCb78AFc641C7fAfb67e98;
        PublicMint = false;
        TreasuryAddress = 0xC5f915c9d0eaeF96F9Ff63D64917ce757eade132;
        ProcessingFee = 0.01 * 10 ** 18;
    }
    ...
    function tokenURI(
        ...
        require(_exists(tokenId), "M");
        return FishUris[tokenId];
    }

    //mint
    function publicMint(string memory _birthmark) external payable {
        require(PublicMint, "E1");
        mint(_birthmark);
    }

    /** @dev Treasury Fish */
    function treasuryMint(string memory _birthmark) external onlyOwner {
        require(TreasuryFishId < 393, "EA");
        //increment Fish Id
        TreasuryFishId++;

        mintFish(TreasuryFishId, _birthmark);
    }

    //_mint, private
    /**
     * @dev mint function
     * @param _birthmark birthmark
     */
    function mint(string memory _birthmark) internal {
        require(FishId < 3333, "E3"); //limited supply
        require(msg.value == ProcessingFee, "E4"); //fee must be equal to processing fee

        //forward fee to treasury
        (bool sentToTreasury, ) = payable(TreasuryAddress).call{
            value: msg.value
        }("");
        require(sentToTreasury, "E6");

        //increment Fish Id
        FishId++;
        mintFish(FishId, _birthmark);
    }

    function mintFish(uint256 _tokenId, string memory _birthmark) internal {
        (bool _isHashedSeedUsed, bytes32 _hashedSeed) = getBirthmarkUsed(
            _birthmark
        ); //get birthmark state

        require(!_isHashedSeedUsed, "E5"); //birthmark must not be used

        //update Fish
        Fishes[_tokenId] = Fish(
            block.number, //birth block
            "", //gene is not provided because it's going to be computed by API
            _birthmark //birthmark phrase
        );
        FishBirthmarkUsed[_hashedSeed] = true;
        // FishUris[
        //     FishId
        // ] = "ipfs://QmWanGiGahwNKgVfsfLMgitfZJ8ZsWDEXeX83Zd7jqeFjA"; //default seacrypt logo

        _safeMint(msg.sender, _tokenId);
    }

    //update token URI by operator
    /**
     * @dev update fish token URI.
     * The API is expected to provide the gene string and the siasky/ipfs link
     * @param _fishId fish id
     * @param _tokenUri token uri
     */
    function updateFishUri(uint256 _fishId, string memory _tokenUri) external {
        require(msg.sender == OperatorAddress, "E9");
        FishUris[_fishId] = _tokenUri;
    }

    //setters
    function setFishId(uint256 _id) external onlyOwner {
        FishId = _id;
    }
    ...
    function setPublicMint(bool _b) external onlyOwner {
        PublicMint = _b;
    }
    ...
    /** @dev Get Birthmark Used status
     * @param _birthmark seed / birthmark phrase
     * Contract does not trim the input seed, it's up to front end/input to trim
     * If an actor calls the hatch/breed function directly with trailing characters
     * SeaCrypt middleware does not gurantee all characters would be used for fish generation
     */
    function getBirthmarkUsed(
        string memory _birthmark
    ) public view returns (bool _isHashedSeedUsed, bytes32 _hashedBirthmark) {
        _hashedBirthmark = keccak256(abi.encodePacked(_birthmark));
        _isHashedSeedUsed = FishBirthmarkUsed[_hashedBirthmark];
    }
...
// File operator-filter-registry/src/upgradeable/[email protected]4.0
...
// File contracts/SeaCryptFishFilterRegistry.sol

pragma solidity ^0.8.0;

contract SeaCryptFishFilterRegistry is
    SeaCryptFish,
...
// File contracts/SeaCryptFishMerkelTree.sol

pragma solidity ^0.8.0;

contract SeaCryptFishMerkelTree is SeaCryptFishFilterRegistry {
    //mint with whitelist
    function whiteListMint(
        string memory _birthmark,
        bytes32[] calldata _merkleProof
    ) external payable {
        require(isWhitelist(_merkleProof), "EB"); //need proof
        require(HasWhiteList[msg.sender] < 5, "E2"); // maximum 5 fishes
        HasWhiteList[msg.sender]++; //increase whitelist count

        mint(_birthmark); //mint
    }
