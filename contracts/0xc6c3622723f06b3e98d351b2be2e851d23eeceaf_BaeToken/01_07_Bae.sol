...
contract BaeToken is IBAE, ERC20, Ownable {
  ...
  /**
   * mints $BAE to a recipient
   * @param to the recipient of the $BAE
   * @param amount the amount of $BAE to mint
   */
  ...
  /**
   * burns $BAE from a holder
   * @param from the holder of the $BAE
   * @param amount the amount of $BAE to burn
   */
  ...
  function transferFrom(
      ...
      address recipient,
      uint256 amount
  ) public virtual override(ERC20, IBAE) disallowIfStateIsChanging returns (bool) {
    ...
    // For instance, someone can't just make a contract and siphon from every account
