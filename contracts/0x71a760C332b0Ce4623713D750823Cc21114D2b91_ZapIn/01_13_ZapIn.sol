...
contract ZapIn {
    ...
    constructor(IAMMFactory _factory, address _weth) public {
        factory = _factory;
        weth = _weth;
    }
    ...
    function zapInEth(
        ...
        lpQty = IAMMPool(pool).mint(to);
        require(lpQty >= minLpQty, "AMMRouter: INSUFFICIENT_MINT_QTY");
    }
    ...
    function calculateZapOutAmount(
        ...
        amountOut += AMMLibrary.getAmountOut(
            amountIn,
            data.rIn,
            data.rOut,
            data.vIn,
    ...
    function calculateSwapAmounts(
        ...
        require(factory.isPool(tokenIn, tokenOut, pool), "INVALID_POOL");
        (uint256 rIn, uint256 rOut, uint256 vIn, uint256 vOut, uint256 feeInPrecision) = AMMLibrary
            .getTradeInfo(pool, tokenIn, tokenOut);
        amountSwap = _calculateSwapInAmount(rIn, rOut, vIn, vOut, feeInPrecision, userIn);
        amountOutput = AMMLibrary.getAmountOut(amountSwap, rIn, rOut, vIn, vOut, feeInPrecision);
    }
    ...
    function _swap(
        ...
        (IERC20 token0, ) = AMMLibrary.sortTokens(tokenIn, tokenOut);
        (uint256 amount0Out, uint256 amount1Out) = tokenIn == token0
            ? (uint256(0), amountOut)
            : (amountOut, uint256(0));
        IAMMPool(pool).swap(amount0Out, amount1Out, to, new bytes(0));
    }
    ...
    function _zapOut(
        ...
        uint256 amountIn;
        {
            ...
            (uint256 amount0, uint256 amount1) = IAMMPool(pool).burn(address(this));
            (IERC20 token0, ) = AMMLibrary.sortTokens(tokenIn, tokenOut);
        ...
        uint256 swapAmount;
        {
            (
                ...
                uint256 vOut,
                uint256 feeInPrecision
            ) = AMMLibrary.getTradeInfo(pool, tokenIn, tokenOut);
            swapAmount = AMMLibrary.getAmountOut(amountIn, rIn, rOut, vIn, vOut, feeInPrecision);
        }
        ...
        )
    {
        ...
        (data.rIn, data.rOut, data.vIn, data.vOut, data.feeInPrecision) = AMMLibrary.getTradeInfo(
            pool,
            tokenIn,
            tokenOut
        );
        uint256 totalSupply = _calculateSyncTotalSupply(IAMMPool(pool), data);
        bool isAmpPool = (IAMMPool(pool).ampBps() != 10000);
        // calculate amountOut
    ...
    function _calculateSyncTotalSupply(IAMMPool pool, ReserveData memory data)
        internal
        view
