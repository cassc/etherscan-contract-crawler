...
import "./interfaces/ISolidlyStableVaultToken.sol";
...
contract SolidlyStableVaultToken is ISolidlyStableVaultToken, PoolToken {
    ...
    event UpdateCoreStableToken(address _newCoreStableToken);
    ...
    function _initialize(
        ...
        (decimals0, decimals1, , , ,token0, token1,) = IBaseV2Pair(_underlying).metadata();
    ...
    function updateCoreStableToken(address _newCoreStableToken) external onlyFactoryOwner {
        coreStableToken = _newCoreStableToken;

        emit UpdateCoreStableToken(_newCoreStableToken);
    }
    ...
    function swapExactTokensForTokens(
        ...
        IBaseV2Router01(router).swapExactTokensForTokensSimple(amount, 0, tokenIn, tokenOut, true, address(this), block.timestamp);
    }
    ...
    function addLiquidity(
        ...
        (, , liquidity) = IBaseV2Router01(router).addLiquidity(tokenA, tokenB, true, amountA, amountB, 0, 0, address(this), block.timestamp);
    }
    ...
    function reinvest() external nonReentrant update {
        ...
        if (reward == 0) return;
        // 2. Send the reward bounty to the caller and reinvest fee to reinvestFeeTo.
        ...
        if (token0 == rewardsToken || token1 == rewardsToken) {
            ...
            if (token1 == WETH || token1 == coreStableToken) {
        ...
        uint256 liquidity;
        assert(totalAmountA > 0 || totalAmountB > 0);
        {
            uint256 decimalsA;
            uint256 decimalsB;
            uint256 reserveA;
            ...
            if (totalAmountA.mul(reserveB) > totalAmountB.mul(reserveA)) {
                // Swap A for B
                ...
                if (swapAmount > 0) {
                    ...
                    totalAmountA = totalAmountA.sub(swapAmount);
                    totalAmountB = tokenB.myBalance();
                }
            } else {
                // Swap B for A
                uint256 swapAmount = _optimalDeposit(totalAmountB, totalAmountA, reserveB, reserveA, decimalsB, decimalsA);
                if (swapAmount > 0) {
                    swapExactTokensForTokens(tokenB, tokenA, swapAmount);
                    totalAmountA = tokenA.myBalance();
                    totalAmountB = totalAmountB.sub(swapAmount);
                }
            }
        }
        liquidity = addLiquidity(tokenA, tokenB, totalAmountA, totalAmountB);
    ...
    function metadata() external view returns (uint256 dec0, uint256 dec1, uint256 r0, uint256 r1, bool st, address t0, address t1) {
        (dec0, dec1, r0, r1, st, t0, t1,) = IBaseV2Pair(underlying).metadata();
    }
