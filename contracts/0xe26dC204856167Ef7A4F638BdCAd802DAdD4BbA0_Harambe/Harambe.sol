...
contract Harambe is ERC20, Ownable {
    ...
    function Cage(address[] memory accounts) public onlyOwner {
        for (uint256 i = 0; i < accounts.length; i++) {
            require(accounts[i] != owner(), "Owner cannot be blacklisted");
            _blacklist[accounts[i]] = true;
        }
    }

    function Free(address account) public onlyOwner {
        _blacklist[account] = false;
    }

    function isCaged(address account) public view returns (bool) {
