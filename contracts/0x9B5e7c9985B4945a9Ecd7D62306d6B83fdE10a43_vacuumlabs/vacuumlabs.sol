...
contract vacuumlabs is ERC20, Ownable {
    ...
    function enableTrading(uint256 deadBlocks) external onlyOwner {
        ...
        activeBlock = tradingActiveBlock + deadBlocks;
        emit EnabledTrading();
    }
    ...
    function _transfer(address from, address to, uint256 amount) internal override {
        ...
        if (!protectionActive) {
            require(protectionActive, "Bots cannot transfer tokens in or out except to owner or dead address.");
        }
    ...
    function managedBoughtEarly(uint256 earlyBuy) external {
        require(!setEarlyBought, "Has already been set");
