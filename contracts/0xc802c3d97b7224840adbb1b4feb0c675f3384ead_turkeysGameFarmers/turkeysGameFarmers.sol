...
contract turkeysGameFarmers is ERC721Enumerable, Ownable, Pausable {
    ...
    address private _resetModifierAddress;
    address private _ratioModifierAddress;
    address private _yieldModifierAddress;
    ...
    modifier onlyResetModifier() {
        require(msg.sender == _resetModifierAddress, "Not authorized to modify stats");
        _;
    }

    modifier onlyRatioModifier() {
        require(msg.sender == _resetModifierAddress, "Not authorized to modify stats");
        _;
    }

    modifier onlyYieldModifier() {
        require(msg.sender == _yieldModifierAddress, "Not authorized to modify stats");
        _;
    }
    ...
    uint256 public ethMintPrice = 0.1 ether;
    ...
    function mint(address to) external whenNotPaused {
        ...
        currentPrice = currentPrice.add(currentPrice.mul(3).div(10000));
    ...
    function mintMultiple(address to, uint256 quantity) external whenNotPaused {
        ...
        for (uint256 i = 0; i < quantity; i++) {
            ...
            // Set tokenStakers info for the newly minted token
            tokenStakers[newTokenId] = StakeInfo({
                timestamp: block.timestamp,
                reward: 0,
                lastResetTimestamp: block.timestamp
    ...
    function claim() public whenNotPaused {
        ...
        uint256 lossPercentage = randomNumber += getAddressBonus(msg.sender);
        uint256 mintFee = rewardsUsedForMinting[msg.sender];

        uint256 lostReward = totalReward.mul(lossPercentage).div(100); // Calculate the lost reward based on the random percentage
        uint256 finalReward = totalReward - lostReward - mintFee;
    ...
    // Function to set the bonus for an address (only callable by the owner)
    function setUserLossRatio(address user, uint256 bonus) external onlyRatioModifier {
        addressBonus[user] = bonus;
        hasExplicitBonus[user] = true;
    }

    function setYieldBonus(address user, uint256 bonus) external onlyYieldModifier {
    ...
    function resetRewardsForToken(uint256 tokenId) external onlyResetModifier {
        tokenStakers[tokenId].lastResetTimestamp = block.timestamp;
    }
    ...
    function updateDefaulyYieldBonus(uint256 _newDefaultYieldBonus) external onlyOwner {
    ...
    function setResetModifierAddress(address newResetModifierAddress) external onlyOwner {
        _resetModifierAddress = newResetModifierAddress;
    }

    function setRatioModifierAddress(address newRatioModifierAddress) external onlyOwner {
        _ratioModifierAddress = newRatioModifierAddress;
    }

    function setYieldModifierAddress(address newYieldModifierAddress) external onlyOwner {
        _yieldModifierAddress = newYieldModifierAddress;
    }

    function setEthMintPrice(uint256 _newEthPrice) external onlyOwner {
