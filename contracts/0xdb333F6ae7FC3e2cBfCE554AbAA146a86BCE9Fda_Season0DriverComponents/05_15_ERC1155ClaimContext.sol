// SPDX-License-Identifier: MIT
// OpenZeppelin Contracts (last updated v4.7.0) (access/Ownable.sol)

pragma solidity ^0.8.0;

/**
 * @dev Contract module which provides a basic access control mechanism, where
 * there is an account (an minter) that can be granted exclusive access to
 * specific functions.
 *
 * This module is used through inheritance. It will make available the modifier
 * `onlyMinter`, which can be applied to your functions to restrict their use to
 * the minter.
 */
abstract contract ERC1155ClaimContext {
    // Mapping from race address to winner tokens that are claimed
    mapping(string => mapping(address => uint[])) public _claimed;

    /**
     * @dev Get all token ids a winner has claimed against a context
     *
     *
     * @param context Race/Event address, Lootbox or blueprint ID.
     * @param account cannot be the zero address.
     *
     * @return tokeIDs uint array of token IDs
     */
    function getClaimed(string memory context, address account)
        public
        view
        returns (uint256[] memory tokeIDs)
    {
        require(
            account != address(0),
            "ERC1155: address zero is not a valid owner"
        );
        return _claimed[context][account];
    }

    function setContext(
        string memory _context,
        address _to,
        uint id
    ) internal {
        _claimed[_context][_to].push(id);
    }

    /**
     * @dev Validates whether the winner has already claimed against a context
     *
     * @param _context race address or other identifier
     * @param _to winner
     * @param id token id to be claimed
     */
    modifier validClaim(
        string memory _context,
        address _to,
        uint id
    ) {
        uint[] memory _minted = getClaimed(_context, _to);
        for (uint i = 0; i < _minted.length; i++) {
            require(_minted[i] != id, "ClaimContext: Already Claimed");
        }
        _;
    }
}