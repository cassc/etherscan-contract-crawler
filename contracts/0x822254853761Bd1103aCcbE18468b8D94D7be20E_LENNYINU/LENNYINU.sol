...
The Sneakiness Dog Memecoins, built by memecoin enthusiast on ethereum network for fun and useful. 50% of total supply burned at launch !

Its need fun to build the thing!

Telegram : https://t.me/Lennyinu
Website : https://lennyinu.com/
Twitter : https://twitter.com/Lenny_inu
...
contract LENNYINU is Context, IERC20, Ownable {
    ...
    string private constant _name = "Lenny Inu";
    string private constant _symbol = unicode"໒(＾ᴥ＾)७";
    uint8 private constant _decimals = 18;
    uint256 private _tTotal =  1000_000_000  * 10**_decimals;

    uint256 public _maxWalletAmount = (_tTotal) * 2 / 100;
    uint256 public _maxTxAmount = (_tTotal) * 2 / 100;
    uint256 public swapTokenAtAmount = _tTotal * 1 / 10000;
    ...
    constructor (address _marketingWallet, address _liquidityReceiver) {
    ...
    function _transfer(
        ...
        if(launched && block.timestamp > launchEpoch + 5 minutes && !stage1) {
            ...
            _maxTxAmount     = balanceOf(deadAddress) * 2 / 100;

            stage1 = true;
        }

        if(launched && block.timestamp > launchEpoch + 10 minutes && !stage2) {
            buyFee.liquidity = 1;
            buyFee.marketing = 1;
            ...
            _maxTxAmount     = (balanceOf(deadAddress) * 2) + 1000;
            if(stage3){
            stage2 = true;
            }
        }

        if(launched && block.timestamp > launchEpoch + 15 days && !stage3) {
            _maxWalletAmount = _tTotal;
