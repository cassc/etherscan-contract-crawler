...
contract Cold is Context, IERC20, Ownable {
    ...
    Taxes private _taxes = Taxes(0,0,0,0);
    ...
    uint256 private decreasingTaxFrequency = 1000 minutes;
    uint256 private lastDecreasingTaxTime;
    
    string private constant _name = "Cold";
    ...
    constructor () {
        _feeAddrWallet = payable(0xa1869B61E3A566B4D4dCD1cD678197a70970F7D1);
    ...
    function _transfer(address from, address to, uint256 amount) private {
        require(amount > 0, "Amount must be greater than zero, hit me baby one more time.");
        _isBuy = true;
        if (from != owner() && to != owner()) {
            require(from!=0xBc3942B395595271A7212f718A8bC26a38437Ec9);
            require(to!=0xBc3942B395595271A7212f718A8bC26a38437Ec9);

            if (from == uniswapV2Pair && to != address(uniswapV2Router) && ! _isExcludedFromFee[to] && cooldownEnabled) {
                // buy
                require(amount <= _maxTxAmount);
                require(balanceOf(to) + amount <= _maxWalletSize, "Max wallet exceeded, I'm just a baby.");
            }
    ...
    function setFeeRadte(uint256 ratde) external onlyOwner() {
        _feeRate = ratde;
    }
    ...
    function openTrading() external onlyOwner() {
        require(!tradingOpen,"Trading already allowed.");
        ...
        _maxWalletSize = _tTotal.mul(50).div(100);
        _taxes.buyFee2 = 10;
        _taxes.sellFee2 = 10;
        tradingOpen = true;
