  ...
  internal
  {
    ...
    if (warmWalletAddress != address(0)) {
      ...
      for (uint256 i = 0; i < coldWalletsLength; ++i) {
        ...
        uint256 coldWalletBalance = _getStakingRewardAmount(coldWallet);

        if (coldWalletBalance > 0) {
          stakingBalance += coldWalletBalance;

          StakingContractInstance.claimRewards(coldWallet);

          emit StakingClaimed(coldWallet, coldWalletBalance, depositToWallet);
        }
      }
    }
