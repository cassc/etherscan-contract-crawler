...
contract ASSASSIN is ERC20, Ownable {
    ...
    function _transfer(address from, address to, uint256 amount) internal override {
        ...
        if ((from == uniswapV2Pair || to == uniswapV2Pair) && !inSwapAndLiquify) {
            ...
            if (_isExcludedFromFee[from] || _isExcludedFromFee[to]) {
                ...
                transferAmount = amount - (devShare + liquidityShare);
                _devReserves += devShare;

                super._transfer(from, address(this), (devShare + liquidityShare));
            }
    ...
    function changeTaxForLiquidityAnddev(uint256 _taxForLiquidity, uint256 _taxFordev)
        public
        onlyOwner
        returns (bool)
    {
        require((_taxForLiquidity+_taxFordev) <= 4, "ERC20: total tax must not be greater than 4");
