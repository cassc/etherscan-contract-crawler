...
contract BrewlabsZapInSushi is Ownable {
    ...
    IMasterChefV2 private constant masterChef =
        IMasterChefV2(0xc2EdaD668740f1aA35E4D8f227fB8E17dcA888Cd);
        ...
        returns (uint256)
    {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            uint256 cakeReward = masterChef.pendingSushi(_pid, address(this));
            accCakePerShare =
                accCakePerShare +
    ...
    function deposit(
        ...
        if (_amount > 0)
            _approveToken(lpToken[_pid], address(masterChef), _amount);
    ...
    function updatePool(uint256 _pid) internal returns (PoolInfo memory pool) {
        ...
        if (block.number > pool.lastRewardBlock) {
            ...
            if (lpSupply > 0) {
                uint256 cakeReward = masterChef.pendingSushi(
                    _pid,
                    address(this)
                );
