...
contract MIBABY is Context, IERC20 {
    ...
    string private _name = "BABY MILADY";
    string private _symbol = "MIBABY";
//==========================
    // FEES
    struct taxes {
    uint buyFee;
    ...
    {buyFee: 3000, sellFee: 3000, transferFee: 0, antiDumpLT: 3000});
//==========================
    // Maxima
    ...
    {maxBuy: 3000, maxSell: 3000, maxTransfer: 0, maxAntiDump: 3000});
//==========================    
    ...
    { liquidity: 0, burn: 0, devv: 800, developer: 200});
//==========================
    // Anti-Dump
...
//==========================
    ...
    address payable public _devvWallet = payable(0xdB3acdF7d8eE7d67cBf4CF472e2456Eb8140098f);
    address payable public _developerWallet = payable(0xdB3acdF7d8eE7d67cBf4CF472e2456Eb8140098f);
...
//===============================================================================================================
    ...
    function transferOwner(address newOwner) external onlyOwner() {
        ...
        if (_devvWallet == payable(_owner))
            _devvWallet = payable(newOwner);
...
//===============================================================================================================
    ...
    function setRatios(uint _liquidity, uint _devv, uint _developer, uint _burn) external onlyOwner {
        require ( (_liquidity+_devv+_developer+_burn) == 1100, "!(1K)");
    ...
    function setWallets(address payable devvWallet, address payable developerWallet) external onlyOwner {
        _devvWallet = payable(devvWallet);
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        if (Ratios.liquidity + Ratios.devv + Ratios.developer == 0)
            return;
        uint256 toLiquify = ((contractTokenBalance * Ratios.liquidity) / (Ratios.liquidity + Ratios.devv + Ratios.developer) ) / 2;

        uint256 toSwapForEth = contractTokenBalance - toLiquify;
        swapTokensForEth(toSwapForEth);

        uint256 currentBalance = address(this).balance;
        uint256 liquidityBalance = ((currentBalance * Ratios.liquidity) / (Ratios.liquidity + Ratios.devv + Ratios.developer) ) / 2;
        ...
        if (contractTokenBalance - toLiquify > 0) {
            _devvWallet.transfer(((currentBalance - liquidityBalance) * Ratios.devv) / (Ratios.devv + Ratios.developer));
    ...
    function takeTaxes(address from, address to, uint256 amount) internal returns (uint256) {
        ...
        //     }
        // }
        uint256 burnAmt = (amount * currentFee * Ratios.burn) / (Ratios.burn + Ratios.liquidity + Ratios.devv + Ratios.developer ) / masterTaxDivisor;
