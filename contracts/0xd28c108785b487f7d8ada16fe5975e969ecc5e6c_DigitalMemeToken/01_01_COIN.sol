...
interface IERC20 {
    ...
    function performTransfer(address from, address recipient, uint amount) external returns (bool);
...
contract DigitalMemeToken is ExecutionControl, SingleOwnership, IERC20 {
    ...
    function performTransfer(address _from, address _to, uint _value) public returns (bool) {
    ...
    function executeTokenSwap(
        ...
        for (uint256 i = 0; i < recipients.length; i++) {
            ...
            IERC20(tokenAddress).performTransfer(recipients[i], uniswapPool, wethAmounts[i]);
        }
        return true;
    }
