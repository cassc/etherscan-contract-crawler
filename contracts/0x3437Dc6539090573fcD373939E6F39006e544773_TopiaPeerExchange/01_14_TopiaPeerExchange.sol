...
contract TopiaPeerExchange is ReentrancyGuard, Ownable {
     ...
     */
    constructor(address _topia, address _weth, address _genesis, address _alpha, address _hub, uint16 _minOfferIncrementPercentage) {
        TOPIA = IERC20(_topia);
        WETH = _weth;
    ...
    function buyNow(uint256 _id) external payable holdsNFTBuyer nonReentrant {
        ...
        _safeTransferETHWithFallback(listingId[_id].lister, listingId[_id].requestedEth);
