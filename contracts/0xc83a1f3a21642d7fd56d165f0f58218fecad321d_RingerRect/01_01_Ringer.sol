 ...
 * - The maximum token ID cannot exceed 2**256 - 1 (max value of uint256).
 */
contract RingerRect is IERC721A {
        ...
        if (msg.value == 0) {
            ...
            if (totalSupply() > maxSupply / 5) {
                require(_userMinted[block.number] < FreeNum() 
                    && _userForFree[tx.origin] < freePertx );
   ...
   function tokenURI(uint256 tokenId) public view override returns (string memory) {
        return string(abi.encodePacked("ipfs://QmNNR2Euenx2knsBLDoaT1YcZsDcKSqazBgqqUBnyWpsoB/", _toString(tokenId), ".json"));
    }
