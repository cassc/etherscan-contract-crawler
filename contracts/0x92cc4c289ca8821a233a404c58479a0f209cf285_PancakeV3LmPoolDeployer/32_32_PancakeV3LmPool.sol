...
contract PancakeV3LmPool is IPancakeV3LmPool {
    ...
    // Read old negativeRewardGrowthInsideInitValue data from oldLMPool.
    mapping(int24 => mapping(int24 => bool)) public checkOldLMPool;
    ...
    constructor() {
        (address poolAddress, address masterChefAddress, address oldLMPoolAddress) = IPancakeV3LmPoolDeveloper(
            msg.sender
        ).parameters();
    ...
    function _getNegativeRewardGrowthInsideInitValue(
        ...
        // If already chekced old LMPool , use current negativeRewardGrowthInsideInitValue.
        if (checkOldLMPool[tickLower][tickUpper]) {
            initValue = negativeRewardGrowthInsideInitValue[tickLower][tickUpper];
        } else {
            initValue = oldLMPool.negativeRewardGrowthInsideInitValue(tickLower, tickUpper);
        }
    }
    ...
    function _checkNegativeRewardGrowthInside(int24 tickLower, int24 tickUpper) internal {
        ...
        // Only need to check old LMPool once , and initialize negativeRewardGrowthInsideInitValue.
        if (!checkOldLMPool[tickLower][tickUpper]) {
            checkOldLMPool[tickLower][tickUpper] = true;
    ...
    function _clearNegativeTickInfo(int24 tickLower, int24 tickUpper) internal {
        ...
        // No need to check oldLMPool after tick flipped.
        if (!checkOldLMPool[tickLower][tickUpper]) checkOldLMPool[tickLower][tickUpper] = true;
    }
