...
contract BattleZoneHelper {
     ...
     * @notice Return all migrated bots that are staked
     */
    function migratedStakedBotsOfOwner(
        uint256[] memory botIds,
        address user
    ) public view returns (bool[] memory) {
        unchecked {
    ...
    /**
     * @notice Check which inscriptions have been claimed
     */
    function inscriptionsClaimed(
        uint256[] memory inscriptionIds
    ) public view returns (bool[] memory) {
        bool[] memory claimed = new bool[](inscriptionIds.length);
        for (uint256 b; b < inscriptionIds.length; ++b) {
            claimed[b] = _battleZone.inscriptionClaimed(inscriptionIds[b]);
        }
        return claimed;
    }
}