    ...
    Drop
{
    ...
    uint128 public constant MIN_CLAIM_QUANTITY = 5;
    uint128 public maxSupply;
    mapping(address => bool) public claimedFreeTokens;

    uint128 freeTokenConditionId = 0;
    ...
    function _beforeClaim(
        ...
        // Check if it's the mint pass pre-sale and if so, apply the discount
        if ((activeConditionId == freeTokenConditionId) && !claimedFreeTokens[_dropMsgSender()]) {
            mintPassFreeTokenCount = 2;
            claimedFreeTokens[_dropMsgSender()] = true;
        }
    ...
    function _transferToken(
        ...
        (bool success, ) = saleRecipient.call{value: _totalValueToTransfer}("");
    ...
    function setFreeTokenConditionId(uint128 _freeTokenConditionId) external {
        require(hasRole(ADMIN_ROLE, msg.sender), "Caller is not an admin");
        freeTokenConditionId = _freeTokenConditionId;
    }
}