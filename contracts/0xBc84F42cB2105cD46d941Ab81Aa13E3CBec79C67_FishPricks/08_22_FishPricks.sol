    ...
    DefaultOperatorFilterer
{
    ...
    uint256 public publicSaleCost = 0.028 ether;
    uint256 public preSaleCost = 0.014 ether;
    ...
    function preSale(uint256 quantity) public payable {
        ...
        if (msg.sender != owner()) {
            require(preSale_mint_status, "Presale mint is off");
            require(
                balanceOf(msg.sender) + quantity <= max_per_wallet,
                "Per wallet limit reached"
            );
            require(
                preSaleCount + quantity <= preSaleLimit,
                "Per wallet limit reached"
            );
            require(            
                msg.value >= (preSaleCost * quantity),
                "Not enough ether sent"
            );
        ...
        preSaleCount = preSaleCount + quantity;
    }
    ...
    function toggle_preSale_mint_status() public onlyOwner {
    ...
    function setPreSaleCount(uint256 _preSaleCount) public onlyOwner {
