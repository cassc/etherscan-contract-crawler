...
interface IShotgunClause {
	...
	function getCounterclaimantContribution(
		address counterclaimant
	...
	function getRequiredWeiForCounterclaim() external view returns (uint);
}
...
contract ShardRegistry is ERC20Detailed, ERC20Capped, ERC20Burnable, ERC20Pausable {
	...
	constructor (
		...
		address shardGovernorAddress
	) ERC20Detailed(name, symbol, 18) ERC20Capped(cap) public {
		_shardGovernor = IShardGovernor(shardGovernorAddress);
	...
	function burnAndCollectEther(address shotgunClause) external {
		IShotgunClause _shotgunClause = IShotgunClause(shotgunClause);
