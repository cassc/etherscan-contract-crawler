...
contract WPDC is ERC721A, Ownable {
    using SafeMath for uint256;
    using Address for address;
     ...
     */
    event NFTMintEvent(uint256 token, address to, uint256 quantity, uint256 price);
    ...
    event TokenURIEvent(uint256 tokenId, string _baseURI);

    event CommonTokenURIEvent(string _commonTokenURI);

    event PriceEvent(uint256 _price);
    ...
    function setTokenURI(uint256 tokenId, string memory _tokenuri) public onlyExecutor {
        require(bytes(_tokenuri).length > 0, "URI length not null");
        _tokenURI[tokenId] = _tokenuri;
        emit TokenURIEvent(tokenId, _tokenuri);
    }
    ...
    function setPrice(uint256 _price) public onlyExecutor {
        price = _price;
        emit PriceEvent(_price);
    }
    ...
    function executorMint(address to, uint256 quantity) public onlyExecutor nftIsOpen {
        ...
        _mint(to, quantity);
        emit NFTMintEvent(tokenId, to, quantity, 0);
    }

    function mint(address to) public nftIsOpen payable {
        require(msg.sender == tx.origin, "only origin");
        require(to == msg.sender, "The address of to cannot be the address of the caller");
        ...
        require(msg.value >= price, "The price error");
        ...
        mintedPool[to] = true;
        _mint(to, quantity);
        emit NFTMintEvent(tokenId, to, quantity, price);
    }
