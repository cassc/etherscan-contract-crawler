...
// ----------------------------------------------------------------------------
// CryptoCatsHelper v0.9.2
//
// https://github.com/bokkypoobah/TokenToolz
//
// Deployed to Mainnet 0x373F3AaADb81CE97D1E27c3AAb9d378f48284a4D
//
...
contract CryptoCatsHelper {
    function getCatData(CryptoCatsMarkets cryptoCatsMarket, uint[] memory catIds) public view returns (
        ...
        for (uint i = 0; i < length;) {
            owners[i] = cryptoCatsMarket.catIndexToAddress(catIds[i]);
            for (uint j = 0; j < 6; j++) {
                attributes[i][j] = cryptoCatsMarket.catAttributes(catIds[i], j);
            }
            offers[i] = cryptoCatsMarket.catsForSale(catIds[i]);
            unchecked {
                i++;
            }
        }
