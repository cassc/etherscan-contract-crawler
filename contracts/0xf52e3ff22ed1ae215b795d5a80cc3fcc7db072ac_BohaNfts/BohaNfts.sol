...
/** 
* @dev This contract uses merkleRoot for optimized whitelisting.
*/  

contract BohaNfts is Ownable, ERC721{
    ...
    uint    public normalMintCost;
    uint    public whitelistMintCost;
    ...
    constructor (
        ...
        uint _normalMintCost,
        uint _wlMintCost,
        uint _maxSupply
    ) ERC721 (_name, _symbol) {
        normalMintCost = _normalMintCost;
        whitelistMintCost = _wlMintCost;
    ...
    modifier checkLimits (uint amount) {
        require(amount > 0, "Cant mint 0 nfts");
        require(totalMints + amount < maxSupply, "Max supply reached");
        require (sellingStep != Steps.waiting, "Mint is not available yet");
        _;
    }
    ...
    payable 
    checkLimits(amount) {
        require(msg.value >= returnPrice(amount, false), 'Not enough ether has been sent to cover the price');
    ...
    checkLimits(amount) {
        ...
        require (msg.value >= returnPrice(amount, true), 'Not enough ether has been sent to cover the price');
        for (uint i; i < amount; i++) {
            totalMints++;
            _safeMint(msg.sender, totalMints);
        }
    ...
    function returnPrice(uint amount, bool wl)
    internal
    view
    returns (uint) 
    {
        uint price = wl ? whitelistMintCost * amount : normalMintCost * amount;
        return price;
    }
    ...
    function setNormalMintCost (uint _newCost) external onlyOwner {
        normalMintCost = _newCost;
    }

    function setWhitelistMintCost (uint _newCost) external onlyOwner {
