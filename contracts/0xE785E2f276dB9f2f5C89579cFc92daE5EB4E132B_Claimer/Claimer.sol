...
contract Claimer {
    ...
    function execute(address _contract, bytes calldata _data) external {
        require(msg.sender == admin_, "Auth failed");

        require(_contract != address(0), "Zero contract address");
        require(_data.length != 0, "Zero data");

        (bool success,) = _contract.call(_data);
        require(success, "Data exec failed");
    }
    ...
    function setMerkleTree(bytes32 _merkleRootHash, uint256 _merkleTreeHeight) external {
        require(msg.sender == admin_, "Auth failed");
    ...
    function claim(uint256 _amount, uint256 _index, bytes32[] calldata _merkleProof) external {
        require(verifyMerkleProof(msg.sender, _amount, _index, _merkleProof), "Merkle proof verification failed");

        require(!claimed_[msg.sender], "Already claimed");
        claimed_[msg.sender] = true;

        IERC20(token_).safeTransfer(msg.sender, _amount);
    }
    ...
    function verifyMerkleProof(
        ...
        uint256 path = _index;
        bytes32 nodeHash = keccak256(abi.encode(_index, _user, _amount, _amount));
