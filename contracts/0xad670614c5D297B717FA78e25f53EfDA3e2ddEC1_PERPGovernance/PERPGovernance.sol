...
contract PERPGovernance is Ownable {
    struct Proposal {
        string description;
        uint256 yesVotes;
        uint256 noVotes;
    ...
    constructor(IERC20 token, uint256 _minimumProposalTokens, uint256 _votingPeriod) {
        _token = token;
        minimumProposalTokens = _minimumProposalTokens;
        votingPeriod = _votingPeriod;
    }

    function createProposal(string memory description) external {
        require(_token.balanceOf(msg.sender) >= minimumProposalTokens, "Insufficient tokens to create proposal");
    ...
    function vote2(uint256 proposalId, bool vote) external {
        ...
        if (vote) {
            currentProposal.yesVotes += tokenBalance;
        } else {
            currentProposal.noVotes += tokenBalance;
        }
