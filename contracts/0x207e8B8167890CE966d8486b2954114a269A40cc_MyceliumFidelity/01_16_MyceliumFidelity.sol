...
contract MyceliumFidelity is ERC721, ERC721Enumerable, ERC721URIStorage, ERC721Burnable, Ownable
{
    ...
    //keep track of what part of 
    //friends and family mint we are in...
    enum FriendsState {Init, FirstDone, SecondDone, Done}
    FriendsState public friendsState = FriendsState.Init;
	...
	uint256 toMint = 0;
	if(friendsState == FriendsState.Init)
	{
            toMint = 75;
	}
	else if(friendsState == FriendsState.FirstDone)
	{
	    toMint = 100;
	}
	else if(friendsState == FriendsState.SecondDone)
	{
	    toMint = 100;
	}
	else
	...
	require(totalSupply().add(toMint) <= MAX_MYFI_PIECES, 'not enougn MyFi pieces available for connection');
	...
	if(friendsState == FriendsState.Init)
	{
            friendsState = FriendsState.FirstDone;
	}
	else if(friendsState == FriendsState.FirstDone)
	{
            friendsState = FriendsState.SecondDone;
	}
	else if(friendsState == FriendsState.SecondDone)
	{
            friendsState = FriendsState.Done;
