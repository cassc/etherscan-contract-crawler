...
contract DreamGirlsV3 is ERC721AUpgradeable, DefaultOperatorFiltererUpgradeable,
    OwnableUpgradeable {
    ...
    */
    ...
    function burnToken(uint256[] calldata tokenIDs) external{
        for(uint i; i < tokenIDs.length;){
            require(msg.sender == ownerOf(tokenIDs[i]), "You are not the owner of this token");
            _burn(tokenIDs[i]);
