...
contract CLIPPYNFT is ERC721A, Ownable, ERC2981, DefaultOperatorFilterer {
    using Strings for uint256;
    ...
    uint256 public MAX_SUPPLY = 1998;
    uint256 public publicSaleCost = 0.015 ether;
    uint256 public max_per_wallet = 25;  

    //Reveal Dates
    uint256 public revealDate;
    uint256 public daysToReveal = 604800;

    //Minting Dates
    uint256 public mintDate;
    uint256 public daysToMint = 432000;

    mapping(address => uint256) public publicMinted;

    constructor(string memory _initBaseURI, string memory _initNotRevealedUri, string memory _contractURI) ERC721A("CLIPPY NFT", "CLIPPY") {
     
    setBaseURI(_initBaseURI);
    setNotRevealedURI(_initNotRevealedUri);   
    setRoyaltyInfo(0x2F91ffaed0F23B8B05efE2D29176A79fBc3f7d37,1000);
    contractURI = _contractURI;  
    revealDate = block.timestamp + daysToReveal;
    mintDate = block.timestamp + daysToMint;
    mint(1);    
    }

    function setMintDate(uint256 newDateMint) external onlyOwner {
        mintDate = newDateMint;
    }

    function setRevealDate(uint256 newDate) external onlyOwner {
        revealDate = newDate;
    }
    ...
    function mint(uint256 quantity) public payable  {
        ...
        if (msg.sender != owner()) {
            require(mintDate < block.timestamp, "Minting not yet started");
    ...
    function setDaysToMint(uint256 _daysToMint) public onlyOwner {
       daysToMint = _daysToMint;
    }
