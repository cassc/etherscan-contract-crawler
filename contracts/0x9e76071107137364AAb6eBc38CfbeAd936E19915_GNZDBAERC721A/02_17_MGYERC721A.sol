...
contract MGYERC721A is Ownable,ERC4907A, ReentrancyGuard, ERC2981{
  ...
  mapping(uint256 => uint256) internal _claimedLastTime; // tokenId -> last claimed time
  ...
  function _beforeTokenTransfers(address from_,address to_,uint256 startTokenId_,uint256 quantity_) internal virtual override {
    ...
    for (uint256 tokenId = startTokenId_; tokenId < startTokenId_ + quantity_; tokenId++) {
      ...
      if (_stakingStartedTimestamp[tokenId] != NULL_STAKED) {
        ...
        _claimedLastTime[tokenId] = NULL_STAKED;
  ...
  function burnAndMint(uint256 _amount,uint256[] calldata _tokenids) external payable virtual nonReentrant {
    ...
    require( _tokenids.length == otherContractCount,"other token count is not correct.");
  ...
  function _getStakingInfo(uint256 _tokenId) internal view virtual returns (uint256 startTimestamp, uint256 currentStakingTime, uint256 totalStakingTime, bool isStaking,uint256 claimedLastTime ){
    ...
    claimedLastTime = _claimedLastTime[_tokenId];
  }
  //get staking information.
  function getStakingInfo(uint256 _tokenId) external view virtual returns (uint256 startTimestamp, uint256 currentStakingTime, uint256 totalStakingTime, bool isStaking,uint256 claimedLastTime ){
    (startTimestamp, currentStakingTime, totalStakingTime, isStaking, claimedLastTime) = _getStakingInfo(_tokenId);
  }
  ...
  function _toggleStaking(uint256 _tokenId) internal virtual {
    ...
    if (startTimestamp == NULL_STAKED) { 
      ...
      _claimedLastTime[_tokenId] = NULL_STAKED;
    }
  }
  //toggle staking status
  ...
  function _claimReward(uint256 _tokenId) internal virtual {
    ...
    (uint256 startTimestamp, uint256 currentStakingTime, uint256 totalStakingTime, bool isStaking,uint256 claimedLastTime ) = _getStakingInfo(_tokenId);
    uint256 _lastTime = block.timestamp;
    //call reword. other contract 
    _rewardContractFactory.claimReward(_tokenId, startTimestamp,  currentStakingTime,  totalStakingTime,  isStaking,  claimedLastTime,  _lastTime);

    _claimedLastTime[_tokenId] = _lastTime;
