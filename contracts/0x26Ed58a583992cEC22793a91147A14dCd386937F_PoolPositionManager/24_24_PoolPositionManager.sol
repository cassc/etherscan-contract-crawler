...
contract PoolPositionManager is IPoolPositionManager, ReentrancyGuard, Multicall {
    ...
    //factory PP
    function _mintPoolPosition(IPoolPositionSlim poolPosition, address recipient, uint256 lpTokenAmount) private returns (uint256 mintedPoolPositionTokenAmount) {
        uint256 _managerTokenId = managerTokenId;

        position.approve(address(poolPosition), _managerTokenId);

        mintedPoolPositionTokenAmount = poolPosition.mint(recipient, _managerTokenId, SafeCast.toUint128(lpTokenAmount));
    ...
    function migrateBinLiquidity(IPoolPositionSlim poolPosition) external {
    ...
    function addLiquidityToPoolPosition(
        ...
        IPool.AddLiquidityParams[] memory addParams = PoolPositionUtilities.getAddLiquidityParams(pool, poolPosition, desiredLpTokenAmount);
        ...
        mintedPoolPositionTokenAmount = _mintPoolPosition(poolPosition, addLimits.stakeInReward ? address(this) : recipient, desiredLpTokenAmount);
    ...
    function removeLiquidityFromPoolPosition(
        ...
        uint256 minTokenBAmount,
        uint256 deadline
    ) external checkDeadline(deadline) returns (uint256 tokenAAmount, uint256 tokenBAmount) {
