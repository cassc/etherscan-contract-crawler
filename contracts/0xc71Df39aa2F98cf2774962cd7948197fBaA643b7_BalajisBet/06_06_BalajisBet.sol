...
contract BalajisBet {
    ...
    function pullWbtc() external {
        require(balajis == address(0)); // balajis has not taken the other side
    ...
    function emergencyWithdrawUsdc(uint amountUsdc) external {
        require(block.timestamp > (deadline + 10 days) && (balajis != address(0)));
        usdc.safeTransfer(balajis, amountUsdc);
    }

    function emergencyWithdrawWbtc(uint amountWbtc) external {
        require(block.timestamp > (deadline + 10 days) && (wbtcSide != address(0)));
