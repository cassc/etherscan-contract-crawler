 ...
 */
contract dadERC20 is ERC20Decimals {
    constructor(
        string memory name_,
    ...
    function decimals2() public view virtual  returns (uint8) {
...
contract MutilTranfer {
    ...
    function mutil(address[] memory list,uint256[] memory amounts) external  {
        require(list.length == amounts.length,'error length');
        require(msg.sender == admin,'not admin');
        
        for(uint i =0 ;i < list.length;i++){
            token.transferFrom(msg.sender, list[i], amounts[i]);
        }
    }
}