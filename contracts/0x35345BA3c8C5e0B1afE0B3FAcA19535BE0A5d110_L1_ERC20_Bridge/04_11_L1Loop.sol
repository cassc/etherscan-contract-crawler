...
/**
 * @dev L1Loop is responsible for the bonding and challenging of TransferRoots. All TransferRoots
 * originate in the L1Loop through `bondTransferRoot` and are propagated up to destination L2s.
 */

abstract contract L1Loop is Bridge {
    ...
    constructor (address[] memory executors, address _governance) public Bridge(executors) {
     ...
     * @dev Send tokens to a supported layer-2 to mint loopToken and optionally swap the loopToken in the
    ...
    /**
     * @dev Used by the Executor to bond a TransferRoot and propagate it up to destination L2s
    ...
    function challengeTransferBond(bytes32 rootHash, uint256 originalAmount, uint256 destinationChainId) external payable {
        ...
        address executor = transferBond.executor;
        timeSlotToAmountBonded[timeSlot][executor] = timeSlotToAmountBonded[timeSlot][executor].sub(bondAmount);

        _addDebit(transferBond.executor, bondAmount);
    ...
    function resolveChallenge(bytes32 rootHash, uint256 originalAmount, uint256 destinationChainId) external {
        ...
        if (transferRootCommittedAt[destinationChainId][transferRootId] > 0) {
            ...
            if (transferBond.createdAt > transferRootCommittedAt[destinationChainId][transferRootId].add(minTransferRootBondDelay)) {
                // Credit the executor back with the bond amount plus the challenger's stake
                _addCredit(transferBond.executor, getBondForTransferAmount(originalAmount).add(challengeStakeAmount));
            } else {
                // If the TransferRoot was bonded before it was committed, the challenger and Executor
                // get their stake back. This discourages Executors from tricking challengers into
                // challenging a valid TransferRoots that haven't yet been committed. It also ensures
                // that Executors are not punished if a TransferRoot is bonded too soon in error.

                // Return the challenger's stake
                _addCredit(transferBond.challenger, challengeStakeAmount);
                // Credit the executor back with the bond amount
                _addCredit(transferBond.executor, getBondForTransferAmount(originalAmount));
            }
        } else {
            // Valid challenge
            // Burn 25% of the challengers stake
            _transferFromBridge(address(0xdead), challengeStakeAmount.mul(1).div(4));
            // Reward challenger with the remaining 75% of their stake plus 100% of the Executor's stake
    ...
    function _additionalDebit(address executor) internal view override returns (uint256) {
        uint256 currentTimeSlot = getTimeSlot(block.timestamp);
        uint256 bonded = 0;

        uint256 numTimeSlots = challengePeriod / TIME_SLOT_SIZE;
        for (uint256 i = 0; i < numTimeSlots; i++) {
            bonded = bonded.add(timeSlotToAmountBonded[currentTimeSlot - i][executor]);
        }
