...
contract VoteBountyManagerFactory is Owned {
    address public platform;

    /// @notice Emitted when a new vote bounty manager is deployed.
    event VoteBountyManagerDeployed(address depositor, address owner);
    event PlatformChanged(address oldPlatform, address newPlatform);

    ////////////////////////////////////////////////////////////////
    /// --- CONSTRUCTOR
    ///////////////////////////////////////////////////////////////

    /// @notice Creates a VoteBountyManager factory.
    /// @param _owner The owner of the factory.
    /// @param _platform The Bounty platform.
    constructor(address _owner, address _platform) Owned(_owner) {
        platform = _platform;
    }
    ...
    function deployVoteBountyManager(VoteBountyManager.BountyData memory _bounty, address _owner, uint256 _amount) external returns (VoteBountyManager manager) {
        // Deploy the votesBountyManager.
        manager = new VoteBountyManager(_bounty, _owner, platform);
        ...
        emit VoteBountyManagerDeployed(address(manager), _owner);
    }

    /// @notice Set a new platform
    /// @param _platform The platform address.
    function setPlatform(address _platform) external onlyOwner {
        emit PlatformChanged(platform, _platform);
        platform = _platform;
    }
}