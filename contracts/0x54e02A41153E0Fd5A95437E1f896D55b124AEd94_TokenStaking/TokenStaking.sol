...
contract TokenStaking is Ownable, ReentrancyGuard {
    ...
    address public _wallet; // fee wallet
    ...
    function setPayWallet(address wallet) public onlyOwner {
        _wallet = wallet;
    }
    ...
    function stake(uint8 _pid, uint256 _amount) external nonReentrant returns (bool) {
        ...
        _stake(_pid, msg.sender, _amount - fee, false);
    ...
    function reinvest(uint8 _pid) public returns (bool) {
        uint256 amount = _payout(_pid, msg.sender);
        if (amount > 0) {
