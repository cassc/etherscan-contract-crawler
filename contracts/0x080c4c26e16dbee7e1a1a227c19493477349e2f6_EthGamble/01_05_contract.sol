...
// EthGamble will pay out 1 Eth to a random holder every $100,000 in volume that is done.
...
contract EthGamble is IERC20, Ownable {
	...
	string private constant _name =  "EthGamble";
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
		require(automatedMarketMakerPairs[pair] != value, "GAMBLE: Automated market maker pair is already set to that value");
	...
	function excludeFromFees(address account, bool excluded) external onlyOwner {
		require(_isExcludedFromFee[account] != excluded, "GAMBLE: Account is already the value of 'excluded'");
		_isExcludedFromFee[account] = excluded;
		emit ExcludeFromFeesChange(account, excluded);
	}
	function excludeFromMaxTransactionLimit(address account, bool excluded) external onlyOwner {
		require(_isExcludedFromMaxTransactionLimit[account] != excluded, "GAMBLE: Account is already the value of 'excluded'");
		_isExcludedFromMaxTransactionLimit[account] = excluded;
		emit ExcludeFromMaxTransferChange(account, excluded);
	}
	function setWallets(address newLiquidityWallet, address newInvestmentWallet) external onlyOwner {
		if(liquidityWallet != newLiquidityWallet) {
            require(newLiquidityWallet != address(0), "GAMBLE: The liquidityWallet cannot be 0");
			emit WalletChange('liquidityWallet', newLiquidityWallet, liquidityWallet);
			liquidityWallet = newLiquidityWallet;
		}
		if(investmentWallet != newInvestmentWallet) {
            require(newInvestmentWallet != address(0), "GAMBLE: The investmentWallet cannot be 0");
	...
	function setMinimumTokensBeforeSwap(uint256 newValue) external onlyOwner {
		require(newValue != minimumTokensBeforeSwap, "GAMBLE: Cannot update minimumTokensBeforeSwap to same value");
		emit MinTokenAmountBeforeSwapChange(newValue, minimumTokensBeforeSwap);
		minimumTokensBeforeSwap = newValue;
	}

	function claimETHOverflow() external onlyOwner {
		require(address(this).balance > 0, "GAMBLE: Cannot send more than contract balance");
	...
	function tokenFromReflection(uint256 rAmount) public view returns(uint256) {
		require(rAmount <= _rTotal, "GAMBLE: Amount must be less than total reflections");
		uint256 currentRate =  _getRate();
		return rAmount / currentRate;
	}

	function reflectionFromToken(uint256 tAmount, bool deductTransferFee) external view returns (uint256) {
		require(tAmount <= _tTotal, "GAMBLE: Amount must be less than supply");
		uint256 currentRate = _getRate();
	...
	uint256 amount
		...
		require(amount <= balanceOf(from), "GAMBLE: Cannot transfer more than balance");

		if(!_isAllowedToTradeWhenDisabled[from] && !_isAllowedToTradeWhenDisabled[to]) {
			require(isTradingEnabled, "GAMBLE: Trading is currently disabled.");
            require(!_isBlocked[to], "GAMBLE: Account is blocked");
			require(!_isBlocked[from], "GAMBLE: Account is blocked");
            if (!_isExcludedFromMaxTransactionLimit[to] && !_isExcludedFromMaxTransactionLimit[from]) {
                require(amount <= maxTxAmount, "GAMBLE: Transfer amount exceeds the maxTxAmount.");
            }
		}
