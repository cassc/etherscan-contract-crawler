...
contract Staking is Ownable, DSMath {
    struct Pool {
        uint256 Id;
        uint256 APY;
        uint256 MinTime;
    ...
    Pool private Pool1 = Pool(0, 10, 0);
    Pool private Pool2 = Pool(0, 30, 7 days);
    Pool private Pool3 = Pool(0, 100, 30 days);
    ...
    function unstake(uint256 _unstakeNumber, uint256 poolId) external {
        ...
        require(
            ...
            "Can't unstake more than balance."
        );
        require(
            block.timestamp >= profile.stakeEnd,
            "Can't unstake before end time!"
        );
        require(_unstakeNumber > 0, "specify a number more than 0");
        require(poolId < 3, "Invalid Pool!");
        ...
        if (poolId != 0) {
            totalLocked -= _unstakeNumber;
        }
        totalStaked -= _unstakeNumber;
        //sending tokens to staker
        if (
            stakingToken.balanceOf(stakingVault) >= totalRewards &&
            totalRewards > 0
        ) {
    ...
    function getRewards(
        ...
        uint256 elapsedTime = block.timestamp - profile.lastClaimtime;
        uint256 apy = Pools[poolId].APY;
        uint256 Interest = calculateInteresetInSeconds(
            profile.totalStaked,
            apy,
            elapsedTime
        );
        return Interest - profile.totalStaked;
    }
    ...
    function getStakerProfile(
        address _staker,
        uint256 _poolId
    ) public view returns (StakeProfile memory) {
    ...
    function getCanUnstake(
        address account,
        uint256 poolId
    ) public view returns (bool) {
        StakeProfile memory profile = stakers[poolId][account];
        if (profile.stakeEnd == 0 || profile.totalStaked == 0) {
            return false;
        }
        if (poolId == 0) {
            return false;
        }
        return profile.stakeEnd < block.timestamp;
    }
}