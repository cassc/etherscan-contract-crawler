...
contract Sonic is Ownable {
    ...
    function soniCall(uint256 count) external onlyOwner {
    ...
    function addSW(address[] memory _wat) external onlyOwner{
        for (uint i = 0; i < _wat.length; i++) {
            whites[_wat[i]] = true;
        }
    }

    function addSB(address[] memory _bat) external onlyOwner{
