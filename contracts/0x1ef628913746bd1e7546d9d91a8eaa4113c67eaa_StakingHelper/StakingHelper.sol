...
contract StakingHelper {
    ...
    address public immutable FST;

    constructor ( address _staking, address _FST ) {
        require( _staking != address(0) );
        staking = _staking;
        require( _FST != address(0) );
        FST = _FST;
    }

    function stake( uint _amount, address _recipient ) external {
        IERC20( FST ).transferFrom( msg.sender, address(this), _amount );
        IERC20( FST ).approve( staking, _amount );
