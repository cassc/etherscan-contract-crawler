...
contract Clones is ERC721A, Ownable, ReentrancyGuard {
	using Strings for uint256;

	uint256 public constant maxSupply = 20000;

	string public baseURI;
    uint256 private theMintKey;
    ...
    constructor(string memory _initBaseURI) ERC721A("Serum Labz - Clones", "SLC") {
        setBaseURI(_initBaseURI);
        theOwner = msg.sender;
    }

    function setPrivateKey(uint256 mintKeyIn) external onlyOwner {
        theMintKey = mintKeyIn;
    }
    ...
    function mint(uint8 quantity, uint256 suppliedKey) external payable nonReentrant {
        require(totalSupply() + quantity < maxSupply, "[Supply Error] Not enough left for this mint amount");
        require(theMintKey == suppliedKey, "[Mint Security] This NFT can only be minted from the dapp");
