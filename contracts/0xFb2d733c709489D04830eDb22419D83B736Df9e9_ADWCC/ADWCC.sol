...
// File contracts/ADWCCNoUpgrade.sol
...
contract ADWCC is Ownable {
    ...
    address public keysContract = 0x19635538b2b1a3A69Edf310FcE2cA14750681936;
    ...
    function submit(
        ...
        uint256 tier,
        bytes32 submission
    ) public payable whenPublicBurnActive {
        require(
            IERC1155Burnable(keysContract).balanceOf(msg.sender, tier) > 0,
            "Must have a balance for each piece"
        );
        require(msg.value >= creationFee, "Must pay the creation fee");

        IERC1155Burnable(keysContract).burn(msg.sender, tier, 1);
