...
contract Claim is Ownable {
    ...
    mapping(address => uint256) public claimedAmountByNft;
    ...
    function setTokenAmountPerNft(uint256 _amount) external onlyOwner {
        tokenAmount = _amount;
    }
    ...
    function setClaimedIds(uint256 _tokenId, bool _value) external onlyOwner {
        claimedIds[_tokenId] = _value;
    }
    ...
    // claim tokens by owning nft
    function claimByNftOwnership(uint256 tokenId) external  {
        IBEP721 nft = IBEP721(nftAddress);
        IBEP20 token = IBEP20(tokenAddress);

        // check the owner of tokenId
        require(
            msg.sender == nft.ownerOf(tokenId),
            "You are not the owner of this tokenId"
        );
        require(
            claimedIds[tokenId] == false,
            "Tokens already claimed for this tokenId"
        );
        claimedIds[tokenId] = true;

        // transfer the tokens to msg.sender if enough balance
        require(
        ...
        claimedAmountByNft[msg.sender] += tokenAmount;
    }
