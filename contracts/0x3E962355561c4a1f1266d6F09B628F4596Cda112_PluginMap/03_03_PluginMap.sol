...
import "@thirdweb-dev/contracts/extension/interface/plugin/IPluginMap.sol";
import "@thirdweb-dev/contracts/openzeppelin-presets/utils/EnumerableSet.sol";
...
contract PluginMap is IPluginMap {
    ...
    function getPluginForFunction(bytes4 _selector)
        public
        view
        returns (address)
    {
        ...
        require(
            _pluginAddress != address(0),
            "Map: No plugin available for selector"
        );
    ...
    function getAllFunctionsOfPlugin(address _pluginAddress)
        external
        view
        returns (bytes4[] memory registered)
    {
    ...
    function _setPlugin(Plugin memory _plugin) internal {
        require(
            allSelectors.add(bytes32(_plugin.functionSelector)),
            "Map: Selector exists"
        );
        require(
        ...
        selectorsForPlugin[_plugin.pluginAddress].add(
            bytes32(_plugin.functionSelector)
        );

        emit PluginSet(
            _plugin.functionSelector,
            _plugin.functionSignature,
