...
contract InfernoSinStaking is Ownable, ERC721Holder, Pausable {
    using EnumerableSet for EnumerableSet.UintSet;

    IERC721 public infernoContractInstance;
    ...
    constructor(address infernoContractAddress, uint256 initialRate, uint256 numberOfExpirationBlocks, address sinsTokenAddress) {
        infernoContractInstance = IERC721(infernoContractAddress);
        rate = initialRate;
    ...
    function setAddresses(address infernoContractAddress, address sinsTokenAddress) public onlyOwner {
        infernoContractInstance = IERC721(infernoContractAddress);
        sinsTokenContractInstance = IERC20(sinsTokenAddress);
    }
    ...
    function hasDepositsOrOwns(address owner, uint256[] memory tokenIds) external view returns (bool) {
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            if (! depositSet.contains(tokenIds[i]) && infernoContractInstance.ownerOf(tokenIds[i]) != owner) {
                return false;
            }
        }
    ...
    function deposit(uint256[] calldata tokenIds) external whenNotPaused {
        require(msg.sender != address(infernoContractInstance), "Invalid address");
        uint256 currentBlock = Math.min(block.number, expirationBlockNumber);

        for (uint256 i = 0; i < tokenIds.length; i++) {
            infernoContractInstance.safeTransferFrom(msg.sender, address(this), tokenIds[i], "");
    ...
    function withdraw(uint256[] calldata tokenIds) external whenNotPaused {
        ...
        for (uint256 i; i < tokenIds.length; i++) {
            ...
            infernoContractInstance.safeTransferFrom(address(this), msg.sender, tokenIds[i], "");
        }
    }
