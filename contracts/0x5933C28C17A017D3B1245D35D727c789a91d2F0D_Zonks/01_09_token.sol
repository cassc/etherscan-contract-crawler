...
contract Zonks is ERC721A, Ownable, DefaultOperatorFilterer {
    ...
    string private _baseURL = "ipfs://bafybeiau7o2siehftqs56d4n33wpmlprs4eanswq7s4aqqtzl54buwhu2m";
    
    mapping(address => uint) private _mintedCount;
    

    constructor() ERC721A("Zonks", "ZONKS"){}
    ...
    function calcTotal(uint count) public view returns(uint) {
        require(saleStatus != SaleStatus.PAUSED, "Zonks: Sales are off");
    ...
    function mint(uint count) external payable {
        require(saleStatus != SaleStatus.PAUSED, "Zonks: Sales are off");
        require(_totalMinted() + count <= COLLECTION_SIZE, "Zonks: Number of requested tokens will exceed collection size");
        require(count <= TOKENS_PER_TRAN_LIMIT, "Zonks: Number of requested tokens exceeds allowance (10)");
        require(_mintedCount[msg.sender] + count <= TOKENS_PER_PERSON_PUB_LIMIT, "Zonks: Number of requested tokens exceeds allowance (150)");
        require(msg.value >= calcTotal(count), "Zonks: Ether value sent is not sufficient");
