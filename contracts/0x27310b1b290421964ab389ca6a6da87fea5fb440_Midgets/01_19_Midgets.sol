...
contract Midgets is ERC721AQueryable, ERC721ABurnable, Ownable, ReentrancyGuard {
  ...
  bool public isFreeMintEnabled = true;
  ...
  modifier mintCompliance(uint256 _mintAmount) {
    if(isFreeMintEnabled) {
      ...
      require(balanceOf(_msgSender()) + _mintAmount <= maxMintAmountPerAddress + 1, 'Max mint amount per address exceeded!');
    }
    else {
  ...
  modifier mintPriceCompliance(uint256 _mintAmount) {
    ...
    if (freeMintClaimed[_msgSender()] == false && isFreeMintEnabled == true) {
