  ...
  ERC165
{
    ...
    function setTaskURI(uint256 taskId, string memory uri) external
      onlyPrivileged
      taskMustExist(taskId)
    {
        if (_isTaskAdministrative(taskId)) {
        ...
        _setTaskURI(taskId, uri);
    }

    function isTaskAdministrative(uint256 taskId) external view
      onlyPrivileged
      taskMustExist(taskId)
      returns (bool)
    {
        return _isTaskAdministrative(taskId);
    }

    function getNrOfTasks() external view
      onlyPrivileged
      returns (uint256)
    {
        return _getNrOfTasks();
    }
