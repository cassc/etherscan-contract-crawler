...
contract RomeoAndJuliape is Context, ERC721A {
    ...
    constructor() ERC721A("Romeo and Juliape", "RAJ")  {
        ...
        owner = msg.sender;
        description = "An on-chain human-AI love story. A play written by Kalen Iwamoto and AI.";

    }

    function setOwner(address _owner) public virtual {
        require((owner == msg.sender), "NonFungibleToken: must have admin role");
    ...
    function mint() public virtual payable{
        ...
        require (totalMinted.add(1) <= maxToMint, "Max supply reached");

        _safeMint(msg.sender, 203);

    }

    function set(uint256 tokenId, string memory _userText, string memory _userText2, string memory _userText3, string memory _author) public virtual{

        // We cannot just use balanceOf to create the new tokenId because tokens
        // can be burned (destroyed), so we need a separate counter.
        require(msg.sender == owner || msg.sender == admin, "Not allowed");
        require(! textSet[tokenId], "Already set");

        wordsToTokenId[tokenId] = _userText; //Add word to mapping @tokenId
        wordsToTokenId2[tokenId] = _userText2;
        wordsToTokenId3[tokenId] = _userText3;
        author[tokenId] = _author;

        textSet[tokenId] = true;

    }

    function buildImage(uint256 _tokenId) private view returns (string memory) {
        string memory currentWord = wordsToTokenId[_tokenId];
        string memory currentWord2 = wordsToTokenId2[_tokenId];
        string memory currentWord3 = wordsToTokenId3[_tokenId];
        string memory author1 = author[_tokenId];
        return
            Base64.encode(
                bytes(
                    abi.encodePacked(
                        '<svg width="1200" height="300" xmlns="http://www.w3.org/2000/svg">',
                        '<rect height="300" width="1200" fill="hsl(328, 100%, 100%)"/>',
                        '<text x="50%" y="40%"  fill="hsl(random, 100%, 80%)" text-anchor="middle" font-size="23">',currentWord,'</text>',
                        '<text x="50%" y="50%"  fill="hsl(random, 100%, 80%)" text-anchor="middle" font-size="23">',currentWord2,'</text>',
                        '<text x="50%" y="60%"  fill="hsl(random, 100%, 80%)" text-anchor="middle" font-size="23">',currentWord3,'</text>',
                        '<text x="50%" y="20%" dominant-baseline="middle" fill="hsl(random, 100%, 80%)" text-anchor="middle" font-size="41">',author1,'</text>',
                        '</svg>'
                    )
                )
            );
    ...
    {
        string memory mem = Strings.toString(_tokenId);
        return
            string(
                abi.encodePacked(
