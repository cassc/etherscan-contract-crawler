...
contract ShudokuSwap is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    ShudokuSwapDividendTracker public dividendTracker;
    ...
    constructor() ERC20("ShudokuSwap", "SHU") {
        marketingWallet = payable(0x67aFC01D0466C574fCee4e4E096cc109F3DF72F9);
        devWallet = payable(0x67aFC01D0466C574fCee4e4E096cc109F3DF72F9);
        ...
        dividendTracker = new ShudokuSwapDividendTracker(
            payable(this),
            router,
...
contract ShudokuSwapDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    address public defaultToken; // SHIDO
    ...
    ShudokuSwap public ShudokuSwapContract;
    ...
    constructor(
        ...
        defaultToken = token;
        ShudokuSwapContract = ShudokuSwap(mainContract);
    ...
    function _transfer(
        ...
        require(false, "ShudokuSwap_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "ShudokuSwap_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main ShudokuSwap contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(ShudokuSwapContract);

            uint256 prevBalance = ShudokuSwapContract.balanceOf(address(this));
            ...
            {
                uint256 received = ShudokuSwapContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    ShudokuSwapContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
