...
contract _CollarStake_ is Ownable, ReentrancyGuard, Pausable, SafeERC20 {
    ...
    uint128 constant public max_reward_amount = (200 * 1e6 * 1e18) * 67 /100;
    uint128 public lp_reward ;
    uint128 public native_reward;
    uint128 public user_staked_rewad_Token;
    uint64 public currentID;
    uint64 constant public max_staking_days = 365 * 5;
    ...
    mapping (address => uint256[]) public getUserIDs;
    ...
    function depositTokens(address _tokenAddress, uint256 _tokenAmount) external whenNotPaused nonReentrant {
        ...
        if(_tokenAddress == rewardToken){
            user_staked_rewad_Token += uint128(_tokenAmount);
        }
        ...
        getUserIDs[_msgSender()].push(currentID);
    ...
    function calculateReward(uint _tokenAmount, uint _stakeDays) private pure returns(uint){
        uint perToken = (max_reward_amount * 1e12) / max_staking_amount;
        uint perDay = perToken * 1e16 / max_staking_days;
        return _tokenAmount * perDay * _stakeDays / 1e28 / 86400;
    }
    ...
    function claimReward(uint256 _stakeID) public whenNotPaused nonReentrant{
        ...
        require(user_staked_rewad_Token < uint128(bal - reward), "reward balance exceed" );
    ...
    function withdraw(uint256 _stakeID) external whenNotPaused {
        ...
        if(user.stakeToken == rewardToken){
            user_staked_rewad_Token -= uint128(user.stakeAmount);
        }
    ...
    function recover(address _tokenAddress, address _receiver, uint256 _amount) external onlyOwner {
