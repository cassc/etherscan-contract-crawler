...
import "./IAnonymicePrisoners.sol";
import "./IDNAChip.sol";
...
contract BadgesVerifier is Ownable {
    ...
    function _getGenesisCollectorData(
        uint256[] memory tokens,
        address wallet,
        bool ignoreOwnership
    ) internal view returns (WalletCollection memory walletCollection) {
        for (uint256 index = 0; index < tokens.length; index++) {
            uint256 tokenId = tokens[index];
            require(ignoreOwnership || IAnonymice(genesisAddress).ownerOf(tokenId) == wallet, "not mouse owner");
    ...
    function _claimableBadges(
        ...
        uint256[] memory walletPrisoners = _getWalletPrisonersGenesisIds(wallet);
        WalletCollection memory genesisCollection = _getGenesisCollectorData(genesisMice, wallet, false);
        WalletCollection memory prisonersCollection = _getGenesisCollectorData(walletPrisoners, wallet, true);
        WalletCollection memory babiesCollection = _getBabiesCollectorData(babyMice, wallet);
        bool hasGenesis = IERC721Enumerable(genesisAddress).balanceOf(wallet) > 0;
        for (uint256 collabIndex = 0; collabIndex < collabBadges.length; collabIndex++) {
            ...
            if (hasGenesis && IERC721Enumerable(collabBadge.tokenAddress).balanceOf(wallet) > 0) {
                result[resultIndex++] = collabBadge.badgeId;
            }
        }

        bool hasPrisoner = IERC721Enumerable(genesisAddress).balanceOf(wallet) > 0;
        if (hasGenesis || hasPrisoner || IERC721Enumerable(babiesAddress).balanceOf(wallet) > 0) {
            // 33 => mice army
        ...
        if (
            (genesisCollection.jimDangles || prisonersCollection.jimDangles) &&
            (genesisCollection.glitched || prisonersCollection.glitched) &&
            (genesisCollection.astronaut || prisonersCollection.astronaut) &&
            (genesisCollection.irradiated || prisonersCollection.irradiated) &&
            (genesisCollection.vr || prisonersCollection.vr)
        ) {
        ...
        if ((genesisCollection.glitched || prisonersCollection.glitched)) {
            // 25 => Hold a Gl1tch3d gen mouse
            result[resultIndex++] = 25;
        }
        if ((genesisCollection.astronaut || prisonersCollection.astronaut)) {
            // 21 => Hold an astro gen mouse
            result[resultIndex++] = 21;
        }
        if ((genesisCollection.bane || prisonersCollection.bane)) {
        ...
        if (babiesCollection.halo && (genesisCollection.halo || prisonersCollection.halo)) {
        ...
        returns (WalletCollection memory walletCollection)
    {
        for (uint256 index = 0; index < tokens.length; index++) {
            ...
            bool isRevealed = IAnonymiceBreeding(babiesAddress)._tokenToRevealed(tokenId);
            bool isLegendary = IAnonymiceBreeding(babiesAddress)._tokenIdToLegendary(tokenId);
            if (!isRevealed || isLegendary) continue;
    ...
    function _getWalletPrisonersGenesisIds(address wallet) internal view returns (uint256[] memory) {
        uint256[] memory prisonerIds = _getWalletTokens(prisonersAddress, wallet);
        uint256[] memory genesisIds = new uint256[](prisonerIds.length);

        for (uint256 index = 0; index < prisonerIds.length; index++) {
            genesisIds[index] = IAnonymicePrisoners(prisonersAddress).getPrisonerGenesisId(prisonerIds[index]);
        }
