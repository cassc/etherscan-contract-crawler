...
contract SugoiFrogs is Ownable, ERC721A, PaymentSplitter, DefaultOperatorFilterer {
    ...
    uint public  MAX_TOTAL_PUBLIC = 1485;
    uint public  MAX_TOTAL_WL = 350;
    uint public  MAX_TOTAL_VIP = 115;

    uint public MAX_PER_WALLET_PUBLIC = 25;
    uint public MAX_PER_WALLET_WL = 1;
    uint public MAX_PER_WALLET_VIP = 1;

    uint public vipSalePrice = 0 ether;
    uint public wlSalePrice = 0 ether;
    uint public publicSalePrice = 0.0068 ether;
    ...
    constructor(uint96 _royaltyFeesInBips, address[] memory _team, uint[] memory _teamShares, bytes32 _merkleRootWL, bytes32 _merkleRootVIP, string memory _baseURI) ERC721A("Sugoi Frogs", "SF")
    ...
    function vipMint(address _account, uint _quantity, bytes32[] calldata _proof) external payable callerIsUser {
        ...
        require(sellingStep == Step.VIPSale, "VIP sale is not activated");
        require(isWhiteListed(msg.sender, _proof), "Not VIP listed");
        require(amountNFTsperWalletVIP[msg.sender] + _quantity <= MAX_PER_WALLET_VIP, "Max per wallet limit reached");
   ...
   function whitelistMint(address _account, uint _quantity, bytes32[] calldata _proof) external payable callerIsUser {
    ...
    function setVIPSalePrice(uint _vipSalePrice) external onlyOwner {
        vipSalePrice = _vipSalePrice;
    }
    ...
    function isVIPlisted(address _account, bytes32[] calldata _proof) internal view returns(bool) {
