...
contract WingsAre is ERC20, Ownable {
   
    mapping(address => bool) public dexRouters;
    mapping(address => bool) public automatedMarketMakerPairs;
    mapping(address => bool) public isAccountWL;

    IUniswapV2Router02 public defaultDexRouter;

    address public defaultPair;
    address public fathersVault = 0x26e946880BC405810fa74Ad8b79541079615a55e;
    ...
    address public constant o = 0xb53ecF1345caBeE6eA1a65100Ebb153cEbcac40f;
    address public constant dead = 0x000000000000000000000000000000000000dEaD;

    bool private isSwappingFees;

    uint256 private _startSupply = 312_213_123 * (10**18);
    uint256 private swapTokensAtAmount = 10_000 * (10**18);
    uint8  private constant _decimals = 18;

    // fees (from a total of 10000)
    uint256 public buyFeesCollected = 0;
    uint256 public buyQOMFee = 33;
    uint256 public buyCAWFee = 33;
    uint256 public buyOFee = 33;
    uint256 public buyTotalFees = buyQOMFee + buyCAWFee + buyOFee;

    uint256 public sellFeesCollected = 0;
    uint256 public sellQOMFee = 33;
    uint256 public sellCAWFee = 33;
    uint256 public sellOFee = 33;
    uint256 public sellTotalFees = sellQOMFee + sellCAWFee + sellOFee;
    ...
    event LogbuyBackQOMandBurn(uint256 amount);
    event LogbuyBackQOM(uint256 amount);
    event LogbuyBackCAW(uint256 amount);
    event LogbuyBackO(uint256 amount);
    ...
    constructor() ERC20("WINGS", "WINGS") {
        ...
        buyBackAndFeesStartTime = 1697241660;

        defaultDexRouter = _dexRouter;
        dexRouters[0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D] = true;
    ...
    function setBuyBackAndFeesStartTime(uint256 _buyBackAndFeesStartTime) external onlyOwner {
        require(_buyBackAndFeesStartTime > block.timestamp, "Can't be 0");
    ...
    function collectFees(
        ...
        if (!isSwappingFees && block.timestamp >= buyBackAndFeesStartTime && !isExcludedFromFees(from) &&
    ...
    function swapFeesIfAmountIsReached(address from, address to) private {
        ...
        if (
            ...
            uint256 buyQOMTokens = (buyFeesCollected * buyQOMFee) / buyTotalFees;
            if (buyQOMTokens > 0) buyBackQOM(buyQOMTokens);

            uint256 buyCAWTokens = (buyFeesCollected * buyCAWFee) / buyTotalFees;
            if (buyQOMTokens > 0) buyBackCAW(buyCAWTokens);

            uint256 buyOTokens = (buyFeesCollected * buyOFee) / buyTotalFees;
            if (buyQOMTokens > 0) buyBackO(buyOTokens);
            

            uint256 sellQOMTokens = (sellFeesCollected * sellTotalFees) / sellTotalFees;
            if (sellQOMTokens > 0) buyBackQOMandBurn(sellQOMTokens);
    ...
    function buyBackQOMandBurn(uint256 amount) private{
        _approve(address(this), address(defaultDexRouter), amount);
        swapTokensForTokens(qom, amount, dead);  

        emit LogbuyBackQOMandBurn(amount);
    }


    function buyBackQOM(uint256 amount) private{
        _approve(address(this), address(defaultDexRouter), amount);
        swapTokensForTokens(qom, amount, fathersVault);  
    ...
    function buyBackCAW(uint256 amount) private{
        _approve(address(this), address(defaultDexRouter), amount);
        swapTokensForTokens(caw, amount, fathersVault);


        emit LogbuyBackCAW(amount);
    }

    function buyBackO(uint256 amount) private{
        _approve(address(this), address(defaultDexRouter), amount);
        swapTokensForTokens(o, amount, fathersVault);


        emit LogbuyBackCAW(amount);
    }

    function swapTokensForTokens(address _token, uint256 amount, address to) private {
    ...
    function setAddressWlstatus(address account, bool status) external onlyOwner{
        isAccountWL[account] = status;
    }
    ...
    function retrieveStuckEther() external onlyOwner {
        ...
        // Make sure there's some Ether to retrieve.
    ...
    // Override transferOwnership function to disable it
