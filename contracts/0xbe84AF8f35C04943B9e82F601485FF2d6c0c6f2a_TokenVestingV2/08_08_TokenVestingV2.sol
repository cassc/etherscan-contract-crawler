...
contract TokenVestingV2 is Ownable, ReentrancyGuard {
    ...
    // Manager for create/update vesting schedules
    ...
    modifier onlyOwnerOrManager() {
        require(owner() == _msgSender() || manager == _msgSender(), "Only owner or manager");
        _;
    }
    ...
    function setManager(address manager_) public onlyOwner {
        require(manager != address(0x0), "Manager cant be zero address");
        manager = manager_;
    }
    ...
    ) external onlyOwnerOrManager onlyIfVestingScheduleNotRevoked(vestingScheduleId) {
     ...
     */
    function withdraw(uint256 amount) external nonReentrant onlyOwnerOrManager {
        require(
