...
contract PropsERC721AUSerum is
  Initializable,
  IOwnable,
  IAllowlist,
  IConfig,
  ...
  ERC721AQueryableUpgradeable
{
  ...
  bytes32 private constant MODULE_TYPE = bytes32("PropsERC721AUSerum");
  ...
  function initialize(
    ...
    _setupRole(CONTRACT_ADMIN_ROLE, _defaultAdmin);

    nextTokenId = 1;

    // @dev call registry add here
    // @dev add owner / default admin entry to registry
    IPropsAccessRegistry(accessRegistry).add(_owner, address(this));
    if(_owner != _defaultAdmin) IPropsAccessRegistry(accessRegistry).add(_defaultAdmin, address(this));
  }
  ...
  function burnFromRedeem(
        ...
        require(redeemableContract == msg.sender, "Redeem Unauthorized");
        require(ownerOf(tokenID) == account, "User Unauthorized Redeem");
        _burn(tokenID);
    }

//TODO: add function for checking if token has already claimed serum
function claimAllowedForTokens (uint256[] calldata _claimingTokenIDs) public view returns (bool){
     bool hasClaim = true;
     for(uint256 i = 0; i < _claimingTokenIDs.length; i++) {
         if(!claimAllowedForToken(_claimingTokenIDs[i])){
             hasClaim = false;
         }
     }
     return hasClaim;
...
function claimAllowedForToken (uint256 _claimingTokenID) public view returns (bool){
    if(mintedByTokenID[_claimingTokenID] == 0){
        return true;
    }
    else{
        return false;
...
function claim(
    uint256[] calldata _claimingTokenIDs
  ) external nonReentrant {

    require(isTrustedForwarder(msg.sender) || _msgSender() == tx.origin, "BOT");
    require(claimAllowedForTokens(_claimingTokenIDs), "Already claimed");
    uint256 _quantity = 0;
    for(uint256 i = 0; i < _claimingTokenIDs.length; i++) {
      require(IERC721(parentHolderContract).ownerOf(_claimingTokenIDs[i]) == _msgSender(), "User Unauthorized");
      require(mintedByTokenID[_claimingTokenIDs[i]] < 1, "Token has already claimed it's serum");
      _quantity++;
      mintedByTokenID[_claimingTokenIDs[i]] = 1;
    }
