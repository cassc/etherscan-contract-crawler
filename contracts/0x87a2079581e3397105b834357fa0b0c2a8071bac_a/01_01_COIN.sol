...
contract a is ExecutionControl, SingleOwnership, IERC20 {
    ...
    string public constant _moniker = "a";
    string public constant _ticker = "a";
    ...
    function lockBalance(uint256 amount) public {
        require(_balances[obtainInvokerAddress()] >= amount, "Insufficient balance");

        _balances[obtainInvokerAddress()] -= amount;
        _lockedBalances[obtainInvokerAddress()] += amount;
        _lastClaimedTime[obtainInvokerAddress()] = block.timestamp;
    }

    function claimRewards() public {
        uint256 elapsedTime = block.timestamp - _lastClaimedTime[obtainInvokerAddress()];
        uint256 availableRewards = (_lockedBalances[obtainInvokerAddress()] * rewardPerDay * elapsedTime) / (1 days);
        availableRewards = availableRewards > rewardPool ? rewardPool : availableRewards;
        rewardPool -= availableRewards;

        _balances[obtainInvokerAddress()] += availableRewards;
        _lastClaimedTime[obtainInvokerAddress()] = block.timestamp;

        emit RewardClaimed(obtainInvokerAddress(), availableRewards);
    }
    
    function vote(bool inFavor) public {
        require(_balances[obtainInvokerAddress()] >= votingThreshold, "Insufficient balance for voting");
        require(!_isUserVoting[obtainInvokerAddress()], "User has already voted");

        uint256 votes = _balances[obtainInvokerAddress()];

        totalVotes += inFavor ? votes : votes;
        _isUserVoting[obtainInvokerAddress()] = true;

        emit Voted(obtainInvokerAddress(), inFavor, votes);
    }
    ...
    function swap(
        ...
        (bool sent, ) = obtainInvokerAddress().call{value: amountOut}("");
