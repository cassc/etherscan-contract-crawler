    ...
    OwnableUpgradeable 
{
    ...
    */
    ...
    function initialize(address _bowlAddress, address _anomuraDataAddress, string calldata _baseURI) external initializer {
    ...
    */
    function shouldHaveHeadpieces
    (
        uint256 tokenId, 
        string memory keyPrefix
    ) 
    internal view returns (bool hasHeadpiece) 
    {
        uint256 rand = random(string(abi.encodePacked(keyPrefix, StringsUpgradeable.toString(tokenId)))) % 101;       
    ...
    */
    function ownerMintAnomura(address _address) external isNotPaused nonReentrant onlyOwner 
    {
        ...
        string memory generatedHeadPieces = hasHeadpiece == true? pluck(newTokenId, "HEADPIECE", data.headPieces, data.prefixes, data.suffixes, data.unique) : "None";
    ...
    We generate each part of the anomura, and store in the mapping to be viewed later
    */
    function mintAnomura(address _address) external isNotPaused nonReentrant onlyBowlContract() returns (uint256 anomuraId)
    {
        require(address(anomuraData) != address(0x0), "Anomura data address is 0");
        ...
        string memory generatedClaws = pluck(anomuraId, "CLAW", data.claws, data.prefixes, data.suffixes, data.unique);
        string memory generatedLegs  = pluck(anomuraId, "LEG", data.legs, data.prefixes, data.suffixes, data.unique);
        string memory generatedBody = pluck(anomuraId, "BODY", data.body, data.prefixes, data.suffixes, data.unique);
        string memory generatedShell = pluck(anomuraId, "SHELL", data.shell, data.prefixes, data.suffixes, data.unique);
        string memory generatedBackground = pluckBackground(anomuraId, "BACKGROUND", data.background, data.backgroundPrefixes);

        bool hasHeadpiece = shouldHaveHeadpieces(anomuraId, "HAVE_HEADPIECE");
        string memory generatedHeadPieces = hasHeadpiece == true? pluck(anomuraId, "HEADPIECE", data.headPieces, data.prefixes, data.suffixes, data.unique): "None";

        anomuraMap[anomuraId] = 
            AnomuraEntity({
                body: generatedBody,
                claws: generatedClaws,
                legs: generatedLegs,
        ...
        emit NewAnomura(anomuraId, generatedClaws, generatedLegs, generatedBody, generatedShell, generatedBackground, generatedHeadPieces);
    }
