...
contract TopDawg is ERC20, ERC20Burnable, Ownable {
    
    uint256 public swapThreshold;
    
    uint256 private _marketingPending;
    uint256 private _procryptodonationsPending;
    uint256 private _liquidityPending;

    address public marketingAddress;
    uint16[3] public marketingFees;

    address public procryptodonationsAddress;
    uint16[3] public procryptodonationsFees;
    ...
    event procryptodonationsAddressUpdated(address procryptodonationsAddress);
    event procryptodonationsFeesUpdated(uint16 buyFee, uint16 sellFee, uint16 transferFee);
    event procryptodonationsFeeSent(address recipient, uint256 amount);
    ...
    constructor()
        ERC20("TopDawg", "TopDawg") 
    {
        address supplyRecipient = 0x5D79dAfb9053404DB1883D791081187079488784;
        
        updateSwapThreshold(375000000 * (10 ** decimals()));

        marketingAddressSetup(0xD23E58c928C5307B063B090F7C2079A252d324d2);
        marketingFeesSetup(100, 100, 0);

        procryptodonationsAddressSetup(0xab0771C8f1DA1b097be8463e5bfA3B3170cc2671);
        procryptodonationsFeesSetup(100, 100, 0);
        ...
        excludeFromLimits(procryptodonationsAddress, true);

        updateMaxWalletAmount(37499000000 * (10 ** decimals()));

        updateMaxTxAmount(37499000000 * (10 ** decimals()));

        updateTradeCooldownTime(60);

        _mint(supplyRecipient, 750000000000 * (10 ** decimals()));
        _transferOwnership(0x5D79dAfb9053404DB1883D791081187079488784);
    }
    ...
    function marketingFeesSetup(uint16 _buyFee, uint16 _sellFee, uint16 _transferFee) public onlyOwner {
        ...
        totalFees[0] = 0 + marketingFees[0] + procryptodonationsFees[0] + liquidityFees[0];
        totalFees[1] = 0 + marketingFees[1] + procryptodonationsFees[1] + liquidityFees[1];
        totalFees[2] = 0 + marketingFees[2] + procryptodonationsFees[2] + liquidityFees[2];
        require(totalFees[0] <= 2500 && totalFees[1] <= 2500 && totalFees[2] <= 2500, "TaxesDefaultRouter: Cannot exceed max total fee of 25%");

        emit marketingFeesUpdated(_buyFee, _sellFee, _transferFee);
    }

    function procryptodonationsAddressSetup(address _newAddress) public onlyOwner {
        procryptodonationsAddress = _newAddress;

        excludeFromFees(_newAddress, true);

        emit procryptodonationsAddressUpdated(_newAddress);
    }

    function procryptodonationsFeesSetup(uint16 _buyFee, uint16 _sellFee, uint16 _transferFee) public onlyOwner {
        procryptodonationsFees = [_buyFee, _sellFee, _transferFee];
        ...
        emit procryptodonationsFeesUpdated(_buyFee, _sellFee, _transferFee);
    }
    ...
    function _transfer(
        ...
        bool canSwap = 0 + _marketingPending + _procryptodonationsPending + _liquidityPending >= swapThreshold;
        
        if (!_swapping && !AMMPairs[from] && canSwap) {
            _swapping = true;
            
            if (false || _marketingPending > 0 || _procryptodonationsPending > 0) {
                uint256 token2Swap = 0 + _marketingPending + _procryptodonationsPending;
                bool success = false;
                ...
                uint256 procryptodonationsPortion = coinsReceived * _procryptodonationsPending / token2Swap;
                if (procryptodonationsPortion > 0) {
                    (success,) = payable(address(procryptodonationsAddress)).call{value: procryptodonationsPortion}("");
                    require(success, "TaxesDefaultRouterWalletCoin: Fee transfer error");
                    emit procryptodonationsFeeSent(procryptodonationsAddress, procryptodonationsPortion);
                }
                _procryptodonationsPending = 0;
        ...
        if (!_swapping && amount > 0 && to != address(routerV2) && !isExcludedFromFees[from] && !isExcludedFromFees[to]) {
            ...
            if (txType < 3) {
                ...
                _procryptodonationsPending += fees * procryptodonationsFees[txType] / totalFees[txType];
