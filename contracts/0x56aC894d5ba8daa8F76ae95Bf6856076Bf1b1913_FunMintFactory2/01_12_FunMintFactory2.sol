...
import {FunMint2} from "./FunMint2.sol";

contract FunMintFactory2 is Ownable {
    ...
    function create(uint256 id) public returns (address) {
        ...
        address deployed = deploy(conf.salt, type(FunMint2).creationCode);
        FunMint2(payable(deployed)).init(conf.name, conf.symbol, conf.owner);
        return deployed;
    }
}