...
contract WETHGateway is IWETHGateway, ERC721HolderUpgradeable, EmergencyTokenRecoveryUpgradeable {
  ...
  function depositETH(address onBehalfOf, DataTypes.Period period, uint16 referralCode) external payable override nonReentrant {
    _checkValidCallerAndOnBehalfOf(onBehalfOf);

    ILendPool cachedPool = _getLendPool();

    WETH.deposit{value: msg.value}();
    cachedPool.deposit(address(WETH), msg.value, onBehalfOf, period,referralCode);
  }

  function withdrawETH(uint256 amount, address to, DataTypes.Period period) external override nonReentrant {
    _checkValidCallerAndOnBehalfOf(to);

    uint8 periodIndex = uint8(period);
    ILendPool cachedPool = _getLendPool();
