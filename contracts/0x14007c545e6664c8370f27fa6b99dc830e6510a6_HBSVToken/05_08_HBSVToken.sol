...
import "./HBSVAdmin.sol";
import "./HBSVLogic.sol";
import "./HBSVStorage.sol";
import "./Pausable.sol";

contract HBSVToken is IERC20Token,Pausable, HBSVAdmin{
    string public constant name = "Huobi BSV";

    string public constant symbol = "HBSV";

    uint8 public constant decimals = 18;

    HBSVLogic private logic;

    event Burning(address indexed from, uint256 value, string proof, string  bsvAddress, address burner);
    event Burned(address indexed from, uint256 value, string proof, string  bsvAddress);
    ...
    function burn(address from,uint256 value,string memory bsvAddress,string memory proof, bytes32 taskHash)
    public whenNotPaused returns(bool){
        require(itemAddressExists(OPERATORHASH, msg.sender), "wrong operator");
        uint256 status = logic.burnLogic(from,value,bsvAddress,proof,taskHash, msg.sender, operatorRequireNum);
        if (status == 1){
           emit Burning(from, value, proof,bsvAddress, msg.sender);
        }else if (status == 3) {
           emit Burning(from, value, proof,bsvAddress,  msg.sender);
           emit Burned(from, value, proof,bsvAddress);
