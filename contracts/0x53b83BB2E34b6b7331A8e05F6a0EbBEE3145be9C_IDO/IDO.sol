...
contract IDO is Ownable{
    ...
    uint256 public _leastFee;

    address public _collectionAddr;

    mapping (address => uint256) public _users;

    constructor(address token,uint maxCount,uint256 rate,address collection,uint256 priceFee){
        _token = token;
    ...
    function setMaxAmount(uint256 _amount) onlyOwner external {
        _totalCount = _amount;
    }
    ...
    function presale() payable external {
        require(_currentAmount < _totalCount,"max error");
