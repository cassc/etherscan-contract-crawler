...
library TradeLogic {
    ...
    function executeTrade(
        ...
        ValidationLogic.validatePreTrade(
            shortReserve,
            longReserve,
        ...
        ValidationLogic.validatePostTrade(
            longReserve,
            vars.cachedData.currLongingPosition,
            vars.receivedAmount
        );
    ...
    function liquidationTrade(
        ...
        ValidationLogic.validateLiquidationTrade(
            shortReserve.configuration.state,
            longReserve.configuration.state,
    ...
    function executeShorting(
        ...
        if (vars.amountToBorrow > 0) {
            uint256 newLockedReserveSupplyRay = reserve.scaledUtilizedSupplyRay.rayMul(vars.reserveCache.currReserveIndexRay)
            + vars.amountToBorrow.unitToRay(vars.unit);
            ...
            if (reserve.scaledUtilizedSupplyRay > 0) {
                reserve.reserveIndexRay = newLockedReserveSupplyRay.rayDiv(reserve.scaledUtilizedSupplyRay);

                reserve.protocolIndexRay = reserve.scaledUtilizedSupplyRay.rayMul(vars.reserveCache.currBorrowIndexRay)
                .rayDiv(reserve.scaledUtilizedSupplyRay) - reserve.reserveIndexRay;
            }
        }
    ...
    function executeLonging(
        ...
        if (vars.amountToRepay > 0) {
            ...
            // @note: since user cannot fully repay their short due to a small decimal of assets
            // this will not be paid fully and will accumulate
            ...
            // @dev: cannot fully paid last borrower's due to precision issue
            uint256 newLockedReserveSupplyRay = reserve.scaledUtilizedSupplyRay.rayMul(vars.reserveCache.currReserveIndexRay)
            - (vars.amountToRepay - vars.protocolClaimableAmount).unitToRay(assetConfigCache.decimals);
