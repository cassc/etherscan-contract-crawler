...
// external libraries
import {ActionUtil} from "grappa/libraries/ActionUtil.sol";
...
contract SimpleSettlementCash is SimpleSettlementBase {
     ...
     */
    function _createPremiumTransfer(IAuctionVaultCash _vault, int256 _premium) internal view returns (ActionArgs memory) {
        ...
        returns (uint256 errorCount, bytes32[] memory errors)
    {
...

        {
            ...
            if (premiumPayer != address(0) || address(grappa) != address(0)) {
                (address addr,) = grappa.assets(vault.getCollaterals()[0].id);
