...
// Miami Sushi Club: Sushi Warriors 

contract SushiWarriorsMSC is ERC721, PaymentSplitter, Ownable {
    ...
    constructor(address[] memory _payees, uint256[] memory _shares) ERC721("MSC: SUSHI WARRIORS", "SW") PaymentSplitter(_payees, _shares) {
        _mintSW(teamWallet, 50);
    }
    ...
    function allowListMint(bytes32[] calldata _merkleProof, uint _amount) external payable isNotPaused(allowListMintEnabled) {
        ...
        require(totalSupply() + _amount < 2_556, 'Exceeds max supply');
    ...
    // Allows the team to mint SW to a destination address
    ...
    function _mintSW(address _to, uint _amount) internal {
