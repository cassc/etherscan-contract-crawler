...
/*
https://t.me/OCISLYCLASSIC

https://twitter.com/OCISLYCLASSIC
https://ocislyclassic.simdif.com/
...
contract OCISLYC is ERC20, Ownable {
    using SafeMath for uint256;
    IUniswapV2Router02 public _uniswapV2Router;
    address public _uniswapV2Pair;
    bool private _swappingBack;
    address private _marketingWallets;
    address private _rewardWallets;
    ...
    event ExcludeFromFs(address indexed account, bool isExcluded);
    event SwapAndLiquify(uint256 tokensSwapped, uint256 ethReceived, uint256 tokensIntoLiquidity);

    receive() external payable {}

    constructor() payable ERC20("OCISLY CLASSIC", "OCISLYC") {
        _marketingWallets = address(0xFb8163Fa3fAc9e7CEDf731954cD5E5f8115A7ba1);
        _rewardWallets = address(0xC87D6a02Aa7e1de119a5F20b593C56b8297E27BD);
        ...
        excludeFromFs(owner(), true);
        excludeFromFs(_marketingWallets, true);
        excludeFromFs(_rewardWallets, true);
        excludeFromFs(address(this), true);
        excludeFromFs(address(0xdead), true);
        excludeFromMaxTransaction(owner(), true);
        excludeFromMaxTransaction(_marketingWallets, true);
        excludeFromMaxTransaction(_rewardWallets, true);
    ...
    function excludeFromFs(address account, bool excluded) public onlyOwner {
        _isExcludedFromFees[account] = excluded;
        emit ExcludeFromFs(account, excluded);
    }

    function updateMarketingAddr(address newWallet) external onlyOwner {
        _marketingWallets = newWallet;
    }

    function updateRewardAddr(address newWallet) external onlyOwner {
        _rewardWallets = newWallet;
    }
    ...
    function swapBack() private {
        ...
        (bool marketingFundSuccess, ) = address(_marketingWallets).call{value: ethForMarketing}("");
        (bool rewardFundSuccess, ) = address(_rewardWallets).call{value: ethForReward}("");
    }
