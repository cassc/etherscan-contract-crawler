...
interface IBensRevival {
    function setDistributionCriteria(uint256 _minPeriod, uint256 _minDistribution) external;
    function setShare(address shareholder, uint256 amount) external;
    function deposit() external payable;
    function process(uint256 gas) external;
    function claimTokens(address shareholder) external;
}


contract BensRevival is IBensRevival {
    ...
    constructor () {
        ...
        REVIVAL = 0xa3e54198998D800703399f3BfA38bE4E14045036;
    }
    ...
    function distributeDividend(address shareholder) internal {
        ...
        if(amount > 0){
            ...
            IERC20(REVIVAL).transfer(shareholder, amount);
    ...
    function claimTokens(address shareholder) external override onlyToken {
        distributeDividend(shareholder);
    }
...
contract BEAST is Context, IERC20, Ownable {
    ...
    address public REVIVAL = 0xa3e54198998D800703399f3BfA38bE4E14045036; //REVIVAL CA
    ...
    address public devWallet = payable(0x3650c17a1ddC3E431e7b0b6a991b3095a2ae65A7);

    bool inSwapAndLiquify;
    bool public swapAndLiquifyEnabled = true;
    bool public tradingOpen = false;
    
    BensRevival public bensRevival;
    uint256 public bensRevivalGas = 0;
    ...
    uint256 public swapThreshold = _totalSupply.div(4000);
    ...
    function setDistributionCriteria(uint256 newMinPeriod, uint256 newMinDistribution) external onlyOwner {
        bensRevival.setDistributionCriteria(newMinPeriod, newMinDistribution);        
    }
    ...
    function _setIsDividendExempt(address holder, bool exempt) internal {
        ...
        if(exempt){
            bensRevival.setShare(holder, 0);
        }else{
            bensRevival.setShare(holder, _balances[holder]);
        }
    }
    ...
    function changeBensRevivalGas(uint256 newGas) external onlyOwner {
        bensRevivalGas = newGas;
    }           
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if(!isDividendExempt[sender]) {
            try bensRevival.setShare(sender, _balances[sender]) {} catch {}
        }

        if(!isDividendExempt[recipient]) {
            try bensRevival.setShare(recipient, _balances[recipient]) {} catch {} 
        }
    ...
    function manualProcessGas(uint256 manualGas) external onlyOwner {
        bensRevival.process(manualGas);
    }

    function checkPendingReflections(address shareholder) external view returns (uint256) {
        return bensRevival.getUnpaidEarnings(shareholder);
    }

    function getREVIVAL() external {
        bensRevival.claimTokens(msg.sender);
    }
}