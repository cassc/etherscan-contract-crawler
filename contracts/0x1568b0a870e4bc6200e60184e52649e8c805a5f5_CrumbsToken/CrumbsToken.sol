...
contract CrumbsToken is ERC20 {
    ...
    address private cockAddress;
    IERC20 private cockToken;

    uint256 private _totalCockStaked;
    mapping(address => stakeTracker) private _stakedBalances;
    
    constructor() public ERC20("Crumbs", "CRUMBS") {
        owner = msg.sender;
        _mint(msg.sender, 2000 * (10 ** 18));
        rewardsVar = 100000;
    }
    
    event Staked(address indexed user, uint256 amount, uint256 totalCockStaked);
    ...
    modifier updateStakingReward(address account) {
        if (block.number > _stakedBalances[account].lastBlockChecked) {
            ...
            if (_stakedBalances[account].cockStaked > 0) {
                _stakedBalances[account].rewards = _stakedBalances[account].rewards
                                                                            .add(
                                                                            _stakedBalances[account].cockStaked
                                                                            .mul(rewardBlocks)
                                                                            / rewardsVar);
            }
    ...
    function setCockAddress(address _cockAddress) public _onlyOwner returns(uint256) {
        cockAddress = _cockAddress;
        cockToken = IERC20(_cockAddress);
    }
    ...
    function getAddressStakeAmount(address _account) public view returns (uint256) {
        return _stakedBalances[_account].cockStaked;
    }
    ...
    function stake(uint256 amount) public updateStakingReward(msg.sender) {
        _totalCockStaked = _totalCockStaked.add(amount);
        _stakedBalances[msg.sender].cockStaked = _stakedBalances[msg.sender].cockStaked.add(amount);
        cockToken.safeTransferFrom(msg.sender, address(this), amount);
        emit Staked(msg.sender, amount, _totalCockStaked);
    }

    function withdraw(uint256 amount) public updateStakingReward(msg.sender) {
        _totalCockStaked = _totalCockStaked.sub(amount);
        _stakedBalances[msg.sender].cockStaked = _stakedBalances[msg.sender].cockStaked.sub(amount);
        cockToken.safeTransfer(msg.sender, amount);
   ...
   function getReward() public updateStakingReward(msg.sender) {
       ...
       _mint(cockAddress, fundingPoolReward);
