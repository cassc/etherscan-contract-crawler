...
contract Nakadoodles is ERC721A, Ownable, ReentrancyGuard {

    using Strings for uint256;

    mapping(address => uint) public mintedPerAddress;
    uint256 public mintCost = 0.00321 ether;
    uint256 public maxSupply = 5000;
    uint256 public maxPerTXN = 10;
    string internal baseUri = "ipfs://bafybeibcdke2w4z33xvnek6t7v7ycez3efxwbepmucp3nu6oyjwh2z2siu/";
    bool private saleStarted = false;



    constructor() ERC721A("Nakadoodles", "NKDDLS") {}
    ...
    function mintModifier(uint256 _amount) internal {
        ...
        require(msg.value >= mintCost * (_amount - free), "1 Free, rest is 0.00321 ETH.");
        mintedPerAddress[msg.sender] += _amount;
        sendMint(_msgSender(), _amount);
    }

    function sendMint(address _wallet, uint256 _amount) internal {
        require(_amount + totalSupply() <= maxSupply, "Zero Supply Left.");
