...
contract POCOPresale is Context, AccessControlEnumerable, ReentrancyGuard, Pausable {
    ...
    event FixUserInfoEvent(address _userAddr, uint256 _amount);
    ...
    */
    ...
    function saleByToken(address _tokenAddress, uint256 _amount) public nonReentrant whenNotPaused isNotContract {
        ...
        USDTInterface token = USDTInterface(_tokenAddress);
        token.transferFrom(_msgSender(), address(this), pay_amount);
    ...
    function saleByToken2(address _tokenAddress, uint256 _amount) public nonReentrant whenNotPaused isNotContract {
        uint256 currentTime = block.timestamp;
        require(currentTime >= _saleStartTime, "sale not start");
        require(currentTime < _saleEndTime, "sale already ended");
        require(_amount >= 10000 * 10**18, "invalid sale amount");
    ...
    */
    ...
    function claim() external nonReentrant whenNotPaused isNotContract {
        ...
        require(_releasePhases.length > 0, "release not start");
    ...
    */
    ...
    function fixUserInfo(address[] memory _userAddrs, uint256[] memory _amounts) external hasAdminRole {
        require(_userAddrs.length == _amounts.length, "invalid parameters");
        for (uint256 i = 0; i < _userAddrs.length; i++) {
            setUserInfo(_userAddrs[i], _amounts[i]);
            emit FixUserInfoEvent(_userAddrs[i], _amounts[i]);
        }
    }
    ...
    function withdraw2(address _tokenAddress) external hasAdminRole {
        ERC20 token = ERC20(_tokenAddress);
