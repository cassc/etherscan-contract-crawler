...
// Root file: contracts/WorldMillionaireChallengeGame.sol
...
contract WorldMillionaireChallengeGame is
    Initializable,
    OwnableUpgradeable,
    ReentrancyGuardUpgradeable
{
    ...
    function initialize() public initializer {
        ...
        token = IERC20Upgradeable(0xE23Ad4e00D2Fb9827761F4A3A0958191ab22721f);
    }
    ...
    function getAccount(
        ...
        require(accountId > 0 && accountId <= _accounts.length, "Wrong ID");
        account = _accounts[accountId - 1];
    }
    ...
    {
        accountId = _accountData[account].id;
        deposited = _accountData[account].deposited;
        claimed = _accountData[account].claimed;
        win = _accountData[account].win;
    }

    function deposit(uint256 amount) external {
        require(amount >= 90 * 10 ** 6, "Min amount");

        usdt.safeTransferFrom(_msgSender(), address(this), amount);
        _totalDeposited += amount;
        _accountData[_msgSender()].deposited += amount;

        uint256 accountId = getAccountId(_msgSender());
        if (accountId == 0) {
            _accounts.push(_msgSender());
            accountId = _accounts.length;
            _accountData[_msgSender()].id = accountId;
            emit NewAccount(_msgSender(), accountId, block.timestamp);
        }
        emit Deposited(_msgSender(), accountId, amount, block.timestamp);
    }
    ...
    function _checkSignature(
        ...
        require(
            _getSigner(day, account, amount, signature) == signer,
            "Not authorized"
        );
    }
    ...
    function _getSigner(
        ...
        return
            ECDSAUpgradeable.recover(
                keccak256(
                    abi.encode(chainId, address(this), day, account, amount)
                ),
                signature
            );
    }
    ...
    function claim(
        ...
        _checkSignature(day, account, amount, signature);
        require(winner[day] == address(0), "Already claimed");

        _totalClaimed += amount;
        _accountData[account].claimed += amount;
        _accountData[account].win++;
        winner[day] = account;

        usdt.safeTransfer(account, amount);
        emit Claimed(account, getAccountId(account), amount, block.timestamp);
    }

    function setNft(address nft_) external onlyOwner {
        require(address(nft) == address(0), "NFT already installed");
        nft = INFT(nft_);
    }

    function claimNft() external {
        require(address(nft) != address(0), "NFT not installed");
        require(nft.balanceOf(_msgSender()) == 0, "NFT already claimed");

        uint256 accountId = getAccountId(_msgSender());
        require(accountId != 0, "Not registered");

        nft.mint(_msgSender(), accountId);
        emit NftClaimed(_msgSender(), accountId, block.timestamp);
    }
}