...
contract PepeAIClub is ERC721A, Ownable, ERC2981, DefaultOperatorFilterer {
    using Strings for uint256;
    ...
    bool public preSale_mint_status = true;
    ...
    uint256 public total_preSale_limit = 10000;
    uint256 public total_presale_count;   

    uint256 public max_per_wallet = 100;  
    uint256 public max_preSale_per_wallet = 100;   
    uint256 public whitelistCost = 0;
    uint256 public wl_per_wallet = 200;    
    uint256 public total_wl_limit = 10000;
    ...
    constructor(string memory _initBaseURI, string memory _initNotRevealedUri, string memory _contractURI) ERC721A("Pepe AI Club", "PAC") {
    ...
    function preSalemint(uint256 quantity) public payable  {
        ...
        if (msg.sender != owner()) {
            ...
            require(preSale_mint_status, "Presale Mint Status is Off");        
            require(preSaleMinted[msg.sender] + quantity <= max_preSale_per_wallet, "Presale Limit per Wallet Reached");        
            require(balanceOf(msg.sender) + quantity <= max_per_wallet, "Per Wallet Limit Reached"); 
            require(total_presale_count + quantity <= total_preSale_limit, "Presale Limit Reached");
            require(msg.value >= (preSaleCost * quantity), "Not Enough ETH Sent");                       
        }
    ...
    function toggle_preSale_mint_status() external onlyOwner {
        
        if(preSale_mint_status==false){
            preSale_mint_status = true;
        }else{
            preSale_mint_status = false;
        }
    } 
    ...
    function setMax_preSale_per_wallet(uint256 _max_preSale_per_wallet) external onlyOwner {
        max_preSale_per_wallet = _max_preSale_per_wallet;
    }

    function setTotal_preSale_limit(uint256 _total_preSale_limit) external onlyOwner {
