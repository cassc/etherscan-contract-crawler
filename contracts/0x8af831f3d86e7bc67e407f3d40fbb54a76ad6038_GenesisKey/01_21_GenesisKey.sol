...
contract GenesisKey is Initializable, ERC721AUpgradeable, ReentrancyGuardUpgradeable, UUPSUpgradeable {
    ...
    function batchTransfer(uint256[] calldata tokenIds, address[] calldata _to) external {
        require(tokenIds.length == _to.length, "GEN_KEY: INVALID_LENGTH");
        for (uint256 i = 0; i < tokenIds.length;) {
            if (_genesisKeyLockUp[tokenIds[i]].currentLockup != 0) revert PausedTransfer();
            _transfer(msg.sender, _to[i], tokenIds[i]);
            unchecked { i++; }
        }
    }
