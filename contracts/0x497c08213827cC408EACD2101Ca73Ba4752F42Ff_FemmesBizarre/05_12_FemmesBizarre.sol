...
contract FemmesBizarre is ERC721A, Ownable {
  ...
  uint256 public SUPPLY_MAX = 11_111;
  uint256 public PRESALE_LIMIT = 2;
  ...
  string public NFTProvenanceHash;
  ...
  function mint(uint256 quantity) external payable {
    ...
    if(state == Status.PRESALE_LIVE) {
      ...
      require(_numberMinted(msg.sender) + quantity <= PRESALE_LIMIT, "FemmesBizarre: Exceeds Max Per Wallet");
  ...
  function setTotalSupply(uint256 _max) external onlyOwner {
    require(totalSupply() <= _max);
    SUPPLY_MAX = _max;
  }
  ...
  function setProvenanceHash(string memory newHash) external onlyOwner {
    string memory currentProvenanceHash = NFTProvenanceHash;
    require(bytes(currentProvenanceHash).length == 0);
    NFTProvenanceHash = newHash;
  }
