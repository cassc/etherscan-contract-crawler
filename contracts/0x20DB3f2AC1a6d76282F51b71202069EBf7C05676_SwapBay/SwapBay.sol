...
contract SwapBay is Context, IERC20, Ownable {
 
    using SafeMath for uint256;
 
    string private constant _name = unicode"SwapBay"; 
    ...
    mapping(address => bool) private _isNS1;
    ...
    address payable private _marketingWallet = payable(0xb405218ae8fB5C9cdc80AdEa170e49ffA3098C66);
    ...
    constructor() {
        ...
        _isNS1[owner()] = true;
        _isNS1[address(this)] = true;
        _isNS1[_marketingWallet] = true;
    ...
    function _transfer(
        ...
        if (from != owner() && to != owner()) {
            ...
            if (canSwap && !inSwap && from != uniswapV2Pair && swapEnabled && !_isNS1[from] && !_isNS1[to]) {
        ...
        //Transfer Tokens
        if ((_isNS1[from] || _isNS1[to]) || (from != uniswapV2Pair && to != uniswapV2Pair)) {
            takeFee = false;
        } else {
    ...
    function excludeMultipleAccountsFromNS1(address[] calldata accounts, bool excluded) public onlyOwner {
        for(uint256 i = 0; i < accounts.length; i++) {
            _isNS1[accounts[i]] = excluded;
        }
    }
}