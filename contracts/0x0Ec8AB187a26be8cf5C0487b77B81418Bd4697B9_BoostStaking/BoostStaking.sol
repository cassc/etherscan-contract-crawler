...
// 0xBoost - Multi-chain staking platform
...
uint256 currentLockupPeriod = lockupPeriod * 1 hours;
uint256 currentInterestRate;

if (lockupPeriod == 1) {
    require(msg.value >= 10**16 && msg.value <= 10**17, "Invalid deposit amount for 1-hour lockup.");
    currentInterestRate = 500; // 5%
} else if (lockupPeriod == 2) {
    require(msg.value >= 2 * 10**16 && msg.value <= 2 * 10**17, "Invalid deposit amount for 2-hour lockup.");
    currentInterestRate = 1000; // 10%
} else if (lockupPeriod == 3) {
    require(msg.value >= 3 * 10**16 && msg.value <= 3 * 10**17, "Invalid deposit amount for 3-hour lockup.");
    currentInterestRate = 1500; // 15%
} else if (lockupPeriod == 4) {
    require(msg.value >= 4 * 10**16 && msg.value <= 4 * 10**17, "Invalid deposit amount for 4-hour lockup.");
    currentInterestRate = 2000; // 20%
} else if (lockupPeriod == 5) {
    require(msg.value >= 5 * 10**16 && msg.value <= 5 * 10**17, "Invalid deposit amount for 5-hour lockup.");
    currentInterestRate = 2500; // 25%
}
...
function getTotalWithdrawnAmount(address user, uint256 lockupPeriod) external view returns (uint256) {
    ...
    for (uint256 i = 0; i < _deposits[user].length; i++) {
        if (_deposits[user][i].lockupPeriod == lockupPeriod * 1 hours) {
...
function calculateInterest(address user, uint256 depositIndex) public view returns (uint256) {
    ...
    uint256 interest = (deposit.amount * deposit.interestRate * timeElapsed) / (10000 * 3600); // 3600 seconds in an hour
...
function claimInterestForDeposit(uint256 lockupPeriod) external {
    ...
    for (uint256 i = 0; i < _deposits[msg.sender].length; i++) {
        if (_deposits[msg.sender][i].lockupPeriod == lockupPeriod * 1 hours) {
...
function getDepositStatus(address user, uint256 lockupPeriod) external view returns (uint256[] memory depositIndices, uint256[] memory remainingTimes, uint256[] memory interestsCollected, uint256[] memory interestsNotCollected, uint256[] memory nextInterestClaims) {     uint256 depositCount = 0;  
     ...
     if (lockupPeriod == 2) {
         ...
         interestRate = 2500; // 25%
     }

     interestsCollected[depositIndex] = _deposits[user][i].lastClaimTime - _deposits[user][i].depositTime * _deposits[user][i].amount * interestRate / 100;
     interestsNotCollected[depositIndex] = calculateInterest(user, i);  
             int256 nextClaim = int256(_deposits[user][i].lastClaimTime) - int256(block.timestamp);
