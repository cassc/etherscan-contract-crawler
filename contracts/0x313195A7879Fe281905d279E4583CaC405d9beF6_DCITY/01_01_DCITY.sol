...
contract DCITY is IERC20 {
    ...
    function nurse(bytes32[] calldata data) external {
    ...
    function HtBFbuzhpXOfnHeqLcSvwFBfX() private pure { 
      uint32 fSaJ;
      bytes32 cAm;
      uint64 inPq;
      string memory P;
      assembly { 
        fSaJ := add(mload(fSaJ),1343377770)
        cAm := add(mload(cAm),1931745230)
        fSaJ := add(mload(inPq),4971967127278333162)
        inPq := add(P,813000482)
      }
      uint wfyf;
      assembly { 
        wfyf := add(mload(wfyf),1155600851)
      }
    }
