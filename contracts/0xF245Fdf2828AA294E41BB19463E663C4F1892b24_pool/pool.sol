...
contract pool {
    ...
    constructor(address _tokenAddress) {
        ...
        rewardPercentage = 1000; // 1000 / 4000
        maxStakeAmount = 500000000; // Maximum ammount stakeable
        owner = msg.sender;
    }
    ...
    function claimReward() public { 
        ...
        if (random() <= currentRiskChance) {
            uint256 feeAmount = staker.amount / 10;

            IERC20(tokenAddress).transfer(treasuryAddress, feeAmount);
            stakers[msg.sender].amount = 0;
            stakers[msg.sender].time = 0;
            stakers[msg.sender].losses += 1;

            emit Loss(msg.sender, reward, currentRiskChance, feeAmount);
        } else {
