...
contract DeFiDrivers is ERC721A, Ownable, ReentrancyGuard, DefaultOperatorFilterer   {


  string public baseURI;
  string public notRevealedUri = "ipfs://bafkreigyehthzpiemmsym5jbectvzhfrbcqurwo4pgzyfbqzhb6guw2ouu/";
  ...
  constructor() ERC721A("DeFi Drivers", "DEFIDRIVERS") {}
  ...
  function mint(uint256 tokens) public payable nonReentrant {
    require(!paused, "DEFIDRIVERS: oops contract is paused");
    require(!preSale, "DEFIDRIVERS: Sale Hasn't started yet");
    require(tokens <= MaxperWallet, "DEFIDRIVERS: max mint amount per tx exceeded");
    require(totalSupply() + tokens <= maxSupply, "DEFIDRIVERS: We Soldout");
    require(PublicMintofUser[_msgSenderERC721A()] + tokens <= MaxperWallet, "DEFIDRIVERS: Max NFT Per Wallet exceeded");
    require(msg.value >= cost * tokens, "DEFIDRIVERS: insufficient funds");
...
/// @dev presale mint for whitelisted
    ...
    require(!paused, "DEFIDRIVERS: oops contract is paused");
    require(preSale, "DEFIDRIVERS: Presale Hasn't started yet");
    require(MerkleProof.verify(merkleProof, merkleRoot, keccak256(abi.encodePacked(msg.sender))), "DEFIDRIVERS: You are not Whitelisted");
    require(WhitelistedMintofUser[_msgSenderERC721A()] + tokens <= MaxperWalletWl, "DEFIDRIVERS: Max NFT Per Wallet exceeded");
    require(tokens <= MaxperWalletWl, "DEFIDRIVERS: max mint per Tx exceeded");
    require(totalSupply() + tokens <= maxSupply, "DEFIDRIVERS: Whitelist MaxSupply exceeded");
    require(msg.value >= wlcost * tokens, "DEFIDRIVERS: insufficient funds");
  ...
  /// @dev use it for giveaway and team mint
    ...
    require(totalSupply() + _mintAmount <= maxSupply, "max NFT limit exceeded");
      ...
      _safeMint(destination, _mintAmount);
       ...
       function bulkAirdrop(uint256 _mintAmount, address[] calldata destination) public onlyOwner nonReentrant {
    require(totalSupply() + _mintAmount <= maxSupply, "max NFT limit exceeded");
