...
// File: contracts/KartParty_v2.sol
...
contract Test0910v2 is Initializable, ERC721Upgradeable, IERC2981Upgradeable, OwnableUpgradeable, UUPSUpgradeable {

    using StringsUpgradeable for uint256;

    uint256 public numToken;
    uint256 public constant totalSupply = 7;
    uint256 public constant wlSupply = 5;
    ...
    uint256 public status;  // 0 - unavailable, 1 - running
    ...
    //event => ( tokenId=> lockState), lockState: 0-unlock, 1-lock
    ...
    function initialize() initializer public {
        __ERC721_init("Test0910v2-YKZ", "YKZ");
        __Ownable_init();
    ...
    modifier isSufficient() {
        require(_mintTokenId < wlSupply, "Test0910v2: Max whitelist supply reached");
        require(numToken < totalSupply, "Test0910v2: Max total supply reached");
        _;
    }

    modifier isRealWallet() {
        require(msg.sender == tx.origin, "Test0910v2: Must from real wallet address");
        _;
    }

    modifier checkStatus(uint256 _status) {
        require(status == _status, "Test0910v2: Not correct status");
        _;
    }
    ...
    function setBaseURI(string memory _uri) public onlyOwner {
        require(bytes(_uri).length > 0, "Test0910v2: wrong base uri");
        _buri = _uri;
    }
    ...
    function setTreasuryAddress(address _address) public onlyOwner {
        require(_address != address(0), "Test0910v2: treasury must not be zero address");
        ...
        checkStatus(1) 
    {
        ...
        require(wlMinted(addr) == false, "Test0910v2: Already minted whitelist address");
        require(_verifyProof(addr, _proof), "Test0910v2: Invalid proof for whitelist");
    ...
    function treasuryMint(address _treasury) external onlyOwner {
        require(_treasury != address(0), "Test0910v2: Invalid treasury address");
        require(_treasury == treasuryAddress, "Test0910v2: inconsistent treasury address");
        require(_treasuryMinted == false, "Test0910v2: Already airdrop to treasury");
    ...
    function lockTokenBatch(string calldata _event, uint256[] calldata _tokenIds) external {
        require(_lockEventState[_event] == 1, "Test0910v2: Event not open");
        require(_tokenIds.length > 0, "Test0910v2: BatchSize too small");
        require(_tokenIds.length <= 50, "BatchSize exceeds");
        for(uint256 i = 0; i<_tokenIds.length; i++) {
            uint256 _tokenId = _tokenIds[i];
            require(_msgSender() == ownerOf(_tokenId), "Test0910v2: Caller must be owner of token");
            require(_eventTokenLock[_event][_tokenId] == 0, "Test0910v2: Token already locked by current event");
    ...
    function unlockTokenBatch(string calldata _event, uint256[] calldata _tokenIds) external {
        require(_tokenIds.length > 0, "Test0910v2: BatchSize too small");
        require(_tokenIds.length <= 50, "Test0910v2: BatchSize exceeds");
        for(uint256 i = 0; i<_tokenIds.length; i++) {
            uint256 _tokenId = _tokenIds[i];
            require(_msgSender() == ownerOf(_tokenId), "Test0910v2: Caller must be owner of token");
            require(_eventTokenLock[_event][_tokenId] == 1, "Test0910v2: Token not locked by current event");
            _eventTokenLock[_event][_tokenId] = 0;
            require(_lockCounts[_tokenId] >= 1, "Test0910v2: Lock counts error");
            _lockCounts[_tokenId] -= 1;
        }
    ...
    function enumerateTokens() external onlyOwner {
        require(_enumerated == false, "Test0910v2: Already enumerate tokens");

        _enumerated = true;
        for (uint256 tokenId = 1; tokenId <= wlSupply; tokenId++) {
            address addr = _ownerOf(tokenId);
            if (addr != address(0)) {
                uint256 idx = 0;
                while (_ownedTokens[addr][idx] != 0) {
                    idx++;
                }
                _ownedTokens[addr][idx] = tokenId;
                _ownedTokensIndex[tokenId] = idx;
            }
        }
        uint256 treasuryNum = totalSupply - wlSupply;
        for (uint256 idx = 0; idx < treasuryNum; idx++) {
            uint256 tokenId = wlSupply + idx + 1;
            _ownedTokens[treasuryAddress][idx] = tokenId;
            _ownedTokensIndex[tokenId] = idx;
        }
    }

    function ownTokens(address _address, string calldata _event) public view returns(uint256[] memory, uint256[] memory) {
        uint256 num = balanceOf(_address);
        uint256[] memory _tokens = new uint256[](num);
        uint256[] memory _states = new uint256[](num);
        for (uint256 i; i < num; i++) {
            _tokens[i] = _ownedTokens[_address][i];
            _states[i] = _eventTokenLock[_event][_tokens[i]];
        }
        return (_tokens, _states);
    }
    ...
    function _beforeTokenTransfer(address from, address to, uint256 tokenId, uint256 batchSize)
        ...
        override
    {
        require (_lockCounts[tokenId] == 0, "Test0910v2: Token locked by event"); // locked
        super._beforeTokenTransfer(from, to, tokenId, batchSize);

        if (from != to) {
            uint256 fromLastTokenIndex = balanceOf(from) - 1;
            uint256 toTokenLength = balanceOf(to);
            uint256 tokenIndex = _ownedTokensIndex[tokenId];

            if (tokenIndex != fromLastTokenIndex) {
                uint256 lastTokenId = _ownedTokens[from][fromLastTokenIndex];

                _ownedTokens[from][tokenIndex] = lastTokenId; // Move the last token to the slot of the to-delete token
                _ownedTokensIndex[lastTokenId] = tokenIndex; // Update the moved token's index
            }

            delete _ownedTokens[from][fromLastTokenIndex];
            _ownedTokens[to][toTokenLength] = tokenId;
            _ownedTokensIndex[tokenId] = toTokenLength;
        }
    }
}