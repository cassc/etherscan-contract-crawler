...
contract Elevated_Yacht_Club is ERC1155, ERC1155Supply, ReentrancyGuard, Ownable {
    ...
    uint256[] public presalePrice = [2.5 ether,0.45 ether]; //Presale price
    uint256[] public mintPrice = [3.5 ether,0.65 ether]; //Mint price
    uint256[] public currentPrice=[2.5 ether,0.45 ether]; //Current price
    ...
    constructor() ERC1155("ipfs://QmRMKSSTFcLFGYvfHVVwg1WpL6CJmi7zrjxBgBzwwtmGP7/{id}.json"){
        _mint(msg.sender, FOUNDER, 50, "");
        _mint(msg.sender, VIP, 100, "");
    }
    ...
    function Mint(address account, uint256 id, uint256 amount) external payable nonReentrant {
        ...
        require(totalSupply(id) < maxSupply[id], "Sold out!");
        require(totalSupply(id) + amount <= maxSupply[id], "Can not Mint that many!");
        require(currentPrice[id] * amount <= msg.value, "Not enough ethers sent!");
        if(id==0){
            require(addressClaimedFounder[account]+amount <= maxMintAmount, "Already Minted or Minting more than allowed!");
            addressClaimedFounder[account]=addressClaimedFounder[account]+amount;
        } else if(id==1){
            require(addressClaimedVIP[account]+amount <= maxMintAmount, "Already Minted or Minting more than allowed!");
