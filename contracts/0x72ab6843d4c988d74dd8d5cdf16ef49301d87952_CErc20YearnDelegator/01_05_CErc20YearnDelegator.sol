...
/**
 * @title Compound's CErc20YearnDelegator Contract
 * @notice CTokens which wrap an EIP-20 underlying and delegate to an implementation
 * @author Compound
 */
contract CErc20YearnDelegator is CTokenInterface, CErc20YearnInterface, CDelegatorInterface {
    /**
    ...
    /**
     * @notice Sender supplies assets into the market and user receives cTokens in exchange
     * @dev Accrues interest whether or not the operation succeeds, unless reverted
     * @param mintAmount The amount of the underlying asset to supply
     * @param user The address of the account receiving the cTokens
     * @return uint 0=success, otherwise a failure (see ErrorReporter.sol for details)
     */
    function mintTo(uint mintAmount, address user) override external returns (uint) {
        bytes memory data = delegateToImplementation(abi.encodeWithSignature("mintTo(uint256,address)", mintAmount, user));
        return abi.decode(data, (uint));
    }
     ...
     * @return uint 0=success, otherwise a failure (see ErrorReporter.sol for details)
     */
    function redeemAndUnwrap(uint redeemTokens) override external returns (uint) {
        bytes memory data = delegateToImplementation(abi.encodeWithSignature("redeemAndUnwrap(uint256)", redeemTokens));
        return abi.decode(data, (uint));
    }
    ...
    /**
      ...
      * @return uint 0=success, otherwise a failure (see ErrorReporter.sol for details)
      */
    function borrowUnderlying(uint borrowAmount) override external returns (uint) {
        bytes memory data = delegateToImplementation(abi.encodeWithSignature("borrowUnderlying(uint256)", borrowAmount));
        return abi.decode(data, (uint));
    }
