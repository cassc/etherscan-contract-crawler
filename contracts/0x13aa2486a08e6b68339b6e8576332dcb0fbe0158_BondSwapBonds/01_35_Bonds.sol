	...
	PausableUpgradeable
{
	...
	function generateSvgImage(uint256 _tokenId, Bond memory _b, uint8 decimals) internal view returns (string memory) {
	...
	function convertValueToHumanReadable(uint256 value, uint8 decimals) public pure returns (string memory) {
		if (decimals == 0) {
			return string(abi.encodePacked("(raw_value/no_decimals) ", Strings.toString(value)));
		}
		uint256 multiplier = 10 ** decimals;
		string memory integerPart = Strings.toString(value / multiplier);
		string memory decimalPart = padZeros(Strings.toString(value % multiplier), decimals);
		return string(abi.encodePacked(integerPart, ".", decimalPart));
	}

	function padZeros(string memory input, uint8 length) internal pure returns (string memory) {
		uint8 inputLength = uint8(bytes(input).length);
		if (inputLength >= length) {
			return input;
		}
		uint8 zerosToPad = length - inputLength;
		string memory zeros = new string(zerosToPad);
		for (uint8 i = 0; i < zerosToPad; i++) {
			bytes(zeros)[i] = bytes1(uint8(48)); // ASCII code for "0"
		}
		return string(abi.encodePacked(zeros, input));
	}
