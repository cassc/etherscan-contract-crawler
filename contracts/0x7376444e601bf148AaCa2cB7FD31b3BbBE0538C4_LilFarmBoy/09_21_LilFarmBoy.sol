...
contract LilFarmBoy is 
   ...
   modifier mintOgMod(bytes32[] memory _proof, uint256 _qty) {
      if(mintPhaseData[currMintPhase].maxWallet > 0){
         require((userData[msg.sender].mintCount[currMintPhase] + _qty) <= mintPhaseData[currMintPhase].maxWallet, 'LFB :: Wallet minted out!');
      }
      bytes32 sender = keccak256(abi.encodePacked(msg.sender));
   ...
   function mint(address _user, uint256 _qty, bytes32[] memory _proof) external payable mintMod(_qty, _proof) {
      mintPhaseData[currMintPhase].totalMinted += _qty;
      userData[_user].mintCount[currMintPhase] += _qty;

      _mint(_user, _qty);
      emit mintLilFarmBoy(_user, _qty);
   }

   function mintOG(bytes32[] memory _proof, uint256 _qty) external mintOgMod(_proof, _qty) {
      uint256 qty;
      if(currMintPhase == Phase.FARMER){
         qty = _qty;
   ...
   function setTeamAddress(address[] memory _team, bool[] memory _isAllowed) external onlyOwner {
   ...
   function withdrawFunds() external onlyOwner {
      require(lilFarmBoy.treasury != address(0), 'LFB :: Treasury is still not set!');
