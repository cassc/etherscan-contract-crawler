                        ...
                        Versioned 
{
    ...
    /**
     * @dev Operator filter related overrides
     */
    function setApprovalForAll(address operator, bool approved) 
        virtual
        public 
        override 
        onlyAllowedOperatorApproval(operator, approved) 
    {
    ...
    /**
     * @dev Allows calling any function in the registry contract
     *
     * To encode the call data, use the following
     * const registryContract = new ethers.Contract(address, abi)
     * const data = registryContract.interface.encodeFunctionData(<function name>, [<function params>])
     *
     * Then call the NFT contract from admin:
     * nftContract.adminCallOperatorFilterRegistry(data)
     */
    function adminCallOperatorFilterRegistry(bytes calldata data)
        public 
        onlyRole(DEFAULT_ADMIN_ROLE) 
        whenNotPaused 
    {
        (bool success, bytes memory result) = address(OPERATOR_FILTER_REGISTRY).call(data);
        if (!success) { 
            if (result.length == 0) {
                revert();
            }
            assembly {
                // revert data, data size                
