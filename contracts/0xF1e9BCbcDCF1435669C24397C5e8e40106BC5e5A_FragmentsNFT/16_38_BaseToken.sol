 ...
 */
contract BaseToken is IMintableToken,
                      ...
                      Versioned 
{
    ...
    /**
     * @dev Batch-mints `amounts` tokens to `accounts`, requires `MINTER_ROLE`
     */
    function batchMint(address[] calldata accounts, uint256[] calldata amounts) 
        external
        onlyRole(MINTER_ROLE) 
    {
        require(accounts.length == amounts.length, "mismatched length");
