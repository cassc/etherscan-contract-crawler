...
abstract contract StrategyCurveBase is BaseStrategy {
    ...
    constructor(
        address _vault,
        uint256[3] memory _params, // maxSingleInvest, minTimePerInvest, slippageProtectionIn(Out)
        address _curveToken,
        address _curvePool,
        address _yvToken,
        ...
        _initializeStrat(_params, _curveToken, _curvePool, _yvToken, _uniswapRouter, strategyName);
    }
    ...
    function _initializeStrat(
        ...
        maxSingleInvest = _params[0];
        minTimePerInvest = _params[1];
        slippageProtectionIn = _params[2];
        slippageProtectionOut = _params[2];
    ...
    function _setupBaseStatics() internal {
    ...
    function _adjustPosition(uint256 _debtOutstanding) internal override {
        ...
        _addLiquidity(_wantToInvest, maxSlip);
    ...
    function _addLiquidity(uint256 investAmount, uint256 maxSlippage) internal virtual;

    function _withdrawLiquidity(uint256 withdrawAmount, uint256 maxSlippage) internal virtual;
    ...
    function _withdrawSome(uint256 _amount) internal returns (uint256 _liquidatedAmount, uint256 _loss) {
        ...
        if (toWithdraw > 0) {
            ...
            _withdrawLiquidity(toWithdraw, maxSlippage);
        }
    ...
    function _prepareMigration(address _newStrategy) internal virtual override {
