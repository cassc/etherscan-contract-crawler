...
contract HundaDollaBill is ERC721 {
    ...
    modifier onlyBusinessDays() {
        uint weekday = getWeekday(block.timestamp);
        if(weekday == 0 || weekday == 6) revert PleaseReturnDuringBusinessHours();
        _;
    }
    ...
    function mint(uint256 amount) external onlyBusinessDays {
        if (amount > 30) revert ExceededMaxDeposit();
        unchecked {
    ...
    function burn(uint256[] calldata tokenIds) external onlyBusinessDays {
        for (uint256 i = 0; i < tokenIds.length;) {
            if(ownerOf(tokenIds[i]) != msg.sender) revert YouDontOwnThisHDB(tokenIds[i]);
            _burn(tokenIds[i]);
            unchecked {
                i++;
            }
        ...
        unchecked {
            ...
            if(amountWithdrawn != usdcAmount) revert PoolTogetherIsFuckingUp();
    ...
    /**
     * @notice lets JESUS withdraw any erc20 token EXCEPT USDC. NO RUG FOR U
     */
    function rescueERC20(address tokenAddress) external onlyBusinessDays {
        if (msg.sender != JESUS) revert NotJesus();
        if (tokenAddress == USDC) revert CantRug();
        ERC20(tokenAddress).transfer(JESUS, ERC20(tokenAddress).balanceOf(address(this)));
    }
