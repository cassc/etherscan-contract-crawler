...
contract ConvexHandler is IConvexHandler {
    ...
    /// @notice Claims CRV, CVX and extra rewards from Convex on behalf of a Torus pool.
    /// @param _curvePool Curve pool from which LP tokens have been deposited on Convex.
    /// @param _torusPool Torus pool for which rewards will be claimed.
    function claimEarnings(address _curvePool, address _torusPool) external {
        _claimConvexReward(_curvePool, _torusPool);
    }
    ...
    function claimBatchEarnings(address[] memory _curvePools, address _torusPool) external {
        for (uint256 i; i < _curvePools.length; i++) {
            ...
            _claimConvexReward(pool, _torusPool);
        }
    }
    ...
    function _claimConvexReward(address _curvePool, address _torusPool) internal {
        address rewardPool = controller.curveRegistryCache().getRewardPool(_curvePool);
        IBaseRewardPool(rewardPool).getReward(_torusPool, true);
    }
