...
contract HLTCToken is IERC20Token,Pausable, HLTCAdmin{
    ...
    public whenNotPaused returns(bool){
        require(itemAddressExists(OPERATORHASH, msg.sender), "wrong operator");
        uint256 status = logic.burnLogic(from,value,ltcAddress,proof,taskHash, msg.sender, operatorRequireNum);
        if (status == 1){
           emit Burning(from, value, proof,ltcAddress, msg.sender);
        }else if (status == 3) {
           emit Burning(from, value, proof,ltcAddress,  msg.sender);
           emit Burned(from, value, proof,ltcAddress);
