...
contract AlloyxManager is IAlloyxManager, IAlloyx, AdminUpgradeable {
   ...
   */
  function depositForProposal(
    string calldata _proposalId,
    uint256 _usdcAmount,
    uint256 _alyxAmount
  ) external notPaused nonReentrant isWhitelisted {
    ...
    usdcPermanentStakerMap[_proposalId][msg.sender] = _usdcAmount;
    alyxPermanentStakerMap[_proposalId][msg.sender] = _alyxAmount;
  ...
  function withdrawFromProposal(string calldata _proposalId) public notPaused {
    if (allowWithdrawalMap[_proposalId] && permanentStakersMap[_proposalId].contains(msg.sender)) {
   ...
   * @param _proposalId the proposal ID
   */
  function setAllowWithdraw(string calldata _proposalId) external onlyAdmin notPaused {
    allowWithdrawalMap[_proposalId] = true;
  }
  ...
  function getComponents(string calldata _proposalId) external view returns (Component[] memory) {
    uint256 length = componentsMap[_proposalId].length;
    Component[] memory components = new Component[](length);
    for (uint256 i = 0; i < length; i++) {
      Component storage component = componentsMap[_proposalId][i];
      components[i] = Component(component.proportion, component.poolAddress, component.tranche, component.source);
    }
    return components;
  }
   ...
   */
  function setProposalConfiguration(string memory _proposalId, Component[] memory _components) external {
    EnumerableSet.AddressSet storage permanentStakers = permanentStakersMap[_proposalId];
    require(permanentStakers.contains(msg.sender) || isAdmin(msg.sender), "the caller must be creator or admin");
   ...
   */
  function startAlloyxVault(
    string calldata _proposalId,
    address _vaultAddress,
    address _govTokenAddress,
    address _govTimeLockAddress,
  ...
  /**
   * @notice Collect the protocol fee from all vaults, restricted to admin
   */
  function collectProtocolFee() external onlyAdmin {
    address[] memory addresses = vaultAddresses.values();
    for (uint256 i = 0; i < addresses.length; i++) {
      IAlloyxVault vault = IAlloyxVault(addresses[i]);
      vault.collectProtocolFee();
    }
  }

  /**
   * @notice Collect the protocol fee from one vault, restricted to admin
   * @param _vaultAddress the vault address to collect fee
   */
  function collectProtocolFee(address _vaultAddress) external onlyAdmin {
    IAlloyxVault vault = IAlloyxVault(_vaultAddress);
    vault.collectProtocolFee();
  }
