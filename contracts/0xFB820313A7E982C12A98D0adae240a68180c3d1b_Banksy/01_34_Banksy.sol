...
contract Banksy is ERC721AQueryable, Ownable, OperatorFilterer, ERC2981, IBanksy {
    ...
    constructor(address otherContract) ERC721A("Banksy Stop Esso", "BSE") {
    ...
    function tokenURI(uint256 tokenId) public view override(IERC721A, ERC721A) returns (string memory) {
        ...
        if (tokenStatus == 0) {
            return string(abi.encodePacked(banksy_uri, _toString(getTokenIDForMetadata(tokenId)), baseExtension));
        }
