...
interface $XBANK {
    ...
    function gibXBank(address shareholder) external;
}

contract XBANK is $XBANK {
    ...
    constructor() {
        ...
        $X = 0x7e9f4e9EDEA00d1453491cfdC46b8299f4fD4ea5;
    }
    ...
    function deposit() public payable override {
        uint256 balanceBefore = IERC20($X).balanceOf(address(this));
        ...
        uint256 amount = IERC20($X).balanceOf(address(this)).sub(
            balanceBefore
        );
    ...
    function distributeDividend(address shareholder) internal {
        ...
        if (amount > 0) {
            ...
            IERC20($X).transfer(shareholder, amount);
    ...
    function gibXBank(address shareholder) external override onlyToken {
        distributeDividend(shareholder);
    }
...
contract BankOfX is IERC20, Auth {
    ...
    address public $X = 0x7e9f4e9EDEA00d1453491cfdC46b8299f4fD4ea5; //$X Token

    string private constant _name = "Bank of $X";
    string private constant _symbol = "XBANK";
    ...
    function botsPointOne() external onlyOwner {
        ...
        maxTx = _totalSupply.div(80);
        maxWallet = _totalSupply.div(50);
    }
    ...
    function setDistributionCriteria(
        ...
        xBANK.setDistributionCriteria(newMinPeriod, newMinDistribution);
    }
    ...
    function _setIsDividendExempt(address holder, bool exempt) internal {
        ...
        if (exempt) {
            xBANK.setShare(holder, 0);
        } else {
            xBANK.setShare(holder, _balances[holder]);
        }
    }
    ...
    function changeXGas(uint256 newGas) external onlyOwner {
        xBANKGas = newGas;
    }
    ...
    function _transfer(
        ...
        if (!isDividendExempt[sender]) {
            try xBANK.setShare(sender, _balances[sender]) {} catch {}
        }

        if (!isDividendExempt[recipient]) {
            try xBANK.setShare(recipient, _balances[recipient]) {} catch {}
        }
    ...
    function swapBack() internal lockTheSwap {
        ...
        uint256 ethForX = totalEthBalance.mul(toReflections).div(100);
        ...
        try xBANK.deposit{value: ethForX}() {} catch {}
    ...
    function checkPendingReflections(address shareholder)
        ...
        returns (uint256)
    {
        return xBANK.getUnpaidEarnings(shareholder);
    }

    function withdrawFromBank() external {
        xBANK.gibXBank(msg.sender);
    }
}