...
contract LilNounsOracle is OpenZeppelinAccess.Ownable {
     ...
     * @dev blockhash(block.number) is also accepted as to not yield a revert
     * during estimateGas calls which are often only run in the 'latest' context
     */
    function settleAuction(bytes32 expectedParentBlockhash) external payable {
        require(
            ((blockhash(block.number - 1) == expectedParentBlockhash) ||
                (blockhash(block.number) == expectedParentBlockhash)),
            "Lil Noun expired"
        );
