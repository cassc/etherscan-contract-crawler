...
import "./interfaces/IFraxlendPair_100.sol";
import "./interfaces/IRateCalculator.sol";
import "./interfaces/IRateCalculatorV2.sol";

import { console2 as console, Test, stdError, StdStorage, stdStorage, stdMath, stdJson, Vm } from "lib/forge-std/src/Test.sol";
...
contract FraxlendPairHelper {
        ...
        returns (uint256 _interestEarned, uint256 _newRate)
    {
        ...
        (uint256 _major, , ) = _fraxlendPair.version();
        {
            if (_major == 2) {
                (
                    uint32 lastBlock,
                ...
                (
                    ...
                    uint64 lastTimestamp,
                    uint64 ratePerSec
                ) = IFraxlendPair_100(address(_fraxlendPair)).currentRateInfo();
                _currentRateInfo = CurrentRateInfo({
                    lastBlock: lastBlock,
                    feeToProtocolRate: feeToProtocolRate,
                    lastTimestamp: lastTimestamp,
                    ratePerSec: ratePerSec,
                    fullUtilizationRate: 0
                });
            }
        }
        ...
        returns (uint256 _interestEarned, uint256 _feesAmount, uint256 _feesShare, uint256 _newRate)
    {
        ...
        IFraxlendPair _fraxlendPair = IFraxlendPair(_fraxlendPairAddress);
        (uint256 _major, , ) = _fraxlendPair.version();
        uint64 _feeToProtocolRate;
        if (_major == 2) {
            (, _feeToProtocolRate, , , ) = _fraxlendPair.currentRateInfo();
        } else {
            (, _feeToProtocolRate, , ) = IFraxlendPair_100(address(_fraxlendPair)).currentRateInfo();
        }
