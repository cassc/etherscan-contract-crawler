 ...
 */
contract STRONGHOLD_LP_LOCK {
    using SafeMath for uint256;

    address public blurr;
    ...
    modifier onlyBlurr() {
        require(msg.sender == blurr, "Only blurr can call this function");
        _;
    }
    ...
    function lockNFT(uint256 tokenId) external onlyBlurr {
    ...
    function triggerWithdrawal() external onlyBlurr {
    ...
    function cancelWithdrawalTrigger() external onlyBlurr {
        require(isNFTLocked, "NFT is not locked");
    ...
    function withdrawNFT(uint256 tokenId) external onlyBlurr {
        require(isNFTLocked, "NFT is not locked");
        require(lockUpEndTime != 0, "Withdrawal is not triggered");
    ...
    function changeOwner(address newOwner) external onlyBlurr {
