...
abstract contract WildNFTRandom is IWildNFT, Ownable, RevokableDefaultOperatorFilterer, ERC721, ERC2981, PaymentSplitter {
    ...
    // used to be called randomMint but renamed for compatability
    function mint(address to) public onlyMinter returns (uint256) {
        require(_currentTokenId < maxSupply, 'Max supply reached');
        require(to != address(0), 'ERC721: mint to the zero address');

        uint256 tokenId = getRandomAvailableTokenId(to, maxSupply - _currentTokenId++);

        return _mintTo(to, tokenId);
    }

    function getRandomAvailableTokenId(address to, uint256 updatedNumAvailableTokens) internal returns (uint256) {
        uint256 randomNum = uint256(keccak256(abi.encode(to, tx.gasprice, block.number, block.timestamp, block.prevrandao, blockhash(block.number - 1), address(this), updatedNumAvailableTokens)));
    ...
    function getAvailableTokenAtIndex(uint256 indexToUse, uint256 updatedNumAvailableTokens) internal returns (uint256) {
