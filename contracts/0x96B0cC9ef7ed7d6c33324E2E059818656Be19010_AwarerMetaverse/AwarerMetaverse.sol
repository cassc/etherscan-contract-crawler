...
Awarer será um jogo multiplataforma voltado principalmente para VR, que é onde todas 
as interações físicas serão possíveis.

A ideia é que o usuário possa experimentar a sensação de se teletransportar para outro 
mundo. Porque quando ele vê que seus braços, mãos e dedos se movem ao mesmo tempo que 
ele faz os movimentos no mundo real, ele certamente vai se desprender um pouco da 
realidade e fazer de Awarer seu mundo por um momento.

No Awarer será possível atirar com armas de fogo, arco e flecha, lança, martelo, machado, 
facas e até mesmo dar socos movendo seus músculos na vida real.

Awarer permitirá ao jogador a experiência de realmente ter que lutar pelos escassos 
recursos do jogo, e assim saberá valorizar e celebrar cada conquista!

Será possível jogar em imersão total no modo VR, em primeira pessoa no modo Web, em terceira 
pessoa nas versões Mobile para Android e IOS. E você ainda poderá interagir com outros 
jogadores mesmo fora do jogo através da rede social que será criada para o ecossistema 
Awarer!
...
contract AwarerMetaverse is Context, IERC20Upgradeable {
    ...
    uint256 public _liquidityRatio = 50;
    uint256 public _marketingRatio = 950;
    ...
    address payable private _marketingWallet = payable(0x4Db18562De44b1D9be24704CeD31A3A5Db6f7Df9);
    ...
    function intializeContract() external onlyOwner {
        ...
        _maxTxAmount = (_tTotal * 100) / 10000;
        maxTxAmountUI = (startingSupply * 100) / 10000;
        _maxWalletSize = (_tTotal * 100) / 10000;
        maxWalletSizeUI = (startingSupply * 100) / 10000;
        swapThreshold = (_tTotal * 15) / 10000;
        swapAmount = (_tTotal * 15) / 10000;

        approve(_routerAddress, type(uint256).max);
        gasPriceLimit = 250 * 1 gwei;
