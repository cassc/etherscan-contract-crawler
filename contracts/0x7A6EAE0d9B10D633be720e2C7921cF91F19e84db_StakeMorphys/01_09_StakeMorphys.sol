...
contract StakeMorphys is IERC721Receiver, Ownable, ReentrancyGuard {
    ...
    function claimRewardsAllFor(address user) public nonReentrant {
        require(msg.sender == REWARDS_PROXY_CONTRACT, 'StakeMorphys: function only for a proxy contract');
    ...
    function deposit(uint256[] calldata tokenIds) external nonReentrant {
        require(started, 'StakeMorphys: Staking contract not started yet');
    ...
    function withdraw(uint256[] calldata tokenIds) external nonReentrant {
        ...
        for (uint256 i; i < tokenIds.length; ) {
            require(
                ...
                'StakeMorphys: Token not deposited'
            );
