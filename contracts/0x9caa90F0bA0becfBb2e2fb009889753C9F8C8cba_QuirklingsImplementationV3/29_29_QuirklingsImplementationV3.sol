...
contract QuirklingsImplementationV3 is
    OwnableUpgradeable,
    ERC721Upgradeable,
    ERC2981Upgradeable,
    DefaultOperatorFiltererUpgradeable
{
    ...
    function setStakingContract(
        ...
        QuirklingsV1Storage.layout().stakingContractsMap[
            stakingContract_
        ] = isStakingContract_;
    }
    ...
    function setStaked(uint256 tokenID_, bool staked_) external {
        ...
        if (staked_) {
            // check not staked
            if (
                QuirklingsV1Storage.layout().stakedMap[tokenID_] != address(0)
            ) {
                revert ErrStaked();
            }
    ...
    function _beforeTokenTransfer(
        ...
        if (
            QuirklingsV1Storage.layout().stakedMap[firstTokenId] != address(0)
        ) {
            revert ErrStaked();
        }
    }
