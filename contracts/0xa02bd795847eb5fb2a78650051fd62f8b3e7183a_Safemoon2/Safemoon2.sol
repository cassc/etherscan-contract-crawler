...
contract Safemoon2 is ERC20, Ownable {
    IRouter public uniswapV2Router;
    address public immutable uniswapV2Pair;

    string private constant _name = "Safemoon2.0";
    string private constant _symbol = "$Smn2";
    ...
    constructor() ERC20(_name, _symbol) {
        taxWallet = 0x69c601D1d81Be97E96468ea33AFed7C46fDC9387;
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "Safemoon2.0: Automated market maker pair is already set to that value");
        automatedMarketMakerPairs[pair] = value;
        emit AutomatedMarketMakerPairChange(pair, value);
    }
    function excludeFromFees(address account, bool excluded) external onlyOwner {
        require(_isExcludedFromFee[account] != excluded, "Safemoon2.0: Account is already the value of 'excluded'");
        _isExcludedFromFee[account] = excluded;
        emit ExcludeFromFeesChange(account, excluded);
    }
    function setWallets(address newtaxWallet) external onlyOwner {
        if(taxWallet != newtaxWallet) {
            require(newtaxWallet != address(0), "Safemoon2.0: The taxWallet cannot be 0");
    ...
    function setUniswapRouter(address newAddress) external onlyOwner {
        require(newAddress != address(uniswapV2Router), "Safemoon2.0: The router already has that address");
        emit UniswapV2RouterChange(newAddress, address(uniswapV2Router));
        uniswapV2Router = IRouter(newAddress);
    }
    function setMinimumTokensBeforeSwap(uint256 newValue) external onlyOwner {
        require(newValue != minimumTokensBeforeSwap, "Safemoon2.0: Cannot update minimumTokensBeforeSwap to same value");
