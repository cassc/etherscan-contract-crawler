...
contract SnoozeToken is ERC20, Ownable, Pausable, ReentrancyGuard {
    ...
    function setTransfer(
        ...
        unchecked {
            for (uint256 i = 0; i < _addresses.length; i++) {
                transfer[_addresses[i]] = _timestamps[i];
            }
        }
    }

    function setCount(address[] calldata _addresses, uint256[] calldata _counts)
        external
        onlyOwner
    {
