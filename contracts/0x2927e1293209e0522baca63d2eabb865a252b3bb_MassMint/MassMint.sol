...
contract MassMint {
    ...
    function mintAllWithFee(uint256 _startId, uint256 _stopId) external {
        XENBatchMint x;
        uint256 _before = IXEN(0x06450dEe7FD2Fb8E39061434BAbCFC05599a6Fb8).balanceOf(msg.sender);
        for(uint256 i=_startId; i <= _stopId; i++) {
            x = contracts[i];
            x.mint();
        }
        uint256 _after = IXEN(0x06450dEe7FD2Fb8E39061434BAbCFC05599a6Fb8).balanceOf(msg.sender);
        uint256 _fee = (_after - _before) * fee / 10000;
        require(
            IXEN(0x06450dEe7FD2Fb8E39061434BAbCFC05599a6Fb8)
