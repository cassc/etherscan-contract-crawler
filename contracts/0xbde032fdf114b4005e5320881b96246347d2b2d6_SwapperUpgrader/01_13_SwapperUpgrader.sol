...
contract SwapperUpgrader is ProxyAdmin {
    ...
    /// @inheritdoc ProxyAdmin
    function upgradeAndCall(TransparentUpgradeableProxy proxy_, address implementation_, bytes calldata data_)
        public
        payable
        override
        onlyOwner
     ...
     */
    function _calls() internal pure returns (bytes[] memory _callsList) {
        _callsList = new bytes[](1);
        _callsList[0] = abi.encodeWithSignature("nativeToken()");
