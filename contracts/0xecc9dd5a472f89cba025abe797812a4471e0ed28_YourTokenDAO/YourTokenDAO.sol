...
// File contracts/YourTokenDAO.sol

pragma solidity ^0.8.4;

// GM, Chad - unchainedchad.eth
// DAO Smart Contract for the $GM CHAD Governance Protocol
// https://www.gmchad.com

contract YourTokenDAO {
    struct Proposal {
        string title;
        string description;
        uint256 yesVotes;
    ...
    // This nested mapping tracks if a user has voted on a particular proposal
    mapping(uint256 => mapping(address => bool)) public hasVotedOnProposal;

    // This mapping tracks if a user has a live proposal
    mapping(address => bool) public hasLiveProposal;

    uint256 public proposalRequirement; // 0.5% of total supply
    ...
    constructor(address tokenAddress) {
        token = GMchad(tokenAddress);

        proposalRequirement = token.totalSupply() / 200;
    }
    ...
    function createProposal(
        ...
        require(
            bytes(title).length <= 200,
            "Title should be no longer than 200 characters."
        );
        require(
            bytes(description).length <= 500,
            "Description should be no longer than 500 characters."
        );
        require(
            token.balanceOf(msg.sender) >= proposalRequirement,
            "You must hold 0.5% of total supply to make proposals."
        );
        require(
            !hasLiveProposal[msg.sender],
            "You can only have one live proposal at a time."
        );
        ...
        hasLiveProposal[msg.sender] = true;
    ...
    function vote(uint256 proposalIndex, bool voteYes) public {
        ...
        require(weight > 1, "You must have tokens to vote.");
        require(
            !hasVotedOnProposal[proposalIndex][msg.sender],
            "You have already voted on this proposal."
        );
        require(
            proposals[proposalIndex].endTimestamp > block.timestamp,
        ...
        hasVotedOnProposal[proposalIndex][msg.sender] = true;

        if (voteYes) {
            proposals[proposalIndex].yesVotes += weight;
        }

        emit VoteCasted(proposalIndex, msg.sender, voteYes, weight);
    }

    function isProposalPassed(
        uint256 proposalIndex
    ) public view returns (bool) {
        return proposals[proposalIndex].yesVotes * 2 > token.totalSupply();
    }
    ...
    function endProposal(uint256 proposalIndex) public {
        require(
            proposals[proposalIndex].endTimestamp <= block.timestamp,
            "Proposal duration has not yet ended."
        );
        hasLiveProposal[proposals[proposalIndex].proposer] = false;
    }
}