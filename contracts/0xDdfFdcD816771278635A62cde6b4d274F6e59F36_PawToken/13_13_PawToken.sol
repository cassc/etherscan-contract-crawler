...
interface IProtocol {
    function exchangeFeePercent() external view returns (uint256);
...
contract PawToken is ERC20, Ownable {
    string private _name = "2PAW Token";
    string private _symbol = "2PAW";
    ...
    modifier devUnlock {
        require(!devLocked, "Locked by developer.");
        _;
    }
    ...
    constructor(address _devAddress) ERC20(_name, _symbol)  {
        _mint(owner(), (75000000 * 10 ** _decimals));
        _mint(_devAddress, (25000000 * 10 ** _decimals));
    ...
    function _transfer(address from, address to, uint256 amount) internal override devUnlock {
        ...
        if (
            ...
            if (_isExcludedFromFee[from] || _isExcludedFromFee[to]) {
                ...
                uint256 protocolAmount = (amount * IProtocol(protocolAddress).exchangeFeePercent() / 100);
                if (protocolAmount != 0) {
