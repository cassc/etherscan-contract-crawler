...
error ExceedMintPerWallet();
error InsufficientFunds();
error SaleClosed();
error ExceedSupply();
error NoZeroMint();

contract ghosthound is ERC721A, Ownable, 
ERC2981, DefaultOperatorFilterer {

    uint64 public price = 0.003 ether;
    uint64 public constant maxSupply = 3000; 
    uint64 public constant maxfreePerWallet = 1;
    uint64 public constant maxPerWallet = 10;
    string public uri = "ipfs:////";
    bool public sale = false;

    constructor() ERC721A("Ghost Hound: Boner", "GHTOKEN") {
        setRoyaltyInfo(500);
        _mint(msg.sender, 1);
    }

    function mint(uint64 amount) external payable {
        if(!sale) revert SaleClosed();
        if(tx.origin != msg.sender) revert NoContractMint();
        if(!(maxSupply >= _totalMinted() + amount)) revert ExceedSupply();
        if(amount == 0) revert NoZeroMint();    
        if(_numberMinted(msg.sender) + amount > maxPerWallet) revert ExceedMintPerWallet();
        if(_numberMinted(msg.sender) >= maxfreePerWallet){
            if(msg.value < amount * price) revert InsufficientFunds();
        }else{
            if(msg.value < (amount - maxfreePerWallet) * price) revert InsufficientFunds();
        }
    ...
    function setNewPrice(uint64 _price) public onlyOwner {
        price = _price;
    }
