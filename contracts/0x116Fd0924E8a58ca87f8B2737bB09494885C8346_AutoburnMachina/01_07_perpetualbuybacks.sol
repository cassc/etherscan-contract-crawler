...
/*
    Autoburn Machina - SMART CONTRACT MAGIC

    The Perpetual Self-Pumping Token
    You might ask: "Why such high fees?". Well, every single bit of the fees go back into liquidity 
    and buybacks burns. These liquidity injections and buy back burns end up pumping the bags of holders 
    exponentially. All of this is done through the smart contract. Let's grab some popcorn and watch our 
    bags pump exponentially through smart contract magic.

    Trustless Safe Launch
    It was crucial for us to ensure that the Autoburn Machina launch was completely SAFE and all safety measures 
    were built into the smart contract. Through smart contract mechanics, liquidity tokens are automatically 
    locked into the contract on launch. To unlock lp tokens, the dev needs to run requestToWithdrawLP() in 
    the smart contract to request a unlock. 14 days after this request is made the tokens can be unlocked. 
    Additionally all smart contract functions were audited and built with approriate safeguards and limits to 
    ensure that there is absolutely no honeypot functions. We wanted to make sure that this is a trustless 
    stress-free moonride for all holders.


    https://autoburnmachina.com/
    https://t.me/AutoburnMachina
...
contract AutoburnMachina is ERC20, Ownable {
    ...
    uint256 public lpWithdrawRequestDuration = 14 days;
    ...
    constructor() ERC20("Autoburn Machina", "ABM") payable {
    ...
    function updateBuyFees(uint256 _buybackFee, uint256 _liquidityFee, uint256 _burnFee) external onlyOwner {
        ...
        buyBurnFee = _burnFee;
        buyTotalFees = buyBuybackFee + buyLiquidityFee + buyBurnFee;
        require(buyTotalFees <= 12 * FEE_DIVISOR / 100, "Must keep fees at 12% or less");
    }
    ...
    function updateSellFees(uint256 _buybackFee, uint256 _liquidityFee,uint256 _burnFee) external onlyOwner {
        ...
        sellTotalFees = sellBuybackFee + sellLiquidityFee + sellBurnFee;
        require(sellTotalFees <= 12 * FEE_DIVISOR / 100, "Must keep fees at 12% or less");
    }
    ...
    function incipiamInferno(uint256 _penaltyBlocks) external onlyOwner {
        ...
        swapEnabled = true;
        tradingActiveBlock = block.number;
        blockForPenaltyEnd = tradingActiveBlock + _penaltyBlocks;
        emit EnabledTrading();
