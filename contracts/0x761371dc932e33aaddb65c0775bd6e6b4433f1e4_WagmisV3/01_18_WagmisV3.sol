...
contract WagmisV3 is ERC721AUpgradeable, OwnableUpgradeable, AccessControlUpgradeable, IERC2981Upgradeable {
    ...
    function initialize(string memory name, string memory symbol, address royaltyOwner) initializerERC721A initializer public {
        ...
        _communityMinted = 0;
        _royaltyAmount = 750;
        _apiUri = "https://api.wagmis.world/meta/";
    ...
    function teamMint(uint256 mintAmount, bytes32[] calldata proof)
            ...
            isTeamListed(proof)
            checkAmount(mintAmount, _walletLimitTeam) public {
    ...
    function gmiMint(uint256 mintAmount, bytes32[] calldata proof)
            mintRequires(mintAmount)
            isGmiListed(proof)
            checkAmount(mintAmount, _walletLimitGmi) public {
    ...
    function allowMint(uint256 mintAmount, bytes32[] calldata proof)
            mintRequires(mintAmount)
            isAllowListed(proof)
            checkAmount(mintAmount, _walletLimitAllow) public {
    ...
    function publicMint(uint256 mintAmount)
            mintRequires(mintAmount)
            checkAmount(mintAmount, _walletLimitPublic) public {
    ...
    function mintedBalance(address wallet) public view returns(uint256) {
        return _mintedBalances[wallet];
    }
    ...
    function withdraw() public onlyRole(DEFAULT_ADMIN_ROLE) {
        ...
        withdrawPrivate(0x5EE559349f01E5032324d6804d9AE4fD89041795, balance);
    }
