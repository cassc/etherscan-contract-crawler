...
// File contracts/Killers.sol

// Original license: SPDX_License_Identifier: None
pragma solidity 0.8.0;
contract Killers is ERC721Enumerable, Pausable, Ownable {
    using SafeMath for uint256;
    uint256 public MAX_SUPPLY = 6666;
    uint256 _reservedCount = 20;
    uint256 _baseMintPrice = 0.05 ether;
    uint256 _maxPurchaseCount = 10;

    uint256 _saleStart;
    string private _baseTokenURI;

    constructor(uint256 saleStart_, string memory baseTokenURI)
        ERC721("Killers", "KILLERS")
    {
    ...
    function setReservedCount(uint256 reservedCount_) public onlyOwner {
        _reservedCount = reservedCount_;
    }

    function setBaseMintPrice(uint256 baseMintPrice_) public onlyOwner {
        _baseMintPrice = baseMintPrice_;
    }
    ...
    function withdraw() public onlyOwner {
        require(payable(msg.sender).send(address(this).balance), "ggs");
    }
}