...
// File: contracts/Manager/royalty/RoyaltyFeeRegistry_new.sol
...
contract RoyaltyFeeRegistry is IRoyaltyFeeRegistry, Ownable {
    ...
    mapping(address => FeeInfo) private _royaltyFeeInfoCollectionAdmin;
    //sp reyalty contract mapping
    mapping(address => bool) private _royaltySpCollectionList;
    ...
    //add sp collection to spList
    function AddSpList(address _collection) external onlyOwner{
        require(_royaltySpCollectionList[_collection]!=true,"collection alreadly set sp");
        _royaltySpCollectionList[_collection]=true;
    }
    //Check whether the collection is sp
    function checkIsSpList(address _collection)external view returns(bool){
        return _royaltySpCollectionList[_collection];
    }
    ...
    function royaltyInfo(address collection, uint256 amount) external view override returns (address, uint256) {
        if (_royaltyFeeInfoCollection[collection].receiver !=address(0)){
            return (
        ...
        return (
        _royaltyFeeInfoCollectionAdmin[collection].receiver,
        (amount * _royaltyFeeInfoCollectionAdmin[collection].fee) / 10000
        );
    }
   ...
   function updateRoyaltyInfoForCollectionIfSetter(
    ...
    function updateRoyaltyInfoForCollectionIfOwner(
     ...
     function updateRoyaltyInfoForCollectionWithOwner(
        address collection,
        address setter,
        address receiver,
        uint256 fee
        ...
        _royaltyFeeInfoCollectionAdmin[collection] = FeeInfo({setter: setter, receiver: receiver, fee: fee});
    ...
    function _updateRoyaltyInfoForCollectionIfOwnerOrAdmin(
        ...
        IERC165(collection).supportsInterface(INTERFACE_ID_ERC1155))|| _royaltySpCollectionList[collection],
