...
contract KokoPresale {
    ...
    uint256 public immutable presalePrice = 9000000000 * 1e18;
    uint256 public totalPurchased = 0;
    uint256 public presaleMax;

    address public KOKO;
    address public immutable UNISWAP_V2_ROUTER = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;

    address public immutable owner;
    uint256 public immutable startTime;
    uint256 public immutable endTime;

    constructor(uint256 _startTime, address _KOKO, uint256 _max) {
        owner = msg.sender;
    ...
    function claim() external {
        ...
        IERC20(KOKO).transfer(msg.sender, amount);
    }
    ...
    function setKoko(address _koko) external onlyOwner{
        KOKO = _koko;
    }

    function addLiquidity() external onlyOwner {
        require(block.timestamp > endTime, "Not finished");
        IERC20(KOKO).approve(UNISWAP_V2_ROUTER, type(uint256).max);
        uint256 totalAmount = address(this).balance;
        (bool success,) = owner.call{value: totalAmount * 20 / 100}("");
        require(success);
