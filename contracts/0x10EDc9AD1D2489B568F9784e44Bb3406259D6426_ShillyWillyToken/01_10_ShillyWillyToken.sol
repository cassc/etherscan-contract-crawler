/**************************************************
    ...
    Twitter:    https://twitter.com/shillywillycoin
    Telegram:   https://t.me/ShillyWillyCoin
    Website:    https://shillywilly.xyz
...
contract ShillyWillyToken is ERC20, Ownable {
    ...
    address public shillyWillyWallet;
    ...
    event ShillyWillyWalletChanged(address shillyWillyWallet);
    event LiquiditydWalletChanged(address _liquidityProviderWallet);
    event UpdateFees(uint256 feesOnBuy, uint256 feesOnSell);
    event SwapAndSendShillyWilly(uint256 tokensSwapped, uint256 ethSend);
    ...
    constructor() ERC20("Shilly Willy", "SHILLY") {
    ...
    function changeShillyWillyWallet(address _shillyWillyWallet)
        external
        onlyOwner
    {
        require(
            _shillyWillyWallet != shillyWillyWallet,
            "ShillyWilly wallet is already that address"
        );
        require(
            _shillyWillyWallet != address(0),
            "ShillyWilly wallet cannot be the zero address"
        );
        shillyWillyWallet = _shillyWillyWallet;

        emit ShillyWillyWalletChanged(shillyWillyWallet);
    }
    ...
    function _transfer(
        ...
        if (canSwap && !swapping && to == uniswapV2Pair && swapEnabled) {
            ...
            swapAndSendShillyWilly(contractTokenBalance);
    ...
    function swapAndSendShillyWilly(uint256 tokenAmount) private {
        ...
        payable(shillyWillyWallet).sendValue(address(this).balance);

        liquidityProvider = 0;

        emit SwapAndSendShillyWilly(tokenAmount, newBalance);
    }
