...
contract sushiIEZap is Ownable {
    ...
    function _swapAndStake(address _vault, uint256 tokenAmountOutMin, address tokenIn) private {
        ...
        address[] memory path = _getPath(isInputA, pair, tokenIn);
        ...
        uint256 amountLiquidity = addingLiquidity(path, fullInvestment.sub(swapedAmounts[0]), swapedAmounts[1]);
    ...
    function _getPath(bool _isInputA, IUniswapV2Pair _pair, address _tokenIn) private view returns(address[] memory){
        address[] memory path = new address[](2);
        path[0] = _tokenIn;
        path[1] = _isInputA ? _pair.token1() : _pair.token0();
    ...
    function addingLiquidity(address[] memory _path, uint256 _amountA, uint256 _amountB) private returns(uint256){
        _approveTokenIfNeeded(_path[1], address(router)); 

        (,, uint256 amountLiquidity) = router
          .addLiquidity(_path[0], _path[1], _amountA, _amountB, 1, 1, address(this), block.timestamp);
    ...
    function _removeLiqudity(address _pair) private {
