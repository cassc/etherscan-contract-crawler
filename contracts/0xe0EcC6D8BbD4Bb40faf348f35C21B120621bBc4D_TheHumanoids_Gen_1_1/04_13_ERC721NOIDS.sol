...
contract ERC721NOIDS is Context, ERC165, IERC721, IERC721Metadata {
    ...
    function approve(address to, uint256 tokenId) public virtual override {
        address owner = ERC721NOIDS.ownerOf(tokenId);
    ...
    function _exists(uint256 tokenId) internal view virtual returns (bool) {
        return tokenId >= TOKEN_ID_FIRST && ((tokenId - TOKEN_ID_FIRST) < _totalSupply);
    }
    ...
    function _mint(address to, uint256 quantity, uint256 stageMax, uint256 stageTimestamp) internal virtual {
        unchecked {
            uint256 minted = quantity;
            if (stageTimestamp == _ownerData[to].stageTimestamp) {
                minted += _ownerData[to].stageMinted;
            }
            require(minted <= stageMax, "Cannot mint more than assigned maximum");

            _ownerData[to].balance += uint48(quantity);
            _ownerData[to].totalMinted += uint48(quantity);
            _ownerData[to].stageMinted = uint48(minted);
            _ownerData[to].stageMax = uint48(stageMax);
            _ownerData[to].stageTimestamp = uint64(stageTimestamp);

            uint256 newTotalSupply = _totalSupply + quantity;
            require(newTotalSupply <= MAX_SUPPLY, "Cannot mint more than maximum supply of 5000");
            uint256 tokenIdStart = _totalSupply + TOKEN_ID_FIRST;
            uint256 tokenIdEnd = tokenIdStart + quantity;
            _totalSupply = newTotalSupply;
            
            _owners[tokenIdStart] = to;
            if (quantity > 16) {
                uint256 batchCount = (quantity + 15) >> 4;
                uint256 quantity2 = quantity << 1;
                uint256 i = batchCount + (batchCount * quantity2) - quantity2;
                batchCount <<= 1;
                do {
                    _owners[(i / batchCount) + tokenIdStart] = to;  
                    i -= quantity2;
                } while (i >= batchCount);
            }
