...
contract RootVault is IRootVault, ERC20("YLDR Root Vault", "YLDR"), Ownable {
    ...
    function _redeem(uint256 shares) internal returns (uint256 assets) {
        ...
        assets = shares * _asset.balanceOf(address(this)) / totalSupply();
        ...
        uint256 valueInAsset = pricesOracle.convertFromUSD(valueInUSD, address(_asset));
        uint256 processedPricePerToken = (valueInAsset * (10 ** decimals())) / shares;
        uint256 adjustedPricePerToken = processedPricePerToken;

        if (processedPricePerToken > lastPricePerToken) {
            adjustedPricePerToken =
                lastPricePerToken + ((processedPricePerToken - lastPricePerToken) * (1000 - feeInPpm)) / 1000;
        }

        uint256 fee = (assets * (processedPricePerToken - adjustedPricePerToken)) / processedPricePerToken;
        if (fee > 0) {
            _asset.safeTransfer(feeTreasury, fee);
            assets -= fee;
        }
    }
