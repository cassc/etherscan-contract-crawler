...
contract NFT is Ownable, ERC721AQueryable, DefaultOperatorFilterer, ERC2981 {
    ...
    */
    ...
    function mint(
        ...
        uint256 tokensToClaim = getFreeClaimableTokenAmount(
            _msgSenderERC721A()
        );

        require(
            _mintAmount >= tokensToClaim,
            "Mint Amount needs to be greater than tokens to claim"
        );

        uint256 paidMintAmount = _mintAmount - tokensToClaim;
        uint256 totalPrice = PUBLIC_MINT_PRICE * paidMintAmount; // public mint price
        ...
        if (
            ...
            userMintInfo[_msgSenderERC721A()].wlMintAmount + paidMintAmount <=
            MAX_TOKENS_PER_PURCHASE
        ) {
            _recordFreeClaim(tokensToClaim);
            _recordPaidMintAmount(paidMintAmount);

            totalPrice = MINT_PRICE * paidMintAmount;
        }

        require(
            _mintAmount > 0 && paidMintAmount <= MAX_TOKENS_PER_PURCHASE,
    ...
    function _recordFreeClaim(uint256 _tokensToClaim) internal {
        if (_tokensToClaim > 0) {
            uint256[] memory tokenIds = beta.tokensOfOwner(_msgSenderERC721A());
            ...
            userMintInfo[_msgSenderERC721A()]
                .freeClaimedAmount += _tokensToClaim;
    ...
    function getFreeClaimableTokenAmount(
        address _account
    ) public view returns (uint256) {
        uint256 unclaimedBetaBalance;
        uint256[] memory tokenIds = beta.tokensOfOwner(_account);
        for (uint256 i; i < tokenIds.length; ++i) {
            uint256 tokenId = tokenIds[i];
            if (!betaClaimed[tokenId]) {
        ...
        return unclaimedBetaBalance * DISTRIBUTION_AMOUNT;
    }
