...
/**
 * @title Asymetrix Protocol V1 TwabRewards
 * @author Asymetrix Protocol Inc Team
 * @notice Contract to distribute rewards to depositors in a pool. This contract
 *         supports the creation of several promotions that can run
 *         simultaneously. In order to calculate user rewards, we use the TWAB
 *         (Time-Weighted Average Balance) from the Ticket contract. This way,
 *         users simply need to hold their tickets to be eligible to claim
 *         rewards. Rewards are calculated based on the average amount of
 *         tickets they hold during the epoch duration.
 ...
 */
contract TwabRewards is Initializable, Constants, ITwabRewards {
    ...
    /// @notice Period during which the promotion owner can't destroy a
    ///         promotion.
     ...
     * @dev Starts at 0 and is incremented by 1 for each new promotion. So the
     *      first promotion will have ID 1, the second 2, etc.
     */
     ...
     * @dev We pack epochs claimed by a user into a uint256. So we can't store
     *      more than 256 epochs.
     */
     ...
     * @param recipient Address of the recipient that will receive the remaining
     *                  rewards.
     ...
     * @param recipient Address of the recipient that will receive the unclaimed
     *                  rewards.
     * @param amount Amount of tokens transferred to the recipient.
     */
     ...
     * @param promotionId ID of the promotion for which epoch rewards were
     *                    claimed.
     ...
     * @param _ticket Prize Pool ticket address for which the promotions will be
     *                created.
     */
    function initialize(ITicket _ticket) external virtual initializer {
        __TwabRewards_init_unchained(_ticket);
    }

    function __TwabRewards_init_unchained(
        ITicket _ticket
    ...
    function createPromotion(
        ...
        require(
            ...
            "TwabRewards/start-timestamp-is-in-the-past"
        );
        ...
        /**
         * @dev The transaction will revert if the amount of reward tokens
         *      provided is not equal to `_tokensPerEpoch` * `_numberOfEpochs`.
         *      This scenario could happen if the token supplied is a fee on
         *      transfer one. It is possible in case you will do it with Eth Gas
         *      Station, there could be a situation when they charge you with
         *      the token you are depositing to the promotion and you will end
         *      up with an insufficient amount of tokens for the transaction.
         */
        require(
            _beforeBalance + _amount == _afterBalance,
    ...
    function destroyPromotion(
        ...
        /**
         * @dev Zeroing promotion's fields. All other functions checks for zero
         *      values after the queering of the promotion by its ID.
         */
    ...
    function claimRewards(
        ...
        require(
            _epochIds.length <= MAX_EPOCH_IDS_LENGTH,
            "TwabRewards/wrong-array-length"
        );
        ...
        for (uint256 index = 0; index < _epochIdsLength; index++) {
            ...
            _rewardsAmount += _calculateRewardAmount(
                _user,
                _promotion,
                _epochId
            );
            _userClaimedEpochs = _updateClaimedEpoch(
                _userClaimedEpochs,
                _epochId
            );
        }
    ...
    function getPromotion(
        uint256 _promotionId
    ) external view override returns (Promotion memory) {
    ...
    function getRewardsAmount(
        ...
        for (uint256 index = 0; index < _epochIdsLength; index++) {
            if (
                ...
                _rewardsAmount[index] = _calculateRewardAmount(
                    _user,
                    _promotion,
                    _epochIds[index]
                );
    ...
    /**
     * @notice Allow a promotion to be created or extended only by a positive
     *         number of epochs.
     ...
     */
    function _requirePromotionActive(
     ...
     * @dev Epoch IDs and their boolean values are tightly packed and stored in
     *      a uint256, so epoch ID starts at 0.
     * @dev We return the current epoch ID if the promotion has not ended. If
     *      the current timestamp is before the promotion start timestamp, we
     *      return 0. Otherwise, we return the epoch ID at the current
     *      timestamp. This could be greater than the number of epochs of the
     *      promotion.
     ...
     * @dev Will revert if `_epochId` is over the total number of epochs or if
     *      epoch is not over.
    ...
    function _calculateRewardAmount(
        ...
        uint64 _epochStartTimestamp = _promotion.startTimestamp +
            (_epochDuration * _epochId);
        uint64 _epochEndTimestamp = _epochStartTimestamp + _epochDuration;

        require(
            block.timestamp >= _epochEndTimestamp,
        ...
        if (_averageBalance > 0) {
            ...
            return
                (_promotion.tokensPerEpoch * _averageBalance) /
                _averageTotalSupply;
        }
     ...
     * @param _promotion Promotion to get the total amount of tokens left to be
     *                   rewarded for.
     * @return Amount of tokens left to be rewarded.
     */
    function _getRemainingRewards(
        Promotion memory _promotion
    ) internal view returns (uint256) {
     ...
     *      To set the boolean value to 1 for the epoch ID 2, we need to create
     *      a mask by shifting 1 to the left by 2 bits.
     *      We get: 0000 0001 << 2 = 0000 0100.
     *      We then OR the mask with the word to set the value.
     *      We get: 0110 0011 | 0000 0100 = 0110 0111.
     * @param _userClaimedEpochs Tightly packed epoch IDs with their boolean
     *                           values.
     * @param _epochId ID of the epoch to set the boolean for.
     * @return Tightly packed epoch IDs with the newly boolean value set.
     */
    function _updateClaimedEpoch(
    ...
    /**
     * @notice Check if rewards of an epoch for a given promotion have already
     *         been claimed by the user.
     * @dev Bits are stored in a uint256 from right to left.
     *      Let's take the example of the following 8 bits word: 0110 0111.
     *      To retrieve the boolean value for the epoch ID 2, we need to shift
     *      the word to the right by 2 bits.
     ...
     * @return `true` if the rewards have already been claimed for the given
     *         epoch, `false` otherwise.
     */
