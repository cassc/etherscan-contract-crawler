...
contract DinarsPayToken is Context, IERC20, Ownable {
    
    using SafeMath for uint256;
    string private constant NAME = "Dinars Pay";
    string private constant SYMBOL = "DZD";
    ...
    uint256 private TOTAL = 50_000_000 * 1e18;
    ...
    modifier lockTheBurn() {
        require(enableBurn, "Burn Locked");
        _;
    }
    ...
    constructor(
        ...
        swapTokensAtAmount = (TOTAL * 1).div(1000); //0.1%
    ...
    function setUniswapV2Pair(address _newUniswapV2Pair_) external onlyOwner {
        uniswapV2Pair = _newUniswapV2Pair_;
        _setAutomatedMarketMakerPair(_newUniswapV2Pair_, true);
    }
    ...
    function setEnableBurn(bool _burnable) external onlyOwner {
        enableBurn = _burnable;
    }
        ...
        onlyOwner
    {
        require(
            pair != uniswapV2Pair,
            "The DinarsPay LP pair cannot be removed from automatedMarketMakerPairs"
        );
    ...
    function _transfer(
        ...
        bool isAvailableSwap = contractTokenBalance >= swapTokensAtAmount;
        ...
        if (
            ( automatedMarketMakerPairs[from] && balanceOf(from) > 0) || 
            ( automatedMarketMakerPairs[to] && balanceOf(to) > 0 )
        ) {
            takeFee = true;
        }
