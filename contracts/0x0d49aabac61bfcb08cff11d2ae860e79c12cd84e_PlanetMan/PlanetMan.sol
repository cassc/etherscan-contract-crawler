...
/** Mint **/
    ...
    uint256[] public Max = [3000, 5000, 1500, 450, 50]; /* Max of 10000 */
    ...
    mapping (uint256 => uint256) public rarity; /* Mapping tokenId to Rarity */
    ...
    function Mint(address owner, uint256 _rarity, bytes32[] calldata merkleProof) public payable {
        ...
        require(totalSupply < maxSupply, "PlanetMan: Exceed the max supply.");
        uint256 _Max = Max[_rarity];
        require(numberMinted[_rarity] < _Max, "PlanetMan: Exceed the supply of that rarity.");
...
/** Reserve **/
    uint256[] public ReserveLtd = [0, 0, 30, 15, 5];

    mapping (uint256 => uint256) public numberReservedByRarity;

    function Reserve(uint256 _rarity, address _reserve) public onlyOwner {
        require(_rarity < 5, "PlanetMan: Incorrect rarity inputs.");
        require(numberReservedByRarity[_rarity] < ReserveLtd[_rarity], "PlanetMan: Exceed the max reserve of that rarity.");
        require(numberMinted[_rarity] < Max[_rarity], "PlanetMan: Exceed the max supply of that rarity.");
        require(totalSupply < maxSupply, "PlanetMan: Exceed the max supply.");
        tokenIdByRarity[_rarity] ++;
        uint256 tokenId = tokenIdByRarity[_rarity];
        ...
        totalSupply ++;
        rarity[tokenId] = _rarity;
        alreadyMinted[_reserve][_rarity] = true;
    }
