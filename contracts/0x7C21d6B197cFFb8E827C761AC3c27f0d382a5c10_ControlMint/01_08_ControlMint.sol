...
contract ControlMint is ContractRestricted, IControlMint {
    ...
    Counters.Counter private mintsFreemintList;
    ...
    function addAddressFreemintList(address[] memory newMinters) public override onlyOwner{
        ...
        require(mintsFreemintList.current() + newMinters.length < maxMintAddressFreemintList, "Insufficient gaps in the whitelist.");
    ...
    function addAddressWhitelistList(address[] memory newMinters) public override onlyOwner{
        ...
        require(newMinters.length <= 50, "List can only have a maximum of 50.");
        require(mintsWhitelistList.current() + newMinters.length < maxMintAddressWhitelistList, "Insufficient gaps in the whitelist.");
    ...
    function getAddressFreemintListCount() public override virtual view onlyOwner returns(uint256){
        return implementAddressFreemintList.current();
    }

    function getAddressWhitelistListCount() public override virtual view onlyOwner returns(uint256){
        return implementAddressWhitelistList.current();
    }

    function getMintsFreemintCount() public override virtual view returns(uint256){
        return mintsFreemintList.current();
    }

    function getMintsWhitelistCount() public override virtual view returns(uint256){
    ...
    function minterUseWhitelist(address minter) public override virtual onlyOwnerOrContract returns(bool){
    ...
    function getAvailableMintForAddressWL(address minter) public override view returns(uint32){
        if(addressFreemintList[minter].active && quantityMintForAddress[minter] < numMaxForAddress){
            if(event1More){
                if(event1MoreLastMint[minter] < event1MoreCount && addressWhitelistList[minter].numMints > 0){
                    ...
                    return numMaxForAddress - quantityMintForAddress[minter] >= 2 ? 2 : 1;
