...
contract OmnihorseNFT is ERC721A, Ownable, ERC721ABurnable, ERC721AQueryable {
    ...
    IGetPrice public priceController;
    ...
    function setPriceController(address priceController_) public onlyOwner {
        priceController = IGetPrice(priceController_);
    }
    ...
    function getOnlyPrice(address user_, uint256 amount_) public view returns(uint256) {
        require(amount_ > 0, "Cannot mint zero");
        return priceController.getOnlyPrice(user_, amount_);
    }
    ...
    function setUsed(address user_, uint256[] calldata tokenId_, bool state_) public onlyOwner {
        priceController.setUsed(tokenId_, user_, state_);
    }
    ...
    function isUsed(uint256 tokenId_) external view returns(bool) {
        return priceController.isUsed(tokenId_);
    }
    ...
    function getData(uint256 tokenId_) external view returns(IGetPrice.used memory) {
        return priceController.getData(tokenId_);
    }
    ...
    function getUser(address user_) external view returns(IGetPrice.user memory) {
        return priceController.getUser(user_);
    }
