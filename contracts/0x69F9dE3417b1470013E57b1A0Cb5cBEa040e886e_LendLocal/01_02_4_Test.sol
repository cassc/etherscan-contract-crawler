...
contract LendLocal is Ownable, ReentrancyGuard {
    ...
    function countCollateralFromFiat(
        ...
        uint256 result;
        // fiatAmount decimal 18, token price decimal 18, fiatPrice decimal 8
        // result = _fiatAmount * fiatPrice * (10 ** tokenDecimals)/ tokenPrice / 10 ** 8
        if (tokenDecimals < 8) {
            // result = _fiatAmount.mul(fiatPrice).mul(FEEDEN).div(_limit).div(tokenPrice).div(10**(18 - tokenDecimals));
            result = _fiatAmount.mul(fiatPrice).div(tokenPrice).div(
                10**(8 - tokenDecimals)
            );
        } else if (tokenDecimals > 8) {
            result = _fiatAmount
                .mul(fiatPrice)
                .mul(10**(tokenDecimals - 8))
                .div(tokenPrice);
        } else {
    ...
    function countFiatFromCollateral(
        ...
        uint256 fiatPrice = getFiatInUSD(_fiatId);
        uint256 tokenDecimals = IERC20(_collateralToken).decimals();
        // fiatAmount decimal 18, token price decimal 18, fiat price decimal 8,
        // uint256 result = _colTokenAmount / (10 ** tokenDecimals) * (tokenPrice) / (10 ** 18) * (fiatPrice) / (10 ** 8) * (10 ** 18)
        // uint256 result = _colTokenAmount / (10 ** tokenDecimals) * (tokenPrice)  * (fiatPrice) / (10 ** 8)
        uint256 result = _colTokenAmount.mul(tokenPrice).mul(fiatPrice).div(10**(tokenDecimals + 8));
    ...
    function loanTokenFromCollateral(
        address _collateralToken,
        uint256 _kiosk,
        uint256 _fiatPoolId,
        ...
        require(fiatPoolId < fiatPools.length, "loanToken:: No valid fiat Pool Id");
        require(
            !fiatPools[fiatPoolId].closed,
            "loanToken:: Fiat Pool is closed"
        );
    ...
    function loanEther(
        ...
        uint256 fiatPoolId = _fiatPoolId;
        uint256 fiatAmount = _fiatAmount;
        require(fiatPoolId < fiatPools.length, "loanEther:: No valid fiat Pool Id");
        require(
            !fiatPools[fiatPoolId].closed,
    ...
    function loanEtherFromCollateral(uint256 _kiosk, uint256 _fiatPoolId)
        external
        payable
        nonReentrant
    {
    ...
    function updateInfo(
        ...
        uint256 _feeDen,
        uint256 _limit,
        uint256[] memory dayLimit,
        uint256[] memory dayFeeNum
