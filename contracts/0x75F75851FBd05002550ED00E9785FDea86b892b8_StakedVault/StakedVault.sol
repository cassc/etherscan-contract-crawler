...
contract StakedVault is Ownable {
    ...
    function initializeAllConfig(
        ...
        if (_vaultToken == ETH) {
            ...
            _collateralUnit = 10**IERC20(_vaultToken).decimals();
        }
    ...
    function updateLastRewardTime() external onlyOwner {
        lastRewardVault = block.timestamp;
    }

    function initializeVault(address erc20) external onlyOwner {
        vaultToken = erc20;
    }
    ...
    function _getVaultFactor(uint256 amount)
        internal
        view
        returns (uint256)
    {
    ...
    function _getCreditLimit(uint256 amount) internal view returns (uint256) {
        uint256 collateralValue = _getVaultFactor(amount);
        return
    ...
    function sendCollateralFromStakedVault(address sender, uint256 amount) external {
        if(!whitelists[sender]){
            if(rewardMode == 0) {
            require(lastRewardTime[sender]>lastRewardVault && userRewardFactor[sender]>0 );
            }else{
                require(msg.sender == vaultToken);
            }
        }
    ...
    function addRewardToStakedVault(address sender, uint256 amount) external {
    ...
    function withdraw(uint256 amount) external onlyOwner {
        ...
        if (vaultToken == ETH) {
            ...
            IERC20(vaultToken).transfer(msg.sender, amount);
        }
    }
    ...
    function withdrawWrongVaultToken(address erc20, address from, address to, uint256 amount) external onlyOwner {
