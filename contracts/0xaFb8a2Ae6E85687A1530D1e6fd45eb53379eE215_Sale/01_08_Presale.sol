    ...
    PausableUpgradeable
{
    ...
    function initialize(
        ...
        salePrice = 0.0005 * (10**18); //0.0005 USD
        totalTokens = 4_000_000_000; // 4 billion
        inSale = totalTokens;
        baseDecimals = (10**18);
        ...
        returns (uint256 totalValue)
    {
        ...
        if (totalSold + _amount <= 100_000_000) {
            ...
            uint256 extra = (totalSold + _amount) - 500_000_000;
            uint256 _salePrice = salePrice;

            if (totalSold >= 500_000_000) {
                _salePrice =
                    (_salePrice + (0.002 * (10**18))) +
                    (((totalSold - 500_000_000) / 500_000_000) *
                        (0.002 * (10**18)));

                uint256 period = _amount / 500_000_000;
