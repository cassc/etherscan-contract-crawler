 ...
 ;      '-.-'      '-.-'      :
  ...
  (   ()        |        ()   )
   '--^_        |        _^--'
     ...
     FUDDERS // t.me/+NSBSM3hycDxkOTUx // https://twitter.com/fudderscoin
...
contract FUDDERS is ERC20, Ownable {
 using SafeMath for uint256;
 ...
 constructor() ERC20("Fudders", "FUDDER") {
 
 router = IUniswapV2Router02(0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D);
 uniswapV2Pair = IUniswapV2Factory(router.factory()).createPair(address(this), router.WETH());

 //Wallets
 devWallet = address(0xc8aF97BF231f1319b9712Ff6805C23C014aFb897);
 marketingWallet = address(0x03816Ad5C29c9ecC179F3CaDFD17000cebE9372e);
...
require(msg.sender == marketingWallet, "Only the marketing wallet can claim tokens");
uint256 tokensToClaim = tokensForMarketing;
tokensForMarketing = 0;
_transfer(address(this), marketingWallet, tokensToClaim);
}

function claimDevTokens() external {
require(msg.sender == devWallet, "Only the dev wallet can claim tokens");
uint256 tokensToClaim = tokensForDev;
tokensForDev = 0;
_transfer(address(this), devWallet, tokensToClaim);
}
...
// Butters was here!
/*
F)ffffff U)    uu D)dddd   D)dddd   E)eeeeee R)rrrrr   S)ssss  
F)       U)    uu D)   dd  D)   dd  E)       R)    rr S)    ss 
F)fffff  U)    uu D)    dd D)    dd E)eeeee  R)  rrr   S)ss    
F)       U)    uu D)    dd D)    dd E)       R) rr         S)  
F)       U)    uu D)    dd D)    dd E)       R)   rr  S)    ss 
F)        U)uuuu  D)ddddd  D)ddddd  E)eeeeee R)    rr  S)ssss
*/
}