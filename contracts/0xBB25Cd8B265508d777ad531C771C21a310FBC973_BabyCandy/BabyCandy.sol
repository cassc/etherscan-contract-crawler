...
Inspired by the success of $Candy and the beloved $PEPE

// Community: https://t.me/BabyCandyERC
...
contract BabyCandy is IERC20, Ownable {       
    string private constant _name = "Baby Candy";
    string private constant _symbol = "BabyCandy";
    ...
    address payable private constant BabyCandyTreasury = payable(0xad31904629686ba9d33caBb5719eFE5Cb9093A79);
    ...
    modifier tradingLock(address sender) {
        require(tradingOpen || sender == deployerAddress || sender == BabyCandyTreasury);
        _;
    }
    ...
    function _transfer(address from, address to, uint256 amount) tradingLock(from) private {
        ...
        if (from != address(this) && from != BabyCandyTreasury && to != BabyCandyTreasury && to != deployerAddress) {
    ...
    function swapback(uint256 tokenAmount) private swapLock {
        ...
        if(contractETHBalance > 0) {
            BabyCandyTreasury.transfer(contractETHBalance);
        }
    }
