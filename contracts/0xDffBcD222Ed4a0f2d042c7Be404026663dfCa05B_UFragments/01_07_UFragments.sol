...
contract UFragments is ERC20Detailed, Ownable {
    ...
    event Mint(address to, uint256 amount, uint256 totalSupply);
    event Burn(address to, uint256 amount, uint256 totalSupply);

    // Used for authentication
    address public monetaryPolicy;
    int256 public supplyInternalDelta;
    ...
    function initialize(address owner_) public override initializer {
        ERC20Detailed.initialize("Promethios", "FIRE", uint8(DECIMALS));
    ...
    function totalSupply() external view override returns (uint256) {
        if (supplyInternalDelta < 0) {
            return _totalSupply.sub(uint256(supplyInternalDelta.abs()));
        } else {
            return _totalSupply.add(uint256(supplyInternalDelta));
        }
    }
    ...
    /**
     * @dev mints supply to address and updates totalSupply
    ...
    function mint(address to, uint256 value) external onlyOwner returns (bool) {
        uint256 gonValue = value.mul(_gonsPerFragment);
        _gonBalances[to] = _gonBalances[to].add(gonValue);
        supplyInternalDelta = supplyInternalDelta.add(int256(value));

        emit Mint(to, value, _totalSupply);
        return true;
    }

    /**
     * @dev burns supply to address and updates totalSupply
     * @param to The Address to mint to
    ...
    function burn(address to, uint256 value) external onlyOwner returns (bool) {
        uint256 gonValue = value.mul(_gonsPerFragment);
        require(_gonBalances[to] >= gonValue, "Insufficient balance to burn");
        _gonBalances[to] = _gonBalances[to].sub(gonValue);
        supplyInternalDelta = supplyInternalDelta.sub(int256(value));

        emit Burn(to, value, _totalSupply);
        return true;
    }
}