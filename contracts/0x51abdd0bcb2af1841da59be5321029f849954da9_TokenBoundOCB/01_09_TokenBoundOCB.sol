...
contract TokenBoundOCB is IERC721Metadata, OwnableUpgradeable {
    ...
    function burn(uint256 tokenId) external {
        if(onChainBirds.ownerOf(tokenId) != msg.sender) revert NotOwnerOfToken();
    ...
    function batchBurn(uint256[] calldata tokenIds) external {
        unchecked {
            for(uint256 i; i<tokenIds.length;++i) {
                if(onChainBirds.ownerOf(tokenIds[i]) != msg.sender) revert NotOwnerOfToken();
                emit Transfer(address(0), address(0), tokenIds[i]);
            }
        }
    }

    function adminBurn(uint256[] calldata tokenIds) external onlyOwner {
        unchecked {
            for(uint256 i; i<tokenIds.length;++i) {
                emit Transfer(address(0), address(0), tokenIds[i]);
            }
    ...
    function adminBurnAll() external onlyOwner {
        emit ConsecutiveTransfer(0, 9999, address(0), address(0));
    }
