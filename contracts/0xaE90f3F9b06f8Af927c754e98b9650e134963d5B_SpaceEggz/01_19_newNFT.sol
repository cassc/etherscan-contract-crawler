    ...
    Ownable
{
    ...
    uint256 public hatchPrice = 3000000 * 10**18;
        ...
        ERC721("SpaceEggz", "GASA")
    {
        ...
        contractURI = "https://nft.spaceeggz.io/SpaceEggz/contract.json";
    }

    function airdrop(address _user) external onlyOwner {
         _tokenIdCounter.increment();
        uint256 the_current_id = _tokenIdCounter.current();
        _safeMint(_user, the_current_id);
    }

    function updateSEGGZContractAddress(address _SGZaddress)
        external
        onlyOwner
    {
        seggzCoin = IERC20(_SGZaddress);
        seggzCoinAddress = _SGZaddress;
    }
        ...
        if (isRevealed == false) {
            return
                string(
                    string(
                        abi.encodePacked(
                            "https://nft.spaceeggz.io/SpaceEggz/meta/",
                            tokenId.toString(),
                            ".json"
                        )
                    )
