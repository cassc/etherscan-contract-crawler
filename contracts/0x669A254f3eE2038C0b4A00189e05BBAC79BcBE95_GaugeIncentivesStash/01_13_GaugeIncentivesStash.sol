...
contract GaugeIncentivesStash is OwnableUpgradeable, UUPSUpgradeable {
	...
	function numBlacklistedAddresses() external view returns (uint) {
		return globalBlacklistedAddresses.length;
	}

	function numGaugeBlacklistedAddresses(address _gauge) external view returns (uint) {
	...
	function updateMerkleRoot(address _token, bytes32 _merkleRoot) public onlyOwner {
	...
	// Updates merkle root, transfers fee, refunds limit orders
	function checkpointClaimPeriod(address _token, bytes32 _merkleRoot, uint256 _periodFee, refundData[] memory _refunds) public onlyOwner {
	...
	event RefundIssued(address indexed token, uint256 indexed tokenClaimPeriod, address indexed account, uint256 amount);
