...
contract HasNoEther is Ownable {
  ...
  * @dev The `payable` flag is added so we can access `msg.value` without 
compiler warning. If we
  * leave out payable, then Solidity will allow inheriting contracts to 
implement a payable
  * constructor. By doing it this way we prevent a payable constructor 
from working. Alternatively
  ...
  */
  ...
  /**
   * @dev Disallows direct send by settings a default function without the 
`payable` flag.
   */
  function() external {
  }
...
contract Claimable is Ownable {
  ...
  /**
   * @dev Modifier throws if called by any account other than the 
pendingOwner.
   */
...
the spender.
  ...
  /**
   * @dev Increase the amount of tokens that an owner allowed to a 
spender.
   *
...
contract gCOTI is HasNoEther, Claimable, MintableToken, BurnableToken {
    string public constant name = "gCOTI Token";
    string public constant symbol = "gCOTI";
