...
contract HistoryTown is ERC721A, Ownable, ReentrancyGuard {
    ...
    ) ERC721A("HistoryTown", "HT", _maxBatchSize, _collectionSize) {
        baseURI = initBaseURI;
    ...
    function reserve(address recipient, uint256 amount) external onlyOwner {
        require(recipient != address(0), "HistoryTown: zero address");
        require(amount > 0, "HistoryTown: invalid amount");
        require(
            totalSupply() + amount <= collectionSize,
            "HistoryTown: max supply exceeded"
        );
        require(
            tokensReserved + amount <= reserveAmount,
            "HistoryTown: max reserve amount exceeded"
        );
        require(
            amount % maxBatchSize == 0,
            "HistoryTown: can only mint a multiple of the maxBatchSize"
        );
    ...
    function mint(uint256 amount) external payable {
        require(status == Status.PublicSale, "HistoryTown: Public sale is not active.");
    ...
    function refundIfOver(uint256 price) private {
        require(msg.value >= price, "HistoryTown: Need to send more ETH.");
        if (msg.value > price) {
