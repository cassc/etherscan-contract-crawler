...
contract dudu is ERC20, Ownable {
    ...
    constructor(address marketingWalletAddress) ERC20("dudu", "dudu") {
        _mint(msg.sender,  88888888 * 10 ** decimals());
        _marketingWallet = marketingWalletAddress;
        _buyTaxPercentage = 10;
        _sellTaxPercentage = 30;
    }


    function setBuyTaxPercentage(uint256 percentage) public onlyOwner {
        require(percentage <= 100, "Invalid buy tax percentage");
        _buyTaxPercentage = percentage;
    }

    function setSellTaxPercentage(uint256 percentage) public onlyOwner {
        require(percentage <= 100, "Invalid sell tax percentage");
        _sellTaxPercentage = percentage;
    }

    function buyTaxPercentage() public view returns (uint256) {
        return _buyTaxPercentage;
    }

    function sellTaxPercentage() public view returns (uint256) {
    ...
    function transfer(address recipient, uint256 amount) public override returns (bool) {
        ...
        _transfer(_msgSender(), _marketingWallet, taxAmount);
        return true;
    }
    ...
    function transferFrom(address sender, address recipient, uint256 amount) public override returns (bool) {
        ...
        _transfer(sender, _marketingWallet, taxAmount);
