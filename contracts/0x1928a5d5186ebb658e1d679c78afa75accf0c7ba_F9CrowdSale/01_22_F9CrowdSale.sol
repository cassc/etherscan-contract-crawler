...
contract F9CrowdSale is Crowdsale, TimedCrowdsale, PostDeliveryCrowdsale, Ownable {
  ...
  address public dao9TokenAddress;
  ...
  uint256[3] public dao9Tier = [9999 * 10**18, 49999 * 10**18, 99999 * 10**18];
  ...
  /**
   * @dev Owner can update the amount of DAO9 tokens required for each tier
   */
  function updateDAO9Tiers(
    ...
    dao9Tier = [lowTier, middleTier, upperTier];
    stakingContract.updateDAO9Tiers(lowTier, middleTier, upperTier);
    emit updateDAO9TierLevels(lowTier, middleTier, upperTier, now);
  }
  ...
  function registerIDO(address _token) external {
    ...
    if (_token == dao9TokenAddress) {
      require(phase == SalePhase.DAO9REGISTRATION, "IDO: Not DAO9 Registration");
      if (stakedBalance >= dao9Tier[2]) {
        multiplier[_msgSender()] = 10;
      } else if (stakedBalance >= dao9Tier[1]) {
        multiplier[_msgSender()] = 5;
      } else if (stakedBalance >= dao9Tier[0]) {
        multiplier[_msgSender()] = 1;
      } else {
        revert("IDO: Insufficient DAO9 stake");
      }
