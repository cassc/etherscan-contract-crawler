...
contract RandomStaking is Ownable, IERC721Receiver {
    ...
    event retrieveTokens(address owner, uint256 amount);
    ...
    function stake(
        ...
        require(
            stakeAllowedCollections[collection] == true,
        ...
        for (uint i = 0; i < nftIds.length; i++) {
            nftId = nftIds[i];
            require(
                ContractErc721(collection).ownerOf(nftId) == msg.sender,
                "You do not own this NFT"
            );
            bool isApproved = ContractErc721(collection).isApprovedForAll(
                msg.sender,
                address(this)
            );
            require(
            ...
            ContractErc721(collection).transferFrom(msg.sender, address(this), nftId);
    ...
    function _claim(
        ...
        for (uint i = 0; i < nftIds.length; i++) {
            ...
            require(
                ...
                "not an nft staker, or not the original owner"
            );
        }
        earned = _calculateEarningReturns(nftIds, collection, block.timestamp);
        require(earned > 0 || isUnstaking, "Nothing to claim");
        ...
        for (uint i = 0; i < nftIds.length; i++) {
            ...
            if (isUnstaking) {
                if (staked.lockupPeriod == LockupPeriod.sevenDays) {
                    ...
                    ContractErc721(collection).transferFrom(address(this), account, nftId);
                ...
                randomsStaked[nftId][collection].timestamp = uint48(
                    block.timestamp
                );
            }
        }
    ...
    {
        uint256 balance = 0;
        uint256 supply = ContractErc721(collection).totalSupply();
    ...
    function unstakeForAddress(
        address account,
        address collection,
        uint256[] calldata nftIds
    ) external onlyOwner {
        _claim(account, nftIds, collection, true);
    }
        ...
        returns (uint256[] memory ownerTokens)
    {
        ...
        uint256 index = 0;
        for (uint nftId = 0; nftId < supply; nftId++) {
    ...
    function getStakedNftForOwner(address account, address collection)
        public
        view
        returns (Stake[] memory)
    {
        uint256 supply = ContractErc721(collection).totalSupply();
        Stake[] memory stakedNfts = new Stake[](supply);
        uint256 index = 0;
        for (uint nftId = 1; nftId <= supply; nftId++) {
            if (randomsStaked[nftId][collection].owner == account) {
                stakedNfts[index] = randomsStaked[nftId][collection];
                index += 1;
            }
        }
        return stakedNfts;
