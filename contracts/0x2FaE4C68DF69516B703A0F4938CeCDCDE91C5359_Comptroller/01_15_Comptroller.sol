 ...
 * @dev modified enterMarketsFrom, setMigrator, and liquidateCalculateSeizeTokens
 */
contract Comptroller is ComptrollerV6Storage, ComptrollerG3Interface, ComptrollerErrorReporter, ExponentialNoError {
     ...
     * @notice Add assets to be included in account liquidity calculation
     * @param cTokens The list of addresses of the cToken markets to be enabled
     * @param user The user address to be entered
     * @return Success indicator for whether each corresponding market was entered
     */
    function enterMarketsFrom(address[] memory cTokens, address user) public returns (uint[] memory) {
        require(isMigrator[msg.sender], "only migrator can enter markets from");
        uint len = cTokens.length;
    ...
    /**
     * @notice Set COMP speed for markets
     * @param cToken The market whose COMP speed to update
     * @param supplySpeeds New supply-side COMP speed for market
     * @param borrowSpeeds New borrow-side COMP speed for market
     */
    function _setCompSpeeds(CToken[] memory cToken, uint[] memory supplySpeeds, uint[] memory borrowSpeeds) public {
        ...
        for (uint i = 0; i < numTokens; ++i) {
            setCompSpeedInternal(cToken[i], supplySpeeds[i], borrowSpeeds[i]);
        }
    }
     ...
     * @param cToken The market whose COMP speed to update
     * @param supplySpeed New COMP speed for market
     * @param borrowSpeed New COMP speed for market
     */
     ...
     */
    function _setCompAddress(address _comp) external {
     ...
     */
    function _setMigrator(address _migrator, bool _status) external {
        require(msg.sender == admin, "only admin can set is migrator");
        isMigrator[_migrator] = _status;
    }
}