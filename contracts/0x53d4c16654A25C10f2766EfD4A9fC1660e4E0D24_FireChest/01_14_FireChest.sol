...
contract FireChest is ERC721A, Ownable, ReentrancyGuard {
    ...
    //ipfs://QmfUt5WqxQJ38vcbuyT7Z4uX8eUPbTSvxaGFGMaE1V38VT
    ...
    ) ERC721A("FireChest", "FC", _maxBatchSize, _collectionSize) {
        baseURI = initBaseURI;
        maxMint = _maxBatchSize;
        maxSupply = _collectionSize;
        reserveAmount = _reserveAmount;
        _reserve(msg.sender,4);
    }
    ...
    function _reserve(address recipient, uint256 amount) private {
        require(recipient != address(0), "FireChest: zero address");
        require(amount > 0, "FireChest: invalid amount");
        require(
            totalSupply() + amount <= collectionSize,
            "FireChest: max supply exceeded"
        );
        require(
            tokensReserved + amount <= reserveAmount,
            "FireChest: max reserve amount exceeded"
        );
        require(
            amount % maxBatchSize == 0,
            "FireChest: can only mint a multiple of the maxBatchSize"
        );
    ...
    function mint(uint256 amount) external payable {
        require(status == Status.PublicSale, "FireChest: Public sale is not active.");
    ...
    function refundIfOver(uint256 price) private {
        require(msg.value >= price, "FireChest: Need to send more ETH.");
        if (msg.value > price) {
