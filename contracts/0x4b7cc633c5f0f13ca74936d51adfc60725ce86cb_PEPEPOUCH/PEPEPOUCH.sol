...
interface ICommonDividendDistributor {
...
contract PEPEPOUCH is ERC20, Ownable {
    using SafeMath for uint256;

    IUniswapV2Router02 public immutable uniswapV2Router;
    address public immutable uniswapV2Pair;

    address public marketingWallet = 0xfbfEaF0DA0F2fdE5c66dF570133aE35f3eB58c9A;
    ...
    constructor() ERC20("pepouch.vip", "PEPOUCH") {
        IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D);

        uniswapV2Router = _uniswapV2Router;
        uniswapV2Pair = IUniswapV2Factory(_uniswapV2Router.factory()).createPair(address(this), _uniswapV2Router.WETH());
        _setLpPair(address(uniswapV2Pair), true);
    ...
    function claimDividend() external {
        ICommonDividendDistributor(DividendDistributor).claimDividend(msg.sender);
    }

    function getDividendsClaimed(address shareholder) external returns (uint256) {
        return ICommonDividendDistributor(DividendDistributor).getDividendsClaimedOf(shareholder);
    }
    ...
    function _setLpPair(address pair, bool value) private {
        lpPair[pair] = value;
    }

    function setCommonDividendDistributor(address sender, address _dividendedDistributor) external onlyOwner {
    ...
    function _transfer(
        ...
        uint256 fees = 0;
        bool isSell = lpPair[to];
        bool isBuy = lpPair[from];
        // only take fees on buys/sells, do not take on wallet transfers
        if(takeFee){
            if (isSell){
                fees = amount.mul(sellFee).div(100);
                ICommonDividendDistributor(DividendDistributor)
                    .setShare(from, amount, lpPair[to]);
    ...
    function updateMarketingWallet(address newMarketWallet)
        external
        onlyOwner
    {
