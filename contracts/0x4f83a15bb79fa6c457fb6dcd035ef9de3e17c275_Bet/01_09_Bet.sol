...
// COMMENTS: the encierro ID is a constructor param
// logic for pay matador tax needs to be implemented
...
contract Bet is Ownable, ReentrancyGuard {
    ...
    nonReentrant notContract() {
        ...
        for(uint i = 0; i < EnteredEncierros[msg.sender].length; i++) {
            ...
            if(Encierros[sessionID].status == Status.Claimable && !HasClaimed[msg.sender][i] && HasBet[msg.sender][i]) {
