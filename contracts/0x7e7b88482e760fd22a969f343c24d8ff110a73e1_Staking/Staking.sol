...
contract Staking is Initializable, OwnableUpgradeable, PausableUpgradeable {
    ...
    event StakePlanUpdated(uint256 _planId, Plan _plan);
    ...
    event StakeRemoved(address user, uint256 _rewards);
    ...
    function createStakePlans(
        ...
        for (uint i; i < _plans.length; ) {
            ...
            emit StakePlanUpdated(planId, _plans[i]);
    ...
    function updateStakePlans(
        ...
        for (uint i; i < _plans.length; ) {
            ...
            emit StakePlanUpdated(_planIds[i], _plans[i]);
    ...
    function updateStakePlanStatus(
        ...
        for (uint i; i < _planIds.length; ) {
            ...
            emit StakePlanUpdated(_planIds[i], plan[_planIds[i]]);
    ...
    function createStake(
        ...
        uint256 rewards;
        UserStake memory _userStake = userStake[msg.sender][_planId];

        if (_userStake.amount == 0) {
            userStake[msg.sender][_planId] = UserStake(
                amount,
                _planId,
                block.timestamp,
            ...
            require(_userStake.endTime > block.timestamp, "Stake ended");

            rewards = calculate(
                _userStake.planNo,
                _userStake.amount,
                block.timestamp - _userStake.startTime
            );

            userStake[msg.sender][_planId] = UserStake(
                _userStake.amount + amount,
                _planId,
                block.timestamp,
                block.timestamp + _plan.minLock,
                rewards
    ...
    function calculate(
        ...
        require(amount > 0 && _planId < planId, "Invalid params");

        rewards = (amount * time * plan[_planId].apy) / (3153600000);
    }

    function removeStake(uint256 _planId) external returns (bool) {
        require(_planId < planId, "Invalid Plan Id");
        UserStake memory _userStake = userStake[msg.sender][_planId];
        require(_userStake.amount > 0, "Invalid user stake");
        require(block.timestamp >= _userStake.endTime, "Can't withdraw early");

        uint256 rewards = calculate(
            _userStake.planNo,
            _userStake.amount,
            plan[_userStake.planNo].minLock
        );
        rewards += _userStake.pendingReward;

        delete userStake[msg.sender][_planId];

        stakedBalance -= _userStake.amount;
        rewardBalance -= rewards;

        token.transfer(msg.sender, (_userStake.amount + rewards));
        emit StakeRemoved(msg.sender, rewards);
        return true;
    }
}