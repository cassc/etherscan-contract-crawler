    ...
    DefaultOperatorFiltererUpgradeable
{
       ...
       MintCart memory cart;
        ...
        for (uint256 i = 0; i < _quantities.length; i++) {
            ...
            totalMintedByAllowlist[_allowlistIds[i]] += _quantities[i];
             for (uint256 q = 0; q < _config[i].length; q++) {
                mintedByConfig[_config[i][q]] += _quantities[i];
             }
        ...
        unchecked {
            ...
            for (uint256 i = 0; i < _quantities.length; i++) {
                ...
                for (uint256 j = 0; j < _quantities[i]; j++) {
                    ...
                    cart._tokensConfigured = string(
                        abi.encodePacked(
                            cart._tokensConfigured,
                            nextTokenId.toString(),
                            ":",
                            _config[i][0],
                            ",",
        ...
        emit Configured(_msgSender(), cart._tokensConfigured);
    ...
    function revertOnConfigCheckFailure(
        ...
        string[] calldata _config
         ...
         for (uint256 i = 0; i < _config.length; i++) {
            require(configSupplyLimit[_config[i]] == 0 || mintedByConfig[_config[i]] + _quantity <= configSupplyLimit[_config[i]], string(abi.encodePacked("S:", _config[i]))) ;
         }
    }
  ...
  //////////////////////////////////////////////////////////////*/
    ...
    event Configured(address indexed account, string tokens);
