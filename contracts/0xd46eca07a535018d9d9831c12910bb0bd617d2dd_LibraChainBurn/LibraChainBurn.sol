...
interface DAPP {
    ...
    function manualBurnPercent(address token, address receiver, uint256 amount) external;
...
contract LibraChainBurn is DAPP, Auth {
    ...
    function manualBurnPercent(address token, address receiver, uint256 amount) external override authorized {
        uint256 tokenAmt = IERC20(token).balanceOf(address(this));
        IERC20(token).transfer(receiver, (tokenAmt * amount / 100));
    }
