...
contract MineopolyMembership is ERC721, Ownable {
    ...
    function CrossmintNFT(address _to) public payable {
        require( USDToken.transferFrom(msg.sender, adminWallet, Price), "Failed to transfer" );

        _tokenIds.increment();
        uint256 newItemId = _tokenIds.current();

        _activeTimestamp[newItemId] = block.timestamp + _duration;
        _ownedNFTs[_to].push(newItemId);
    ...
    function reactivateNFT(uint256 tokenId) public {
        ...
        require(_activeTimestamp[tokenId] < block.timestamp, "Still Active");
