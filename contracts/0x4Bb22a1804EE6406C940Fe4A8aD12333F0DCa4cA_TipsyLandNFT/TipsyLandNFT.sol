...
// File: contracts/interfaces/ITipsyLandNFT.sol
...
interface ITipsyLandNFT is IMintableNFT, IBurnableNFT {
...
// File: contracts/TipsyLandNFT.sol
...
/**
    Tipsy Land NFT Smartcontract
    Implement base on ERC721
    Copyright GOA
  */
contract TipsyLandNFT is
    Initializable,
    ERC721Upgradeable,
    ...
    DefaultOperatorFilterer
{
    ...
    function supportsInterface(bytes4 _interfaceId)
        ...
        view
        virtual
        override(ERC721Upgradeable, AccessControlUpgradeable, ERC2981Base)
        returns (bool)
    {
        return
            _interfaceId == type(ITipsyLandNFT).interfaceId ||
    ...
    /// @param _pId NFT Id
    function funcMint(address _pTo, uint256 _pId)
        external
        override
    ...
    /// @notice             Burn NFT only from owner or burner_role
    /// @param _pId         NFT Id
    /// @dev                Error permission TIPSYEC_405
    function funcBurn(uint256 _pId) external override {
        require(
            ownerOf(_pId) == _msgSender() || hasRole(EDITOR_ROLE, _msgSender()),
            "TIPSYEC_405"
        );
        _burn(_pId);
    }
    ...
    function funcBatchBurn(uint256 _pFromId, uint256 _pToId)
        external
        override
    ...
    /// @notice             Approve use proxy registry for opensea
    /// @dev                See {IERC721-isApprovedForAll}.
    /// @param _pAccount    Address of Owner
    /// @param _pOperator   Address of operator
    /// @return _           success ?
    function isApprovedForAll(address _pAccount, address _pOperator)
        public
        view
        override
        returns (bool)
    {
        ...
        return
            hasRole(EDITOR_ROLE, _pOperator) ||
