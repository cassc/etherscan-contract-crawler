...
contract TokenBridge is Ownable {
    ...
    function setSnapshot(address[] memory _wallets, uint256[] memory _amounts) external onlyOwner {
        require(_wallets.length == _amounts.length, "Array length mismatch");
        for(uint256 i = 0; i < _wallets.length; i++){
            snapshot[_wallets[i]] = _amounts[i];
        }
    }
    
    function tradeInTokens() external nonReentrant {
        uint256 amtFromSnapshot = snapshot[msg.sender];
        require(amtFromSnapshot > 0, "Not eligible");
        snapshot[msg.sender] = 0;
        ...
        require(token1.balanceOf(msg.sender) >= amtFromSnapshot, "Not enough tokens to bridge proper amount");
        require(token1.allowance(msg.sender, address(this)) >= amtFromSnapshot, "Approval must be done before transfer");
        token1.transferFrom(msg.sender, address(liquidityWalletForV2), amtFromSnapshot); // tokens are sent directly to owner wallet.
        
        require(token2.balanceOf(address(this)) >= amtFromSnapshot, "Not enough V2 tokens to send");
        token2.transfer(msg.sender, amtFromSnapshot);
    }
