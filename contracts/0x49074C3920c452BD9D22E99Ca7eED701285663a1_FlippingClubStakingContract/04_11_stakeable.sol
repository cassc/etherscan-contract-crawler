 ...
 *                               |___/
 *
 * @title Flipping Club Staking Contract - Dependency v4.0 - flippingclub.xyz
...
contract Stakeable is ReentrancyGuard {
    ...
    function _stake(
        ...
        require(canStake(_Sender, _totalReturn), "Max open positions.");
    ...
    function _migrateStake(
        ...
        require(poolGroups[poolID].length > 0, "No Pool");
        require(canStake(msg.sender, _totalReturn), "Max open positions.");
    ...
    function canStake(address _staker, uint256 _totalReturn)
        private
        view
        returns (bool result)
    {
        ...
        for (uint256 s = 0; s < summary.stakes.length; s += 1) {
            if (summary.stakes[s].totalReturn == _totalReturn) {
                return false;
            }
        }
        return true;
