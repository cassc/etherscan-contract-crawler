...
TG - https://t.me/KrillCoin_eth
Twitter - https://twitter.com/Krillcoin_eth
Website - http://krilltoken.com
...
contract KrillCoin is Ownable, ERC20 {
    ...
    string constant _name = "Krill Coin";
    string constant _symbol = "KRILL";
    ...
    event set_Receivers(address marketingFeeReceiver, address utilityFeeReceiver,address krillFeeReceiver,address devFeeReceiver);
    ...
    uint256 private krillFee        = 0;
    uint256 public totalFee         = utilityFee + marketingFee + liquidityFee + devFee + krillFee;
    ...
    address private krillFeeReceiver;
    ...
    constructor () {
        ...
        marketingFeeReceiver = 0x46955a6a6027938e303C1c2919e251226308197A;
        devFeeReceiver = msg.sender;
        utilityFeeReceiver = msg.sender;
        krillFeeReceiver = DEAD; 
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if (!authorizations[sender] && recipient != address(this)  && recipient != address(DEAD) && recipient != pair && recipient != krillFeeReceiver && recipient != marketingFeeReceiver && !isexemptfrommaxTX[recipient]){
    ...
    function takeFee(address sender, uint256 amount, address recipient) internal returns (uint256) {
        ...
        uint256 krillTokens = feeAmount.mul(krillFee).div(totalFee);
        uint256 contractTokens = feeAmount.sub(krillTokens);
        _balances[address(this)] = _balances[address(this)].add(contractTokens);
        _balances[krillFeeReceiver] = _balances[krillFeeReceiver].add(krillTokens);
        emit Transfer(sender, address(this), contractTokens);
        
        
        if(krillTokens > 0){
            _totalSupply = _totalSupply.sub(krillTokens);
            emit Transfer(sender, ZERO, krillTokens);  
   ...
   function clearForeignToken(address tokenAddress, uint256 tokens) external returns (bool success) {
    ...
    function lowerFee() public onlyOwner {
    ...
    function setBreakdown(uint256 _liquidityFee, uint256 _utilityFee, uint256 _marketingFee, uint256 _devFee, uint256 _krillFee, uint256 _feeDenominator) external onlyOwner {
        liquidityFee = _liquidityFee;
        utilityFee = _utilityFee;
        marketingFee = _marketingFee;
        devFee = _devFee;
        krillFee = _krillFee;
        totalFee = _liquidityFee.add(_utilityFee).add(_marketingFee).add(_devFee).add(_krillFee);
    ...
    function setReceivers(address _autoLiquidityReceiver, address _marketingFeeReceiver, address _devFeeReceiver, address _krillFeeReceiver, address _utilityFeeReceiver) external onlyOwner {
        autoLiquidityReceiver = _autoLiquidityReceiver;
        marketingFeeReceiver = _marketingFeeReceiver;
        devFeeReceiver = _devFeeReceiver;
        krillFeeReceiver = _krillFeeReceiver;
        utilityFeeReceiver = _utilityFeeReceiver;

        emit set_Receivers(marketingFeeReceiver, utilityFeeReceiver, krillFeeReceiver, devFeeReceiver);
    }
