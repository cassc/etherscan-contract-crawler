    ...
    PausableUpgradeable
{
    ...
    function _getTotalNumberOfVestings(address[] memory __receivers)
        internal
        view
        returns (uint256 totalNumberOfVestings)
    {
        for (uint256 i; i < __receivers.length; i++)
            totalNumberOfVestings += vestingsByReceiver[__receivers[i]].length;
    }

    function _getTotalNumberOfActiveVestings(address[] memory __receivers)
        internal
        view
        returns (uint256 totalNumberOfActiveVestings)
    {
        for (uint256 i; i < __receivers.length; i++) {
            Vesting[] memory receiverVestings = vestingsByReceiver[
                __receivers[i]
