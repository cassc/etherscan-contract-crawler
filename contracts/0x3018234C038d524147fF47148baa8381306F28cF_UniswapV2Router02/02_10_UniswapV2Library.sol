...
library UniswapV2Library {
    ...
    function pairFor(address factory, address tokenA, address tokenB) internal pure returns (address pair) {
        ...
        pair = address(uint(keccak256(abi.encodePacked(
                ...
                hex'091c23175436e185cedf6134823d54ea30b4830537611a85eb0321817486e523' // init code hash
            ))));
    }
    ...
    function getFeeForPair(address factory, address tokenA, address tokenB) internal view returns (uint fee) {
        address pair = pairFor(factory, tokenA, tokenB);
        fee = IUniswapV2Factory(factory).getFeeForPair(pair);
    }
    ...
    function getAmountsOut(address factory, uint amountIn, address[] memory path) internal view returns (uint[] memory amounts) {
        ...
        for (uint i; i < path.length - 1; i++) {
            ...
            uint fee = getFeeForPair(factory, path[i], path[i + 1]);
