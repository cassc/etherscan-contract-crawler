...
contract LOCKDToken is ERC20 {
    using SafeMath for uint256;

    address public admin;
    uint256 public lockEndTime;

    constructor() ERC20("Dr.D 1h LOCK", "DOC") {
        admin = msg.sender;
        _mint(msg.sender, 68 * (10 ** 6) * (10 ** uint256(decimals())));
        lockEndTime = block.timestamp.add(1 hours);
    }
    ...
    modifier canTransfer() {
        require(block.timestamp >= lockEndTime, "Token is locked and cannot be transferred.");
        _;
    }
    ...
    function setLockPeriod(uint256 lockPeriod) public onlyAdmin {
