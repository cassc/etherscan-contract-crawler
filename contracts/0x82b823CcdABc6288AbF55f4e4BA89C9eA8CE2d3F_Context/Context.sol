...
contract  Eeyore is ERC20, Ownable {
    ...
    uint256 private totalBuyFees;
     ...
     EeyoreDividendTracker public dividendTracker;
    ...
    constructor() ERC20(" Eeyore", "EOR") {
        ...
        address router = (0x1F9817c8c224089413b113E97367D18411605266);
        ...
        dividendTracker = new EeyoreDividendTracker(
            payable(this),
            router,
        ...
        uint256 totalTokenSupply = (147_000_000_000) * (10**18);
        _mint(owner(), totalTokenSupply); // only time internal mint function is ever called is to create supply
        maxWallet = totalTokenSupply / 14700000; // 0.005%
        swapTokensAtAmount = totalTokenSupply / 147000000; // 0.05%;
    ...
    function setmaxWallet(uint256 value) external onlyOwner {
        value = value * (10**18);
        require(value >= _totalSupply / 147000, "max wallet cannot be set to less than 0.05%");
        maxWallet = value;
    }
...
contract EeyoreDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor(
        ...
        defaultToken = token;
         EeyoreContract =  Eeyore(mainContract);
    ...
    function _transfer(
        ...
        require(false, " Eeyore_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            " Eeyoreh_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main  Eeyore contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(EeyoreContract);

            uint256 prevBalance =  EeyoreContract.balanceOf(address(this));
            ...
            {
                uint256 received =  EeyoreContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                     EeyoreContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
