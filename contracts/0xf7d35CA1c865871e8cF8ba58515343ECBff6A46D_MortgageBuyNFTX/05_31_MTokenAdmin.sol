...
contract MTokenAdmin is MTokenCommon, MTokenAdminInterface {
    ...
    function initialize(address underlyingContract_,
        ...
        err = _setGlobalAuctionParameters(auctionMinGracePeriod, 500, 0, 5e16, 2.5e16);
     ...
     */
    function redeemInternal(uint240 mToken, uint redeemTokensIn, uint redeemAmountIn) internal returns (uint) {
        ...
        err = mtroller.redeemAllowed(mToken, redeemer, vars.redeemTokens);
        if (err != 0) {
            return failOpaque(Error.MTROLLER_REJECTION, FailureInfo.REDEEM_MTROLLER_REJECTION, err);
        }
        ...
        doTransferOut(redeemer, underlyingID, vars.redeemAmount);
    ...
    function borrowPrivate(address payable borrower, uint240 mToken, uint borrowAmount, address payable receiver) private nonReentrant returns (uint, uint) {
        ...
        uint256 underlyingID = underlyingIDs[mToken];
        if (receiver != address(0)) {
            vars.amountBorrowerReceived = doTransferOut(receiver, underlyingID, vars.amountPaidOut);
        }
        else {
            vars.amountBorrowerReceived = doTransferOut(borrower, underlyingID, vars.amountPaidOut);
        }
    ...
    function _reduceReservesFresh(uint240 mToken, uint reduceAmount) internal returns (uint) {
         ...
         */
        uint256 underlyingID = underlyingIDs[mToken];
        doTransferOut(admin, underlyingID, reduceAmount);
