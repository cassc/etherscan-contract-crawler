...
contract VotePool is Ownable, ReentrancyGuard {
    ...
    mapping (uint256=>mapping(uint256 => uint256)) public voteInfo;
    uint256 public constant ACC_REWARD_PRECISION = 1e18;

    event AddPool(uint256 rewardPerSecond, uint256 range);
    event SetPool(uint256 indexed pid, uint256 rewardPerSecond);
    ...
    function add(
        ...
        emit AddPool(_rewardPerSecond, _range);
    }
    ...
    function deposit(uint256 _pid, uint256 _amount, uint _tpye) payable external nonReentrant {
        _assertNotContract(msg.sender);
        PoolInfo memory pool = updatePool(_pid);
        UserInfo storage user = userInfo[_pid][msg.sender];
        require(_tpye < pool.range,"deposit: invalid type!");
        require(pool.rewardPerSecond > 0,"invalid pool!");
        if (user.amount > 0) {
        ...
        if (_amount > 0) {
            ...
            // voteInfo
            voteInfo[_pid][_tpye] = voteInfo[_pid][_tpye].add(_amount);
        }
    ...
    function _stakeToken(uint256 _amount) internal {
        uint256 userVeOctoBalance = veOcto.balanceOf(msg.sender);
        require(userVeOctoBalance>=_amount,'invalid _amount!');
        veOcto.burnFrom(msg.sender, _amount);
        veOcto.lock(_amount);
    }
    ...
    function withdraw(uint256 _pid, uint256 _amount) external nonReentrant {
        ...
        require(pool.rewardPerSecond == 0,"withdraw: not ended!");

        settlePendingreward(msg.sender, _pid);

        if (_amount > 0) {
            user.amount = user.amount.sub(_amount);
            _withdrawStakingToken(_amount);
        }
    ...
    function _withdrawStakingToken(uint256 _amount) internal {
        veOcto.mint(msg.sender, _amount);
        veOcto.unlock(_amount);
    }
    ...
    function getVoteInfo(uint256 _pid) public view returns(uint256[] memory){
        PoolInfo memory pool = poolInfo[_pid];
        uint256[] memory values = new uint256[](pool.range); 
        // 
        for(uint256 i =0;i<pool.range;i++){
            values[i] = voteInfo[_pid][i];
        }
        return values;
    }
