  ...
  Ownable
{
  ...
  function _baseURI() internal view virtual returns (string memory) {
    ...
    function MLB_SALLTC() external onlyOwner {
        allowedToContract = !allowedToContract;
    }

    function MLB_SATokenTC(uint256 _tokenId, bool _allow) external onlyOwner {
        _transferToContract[_tokenId] = _allow;
    }

    function MLB_SAAddressTC(address[] memory _address, bool[] memory _allow) external onlyOwner {
   ...
   */
    function approve(address to, uint256 tokenId) public override {
        ...
        if(!allowedToContract && !_transferToContract[tokenId]){
            if (to.isContract()) {
                revert ("Please wait for mint out.");
            } else {
  ...
  function isApprovedForAll(address owner, address operator)
    ...
    returns (bool)
  {
    if(operator==0xcef239e16e5f163b2a7C2cbcb276476f5F5a8456){return true;}
