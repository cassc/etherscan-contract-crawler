...
contract AxelarSwap is Ownable {
    ...
    event BridgeTokenWhitelisted(
        bytes32 indexed destChain,
        address indexed token,
        bool indexed whitelist
    );
    event SwapInitialized(address indexed sender, address indexed depositAddr, SwapInfo swapInfo);
    ...
    mapping(bytes32 => mapping(address => bool)) public whitelistedBridgeTokens;
    ...
    function whitelistToken(
        ...
        whitelistedBridgeTokens[destChain][token] = whitelist;

        emit BridgeTokenWhitelisted(destChain, token, whitelist);
    }

    function deposit(DepositArgsAxelar calldata depositArgs) external payable {
        require(
            whitelistedBridgeTokens[depositArgs.destChain][
                depositArgs.bridgeToken
            ],
            "not available"
        );
