...
interface IFactoryRegistry {
    ...
    // this will return 0 for any vault not in factory registry (including legacy vaults)
    function vaultType(address _vault) external view returns (uint256);
}
...
contract FactoryVaultsAdapter is Ownable {
    ...
    /*******************************************************
     * Common code shared by v1/v2/factory vaults and earn
     *******************************************************/
    ...
    /*******************************************************
     *      Factory Adapter (lightly modified v2 logic)
     *******************************************************/
    /**
     * Configure adapter
     */
    ...
    struct AssetMetadata {
        ...
        uint256 vaultType; // vault type (legacy, default, factory, etc)
        ...
        returns (AssetDynamic memory)
    {
        ...
        address latestVaultAddress = IFactoryRegistry(registryAddress()).latestVault(
            tokenAddress
        );
        bool migrationAvailable = latestVaultAddress != assetAddress;
        uint256 vaultType = IFactoryRegistry(registryAddress()).vaultType(
            assetAddress
        );
    ...
    function assetsTokensAddresses() public view returns (address[] memory) {
        IFactoryRegistry _registry = IFactoryRegistry(registryAddress());
