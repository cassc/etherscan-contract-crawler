...
// @Kairos V2.0
// This version includes a maxMintPerWallet property
// Also returns AlreadyMinted on already minted error instead InvalidSignature
// Added some minnor optimizations
    ...
    ITokenERC721
{
    ...
    // New on V2
    /// @dev Maximum aallowed amount of mints per wallet
    uint32 public maxMintsPerWallet;
    mapping(address => uint32) private mintsPerWallet;
    ...
    function tryFiatMint (FiatMintRequest[] calldata _reqArray)        
        ...
        returns (uint256)
    {
        uint256 arrayLength = _reqArray.length;
    ...
    /// New on V2
    /// @dev Lets a module admin set the maxMintsPerWallet.
    function setMaxMintsPerWallet(uint32 _maxMintsPerWallet) external onlyRole(DEFAULT_ADMIN_ROLE) {
    ...
    function _mintTo(address _to, string calldata _uri) internal returns (uint256 tokenIdToMint) {
        ...
        if (maxMintsPerWallet != 0 && mintsPerWallet[_to] >= maxMintsPerWallet) {
            revert MaxMintReached(maxMintsPerWallet);
        }
        mintsPerWallet[_to] +=1;
