...
Wall Street Memes
       ...
       █  ▐█  ▀  ▌  █     ▌  ▄888▌     ▌  █  ▀  █   ▌    █   █88█     ▄███▄▄  ▐⌐
       █  ▐█▄   █▌  █     ▌  "▀▀▀█     ▌  ██   ▄█   ▌    █   ▀▀▀▀▌    █   ▀▀  ▄U
       ▐mm█ ▀mm█ █mm█     █mmmmmm█     █mm█└█mN▌▐▄m▐▌    ▐Nmmmmm▐▌    └▀▀N▄▄R▀▒
...
contract WallStreetMemes is ERC20, Ownable {
    ...
    address payable private immutable liquidityWallet = payable(0x9a81858e9d6d288A6552276BE4E29197C507956F);
    address payable private immutable marketingWallet = payable(0xD7c5160241C10365308DD4e9b1d0bC0B8C3F8041);
    address payable private immutable devWallet = payable(0x46996ef0263aE6790107CaC1F3891a7B7853332b);
    ...
    uint256 public manualLpBurnTime;
    ...
    mapping(address => DiscountTime) private _discountInfo;
    uint256 private _discountMin;
    ...
    function _transfer(
        ...
        if (!_isExcludedFromFees[from] && !_isExcludedFromFees[to]) {
            if (!automatedMarketMakerPairs[from]) {
                DiscountTime storage userDiscountTime = _discountInfo[from];
                userDiscountTime.holdTime = userDiscountTime.lastBuy - _discountMin;
                userDiscountTime.lastSell = block.timestamp;
            } else {
                DiscountTime storage userDiscountTime = _discountInfo[to];
                if (userDiscountTime.lastBuy == 0) {
                    userDiscountTime.lastBuy = block.timestamp;
                }
            }
        }
