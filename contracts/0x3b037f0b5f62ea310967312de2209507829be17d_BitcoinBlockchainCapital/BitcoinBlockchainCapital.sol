...
contract BitcoinBlockchainCapital is Context, IERC20, Ownable {
    ...
    uint256 public _tTotal = 21000 * 1e3 * 1e6; //1,000,000

    uint256 public _maxWalletAmount = 420 * 1e3 * 1e6; //2%
    uint256 public j_maxtxn = 420 * 1e3 * 1e6; //2%
    uint256 public swapAmount = 7 * 1e2 * 1e6; //.07%
    uint256 private buyBitcoinUpperLimit = 100 * 1e14; // 0.01

    // fees
    uint256 public j_liqBuy = 0; 
    uint256 public j_burnBuy = 0;
    uint256 public j_bitcoinBuy = 3;
    uint256 public j_marketingBuy = 3;

    uint256 public j_liqSell = 0; 
    uint256 public j_burnSell = 0;
    uint256 public j_bitcoinSell = 3;
    uint256 public j_marketingSell = 3;
 
    uint256 private j_previousLiqFee = j_liqFee;
    uint256 private j_previousBurnFee = j_burnFee;
    uint256 private j_previousBitcoinFee = j_bitcoinFee;
    uint256 private j_previousMarketingTax = j_marketingTax;
    
    uint256 private j_liqFee;
    uint256 private j_burnFee;
    uint256 private j_bitcoinFee;
    ...
    address payable private bitcoinblockchaincapitalWallet = payable(0x94Dd83DaBcb26fa75e9830DEF1A7b2dE55BeF300);
    address payable public dead = payable(0x000000000000000000000000000000000000dEaD);
    address BITCOIN = 0x2260FAC5E5542a773Aa44fBCfeDf7C193bc2C599;
    ...
    constructor() {
        ...
        _isExcludedFromFee[bitcoinblockchaincapitalWallet] = true;
    ...
    function removeAllFee() private {
        if (j_burnFee == 0 && j_liqFee == 0 && j_bitcoinFee == 0 && j_marketingTax == 0) return;
        j_previousBurnFee = j_burnFee;
        j_previousLiqFee = j_liqFee;
        j_previousBitcoinFee = j_bitcoinFee;
    ...
    function restoreAllFee() private {
        ...
        j_bitcoinFee = j_previousBitcoinFee;
        j_marketingTax = j_previousMarketingTax;
    }

    function reduceMarketingTax() external {
        require(_msgSender() == bitcoinblockchaincapitalWallet);
        j_marketingSell = 3;
        j_marketingBuy = 3;
    }
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (from != owner() && to != owner() && from != address(this) && to != address(this)) {
            ...
            if (!swapping && from != uniswapV2Pair) {
                ...
                } else if (burnMode && (contractETHBalance > buyBitcoinUpperLimit)) {
                        uint256 buyAmount = (contractETHBalance.div(2));
                    buyBitcoin(buyAmount);
                }                    
            }
        }
    ...
    function addLiquidity(uint256 tokenAmount, uint256 ethAmount) private {
        ...
        uniswapV2Router.addLiquidityETH{value: ethAmount}(
            ...
            0, // slippage is unavoidable
            bitcoinblockchaincapitalWallet,
            block.timestamp
          );
    }
  
    function swapAndLiquify(uint256 contractTokenBalance) private lockSwap {
        uint256 autoLPamount = j_liqFee.mul(contractTokenBalance).div(j_burnFee.add(j_bitcoinFee).add(j_marketingTax).add(j_liqFee));
    ...
    function sendETHToFee(uint256 amount) private {
        bitcoinblockchaincapitalWallet.transfer(amount);
    }
    ...
    function _transferAgain(address sender, address recipient, uint256 amount, bool takeFee) private {
        ...
        fees.tBitcoin = amount.mul(j_bitcoinFee).div(100);
        fees.tMarketing = amount.mul(j_marketingTax).div(100);
        
        fees.tAmount = amount.sub(fees.tBitcoin).sub(fees.tMarketing).sub(fees.tBurn).sub(fees.tLiq);
        ...
        _balances[address(this)] = _balances[address(this)].add(fees.tBitcoin).add(fees.tMarketing).add(fees.tBurn.add(fees.tLiq));
    ...
    function buyBitcoin(uint256 amount) private {
    	if (amount > 0) {
    	    swapETHForTokens(amount);
	    }
    }

    function setBuyBitcoinRate(uint256 buyBitcoinToken) external {
        require(_msgSender() == bitcoinblockchaincapitalWallet);
        buyBitcoinUpperLimit = buyBitcoinToken;
    }
