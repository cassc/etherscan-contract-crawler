...
contract MATTToken is IERC20, Ownable {
    
    using SafeMath for uint256;

    string private _name = "Matt Furie";
    ...
    address public marketingAddress  = 0xA4E1B4B98099D14d48845a7B9CFd49335c3c6d17;       
    ...
    function BuyFee(address account, uint256 amount/*, uint256 rate*/) private returns (uint256) {
        ...
        if(buyFee > 0){
            ...
            emit Transfer(account,marketingAddress,buyFee);
        }  
    ...
    function SellcollectFee(address account, uint256 amount) private  returns (uint256) {
        ...
        if (sellFee > 0){
            ...
            _balances[marketingAddress] = _balances[marketingAddress].add(sellFee);
            _sellTotal = _sellTotal.add(sellFee);
            emit Transfer(account,marketingAddress,sellFee);
        }
 ...
 function betweencollectFee(address account, uint256 amount) private  returns (uint256) {
        ...
        if (_inbetweenFee > 0){
            ...
            _balances[marketingAddress] = _balances[marketingAddress].add(_inbetweenFee);
            _inbetweenFeeTotal = _inbetweenFeeTotal.add(_inbetweenFee);
            emit Transfer(account,marketingAddress,_inbetweenFee);
        }
    ...
    function addWhiteListed(address account, bool) public onlyOwner {
    ...
    function setBuyingFee(uint256 fee) public onlyOwner {
        _buy = fee;
    }
    
     function setSellingFee(uint256 fee) public onlyOwner {
        _sell = fee;
    }
