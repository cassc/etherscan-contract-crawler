...
interface IMarketplaceCore {
     ...
     * @param completable     - Whether or not Christie's has approved the sale to be finalized
     ...
     * @dev Create listing
     */
    function createListing(address payable seller, MarketplaceLib.ListingDetails calldata listingDetails, MarketplaceLib.TokenDetails calldata tokenDetails, MarketplaceLib.ListingReceiver[] calldata listingReceivers, bool enableReferrer) external returns (uint40);
    ...
    /**
     * @dev Complete listing (allow winner to finalize)
     */
    function completeListing(uint40 listingId, MarketplaceLib.DeliveryFees calldata fees) external;

    /**
     * @dev Cancel listing
     */
    function cancelListing(uint40 listingId, uint16 holdbackBPS) external;
