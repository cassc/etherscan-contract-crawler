...
/**
 * @dev This contract allows the dot dev to lock an ERC721 NFT (in this case a UniswapV3 LP token)
 * and at some point in the future trigger a 6-month timer. After the timer expires, the dot dev can withdraw the locked NFT.
...
contract DOTLPLock {
    ...
    modifier onlydot() {
        require(msg.sender == dot, "Only dot can call this function");
        _;
    }
    ...
    function lockNFT(uint256 tokenId) external onlydot {
    ...
    function triggerWithdrawal() external onlydot {
    ...
    function cancelWithdrawalTrigger() external onlydot {
        require(isNFTLocked, "NFT is not locked");
    ...
    function withdrawNFT(uint256 tokenId) external onlydot {
        require(isNFTLocked, "NFT is not locked");
        require(lockUpEndTime != 0, "Withdrawal is not triggered");
    ...
    function changeOwner(address newOwner) external onlydot {
