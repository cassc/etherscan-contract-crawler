...
contract MagpieRouter is ReentrancyGuard, Ownable, IMagpieRouter {
    ...
    modifier onlyMagpieCore() {
        require(msg.sender == magpieCoreAddress, "MagpieRouter: only MagpieCore allowed");
        _;
    }
    ...
    function withdraw(address weth, uint256 amount) external override onlyMagpieCore {
        IWETH(weth).withdraw(amount);
        (bool success, ) = msg.sender.call{value: amount}(new bytes(0));
        require(success, "MagpieRouter: eth transfer failed");
    }

    function swap(SwapArgs memory swapArgs) external override onlyMagpieCore returns (uint256[] memory amountOuts) {
