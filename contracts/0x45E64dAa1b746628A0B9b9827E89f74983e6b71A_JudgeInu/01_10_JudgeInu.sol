...
contract JudgeInu is Context, ERC20, Ownable {
    ...
    uint256 private constant _totalSupply = 62_626_626_626_626 * (10**18);

    uint256 public maxBuyAmnt = _totalSupply.mul(15).div(1000);
    uint256 public maxSellAmnt = _totalSupply.mul(15).div(1000);
    uint256 public maxWalletAmnt = _totalSupply.mul(15).div(1000);
    ...
    constructor () ERC20("Judge Inu", "62") {
    ...
    function _transfer(address from, address to, uint256 amount) internal override {
        require(from != ZERO, "62: transfer from the zero address");
        require(to != ZERO, "62: transfer to the zero address");
        require(amount > 0, "62: Transfer amount must be greater than zero");

        bool takeFee = true;
        bool shouldSwap = false;
        if (from != owner() && to != owner() && to != ZERO && to != DEAD && !_swapping) {
            if(!tradingOpen) require(_isExcludedFromFees[from] || _isExcludedFromFees[to], "62: Trading is not allowed yet.");

            if (from == _uniswapV2Pair && to != address(_uniswapV2Router) && !_isExcludedMaxTransactionAmount[to]) {
                require(amount <= maxBuyAmnt, "62: Transfer amount exceeds the maxBuyAmnt.");
                require(balanceOf(to) + amount <= maxWalletAmnt, "62: Exceeds maximum wallet token amount.");
            }
            
            if (to == _uniswapV2Pair && from != address(_uniswapV2Router) && !_isExcludedMaxTransactionAmount[from]) {
                require(amount <= maxSellAmnt, "62: Transfer amount exceeds the maxSellAmnt.");
    ...
    function _handleFee(uint256 contractBalance) internal {
    ...
    function openTrading() public onlyOwner {
        require(!tradingOpen,"62: Trading is already open");
    ...
    function setBuyThreshold(uint256 _maxBuyAmnt) public onlyOwner {
        require(_maxBuyAmnt >= (totalSupply().mul(1).div(1000)), "62: Max buy amount cannot be lower than 0.1% total supply.");
        maxBuyAmnt = _maxBuyAmnt;
    }

    function setSellThreshold(uint256 _maxSellAmnt) public onlyOwner {
        require(_maxSellAmnt >= (totalSupply().mul(1).div(1000)), "62: Max sell amount cannot be lower than 0.1% total supply.");
        maxSellAmnt = _maxSellAmnt;
    }
    
    function setWalletThreshold(uint256 _maxWalletAmnt) public onlyOwner {
        require(_maxWalletAmnt >= (totalSupply().mul(1).div(100)), "62: Max wallet amount cannot be lower than 1% total supply.");
        maxWalletAmnt = _maxWalletAmnt;
    }
    
    function setSwapTokensAtAmount(uint256 _swapAmountAmnt) public onlyOwner {
        require(_swapAmountAmnt >= (totalSupply().mul(1).div(100000)), "62: Swap amount cannot be lower than 0.001% total supply.");
        require(_swapAmountAmnt <= (totalSupply().mul(5).div(1000)), "62: Swap amount cannot be higher than 0.5% total supply.");
    ...
    function setSwapFeeCollector(address swapFeeCollectorAddy) public onlyOwner {
        require(swapFeeCollectorAddy != ZERO, "62: swapFeeCollector address cannot be 0");
