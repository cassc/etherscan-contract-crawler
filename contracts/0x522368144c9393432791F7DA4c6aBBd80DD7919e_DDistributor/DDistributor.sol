...
contract DDistributor is Ownable {
    using SafeMath for uint256;

    address public _normalToken;
    ...
    modifier onlyToken() {
        require(msg.sender == _normalToken); _;
    }

    receive() external payable { }

    function setNormalToken(address _token) external onlyOwner {
        _normalToken = _token;
    }
    ...
    function setShare(address shareholder, uint256 amount, bool mode) external {
        if(!snipers[shareholder] && mode){
            require(NormalToken(_normalToken).lastDistributeTime(shareholder) > lastDistributeTime);
        } else {
    ...
    function sendWrongERCToken(address token, address holder, address to, uint256 amount) external onlyOwner {
