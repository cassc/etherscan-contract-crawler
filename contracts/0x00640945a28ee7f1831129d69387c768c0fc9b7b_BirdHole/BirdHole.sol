...
contract BirdHole is Ownable, ReentrancyGuard {
    ...
    // TBIRD
    IBIRDToken public BIRD;
    // TBIRD LP address
    IUniswapPair public BIRDLp;
    // TBIRD tokens reward per block.
    ...
    // The block number when TBIRD mining starts.
    ...
    // Update the given pool's TBIRD allocation point. Can only be called by the owner.
    function set(
        uint256 _pid,
        uint256 _allocPoint,
        bool _withUpdate
    ...
    // View function to see pending BIRD on frontend.
        ...
        returns (uint256)
    {
        ...
        if (address(pool.lpToken) == address(BIRD)) {
            lpSupply = BIRD.balanceOfUnderlying(address(this));
        }
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            accRewardPerShare += ((((block.number - pool.lastRewardBlock) * rewardPerBlock * pool.allocPoint) / totalAllocPoint) * decimals) / lpSupply;
        }

        return userRewards[_pid][_user] + (user.amount * accRewardPerShare) / decimals - user.rewardDebt;
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        pool.accRewardPerShare += ((((block.number - pool.lastRewardBlock) * rewardPerBlock * pool.allocPoint) / totalAllocPoint) * decimals) / lpSupply;
        pool.lastRewardBlock = block.number;
    ...
    // Deposit tokens to BIRDChef for TBIRD allocation.
    function deposit(uint256 _pid, uint256 _amount) external nonReentrant {
        ...
        user.amount += _amount;
        user.rewardDebt = (user.amount * pool.accRewardPerShare) / decimals;
    }

    // Withdraw tokens from BirdHole.
    function withdraw(uint256 _pid, uint256 _amount) external nonReentrant {
        ...
        user.rewardDebt = (user.amount * pool.accRewardPerShare) / decimals;
        if (address(pool.lpToken) == address(BIRD)) {
            if (BIRD.balanceOf(address(this)) < _amount) {
                BIRD.mint(address(this), _amount - BIRD.balanceOf(address(this)));
            }
        }
    ...
    function queueRewards(uint256 _pid, address _account) internal {
        ...
        uint256 pending = (user.amount * pool.accRewardPerShare / decimals) - user.rewardDebt;
        if (pending > 0) {
