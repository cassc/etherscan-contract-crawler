...
/// @title The Nouns Composable descriptor
...
import { INounsComposableDescriptor } from './interfaces/INounsComposableDescriptor.sol';
...
contract NounsComposableDescriptor is INounsComposableDescriptor, Ownable {
    ...
    /// @notice The contract responsible for composability extension
    INounsComposer public composer;
    ...
    constructor(INounsArt _art, ISVGRenderer _renderer, INounsComposer _composer) {
        art = _art;
        renderer = _renderer;
        composer = _composer;
    }
    ...
    /**
     * @notice Set the Composer.
     * @dev Only callable by the owner.
     */
    function setComposer(INounsComposer _composer) external onlyOwner {
        composer = _composer;

        emit ComposerUpdated(_composer);
    }
    ...
    function dataURI(uint256 tokenId, INounsSeeder.Seed memory seed) public view override returns (string memory) {
        ...
        string memory name = string(abi.encodePacked('CX Noun ', nounId));
        string memory description = string(abi.encodePacked('CX Noun ', nounId, ' is a member of the Composables DAO'));

        return genericDataURI(name, description, tokenId, seed);
    }

    /**
     * @notice Given a name, description, tokenId and seed, construct a base64 encoded data URI.
     */
    ...
    /**
     * @notice Given a tokenId and seed, construct a base64 encoded SVG image.
     */
    ...
    /**
     * @notice Get all Noun parts for the passed tokenId and seed.
     */
    function getPartsForSeed(uint256 tokenId, INounsSeeder.Seed memory seed) public view returns (ISVGRenderer.Part[] memory) {
		...
		//Get the composable parts
    	ISVGRenderer.Part[] memory composableParts = composer.getParts(tokenId);
		uint256 len = composableParts.length;
    	
    	if (len < 12) {
    		return parts; //fallback for misconfigured composers
    	}

		//Merge the base parts and composable parts
		//Use the base parts if they have not been replaced
		if (composableParts[4].image.length == 0) {
			composableParts[4] = parts[0];
		}
		if (composableParts[6].image.length == 0) {
			composableParts[6] = parts[1];
		}
		if (composableParts[8].image.length == 0) {
			composableParts[8] = parts[2];
		}
		if (composableParts[10].image.length == 0) {
			composableParts[10] = parts[3];
		}

		//Count the number of non-empty spots
    	uint256 mergedCount = 0;
    	for (uint256 i = 0; i < len;) {
        	if (composableParts[i].image.length > 0) {
				unchecked {
	            	mergedCount++;
	        	}
            }
		...
		//Merge into resized array with no empty slots for renderer to handle
		uint256 mergedIndex = 0;
		ISVGRenderer.Part[] memory mergedParts = new ISVGRenderer.Part[](mergedCount);
    	for (uint256 i = 0; i < len;) {
        	if (composableParts[i].image.length > 0) {
            	mergedParts[mergedIndex] = composableParts[i];
				unchecked {
	            	mergedIndex++;
	        	}
            }
