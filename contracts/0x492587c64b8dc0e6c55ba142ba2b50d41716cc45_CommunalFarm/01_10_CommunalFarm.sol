...
contract CommunalFarm is Ownable, ReentrancyGuard {
    ...
    uint256 public lock_time_for_max_multiplier = 86400; // 1 day
    uint256 public lock_time_min = 60 * 10; // 86400; // 10 minutes
        ...
        if (_total_liquidity_locked == 0 || _total_combined_weight == 0) {
            ...
            for (uint256 i = 0; i < rewardsPerTokenStored.length; i++) {
                newRewardsPerTokenStored[i] = rewardsPerTokenStored[i].add(
                    lastTimeRewardApplicable().sub(lastUpdateTime).mul(rewardRates[i]).mul(1e18).div(
    ...
    function stakeLockedFor(uint256 liquidity, uint256 secs, address account) public nonReentrant {
