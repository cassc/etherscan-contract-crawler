...
contract ScalarFarmPools is Ownable, ReentrancyGuard {
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint;       // How many allocation points assigned to this pool. scalars to distribute per block.
        uint256 duration;
        uint256 startBlock;
        uint256 bonusEndBlock;
        uint256 lastRewardBlock;  // Last block number that scalars distribution occurs.
        uint256 accTokenPerShare;   // Accumulated scalars per share, times 1e12. See below.
        uint256 accReflectionPerShare;   // Accumulated scalars per share, times 1e12. See below.
    ...
    IERC20 public scalar;
    // Reflection Token
    address public reflectionToken;
    uint256 public accReflectionPerPoint;
    bool public hasDividend;
    uint256 public rewardPerBlock;
    // Bonus muliplier for early scalar makers.
    uint256 public constant BONUS_MULTIPLIER = 1;
    // Deposit Fee address
    address public feeAddress;
    address public treasuryWallet = 0xe6C9e75927058e0E1503fdd04DB80Ea1160037A3;
    ...
    // The block number when scalar mining starts.
    ...
    event SetTreasuryUpdate(address indexed user, address newAddress);
    event SetPerformanceFee(uint256 fee);
    event UpdateEmissionRate(address indexed user, uint256 rewardPerBlock);

    constructor(IERC20 _scalar, address _reflectionToken, uint256 _rewardPerBlock, bool _hasDividend) {
        scalar = _scalar;
        ...
        startBlock = block.number.add(30 * 6219);
    }
    ...
    // Update the given pool's scalar allocation point and deposit fee. Can only be called by the owner.
    ...
    // View function to see pending scalar on frontend.
    function pendingRewards(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 scalarReward = multiplier.mul(rewardPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
            accTokenPerShare = accTokenPerShare.add(scalarReward.mul(1e12).div(lpSupply));
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number, pool.bonusEndBlock);
        uint256 scalarReward = multiplier.mul(rewardPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        pool.accTokenPerShare = pool.accTokenPerShare.add(scalarReward.mul(1e12).div(lpSupply));
    ...
    function compoundReward(uint256 _pid) external payable nonReentrant {
        ...
        if(address(scalar) != address(pool.lpToken)) {
            uint256 tokenAmt = pending / 2;
            uint256 tokenAmt0 = tokenAmt;
            address token0 = address(scalar);
    ...
    function _calculateTotalStaked(uint256 _pid, IERC20 _lpToken, uint256 _amount, bool _deposit) internal {
        if(_deposit) {
            ...
            if(address(_lpToken) == address(scalar)) {
    ...
    function availableRewardTokens() public view returns (uint256) {
        if(address(scalar) == reflectionToken) return totalEarned;

        uint256 _amount = scalar.balanceOf(address(this));
    ...
    // Safe scalar transfer function, just in case if rounding error causes pool to not have enough scalars.
    function safeTokenTransfer(address _to, uint256 _amount) internal {
        uint256 scalarBal = scalar.balanceOf(address(this));
        bool transferSuccess = false;
        if (_amount > scalarBal) {
            transferSuccess = scalar.transfer(_to, scalarBal);
        } else {
            transferSuccess = scalar.transfer(_to, _amount);
        }
    ...
    function SetTreasuryWallet(address _addr) external {
        ...
        emit SetTreasuryUpdate(msg.sender, _addr);
    }

    //scalar has to add hidden dummy pools inorder to alter the emission, here we make it simple and transparent to all.
    ...
    function depositRewardsFarm(uint _amount) external nonReentrant {
        require(_amount > 0);

        uint256 beforeAmt = scalar.balanceOf(address(this));
        scalar.safeTransferFrom(msg.sender, address(this), _amount);
        uint256 afterAmt = scalar.balanceOf(address(this));
    ...
    function recoverWrongToken(address _token) external onlyOwner {
        require(_token != address(scalar) && _token != reflectionToken, "cannot recover reward token or reflection token");
