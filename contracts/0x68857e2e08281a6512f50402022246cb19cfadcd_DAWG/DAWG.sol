...
contract DAWG is ERC20 {
    mapping(address => bool) private _taxExempt;
    address private _taxCollector;
    uint256 private constant _taxRate = 1;

    constructor(address initialOwner, address taxCollector) ERC20("Barnyard Dawg", "DAWG", 18, initialOwner) {
        require(taxCollector != address(0), "DAWG: tax collector is the zero address");
    ...
    function setTaxCollector(address collector) public onlyOwner {
        require(collector != address(0), "DAWG: collector is the zero address");
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual override {
        if (_taxExempt[recipient] && !_taxExempt[sender] && _taxCollector != address(0)) {
            ...
            require(amount == netAmount + tax, "DAWG: tax calculation overflow");
