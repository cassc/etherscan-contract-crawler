...
contract TWMStaking is Ownable, ReentrancyGuard {
    ...
    function deposit(
        ...
        for (uint256 i; i < tokenIds.length; i++) {
            ...
            if (
                user.currentYield != 0 ||
                contractType != ContractTypes.SECONDCOL
            ) {
        ...
        if (
            user.currentYield == 0 &&
            newYield != 0 &&
            user.stakedSECOND.length > 0
        ) {
            for (uint256 i; i < user.stakedSECOND.length; i++) {
                uint256 tokenYield = getTokenYield(
                    address(SecondCollection),
                    user.stakedSECOND[i]
                );
                newYield += tokenYield;
            }
    ...
    function withdraw(
        ...
        if (user.stakedFIRST.length == 0 && user.stakedTHIRD.length == 0) {
            newYield = 0;
        }
    ...
    function registerDeposit(
        ...
        if (user.currentYield != 0 || contractType != ContractTypes.SECONDCOL) {
            newYield += getTokenYield(contractAddress, tokenId);
        }

        if (contractType == ContractTypes.SECONDCOL) {
            user.stakedSECOND.push(tokenId);
        }
        if (contractType == ContractTypes.THIRDCOL) {
            user.stakedTHIRD.push(tokenId);
        }
