...
contract LGPN is ERC721Enumerable, Ownable {
    ...
    string private _uri = "https://nft.lgpdao.com/";
    string private _suffix = ".json";
    mapping(uint256 => uint256) public cidTotals;
    ...
    constructor() ERC721("LGP NFT", "LGPN") {
        manager = 0x1Df6Bcef949B52192D04923d59e630f7F5ca5E88;
        boxAddress = 0xc58872158Dd04C43ffA3B613AE108bB32bafdE30;
        cidTotals[1] = 20;
    }
    ...
    function setCID(uint256 level, uint256 total) public {
        if (owner() == _msgSender() || manager == _msgSender()) {
            cidTotals[level] = total;
        }
    }
        ...
        returns (string memory)
    {
        ...
        Card memory card = _cards[tokenId];
        uint256 cid = (card.levelIndex % cidTotals[card.level]) + 1;
        string memory uri = _strConcat(
            _baseURI(),
            _strConcat(card.level.toString(), "/")
        );
        return _strConcat(uri, _strConcat(cid.toString(), _suffix));
    }
