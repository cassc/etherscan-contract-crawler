...
contract NFTFactoryV3 is NFTFactoryV2 {
    /**
     * @dev Function deploying a new collection proxy, initializing it with the specified parameters,
     * recording the new deployed collection locally and emitting an event
     * @param data byte encoded parameters to deploy a collection
     */
    function _newCollection(bytes calldata data, address _baseOperators) internal virtual returns (address) {
        TransparentUpgradeableProxy proxy = new TransparentUpgradeableProxy(
            implementation,
            proxyAdmin,
            abi.encodeWithSignature("initialize(bytes,address)", data, _baseOperators)
        );
     ...
     * @param data byte encoded parameters to deploy a collection
     */
    function newCollection(bytes calldata data) public virtual onlyOperator returns (address) {
        return _newCollection(data, collectionBaseOperators);
    }
