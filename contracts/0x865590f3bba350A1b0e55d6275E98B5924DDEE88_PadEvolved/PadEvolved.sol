...
contract ERC721 is ERC165, IERC721, IERC721Metadata, Ownable, DefaultOperatorFilterer {
    ...
    function _mintID(address to, uint tokenId) internal {
        _owners[tokenId] = to;
        unchecked {
            ++_balances[to];
        }
...
contract PadEvolved is Ownable, IERC2981, ERC721 {

    bool public evolvingEnabled;

    uint private EIP2981RoyaltyPercent;

    IPAD PAD;
    IShroomies SHROOMIES;
    ...
    function initialize(
        ...
        SHROOMIES = IShroomies(shroomies);
        PAD = IPAD(pad);
    ...
    function evolve(uint id, uint shroomie) external {
        require(evolvingEnabled, "Evolving is not enabled!");
        require(PAD.ownerOf(id) == msg.sender, "caller not PAD owner!");
        require(_owners[id] == address(0), "allready evolved!");
        SHROOMIES.burnShroomie(shroomie, msg.sender);
        _mintID(msg.sender, id);
    }

    function evolveBatch(uint[] memory ids, uint[] memory shroomies) external {
        require(evolvingEnabled, "Evolving is not enabled!");
        uint amount = ids.length;
        require(amount == shroomies.length, "array length missmatch!");
        SHROOMIES.burnShroomies(shroomies, msg.sender);
        _batchMintID(msg.sender, ids, amount);
    }

    function _batchMintID(address to, uint[] memory ids, uint amount) internal {
        uint cId;
        ...
        for (uint i; i < amount;) {
            ...
            require(_owners[cId] == address(0), "PAD id already evolved");

            _owners[cId] = to;
            emit Transfer(address(0), to, cId);
            
            unchecked { ++i; }

            require(PAD.ownerOf(cId) == to, "PAD owner id missmatch!");

            require(
                _checkOnERC721Received(address(0), to, cId, ""),
                "ERC721: transfer to non ERC721Receiver implementer"
            );
        }
    }
    ...
    function toggleMinting() external onlyOwner {
        evolvingEnabled = !evolvingEnabled;
    }
