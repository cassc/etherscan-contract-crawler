...
contract SDYCAggregator is OwnableUpgradeable, UUPSUpgradeable {
     ...
     * @param principal is the balance with 6 decimals of precision
     * @return roundId of the new round data
     */
    function reportBalance(uint256 principal, uint256 interest) external returns (uint80 roundId) {
        _checkOwner();
        ...
        uint256 answer = FixedPointMathLib.mulDivDown(principal + interest, 10 ** decimals, token.totalSupply());

        rounds[roundId] = RoundData(int256(answer), interest, block.timestamp);

        totalInterestAccrued += interest;

        emit BalanceReported(roundId, principal, answer, block.timestamp);
    }
}