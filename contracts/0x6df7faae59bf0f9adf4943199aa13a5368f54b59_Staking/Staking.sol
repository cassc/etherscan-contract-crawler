...
contract Staking is Ownable{
    ...
    function setFeeReciever(address _newAdd) external onlyOwner{
        feeReceiver = _newAdd;
    }
    ...
    function rewardCalculation(address _user) public view returns(uint256){
        uint256 point = ((block.timestamp - users[_user].stakedTime)/86400 seconds)*10**18;
        uint256 totalReward= ((percentage*point)/10000)*users[_user].stakedTokens;
