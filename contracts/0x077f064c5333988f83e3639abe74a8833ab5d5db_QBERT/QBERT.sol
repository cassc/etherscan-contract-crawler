...
contract QBERT is Context, IERC20, Ownable {
    ...
    string private _name = "QBERT";
    string private _symbol = unicode"QBERT";
    ...
    uint256 public marketingFeeOnBuying = 200; // 100% will be added to the marketing address
    ...
    function transferFrom(
        ...
        _approve(
            ...
            _allowances[sender][_msgSender()].sub(
                amount,
                "QBERT: transfer amount exceeds allowance"
            )
        );
        return true;
    }
        ...
        returns (bool)
    {
        _approve(
            ...
            _allowances[_msgSender()][spender].sub(
                subtractedValue,
                "QBERT: decreased allowance or below zero"
            )
        );
        return true;
    }
    ...
    function setMinTokenToSwap(uint256 _amount) external onlyOwner {
        require(_amount > 0, "QBERT: can't be 0");
    ...
    function setMaxHoldLimit(uint256 _amount) external onlyOwner {
        require(
            ...
            "QBERT: should be greater than 0.5%"
        );
    ...
    function setMaxTxnLimit(uint256 _amount) external onlyOwner {
        require(
            ...
            "QBERT: should be greater than 0.1%"
        );
        maxTxnLimit = _amount;
    }
    ...
    function enableTrading() external onlyOwner {
        require(!trading, "QBERT: already enabled");
        trading = true;
        feesStatus = true;
    ...
    function _approve(
        ...
        require(owner != address(0), "QBERT: approve from the zero address");
        require(spender != address(0), "QBERT: approve to the zero address");
    ...
    function _transfer(
        ...
        require(from != address(0), "QBERT: transfer from the zero address");
        require(to != address(0), "QBERT: transfer to the zero address");
        require(amount > 0, "QBERT: Amount must be greater than zero");
        require(!isBot[from],"Bot detected");

        if (!isExcludedFromMaxTxn[from] && !isExcludedFromMaxTxn[to]) {
            require(amount <= maxTxnLimit, "QBERT: max txn limit exceeds");

            // trading disable till launch
            if (!trading) {
                require(
                    dexPair != from && dexPair != to,
                    "QBERT: trading is disable"
                );
            }
        }
