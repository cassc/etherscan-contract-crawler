...
contract ChallengeDetail is IERC721Receiver{
    ...
    */
     ...
     */
    function sendDailyResult(uint256[] memory _day, uint256[] memory _stepIndex, string memory message, uint8 v, bytes32 r, bytes32 s, uint256[] memory _listIndexReward)
    public
    available
    onTimeSendResult
    onlyChallenger
    ...
    rejectDoubleMessage(message)
    {
        ...
        if(_listIndexReward.length > 0) {
            IGacha(ExerciseSupplementNFT(erc721Address[0]).gachaContractAddress()).randomRewards(
                _listIndexReward,
                address(this)
            );
        }
