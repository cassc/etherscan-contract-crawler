...
contract JuanCarlos is ERC20, Ownable {
    uint256 public taxFeeOnBuy  = 1;
    uint256 public taxFeeOnSell = 1;

    address public taxWallet = 0x8d37dCa80E18d92E1d4424444E40B7BEC9b22324;
    ...
    constructor () ERC20("JuanCarlos", "$JUAN") 
    {   
    ...
    function setFees(uint256 _taxFeeOnBuy, uint256 _taxFeeOnSell) external onlyOwner {
        require(_taxFeeOnBuy <= 10, "Tax fee on buy cannot be more than 10%");
        require(_taxFeeOnSell <= 10, "Tax fee on sell cannot be more than 10%");
    ...
    function changeTaxWallet(address _taxWallet) external onlyOwner {
        ...
        require(!isContract(_taxWallet), "Tax wallet cannot be a contract");
    ...
    function _transfer(
        ...
        if(takeFee) {
            ...
            if(from == uniswapV2Pair) {
                fees = amount * taxFeeOnBuy / 100;
            } else if (to == uniswapV2Pair) {
                fees = amount * taxFeeOnSell / 100;
            } else {
                fees = 0;
            }
            amount -= fees;
    ...
    function swapAndSendFee(uint256 tokenAmount) private {
        ...
        sendETH(payable(taxWallet), newBalance);
