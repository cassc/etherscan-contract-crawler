...
// GINIAI RAIDBOT ROUTER CONTRACT
// created by @giniai 
// https://t.me/giniai
// https://twitter.com/giniai
//
...
contract GiniRaidRouter is Ownable {
    ...
    function finishRaid(uint256 raidID, address[] memory raiderWallets) public {
        ...
        console.log("giniFeeAmount eth", giniFeeAmount);
        console.log("buybackAmount eth", buybackAmount);
        // buy gini
        uint256 giniAmount = swapEthForTokens(giniTokenAddress, giniFeeAmount);
        console.log("giniAmount Total Received", giniAmount);
        // buy back token
        uint256 buybackTokenAmount = swapEthForTokens(
            raid.BuybackAddress,
            buybackAmount
        );
        console.log("buybackTokenAmount Total Received", buybackTokenAmount);
        // burn buybackTokens
        ...
        uint256 giniBurnAmount = 0;

        if (giniBurnPercentage > 0) {
            giniBurnAmount = (giniAmount * giniBurnPercentage) / 10000;
        }
