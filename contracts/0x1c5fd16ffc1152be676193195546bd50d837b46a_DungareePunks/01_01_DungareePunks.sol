...
contract DungareePunks is ERC721A, Ownable {
    using Strings for uint256;
    ...
    uint256 public maxFreeSupply = 1300;
    ...
    constructor() ERC721A("Dungaree Punks", "DGP") {
    ...
    function mint(uint256 _quantity) external payable {
        ...
        require(_quantity + totalSupply() <= maxSupply, "DGP out");
    ...
    function teamMint(uint256 _quantity) external onlyOwner {
        require(totalSupply() + _quantity <= maxSupply, "No more DGP");
        _safeMint(msg.sender, _quantity);
    }
    ...
    function airdrop(address _to, uint256 _quantity) external onlyOwner {
        require(!paused, "The contract is paused!");
        require(_quantity + totalSupply() <= maxSupply, "No more DGP");
