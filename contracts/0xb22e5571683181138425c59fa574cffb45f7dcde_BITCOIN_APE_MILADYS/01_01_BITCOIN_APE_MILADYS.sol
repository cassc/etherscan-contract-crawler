...
contract BITCOIN_APE_MILADYS is ERC721A, Ownable, ReentrancyGuard {
    ...
    uint256 public release = 3000;
    uint256 public MAX_BITCOIN_APE_MILADYS = 9999;
    ...
    constructor() ERC721A("BITCOIN_APE_MILADYS", "BAM") {
    ...
    /**
     * @notice Mint Ape Milady
     */
    function mintApeMilady(uint256 _quantity) external payable {
        // Normal requirements
        ...
        require(
            _quantity + totalSupply() <= MAX_BITCOIN_APE_MILADYS,
            "Sold out"
        );
    ...
    function teamMint(uint256 _quantity) external onlyOwner {
        require(
            totalSupply() + _quantity <= MAX_BITCOIN_APE_MILADYS,
            "Sold out"
        );
        _safeMint(msg.sender, _quantity);
    }
    ...
    function setRelease(uint256 _release) public onlyOwner {
        release = _release;
    }

    function setMAX_BORED_MILADYS(uint256 _MAX_BITCOIN_APE_MILADYS)
        public
        onlyOwner
    {
        MAX_BITCOIN_APE_MILADYS = _MAX_BITCOIN_APE_MILADYS;
    }
        ...
        returns (uint256[] memory)
    {
        ...
        while (
            ...
            currentTokenId <= MAX_BITCOIN_APE_MILADYS
        ) {
