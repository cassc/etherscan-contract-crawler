...
contract UCFC is ERC721A, DefaultOperatorFilterer, Ownable, ERC2981 {
    using Strings for uint256;

    //------------------//
    //     VARIABLES    //
    //------------------//
    uint256 public cost = 2.69 ether;
    ...
    {
        _setDefaultRoyalty(ethRecipient, 250);
    ...
    modifier onlyTeam() {
        if (msg.sender != owner() && msg.sender != ethRecipient) {
            revert AccessDenied();
        }
        _;
    }
    ...
    function mint(address reciever) external payable {
        if (sale == false) revert Paused();
        if (cost != msg.value) revert EthValue();

        _safeMint(reciever, 1);
    }
