...
contract GimbutisToken is OwnableUpgradeable, ERC20BurnableUpgradeable {
    ...
    function transfer(address to, uint256 amount) public override onlyActive returns (bool) {
        ...
        } else if( from == factoryAddress || to == factoryAddress 
        		|| from == routerAddress  || to == routerAddress) { 
     ...
     * @param _usdAddress USDC/USDT contract address
     * @param _sender sender address
     * @param _amount amount
     */
    function _buyTokenWithUsd(address _usdAddress, address _sender, uint256 _amount) private {
        // Note: _amount in USDC (== USDT)
        uint256 _gxagAmount = (_amount * 1e18) / getTokenPrice();

        require(_gxagAmount >= minBuyAmount, "GimbutisToken: Too small amount to buy minimum GXAG");
        require(
            super.balanceOf(address(this)) >= _gxagAmount,
            "GimbutisToken: Not enough GXAG tokens"
        );

        IERC20Upgradeable(_usdAddress).safeTransferFrom(_sender, adminAddress, (_amount / 1e12));
    ...
    function _buyToken(
        ...
        IERC20Upgradeable(_token).safeTransferFrom(_sender, adminAddress, _amount);
    ...
    //	_contract.approve(adminAddress, _amount);
