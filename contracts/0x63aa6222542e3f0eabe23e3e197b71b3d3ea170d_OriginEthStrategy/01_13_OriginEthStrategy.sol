...
contract OriginEthStrategy is Ownable, Operator, Beneficiary, Withdrawable{
    ...
    // Addresses of redeem tokens: stETH, fraxETH, renETH
    ...
    /**
     * @dev turn off or turn on rebasing for this smart-contract 
     * 
     * @param isRebase - if true -- rebasing will turn on, if false -- rebasing will turn off
     */
    function setRebase(bool isRebase) external onlyOperator{
        if (isRebase){
            IOriginETH(OETH).rebaseOptIn();
        }
        else{
            IOriginETH(OETH).rebaseOptOut();
        }
    }
    ...
    */
    ...
    function depositFromBank(uint256 baseAmount, bool isSafeTransfer) public onlyOperator{
        ...
        require(allowance >= baseAmount, "depositFromBank: allowance less than baseAmount");
        require(IERC20(baseToken).balanceOf(bank) >= baseAmount, "depositFromBank: balance less than baseAmount");
    ...
    /**
     * @dev withdrawing all base tokens back to bank
     *
     */
    function withdrawToBankAll() public onlyOperator(){
        withdrawToBank(getBaseTokenBalance());
    }
    ...
    function withdrawToBank(uint256 baseAmount) public onlyOperator{
        require(getBaseTokenBalance() >= baseAmount, "withdrawToBank: balance less than baseAmount");
     ...
     *
     */
    function runStrategyAll() public onlyOperator{
        runStrategy(getBaseTokenBalance());
    }
