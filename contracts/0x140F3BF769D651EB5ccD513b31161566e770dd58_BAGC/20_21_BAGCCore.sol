...
contract BAGCCore is OwnableUpgradeable {
    ...
    string public invitationBaseURI;
    ...
    modifier onlyClaimContract() {
        if (msg.sender != claimAddress) {
            revert BAGC_Not_Claim_Contract();
        }
        _;
    }
    ...
    function updateClaimAddress(address claimAddress_) public onlyOwner {
        claimAddress = claimAddress_;
    }

    function updateInvitationBaseURI(string memory invitationBaseURI_) public onlyOwner {
        invitationBaseURI = invitationBaseURI_;
    }
    ...
    function verifyLockSignature(
        ...
        bytes32 recoveredHash = keccak256(abi.encodePacked(tokenId, value));
