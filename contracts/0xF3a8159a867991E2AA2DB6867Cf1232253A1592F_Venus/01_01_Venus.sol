...
contract Venus is ERC721, Ownable, PaymentSplitter {
  using Strings for uint256;
  ...
  // Addresses to pay to
  uint256 public immutable totalPayees = 2;
  address[] public _payees = [
    0xe36699542391Bd285C28402aD917629aA42dD645,
    0xd78E8ec02505f7261A47Ed1E9B1E91374da5b6Dd
  ];
  uint256[] public _shares = [97, 3];
  ...
  function mint(uint256 _mintAmount) public payable {
    ...
    require(_mintAmount > 0, "Num NFTs must be >= 1");
    require(_mintAmount < 3, "Num NFTs must be <= 2");
    require(balanceOf(msg.sender) + _mintAmount < 3, "Num mints MAX 2");
    uint256 supply = _tokenSupply.current();
    require(msg.value >= cost * _mintAmount, "Not enough ETH");
    require(supply + _mintAmount < maxSupply + 1, "max supply reached");

    for (uint256 i = 0; i < _mintAmount; i++) {
      uint256 new_token = _tokenSupply.current() + 1;
      _safeMint(msg.sender, new_token);
      _tokenSupply.increment();
    }
  }

  // public mint for airdrops, giveaways.. etc
  function freeMint(uint256 _mintAmount) external onlyOwner {
    uint256 supply = _tokenSupply.current();
    require(supply + _mintAmount < maxSupply + 1, "max supply reached");
    for (uint256 i = 0; i < _mintAmount; i++) {
      uint256 new_token = _tokenSupply.current() + 1;
  ...
  /*
   * Until reveal it returns the URI with the spinning NFT
   * When we reveal it returns the URI with the actual NFT
   * @param _tokenId
   */
