...
contract PudgyKarma is ERC721A, Ownable, ReentrancyGuard {
    bool public Minting  = false;
    uint256[] public freeMintArray = [3,2,1];
    uint256[] public supplyMintArray = [6000,7888,8488];
    uint256 public price = 3000000000000000;
    string public baseURI;  
    uint256 public maxPerTx = 20;  
    uint256 public maxSupply = 8888;
    uint256 public teamSupply = 88;  
    mapping (address => uint256) public minted;
    constructor() ERC721A("Pudgy Karma", "Pudgy Karma"){}
    ...
    {
        require(Minting , "Pudgy Karma Minting Close !");
        require(qty <= maxPerTx, "Pudgy Karma Max Per Tx !");
        require(totalSupply() + qty <= maxSupply-teamSupply,"Pudgy Karma Soldout !");
        _safemint(qty);
    }
    ...
    function _safemint(uint256 qty) internal  {
        ...
        if(minted[msg.sender] < freeMint) 
        {
           ...
           require(msg.value >= (qty - freeMint) * price,"Pudgy Karma Insufficient Funds !");
            minted[msg.sender] += qty;
           _safeMint(msg.sender, qty);
        }
        else
        {
           require(msg.value >= qty * price,"Pudgy Karma Insufficient Funds !");
            minted[msg.sender] += qty;
           _safeMint(msg.sender, qty);
        }
    }
