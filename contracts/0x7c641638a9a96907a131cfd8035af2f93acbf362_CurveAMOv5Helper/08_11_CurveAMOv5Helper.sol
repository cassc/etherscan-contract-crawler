...
contract CurveAMOv5Helper {
        ...
        returns (uint256[10] memory allocations) 
    {
        ...
        for (uint256 i = 0; i < _poolArrayLength; i++) {
            ...
            (, , bool _hasFrax, , bool _hasUsdc) = curveAMO.showPoolInfo(_poolAddress);
            (, uint256 _fraxIndex, uint256 _usdcIndex, ) = curveAMO.showPoolCoinIndexes(_poolAddress);
            try curveAMO.showPoolAccounting(_poolAddress) returns (uint256[] memory ,uint256[] memory _depositedAmounts,uint256[] memory, uint256[3] memory){
                if (_hasFrax){
            ...
            try curveAMO.calcFraxUsdcOnlyFromFullLPExit(_poolAddress) returns (uint256[4] memory _withdrawables) {
    ...
    function calcFraxAndUsdcWithdrawable(address _curveAMOAddress, address _poolAddress, address _poolLpTokenAddress, uint256 _lpAmount) 
        public view 
        returns (uint256[4] memory _withdrawables) 
    {   
        ICurveAMOv5 curveAMO = ICurveAMOv5(_curveAMOAddress);
        (bool _isMetapool, bool _isCrypto, bool _hasFrax, , bool _hasUsdc) = curveAMO.showPoolInfo(_poolAddress);
        (, uint256 _fraxIndex, uint256 _usdcIndex, uint256 _baseTokenIndex) = curveAMO.showPoolCoinIndexes(_poolAddress);
        ...
        if(_hasFrax){
            ...
            if (_hasUsdc){
                _withdrawables[0] = pool.balances(_fraxIndex) * _lpAmount / _lpTotalSupply;
                _withdrawables[1] = pool.balances(_usdcIndex) * _lpAmount / _lpTotalSupply;
                _withdrawables[2] = _withdrawables[0];
                _withdrawables[3] = _withdrawables[1];
            } else if(_isMetapool) {
                _withdrawables[0] = pool.balances(_fraxIndex) * _lpAmount / _lpTotalSupply;
                _withdrawables[1] = 0;
                uint256 _totalWithdrawable = pool.balances(_baseTokenIndex) * _lpAmount / _lpTotalSupply;
                address _baseTokenAddress = pool.coins(_baseTokenIndex);

                // Recursive call
                uint256[4] memory _poolwithdrawables = curveAMO.calcFraxAndUsdcWithdrawable(curveAMO.lpTokenToPool(_baseTokenAddress) , _totalWithdrawable);
                ...
                if (_lpAmount > 0) {
                    if(_isCrypto) {
                        _withdrawables[0] = pool.calc_withdraw_one_coin(_lpAmount, _fraxIndex);
                    } else {
                        int128 _index = int128(uint128(_fraxIndex));
                        _withdrawables[0] = pool.calc_withdraw_one_coin(_lpAmount, _index);
                    }
                }
                _withdrawables[2] = _withdrawables[0];
            }
            ...
            if (_hasUsdc){
                ...
                if (_lpAmount > 0) {
                    if(_isCrypto) {
                        _withdrawables[1] = pool.calc_withdraw_one_coin(_lpAmount, _usdcIndex);
                    } else {
                        int128 _index = int128(uint128(_usdcIndex));
                        _withdrawables[1] = pool.calc_withdraw_one_coin(_lpAmount, _index);
                    }
                }
                _withdrawables[2] = _withdrawables[0];
                _withdrawables[3] = _withdrawables[1];
                ...
                if (_lpAmount > 0) {
                    if(_isCrypto) {
                    _totalWithdrawable = pool.calc_withdraw_one_coin(_lpAmount, _baseTokenIndex);
                    } else {
                        int128 _index = int128(uint128(_baseTokenIndex));
                        _totalWithdrawable = pool.calc_withdraw_one_coin(_lpAmount, _index);
                    }
                }
                address _baseTokenAddress = pool.coins(_baseTokenIndex);
                uint256[4] memory _poolwithdrawables = curveAMO.calcFraxAndUsdcWithdrawable(curveAMO.lpTokenToPool(_baseTokenAddress) , _totalWithdrawable);
    ...
    function showPoolAssetBalances(address _curveAMOAddress, address _poolAddress) 
        public view 
        returns (
    ...
    function showOneStepBurningLp(address _curveAMOAddress, address _poolAddress) 
        public view 
        returns (
            uint256 _oneStepBurningLp
        ) 
    {
        ICurveAMOv5 curveAMO = ICurveAMOv5(_curveAMOAddress);
        (,,, uint256[3] memory _allocations) = curveAMO.showPoolAccounting(_poolAddress);
    ...
    function getTknsForLPAtCurrRatio(address _curveAMOAddress, address _poolAddress, address _poolLpTokenAddress, uint256 _lpAmount) 
        public view 
