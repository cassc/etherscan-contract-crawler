...
abstract contract LSSVMPairMissingEnumerable is LSSVMPair {
    ...
    /**
        ...
        This function acts as the primary check to ensure that only valid stars can enter a pool.
    ...
    function onERC721Received(
        ...
        if (msg.sender == address(_ecliptic)) {
            idSet.add(id);
        }
        ...
        onlyOwner
    {
        ...
        // Otherwise, withdraw and also remove the ID from the ID set
            ...
            for (uint256 i; i < numNFTs; ) {
                _ecliptic.safeTransferFrom(address(this), msg.sender, nftIds[i]);
                idSet.remove(nftIds[i]);
