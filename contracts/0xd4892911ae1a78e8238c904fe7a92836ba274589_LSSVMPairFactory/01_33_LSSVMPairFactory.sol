...
// Adapted for Urbitex by ~dosdel-falrud based on original work by boredGenius and 0xmons
//
// The alterations for Urbitex include replacing the generic NFT contract input with Urbit's Ecliptic contract.
// Eclptic must be derived by calling Azimuth's owner, otherwise this would risk trapping
...
contract LSSVMPairFactory is Ownable, ILSSVMPairFactoryLike {
    ...
    function createPairETH(
        ...
        // get ecliptic by calling azimuth's owner
        IERC721 _ecliptic = IERC721(azimuth.owner());   

        // The current version of Ecliptic doesn't support ERC721Enumerable, but future versions might.
        address template;
        try
            IERC165(address(_ecliptic)).supportsInterface(
    ...
    function depositNFTs(
        ...
        for (uint256 i; i < numNFTs; ) {
            _ecliptic.safeTransferFrom(msg.sender, recipient, ids[i]);
