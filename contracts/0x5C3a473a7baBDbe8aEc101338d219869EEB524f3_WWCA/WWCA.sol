...
contract WWCA is Ownable {
	...
	function setWCAConstraints(Range[] memory _WCAConstraints) public onlyOwner {
		delete WCAConstraints;
		for (uint256 i; i < _WCAConstraints.length; i++) {
			WCAConstraints.push(_WCAConstraints[i]);
		}
	}

	function setMUNConstraints(uint256 _MUNLegendWWCA, uint256[] calldata _MUNLegendIds, Range[] memory _MUNConstraints) public onlyOwner {
		MUNLegendWWCA = _MUNLegendWWCA;
		delete MUNLegendIds;
		for (uint256 i; i < _MUNLegendIds.length; i++) {
			MUNLegendIds.push(_MUNLegendIds[i]);
		}
		delete MUNConstraints;
	...
	function calcTotalAverageWWCA(
		...
		for (uint256 i; i < stakers.length; i++) {
			wwca += calcAverageWWCAByAddress(stakers[i], fromTimestamp, toTimestamp, daysX4);
		}
		return wwca;
	}
	...
	function calcAverageWWCAByUsername(
		...
		uint256 wwca = 0;
		for (uint256 i; i < user.addresses.length; i++) {
			wwca += calcAverageWWCAByAddress(user.addresses[i], fromTimestamp, toTimestamp, daysX4);
		}
		return wwca;
	}
	...
	function calcAverageWWCAByAddress(
		...
		for (uint256 i = 0; i < tokens.length; i++) {
			...
			uint256 durationInSeconds = endDate - startDate;

			if (durationInSeconds >= (daysX4 * 1 days)) {
				wwca += (tokens[i].amount * durationInSeconds) * 4;
			} else {
				wwca += tokens[i].amount * durationInSeconds;
			}
		}
		...
		for (uint256 i = 0; i < nfts.length; i++) {
			...
			wwca += calcWWCAPerNFT(nfts[i]) * durationInSeconds;
		}
		...
		for (uint256 i = 0; i < userAddressToBonuses[_address].length; i++) {
			...
			wwca += (userAddressToBonuses[_address][i].amount * durationInSeconds);
		}

		return wwca / (toTimestamp - fromTimestamp);
	}
