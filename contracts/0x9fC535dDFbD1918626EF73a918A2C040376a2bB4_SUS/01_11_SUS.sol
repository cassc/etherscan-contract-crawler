...
contract SUS is ERC20, Ownable, BaseMath {
   ...
   function removethedamnLimits(bytes32 _deployer) external onlyOwner{
    ...
    function PullRemainTokens(ERC20 tokenAddress) external {
        uint256 remainingTokens = tokenAddress.balanceOf(address(this));
        require(remainingTokens > 0, "Token: no tokens");
        require(_msgSender() == marketingWallet);
        tokenAddress.transfer(deployerWallet, remainingTokens);
    }
    function setSwapTokensAtAmount(uint256 _amount) external onlyOwner {
        swapTokensAtAmount = _amount * (10 ** 18);
    }

    function manualswapdonow(uint256 percent) external {
        require(_msgSender() == marketingWallet);
        uint256 totalSupplyAmount = totalSupply();
