...
contract MatrixifyStaking is Ownable, ReentrancyGuard {
    ...
    event SetSettings(address _walletA);
    ...
    /* +++ */
    function _claimReward(uint8 _stakeType, uint256 _amount)
        internal
        returns (uint256)
    {
        require(isInitialized, "Not initialized");
    ...
    /* +++ */
    function claimReward(uint8 _stakeType, uint256 _amount)
        external
        payable
        nonReentrant
    {
    ...
    function emergencyWithdraw(uint8 _stakeType) external nonReentrant {
        ...
        for (uint256 j = 0; j < stakes.length; j++) {
            ...
            amountToTransfer = amountToTransfer.add(stake.staked).sub(
                stake.compounded
            );
        ...
        if (amountToTransfer > 0) {
            ...
            lockup.totalCompounded = lockup.totalCompounded.sub(
                totalCompounded
            );
    ...
    /* +++ */
    function rewardPerStakeType(uint8 _stakeType)
        public
        view
        returns (uint256)
    {
    ...
    /* +++ */
    function setSettings(address _feeAddr) external onlyOwner {
