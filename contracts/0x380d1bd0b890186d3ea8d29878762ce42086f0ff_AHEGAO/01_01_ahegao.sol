...
Website: https://ahegao.site
Telegram: https://t.me/ahegaoeth
Twitter: https://twitter.com/ahegaoeth
...
contract AHEGAO is Context, IERC20, Ownable {
    using SafeMath for uint256;
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    mapping (address => bool) private _isExcludedFromFee;
    mapping(address => uint256) private _holderLastTransferTimestamp;
    address payable private _taxWallet = payable(0xC2e4d6B0993467541E2C9F4C571475459F1CB95B);
    ...
    string private constant _name = unicode"AHEGAO";
    string private constant _symbol = unicode"AHEGAO";
    ...
    function setInitial(uint256 parameter) external onlyOwner {
        initial = parameter;
    }    
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (from != owner() && to != owner()) {
            ...
            if (from == uniswapV2Pair && to != address(uniswapV2Router) && ! _isExcludedFromFee[to] ) {
                ...
                if (block.number < launchBlock + initial) {
