    ...
    IThePlague
{
    ...
    function _accruedRewards(
        ...
        // Only apply bonus to periods after the min stake period
        if (
        ...
        return baseReward * periodsTotal;
    }
    ...
    function calculateReward(uint256 tokenId) public view returns (uint256) {
        ...
        unchecked {
            ...
            uint256 accrued = _accruedRewards(
                config,
                uint256(sinceDeposit) / config.periodDenominator,
                sinceDeposit
            );

            // Subtract out all previous claims
            if (claimedAt > stakedAt) {
                uint256 alreadyClaimed = _accruedRewards(
                    config,
                    uint256(claimedAt - stakedAt) / config.periodDenominator,
                    sinceDeposit
                );
                accrued -= alreadyClaimed;
            }
