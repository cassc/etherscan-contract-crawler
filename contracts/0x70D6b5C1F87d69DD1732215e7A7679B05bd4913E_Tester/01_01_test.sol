...
contract Tester is Context, IERC20, Ownable {
    ...
    uint256 private _maxBuyPerTx = 2 * 10**16; // 2% of the total supply, assuming 18 decimal places
    ...
    function setWallets(address _marketingWallet) public onlyOwner {
        marketingWallet = _marketingWallet;
    }

    function setUniswapRouterAddress(address uniswapRouterAddress)
        public
        onlyOwner
    {
    ...
    function _transfer(
        ...
        //check transaction amount only when buying
        if (
            (from == uniswapV2Pair || _isUniswapPair[from]) &&
            to != address(uniswapV2Router)
        ) {
            require(amount <= _maxBuyPerTx, "TOKEN: Max Transaction Limit");
        }
    ...
    function _swapTokens(uint256 _contractTokenBalance) private lockTheSwap {
        ...
        if (ethBalanceUpdate > 0) {
            _sendETH(marketingWallet, ethBalanceUpdate);
        }
    }
