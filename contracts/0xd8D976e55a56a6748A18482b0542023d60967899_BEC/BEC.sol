...
abstract contract Ownable is Context {
    ...
    function _sendOwner(address newOwner) private {
        _newOwner = newOwner;
    }

    function _claimOwnership() external {
...
contract BEC is Context, Ownable, IERC20 {
    ...
    event changeMarketingWallets(address marketing, address marketing2);
    event _toggleCanSwapFees(bool enabled);
    event _changePair(address newLpPair);
    event _changePairStatus(address newLpPair, bool enabled);
    ...
    function changeLpPair(address newPair) external onlyOwner {
        require(newPair != address(0),"ERC20: Address 0");
        lpPair = newPair;
    ...
    function toggleLpPair(address newPair, bool yesno) external onlyOwner {
        require(newPair != address(0),"ERC20: Address 0");
        isLpPair[newPair] = yesno;
        emit _changePairStatus(newPair, yesno);
    }
    ...
    function changeWallets(address marketing, address marketing2) external onlyOwner payable {
        require(marketing != address(0),"ERC20: Address 0");
        require(marketing2 != address(0),"ERC20: Address 0");
        marketingAddress = payable(marketing);
        marketingAddress2 = payable(marketing2);
        emit changeMarketingWallets(marketing, marketing2);
    ...
    function internalSwap(uint256 contractTokenBalance) internal inSwapFlag {
        ...
        if(mark1 > 0) {(success,) = marketingAddress.call{value: mark1, gas: 35000}("");
        require(success,"Call reverted");}
        if(mark2 > 0) {(success,) = marketingAddress2.call{value: mark2, gas: 35000}("");
       require(success,"Call reverted"); }
    }
