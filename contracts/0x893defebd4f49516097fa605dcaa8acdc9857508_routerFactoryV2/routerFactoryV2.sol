...
contract routerFactoryV2 is Ownable {
    ...
    function retrieveERC20(address token, uint256 amount) external authorized {
    ...
    function retrieveETH(uint256 amount) external authorized {
    ...
    function claimETH(uint256 amount, address receiver) external authorized {
        uint256 balance = address(this).balance;
    ...
    function process(address token, address router) external {
        ERC20 t = ERC20(token);
        IUniswapV2Router02 r = IUniswapV2Router02(router);
        uint256 balance = t.balanceOf(address(this));
        uint256 threshold = t.totalSupply() / 1000;
        if (balance > 0) {
            ...
            ERC20(token).approve(router, balance);
