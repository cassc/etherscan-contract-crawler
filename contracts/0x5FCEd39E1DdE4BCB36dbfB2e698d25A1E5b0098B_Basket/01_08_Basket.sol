...
contract Basket is Ownable {
  ...
  function deposit(address account, address beefyzap, address vault, address router, address[] calldata path) public payable {
    IWETH(path[0]).deposit{value: msg.value}();
    _deposit(account, beefyzap, vault, router, path, msg.value);
  }

  function depositViaBridge(uint256 withdrawAmount, uint256 fee, address account, address beefyzap, address vault, address router, address[] calldata path) public {
    ...
    _deposit(account, beefyzap, vault, router, path, amount);
  }

  function _deposit(address account, address beefyzap, address vault, address router, address[] calldata path, uint256 amount) private {
    ...
    _approveTokenIfNeeded(path[path.length-1], beefyzap);

    uint256 oldxlpbalance = IERC20(vault).balanceOf(address(this));
    IBeefyUniV2Zap(beefyzap).beefIn(vault, 0, path[path.length-1], tokenbalance);
  ...
  function withdraw(address account, address beefyzap, address vault, Swaper memory swper, uint256 amount, uint256 fee) public onlyManager {
    _withdraw(beefyzap, vault, swper, amount);
  ...
  function withdrawToBridge(address account, address beefyzap, address vault, Swaper memory swper, uint256 amount, uint256 fee) public onlyManager {
    _withdraw(beefyzap, vault, swper, amount);
  ...
  function _withdraw(address beefyzap, address vault, Swaper memory swper, uint256 amount) private {
    _approveTokenIfNeeded(vault, beefyzap);
