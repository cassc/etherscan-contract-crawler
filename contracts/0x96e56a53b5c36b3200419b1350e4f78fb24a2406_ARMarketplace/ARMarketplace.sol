...
contract ARClaimToken is ERC721, Ownable {
  ...
  event ClaimCreated(uint256 tokenId, address claimCreator, string tokenURI, string barcodeId, string material);
  ...
  function mintClaim(
    ...
    require(totalPercent <= 10_000, "Sum of beneficiary percentages exceeds 100%");

    _safeMint(msg.sender, tokenId);
    _setTokenURI(tokenId, _tokenUri);

    emit ClaimCreated(tokenId, msg.sender, _tokenUri, _barcodeId, _material);
  }
...
// File: contracts/ARMarketplace.sol
...
contract ARMarketplace is Ownable {
  ...
  event ListingCreated(uint256 tokenId, address claimOwner);
  event ListingCancelled(uint256 tokenId, address claimOwner);
  event PriceChanged(uint256 tokenId, uint256 newValue, address sender);
  ...
  function createListing(uint256 tokenId, uint256 price) public {
    ...
    emit ListingCreated(tokenId, msg.sender);
  }
  ...
  function changeListingPrice(uint256 tokenId, uint256 newValue) public {
    ...
    emit PriceChanged(tokenId, newValue, msg.sender);
  }
  ...
  function buyItem(uint256 _tokenId) public {
    ...
    IERC20(paymentTokenAddress).transferFrom(msg.sender, listing.seller, listing.price);
  ...
  function transferToBeneficiaries(uint256 _tokenId) public {
    ...
    require(totalPercent <= 10000, "Total percentage is not 100%");

    for (uint256 i = 0; i < claim.beneficiaries.length; i++) {
      uint256 amount = (claim.value * claim.percentages[i]) / 10000;
      IERC20(paymentTokenAddress).transfer(claim.beneficiaries[i], amount);
    }

    if (totalPercent < 10000) {
      uint256 amount = (claim.value * (10000 - totalPercent)) / 10000;
