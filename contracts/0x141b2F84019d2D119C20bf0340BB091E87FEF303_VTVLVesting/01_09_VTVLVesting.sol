...
contract VTVLVesting is Context, AccessProtected {
        ...
        returns (uint112)
    {
        ...
        if (_claim.isActive) {
            ...
            if (_referenceTs > _claim.startTimestamp) {
                ...
                uint112 linearVestAmount = uint112(
                    (uint256(_claim.linearVestAmount) *
        ...
        return
            (vestAmt > _claim.amountWithdrawn)
                ? vestAmt
    ...
    */
     ...
     */
    function withdraw() external hasActiveClaim(_msgSender()) {
    ...
    function withdrawAdmin(uint112 _amountRequested) public onlyAdmin {
        ...
        uint256 amountRemaining = tokenAddress.balanceOf(address(this)) -
