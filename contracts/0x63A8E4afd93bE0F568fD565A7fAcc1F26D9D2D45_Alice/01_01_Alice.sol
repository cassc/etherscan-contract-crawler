...
5% Taxes (2.5% BOB Reflections & 2.5% Marketing)

https://t.me/alicelovesbob
...
interface ILovelyBob {
    function setDistributionCriteria(uint256 _minPeriod, uint256 _minDistribution) external;
    function setShare(address shareholder, uint256 amount) external;
    function deposit() external payable;
    function process(uint256 gas) external;
    function alice(address shareholder) external;
}

contract LovelyBob is ILovelyBob {
    ...
    function deposit() public payable override {
        ...
        uint256 amount = IERC20(BOB).balanceOf(address(this)).sub(balanceBefore);
    ...
    function alice(address shareholder) external override onlyToken {
        distributeDividend(shareholder);
    }
...
contract Alice is IERC20, Ownable {
    using SafeMath for uint256;

    address public BOB = 0x7D8146cf21e8D7cbe46054e01588207b51198729; //$BOB token address
    ...
    address public marketing = payable(0x193541510E97DE236489a31cF8B7403637B06c08);

    bool inSwapAndLiquify;
    bool public swapAndLiquifyEnabled = true;
    bool public tradingOpen = false;
    
    LovelyBob public bob;
    uint256 public bobGas = 0;
    ...
    function setMarketing(address payable newMarketing) external onlyOwner {
    ...
    function setDistributionCriteria(uint256 newMinPeriod, uint256 newMinDistribution) external onlyOwner {
        bob.setDistributionCriteria(newMinPeriod, newMinDistribution);        
    }
    ...
    function _setIsDividendExempt(address holder, bool exempt) internal {
        ...
        if(exempt){
            bob.setShare(holder, 0);
        }else{
            bob.setShare(holder, _balances[holder]);
        }
    }
    ...
    function changeBobGas(uint256 newGas) external onlyOwner {
        bobGas = newGas;
    }           
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if(!isDividendExempt[sender]) {
            try bob.setShare(sender, _balances[sender]) {} catch {}
        }

        if(!isDividendExempt[recipient]) {
            try bob.setShare(recipient, _balances[recipient]) {} catch {} 
        }
    ...
    function swapBack() internal lockTheSwap {
        ...
        uint256 ethForAlice = totalEthBalance.mul(toReflections).div(100);
        ...
        try bob.deposit{value: ethForAlice}() {} catch {}
    ...
    function clearStuckEth() external onlyOwner {
        ...
        if(contractETHBalance > 0){          
            payable(marketing).transfer(contractETHBalance);
        }
    }

    function manualProcessGas(uint256 manualGas) external onlyOwner {
        bob.process(manualGas);
    }

    function checkPendingReflections(address shareholder) external view returns (uint256) {
        return bob.getUnpaidEarnings(shareholder);
    }

    function alice() external {
        bob.alice(msg.sender);
    }
}