...
import "./IIceCream.sol";

contract IceCream is IIceCream, ERC721AQueryable, PaymentSplitter, Ownable, ReentrancyGuard, DefaultOperatorFilterer {
  ...
  ) ERC721A("ICE CREAM!", "ICE CREAM$") PaymentSplitter(_payees, _shares) {}
  ...
  function whitelistMint(uint256 _amount, uint256 _freeAmount, bytes32[] memory _proof) external payable {
    _verify(_freeAmount, _proof);
    ...
    if (_numberMinted(msg.sender) + _amount > mintRules.whitelistExtraPerWallet + _freeAmount) {
  ...
  function _verify(uint256 _freeAmount, bytes32[] memory _proof) private view {
