...
// File: DPC/interfaces/IDividendsV2.sol
...
// File: DPC/interfaces/IESDPCTokenUsage.sol


pragma solidity ^0.8.19;

interface IESDPCTokenUsage {
...
// File: DPC/interfaces/tokens/IESDPCToken.sol


pragma solidity ^0.8.19;


interface IESDPCToken is IERC20 {
...
// File: DPC/interfaces/tokens/IDPCTokenV2.sol


pragma solidity ^0.8.19;


interface IDPCTokenV2 is IERC20{
...
/*
 * esDPC is developerCoin's escrowed governance token obtainable by converting DPC to it
 * It's non-transferable, except from/to whitelisted addresses
 * It can be converted back to DPC through a vesting process
 * This contract is made to receive esDPC deposits from users in order to allocate them to Usages (plugins) contracts
 */
contract ESDPCToken is Ownable, ReentrancyGuard, ERC20("DPC escrowed token", "esDPC"), IESDPCToken {
    using Address for address;
    using SafeMath for uint256;
    using EnumerableSet for EnumerableSet.AddressSet;
    using SafeERC20 for IDPCTokenV2;

    struct ESDPCBalance {
        uint256 allocatedAmount; // Amount of esDPC allocated to a Usage
        uint256 redeemingAmount; // Total amount of esDPC currently being redeemed
    }

    struct RedeemInfo {
        uint256 dpcAmount; // DPC amount to receive when vesting has ended
        uint256 esDPCAmount; // esDPC amount to redeem
        uint256 endTime;
        IESDPCTokenUsage dividendsAddress;
        uint256 dividendsAllocation; // Share of redeeming esDPC to allocate to the Dividends Usage contract
    }

    IDPCTokenV2 public immutable dpcToken; // DPC token to convert to/from
    IESDPCTokenUsage public dividendsAddress; // DPC dividends contract

    EnumerableSet.AddressSet private _transferWhitelist; // addresses allowed to send/receive esDPC

    mapping(address => mapping(address => uint256)) public usageApprovals; // Usage approvals to allocate esDPC
    mapping(address => mapping(address => uint256)) public override usageAllocations; // Active esDPC allocations to usages

    uint256 public constant MAX_DEALLOCATION_FEE = 200; // 2%
    mapping(address => uint256) public usagesDeallocationFee; // Fee paid when deallocating esDPC
    ...
    // Adjusted dividends rewards for redeeming esDPC
    uint256 public redeemDividendsAdjustment = 50; // 50%

    mapping(address => ESDPCBalance) public esDPCBalances; // User's esDPC balances
    mapping(address => RedeemInfo[]) public userRedeems; // User's redeeming instances

    bool public isRedeemBurn = false;

    constructor(IDPCTokenV2 dpcToken_) {
        dpcToken = dpcToken_;
    ...
    event Redeem(address indexed userAddress, uint256 esDPCAmount, uint256 dpcAmount, uint256 duration);
    event FinalizeRedeem(address indexed userAddress, uint256 esDPCAmount, uint256 dpcAmount);
    event CancelRedeem(address indexed userAddress, uint256 esDPCAmount);
    ...
    event setRedeemBurn(bool isBurned,bool isBurn);
   ...
   */
    ...
    /*
     * @dev Returns user's esDPC balances
   */
    function getESDPCBalance(address userAddress) external view returns (uint256 allocatedAmount, uint256 redeemingAmount) {
        ESDPCBalance storage balance = esDPCBalances[userAddress];
        return (balance.allocatedAmount, balance.redeemingAmount);
    }

    /*
     * @dev returns redeemable DPC for "amount" of esDPC vested for "duration" seconds
   */
    function getDPCByVestingDuration(uint256 amount, uint256 duration) public view returns (uint256) {
   ...
   */
    function getUserRedeem(address userAddress, uint256 redeemIndex) external view validateRedeem(userAddress, redeemIndex) returns (uint256 dpcAmount, uint256 esDPCAmount, uint256 endTime, address dividendsContract, uint256 dividendsAllocation) {
        RedeemInfo storage _redeem = userRedeems[userAddress][redeemIndex];
        return (_redeem.dpcAmount, _redeem.esDPCAmount, _redeem.endTime, address(_redeem.dividendsAddress), _redeem.dividendsAllocation);
    }

    /**
     * @dev returns approved esDPC to allocate from "userAddress" to "usageAddress"
   */
    ...
    /**
     * @dev returns allocated esDPC from "userAddress" to "usageAddress"
   */
   ...
   */
    ...
    /**
     * @dev returns if "account" is allowed to send/receive esDPC
   */
   ...
   * Must only be called by owner
   */
    function updateDividendsAddress(IESDPCTokenUsage dividendsAddress_) external onlyOwner {
   ...
   */
    function updateTransferWhitelist(address account, bool add) external onlyOwner {
        require(account != address(this), "updateTransferWhitelist: Cannot remove esDPC from whitelist");
    ...
    /**
     * @dev Approves "usage" address to get allocations up to "amount" of esDPC from msg.sender
   */
    function approveUsage(IESDPCTokenUsage usage, uint256 amount) external nonReentrant {
    ...
    /**
     * @dev Convert caller's "amount" of DPC to esDPC
   */
    ...
    /**
     * @dev Convert caller's "amount" of DPC to esDPC to "to" address
   */
    ...
    /**
     * @dev Initiates redeem process (esDPC to DPC)
   *
   * Handles dividends' compensation allocation during the vesting process if needed
   */
    function redeem(uint256 esDPCAmount, uint256 duration) external nonReentrant {
        require(esDPCAmount > 0, "redeem: esDPCAmount cannot be null");
        require(duration >= minRedeemDuration, "redeem: duration too low");

        _transfer(msg.sender, address(this), esDPCAmount);
        ESDPCBalance storage balance = esDPCBalances[msg.sender];

        // get corresponding DPC amount
        uint256 dpcAmount = getDPCByVestingDuration(esDPCAmount, duration);
        emit Redeem(msg.sender, esDPCAmount, dpcAmount, duration);

        // if redeeming is not immediate, go through vesting process
        if(duration > 0) {
            // add to SBT total
            balance.redeemingAmount = balance.redeemingAmount.add(esDPCAmount);

            // handle dividends during the vesting process
            uint256 dividendsAllocation = esDPCAmount.mul(redeemDividendsAdjustment).div(100);
            ...
            // add redeeming entry
            userRedeems[msg.sender].push(RedeemInfo(dpcAmount, esDPCAmount, _currentBlockTimestamp().add(duration), dividendsAddress, dividendsAllocation));
        } else {
            // immediately redeem for DPC
            _finalizeRedeem(msg.sender, esDPCAmount, dpcAmount);
        }
    }
   ...
   */
    function finalizeRedeem(uint256 redeemIndex) external nonReentrant validateRedeem(msg.sender, redeemIndex) {
        ...
        balance.redeemingAmount = balance.redeemingAmount.sub(_redeem.esDPCAmount);
        _finalizeRedeem(msg.sender, _redeem.esDPCAmount, _redeem.dpcAmount);

        // handle dividends compensation if any was active
        if(_redeem.dividendsAllocation > 0) {
            // deallocate from dividends
            IESDPCTokenUsage(_redeem.dividendsAddress).deallocate(msg.sender, _redeem.dividendsAllocation, new bytes(0));
        }
   ...
   */
    function cancelRedeem(uint256 redeemIndex) external nonReentrant validateRedeem(msg.sender, redeemIndex) {
        ...
        // make redeeming esDPC available again
        balance.redeemingAmount = balance.redeemingAmount.sub(_redeem.esDPCAmount);
        _transfer(address(this), msg.sender, _redeem.esDPCAmount);
        ...
        emit CancelRedeem(msg.sender, _redeem.esDPCAmount);
    ...
    /**
     * @dev Allocates caller's "amount" of available esDPC to "usageAddress" contract
   *
   ...
   */
    function allocate(address usageAddress, uint256 amount, bytes calldata usageData) external nonReentrant {
        ...
        // allocates esDPC to usageContract
        IESDPCTokenUsage(usageAddress).allocate(msg.sender, amount, usageData);
    }

    /**
     * @dev Allocates "amount" of available esDPC from "userAddress" to caller (ie usage contract)
   *
   * Caller must have an allocation approval for the required esDPC esDPC from "userAddress"
   */
    ...
    /**
     * @dev Deallocates caller's "amount" of available esDPC from "usageAddress" contract
   *
   * args specific to usage contract must be passed into "usageData"
   */
    function deallocate(address usageAddress, uint256 amount, bytes calldata usageData) external nonReentrant {
        ...
        // deallocate esDPC into usageContract
        IESDPCTokenUsage(usageAddress).deallocate(msg.sender, amount, usageData);
    }

    /**
     * @dev Deallocates "amount" of allocated esDPC belonging to "userAddress" from caller (ie usage contract)
   *
   * Caller can only deallocate esDPC from itself
   */
    ...
    /**
     * @dev Convert caller's "amount" of DPC into esDPC to "to"
   */
    function _convert(uint256 amount, address to) internal {
        ...
        dpcToken.safeTransferFrom(msg.sender, address(this), amount);
    }

    /**
     * @dev Finalizes the redeeming process for "userAddress" by transferring him "dpcAmount" and removing "esDPCAmount" from supply
   *
   * Any vesting check should be ran before calling this
   * DPC excess is automatically send to dividends contract
   */
    function _finalizeRedeem(address userAddress, uint256 esDPCAmount, uint256 dpcAmount) internal {
        uint256 dpcExcess = esDPCAmount.sub(dpcAmount);

        // sends due DPC tokens
        dpcToken.safeTransfer(userAddress, dpcAmount);

        // send DPC excess to dividends if any
        if(!isRedeemBurn && (dpcExcess > 0)){
            IERC20(dpcToken).approve(address(dividendsAddress), dpcExcess);
            IDividendsV2(address(dividendsAddress)).addDividendsToPending(address(dpcToken),dpcExcess); 
        }else if(dpcExcess > 0) {
            dpcToken.safeTransfer(0x000000000000000000000000000000000000dEaD, dpcExcess);
        }

        _burn(address(this), esDPCAmount);

        emit FinalizeRedeem(userAddress, esDPCAmount, dpcAmount);
    }

    /**
     * @dev Allocates "userAddress" user's "amount" of available esDPC to "usageAddress" contract
   *
   */
    function _allocate(address userAddress, address usageAddress, uint256 amount) internal {
        ...
        uint256 approvedESDPC = usageApprovals[userAddress][usageAddress];
        require(approvedESDPC >= amount, "allocate: non authorized amount");

        // remove allocated amount from usage's approved amount
        usageApprovals[userAddress][usageAddress] = approvedESDPC.sub(amount);
    ...
    /**
     * @dev Deallocates "amount" of available esDPC to "usageAddress" contract
   *
   * args specific to usage contract must be passed into "usageData"
   */
    function _deallocate(address userAddress, address usageAddress, uint256 amount) internal {
        ...
        // check if there is enough allocated esDPC to this usage to deallocate
        ...
        // burn corresponding DPC and esDPC
        dpcToken.transfer(0x000000000000000000000000000000000000dEaD, deallocationFeeAmount);
   ...
   */
    ...
    /**
     * @dev Set whether the DPC is destroyed beyond the redemption part
   */
    function updateRedeemBurn(bool isBurn) external onlyOwner{
        bool old = isRedeemBurn;
        isRedeemBurn = isBurn;
        emit setRedeemBurn(old,isBurn);
    }
