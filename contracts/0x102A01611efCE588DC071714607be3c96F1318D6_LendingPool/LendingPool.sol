...
interface IVariableDebtToken is IScaledBalanceToken, IInitializableDebtToken {
   ...
   * @param index The variable debt index of the reserve
   **/
  function burn(address user, uint256 amount, uint256 index) external payable;
...
interface ISturdyAPRDataProvider {
  function updateAPR(address _borrowReserve, uint256 _yield, uint256 _totalSupply) external;
}
...
contract LendingPool is VersionedInitializable, ILendingPool, LendingPoolStorage {
  ...
  uint256 private constant LENDINGPOOL_REVISION = 0x5;
