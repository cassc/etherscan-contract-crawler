...
contract SweepersNFTVault is ReentrancyGuard, Ownable, IERC721Receiver, IERC1155Receiver {
    ...
    modifier holdsDust() {
        require(!mustHold || SWEEPERS.balanceOf(msg.sender) > 0, "Must hold a Sweeper NFT");
        _;
    }
    ...
    function createAuction(address _nftContract, uint256 _tokenId, bool _is1155, uint32 _startTime, uint32 _endTime, uint256 _startingPrice, string calldata _slug) external onlySweepersTreasury nonReentrant {
    ...
    function createManyAuctionSameProject(address _nftContract, uint256[] calldata _tokenIds, bool _is1155, uint32 _startTime, uint32 _endTime, uint256 _startingPrice, string calldata _slug) external onlySweepersTreasury nonReentrant {
    ...
    function createBlindAuction(address _nftContract, bool _is1155, uint32 _startTime, uint32 _endTime, string calldata _hiddenImage, uint256 _startingPrice, string calldata _slug) external onlySweepersTreasury nonReentrant {
        uint32 id = currentAuctionId++;
    ...
    function createManyBlindAuctionSameProject(address _nftContract, bool _is1155, uint16 _numAuctions, uint32 _startTime, uint32 _endTime, string calldata _hiddenImage, uint256 _startingPrice, string calldata _slug) external onlySweepersTreasury nonReentrant {
    ...
    function updateBlindAuction(uint32 _id, uint256 _tokenId) external onlySweepersTreasury {
    ...
    function updateManyBlindAuction(uint32[] calldata _ids, uint256[] calldata _tokenIds) external onlySweepersTreasury {
    ...
    function updateBlindAuction1155(uint32 _id, bool _is1155) external onlySweepersTreasury {
        auctionId[_id].is1155 = _is1155;
    }

    function updateManyBlindAuction1155(uint32[] calldata _ids, bool _is1155) external onlySweepersTreasury {
        for(uint i = 0; i < _ids.length; i++) {
    ...
    function _cancelAuction(uint32 _id) private {
        ...
        if (lastBidder != address(0)) {
            DUST.mint(lastBidder, auctionId[_id].currentBid);
     ...
     * @dev This contract only accepts payment in DUST.
     */
    function createBid(uint32 _id, uint256 _bidAmount) external payable holdsDust nonReentrant {
        ...
        DUST.burnFrom(msg.sender, _bidAmount);
