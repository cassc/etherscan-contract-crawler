    ...
    PullPaymentUpgradeable, ReentrancyGuardUpgradeable
{
  ...
  function claimReferralCode(bytes32 code) public whenNotPaused {
    require(partnersByReferralCode[code] == address(0), 'the generated code already exists');
    partnersByReferralCode[code] = msg.sender;
    referralCodesByPartner[msg.sender].push(code);
  }

  function makeReferralCode() public whenNotPaused returns (bytes32) {
    bytes32 code = keccak256(abi.encodePacked(
        'EthernaLotto_Referral', block.timestamp, msg.sender));
  ...
  function buyTicket(bytes32 referralCode, uint8[] calldata playerNumbers)
      public payable whenNotPaused nonReentrant
  {
    ...
    require(referralCode == 0 || partnerAccount != address(0), 'invalid referral code');
    ...
    if (referralCode != 0) {
      ...
      payable(partnerAccount).sendValue(fee);
    } else {
      payable(owner()).sendValue(msg.value / 5);
    }
  }
