...
contract Marketplace is Initializable, Pausable, AccessControlUpgradeable, IMarketplace {
    ...
    function initialize(address _nft) public initializer {
        ...
        foundationFee = 200;
        // 2% to foundationAddress (only initial sale)
        adminFee = 800;
        // 8% to adminAddress on initial sale

        adminFeeOnResell = 250;
        // 2.5% to adminAddress on resell
        creatorFee = 750;
        // 7.5% creator (init artist) on resell (from this 7.5% we also pay collaborators fees)
    ...
    function bidAuction(uint256 _tokenId, uint256 _newBid) external onlyActiveAuction(_tokenId) returns (bool){
    ...
    function endAuction(uint256 _tokenId, address winner, bytes memory signature)
    external onlyActiveAuction(_tokenId)
    returns (bool){

        uint256 creationTime = _auctionState[_tokenId].creationTime;
        address seller = _auctionState[_tokenId].seller;

        bytes32 hashWithoutPrefix = keccak256(abi.encode(_tokenId, winner, creationTime, seller));

        address signer = recoverSigner(prefixed(hashWithoutPrefix), signature);
    ...
    internal {
        ...
        _auctionState[_tokenId] = AuctionState(true, block.timestamp, 0, _minPrice, 0, msg.sender, _resell, 0, address(0));
    ...
    function _endAuction(uint256 _tokenId, address _winner) internal {
        ...
        if (_winner == address(0)) {
            ...
            require(_auctionState[_tokenId].highestBidder == _winner, "Winner is not highest bidder");
