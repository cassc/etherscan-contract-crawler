...
contract Presale is IPresale, Pausable, Ownable, ReentrancyGuard {
    ...
    modifier checkSaleState(uint256 amount) {
        ...
        require(
            amount + totalTokensSold <= token_amount[maxStepIndex],
            "Insufficient funds"
        );
        _;
    }
        ...
        returns (uint256 soldOnCurrentStage)
    {
        soldOnCurrentStage =
            totalTokensSold -
            ((currentStep == 0) ? 0 : token_amount[currentStep - 1]);
    }
    ...
    function calculatePrice(uint256 _amount) public view returns (uint256) {
        require(
            _amount + totalTokensSold <= token_amount[maxStepIndex],
