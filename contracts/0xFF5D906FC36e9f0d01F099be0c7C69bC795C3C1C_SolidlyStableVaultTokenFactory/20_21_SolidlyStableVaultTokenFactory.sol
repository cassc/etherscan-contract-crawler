...
import "./SolidlyStableVaultToken.sol";
import "./interfaces/ISolidlyStableVaultTokenFactory.sol";

contract SolidlyStableVaultTokenFactory is Ownable, ISolidlyStableVaultTokenFactory {
        ...
        returns (address vaultToken)
    {
        ...
        bytes memory bytecode = type(SolidlyStableVaultToken).creationCode;
        assembly {
            vaultToken := create2(0, add(bytecode, 32), mload(bytecode), _underlying)
        }
        SolidlyStableVaultToken(vaultToken)._initialize(
            _underlying,
            optiSwap,
            router,
            voter,
