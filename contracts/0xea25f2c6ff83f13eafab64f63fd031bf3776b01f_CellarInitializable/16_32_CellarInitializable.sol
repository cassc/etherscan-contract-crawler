...
contract CellarInitializable is Cellar, Initializable {
    ...
    function initialize(bytes calldata params) external initializer {
        (Registry _registry, ERC20 _asset, address _owner, string memory _name, string memory _symbol, bytes memory _params) = abi
            .decode(params, (Registry, ERC20, address, string, string, bytes));
        // Initialize Cellar
        registry = _registry;
        asset = _asset;
        owner = _owner;
