...
contract FeistyFelinez is ERC721A, Ownable, ReentrancyGuard {

  string public        baseURI;
  uint public          maxSupply         = 6969;
  uint public          totalFree         = 1000;

  uint private         _price            = 0.005 ether;
  uint private         _maxPerTx         = 10;
  uint private         _maxPerWallet     = 21;

  uint public          nextOwnerToExplicitlySet;
  bool public          mintEnabled;

  constructor() ERC721A("FeistyFelinez", "FF"){}

  function mint(uint256 amt) external payable
  {
    require(mintEnabled, "Minting is not enabled.");
    require(msg.sender == tx.origin,"Origin does not match sender.");
