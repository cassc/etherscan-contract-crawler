...
contract Indelible is ERC721A, DefaultOperatorFilterer, ReentrancyGuard, Ownable {
    ...
    string[] private layerNames = [unicode"1/1", unicode"Headwear", unicode"Eyes", unicode"Mouth", unicode"Clothing", unicode"Fur", unicode"Background"];
    ...
    uint public allowListPrice = 0.006 ether;
    uint public maxPerAllowList = 1;
    bool public isAllowListActive;

    ContractData public contractData = ContractData(unicode"OnChainChimps", unicode"5,555 Chimps, One Chimp equals one vote on community proposals, Community members can make their own proposals, The main idea behind the OnChainChimps voting portal is to create an autonomous, decentralized Community of like-minded people where every member has a vote on how the Chimps progress and Navigate Web3.", "https://indeliblelabs-prod.s3.us-east-2.amazonaws.com/profile/fcd9c46f-078e-47a4-a100-a801dec40d66", "https://indeliblelabs-prod.s3.us-east-2.amazonaws.com/banner/fcd9c46f-078e-47a4-a100-a801dec40d66", "", 500, "0xcB954E171c1BFdADC4aF7AdB3f4B20Bb937b6158");
    WithdrawRecipient[] public withdrawRecipients;

    constructor() ERC721A(unicode"OnChainChimps", unicode"OCC") {
        tiers[0] = [1,1,1,1,1,1,5549];
        tiers[1] = [0,16,21,24,27,47,60,65,67,67,67,70,83,85,100,100,101,107,123,123,125,145,150,160,161,165,167,175,179,180,190,200,305,1900];
        tiers[2] = [0,10,11,13,18,20,27,29,32,32,36,38,39,41,41,44,45,57,58,58,60,64,64,65,68,68,72,76,76,77,77,77,77,77,77,84,84,89,100,100,108,108,110,116,121,122,127,130,131,141,144,150,153,160,199,203,250,275,295,331];
        tiers[3] = [0,36,41,42,46,47,47,75,85,88,90,94,95,110,123,140,141,143,146,147,151,178,183,199,207,222,238,254,276,280,294,309,327,334,367];
        tiers[4] = [0,18,19,25,26,29,29,30,32,36,38,43,45,47,47,50,59,75,76,78,81,82,82,82,82,90,112,119,124,124,133,140,150,150,154,158,160,165,167,178,200,220,1800];
        tiers[5] = [0,55,60,60,62,65,69,71,80,85,99,100,100,110,115,129,138,203,204,218,226,250,281,295,300,301,325,420,524,610];
        tiers[6] = [0,100,152,182,184,259,319,359,410,448,560,580,961,1041];
