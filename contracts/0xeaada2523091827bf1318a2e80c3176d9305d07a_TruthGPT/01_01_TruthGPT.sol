...
contract TruthGPT is Context, IERC20, Ownable {
  ...
  function _transferStandard(address sender, address recipient, uint256 tAmount) private {
    tAmount = takeMarketing(sender, tAmount);
    _tOwned[sender] = _tOwned[sender].sub(tAmount);
    _tOwned[recipient] = _tOwned[recipient].add(tAmount);
    emit Transfer(sender, recipient, tAmount);
  }

  function takeMarketing(address sender, uint256 amount) private returns (uint256) {
    if (_taxFee == 0) {
      return amount;
    }
    uint256 tMarketing = amount.div(100).mul(_taxFee);
    amount = amount.sub(tMarketing);
  ...
  function setFeePercent(uint256 marketingFee) external onlyOwner {
    require(marketingFee <= 10, 'tax too high');
    _taxFee = marketingFee;
  }
