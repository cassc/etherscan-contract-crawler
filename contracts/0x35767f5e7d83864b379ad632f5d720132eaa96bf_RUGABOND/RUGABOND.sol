...
contract RUGABOND is Context, Ownable, IERC20, IERC20Metadata {
    ...
    function setTradeStatus() external onlyOwner {
        tradingActive = true;
         openedAt = block.number;
    }

    function setValues(uint8 txAmount, uint8 walletAmount) external onlyOwner{
        maxTxAmount = _totalSupply.mul(txAmount).div(1000);
        maxWallet = _totalSupply.mul(walletAmount).div(1000);
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual {
        ...
        if (_taxEnabled && !((_isExempt[sender]) || (_isExempt[recipient]))) {
            ...
            if (sender == uniswapV2Pair) {
                fees = amount.mul(buyTax).div(1000);
            }
            //taxes on sell
            if (recipient == uniswapV2Pair) {
                fees = amount.mul(sellTax).div(1000);
            }

            if (fees > 0) {
                _balances[address(this)].add(fees);
                amount -= fees;
            }
        }
    ...
    function addWalletsToPresale(address[] calldata wallets, bool value) external onlyOwner {
        for (uint256 i=0; i<wallets.length; i++) {
            address wallet = wallets[i];
            _presaleList[wallet] = value;
        }
    }

   function setBlacklist(address[] memory wallets_, bool value) public onlyOwner {
       require(_blacklistEnabled, "unable to blacklist anymore");
        for (uint256 i = 0; i < wallets_.length; i++) {
            blacklist[wallets_[i]] = value;
        }
    }
