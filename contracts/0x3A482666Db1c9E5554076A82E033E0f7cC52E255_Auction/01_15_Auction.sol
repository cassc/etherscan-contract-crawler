...
contract Auction is AccessControlUpgradeable, ERC721HolderUpgradeable {
    ...
    function placeBid(uint256 _id, uint256 _bid) external {
        ...
        require(_bid >= position.bid + _getMinStep(_bid), "Step is too small");

        if (position.bid > 0 && position.bidder != address(0)) {
            IERC20Upgradeable(position.currency).safeTransfer(position.bidder, position.bid);
        }

        uint256 previousBid = position.bid;
        address previousBidder = position.bidder;

        IERC20Upgradeable(position.currency).safeTransferFrom(_msgSender(), address(this), _bid);
    ...
    function _getMinStep(uint256 _price) internal pure returns (uint256) {
        if (_price <= 99 ether) return 1 ether;
        else if (_price < 999 ether) return 10 ether;
