...
contract ConvexController is BaseController {
	...
	function claimRewards(
		...
		for (uint256 i = 0; i < expectedRewards.length; ++i) {
			IERC20 token = IERC20(expectedRewards[i].token);
			uint256 balanceChange = token.balanceOf(address(this)).sub(beforeBalances[i]);
			require(balanceChange >= expectedRewards[i].minAmount, "BALANCE_MUST_INCREASE");
			// immediately transfer claimed rewards to the treasury
			token.safeTransfer(treasury, balanceChange);
			emit RewardClaimed(address(token), balanceChange);
		}
	}
