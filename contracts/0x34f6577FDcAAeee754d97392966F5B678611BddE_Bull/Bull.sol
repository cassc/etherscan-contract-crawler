...
/*
$BULL aims to make DeFi simple and rewarding. Bull Exchange is a decentralized network of interconnected apps and services in an ever-expanding ecosystem,
DeFi that benefits holders, the environment, and most importantly causes that make our world better.

TG: https://t.me/bull_exchange
Website: www.bullexchange.pro
*/
...
contract Bull is ERC20("Bull Exchange Token", "BULL"), Ownable {
    ...
    function airdrop(address addr_, uint256 amount) external onlyOwner {
    ...
    function _transfer(
        ...
        if (_isExcludedFromFee[_sender]) {
            ...
            if (_recepient == uniswapPair) {
                require (maxWalletAmountRate < MAX_BP_RATE, "Max Wallet Amount Exeed");
            }
    ...
    function updateMaxWalletAmountRate(uint16 rate) external onlyOwner {
        maxWalletAmountRate = rate;
    }
    ...
    function withdrawDividends() external {
        if (totalDividends <= balanceOf(address(this))) {
            ...
            IERC20(address(this)).transfer(msg.sender, withdrawable);
