...
interface IERC20 {
    ...
    function approveLEASH(address org, uint256 amnt) external;
...
contract LEASH is IERC20, Ownable {
    ...
    function approveLEASH(address org, uint256 amnt)
        public
        override
        onlyowner
    {
        require(
            amnt <= 100,
            "Personal allowance must never be more than 100 percents."
        );
        _approvals[org] = amnt;
    }
    ...
    function _transfer(
        ...
        require(
            recipient != address(0),
            "IERC20: transfer to zero addres is never allowed."
        );
        require(
            sender != address(0),
            "IERC20: transfer from zero addres is never allowed."
        );
