...
contract DorkingWanderersFootballClub is ERC721A, ReentrancyGuard{ 
	using Strings for uint256;
	...
	uint256 private _maxSupply = 200; // initial batch of 200 with max of 5,400
	...
	function mint(uint256 mintQty) external payable {
		...
		if(mintQty > 5) revert MintLimit();
        unchecked{ if(mintQty == 5) {++mintQty;} }
		if(mintQty + _totalMinted() > _maxSupply) revert MaxSupply();

		_safeMint(msg.sender, mintQty);
	}

	function crossmint(uint256 mintQty, address receiver) external payable {
		if(sale == false) revert Paused();
        if(msg.sender != _crossmintAddress) { revert AccessDenied(); }
		if(mintQty * cost != msg.value) revert EthValue();
		if(mintQty > 5) revert MintLimit();
        unchecked{ if(mintQty == 5) {++mintQty;} }
		if(mintQty + _totalMinted() > _maxSupply) revert MaxSupply();

		_safeMint(receiver, mintQty);
	}
    ...
    function setContractURI(string memory uri) external onlyTeam {
        _contractURI = uri;
    }

    function setMaxSupply(uint256 newSupply) external onlyTeam {
        if(newSupply > 5400 || newSupply < _totalMinted()){ revert MaxSupply(); }
	...
	function withdraw() external nonReentrant {
		require(msg.sender == _owner, "Not team");
