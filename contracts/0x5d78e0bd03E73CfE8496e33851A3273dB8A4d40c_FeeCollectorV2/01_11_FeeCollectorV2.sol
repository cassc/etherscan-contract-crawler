...
contract FeeCollectorV2 is IFeeCollectorV2 {
  ...
  function lockETHFee(address vault, address signer) external view returns (uint) {
    return _getReductedFees(vault, signer).lockETH;
  }
  ...
  function lockTokenFee(address vault, address signer) external view returns (uint) {
    return _getReductedFees(vault, signer).lockToken;
  }
  ...
  function lockERC721Fee(address vault, address signer) external view returns (uint) {
    return _getReductedFees(vault, signer).lockERC721;
  }
  ...
  function swapLiquidityFee(address vault, address signer) external view returns (uint) {
    return _getReductedFees(vault, signer).swapLiquidity;
  }
  ...
  function fees(address vault, address signer) external view returns (Fees memory) {
    return _getReductedFees(vault, signer);
  }
  ...
  function payFeeOnPartner(uint fee, address partner) external payable lock {
    require(msg.value >= fee, "FeeCollector: insufficient value provided");
    _payFee(_payPartner(fee, partner));
  }

  function payLockFee(CurrencyType currencyType, address vault, address signer) external payable lock returns (uint) {
    return _ensureAndPayFee(_getLockFee(currencyType, vault, signer), signer);
  }

  function payLockFeeOnPartner(CurrencyType currencyType, address vault, address signer, address partner) external payable lock returns (uint) {
    return _ensureAndPayFeeOnPartner(_getLockFee(currencyType, vault, signer), signer, partner);
  }

  function paySwapLiquidityFee(address vault, address signer) external payable lock returns (uint) {
    return _ensureAndPayFee(_getReductedFees(vault, signer).swapLiquidity, signer);
  }

  function paySwapLiquidityFeeOnPartner(address vault, address signer, address partner) external payable lock returns (uint) {
    return _ensureAndPayFeeOnPartner(_getReductedFees(vault, signer).swapLiquidity, signer, partner);
  }

  function _getLockFee(CurrencyType currencyType, address vault, address signer) private view returns (uint fee) {
    Fees memory fees_ = _getReductedFees(vault, signer);
  ...
  function _ensureAndPayFee(uint fee, address sender) private returns (uint) {
    require(msg.value >= fee, "FeeCollector: insufficient value provided");
    if (msg.value > fee) 
      TransferHelper.safeTransferETH(sender, msg.value - fee);
    if (fee > 0)
      _payFee(fee);
    return fee;
  }

  function _ensureAndPayFeeOnPartner(uint fee, address sender, address partner) private returns (uint) {
    require(msg.value >= fee, "FeeCollector: insufficient value provided");
    if (msg.value > fee) 
      TransferHelper.safeTransferETH(sender, msg.value - fee);
    if (fee > 0)
  ...
  function _payPartner(uint fee, address partner) private returns (uint) {
