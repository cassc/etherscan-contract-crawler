...
// File: contracts/nft/TruthAidoge.sol
...
contract TruthAidoge is Ownable, ERC721, ERC2981 {
    using EnumerableSet for EnumerableSet.AddressSet;

    uint256 public constant MAX_SUPPLY = 10000;

    event Open(uint256 indexed id, address indexed owner, uint256 openTime);

    uint256 public nextTokenId = 1;
    uint256 public startOpenTime;
    bool public mintEnded;
    mapping (uint256 => bool) opened;
    string public baseURI;
    EnumerableSet.AddressSet private _minters;

    constructor(uint256 _startOpenTime) ERC721("Truth AIDOGE", "AIDO") {
        startOpenTime = _startOpenTime;
    }
    ...
    function mint(address to) external onlyMinter returns(uint256) {
        require(!mintEnded, "TruthAidoge: MINT_ENDED");
        require(nextTokenId <= MAX_SUPPLY, "AvatarNft: MAX_SUPPLY");
    ...
    function open(uint256 id) external  {
        require(block.timestamp >= startOpenTime, "TruthAidoge: NOT_START");
        require(ownerOf(id) == msg.sender, "TruthAidoge: NOT_OWNER");
        require(!opened[id], "TruthAidoge: OPENED");

        opened[id] = true;
        emit Open(id, msg.sender, block.timestamp);
    }

    function setStartOpenTime(uint256 _startOpenTime) external onlyOwner {
        startOpenTime = _startOpenTime;
    }
    ...
    function addMinter(address minter) public onlyOwner returns (bool) {
        require(minter != address(0), "TruthAidoge: minter is the zero address");
    ...
    function getMinter(uint256 index) public view returns (address) {
        require(index <= getMinterLength() - 1, "TruthAidoge: index out of bounds");
        return _minters.at(index);
    }

    // modifier for mint function
    modifier onlyMinter() {
        require(isMinter(msg.sender), "TruthAidoge: caller is not the minter");
        _;
    }
}