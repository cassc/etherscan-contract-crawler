...
contract BaseDutchAuctionERC721A is ERC721A, LinearDutchAuction, ReentrancyGuard {
    ...
    function _hashMintWhitelistAmount(string memory _prefix, address _address, uint256 amount) internal view returns (bytes32) {
        return keccak256(abi.encodePacked(_prefix, _address, amount));
    }

    function mintWhitelistAmount(bytes32 hash, bytes calldata signature, uint256 numberOfTokens, uint256 customLimit) external payable {
        require(_verify(hash, signature), "This hash's signature is invalid.");
        require(_hashMintWhitelistAmount(prefixDiscounted, msg.sender, customLimit) == hash, "Hash invalid.");
        require(_whitelistClaimed[msg.sender] + numberOfTokens <= customLimit, 'You cannot mint this many.');
        require(_whitelistClaimed[msg.sender] + numberOfTokens <= whitelistMaxMint, 'You cannot mint this many.');

        _whitelistClaimed[msg.sender] += numberOfTokens;
        _mintWhitelistPrice(discountedPrice, numberOfTokens);
    }
