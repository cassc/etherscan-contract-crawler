...
/**
 * @title Regular Job Timestamps v1.1
 *
...
// Admin
    ...
    function setManyWithOffset(uint[] memory _jobIds, uint _startIndex, uint32 _timestamp) public onlyRole(MINTER_ROLE) {
        for (uint i = _startIndex;i < _jobIds.length; i++){
            timestamps[_jobIds[i]] = _timestamp;
        }
    }
