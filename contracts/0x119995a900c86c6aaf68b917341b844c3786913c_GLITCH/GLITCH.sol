...
contract Balancer {
    ...
    GLITCH private _tokenContract;
    
    constructor(GLITCH tokenContract, IUniswapV2Router02 uniswapV2Router) public {
...
contract GLITCH is Context, IERC20, Ownable {
    ...
    uint256 private _alchemyInterval = 1800;
    ...
    function reflect(uint256 tAmount) public {
        ...
        require(!_isExcluded[sender], "Glitch: Excluded addresses cannot call this function");
    ...
    function tokenFromReflection(uint256 rAmount) public view returns(uint256) {
        require(rAmount <= _rTotal, "Glitch: Amount must be less than total reflections");
        uint256 currentRate =  _getRate();
        return rAmount.div(currentRate);
    }

    function excludeAccount(address account) external onlyOwner() {
        require(account != 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D, 'Glitch: We can not exclude Uniswap router.');
        require(account != address(this), 'Glitch: We can not exclude contract self.');
        require(account != _rewardWallet, 'Glitch: We can not exclude reweard wallet.');
        require(!_isExcluded[account], "Glitch: Account is already excluded");
    ...
    function includeAccount(address account) external onlyOwner() {
        require(_isExcluded[account], "Glitch: Account is already included");
    ...
    function _approve(address owner, address spender, uint256 amount) private {
        require(owner != address(0), "Glitch: approve from the zero address");
        require(spender != address(0), "Glitch: approve to the zero address");
    ...
    function alchemy() public lockTheSwap {
        require(balanceOf(_msgSender()) >= _minTokenForAlchemy, "Glitch: You have not enough GLITCH to ");
        require(now > _lastAlchemy + _alchemyInterval, 'Glitch: Too Soon.');
    ...
    function _setFeeDecimals(uint256 feeDecimals) external onlyOwner() {
        require(feeDecimals >= 0 && feeDecimals <= 2, 'Glitch: fee decimals should be in 0 - 2');
        _feeDecimals = feeDecimals;
        emit FeeDecimalsUpdated(feeDecimals);
    }
    
    function _setTaxFee(uint256 taxFee) external onlyOwner() {
        require(taxFee >= 0  && taxFee <= 5 * 10 ** _feeDecimals, 'Glitch: taxFee should be in 0 - 5');
        _taxFee = taxFee;
        emit TaxFeeUpdated(taxFee);
    }
    
    function _setLockFee(uint256 lockFee) external onlyOwner() {
        require(lockFee >= 0 && lockFee <= 5 * 10 ** _feeDecimals, 'Glitch: lockFee should be in 0 - 5');
        _lockFee = lockFee;
        emit LockFeeUpdated(lockFee);
    }
    
    function _setMaxTxAmount(uint256 maxTxAmount) external onlyOwner() {
        require(maxTxAmount >= 500000e9 , 'Glitch: maxTxAmount should be greater than 500000e9');
        _maxTxAmount = maxTxAmount;
        emit MaxTxAmountUpdated(maxTxAmount);
    }
    
    function _setMinTokensBeforeSwap(uint256 minTokensBeforeSwap) external onlyOwner() {
        require(minTokensBeforeSwap >= 50e9 && minTokensBeforeSwap <= 25000e9 , 'Glitch: minTokenBeforeSwap should be in 50e9 - 25000e9');
        require(minTokensBeforeSwap > _autoSwapCallerFee , 'Glitch: minTokenBeforeSwap should be greater than autoSwapCallerFee');
    ...
    function _setAutoSwapCallerFee(uint256 autoSwapCallerFee) external onlyOwner() {
        require(autoSwapCallerFee >= 1e9, 'Glitch: autoSwapCallerFee should be greater than 1e9');
    ...
    function _setLiquidityRemoveFee(uint256 liquidityRemoveFee) external onlyOwner() {
        require(liquidityRemoveFee >= 1 && liquidityRemoveFee <= 10 , 'Glitch: liquidityRemoveFee should be in 1 - 10');
    ...
    function _setAlchemyCallerFee(uint256 alchemyCallerFee) external onlyOwner() {
        require(alchemyCallerFee >= 1 && alchemyCallerFee <= 15 , 'Glitch: alchemyCallerFee should be in 1 - 15');
    ...
    function _updateWhitelist(address poolAddress, address pairTokenAddress) public onlyOwner() {
        require(poolAddress != address(0), "Glitch: Pool address is zero.");
        require(pairTokenAddress != address(0), "Glitch: Pair token address is zero.");
        require(pairTokenAddress != address(this), "Glitch: Pair token address self address.");
        require(pairTokenAddress != currentPairTokenAddress, "Glitch: Pair token address is same as current one.");
