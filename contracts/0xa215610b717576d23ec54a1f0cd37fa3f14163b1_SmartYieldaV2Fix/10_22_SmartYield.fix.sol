 ...
 **/
contract SmartYieldaV2Fix is ISmartYield, ReentrancyGuardUpgradeable {
    ...
    function repay(uint256 _debtId) external override nonReentrant defaultCheck {
        ...
        IERC20Upgradeable(debt.borrowAsset).safeTransferFrom(msg.sender, controller, compoundBalance);
    ...
    function _computeHealthFactor(Debt memory _debt) public view returns (uint256, uint256) {
        ...
        uint256 healthFactor = (((_debt.collateralAmount * liquidationThreshold) *
            10**(IERC20MetadataUpgradeable(_debt.borrowAsset).decimals())) * 10**18) /
            10000 /
            compoundedBalance /
            10**(IERC20MetadataUpgradeable(underlying).decimals());
