    ...
    Ownable
{
    ...
    /**
     * @notice Pay to steal a color from someone else, the person who got stolen from gets steal price
    ...
    function steal(string calldata _hexColor) external payable {
        ...
        // Send 90% of steal price funds to prev owner
        (bool s1, ) = payable(prevOwner).call{
            value: stealPrice.mul(ONE_PERCENT * 90)
        }("");
    ...
    /**
     * @notice Airdrop colors
     * @param _colors The colors
     * @param _addresses The addresses
     */
    function airdrop(
        string[] calldata _colors,
        address[] calldata _addresses
    ) external onlyOwner {
        for (uint256 i; i < _colors.length; i++) {
            string memory normalizedColor = _toLower(_colors[i]);
            uint256 nextId = _nextTokenId();
            hexToTokenId[normalizedColor] = nextId;
            tokenIdToHex[nextId] = normalizedColor;
