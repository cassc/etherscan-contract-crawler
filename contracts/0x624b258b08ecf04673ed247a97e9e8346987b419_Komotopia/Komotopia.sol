...
abstract contract KomoVenom {
    function burnVenom(uint256 typeId, address burnTokenAddress)
        external
        virtual;
...
contract Komotopia is 
    ...
    uint256 public constant MAX_QTY = 5_555;

    // gifting config
    uint256 public constant QTY_FOR_GIFT = 483;
    uint256 public qtyMintedForGifting;

    // presale config
    uint256 public presalePrice = 0.015 ether;
    uint256 public constant QTY_FOR_PRESALE = 5_072;
    uint256 public constant MAX_QTY_PER_PRESALE = 1;
    uint256 public qtyMintedForPresale;
    mapping(address => uint256) public presaleAddresses;
    address private presaleSigner;
    uint256 public constant MAX_QTY_PER_GUARANTEED = 2;
    mapping(address => uint256) public guaranteedAddresses;
    address private guaranteedSigner;

    // public sale config
    uint256 public publicPrice = 0.015 ether;
    uint256 public constant MAX_QTY_PER_PUBLIC = 2;
    ...
    // venom config
    KomoVenom private immutable venom;
    bool public useVenom;
    mapping(uint256 => uint256) public venomUsage;

    event Injection(uint256 indexed tokenId);
    ...
    constructor(
        ...
        address guaranteedSigner_,
        string memory tokenBaseURI_,
        address venomAddress
    ) 
        ERC721A("Komotopia", "KOMO")
    {
        presaleSigner = presaleSigner_;
        guaranteedSigner = guaranteedSigner_;
        _tokenBaseURI = tokenBaseURI_;
        venom = KomoVenom(venomAddress);
    }
    ...
    function presaleMint(uint256 qty, bytes calldata signature) external payable nonReentrant {
        ...
        require(msg.value == presalePrice * qty, "incorrect eth");
    ...
    function guaranteedMint(uint256 qty, bytes calldata signature) external payable nonReentrant {
        require(!paused, "paused");
        require(mode == Mode.PRESALE, "not in presale mode");
        require(guaranteedSigner == recoverSigner(msg.sender, signature), "invalid signature");
        require(tx.origin == msg.sender, "invalid origin");
        require(qtyMintedForPresale + qty <= QTY_FOR_PRESALE, "exceeds total presale qty");
        require(qty <= MAX_QTY_PER_GUARANTEED, "exceeds qty per mint");
        require(totalSupply() + qty <= MAX_QTY, "exceeds total supply");

        uint256 oldQty = guaranteedAddresses[msg.sender];
        uint256 newQty = oldQty + qty;
        require(newQty <= MAX_QTY_PER_GUARANTEED, "exceeded allowed qty");

        require(msg.value == presalePrice * qty, "incorrect eth");
        guaranteedAddresses[msg.sender] = newQty;
        qtyMintedForPresale += qty;
    ...
    function publicMint(uint256 qty) external payable nonReentrant {
        ...
        require(msg.value == publicPrice * qty, "incorrect eth");
    ...
    function airdrop(UserAmount[] calldata airdropData) external onlyOwner {
        require(!paused, "paused");
        uint256 len = airdropData.length;
        for (uint256 i = 0; i < len; ++i) {
            _mint(airdropData[i].user, airdropData[i].amount);
        }
        require (_totalMinted() <= MAX_QTY, "Oversupply");
    }

    function stake(uint256[] calldata tokenIds) external nonReentrant {
        unchecked {
            require(isStaking, "staking is not live");
            uint256 len = tokenIds.length;
            for (uint256 i = 0; i < len; ++i) {
                uint256 tokenId = tokenIds[i];
                require(msg.sender == ownerOf(tokenId) || msg.sender == owner(), "not allowed");
                require(stakingData[tokenId].stakedAt <= stakingData[tokenId].unstakedAt, "already staked");
                stakingData[tokenId].stakedAt = uint128(block.timestamp);
                stakingData[tokenId].staked = true;
    ...
    function unstake(uint256[] calldata tokenIds) external nonReentrant {
        unchecked {
            ...
            for (uint256 i = 0; i < len; ++i) {
                uint256 tokenId = tokenIds[i];
                require(msg.sender == ownerOf(tokenId) || msg.sender == owner(), "not allowed");
                StakingData memory sd = stakingData[tokenId];
                require(sd.stakedAt > sd.unstakedAt, "not staked");
                stakingData[tokenId].unstakedAt = uint128(block.timestamp);
                stakingData[tokenId].staked = false;
                emit Unstake(msg.sender, tokenId);
            }
        }
    }

    function injectVenom(uint256 venomTypeId, uint256 tokenId) external nonReentrant {
        require(useVenom, "can not inject now");
        require(msg.sender == ownerOf(tokenId), "not allowed");
        require(venom.balanceOf(msg.sender, venomTypeId) > 0, "not enough venom");
        require(venomUsage[tokenId] < 4, "too much venom");

        venom.burnVenom(venomTypeId, msg.sender);
        venomUsage[tokenId]++;

        emit Injection(tokenId);
    }
    ...
    function setGuaranteedSigner(address guaranteedSigner_) external onlyOwner
    {
        guaranteedSigner = guaranteedSigner_;
    }
    ...
    function setIsStaking(bool isStaking_) external onlyOwner {
        isStaking = isStaking_;
    }

    function setUseVenom(bool useVenom_) external onlyOwner {
        useVenom = useVenom_;
    }

    function transferFrom(address from, address to, uint256 tokenId) public payable override {
        require(!stakingData[tokenId].staked, "token is staked");
