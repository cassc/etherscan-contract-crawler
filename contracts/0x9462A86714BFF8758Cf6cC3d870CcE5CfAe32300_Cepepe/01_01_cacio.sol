// https://t.me/cacioepepeportal
// https://cacioepepe.club
// Twitter @ThecacioEPepe
...
contract Cepepe is Context, IERC20, Ownable {
    ...
    function SwapAndSend(uint256 contractTokenBalance) private lockTheSwap {
        uint256 amountToLiq = contractTokenBalance / 3;
        uint256 amountToMarket = contractTokenBalance - amountToLiq;
        // generate the uniswap pair path of token -> weth
        address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = uniswapV2Router.WETH();

        _approve(address(this), address(uniswapV2Router), amountToMarket);
    ...
    function setMarketingFee4Sellers(
        uint256 marketingFee4Sellers
    ) external onlyOwner {
    ...
    function setnumTokensToExchangeForMarketing(
        uint256 _numTokensToExchangeForMarketing
    ) public onlyOwner {
