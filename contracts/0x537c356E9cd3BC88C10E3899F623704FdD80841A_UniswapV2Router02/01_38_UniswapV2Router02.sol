...
contract UniswapV2Router02 {
    ...
    function addLiquidityETH(
        ...
        SafeERC20.safeTransfer(IERC20(WETH), pair, amountETH);
        liquidity = IUniswapV2Pair(pair).mint(to);
    ...
    function swapExactTokensForTokens(
        ...
        SafeERC20.safeTransferFrom(
            IERC20(path[0]), msg.sender, UniswapV2Library.pairFor(factory, path[0], path[1]), amounts[0]
        );
        ...
        returns (uint[] memory amounts)
    {
        ...
        SafeERC20.safeTransfer(IERC20(WETH), UniswapV2Library.pairFor(factory, path[0], path[1]), amounts[0]);
        _swap(amounts, path, to);
    }
        ...
        ensure(deadline)
    {
        ...
        SafeERC20.safeTransfer(IERC20(WETH), UniswapV2Library.pairFor(factory, path[0], path[1]), amountIn);
        uint balanceBefore = IERC20(path[path.length - 1]).balanceOf(to);
        _swapSupportingFeeOnTransferTokens(path, to);
        require(
            IERC20(path[path.length - 1]).balanceOf(to) - balanceBefore >= amountOutMin,
