      ...
      returns(uint256 amountOutputReceived)
{
  ...
  /**
   * Try updating Vault Rate for Morpho vaults
   *
   * @param vault         Address of the vault
   */
  function _tryUpdateVaultRate(SupplyVault vault) internal returns (bool updated){
      try vault.morpho() returns (address _morpho){
        Morpho morpho;
        morpho = Morpho(_morpho);
        address poolToken = vault.poolToken();
        try morpho.updateIndexes(poolToken){
          // update indexes for Aave
          updated = true;
        } catch {
          // update indexes for Compound
