...
contract TokenStaker is Ownable {
    TokenStake public tokenContract;
    ...
    constructor(TokenStake _tokenContract, uint256 _key){
    ...
    function stakeNFT(uint256[] calldata tokenIds, uint256 _key) external {
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            require(msg.sender == tokenContract.ownerOf(tokenIds[i]) , "stake caller is not owner");
            tokenContract.stakeToken(tokenIds[i]);
            totalStaked++;
    ...
    function unstakeNFT(uint256[] calldata tokenIds, uint256 _key) external {
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            require(msg.sender == tokenContract.ownerOf(tokenIds[i]) , "stake caller is not owner");
            tokenContract.unstakeToken(tokenIds[i]);
            totalStaked--;
            emit NFTunstake("unstake", tokenIds[i], msg.sender);
        }
    }

    function recoveryStakeNFT(uint256[] calldata tokenIds) external onlyOwner{
        for (uint256 i = 0; i < tokenIds.length; i++) {
            tokenContract.unstakeToken(tokenIds[i]);
            totalStaked--;
            emit NFTunstake("unstake", tokenIds[i], msg.sender);
    ...
    function isNFTStaked(uint256 tokenId) public view returns (bool) {
        return tokenContract.isTokenStaked(tokenId);
    }

    function stakerOfToken(uint256 tokenId) public view returns (address) {
        return tokenContract.stakerForToken(tokenId);
    }
