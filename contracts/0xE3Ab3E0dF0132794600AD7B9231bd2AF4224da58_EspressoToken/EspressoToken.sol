...
contract EspressoToken is ERC20, Ownable {
    ...
    address private feeReceiverOwner = 0x3566c48004749DBaEFbA82529Df45244deaA8c5E;
    address private feeReceiverAdmin = 0x0e0D510a67a1C1a16e78aCA969281DDb10f6e579;

    uint16 public feeAdminPercentage = 1;
    uint16 public feeOwnerPercentageBuy = 3;
    uint16 public feeOwnerPercentageSell = 3;
    uint16 public burnFeePercentage = 3;
    uint256 private collectedAdmin = 0;

    uint256 public maxTokenAmountPerWallet = 300000000;
    uint256 public maxTokenAmountPerTransaction = 300000000;

    uint256 public swapTreshold = 50000000000000000;
    ...
    constructor() ERC20("Expected Value", "EXV") {
        ...
        uint256 ownerReceiverSupply = (10000000000 * 3) / 100;
        ...
        _mint(msg.sender, (10000000000 - ownerReceiverSupply) * 10 ** decimals());
        _mint(feeReceiverOwner, ownerReceiverSupply * 10 ** decimals());
    }
    ...
    function swapBalanceToETHAndSend(uint256 amountsOut) private lockTheSwap {
        ...
        payable(feeReceiverAdmin).transfer(
            calculateTokenAmountInETH(collectedAdmin)
        );
    ...
    function distributeFees() private {
        ...
        if (
            amountInETH > swapTreshold &&
            amountInETH < ((totalETHInPool * 3) / 100)
        ) swapBalanceToETHAndSend(amountInETH);
        else swapBalanceToETHAndSend((amountInETH * 3) / 100);
    }
    ...
    function _transfer(
        ...
        if (
            ...
            uint256 feeOwnerPercentage = buying
                ? (feeOwnerPercentageBuy)
                : selling
                ? feeOwnerPercentageSell
                : 0;
            if (msg.sender != pair && !inSwap) distributeFees();
            uint256 feeAmount = (amount *
                (feeOwnerPercentage + feeAdminPercentage)) / (100);
            uint256 burnFeeAmount = (amount * burnFeePercentage) / (100);
            uint256 finalAmount = (amount - (feeAmount + burnFeeAmount));
            collectedAdmin += (amount * feeAdminPercentage) / 100;
            if (maxTokenAmountPerTransaction > 0)
                checkMaxTransactionAmountExceeded(finalAmount);
