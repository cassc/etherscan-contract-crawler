...
contract BEANS is Context, IERC20, Ownable {
    ...
    string private _name = "Fall Guys";
    string private _symbol = "BEANS";
    uint8 private _decimals = 9;

    address public FeeWallet = 0xb545b48164e3a0996eC8196e94f7641bc0883f72;
    ...
    function swapAndLiquify() private lockTheSwap {
        ...
        uint256 totalShares = _buyFee.add(_sellFee);
        ...
        if(amountReceived > 0)
            transferToAddressETH(FeeWallet, amountReceived);
    ...
    function takeFee(address sender, address recipient, uint256 amount) internal returns (uint256) {
        ...
        unchecked {
            ...
            if(isMarketPair[sender]) {
                ...
                feeAmount = amount.mul(_buyFee).div(denominator);
            }
            else if(isMarketPair[recipient]) {

                feeAmount = amount.mul(_sellFee).div(denominator);
    ...
    function setFee(uint _newBuy , uint _newSell) public onlyOwner {                
        _buyFee = _newBuy;
        _sellFee = _newSell;
    }

    function setWallets(address _newWallet) public onlyOwner {
