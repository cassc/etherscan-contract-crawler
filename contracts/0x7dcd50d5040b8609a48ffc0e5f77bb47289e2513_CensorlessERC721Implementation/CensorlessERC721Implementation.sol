...
contract CensorlessERC721Implementation is ERC721, ERC721TokenURI, Controllerable {

    constructor(address owner_) ERC721(
        "CensorlessERC721",
        "CERC721"
    ) {
    ...
    function proxyInitialize(string memory name_, string memory symbol_,
    address owner_) public {
        // ERC721
        name = name_;
        symbol = symbol_;

        // Ownable (Set as caller for Controllerable Setting)
        owner = msg.sender;

        // Mint ERC721 (to, tokenId)
        _mint(owner_, 0);

        // Set Owner as a Controllerable MINTER
        setController("MINTER", owner_, true);

        // Set Owner as a Controllerable TOKENURISETTER
        setController("TOKENURISETTER", owner_, true);

        // Set Owner as a Controllerable TOKENDATASETTER
        setController("TOKENDATASETTER", owner_, true);

        // To use Controllerable BURNER functions, set it yourself

        // Ownable (Final Set as the owner_)
        owner = owner_;
    }

    function setNameAndSymbol(string calldata name_, string calldata symbol_)
    external onlyController("TOKENDATASETTER") {
        name = name_;
        symbol = symbol_;
    }

    function controllerMint(address to_, uint256 amount_) external 
    onlyController("MINTER") {
        _mint(to_, amount_);
    }
    function controllerBurn(uint256 tokenId_) external 
    onlyController("BURNER") {
        _burn(tokenId_);
    }
