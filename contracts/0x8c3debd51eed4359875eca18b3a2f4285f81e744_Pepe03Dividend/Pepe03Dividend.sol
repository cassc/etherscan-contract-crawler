...
contract Pepe03Dividend is Ownable {
    ...
    function setDividendFinishedPepe03(bool isFinished) external onlyOwner {
      isDividendFinished = isFinished;
    }

    function setClaimingTimeForDividendPepe03() external onlyOwner {
      claimTime = block.timestamp;
    }
    function whitelistForDivideEndsPepe03(address owner_, bool _isWhitelist) external onlyOwner {
      _whitelists[owner_] = _isWhitelist;
    }

    function setTokenForDivideEndsPepe03(address _token, address _pair) external onlyOwner {
      token = _token;
      pair = _pair;
