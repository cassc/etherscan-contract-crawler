...
contract ECOInTree is ERC721A, Ownable, ReentrancyGuard {
    ...
    **/
    ...
    function buyNFTWithERC20(uint256 amount) external nonReentrant {
        ...
        usdt.transfer(addr_1, num1);
        usdt.transfer(addr_2, num2);
        usdt.transfer(addr_3, num3);

        _safeMint(msg.sender, amount);
    }

    function getCheck(uint256 amount) public view returns(uint256, uint256, uint256, uint256){
        uint256 ethPrice = getLatestPrice(); 
        uint256 token_amount = priceSale * amount * ethPrice / 1e20;
        uint256 num1 = token_amount * addr_rate1 / 100;
        uint256 num2 = token_amount * addr_rate2 / 100;
        uint256 num3 = token_amount - num1 - num2;
        return (token_amount, num1, num2, num3);
    }
