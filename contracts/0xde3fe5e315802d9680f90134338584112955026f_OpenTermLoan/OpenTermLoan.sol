...
abstract contract BaseLineOfCredit {
    ...
    function _beforeFundingLoan () internal virtual;
...
contract OpenTermLoan is BaseLoan {
    ...
    /**
     * @notice The minimum payment to make at the end of the billing cycle
     */
     ...
     */
    function callLoan (uint256 callbackPeriodInDays, uint256 gracePeriodInDays) public onlyLender onlyIfActive onlyIfPendingDebt {
    ...
    function repay (uint256 paymentAmountTokens) public onlyBorrower onlyIfActive onlyIfPendingDebt ifNotDefaulted {
        ...
        require(paymentAmountTokens >= minimumPaymentAmount, "Minimum payment amount required");
     ...
     */
    function withdrawInterests () public onlyLender onlyIfActive {
    ...
    // This function gets called when the lender funds the loan
    function _beforeFundingLoan () internal override {
        ...
        _remainingDebt += (_loanAmountInPrincipalTokens + applicableInterest);
    ...
    function _updateMinimumPaymentAmount () private {
        uint256 totalPendingDebt = getPaymentAmount();
        uint256 minRequiredAmount = _loanAmountInPrincipalTokens * apr * _paymentIntervalInSeconds / 1e4 / 365 days;
        minimumPaymentAmount = (totalPendingDebt <= minRequiredAmount) ? totalPendingDebt : minRequiredAmount;
    }
