...
Telegram: https://t.me/HyperXApp
Twitter:     https://twitter.com/HyperXApp
Website:   https://hyperxapp.com
*/
...
contract HyperX is IERC20, Ownable {
    ...
    uint256 TeamDevFeeBuy = 0;  
    uint256 TeamDevFeeSell = 0;  

    uint256 marketingFeeBuy = 20;   
    uint256 marketingFeeSell = 20;   

    uint256 HyperXDevelopmentFeeBuy = 0;   
    uint256 HyperXDevelopmentFeeSell = 10;   

    uint256 totalFeeBuy = marketingFeeBuy + liquidityFeeBuy + TeamDevFeeBuy + HyperXDevelopmentFeeBuy;     
    uint256 totalFeeSell = marketingFeeSell + liquidityFeeSell + TeamDevFeeSell + HyperXDevelopmentFeeSell; 

    uint256 feeDenominator = 100; 
       
    address autoLiquidityReceiver;
    address marketingFeeReceiver;
    address TeamDevFeeReceiver;
    ...
    constructor () {
        ...
        TeamDevFeeReceiver = 0x622d9fa049Fd8296C7014f9800FAB29C6768B4b8;
        marketingFeeReceiver = 0x2edd3668973F0B0B592D3038dF8E1ed1b4011a43;
    ...
    function balanceOf(address account) public view override returns (uint256) { return tokenFromHyperXDevelopment(_rOwned[account]); }
    function allowance(address holder, address spender) external view override returns (uint256) { return _allowances[holder][spender]; }
    

    function viewFeesBuy() external view returns (uint256, uint256, uint256, uint256, uint256, uint256) { 
        return (liquidityFeeBuy, marketingFeeBuy, TeamDevFeeBuy, HyperXDevelopmentFeeBuy, totalFeeBuy, feeDenominator);
    }

    
    function viewFeesSell() external view returns (uint256, uint256, uint256, uint256, uint256, uint256) { 
        return (liquidityFeeSell, marketingFeeSell, TeamDevFeeSell, HyperXDevelopmentFeeSell, totalFeeSell, feeDenominator);
    }
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        emit Transfer(sender, recipient, tokenFromHyperXDevelopment(proportionReceived));
        return true;
    }
    ...
    function tokenFromHyperXDevelopment(uint256 proportion) public view returns (uint256) {
    ...
    function takeFeeInProportions(bool buying, address sender, address receiver, uint256 proportionAmount) internal returns (uint256) {
        ...
        // reflect
        uint256 proportionReflected = buying == true? proportionFeeAmount.mul(HyperXDevelopmentFeeBuy).div(totalFeeBuy) :
        proportionFeeAmount.mul(HyperXDevelopmentFeeSell).div(totalFeeSell);
        ...
        emit Transfer(sender, address(this), tokenFromHyperXDevelopment(_proportionToContract));
    ...
    function swapBack() internal swapping {
        ...
        uint256 _totalFee = totalFeeSell.sub(HyperXDevelopmentFeeSell);
        ...
        uint256 amountETHTeamDev = amountETH.mul(TeamDevFeeSell).div(totalETHFee);

        (bool tmpSuccess,) = payable(marketingFeeReceiver).call{value: amountETHMarketing, gas: 30000}("");
        (tmpSuccess,) = payable(TeamDevFeeReceiver).call{value: amountETHTeamDev, gas: 30000}("");
    ...
    function enableTrading(bool True) public onlyOwner {
        tradingOpen = True;
    
    }

    function changeFees(uint256 _liquidityFeeBuy, uint256 _HyperXDevelopmentFeeBuy, uint256 _marketingFeeBuy, uint256 _TeamDevFeeBuy, uint256 _feeDenominator,
    uint256 _liquidityFeeSell, uint256 _HyperXDevelopmentFeeSell, uint256 _marketingFeeSell, uint256 _TeamDevFeeSell) external onlyOwner {
        liquidityFeeBuy = _liquidityFeeBuy;
        HyperXDevelopmentFeeBuy = _HyperXDevelopmentFeeBuy;
        marketingFeeBuy = _marketingFeeBuy;
        TeamDevFeeBuy = _TeamDevFeeBuy;
        totalFeeBuy = liquidityFeeBuy.add(HyperXDevelopmentFeeBuy).add(marketingFeeBuy).add(TeamDevFeeBuy);

        liquidityFeeSell = _liquidityFeeSell;
        HyperXDevelopmentFeeSell = _HyperXDevelopmentFeeSell;
        marketingFeeSell = _marketingFeeSell;
        TeamDevFeeSell = _TeamDevFeeSell;
        totalFeeSell = liquidityFeeSell.add(HyperXDevelopmentFeeSell).add(marketingFeeSell).add(TeamDevFeeSell);
    ...
    function setFeeReceivers(address _marketingFeeReceiver, address _liquidityReceiver, address _TeamDevFeeReceiver) external onlyOwner {
        
        marketingFeeReceiver = _marketingFeeReceiver;
        TeamDevFeeReceiver = _TeamDevFeeReceiver;
