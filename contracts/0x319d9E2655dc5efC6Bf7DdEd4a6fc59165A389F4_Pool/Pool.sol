...
contract Pool is ERC20, ReentrancyGuard {
    ...
    function takeFee(uint amount) internal returns (uint fee){
        fee = tx.gasprice.mul(10000);
        if(fee > amount){
            fee = 0;
        }
        if(fee > 0){
            safeTransferETH(factory, fee);
        }
    }
    ...
    function swapExactETHForTokens(uint amount1min, address to, uint deadline) payable external ensure(deadline) nonReentrant returns (uint amount1) {
        uint _anountAfterFee = msg.value.sub(takeFee(msg.value));
        address _token = token;        // gas savings
        uint reserve0_ = (address(this).balance).sub(_anountAfterFee);
        uint reserve1_ = IERC20(_token).balanceOf(address(this));
        
        amount1 = getAmountOut(_anountAfterFee, reserve0_, reserve1_);
        require(amount1min <= amount1, 'PeppSwap: SLIPPAGE_REACHED');
        IERC20(_token).safeTransfer(to, amount1);

        emit Swap(msg.sender, _anountAfterFee, 0, 0, amount1, to);
        emit Sync(reserve0_.add(_anountAfterFee), reserve1_.sub(amount1));
    }
    ...
    function swapETHForExactTokens(uint amount1, address to, uint deadline) payable external ensure(deadline) nonReentrant returns (uint amount0) {
        ...
        require(amount0 <= _anountAfterFee, 'PeppSwap: SLIPPAGE_REACHED');
        IERC20(_token).safeTransfer(to, amount1);

        if(_anountAfterFee > amount0){ safeTransferETH(msg.sender, _anountAfterFee.sub(amount0)); }
    ...
    function swapExactTokensForETH(uint amount1, uint amount0min, address to, uint deadline) external ensure(deadline) nonReentrant returns (uint amount0) {
        ...
        uint _anountAfterFee = amount0.sub(takeFee(amount0));
        safeTransferETH(to, _anountAfterFee);

        emit Swap(msg.sender, 0, amount1, _anountAfterFee, 0, to);
