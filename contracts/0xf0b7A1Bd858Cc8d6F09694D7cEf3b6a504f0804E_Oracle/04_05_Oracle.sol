...
contract Oracle is Ownable {
    ...
    function migrateOracle(
        ...
        for (uint256 i; i < _expiries.length; i++) {
            storedPrice[_asset][_expiries[i]] = Price(_prices[i], block.timestamp);
        }
    }
    ...
    function getPrice(address _asset) external view returns (uint256) {
        ...
        if (price == 0) {
            ...
            price = PricerInterface(assetPricer[_asset]).getPrice();
        }
    ...
    function isLockingPeriodOver(address _asset, uint256 _expiryTimestamp) public view returns (bool) {
        ...
        if (price == 0) {
            ...
            return block.timestamp > _expiryTimestamp.add(lockingPeriod);
        }
