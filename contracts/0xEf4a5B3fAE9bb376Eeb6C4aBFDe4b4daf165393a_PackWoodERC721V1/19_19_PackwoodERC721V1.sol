...
contract PackWoodERC721V1 is ERC721URIStorageUpgradeable, OwnableUpgradeable, PackWoodERC721StorageV1{
    using StringsUpgradeable for uint256;
    
    // constructor initialisation section

    function initialize() public initializer {
        __ERC721_init("Packwoods", "Packwoods");
        __Ownable_init();

        tokenCounter = 1; 
        _before = "https://liveassets.monsterbuds.io/Packwood-monster-uri";
       ...
       ERC1155Parent = IPackWoodERC1155(0xadc5f935D9aFB186d4EF210B0F537Ddb0D6C524C);
       ...
       SmartContractCommunity = payable(0x7f62Db798f29a6B074fE6b1B5027d883831cf1D7); // community wallet
    }

    // modifiers
    modifier onlyGenOne(uint256 tokenId) {
        require((866 >= tokenId && tokenId <= 10420) || checkTokenId[tokenId], "$MONSTERBUDS: Only generation 1 monsterbuds can process");
        _;
    }
    ...
    function updateGenerationOneTokens(uint256[] calldata _data) external virtual onlyOwner{
    ...
    function updateAllowedAddress(address[] calldata _address, bool _status, bool _public_access_status) external virtual onlyOwner{
     ...
     * - only owner can update value.
     */
    function updateSmartContractCommunity(address payable nextOwner) external onlyOwner returns (address){
    ...
    function process(uint256 _monsterTokenId, uint256 _packwoodERC1155TokenId) external virtual onlyAllowed onlyGenOne(_monsterTokenId) returns(uint256){
        require(MonsterParent.ownerOf(_monsterTokenId) == msg.sender, "PackwoodERC721: You are not owner of token");
        require(MonsterParent.breedingInfo(_monsterTokenId).timstamp <= block.timestamp, "PackwoodERC721: Can not process now");
    ...
    function selfBreedCollectiable(SelfBreed calldata breed, bytes calldata signature) external payable virtual onlyAllowed returns (uint256) {
        ...
        require(breedValue == msg.value, "$PackwoodERC721: Amount is incorrect");
        ...
        require(owner_req == owner_accept && owner_req == msg.sender && breed.req_token_id != breed.accept_token_id, "$PackwoodERC721: Cannot Self Breed");
        require(breedInfo[breed.req_token_id].breedCount < 2 && breedInfo[breed.accept_token_id].breedCount < 2, "$PackwoodERC721: Exceeds max breed count");
        require(block.timestamp >= breedInfo[breed.req_token_id].timstamp && block.timestamp >= breedInfo[breed.accept_token_id].timstamp,"$PackwoodERC721: cannot breed now");
        ...
        uint countOfReq = breedInfo[breed.req_token_id].breedCount;
        uint countOfAccept = breedInfo[breed.accept_token_id].breedCount;
    ...
    function purchase(Order memory order, bytes memory signature) external payable virtual onlyAllowed returns(bool){
    ...
    function createPfpVersion(uint256 token_id) external payable virtual onlyAllowed returns(bool){
    ...
    function selfBreedMonsterBuds(SelfBreed calldata breed, bytes calldata signature) external payable virtual onlyAllowed returns (uint256) {
        bool status = SignatureCheckerUpgradeable.isValidSignatureNow(owner(), breed.signKey, signature);
        require(status == true, "$PackwoodERC721: cannot breed[ERROR]");
        require(breedValue == msg.value, "$PackwoodERC721: Amount is incorrect");
        ...
        require(selfBreedStatus == true,"$PackwoodERC721: Breeding is closed");
        require(owner_req == owner_accept && owner_req == msg.sender, "$PackwoodERC721: Cannot Self Breed");
        require(breedInfo[breed.accept_token_id].breedCount < 2, "$PackwoodERC721: Exceeds max breed count");
        require(block.timestamp >= breedInfo[breed.accept_token_id].timstamp,"$PackwoodERC721: cannot breed now");
        ...
        uint256 newItem = MonsterParent.breedUpdation(breed.req_token_id, msg.sender);
