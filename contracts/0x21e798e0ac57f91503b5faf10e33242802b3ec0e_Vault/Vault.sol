...
/// @notice Global variables to keep track of the accounts and the deposits they make
    mapping(address => uint256) public brxTokenBalance;
...
/// @notice Total amount of BRX in the contract
    uint256 public brxTotal;
...
/// @dev Events for proper vault functionality and transaction execution 
    ...
    event BRX_Removed(address indexed depositer, uint256 amount, uint256 timestamp);
    event BRXPurchased(address indexed purchaser, uint256 usdcAmount, uint256 cmtAllotedAmount);
...
/// @param _amount The amount of USDC to be removed from the vault
    function removePaymentTokens(uint256 _amount) public onlyAdmin {
...
/// @param _amount The amount of BRX to add to the Vault
    function supplyBRX (uint256 _amount) public onlyOwner {
        ...
        BRX.safeTransferFrom(tx.origin, address(this), _amount);
    }

/// @param _amount This is the amount of BRX to be removed by contract owner
    function removeBRX (uint256 _amount) public onlyAdmin {
        require(_amount > 0, "The amount must be greater than zero");
        require(brxTotal >= _amount, "There are not enough BRX tokens in the vault");
        brxTotal -= _amount;
        emit BRX_Removed(
            msg.sender,
            brxTotal,
        ...
        BRX.safeTransfer(tx.origin, _amount);
    }

/// @notice Checks the balance of BRX
    function balanceOfBRX() public view returns(uint256) {
        return brxTotal;
    }
...
/// @dev Allows only the deployer / supplier to use the function
    modifier onlyOwner() {
        require(tx.origin == owner, "This address is not the owner");
        _;
    }

/// @dev Allows only the swap contract to use this function
    modifier onlySwap() {
        require(msg.sender == swapAddress, "This address is not the swap contract");
        _;
    }

/**
    @param _user Address purchasing BRX
    @param _usdcAmount Amount of USDC being swapped for BRX
    @param _brxAmount Requested amount of BRX being purchased
*/
    
    function buyBRX(address _user, uint256 _usdcAmount, uint256 _brxAmount) public onlySwap nonReentrant {
        require (_user != address(0), "Invalid address");
        if (_usdcAmount <= 0) { revert NoTokensDepositError(); }
        /// @notice Penalty amount for BRX purchase 
        uint256 unrefundableAmount = _usdcAmount * penalty / 10000;
        uint256 storedAmount = _usdcAmount - unrefundableAmount;
        /// @dev USDC and BRX token balances for each user who interact with the Vault
        usdcTokenBalance[_user] += storedAmount;
        brxTokenBalance[_user] += _brxAmount;
        ...
        /// @dev Penalty portion of payment sent to fundsAddress upon BRX purchase
        ...
        /// @dev BRXPurchased event 
        emit BRXPurchased(_user, _usdcAmount, _brxAmount);
    }

/// @notice Allows an account to withdraw the BRX that was purchased
/// @param _user The buyers address
/// @param _amount The amount of BRX to send to the buyer
    function withdraw(address _user, uint256 _amount, uint256 _usdcAdjustment) public onlySwap {
        require(_amount <= brxTokenBalance[_user], "The amount is greater than your balance");
        require(_amount > 0, "The amount must be greater than zero");
        require(_amount <= brxTotal, "Please contact support"); 
        
        /// @dev Adjusts the balances of the users account
        brxTokenBalance[_user] -= _amount;
        usdcTokenBalance[_user] -= _usdcAdjustment;
        /// @dev Removes the BRX that was sent from the total amount in the contract
        brxTotal -= _amount;
        /// @dev Allows a BRX token withdraw event from the verified sender
        emit TokenWithdraw(
            msg.sender,
            'BRX',
            _amount
        );
        /// @dev Sends the BRX to the buyer who requested a withdraw
        BRX.safeTransfer(_user, _amount);
    }
...
/** 
    ...
    @param _brxAdjustment The amount of BRX to be removed from the buyers account
*/

    function refund(address _buyer, uint256 _amount, uint256 _brxAdjustment) public onlySwap {
        ...
        brxTokenBalance[_buyer] -= _brxAdjustment;
