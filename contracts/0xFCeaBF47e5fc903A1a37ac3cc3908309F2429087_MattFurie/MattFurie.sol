...
contract MattFurie is Context, IERC20, Ownable {
    ...
    bool public tradeEnable = true; // Торговля по умолчанию разрешена
    ...
    constructor () {
        uniswapV2Router = IUniswapV2Router02(0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D); // PCS BSC Mainnet Router
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (from != uniswapV2Pair && contractTokenBalance > SwapTokens && _Buys_In > 1) {
    ...
    function setMarketingWallet(address payable _newWallet) external onlyOwner {
        require(_newWallet != address(this), "Contract address cannot be the Fee Receiver");
        require(_newWallet != address(0), "Zero address cannot be the Fee Receiver");
