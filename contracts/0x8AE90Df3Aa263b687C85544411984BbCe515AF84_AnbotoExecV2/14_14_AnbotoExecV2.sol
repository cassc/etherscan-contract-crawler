 ...
 * @member maxFeeAbsolute Agreed max fee amount for order in output token.
...
contract AnbotoExecV2 is Ownable, EIP712 {
    ...
    bytes32 private constant ORDER_TYPEHASH =
        keccak256(
            "Order(address inputToken,uint256 totalAmount,address outputToken,uint256 outMin,uint256 maxGasPrice,uint256 maxFeeAbsolute,uint256 feePercent,uint256 deadline,uint256 salt)"
        );
    ...
    function executeOrder(
        ...
        outputToken.safeTransfer(
            _maker,
            swapOutputBalance - getFee(_order, sliceInputAmount, swapOutputBalance, tx.gasprice)
        );
    ...
    function settleCow(
        ...
        outputToken.safeTransfer(
            _maker,
            buyAmount - getCowFee(_order, sellAmount, buyAmount)
        );
    ...
    function getFee(
        ...
        if (_order.maxFeeAbsolute > 0) {
            ...
            // slice as compared to the total order. This amount is multiplied by ratio of tx gas price to max gas price
            // so we could take the fee to cover transaction cost
            feeAmount += (_gasPrice * _order.maxFeeAbsolute * _soldAmount) / _order.maxGasPrice / _order.totalAmount;
        }
        if (_order.feePercent > 0){
            // Take percent fee when absolute fee is not specified.
            // Percent fees are percentage of the bought amount.
            feeAmount += (_boughtAmount * _order.feePercent) / FULL_PERCENT;
        }
        if (feeAmount > _boughtAmount) {
            revert("AnbotoExecV2::getFee: Fee larger than swapped amount.");
        }
    }
    ...
    function getCowFee(
        ...
        if (feeAmount > _boughtAmount) {
            revert("AnbotoExecV2::getCowFee: Fee larger than swapped amount.");
        }
    }
