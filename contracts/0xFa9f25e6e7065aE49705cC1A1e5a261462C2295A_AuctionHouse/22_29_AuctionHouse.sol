    ...
    Ownable
{
    ...
    function setVariablesBatch3(
        address _payee,
        address _admin
    ) public onlyOwner onlyOnce(3) {
        payee = payable(_payee);
    ...
    function allowlistMint(uint256 _qty, bytes memory _signature) payable external onlyUnsanctioned(msg.sender) {
        _allowlistMint(_qty, msg.sender, _signature);
    }

    function allowlistMintDelegated(uint256 _qty, address _vault) payable external onlyUnsanctioned(msg.sender) {
        require(
            IDelegationRegistry(0x00000000000076A84feF008CDAbe6409d2FE638B)
                .checkDelegateForContract(msg.sender, _vault, address(oasis))
        );
