...
contract TornadoStakingRewards is Initializable, EnsResolve {
    ...
    /**
     * @notice Approximately 1e25, but not exactly
     */
    ...
    /**
     * @notice The TORN token contract
     */
    IERC20 public immutable torn;

    /**
     * @notice The contract authorized to call burn rewards (besides Governance)
     */
    ...
    constructor(address _governanceAddress, address _tornAddress, address _burner) public {
        ratioConstant = IERC20(_tornAddress).totalSupply();
        governance = ITornadoGovernance(_governanceAddress);
        torn = IERC20(_tornAddress);
        burner = _burner;
    }

    modifier onlyGovernance() {
        require(msg.sender == address(governance), "TornadoStakingRewards: only governance");
        _;
    }
    ...
    function addBurnRewards(uint256 amount) external {
        require(
            msg.sender == burner || msg.sender == address(governance), // Lazy eval
            "TornadoStakingRewards: only addrs authed for burning"
        );
