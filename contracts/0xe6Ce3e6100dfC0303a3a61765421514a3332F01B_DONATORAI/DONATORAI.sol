...
Twitter: https://twitter.com/Donator_AI
Telegram: https://t.me/donatoraiportal
...
contract DONATORAI is BEP20, Ownable {
    ...
    uint256 public tokenLiquidityThreshold = 3e5 * 10**18; 

    uint256 public genesis_block;
    uint256 private deadline = 1;
   

    address public donatorWallet = 0xAf127F4e6EF2a137D450Cdb327e7B93417EdE789;
    address public marketingWallet = 0x6f8CFF25bFc09FC095C5Ce3867b8ff06816340d4;
    ...
    constructor() BEP20("DONATOR AI", "DONATOR") {
    ...
    function _transfer(
       ...
       if (!exemptFee[sender] && !exemptFee[recipient]) {
        ...
        if((block.number <= (genesis_block + deadline) && sender == pair)){
        ...
        if (_interlock || exemptFee[sender] || exemptFee[recipient])
            ...
            feeswap = sellTaxes.liquidity + sellTaxes.marketing + sellTaxes.donator;
            feesum = feeswap;
            currentTaxes = sellTaxes;
        } else if (!useLaunchFee) {
            feeswap = taxes.liquidity + taxes.marketing + taxes.donator;
            feesum = feeswap;
            currentTaxes = taxes;
        } 
    ...
    function Liquify(uint256 feeswap, Taxes memory swapTaxes) private lockTheSwap {
        ...
        if (contractBalance >= tokenLiquidityThreshold) {
            ...
            if (ethToAddLiquidityWith > 0) {
             ...
             uint256 donatorWalletAmt = unitBalance * 2 * swapTaxes.donator;
             if (donatorWalletAmt > 0) {
                 payable(donatorWallet).sendValue(donatorWalletAmt);
        }
    ...
    function SetBuyTaxes(uint256 _marketing, uint256 _liquidity, uint256 _donator) external onlyOwner {
        taxes = Taxes(_marketing, _liquidity, _donator);
        require((_marketing + _liquidity + _donator) <= 20,"Must keep fees at 20% or less");
    }

    function SetSellTaxes(uint256 _marketing, uint256 _liquidity, uint256 _donator) external onlyOwner {
        sellTaxes = Taxes(_marketing, _liquidity, _donator);
        require((_marketing + _liquidity + _donator) <= 50,"Must keep fees at 50% or less");
    }
   ...
   function updatedonatorWallet(address newWallet) external onlyOwner {
        require(newWallet != address(0), "Fee Address cannot be zero address");
        donatorWallet = newWallet;
    }
