...
contract PxnRaffleDraw is Ownable  {
   event AddressDrawn(address indexed drawnAddress, address[] inputAddresses);

    function raffleDraw(address[] memory addresses) external onlyOwner {
        ...
        address drawnAddress = addresses[randomIndex];

        emit AddressDrawn(drawnAddress, addresses);
    }
