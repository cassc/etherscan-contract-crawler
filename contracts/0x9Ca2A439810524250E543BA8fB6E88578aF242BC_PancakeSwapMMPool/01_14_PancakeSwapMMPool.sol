...
contract PancakeSwapMMPool is EIP712, ReentrancyGuard, Ownable {
    ...
    constructor(address weth) EIP712("PCS MM Pool", "1") {
    ...
    function swap(
        ...
        if (_quote.baseToken == address(0) || _quote.baseToken == ETH_ADDRESS) {
            require(msg.value >= _quote.baseTokenAmount, "not enough amount");
            IWETH9(WETH_ADDRESS).safeDeposit(_quote.baseTokenAmount);
            IWETH9(WETH_ADDRESS).safeTransfer(mmInfo.treasury, _quote.baseTokenAmount);
        ...
        if (_quote.quoteToken == address(0) || _quote.quoteToken == ETH_ADDRESS) {
            IWETH9(WETH_ADDRESS).safeTransferFrom(mmInfo.treasury, address(this), _quote.quoteTokenAmount);
            IWETH9(WETH_ADDRESS).safeWithdraw(_quote.quoteTokenAmount);
    ...
    function redeemToken(address _token) external onlyOwner {
        if (_token == address(0) || _token == ETH_ADDRESS) {
