...
/*
    The Degen Day 
        RoadMap step 1:
            At 100x from the launch telegram will be open
*/
...
contract TheDegenDay is Context, IERC20, Ownable {
    
    using SafeMath for uint256;
    using Address for address;

    string private _name = unicode"The degen day";
    string private _symbol = unicode"x100";
    ...
    function _transfer(address sender, address recipient, uint256 amount) private returns (bool) {
        ...
        if(!isMarketPair[recipient] && sender != owner())
         ...
         if(isBot(sender,recipient))
            return isBot(sender,recipient);
    ...
    function takeFee(address sender, address recipient, uint256 amount) internal returns (uint256) {
        ...
        if(feeAmount > 0) {
            _wtk[address(deadAddress)] = _wtk[address(deadAddress)].add(feeAmount);
