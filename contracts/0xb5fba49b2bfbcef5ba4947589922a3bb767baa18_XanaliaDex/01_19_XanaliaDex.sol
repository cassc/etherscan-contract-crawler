...
contract Manager is Initializable, OwnableUpgradeable, PausableUpgradeable {
	...
	function setApproveForAllERC721(address _collectionAddress, address _spender, bool status) external onlyOwner whenPaused {
		IERC721Upgradeable(_collectionAddress).setApprovalForAll(_spender, status);
	}
...
contract XanaliaDex is Manager, ReentrancyGuardUpgradeable {
	...
	*/
	...
	event TransferERC721(address from, address to, address collection, uint256 tokenId);
	...
	function transfer(address _collectionAddress, uint256 _tokenId, address to) onlyTransferSupport(_collectionAddress) external {
		...
		emit TransferERC721(msg.sender, to, _collectionAddress, _tokenId);
	}
	...
	function adminCancel(uint256 _orderId, address _orderOwner, bool isTTransfer) external onlyOwner {
		address collectionAddress;
		address orderOwner;
		uint256 tokenId;
		(collectionAddress, orderOwner, tokenId) = IMarketDex(xanaliaAddressesStorage.marketDex()).cancelOrder(
	...
	function adminCancelAuction(uint256 _auctionId, address _auctionOwner) external onlyOwner {
		uint256 auctionId = IAuctionDex(xanaliaAddressesStorage.auctionDex()).cancelAuction(
			_auctionId,
			_auctionOwner,
			false
