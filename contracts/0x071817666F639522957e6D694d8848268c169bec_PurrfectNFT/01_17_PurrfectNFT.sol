 ...
 * - Royalties
 */
contract PurrfectNFT is OwnableUpgradeable, ReentrancyGuardUpgradeable, ERC721EnumerableUpgradeable, ERC721URIStorageUpgradeable {
    /**
    ...
    constructor(
        ...
        __ERC721_init("PurrfectNFT", "PNFT");
        __Ownable_init();
    ...
    // Minting vars
    uint256 constant public MINTS_PER_OWNER = 9;
    uint256 constant public MINTS_TOTAL = 9999;
    ...
    function setWhitelistRootHash(bytes32 rootHash) external onlyOwner {
        require(
            MINT_STATE != MintState.Public,
            "The whitelist root hash cannot be changed during the Public phase."
        );
        _rootHash = rootHash;
    }
        ...
        MintPhase mintPhase)
    {
        if (MINT_STATE == MintState.Waiting) {
            return (totalSupply(), 0, MintPhase.Waiting);
        }

        if (MINT_STATE == MintState.Whitelist) {
            return (totalSupply(), MINTS_TOTAL, MintPhase.Whitelist);
        }

        if (totalSupply() >= MINTS_TOTAL) {
            return (totalSupply(), MINTS_TOTAL, MintPhase.SoldOut);
        }

        return (totalSupply(), MINTS_TOTAL, MintPhase.Public);
    }
