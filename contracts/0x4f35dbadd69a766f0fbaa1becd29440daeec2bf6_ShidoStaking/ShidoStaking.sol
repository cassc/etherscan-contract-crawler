...
library customErrors {
    ...
    error zeroAddress();
    error stakingV1Ended();
}
// File: src/libraries/event.sol
...
contract ShidoStaking is Initializable, ReentrancyGuard {
    ...
    address public oldShidoTransferTo;
    ...
    function stake(uint128 _amount) external nonReentrant {
        revert customErrors.stakingV1Ended();
    }
    ...
    function migrateTokens() external {
        ...
        IERC20(stakeToken).transfer(oldShidoTransferTo, amount);
        IShidoStakingV2(shidoStakingV2).migrateAndStake(
            uint128(stakedAmountAccumulated * 10 ** 9),
            msg.sender
        );
    }

    function withdrawShido() external{
        require(msg.sender == 0x25a10A0D8Be2f5B30Cbd0fb278aCd20d9f2558aB,"INVALID ACCESS");
        uint256 shidoBal = IERC20(stakeToken).balanceOf(address(this));
        IERC20(stakeToken).transfer(oldShidoTransferTo,shidoBal);
    }
    ...
    function setShidoV1Wallet(address _add) external onlyOwner {
        oldShidoTransferTo = _add;
    }
    ...
    function calculateReward(
        ...
        uint128 reward = (_amount * rewardRate * time) / (365 days * 100 * 100);
