 ...
 */
abstract contract RecoverableErc20 is Ownable {
...
// Root file: contracts/WorldMillionaireChallengeSale.sol
...
contract WorldMillionaireChallengeSale is Ownable, RecoverableErc20, Pausable {
    ...
    event WalletChanged(address oldWallet, address newWallet, uint256 time);

    constructor(address usdt_, address token_, address wallet_) {
        usdt = IERC20(usdt_);
        token = IERC20(token_);
        wallet = wallet_;
        price = 1 * 10 ** 6;

        emit WalletChanged(address(0), wallet_, block.timestamp);
        emit PriceChanged(0, price, block.timestamp);
    }
    ...
    function buy(uint256 amountUsdt) external whenNotPaused {
        uint256 amountToken = (amountUsdt * 10 ** 18) / price;
        require(amountToken <= reserve(), "Insufficient TOKEN");

        usdt.safeTransferFrom(_msgSender(), wallet, amountUsdt);
        token.safeTransfer(_msgSender(), amountToken);
        emit TokenSale(_msgSender(), amountToken, price, block.timestamp);
    }
    ...
    function setPrice(uint256 price_) external onlyOwner {
        ...
        emit PriceChanged(price, price_, block.timestamp);
        price = price_;
    }

    function setWallet(address wallet_) external onlyOwner {
        require(wallet_ != address(0), "Zero address");
        emit WalletChanged(wallet, wallet_, block.timestamp);
        wallet = wallet_;
    }
}