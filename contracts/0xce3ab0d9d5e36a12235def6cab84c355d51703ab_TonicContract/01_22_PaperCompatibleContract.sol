    ...
    DefaultOperatorFilterer
{
    ...
    event SeederStatusChanged(address indexed _seeder, bool indexed _authStatus);
    ...
    ERC721 ("Interference by Phaust", "INTERFERENCE")
    {
        _name = ERC721. name () ;
    ...
    function publicMint(address _to, uint quantity)
        external
        payable
    {
    ...
    function setSeederStatus(address _wallet, bool _approvedStatus) external onlyOwner {
        approvedToSeed[_wallet] = _approvedStatus;
        emit SeederStatusChanged(_wallet, _approvedStatus);
    }
 
    function setSeeds(uint[] calldata tokenIds, string[] calldata seeds) external {
    ...
    function setProjectCode(string calldata _projectCode) external {
        require(approvedToSeed[msg.sender], "unauthorized");		
