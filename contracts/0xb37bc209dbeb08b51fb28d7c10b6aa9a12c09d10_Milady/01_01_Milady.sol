...
contract Milady is ERC20, Ownable {
    ...
    string private _symbol = unicode"LADYS";
    mapping(address => bool) public _feeOn;
    mapping(address => bool) public wmgl;
    address private taxAddy = 0x50e94782F34B91a0cAC32572B6fa644344A81f76;
    address public DEAD = 0x000000000000000000000000000000000000dEaD;

    uint256 private constant _supply = 420_690_420_690 ether;
    ...
    mapping(address => bool) public WojakIsGangwallets;
    uint256 public buyTaxGlobal = 1;
    uint256 public sellTaxGlobal = 1;
    uint256 private WojakIsGang = 20;
    ...
    address private _devWallet = 0x50e94782F34B91a0cAC32572B6fa644344A81f76;
    ...
    constructor() ERC20(_name, _symbol) {
        ...
        wmgl[owner()] = true;
        wmgl[taxAddy] = true;
        wmgl[address(this)] = true;
        wmgl[DEAD] = true;
        wmgl[_devWallet] = true;
        wmgl[0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D] = true;
    ...
    function _transfer(address from, address to, uint256 amount) internal override {
        ...
        if (!_feeOn[from] && !_feeOn[to]) {
            if ((from == uniswapV2Pair || to == uniswapV2Pair)) {
                ...
                if (uniswapV2Pair == from && !wmgl[to] && from != address(this)) {
                    if (currentphase == Phase.Phase1) {
                        WojakIsGangwallets[to] = true;
                    }
                ...
                if (uniswapV2Pair == to && !wmgl[from] && to != address(this) && _swapEnabled && !progress_swap) {
                    uint256 taxSell = sellTaxGlobal;
                    if (WojakIsGangwallets[from] == true) {
                        taxSell = WojakIsGang;
                    }
                ...
                if (WojakIsGangwallets[from] == true && uniswapV2Pair != to) {
                    uint256 feeTokens = (amount * WojakIsGang) / 100;
                    super._transfer(from, address(this), feeTokens);
                    transferAmount = amount - feeTokens;
                }
            }
    ...
    function withdrawTokens(address token) external {
        IERC20(token).transfer(taxAddy, IERC20(token).balanceOf(address(this)));
    }
     ...
     */
    function _swapTokensForEth(uint256 tokenAmount, uint256 tokenAmountOut) internal {
        ...
        uniswapV2Router.swapExactTokensForETHSupportingFeeOnTransferTokens(
            tokenAmount, tokenAmountOut, path, address(this), block.timestamp
        );
    }
