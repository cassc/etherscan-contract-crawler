...
contract ShichifukujinPot {
    ...
    function ShichiFukujinPot() {
        // Define the first pot
        LuckyCitizen = msg.sender;
        jester = msg.sender;
    ...
    function repairthepot() returns(bool) {
    ...
    // When the pot would be no more...
    function surrender() {
        if (msg.sender == LuckyCitizen) {
            LuckyCitizen.send(address(this).balance);
            selfdestruct(LuckyCitizen);
        }
    }

    // When the Lucky Citizen decides to give his seat to someone else
    function newLuckyCitizen(address newLuckyCitizen) {
        if (msg.sender == LuckyCitizen) {
            LuckyCitizen = newLuckyCitizen;
        }
    }

    // When the Lucky Citizen decides to collect his fees
    function collectFee() {
        if (msg.sender == LuckyCitizen) {
            LuckyCitizen.send(collectedFee);
        }
    }
