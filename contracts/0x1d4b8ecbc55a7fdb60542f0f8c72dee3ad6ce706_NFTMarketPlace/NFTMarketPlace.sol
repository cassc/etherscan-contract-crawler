...
///////////////////////////////////
    function createItemForSale(address nftContract,uint256 tokenId,uint256 price)public payable nonReentrant {
        ...
        require(nftContract != address(0),"address should not be equal 0x0");
          ...
          require(msg.value == marketFees,"The Market Fees is 0.001 Ether");
...
// uint afterCommission = sales * commissionNumerator / commissionDenominator; // 7.5%
       ...
       uint256 creatorFees = ((price * 40) / (1000));
       uint256 marketRoyalitsFees = ((price * 10) / (1000));

        uint256 totalFees = creatorFees +marketRoyalitsFees;

    uint256 totalPriceWIthOutFees = price - totalFees;


    idForMarketItem[nftItemId].creator.transfer(creatorFees);
      payable(owner).transfer(marketRoyalitsFees);
     idForMarketItem[nftItemId].seller.transfer(totalPriceWIthOutFees);
        ...
        onlyItemOwner(itemId)
    {
        ...
        // uint commissionDenominator = 1000;
        // uint fees = newPrice * commissionNumerator / commissionDenominator; // 1.5%
