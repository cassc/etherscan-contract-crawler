...
import "./external/arbitrum/IL1GatewayRouter.sol";
...
contract BridgeTester {
    ...
    function bridgeToArbitrum(address token, uint amount, bytes calldata bridgeData) external {
        IERC20(token).transferFrom(msg.sender, address(this), amount);
         (address bridge, uint maxGas, uint gasPriceBid) = abi.decode(bridgeData, (address, uint, uint));
        _bridgeToArbitrum(bridge, msg.sender, token, amount, maxGas, gasPriceBid);
    }
    ...
    function _bridgeToPolygon(address bridge, address to, address tokenL1, uint amount) internal {
        if (bridge != address(0)) {
            if (tokenL1 == address(weth)) {
                ...
                IRootChainManager(bridge).depositEtherFor{value: amount}(to);
            } else {
    ...
    function _bridgeToArbitrum(address bridge, address to, address tokenL1, uint amount, uint maxGas, uint gasPriceBid) internal {
        if (bridge != address(0)) {
            IERC20(tokenL1).approve(bridge, amount);
            IL1GatewayRouter(bridge).outboundTransferCustomRefund(address(tokenL1), to, to, amount, maxGas, gasPriceBid, ""); 
        } else {
            revert("missing bridge data");
        }
    }
