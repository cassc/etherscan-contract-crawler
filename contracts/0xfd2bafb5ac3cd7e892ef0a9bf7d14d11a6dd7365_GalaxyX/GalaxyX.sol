...
contract GalaxyX is ERC20, Ownable {
    ...
    event UpdateBuyFees(uint256 burnFeeOnBuy, uint256 marketingFeeOnBuy);
    event UpdateSellFees(uint256 burnFeeOnSell, uint256 marketingFeeOnSell);
    event UpdateWalletToWalletTransferFee(uint256 walletToWalletTransferFee);
    event SwapAndLiquify(uint256 tokensSwapped,uint256 bnbReceived,uint256 tokensIntoLiqudity);
    event SwapAndSendMarketing(uint256 tokensSwapped, uint256 bnbSend);
    event SwapTokensAtAmountUpdated(uint256 swapTokensAtAmount);

    constructor () ERC20("GalaxyX", "$GalaxyX") 
    {   
        address router;
        if (block.chainid == 56) {
        ...
        _totalFeesOnBuy    = burnFeeOnBuy  + marketingFeeOnBuy;
        _totalFeesOnSell   = burnFeeOnSell + marketingFeeOnSell;

        walletToWalletTransferFee = 1;

        marketingWallet = 0x5321f46a9743a552301bCEE908d07A1Ee19d52EC;
    ...
    function updateBuyFees(uint256 _burnFeeOnBuy, uint256 _marketingFeeOnBuy) external onlyOwner {
        ...
        emit UpdateBuyFees(burnFeeOnBuy, marketingFeeOnBuy);
    }

    function updateSellFees(uint256 _burnFeeOnSell, uint256 _marketingFeeOnSell) external onlyOwner {
        ...
        emit UpdateSellFees(burnFeeOnSell, marketingFeeOnSell);
    }
    ...
    function _transfer(address from,address to,uint256 amount) internal  override {
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to] || swapping) {
            ...
            if(block.number <= tradingBlock + 4){
                ...
                if (burnFeeOnBuy > 0){
                    uint256 burnAmount = amount * burnFeeOnBuy / 1000;
            ...
            if (burnFeeOnSell > 0){
                uint256 burnAmount = amount * burnFeeOnSell / 1000;
                super._transfer(from, address(0xdead), burnAmount);
                amount -= burnAmount;
            }
        } else {
        ...
        if (_totalFees > 0) {
            uint256 fees = (_amount * _totalFees) / 1000;
            amount = amount - fees;
