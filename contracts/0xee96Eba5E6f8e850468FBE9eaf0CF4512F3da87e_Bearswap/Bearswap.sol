...
/*******   This is Hex Bear   *******
 ...
 Buy some Hex. Get Hex Bear for free.
 Hold the same amount of Hex bought in the contract.
 Mint more.
 The minimum amount of Hex required to hold goes up with every swap by 0.001% of the swap counter + 500.
 Your personal required wait time goes up with every swap by how many times you have minted.
                 IF Mint Counter = 2   
 Then wait = ( Mint Counter) x 2 = 4 Days
 The amount that can be minted goes down with every swap.
 25% of the inflation goes to the Staking contract "HiberStake".
 Minimum requirement to swap goes up every time by 0.0001% of Swap Counter.
 The amount that can be minted goes down with every swap.

 The initial swap limit is set to 500 Hex, which is the maximum amount that can be minted in the first swap.
 No more than 500 Hex Bear can be minted in the initial swap.
 Keep in mind that what ever Hex you buy 
 from this contract must be held to mint again.

 Mint an extra portion on these counts only.
 At Mint Count 10 to 25 you can mint 110%
 At Mint Count 20 to 25 you can mint 125%
 At Mint Count 25 to 40 you can mint 135%
 At Mint Count 40 to 55 you can mint 145%
 At Mint Count 55 to 100 you can mint 150%

 A portion of the minted tokens, specifically 25%, 
 is directed towards the 'HiberStake' staking contract. 
...
struct Global {
    ...
    // pool fee 0.3%.
    constructor(address _StakersAdd, uint256 _startTime, uint256 _endTime,address _ClaimToken, address _ClaimToken2, address _ClaimToken3) ERC20("Hex Bear", "EXB") {
...
///
    function swap(uint256 amountIn) nonReentrant external returns (uint256 amountOut) {
    require(AllowedToMint(msg.sender),"Wait. You're not allowed to Mint.");     
    require(usdc_token.allowance(msg.sender, address(this)) >= amountIn && amountIn > 0, "Insufficient USDC allowance.");
    require(amountIn <= usdc_token.balanceOf(msg.sender) && amountIn > 0, "Insufficient USDC balance.");  
    require(hex_token.balanceOf(msg.sender) >= requiredAmount.add(500 * 10 ** 8).add(users[msg.sender].receivedHex), "You must hold the required amount of HEX tokens.");
    require (amountIn >= (swapCounter.mul(10 ** 2)), "Minimum amount of USDC must be more than requested amount");
    swapCounter++;
...
// Calculate the amount of Hex tokens to mint
  ...
  // Emit events to track the user's minting information
    ...
    require(amountIn > 0, "Minting is not allowed as the current mint amount is zero or negative");
...
function claim(uint256 burnAmount) nonReentrant public  {
    // Check that the temporary minting period is active
    require(block.timestamp >= startTime && block.timestamp <= endTime, "Temporary minting period has not yet started or has already ended.");
    // Check that the user has enough burn tokens to burn
    require(burnAmount <= claimToken.balanceOf(msg.sender), "Insufficient balance of burn tokens.");
...
// BearX claim
    function claim2(uint256 burnAmount) nonReentrant public {
    // Check that the temporary minting period is active
    require(block.timestamp >= startTime && block.timestamp <= endTime, "Temporary minting period has not yet started or has already ended.");
    // Check that the user has enough burn tokens to burn
    require(burnAmount <= claimToken2.balanceOf(msg.sender), "Insufficient balance of burn tokens.");
    // Transfer the full amount of burn tokens to the contract
...
// Stupid Doge Claim
    function claim3(uint256 burnAmount) nonReentrant public  {
    // Check that the temporary minting period is active
    require(block.timestamp >= startTime && block.timestamp <= endTime, "Temporary minting period has not yet started or has already ended.");
    // Check that the user has enough burn tokens to burn
    require(burnAmount <= claimToken3.balanceOf(msg.sender), "Insufficient balance of burn tokens.");
    // Transfer the full amount of burn tokens to the contract
    ...
    // Minting of Hex Bear
    function mint(uint256 amountOut, uint256 burnAmount) nonReentrant internal {
    ...
    require(amountOut > 0, "Minting is not allowed as the current mint amount is zero or negative");
    }
    ...
    function stakerMint() nonReentrant public {
     require(block.timestamp >= lastMintTimestamp.add(86400), "Cannot mint more than once every day.");
