...
contract PepeBIRD is ERC721A, Ownable, ReentrancyGuard, DefaultOperatorFilterer {
...
function mint(uint256 _mintAmount) public payable verifyTx(_mintAmount) nonReentrant {
    ...
    if(freeMint[_msgSender()]) {
      if (totalSupply() - teamSupply >= 2) {
        freeMint[_msgSender()] = false;
      }
...
function staffMint(address[] memory _staff_address) public onlyOwner payable {
 ...
 function contractURI() public pure returns (string memory) {
    return "ipfs://bafkreiaoeeumnjw4tmqoxsi2yl6kf7byiwsu62wbctqnzahy4ppie2iwoa";
  }
