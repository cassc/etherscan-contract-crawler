...
contract DunePass is ERC721A, Ownable, ReentrancyGuard {
  ...
  constructor(string memory _blindboxURI) ERC721A("Dune Pass", unicode"☉"){
  ...
  function mint(uint256 amt) external payable
  {
    ...
    require(totalSupply() + amt < maxSupply + 1,"No more Dune Passes");
    if(totalSupply() + amt <= totalFree){
        _safeMint(msg.sender, amt);
    } else {
        require(saleEnabled, "Selling is not live yet.");
        uint cost = price;
  ...
  function toggleSelling() external onlyOwner {
  ...
  function setBlindboxURI(string calldata _uri) external onlyOwner {
    blindboxURI = _uri;
  }
