...
contract RebelRacoon is ERC721AQueryable, Ownable, ReentrancyGuard {

  using Strings for uint256;

  bytes32 public merkleRoot;
  mapping(address => uint256) public racoonMinted;
  ...
  constructor(
    ...
    uint256 _maxMintForTeam,
    string memory _hiddenMetadataUri
  ) ERC721A(_tokenName, _tokenSymbol) {
    setCost(_cost);
    maxSupply = _maxSupply;
    maxMintForTeam = _maxMintForTeam;
  ...
  modifier mintCompliance(uint256 _mintAmount) {
    ...
    require(racoonMinted[_msgSender()] + _mintAmount <= maxMintAmountPerWallet, "You owned 2 already F*ker!");
    _;
  }
  ...
  function whitelistMint(uint256 _mintAmount, bytes32[] calldata _merkleProof) external payable mintCompliance(_mintAmount) mintPriceCompliance(_mintAmount) {
    ...
    racoonMinted[_msgSender()] += _mintAmount;
 ...
 function internalMint(uint256 _teamAmount) external onlyOwner  {
    ...
    require(_teamAmount > 0 && _teamAmount <= maxMintForTeam, "Please input lesser or equal to 24!");
    require(racoonMinted[_msgSender()] + _teamAmount <= maxMintForTeam, "We are cleared that we only mint 24 for team!");
    racoonMinted[_msgSender()] += _teamAmount;
    _mint(_msgSender(), _teamAmount);
  }
