...
contract VotingEscrow is Ownable, ReentrancyGuard {
    ...
    function bpt_amount_to_ush_units(uint256 amount) internal view returns (uint256) {
        ...
        (uint256 ush_balance, , , ) = IBPTVault(balancer_vault).getPoolTokenInfo(pool_id, tokenB);
    ...
    function weighted_amount(uint256 bptAmount, uint256 pool2Amount, uint256 tokenAmount) public view returns (uint256) {
        ...
        return (scaled_bpt_amount * bpt_boost_weight) / 1e18 + (scaled_pool2_amount * bpt_boost_weight) + tokenAmount;
    }
    ...
    function _deposit_for(
        ...
        if (_valueA != 0) {
            IERC20(tokenA).safeTransferFrom(_addr, address(this), _valueA);
        }

        if (_valueB != 0) {
            IERC20(tokenB).safeTransferFrom(_addr, address(this), _valueB);
        }

        if (_valueC != 0) {
            IERC20(tokenB).safeTransferFrom(_addr, address(this), _valueC);
        }
