...
contract AsagiTama is ERC721Enumerable,ERC2981,Ownable,Pausable,ReentrancyGuard{
  ...
  function firstMint(bytes32[] calldata _merkleProof) public nonReentrant {
  ...
  function stake() public{
    require(stakeStart);
    dailyPointMap[msg.sender]=dailyPointMap[msg.sender]+checkStakePoint(msg.sender);
    uint256[] memory item = at_Contract.walletOfOwner(msg.sender);
    for (uint256 i; i < item.length; i++) {
      uint256 id = item[i];
      timeStampMap[id]=block.timestamp;
      stampOwnerMap[id]=msg.sender;
    }
  }
  ...
  function checkStakePoint(address _address) public view returns (uint256) {
      uint256 point = 0;
      for (uint256 i; i < at_Contract.walletOfOwner(_address).length; i++) {
        if (stampOwnerMap[at_Contract.walletOfOwner(_address)[i]] == _address){
          point=point+(block.timestamp-timeStampMap[at_Contract.walletOfOwner(_address)[i]]);
        }
      }
    return point;
  }
