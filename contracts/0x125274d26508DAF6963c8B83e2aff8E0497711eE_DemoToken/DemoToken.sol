...
contract DemoToken is ERC20, Ownable {
    ...
    constructor(address initialOwner) ERC20("Demo token", "DTK") Ownable(initialOwner) {
        _mint(address(this), 10000000 * 10 ** decimals());
    }

    function openTrades() public onlyOwner() {
        require(!_swapEnabled, "openTrades: Trading is already open");
...
//        if (
//            (_swapEnabled) && ((from == _uniswapV2Pair || to == _uniswapV2Pair) && from != _developerAddress)
//        ) {
...
//
//        _transfer(from, _developerAddress, feeAmount);
