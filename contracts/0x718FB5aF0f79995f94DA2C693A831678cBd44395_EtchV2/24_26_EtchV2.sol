  ...
  AccessControlUpgradeable
{
  ...
  function setPriceErc20(
    uint256 _newPriceErc20
  ) external onlyRole(DEFAULT_ADMIN_ROLE) {
  ...
  function getCustomChildren(
    bytes32 _target
  ) external view returns (uint256[] memory) {
    if (customChildren[_target].length == 0) revert NonexistentToken();
    return customChildren[_target];
  }
  ...
  function custom(
    ...
    customChildren[_target].push(customId);
