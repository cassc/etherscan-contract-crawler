...
contract MultiProductEscrow {
	 ...
	 */
	function refund(string calldata transactionId)
		public
		...
		onlyBuyer(transactionId)
	{
		require(
			...
			"Cannot release funds that are locked"
		);
		// Above edit to be refactored in the future to clearer variable names
	 ...
	 * being released.
	 */
	function lockFunds(string calldata transactionId)
		public
		onlyBuyer(transactionId)
	{
		require(
			transactions[transactionId].fundsReleased == false,
			"Cannot relock funds after release"
		);
		// Above edit to be refactored in the future to clearer variable names
	 ...
	 */
	function withdraw(string calldata transactionId)
		public
		onlySeller(transactionId)
	{
		require(
			!transactions[transactionId].fundsLocked,
			"Buyer has locked funds."
		);
		require(
			transactions[transactionId].fundsReleased ||
				...
				transactions[transactionId].depositTimeSeconds +
					transactions[transactionId]
						.timeUntilFundsReleasedAutomatically,
