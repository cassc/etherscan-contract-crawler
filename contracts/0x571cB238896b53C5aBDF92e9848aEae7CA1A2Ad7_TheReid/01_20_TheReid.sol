 ...
 * @notice This contract handles minting The Reid tokens.
 */
contract TheReid is
    ERC1155Supply,
    Ownable,
    ...
    RevokableOperatorFilterer
{
    ...
    address private presaleSignerAddress =
        0xbB61C196b5b347fa3f50326d70b34cA6fB655634;
    // Address where HeyMint fees are sent
    address public heymintPayoutAddress =
        0xE1FaC470dE8dE91c66778eaa155C64c7ceEFc851;
    address public royaltyAddress = 0x86Fa38c1910008D295D548045D364640fc110516;
    address[] public payoutAddresses = [
        0x86Fa38c1910008D295D548045D364640fc110516
    ];
    ...
    mapping(uint256 => address) public tokenFreeClaimContractAddress;
    mapping(uint256 => bool) public tokenFreeClaimActive;
    ...
    // If true, token has already been used to claim for the tokenId specified on this contract and cannot be used again
    mapping(uint256 => mapping(uint256 => bool)) public tokenFreeClaimUsed;
    ...
    mapping(uint256 => uint256) public tokenMintsPerClaim;
    ...
    string public name = "The Reid";
    string public symbol = "REID";
    ...
    constructor(uint256 _heymintFeePerToken)
        ERC1155(
            "ipfs://bafybeih2oi3v4rmf3morjdqmpi356w26qv57gu3jl4twbsnbw2oxakmnu4/{id}"
        )
        RevokableOperatorFilterer(
        ...
        )
    {
        ...
        tokenMaxSupply[1] = 504;
        tokenPublicPrice[1] = 0 ether;
        tokenPublicMintsPerAddress[1] = 1;
        tokenPublicSaleStartTime[1] = 1683208800;
        tokenPublicSaleEndTime[1] = 1998529171;
        tokenUsePublicSaleTimes[1] = true;
        tokenFreeClaimContractAddress[
            1
        ] = 0xB26F5E241b03F5ab00684D17968dfF6A18544a37;
        tokenMintsPerClaim[1] = 1;
        require(
     ...
     */
    function setTokenFreeClaimState(
        uint256 _tokenId,
        bool _tokenFreeClaimActive
    ) external onlyOwner {
        require(
            tokenFreeClaimActive[_tokenId] != _tokenFreeClaimActive,
            "NEW_STATE_IDENTICAL_TO_OLD_STATE"
        );
        tokenFreeClaimActive[_tokenId] = _tokenFreeClaimActive;
    }
     ...
     */
    function updateTokenMintsPerClaim(
        uint256 _tokenId,
        uint256 _tokenMintsPerClaim
    ) external onlyOwner {
        tokenMintsPerClaim[_tokenId] = _tokenMintsPerClaim;
    }

    /**
     * @notice Update the contract address for the ERC721 NFT eligible for claiming free mints
     */
    function updateTokenFreeClaimContractAddress(
        uint256 _tokenId,
        address _tokenFreeClaimContractAddress
    ) external onlyOwner {
        tokenFreeClaimContractAddress[
            _tokenId
        ] = _tokenFreeClaimContractAddress;
    }

    /**
     * @notice Free claim token when msg.sender owns the tokens in the external contract
     */
    function freeClaim(uint256 _tokenId, uint256[] calldata _claimTokenIDs)
        external
        payable
        originalUser
        nonReentrant
    {
        require(tokenFreeClaimActive[_tokenId], "FREE_CLAIM_IS_NOT_ACTIVE");
        require(
            !tokenMintingPermanentlyDisabled[_tokenId],
            "MINTING_PERMANENTLY_DISABLED"
        );
        uint256 totalMints = _claimTokenIDs.length *
            tokenMintsPerClaim[_tokenId];
        require(
            tokenMaxSupply[_tokenId] == 0 ||
                totalSupply(_tokenId) + totalMints <= tokenMaxSupply[_tokenId],
            "MAX_SUPPLY_EXCEEDED"
        );
        require(
            tokenFreeClaimContractAddress[_tokenId] != address(0),
            "FREE_CLAIM_ADDRESS_NOT_SET"
        );
        require(_claimTokenIDs.length > 0, "NO_TOKEN_IDS_PROVIDED");
        uint256 heymintFee = totalMints * heymintFeePerToken;
        require(msg.value == heymintFee, "PAYMENT_INCORRECT");
        (bool success, ) = heymintPayoutAddress.call{value: heymintFee}("");
        require(success, "Transfer failed.");
        IERC721Basic ExternalERC721FreeClaimContract = IERC721Basic(
            tokenFreeClaimContractAddress[_tokenId]
        );
        for (uint256 i = 0; i < _claimTokenIDs.length; i++) {
            ...
            require(
                !tokenFreeClaimUsed[_tokenId][_claimTokenIDs[i]],
                "TOKEN_ALREADY_CLAIMED"
            );
            tokenFreeClaimUsed[_tokenId][_claimTokenIDs[i]] = true;
        }
        tokensMintedByAddress[msg.sender][_tokenId] += totalMints;
