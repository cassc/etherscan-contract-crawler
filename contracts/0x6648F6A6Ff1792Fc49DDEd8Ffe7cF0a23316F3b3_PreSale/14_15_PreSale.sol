...
contract PreSale is Validatable, ReentrancyGuardUpgradeable {
     ...
     * emit {Bought} events
     */
    function buy(uint256 id, uint256 times) external payable nonReentrant notZero(times) validPreSaleId(id) {
        ...
        IGenesis(preSaleInfo.collection).mintBatch(_msgSender(), times);
