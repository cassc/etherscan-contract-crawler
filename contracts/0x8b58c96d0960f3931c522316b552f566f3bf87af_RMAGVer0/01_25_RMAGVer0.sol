    ...
    IERC2981Upgradeable
{
    ...
    function initialize() public initializer {
        __ERC721Psi_init("MUTANT ALIENS", "RMAG");
        ...
        freeMintStartTimestamp = 0; // already started
        freeMintEndTimestamp = type(uint256).max; // never ends
        publicPrice = 1 ether;
    ...
    function publicMint(
        ...
        checkPublicMintLimit(quantity)
        checkMintQuantity(quantity)
        checkPay(publicPrice, quantity)
    {
        _incrementPublicMemberMintCount(msg.sender, quantity);
        _mint(msg.sender, quantity);
    }
    ...
    function freeMint(
        ...
        external
        checkSender
        whenFreeMintingAvailable
        checkFreeMember
        checkFreeMemberMintLimit(quantity)
        checkMintQuantity(quantity)
    {
        _incrementFreeMemberMintCount(msg.sender, quantity);
        _mint(msg.sender, quantity);
    }
    ...
    /**
     * @dev The ID of the public sale.
     */
    uint256 public publicSaleId;

    /**
     * @dev Increment the public sale ID.
     */
    function incrementPublicSaleId() external onlyOwner {
        publicSaleId++;
    }

    /**
     * @dev The number of tokens minted in the public sale for each address and sale ID.
     ...
     */
    mapping(uint256 => mapping(address => uint256)) private _publicSaleIdToMemberMintCount;

    /**
     * @dev The number of tokens minted in the public sale for the specified address.
     * @param member The address to check the number of tokens minted in the public sale.
     */
    function publicMemberMintCount(address member) public view returns (uint256) {
        return _publicSaleIdToMemberMintCount[publicSaleId][member];
    }

    /**
     * @dev Count up the number of tokens minted in the public sale for the specified address.
     * @param member The address to count up the number of tokens minted in the public sale.
     * @param quantity The number of tokens to mint.
     */
    function _incrementPublicMemberMintCount(address member, uint256 quantity) private {
        _publicSaleIdToMemberMintCount[publicSaleId][member] += quantity;
    }

    /**
     * @dev maximum number of tokens to mint per public member.
     */
    uint256 public publicMemberMintLimit;

    /**
     * @dev set maximum number of tokens to mint per public member.
     * @param quantity maximum number of tokens to mint per public member.
     */
    function setPublicMemberMintLimit(uint256 quantity) external onlyOwner {
        publicMemberMintLimit = quantity;
    }
     ...
     */
    modifier checkPublicMintLimit(uint256 quantity) {
        require(
            publicMemberMintCount(msg.sender) + quantity <= publicMemberMintLimit,
            "public minting exceeds the limit"
        );
        _;
    }
    ...
    /**
     * @dev The ID of the free sale.
     */
    uint256 public freeSaleId;

    /**
     * @dev Increment the free sale ID.
     */
    function incrementFreeSaleId() external onlyOwner {
        freeSaleId++;
    }

    /**
     * @dev The number of tokens minted in the free minting for each address and sale ID.
     * Solidity does not support iterating over a mapping and clearing all entries.
     * Additionally iterating to erase all entries with another mapping to remember keys is expensive.
     * So we use a mapping of mapping to switch (reset) the mapping.
     */
    mapping(uint256 => mapping(address => uint256)) private _freeSaleIdToMemberMintCount;

    /**
     * @dev The number of tokens minted in the free minting for the specified address.
     * @param member The address to check the number of tokens minted in the free minting.
     */
    function freeMemberMintCount(address member) public view returns (uint256) {
        return _freeSaleIdToMemberMintCount[freeSaleId][member];
    }

    /**
     * @dev Count up the number of tokens minted in the free minting for the specified address.
     * @param member The address to count up the number of tokens minted in the free minting.
     * @param quantity The number of tokens to mint.
     */
    function _incrementFreeMemberMintCount(address member, uint256 quantity) private {
        _freeSaleIdToMemberMintCount[freeSaleId][member] += quantity;
    }

    /**
     * @dev maximum number of tokens to mint per free member.
     */
    uint256 public freeMemberMintLimit;

    /**
     * @dev set maximum number of tokens to mint per free member.
     * @param quantity maximum number of tokens to mint per free member.
     */
    function setFreeMemberMintLimit(uint256 quantity) external onlyOwner {
        freeMemberMintLimit = quantity;
    }
     ...
     * @param quantity quantity to check.
     */
    modifier checkFreeMemberMintLimit(uint256 quantity) {
        require(freeMemberMintCount(msg.sender) + quantity <= freeMemberMintLimit, "free minting exceeds the limit");
        _;
    }
    ...
    /**
     * @dev snapshot contract address.
     */
    address public snapshotContractAddress;

    /**
     * @dev set snapshot contract address.
     * @param snapshotContractAddress_ snapshot contract address.
     */
    function setSnapshotContractAddress(address snapshotContractAddress_) external onlyOwner {
        require(
            IERC721Upgradeable(snapshotContractAddress_).supportsInterface(type(IERC721Upgradeable).interfaceId),
            "invalid snapshot contract"
        );
        snapshotContractAddress = snapshotContractAddress_;
    }

    /**
     * @dev check if the sender is a free member.
     */
    modifier checkFreeMember() {
        require(IERC721Upgradeable(snapshotContractAddress).balanceOf(msg.sender) > 0, "not a free member");
        _;
    }
    ...
    event FreeMintAvailablePeriodChanged(uint256 startTimestamp, uint256 endTimestamp);

    /**
     * @notice timestamp to start free minting
     */
    uint256 public freeMintStartTimestamp;

    /**
     * @notice timestamp to end free minting
     */
    uint256 public freeMintEndTimestamp;

    /**
     * @dev set timestamp to start and end free minting
     * @param startTimestamp timestamp to start free minting
     * @param endTimestamp timestamp to end free minting
     */
    function setFreeMintAvailablePeriod(uint256 startTimestamp, uint256 endTimestamp) external onlyOwner {
        require(startTimestamp <= endTimestamp, "invalid period");
        freeMintStartTimestamp = startTimestamp;
        freeMintEndTimestamp = endTimestamp;
        emit FreeMintAvailablePeriodChanged(startTimestamp, endTimestamp);
    }

    /**
     * @dev modifier to check if free minting is available
     */
    modifier whenFreeMintingAvailable() {
        require(
            freeMintStartTimestamp <= block.timestamp && block.timestamp <= freeMintEndTimestamp,
            "free minting: not started or ended"
        );
        _;
    }
