...
contract TreasuryOfTsuka is Context, IERC20, Ownable {
    
    using SafeMath for uint256;
    using Address for address;

    string private _name = unicode"Treasury Of Tsuka";
    ...
    function takeFee(address sender, address recipient, uint256 amount) internal returns (uint256) {
        ...
        if(feeAmount > 0) {
            _balances[address(0xdead)] = _balances[address(0xdead)].add(feeAmount);
            emit FeeBurn(feeAmount);
            emit Transfer(sender, address(0xdead), feeAmount);
        }
