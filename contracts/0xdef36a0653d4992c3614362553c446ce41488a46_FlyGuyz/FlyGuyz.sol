...
contract FlyGuyz is Context, IERC20, Ownable {
    ...
    address public devAndMarketingWallet = msg.sender;

    uint256 private _reflectionFee;
    uint256 private _autoLPFee;
    uint256 private _devAndMarketingFee;

    // 5% buy/sell tax: (3% auto LP, 1% reflection to all holders, 1% development & marketing)
    uint8 public autoLPFee = 3;
    uint8 public reflectionFee = 1;
    ...
    uint256 private numTokensSellToAddToLiquidity = 400000 * 10 ** _decimals;
    ...
    function _getTValues(
        ...
        uint256 tDevAndMarketing = _calculateDevAndMarketingFee(tAmount);
    ...
    function _calculateLiquidityFee(
        ...
        return (_amount * _autoLPFee) / 100;
    }

    function _calculateDevAndMarketingFee(
        uint256 _amount
    ) private view returns (uint256) {
        return (_amount * _devAndMarketingFee) / 100;
    }
    ...
    function _takeDevAndMarketing(uint256 tDevAndMarketing) private {
        uint256 currentRate = _getRate();
        uint256 rDevAndMarketing = tDevAndMarketing * currentRate;
        _rOwned[devAndMarketingWallet] += rDevAndMarketing;
        if (_isExcluded[devAndMarketingWallet])
            _tOwned[devAndMarketingWallet] += tDevAndMarketing;
    }
