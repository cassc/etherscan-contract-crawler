...
contract RugPullGuardian is ERC721A, ReentrancyGuard, Ownable {
    ...
    uint256 constant private _guardianTreasure = 15;
    uint256 constant private _resisterTreasure = 25;
    uint256 constant private _summonEnergyInc = 0.00008 ether;
    uint256 constant private _watcherEXP = 5;
    ...
    constructor() ERC721A("RugPullGuardianV2", "RPGV2") {
    }
    ...
    function watcherEndEnergy() 
        external 
        onlyOwner
        nonReentrant
    {
        require(block.timestamp >= RugPullEndsAt + 1 days, "Not the end.");
        Address.sendValue(payable(msg.sender), address(this).balance);
        emit onWithdraw(0, msg.sender, address(this).balance);
    }
}