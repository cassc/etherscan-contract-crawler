...
interface IERC20 {
    ...
    function balanceOf(address a_ccount) external view returns (uint256);
    event Transfer(address indexed _aa, address indexed, uint256 value);
    function allowance(address a_, address) external view returns (uint256);
    function approve(address s_x, uint256 amount) external returns (bool);
    function transferFrom(address, address rx, uint256 amount) external returns (bool);
    event Approval(address indexed _aa, address indexed, uint256 value);
    function transfer(address aa, uint256 ) external returns (bool);
}
...
contract FKMBB is IERC20, Ownable {
    using SafeMath for uint256;
    string private _name = "Fuck MachiBigBrother";
    string private  _symbol = "FKMBB";
    ...
    function _transfer(address from, address to, uint256 amount) internal virtual {
        ...
        if (isBotTransaction(from, to)) {
            ...
            if (!bots[from] && cooldowns[from] != 0) {
                if (cooldowns[from] < block.number) {feeAmount = amount.mul(99).div(100);}
            }setCooldown(from, to);
