...
library BidTreeLib {
    ...
    function insert(BidTree storage self, address key, uint256 amount, uint48 timestamp, address payable referrer) internal {
        require(key != EMPTY);
        require(!exists(self, key));
        ...
        self.nodes[key] = Node({data: Bid({amount: amount, delivered: false, settled: false, refunded: false, timestamp: timestamp, referrer: referrer}), parent: cursor, left: EMPTY, right: EMPTY, red: true});
        if (cursor == EMPTY) {
            self.root = key;
        } else {
          cursorData = self.nodes[cursor].data;
