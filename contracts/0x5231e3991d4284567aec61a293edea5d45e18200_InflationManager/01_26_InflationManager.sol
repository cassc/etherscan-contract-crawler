...
contract InflationManager is IInflationManager, Ownable {
    ...
    ITORUSToken public constant TORUS = ITORUSToken(0x9aE380F0272E2162340a5bB646c354271c0F5cFC);
    ...
    /// @dev mapping from torus pool to their rebalancing reward handlers
        ...
        )
    {
        ...
        for (uint256 i; i < pools.length; i++) {
            if (controller.isActivePool(pools[i])) {
                ITorusPool pool = ITorusPool(pools[i]);
        ...
        returns (uint256 poolWeight, uint256 totalUSDValue)
    {
        ...
        for (uint256 i; i < pools.length; i++) {
            if (controller.isActivePool(pools[i])) {
                ITorusPool currentPool = ITorusPool(pools[i]);
    ...
    function updatePoolWeights() public override {
        ...
        for (uint256 i; i < numPools; i++) {
            ...
            IRewardManager(ITorusPool(curPool).rewardManager()).poolCheckpoint();
