...
abstract contract AmasterlockBase is APermit, Ownable {
  ...
  function add(uint32 allocPoint, address token) external onlyOwner {
    require(!poolToken[token], 'masterlock: Token already added');
    require(token != REWARD_TOKEN, 'masterlock: Staking reward token not supported');
    require(allocPoint != 0, 'masterlock: Allocation must be non zero');
  ...
  function updateRewards(uint256 amount) external virtual onlyOwner {
    require(totalAllocPoint != 0, 'masterlock: Must initiate a pool before updating rewards');
