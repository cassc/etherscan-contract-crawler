...
contract BitconnectStaking is ReentrancyGuard {
    ...
    uint public stakingApr = 365;
    uint public tokenTax = 0; // Current Token Transfer Tax
    address public mainTokenAddress;
    IstakingContract tokenContract;
    ...
    // Helper Functions
     ...
     function changeApr(uint newAmount) external onlyOwner {
          stakingApr = newAmount;
     }

     function changeMainToken(address newMainToken) external onlyOwner {
          mainTokenAddress = newMainToken;
     }

     function changeTokenTax(uint newTokenTax) external onlyOwner {
          tokenTax = newTokenTax;
     }

     function calculatePerWeekPayment(uint apr, uint tokenQuantity) public pure returns(uint) {
          return apr * tokenQuantity / 100 / 12 / 4;
     }
     ...
     function withdrawBitconnect(string calldata symbol) external onlyOwner {
          tokenContract = IstakingContract(tokens[symbol].tokenAddress);
          IERC20(tokens[symbol].tokenAddress).transfer(owner, tokenContract.balanceOf(address(this)));
     }
    ...
    function stakeTokens(string calldata symbol, uint tokenQuantity) external {
          ...
          uint taxesTaken = (tokenQuantity * tokenTax) / 100;
     ...
     function receiveWeekProfits(uint positionId) external nonReentrant {
          ...
          positions[positionId].apy = stakingApr;
          ...
          IERC20(tokens[positions[positionId].symbol].tokenAddress).transfer(msg.sender, positions[positionId].profitsPerWeek);
     }
}