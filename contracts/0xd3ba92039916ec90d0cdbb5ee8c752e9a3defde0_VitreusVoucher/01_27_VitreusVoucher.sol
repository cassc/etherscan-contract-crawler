...
import {IAffiliatePayoutRegistry} from "./interfaces/IAffiliatePayoutRegistry.sol";
    ...
    ReentrancyGuardUpgradeable
{
    ...
    IAffiliatePayoutRegistry private _payoutRegistry;

    // EIP-712 domain separator
    bytes32 public domainSeparator;

    // Affiliate signature type hash
    bytes32 internal constant AFFILIATE_TYPE_HASH = keccak256("Affiliate(address wallet)");
    ...
    /**
     * @dev Contract reinitialization
     *
     * @param payoutRegistry_ payout registry address
     * @param proverAddress_ the address which signed the EIP712 signatures
     */
    function initializeV2(address payoutRegistry_, address proverAddress_) external reinitializer(2) {
        require(payoutRegistry_ != address(0), "Zero payout registry address");
        require(proverAddress_ != address(0), "Zero prover address");
        require(
            ERC165Checker.supportsInterface(payoutRegistry_, type(IAffiliatePayoutRegistry).interfaceId),
            "Incorrect payout registry"
        );
        ...
        _payoutRegistry = IAffiliatePayoutRegistry(payoutRegistry_);
    ...
    receive() external payable whenNotPaused onlyNotUSDCReceiver {
        ...
        if (balanceOf(msg.sender) > 0) {
            _deposit(_tokenOwners[msg.sender], _swapETHForUSDC(msg.value), address(0));
        } else {
    ...
    /**
     * @dev Mints the new voucher to the specified address automatically swapping ETH to USDC
     * and saves affiliate to Affiliate Payout Registry
     *
     * @param affiliateSignature The EIP712 signature
     * @param affiliateWallet The affiliate's address
     */
    function affiliateMint(
        ...
        require(msg.value != 0, "Value must not be zero");
        require(affiliateSignature.length != 0, "Empty signature");

        require(verifySignature(affiliateWallet, affiliateSignature), "Incorrect prover");

        _mint(msg.sender, _nextTokenId(), _swapETHForUSDC(msg.value), affiliateWallet);
    }
    ...
    function affiliateDeposit(
        ...
        _deposit(_tokenOwners[msg.sender], _swapETHForUSDC(msg.value), affiliateWallet);
    }
    ...
    function mintWithUSDC(address to_, uint96 value_) external whenNotPaused onlyNotUSDCReceiver {
        ...
        _mint(to_, _nextTokenId(), value_, address(0));
     ...
     * @param affiliateWallet The affiliate's address
     */
    function affiliateMintWithUSDC(
        address to_,
        uint96 value_,
        bytes calldata affiliateSignature,
        address affiliateWallet
    ...
    function depositUSDC(uint256 amount_) external whenNotPaused onlyNotUSDCReceiver {
        ...
        _deposit(_tokenOwners[msg.sender], amount_, address(0));
    ...
    /**
     * @dev Updates the payout registry contract
     *
     * @param payoutRegistry_ new payout registry contract
     */
    function setPayoutRegistry(address payoutRegistry_) external onlyOwner {
        require(payoutRegistry_ != address(0), "Zero payout registry address");
        require(
            ERC165Checker.supportsInterface(payoutRegistry_, type(IAffiliatePayoutRegistry).interfaceId),
            "Incorrect payout registry"
    ...
    /**
     * @notice Verifies the affiliate signature
     *
     * @param affiliate the affiliate wallet address
     * @param signature the affiliate signature
     *
    ...
    function verifySignature(address affiliate, bytes calldata signature) internal view returns (bool) {
        ...
        bytes32 digest = keccak256(
            abi.encodePacked("\x19\x01", domainSeparator, keccak256(abi.encode(AFFILIATE_TYPE_HASH, affiliate)))
        );

        return digest.recover(signature) == _proverAddress;
    }
     ...
     */
    function _mint(address to_, uint256 tokenId_, uint256 value_, address affiliateWallet) internal nonReentrant {
     ...
     */
    function _deposit(uint256 tokenId_, uint256 value_, address affiliateWallet) internal {
        ...
        if (affiliateWallet != address(0)) {
            _payoutRegistry.registerDeposit(affiliateWallet, _currentPresaleRoundNumber(), value_);
        }
    ...
    function _safeTransfer(address from, address to, uint256 tokenId, bytes memory data) internal virtual {
        _transfer(from, to, tokenId);
        require(_checkOnVoucherReceived(from, to, tokenId, data), "Transfer to non ERC721Receiver implementer");
    }
