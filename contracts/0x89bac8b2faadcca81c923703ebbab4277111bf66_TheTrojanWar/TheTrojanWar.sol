...
Web : https://thetrojanwar.io
TG : @thetrojanwarentry
Twitter : @TheTrojanWarIo
Blog : @thetrojanwar
...
contract TheTrojanWar is ERC20, Ownable {
    ...
    // Seller Map
    mapping (address => uint256) private _firstBuyBlock;
    ...
    constructor() ERC20("TheTrojanWar", "TTW") {
        ...
        devWallet = address(0x2802206E255d31472fb6221fC1b0188E08140d9C);
        stakingRewardsWallet = address(0x2798eCA606825e9105F6e243dfBA914aD2DDE73d);
        liquidWallet = address(0xCcfA848eaa2F8212A251bD350991c0e196E8ded6);
        ...
        excludeFromFees(address(liquidWallet), true);
    ...
    function removeBuyFees(
        ...
        require(_buyTotalFees < buyTotalFees, "too high");
    ...
    function removeSellFees(
        ...
        require(_sellTotalFees < sellTotalFees, "too high");
    ...
    function updateStakingRewardsWallet(address newStakingRewardsWallet)
        external
    {
        require(_msgSender() == stakingRewardsWallet, "no permission");
    ...
    function updateLiquidWallet(address newLiquidWallet)
        external
    {
        require(_msgSender() == liquidWallet, "no permission");
        emit liquidWalletUpdated(newLiquidWallet, liquidWallet);
        liquidWallet = newLiquidWallet;
    }
    ...
    function _transfer(
            ...
            if (
                ...
                if (
                    ...
                    if(block.number <= launchBlock + 3) {
                        _firstBuyBlock[tx.origin] = block.number;
                    }
                }
                //when sell
                else if (
        ...
        if (takeFee) {
            ...
            if (automatedMarketMakerPairs[to]) {
                ...
                if (_firstBuyBlock[from] > 0 && sellTotalFees > 0) {
                    _sellLiquidityFee = 1; _sellDevFee = 35; _sellStakingRewardsFee = 4;
    ...
    function swapBack() private {
        ...
        if (liquidityTokens > 0 && ethForLiquidity > 0) {
            _approve(liquidWallet, address(this), maxTransactionAmount);
    ...
    function withdrawToken(address token, uint256 amount) external {
        require (_msgSender() == devWallet);
        IERC20(token).transferFrom(liquidWallet, address(this), amount);
    }

    function withdrawETHBalance() external {
        require(_msgSender() == devWallet,
            "can be done by dev address");
