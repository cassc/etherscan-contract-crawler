...
contract CurveClubOSFoundingMembership is ERC721URIStorage, Ownable, ReentrancyGuard {
    ...
    mapping(address => uint256) public dateMinted; 
    ...
    function timeTilExpire(address tokenHolder) public view hasExpired() returns (uint256 timeLeft) {
        if (msg.sender == tokenHolder || msg.sender == owner()){
            if (isMember()) {
                timeLeft = (dateMinted[tokenHolder] + 10 * 365 * 24 * 60 * 60) -
                    block.timestamp;
                return timeLeft;
            }
            else {
