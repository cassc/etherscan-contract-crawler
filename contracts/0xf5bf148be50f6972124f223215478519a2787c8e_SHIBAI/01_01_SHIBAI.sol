...
contract SHIBAI is Context, IERC20, Ownable {
  ...
  uint256 private _totalSupply = 2 * 10 ** 9 * 10 ** decimals;

  string private name = "SHIBA AI";
  string private symbol = "SHIB AI";
  ...
  event AddLiquidity(uint256 tokenAmount, uint256 amountEth);
  ...
  address payable private treasuryWallet; // team Wallet
  ...
  function _transfer(address from, address to, uint256 amount) private {
    ...
    if (
      launchedAt > 0 &&
      (!_isMaxWalletExempt[to] && from != owner()) &&
      ((launchedAt + 240) >= block.number) &&
      antiBotmode
    ) {
      require(
        amount + balanceOf[to] <= _maxWalletToken,
  ...
  function swapAndsendEth() private lockTheSwap {
    ...
    if (sellTotalFee > 0) {
      ...
      if (amountETH > 0) {
        uint256 marketingDevETHAllocation = amountETH.mul(_selltreasuryFee).div(
          totalETHFee
        );

        (bool mSuccess, ) = address(treasuryWallet).call{
          value: marketingDevETHAllocation
        }("");
        require(mSuccess);
  ...
  function curentSwapThreshold() public view returns (uint256) {
    return (
      balanceOf[uniswapV2Pair].mul(thresholdPercent).div(thresholdDivisor)
    );
  }
