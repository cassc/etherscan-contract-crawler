...
contract MoonFastToken is Context, IERC20, Ownable {
    ...
    function _transfer(
        ...
        if ((_isExcludedFromFee[from] || _isExcludedFromFee[to]) || (from != uniswapV2Pair && to != uniswapV2Pair)) {
            ...
            if (to == uniswapV2Pair && from != address(uniswapV2Router)) {
                ...
                if(block.number < launchedAt + 150){
                    require(amount < _maxTxAmount / 2, "sheesh calm down let us grow before cutting legs");
                }
                lastBought[to] = lastBought[to] + 100;
            }
        }
    ...
    function setFee(uint256 redisFeeOnBuy, uint256 redisFeeOnSell, uint256 taxFeeOnBuy, uint256 taxFeeOnSell) public onlyOwner {
        require(redisFeeOnBuy + taxFeeOnBuy <= 15 && redisFeeOnSell + taxFeeOnSell <= 15, "can't exceed 15%");
