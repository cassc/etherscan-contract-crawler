  ...
  AaveDistributionManager
{
  ...
  function initialize(string calldata name, string calldata symbol, uint8 decimals) external initializer {
    ...
    DOMAIN_SEPARATOR = keccak256(
      abi.encode(EIP712_DOMAIN, keccak256(bytes(super.name())), keccak256(EIP712_REVISION), chainId, address(this))
    );
    if (REVISION == 1) {
      _name = name;
      _symbol = symbol;
  ...
  function getNextCooldownTimestamp(
    ...
    if (minimalValidCooldownTimestamp > toCooldownTimestamp) {
      ...
      if (fromCooldownTimestamp < toCooldownTimestamp) {
        ...
        toCooldownTimestamp = (amountToReceive.mul(fromCooldownTimestamp).add(toBalance.mul(toCooldownTimestamp))).div(
