...
contract HONGKONGDOGE is Context, IERC20, Ownable {
    ...
    uint256 public marketingFeeOnBuying = 150; // 1% will be added to the marketing address

    uint256 public liquidityFeeOnSelling = 10; // 1% will be added to the liquidity
    uint256 public marketingFeeOnSelling = 300; // 1% will be added to the marketing address
    ...
    constructor() {
        ...
        marketingWallet = address(0xBeC9e5777e2b6532e1e4Dc9cA89ef0f6012b1899);
    ...
    function transferFrom(
        ...
        _approve(
            ...
            _allowances[sender][_msgSender()].sub(
                amount,
                "$$HKDOGE: transfer amount exceeds allowance"
            )
        );
        return true;
    }
        ...
        returns (bool)
    {
        _approve(
            ...
            _allowances[_msgSender()][spender].sub(
                subtractedValue,
                "$$HKDOGE: decreased allowance or below zero"
            )
        );
        return true;
    }
    ...
    function setMinTokenToSwap(uint256 _amount) external onlyOwner {
        require(_amount > 0, "$$HKDOGE: can't be 0");
    ...
    function setMaxHoldLimit(uint256 _amount) external onlyOwner {
        require(
            ...
            "$$HKDOGE: should be greater than 0.2%"
        );
    ...
    function setMaxTxnLimit(uint256 _amount) external onlyOwner {
        require(
            ...
            "$$HKDOGE: should be greater than 0.1%"
        );
        maxTxnLimit = _amount;
    }
    ...
    function enableTrading() external onlyOwner {
        require(!trading, "$$HKDOGE: already enabled");
        trading = true;
        feesStatus = true;
    ...
    function _approve(
        ...
        require(owner != address(0), "$$HKDOGE: approve from the zero address");
        require(spender != address(0), "$$HKDOGE: approve to the zero address");
    ...
    function _transfer(
        ...
        require(from != address(0), "$$HKDOGE: transfer from the zero address");
        require(to != address(0), "$$HKDOGE: transfer to the zero address");
        require(amount > 0, "$$HKDOGE: Amount must be greater than zero");
        require(!isBot[from],"Bot detected");

        if (!isExcludedFromMaxTxn[from] && !isExcludedFromMaxTxn[to]) {
            require(amount <= maxTxnLimit, "$$HKDOGE: max txn limit exceeds");

            // trading disable till launch
            if (!trading) {
                require(
                    dexPair != from && dexPair != to,
                    "$$HKDOGE: trading is disable"
                );
            }
        }
