...
contract FashionApe is ERC721Enumerable, Ownable, ReentrancyGuard {
    ...
    address constant team_address = 0x744FcF002252CB41E953499b355456b6D68D721a;
    ...
    function presaleMint(uint256 ammount, bytes32[] calldata _merkleProof) external payable nonReentrant
    {
        ...
        require(tokensPerWalletWhitelist[msg.sender] + ammount <= MAXIMUM_MINT_WL, string(abi.encodePacked("FASHION APE: Presale mint is ", MAXIMUM_MINT_WL.toString(), " token only.")));
    ...
    function raffleMint(uint256 ammount) public payable nonReentrant {
        ...
        require(msg.value >= price * ammount, "FASHION APE: Not enough ETH sent");
        require(ammount <= MAXIMUM_MINT_RAFFLE, string(abi.encodePacked("FASHION APE: You can only mint up to ", MAXIMUM_MINT_RAFFLE.toString(), " token at once!")));
        require(tokensPerWalletRaffle[msg.sender] + ammount <= MAXIMUM_MINT_RAFFLE, string(abi.encodePacked("FASHION APE: You cant mint more than ", MAXIMUM_MINT_RAFFLE.toString(), " tokens!")));
