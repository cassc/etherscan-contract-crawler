...
contract CuriositiesStaking is OwnableUpgradeable, PausableUpgradeable {
    ...
    function calculateRewards() public view returns (uint256 native, uint256 fragments) {
        uint256[] memory tokenIds = IERC721AQueryable(address(curiosities)).tokensOfOwner(tx.origin);
    ...
    function claimRewards() external whenNotPaused {
        (uint256 native, uint256 fragments) = calculateRewards();
    ...
    function claimRewardsFor(uint256 tokenId) external whenNotPaused {
        (uint256 native, uint256 fragments) = calculateRewardsFor(tokenId);
        require(native > 0 || fragments > 0, "nothing_claimable");
        if(native > 0) nativeToken.mintFor(tx.origin, native);
        if(fragments > 0) fragmentStorage[tx.origin] += fragments;
    }

    function claimRewardsForMany(uint256[] calldata tokenIds) external whenNotPaused {
        uint16 tokenId;
        uint256 native;
        uint256 fragments;
        for(uint16 i=0;i<tokenIds.length;++i) {
            (uint256 _native, uint256 _fragments) = calculateRewardsFor(tokenId);
            unchecked {
                native += _native;
                fragments += _fragments;
            }
