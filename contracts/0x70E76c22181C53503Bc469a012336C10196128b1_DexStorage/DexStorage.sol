...
contract SecondDexStorage{
    ...
    struct collectionRoaylty {
      address _address;
      uint256 royalty;
    }
    mapping (address => collectionRoaylty) collectionRoyalties;
     mapping(address => uint256) blindNFTSaleLock;
}
...
contract DexUpdateSecondTrade is Ownable, DexStorage, SecondDexStorage {
 ...
 event Bid(address indexed bidder, address nftContract, uint256 tokenId, uint256 amount);
  ...
  function adminCancelCollection(address nftContract, uint256[] memory tokenIds) isValid(nftContract) public{
    ...
    for (uint256 i=0; i < tokenIds.length; i++) 
    {
        ...
        emit CancelSell(_saleTokens[tokenId][nftContract].seller, nftContract, tokenId);
         delete _saleTokens[tokenId][nftContract];
    }else {
        require(_auctionTokens[tokenId][nftContract].bidder == address(0),"109");
        INFT(nftContract).transferFrom(address(this), _auctionTokens[tokenId][nftContract].seller, tokenId);
        emit CancelSell(_auctionTokens[tokenId][nftContract].seller, nftContract, tokenId);
        delete _auctionTokens[tokenId][nftContract];
    }      
    }
  }

  function setCollectionRoyalty(address _add, uint256 royalty, address ath) public {
    require(admin == msg.sender, "101");
    collectionRoyalties[_add]._address = ath;
    collectionRoyalties[_add].royalty = royalty;
    _supportNft[_add] = true;
    collectionsWithRoyalties[_add] = true;
  }
 ...
 function calculatePrice(uint256 _price, uint256 base, uint256 currencyType, uint256 tokenId, address seller, address nft_a) public view returns(uint256 price) {
  ...
  function getPercentages(uint256 tokenId, address nft_a) public view returns(uint256 mainPerecentage, uint256 authorPercentage, address blindRAddress) {
    if(nft_a == address(XNFT) || collectionsWithRoyalties[nft_a]) { // royality for XNFT only (non-user defined collection)
     ...
     blindRAddress =_tokenAuthors[tokenId][nft_a]._address == address(0x0) ?  collectionRoyalties[nft_a]._address : _tokenAuthors[tokenId][nft_a]._address;
    if(blindRAddress == address(0x0) && (nft_a == address(XNFT) || collectionsWithRoyalties[nft_a])){
        blindRAddress = platform;
    }
  }
