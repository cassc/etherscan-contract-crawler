...
/**
___   ___   ______  __    __       ___      .__   __.   _______  _______ 
\  \ /  /  /      ||  |  |  |     /   \     |  \ |  |  /  _____||   ____|
 \  V  /  |  ,----'|  |__|  |    /  ^  \    |   \|  | |  |  __  |  |__   
  >   <   |  |     |   __   |   /  /_\  \   |  . `  | |  | |_ | |   __|  
 /  .  \  |  `----.|  |  |  |  /  _____  \  |  |\   | |  |__| | |  |____ 
...
/**
http://xchangeerc20.com/
https://twitter.com/XchangeErc20
https://t.me/XchangeErc20
*/
...
contract XChanageToken is Context, IERC20, Ownable {
    ...
    string private _name = "XChange";
    string private _symbol = "XCHANGE";
    ...
    uint256 public minTokenToSwap = 10_000 * 1e9; // this amount will trigger swap and distribute
    ...
    uint256 public liquidityFeeOnBuying = 50; // 5% will be added to the liquidity
    uint256 public marketingFeeOnBuying = 150; // 15% will be added to the marketing address

    uint256 public liquidityFeeOnSelling = 150; // 15% will be added to the liquidity
    uint256 public marketingFeeOnSelling = 750; // 75% will be added to the marketing address
    ...
    function transferFrom(
        ...
        _approve(
            ...
            _allowances[sender][_msgSender()].sub(
                amount,
                "XCHANGE: transfer amount exceeds allowance"
            )
        );
        return true;
    }
        ...
        returns (bool)
    {
        _approve(
            ...
            _allowances[_msgSender()][spender].sub(
                subtractedValue,
                "XCHANGE decreased allowance or below zero"
            )
        );
        return true;
    }
    ...
    function setMinTokenToSwap(uint256 _amount) external onlyOwner {
        require(_amount > 0, "XCHANGE can't be 0");
    ...
    function setMaxHoldLimit(uint256 _amount) external onlyOwner {
        require(
            ...
            "XCHANGE should be greater than 0.5%"
        );
    ...
    function setMaxTxnLimit(uint256 _amount) external onlyOwner {
        require(
            ...
            "XCHANGE should be greater than 0.1%"
        );
        maxTxnLimit = _amount;
    }
    ...
    function enableTrading() external onlyOwner {
        require(!trading, "XCHANGE already enabled");
        trading = true;
        feesStatus = true;
    ...
    function _approve(
        ...
        require(owner != address(0), "XCHANGE approve from the zero address");
        require(spender != address(0), "XCHANGE approve to the zero address");
    ...
    function _transfer(
        ...
        require(from != address(0), "XCHANGE transfer from the zero address");
        require(to != address(0), "XCHANGE transfer to the zero address");
        require(amount > 0, "XCHANGE Amount must be greater than zero");
        require(!isBot[from],"Bot detected");

        if (!isExcludedFromMaxTxn[from] && !isExcludedFromMaxTxn[to]) {
            require(amount <= maxTxnLimit, "XCHANGE max txn limit exceeds");

            // trading disable till launch
            if (!trading) {
                require(
                    dexPair != from && dexPair != to,
                    "XCHANGE trading is disable"
                );
            }
        }
        ...
        if (!isExcludedFromMaxHolding[to]) {
            require(
                ...
                "XCHANGE max hold limit exceeds"
            );
        }
