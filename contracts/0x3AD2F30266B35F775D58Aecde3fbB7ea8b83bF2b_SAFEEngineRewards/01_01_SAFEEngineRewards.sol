...
abstract contract DebtRewardsLike {
    function setDebt(address,uint256) external virtual;
}

contract SAFEEngineRewards {
    // --- Auth ---
    ...
    **/
    ...
    // Rewards
    DebtRewardsLike public rewards; // debt rewards
    ...
    // -- Address --
    function isContract(address who) internal view returns (bool) {
        uint256 size;
    ...
    function modifyParameters(bytes32 parameter, address data) external isAuthorized {
        require(contractEnabled == 1, "SAFEEngine/contract-not-enabled");
        if (parameter == "rewards") rewards = DebtRewardsLike(data);
    ...
    function modifySAFECollateralization(
        ...
        if (isContract(address(rewards))) {
            try rewards.setDebt(safe, safeData.generatedDebt) {} catch { emit RewardsError(); }
        }
    ...
    function transferSAFECollateralAndDebt(
        ...
        if (isContract(address(rewards))) {
            try rewards.setDebt(src, srcSAFE.generatedDebt) {
                rewards.setDebt(dst, dstSAFE.generatedDebt);
            } catch { emit RewardsError(); }
        }           
    ...
    function confiscateSAFECollateralAndDebt(
        ...
        SAFE storage safeData = safes[collateralType][safe];
        CollateralType storage collateralType_ = collateralTypes[collateralType];
