...
contract InvestisizeToken is ERC1400, IExtensionTypes {

    uint256 public constant ISSUED_TOKENS = 23_113_639 * 10 ** 18;
    uint256 public constant FOR_SALE_TOKENS = 20_802_275 * 10 ** 18;

    bytes32 public constant INVESTISIZE_DEFAULT_PARTITION = keccak256(abi.encodePacked("INVESTISIZE_DEFAULT_PARTITION"));

    bytes32[] private INVESTISIZE_DEFAULT_PARTITIONS = [bytes32(INVESTISIZE_DEFAULT_PARTITION)];
   ...
   */
    ...
    ERC1400("Investisize", "ETFD", GRANULARITY, INVESTISIZE_DEFAULT_PARTITIONS)
    {
        require(newOwner != address(0), "InvestisizeToken: wrong address");
        require(seller != address(0), "InvestisizeToken: wrong address");

        transferOwnership(newOwner);

        _issueByPartition(INVESTISIZE_DEFAULT_PARTITION, msg.sender, newOwner, ISSUED_TOKENS, "");
        _isIssuable = true;
