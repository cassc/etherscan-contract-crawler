  ...
  IUSDCBurnableSourceBridge
{
  ...
  /**
   * @dev this executes a burn on the source chain.
   */
  function burnAllLockedUSDC() external onlyOwner {
    if (!usdc.isMinter(address(this))) revert NoBurnCapabilities(address(this));

    // Burn the whole balance of USDC locked in this smart contract
    uint256 usdcAmountInContract = usdc.balanceOf(address(this));
    if (usdcAmountInContract > 0 && balance > 0) {
      uint256 amountToBurn = balance;
      balance = 0;
      usdc.burn(amountToBurn);
    }
    emit AllLockedUSDCBurnt(msg.sender);
  }
}