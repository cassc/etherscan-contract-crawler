...
contract Meemo is IERC20 {
    ...
    string constant private _name = "Meemo";
    string constant private _symbol = "TEMP";
    ...
    TaxWallets public _taxWallets = TaxWallets({
        marketing: payable(0x39ee722ada75879B7e988c909B43CA3F5313B435),
        development: payable(0xA18c47B388A90F438d10573ed41d30866BbbB805),
        staking: payable(address(0))
    });
...
//===============================================================================================================
    ...
    function setRatios(uint16 marketing, uint16 development, uint16 burn, uint16 staking) external onlyOwner {
        ...
        require(_ratios.totalSwap + _ratios.burn + _ratios.staking <= total, "Cannot exceed sum of buy and sell fees.");
    }

    function setWallets(address payable marketing, address payable development, address payable staking) external onlyOwner {
        require(marketing != address(0) && development != address(0) && staking != address(0), "Cannot be zero address.");
    ...
    function takeTaxes(address from, uint256 amount, bool buy, bool sell) internal returns (uint256) {
        Ratios memory ratios = _ratios;
        uint256 total = ratios.burn + ratios.staking + ratios.totalSwap;
        uint256 currentFee;
        if (buy) {
        ...
        uint256 stakingAmt = (feeAmount * ratios.staking) / total;
        uint256 swpAmt = feeAmount - (burnAmt + stakingAmt);
        if (swpAmt > 0) {
        ...
        if (stakingAmt > 0) {
            _tOwned[_taxWallets.staking] += stakingAmt;
            emit Transfer(from, _taxWallets.staking, stakingAmt);
        }
