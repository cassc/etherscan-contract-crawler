...
contract ETH {
    ...
    *
    * @description
    * 1) register token with providing token address and price
    * 2) approve ETH contract to spend tokens (ERC20.approve(box, amount))
    * 3) sell tokens
    */
    ...
    function unregister(uint256 id) public {
        require(tokenInfo[id].owner == msg.sender, "ETH::unregister: not owner");
    ...
    function setPrice(uint256 id, uint256 amount, uint256 price) public {
        require(tokenInfo[id].owner == msg.sender, "ETH::setPrice: not owner");
    ...
    receive() external payable {
        ...
        require(amount > 0, "ETH::buy: invalid amount");
        require(address(info.token) != address(0), "ETH::buy: invalid token");
        require(available(0) >= amount, "ETH::buy: insufficient balance");
    ...
    function buy(uint256 id, uint256 quantity) public payable {
        ...
        require(address(token) != address(0), "ETH::buy: invalid token");
        require(tokenInfo[id].price * quantity == msg.value, "ETH::buy: not enough");
        require(available(id) >= amount, "ETH::buy: insufficient balance");
