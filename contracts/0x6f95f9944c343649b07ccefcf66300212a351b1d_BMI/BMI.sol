...
contract BMIDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SignedSafeMath for int256;
    ...
    constructor() DividendPayingToken("BMI_Dividend_Tracker", "BMI_Dividend_Tracker") {
        claimWait = 3600;
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "BMI_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");	
        require(newClaimWait != claimWait, "BMI_Dividend_Tracker: Cannot update claimWait to same value");
...
contract BMI is ERC20, Ownable, SafeToken {
    using SafeMath for uint256;
    ...
    BMIDividendTracker public dividendTracker;

    uint256 public swapTokensAtAmount = 10000 * (10**18);
    uint256 public _maxWalletSize = 300000 * 10**18; // 3%
    uint256 public _maxTxAmount = 100000 * 10**18; // 1%
    ...
    function setFee(uint256 _ethRewardFee, uint256 _liquidityFee, uint256 _marketingFee, uint256 _charityFee) public onlyOwner {
        ...
        totalFees = EthRewardFee.add(liquidityFee).add(marketingFee).add(charityFee); // total fee transfer and buy
    }

    function setMaxWalletPercent(uint256 maxWallPercent) external onlyOwner() {
        require(maxWallPercent > 0, "wallet percent can't be zero");
        _maxWalletSize = totalSupply().mul(maxWallPercent).div(
            10**2
        );
    }

     function setMaxTxPercent(uint256 maxTxPercent) external onlyOwner() {
         require(maxTxPercent > 0, "maxTxPercent percent can't be zero");
        _maxTxAmount = totalSupply().mul(maxTxPercent).div(
            10**2
        );
    }
    
    function setMarketingWallet(address payable _newWallet) public onlyOwner {
        marketingWallet = _newWallet;
    }
    
    function setcharityWallet(address payable _newWallet) public onlyOwner {
        charityWallet = _newWallet;
    }

    constructor() ERC20("BullMoon Inu", "BMI") {
        EthRewardFee = 2;
        liquidityFee = 3;
        marketingFee = 4;
        charityFee = 3;
        marketingWallet = payable(0x2904842dD01518c3C0b24E62c61820c5bd90CD51);
        charityWallet = payable(0xFa9216272F05673ad169aAF391B2DD5F84102E39);

        totalFees = EthRewardFee.add(liquidityFee).add(marketingFee).add(charityFee); // total fee transfer and buy

    	dividendTracker = new BMIDividendTracker();
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "BMI: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "BMI: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "BMI: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "BMI: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "BMI: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "BMI: Cannot update gasForProcessing to same value");
