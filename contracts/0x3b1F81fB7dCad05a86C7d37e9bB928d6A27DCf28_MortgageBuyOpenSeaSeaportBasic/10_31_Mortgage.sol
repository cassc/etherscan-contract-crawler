...
contract MortgageBuyNFTX is MortgageERC721UsingETH {
    ...
    function executeFlashOperation(address payable borrower, uint240 mToken, uint borrowAmount, uint paidOutAmount, bytes calldata flashParams) external nonReentrant returns (uint) {
        ...
        uint256 underlyingID;
        {
            ...
            require(mERC721.ownerOf(mERC721.mTokenFromUnderlying(underlyingID)) != borrower, "already owner");
...
contract MortgageBuyOpenSeaSeaportBasic is MortgageERC721UsingETH {
    ...
    constructor(address _mERC721Address) public MortgageERC721UsingETH(_mERC721Address) {
        // allow to OpenSea conduit to access all WETH of this contract
    ...
    function executeTransfer(uint256 tokenId, address payable seller, uint sellPrice, bytes calldata transferParams) external nonReentrant returns (uint) {
...

        {
            ...
            uint256 newWETHBalance = WETHContract.balanceOf(address(this));
            require(newWETHBalance >= oldWETHBalance, "WETH balance corrupted");
            newWETHBalance = newWETHBalance - oldWETHBalance;
            require(newWETHBalance >= sellPrice, "insufficient price received");

            /* convert received WETH -> ETH -> mEther (calls revert on any failure) */
            WETHContract.withdraw(newWETHBalance);
            mERC721.tokenAuction().paymentToken().mintTo.value(newWETHBalance)(seller);
        }
