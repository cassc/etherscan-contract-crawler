...
contract FxERC721RootTunnelTokenURI is FxBaseRootTunnel, Create2, IERC721Receiver {
    ...
    function deposit(
        ...
        // DEPOSIT, encode((rootToken, depositor, to, tokenId, tokenURI, depositData))
        bytes memory message = abi.encode(DEPOSIT, abi.encode(rootToken, msg.sender, user, tokenId, tokenURI, data));
        _sendMessageToChild(message);
