...
contract GreenBean is ERC721A, Ownable, ReentrancyGuard {
    ...
    ) ERC721A("GreenBean", "GB", _maxBatchSize, _collectionSize) {
        baseURI = initBaseURI;
    ...
    function reserve(address recipient, uint256 amount) external onlyOwner {
        require(recipient != address(0), "GreenBean: zero address");
        require(amount > 0, "GreenBean: invalid amount");
        require(
            totalSupply() + amount <= collectionSize,
            "GreenBean: max supply exceeded"
        );
        require(
            tokensReserved + amount <= reserveAmount,
            "GreenBean: max reserve amount exceeded"
        );
        require(
            amount % maxBatchSize == 0,
            "GreenBean: can only mint a multiple of the maxBatchSize"
        );
    ...
    function mint(uint256 amount) external payable {
        require(status == Status.PublicSale, "GreenBean: Public sale is not active.");
    ...
    function refundIfOver(uint256 price) private {
        require(msg.value >= price, "GreenBean: Need to send more ETH.");
        if (msg.value > price) {
