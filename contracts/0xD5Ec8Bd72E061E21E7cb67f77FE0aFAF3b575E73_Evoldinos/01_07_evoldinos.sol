...
contract Evoldinos is Ownable, ERC721A, ReentrancyGuard {
    ...
    address payable public immutable secondaryBeneficiaryAddress;
    ...
    constructor() ERC721A("Evoldinos Club", "EVOLDINOS") {
        PRICE = 0.03 ether;
        secondaryBeneficiaryAddress = payable(
            0xC496e76c8eB3507e3eB9dC8caa9696E5Ac9DA37B
        );
    }
    ...
    function withdraw() external onlyOwner nonReentrant {
        ...
        uint256 ownerShare = (totalBalance * 88) / 100;
        uint256 secondaryBeneficiaryShare = totalBalance - ownerShare;

        (bool success_1, ) = msg.sender.call{value: ownerShare}("");
        require(success_1, "Transfer to owner failed.");

        (bool success_2, ) = secondaryBeneficiaryAddress.call{
            value: secondaryBeneficiaryShare
        }("");
        require(success_2, "Transfer to secondary beneficiary failed.");
    }
}