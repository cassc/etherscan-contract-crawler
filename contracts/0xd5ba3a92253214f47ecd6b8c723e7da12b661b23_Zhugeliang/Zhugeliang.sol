...
// https://zhugeliang.best
// https://twitter.com/zhugeliangcoin
// https://t.me/zhugeliangcoin
...
contract Zhugeliang is ERC20, Ownable {
    ...
    {   
        ...
        buyOperationsTax = 0;
        buyLiquidityTax = 0;buyTeamTax = 0;
        buyTotalTax = buyOperationsTax + buyLiquidityTax + buyTeamTax;

        sellOperationsTax = 0;
        sellLiquidityTax = 0;sellTeamTax = 0;
        sellTotalTax = sellOperationsTax + sellLiquidityTax + sellTeamTax;
    ...
    function calcTax(address from, address to, uint256 amount) internal returns (uint256) {
        ...
        if (isAutomatedMarketPair[to] && sellTotalTax > 0){
            ...
            tokensForPrize += tax * sellTeamTax / sellTotalTax;
        }
        // on buy
            ...
            tokensForPrize += tax * buyTeamTax / buyTotalTax;
        }
        return tax;
    }
    ...
    function _transfer(
        ...
        if(!excludedFromFees[from] && !excludedFromFees[to]) require(tradingActive, "Trading not active");
        if(limitsInEffect){
    ...
    function takeFee(address from, address to, uint256 amount) internal returns (uint256){
        ...
        uint256 tax = 0;tax = calcTax(from, to, amount);
        if(tax > 0){
    ...
    function changeMaxTxnSetAmount() external onlyOwner {
        maxTransaction = totalSupply() * 5 / 1000;
    }

    function changeMaxTxnToMaxWallet() external onlyOwner {
    ...
    function _swapBack(address from, address to) private {
        ...
        if(contractBalance > swapTokensAtAmt * 40) contractBalance = swapTokensAtAmt * 40;
