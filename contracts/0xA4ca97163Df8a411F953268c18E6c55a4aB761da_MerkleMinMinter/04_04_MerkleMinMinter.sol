...
contract MerkleMinMinter {
    ...
    modifier onlyAdmin(address _dropAddress) virtual {
        require(INM721A(payable(_dropAddress)).hasRole(INM721A(payable(_dropAddress)).ADMIN_ROLE(), msg.sender), "not admin");
        _;
    }

    modifier onlyMinter(address _dropAddress) virtual {
        require(INM721A(payable(_dropAddress)).hasRole(INM721A(payable(_dropAddress)).MINTER_ROLE(), msg.sender), "not minter");
        _;
    }
