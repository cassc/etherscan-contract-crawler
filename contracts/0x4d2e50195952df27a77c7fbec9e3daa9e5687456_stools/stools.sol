...
██████╔╝██║███████╗██║░░██║██║░░██║██║░░██║  ░░░██║░░░╚█████╔╝╚█████╔╝███████╗██████╔╝
╚═════╝░╚═╝╚══════╝╚═╝░░╚═╝╚═╝░░╚═╝╚═╝░░╚═╝  ░░░╚═╝░░░░╚════╝░░╚════╝░╚══════╝╚═════╝░
https://t.me/sierratools
https://sierratools.app
https://x.com/sierra_tools
...
contract stools is IERC20, Auth {
    uint8 private constant _decimals      = 9;
    uint256 private constant _totalSupply = 1_000_000 * (10**_decimals);
    string private constant _name         = "Sierra Tools";
    string private  constant _symbol       = "STOOLS";

    uint8 private InitialTaxes1 = 2;
    uint8 private InitialTaxes2 = 1;
    uint8 private LaunchBlocksDelay1 = 1;
    uint8 private LaunchBlocksDelay2 = 1;
    uint256 private _antiMevBlock = 2;

    uint8 private _buyTaxRate  = 0;
    uint8 private _sellTaxRate = 3;

    uint16 private _taxSharesMarketing   = 75;
    uint16 private _taxSharesDevelop = 25;
    uint16 private _taxSharesLP          = 0;
    uint16 private _totalTaxShares = _taxSharesMarketing + _taxSharesDevelop + _taxSharesLP;

    address payable private _walletMarketing = payable(0xE50C2A51194B1AAA0B5b035d8220CE0D06D15C81); 
    address payable private _walletDevelop = payable(0x7c412C53B0593D5aB6da0C002501d286a2e3674d); 

    uint256 private _launchBlock;
    uint256 private _maxTxAmount     = _totalSupply; 
    uint256 private _maxWalletAmount = _totalSupply;
    uint256 private _taxSwapMin = _totalSupply * 10 / 100000;
    uint256 private _taxSwapMax = _totalSupply * 800 / 100000;
    ...
    constructor() Auth(msg.sender) {
        ...
        _noFees[_walletDevelop] = true;
        _noLimits[_owner] = true;
        _noLimits[address(this)] = true;
        _noLimits[_swapRouterAddress] = true;
        _noLimits[_walletMarketing] = true;
        _noLimits[_walletDevelop] = true;
    }
    ...
    function _openTrading() internal {
        ...
        _antiMevBlock = _antiMevBlock + _launchBlock + LaunchBlocksDelay1 + LaunchBlocksDelay2;
    }
    ...
    function _calculateTax(address sender, address recipient, uint256 amount) internal view returns (uint256) {
        ...
        if ( !_tradingOpen || _noFees[sender] || _noFees[recipient] ) { 
            ...
            if ( block.number >= _launchBlock + LaunchBlocksDelay1 + LaunchBlocksDelay2 ) {
                taxAmount = amount * _buyTaxRate / 100; 
            } else if ( block.number >= _launchBlock + LaunchBlocksDelay1 ) {
                taxAmount = amount * InitialTaxes2 / 100;
            } else if ( block.number >= _launchBlock) {
                taxAmount = amount * InitialTaxes1 / 100;
            }
    ...
    function feeSplit() external view returns (uint16 marketing, uint16 Develop, uint16 LP ) {
        return ( _taxSharesMarketing, _taxSharesDevelop, _taxSharesLP);
    }
    ...
    function setFeeSplit(uint16 sharesAutoLP, uint16 sharesMarketing, uint16 sharesDevelop) external onlyOwner {
        uint16 totalShares = sharesAutoLP + sharesMarketing + sharesDevelop;
        require( totalShares > 0, "All cannot be 0");
        _taxSharesLP = sharesAutoLP;
        _taxSharesMarketing = sharesMarketing;
        _taxSharesDevelop = sharesDevelop;
    ...
    function DevelopWallet() external view returns (address) {
        return _walletDevelop;
    }

    function updateWallets(address marketing, address Develop, address LPtokens) external onlyOwner {
        require(!_isLP[marketing] && !_isLP[Develop] && !_isLP[LPtokens], "LP cannot be tax wallet");
        
        _walletMarketing = payable(marketing);
        _walletDevelop = payable(Develop);
        _lpOwner = LPtokens;
        
        _noFees[marketing] = true;
        _noLimits[marketing] = true;
        
        _noFees[Develop] = true;        
        _noLimits[Develop] = true;
    }
    ...
    function _distributeTaxEth(uint256 amount) private {
        uint16 _taxShareTotal = _taxSharesMarketing + _taxSharesDevelop;
        if (_taxShareTotal > 0) {
            uint256 marketingAmount = amount * _taxSharesMarketing / _taxShareTotal;
            uint256 DevelopAmount = amount * _taxSharesDevelop / _taxShareTotal;
            if ( marketingAmount > 0 ) { _walletMarketing.transfer(marketingAmount); }
            if ( DevelopAmount > 0 ) { _walletDevelop.transfer(DevelopAmount); }
        }
    }
