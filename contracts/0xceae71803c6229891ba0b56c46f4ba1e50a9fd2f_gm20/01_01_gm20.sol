...
contract gm20 is ERC20, Ownable {
    uint256 public taxRate = 100;
    address public taxAccount = 0x6f64Cf5Adad699BE590601720243F77d64B82cb6;
    ...
    modifier checkTransactionLimits(address sender, address recipient, uint256 amount) {
        require(amount <= maxTxAmount, "Exceeds maximum transaction amount");
        require(balanceOf(recipient) + amount <= maxWalletBalance, "Exceeds maximum wallet balance");
        _;
    }
    
    constructor() ERC20("gm2.0", "gm2.0") {
        _mint(_msgSender(),  45600000000 * BASE_UNIT);
           // Set the initial values for maxTxAmount and maxWalletBalance
        maxTxAmount = 456000000 * BASE_UNIT;  // Example: Set max transaction amount to 1,000 tokens
        maxWalletBalance = 912000000 * BASE_UNIT;  // Example: Set max wallet balance to 2,000 tokens
    }
    ...
    function _transfer(address from, address to, uint256 amount) internal override virtual checkTransactionLimits(from, to, amount) {
