...
contract Pika is Base {
    ...
    function _transfer(
        ...
        if (transferFee > 0) {
            ...
            uint256 burnAmount = _calculateFee(transferFee, burnPercentage);
            if (burnAmount > 0) {
                _burn(swapEnabled ? address(this) : beneficiaryAddress, burnAmount);
            }
    ...
    function setBurn(uint256 newBurn) external virtual onlyOwner {
        emit BurnUpdated(burnPercentage, newBurn);
