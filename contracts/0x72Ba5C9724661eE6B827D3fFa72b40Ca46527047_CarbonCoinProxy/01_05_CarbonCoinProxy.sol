...
contract CarbonCoinProxy is ERC20 {
    ...
    function exchangeToken (uint amountInGCX) external returns (bool) {
        ...
        exchangeableToken.transferFrom(msg.sender, address(this), amountInUSDT + fee);
        exchangeableToken.transfer(exchangeCollector, amountInUSDT);
        if (fee > 0) {
            exchangeableToken.transfer(exchangeFeeCollector, fee);
        }
