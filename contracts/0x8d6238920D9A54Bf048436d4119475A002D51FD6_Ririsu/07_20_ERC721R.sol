...
abstract contract ERC721R is Context, ERC165, IERC721, IERC721Metadata, IERC721Enumerable {
    ...
    function _safeMint(
        ...
        uint256 startIdx = totalSupply();
        require(to != address(0), "ERC721: mint to the zero address");
        require(!_exists(startIdx), "ERC721: token already minted");

        for (uint256 i = 0; i < qty; i++) {
            _beforeTokenTransfer(address(0), to, startIdx + i);
            _owners.push(to);
            emit Transfer(address(0), to, startIdx + i);
        }

        require(
            _checkOnERC721Received(address(0), to, startIdx, _data),
    ...
    function _transfer(
        ...
        require(ERC721R.ownerOf(tokenId) == from, "ERC721: transfer of token that is not owned");
