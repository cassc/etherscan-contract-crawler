...
contract Jimz is ERC721AQueryable, Ownable {
  using Strings for uint256;
  uint256 public MAX_SUPPLY = 6543;
  uint256 public MAX_FREE = 6543;
  ...
  constructor() ERC721A("Jimz", "Jimz") {
  _baseTokenURI = "ipfs://bafybeifaletieylazuus4umy2n5wmahcauutlx3vce4sbzbi27sfm7gpyy/";
  }
  ...
  function mint(uint256 _quantity) external payable callerIsUser underMaxSupply(_quantity) {
    require(balanceOf(msg.sender) < maxPublicMintPerWallet, "Not enough for Jimz to Jamz.");
    ...
    if (_totalMinted() < (MAX_SUPPLY)) {
      if (freeMintCount >= MAX_FREE) {
        ...
        require(msg.value >= (_quantity * publicTokenPrice), "Not Enough Ethereum For Jimz.");
        _mint(msg.sender, _quantity);
      }
    }
  }
