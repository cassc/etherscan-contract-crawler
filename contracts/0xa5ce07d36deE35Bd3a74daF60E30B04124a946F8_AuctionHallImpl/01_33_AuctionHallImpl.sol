...
contract AuctionHallImpl is ChainSchema, ThemisStorage, IAuctionHall {
    ...
    function executePositions(address[] calldata closedPositions, address[] calldata legacyPositions) external override {
        ...
        if (closedPositions.length > 0) {
            _updatePhase1State(closedPositions);
        }
        uint256[] memory lastPositionsState = _getBatchPositionState(legacyPositions);
        uint256[] memory newPositionsState = new uint256[](legacyPositionCount);
        for (uint256 i = 0; i < legacyPositionCount; i++) {
            require(lastPositionsState[i] == CLOSING_STATE, "AuctionHall: Not a closing position");
            if ((block.number.sub(auctionPositonInfoMap[legacyPositions[i]].closingBlock) > auctionMaxBlock && !phase1Infos[legacyPositions[i]].flag && !phase2Infos[legacyPositions[i]].flag) || _estimatePositionState(legacyPositions[i]) == OVERDRAWN_STATE) {
                newPositionsState[i] = OVERDRAWN_STATE;
            } else {
                newPositionsState[i] = 0;
            }
            emit Phase1Rollback(legacyPositions[i]);
        }
        tradingHub.batchUpdatePositionState(legacyPositions, newPositionsState);
    }
