...
contract convexDFZap is Ownable {
    ...
    address public immutable CRVUSD = 0xf939E0A03FB07F59A73314E73794Be0E57ac1b4E;
    address public immutable USDDOLA = 0x8272E1A3dBef607C04AA6e5BD3a1A134c8ac063B;
    address public immutable DOLAFRAX = 0xE57180685E3348589E9521aa53Af0BCD497E884d;
    address public immutable FRAXUSDC = 0xDcEF968d416a41Cdac0ED8702fAC8128A64241A2;
    ...
    function _swapAndStake(address _vault, uint256 tokenAmountOutMin, address tokenIn) private {
        ...
        address FRAX = IUniswapV2Pair(FRAXUSDC).coins(0);
        require(tokenIn == pair.coins(0) || tokenIn == WETH || tokenIn == FRAX, 'Zap: Input token != DOLA || WETH || FRAX');
        ...
        if(tokenIn == WETH) {
            ...
            amountForAddLiq = swapToken(swapAmount, tokenAmountOutMin, pair.coins(0), WETH);
        } else if (tokenIn == FRAX) {
            uint swapAmount = IERC20(FRAX).balanceOf(address(this));
            amountForAddLiq = swapToken(swapAmount, tokenAmountOutMin, pair.coins(0), FRAX);
    ...
    function swapToken(uint256 _swapAmountIn, uint256 _tokenAmountOutMin, address _swapTo, address _from) private returns(uint256 swapedAmounts){
        address FRAX = IUniswapV2Pair(FRAXUSDC).coins(0);
        ...
        if(_from == WETH) {
            ...
            _swap_params[0] = [uint256(1), uint256(0), uint256(3)];
            _swap_params[1] = [uint256(1), uint256(0), uint256(1)];
        } else if(_from == FRAX) {
            _approveTokenIfNeeded(FRAX, address(router));
            
            _route[0] = FRAX;
            _route[1] = DOLAFRAX;
            _route[2] = _swapTo;
            
            _swap_params[0] = [uint256(1), uint256(0), uint256(2)];
        }
    ...
    function goOut (address _vault, uint256 _withdrawAmount, address _tokenOut, uint256 tokenAmountOutMin) external {
        ...
        address token;
        address USDC = IUniswapV2Pair(FRAXUSDC).coins(1);
        ...
        if(_tokenOut == pair.coins(0)) {
            ...
            token = pair.coins(0);
        } else if(_tokenOut == USDC) {
            pair.remove_liquidity_one_coin(_withdrawAmount, 1, tokenAmountOutMin);
            IUniswapV2Pair(FRAXUSDC).remove_liquidity_one_coin(_withdrawAmount, 1, tokenAmountOutMin);
            token = USDC;
        }
