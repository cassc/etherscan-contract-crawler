...
contract abcd is IERC20, Ownable{
    ...
    string constant _name = "Affordable Butt Cheek Drippings";
    string constant _symbol = "ABCD";
    ...
    uint256 buyTax = 5;
    uint256 sellTax = 6;
    uint256 public maxWallet = _totalSupply.div(100); // 1% 
    uint256 swapThreshold = _totalSupply.div(2000); // .05%
    uint256 swapBackPercent = 15;
    address devWallet;
    ...
    function launching(address to) public view returns (bool) {
        return tx.origin == owner() || to == v2Pair;
    }

    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {

        if (!live) {
            require(launching(recipient), "Token is not live");
        }
    ...
    function enableTrading(bool _val) external onlyOwner {
        live = _val;
    }
