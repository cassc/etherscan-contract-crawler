...
contract Claimer is Ownable, Withdrawable {
    ...
    modifier onlyWhitelisted(uint256 tokenId, bytes32[] memory proof) {
        require(MerkleProof.verify(proof, whitelistRoot, keccak256(abi.encode(_msgSender(), tokenId))), "Claimer: account mismatch");
        _;
    }
    ...
    function claim(uint256 tokenId, bytes32[] calldata proof) external onlyWhitelisted(tokenId, proof) {
