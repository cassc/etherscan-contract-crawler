...
contract RandomMarketPlace is Ownable {
    ...
    function isValidMongoId(string memory _mongoId) public pure returns (bool) {
        bytes memory b = bytes(_mongoId);
    ...
    modifier allArraysAreSameLength(
        ...
        for (uint256 i = 0; i < __mongoIds.length; i++) {
            require(
                isValidMongoId(__mongoIds[i]),
                "One of the mongo ids is not valid"
            );
        }
        require(
    ...
    modifier validMongoId(string memory _mongoId) {
        require(isValidMongoId(_mongoId), "Mongo id is not valid");
        _;
    }
    ...
    function createManyItemsSimpleMethod(
        string memory name,
        uint256 price,
        uint256 ammount,
        string memory mongoId,
        address _seller
    ) public onlyController validMongoId(mongoId) {
        for (uint256 i = 0; i < ammount; i++) {
            createItem(name, price, mongoId, _seller);
        }
    }
    
    function createManyNftItemsSimpleMethod(
        uint256 _price,
        uint256[] memory _tokenIds,
        address _tokenCollection
    ) public onlyController {
        require(
            IERC721(_tokenCollection).isApprovedForAll(msg.sender, address(this)),
            "You need to approve this contract before you sell your NFTs"
        );
        for(uint256 i = 0; i < _tokenIds.length; i++){
            require(
                IERC721(_tokenCollection).ownerOf(_tokenIds[i]) == msg.sender,
                "You are not the owner of this NFT"
            );
            ...
            nftItems[_tokenCollection][_tokenIds[i]] = _item;
    ...
    function setPaused(bool _isPaused) public onlyController {
        isPaused = _isPaused;
    }
