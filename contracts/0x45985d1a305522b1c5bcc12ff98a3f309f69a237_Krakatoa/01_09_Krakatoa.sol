...
contract Krakatoa is Ownable, Additional, EIP712("Krakatoa", "2.0") {
    ...
    checkRequest(orderId, finalTimestamp)
    {
        ...
        for (uint8 i = 0; i < products.length; i++) {
            ...
            require(IERC20(token).transferFrom(buyer, product.seller, prices[i]), "KR: Unable to send funds");
        }

        require(IERC20(token).transferFrom(buyer, feeWallet(), fees), "KR: Unable to send fee");
    ...
    {
        require(_nonces[product.productId] < product.nonce, "KR: Item already sold");
        require(IERC20(token).transferFrom(buyer, address(this), product.price), "KR: Unable to transfer funds");
    ...
    function _processOrders(uint64[] memory orders, bool[] memory isWithdraw, uint8 _type) internal {
        ...
        if (total > 0) {
            require(IERC20(token).transfer(_msgSender(), total), "KR: Unable to send funds");
        }

        if (fee > 0) {
            require(IERC20(token).transfer(feeWallet(), fee), "KR: Unable to send fee");
        }
    }
    ...
    */
    ...
    checkSignature(abi.encode(orders, isWithdraw), signature)
    {
        ...
        for (uint8 i = 0; i < orders.length; ++i) {
            ...
            if (isWithdraw[i]) {
                ...
                require(IERC20(token).transfer(r.seller, r.price), "KR: Unable to send funds");
            } else {
                require(IERC20(token).transfer(r.buyer, r.price + r.fee), "KR: Unable to send funds");
            }
    ...
    checkSignature(abi.encode(orders), signature)
    {
        ...
        require(IERC20(token).transfer(feeWallet(), value), "KR: Unable to send fee");
    }
    ...
    checkSignature(abi.encode(orders, wallet), signature)
    {
        ...
        require(IERC20(token).transfer(wallet, value), "KR: Unable to send funds");
    }
