...
contract MonyMarketIII is Ownable {
    ...
    function writeListing(uint256 erc, address collection, address currency, uint256 tokenId, uint256 price) public payable{
        if(checkFormat(erc,price)){
            ...
            createListing(erc,collection,currency,tokenId,price);
        }
    }

    function writeListingFriend(bytes32[] calldata proof, uint256 erc, address collection, address currency, uint256 tokenId, uint256 price) public {
        if(checkFormat(erc,price)){
            require(marketOpen, "We are closed, come back later");
    ...
    function editPrice(uint256 newPrice, uint256 listingId) public {
        listing storage target = listingList[listingId];
        require(target.seller == msg.sender,"this is not your listing");
    ...
    function checkFormat (uint256 erc, uint256 price) public pure returns (bool) {
