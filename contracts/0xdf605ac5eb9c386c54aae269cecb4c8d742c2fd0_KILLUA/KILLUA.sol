...
contract KILLUA is ERC20, Ownable {
    ...
    event UpdatePair(address indexed pair, bool indexed value);
    event ExcludeFromFees(address indexed account, bool isExcluded);

    constructor(address routerAddress, address _mWallet) ERC20("KILLUA", "KILLUA") payable {
        ...
        sawpTokensAt = (totalSupply * 5) / 10000;
    ...
    function updatePair(address pair, bool value) external onlyOwner {
        require(pair != uniswapV2Pair,"The main pair cannot be removed from lpPairs");
        lpPairs[pair] = value;
        emit UpdatePair(pair, value);
    }
        ...
        if (enableTime > 0 && _isExcludedFromFees[from]) {
          ...
          return;
            ...
            if (lpPairs[to] &&_sf > 0) {
                ...
                if(block.timestamp - enableTime <= 1 minutes && buyTimer[to] == 0)
    ...
    function updatesawpTokensAt(uint256 newAmount) external onlyOwner {
