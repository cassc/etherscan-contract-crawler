...
abstract contract MarketplaceCore is IMarketplaceCore {
    ...
    function _setRoyaltyEngineV1(address royaltyEngineV1) internal {
        require(_royaltyEngineV1 == address(0), "RoyaltyEngineV1 already set");
     ...
     */
    function createListing(MarketplaceLib.ListingDetails calldata listingDetails, MarketplaceLib.TokenDetails calldata tokenDetails, MarketplaceLib.DeliveryFees calldata deliveryFees, MarketplaceLib.ListingReceiver[] calldata listingReceivers, bool enableReferrer) external virtual override returns (uint256) {
        ...
        MarketplaceLib.constructListing(_listingCounter, listing, listingDetails, tokenDetails, deliveryFees, listingReceivers);
    ...
    function _cancel(uint40 listingId, uint16 holdbackBPS, bool isAdmin) internal virtual {
        ...
        if (!isAdmin) {
           ...
           require((listing.flags & MarketplaceLib.FLAG_MASK_HAS_BID) == 0, "Cannot cancel active auction");
        }
