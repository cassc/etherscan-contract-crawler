 ...
 */
contract CCX is ERC20Upgradeable, OwnableUpgradeable {
     ...
     */
    function isBlacklisted(address _blacklistedAddress) external view virtual returns (bool) {
     ...
     */
    function setBlacklisted(address _blacklistedAddress) external virtual onlyOwner {
        require(_blacklistedAddress != address(0), "Zero address cannot be blacklisted");
     ...
     */
    function removeBlacklisted(address _blacklistedAddress) external virtual onlyOwner {
    ...
    function _transfer(
        ...
        if (!s_isOperational[from] && !s_isOperational[to]) {
            require(!s_isBlacklisted[from] && !s_isBlacklisted[to], 'The sender or recipient is blacklisted');
