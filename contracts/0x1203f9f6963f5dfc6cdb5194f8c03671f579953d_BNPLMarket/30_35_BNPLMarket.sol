    ...
    IBNPLMarket
{
    ...
    string public constant CONTRACT_VERSION = "3.1"; //used for offchain signature domain separator
    ...
    */
    function onUpgrade() external {
        ...
        //have to approve self as market forwarder until TellerV2 is updated
        TellerV2Context(_tellerV2).approveMarketForwarder(
            marketId,
            address(this)
        );
    }
    ...
    function acceptDiscreteOrder(
        ...
        uint256 _bidId = _submitTellerV2Bid(
            order.submitBidArgs,
            wethAddress,
            order.borrower
        );
