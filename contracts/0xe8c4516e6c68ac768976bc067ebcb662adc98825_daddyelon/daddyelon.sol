...
contract daddyelon is ERC20, Ownable {
    using SafeMath for uint256;
    uint256 private maxTransactionAmount;

    constructor() ERC20("DADDY ELON", "$DADDYE") {
        uint8 tokenDecimals = decimals();
        uint256 initialSupply = 2000000000000 * (10**uint256(tokenDecimals));
        _mint(msg.sender, initialSupply);
        maxTransactionAmount = 100000000 * (10**uint256(tokenDecimals));
    }
    ...
    function renounceOwnership() public override onlyOwner {
        // Impede a execução da função renounceOwnership
...
function transfer(address recipient, uint256 amount) public override returns (bool) {
    ...
    uint256 burnAmount = amount.mul(1).div(10000);
