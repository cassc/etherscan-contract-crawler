...
contract UTAutographFactory is Ownable, BlackHolePrevention {
    ...
    string      public constant     REGISTRY_KEY_FACTORY = "TRAIT_TYPE_5_FACTORY";
    ...
    // Errors
    error UTAutographFactoryNotCurrent(address);
    error UTAutographFactoryInvalidCommunityId(uint32);
    error UTAutographFactoryUnauthorized();
    error UTAutographFactoryTraitRegistryNotInstalled();
    error UTAutographFactoryTokenNotInstalled();
    ...
    function addTrait(
        ...
        if(GalaxisRegistry.getRegistryAddress(REGISTRY_KEY_FACTORY) != address(this)) {
            revert UTAutographFactoryNotCurrent(address(this));
        }
        ...
        if(crAddr == address(0)) {
            revert UTAutographFactoryInvalidCommunityId(_inputTrait.communityId);
        }
        ...
        // NFT must exist!
        if(ERC721 == address(0)) {
            revert UTAutographFactoryTokenNotInstalled();
        }
