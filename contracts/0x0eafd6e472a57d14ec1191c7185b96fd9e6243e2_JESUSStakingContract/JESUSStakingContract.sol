...
contract JESUSStakingContract is Owned, ReentrancyGuard {
    using SafeMath for uint256;
    uint256 private JESUSDecimals = 18;
    address public JESUSContract =
        0xe410EF63853666Cc83085f5De620b70Fdc93B905;
    address private JESUSStakeWallet = 0x58A64f48654A794bEd8FcAEA7f63Bd2Face3305E;
        ...
        returns (bool)
    {
        ...
        require(
            IERC20(JESUSContract).transferFrom(
                msg.sender,
                JESUSStakeWallet,
                tokens
            ),
    ...
    function addStakeOption( uint256 periodInSeconds, uint256 rewardMultiplicatorMilions) external onlyOwner {
        /*
        rewardMultiplicatorMilions should be amount of tokens user can withdraw after staking 1 million tokens
        */
    ...
    function UpdateStakeOptionReward(uint256 stakeOptionId, uint256 rewardMultiplicatorMilions) external onlyOwner {
         stakeOptions[stakeOptionId].rewardMultiplicatorMilions = rewardMultiplicatorMilions;
    }
    ...
    function importStakers(uint256 stakeOptionId, address[] memory accounts, uint256[] memory startPeriods, uint256[] memory stakedTokens) external onlyOwner {
        ...
        for(uint256 i = 0; i < accounts.length; i++) {
                addStaker(stakeOptionId, accounts[i], stakedTokens[i].mul(10 ** JESUSDecimals), startPeriods[i]);
        }
    }
