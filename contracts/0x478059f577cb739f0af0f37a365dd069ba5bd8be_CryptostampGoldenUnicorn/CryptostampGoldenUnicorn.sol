...
// File: contracts/CryptostampGoldenUnicorn.sol
...
contract CryptostampGoldenUnicorn is ERC721SimpleMapsURI {
    using SafeMath for uint256;
    ...
    constructor(address _createControl, address _shippingControl, address _tokenAssignmentControl, uint256 _finalSupply)
    ERC721SimpleMapsURI("Crypto stamp Golden Unicorn", "CSGU", "https://test.crypto.post.at/CSGU/meta/")
    public
    {
    ...
    modifier onlyMinter()
    {
        // Both the actual create control key and the shipping control key that delivers the on-chain raffle can mint Golden Unicorns.
        require(msg.sender == createControl || msg.sender == shippingControl, "createControl or shippingControl key required for this function.");
        _;
    }
    ...
    requireMinting
    {
        require(_tokenId < finalSupply, "Cannot mint token with too high ID");
    ...
    requireMinting
    {
        ...
        require(_tokenIdStart + addrcount - 1 < finalSupply, "Cannot mint token with too high ID");
    ...
    function sourceSale(uint256 _tokenId)
    public view
    returns (bool) {
        require(_tokenId < finalSupply, "Property query for nonexistent token");
        return _tokenId >= finalSupply / 2 - 1 && _tokenId < finalSupply - 1;
    }

    function sourceRaffle(uint256 _tokenId)
    public view
    returns (bool) {
        return !sourceSale(_tokenId);
    }

    function sourceOnChainRaffle(uint256 _tokenId)
    public view
    returns (bool) {
        require(_tokenId < finalSupply, "Property query for nonexistent token");
        return _tokenId <= finalSupply * 3 / 10 - 2;
    }

    function isLastUnicorn(uint256 _tokenId)
    public view
    returns (bool) {
        require(_tokenId < finalSupply, "Property query for nonexistent token");
        return _tokenId == finalSupply - 1;
    }
