...
contract SDYCAggregator is OwnableUpgradeable, UUPSUpgradeable {
        ...
        returns (uint80 roundId, int256 answer, uint256 startedAt, uint256 updatedAt, uint80 answeredInRound)
    {
        ...
        // decimals calculated as {decimals|8} + {token.decimals|6} - {balance decimals|2}
        answer = int256(FixedPointMathLib.mulDivDown(round.balance, 1e12, token.totalSupply()));

        return (_roundId, answer, round.updatedAt, round.updatedAt, _roundId);
    }

    /**
     * @notice get detail data about a round
     * @return roundId is the round ID for which data was retrieved
     * @return balance the total USD balance
     * @return updatedAt is the timestamp when the round last was updated (i.e. answer was last computed)
     */
    function getRoundDetails(uint80 _roundId) public view returns (uint80 roundId, uint256 balance, uint256 updatedAt) {
        RoundData memory round = rounds[_roundId];

        return (_roundId, round.balance, round.updatedAt);
    }
     ...
     * @return updatedAt is the timestamp when the round last was updated (i.e. answer was last computed)
     */
    function latestRoundDetails() external view returns (uint80 roundId, uint256 balance, uint256 updatedAt) {
    ...
    function reportBalance(uint256 _principal, uint256 _interest) external returns (uint80 roundId) {
        ...
        uint256 answer = FixedPointMathLib.mulDivDown(balance, 1e12, token.totalSupply());

        rounds[roundId] = RoundData(int256(answer), balance, block.timestamp);

        // interest accrued in terms of SDYC (interest is 2 decimals, SDYC is 6 decimals)
        totalInterestAccrued += _interest * 1e4;

        emit BalanceReported(roundId, balance, answer, block.timestamp);
    }
}