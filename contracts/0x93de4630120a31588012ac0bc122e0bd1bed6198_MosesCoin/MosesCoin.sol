...
contract MosesCoin is ERC20, Ownable {
    ...
    address private burnerWallet = 0xD9EcB8BEFA8EfE4A0E5ef56e95ef7dfd5D3C1C3A;

    uint256 public contractStartTime = block.timestamp;
    uint256 public burnTime = 6 * 30 days;  // 6 months
    ...
    function _transfer(
        ...
        if (lockBurner == true){
            require(from != burnerWallet, "Account blocked");
            require(to != burnerWallet, "Account blocked");
        }
    ...
    function burnAfterSixMonths() external onlyBurner(){
        require(block.timestamp >= contractStartTime + burnTime, "Burning not allowed yet");
        lockBurner = false;
