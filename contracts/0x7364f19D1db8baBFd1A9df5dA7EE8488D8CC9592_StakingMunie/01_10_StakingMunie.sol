...
contract StakingMunie is Ownable, ERC721Holder, ReentrancyGuard, Pausable {
    IERC721 private stakingToken; // Munie NFT
    IERC20 private rewardToken; // HanChain token
    ...
    uint256 private constant rewardsDuration = 365 days; // The total amount of HAN token available for reward = rewardTokenPerstakingToken * rewardsDuration
    ...
    function stake(uint256 _tokenId) public nonReentrant whenNotPaused {
        ...
        require(rewardToken.balanceOf(address(this)) - rewardsDuration * (totalTokenIds.length * rewardTokenPerStakingToken) > rewardsDuration * rewardTokenPerStakingToken, "Total amount of rewards is too high");
