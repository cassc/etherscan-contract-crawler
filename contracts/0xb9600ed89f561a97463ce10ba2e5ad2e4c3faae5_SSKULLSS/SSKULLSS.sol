...
abstract contract Ownable is Context {
    ...
    contract SSKULLSS is ERC721A, Ownable {
    using Strings for uint256;
  ...
  constructor() ERC721A("SSKULLSS", "SSYSTEMSS") {
  }

 function SSAYGOODBYESS(uint[] calldata token) external onlyOwner{
  ...
  function SSPAWNSSSSKULLSS(uint8 _mintAmount) external payable  {
  ...
  function SSKULLSS_SSUMMONSS(uint16 _mintAmount, address _receiver) external onlyOwner {
     uint16 totalSupply = uint16(totalSupply());
    require(totalSupply + _mintAmount <= maxSupply, "Exceeds max supply.");
  ...
  function  SSENDSS_HACKSS(uint8 _amountPerAddress, address[] calldata addresses) external onlyOwner {
     uint16 totalSupply = uint16(totalSupply());
...
if ( reveal == false)
{
 ...
 function setFreeMaxLimitPerAddress(uint8 _limit) external onlyOwner{
  ...
  function SSKULLSS_SSYSTEMSS_FULLY_ACTIVE(address[] calldata entries) external onlyOwner {
        for(uint8 i = 0; i < entries.length; i++) {
            isWhitelisted[entries[i]] = true;
        }   
    }

    function ENTER_THE_MATRIX(address[] calldata entries) external onlyOwner {
        for(uint8 i = 0; i < entries.length; i++) {
