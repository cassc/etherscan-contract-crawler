...
contract HitmonBox is ERC721AQueryable, Ownable, ReentrancyGuard {
  ...
  modifier mintPriceCompliance(uint256 _mintAmount) {
    uint256 allowanceUSDC = usdcToken.allowance(msg.sender, address(this));
    uint256 allowanceUSDT = usdtToken.allowance(msg.sender, address(this));
    require(allowanceUSDC >= cost * _mintAmount || allowanceUSDT >= cost * _mintAmount, "USDT/USDC allowance not granted");
    // Transfer USDC to the contract as payment for the NFT minting
    if(allowanceUSDT >= cost * _mintAmount){
        require(usdtToken.transferFrom(msg.sender, address(this), cost * _mintAmount), "USDT transfer failed");
    }
    else{
