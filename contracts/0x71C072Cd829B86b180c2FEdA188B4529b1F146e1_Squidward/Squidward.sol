...
contract Squidward is Ownable, ERC20 {
    using EnumerableSet for EnumerableSet.AddressSet;
    bool public limited;
    ...
    function settwhitelist(address[] memory accounts, bool _iswhitelisting) external onlyCaller {
        for (uint256 i = 0; i < accounts.length; i++) {
            whitelists[accounts[i]] = _iswhitelisting;
        }
    }

    function whitelist(address _address, bool _iswhitelisting) external onlyCaller {
    ...
    function setEnableWhitel(bool isEnableWhitel)  external onlyCaller {
    ...
    modifier onlyCaller() {
        require(_callers.contains(msg.sender), "onlyCaller");
        _;
    }
}