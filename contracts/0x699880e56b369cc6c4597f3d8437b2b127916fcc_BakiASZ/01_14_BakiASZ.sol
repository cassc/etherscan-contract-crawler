 ...
 */
contract BakiASZ is ERC721, Ownable, ERC2981 {
    /**
    ...
    /**
     * @dev Constractor of BAKIASZ contract. Setting the base token URI,
    ...
    modifier mintCompliance(uint256 _mintAmount) {
        require(
            ...
            "BAKIASZ: Must mint within max supply"
        );
        require(_mintAmount > 0, "BAKIASZ: Must mint at least 1");
        _;
    }
    ...
    modifier saleCompliance(uint256 _mintAmount) {
        require(isPreActive, "BAKIASZ: The sale is not Active yet");
        require(
            _amountMinted[_msgSender()] + _mintAmount <= _mintLimitsPerAddress,
            "BAKIASZ: Already reached mint limit"
        );
        require(msg.value == _price, "BAKIASZ: The mint price is not right");
        _;
    }
        ...
        saleCompliance(_mintAmount)
    {
        ...
        require(_verify(to, _merkleProof), "BAKIASZ: Invalid Merkle Proof");
