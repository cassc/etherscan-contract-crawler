    ...
    PausableUpgradeable
{
    ...
    function _sendPaymentAndPushInQueue(
        ...
        IERC20Upgradeable(Token).transfer(paymentToUser, amountForlvl);
    ...
    function _sendToRefferalPercent(address _user) internal {
        ...
        IERC20Upgradeable(Token).transfer(reseiver, amountBonus);
    ...
    function witdhraw(address token, uint256 amount)
        ...
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        IERC20Upgradeable(token).transfer(address(_msgSender()), amount);
    }
