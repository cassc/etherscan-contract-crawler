...
contract GimbutisToken is OwnableUpgradeable, ERC20BurnableUpgradeable {
    ...
    function _buyTokenWithUsdt(address _sender, uint256 _amount) private {
		...
		// from _sender to the contract admin address
        IERC20 _usdt_contract = IERC20(usdtAddress);
        uint256 _usdt_amount = _amount / 1e12;
        require(_usdt_amount > 0, "GimbutisToken: _usdt_amount is null");
        _usdt_contract.transferFrom( _sender, adminAddress, _usdt_amount);
        _transfer(address(this), _sender, _gxagAmount);
    }
