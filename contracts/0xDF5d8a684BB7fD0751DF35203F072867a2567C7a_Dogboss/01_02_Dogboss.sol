...
import "./DogbossLib.sol";

contract Dogboss is ISafemoon, Initializable, ContextUpgradeable, OwnableUpgradeable {
    ...
    function __Safemoon_v2_init_unchained(address _router) internal initializer {
        ...
        _tTotal = 25 * 10 ** 6 * 10 ** 18; // 25M
        _rTotal = (MAX - (MAX % _tTotal));
        _maxFee = 2000; // 20%

        // swapAndLiquifyEnabled = true;

        _maxTxAmount = 1 * 10 ** 6 * 10 ** 18; // 1M
        numTokensSellToAddToLiquidity = 100_000 * 10 ** 18; // 100K
    ...
    function __Safemoon_tiers_init() internal initializer {
        _defaultFees = _addTier(200, 0, 100, 0, 200, address(0), address(0));
    }
    ...
    function excludeWhitelistedAddress(address _account) external onlyOwner {
        require(_account != address(0), "Safemoon: Invalid address");
        require(_accountsTier[_account] > 0, "Safemoon: Account is not in whitelist");
        _accountsTier[_account] = 0;
    }

    function accountTier(address _account) external view returns (FeeTier memory) {
    ...
    function _checkFeesChanged(FeeTier memory _tier, uint256 _oldFee, uint256 _newFee) internal view {
        uint256 _fees = _tier
            .ecoSystemFee
    ...
    function updateRouterAndPair(address _uniswapV2Router, address _uniswapV2Pair) external onlyOwner {
    ...
    function setSwapAndEvolveEnabled(bool _enabled) external onlyOwner {
    ...
    function swapAndEvolve() external onlyOwner lockTheSwap {
    ...
    function setNumberOfTokenToCollectBNB(uint256 _numToken) external onlyOwner {
        numTokensToCollectBNB = _numToken;
    }

    function setNumOfBnbToSwapAndEvolve(uint256 _numBnb) external onlyOwner {
