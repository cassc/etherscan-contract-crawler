...
import "./HBCHAdmin.sol";
import "./HBCHLogic.sol";
import "./HBCHStorage.sol";
import "./Pausable.sol";

contract HBCHToken is IERC20Token,Pausable, HBCHAdmin{
    string public constant name = "Huobi BCH";

    string public constant symbol = "HBCH";

    uint8 public constant decimals = 18;

    HBCHLogic private logic;

    event Burning(address indexed from, uint256 value, string proof, string  bchAddress, address burner);
    event Burned(address indexed from, uint256 value, string proof, string  bchAddress);
    ...
    function burn(address from,uint256 value,string memory bchAddress,string memory proof, bytes32 taskHash)
    public whenNotPaused returns(bool){
        require(itemAddressExists(OPERATORHASH, msg.sender), "wrong operator");
        uint256 status = logic.burnLogic(from,value,bchAddress,proof,taskHash, msg.sender, operatorRequireNum);
        if (status == 1){
           emit Burning(from, value, proof,bchAddress, msg.sender);
        }else if (status == 3) {
           emit Burning(from, value, proof,bchAddress,  msg.sender);
           emit Burned(from, value, proof,bchAddress);
