...
import './../PoolStakingRewards/PoolStakingRewards5.sol';
import './../PoolCore/Pool13.sol';

// import "hardhat/console.sol";

contract HomeBoost4 is
  PausableUpgradeable,
  ...
  ERC721Upgradeable
{
    ...
    // use real year (365 days)
    uint256 constant SECONDS_PER_YEAR = SECONDS_PER_DAY * 365;
    // 90 days
    uint256 constant LEVEL_1_SECONDS_PER_ITERATION = SECONDS_PER_DAY * 90;
    // 1 year
    uint256 constant LEVEL_2_SECONDS_PER_ITERATION = SECONDS_PER_YEAR;

    uint256 constant LEVEL_1_RATE = 2 * 10 ** 16;
    ...
    function getPerYearRateForLevel(uint16 level, uint64 startTime, uint256 endTime) private view returns(uint256) {
        if (level == 1) {
            return LEVEL_1_RATE; // 2% apy over 90 days in a 365 day year
        } else if (level == 2) {
            return getPerYearRateForLevel2(startTime, endTime);
        }

        return 0;
    }

    function getWeeklyInterestRate(uint256 week) public view returns(uint256) {
      if (week >= weeklyInterestRates.length)
        return 0;
      return weeklyInterestRates[week].mul(10 ** 12); // 18 digits of precision for computations down the line.
    }

    function getPerYearRateForLevel2(uint64 startTime, uint256 endTime) public view returns(uint256) {
        ...
        uint256 fracitonalSum = (getWeeklyInterestRate(endWeekNumber) * endWeekNumberFraction);
        ...
        fracitonalSum += (getWeeklyInterestRate(startWeekNumber) * startWeekFraction);
    ...
    function mint(address to, uint256 principal, uint16 level, bool autoRenew) public whenNotPaused nonReentrant returns (bool) {
        ...
        require(level > 0 && level <=2, "Invalid level");
    ...
    function claimPrincipal(uint256 boostId) public whenNotPaused nonReentrant returns (uint256) {
        ...
        PoolStakingRewards5(poolStakingRewardAddress).unstakeForWallet(msg.sender, amountStaked);
    ...
    function claimPreBoostStake() public whenNotPaused nonReentrant returns (uint256) {
        return claimPreBoostStakeForAddress(msg.sender);
    }

    // Internal function to unstake all HOME staked using the old staking methods.
    function claimPreBoostStakeForAddress(address wallet) private returns (uint256) {
        uint256 ownedTokenCount = balanceOf(wallet);
        uint256 stakedAmount = PoolStakingRewards5(poolStakingRewardAddress).getCurrentBalance(wallet);
        ...
        if (ownedTokenCount > 0) {
            ...
            // Index 0 is a placeholder so skip it.
            for(uint256 currentId = 1; currentId < boostData.length; currentId++) {
                if (!_exists(currentId) || ownerOf(currentId) != wallet){
                    continue;
                }
        ...
        PoolStakingRewards5(poolStakingRewardAddress).unstakeForWallet(wallet, amount);

        return amount;
    }

    function unstakePreBoostStakes(address[] memory addresses) public whenNotPaused nonReentrant {
        require(msg.sender == guardianAddress, "must be guardian");

        for (uint256 currentAddressIndex = 0; currentAddressIndex < addresses.length; currentAddressIndex++) {
            claimPreBoostStakeForAddress(addresses[currentAddressIndex]);
        }
    }
    ...
    function computeTotalRewards(Boost storage boost, uint256 nextRewardTimestamp) private view returns (uint256) {
        ...
        uint256 endTime = nextRewardTimestamp < block.timestamp ? nextRewardTimestamp : block.timestamp;
        uint256 rate = getPerYearRateForLevel(boost.level, boost.startTime, endTime);
        uint256 secondsPerIteration = getSecondsPerIteration(boost.level);
        ...
        if (nextRewardTimestamp < block.timestamp) {
            ...
            totalRewardAmount = boost.principal.mul(boost.endIteration.mul(secondsPerIteration)).mul(rate).div(SECONDS_PER_YEAR.mul(10 ** 6));
        } else {
            // Otherwise the boost isn't mature. We can pay rewards up to the last full iteration plus whatever has been earned in the current iteration
            uint256 completeIterations = (block.timestamp - boost.startTime).div(secondsPerIteration);
            uint256 partialIteration = (block.timestamp - boost.startTime).mod(secondsPerIteration);
            uint256 rewardPerYear = boost.principal.mul(rate).div(1_000_000);
            // console.log('completeIterations %s, partial Iteration: %s', completeIterations, partialIteration);
            totalRewardAmount = rewardPerYear.mul(completeIterations.mul(secondsPerIteration)+partialIteration).div(SECONDS_PER_YEAR);
        }
        // console.log('total rewards before mature rewards: %s', totalRewardAmount);
        totalRewardAmount += boost.principal.mul(10 ** 10).mul(boost.additionalMatureSeconds + newMatureSeconds).div(SECONDS_PER_YEAR);
        // console.log('total rewards after mature rewards: %s', totalRewardAmount);
        return totalRewardAmount.div(10**12); // return 6 digits of precision
    }
    ...
    function _beforeTokenTransfer(
        ...
        Boost memory boost = boostData[tokenId];

        uint256 stake = getStakeAmount(boost.level, boost.principal);

        PoolStakingRewards5(poolStakingRewardAddress).transferStake(stake, from, to);
    }
}