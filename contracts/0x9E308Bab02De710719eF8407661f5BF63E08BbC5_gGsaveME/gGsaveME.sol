...
contract gGsaveME is IERC721, Ownable, Functional {
    ...
    constructor() {
        _name = "gGsaveME";
        _symbol = "GGME";
        _baseURI = "https://ggsave.me/metadata/";
        _postURI = ".json";
        _hideTokens = true;
        
        totalTokens = 3334;
        
        approvedSigner = 0x024DFD7220d8bEB7b81dfa8A99B2088eED5caa5d;
    }
    ...
    function supportsInterface(bytes4 interfaceId) public view virtual override returns (bool) {
        return  interfaceId == type(IERC721).interfaceId ||
                ...
                interfaceId == gGsaveME.onERC721Received.selector;
    }
    ...
    function mint(uint256 qty, uint256 maxMints, bytes memory signature) public reentryLock {
        require (qty + _mintTracker[_msgSender()] <= maxMints, "exceeded limit");
		require ((numberMinted + qty) <= totalTokens, "Sold Out");
		
        string memory newAddress = toAsciiString( _msgSender() );

        string memory checkstring = string.concat(toString(maxMints), "allowedby0x", newAddress);

        bytes32 hash = keccak256( abi.encodePacked(checkstring) );
        bytes32 messageDigest = keccak256(abi.encodePacked("\x19Ethereum Signed Message:\n32", hash));
        address hasher = ECDSA.recover(messageDigest, signature);

        require (hasher == approvedSigner, "Invalid Signature");

        _mintTracker[_msgSender()] += qty;
        //send tokens
        for(uint256 i = 0; i < qty; i++) {
            _safeMint(_msgSender(), numberMinted + i);
        }
        numberMinted += qty;
    }
    ...
    function getNumberMinted( address minter ) external view returns(uint256){
        return _mintTracker[minter];
    }

    function setSigner(address newSigner) external onlyOwner {
        approvedSigner = newSigner;
    }
    ...
    function setPostURItoNULL() public onlyOwner {
    	_postURI = "";
    }
