...
contract RebalancerModule is Helpers, Events {
    ...
    function collectRevenue(uint256 amount_) external nonReentrant onlyRebalancer {
        if (amount_ == type(uint256).max) amount_ = revenue;

        if (amount_ == 0) revert RebalancerModule__NotValidCollectReveuneAmount();
        if (amount_ > revenue) revert RebalancerModule__CollectReveuneAmountIsHigh();
