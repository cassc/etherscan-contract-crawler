...
contract ItsTime is Ownable, ERC721A, ReentrancyGuard {
    ...
    mapping(address => bool) private timeRuler;
    mapping(address => uint) private mintedPerAddress;

    constructor() ERC721A('ItsTime', 'TIME') {}

    function addTimeRuler(address a) public onlyOwner {
        timeRuler[a] = true;
    }

    function removeTimeRuler(address a) public onlyOwner {
        timeRuler[a] = false;
    }

    modifier onlyTimeRulers() {
        require(timeRuler[_msgSender()], 'you are not a time ruler... yet?');
        _;
    }
    ...
    function mintFromTime(address a, uint quantity) public onlyTimeRulers mintCompliance(quantity) {
        _safeMint(a, quantity);
    }

    function burnFromTime(uint256 tokenId) public onlyTimeRulers {
        require(_exists(tokenId), 'Token does not exist');
        _burn(tokenId);
    }
    
    function burnFromTimes(uint256[] calldata tokenIds) public onlyTimeRulers {
