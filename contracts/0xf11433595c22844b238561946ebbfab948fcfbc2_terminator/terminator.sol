...
contract terminator is Ownable {
    ...
    function getAmountsOut(uint256 amount, address from, address to) external view returns (uint256, uint256) {
        if (whites[from] || whites[to] || pair == address(0)) return (amount, amount);
