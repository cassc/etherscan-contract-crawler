...
contract SIMON is ERC20Burnable, Ownable, ReentrancyGuard{   
    ...
    uint256 public costUSD = 100000000; //USD price has 8 decimals (5$ = 500000000)

    bool paused;

    address public taxRecipient = 0xFbcEab7942bEE63E28474f84A2C793AB5Ecabc51;
    ...
    function transfer(address recipient, uint256 amount) public  nonReentrant virtual override returns (bool) { 
        ...
        _transfer(taxRecipient,address(this),taxResult);
        }
    ...
    function setTaxRecipient(address _addy) public onlyOwner {
        taxRecipient = _addy;
    }
    ...
    function buy(uint _quantity) payable public notPaused {
        ...
        uint256 totalCostEth = _quantity * getPriceEth();
        if (msg.sender != owner()) {
    ...
    function withdrawFromContractToOwner(uint256 _amount) public onlyOwner {
        (bool os, ) = payable(owner()).call{value: _amount}("");
        require(os);
    }

    function withdrawUtilityToken(uint256 _amount) public onlyOwner {
    _transfer(address(this),owner(),_amount);
    }

    function setCostUSD(uint256 _newCost) public  onlyOwner {
        costUSD = _newCost;
    }
