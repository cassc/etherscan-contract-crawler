...
import {IInitializableStaticAToken} from '../../interfaces/IInitializableStaticAToken.sol';
import {IERC20Detailed} from '../../dependencies/openzeppelin/contracts/IERC20Detailed.sol';
import {Errors} from '../libraries/helpers/Errors.sol';

/**
 * @title StaticAToken
 * @dev Wrapper token that allows to deposit tokens on the Sturdy protocol and receive
 * a token which balance doesn't increase automatically, but uses an ever-increasing exchange rate
 * - Only supporting deposits and withdrawals
 * @author Sturdy, inspiration from Aave
 **/
contract StaticAToken is
  VersionedInitializable,
  ERC20('STATIC_ATOKEN_IMPL', 'STATIC_ATOKEN_IMPL'),
  IInitializableStaticAToken
{
  ...
  function initialize(
    ...
    emit Initialized(address(lendingPool), aToken, staticATokenName, staticATokenSymbol);
  }

  /**
   * @dev Deposits `ASSET` in the Sturdy protocol and mints static aTokens to msg.sender
   ...
   * - `false` if the msg.sender comes already with aTokens (e.g. sUSDC)
   ...
   * @param recipient The address that will receive the amount of `ASSET` withdrawn from the Sturdy protocol
   * @param amount The amount to withdraw, in static balance of StaticAToken
   * @param toUnderlying bool
   * - `true` for the recipient to get underlying tokens (e.g. USDC)
   * - `false` for the recipient to get aTokens (e.g. sUSDC)
  ...
  function permit(
    ...
    require(owner != address(0), Errors.SAT_INVALID_OWNER);
    //solium-disable-next-line
    require(block.timestamp <= deadline, Errors.SAT_INVALID_EXPIRATION);
    ...
    require(owner == ecrecover(digest, v, r, s), Errors.SAT_INVALID_SIGNATURE);
    _nonces[owner] = currentValidNonce + 1;
    _approve(owner, spender, value);
  }

  /**
   * @dev Allows to deposit on Sturdy via meta-transaction
   * https://github.com/ethereum/EIPs/blob/8a34d644aacf0f9f8f00815307fd7dd5da07655f/EIPS/eip-2612.md
  ...
  function metaDeposit(
    ...
    require(depositor != address(0), Errors.SAT_INVALID_DEPOSITOR);
    //solium-disable-next-line
    require(block.timestamp <= deadline, Errors.SAT_INVALID_EXPIRATION);
  ...
  /**
   * @dev Allows to withdraw from Sturdy via meta-transaction
   * https://github.com/ethereum/EIPs/blob/8a34d644aacf0f9f8f00815307fd7dd5da07655f/EIPS/eip-2612.md
   * @param owner Address owning the staticATokens
   * @param recipient Address that will receive the underlying withdrawn from Sturdy
  ...
  function metaWithdraw(
    ...
    require(owner != address(0), Errors.SAT_INVALID_DEPOSITOR);
    //solium-disable-next-line
    require(block.timestamp <= deadline, Errors.SAT_INVALID_EXPIRATION);
    uint256 currentValidNonce = _nonces[owner];
    bytes32 digest = keccak256(
   ...
   * using the current liquidity index on Sturdy
  ...
  /**
   * @dev Returns the Sturdy liquidity index of the underlying aToken, denominated rate here
  ...
  function _deposit(
    ...
    require(recipient != address(0), Errors.SAT_INVALID_RECIPIENT);
  ...
  function _withdraw(
    ...
    require(recipient != address(0), Errors.SAT_INVALID_RECIPIENT);
    require(staticAmount == 0 || dynamicAmount == 0, Errors.SAT_ONLY_ONE_AMOUNT_FORMAT_ALLOWED);
