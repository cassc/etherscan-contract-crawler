...
import "./StakingDataController/StakeDataV2.sol";
import "./StakingCalculation.sol";
import "./StakingDataController/RoundData.sol";
import "@openzeppelin/contracts/token/ERC20/IERC20.sol";

contract StakingDataV2 is BaseContract
{
            ...
            _stakeDataImplementationAddress()
                .delegatecall(
                    abi.encodeWithSelector(
                        StakeDataV2.getTotalSupplyForBlockRange.selector,                       
                        vaultAddress, 
                        endBlock,
                        startBlock
                    )
    ...
    function _getTotalUnstakingsForBlockRange(address vaultAddress, uint endBlock, uint startBlock) internal returns(uint) {
        ...
        (bool success, bytes memory result) = 
            _stakeDataImplementationAddress()
                .delegatecall(
                    abi.encodeWithSelector(
                         StakeDataV2.getTotalUnstakingsForBlockRange.selector,
                        vaultAddress, 
                        endBlock,
                        startBlock
                    )
    ...
    function startUnstake(address account, uint256 value) external returns(bool) {
        ...
        (bool result, ) = _stakeDataImplementationAddress()
            .delegatecall(abi.encodeWithSelector(StakeDataV2.startUnstakeForAddress.selector, _vaultAddress(), account, value));
        return result;
    }
    ...
    function getAccountStakingTotal(address account) external view returns (uint) {
        ...
        uint balance = IERC20(msg.sender).balanceOf(account);
        
        for(uint i = 0; i < _AccountUnstakings[msg.sender][account].length; i++){
            if(_AccountUnstakings[msg.sender][account][i].endBlock == 0 && balance > 0){
                balance = balance.sub(balance >=  _AccountUnstakings[msg.sender][account][i].amount ?  _AccountUnstakings[msg.sender][account][i].amount : balance);
            }
        }
    ...
    function createStake(uint amount, address account) external returns(bool) {
        _validateVault();
        (bool result, ) = _stakeDataImplementationAddress()
            .delegatecall(abi.encodeWithSelector(StakeDataV2.createStake.selector,_vaultAddress(),amount,account));
        return result;
    }
    ...
    function removeStake(uint amount, address account) external returns(bool) {
        _validateVault();
        (bool result, ) = _stakeDataImplementationAddress()
            .delegatecall(abi.encodeWithSelector(StakeDataV2.removeStake.selector, _vaultAddress(), amount, account));
        return result;
    }
     ...
     */
     ...
     function authoriseUnstake(address account, uint timestamp, uint amount) external {
         _validateVault();
        (bool result, ) = _stakeDataImplementationAddress()
            .delegatecall(abi.encodeWithSelector(StakeDataV2.authoriseUnstake.selector, _vaultAddress(), account, timestamp, amount));            
        require(result);
    }
    ...
    returns(
        ...
        uint balance = IERC20(vaultAddress).balanceOf(account);
        
        for(uint i = 0; i < _AccountUnstakings[vaultAddress][account].length; i++){
            if(_AccountUnstakings[vaultAddress][account][i].endBlock == 0 && balance > 0){
                unStakingAmounts[i] = balance >=  _AccountUnstakings[vaultAddress][account][i].amount ?  _AccountUnstakings[vaultAddress][account][i].amount : balance ;
                unStakingStarts[i] = _AccountUnstakings[vaultAddress][account][i].startDateTime;
                balance = balance.sub(unStakingAmounts[i]);
            }
        }

        return( 
            balance, //_AccountStakes[vaultAddress][account].total.sub(_AccountUnstakingTotals[vaultAddress][account]),
            IERC20(vaultAddress).balanceOf(account).sub(balance),
            unStakingAmounts,
            unStakingStarts
        );
    }
    ...
    function _getAllAcountUnstakesForAddress(address vaultAddress) internal view returns (address[] memory accounts, uint[] memory startTimes, uint[] memory values) {
        ...
        for(uint i = 0; i < _UnstakingRequests[vaultAddress].length; i++) {
            if(_UnstakingRequests[vaultAddress][i].endBlock == 0 ) {
                ...
                //Get the balance before we start any unstakings
                uint balance = IERC20(vaultAddress).balanceOf(_UnstakingRequests[vaultAddress][i].account);

                // Go through each request and sub the unstaking amount - this will only be as big as the unstaking requests
                for(uint a = 0; a < accounts.length; a++){
                    if(accounts[a] != _UnstakingRequests[vaultAddress][i].account) continue;
                    balance = balance.sub(values[a]);
                }

                accounts[i] = _UnstakingRequests[vaultAddress][i].account;
                startTimes[i] = _UnstakingRequests[vaultAddress][i].startDateTime;
                // Now check if we have enough balance, if not then we can only return the balance as something like a burn has happened
                values[i] = balance >=  _UnstakingRequests[vaultAddress][i].amount ?  _UnstakingRequests[vaultAddress][i].amount : balance;
            }
        }        
