...
// File contracts/StakingNFT/IERC721Mintable.sol
...
interface IERC721 is IERC165 {
    ...
    function stakeLand(uint256 tokenId, address from, address to) external;
}


// File contracts/StakingNFT/IERC20.sol
...
// File contracts/StakingNFT/SafeERC20.sol
...
// File contracts/StakingNFT/stakingGlobals.sol
...
// File contracts/StakingNFT/IOracle.sol
...
// File contracts/StakingNFT/MerkleProof.sol
...
// File contracts/StakingNFT/stakingnft.sol
...
contract StakingLand is  OwnableUpgradeable, ReentrancyGuardUpgradeable, PausableUpgradeable, GlobalsAndUtils {
    ...
    function setLandAndGenesis() onlyOwner public {
     ...
     function setGenesisAddress(address _add) onlyOwner public {
        GENESIS_ADDRESS = _add;
        genesis = IERC721(_add);
    }
    /**********************************************************/
    /******************* Public Methods ***********************/
    /**********************************************************/
...
/**
 ...
 /**
    ...
    function adminUnstake(address stakeOwner, uint256 _stakeId, bool unstakeApplied) external onlyOwner {
        if (!unstakeApplied) {
            if (userStakeIds[stakeOwner].length > 1) {
                for (uint256 x = 0; x < userStakeIds[stakeOwner].length; x++) {
                    // find the index of stake id in userStakes
                    if (userStakeIds[stakeOwner][x] == _stakeId) {
                        if (userStakeIds[stakeOwner].length > 1) {
                            userStakeIds[stakeOwner][x] = userStakeIds[stakeOwner][userStakeIds[stakeOwner].length.sub(1)];
                            userStakeIds[stakeOwner].pop();
                        } else {
                            userStakeIds[stakeOwner].pop();
                        }
                    }
            ...
            userUnStakeIds[stakeOwner].push(_stakeId);
            unStakes[stakeOwner][_stakeId] = UnStake(stakes[stakeOwner][_stakeId].stakedAt, block.timestamp,0,stakes[stakeOwner][_stakeId].tokenId,LAND_ADDRESS,stakes[stakeOwner][_stakeId].genesisTokenIds,stakes[stakeOwner][_stakeId].size,stakes[stakeOwner][_stakeId].rarity,stakes[stakeOwner][_stakeId].xTickets, true);
            delete stakes[stakeOwner][_stakeId];
            emit AppliedUnstake(msg.sender, _stakeId, unStakes[stakeOwner][_stakeId].tokenId, unStakes[msg.sender][_stakeId].genesisTokenIds);
        }
        
        UnStake memory usrStake = unStakes[stakeOwner][_stakeId];
        unStakes[stakeOwner][_stakeId].isAppliedFor = false;
        unStakes[stakeOwner][_stakeId].unStakedAt = block.timestamp;
        
        land.transferFrom(address(this), stakeOwner, usrStake.tokenId);

        genesis.bulkTransfer(usrStake.genesisTokenIds, stakeOwner, address(this));
        
        emit StakeEnd(stakeOwner, _stakeId, usrStake.tokenId, usrStake.genesisTokenIds);
    }
}