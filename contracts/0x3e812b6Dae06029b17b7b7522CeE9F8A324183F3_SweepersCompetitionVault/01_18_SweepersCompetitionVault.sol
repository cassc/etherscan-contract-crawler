...
contract SweepersCompetitionVault is ReentrancyGuard, Ownable, IERC721Receiver, IERC1155Receiver, OpsReady {
    ...
    mapping(uint32 => bytes32) public pickWinnerTaskId;
    mapping(uint32 => bytes32) public settleTaskId;
    ...
    function setDistribution(uint16 _sweepersCut, uint16 _legacyCut, uint16 _devCut) external onlyOwner {
        require(_sweepersCut + _legacyCut + _devCut == 10000, "Sets must equal 10000");
    ...
    function updateCompEntryCost(uint32 _id, uint256 _entryCost) external onlySweepersTreasury {
        compId[_id].entryCost = _entryCost;
    }
    ...
    function startSettleTask(uint32 _id) internal {
        settleTaskId[_id] = IOps(ops).createTaskNoPrepayment(
            address(this), 
    ...
    function settleComp(uint32 _id, bool shouldStopTask) external {
        if(shouldStopTask) { stopTask(settleTaskId[_id]); }
        _settleComp(_id);
    }
