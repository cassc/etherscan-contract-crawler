...
contract BET is ERC20, Ownable {
    ...
    uint256 public  blockfundFeeOnBuy;
    uint256 public  blockfundFeeOnSell;
    ...
    event UpdateFees(uint256 blockfundFeeOnBuy, uint256 blockfundFeeOnSell);
    ...
    function updateFees(uint256 _blockfundFeeOnSell, uint256 _blockfundFeeOnBuy) external onlyOwner {
        require(_blockfundFeeOnSell + _blockfundFeeOnBuy <= blockfundFeeOnSell + blockfundFeeOnBuy, "You can only decrease the fees");

        blockfundFeeOnSell = _blockfundFeeOnSell;
        blockfundFeeOnBuy = _blockfundFeeOnBuy;

        emit UpdateFees(blockfundFeeOnSell, blockfundFeeOnBuy);
    }
    ...
    function _transfer(address from,address to,uint256 amount) internal  override {
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to] || swapping) {
            ...
            if (block.number < tradingBlock + 5){
                _totalFees = 99;
            } else {
                _totalFees = blockfundFeeOnBuy;
            }
        } else if (to == uniswapV2Pair) {
            if(block.timestamp > tradingTime + 600){
                _totalFees = blockfundFeeOnSell;
            } else{
                _totalFees =  16;
            }
        } else {
    ...
    function swapAndSendBlockFund(uint256 tokenAmount) private {
        ...
        payable(blockFundWallet).sendValue(newBalance);
