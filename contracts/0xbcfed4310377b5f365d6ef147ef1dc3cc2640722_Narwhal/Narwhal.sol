...
contract Narwhal is Context, IERC20, Ownable {
    ...
    string private _name = "Narwhal";
    string private _symbol = "Narwhal";
    uint8 private _decimals = 9;

    address payable public marketingWallet = payable(0x000000000000000000000000000000000000dEaD);
    address payable public fundWallet = payable(0x000000000000000000000000000000000000dEaD);
    ...
    uint256 public _totalTax = _marketingFee.add(_fundFee).add(_burnFee);
    ...
    constructor () {
        ...
        _totalTax = _marketingFee.add(_fundFee).add(_burnFee);
    ...
    function shouldRebase() internal view returns (bool) {
        ...
        _lastRebasedTime != 0 &&
        block.timestamp <= (_launchTime + 365 days);
    }


    function rebase() internal{
        if ( inSwapAndLiquify ) return;
        uint256 rebaseRate = 39435601;
    ...
    function _transfer(address sender, address recipient, uint256 amount) private returns(bool){
        ...
        if(isMarketPair[recipient]){
            pairBalance = pairBalance.add(finalAmount);
        }else{
    ...
    function swapAndLiquify(uint256 tAmount) private lockTheSwap {
        ...
        uint256 fundAmount = tAmount.mul(_fundFee).div(_totalTax);
        if(fundAmount>0){
             _basicTransfer(address(this), fundWallet, fundAmount);
        }
        uint256 AmountForBNB = tAmount.sub(fundAmount).sub(burnAmount);
