...
contract GimbutisToken is OwnableUpgradeable, ERC20BurnableUpgradeable {
    ...
    function setSilverRatio(uint256 _silverRatio) external onlyAdmin {
        silverRatio = _silverRatio;
		require( _silverRatio >= 100, "GimbutisToken: SiverRatio less than 100" );
    ...
    function buy(address _token, uint256 _amount) external onlyActive {
        ...
        if (_token == usdcAddress) {
            ...
            _buyTokenWithUsdt(msg.sender, _amount);
        } else {
    ...
    function transfer(address to, uint256 amount) public override onlyActive returns (bool) {
        ...
        if( from == adminAddress || from == owner() ) {
    ...
    function getTokenPrice() public view returns (uint256) {
        ...
        // Note: _price of an ounce of silver with decimals
        require( _price > 0, "GimbutisToken: Aggregator price less or equal 0" );
        uint256 _decimals = IAggregatorV3(oracleAddress).decimals();
        return (uint256(_price) * 1e16) * silverRatio / (10 ** _decimals);
    }
    ...
    function _buyTokenWithUsdc(address _sender, uint256 _amount) private {
        ...
        IERC20Upgradeable(usdcAddress).safeTransferFrom(_sender, adminAddress, (_amount / 1e12)); // 2023-06-19
        _transfer(address(this), _sender, _gxagAmount);
    }

    /**
     * @notice Functon to buy gxag tokens with usdt
     * @param _sender sender address
     * @param _amount amount
     */
    function _buyTokenWithUsdt(address _sender, uint256 _amount) private {
    	// Note: _amount in USDT, but getTokenPrice() is the same as for USDC 
        uint256 _gxagAmount = (_amount * 1e18) / getTokenPrice();
		...
		// from _sender to the conract address
        //IERC20(usdtAddress).transfer( address(this), (_amount / 1e12));
        // Fixed:  from _sender to the admin address
        IERC20(usdtAddress).transferFrom( _sender, adminAddress, (_amount / 1e12));  // 2023-06-23
        _transfer(address(this), _sender, _gxagAmount);
    }
    ...
    function _buyToken(
        ...
        IERC20Upgradeable(_token).safeTransferFrom(_sender, adminAddress, _amount);// 2023-06-19
    ...
    //TODO: 
    //function claim() (address _tokenContractAddress, uint256 _amount) external onlyAdmin {
    //	IERC20 _contract = IERC20(_tokenContractAddress); 
    //	_contract.allow
    //	_contract.transferFrom(address(this), adminAddress, _amount);
    //}
}