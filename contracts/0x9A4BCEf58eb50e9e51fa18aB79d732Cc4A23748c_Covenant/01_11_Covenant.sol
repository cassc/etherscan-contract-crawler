...
contract Covenant is Owner, ReentrancyGuard, Pausable {
    ...
    // voter => reward receiver
    mapping(address => address) public rewardReceiver;
    // receiver => voters
    mapping(address => address[]) private rewardForwarders;
    ...
    event Accrued(uint256 indexed id, address indexed voter, address receiver, uint256 indexed period);
    ...
    event SetRewardReceiver(address indexed voter, address indexed receiver);
    ...
    function getRewardForwarders(address user) external view returns(address[] memory){
        return rewardForwarders[user];
    }
    ...
    function createCovenant(
        ...
        covenants[id] = CovenantParams({
            ...
            creator: msg.sender,
            duration: safe48(duration),
            firstPeriod: safe48(firstPeriod)
        });
    ...
    function updatePeriodRewardsMultiple(uint256[] calldata ids, uint256 period) external nonReentrant whenNotPaused {
        uint256 length = ids.length;
        if(length == 0) revert EmptyList();
        for(uint256 i; i < length;){
            _updatePeriodRewards(ids[i], period);
            unchecked{ ++i; }
        }
    }
    ...
    function accrueMultipleVoterRewards(uint256 id, address[] calldata voters) external nonReentrant whenNotPaused {
        uint256 length = voters.length;

        for(uint256 i; i < length;) {
            _accrueAllRewards(id, voters[i]);
            unchecked{ ++i; }
        }
    }

    function claimRewards(uint256 id, address user) external nonReentrant whenNotPaused returns(uint256 amount) {
        address[] memory _list = rewardForwarders[user];
        uint256 length = _list.length;

        // If the user is a reward receiver for voters
        if(length > 0) {
            for(uint256 i; i < length;) {
                _accrueAllRewards(id, _list[i]);
                unchecked{ ++i; }
            }
        } else {
            // Otherwise, the user is considered as a voter himself
            _accrueAllRewards(id, user);
        }

        address token = covenants[id].token;

        amount = accruedAmount[user][token];
        accruedAmount[user][token] = 0;

        if(amount > 0) {
            IERC20(covenants[id].token).safeTransfer(user, amount);
        }

        emit Claimed(token, user, amount);
    }

    function claimRewardSimple(address token, address user) external nonReentrant whenNotPaused returns(uint256 amount) {
        amount = accruedAmount[user][token];
        accruedAmount[user][token] = 0;
    ...
    function _accrueRewards(uint256 id, address voter, uint256 period) internal {
        if(!isAllowedVoter[voter][id]) return;
        ...
        if(gaugeSumBias != 0 && voterBias != 0 && lastUserVote < period) {
            ...
            address receiver = rewardReceiver[voter] != address(0) ? rewardReceiver[voter] : voter;
            
            accruedAmount[receiver][covenants[id].token] += reward;
            claimedAmount[id][period] += reward;

            emit Accrued(id, voter, receiver, period);
        }
    }
    ...
    function setRewardReceiver(address voter, address receiver) external {
        if(msg.sender != voter && msg.sender != owner()) revert Unauthorized();
        if(voter == receiver) revert SameAddress();
        if(voter == address(0)) revert AddressZero();

        address previousReceiver = rewardReceiver[voter];
        if(previousReceiver != address(0)) {
            address[] memory _list = rewardForwarders[previousReceiver];
            uint256 length = _list.length;

            for(uint256 i; i < length;){
                if(_list[i] == voter){
                    if(i != length - 1){
                        rewardForwarders[previousReceiver][i] = _list[length - 1];
                    }
                    rewardForwarders[previousReceiver].pop();
        ...
        rewardReceiver[voter] = receiver;
        rewardForwarders[receiver].push(voter);

        emit SetRewardReceiver(voter, receiver);
    }
