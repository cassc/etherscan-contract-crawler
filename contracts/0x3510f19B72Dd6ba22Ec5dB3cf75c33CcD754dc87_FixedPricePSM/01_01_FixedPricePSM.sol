// File: contracts/peg/IPriceBound.sol
...
// File: contracts/pcv/IPCVDepositBalances.sol
...
// File: contracts/peg/IPegStabilityModule.sol
...
/**
 * @title Chi Peg Stability Module
 * @notice  The Chi PSM is a contract which holds a reserve of assets in order to exchange Chi at $1 of underlying assets with a fee.
 * `mint()` - buy Chi for $1 of underlying tokens
 * `redeem()` - sell Chi back for $1 of the same
 *
...
interface IPegStabilityModule {
    ...
    /// @notice mint `amountChiOut` Chi to address `to` for `amountIn` underlying tokens
    ...
    /// @notice redeem `amountChiIn` Chi for `amountOut` underlying tokens and send to address `to`
    ...
    /// @notice the underlying token exchanged for Chi
    ...
    event Redeem(address to, uint256 amountChiIn, uint256 amountAssetOut);

    /// @notice event emitted when chi gets minted
    event Mint(address to, uint256 amountIn, uint256 amountChiOut);
}
...
abstract contract OracleRef is IOracleRef, CoreRef {
    ...
    /// @dev the inverted peg would be X per CHI
    ...
    /// @dev the peg is defined as CHI per X with X being ETH, dollars, etc
    ...
    function _setDecimalsNormalizerFromToken(address token) internal {
        int256 chiDecimals = 18;
        int256 _decimalsNormalizer = chiDecimals - int256(uint256(IERC20Metadata(token).decimals()));
...
// File: contracts/utils/RateLimited.sol
...
// File: contracts/chi/minter/RateLimitedMinter.sol
...
// File: contracts/peg/PegStabilityModule.sol
...
// File: contracts/peg/PriceBoundPSM.sol
...
/// @notice contract to create a price bound LUSD PSM
/// This contract will allow swaps when the price of LUSD is between 98 cents and 1.02 by default
...
// File: contracts/peg/FixedPricePSM.sol
