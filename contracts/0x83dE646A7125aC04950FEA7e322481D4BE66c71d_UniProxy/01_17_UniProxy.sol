...
contract UniProxy is ReentrancyGuard {
  ...
  ) nonReentrant public onlyAddedPosition(pos) returns (uint256 shares) {
  ...
  function deposit(
    ...
    address to,
    address pos
	) external onlyAddedPosition(pos) returns (uint256 shares) {
		uint256[4] memory minIn;
		minIn[0] = 0;
		minIn[1] = 0;
		minIn[2] = 0;
		minIn[3] = 0;
		return deposit(deposit0, deposit1, to, pos, minIn);
	}
