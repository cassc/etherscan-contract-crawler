...
contract RoyaltiesRegistry is IRoyaltiesProvider, OwnableUpgradeable {
    ...
    function getRoyalties(address token, uint tokenId) override external returns (LibPart.Part[] memory) {
        ...
        //case royaltiesType = 2, royalties domino v2
        if (royaltiesType == 2) {
            return getRoyaltiesDominoV2(token,tokenId);
        }

        //case royaltiesType = 3, royalties domino v1
        if (royaltiesType == 3) {
            return getRoyaltiesDominoV1(token, tokenId);
        }
    ...
    /// @dev tries to get royalties domino-v2 for token and tokenId
    function getRoyaltiesDominoV2(address token, uint tokenId) internal view returns (LibPart.Part[] memory) {
        try RoyaltiesV2(token).getDominoV2Royalties(tokenId) returns (LibPart.Part[] memory result) {
            return result;
        } catch {
            return new LibPart.Part[](0);
        }
    ...
    /// @dev tries to get royalties domino-v1 for token and tokenId
    function getRoyaltiesDominoV1(address token, uint tokenId) internal view returns (LibPart.Part[] memory) {
