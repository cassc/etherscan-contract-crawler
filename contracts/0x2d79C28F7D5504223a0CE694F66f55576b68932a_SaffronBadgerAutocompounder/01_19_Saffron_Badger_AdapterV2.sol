...
contract SaffronBadgerAutocompounder is ReentrancyGuard {
  ...
  function _autocompound(
    ...
    _withdraw_rewards(tokens, amountsToClaim);
  }

  function _withdraw_rewards(address[] calldata tokens, uint256[] calldata claimAmounts) internal {
    ...
    for (uint256 token_idx = 0; token_idx < tokens.length; token_idx++) {
      ...
      if (current_token == B_AURA_BAL) {
        bauraBAL_amount = claimAmounts[token_idx];
      } else if (current_token == B_GRAV_AURA) {
        graviAURA_amount = claimAmounts[token_idx];
      }else if (current_token == B_BAL_AAVE_STABLE) {
        bbbaUSD_amount = claimAmounts[token_idx];
      }
    }
