...
/// @title Burn Ghost Test Genesis Pass
/// @notice This is Burn Ghost's Test Genesis Pass ERC721 NFT contract 
...
contract TestGenesisPass is DefaultOperatorFilterer, ERC721, Ownable {
    ...
    uint256 public reserved = 6;

    /// @notice Flag to pause sale
    ...
    /// @dev Counters library to track token id
    using Counters for Counters.Counter;
    Counters.Counter private _tokenIdCounter;

    constructor(string memory uri) ERC721("Test Genesis Pass", "TGP") {
        baseURI = uri;
    }
    ...
    function safeMint(bytes32[] calldata proof) external {
        ...
        require(salePaused == false, "BGTGP: Sale paused");

        /// Check if user is on the allow list
        bytes32 leaf = keccak256(bytes.concat(keccak256(abi.encode(_msgSender()))));
        require(MerkleProof.verify(proof, root, leaf), "BGTGP: Invalid proof");

        /// Check if user has minted
        require(minted[_msgSender()] == false, "BGTGP: User already minted");

        /// Check balance of supply
        require(_tokenIdCounter.current() < maxSupply - reserved, "BGTGP: Max allow list supply minted"); 
    ...
    function airdrop(address[] calldata to) external onlyOwner {
        /// Check balance of supply
        require(_tokenIdCounter.current() + to.length - 1 < maxSupply, "BGTGP: Airdrop amount exceeds maximum supply"); 
