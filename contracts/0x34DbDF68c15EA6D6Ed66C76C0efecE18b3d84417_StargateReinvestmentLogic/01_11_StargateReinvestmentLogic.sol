...
/*
Note: reward increased per block
*/
contract StargateReinvestmentLogic is Initializable, Ownable, IERC165, IReinvestmentLogic {
    ...
    function initialize(
        ...
        (uint256 poolId_) = abi.decode(data, (uint256));
    ...
    function invest(
        ...
        IStargateLPStaking(platform()).deposit(poolId(), amount);
    }
    ...
    function divest(
        ...
        IStargateLPStaking(platform()).withdraw(poolId(), amount);

        require(
            IERC20Upgradeable(asset()).balanceOf(address(this)) >= amount,
            "contract does not hold asset amount"
        );
    ...
    function emergencyWithdraw() external override onlyLedger returns (uint256) {
        //Claim rewards
        IStargateLPStaking(platform()).deposit(poolId(), 0);
        ...
        IStargateLPStaking(platform()).emergencyWithdraw(poolId());

        uint256 balance = IERC20Upgradeable(asset()).balanceOf(address(this));
        IERC20Upgradeable(asset()).safeTransfer(ledger(), balance);
    ...
    function _calculateGlobalRewards(
        RewardConfig memory reward
    ) internal view returns (RewardConfig memory) {
        uint256 accruedBalance = IERC20Upgradeable(reward.asset).balanceOf(address(this));
    ...
    function totalSupply() public view override returns (uint256) {
        return IStargateLPStaking(platform()).userInfo(poolId(), address(this)).amount;
    }
    ...
    function rewardOf(address user, uint256 currBalance) public view override returns (Reward[] memory) {
        ...
        for (uint256 i = 0; i < rewardLength(); i++) {
            ...
            // NOTE: newRewards only up-to-date when tx happens in Stargate
            newRewards = newRewards + IStargateLPStaking(platform()).pendingStargate(poolId(), address(this));
