...
contract ConstantReturnStaking is Ownable {
    ...
    address public constant TRUSTED_TOKEN_ADDRESS = 0xC931a78C34B9F8a4436f7058CbCf0e91C80ef9e8; //craz token
    ...
    // max pool size
    uint public constant maxPoolSize = 13333333333e18;
    ...
    function unstake(uint amountToWithdraw) external noContractsAllowed {
        ...
        uint stakingEndTime = contractStartTime.add(REWARD_INTERVAL);
        if (holders.length() == 0 && now > stakingEndTime) {
            uint tokenBalanceContract = Token(TRUSTED_TOKEN_ADDRESS).balanceOf(address(this));
            require(Token(TRUSTED_TOKEN_ADDRESS).transfer(address(0x000000000000000000000000000000000000dEaD), tokenBalanceContract), "Could not burn Craze Tokens.");
        }
    }
