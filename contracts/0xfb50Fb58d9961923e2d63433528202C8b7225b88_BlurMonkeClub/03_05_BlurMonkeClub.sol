...
contract BlurMonkeClub is ERC721A, Ownable  {
  uint256 public cost = 0.0015 ether;
  uint256 public maxSupply = 5555;
  uint256 public maxPerWallet = 20;
  uint256 public maxPerTx = 10;
  uint256 public freeMintMax = 1;
  uint256 public freeMintSupply = 5555;

  bool public sale = false;

  constructor(
  ) ERC721A("BlurMonkeClub", "BMC") payable {
  }

  function mintMonke(uint256 _amount) external payable {
    require(sale, "Sale isn't active");
    require(_totalMinted() + _amount <= maxSupply, "No more monkes");
    require(_numberMinted(msg.sender) + _amount <= maxPerWallet, "20 monkes only");
    require(_amount <= maxPerTx, "10 monkes per tx");
