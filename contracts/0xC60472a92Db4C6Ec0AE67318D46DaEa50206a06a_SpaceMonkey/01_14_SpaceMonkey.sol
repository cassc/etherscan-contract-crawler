...
contract SpaceMonkey is ERC721A, Ownable, ReentrancyGuard {
    ...
    ) ERC721A("SpaceMonkey", "SM", _maxBatchSize, _collectionSize) {
        baseURI = initBaseURI;
    ...
    function reserve(address recipient, uint256 amount) external onlyOwner {
        require(recipient != address(0), "SpaceMonkey: zero address");
        require(amount > 0, "SpaceMonkey: invalid amount");
        require(
            totalSupply() + amount <= collectionSize,
            "SpaceMonkey: max supply exceeded"
        );
        require(
            tokensReserved + amount <= reserveAmount,
            "SpaceMonkey: max reserve amount exceeded"
        );
        require(
            amount % maxBatchSize == 0,
            "SpaceMonkey: can only mint a multiple of the maxBatchSize"
        );
    ...
    function mint(uint256 amount) external payable {
        require(status == Status.PublicSale, "SpaceMonkey: Public sale is not active.");
    ...
    function refundIfOver(uint256 price) private {
        require(msg.value >= price, "SpaceMonkey: Need to send more ETH.");
        if (msg.value > price) {
