...
//6. 1 USH = 1.5625 vdUSH in the BPT, and 2.5 vdUSH in the Pool2 token. ~$1 of LP token is worth $1.25 in USH tokens
//7. Pool2 included for easy migration of existing liquidity + enable locking in chains where BPT is not supported
...
contract VotingEscrow is Ownable, ReentrancyGuard {
    ...
    uint public constant MAXTIME = 52 weeks; //max lock 1 year
    ...
    constructor(address tokenA_addr, address tokenB_addr, address tokenC_addr) {
        ...
        tokenB = tokenB_addr;
        tokenC = tokenC_addr;
        bpt_boost_weight = 1.5625 ether;
        pool2_boost_weight = 2.5 ether;
    ...
    function pool2_amount_to_ush_units(uint256 amount) internal view returns (uint256) {
        ...
        //get vault and pool info
        uint256 ush_balance = IERC20(pool2_address).balanceOf(tokenC);
    ...
    function weighted_amount(uint256 bptAmount, uint256 pool2Amount, uint256 tokenAmount) public view returns (uint256) {
        ...
        return (scaled_bpt_amount * bpt_boost_weight) / 1e18 + (scaled_pool2_amount * bpt_boost_weight) / 1e18 + tokenAmount;
    }
    ...
    function _create_lock(uint _valueA, uint _valueB, uint _valueC, uint _unlock_time) internal {
        require(_valueA > 0 || _valueB > 0); // dev: need non-zero value
        ...
        require(_unlock_time <= block.timestamp + MAXTIME, "Voting lock can be 3 years max");
