...
contract ERC20 is Context, IERC20, IERC20Metadata {
    ...
    contract FREEDOKWON is ERC20, Ownable {
        using SafeMath for uint256;
        using Address for address;

        string private    _name = "FREEDOKWON";
        string private  _symbol = "FREEDOKWON";
    ...
    uint256 public _maintenanceFee = 3; 
    
   
    
    uint256 private _previousLiquidityFee;
    uint256 private _previousMaintenanceFee;
  
   address payable maintenanceWallet = payable(0x5c4A05B7E3d6b3FC832C71123733898C7E4c44A8);
    ...
    constructor () ERC20() {
        ...
        _isExcludedFromFee[maintenanceWallet]       = true;
    ...
    function totalFees() public view returns (uint256) {
        return _liquidityFee.add(_maintenanceFee);
    }
    
     function changeLiquidityPercentage(uint256 percentage) public onlyOwner
    {
        require(percentage<=5,"cannot set higher than 5");
        _liquidityFee = percentage;
    }
   
   function changeTaxPercentage(uint256 percentage) public onlyOwner
    {
        require(percentage<=5,"cannot set higher than 5");
        _maintenanceFee = percentage;
    }
    ...
    function changeMaintenanceWallet(address payable wallet) public onlyOwner
    {
        maintenanceWallet = wallet;
    }
    ...
    function removeAllFee() private {
        if (_liquidityFee == 0 && _maintenanceFee == 0 ) {
            return;
        }
    ...
    function swapBack(uint256 tokensToLiquify) internal lockTheSwap {
        ...
        uint256 ethForMaintenance = ethBalance.mul(_maintenanceFee).div(ethFeeFactor);
      
      

        addLiquidity(tokensToLP, ethForLiquidity);

        payable(maintenanceWallet).transfer(ethForMaintenance);
    }
