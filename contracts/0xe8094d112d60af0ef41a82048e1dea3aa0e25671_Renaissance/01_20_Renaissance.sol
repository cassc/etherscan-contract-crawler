 ...
 * @title  Renaissance Limited Editions by Charlesai
 * @notice Celebrated AI artist Charlesai's collection - Renaissance 2.0: The Digital Evolution
 * of Art - is a series of three artworks. The collection introduces us to a world in which the
 * act of experiencing art is at once grand and futuristic. Renaissance 2.0 prophesies the effects
 * of modern day art and tech, and we are invited to consider how the future might look.
 *
 * Features:
 *   - 1/1 and limited edition NFTs
 *   - Auction with "Buy it now" for 1/1
 ...
 */
contract Renaissance is ERC721Base, Withdraw {
    /* ------------------------------------------------------------------------
       S T O R A G E
    ------------------------------------------------------------------------ */
    ...
    function _collectOneOfOne(
        ...
        // then refund the highest bidder, end the auction early,
        // and decrease the active auctions count.
        if (auction.startTime > 0) {
            if (auction.firstBidTime > 0) {
                ...
                // We just set the duration to zero effectively ending the auction
                auction.duration = 0;
            }
        ...
        // Transfer the NFT from Charlesai to the `to` address
     ...
     *
     * Reverts if:
     *  - the edition size exceeds the max allowed value (`MAX_EDITION_SIZE`)
    ...
    function auctionHasEnded(uint256 baseId) external view returns (bool) {
        ...
        return
            (hasStarted && block.timestamp >= auction.firstBidTime + auction.duration) ||
            auction.duration == 0;
    }
