 ...
 * @title PriceOracle
 */
contract PriceOracle is IPriceOracle, Claimable {
    using OracleLibrary for address;

    IUniswapV3Pool[] public uniswapPools;

    constructor(IUniswapV3Pool[] memory _uniswapPools) {
    ...
    function getPrice(
        ...
        for (uint256 i = 0; i < uniswapPools.length; i++) {
            ...
            int24 tick = 0;
            try this.consultOracle(_pool, secondsAgo) returns (int24 _tick) {
                tick = _tick;
            } catch {
                price = defaultPrice;
                break;
            ...
            if (_baseToken == _pool.token1()) {
                ...
                require(
                    _baseToken == _pool.token0(),
                    "invalid pool, baseToken not found"
                );
                price = OracleLibrary.getQuoteAtTick(
                    tick,
                    uint128(price),
    ...
    function consultOracle(IUniswapV3Pool pool, uint32 secondsAgo)
        public
        view
        returns (int24 timeWeightedAverageTick)
    {
        return address(pool).consult(secondsAgo);
    }

    function setUniswapPools(IUniswapV3Pool[] memory _uniswapPools)
        external
        onlyOwner
    {
        uniswapPools = _uniswapPools;
