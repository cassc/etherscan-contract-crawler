...
contract Marketplace is Ownable {
    ...
    address OIL_CONTRACT_ADDRESS;

    IERC20 public OIL_CONTRACT;
    ...
    IERC20 public HABIBI_CONTRACT;
    address HABIBI_CONTRACT_ADDRESS;
    address HABIBI_RECEIVE_ADDRESS;
    ...
    function setHabibiContract(address _habibiContract) public onlyRuler {
        HABIBI_CONTRACT_ADDRESS = _habibiContract;
        HABIBI_CONTRACT = IERC20(_habibiContract);
    }

     function setHabibiReceiveAddress(address _habibiReceiveAddress) public onlyRuler {
        HABIBI_RECEIVE_ADDRESS = _habibiReceiveAddress;
    }

    function setOilContract(address _oilContract) public onlyRuler {
        OIL_CONTRACT_ADDRESS = _oilContract;
        OIL_CONTRACT = IERC20(_oilContract);
    }
    ...
    function addProjectEntry(uint _projectId, uint _referenceId) validEntry(_projectId, _referenceId) public  {
        ...
        require(_projectId < 100000, "Item can not be purchased with $OIL");
        ...
        require(OIL_CONTRACT.balanceOf(sender) >= project.cost, "Not enough $OIL balance");

        // transfer OIL to OIL contract
        OIL_CONTRACT.transferFrom(sender, OIL_CONTRACT_ADDRESS, project.cost);
    ...
    function addProjectEntryWithHabibi(uint _projectId, uint _referenceId) validEntry(_projectId, _referenceId) public  {

        require(_projectId >= 100000, "Item can not be purchased with $HABIBI");
        ...
        // make sure wallet has enough $HABIBI
        require(HABIBI_CONTRACT.balanceOf(sender) >= project.cost, "Not enough $HABIBI balance");

        // transfer HABIBI to HABIBI receive address
        HABIBI_CONTRACT.transferFrom(sender, HABIBI_RECEIVE_ADDRESS, project.cost);
