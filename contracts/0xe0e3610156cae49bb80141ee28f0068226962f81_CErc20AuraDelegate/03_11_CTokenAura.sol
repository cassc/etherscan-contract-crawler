 ...
 * @author Compound
 */
abstract contract CTokenAura is CTokenInterface, Exponential, TokenErrorReporter {
    /**
     ...
     * @param user The address of the user who receive cTokens
     * @return (uint, uint) An error code (0=success, otherwise a failure, see ErrorReporter.sol), and the actual mint amount.
     */
    function mintInternal(uint mintAmount, address user) internal nonReentrant returns (uint, uint) {
        ...
        return mintFresh(msg.sender, user, mintAmount);
    }
     ...
     * @return (uint, uint) An error code (0=success, otherwise a failure, see ErrorReporter.sol), and the actual mint amount.
     */
    function mintFresh(address minter, address user, uint mintAmount) internal returns (uint, uint) {
        ...
        /*
         * We calculate the new total supply of cTokens and user token balance, checking for overflow:
         *  totalSupplyNew = totalSupply + mintTokens
         *  accountTokensNew = accountTokens[user] + mintTokens
         */
        (vars.mathErr, vars.totalSupplyNew) = addUInt(totalSupply, vars.mintTokens);
        require(vars.mathErr == MathError.NO_ERROR, "MINT_NEW_TOTAL_SUPPLY_CALCULATION_FAILED");

        (vars.mathErr, vars.accountTokensNew) = addUInt(accountTokens[user], vars.mintTokens);
        require(vars.mathErr == MathError.NO_ERROR, "MINT_NEW_ACCOUNT_BALANCE_CALCULATION_FAILED");

        /* We write previously calculated values into storage */
        totalSupply = vars.totalSupplyNew;
        accountTokens[user] = vars.accountTokensNew;

        /* We emit a Mint event, and a Transfer event */
        emit Mint(user, vars.actualMintAmount, vars.mintTokens);
        emit Transfer(address(this), user, vars.mintTokens);

        /* We call the defense hook */
        comptroller.mintVerify(address(this), user, vars.actualMintAmount, vars.mintTokens);
     ...
     * @param redeemType The type of redeem 
     ...
     */
    function redeemInternal(uint redeemTokens, uint redeemType) internal nonReentrant returns (uint) {
        uint error = accrueInterest();
        if (error != uint(Error.NO_ERROR)) {
    ...
    function redeemUnderlyingInternal(uint redeemAmount) internal nonReentrant returns (uint) {
        ...
        // redeemFresh emits redeem-specific logs on errors, so we don't need to
        return redeemFresh(msg.sender, 0, redeemAmount, 0);
    }
     ...
     *  redeemType == 2 (restake again)
     */
    function redeemFresh(address payable redeemer, uint redeemTokensIn, uint redeemAmountIn, uint redeemType) internal returns (uint) {
        ...
        if (redeemType == 0) {
            ...
            require(migrator != address(0), "migrator not ready");
            doTransferOut(migrator, vars.redeemAmount);
            IAuraLPMigration(migrator).redeem(redeemer, vars.redeemAmount, redeemType);
        } else {
    ...
    function setMigrator(address payable _migrator) external {
        // Check caller is admin
        require(msg.sender == admin, 'only admin');

        require(_migrator != address(0), 'INV_MIGRATOR');
        migrator = _migrator;
    }
