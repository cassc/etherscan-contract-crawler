...
contract EquityPool is Pausable, AccessControl, ReentrancyGuard {
	...
	// intervalSeconds is the number of milliseconds of the update windows ie 30 days
	uint256 public immutable intervalMilliSeconds;
	// updateWindowSeconds is number of milliseconds the interval of each update ie 7 days
	...
	constructor(
		...
		// we add one day to the current timestamp so that have 1 day to equity 
		// holders before window kicks in
		lastEndOfIntervalTimestamp = block.timestamp.add(86400);
	}
	...
	function withinUpdateWindow() public view returns (bool) {
		// we check if we are behind the first timestamp to start checking
		if (block.timestamp < lastEndOfIntervalTimestamp) {
			return true;
		}
