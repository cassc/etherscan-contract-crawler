...
contract Joke is ERC721A, Ownable {
    ...
    constructor(
        string memory signatureTokenUri,
        string memory silverTokenUri,
        string memory goldTokenUri
    ) ERC721A("ITS JUST A JOKE", "JOKE") {
    ...
    function setMaxSignatureSupply(
    ...
    function _setTokenIdToTokenType(
        uint256 tokenId,
        uint256 tokenType
    ) internal {
        require(
            _exists(tokenId),
            "token type mapping set of nonexistent token"
        );
    ...
    function tokenURI(
        ...
        if (tokenType == signature.id) {
            return
                bytes(signature.tokenUri).length > 0
                    ? string(
                        abi.encodePacked(
                            signature.tokenUri,
            ...
            return
                bytes(silver.tokenUri).length > 0
                    ? string(
                        abi.encodePacked(
                            silver.tokenUri,
                            Strings.toString(_tokenId),
    ...
    function mint(
        ...
        if (_address != owner()) {
            ...
            if (saleType == 1) {
                if (tokenType == 1) {
                    require(
                        mintIndex + _count <= MAX_SIGNATURE,
                        "Total supply exceeded."
                    );
                    mintPrice = signature.price;
                } else if (tokenType == 2) {
                    require(
                        mintIndex + _count <= MAX_SILVER,
                        "Total supply exceeded."
                    );
                    mintPrice = silver.price;
                } else if (tokenType == 3) {
                    require(
                        mintIndex + _count <= MAX_GOLD,
                        "Total supply exceeded."
                    );
