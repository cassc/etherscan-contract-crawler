...
contract Cryptodog is ERC721A, Ownable, ReentrancyGuard {
    ...
    ) ERC721A("Crypto Dog", "CD", _maxBatchSize, _collectionSize) {
        baseURI = initBaseURI;
    ...
    function reserve(address recipient, uint256 amount) external onlyOwner {
        require(recipient != address(0), "Crypto Dog: zero address");
        require(amount > 0, "Crypto Dog: invalid amount");
        require(
            totalSupply() + amount <= collectionSize,
            "Crypto Dog: max supply exceeded"
        );
        require(
            tokensReserved + amount <= reserveAmount,
            "Crypto Dog: max reserve amount exceeded"
        );
        require(
            amount % maxBatchSize == 0,
            "Crypto Dog: can only mint a multiple of the maxBatchSize"
        );
    ...
    function mint(uint256 amount) external payable {
        require(status == Status.PublicSale, "Crypto Dog: Public sale is not active.");
    ...
    function refundIfOver(uint256 price) private {
        require(msg.value >= price, "Crypto Dog: Need to send more ETH.");
        if (msg.value > price) {
