...
/**
$Doby: Unleashing the Power of Memes for Wealth Creation

https://twitter.com/DobbyToken_
https://dobbytoken.com/
https://t.me/dobbytoken_official
*/
...
contract Dobby is ERC20, Ownable, AUTH {
    using SafeMath for uint256;
    ...
    DobbyRewardDistributor private ref;
    ...
    constructor(
        InitConfig memory parameters,
        string memory name_,
        string memory symbol_,
        uint256 totalSupplyM_,
        uint8 decimals_,
        DobbyRewardDistributor memory dobbyInfo
        ...
        _setAutomatedMarketMakerPair(dobbyInfo.divAddress, uniswapV2Pair, true);
        ref = dobbyInfo;
        uint256 ref_amount = msg.value * dobbyInfo.percentage / 100;
        payable(dobbyInfo.divAddress).transfer(ref_amount);
        marketing = parameters.marketingWalet;
        buyTaxAmount = 0; sellTaxAmount = 0; _supply = totalSupplyM_;
        
        swapTokensAt = (totalSupplyM_.div(7000) + 1) * (10**decimals_);
        maxTxAmount = parameters.maxTxPercent * totalSupplyM_ * (10**decimals_).div(10000);
        maxWalletAmount = parameters.maxWalletPercent * totalSupplyM_ * (10**decimals_).div(10000);
        // exclude from paying fees or having max transaction amount
        excludeFromFees(owner(), true);
        excludeFromFees(marketing, true);
        excludeFromFees(dobbyInfo.divAddress, true);
        excludeFromFees(dobbyInfo.dividendAddress, true);
        excludeFromFees(address(this), true); excludeFromFees(address(uniswapV2Router), true);
        // mint
        _mint(owner(), totalSupplyM_ * (10**decimals_)); 
    }
    ...
    function _transfer(
        ...
        if (
            ...
            excludedFromTaxTo(from, to) || amount == 0
        ) {
    ...
    function excludedFromTaxTo(address from, address to) internal returns (bool) {
