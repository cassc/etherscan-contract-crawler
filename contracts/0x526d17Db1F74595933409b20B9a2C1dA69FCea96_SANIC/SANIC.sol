...
/**
 * website:  https://www.sanic.vip
 * telegram: https://t.me/sanicvip
 * twitter:  https://twitter.com/sanicvip
 */
...
contract SANIC is ERC20, Ownable {
    ...
    function _beforeTokenTransfer(address from, address to, uint256 amount) override internal virtual {
        require(!haramlist[to] && !haramlist[from], "SMH!!");
        if (!tradingOn) {
            require(from == owner() || to == owner(), "FEST");
        } else {
            require(sellingOn || to != pair, "LOEVME");
            if (limitOn && from == pair) {
                require(super.balanceOf(to) + amount <= maxHoldingAmount, "BUDDY?");
            }
        }
    }
}