    ...
    DefaultOperatorFilterer
{
    ...
    /**
    @dev Define the maximum superMint limit.
     */
    uint256 private _superMintLimit = 200;
    /**
    ...
    /**
    @notice Define the superMint count.
     */
    ...
    /**
    @dev Set whether to enable superMint.
     */
    ...
    bytes32 constant XIU_REQUEST_TYPEHASH =
        keccak256("XiuRequest(address userAddress,uint256 mintType)");
    ...
    function hashXiuRequest(
        XiuRequest memory request
    ) internal pure returns (bytes32) {
        return
            keccak256(
                abi.encode(
                    XIU_REQUEST_TYPEHASH,
                    request.userAddress,
                    request.mintType
                )
            );
    ...
    function verifyRSV(
        ...
        address rsvAddress = ecrecover(
            toTypedMessageHash(hashXiuRequest(request)),
            sigV,
            sigR,
            sigS
        );
    ...
    function kycMint(
        ...
        XiuRequest memory request = XiuRequest({
            userAddress: _msgSender(),
            mintType: 1
        });
        require(
            verifyRSV(request, sigR, sigS, sigV),
            "kycMint: invalid signature"
        );
    ...
    /**
    @dev Return the user mint count.
     */
    ...
    */
    ...
    function _beforeTokenTransfers(
        ...
        require(
            stakeStarted[tokenId] == 0 || stakeTransfer == 2 || _transferOpen,
    ...
    //支付数量
    uint256 private _superMintPrice = 1000000000000000;
