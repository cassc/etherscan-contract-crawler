...
contract RenderV4 is Ownable, SSt {
    ...
    function getTraitBytes(address _collection, uint8 _layerId, uint8 _traitId, bool _wholeToken) internal view returns(string memory) { 
      ...
      if(_wholeToken) {
        ...
        if(renderers[_collection].addr != address(0)) return string.concat(_dataType,Base64.encode(IGenericRender(renderers[_collection].addr).getTraitData(_layerId, _traitId)));
