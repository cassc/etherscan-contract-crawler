...
/// @title BaseBot
/// @notice Base contract with common inheritable functionalities for bots
abstract contract BaseBot is IBot, AddressProviderService {
    error UnauthorizedBot(address);
    error NotBotManager(address);
    error NotFundManager(address);
    error TaskDoesNotExist(bytes32);

    constructor(address _addressProvider) AddressProviderService(_addressProvider) {}

    /// @notice helper function to check if msg.sender is bot manager
    function _onlyBotManager() internal view {
        if (msg.sender != addressProvider.botManager()) revert NotBotManager(msg.sender);
    }

    /// @notice helper function to check if msg.sender is fund manager
    function _onlyFundManager() internal view {
        if (msg.sender != addressProvider.fundManager()) revert NotFundManager(msg.sender);
    }
}

/// @title GelatoBot
/// @notice Bot that manages interactions with gelato for automation
contract GelatoBot is BaseBot {
    error InvalidAmount();
    error GelatoTaskNotCreated();

    /// @notice address of native ETH
    address private constant ETH = 0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE;

    /// @notice address of the dedicated msg.sender assigned to GelatoBot
    address public immutable dedicatedMsgSender;
    /// @notice address of gelato automate contract
    IAutomate public immutable automate;
    /// @notice address of gelato task treasury
    ITaskTreasuryUpgradable public immutable taskTreasury;

    /// @notice mapping of internal task IDs to gelato's keeper task IDs
    mapping(bytes32 internalId => bytes32 keeperTaskId) taskIds;

    constructor(address _automate, address _opsProxyFactory, address _addressProvider) BaseBot(_addressProvider) {
        automate = IAutomate(_automate);
        (dedicatedMsgSender,) = IOpsProxyFactory(_opsProxyFactory).getProxyOf(address(this));

        taskTreasury = automate.taskTreasury();
    }

    /// @notice Helper to check if an address is the dedicated message sender
    /// @param _bot address of bot
    function isBot(address _bot) public view returns (bool) {
        return _bot == dedicatedMsgSender;
    }

    /// @notice Fund manager function to deposit funds for execution of automations
    ...
    function depositFunds(uint256 _amount) external payable {
        ...
        taskTreasury.depositFunds{value: _amount}(address(this), ETH, _amount);
    }

    /// @notice Fund manager function to withdraw funds from treasury of automations
    /// @param _amount amount to withdraw
    function withdrawFunds(uint256 _amount) external {
        address fundManager = addressProvider.fundManager();
        if (msg.sender != fundManager) revert NotFundManager(msg.sender);

        taskTreasury.withdrawFunds(payable(fundManager), ETH, _amount);
    }
        ...
        external
    {
        ...
        moduleData.args[0] = _resolverModuleArg(
            ...
            abi.encodeCall(this.resolver, (strategy, wallet, subAccount, automationId)) // callData for resolver call
        );
        ...
        if (currentTask == bytes32(0)) revert GelatoTaskNotCreated();
    ...
    function removeTask(bytes32 internalId) external {
        ...
        bytes32 taskId = getTask(internalId);
        taskIds[internalId] = bytes32(0);
        automate.cancelTask(taskId);
    }

    /// @inheritdoc IBot
    function getTask(bytes32 internalId) public view returns (bytes32) {
        if (!isTask(internalId)) revert TaskDoesNotExist(internalId);

        return taskIds[internalId];
    }

    /// @inheritdoc IBot
    function isTask(bytes32 internalId) public view returns (bool) {
        return taskIds[internalId] != bytes32(0);
    }

    /// @notice Hook for gelato's keepers to check if an automation can be executed
    /// @param strategy address of strategy
    /// @param wallet address of wallet
    /// @param subAccount address of sub-account
    /// @param automationId ID of automation
    /// @return canExecute bool indicating if an automation can be executed
    /// @return data encoded call data that must be executed if resolver passes
    function resolver(address strategy, address wallet, address subAccount, bytes32 automationId)
        external
        view
    ...
    /// @notice Hook for gelato's keepers to execute an automation
    /// @param strategy address of strategy
    /// @param wallet address of wallet
    /// @param subAccount address of sub-account
    /// @param automationId ID of automation
    ...
    /// @notice Helper to get the resolver module argument
    /// @param _resolverAddress address of resolver
    /// @param _resolverData data to be passed while checking resolver for execution
    /// @return resolverModuleArg encoded resolver module arg
    ...
    /// @notice Helper to get proxy module arg
    /// @return proxyModuleArg proxy module arg
