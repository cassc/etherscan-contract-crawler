...
contract CatAi_Staking {
    ...
    uint256 private owner_fee = 1000000000000;
    uint256 private max_percent = 5000000000000;
    uint256 private minute_percent = 1902591100;
    ...
    constructor(address token) {
        owner = 0x2D21c97Be38ce222801CB908DE180c773C7Cb3cE;
        _token = IBEP20(token);
    }
    ...
    function userDeposit(
        ...
        users[msg.sender].total_deposit = users[msg.sender].total_deposit.add(
            value
        );
    ...
    function calcReward(address to) public view returns (uint256) {
        ...
        for (uint256 i = 0; i < count; i++) {
            ...
            eachReward = eachReward.mul(period).mul(minute_percent).div(
                percent
            );
            value = value.add(eachReward);
        }
