...
contract PRESALE_LP_LOCK {
    ...
    function lockNFT(uint256 tokenId) external onlyBlobby {
        ERC721(nftContract).transferFrom(msg.sender, address(this), tokenId); // LP token ID is 509989
    }
    ...
    function withdrawNFT(uint256 tokenId) external onlyBlobby {
        ...
        ERC721(nftContract).transferFrom(address(this), msg.sender, tokenId); // LP token ID is 509989
        lockUpEndTime = 0;
    }
