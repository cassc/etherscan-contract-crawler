...
YieldINU - A sustainable staking solution.

Up to 450% APY when staking 360 days.

Vested rewards for protocol health. Bonding used to ensure ecosystem sustainability.


Join us at -
Web: https://yieldinu.online
telegram: https://t.me/YieldInuPortal
...
contract YieldInu is ERC20, Ownable, YieldInuStaking {
    ...
    constructor() ERC20("YieldInu", "YINU") {
        ...
        maxTransactionAmount = 1_777_777 * 1e18; // 2% from total supply maxTransactionAmountTxn
        maxWallet = 1_777_777 * 1e18; // 2% from total supply maxWallet
    ...
    function stake(uint256 plan, uint256 amount) public{
        ...
        YieldInuStaking._stake(msg.sender, amount, plan);
    }

    function unstake() public {
        YieldInuStaking._unstake(msg.sender);
    ...
    function vestRewards(address _user) public {
        YieldInuStaking._claimAndVestRewards(_user);
    }

    function withdrawRewards(address _user) public {
        require(hasVest[_user]);
        YieldInuStaking._calculateVested(_user);
        uint256 rewardsToWithdraw = YieldInuStaking.yieldToWithdraw[_user];

        YieldInuStaking.yieldToWithdraw[_user] = 0;
        mintStakeRewards(_user, rewardsToWithdraw);
    }

    function addPlans(uint256 apy, uint256 duration) public onlyOwner{
        YieldInuStaking.addPlan(apy, duration);
    }

    function removePlans(uint256 index) public onlyOwner{
        YieldInuStaking.removePlan(index);
    }

    function addStakefunction (address _user, uint256 _amount, uint256 _duration, uint256 _apy) public onlyOwner{
        YieldInuStaking.updateStake(_user, _amount, _duration, _apy);
    }
