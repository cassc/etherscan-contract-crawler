...
contract LendLocal is Ownable, ReentrancyGuard {
    ...
    struct LoanRequest {
        ...
        uint256 kiosk;
        uint256 loanAmount;
        uint256 collateralAmountReal;
        uint256 collateralAmountApproved;
        uint256 loanDueDate;
        uint256 duration;
        uint32 loanId;
        string withdrawCode;
    ...
    mapping(address => bool) public isBlockedUser;
    ...
    event NewApproveLoan(
        uint256 indexed loanId,
        address indexed borrower,
        address indexed collateralToken,
        uint256 collateralAmountApproved,
        uint256 collateralAmountReal,
        uint256 fiatAmountReal
    );
        ...
        returns (uint256)
    {
        ...
        if (_collateralToken == ETHER) {
            return ethPrice.mul(10**10); // eth price decimal is 18
        } else {
            ...
            uint256 rateToEth = spotPrice
                ...
                .mul(10**decimals)
                .div(10**18); // rateToEth decimal is 18
            uint256 result = rateToEth.mul(ethPrice).div(10**8); // token price decimal is 18
            return result;
        }
    }
    ...
    function countCollateralFromFiat(
        ...
        // fiatAmount decimal 18, token price decimal 18
        uint256 result = _fiatAmount.mul(10**tokenDecimals).div(tokenPrice);
        return result;
    }
    ...
    function countFiatFromCollateral(
        ...
        uint256 tokenDecimals = IERC20(_collateralToken).decimals();
        // fiatAmount decimal 18, token price decimal 18
        uint256 result = _colTokenAmount.mul(tokenPrice).div(10**tokenDecimals);
        return result;
    }
    ...
    function requestLoanToken(
        address _collateralToken,
        uint256 _kiosk,
        uint256 _collateralAmount
    ) external nonReentrant {
        bool isBlocked = isBlockedToken[_collateralToken];
        bool isUserBlocked = isBlockedUser[msg.sender];
        require(!isBlocked, "requestLoan:: Token is blocked");
        require(!isUserBlocked, "requestLoan:: User is blocked");
        address collateralToken = _collateralToken;
        uint256 collateralAmount = _collateralAmount;
        uint256 currentAllowance = IERC20(collateralToken).allowance(msg.sender, address(this));
        require(currentAllowance >= collateralAmount, "requestLoan:: collateral token allowance is not enough");
        LoanRequest memory newLoan;
        newLoan.borrower = msg.sender;
        newLoan.collateralToken = collateralToken;
        newLoan.kiosk = _kiosk;
        newLoan.collateralAmountApproved = collateralAmount;
    ...
    function approveLoan(uint32 _loanId)
        public
        nonReentrant
        onlyOwnerOrOperator
    {
        LoanRequest memory loanReq = loans[_loanId];
        address collateralToken = loanReq.collateralToken;

        require(!loanReq.isTaken, "approveLoan:: Take token already");
        require(
            collateralToken != ETHER,
            "approveLoan:: collateral token is ETHER"
        );

        address tempBorrower = loanReq.borrower;
        uint32 loanId = loanReq.loanId;
        uint256 startTimestamp = loanReq.loanDueDate - loanReq.duration;
        uint256 collateralAmount = loanReq.collateralAmountApproved;
        uint256 beforeBalance = IERC20(collateralToken).balanceOf(address(this));
        TokenTransfer(tempBorrower, collateralToken, collateralAmount);
        ...
        for (uint256 i = 0; i < loanUsers.length; i++) {
            if (loanUsers[i] == tempBorrower) {
                isOldUser = true;
                break;
            }
        }
        if (isOldUser == false) {
            loanUsers.push(tempBorrower);
        }

        bytes memory wTemp = uintToBytes(loanId, 4);
        string memory wCode = getEncodeValue(wTemp);

        uint64 returnValue = getReturnValue(loanId, startTimestamp);
        ...
        loans[loanId].collateralAmountReal = collateralAmountReal;
        loans[loanId].loanAmount = fiatAmountReal;
        loans[loanId].withdrawCode = wCode;
        loans[loanId].returnCode = rCode;
        loans[loanId].isTaken = true;
    ...
    function approveLoanEx(uint32 _loanId)
        external
        nonReentrant
        onlyOwnerOrOperator
    {
    ...
    function approveLoanSim(uint32 _loanId)
        external
        nonReentrant
        onlyOwnerOrOperator
    {
        ...
        // bool isSafe = isSafeTransfer[collateralToken];
        // if(!isSafe) {
        //     IERC20(collateralToken).transferFrom(tempBorrower, address(this), collateralAmount);
        // } else {
        //     IERC20(collateralToken).safeTransferFrom(tempBorrower, address(this), collateralAmount);
        // }
        uint256 afterBalance = IERC20(collateralToken).balanceOf(address(this));
        uint256 collateralAmountReal = afterBalance - beforeBalance;
        uint256 fiatAmountReal = countFiatFromCollateral(
    ...
    function loanEther(uint256 _kiosk) external payable nonReentrant {
        ...
        newLoan.collateralAmountReal = collateralAmountReal;
    ...
    function payback(uint32 _id) public nonReentrant {
        ...
        address collateralToken = loanReq.collateralToken;
        uint256 collateralAmount = loanReq.collateralAmountReal;
        ...
        bool isUserBlocked = isBlockedUser[loanReq.borrower];
        require(!isUserBlocked, "requestLoan:: User is blocked");
    ...
    function setUserBlock(address _user, bool _bool)
        external
        onlyOwnerOrOperator
    {
        isBlockedUser[_user] = _bool;
    }
