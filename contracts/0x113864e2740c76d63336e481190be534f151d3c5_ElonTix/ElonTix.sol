...
contract ElonTix is ERC20, Ownable {
    ...
    event claimedStuckTokens(address token);
    event disabledTax();

    constructor () ERC20("ElonTix", "ELONTIX") 
    {   
        address router;
        if (block.chainid == 56) {
        ...
        marketingWallet = 0x60e94E8773D587ED70A23537f9C25d33C728291A;

        _isExcludedFromFees[owner()] = true;
        _isExcludedFromFees[address(0xdead)] = true;
        _isExcludedFromFees[address(this)] = true;
        _isExcludedFromFees[marketingWallet] = true; //pinklock
    ...
    function setBuyTax(uint256 buyTax) external onlyOwner {
        require(!taxDisabled, "Already disabled.");
        require(buyTax <= 10, "Buy Tax cannot be more than 10%");
        marketingFeeOnBuy = buyTax;
        emit BuyTaxChanged(buyTax);
    }

    function setSellTax(uint256 sellTax) external onlyOwner {
        require(!taxDisabled, "Already disabled.");
        require(sellTax <= 10, "Sell Tax cannot be more than 10%");
        marketingFeeOnSell = sellTax;
