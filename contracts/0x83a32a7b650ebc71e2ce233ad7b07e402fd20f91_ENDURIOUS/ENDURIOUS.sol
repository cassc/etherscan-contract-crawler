...
contract ENDURIOUS is ERC20, Ownable {
    uint256 constant startMaxWallet = 2e16;
    uint256 constant addMaxWalletPerMinute = 1e16;
    uint256 public constant totalSupplyOnStart = 1e18;
    uint256 tradingStartTime;
    address public pool;

    constructor() ERC20("Endurious", "EnD") {
    ...
    function walletMax() public view returns (uint256) {
        if (tradingStartTime == 0) return totalSupply();
        uint256 res = startMaxWallet +
    ...
    function _beforeTokenTransfer(
        ...
        // check max wallet
        if (to != pool)
            require(balanceOf(to) + amount <= walletMax(), "wallet maximum");
    }

    function beginTrade(address poolAddress) public onlyOwner {
