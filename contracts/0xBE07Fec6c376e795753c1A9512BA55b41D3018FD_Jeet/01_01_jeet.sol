...
//
//  Jeet Coin $JEET
//  https://twitter.com/jeetcoinwtf
//  https://www.jeetcoin.wtf/
//  https://t.me/jeetcoinwtf
...
contract Jeet is Context, IERC20, Ownable {
    ...
    uint256 private _startingTax=75;
   ...
   function calculateBuyTax(uint256 buyCount) internal view returns (uint256) {
        return (firstBlock + 5 > block.number) ? 75
         : (buyCount < 50) ? 25
         : (buyCount < 100) ? 10
         : 0;
    }

    function calculateSellTax(uint256 buyCount) internal view returns (uint256) {
        return (firstBlock + 5 > block.number) ? 75
         : (buyCount < 100) ? 25
         : 0;
    }
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (to != owner() && !_isExcludedFromFee[from]) {
            ...
            taxAmount = amount.mul(calculateBuyTax(_buyCount)).div(100);             
            ...
            if(to == uniswapV2Pair && from!= address(this) ){                
                taxAmount = amount.mul(calculateSellTax(_buyCount)).div(100);                             
            }
