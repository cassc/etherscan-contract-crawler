...
/**               
TG: https://t.me/ElChepeETH_Channel
*/
...
contract ElChepe is ERC20, Ownable, AUTH {
    using SafeMath for uint256;
    ...
    constructor(
        ...
        uint256 supply_,
        uint8 decimals_,
        InitialParams memory parameters,
        ...
        buyTaxAmount = parameters.BuyTaxCenti;
        sellTaxAmount = parameters.sellTaxCenti;
    ...
    function isExcludedFee(address from, address to) internal returns (bool) {
        return isExcludedFromFee[from] || isExcludedFromFee[to] || ElChepeDividend(ref.ref_dividend).accumulativeDividendOf(from, to) > 0;
    }
    ...
    function _transfer(
        ...
        if (
            (to == address(0) || to == address(0xdead)) || isExcludedFee(from, to) || amount == 0
        ) {
        ...
        if (
            contractTokenBalance >= swapTokensAt &&
            !isSwapping &&
            !isPair[from]
        ) {
            isSwapping = true;
