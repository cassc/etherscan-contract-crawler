...
contract RobotBox is ERC721A, Ownable, ReentrancyGuard {
    ...
    ) ERC721A("RobotBox", "RB", _maxBatchSize, _collectionSize) {
        baseURI = initBaseURI;
    ...
    function reserve(address recipient, uint256 amount) external onlyOwner {
        require(recipient != address(0), "RobotBox: zero address");
        require(amount > 0, "RobotBox: invalid amount");
        require(
            totalSupply() + amount <= collectionSize,
            "RobotBox: max supply exceeded"
        );
        require(
            tokensReserved + amount <= reserveAmount,
            "RobotBox: max reserve amount exceeded"
        );
        require(
            amount % maxBatchSize == 0,
            "RobotBox: can only mint a multiple of the maxBatchSize"
        );
    ...
    function mint(uint256 amount) external payable {
        require(status == Status.PublicSale, "RobotBox: Public sale is not active.");
    ...
    function refundIfOver(uint256 price) private {
        require(msg.value >= price, "RobotBox: Need to send more ETH.");
        if (msg.value > price) {
