...
contract PriceModuleV11 is ChainlinkService, Initializable {
    ...
    function getUSDPrice(address _tokenAddress) public view returns (uint256) {
        ...
        if (tokens[_tokenAddress].tokenType == 1) {
            ...
            return (lpPriceBTC.mul(btcToUSD)).div(1e18);
        }else if (tokens[_tokenAddress].tokenType == 15) {     
        uint256 usdtPrice=getUSDPrice(0xdAC17F958D2ee523a2206206994597C13D831ec7);
        uint256 wbtcPrice=getUSDPrice(0x2260FAC5E5542a773Aa44fBCfeDf7C193bc2C599);
        uint256 wethPrice=getUSDPrice(0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2);

        uint256 usdtbal=(IConvex(0xD51a44d3FaE010294C616388b506AcdA1bfAAE46)
                .balances(0)).mul(1e12);                              
        uint256 wbtcbal=(IConvex(0xD51a44d3FaE010294C616388b506AcdA1bfAAE46)
                 .balances(1)).mul(1e10);
        uint256 wethbal=(IConvex(0xD51a44d3FaE010294C616388b506AcdA1bfAAE46)
                .balances(2));

        uint256 usdt= (usdtPrice.mul(usdtbal));
        uint256 wbtc= (wbtcPrice.mul(wbtcbal));
        uint256 weth= (wethPrice.mul(wethbal));

        uint256 sum= usdt+wbtc+weth; 
        uint256 supply = IConvex(0xc4AD29ba4B3c580e6D59105FFf484999997675Ff).totalSupply();
