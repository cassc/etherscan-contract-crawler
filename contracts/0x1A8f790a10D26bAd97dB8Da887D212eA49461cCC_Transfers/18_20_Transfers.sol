...
contract Transfers is Context, Ownable, Pausable, ReentrancyGuard, Sweepable, ITransfers {
    ...
    function swapAndTransferUniswapV3Native(TransferIntent calldata _intent, uint24 poolFeesTier)
        ...
        validIntent(_intent)
    {
        // Since the payer is paying in native currency, we need to pass the wrapped currency address
        // to uniswap. Ensure the wrapped currency is approved to transfer between this contract and uniswap.
        if (wrappedNativeCurrency.allowance(address(this), address(uniswap)) != 0) {
            wrappedNativeCurrency.safeApprove(address(uniswap), 0);
        }

        wrappedNativeCurrency.safeApprove(address(uniswap), type(uint256).max);
