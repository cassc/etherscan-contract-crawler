...
contract YFMBVault is Ownable {
    ...
    uint256 public accYFMBPerShare;
    ...
    constructor(address _LPToken, address _YFMBToken, address _rewardPool, uint256 _dailyReward) Ownable() public {
        LPToken = IERC20(_LPToken);
        YFMBToken = IERC20(_YFMBToken);
    ...
    function getRewardPerBlock() public view returns(uint256) {
        return YFMBToken.balanceOf(rewardPool).div(6500).div(10000).mul(dailyReward);
    }
    ...
    function updatePool() public {
        ...
        YFMBToken.transferFrom(rewardPool, address(this), lucoReward);

        accYFMBPerShare = accYFMBPerShare.add(lucoReward.mul(1e18).div(totalStaked));
        lastRewardBlock = block.number;
    ...
    function stakeLP(uint256 _amount) public {
        ...
        if(user._LPDeposited > 0) {
            uint256 _pendingRewards = user._LPDeposited.mul(accYFMBPerShare).div(1e18).sub(user._rewardDebt);

            if(_pendingRewards > 0) {
                YFMBToken.transfer(_msgSender(), _pendingRewards);
        ...
        user._rewardDebt = user._LPDeposited.mul(accYFMBPerShare).div(1e18);
