...
// File: contracts/BTE_Contracts/BlogToEarnICO.sol
...
contract BlogToEarnICO is Ownable {
    ...
    function invest(uint256 _amount) public returns (bool) {
        ...
        require(
            SafeMath.add(investedAmountOf[msg.sender], totalPrice) <=
                maxInvestment,
        ...
        raisedAmount = SafeMath.add(raisedAmount, totalPrice);
        bteSold = SafeMath.add(bteSold, amount);
        investedAmountOf[msg.sender] = SafeMath.add(
            investedAmountOf[msg.sender],
            totalPrice
        );

        usdt.safeTransferFrom(msg.sender, ICOWallet, totalPrice);
    ...
    function changeBTEPrice(uint256 _newPrice) external onlyOwner {
        require(ICOState == State.HALTED, "ICO isn't on halt yet");
        btePrice = _newPrice;
    }
