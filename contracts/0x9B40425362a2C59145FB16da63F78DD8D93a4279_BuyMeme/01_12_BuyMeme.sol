...
contract BuyMeme {
    ...
    constructor(address _feeClaimer, address _mmtrAddress, address _adminAddress) {
        feeClaimer = _feeClaimer;
        memeAddress = _mmtrAddress;
    ...
    address public immutable memeAddress;
    address public immutable adminAddress;

    mapping(address => uint256) claimRecord;
    ...
    function recoverClaimRecords(ClaimRecord[] memory records) public {
        ...
        for (uint256 i = 0; i < records.length; i++) {
            claimRecord[records[i].user] = records[i].amount;
        }
    }

    function userBuyTokenWithERC20(address erc20, uint256 payAmount) public {
    ...
    function userBuyTokenWithETH() public payable {
        emit Buy(address(0), msg.value);
    }

    function buyerClaimToken() public {
        require(claimRecord[msg.sender] > 0, "No token to claim");
        require(address(memeAddress) != address(0), "Meme address not set");

        uint256 amount = claimRecord[msg.sender];
        IERC20 meme = IERC20(memeAddress);
        meme.safeApprove(address(this), amount * (10 ** 18));
        meme.safeTransferFrom(address(this), msg.sender, amount * (10 ** 18));
        claimRecord[msg.sender] = 0;
    }

    function getPurchasedMeme(address user) public view returns (uint256) {
        return claimRecord[user];
    }

    function getContractERC20Balance(address token) public view returns (uint256) {
