...
abstract contract RWAHub is IRWAHub, ReentrancyGuard, AccessControlEnumerable {
  ...
  address public constant assetRecipient =
    0x6bE395eA63e7F47a41b1d5AFc7D83f5fCd5B54bA;
  ...
  /**
   * @notice Admin function to set the address of the RWA contract
   *
   * @param newRWA The address of the new pricer contract
   */
  function setRWA(address newRWA) external onlyRole(MANAGER_ADMIN) {
    if (IERC20Metadata(newRWA).decimals() != 18) {
      revert RWAIncorrectDecimals();
    }
    address oldRWA = address(rwa);
    rwa = IRWALike(newRWA);
    emit NewRWASet(oldRWA, newRWA);
  }
