...
contract PaymentGateway is Ownable, Pausable {
    ...
    modifier checkSigner(bytes memory _signature, address _to, uint _amount) {
        bytes32 _hash = keccak256(abi.encodePacked(_to, msg.sender, address(this), _amount, _useNonce(msg.sender))).toEthSignedMessageHash();
    ...
    function withdraw(address _token, address _to, uint256 _amount, bytes memory _signature) external checkSigner(_signature, _to, _amount) whenNotPaused{
