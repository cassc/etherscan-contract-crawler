...
interface ISggcNFT {
    struct CollectionInfo {
        string barcode; // 条形码
        uint256 tokenId;
        bool is_delivered; // 是否已经提货
        uint256 delivered_at; // 提货时间
        string bill_no; // 提货单号
        string delivery_proof; // 提货凭证
    }

    event DeliveredCollection(uint256 indexed tokenId, string bill_no, string delivery_proof);
    event BatchMetadataUpdate(uint256 _fromTokenId, uint256 _toTokenId);
    event ContractURIUpdated(string newContractURI);
    event MaxSupplyUpdated(uint256 newMaxSupply);
    event ProvenanceHashUpdated(bytes32 previousHash, bytes32 newHash);

    function deliverCollection(uint256 _tokenId, string memory _bill_no, string memory _delivery_proof) external;

    function mintSggc(address _to, uint256 _tokenId, string memory barcode) external;
    ...
    function getHistoryBaseURIs() external view returns (string[] memory);

    function getCollectionInfo(uint256 _tokenId) external view returns (CollectionInfo memory);
...
contract SggcNFT is ISggcNFT, ERC721Enumerable, Ownable {
    ...
    constructor(string memory _name, string memory _symbol, uint256 _max_supply) ERC721(_name, _symbol){
