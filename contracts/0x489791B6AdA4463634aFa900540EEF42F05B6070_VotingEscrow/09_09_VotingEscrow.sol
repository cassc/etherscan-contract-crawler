...
contract VotingEscrow is Ownable, ReentrancyGuard {
    ...
    address public immutable tokenA; //80/20 USH/unshETH BPT
    address public immutable tokenB; //USH token
    uint public supply;
    ...
    /// @param tokenA_addr USH/unshETH 80/20 BPT
    /// @param tokenB_addr USH token
    constructor(address tokenA_addr, address tokenB_addr) {
        tokenA = tokenA_addr;
        tokenB = tokenB_addr;
    ...
    function bpt_amount_to_tokenA_units(uint256 amount) internal view returns (uint256) {
        ...
        (uint256 tokenA_balance, , , ) = IBPTVault(balancer_vault).getPoolTokenInfo(pool_id, tokenB);
        ...
        uint256 tokenB_units = (amount * tokenA_balance) / total_supply;
        //return the answer as a uint256
        return tokenB_units;
    }

    //  They're instead stored in the Balancer Vault (ugh).  So what you have to do is read the vault address and poolID from the BPT token.  Then you go to the vault and call "getPoolTokenInfo (<poolId>, <ushAddress>)" to get the ush balance in the pool
    ...
    function weighted_amount(uint256 bptAmount, uint256 tokenAmount) public view returns (uint256) {
    ...
    function checkpoint() external notUnlocked {
        _checkpoint(
            address(0x0),
            LockedBalance(int_weighted_amount(0, 0), 0, 0, 0),
            LockedBalance(int_weighted_amount(0, 0), 0, 0, 0)
        );
    }
    ...
    function _deposit_for(
        ...
        _locked.amount = int128(int(weighted_amount(_locked.amountA, _locked.amountB)));
        ...
        _locked.amountA += _value1;
        _locked.amountB += _value2;
        ...
        if (_value1 != 0) {
            IERC20(tokenA).safeTransferFrom(_addr, address(this), _value1);
        }

        if (_value2 != 0) {
            IERC20(tokenB).safeTransferFrom(_addr, address(this), _value2);
        }
    ...
    function _withdraw() internal {
        LockedBalance memory _locked = locked[msg.sender];
        uint256 token_a_balance = _locked.amountA; //TODO: where the fuck are we updating this? in _deposit_for()
