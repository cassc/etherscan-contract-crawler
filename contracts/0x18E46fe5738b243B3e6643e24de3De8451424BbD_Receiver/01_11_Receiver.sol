...
import "./IChildERC20Withdraw.sol";
...
contract Receiver is ReentrancyGuard, Ownable {
    ...
    event PolygonMATICWithdrawal(address initiator, uint256 amount);
    event PolygonETHWithdrawal(address initiator, uint256 amount);
    ...
    function withdrawMATIC() external onlyAuthorizedInitiators onlyOnPolygon {
        ...
        emit PolygonMATICWithdrawal(msg.sender, amount);
    ...
    function withdrawEther() external onlyAuthorizedInitiators onlyOnPolygon {
        IChildERC20Withdraw e;
        // Addresses are PoS-WETH from
        // https://docs.polygon.technology/docs/develop/network-details/mapped-tokens/
        if (block.chainid == 137) { // polygon mainnet
            e = IChildERC20Withdraw(0x7ceB23fD6bC0adD59E62ac25578270cFf1b9f619);
        } else if (block.chainid == 80001) { // polygon mumbai
            e =  IChildERC20Withdraw(0xA6FA4fB5f76172d178d61B04b0ecd319C5d1C0aa);
        } else {
            revert Unsupported();
        }

        uint256 amount = e.balanceOf(address(this));
        emit PolygonETHWithdrawal(msg.sender, amount);
        e.withdraw(amount);
    }
}