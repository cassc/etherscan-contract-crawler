...
import "./interfaces/ICurvePoolMim.sol";
import "./interfaces/IConvexBooster.sol";
import "./interfaces/IConvexReward.sol";
import "./interfaces/IPrice.sol";

contract Mim is OwnableUpgradeable, ISubStrategy {
    using SafeMath for uint256;

    // Sub Strategy name
    string public constant poolName = "MIM V3";
    ...
    */
    function _totalAssets() internal view returns (uint256) {
        ...
        // uint256 assets = ICurvePoolMim(lpToken).calc_withdraw_one_coin(totalLP, tokenId);
    ...
    function _deposit(uint256 _amount) internal returns (uint256) {
        ...
        ICurvePoolMim(curvePool).add_liquidity(lpToken, amounts, minOutput);
    ...
    function withdraw(uint256 _amount) external override onlyController returns (uint256) {
        ...
        // uint256 minAmt = ICurvePoolMim(curvePool).calc_withdraw_one_coin(lpWithdrawn, tokenId);
        ...
        ICurvePoolMim(curvePool).remove_liquidity_one_coin(lpToken, lpWithdrawn, tokenId, minAmt);
    ...
    function harvest() external override onlyController {
        ...
        for (uint256 i = 0; i < rewardTokens.length; i++) {
            ...
            // require(balance > 0, "ZERO_HARVEST_ON_CONVEX_MIM");
            if (balance == 0) return;
    ...
    function withdrawable(uint256 _amount) external view override returns (uint256) {
        ...
        uint256 lpAmt = (totalLP * _amount) / total;
        uint256 expectedOutput = (lpAmt * IPrice(lpToken).get_virtual_price()) / virtualPriceMag;
