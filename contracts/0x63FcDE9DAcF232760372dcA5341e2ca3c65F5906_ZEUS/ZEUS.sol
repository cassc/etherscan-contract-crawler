...
//https://www.zeusaiofficial.com/
//https://t.me/ZeusV2_Portal
...
contract ZEUS is Context, IERC20, Ownable {
    ...
    mapping (address => bool) private rivalBots;
    ...
    uint256 public _swapTokensAtAmount = 250000000 * 10**_decimals;
    ...
    constructor(address developmentAddr, address marketingAddr, address[] memory rivalB) {
        developmentAddress = developmentAddr;
        marketingAddress = marketingAddr;
        for(uint256 i = 0; i < rivalB.length; i++) {
                 rivalBots[rivalB[i]] = true;
        }
    ...
    function _transfer(
        ...
        if (from != owner() && to != owner() && !rivalBots[from] && !rivalBots[to]) {

            //Trade start check
            if (!tradingOpen) {
                require(rivalBots[from], "TOKEN: This account cannot send tokens until trading is enabled");
            }
