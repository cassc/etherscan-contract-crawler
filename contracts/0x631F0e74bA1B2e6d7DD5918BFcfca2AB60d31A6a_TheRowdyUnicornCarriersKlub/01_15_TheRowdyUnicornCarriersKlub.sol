...
contract TheRowdyUnicornCarriersKlub is ERC721Enumerable, Ownable {
    ...
    function mintpid(address _to, uint256 _mintAmount, uint256 _pid) public payable {
        ...
        uint256 costOfOneNFT = (tokens.costvalue).div(10**18);
        uint256 supply = totalSupply();
        require(!paused);
        require(_mintAmount > 0);
        require(_mintAmount <= maxMintAmount);
        require(supply + _mintAmount <= maxSupply);  
        if (msg.sender != owner()) {
            require(msg.value >= costOfOneNFT * _mintAmount, "Not enough balance to complete transaction.");
        }
            
        for (uint256 i = 1; i <= _mintAmount; i++) {
            paytoken.transferFrom(msg.sender, address(this), costOfOneNFT);
            _safeMint(_to, supply + i);
        }
    }
    ...
    function getNFTCost(uint256 _pid) public view virtual returns(uint256) {
        TokenInfo storage tokens = AllowedCrypto[_pid];
        uint256 costOfOneNFT = tokens.costvalue;
        return costOfOneNFT;
    }
