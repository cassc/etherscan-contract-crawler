...
contract Expansion is IExpansion, ERC1155SupplyUpgradeable, AccessControlUpgradeable, HasSecondarySaleFees, Royalty {
    ...
    function initialize(
        ...
        address _artist,
        Royalty memory initialRoyalty,
        address _broadcast,
        IOperatorFilterer.OperatorFiltererInputs memory operatorFiltererInputs,
    ...
    function _getTokenPack(uint256 tokenId) private view returns (Pack memory) {
        ...
        for (uint256 i = 1; i <= numPacksTemp; i++) {
            ...
            for (uint256 j = 0; j < itemsLength; j++) {
                packSize += pack.itemSizes[j];
            }
            uint256 endTokenId = pack.startTokenId + packSize - 1;
            if (tokenId <= endTokenId && tokenId >= pack.startTokenId) {
                return pack;
            }
            if (endTokenId > tokenId) {
