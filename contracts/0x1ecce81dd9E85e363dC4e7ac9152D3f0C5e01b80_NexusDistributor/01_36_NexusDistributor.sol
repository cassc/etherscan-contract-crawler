    ...
    ReentrancyGuardUpgradeable
{
    ...
    // @param maxInForSwap max amount to be spent on token swap
    ...
    function buyCover(
        ...
        if (
            coverAsset != ETH && IERC20(coverAsset).balanceOf(address(this)) > 0
        ) {
    ...
    // @param asset ERC20 paye to buy cover
    // @param amountOut uint256 expected amount out from SDK
    // @param maxInForSwap uint256 max amout for swap with slippage tolerance
    // @param priceWithFee uint256 price with fee
    // @param data Uniswap route data
    function _buyWNXM(
        address asset,
        uint256 amountOut,
        uint256 maxInForSwap,
