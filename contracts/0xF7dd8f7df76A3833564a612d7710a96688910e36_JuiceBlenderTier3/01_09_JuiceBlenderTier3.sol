...
contract JuiceBlenderTier3 is Administration {
    ...
    function mergeTier3(address account, uint256 value) public whenNotPaused {
        ...
        uint256 currentTier = 2;
        uint256 newTier = 3;

        uint256 preBalance = _juice.balanceOf(account, currentTier);
        ...
        unchecked {
            ...
            if (
                (_juice.totalMinted(newTier) + eligibleToMint) >
                (_juice.maxSupply(newTier) - 1)
            ) {
        ...
        unchecked {
            ...
            if (
                _juice.balanceOf(account, currentTier) !=
                preBalance - amountToTransfer
            ) {
