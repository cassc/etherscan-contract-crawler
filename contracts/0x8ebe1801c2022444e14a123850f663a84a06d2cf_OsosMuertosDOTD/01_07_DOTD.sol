...
contract OsosMuertosDOTD is ERC721A, Ownable {
    using Strings for uint256;

   event DotdClaimedForOsos(uint256 ososId);

    mapping (uint256 => bool) public dotdClaimedForOsos;
    ...
    uint256 public maxMintAmountPerTx = 777;
    ...
    constructor() ERC721A("OsosMuertosDOTD", "OSOSDOTD") {
    ...
    modifier mintCompliance(uint256 _mintAmount) {
        ...
        require(
            _nextTokenId() + _mintAmount <= maxSupply + 1,
            "Max supply exceeded!"
        );
        _;
    }
        ...
        public
    {
        ...
        IERC721A token = IERC721A(ososAddress);
        ...
        for (uint256 i = 0; i < ososIds.length; i++) {
          ...
          require(!dotdClaimedForOsos[ososId], "Already claimed!");
          dotdClaimedForOsos[ososId] = true;
          emit DotdClaimedForOsos(ososId);
        }
