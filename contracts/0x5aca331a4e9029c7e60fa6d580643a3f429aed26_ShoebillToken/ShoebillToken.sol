...
contract ShoebillToken is Context, Ownable, ERC20, AccessControl {
    address public deployer = address(0);
    // Role
    bytes32 public constant LOGIC_CONTRACT_ROLE = keccak256("UPDATE_BALANCE");
    constructor(uint256 initialSupply) ERC20("Shoebill", "SHBL") {
        _mint(msg.sender, initialSupply * 10 ** decimals());
    }
    function mint(address account, uint256 amount) public {
        require(hasRole(LOGIC_CONTRACT_ROLE, _msgSender()), "Only Allowed Contract");
        require(totalSupply() + amount <= 8888888880 * (10 ** decimals()), "Maximum total supply exceeded");
