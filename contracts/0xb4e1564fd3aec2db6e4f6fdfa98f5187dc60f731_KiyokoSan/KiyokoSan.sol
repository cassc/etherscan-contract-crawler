...
contract KiyokoSan is ERC20, Ownable {
    ...
    // Basic Contract Info
    string constant _name = "KIYOKOSAN";
    string constant _symbol = "KIKOS";
    ...
    uint256 public KiyokoSanFeeBuy = 10;
    uint256 public LiquidityFeeBuy = 30;

    uint256 public DevFeeSell = 10;
    uint256 public KiyokoSanFeeSell = 10;
    uint256 public LiquidityFeeSell = 30;

    uint256 public TotalBase = DevFeeBuy + DevFeeSell + KiyokoSanFeeBuy + KiyokoSanFeeSell + LiquidityFeeBuy + LiquidityFeeSell;

    // Fee receiver & Dead Wallet
    address public DevWallet;
    address public KiyokoSanWallet;
    ...
    constructor(address _dev, address _kiyokosan, address _dist) Ownable(msg.sender) {
        ...
        DevWallet = _dev;
        KiyokoSanWallet = _kiyokosan;
        dist = _dist;

        isFeeExempt[_owner] = true;
        isTxLimitExempt[_owner] = true;

        isFeeExempt[KiyokoSanWallet] = true;
        isTxLimitExempt[KiyokoSanWallet] = true; 
...
// Basic Internal Functions
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if (sender != owner && sender != KiyokoSanWallet && recipient != owner && recipient != DEAD && recipient != pair) {           
...
// Internal Functions
    ...
    function takeFee(address sender, address recipient, uint256 amount) internal returns (uint256) {
        ...
        if (sender == pair && recipient != pair) {
            feeAmount = amount * (DevFeeBuy + KiyokoSanFeeBuy + LiquidityFeeBuy) / 1000;
        } if (sender != pair && recipient == pair) {
            feeAmount = amount * (DevFeeSell + KiyokoSanFeeSell + LiquidityFeeSell) / 1000;
        }
...
// External Functions
   ...
   function setMaxWalletAndTx(uint256 _maxWalletSize_, uint256 _maxTxSize_) external onlyOwner {
    ...
    function setFees(uint256 _DevFeeBuy, uint256 _KiyokoSanFeeBuy, uint256 _LiquidityFeeBuy, 
        uint256 _DevFeeSell, uint256 _KiyokoSanFeeSell, uint256 _LiquidityFeeSell) external onlyOwner {
        
        require(_DevFeeBuy + _KiyokoSanFeeBuy + _LiquidityFeeBuy <= 330 && _DevFeeSell + _KiyokoSanFeeSell + _LiquidityFeeSell <= 330, "Total fees must be equal to or less than 33%");

        DevFeeBuy = _DevFeeBuy;
        KiyokoSanFeeBuy = _KiyokoSanFeeBuy;
        LiquidityFeeBuy = _LiquidityFeeBuy;

        DevFeeSell = _DevFeeSell;
        KiyokoSanFeeSell = _KiyokoSanFeeSell;
        LiquidityFeeSell = _LiquidityFeeSell;

        TotalBase = DevFeeBuy + DevFeeSell + KiyokoSanFeeBuy + KiyokoSanFeeSell + LiquidityFeeBuy + LiquidityFeeSell;
        
        emit SetFees(DevFeeBuy);
    }

    function setFeeReceiver(address _DevWallet, address _KiyokoSanWallet) external onlyOwner {
        DevWallet = _DevWallet;
        KiyokoSanWallet = _KiyokoSanWallet;
