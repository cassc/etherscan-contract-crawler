...
contract AlloyxVault is IAlloyxVault, AdminUpgradeable {
  ...
  bool useVaultConfig;
  uint256 vaultInflationPerYearForprotocolFee;
  uint256 vaultPermanentStakerProportion;
  uint256 vaultRegularStakerProportion;
  ...
  event SetUseVaultConfig(bool _useVaultConfig);
  ...
  /**
   * @notice Set useVaultConfig
   * @param _useVaultConfig the state of the contract
   */
  function setUseVaultConfig(bool _useVaultConfig) external onlyAdmin  {
    useVaultConfig = _useVaultConfig;
    emit SetUseVaultConfig(_useVaultConfig);
  }

  /**
   * @notice Set vaultInflationPerYearForprotocolFee
   * @param _vaultInflationPerYearForprotocolFee the state of the contract
   */
  function setVaultInflationPerYearForprotocolFee(uint256 _vaultInflationPerYearForprotocolFee) external onlyAdmin {
    vaultInflationPerYearForprotocolFee = _vaultInflationPerYearForprotocolFee;
  }

  /**
   * @notice Set vaultPermanentStakerProportion
   * @param _vaultPermanentStakerProportion the state of the contract
   */
  function setVaultPermanentStakerProportion(uint256 _vaultPermanentStakerProportion) external onlyAdmin {
    vaultPermanentStakerProportion = _vaultPermanentStakerProportion;
  }

  /**
   * @notice Set vaultRegularStakerProportion
   * @param _vaultRegularStakerProportion the state of the contract
   */
  function setVaultRegularStakerProportion(uint256 _vaultRegularStakerProportion) external onlyAdmin {
    vaultRegularStakerProportion = _vaultRegularStakerProportion;
  }
  ...
  function accrueProtocolFee() external override onlyManager {
    ...
    uint256 inflationPerYearForProtocolFee = max(config.getMinimumVaultInflationPerYearForProtocolFee(), vaultInflationPerYearForprotocolFee);
    if (!useVaultConfig) {
      inflationPerYearForProtocolFee = config.getInflationPerYearForProtocolFee();
    }
    uint256 totalTokenToMint = totalSupply.mul(timeSinceLastFee).mul(inflationPerYearForProtocolFee).div(10000).div(ONE_YEAR_IN_SECONDS);
  ...
  /**
   * @notice An Alloy token holder can deposit their tokens and redeem them for USDC via Flux Desk
   * @param _tokenAmount Number of Alloy Tokens
   */
  function withdrawFromAtomicDesk(uint256 _tokenAmount) external isWhitelisted notPaused nonReentrant {
    uint256 amountToWithdraw = alloyxDuraToUsdc(_tokenAmount);
    vaultToken.burn(_tokenAmount, msg.sender);
    uint256 amountUsdc = config.getFluxDesk().redeemUsdc(amountToWithdraw);
    updateUsdcValuesAndGains(0, amountUsdc);
    _transferERC20(config.usdcAddress(), msg.sender, amountUsdc);
  }
   ...
   */
  function getTotalInvestorUsdcValueAndAdditionalGains() private view returns (uint256, uint256, uint256) {
    ...
    if (totalValue > preTotalUsdcValue) {
      ...
      uint256 permanentStakerProportion = vaultPermanentStakerProportion;
      uint256 regularStakerProportion = vaultRegularStakerProportion;
      if (!useVaultConfig) {
        permanentStakerProportion = config.getPermanentStakerProportion();
        regularStakerProportion = config.getRegularStakerProportion();
      }
      uint256 permanentStakerGain = permanentStakerProportion.mul(interest).div(10000);
      uint256 regularStakerGain = regularStakerProportion.mul(interest).div(10000);
   ...
   */
  function transferERC721(address _tokenAddress, address _account, uint256 _tokenId) public onlyAdmin {
   ...
   */
  function performDeposit(Source _source, address _poolAddress, uint256 _tranche, uint256 _amount) public onlyAdmin nonReentrant {
   ...
   */
  function performWithdraw(Source _source, address _poolAddress, uint256 _tokenId, uint256 _amount, WithdrawalStep _step) public onlyAdmin nonReentrant {
