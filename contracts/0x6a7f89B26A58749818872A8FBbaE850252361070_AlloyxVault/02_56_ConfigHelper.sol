...
library ConfigHelper {
  ...
  function alloyxV1TreasuryAddress(AlloyxConfig config) internal view returns (address) {
    return config.getAddress(uint256(ConfigOptions.Addresses.AlloyxV1Treasury));
  }
  ...
  function getMinimumVaultInflationPerYearForProtocolFee(AlloyxConfig config) internal view returns (uint256) {
    uint256 inflationPerYearForProtocolFee = config.getNumber(uint256(ConfigOptions.Numbers.MinimumVaultInflationPerYearForProtocolFee));
    require(inflationPerYearForProtocolFee <= 10000, "inflation per year should be smaller or equal to 10000");
    return inflationPerYearForProtocolFee;
  }
