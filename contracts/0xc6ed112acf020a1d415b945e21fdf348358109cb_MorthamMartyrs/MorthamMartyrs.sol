...
contract MorthamMartyrs is ERC721A, Ownable, ReentrancyGuard, DefaultOperatorFilterer   {


  string public baseURI;
  string public notRevealedUri = "ipfs://bafybeidds7hszxyxskenl2xsgtdnc2cktsx2o7hbnj3a2vz6ttre5gxpii/hidden.json";
  ...
  constructor() ERC721A("Mortham Martyrs", "Mortham") {}
  ...
  function mint(uint256 tokens) public payable nonReentrant {
    require(!paused, "Mortham: oops contract is paused");
    require(!preSale, "Mortham: Sale Hasn't started yet");
    require(tokens <= MaxperWallet, "Mortham: max mint amount per tx exceeded");
    require(totalSupply() + tokens <= maxSupply, "Mortham: We Soldout");
    require(PublicMintofUser[_msgSenderERC721A()] + tokens <= MaxperWallet, "Mortham: Max NFT Per Wallet exceeded");
    require(msg.value >= cost * tokens, "Mortham: insufficient funds");
...
/// @dev presale mint for whitelisted
    ...
    require(!paused, "Mortham: oops contract is paused");
    require(preSale, "Mortham: Presale Hasn't started yet");
    require(MerkleProof.verify(merkleProof, merkleRoot, keccak256(abi.encodePacked(msg.sender))), "Mortham: You are not Whitelisted");
    require(WhitelistedMintofUser[_msgSenderERC721A()] + tokens <= MaxperWallet, "Mortham: Max NFT Per Wallet exceeded");
    require(tokens <= MaxperWallet, "Mortham: max mint per Tx exceeded");
    require(totalSupply() + tokens <= maxSupply, "Mortham: Whitelist MaxSupply exceeded");
    require(msg.value >= cost * tokens, "Mortham: insufficient funds");
