...
contract SOSPEPE is ERC721A, OperatorFilterer, Ownable, ReentrancyGuard {
    ...
    uint256 public teamMintLimit = 99;
    uint256 public frenslistMintCost = 0.001 ether;
    uint256 public publicMintCost = 0.001 ether;
    uint256 public maxFreeFrenslistMintLimit = 5;
    uint256 public maxFrenslistMintLimit = 15;
    uint256 public maxPublicMintLimit = 15;

    bytes32 public merkleRoot;
    string public baseURI;

    constructor(string memory _initBaseURI) ERC721A("SOSPEPE", "SOS") {
   ...
   function setpublicMintCost(uint256 newPrice) external onlyOwner {
    ...
    modifier mintCompliance(uint256 _mintAmount) {
        require(balanceOf(msg.sender) <= 15, "You have already minted 15!");
        require(_mintAmount <= 15, "Max mint per transaction is 15!");
        require(_mintAmount <= 15 - (balanceOf(msg.sender)), "You cannot mint this many tokens.");
    ...
    function frenslistMint(uint256 _mintAmount, bytes32[] calldata proof) public payable mintCompliance(_mintAmount) nonReentrant {
        ...
        if(balanceOf(_msgSender()) > 4) {
            require(msg.value >= _mintAmount * frenslistMintCost, "Insufficient Funds1!");
        } else {
            require(msg.value >= (_mintAmount - 5) * frenslistMintCost, "Insufficient Funds2!");
        }
