...
contract ESMET is ReentrancyGuard, Governable, GovernanceToken {
    ...
    event MetLocked(uint256 tokenId, address account, uint256 amount, uint256 lockPeriod);

    /// Emitted when a position is burned due to unlock or kick
    event MetUnlocked(uint256 tokenId, uint256 amount, uint256 unlocked, uint256 penalty);
    ...
    function _burn(uint256 tokenId_, bool onlyIfExpired_, address _account) private {
        ...
        MET.safeTransfer(_account, _toTransfer);

        emit Transfer(_account, address(0), _boosted);
        emit MetUnlocked(tokenId_, _locked, _toTransfer, _locked - _toTransfer);
    }
