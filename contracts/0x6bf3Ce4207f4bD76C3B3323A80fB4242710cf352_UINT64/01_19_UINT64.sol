...
contract UINT64 is ERC721A, Ownable, ReentrancyGuard, DefaultOperatorFilterer {
  ...
  function fuse(uint64[] memory tokens) public {
        ...
        baseColor[tokens[0]] = randoUINT64(tokens[0], 1, 4);
  }

  function getUINT64(uint64 tokenId) public view returns (uint64) {
        if (!_exists(tokenId)) {
            return 0;
  ...
  function randoUINT64(uint64 input, uint64 min, uint64 max) internal pure returns (uint64) {
