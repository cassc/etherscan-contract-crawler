...
/**
 * @title  HourglassFraxFixedZone
 * @author Hourglass - based on Seaport's Pausable Zone
 * @notice This Seaport derived zone adds additional checks to the validateOrder function
 *         to ensure that the maturity date of the token being traded is less than or equal
 *         to the offerer's defined max maturity timestamp supplied within offerer's `zonehash`.
 *         Other zones will be able to perform different checks as needed.
 *         Unlike Seaport zones, this is not from a clone factory.
 *         AccessControl is utilized in place of `_controller` & `operator`.
 *         Instead of `selfdestruct`, `pause` & `unpause` are used - paused sends all ETH to payee.
 */
contract HourglassFraxFixedZone is AccessControl, ZoneInterface {
    
    /// @notice The role that controls the zone pause state, order cancellation, & operator.    
    bytes32 internal constant CONTROLLER = keccak256("CONTROLLER");

    /// @notice The role with permissions to match orders - fulfillment is not permissioned.
    bytes32 internal constant OPERATOR = keccak256("OPERATOR");

    /// @notice Pauses order filling without selfdestructing
    bool public isPaused;

    /**
     * @notice Set the deployer as the controller of the zone.
     */
    constructor(address _admin, address _controller) {
        // Set roles & role admins
        _setRoleAdmin(CONTROLLER, DEFAULT_ADMIN_ROLE);
        _setRoleAdmin(OPERATOR, CONTROLLER);
        _grantRole(DEFAULT_ADMIN_ROLE, _admin);
        _grantRole(OPERATOR, _controller);
    ...
    modifier notPaused() {
        require(!isPaused, "Zone is paused");
        _;
    }
    ...
    function cancelOrders(
        ...
        OrderComponents[] calldata orders
    ) external  onlyRole(CONTROLLER) returns (bool cancelled) {
     ...
     */
     /// TODO Should we move the eth transfer out of here & use a PAUSER role?
    function pause(address payee) external onlyRole(CONTROLLER) {
        ...
        // withdraw all ETH to payee - selfdestruct is deprecated
        payable(payee).transfer(address(this).balance);
    }

    /**
     * @notice Unpause the contract, allowing orders to utilize the zone.
     */
    function unpause() external onlyRole(CONTROLLER) {
        isPaused = false;
        emit Unpaused();
    }

    ////////// ORDER FILLING & MATCHING //////////
     ...
     *                   successfully fulfilled.
     */
    function executeFulfillAdvancedOrder(
        SeaportInterface seaport,
        AdvancedOrder calldata advancedOrder,
        CriteriaResolver[] calldata criteriaResolvers,
        bytes32 fulfillerConduitKey,
     ...
     *                         this array.
     */
    function executeFulfillAvailableAdvancedOrders(
        SeaportInterface seaport,
        AdvancedOrder[] calldata advancedOrders,
        CriteriaResolver[] calldata criteriaResolvers,
        FulfillmentComponent[][] calldata offerFulfillments,
        ...
        returns (bool[] memory availableOrders, Execution[] memory executions) 
    {
        (availableOrders, executions) = seaport.fulfillAvailableAdvancedOrders(
            advancedOrders,
            criteriaResolvers,
            offerFulfillments,
    ...
    /**
     * @notice Check if a given buy-receipt offer is being filled with valid maturity.
     *
     * @dev This function is called by Seaport whenever any extraData is
     *      provided by the caller or the order type is `Restricted`.
     *
    ...
    {
        // Check that the maturity <= offerer's max maturity timestamp (zonehash)
        require(
            zoneParameters.consideration[0].identifier 
            <= 
            uint256(zoneParameters.zoneHash), 
            "zone: !fixedMaturity"
        );
