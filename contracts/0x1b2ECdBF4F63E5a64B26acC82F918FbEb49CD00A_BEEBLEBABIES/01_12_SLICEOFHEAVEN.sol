...
contract BEEBLEBABIES is ERC721A, Ownable {
    using Strings for uint256;
    uint256 public constant maxSupply = 555;
    uint256 public constant freeMintSupply = 444;
    uint256 public antiBotCost = 0.01 ether;
    uint256 public cost = 0.05 ether;
    uint256 public WLcost = 0.08 ether;
    uint256 public pubCost = 0.2 ether;
    ...
    constructor() ERC721A("BEEBLE BABIES", "BEEBLEBABIES") {
    }

    function mintFreeHolder(uint256 _quantity) external payable {
        uint256 supply = totalSupply();
        holderQ = getHolderWL(msg.sender);
        require(supply + 1 <= freeMintSupply, "Cant go over supply");
        require(holderSaleActive, "HOLDERSALE_INACTIVE");
        require(holderFreeMintAmount[msg.sender] + 1 <= holderQ, "HOLDERSALEFREE_MAXED");
        require(msg.value >= antiBotCost, "INCORRECT_ETH");
        unchecked {
    ...
    function mintPaidHolder(uint256 _quantity) external payable {
        ...
        require(holderSaleActive, "HOLDERSALE_INACTIVE");
        require(holderPaidMintAmount[msg.sender] + 1 <= holderQ*2, "HOLDERSALEPAID_MAXED");
    ...
    function mintPaidWL(uint256 _quantity) public payable {
        require(holderSaleActive, "HOLDERSALE_INACTIVE");
        holderQ = getHolderWL(msg.sender);
        require(holderQ == 0);
        ...
        require(whitelistPaidClaimed[msg.sender] + _quantity <= 4, "WLPAID_MAXED");
        unchecked {
