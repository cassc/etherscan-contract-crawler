...
error SketchCards__OnlyPowerPackContractCanCall();
error SketchCards__MaxSupplyReached();
error SketchCards__PowerPackContractNotSet();
error SketchCards__PackAlreadyOpened();
error SketchCards__InvalidAddress();

contract SketchCards is
  ERC721AQueryable,
  Ownable,
  ReentrancyGuard,
  ERC2981,
  UpdatableOperatorFilterer
{
  ...
  uint256 public constant MAX_SUPPLY = 192;
  ...
  constructor(
    ...
    ERC721A("247 Artist Sketch Editions: Genesis", "ArtSketch1")
    UpdatableOperatorFilterer(
  ...
  function mintToOpenedPack(
    ...
    if (s_powerPackContractAddress == address(0)) {
      revert SketchCards__PowerPackContractNotSet();
    }
    if (msg.sender != s_powerPackContractAddress) {
      revert SketchCards__OnlyPowerPackContractCanCall();
    }
    if (s_packInfo[packId].valid) {
      revert SketchCards__PackAlreadyOpened();
    }
    if (nextTokenId > MAX_SUPPLY) {
      revert SketchCards__MaxSupplyReached();
    }
  ...
  function emergencyMint(address to, uint256 quantity) external onlyOwner {
    if (to == address(0)) {
      revert SketchCards__InvalidAddress();
    }
