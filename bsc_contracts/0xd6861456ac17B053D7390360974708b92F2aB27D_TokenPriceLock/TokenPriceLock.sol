...
contract TokenPriceLock is ITokenToolLock,Ownable {
    ...
    function lock(
        ...
        if (isLpToken) {
            ...
            try IUniswapV2Pair(token).factory() returns (address _factory) {
            possibleFactoryAddress = _factory;
            } catch {
            ...
            "This token is not a LP token."
                ...
                require( unlockPrice > getPrice(token,isLpToken), "Unlock date should be after current time");


            
              id = _lockLpToken(owner,token,possibleFactoryAddress,amount,unlockPrice,true);
            } else {
                require(unlockPrice > getPrice(token,isLpToken), "Unlock date should be after current time" );
                 id = _lockNormalToken(owner, token, amount, unlockPrice,false);
                 }
        safeTransferFromEnsureExactAmount(token, msg.sender, address(this), amount);
        emit LockAdded(id, token, owner, amount, unlockPrice);
        return id;
    }
    ...
    function _lockLpToken(
        ...
        id = _addLock(owner, token, amount, unlockPrice,isLpToken);
    ...
    function _addLock(
        ...
        id = _locks.length;
        PriceLock memory newLock = PriceLock({
            id: id,
            token: token,
            owner: owner,
            amount: amount,
            lockPrice: getPrice(token,isLpToken),
    ...
    function unlock(uint256 lockId) external override validLock(lockId) {
        PriceLock storage userLock = _locks[lockId];
        ...
        if (isLpToken) {
            ...
            require( userLock.unlockPrice > getPrice(userLock.token,true), "Unlock date should be after current time");
            _userLpLockIds[msg.sender].remove(lockId);
        } else {
            require(getPrice(userLock.token,false) >= userLock.unlockPrice, "It is not time to unlock");
    ...
    function editLock(
        ...
        PriceLock storage userLock = _locks[lockId];
        require(userLock.owner == msg.sender, "You are not the owner of this lock");
        require(userLock.amount > 0, "PriceLock was unlocked");
        if (newUnlockPrice > 0) {
            if(userLock.isLpToken){
                            require(
                newUnlockPrice >= userLock.unlockPrice && newUnlockPrice > getPrice(userLock.token,true),
                "New unlock time should not be before old unlock time or current time"
            );

            }else{
                            require(
                newUnlockPrice >= userLock.unlockPrice && newUnlockPrice > getPrice(userLock.token,false),
                "New unlock time should not be before old unlock time or current time"
            );
            }

            userLock.unlockPrice = newUnlockPrice;
        }
    ...
    function allLocks() public view returns (PriceLock[] memory) {
        return _locks;
    }
    ...
    function getLock(uint256 index) public view returns (PriceLock memory) {
        return _locks[index];
    }
    ...
    function lpLocksForUser(address user) public view returns (PriceLock[] memory) {
        uint256 length = _userLpLockIds[user].length();
        PriceLock[] memory userLocks = new PriceLock[](length);
    ...
    function getLocksForToken(
        ...
        uint256 start,
        uint256 end
    ) public view returns (PriceLock[] memory) {
        if (end >= _tokenToLockIds[token].length()) {
            end = _tokenToLockIds[token].length() - 1;
        }
        uint256 length = end - start + 1;
        PriceLock[] memory locks = new PriceLock[](length);
        uint256 currentIndex = 0;
        for (uint256 i = start; i <= end; i++) {
    ...
    function _isValidLpToken(address token, address _factory)
    private
    view
    returns (bool)
    {
        IUniswapV2Pair pair = IUniswapV2Pair(token);
        address factoryPair = IUniswapV2Factory(_factory).getPair(
            pair.token0(),
            pair.token1()
        );
        return factoryPair == token;
    }
    function getPrice(address token,bool isLpToken)public  view  returns (uint256){

        address temptoken;
        address[] memory path=new address[](3);
            if (isLpToken) {
            //获取代币地址
            temptoken = IUniswapV2Pair(token).token0();
                if(temptoken==WETH || temptoken==USDT || temptoken==BUSD){
                temptoken=IUniswapV2Pair(token).token1();
                 }
        ...
        address factoryPair1 = IUniswapV2Factory(factory).getPair(
            temptoken,
            WETH
        );
         uint256  factoryPair1_balance = IERC20(temptoken).balanceOf(factoryPair1);

         address factoryPair2 = IUniswapV2Factory(factory).getPair(
            temptoken,
            USDT
        );
        uint256  factoryPair2_balance = IERC20(temptoken).balanceOf(factoryPair2);


        address factoryPair3 = IUniswapV2Factory(factory).getPair(
            temptoken,
            BUSD
        );
        uint256  factoryPair3_balance = IERC20(temptoken).balanceOf(factoryPair3);
        
        require(factoryPair1_balance>0 || factoryPair2_balance>0 || factoryPair1_balance>0,"Invalid index");
        if(factoryPair1_balance>factoryPair2_balance && factoryPair1_balance>factoryPair3_balance ){
           path[0]=temptoken;
           path[1]=WETH;
           path[2]=USDT;
           uint256[] memory AmountsOuts= IUniswapV2Router02(Router).getAmountsOut(1*10**IERC20MetadataUpgradeable(temptoken).decimals(),path);

        return AmountsOuts[2];

        }else if(factoryPair2_balance>factoryPair3_balance && factoryPair2_balance>factoryPair1_balance){
           path[0]=temptoken;
           path[1]=USDT;
           uint256[] memory AmountsOuts= IUniswapV2Router02(Router).getAmountsOut(1*10**IERC20MetadataUpgradeable(temptoken).decimals(),path);

           return AmountsOuts[1];
        }else if(factoryPair3_balance>=factoryPair2_balance && factoryPair3_balance>factoryPair1_balance){
