...
contract ObiDient is ERC20, Ownable, BaseToken {
    ...
    function purchase(uint256 bnbAmount, address ref) internal returns (bool) {
        ...
        uint256 referralTokens = tokensToSend.mul(bReferralFee).div(bTotalFees);
    ...
    function sell(uint256 tokenAmount) public returns (bool) {
        ...
        super._transfer(
            msg.sender,
            address(this),
            totalFeesTokens.mul(sLiquidityFee).div(sTotalFees)
        );
        if (refs[msg.sender] != address(0)) {
            super._transfer(
                msg.sender,
        ...
        uint256 rewardTokens = totalFeesTokens.mul(sTokenRewardsFee).div(
            sTotalFees
        );
        super._transfer(msg.sender, address(this), rewardTokens);
