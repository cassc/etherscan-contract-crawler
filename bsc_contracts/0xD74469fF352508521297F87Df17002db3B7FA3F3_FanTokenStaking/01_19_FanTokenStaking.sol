...
//     uint32 public constant monthInSeconds = 300; // 5 min
   ...
   */
    ...
    */
    ...
    function calcRewardsFromPeriod(uint256 _period) internal view returns(uint256 totalRewards) {
    ...
    function distributeRewords(uint256 _startPeriod, uint256 _endPeriod) external {
        uint256 totalAmount = getTotalDistributeRewardsFromPeriods(_startPeriod, _endPeriod);
    ...
    function getTotalDistributeRewardsFromPeriods(uint256 _from, uint256 _to) public view
    returns(uint256)
    {
        uint256 totalDistributeAmount = 0;

        for (uint256 _period = _from; _period <= _to; _period++) {
            totalDistributeAmount += calcRewardsFromPeriod(_period);
        }
