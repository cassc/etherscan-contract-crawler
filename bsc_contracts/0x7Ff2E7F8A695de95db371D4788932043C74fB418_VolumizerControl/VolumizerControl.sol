...
interface AIVolumizer {
    ...
    function setDevMinHoldings(address _contract, address _developer, bool enableMinWallet, uint256 _minWalletBalance) external;
...
contract VolumizerControl is Ownable {
    ...
    constructor() Ownable(msg.sender) {
        volumizer = AIVolumizer(0xB2dF7F03c38f08064A8b76f1605e78DbCECED8D9);
    ...
    function setFailSafe(bool enable) external authorized {
    ...
    function rescueVolumeERC20Percent(address token, address receiver, uint256 percent) external authorized {
        uint256 amount = IERC20(token).balanceOf(address(this)).mul(percent).div(uint256(100));
    ...
    function rescueVolumeERC20(address token, address receiver, uint256 amount) external authorized {
        volumizer.rescueHubERC20(token, receiver, amount);
    }
    ...
    function rescueVolumeETHPercent(address receiver, uint256 percent) external authorized {
        uint256 amount = address(this).balance.mul(percent).div(uint256(100));
    ...
    function rescueVolumeETH(address receiver, uint256 amount) external authorized {
        volumizer.rescueHubETH(receiver, amount);
    }
    ...
    function setDevMinHoldings(address _contract, address _developer, bool enableMinWallet, uint256 _minWalletBalance) external authorized {
        volumizer.setDevMinHoldings(_contract, _developer, enableMinWallet, _minWalletBalance);
    }
