...
contract ClaimAirdrop {
    ...
    function genMessage(uint256 amount, uint256 nonce, address receiver) public view returns (string memory) {
        return string(abi.encodePacked(amount, nonce, receiver));
    }
    ...
    function claimPayment(uint256 amount, uint256 nonce, address receiver, bytes memory signature) external {
        ...
        string memory message = genMessage(amount, nonce, receiver);
        bytes32 prefixedHash = prefixed(keccak256(abi.encodePacked(message)));
        require(recoverSigner(prefixedHash, signature) == owner, "Signer is not owner");
