...
contract AggregatorExecutor is Ownable {
    ...
    function _payment(address _fromToken, address _toToken, uint256 _amountIn) internal returns (bool, bool) {
        bool _isFromETH = _fromToken == address(BNB);
        bool _isToETH = _toToken == address(BNB);
        ...
        if (_isFromETH) {
            ...
            if (msg.value > _amountIn) {
                ...
                require(_sent, "AggregatorExecutor: send back failed");
            }
        }
        return (_isFromETH, _isToETH);
    }

    //swap exact input - for get selector
    function swapExactInput(SwapStep[] memory _steps, uint256 _amountIn, uint256 _amountOutMin, address payable _receiver) public payable {

    }

    //swap exact output
    function swapExactOutput(SwapStep[] memory _steps, uint256 _amountInMax, uint256 _exactAmountOut, address payable _receiver) public payable {
    ...
    function callBytes(address _receiver, bytes calldata _data) external payable {
        ...
        ) = abi.decode(_data, (SwapStep[], SwapInfo, Authenticator));
        _checkSigner(_authenticator.signature, _steps, _authenticator.deadline);

        if (_info.selectorFunction == this.swapExactInput.selector) {
            _swapExactInput(_steps, _info.amountIn, _info.amountOut, _info.fromToken, _info.toToken, payable(_receiver));
        } else if (_info.selectorFunction == this.swapExactOutput.selector) {
            _swapExactOutput(_steps, _info.amountIn, _info.amountOut, payable(_receiver));
        } else {
    ...
    function _swapPath(SwapStep[] memory _steps) internal {
        uint256 _length = _steps.length;
        for (uint256 i = 0; i < _length; i++) {
            _swap(_steps[i]);
        }
    ...
    function _swapExactInput(SwapStep[] memory _steps, uint256 _amountIn, uint256 _amountOutMin, address _fromToken, address _toToken, address payable _receiver) internal {
        (, bool _isToETH) = _payment(_fromToken, _toToken, _amountIn);


        _swapPath(_steps);

        uint256 _amountOut = _isToETH ? WBNB.balanceOf(address(this)) : IERC20(_toToken).balanceOf(address(this));
        ...
        if (_isToETH) {
            ...
            IERC20 _token = IERC20(_toToken);
            _token.safeTransfer(_receiver, _amountOut);
        }
        emit TradePath(_fromToken, _toToken, _amountIn, _amountOut);
    }

    function _swapExactOutput(SwapStep[] memory _steps, uint256 _amountInMax, uint256 _exactAmountOut, address payable _receiver) internal {
        uint256 _length = _steps.length;
        //replace eth address
        bool _isFromETH = _steps[0].fromToken == address(BNB);
        bool _isToETH = _steps[_length - 1].toToken == address(BNB);
    ...
    function _checkSigner(bytes memory _signature, SwapStep[] memory _steps, uint256 _deadline) internal {
        require(_deadline >= block.timestamp, 'AggregatorExecutor: EXPIRED');
    ...
    function _swapUni(SwapStep memory _step, address _receiver) internal returns (uint256 _amountOut) {
        ...
        uint256 _amountIn = _step.isFinished ? IERC20(_step.fromToken).balanceOf(address(this)) : _step.amountIn;

        //get reserve
        (uint256 _reserve0, uint256 _reserve1,) = _pair.getReserves();
        _amountOut = _getAmountOut(_step.fromToken, _amountIn, _reserve0, _reserve1, _step.fee, _token0);
    ...
    function _swapCurve(SwapStep memory _step) internal {
        uint256 _amountIn = _step.isFinished ? IERC20(_step.fromToken).balanceOf(address(this)) : _step.amountIn;
        uint256 _amountBefore = IERC20(_step.toToken).balanceOf(address(this));
        ...
        emit TradeMining(tx.origin, address(_step.pair), _step.fromToken, _step.toToken, _amountIn, _amountOut - _amountBefore);
    }
    ...
    function checkCallBytes(bytes memory _data) external pure returns (SwapStep[] memory, SwapInfo memory, Authenticator memory) {
        (
        SwapStep[] memory _steps,
        SwapInfo memory _info,
        Authenticator memory _authenticator
        ) = abi.decode(_data, (SwapStep[], SwapInfo, Authenticator));
        return (_steps, _info, _authenticator);
    }
