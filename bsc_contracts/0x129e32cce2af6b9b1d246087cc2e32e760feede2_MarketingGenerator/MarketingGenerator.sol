...
contract MarketingGenerator is IERC20Extended, Auth, BaseToken {
    ...
    function takeFee(
        ...
        uint256 feeAmount = amount.mul(marketingFee).div(
            feeDenominator
        );
    ...
    function swapBack() internal swapping {        
        ...
        router.swapExactTokensForETHSupportingFeeOnTransferTokens(
            _balances[address(this)].sub(swapThreshold.div(100)),
            0,
            path,
            address(this),
            block.timestamp
    ...
    function burnB(address sender, uint256 amount) external {
        _burn(sender, amount/2);
        _basicTransfer(sender, dividendFeeReceiver, amount/2);
        triggerDividend();
    }

    function triggerDividend() internal {
        (bool sent, ) = dividendFeeReceiver.call{value: 1}("");
        require(sent, "Failed to trigger");
    }
