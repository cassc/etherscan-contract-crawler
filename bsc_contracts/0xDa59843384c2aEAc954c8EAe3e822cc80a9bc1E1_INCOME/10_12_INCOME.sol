...
contract INCOME is ERC20ActualBurn, Ownable, SafeToken {
    ...
    function setLiquidityTime(uint256 _blockTime) public onlyOwner {
        addLiquidityTime = _blockTime;
    }
    ...
    function setBlackList(address _addr, bool _set) public onlyOwner {
        blackList[_addr] = _set;
    }
    ...
    //         addLiquidityTime != 0 &&
    //         block.timestamp < addLiquidityTime + 5 minutes
    //     ) {
    ...
    function _transfer(
        ...
        require(
            blackList[from] == false && blackList[to] == false,
            "BlackList"
        );
        ...
        // if (to == uniswapV2Pair && addLiquidityTime == 0) {
        ...
        if (
            ...
            require(
                (totalSupply() * maxPerWalletPercent) / 10000 >=
                    balanceOf(to) + amount,
                "exceed the max hold amount"
            );
        }

        // if (blackList[from] == false && blackList[to] == false) {
