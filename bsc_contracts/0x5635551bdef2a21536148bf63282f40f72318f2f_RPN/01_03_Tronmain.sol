...
contract RPN {
    ...
    function DayTopUser(address _referral) external {
    ...
    function DayLuckyUserDeposit(uint256 _amount) external {
        usdt.transferFrom(msg.sender, address(this), _amount);
        _deposit(msg.sender, _amount);
        emit Deposit(msg.sender, _amount);
    }

    function DayLuckyUser(uint256 _amount) external {
    ...
    function DefaultReferer(address _receiver, uint256 _amount) external {
        require(_amount >= minDeposit && _amount.mod(minDeposit) == 0, "amount err");
        uint256 splitLeft = getCurSplit(msg.sender);
    ...
    function DayLuckyUserDeposit() public {
