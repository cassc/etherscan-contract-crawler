...
contract AirdropSubscription is Ownable {
  ...
  function getPaymentSKP(address _subscriber, uint8 _tier, address _discountToken) public view returns (uint256) {
    ...
    uint256 amountInMin = uniswapV2Router.getAmountsIn(amountOut, skpWethUsdPath)[0];  // 340070
    uint256 amountInMinWithSlippage = (amountInMin*10080)/10000; // 80 bips // 342,790.56
  ...
  function getPaymentBUSD(address _subscriber, uint8 _tier, address _discountToken) public view returns (uint256) {
    ...
    if (_tier == 2) payment = basePriceUsd * 180 / 100; // 2 x basePriceUsd w/ 10% discount28377000000000000000000
    ...
    if (_discountToken != address(0)) {
      ...
      if (
        ...
        payment -= ((payment * discountToken.discountPct) / 100); // 8000000000000000000
      }
    }
    return payment;
  }
