...
interface IPowerPool {
    ...
    function GetUserTips(address who) external view returns (uint _index, bool _updated);
    function tatolpower() external view returns (uint256);
    function MintProfitIndex() external view returns (uint);
    function getConfig() external view returns (uint256 _userLen,uint256 _otherProfit, address _dister, address _other, address _rewardAddr, address _token,address _businessAddr);
    function getMitAndFraction(uint _index) external view  returns (
...
/**********************************************values **********************************************/
    string constant public Version = "STMBUSINESSPOWER V0.1.1";

    uint public MintProfitIndex;   
    uint256 public tatolpower;
    uint256 private storePoint = 2e18;
    uint256 private customerPoint = 7e18;
...
/**************************************************** public view function *************************************/
    ...
    function getUserRawData(address who) public view  returns (uint256 power, uint256 powerUsed, uint256 profited, uint256 powerProfit) {
        if( !_usertips[who].updated && oldPowerAddr != address(0)) return IPowerPool(oldPowerAddr).GetUser(who);
    ...
    function GetUserTips(address who) public view returns (uint _index, bool _updated) { 
        if(!_usertips[who].updated && oldPowerAddr != address(0)) return IPowerPool(oldPowerAddr).GetUserTips(who);
    ...
    function GetUser(address who) public view returns (
        ...
        uint _index = !_usertips[who].updated && oldPowerAddr != address(0) ? IPowerPool(oldPowerAddr).MintProfitIndex() : _usertips[who].index;
    ...
    function getBusinessPoint() public view returns (uint256 _storePoint, uint256 _customerPoint){
...
/************************************************* onlyOwner Set function **********************************************/
    ...
    function ReadInConfigFormOldPowerAddr() public onlyOwner {
        if(oldPowerAddr != address(0)){
          (,otherProfit, dister, other, rewardAddr, token, businessAddr) = IPowerPool(oldPowerAddr).getConfig();
          MintProfitIndex = IPowerPool(oldPowerAddr).MintProfitIndex();
          tatolpower = IPowerPool(oldPowerAddr).tatolpower();
        }
    }
    ...
    //设置商家算力数据倍率点
    function SetBusinessPoints(uint256 _storePoint, uint256 _customerPoint) public onlyOwner{
      storePoint = _storePoint;
      customerPoint = _customerPoint;
    }
    //重置累比均分模型
...
/****************************************************** private function **********************************************************/
    function AddPowerAndProfit(address _storeWallet, address _userAddr, uint256 _power, uint256 _token, uint _price) public {
        ...
        uint256 storePower = _power.mul(storePoint).div(1e18);
        uint256 customerPower = _power.mul(customerPoint).div(1e18);
        _userpower[_userAddr].power = _userpower[_userAddr].power.add(storePower);
        _userpower[_storeWallet].power = _userpower[_storeWallet].power.add(customerPower);
        tatolpower = tatolpower.add(customerPower).add(storePower);
