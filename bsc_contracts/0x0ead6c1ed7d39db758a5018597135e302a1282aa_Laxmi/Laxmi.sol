...
/**
Hindu Mata Lakshmi is the goddess of divine wealth and power, and tradition holds that carrying her image or likeness in your wallet brings good luck.
The Lakshmi (LAXMI) token brings this tradition to the modern digital world. Hold the token in your wallet for luck and share it with your friends and family.
Join us in believing in the power of Lakshmi!
*/
...
contract Laxmi is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    LaxmiDividendTracker public dividendTracker;
    ...
    constructor() ERC20("Lakshmi", "LAXMI") {
        marketingWallet = payable(0x4e344883f98a6F3f0A543De5f7AA16BaDe40f0f7);
        devWallet = payable(0x91279C222d5373A14662f653c2Bf5FAEf6fd6cF5);
        ...
        dividendTracker = new LaxmiDividendTracker(
            payable(this),
            router,
        ...
        uint256 totalTokenSupply = (444_888_444_888) * (10**18);
        _mint(owner(), totalTokenSupply); // only time internal mint function is ever called is to create supply
        maxWallet = totalTokenSupply / 2000; // No one holder can have more than 5% on default release
        swapTokensAtAmount = totalTokenSupply / 2000; // Once contract wallet is holding 5% of supply then rewards swaps can be triggered
    ...
    function setmaxWallet(uint256 value) external onlyOwner {
        value = value * (10**18);
        require(value >= _totalSupply / 2000, "max wallet cannot be set to less than 5%");
        maxWallet = value;
    }
    ...
    function updateTransferFee(uint256 newTransferFee) public onlyOwner {
        require (newTransferFee <= 6, "transfer fee cannot exceed 6%");
...
contract LaxmiDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor(
        ...
        defaultToken = token;
        LaxmiContract = Laxmi(mainContract);
    ...
    function _transfer(
        ...
        require(false, "Laxmi_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "Laxmi_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main Laxmi contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(LaxmiContract);

            uint256 prevBalance = LaxmiContract.balanceOf(address(this));
            ...
            {
                uint256 received = LaxmiContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    LaxmiContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
