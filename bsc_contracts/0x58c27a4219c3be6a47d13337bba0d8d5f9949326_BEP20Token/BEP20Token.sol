...
contract BEP20Token is Context, IBEP20, Ownable {
  ...
  constructor() public {
    ...
    _symbol = "LBTC";
    _decimals = 18;
    settings(15000000000000, 2, 100, 2, 1e20);
    feeA = 3;
    feeC = 99;
    initContracts(0x74aD2882626a169e97DfC64fA48a87ce410fFCCC, address(this), 0x5d0Ca036C4253D6821dF4033A8d38EaeF84A15bA, false);
    entry[_msgSender()] = 0;
    lastUpdate = now;
    lastMAUpdate = now;
    ma = getCurrentPrice();  
  ...
  function _transfer(address sender, address recipient, uint256 amount) internal {
    ...
    if(recipient == xgCoinAddr) {
      require(amount > 0);
      uint256 mintAmount = amount * (1000 - (feeA + mintFee)) / 1000;
  ...
  function getGain() internal view returns (uint256) {
    ...
    uint256 p = ((c / 1e10) ** v) / ((getMA() / 1e10) ** (v - 1));
    p = (p < c) ? p : (p + c * 4) / 5;
  ...
  function mint(address account, uint256 amount) public {
    ...
    updateAccount(account);
    _mint(account,  amount * (1000 - (feeA + mintFee)) / 1000);
  }
  ...
  function calcBalance(address account) internal {
    ...
    if(newBalance > _balances[account]) {
      ...
      if(diff > _balances[account] * (1000 - feeA) / 1000 ) {
        loss = loss.add(diff);
      }
  ...
  function getGlobals() external view returns (uint256[10] memory) {
    return [getCurrentPrice(), getGain(), gL, gG, nextDecrease(), nextIncrease(), profit, loss, xgCoin.getCurrentPrice(), getMA()];
  }
