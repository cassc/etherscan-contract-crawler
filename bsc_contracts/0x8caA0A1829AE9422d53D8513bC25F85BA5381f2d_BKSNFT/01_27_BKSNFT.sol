...
contract BKSNFT is DefaultOperatorFilterer, ERC721Enumerable, ERC721URIStorage, ERC2981, Ownable {
    ...
    AggregatorV3Interface internal priceFeedForInr;
    ERC20 BUSD;
    ...
    constructor(
        ...
        address chainINRVSUSDAddr
    ) ERC721("BKSNFT", "NFT") {
        owner_ = msg.sender;
        BUSD = ERC20(busdAddr);
        _ORACLE = IStdReference(oracleAddr);
        priceFeed = AggregatorV3Interface(chainlinkAddr);
        priceFeedForInr = AggregatorV3Interface(chainINRVSUSDAddr);
    }
    ...
    function setRoyalties(uint256 tokenId, address recipient, uint256 value) public onlyOwner {
        _setTokenRoyalty(tokenId, recipient, uint96(value));
    }
    ...
    function mintNFT(
        ...
        setRoyalties(newItemId, royaltyAddr, royaltyFee);
    ...
    function getLatestINRPrice() public view returns (int256) {
        (
            ,
            /*uint80 roundID*/
            int256 price, /*uint startedAt*/ /*uint timeStamp*/ /*uint80 answeredInRound*/
        ...
        ) = priceFeedForInr.latestRoundData();
        return price;
    }
    ...
    function payWithBUSD(
        ...
        uint256 usdAmount = eventInfo[eventName].price * _amount * 1000000000000000;
        uint256 eurAmount = uint256(getLatestPrice()) *
            eventInfo[eventName].price *
            _amount *
            10000000;
        uint256 inrAmount = uint256(getLatestINRPrice()) *
            eventInfo[eventName].price *
            _amount *
            10000000;
        uint256 amount = 0;
        if (eventInfo[eventName].currency == 1) {
            amount = usdAmount;
        } else if(eventInfo[eventName].currency == 2) {
            amount = inrAmount;
        } else {
            amount = eurAmount;
        }
