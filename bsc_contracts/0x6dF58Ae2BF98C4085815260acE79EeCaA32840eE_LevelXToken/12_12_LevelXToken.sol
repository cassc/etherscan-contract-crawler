...
contract LevelXToken is Initializable, Ownable, ERC20
{
	...
	function updateContractLevel(address _account, uint256 _level) external onlyOwner
	{
		require(_account.isContract() && _account != address(this), "invalid address");
		_updateEpoch();
		_updateAccount(_account);
		AccountInfo storage _accountInfo = accountInfo[_account];
		int256 _levelBump = _level >= _accountInfo.level ? int256(_level - _accountInfo.level) : -int256(_accountInfo.level - _level);
		emit UpdateContractLevel(_account, _level);
		_postUpdateAccount(_account, _levelBump);
	}
	...
	event UpdateContractLevel(address indexed _account, uint256 _level);
