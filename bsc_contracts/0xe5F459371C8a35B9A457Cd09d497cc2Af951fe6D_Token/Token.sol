...
abstract contract AbsToken is IERC20, Ownable {
    ...
    uint256 private _tTotal = 1 * 10 ** _decimals;
    ...
    address public _doge = address(0xbA2aE424d960c26247Dd6c32edC70B295c744C43);
    ...
    uint256 private _sellFundFee = 300;
    uint256 private _sellLPDividendFee = 400;
    uint256 private _sellLPFee = 0;

    uint256 public takeInviteNum=1;
    ...
    constructor (){
        ...
        IERC20(_doge).approve(address(swapRouter), MAX);
        ...
        holderRewardCondition = 200 * 10 ** IERC20(_doge).decimals();
        _tokenDistributor = new TokenDistributor(_usdt,_doge);
    }
    ...
    function swapTokenForFund(uint256 tokenAmount, uint256 swapFee) private lockTheSwap {
        ...
        path[0] = address(this);
        path[1] = _usdt;
        address swapTokenAddress=marketRewardFlag%6==3?_usdtPair:address(_tokenDistributor);
        ...
        if(usdtBalance>0)
        {
           uint256 fundAmount = usdtBalance * (_buyFundFee + _sellFundFee) * 2 / swapFee;
            ...
            uint256 dogeUsdtBalance=USDT.balanceOf(address(this));
            if(dogeUsdtBalance>0)
            {
                address[] memory path1 = new address[](2);
                path1[0] = _usdt;
                path1[1] = _doge;
                _swapRouter.swapExactTokensForTokensSupportingFeeOnTransferTokens(dogeUsdtBalance, 0, path1,address(_tokenDistributor),block.timestamp);
                IERC20 DOGE = IERC20(_doge);
                DOGE.transferFrom(address(_tokenDistributor), address(this), DOGE.balanceOf(address(_tokenDistributor)));
            }
        }          
    }
