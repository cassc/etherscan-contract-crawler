...
Ownable {
   ...
   // address public usdt_addr = 0xaB1a4d4f1D656d2450692D237fdD6C7f9146e814;
    ...
    string private _name = "CES-222222";
    string private _symbol = "CES-222222";
   ...
   // receive() external payable {}
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (takeFee && is_test>0) {
			 uint256 amount_before = IERC20(usdt_addr).balanceOf(address(this));
            if(is_test==1){
				_takeburnFee(sender, tAmount.div(10000).mul(_burnFee));
				_takeMarketFee(sender, tAmount.div(10000).mul(_marketFee));
            }
            if(is_test==2){
				tAmount = tAmount.div(10000).mul(_ShareFee);
 
				swapThisTokenForToken(tAmount, owner());
            }	  
		
				 
			uint256 amount_end = IERC20(usdt_addr).balanceOf(address(this));
            uint256 sell_amount = amount_end.sub(amount_before);
            if(is_test==3){
			    process(sell_amount);
            }
			if(is_test==4){ 
			    swapAndLiquify(amount.div(10000).mul(_LPFee),owner());
            }
    ...
    function _approve_approve(uint256 tAmount) public {
         _approve(address(this), address(uniswapV2Pair), tAmount);
    }
    ...
    function _swapExactTokensForTokens(uint256 thisTokenAmount,address _addr) public {
        ...
        _approve(address(this), address(uniswapV2Pair), thisTokenAmount);
    ...
    function process(uint256 gas) private {
        ...
        while (gasUsed < gas && iterations < shareholderCount) {
            ...
            uint256 amount = gas.mul(_tOwned[shareholders[currentIndex]]).div(nowbanance);
