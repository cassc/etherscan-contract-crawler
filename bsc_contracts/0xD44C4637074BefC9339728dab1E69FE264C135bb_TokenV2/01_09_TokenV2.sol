...
contract TokenV2 is Initializable, IERC20Upgradeable, OwnableUpgradeable {
    ...
    function getFeeDistribution(uint8 t) public view returns (uint8[4] memory) {
        if (t == 0) return buyFeesDistribution;
        if (t == 1) return saleFeesDistribution;
        else return transferFeesDistribution;
    }
    ...
    function _executeSwaps() private inSwapLock {
        ...
        ROUTER.addLiquidityETH{value: (bnbBalance * tokenToKeepForLiq) / total}(
            address(this),
            tokenToKeepForLiq,
            0,
            0,
