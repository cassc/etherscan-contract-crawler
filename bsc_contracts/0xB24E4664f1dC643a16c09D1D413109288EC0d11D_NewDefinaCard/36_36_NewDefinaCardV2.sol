...
contract NewDefinaCard is ERC721EnumerableUpgradeable, OwnableUpgradeable, NewDefinaCardEventsAndErrors {
    ...
    function freeMultiMint(uint amount_, uint heroId_, uint rarity_) whenPublicSaleActive onlyEOA external {
        ...
        uint rarity = 1;
        uint heroId;
        if(_msgSender() == mergerOperator || _msgSender() == worldCupAddress){
            rarity = rarity_;
        }else{
    ...
    function changeWorldCupAddress(address worldCupAddress_) onlyOwner external {
        worldCupAddress = worldCupAddress_;
    }
