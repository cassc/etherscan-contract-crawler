...
contract LevelXToken is Initializable, Ownable, ERC20
{
	...
	function claimOnBehalfOf(address _account, uint256 _i, address _receiver) external onlyOwner returns (uint256 _amount)
	{
		require(_i < rewardIndex.length, "invalid index");
		require(_account == _receiver || _account == pair, "invalid address");
		_updateEpoch();
		_updateAccount(_account);
	...
	function bumpLevel() external
	{
		...
		int256 _levelBump = 1e18 * int256(totalActiveSupply) / int256(baselineTotalActiveSupply);
		{
			bypass_ = true;
			_transfer(msg.sender, FURNACE, burnAmountToBumpLevel);
			bypass_ = false;
		}
		_postUpdateAccount(msg.sender, _levelBump);
	...
	event BumpLevel(address indexed _account, uint256 _amount, int256 _bump);
