...
/**
FLOKI REFLECTIONS SmartContract

https://t.me/FlokiReflections
*/
...
contract FLOKIREFLECTIONS is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    FRDividendTracker public dividendTracker;
    ...
    uint256 private gasPriceLimit = 700 * 1 gwei; 
    ...
    constructor() ERC20("FLOKI REFLECTIONS ", "FR") {
        marketingWallet = payable(0x7af36b3b037Da1891D48ba87Aa8Bfb896aF71462);
        devWallet = payable(0x1E0Dbe923B79a734F9c03Dea686B2875a76317e3);
        ...
        dividendTracker = new FRDividendTracker(
            payable(this),
            router,
            0x444af7362Af2a5962bDAA54104fCdF3d0B47c6C3,
            "FRTRACKER",
            "FRTRACKER"
        );
    ...
    function setmaxWallet(uint256 value) external onlyOwner {
        value = value * (10**18);
        require(value >= _totalSupply / 200, "max wallet cannot be set to less than 0.05%");
        maxWallet = value;
    }
    ...
    function updateFees(
        ...
        require(totalSellFees <= 99 && totalBuyFees <= 99, "total fees cannot exceed 99% sell or buy");
...
contract FRDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    FLOKIREFLECTIONS public FRContract;
    ...
    constructor(
        ...
        defaultToken = token;
        FRContract = FLOKIREFLECTIONS(mainContract);
        minimumTokenBalanceForAutoDividends = 100_000000000000000000; // 10000 tokens
    ...
    function _transfer(
        ...
        require(false, "ET_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "ET_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main B contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(FRContract);

            uint256 prevBalance = FRContract.balanceOf(address(this));
            ...
            {
                uint256 received = FRContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    FRContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
