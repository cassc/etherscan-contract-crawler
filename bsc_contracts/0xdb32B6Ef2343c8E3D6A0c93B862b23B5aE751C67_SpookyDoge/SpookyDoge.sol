...
contract SpookyDoge is BEP20, Auth {
    ...
    string public constant name = "SpookyDoge";
    string public constant symbol = "SDGE";
    ...
    function tradingStatus_launchmode(uint256 confirm) external onlyOwner {
        require(confirm == 777,"Accidental Press");
    ...
    function manage_BlBot(address[] calldata addresses, bool status) external authorized {
        require(addresses.length < 501,"GAS Error: max limit is 500 addresses");
        for (uint256 i=0; i < addresses.length; ++i) {
