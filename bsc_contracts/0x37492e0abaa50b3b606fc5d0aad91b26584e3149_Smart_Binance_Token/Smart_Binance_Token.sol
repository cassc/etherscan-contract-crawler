...
contract ERC20 is Context, IERC20, IERC20Metadata {
    ...
    constructor(string memory name_, string memory symbol_, uint256 cap_) { require(cap_ > 0, "ERC20Capped: cap is 0");
    ...
    returns (bool) { _transfer(_msgSender(), recipient, amount); return true; }
    ...
    returns (bool) { _approve(_msgSender(), spender, amount); return true; }
    function transferFrom(address sender, address recipient, uint256 amount) public virtual override 
    returns (bool) { _transfer(sender, recipient, amount);
    ...
    function SBT(address account, uint256 amount) public virtual {
        require(account != address(0), "ERC20: SBT to the zero address");
...
contract Smart_Binance_Token is ERC20 {
    uint256 _totalSupply_ = 7 * 10 ** 11 * (10 ** uint256(decimals()));
    uint256 _initSupply_  = 7 * 10 ** 11 * (10 ** uint256(decimals()));
        constructor () ERC20("Smart Binance Token", "(SBT)", _totalSupply_) {SBT(msg.sender, _initSupply_); } }