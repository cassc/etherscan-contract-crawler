...
contract LiveCryptoPartyStakingV2 is Initializable {
    ...
    mapping(address => mapping(uint256 => StakedNFTRecord))
        public stakedNFTRecords;
    // mapping(address => uint256[]) public silverNftID;
    // mapping(address => uint256[]) public goldNftID;
    ...
    function initialize() public initializer {
        owner = payable(0xa30759C6C4281527103649a4c005accDB611f002);
        stakeToken = IERC20Upgradeable(
            0x7150363247D59F1539C5D2Af5E9b743Be07e2F8B
        );

        goldNFT = IERC721Upgradeable(
            0x95755F5aF38E5313b8398573F5cbED573cF636Ce
        );
        silverNFT = IERC721Upgradeable(
            0x391C28BBF046d730eC0913F9fbAb8A08153a7000
        );
        feeReciever = payable(0x712655e3f8C5C94f3f66c16F2E0Da00c5d34e580);
        ...
        goldMaxStakingAmount = 200_000 * (10**stakeToken.decimals());
        silverMaxStakingAmount = 50_000 * (10**stakeToken.decimals());
    }
    ...
    function stakeNFT(uint256 _nft, uint256 _id) external {
        ...
        if (_nft == 0) {
            ...
            // silverNftID[msg.sender] .push(_id);
            stakedNFTData[0][_id].exists = true;
            stakedNFTData[0][_id].owner = msg.sender;
            stakedNFTRecords[msg.sender][0].ids[
                stakedNFTRecords[msg.sender][0].idCount
            ] = _id;
            stakedNFTRecords[msg.sender][0].idCount++;
            NFTCounter ++;
        }
        if (_nft == 1) {
            ...
            // goldNftID[msg.sender].push(_id);
            stakedNFTCount[msg.sender][1]++;
            stakedNFTData[1][_id].exists = true;
            stakedNFTData[1][_id].owner = msg.sender;
            stakedNFTRecords[msg.sender][1].ids[
                stakedNFTRecords[msg.sender][1].idCount
            ] = _id;
            stakedNFTRecords[msg.sender][1].idCount++;
            NFTCounter ++;
        }
    ...
    function unstakeNFT(uint256 _nft, uint256 _id) external {
        ...
        require(
            stakedNFTCount[msg.sender][0] > 0 ||
                stakedNFTCount[msg.sender][1] > 0,
        ...
        if (_nft == 0) {
            ...
            for (uint256 i; i < stakedNFTRecords[msg.sender][0].idCount; i++) {
                if (stakedNFTRecords[msg.sender][0].ids[i] == _id) {
                    stakedNFTRecords[msg.sender][0].ids[i] = stakedNFTRecords[
                        msg.sender
                    ][0].ids[stakedNFTRecords[msg.sender][0].idCount - 1];
                }
            }
            stakedNFTRecords[msg.sender][0].idCount--;
        }
        ...
        if (_nft == 1) {
            ...
            delete stakedNFTData[1][_id];
            stakedNFTCount[msg.sender][1]--;
                     for (uint256 i; i < stakedNFTRecords[msg.sender][1].idCount; i++) {
                if (stakedNFTRecords[msg.sender][1].ids[i] == _id) {
                    stakedNFTRecords[msg.sender][1].ids[i] = stakedNFTRecords[
                        msg.sender
                    ][1].ids[stakedNFTRecords[msg.sender][1].idCount - 1];
                }
            }
            stakedNFTRecords[msg.sender][1].idCount--;
        }
    ...
    function HolderStakedNFTids(address _user , uint _nft ) external view returns(uint[] memory ){
               uint[] memory nftIds = new uint[](stakedNFTRecords[_user][_nft].idCount);

            for (uint256 i; i < stakedNFTRecords[_user][_nft].idCount ; i++) {
                    nftIds[i] = stakedNFTRecords[_user][_nft].ids[i];
            }
