...
contract PresaleBSCV4 is Initializable, ReentrancyGuardUpgradeable, OwnableUpgradeable, PausableUpgradeable {
  ...
  bool public changeLinearStartTimeFlag;
  uint256 public newLinearStartTime;
  uint256 public newLinearPrice;
  uint256 public newIncrement;
  uint256 public capPrice;
  ...
  function calculatePrice(uint256 _amount) public view returns (uint256 totalValue) {
    ...
    if (_amount + totalTokensSold > rounds[0][currentStep]) {
      ...
      if (linearStartTime == 0 || linearStartTime >= block.timestamp) {
        ...
        if (newLinearStartTime != 0 && block.timestamp > newLinearStartTime) {
          uint256 priceStep = (block.timestamp - newLinearStartTime) / (7 * 24 * 60 * 60);
          if (newLinearPrice + (priceStep * newIncrement) > capPrice) {
            USDTAmount = capPrice * _amount;
          } else {
            USDTAmount = (newLinearPrice + (priceStep * newIncrement)) * _amount;
          }
        } else {
   ...
   * @param amount No of tokens to buy
   */
  function buyWithBNB(uint256 amount, uint256 lockup_months) external payable checkSaleState(amount) whenNotPaused nonReentrant returns (bool) {
    uint256 bnbAmount = bnbBuyHelper(amount);
    uint256 usdEq = calculatePrice(amount);
    ...
    emit TokensBought(_msgSender(), amount, address(0), (newBonus), bnbAmount, usdEq, block.timestamp);
    return true;
  }
   ...
   * @param amount No of tokens to buy
   */
  function bnbBuyHelper(uint256 amount) public view returns (uint256 bnbAmount) {
  ...
  function changeLinearStartTime(uint256 _newLinearStartTime, uint256 _newIncrement, uint256 _newLinearPrice, uint256 _capPrice) external {
    require(!changeLinearStartTimeFlag, 'LinearStartTime already changed');
    newLinearStartTime = _newLinearStartTime;
    newIncrement = _newIncrement;
    newLinearPrice = _newLinearPrice;
    capPrice = _capPrice;
    changeLinearStartTimeFlag = true;
  }
}