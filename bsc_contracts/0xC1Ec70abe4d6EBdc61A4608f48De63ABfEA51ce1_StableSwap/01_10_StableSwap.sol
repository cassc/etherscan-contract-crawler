...
contract StableSwap is Ownable, ReentrancyGuard {
    ...
    event PauseGuardianChanged(address _pause_guardian);
    ...
    function initialize(
        ...
        uint256 coinsLength;
        N_COINS = coinsLength = _coins.length;
        isInitialized = true;
        for (uint256 i = 0; i < N_COINS; ++i) {
            ...
            PRECISION_MUL.push(10**(MAX_DECIMAL - coinDecimal));
            RATES.push(PRECISION * PRECISION_MUL[i]);
        }
        // create LP token
        bytes memory bytecode = type(StableSwapLP).creationCode;
        bytes32 salt = keccak256(
            abi.encodePacked(_coins, msg.sender, block.timestamp, block.chainid)
        );
        address lpToken;
        assembly {
        ...
        uint256[] memory initialBalance = new uint256[](coinsLength);
    ...
    function add_liquidity(
        ...
        // Mint pool tokens
        token.mint(_lpTokenReceiver, mint_amount);
    ...
    function exchange(
        ...
        IERC20(jAddress).safeTransfer(receiver, dy);

        emit TokenExchange(receiver, i, dx, j, dy);
    }
    ...
    function apply_new_fee(uint256 new_fee, uint256 new_admin_fee)
        external
        onlyOwner
    {
    ...
    function pause_me() external {
        require (msg.sender == pause_guardian || msg.sender == owner());
            is_paused = true;
    }

    function unpause_me() external {
        require (msg.sender == pause_guardian || msg.sender == owner());
        is_paused = false;
    }

    function checkIsPaused() private view {
        if (is_paused) revert PoolIsPaused();
    }

    function set_pause_guardian (address _pause_guardian) external onlyOwner{
        pause_guardian = _pause_guardian;
        emit PauseGuardianChanged(_pause_guardian);
    }
}
