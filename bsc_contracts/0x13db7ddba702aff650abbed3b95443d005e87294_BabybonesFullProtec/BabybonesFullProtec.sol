...
// File: contracts/BabybonesFullProtec.sol


pragma solidity ^0.8.0;

interface IBabybonesToken {
    function mint(address to, uint256 amount) external;
    function totalSupply() external view returns (uint256);
    function transferUnderlying(address to, uint256 value) external returns (bool);
    function fragmentToBabybones(uint256 value) external view returns (uint256);
    function babybonesToFragment(uint256 babybones) external view returns (uint256);
    function balanceOfUnderlying(address who) external view returns (uint256);
    function burn(uint256 amount) external;
}

contract BabybonesFullProtec is Ownable {
    ...
    IBabybonesToken public babybones;
    ...
    constructor(IBabybonesToken _babybones, uint256 _lockDuration, bool _depositsEnabled) {
        babybones = _babybones;
    ...
    function deposit(uint256 _amount) external {
        ...
        IERC20(address(babybones)).safeTransferFrom(address(msg.sender), address(this), _amount);
        babybones.burn(_amount);
    ...
    function withdraw(uint256 _amount) external {
        ...
        user.amount -= _amount;
        totalStaked -= _amount;
        babybones.mint(address(msg.sender), _amount);
