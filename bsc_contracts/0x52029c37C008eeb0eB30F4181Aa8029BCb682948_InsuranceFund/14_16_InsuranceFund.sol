    ...
    WhitelistManager
{
    ...
    function calculateAmountWithdraw(
        ...
        address _trader,
        uint256 _amount
    ) public view returns (uint256 _busdAmount, uint256 _bonusAmount) {
    ...
    function depositWithBonus(
        ...
        // save gas
        uint256 _memMaximumBUSDBonusAccepted = maximumBUSDBonusAccepted;
        if (_bonusInitialMargin > 0) {
            // save gas
            uint256 _busdBonusUsed = busdBonusBalances[_positionManager][
                _trader
            ];
            uint256 _newBonusBalance = _busdBonusUsed + _bonusInitialMargin;
            if (
                _memMaximumBUSDBonusAccepted != 0 &&
                _newBonusBalance > _memMaximumBUSDBonusAccepted
            ) {
                // if the _busdBonusUsed >= maximumBUSDBonusAccepted, then the _bonusInitialMargin will be 0
                if (_busdBonusUsed >= _memMaximumBUSDBonusAccepted) {
                    _realInitialMargin += _bonusInitialMargin;
                    _bonusInitialMargin = 0;
                } else {
                    // add the difference to the real margin
                    _realInitialMargin +=
                        _newBonusBalance -
                        _memMaximumBUSDBonusAccepted;
                    // only accept the maximum bonus
                    _bonusInitialMargin =
                        _memMaximumBUSDBonusAccepted -
                        _busdBonusUsed;
                }
    ...
    function setManagerAssetMapping(
        address _positionManager,
        address _address
    ) public onlyOwner {
