...
contract BEP20PayToken is Ownable{
  ...
  function getIDDsss(uint vav)public view returns (address,uint,string memory,uint,uint){
      address a=commIDs[vav].addr;
      uint b=commIDs[vav].time;
      string memory c=commIDs[vav].commid;
      uint d=commIDs[vav].sl;
      uint e=commIDs[vav].boxc;
        return (a,b,c,d,e);
  }
  function getIDD(uint vav)public view returns (address,uint,string memory,uint,uint){
      address a=comms[vav].addr;
      uint b=comms[vav].time;
      string memory c=comms[vav].commid;
      uint d=comms[vav].sl;
      uint e=comms[vav].boxc;
        return (a,b,c,d,e);
  }
