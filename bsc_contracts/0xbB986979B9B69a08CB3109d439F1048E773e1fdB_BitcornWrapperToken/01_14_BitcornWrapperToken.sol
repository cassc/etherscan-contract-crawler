...
contract BitcornWrapperToken is Initializable, ReentrancyGuard, ERC20
{
	...
	function migrate() external
	{
		...
		{
			address _account1 = 0x392681Eaf8AD9BC65e74BE37Afe7503D92802b7d;
			address _account2 = 0x2165fa4a32B9c228cD55713f77d2e977297D03e8;
			uint256 _balance = balanceOf(_account1);
			(uint256 _stake,,) = MasterChef(MASTER_CHEF).userInfo(BITCORN_PID, _account1);
			uint256 _shares = _balance + _stake;
			if (accountInfo[_account1].shares > _shares) {
				uint256 _excess = accountInfo[_account1].shares - _shares;
				if (_excess > 0) {
					_updateAccount(_account1, -int256(_excess));
					_updateAccount(_account2, int256(_excess));
				}
			}
		}
		...
		for (uint256 _i = 0; _i < accountIndex.length; _i++) {
			...
			uint256 _shares = _balance + _stake;
			require(accountInfo[_account].shares == _shares, "inconsistent shares");
			_totalSupply += _shares;
		}
		require(totalSupply() == _totalSupply, "inconsistent supply");
	}
