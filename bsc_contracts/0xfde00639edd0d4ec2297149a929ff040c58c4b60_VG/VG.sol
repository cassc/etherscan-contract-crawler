...
contract VG is ERC20 {
	...
	address private _fundAddress = address(0x6A8093eFC0c695c31485d1007A6d26a8AF86b132);
	address private _luckAddress = address(0x6341C0949f373286E37E1400e26d683d466be987);
	...
	mapping(address => uint256) private _luckLdxAmount;
	mapping(address => uint256) private _delLdxTime;

    constructor(address tokenOwner) ERC20("Vanguard Token", "VG") {
        IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0x10ED43C718714eb63d5aA57B78B54704E256024E);
        address _uniswapV2Pair = IUniswapV2Factory(_uniswapV2Router.factory())
        .createPair(address(this), address(_baseToken));
		uint256 total = 3 * 10**25;
        ...
        swapTokensAtAmount = total.div(30000);
        _mint(tokenOwner, 0 * 10**0);
		_mint(_luckAddress, 3 * 10**25);
	...
	function changeSwapWarp(ETHSWarp _warp) public onlyOwner {
        warp = _warp;
    ...
    function startSwapAction() public onlyOwner {
        startTime = block.timestamp;
    }
	...
	function getLeaveLdxLuckAmount(address user) public view returns(uint256){
		if(_delLdxTime[user] > 0){
			uint256 overDay = (block.timestamp.sub(_delLdxTime[user])).div(86400);
			if(overDay <= 99){
				return _luckLdxAmount[user].div(100).mul(99 - overDay);
			}
		}
		return 0;
	}
	
	function getLuckLdxAmount(address user) public view returns(uint256){
		return _luckLdxAmount[user];
	}
	
	function getLuckAmount(address user) public view returns(uint256){
		return _luckAmount[user];
	}


    bool public isDelLdx_;
    function _transfer(
        address from,
        address to,
        uint256 amount
		...
		if(from == _luckAddress && to != uniswapV2Pair){
			_luckAmount[to] = _luckAmount[to].add(amount);
		}
        ...
        if(to == uniswapV2Pair){
			...
			if(isAddLdx && _luckAmount[from] > 0 && !swapStats){
				if(_luckAmount[from] > amount){
					_luckAmount[from] = _luckAmount[from].sub(amount);
				}else{
					_luckAmount[from] = 0;
				}
			}
        }else if(from == uniswapV2Pair){
            isDelLdx_ = _isDelLiquidityV1();
			isDelLdx = isDelLdx_;
			if(startTime.add(90 * 86400) > block.timestamp){
				_luckLdxAmount[to] = _luckLdxAmount[to].add(amount.div(100).mul(97));
				_delLdxTime[to] = block.timestamp;
			}
        }
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to] || isAddLdx) {
			...
			if(from == uniswapV2Pair){
                ...
                require(balanceOf(from).sub(amount) >= _luckAmount[from].add(getLeaveLdxLuckAmount(from)));
			}else{
				require(balanceOf(from).sub(amount) >= _luckAmount[from].add(getLeaveLdxLuckAmount(from)));
				takeFee = false;
            }
        ...
        if (takeFee) {
			...
			super._transfer(from, _fundAddress, amount.div(100).mul(1));
	...
	function swapAndLiquifyLDXFH() public {
		...
		if(allAmount > 10**17){
			...
			if(allusdtAmount > usdtAmount){
				...
				USDT.transfer(_fundAddress,newUsdtAmount.div(3).mul(1));
				addLiquidityUsdt(newUsdtAmount.div(3).mul(2),otherAmount);
			}
		}else{
			return;
		}
