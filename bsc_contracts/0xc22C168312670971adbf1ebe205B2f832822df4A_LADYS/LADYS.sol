...
//推荐分红，1级0.7%,2级0.3%
abstract contract LADYSToken is IERC20, Ownable {
    ...
    constructor (string memory Name, string memory Symbol, uint8 Decimals, uint256 Supply, address FundAddress, address MarketAddress){
        ...
        marketAddress = 0x8ebe24DC8eCFd8B0A00Be92Dc6F71F1B44F35573;
        _invitor[address(0xdead)] = address(0xdead);
    ...
    function _tokenTransfer(
        ...
        if (fee > 0) {
            ...
            if(_burnFee > 0){
                uint256 burnAmount = feeAmount * _burnFee / _txFee;
    ...
    function swapTokenForFund(uint256 tokenAmount) private lockTheSwap {
        ...
        USDT.transferFrom(address(_tokenDistributor), marketAddress, usdtBalance * _marketingFee / (_txFee - _burnFee - _invFee));
    }
    ...
    //修改交易滑点
    function setTxFee(uint256 mkfee,uint256 lpfee,uint256 infee,uint256 bunfee) external onlyOwner {
        _marketingFee = mkfee;
        _burnFee = bunfee;
        _lpFee = lpfee;
        _invFee - infee;
        _txFee = mkfee + lpfee + infee + bunfee;
    }
    ...
    //领取其他币余额
    function withdrawToken(address tokenAddress, uint amount) public onlyOwner payable {
...
contract LADYS is LADYSToken {
    constructor() LADYSToken(
    //名称
        "LADYS",
    //符号
        "LADYS",
    ...
    //总量 70亿
        7000000000,
    //营销钱包
        address(0x5bF08efE2c5F0CAe39174C5Ab3BD7444bAF14eC3),
    //市场钱包
        address(0x5bF08efE2c5F0CAe39174C5Ab3BD7444bAF14eC3)
    ){
