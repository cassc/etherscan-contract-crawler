...
contract FourTest is ERC20, Ownable {
    ...
    uint256 public totalFees = BUSDRewardsFee.add(liquidityFee).add(burnFee).add(ecoSystemFee);
...
constructor(address payable _fundWallet) public ERC20("4TEST Token", "4TEST") {
    ...
    function setBUSDRewardsFee(uint256 value) external onlyOwner{
        ...
        totalFees = BUSDRewardsFee.add(liquidityFee).add(burnFee).add(ecoSystemFee);
    }
