...
contract PepeMints is BoringOwnable, ReentrancyGuard {
    ...
    function addStakesUser(address user, uint _amount) external {
      ...
      stakeNumber[user]++;
      overallStakedToken += _amount;
      emit Stake(user, _amount, stakeNumber[user] - 1, block.timestamp);
    }
    ...
    function calcReward(address _user, uint _stakeIndex) public view returns (uint) {
      ...
      uint multiplier = block.timestamp > stakes[_user][_stakeIndex].lastUpdate ?
        (block.timestamp - stakes[_user][_stakeIndex].lastUpdate) * weiPerSforOnePoint5perDay : 0;
    ...
    function claimRewardsInRange(uint idx0, uint idx1) external nonReentrant {
    ...
    function reinvestInRange(uint idx0, uint idx1, address _referrer) external nonReentrant {
      require(idx0 < stakeNumber[msg.sender], "idx0 is too high for the number of user stakes!");
