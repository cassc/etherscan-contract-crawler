...
contract GameFiBoxERC1155 is ERC721HolderUpgradeable, ERC1155HolderUpgradeable, GameFiTokenERC1155, IGameFiBoxERC1155 {
    ...
    function setupRewardSet(uint256 tokenId, RewardSet[] memory rewardSet) external override onlyOwner {
        ...
        for (uint256 i = 0; i < rewardSet.length; i++) {
            ...
            require(rewardSet[i].reiterations > 0 && rewardSet[i].reiterations <= 50, "GameFiBoxERC1155: wrong reiterations amount");
            require(
                rewardSet[i].standart == TokenStandart.ERC20 ||
                rewardSet[i].standart == TokenStandart.ERC1155,
            ...
            require(rewardSet[i].tokenIds.length > 0 && rewardSet[i].tokenIds.length < 50, "GameFiBoxERC1155: wrong arrays length");
     ...
     */
    function openBox(uint256 tokenId, uint256 boxAmount) external override {
        ...
        for (uint256 k = 0; k < boxAmount; k++) {
            ...
            for (uint256 i = 0; i < _rewardSetByBox[tokenId].length; i++) {
                ...
                for (uint256 j = 0; j < _rewardSetByBox[tokenId][i].reiterations; j++) {
                    ...
                    (RewardSet memory actualizedRewardSet, uint256 higherBoundary) = _actualizeRewardSet(_rewardSetByBox[tokenId][i]);
                    ...
                    for (uint256 l = 0; l < actualizedRewardSet.probabilities.length; l++) {
                        if (randomValue >= lowerBoundary && randomValue < lowerBoundary + actualizedRewardSet.probabilities[l]) {
                        
                            _reward(_msgSender(), actualizedRewardSet, l);
    ...
    function _reward(address target, RewardSet memory rewardSet, uint256 index) internal {
        if (rewardSet.standart == TokenStandart.ERC20) {
            ...
            IERC1155Upgradeable(rewardSet.token).safeTransferFrom(address(this), target, rewardSet.tokenIds[index], rewardSet.amounts[index], "0x");
        } else {
            revert();
        }

        emit BoxReward(_msgSender(), rewardSet.token, rewardSet.tokenIds[index], rewardSet.amounts[index], block.timestamp);
    }

    function _actualizeRewardSet(RewardSet memory originalRewardSet) internal view returns (RewardSet memory actualizedRewardSet, uint256 higherBoundary) {
    ...
    function _pseudoRandom(uint256 salt1, uint256 salt2, uint256 salt3, uint256 salt4, uint256 salt5) internal view returns (uint256) {
        return (
            uint256(
                keccak256(
                    abi.encodePacked(
