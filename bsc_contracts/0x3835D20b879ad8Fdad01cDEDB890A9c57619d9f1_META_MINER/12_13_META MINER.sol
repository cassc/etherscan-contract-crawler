...
contract META_MINER is Ownable, ERC1155Enumerable {
    ...
    function burnBatch(address account, uint256[] memory ids, uint256[] memory values) public override onlyOwner {
     
        uint256 burnvalues = 0;
        for (uint i = 0; i < ids.length; i++) {
            itemInfoes[ids[i]].burnedAmount += values[i];
            burnvalues += values[i];
        }
        _totalSupply -= burnvalues;
