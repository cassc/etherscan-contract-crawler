...
import "../interfaces/ISquidVault.sol";
import "../interfaces/ISquidStrategy.sol";
import "../interfaces/ISquidRegistry.sol";

contract SquidRegistry is ManageableUpgradeable, ISquidRegistry {
    ...
    function _addVault(address _vaultAddress) internal {
        ...
        ISquidVault vault = ISquidVault(_vaultAddress);
        ISquidStrategy strat = _validateVault(vault);
    ...
    function _validateVault(ISquidVault _vault) internal view returns (ISquidStrategy strategy) {
        address vaultAddress = address(_vault);

        try _vault.strategy() returns (ISquidStrategy _strategy) {
            require(ISquidStrategy(_strategy).vault() == vaultAddress, "Vault/Strat Mismatch");
            return ISquidStrategy(_strategy);
        } catch {
            require(false, "Address not a Vault");
        }
    }

    function _collectTokenData(ISquidStrategy _strategy) internal view returns (address[] memory tokens) {
        ...
        )
    {
        ...
        name_ = vault.name();
        strategy_ = ISquidStrategy(vault.strategy());
    ...
    function getStakedVaultsForAddress(address _address) external view returns (VaultInfo[] memory stakedVaults) {
        ...
        for (uint256 vid; vid < _vaultSet.length(); vid++) {
            if (ISquidVault(_vaultSet.at(vid)).balanceOf(_address) > 0) {
                numResults++;
            }
        }
