...
contract SyrupBar is BEP20("SyrupBar Token", "SYRUP") {
    ...
    constructor(IBEP20 _kswap) public {
        kswap = _kswap;
    }

    // Safe kswap transfer function, just in case if rounding error causes pool to not have enough KSWAPs.
    function safeKSwapTransfer(address _to, uint256 _amount) public onlyOwner {
        uint256 kswapBal = kswap.balanceOf(address(this));
        if (_amount > kswapBal) {
            kswap.transfer(_to, kswapBal);
        } else {
            kswap.transfer(_to, _amount);
        }
    }
    ...
    function _delegate(address delegator, address delegatee) internal {
        ...
        uint256 delegatorBalance = balanceOf(delegator); // balance of underlying KSWAPs (not scaled);
