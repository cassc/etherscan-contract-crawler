...
contract OPTC is ERC20, Ownable {
    ...
    address public usdt;
    mapping(address => bool) private wContract;
    uint[]  FeeRate;
    ...
    constructor() ERC20('TEST Farming Games Association', 'TEST OPTC'){
...
//        wContract[pair] = true;
    ...
    function updatePrice(uint price) internal {
        if (block.timestamp - lastPriceChange >= 86400 || lastPrice == 0) {
            lastPrice = price;
            lastPriceChange = block.timestamp - (block.timestamp % 86400);
        }
    }
