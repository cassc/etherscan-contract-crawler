...
contract XNFTHolder is Pausable, ERC1155Holder
{
	...
	function deposit_nfts(uint256 _pool_id, uint256[] memory _xnft_ids) public returns(uint256)
	{
	...
	function withdraw_nfts(uint256 _pool_id, uint256[] memory _xnft_ids) public returns(uint256)
	{
		for(uint256 i=0; i<_xnft_ids.length; i++)
