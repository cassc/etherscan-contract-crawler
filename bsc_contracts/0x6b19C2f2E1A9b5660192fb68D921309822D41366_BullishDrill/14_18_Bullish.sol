...
contract Bullish is ReentrancyGuard, Pausable, XNFTHolder
{
	...
	function make_pool(address _address_token_stake, uint256 _xnft_grade, address _address_token_reward, uint256 _alloc_point,
		uint256 _harvest_interval_block_count, uint256 _withdrawal_interval_block_count, uint256 _fee_e6, bool _refresh_reward) public onlyOperator
	{
		if(_refresh_reward)
		...
		set_deposit_fee_e6(new_pool_id, _fee_e6);
	}
	...
	function deposit(uint256 _pool_id, uint256 _amount, uint256[] memory _xnft_ids) public whenNotPaused nonReentrant
	{
		...
		_renew_user_rewards(address_user, _pool_id, _amount, true);

		if(_amount > 0 || _xnft_ids.length > 0)
		{
			require(pool.xnft_grade == NFT_NOT_USED || _xnft_ids.length > 0, "deposit: Wrong pool id");
			
			if(pool.total_staked_amount == 0)
				pool.last_reward_block_id = block.number; // start emission

			if(pool.xnft_grade == NFT_NOT_USED)
				_deposit_erc20(_pool_id, pool, user, _amount);
			else
				_deposit_erc1155(_pool_id, pool, user, _xnft_ids);
		}
	...
	function withdraw(uint256 _pool_id, uint256 _amount, uint256[] memory _xnft_ids) public nonReentrant
	{
		...
		address address_user = msg.sender;
		_renew_user_rewards(address_user, _pool_id, _amount, false);
		...
		if(_amount > 0 || _xnft_ids.length > 0)
		{
			require(pool.xnft_grade == NFT_NOT_USED || _xnft_ids.length > 0, "withdraw: Wrong pool id");

			if(pool.xnft_grade == NFT_NOT_USED)
				_withdraw_erc20(_pool_id, pool, user, _amount);
			else
				_withdraw_erc1155(_pool_id, pool, user, _xnft_ids);
	...
	function harvest(uint256 _pool_id) public nonReentrant
	{
		...
		address address_user = msg.sender;
		_renew_user_rewards(address_user, _pool_id, 0, true);
		...
		if(amount > 0)
		{
			_safe_reward_transfer(pool.address_token_reward, address_user, amount);
		...
		emit HarvestCB(msg.sender, _pool_id, amount);
	}

	// !!!
	//function get_pending_reward_amount(uint256 _pool_id) external view returns(uint256)
	function get_pending_reward_amount(uint256 _pool_id) public view returns(uint256)
	{
		address address_user = msg.sender;
		return _get_pending_reward_amount(address_user, _pool_id);
	}

	// !!!
	function _get_pending_reward_amount(address address_user, uint256 _pool_id) public view returns(uint256)
	{
	...
	// !!!
	function _get_pending_reward_amount_t1(address address_user, uint256 _pool_id) external view returns(uint256)
	{
		require(_pool_id < pool_info.length, "get_pending_reward_amount: Wrong pool id");
	...
	// !!!
	function _get_pending_reward_amount_t2(address address_user, uint256 _pool_id) external view returns(uint256)
	{
		require(_pool_id < pool_info.length, "get_pending_reward_amount: Wrong pool id");
		...
		return reward.total_alloc_point;
	}
	...
	// Internal Method
	//---------------------------------------------------------------
	function _deposit_erc20(uint256 _pool_id, PoolInfo storage pool, UserInfo storage user, uint256 _amount) internal
	{
		// User -> Bullish
		IERC20 stake_token = IERC20(pool.address_token_stake);
	...
	function _withdraw_erc20(uint256 _pool_id, PoolInfo storage pool, UserInfo storage user, uint256 _amount) internal
	{
		...
		// Bullish -> User
		stake_token.safeTransfer(msg.sender, withdraw_amount);
	...
	function _deposit_erc1155(uint256 _pool_id, PoolInfo storage pool, UserInfo storage user, uint256[] memory _xnft_ids) internal
	{
		// User -> Bullish
		uint256 amount_wei = deposit_nfts(_pool_id, _xnft_ids) * 1e18;
	...
	function _withdraw_erc1155(uint256 _pool_id, PoolInfo storage pool, UserInfo storage user, uint256[] memory _xnft_ids) internal
	{
		// Bullish -> User
		uint256 amount_wei = withdraw_nfts(_pool_id, _xnft_ids) * 1e18;
	...
	// !!!
	//function _renew_user_rewards(address _address_user, uint256 _pool_id, uint256 _amount, bool is_deposit) internal
	function _renew_user_rewards(address _address_user, uint256 _pool_id, uint256 _amount, bool is_deposit) public
	{
		PoolInfo storage pool = pool_info[_pool_id];
		...
		uint256 new_amount = (is_deposit == true)? (user.staked_amount + _amount) : (user.staked_amount - _amount);
		uint256 new_boosted_amount = _get_boosted_user_amount(_pool_id, _address_user, new_amount);
		uint256 next_user_rewards = (new_boosted_amount * pool.accu_reward_amount_per_share_e12) / 1e12;
		
		user.reward_debt = next_user_rewards;
	}
	...
	function _safe_reward_transfer(address _address_token_reward, address _to, uint256 _amount) public
	{
	...
	function set_deposit_fee_e6(uint256 _pool_id, uint256 _fee_e6) public onlyOperator
	{
	...
	function set_deposit_fee_e6_all(uint256 _fee_e6) external onlyOperator
	{
		for(uint256 i=0; i<pool_info.length; i++)
		{
			FeeInfo storage cur_fee = fee_info[i];
			cur_fee.deposit_e6 = _fee_e6;

			emit SetDepositFeeCB(msg.sender, i, _fee_e6);
		}
	}
	...
	function set_withdrawal_fee_e6_all(uint256 _fee_max_e6, uint256 _fee_min_e6, uint256 _period_block_count) external onlyOperator
	{
		for(uint256 i=0; i<pool_info.length; i++)
		{
			FeeInfo storage cur_fee = fee_info[i];
