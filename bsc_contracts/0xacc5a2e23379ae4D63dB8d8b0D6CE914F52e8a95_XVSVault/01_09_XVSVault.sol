...
contract XVSVault is XVSVaultStorage, ECDSA {
    ...
    /**
     * @dev This function resets the storage slot dedicated for afterUpgrade flag to
     *   zero for the affected users. This function will be removed in future.
     */
    function rollbackAfterUpgradeFlag(address user) external onlyAdmin {
        WithdrawalRequest[] storage requests = withdrawalRequests[xvsAddress][0][user];
        requests[0].afterUpgrade = false;
    }
     ...
     */
    function deposit(address _rewardToken, uint256 _pid, uint256 _amount) internal nonReentrant {
        _ensureValidPool(_rewardToken, _pid);
        PoolInfo storage pool = poolInfos[_rewardToken][_pid];
     ...
     * @param _pid The Pool Index
     */
    function executeWithdrawal(address _rewardToken, uint256 _pid) internal nonReentrant {
        _ensureValidPool(_rewardToken, _pid);
        PoolInfo storage pool = poolInfos[_rewardToken][_pid];
        UserInfo storage user = userInfos[_rewardToken][_pid][msg.sender];
     ...
     */
    function requestWithdrawal(address _rewardToken, uint256 _pid, uint256 _amount) internal nonReentrant {
        _ensureValidPool(_rewardToken, _pid);
