...
contract ANDYDividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() public DividendPayingToken("ANDY_Dividen_Tracker", "ANDY_Dividend_Tracker") {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 400 * 10**3 * (10**18); //must hold 100+ tokens
    }

    function _transfer(address, address, uint256) internal override {
        require(false, "ANDY_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override {
        require(false, "ANDY_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main ANDY contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "ANDY_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "ANDY_Dividend_Tracker: Cannot update claimWait to same value");
...
contract ANDY is ERC20, Ownable {
    ...
    ANDYDividendTracker public dividendTracker;
...
constructor(address payable _fundWallet) public ERC20("ANDY", "ANDY") {

      fundWallet = _fundWallet;

    	dividendTracker = new ANDYDividendTracker();
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "ANDY: The dividend tracker already has that address");

        ANDYDividendTracker newDividendTracker = ANDYDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "ANDY: The new dividend tracker must be owned by the ANDY token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "ANDY: The router already has that address");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "ANDY: Account is already the value of 'excluded'");
    ...
    function setRewardTokenFee(uint256 value) external onlyOwner{
        require(value.add(liquidityFee).add(burnFee).add(ecoSystemFee)<=8,"ANDY: Maximum tax is 8%");
    ...
    function setLiquidityFee(uint256 value) external onlyOwner{
        require(RewardTokenFee.add(value).add(burnFee).add(ecoSystemFee)<=8,"ANDY: Maximum tax is 8%");
        liquidityFee = value;
        totalFees = RewardTokenFee.add(liquidityFee).add(burnFee).add(ecoSystemFee);
    ...
    function setEcoSystemFee(uint256 value) external onlyOwner{
        require(RewardTokenFee.add(liquidityFee).add(burnFee).add(value)<=8,"ANDY: Maximum tax is 8%");
        ecoSystemFee = value;
        totalFees = RewardTokenFee.add(liquidityFee).add(burnFee).add(ecoSystemFee);
    ...
    function setBurnFee(uint256 value) external onlyOwner{
        require(RewardTokenFee.add(liquidityFee).add(value).add(ecoSystemFee)<=8,"ANDY: Maximum tax is 8%");
        burnFee = value;
        totalFees = RewardTokenFee.add(liquidityFee).add(burnFee).add(ecoSystemFee);
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "ANDY: The ANDY-BNB pair cannot be removed from automatedMarketMakerPairs");
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "ANDY: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "ANDY: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "ANDY: Cannot update gasForProcessing to same value");
    ...
    function swapAndLiquify(uint256 tokens) private {
       // split the contract balance into halves
        ...
        // swap tokens for ETH
        swapTokensForEth(half); // <- this breaks the ETH -> ANDY swap when swap+liquify is triggered
