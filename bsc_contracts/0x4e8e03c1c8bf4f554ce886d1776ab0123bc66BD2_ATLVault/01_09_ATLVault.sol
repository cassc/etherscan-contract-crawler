...
import "./ATLVaultProxy.sol";
import "./ATLVaultStorage.sol";
import "./ATLVaultErrorReporter.sol";
...
contract ATLVault is ATLVaultStorage, ECDSA {
    ...
    event StoreUpdated(address oldAtl, address oldStore, address newAtl, address newStore);
    ...
    function add(
        ...
        onlyAdmin
    {
        require(address(atlStore) != address(0), "Store contract addres is empty");
        ...
        IATLStore(atlStore).setRewardToken(_rewardToken, true);
    ...
    /**
     * @notice Deposit ATLVault for ATL allocation
        ...
        nonReentrant
    {
        ...
        if (user.amount > 0) {
            ...
            IATLStore(atlStore).safeRewardTransfer(_rewardToken, msg.sender, pending);
        }
        ...
        if (address(pool.token) == address(atlAddress)) {
    ...
    /**
     * @notice Execute withdrawal to ATLVault for ATL allocation
     * @param _rewardToken The Reward Token Address
     * @param _pid The Pool Index
     */
    ...
    /**
     * @notice Request withdrawal to ATLVault for ATL allocation
     * @param _rewardToken The Reward Token Address
     * @param _pid The Pool Index
        ...
        nonReentrant
    {
        ...
        // Update Delegate Amount
        if (_rewardToken == address(atlAddress)) {
    ...
    // View function to see pending ATLs on frontend.
    ...
    /**
     * @notice Get the ATL stake balance of an account (excluding the pending withdrawals)
     * @param account The address of the account to check
     * @return The balance that user staked
     */
    function getStakeAmount(address account) internal view returns (uint96) {
        require(atlAddress != address(0), "ATLVault::getStakeAmount: atl address is not set");

        PoolInfo[] storage poolInfo = poolInfos[atlAddress];

        uint256 length = poolInfo.length;
        for (uint256 pid = 0; pid < length; ++pid) {
            if (address(poolInfo[pid].token) == address(atlAddress)) {
                UserInfo storage user = userInfos[atlAddress][pid][account];
    ...
    function delegateBySig(address delegatee, uint nonce, uint expiry, uint8 v, bytes32 r, bytes32 s)
        external
    {
        bytes32 domainSeparator = keccak256(abi.encode(DOMAIN_TYPEHASH, keccak256(bytes("ATLVault")), getChainId(), address(this)));
        bytes32 structHash = keccak256(abi.encode(DELEGATION_TYPEHASH, delegatee, nonce, expiry));
        bytes32 digest = keccak256(abi.encodePacked("\x19\x01", domainSeparator, structHash));
        address signatory = ECDSA.recover(digest, v, r, s);
        require(nonce == nonces[signatory]++, "ATLVault::delegateBySig: invalid nonce");
        require(block.timestamp <= expiry, "ATLVault::delegateBySig: signature expired");
    ...
    function _moveDelegates(address srcRep, address dstRep, uint96 amount) internal {
        if (srcRep != dstRep && amount > 0) {
            if (srcRep != address(0)) {
                ...
                uint96 srcRepNew = sub96(srcRepOld, amount, "ATLVault::_moveVotes: vote amount underflows");
            ...
            if (dstRep != address(0)) {
                ...
                uint96 dstRepNew = add96(dstRepOld, amount, "ATLVault::_moveVotes: vote amount overflows");
    ...
    function _writeCheckpoint(address delegatee, uint32 nCheckpoints, uint96 oldVotes, uint96 newVotes) internal {
        uint32 blockNumber = safe32(block.number, "ATLVault::_writeCheckpoint: block number exceeds 32 bits");
    ...
    /**
     * @notice Determine the atl stake balance for an account
     * @param account The address of the account to check
     * @param blockNumber The block number to get the vote balance at
     * @return The balance that user staked
     */
    function getPriorVotes(address account, uint256 blockNumber) external view returns (uint96) {
        require(blockNumber < block.number, "ATLVault::getPriorVotes: not yet determined");
    ...
    function _become(ATLVaultProxy atlVaultProxy) external {
        require(msg.sender == atlVaultProxy.admin(), "only proxy admin can change brains");
        require(atlVaultProxy._acceptImplementation() == 0, "change not authorized");
    }

    function setAtlStore(address _atl, address _atlStore) external onlyAdmin {
        address oldAtlContract = atlAddress;
        address oldStore = atlStore;
        atlAddress = _atl;
        atlStore = _atlStore;

        _notEntered = true;

        emit StoreUpdated(oldAtlContract, oldStore, _atl, _atlStore);
    }
}