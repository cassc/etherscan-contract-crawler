...
contract ChipiPaws is ERC20, ERC20Burnable, Ownable {
    ...
    constructor() ERC20("ChipiPaws", "CHIP") {
        _burnRate = 100; // 1% burn rate
        _reflectionRate = 200; // 2% reflection rate
    ...
    function setReflectionRate(uint256 newReflectionRate) external onlyOwner {
        _reflectionRate = newReflectionRate;
    }
    ...
    function _beforeTokenTransfer(
        ...
        override
    {
        if (from != address(0) && _reflectionRate > 0) {
            uint256 reflectionAmount = (amount * _reflectionRate) / 10000;
            _transfer(from, to, reflectionAmount);
        }
    }
    ...
    function _transfer(
        ...
        // Reduce the actual transfer amount
        uint256 adjustedAmount = amount - burnAmount;
