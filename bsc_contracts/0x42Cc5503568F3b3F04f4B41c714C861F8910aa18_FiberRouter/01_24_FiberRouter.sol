...
contract FiberRouter is ReentrancyGuardUpgradeable, OwnableUpgradeable {
    ...
    /*
     ...
     @param crossSwapTargetTokenTo If different than crossTargetFoundryToken, a swap
    ...
    function swapAndCross(
        ...
        address crossTargetFoundryToken
    ) external nonReentrant {
        uint256 currentChainId = getChainId();
        require(
            currentChainId != crossTargetNetwork && path[0] != crossTargetFoundryToken,
            "ERROR: SAME TOKEN WITH SAME NETWORK"
        );
    ...
    @param crossTargetFoundryToken The target network token
    ...
    */
    function _swapAndCross(
        ...
        uint256 deadline,
        uint256 crossTargetNetwork,
        address crossTargetFoundryToken // address crossSwapTargetTokenTo // address crossTargetAddress
    ) internal {
