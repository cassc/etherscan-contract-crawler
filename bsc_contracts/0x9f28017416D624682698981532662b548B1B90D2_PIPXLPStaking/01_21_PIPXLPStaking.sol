...
contract PIPXLPStaking is PausableUpgradeable {
    using SafeERC20Upgradeable for IERC20Upgradeable;

    IPIPXVaultFactory public pipxVaultFactory;
    ...
    function __PIPXLPStaking__init(address _stakingTokenProvider) external initializer {
        __Ownable_init();
    ...
    function setPIPXVaultFactory(address newFactory) external onlyOwner {
        require(address(pipxVaultFactory) == address(0), "pipxVaultFactory is immutable");
        pipxVaultFactory = IPIPXVaultFactory(newFactory);
    }
    ...
    function addPoolForVault(uint256 vaultId) external onlyAdmin {
        require(address(pipxVaultFactory) != address(0), "LPStaking: Factory not set");
        require(vaultStakingInfo[vaultId].stakingToken == address(0), "LPStaking: Pool already exists");
        address _rewardToken = pipxVaultFactory.vault(vaultId);
    ...
    function timelockDepositFor(uint256 vaultId, address account, uint256 amount, uint256 timelockLength) external {
        ...
        require(pipxVaultFactory.excludedFromFees(msg.sender), "Not zap");
        onlyOwnerIfPaused(10);
        // Check the pool in case its been updated.
        updatePoolForVault(vaultId);
    ...
    function emergencyExitAndClaim(address _stakingToken, address _rewardToken) external {
        ...
        _distributeFees(IPIPXVault(_rewardToken).vaultId());
        _claimRewards(pool, msg.sender);
...
//     }
    ...
    function totalUndistributedFees(uint256 vaultId) public view returns (uint256) {
        ...
        return IERC20Upgradeable(vaultStakingInfo[vaultId].rewardToken).balanceOf(pipxVaultFactory.feeDistributor()) * receiverAlloc / 1e18;
    }
