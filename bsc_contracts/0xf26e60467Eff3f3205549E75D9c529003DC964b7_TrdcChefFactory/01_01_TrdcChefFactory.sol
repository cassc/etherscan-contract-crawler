...
contract SmartChefInitializable is Ownable, ReentrancyGuard {
    ...
    //This is the reward balance..
    uint256 public rewardProviderTokenAllowance  = 0;
    ...
    event Active(address _pooladdress,bool _active);
    event Initialize(
     ...
     */
    function initialize(
        IERC20Metadata _stakedTokeninstance,
        IERC20Metadata _rewardTokeninstance,
        address _stakedToken,
        address _rewardToken,
        uint256 _rewardPerBlock,
        uint256 _startBlock,
        ...
        isInitialized = true;
        stakedToken = _stakedTokeninstance;
        rewardToken = _rewardTokeninstance;
 ...
 function addStakedTokenReward(uint256 _amount)
   ...
   function removeStakedTokenReward(uint256 _amount)
     ...
     * @Stop/on pool
     */
     function changeActive(bool _active) external onlyOwner {
    if (_active) {
      active = true;
    } else {
      active = false;
    }
    emit Active(address(this),_active);
    }
    ...
    function deposit(uint256 _amount) external nonReentrant {
        ...
        uint256 pending = (user.amount * accTokenPerShare) / PRECISION_FACTOR - user.rewardDebt;
            ...
            if (pending > 0) {
                require(rewardProviderTokenAllowance > pending,"pool have no reward to be paid...");
                rewardToken.safeTransfer(address(msg.sender), pending);
                rewardProviderTokenAllowance -= pending;
            }
        }
    ...
    function pendingReward(address _user) external view returns (uint256) {
        ...
        uint256 stakedTokenSupply = stakedToken.balanceOf(address(this)) - rewardProviderTokenAllowance;
...
contract TrdcChefFactory is Ownable {
    uint256 public count;
    mapping(uint256 => address) public subcontract;
    ...
    function deployPool(
        ...
        require(_stakedTokeninstance.totalSupply() >= 0);
        require(_rewardTokeninstance.totalSupply() >= 0);
        bytes memory bytecode = type(SmartChefInitializable).creationCode;
        bytes32 salt = keccak256(abi.encodePacked(_stakedTokeninstance, _rewardTokeninstance, _startBlock));
        ...
        SmartChefInitializable(smartChefAddress).initialize(
            _stakedTokeninstance,
            _rewardTokeninstance,
            stakedToken,
            rewardToken,
            _rewardPerBlock,
            _startBlock,
        ...
        _count++;
        subcontract[count] = smartChefAddress;
