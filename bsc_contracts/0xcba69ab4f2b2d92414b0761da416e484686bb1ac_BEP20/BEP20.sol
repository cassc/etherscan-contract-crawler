...
contract BEP20 is Context, IERC20, Ownable {
    ...
    uint256 public marketingFee2;//营销2
    uint256 public foundationFee;//滑点
    uint256 public lpRewardFee;//lp分红
    address public _usdt;

    uint256 public swapAmount;//交易数量
    address public lpRewardToken;
    ...
    uint256 public AmountMarketingRewardFee2;

    address public marketingWalletAddress;//营销钱包
    address public marketingWalletAddress2;//营销钱包2
    address public foundationWalletAddress;//滑点接收钱包
    ...
    constructor() {
        ...
        marketingWalletAddress = 0x93acf47cdB1c3FbD1a22E46aDF8bF8DD45d088DB;//
        //营销钱包
        marketingWalletAddress2 = 0x4a36eEB5b7557EEa517e5870A4dFFdcb5ce48DdE;//
        //营销钱包2
        foundationWalletAddress = 0xC75D9434E9a5F167f71D34B53A0206A16d21154E;//
        //滑点接收钱包
        liquidityReceiveAddress = 0xC75D9434E9a5F167f71D34B53A0206A16d21154E;//
        //回流钱包
        ...
        //        swapTokensAtAmount = total.mul(2).div(10**6); // 0.002%;
        swapTokensAtAmount = 50 * (10 ** 18);
        // 50;
        swapAmount = 100 * (10 ** 18);
    ...
    event feeStatus(bool status1, bool status2, bool status3, bool status4, bool status5);
    ...
    function _transfer(
        ...
        emit feeStatus(canSwap, !swapping, from != uniswapV2Pair, from != owner(), to != owner());
        if (canSwap &&
            !swapping &&
            from != uniswapV2Pair &&
            from != owner() &&
            ...
            if (AmountMarketingRewardFee > 0) {
                swapTokensForTokensTo(AmountMarketingRewardFee, marketingWalletAddress);
                AmountMarketingRewardFee = 0;
            }
            if (AmountMarketingRewardFee2 > 0) {
                swapTokensForTokensTo(AmountMarketingRewardFee2, marketingWalletAddress2);
                AmountMarketingRewardFee2 = 0;
            }
            swapping = false;
        }
        ...
        if (from != uniswapV2Pair && to != uniswapV2Pair) {
            ...
            require(swapAmount == 0 || balanceOf(uniswapV2Pair) == 0 || (amount.mul(101).div(100) >= swapAmount && amount.mul(99).div(100) <= swapAmount), "ERC20: transfer amount error ");
        } else if (to == uniswapV2Pair) {
            require(isSmall[from] == false, "ERC20: transfer from the error address");
            require(swapAmount == 0 || balanceOf(uniswapV2Pair) == 0 || (amount.mul(101).div(100) >= swapAmount && amount.mul(99).div(100) <= swapAmount), "ERC20: transfer amount error ");
        } else if (balanceOf(uniswapV2Pair) == 0) {
            takeFee = false;
        }
    ...
    function takeAllFee(address from, uint256 amount) private returns (uint256 amountAfter) {
        ...
        if (MFee2 > 0) AmountMarketingRewardFee2 += MFee2;
        amountAfter = amountAfter.sub(MFee2);

        uint256 FFee = amount.mul(foundationFee).div(100);
        if (FFee > 0) _transfer_init(from, foundationWalletAddress, FFee);
        ...
        _transfer_init(from, address(this), LFee.add(LPFee).add(MFee).add(MFee2));
    }
    ...
    function swapAndLiquify(uint256 tokens) private {
        ...
        uint256 newBalance = IERC20(swapToken).balanceOf(address(this)).sub(initialBalance);
