...
contract SteakD is ERC20Upgradeable, UUPSUpgradeable, OwnableUpgradeable {
    ...
    modifier antiWhale(address sender, address recipient, uint256 amount) {
        ...
        if (_maxAmt > 0) {
            if (
                ...
                require(amount <= _maxAmt, "AntiWhale: Transfer amount exceeds the maxTransferAmount");
            }
        }
        _;
    }
    ...
    function initialize(
        ...
        pancakeSwapPair = IPancakeFactory(pancakeSwapV2Router.factory()).createPair(WBNB, address(this));
    ...
    function _takeFee(address from, address to, uint256 amount) internal returns (uint256) {
        ...
        if(pairsMap[from]){
            uint256 totalTxFee = buyHoldersFee.add(buyLPFee).add(buyOperationFee).add(buyPrimaryUtitlityFee).add(buySecondUtitlityFee);
            txFee = amount * totalTxFee / 10000;
            amountForHolders = amountForHolders.add(amount.mul(buyHoldersFee).div(10000));
            amountForLP = amountForLP.add(amount.mul(buyLPFee).div(10000));
            amountForOperation = amountForOperation.add(amount.mul(buyOperationFee).div(10000));
            amountForPrimaryUtility = amountForPrimaryUtility.add(amount.mul(buyPrimaryUtitlityFee).div(10000));
            amountForSecondUtility = amountForSecondUtility.add(amount.mul(buySecondUtitlityFee).div(10000));
        }
        
        if(pairsMap[to]){
            uint256 totalTxFee = sellHoldersFee.add(sellLPFee).add(sellOperationFee).add(sellPrimaryUtitlityFee).add(sellSecondUtitlityFee);
            txFee = amount * totalTxFee / 10000;
            amountForHolders = amountForHolders.add(amount.mul(sellHoldersFee).div(10000));
            amountForLP = amountForLP.add(amount.mul(sellLPFee).div(10000));
            amountForOperation = amountForOperation.add(amount.mul(sellOperationFee).div(10000));
            amountForPrimaryUtility = amountForPrimaryUtility.add(amount.mul(sellPrimaryUtitlityFee).div(10000));
            amountForSecondUtility = amountForSecondUtility.add(amount.mul(sellSecondUtitlityFee).div(10000));
        }
    ...
    function _takeTransferFee(address from, address to, uint256 amount) internal returns (uint256) {
        if(excludedFromFee[from] || excludedFromFee[to]){
            return 0;
        }
        uint256 txFee = 0;
        ...
        if(totalFee > 0){
            ...
            super._transfer(from, address(this), txFee);
            amountForOperation = amountForOperation.add(amount * transferOperationFee / 10000);
            amountForTransferUtility = amountForTransferUtility.add(amount * transferUtitlityFee / 10000);
            return txFee;
        }
        return txFee;
    }
    ...
    function _transfer(
        ...
        if(!excludedFromDivTracker[from]){ try IDividendDistributor(distributor).setShare(from, balanceOf(from)) {} catch {} }
        if(!excludedFromDivTracker[to]){ try IDividendDistributor(distributor).setShare(to, balanceOf(to)) {} catch {} }

        try IDividendDistributor(distributor).process(distributorGas) {} catch {}
    }

    function shouldSwapBack() internal view returns (bool) {
        return msg.sender != pancakeSwapPair
        && !inSwap
        && balanceOf(address(this)) >= swapThreshold;
    }

    function _swapBack() internal swapping {
        uint256 amountForUtility = amountForPrimaryUtility + amountForSecondUtility + amountForTransferUtility;
        uint256 amountForBNB = amountForLP.div(2).add(amountForOperation).add(amountForUtility);
        uint256 amountToLiquify = amountForLP.sub(amountForLP.div(2));
        uint256 rewardBNB = swapTokensForBNB(address(this), amountForBNB);
        uint256 bnbToLiquidity = rewardBNB.mul(amountForLP.div(2)).div(amountForBNB);
        uint256 bnbForOperation = rewardBNB.mul(amountForOperation).div(amountForBNB);
        uint256 bnbForUtility = rewardBNB.mul(amountForUtility).div(amountForBNB);
        ...
        uint256 bnbForPrimaryUtilityReceiver = bnbForUtility * (amountForPrimaryUtility + amountForTransferUtility) / amountForUtility;
        payable(primaryUtilityReceiver).transfer(bnbForPrimaryUtilityReceiver);
        payable(secondUtilityReceiver).transfer(bnbForUtility - bnbForPrimaryUtilityReceiver);
    ...
    function swapTokensForBNB(address tokenAddress, uint256 tokenAmount) private returns (uint256){
        ...
        IERC20(address(this)).approve(address(pancakeSwapV2Router), tokenAmount);
    ...
    */
    ...
    function excludeFromFees(address account, bool excluded) external onlyOwner {
        require(excludedFromFee[account] != excluded, "Rematic: Account is already the value of 'excluded'");
    ...
    function excludedAccountFromAntiBot(address account, bool excluded) external onlyOwner {
    ...
    function getParis() external view returns (address[] memory availablePairs){
        uint256 length = 0;
    ...
    function excludeFromDividendTracker(address holder, bool _flag) external onlyOwner {
        ...
        if(_flag){
            ...
            IDividendDistributor(distributor).setShare(holder, balanceOf(holder));
        }
    }
