...
contract HeyReborn is Ownable, IBEP2E,ReentrancyGuard {
  ...
  uint256 internal minLiquidityAmount; //the minimum amount of Hey Reborn token to add liquidity with
  ...
  constructor(address _routerAddress ,address _marketingFundAddress,uint256 _txFee,uint256 _liquidityPoolFee,uint256 _lpBuyFee,uint256 _lpSellFee, uint256 _maxTaxFee) { //payable
      _name = "Hey Reborn"; 
      _symbol = "RB";
      _decimals = 18;
  ...
  */
  ...
  /**
  * @notice the total number of decimals for the RB token
  ...
  */
  ...
  /**
  * @notice creates a liquidity pool pair for the RB/WBNB(BNB) tokens if not created yet
  ...
  */
  ...
  function setTxFee(uint256 _txFee) external onlyOwner {
    require(_txFee <= maxTaxFee, "TxFee cannot be greater than maxTaxFee");
    txFee = _txFee;
  }
  ...
  */
  ...
  function calculateMaxTaxFee() public view returns (uint256) {
    return maxTaxFee.mul(100).div(10**2);
  }
  ...
  */
  ...
  /**
  * @dev Adds Liquidity for the RB/WBNB tokens
  * @param _rbTokenAmount uint256 the RB token amount
  * @param _bnbTokenAmount uint256 the WBNB token amount
  */
  function addLiquidity(uint256 _rbTokenAmount, uint256 _bnbTokenAmount) private{
    _approve(address(this), address(pancakeswapV2Router), _rbTokenAmount);
  ...
  /**
  * @notice removes RB/WBNB liquidity
  * @param _liquidityAmountToRemove uint256 the amount to tokens to remove from the RB/BNB Liquidity pool
  ...
  */
  ...
  function withdraw(uint256 _amount) public onlyOwner nonReentrant returns(bool success){
    ...
    require(accountBalance >= _amount,"Hey Reborn: Insufficient Withdrawal Balance");
