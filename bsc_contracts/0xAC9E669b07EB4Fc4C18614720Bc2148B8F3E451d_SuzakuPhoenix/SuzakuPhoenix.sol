...
/**
  ...
  SuzakuPhoenix [SPX]
  SuzakuPhoenix is a Meme Deflationary Asset on the Binance Smart Chain Network,
  that seeks to lead Defi via several innovations which will be spelt out in the white paper,
  and also onboarding new persons into the space to create a formidable community.
 ...
 * error, which is the standard behSPXor in high level programming languages.
...
contract SuzakuPhoenix is Context, IERC20, Ownable {
    ...
    string private _name = "SuzakuPhoenix";
    ...
    address public teamWallet = 0xE78591D9639d15422207359a95870d85352F7777;
    ...
    uint256 public numTokensSellToAddToLiquidity = 1000000000000000 * 10**18;
    ...
    function removeAllFee() private {
        if(_taxFee == 0 && _liquidityFee == 0 && _teamFee==0 && _burnFee==0) return;
    ...
    returns (uint256, uint256)
    {
        if(_teamFee==0) {  return(tTransferAmount, rTransferAmount); }
        uint256 tTeam = tAmount.div(100).mul(_teamFee);
        ...
        emit Transfer(sender, teamWallet, tTeam);
        return(tTransferAmount, rTransferAmount);
    }
    ...
    function setFees(uint256 taxFee, uint256 liquidityFee, uint256 teamFee, uint256 burnFee) external onlyOwner() {
        _taxFee = taxFee;
