...
// File: contracts/interfaces/ILeonsToken.sol

pragma solidity 0.6.12;

interface ILeonsToken is IBEP20 {
    function mint(address to, uint256 amount) external;
    function transferOwnership(address newOwner) external;
    function renounceOwnership() external;
}

// File: contracts/RewardHolder.sol
...
contract RewardHolder is Ownable {
    // The LEON TOKEN!
    ILeonsToken public leon;
    bytes4 private constant SELECTOR =
        bytes4(keccak256(bytes("transfer(address,uint256)")));

    constructor(ILeonsToken _leon) public {
        leon = _leon;
    }

    // Safe leon transfer function, just in case if rounding error causes pool to not have enough LEONs.
...
// File: contracts/MasterHunter.sol
