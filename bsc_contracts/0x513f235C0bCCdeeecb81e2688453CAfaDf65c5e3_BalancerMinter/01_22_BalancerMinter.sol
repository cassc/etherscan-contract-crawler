...
import "../interfaces/liquidity-mining/IBalancerMinter.sol";
import "../interfaces/liquidity-mining/IBalancerTokenAdmin.sol";
import "../interfaces/liquidity-mining/IGaugeController.sol";
import "../interfaces/liquidity-mining/ILiquidityGauge.sol";

import "../solidity-utils/openzeppelin/ReentrancyGuard.sol";
import "../solidity-utils/openzeppelin/SafeMath.sol";
import "../solidity-utils/openzeppelin/EIP712.sol";
...
contract BalancerMinter is IBalancerMinter, ReentrancyGuard, EIP712 {
     ...
     * @param user Address to mint to
     */
    function mintManyFor(address[] calldata gauges, address user)
        external
        override
        nonReentrant
        returns (uint256)
    ...
    function setMinterApprovalWithSignature(
        ...
        bytes32 structHash = keccak256(
            abi.encode(_SET_MINTER_APPROVAL_TYPEHASH, minter, approval, nonce, deadline)
        );
    ...
    function _mintForMany(address[] calldata gauges, address user)
        internal
