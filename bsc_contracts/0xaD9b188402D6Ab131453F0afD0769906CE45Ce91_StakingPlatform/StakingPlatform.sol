...
contract StakingPlatform is Ownable {
    ...
    // refer to CLS token. Address: 0x24f1b357dd3a2fa36e0b3f92a8167581c9a5ab59
    ...
    function getMode(address addr) public view returns (uint256) {
        return _staker[addr].mode;
    }
    ...
    function updateReward() private{
        ...
        uint256 newReward = stakerStakedAmount.mul(block.timestamp.sub(_staker[msg.sender].lastUpdatedTime)).mul(_rewardRate).div(1 days).div(1e9);
    ...
    function unstakingFeeRate() public view returns (uint256) {
    ...
    function rewardToHarvest(address _address) public view returns (uint256){
        ...
        uint256 newReward = stakerStakedAmount.mul(block.timestamp.sub(_staker[_address].lastUpdatedTime)).mul(_rewardRate).div(1 days).div(1e9);
    ...
    function withdraw(uint256 amount) external{
        ...
        uint256 amountReflectFee = amountTobeWithdrawn.sub(amountTobeWithdrawn.mul(fee).div(1e9));
    ...
    function setProMinAmount(uint256 _minAmount) external onlyOwner {
    ...
    function setLegendMinAmount(uint256 _minAmount) external onlyOwner {
    ...
    function setClassyMinAmount(uint256 _minAmount) external onlyOwner {
    ...
    function harvest() public{
        ...
        uint256 rewardToClaim = curReward.sub(curReward.mul(_rewardFeeRate).div(1e9));
    ...
    function getMinimumStakingAmount(uint256 _mode) public view returns (uint256) {
        ...
        if (_mode == 0) _minStakeAmount = _minStakeAmount_m;
        else if (_mode == 1) _minStakeAmount = _minStakeAmount_c;
        else if (_mode == 2) _minStakeAmount = _minStakeAmount_s;
