...
// File contracts/dwarvenDice/DwarvenDice.sol


/// @dev Interface for burning tokens.
interface IGameToken is IERC1155 {
    function burn(
        address,
        uint256,
        uint256
...
/**
 * @title Contract for the Mithrilverse's Dwarven Dice game.
 * @author Originally by redWizard. Edited by Silencio.
 * @notice Authors assume game pieces have no real world value.
 */
contract DwarvenDice is ERC1155Holder {
    using Counters for Counters.Counter;

    IGameToken public immutable gameToken;
    uint256 public immutable TOKEN_ID;
    uint32 public immutable PLAY_AMOUNT;
    ...
    function currentPlayersCount() public view returns(uint256) {
        return currentPlayers.length;
    }

    /**
    * Allows caller to play game.
    * @dev Caller must first approve this contract to transfer gameToken.
    * @dev Game starts when currentPlayers is 6.
    */
    function playGame() public {
        // Transfer gameToken from caller to this contract.
    ...
    * @dev Winner receives 5 times PLAY_AMOUNT of gameToken.
    * @dev Remaining 1 PLAY_AMOUNT of gameToken is burned.
    */
    function _rollDice() internal returns(address _winner) {
        ...
        uint256 ran = uint256(_hash) % 6;
        _winner = currentPlayers[ran];
