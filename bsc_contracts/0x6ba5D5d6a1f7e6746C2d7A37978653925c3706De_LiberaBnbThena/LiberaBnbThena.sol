...
// File contracts/LiberaBnbThena.sol

//SPDX-License-Identifier: MIT

pragma solidity 0.8.17;
contract LiberaBnbThena  is Ownable, Pausable {
    using SafeERC20 for IERC20;

    // Tokens
    address public constant wbnb = address(0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c);
    address public constant the = address(0xF4C8E32EaDEC4BFe97E0F595AdD0f4450a863a11);
    address public constant libera = address(0x3A806A3315E35b3F5F46111ADb6E2BAF4B14A70D);
    address public want;
    ...
    //Routes
    IThenaRouter.Routes[] public bnbToLiberaPath;
    ...
    constructor(
        ...
        IThenaRouter.Routes[] memory _bnbToLiberaPath
    ) {
        ...
        for (uint i; i < _bnbToLiberaPath.length; ++i) {
            bnbToLiberaPath.push(_bnbToLiberaPath[i]);
        }
    ...
    function chargeFees(address caller) internal {
       uint256 toFee = IERC20(the).balanceOf(address(this));

        IThenaRouter(router).swapExactTokensForTokensSimple(toFee, 0, the, wbnb, stable, address(this), block.timestamp);

        uint256 feeBal = IERC20(wbnb).balanceOf(address(this)) * PLATFORM_FEE / FEE_DIVISOR;

        uint256 callFee = feeBal * CALL_FEE / FEE_DIVISOR;
        IERC20(wbnb).transfer(caller, callFee);

        if(RECIPIENT_FEE > 0){
        uint256 recipientFee = feeBal * RECIPIENT_FEE / FEE_DIVISOR;
        IERC20(wbnb).safeTransfer(grimFeeRecipient, recipientFee);
        }

        uint256 treasuryFee = feeBal * TREASURY_FEE / FEE_DIVISOR;
        IERC20(wbnb).transfer(treasury, treasuryFee);

        uint256 stratFee = feeBal * STRAT_FEE / FEE_DIVISOR;
        IERC20(wbnb).transfer(strategist, stratFee);

    }

    /** @dev Converts reward to both sides of the LP token and builds the liquidity pair */
    function addLiquidity() internal {
        uint256 wbnbHalf = IERC20(wbnb).balanceOf(address(this)) / 2;
        IThenaRouter(router).swapExactTokensForTokensSupportingFeeOnTransferTokens(wbnbHalf, 0, bnbToLiberaPath, address(this), block.timestamp);

        uint256 t1Bal = IERC20(wbnb).balanceOf(address(this));
        uint256 t2Bal = IERC20(libera).balanceOf(address(this));

        IThenaRouter(router).addLiquidity(wbnb, libera, stable, t1Bal, t2Bal, 1, 1, address(this), block.timestamp);
    }
    ...
    function _subAllowance() internal {
        ...
        IERC20(libera).safeApprove(router, 0);
    }
    ...
    function _addAllowance() internal {
        ...
        IERC20(libera).safeApprove(router, type(uint).max);
    }
