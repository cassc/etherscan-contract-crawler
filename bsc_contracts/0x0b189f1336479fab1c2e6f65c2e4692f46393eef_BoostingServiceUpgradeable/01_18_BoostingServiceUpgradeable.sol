...
contract BoostingServiceUpgradeable is UUPSUpgradeable, PausableByOwnerUpgradeable, RecoverableByOwnerUpgradeable {
    ...
    function getAndUpdateStaker() external whenNotPaused returns (Staker memory) {
    ...
    function _claimReward(address owner, address spender) whenNotPaused private {
        Staker storage staker = stakers[owner];
    ...
    function _unstakeShares(address owner, address spender, uint128 shares) whenNotPaused private returns (uint128) {
        Staker storage staker = stakers[owner];
        _compoundAndRecalculateShares(staker, 0, owner);
    ...
    function _unstakeSharesLocked(Staker storage staker, address owner, address spender, uint128 unstakedShares) private returns (uint128) {
    ...
    function _unstakeSharesUnlocked(Staker storage staker, address owner, address spender, uint128 unstakedShares) private returns (uint128) {
        uint128 _stakerShares = staker.shares;
        require(unstakedShares <= _stakerShares, "BoostingService: NOT_ENOUGH_BALANCE");
