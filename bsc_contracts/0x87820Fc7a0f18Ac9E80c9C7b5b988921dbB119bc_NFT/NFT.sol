...
contract NFT is ERC721Enumerable, Ownable{
    ...
    function getTokenId(uint256 _type, uint256 index) public pure returns(uint256){
        return (_type << 32) | index;
    }
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        return string(abi.encodePacked(_uri, Strings.toString(getType(tokenId))));
    }
    ...
    function mint(address to, uint256 _type, uint256 quantity) external {
        ...
        uint256 start = getTokenId(_type, totalSupplyOfType[_type]);
        uint256 end = start + quantity;
        for(uint256 i = start; i < end; i++){
            _mint(to, i);
        }
        totalSupplyOfType[_type] += quantity;
    }
    
    function ownerTokenInfo(address _owner, uint256 start, uint256 limit) external view 
        returns(uint256 balance, uint256[] memory tokenIds, uint256[] memory types){
        ...
        types = new uint256[](len);
        for(uint256 i = 0; i < len; i++){
            uint256 tokenId = tokenOfOwnerByIndex(_owner, start+i);
            tokenIds[i] = tokenId;
            types[i] = getType(tokenId);
        }
    }
}