...
contract FluronixMultiData{
    ...
    function fetchMultiPairs(address exchangeFactory, uint start, uint stop) public view returns(PairResult[] memory pairDetails){
        ...
        for(uint256 i = start; i < stop; i++) {
            ...
            if (token01[0] == address(0) || token01[1] == address(0)){ // continue fetching other pairs if the current pair token01 not found
                continue;
            }
            // get tokens symbols
            ...
            if(reservesStruct[0].reserves01[0] != 0 && reservesStruct[0].reserves01[1] != 0){// if they are reserves in pool then fetch price
                ...
                //get token01 prices
                token01Price[0] = getPrice(token01[0], token01Decimal[0]); 
                token01Price[1] = getPrice(token01[1], token01Decimal[1]); 
            }
    ...
    function getMultiplePrice(GetPrice[] memory token_decimal) public view returns (PriceOutput[] memory prices){
        ...
        for(uint i; i < token_decimal.length; i++){
            prices[i] = getPrice(token_decimal[i].token, token_decimal[i].decimal);
        }
    ...
    function getPrice(address token, uint decimal)public view returns  (PriceOutput memory out){
        address  Pair;
