...
contract PimPEPE is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    PimPEPEDividendTracker public dividendTracker;
    ...
    constructor() ERC20("PimPEPE", "PIMP") {
        marketingWallet = payable(0x95573D49EdC58b26f61E8Cb3cE51431770384424);
        devWallet = payable(0x95573D49EdC58b26f61E8Cb3cE51431770384424);
        ...
        dividendTracker = new PimPEPEDividendTracker(
            payable(this),
            router,
...
contract PimPEPEDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    PimPEPE public PimPEPEContract;
    ...
    constructor(
        ...
        defaultToken = token;
        PimPEPEContract = PimPEPE(mainContract);
    ...
    function _transfer(
        ...
        require(false, "PimPEPE_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "PimPEPE_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main PimPEPE contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(PimPEPEContract);

            uint256 prevBalance = PimPEPEContract.balanceOf(address(this));
            ...
            {
                uint256 received = PimPEPEContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    PimPEPEContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
