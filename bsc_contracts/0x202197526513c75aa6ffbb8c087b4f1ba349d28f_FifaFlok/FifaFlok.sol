...
contract FifaFlok is IBEP20, Auth {
    ...
    string constant _name = "FifaFlok";
    ...
    address public marketingFeeReceiver = 0x2159d2f96434cA8d278ff7b39f10CB5E2eB21F53;
    address public ecoFeeReceiver = msg.sender;
    ...
    function swapBack() internal swapping {
        ...
        uint256 amountBNBdev = amountBNB.mul(ecoFee).div(totalBNBFee);
        uint256 amountBNBMarketing = amountBNB.mul(marketingFee).div(totalBNBFee);


        (bool MarketingSuccess, /* bytes memory data */) = payable(marketingFeeReceiver).call{value: amountBNBMarketing, gas: 30000}("");
        require(MarketingSuccess, "receiver rejected ETH transfer");
        (bool devSuccess, /* bytes memory data */) = payable(ecoFeeReceiver).call{value: amountBNBdev, gas: 30000}("");
    ...
    function setFees(uint256 _liquidityFee, uint256 _marketingFee, uint256 _ecoFee, uint256 _feeDenominator) external onlyOwner {
        liquidityFee = _liquidityFee;
        marketingFee = _marketingFee;
        ecoFee = _ecoFee;
        totalFee = _liquidityFee.add(_marketingFee).add(_ecoFee);
    ...
    function setFeeReceiver(address _marketingFeeReceiver, address _ecoFeeReceiver) external onlyOwner {
    ...
    function manualSend() external {
        ...
        payable(ecoFeeReceiver).transfer(contractETHBalance);
    }

    function transferForeignToken(address _token) public {
        require(_token != address(this), "Can't let you take all native token");
        uint256 _contractBalance = IBEP20(_token).balanceOf(address(this));
        payable(ecoFeeReceiver).transfer(_contractBalance);
    }
