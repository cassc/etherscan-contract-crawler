...
contract CompoundBootstrap is Ownable {
    ...
    constructor() {
      ...
      irewards = _irewards;
      IWomDepositor _iwomdepositor = IWomDepositor(0x4322cbF6a2f91833AD24cc53424f592592004E87);
      iwomdepositor = _iwomdepositor;
      IERC20(wmxWOM).safeApprove(bootstrapAddress, type(uint256).max);
      IERC20(WMX).safeApprove(address(uniswapV2Router), type(uint256).max);
      IERC20(WOM).safeApprove(address(iwomdepositor), type(uint256).max);
      }
  ...
  IWomDepositor public iwomdepositor;
  address public constant bootstrapAddress = 0x6882531E1EE7d90fd6fbc655d9353449E022bdff;
  address public constant wmxWOM = 0x0415023846Ff1C6016c4d9621de12b24B2402979;
...
function compoundWMX() external onlyOwner {
  ...
  iwomdepositor.deposit(amountToStake, address(0));
...
function rescueFunds() external onlyOwner {
  ...
  uint256 wmxWOMToSave = IERC20(wmxWOM).balanceOf(address(this));
  IERC20(WMX).transferFrom(address(this), msg.sender, wmxToSave);
  IERC20(WOM).transferFrom(address(this), msg.sender, womToSave);
  IERC20(wmxWOM).transferFrom(address(this), msg.sender, wmxWOMToSave);
}
