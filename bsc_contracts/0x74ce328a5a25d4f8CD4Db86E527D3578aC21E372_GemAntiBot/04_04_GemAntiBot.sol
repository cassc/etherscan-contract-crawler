...
contract GemAntiBot is ContextUpgradeable {
    ...
    mapping(address => IPancakeRouter02) public pancakeRouter;
    mapping(address => address) public pancakePair;
    ...
    function manageBot(
        ...
        require(_pancake != address(0), "wrong router address");
        require(_pair != address(0), "wrong token 0 address");
        pancakeRouter[token] = IPancakeRouter02(_pancake);
        pancakePair[token] = IPancakeFactory(pancakeRouter[token].factory())
            .getPair(token, _pair);
        if (pancakePair[token] == address(0))
            pancakePair[token] = IPancakeFactory(pancakeRouter[token].factory())
    ...
    function onPreTransferCheck(
        ...
        if (
            ...
            require(
                (from != pancakePair[_msgSender()] &&
                    to != pancakePair[_msgSender()]) || amountLimit > amount,
                "more than limit"
            );
        }
        //gwei limit
        ...
        //cool down
        if (from == pancakePair[_msgSender()]) {
            require(
