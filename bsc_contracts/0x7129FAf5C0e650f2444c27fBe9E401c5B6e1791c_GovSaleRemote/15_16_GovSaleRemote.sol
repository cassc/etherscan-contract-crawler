...
contract GovSaleRemote is Initializable, PausableUpgradeable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
  ...
  event Finalize(uint128 remoteRaised, uint128 remoteRevenue, uint256 remoteSold);
   ...
   */
  function getBooster1EligibleUsers(address _user) external view virtual returns (bool) {
  ...
  function getAdapterParams() internal view virtual returns (bytes memory adapterParams) {
    uint16 version = 1;
   ...
   */
  function getPayload(uint128 _amountIn, address _buyer) internal view virtual returns (bytes memory payload) {
    // change to 6 decimal
   ...
   * @param _buyer User address
   */
  function estimateFees(uint128 _amountIn, address _buyer) public view virtual returns (uint256 fees, uint256 tax) {
    (fees, ) = endpoint.estimateFees(
      dstChainId,
      factory.saleGateway(),
      getPayload(_amountIn, _buyer),
      false,
      getAdapterParams()
    );
  ...
  function isEligible() internal view virtual {
    address sender = _msgSender();
    require((sender == factory.savior() || sender == factory.keeper() || sender == owner()), "??");
  }

  function isSufficient(uint256 _amount) internal view virtual {
    require(payment.balanceOf(address(this)) >= _amount, "less");
  }

  function changeToOriginalDecimal(uint128 _amount) internal view virtual returns (uint128 result) {
    result = uint128((_amount * 10**paymentDecimals) / 1e6);
  }

  // **** MAIN AREA ****

  function releaseToken(address _target, uint256 _amount) internal virtual {
  ...
  function forceMoveFund() external virtual {
    ...
    releaseToken(factory.operational(), payment.balanceOf(address(this)));
  }
   ...
   */
  function buyToken(uint128 _amountIn) external payable virtual whenNotPaused nonReentrant {
    ...
    ISaleGatewayRemote(factory.saleGateway()).buyToken{value: feeNeeded}(
      address(factory),
      getPayload(_amountIn, sender),
      getAdapterParams(),
      tax
    );
  ...
  function finalize(bytes calldata _payload) external virtual {
  ...
  function removePurchase(address _buyer) external virtual onlyOwner {
   ...
   */
  function setBooster1Buyer(address[] calldata _buyers) external virtual onlyOwner {
   ...
   */
  function setRefund(uint128 _refund_d2) external virtual onlyOwner {
   ...
   */
  function setDstChainId(uint16 _dstChainId) external virtual onlyOwner {
