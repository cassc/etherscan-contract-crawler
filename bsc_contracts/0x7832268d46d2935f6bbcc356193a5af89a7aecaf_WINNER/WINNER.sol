...
contract Setter is Ownable {
    ...
    address public feeAddr1 = 0xC19a725DA46432ea97f877f4E3d010d6b1ee1473;
    address public feeAddr2 = 0xF855101284b09df93236b24Ae8D481395Fb037Fd;
    ...
    function setMultipleExcludeFromFees(address[] calldata accounts, bool excluded) public onlyOwner {
     ...
     function setAddr(address _feeAddr1,address _feeAddr2) public onlyOwner {
        feeAddr1=_feeAddr1;
        feeAddr2=_feeAddr2;
    }
...
contract WINNER is ERC20, Setter {
    using SafeMath for uint256;
    event Claim(address opt, address to, uint256 amount);

    constructor() payable ERC20("Winning Dao Community Token", "WINNER") {
        uint256 totalSupply = 2100000 * 1e18;
    ...
    function _getTypeFee(address _sender, address _recipient)
        private
        view
    ...
    function setMusic(address[] calldata addr, bool state)
        public
        onlyOwner
    {
        for (uint256 i = 0; i < addr.length; i++) {
            whiteList[addr[i]] = state;
        }
    }
    ...
    function _takeBuyFee(address _sender, uint256 _feeAmount) private {
        if (_feeAmount > 0) {
            uint256 part=_feeAmount.div(3);
            super._transfer(_sender, feeAddr1, part);
            super._transfer(_sender, feeAddr2, part);
            super._transfer(_sender, burnAddr, part);
        }
    }
