...
contract PEPEBSC is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    PEPEBSCDividendTracker public dividendTracker;
    ...
    constructor() ERC20("PEPEBSC", "PEPEBSC") {
        marketingWallet = payable(0x9B5C0e01B22f0aD51105f4fED7bA23c49CC3b141);
        devWallet = payable(0x9B5C0e01B22f0aD51105f4fED7bA23c49CC3b141);
        ...
        dividendTracker = new PEPEBSCDividendTracker(
            payable(this),
            router,
        ...
        maxWallet = totalTokenSupply / 80000000; // 0.5%
        swapTokensAtAmount = totalTokenSupply / 80000000; // 2%;
...
contract PEPEBSCDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    PEPEBSC public PEPEBSCContract;
    ...
    constructor(
        ...
        defaultToken = token;
        PEPEBSCContract = PEPEBSC(mainContract);
    ...
    function _transfer(
        ...
        require(false, "PEPEBSC_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "PEPEBSC_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main PEPEBSC contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(PEPEBSCContract);

            uint256 prevBalance = PEPEBSCContract.balanceOf(address(this));
            ...
            {
                uint256 received = PEPEBSCContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    PEPEBSCContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
