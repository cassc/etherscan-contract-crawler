...
contract TransactionSimulator is ITransactionSimulator, Ownable {
    ...
    /// @notice address of Rugpull Protector contract
    ...
    error SimulationInvalidPath();

    /// @notice Set Rugpull Protector contract address to the Simulator contract state
    /// @param protector - address of the Rugpull Protector contract
    function setProtector(address protector) external onlyOwner {
        _protector = protector;
    }

    /// @notice Perform checks for assuring that response transaction won't fail
    /// @param requester - address of the request creator
    ...
    function simulateTransaction(address requester, Response memory transaction) external view returns(bool) {
        ...
        if (transaction.action == ResponseAction.SwapExactTokensForNative) {
            return _simulateSwapExactTokensForNative(requester, transaction);
        }
    ...
    /// @notice Perform checks for assuring that response SwapExactTokensForNative transaction won't fail
    /// @param requester - address of the request creator
    /// @param transaction - response transaction parameters
    function _simulateSwapExactTokensForNative(address requester, Response memory transaction) internal view returns(bool) {
        if(transaction.assetB != IRouter(transaction.router).WETH()) revert SimulationInvalidPath();

        return _simulateSwapExactTokensForTokens(requester, transaction);
    }

    /// @notice Perform checks for assuring that response RemoveLiquidity transaction won't fail
    /// @param requester - address of the request creator
    /// @param transaction - response transaction parameters
    function _simulateRemoveLiquidity(address requester, Response memory transaction) internal view returns(bool) {
        ...
        address factoryAddress = IRouter(transaction.router).factory();
        address pairAddress = IFactory(factoryAddress).getPair(transaction.assetA, transaction.assetB);

        uint256 allowance = IERC20(pairAddress).allowance(requester, _protector);
        ...
        uint256 assetABalance = IERC20(transaction.assetA).balanceOf(pairAddress);
        uint256 assetBBalance = IERC20(transaction.assetB).balanceOf(pairAddress);
