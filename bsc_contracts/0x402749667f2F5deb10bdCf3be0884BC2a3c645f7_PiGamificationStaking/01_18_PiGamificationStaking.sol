    ...
    TokensRecoverable
{
    ...
    mapping(address => mapping(uint32 => uint32)) private _stakedTokens;
    // Mapping from token ID to index of the staker tokens list
    mapping(uint32 => uint32) private _stakedTokensIndex;
        ...
        //updateReward(_msgSender())
    {
        ...
        stakingNFT.batchSafeTransferFromSmallInt(
            _msgSender(),
            address(this),
            tokenIds,
            ""
        ...
        uint points = BLLContract.getTotalPointsForTokenIDs(tokenIds);
        _totalSupply += points;
        _balances[_msgSender()] += points;
        emit Staked(_msgSender(), tokenIds);
    }

    function withdraw(uint32[] memory tokenIds)
        public
        nonReentrant
        updateReward(_msgSender())
    {
        ...
        uint256 totalValue = BLLContract.getTotalPointsForTokenIDs(tokenIds);
        _balances[_msgSender()] -= totalValue;
    ...
    function exit(uint32[] memory tokenIds) external {
        withdraw(tokenIds);
        getReward();
    }
        ...
        returns (uint32[] memory)
    {
        if (stakeRefreshed[account]) {
            ...
            for (uint32 x; x < bal; x++) {
                returnVal[x] = _tokenOfStakerByIndex(account, x);
            }
            return returnVal;
        }
    ...
    function _tokenOfStakerByIndex(address account, uint32 index)
        private
        view
        returns (uint32)
    {
     ...
     */
    function _addTokenToStakerEnumeration(address account, uint32 tokenId)
        private
    {
        uint32 length = uint32(_tokenBalances[account]);
     ...
     */
    function _removeTokenFromStakerEnumeration(address account, uint32 tokenId)
        private
    {
        // To prevent a gap in account's tokens array, we store the last token in the index of the token to delete, and
        // then delete the last slot (swap and pop).

        uint32 lastTokenIndex = uint32(_tokenBalances[account]) - 1;
        uint32 tokenIndex = _stakedTokensIndex[tokenId];

        // When the token to delete is the last token, the swap operation is unnecessary
        if (tokenIndex != lastTokenIndex) {
            uint32 lastTokenId = _stakedTokens[account][lastTokenIndex];
