...
contract PancakeFlashSwap {
    ...
    address private constant PANCAKE_ROUTER =
        ...
        address private constant APE_FACTORY =
        0x0841BD0B734E4F5853f0dD8d7Ea041c241fb0Da6;
    address private constant APE_ROUTER =
    ...
    function startArbitrage(address _tokenBorrow,address _tokenSwap, uint256 _amount) external {
        IERC20(_tokenSwap).safeApprove(address(PANCAKE_ROUTER), MAX_INT);
        IERC20(_tokenBorrow).safeApprove(address(PANCAKE_ROUTER), MAX_INT);

        IERC20(_tokenBorrow).safeApprove(address(APE_ROUTER), MAX_INT);
        IERC20(_tokenSwap).safeApprove(address(APE_ROUTER), MAX_INT);
        ...
        bytes memory data = abi.encode(_tokenBorrow,_tokenSwap, _amount, msg.sender);
    ...
    function pancakeCall(
        ...
        (address tokenBorrow,address tokenSwap, uint256 amount, address myAddress) = abi.decode(
            _data,
        ...
        // Place Trades
        uint256 trade1AcquiredCoin = placeTrade(tokenBorrow, tokenSwap, loanAmount,PANCAKE_FACTORY,PANCAKE_ROUTER);
        uint256 trade2AcquiredCoin = placeTrade(tokenSwap, tokenBorrow, trade1AcquiredCoin,APE_FACTORY,APE_ROUTER);
