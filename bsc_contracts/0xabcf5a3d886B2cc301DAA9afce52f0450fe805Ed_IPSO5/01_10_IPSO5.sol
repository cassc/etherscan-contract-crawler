...
contract IPSO5 is ReentrancyGuard, Ownable {
  ...
  constructor(
    ...
    function canInvestMax(address _user) public view returns (uint)
    {
        ...
        uint amountRemainingToInvest = raisingAmount.mul(5).div(2) >= totalAmountInvested ? raisingAmount.mul(5).div(2).sub(totalAmountInvested) : 0;
    ...
    function invest(uint256 _amount) public
    {
        ...
        require (raisingAmount.mul(5).div(2) >= totalAmountInvested, 'IPSO already full');
