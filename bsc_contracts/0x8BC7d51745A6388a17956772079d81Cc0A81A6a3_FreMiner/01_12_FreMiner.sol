...
contract FreMiner is ReentrancyGuard {
    ...
    address private _freaddr =
        address(0xb41ba25fA541172110CCC4C530726b611aaf9278);
    address private _fretrade =
        address(0x28bA842612dF99212540A71A08432C24aD9f837F);
    ...
    // // testnet
    // address private _freaddr =
    //     address(0xB78eA2d98612Cae728c107Dd0889D4E0d7cdB781);
    // address private _fretrade =
    //     address(0x71758C564CD801861A40B3B9c85f0765b4d306bd);
    // address private _bnbtradeaddr =
    ...
    mapping(uint256 => uint256[2]) _checkpoints;
    ...
    constructor() {
        ...
        InitalContract(address(0x95848045d5D15c399860319E152Fe67F3D38D27B));

        addTradingPool(_freaddr, _fretrade, 100, 10, 100);
        addTradingPool(_usdtaddr, _bnbtradeaddr, 100, 10, 100);
        addTradingPool(address(0x0000000000000000000000000000000000000002), _bnbtradeaddr, 100, 10, 100);
        addTradingPool(address(0x2170Ed0880ac9A755fd29B2688956BD959F933F8), address(0x74E4716E431f45807DCF19f284c7aA99F18a4fbc), 100, 10, 100);
        addTradingPool(address(0x7130d2A12B9BCbFAe4f2634d864A1Ee1Ce3Ead9c), address(0x61EB789d75A95CAa3fF50ed7E47b96c132fEc082), 100, 10, 100);

        SetParentByAdmin(address(0x484e8EfAefAe16633F694C690ad7Eff0a54239EF), address(this));
    }
    ...
    function InitalContract(address feeowner) public {
        ...
        _checkpoints[_maxcheckpoint][1] = uint256(100e18).div(28800);
        _currentMulitiper = uint256(100e18).div(28800);
    }

    function getCurrentCheckPoint() public view returns (uint256[2] memory) {
    ...
    function logCheckPoint(
        ...
        if (_nowtotalhash != 0) {
            _checkpoints[_maxcheckpoint][1] = _currentMulitiper.mul(1e18).div(_nowtotalhash);
        } else {
            _checkpoints[_maxcheckpoint][1] = _currentMulitiper;
        }
        ...
        returns (bool)
    {
        ...
        if (!_isUserAddresses[msg.sender]) {
            _useraddresses.push(msg.sender);
            _isUserAddresses[msg.sender] = true;
        }
    ...
    function setCurrentMulitiper(uint256 newCurrentMulitiper) public {
        ...
        for (uint256 i = 0; i < _useraddresses.length; i++) {
            uint256 amount = getPendingCoin(_useraddresses[i]);

            _userInfos[_useraddresses[i]].pendingreward = amount;
            _userInfos[_useraddresses[i]].lastblock = block.number;
            if (_maxcheckpoint > 0)
                _userInfos[_useraddresses[i]].lastcheckpoint = _maxcheckpoint;
        }

        _maxcheckpoint++;
        _checkpoints[_maxcheckpoint][0] = block.number;
        if (_nowtotalhash != 0) {
            _checkpoints[_maxcheckpoint][1] = newCurrentMulitiper.mul(1e18).div(_nowtotalhash);
        } else {
            _checkpoints[_maxcheckpoint][1] = newCurrentMulitiper;
        }
