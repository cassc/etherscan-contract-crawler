...
/**
TG: https://t.me/PereeeeGlobal
Twitter: https://twitter.com/PereeeeBSC
*/
...
contract REEEE is ERC20, Ownable {
    ...
    constructor() ERC20("Pereeee Coin", unicode"REEEE") {
        ...
        uint256 totalSupply = 42 * 1e11 * 1e9;

        maxTransactionAmount = 42 * 1e9 * 1e9;
        maxWallet = 42 * 1e9 * 1e9 * 3;
        swapTokensAtAmount = 42 * 1e8 * 1e9 * 3;
        ...
        devWallet = address(0x9e18B50CD871da5e5c10c4eD925623E5669a6189); 
        teamWallet = address(0x368275c46a8B6271b24bAF9221fC38FE49D820d8);
    ...
    function _transfer(
        ...
        if (takeFee) {
            // on sell
            if (automatedMarketMakerPairs[to] && getDynamicSellFee() > 0) {
                fees = amount.mul(getDynamicSellFee()).div(100);
                tokensForLiquidity += (fees * sellLiquidityFee) / getDynamicSellFee();
                tokensForMarketing += (fees * sellMarketingFee) / getDynamicSellFee();                
            }
            // on buy
