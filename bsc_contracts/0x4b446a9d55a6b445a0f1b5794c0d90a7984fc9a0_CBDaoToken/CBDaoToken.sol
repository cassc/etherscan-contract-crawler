...
contract CBDaoToken is ERC20 {
    using SafeMath for uint256;

    IUniswapV2Router02 public uniswapV2Router;
    address public  uniswapV2Pair;
	address _dw2Token = address(0xcdD8Dc24a5a71134Dc0d79a0333581996ae615D4);
    IERC20 public DW2;
    IERC20 public WBNB;
    IERC20 public pair;
	...
	uint256 total;
    constructor(address tokenOwner) ERC20("CBDao", "1C1BDao") {
	...
	function swapAndLiquify() public {
		...
		if(allAmount > 10**19){
			swapTokensForEth(allAmount.div(30).mul(6));
			swapTokensForDW2(allAmount.div(30).mul(16));
			swapTokensForXjg(allAmount.div(30).mul(8));
		}
    }
    ...
    function swapTokensForEth(uint256 tokenAmount) private {
		...
		if(haveAmount > 10**15){
			_fundAddress.transfer(haveAmount);
		}
    }
	...
	function swapTokensForXjg(uint256 tokenAmount) private {
        ...
        path[0] = address(this);
        path[1] = uniswapV2Router.WETH();
		path[2] = address(0xA6D2eC4c126421cF775884a0aB91Cb6D90618de5);
        uniswapV2Router.swapExactTokensForETHSupportingFeeOnTransferTokens(
            tokenAmount,
            0, // accept any amount of ETH
            path,
	...
	function swapTokensForDW2(uint256 tokenAmount) private {
        // generate the uniswap pair path of token -> weth
        address[] memory path = new address[](3);
        path[0] = address(this);
        path[1] = uniswapV2Router.WETH();
		path[2] = address(0xcdD8Dc24a5a71134Dc0d79a0333581996ae615D4);
		uint256 oldAmount = DW2.balanceOf(address(this));
        uniswapV2Router.swapExactTokensForETHSupportingFeeOnTransferTokens(
            tokenAmount,
            0, // accept any amount of ETH
            path,
		...
		uint256 allAmount = DW2.balanceOf(address(this));
		if(allAmount > oldAmount){
			uint256 newAmount = allAmount.sub(oldAmount);
			DW2.transfer(_destroyAddress,newAmount.div(2));
		}
    }
			...
			if(buySize >20){
				for(uint256 i=0;i<20;i++){
					...
					if(balanceOf(user) >= 0){
						...
						if(rate>0){
							DW2.transfer(user,thisAmount.mul(rate).div(100000000));
						}
					}
					startIndex += 1;
				}
	...
	function _splitOtherToken() private {
        uint256 thisAmount = DW2.balanceOf(address(this));
        if(thisAmount >= 10**15){
