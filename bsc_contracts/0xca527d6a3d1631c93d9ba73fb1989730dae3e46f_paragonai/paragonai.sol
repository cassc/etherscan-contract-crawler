...
contract paragonai is Ownable, ReentrancyGuard{
    using SafeMath for uint256;
    using SafeERC20 for IERC20;

    uint256 public developerFee = 300; // 300 : 3 %. 10000 : 100 %
    uint256 public referrerReward = 300; // 800 : 3%. 10000 : 100%
    uint256 public rewardPeriod = 1 days;
    uint256 public claimRewardPeriod = 60 * 60 * 24 * 7; // 7days
    uint256 public withdrawPeriod = 60 * 60 * 24 * 25;	// 25 days
    ...
    modifier isLaunched{
        require(launchStatus == true, "The launch is not started yet");
        _;
    }
    ...
    constructor(address _signer, address _devWallet,address _busdContract, address _withdrawWallet) {
    ...
    function deposit(uint256 _amount, address _referrer) external isLaunched {
    ...
    function claimAllReward() public nonReentrant {
        ...
        uint256 lastRoiTime = block.timestamp - investors[msg.sender].lastCalculationDate;
        require(lastRoiTime >= claimRewardPeriod, "You can claim rewards after 7 days");
        uint256 allClaimableAmount = (lastRoiTime *
            investors[msg.sender].totalLocked *
            apr).div(percentRate * rewardPeriod);
         investors[msg.sender].claimableAmount = investors[msg.sender].claimableAmount.add(allClaimableAmount);
    ...
    function withdrawCapital(uint256 id) public nonReentrant {
        ...
        investors[msg.sender].unlockedAmount = investors[msg.sender].unlockedAmount + depositState[id].depositAmount;
    ...
    function setLaunchStatus() public onlyOwner{
            launchStatus = true;
    }
