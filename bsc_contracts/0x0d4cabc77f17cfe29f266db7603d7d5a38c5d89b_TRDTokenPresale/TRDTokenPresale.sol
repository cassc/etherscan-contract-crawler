...
contract TRDTokenPresale {
   ...
   function transferTokensToPresale(address _trdPresaleAddress) external {
    // Load the TRD token contract
    TRDToken trdToken = TRDToken(trdTokenAddress);
    
    // Determine the amount of tokens to transfer
    uint256 amountToTransfer = 1000000 * (10 ** 18); // Example: 1,000,000 TRD

    // Execute the token transfer
    trdToken.transfer(_trdPresaleAddress, amountToTransfer);
}
