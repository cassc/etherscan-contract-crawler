...
if (automatedMarketMakerPairs[sender]) _realFee = totalBuyFee;
    ...
    function getRebaseDailyStakedToken(address user) public view returns (uint256 _nextRebase) {
        uint256 nextRebase;
        nextRebase += (stacking[user][0].amount.div(_yields[1]) - stacking[msg.sender][0].amountClaim) * 9 / 1000;
        nextRebase += (stacking[user][1].amount.div(_yields[2]) - stacking[msg.sender][1].amountClaim) * 11 / 1000;
        nextRebase += (stacking[user][2].amount.div(_yields[3]) - stacking[msg.sender][2].amountClaim) * 13 / 1000;
        return nextRebase;
    }
