    ...
    function priceOf(address _token) public view returns (uint bnbPrice, uint usdPrice) {
        if (tokenFeeds[wbnb] != address(0) && tokenFeeds[_token] != address(0)) {
            ...
            if (_token == wbnb) {
                ...
                INovationFactory factory = INovationFactory(INovationRouter02(unirouter).factory());
                if (factory.getPair(_token, wbnb) == address(0)) return (0, 0);
                ...
                (, int256 _bnbUsdPrice, , ,) = IAggregatorV3(tokenFeeds[wbnb]).latestRoundData();
