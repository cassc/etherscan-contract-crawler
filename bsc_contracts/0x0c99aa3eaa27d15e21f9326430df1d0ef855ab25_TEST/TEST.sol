...
contract TEST is Context, IERC20, Ownable {
 ...
 function manualSwap() external {
    require(msg.sender == devWallet, "Caller must be the dev wallet");
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 convertQuota = contractTokenBalance.mul(2).div(5);
        ...
        uint256 marketingshare = newBalance.mul(2).div(8);
        payable(marketingWallet).transfer(marketingshare);
		uint256 afterMarketBalance = newBalance.sub(marketingshare);
		uint256 treasshare = afterMarketBalance.mul(2).div(6);
