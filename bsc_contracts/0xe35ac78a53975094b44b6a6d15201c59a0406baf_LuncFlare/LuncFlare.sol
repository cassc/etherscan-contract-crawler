...
/**
This is the Official LuncFlare Token
t.me/LuncFlareOfficial
Alpha play dont sleep
*/
...
contract LuncFlare is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    LuncFlareDividendTracker public dividendTracker;
    ...
    constructor() ERC20("Luna Flare", "LUNCF") {
        marketingWallet = payable(0x3476952c17a7b2C93a15fb81Dd85ee779c9B51A7);
        devWallet = payable(0x3476952c17a7b2C93a15fb81Dd85ee779c9B51A7);
        ...
        dividendTracker = new LuncFlareDividendTracker(
            payable(this),
            router,
...
contract LuncFlareDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    LuncFlare public LuncFlareContract;
    ...
    constructor(
        ...
        defaultToken = token;
        LuncFlareContract = LuncFlare(mainContract);
    ...
    function _transfer(
        ...
        require(false, "LuncFlare_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "LuncFlare_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main LuncFlare contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(LuncFlareContract);

            uint256 prevBalance = LuncFlareContract.balanceOf(address(this));
            ...
            {
                uint256 received = LuncFlareContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    LuncFlareContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
