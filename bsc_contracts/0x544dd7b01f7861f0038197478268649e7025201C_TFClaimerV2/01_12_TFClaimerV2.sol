...
contract TFClaimerV2 is OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    mapping(uint => mapping(uint => uint)) public claimPercents;    // poolid timeid percents  1e10 = 100%
    ...
    function setClaimPercents(uint _id, uint[] memory _timeIds, uint[] memory _percents) public onlyOwner {
        ...
        require(totalPercent == 1e10, "!totalPercent");
    }
    ...
    function pendingTimePoint(uint _tid, address _user) public view returns (uint timeamount, uint timeclaim) {
        for(uint pid = 0; pid < poolIds.length; pid++) {
            ...
            uint poolamount = allocation[pid][_user].mul(claimPercents[pid][_tid]).div(1e10);
