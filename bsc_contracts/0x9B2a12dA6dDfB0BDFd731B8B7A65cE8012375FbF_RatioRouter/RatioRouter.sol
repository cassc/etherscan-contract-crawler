...
// File: interfaces/IRatioRouter.sol
...
// File: interfaces/IERC20PermitAllowed.sol
...
/// @notice Router contract that helps in swapping across Ratio pools.
contract RatioRouter is IRatioRouter, SelfPermit, Multicall {
    ...
    function exactInputSingle(ExactInputSingleParams calldata params) public payable returns (uint256 amountOut) {
        ...
        gni.transfer(params.tokenIn, msg.sender, params.pool, params.amountIn);
    ...
    function exactInput(ExactInputParams calldata params) public payable returns (uint256 amountOut) {
        ...
        gni.transfer(params.tokenIn, msg.sender, params.path[0].pool, params.amountIn);
    ...
    /// it takes raw ERC-20 tokens from the users and deposits them into `gni`.
    /// @param params This includes the address of token A, pool, amount of token A to swap,
    /// minimum amount of token B after the swap and data required by the pool for the swap.
    /// @dev Ensure that the pool is trusted before calling this function. The pool can steal users' tokens.
    function exactInputSingleWithNativeToken(ExactInputSingleParams calldata params) public payable returns (uint256 amountOut) {
        // Deposits the native ERC-20 token from the user into the pool's `gni`.
        _depositToGniLamp(params.tokenIn, params.pool, params.amountIn);
        // Trigger the swap in the pool.
        amountOut = IPool(params.pool).swap(params.data);
        // Ensure that the slippage wasn't too much. This assumes that the pool is honest.
        if (amountOut < params.amountOutMinimum) revert TooLittleReceived();
    ...
    function exactInputWithNativeToken(ExactInputParams calldata params) public payable returns (uint256 amountOut) {
        // Deposits the native ERC-20 token from the user into the pool's `gni`.
        _depositToGniLamp(params.tokenIn, params.path[0].pool, params.amountIn);
        // Call every pool in the path.
        // Pool `N` should transfer its output tokens to pool `N+1` directly.
        // The last pool should transfer its output tokens to the user.
        for (uint256 i; i < params.path.length; ) {
    ...
    function complexPath(ComplexPathParams calldata params) public payable {
        ...
        for (uint256 i; i < params.initialPath.length; ) {
            if (params.initialPath[i].native) {
                _depositToGniLamp(params.initialPath[i].tokenIn, params.initialPath[i].pool, params.initialPath[i].amount);
            } else {
                gni.transfer(params.initialPath[i].tokenIn, msg.sender, params.initialPath[i].pool, params.initialPath[i].amount);
            }
        ...
        for (uint256 i; i < params.percentagePath.length; ) {
            uint256 balanceShares = gni.balanceOf(params.percentagePath[i].tokenIn, address(this));
            uint256 transferShares = (balanceShares * params.percentagePath[i].balancePercentage) / uint256(10)**8;
            gni.transfer(params.percentagePath[i].tokenIn, address(this), params.percentagePath[i].pool, transferShares);
        ...
        for (uint256 i; i < params.output.length; ) {
            uint256 balanceShares = gni.balanceOf(params.output[i].token, address(this));
            if (balanceShares < params.output[i].minAmount) revert TooLittleReceived();
            if (params.output[i].unwrapGni) {
                gni.withdraw(params.output[i].token, address(this), params.output[i].to, 0, balanceShares);
            } else {
                gni.transfer(params.output[i].token, address(this), params.output[i].to, balanceShares);
            }
            unchecked {
                ++i;
            }
    ...
    function addLiquidity(
        ...
        for (uint256 i; i < tokenInput.length; ) {
            if (tokenInput[i].native) {
                _depositToGniLamp(tokenInput[i].token, pool, tokenInput[i].amount);
            } else {
                gni.transfer(tokenInput[i].token, msg.sender, pool, tokenInput[i].amount);
            }
            unchecked {
                ++i;
            }
    ...
    /// @notice Burn liquidity tokens to get back `gni` tokens.
    /// @param pool Pool address.
    /// @param liquidity Amount of liquidity tokens to burn.
    /// @param data Data required by the pool to burn liquidity.
    /// @param minWithdrawals Minimum amount of `gni` tokens to be returned.
    ...
    function sweep(
        ...
        if (onGni) {
            gni.transfer(token, address(this), recipient, amount);
        } else {
   ...
   /// @notice Wrapper function to allow pool deployment to be batched. 
    ...
    /// @notice Wrapper function to allow gni set master contract approval to be batched, so the first trade can happen in one transaction.
    function approveMasterContract(
        uint8 v,
        bytes32 r,
        bytes32 s
    ) external payable {
        gni.setMasterContractApproval(msg.sender, address(this), true, v, r, s);
    }

    /// @notice Deposit from the user's wallet into GniLamp.
    /// @dev Amount is the native token amount. We let GniLamp do the conversion into shares.
    function _depositToGniLamp(
        address token,
        address recipient,
        uint256 amount
    ) internal {
        gni.deposit{value: token == USE_NATIVE ? amount : 0}(token, msg.sender, recipient, amount, 0);
    }
