...
contract FurMax is BaseContract
{
    ...
    event FurpoolStaked(uint256 amount);
    ...
    function join(bool acceptTerms_, uint256 furbet_, uint256 furbot_, uint256 furpool_) external
    {
        ...
        totalParticipants++;
        _walletIdTracker++;
        walletId[msg.sender] = _walletIdTracker;
    ...
    function distribute(address participant_, uint256 amount_) external onlyDistributors
    {
        ...
        if(walletId[participant_] == 0) {
            _walletIdTracker++;
            walletId[participant_] = _walletIdTracker;
        }
     ...
     * Stake Furpool.
     */
    function stakeFurpool() external
    {
        ...
        emit FurpoolStaked(totalFurpoolPendingInvestment);
        totalFurpoolInvestment += totalFurpoolPendingInvestment;
     ...
     * Claim Furpool.
     */
    function claimFurpool() external
    {
        _furpool.claimRewards();
        totalFurpoolDividends = _furpool.usdcClaimed(address(this));
        uint256 _balance_ = _usdc.balanceOf(address(this));
        if(totalFurpoolDividends < _balance_) totalFurpoolDividends = _balance_;
    }
