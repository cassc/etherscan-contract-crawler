...
library ListAddress
{
    ...
    function remove(ListStruct storage self, address account) internal
    {
        ...
        if (itemIndex > 0)
        {
            // @@@ audit revision: added self.Array.length > 0 check so this function will not revert in any case
            if (self.Array.length > 0)
            {
...
contract MORE is IERC20
{
    ...
    */
    ...
    event Locked(address indexed account, uint128 amount, uint64 lockIndex, uint64 isAddition);
    event Unlocked(address indexed account, uint256 lockIndex);
    ...
    constructor()
    {
        ...
        /*
        @@@ audit revision: all tokens are initially going to the deployer address
        tokens for the liquidity injection will be locked initially and sent to the contract before launch
        the purpose of this is purely to make tokens counted as locked on the presale page
        if they are initially on token's contract then they will be marked as unlocked despite we can't access them
        */
    ...
    locks are generally will be done through a dapp, so if user trusts the dapp - they enable locks
    ...
    */
    function lockTokens(uint128 amount, uint32 duration) external
    {
        ...
        Agent.tryToLock(msg.sender, amount, duration);
        ...
        emit Locked(msg.sender, amount, lockConfig.nextLockIndex - 1, 0);
    }

    /*
    @@@ audit revision
    it is possible to add extra tokens to the existing lock
    this action will not change the unlock time
    */
    function lockExtraTokens(uint256 lockIndex, uint128 amount) external
    {
        _beforeLock(amount);

        Agent.tryToLockExtra(msg.sender, lockIndex, amount);
        ...
        emit Locked(msg.sender, amount, uint64(lockIndex), 1);
    }

    /*
    amount that is locked is stored in lockConfig.totalLocked for every user
    there are no automatic unlocks as the number of locks is not limited
    user must make a reasonable amount of locks as its only possible to lock once in a single tx
    butit is possible to unlock multiple lock instances using external contracts
    */
    function _beforeLock(uint128 amount) private
    {
        if (Modifiers[msg.sender].oldMultiplierBalance > 0)
        {
    ...
    */
    ...
    {
        // leaves an opportunity to cancel the user's lock in case of an emergency
        ...
        emit Unlocked(account, lockIndex);
    }

    // leaves an opportunity to cancel the user's lock in case of an emergency
    function unlockTokensEarly(uint256 lockIndex) external
    {
        ...
        emit Unlocked(msg.sender, lockIndex);
    }
