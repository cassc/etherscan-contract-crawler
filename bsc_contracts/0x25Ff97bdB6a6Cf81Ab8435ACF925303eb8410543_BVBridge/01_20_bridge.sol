...
contract BVBridge is ERC721HolderUpgradeable, ERC1155HolderUpgradeable, OwnableUpgradeable, DivestorUpgradeable {
    ...
    event AcrossERC20(address indexed account, address indexed token, bool indexed isNew, uint256 amount);
    ...
    function updateNewInfo(address newBvg_) public onlyOwner {
        tokenInfo[newBvg_] = Token({isOpen : true, tax : 1, mode : 20, minAmount : 50000 ether, token : newBvg_});
    }
    ...
    function acrossERC20(
        ...
        emit AcrossERC20(msg.sender, token_, isNew_, amount_);
    }
    ...
    function sendERC20(
        ...
        if (token_ == 0x29AB77a43Ec69D0DFFf3Ff72bff457e09b258351) {
            amount_ /= 5000;
        }
    ...
    function sedERC721(
        ...
        for (uint256 i; i < cardIds_.length; i++) {
            I721(token_).mint(account_, cardIds_[i]);
        }
