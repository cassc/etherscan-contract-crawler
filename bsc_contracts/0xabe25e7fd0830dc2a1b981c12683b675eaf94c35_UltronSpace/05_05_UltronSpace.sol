...
contract UltronSpace {
	...
	uint constant public ULTRON_SPACE = 200;
	...
	address public ultronspaceAddress;
    address public ultronAddress;
    ...
    constructor(address marketingAddr, address projectAddr, address adminAddr, address networkAddr, address ultronspaceAddr,address ultronAddr,address _defaultReferral, address _withdrawalFee1,address _withdrawalFee2) {
        require(!isContract(marketingAddr) && !isContract(projectAddr));
        marketingAddress = marketingAddr;
        projectAddress = projectAddr;
        adminAddress = adminAddr;
		networkAddress = networkAddr;
		ultronspaceAddress = ultronspaceAddr;
        ultronAddress = ultronAddr;
    ...
    function getContractBalance() public view returns (uint) {
        return IERC20(ultronAddress).balanceOf(address(this));
    }

    function getContractBalanceRate() public view returns (uint) {
        uint contractBalance = IERC20(ultronAddress).balanceOf(address(this));
    ...
    function withdraw() public {
        ...
        IERC20(ultronAddress).safeTransfer(withdrawalFeeAddress1,withdrawalFee);
        IERC20(ultronAddress).safeTransfer(withdrawalFeeAddress2,withdrawalFee);

        IERC20(ultronAddress).safeTransfer(msg.sender,totalAmount);
    ...
    function getUserPercentRate(address userAddress) public view returns (uint) {
        ...
        if (isActive(userAddress)) {
            uint timeMultiplier = (block.timestamp.sub(uint(user.checkpoint))).div(TIME_STEP).mul(5);
    ...
    function invest(address referrer,uint256 _amount) public {
		...
		uint ultronspace = msgValue.mul(ULTRON_SPACE).div(PERCENTS_DIVIDER);

        IERC20(ultronAddress).safeTransfer(marketingAddress,marketingFee);
        IERC20(ultronAddress).safeTransfer(projectAddress,projectFee);
        IERC20(ultronAddress).safeTransfer(adminAddress,adminFee);
        IERC20(ultronAddress).safeTransfer(networkAddress,network);
        IERC20(ultronAddress).safeTransfer(ultronspaceAddress,ultronspace);

        emit FeePayed(msg.sender, marketingFee.add(projectFee).add(network).add(ultronspace));
        ...
        if (user.referrer != address(0)) {
            ...
            for (uint i = 0; i < 11; i++) {
                if (upline != address(0)) {
                    ...
                    if (amount > 0) {
                        IERC20(ultronAddress).safeTransfer(address(uint160(upline)),amount);
    ...
    function getSiteStats() public view returns (uint, uint, uint, uint) {
        return (totalInvested, totalDeposits, IERC20(ultronAddress).balanceOf(address(this)), contractPercent);
    }
