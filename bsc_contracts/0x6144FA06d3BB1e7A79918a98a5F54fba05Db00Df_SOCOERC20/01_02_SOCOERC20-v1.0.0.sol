...
interface ISOCOAffiliate {
    function bind(address sender, address receiver, uint256 amount) external;
...
contract SOCOERC20 is IERC20, Ownable {
    ...
    function _safeTransfer(address sender, address recipient, uint256 amount) internal virtual {
        ...
        if (sender != swapAddr) {
            ...
            if (!isContract(sender) && !isContract(recipient) && _affiliateAddr != address(0)) {
                _affiliateLike.bind(sender, recipient, amount);
            }
        } else {
            ...
            if (!isContract(recipient) && _bonusAddr != address(0) && _affiliateAddr != address(0)) {
