...
contract Durt is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    DurtDividendTracker public dividendTracker;
    ...
    constructor() ERC20("DUTCH RABBIT", "DURT") {
        marketingWallet = payable(0x35C5318D5a3F429828fcF73a8C974e6bc6ca37B3);
        devWallet = payable(0x35C5318D5a3F429828fcF73a8C974e6bc6ca37B3);
        ...
        dividendTracker = new DurtDividendTracker(
            payable(this),
            router,
...
contract DurtDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor(
        ...
        defaultToken = token;
        DurtContract = Durt(mainContract);
    ...
    function _transfer(
        ...
        require(false, "Durt_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "Durt_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(DurtContract);

            uint256 prevBalance = DurtContract.balanceOf(address(this));
            ...
            {
                uint256 received = DurtContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    DurtContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
