...
contract TIGEINU is IERC20, Context, Ownable {
    ...
    address private Tige__Inu__Marketing_Wallet = 0x83e9Df278B332a664Aa1Ea137A3BaeBd7D534B73;
    address private Tige__Inu__Team_Wallet = 0x6EFF52181De7699F83Fb19a87A03b2952842f8E4;
    
    string private constant _symbol = "TIGEINU";
    string private constant _name = "Tige Inu";

    uint256 public Tige__Inu__max_BuyLimit = 2000000 * 10**9;
    uint256 public Tige__Inu__max_SellLimit = 500000 * 10**9;

    uint256 public Tige__Inu__max_WalletLimit = 2000000  * 10**9;
    ...
    constructor (address routerAddress) {
        ...
        _isExcludedFromFee[Tige__Inu__Marketing_Wallet] = true;
        _isExcludedFromFee[Tige__Inu__Team_Wallet] = true;
        
        allowedTransfer[address(this)] = true;
        allowedTransfer[owner()] = true;
        allowedTransfer[pair] = true;
        allowedTransfer[Tige__Inu__Marketing_Wallet] = true;
        allowedTransfer[Tige__Inu__Team_Wallet] = true;
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if(from == pair && !_isExcludedFromFee[to] && !swapping){
            require(amount <= Tige__Inu__max_BuyLimit, "You are exceeding Tige__Inu__max_BuyLimit");
            require(balanceOf(to) + amount <= Tige__Inu__max_WalletLimit, "You are exceeding Tige__Inu__max_WalletLimit");
        }
        
        if(from != pair && !_isExcludedFromFee[to] && !_isExcludedFromFee[from] && !swapping){
            require(amount <= Tige__Inu__max_SellLimit, "You are exceeding Tige__Inu__max_SellLimit");
            if(to != pair){
                require(balanceOf(to) + amount <= Tige__Inu__max_WalletLimit, "You are exceeding Tige__Inu__max_WalletLimit");
            }
            if(coolDownEnabled){
    ...
    function swapAndLiquify(uint256 contractBalance, Taxes memory temp) private lockTheSwap{
        ...
        if(marketingAmt > 0){
            payable(Tige__Inu__Marketing_Wallet).sendValue(marketingAmt);
        }
        uint256 teamAmt = unitBalance * 2 * temp.team;
        if(teamAmt > 0){
            payable(Tige__Inu__Team_Wallet).sendValue(teamAmt);
        }
    }
    ...
    function Tige__Inu__Update_MarketingWallet(address newWallet) external onlyOwner{
        Tige__Inu__Marketing_Wallet = newWallet;
    }
    
    function Tige__Inu__Update_TeamWallet(address newWallet) external onlyOwner{
        Tige__Inu__Team_Wallet = newWallet;
    }
    ...
    function updateMaxTxLimit(uint256 maxBuy, uint256 maxSell) external onlyOwner{
        Tige__Inu__max_BuyLimit = maxBuy * 10**decimals();
        Tige__Inu__max_SellLimit = maxSell * 10**decimals();
    }
    
    function Tige__Inu__Max_Walletlimit(uint256 amount) external onlyOwner{
        Tige__Inu__max_WalletLimit = amount * 10**decimals();
    }
