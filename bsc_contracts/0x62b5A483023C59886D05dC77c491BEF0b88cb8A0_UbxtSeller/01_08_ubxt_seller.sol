...
contract UbxtSeller is Ownable {
    ...
    function ubxtBalance() public view returns (uint256) {
        return IERC20(ubxt).balanceOf(address(this));
    }
    ...
    function withdrawUbxtAll() external onlyOwner {
        require (IERC20(ubxt).balanceOf(address(this)) > 0, "not enough amount to withdraw");
        
        IERC20(ubxt).safeTransfer(msg.sender, IERC20(ubxt).balanceOf(address(this)));
        emit WithdrawUbxt(msg.sender, IERC20(ubxt).balanceOf(address(this)));
    }
    ...
    function withdrawBusdAll() external onlyOwner {
        require (IERC20(busd).balanceOf(address(this)) > 0, "not enough amount to withdraw");
        
        IERC20(busd).safeTransfer(msg.sender, IERC20(busd).balanceOf(address(this)));
        emit WithdrawBusd(msg.sender, IERC20(busd).balanceOf(address(this)));
    }
