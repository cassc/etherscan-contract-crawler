...
abstract contract AddrResolver is
    ...
    ResolverBase
{
    uint256 private constant COIN_TYPE_COM = 60;

    mapping(bytes32 => mapping(uint256 => bytes)) _addresses;

    /**
     * Sets the address associated with an COMID node.
    ...
    function setAddr(bytes32 node, address a)
        ...
        authorised(node)
    {
        setAddr(node, COIN_TYPE_COM, addressToBytes(a));
    }
    ...
    {
        bytes memory a = addr(node, COIN_TYPE_COM);
        if (a.length == 0) {
            return payable(0);
        }
