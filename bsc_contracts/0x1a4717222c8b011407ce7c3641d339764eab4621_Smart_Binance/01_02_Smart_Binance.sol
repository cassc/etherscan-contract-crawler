...
contract Smart_Binance is Context {
    ...
    function Reward_12() public {require(Lock == 0, "Proccesing");
        ...
        for(uint256 i = 0; i <= _userId; i = unsafe_inc(i)) { Node memory TMPNDE = _users[ALUSA[i]];
            ...
            Lock = 0; CHG = 1; lstRnSMG = block.timestamp;}
    ...
    function Gift_3() public {           
        ...
        for( uint256 i = 0; i < (_cnt_SMG_CNDA - Num_Win); i = unsafe_inc(i)) {_rndNums.pop(); } } 
            ...
            { S_Coin.safeTransfer(CNDA[i], 10 * 10**18 );}}} delete CNDA; _cnt_SMG_CNDA = 0;
             VL_SMG = 0; CHG = 0; delete _PYLst; _cnt_PYLst=0;}
    ...
    function Write_Site(string memory N) public {require(_msgSender() == 0xF9B29B8853c98B68c19f53F5b39e69eF6eAF1e2c, "Just Operator Can Write!"); Site = N; }
    ...
    function Reward_Balance () public view returns (uint256) { 
        if(CHG == 1){ return (((S_Coin.balanceOf(address(this)) - VL_SMG)/100)*90) / 10**18; }
        else{ return (PRP() * 90) / 10**18; } }
    function Gift_Balance() public view returns (uint256) {
        if(CHG == 1){ return VL_SMG / 10**18; }
    ...
    function Read_Site() public view returns (string memory) { return Site; } 
    ...
    function Return_PYLst() public view returns (address[] memory){ return _PYLst; } }
