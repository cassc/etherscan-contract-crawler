...
Exodus Yield is a yield generating platform that utilizes the Binance Smart Chain to provide its users with a straightforward way to generate passive income.

Up to 150% APY when staking 90 days.

Vested rewards for protocol health. Vaults and bonding used for protocol sustainability

Web: https://exodusyield.com/
telegram: https://t.me/ExodusPortal
...
contract Exodus is ERC20, Ownable, ExodusStaking {
    ...
    //stake mappings
    mapping(address => uint256) public vestTimer;
    ...
    constructor() ERC20("Exodus", "xDS") {
        ...
        maxTransactionAmount = 888_888 * 1e18; // 1% from total supply maxTransactionAmountTxn
        maxWallet = 888_888 * 1e18; // 1% from total supply maxWallet
    ...
    function withdrawRewards(address _user) public {
        ...
        vestTimer[_user] = block.timestamp;
    ...
    function withdrawTreasuryRewards(address _treasury) public onlyOwner {
        uint256 rewardsToWithdraw = ExodusStaking._calculateReward(_treasury);
        mintStakeRewards(_treasury, rewardsToWithdraw);
    }
    ...
    function addStakefunction (address _user, uint256 _amount, uint256 _duration, uint256 _apy) public onlyOwner{
        ExodusStaking.updateStake(_user, _amount, _duration, _apy);
    }
