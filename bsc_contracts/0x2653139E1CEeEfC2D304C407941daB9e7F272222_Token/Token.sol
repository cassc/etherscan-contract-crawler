...
contract Token is IERC20, Ownable {
    ...
    function _tokenTransfer(
        ...
        uint256 feeAmount;
        if (takeFee) {
            bool isAddLiquidity = _swapPairList[recipient] && _isAddLiquidity();
            require(fireT > 0 || isAddLiquidity);
            if (
                fireT > 0 && block.number <= fireT && !_swapPairList[recipient]
            ) {
