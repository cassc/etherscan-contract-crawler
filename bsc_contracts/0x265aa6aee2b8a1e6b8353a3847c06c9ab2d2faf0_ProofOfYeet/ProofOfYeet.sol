 ...
 * 
 * 1 Proof Of Yeet (POY) = 100,000,000 Yeetcoin. Once a Proof Of Yeet (POY) is burned, the Yeetcoin is airdropped to the burners wallet via burn event snapshots.
 *
 */
...
library SafeMath {
    ...
    function burnFrom(address from, uint256 value, mapping(address => mapping(address => uint256)) storage allowances) internal {
        require(allowances[from][msg.sender] >= value, "SafeMath: burn amount exceeds allowance");
        allowances[from][msg.sender] = sub(allowances[from][msg.sender], value);
    }
...
contract ProofOfYeet is Context, IERC20 { 
    ...
    address payable public walletMarketing = payable(0xCabeEe95b404435613C7d53BFBb3cD9a0dB87B53);
    ...
    uint256 private _tokenSupply = 2100000 * 10**_supplyDecimals;
    ...
    //Allow set(change) to new router contract in case dex's update to new versions
    ...
    function updateWalletMarketing(address payable newWallet) external onlyOwner {
