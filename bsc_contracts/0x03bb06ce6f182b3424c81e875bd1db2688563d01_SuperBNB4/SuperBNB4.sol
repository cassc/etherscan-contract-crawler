...
contract SuperBNB4 is IBEP20, Ownable {
    ...
    string private _name = "SUPER BNB 4.0 PROOF OF DEGEN";
    string private _symbol = "SBNBDEG";
    ...
    TierTaxes public _buyTaxes = TierTaxes({first: 10, second: 10, third: 10});

    TierTaxes public _sellTaxes = TierTaxes({first: 20, second: 20, third: 20});
    ...
    TaxWallets public _taxWallet =
        TaxWallets({
            dev: 0x3F4bfF765707036d0dBd584678C2F7008065Ee35,
            events: 0x3F4bfF765707036d0dBd584678C2F7008065Ee35,
            marketing: 0x3F4bfF765707036d0dBd584678C2F7008065Ee35
        });
    ...
    //TransferTax -- used to stop transfer of whitelisted tokens
    uint256 TransferTax = 99;
    ...
    uint256 private blacklistWindow = 12 hours;
    ...
    address public mainReward = 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c;
    ...
    function createLPandBNB(uint16 permilleOfPancake, bool ignoreLimits)
        external
        onlyOwner
    {
    ...
    function launch() external onlyOwner {
        if (IBEP20(_pancakePairAddress).totalSupply() == 0)
    ...
    function updateTransferTax(
        uint8 newTransferTax
    ) external onlyOwner {
        require(TransferTax > 0, "Launch Transfer Tax has been removed and cannot be re-enabled");
    ...
    function withdrawevents() external authorized {
        ...
        eventsBalance = 0;
        _sendBnb(_taxWallet.events, amount);
    }
    ...
    function recoverMiscToken(address tokenAddress) external authorized {
        if (
            tokenAddress == _pancakePairAddress || tokenAddress == address(this)
        ) revert InvalidInput();
    ...
    function getLiquidityRatio() public view returns (uint256) {
        uint256 ratio = (100 * _balances[_pancakePairAddress]) /
            _circulatingSupply;
        return ratio;
    }
    ...
    {
        return (buybackBalance, devBalance, eventsBalance, marketingBalance);
    }
    ...
    function _distributeStake(uint256 bnbAmount, bool newStakingReward)
        private
    {
        uint256 marketingSplit = (bnbAmount * _taxRatios.marketing) /
            distributeRatio;
        uint256 devSplit = (bnbAmount * _taxRatios.dev) / distributeRatio;
        uint256 buybackSplit = (bnbAmount * _taxRatios.buyback) /
            distributeRatio;
        uint256 stakingSplit = (bnbAmount * _taxRatios.rewards) /
            distributeRatio;
        uint256 eventsSplit = (bnbAmount * _taxRatios.events) / distributeRatio;
    ...
    function _swapContractToken(uint16 permilleOfPancake, bool ignoreLimits)
        private
        lockTheSwap
    {
    ...
    function _taxedTransfer(
        ...
        if (isSell) {
            ...
            if (amount <= 10**(TOKEN_DECIMALS)) {
                //transfer less than 1 token to ClaimBNB
                if (mainReward == _pancakeRouter.WETH())
                    claimBNBTo(
                        msg.sender,
                        msg.sender,
            ...
            if (TransferTax > 0) {
                TransferTaxAmount = (amount * TransferTax) / 100;
            }
    ...
    function subtractStake(
        ...
        if (_excludedFromStaking.contains(addr)) {
            if (main) toBePaidMain[addr] -= amount;
            else toBePaidMisc[addr] -= amount;
        } else {
            uint256 newAmount = newStakeOf(addr, main);
            if (main) {
                alreadyPaidMain[addr] = mainRewardShare * _balances[addr];
    ...
    {
        if (main) {
            if (_excludedFromStaking.contains(addr)) return toBePaidMain[addr];
            return newStakeOf(addr, true) + toBePaidMain[addr];
        } else {
            if (_excludedFromStaking.contains(addr)) return toBePaidMisc[addr];
