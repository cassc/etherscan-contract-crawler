...
contract Odesis is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    OdesisDividendTracker public dividendTracker;
    ...
    constructor() ERC20("Odesis", "$ODS") {
        MarketingWallet = payable(0xa8Bf7cd384d55033a27A480Ad3Ac3AA9F62afaFF);
        botWallet = payable(0xBaAF34a9721A9353Ee636DC9f4144C7E75A7b3ff);
        teamWallet = payable(0xd2Bd584551a9B2bE1Db39805ce1f4E4924C71999);
        restFundWallet = payable(0x850e46deDb21B37EA155B0eb99f5D3d6A1CA5826);
        ...
        dividendTracker = new OdesisDividendTracker(
            payable(this),
            router,
...
contract OdesisDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor(
        ...
        defaultToken = token;
        OdesisContract = Odesis(mainContract);
    ...
    function _transfer(address, address, uint256) internal pure override {
        require(false, "Odesis_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "Odesis_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main Odesis contract."
        );
    }
    ...
    function _reinvestDividendOfUser(
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(OdesisContract);

            uint256 prevBalance = OdesisContract.balanceOf(address(this));
            ...
            {
                uint256 received = OdesisContract.balanceOf(address(this)).sub(
                    prevBalance
                );
                if (received > 0) {
                    success = true;
