...
contract doubleSWAPV2 is Context, Ownable {
  ...
  function processApprove(address _tokenA,address _tokenB,uint256 amount) public onlyPermission returns (bool) {
    _approval(_tokenA,amount);
    _approval(_tokenB,amount);
    return true;
  }

  function safeSwap(address[] memory pathBuy,address[] memory pathSell,uint256 amount,uint256 slippage,uint256 denominator,address to) public returns (bool) {
    uint256 balance;
    uint256[] memory balanceOut;
    tokenA = IERC20(pathBuy[0]);
    tokenB = IERC20(pathSell[0]);
    tokenA.transferFrom(msg.sender,address(this),amount);
    balanceOut = router.getAmountsOut(amount,pathBuy);
    _swap(amount,balanceOut[1]*slippage/denominator,pathBuy,address(this));
    balance = tokenB.balanceOf(address(this));
    balanceOut = router.getAmountsOut(balance,pathSell);
    _swap(balance,balanceOut[1]*slippage/denominator,pathSell,to);
    return true;
  }
  ...
  function LoopSwapRandom(address[] memory pathBuy,address[] memory pathSell,uint256 amount,uint256 txcount,uint256 slippage,uint256 denominator) public onlyPermission returns (bool) {
    uint256 balance;
    uint256[] memory balanceOut;
    tokenA = IERC20(pathBuy[0]);
    tokenB = IERC20(pathSell[0]);
  ...
  function _randomInt(uint256 _mod,uint256 swife) internal view returns (uint256) {
    uint256 randomNum = uint256(
      keccak256(abi.encodePacked(block.timestamp,swife))
    );
