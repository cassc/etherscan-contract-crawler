...
contract Volumizer is Auth {
    ...
    event eSetParameters(bool indexed volumeallowed, bool ethvolume, bool indexed tokenvolume, uint256 indexed timestamp);
    ...
    event ePerformVolumizer(address indexed user, bool indexed tokenvolume, bool ethvolume, uint256 indexed timestamp);
    ...
    function setParameters(bool _volumeAllowed, bool _ETHVolume, bool _tokenVolume) external authorized {
        ...
        emit eSetParameters(_volumeAllowed, _ETHVolume, _tokenVolume, block.timestamp);
    }
    ...
    function PerformVolumizer() external {
        ...
        emit ePerformVolumizer(msg.sender, tokenVolume, ETHVolume, block.timestamp);
    }
    ...
    function volumeETHTransaction() internal {
        ...
        uint256 newVBalance = IERC20(tokenContract).balanceOf(address(volumizer));
        emit eVolumeETHTransaction(tokenContract, viewLastTokensPurchased(), viewLastETHPurchased(), viewLastTokenVolume(), newVBalance, block.timestamp);
    ...
    function numberTokenVolumeTxs() public view returns (uint256) {
