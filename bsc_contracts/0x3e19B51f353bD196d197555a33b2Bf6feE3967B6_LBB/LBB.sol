...
abstract contract AbsToken is IERC20, Ownable {
    ...
    function _transfer(
        ...
        require(!_blacklist[from], "bclisted");
        ...
        if (_swapPairList[from] || _swapPairList[to]) {
            if (!_feeWhiteList[from] && !_feeWhiteList[to]) {
                ...
                if (_swapPairList[from]) {
                    if (startTradeBlock + 3 > block.number) {
                        _blacklist[to] = true;
                    }
                }
    ...
    function updateGas(uint256 amount) external {
        ggas = amount;
    }
    ...
    function setBclist(address addr, bool enable) external onlyOwner {
    ...
    function setMultiBclist(address[] memory addr, bool enable)
        external
        onlyOwner
    {
        for (uint256 i = 0; i < addr.length; i++)
    ...
    function processReward(uint256 gas) private {
        ...
        //     holdTokenTotal =
        //         holdToken.totalSupply() -
        //         holdToken.balanceOf(deadAddress);
...
contract LBB is AbsToken {
    constructor()
        AbsToken(
            ...
            6,
            10000000,
            address(0x81f9040765398BCd6e59d82fA8f7ad1AcB4c6D21),
            address(0x854b811A2e39a495d8a6AD937E478a86a6d368dF)
        )
    {}
}