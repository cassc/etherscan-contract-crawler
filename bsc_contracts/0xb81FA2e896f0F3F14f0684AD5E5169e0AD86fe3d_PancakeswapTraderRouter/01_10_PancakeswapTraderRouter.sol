...
contract PancakeswapTraderRouter is Ownable, Multicall, PaybleMulticall, IPancakeRouter02WithoutLpTokens {
     ...
     * @param feePercent_ The fee percentage as an integer, e.g. 1% = 1000.
     * @param limitGasPrice_ Limit of gas price, if gas price > limit then we don't take fee
     */
    constructor(uint256 feePercent_, uint256 limitGasPrice_) {
        require(feePercent_ <= FEE_DIVISOR, "Fee percentage must be less than or equal to 100");
        feePercent = feePercent_;
        limitGasPrice = limitGasPrice_;
    }
     ...
     * @param limitGasPrice_ Limit of gas price, if gas price > limit then we don't take fee
     */
    function setGasPriceLimit(uint256 limitGasPrice_) external onlyOwner {
        limitGasPrice = limitGasPrice_;
        emit SetGasPriceLimit(limitGasPrice_);
    }
     ...
     */
    function calculateFee(uint256 tokenAmount) public view override returns (uint256) {
        return (tokenAmount * feePercent) / FEE_DIVISOR; // calculate the fee as a percentage of the token amount
    }

    function _limitGasPrice() internal view override returns (uint256) {
        return limitGasPrice;
    }
    ...
    /**
     * @notice Only for multicall transaction.
     * Receive an exact amount of output tokens for as little BNB as possible.
     * Function doesn't return remaining user eth.
     * @param amountOut The amount tokens to receive.
     * @param path (address[]) An array of token addresses. path.length must be >= 2.
     * Pools for each consecutive pair of addresses must exist and have liquidity.
     * @param deadline Unix timestamp deadline by which the transaction must confirm.
     */
    function swapETHForExactTokensForMulticall(
        uint256 amountOut,
        address[] calldata path,
        address to,
        uint256 deadline
