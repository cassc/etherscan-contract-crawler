...
contract FunRun is ERC20, ERC20Permit, AccessControlEnumerable {
    ...
    uint[2] public percents; // fee percent (60%=>profit; 40%=>market)
    ...
    constructor(
        ...
        address _profit,
        address _market
    ) ERC20("Fun Run", "FR") ERC20Permit("Fun Run") {
        ...
        _mint(_lpMiner, MAX_SUPPLY * 6 / 10);
        _mint(_basePool, MAX_SUPPLY * 2 / 10);
        _mint(_foundation, MAX_SUPPLY * 1 / 10);
        _mint(_nftAirdrop, MAX_SUPPLY * 1 / 10);
    }
    ...
    function _transfer(
        ...
        if(fee > 0) {
            ...
            uint marketFee = fee * percents[1] / 10;
    ...
    function setPercents(uint[2] memory _percents) external onlyRole(PARAM_ROLE) {
        require(_percents[0] + _percents[1] == 10, "!10");
        percents = _percents;
    }
}