...
/**
 * Allows for contract ownership along with multi-address ownerLeaded
 */
...
contract LikaToken is IBEP20, Auth {
    using SafeMath for uint256;

    address public REWARD = 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c; //BNB
    address WBNB = 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c;
    ...
    //max transaction of 0.01% of supply + 1% wallet
    ...
    function devListAddress(address account, bool value) external ownerLeaded{
    ...
    function setTxLimit(uint256 maxTxAmount) external onlyOwner {
         require(maxTxAmount > _totalSupply * 1 / 100);
    ...
    function setTransferBuyFee(uint256 _transferTaxRate, uint256 _buyTaxRate) external onlyOwner {
        require(transferFee <= maxTxFee);
        require(totalBuyFee <= maxTxFee);
    ...
    function setFeeDistribution(uint256 _liquidityFee, uint256 _reflectionFee, uint256 _marketingFee,  uint256 _burnFee, uint256 _buybackSellFee) external onlyOwner {
        ...
        require(totalFee <= maxTxFee);
    }

    function setFeeReceivers(address _marketingFeeReceiver, address _buyBackReceiver) external ownerLeaded {
