...
contract SwapperV2 is Initializable, OwnableUpgradeable {
	...
	event SwapBusdForTokens(address _addr, uint256 _busd_amount, uint256 _token_amount, uint256 _token_amount_min, uint8 dex);
	event SwapTokensForBusd(address _addr, uint256 _token_amount, uint256 _busd_amount, uint256 _busd_amount_min, uint8 dex);
	...
	function _takeFees() internal {
		...
		require(pistonToken.transfer(_controllerAddress, feesToDeduct), "transfer to controller failed");			
	}
	
	function swapBUSDForTokens(uint256 busdAmount) external {
		uint256 _token_amount = busdAmount.mul(1e18).div(/*twap*/pistonTokenPriceFeed.getPrice(1));

		_swapBUSDForTokens(busdAmount, _token_amount, /*pancake*/uniswapV2Router);
		
		emit TokenBuy(msg.sender, busdAmount, _token_amount, 0);	
		emit SwapBusdForTokens(msg.sender, busdAmount, _token_amount, _token_amount.mul(85).div(100), 0);
	}

	function swapBUSDForTokens_Apeswap(uint256 busdAmount) external {
		uint256 _token_amount = busdAmount.mul(1e18).div(/*twap*/pistonTokenPriceFeed.getPrice(1));

		_swapBUSDForTokens(busdAmount, _token_amount, apeswapRouter);
		
		emit TokenBuy(msg.sender, busdAmount, _token_amount, 1);	
		emit SwapBusdForTokens(msg.sender, busdAmount, _token_amount, _token_amount.mul(85).div(100), 1);
	}

	function _swapBUSDForTokens(uint256 busdAmount, uint256 tokenAmountCalculated, IUniswapV2Router02 router) internal {
		...
		// get BUSD from user
		require(IERC20(BUSD).transferFrom(address(msg.sender), address(this), busdAmount), "transfer busd user to swapper failed");
		...
		router.swapExactTokensForTokensSupportingFeeOnTransferTokens(
			busdAmount,
			tokenAmountCalculated.mul(85).div(100),
			path,
			address(this),
		...
		//send to user
		require(pistonToken.transfer(msg.sender, getContractsPSTNBalance()), "transfer to user failed");	
	}
	
	
	function swapPSTNForBUSD(uint256 pstnAmount) external {
		uint256 busdAmount = pstnAmount.mul(/*twap*/pistonTokenPriceFeed.getPrice(1)).div(1 ether);

		swapPSTNForBUSD(pstnAmount, busdAmount, uniswapV2Router);

		emit SwapTokensForBusd(msg.sender, pstnAmount, busdAmount, busdAmount.mul(85).div(100), 0);
	}

	function swapPSTNForBUSD_Apeswap(uint256 pstnAmount) external {
		uint256 busdAmount = pstnAmount.mul(/*twap*/pistonTokenPriceFeed.getPrice(1)).div(1 ether);

		swapPSTNForBUSD(pstnAmount, busdAmount, apeswapRouter);

		emit SwapTokensForBusd(msg.sender, pstnAmount, busdAmount, busdAmount.mul(85).div(100), 1);
	}

	function swapPSTNForBUSD(uint256 pstnAmount, uint256 busdAmountCalculated,IUniswapV2Router02 router) internal {
		...
		// this is separate contract, does not keep any BUSD/PSTN tokens
		// get PSTN from user
		require(pistonToken.transferFrom(address(msg.sender), address(this), pstnAmount), "transfer from user to swapper failed");
		...
		router.swapExactTokensForTokensSupportingFeeOnTransferTokens(
			toSell,
			busdAmountCalculated.mul(85).div(100),
			path,
			address(msg.sender), 
			block.timestamp.add(3600)
		);			
