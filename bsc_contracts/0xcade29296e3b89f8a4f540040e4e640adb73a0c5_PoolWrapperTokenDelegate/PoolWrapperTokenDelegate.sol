...
interface Vault{
    ...
    function depositWithPidByDelegate(uint256 _pid, uint256 _amount) external ;
...
//providing defi yield token define

contract PoolWrapperTokenDelegate is WrapperToken{
    using SafeMath for uint256;
    ...
    event DelegateStaked(address indexed user, address toUser, uint256 amount);
    ...
    function stakeByDelegate(uint256 amount, address user) external  onlyNotBlackList nonReentrant
    {
        require( openDelegate == true , "delegate is closed.");
        require( msg.sender == delegateCaller , "caller is invalid.");
        ...
        Vault(vaultAddress).depositWithPidByDelegate(pid,amount);

        emit DelegateStaked(msg.sender, user, amount);
    }
