...
contract NSIMidWallet_V2 is AccessControl {
    ...
    modifier onlyOwnerOrAdmin {
        require(hasRole(OWNER_ROLE, msg.sender) || INSIFactory(factoryAddress).hasRole(ADMIN_ROLE, msg.sender)  || msg.sender == factoryAddress, "Just Owner or Admin!");
        _;
    }
    ...
    constructor(address _owner) {
        ...
        uniswapV2Router = IUniswapV2Router02(0x10ED43C718714eb63d5aA57B78B54704E256024E); //testnet
        owner = _owner;
    }
    ...
    function depositTokens(address _token , uint _amount) external checkAllowance(_token, _amount) onlyOwner {
        ...
        require(disabled[_token] == false, "This token is disabled by the owner. Enable it again before depositing in mid wallet.");
    ...
    function clearStuckToken(address tokenAddress, uint256 tokens) external onlyOwner returns (bool success) {
        ...
        require(disabled[tokenAddress] == false, "This token is disabled by the owner. Enable it again before withdrawing from the mid wallet.");
    ...
    function Trade_Token(address _tokenIn, address _tokenOut, uint256 _amountIn, address[] memory path,  uint256 _amountOutMin) public onlyAdmin{
        paused ==  true;
    ...
    function Trade_Token_Supporting_Fee(address _tokenIn, address _tokenOut, uint256 _amountIn, address[] memory path, uint256 _amountOutMin) public onlyAdmin{
        paused == true;
        address _to = address(this);
        address  WETH = uniswapV2Router.WETH();
        if(_tokenIn == WETH){
    ...
    function getAmountOutMin(address[] memory path, uint256 _amountIn) external view returns (uint256) {
