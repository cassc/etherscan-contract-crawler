...
contract ATHPool2 is Ownable {
    ...
    constructor(IATHPool _athPool, IATHPool _athPool1, IATHPool _athPool2, IATHPool _athPool3) {
        athPool = _athPool;
        athPool1 = _athPool1;
        athPool2 = _athPool2;
        athPool3 = _athPool3;
    ...
    function availableClaimAmount(uint _pid, address _user) public view returns(uint available) {
        ...
        uint buyAmount = athPool.isBuyer(_user, idoToken) + athPool1.isBuyer(_user, idoToken) + athPool2.isBuyer(_user, idoToken);
        uint userClaimed = athPool.claimed(_user, idoToken) + athPool1.claimed(_user, idoToken) + athPool2.claimed(_user, idoToken) + athPool3.claimed(_user, idoToken) + claimed[_user][idoToken];
        uint firstClaim = buyAmount * p.tgePercent / 100;
        available = buyAmount * p.vestingPercent / 100 * round + firstClaim - userClaimed;
