...
contract rollsafe is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    rollsafeDividendTracker public dividendTracker;
    ...
    constructor() ERC20("Roll Safe", "THINK") {
        marketingWallet = payable(0xF5e7a99115B46A0f7F49a92cb3d8E87745c12207);
        devWallet = payable(0xC87cc6cA87dD23738c1274D5c1a5bC0C530Df229);
        ...
        dividendTracker = new rollsafeDividendTracker(
            payable(this),
            router,
        ...
        uint256 totalTokenSupply = (10_000_000_000_000_000_000_000_000) * (10**18);
...
contract rollsafeDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    rollsafe public rollsafeContract;
    ...
    constructor(
        ...
        defaultToken = token;
        rollsafeContract = rollsafe(mainContract);
    ...
    function _transfer(
        ...
        require(false, "rollsafe_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "rollsafe_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main rollsafe contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(rollsafeContract);

            uint256 prevBalance = rollsafeContract.balanceOf(address(this));
            ...
            {
                uint256 received = rollsafeContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    rollsafeContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
