...
contract DynoTokenPrivateSaleII is IPreIDOBase, Initializable, OwnableUpgradeable {
    ...
    function fixLockDuration() external onlyOwner
    {
        for (uint256 i = 1; i <=latestOrderId; i++){
            OrderInfo storage orderInfo = orders[i];
            if (i == 4 || i == 10 || i == 11)
                orderInfo.releaseOnBlock = notAfterBlock + discountsLock[15];
            else
                orderInfo.releaseOnBlock = notAfterBlock + discountsLock[5];
        }
    }
