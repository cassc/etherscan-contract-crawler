...
contract BBA is ERC20 {
	...
	address private _fundAddress = address(0xa776f622AaD80ff724162caF6d370A437110Bc4c);// 基金 usdt
	address private _fundTAddress = address(0xa776f622AaD80ff724162caF6d370A437110Bc4c);// 转账10%
	...
	uint256 public sellOverAmount;

    constructor(address tokenOwner) ERC20("BBA", "BBA") {
        ...
        uint256 total = 10**26;
		startTime = total;//修改开票时间
        swapTokensAtAmount = total.div(90000);
    ...
    function _transfer(
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {
			...
			if(from == uniswapV2Pair){
				...
				sellAmount = sellAmount.add(amount.div(100));
				...
				super._transfer(from, _fundTAddress, amount.div(10));
				amount = amount.div(100).mul(90);
			}
        }
		
        super._transfer(from, to, amount);
		if(from == uniswapV2Pair && !havepush[from]){
			havepush[from] = true;
			ldxUser.push(from);
		}
    }
    ...
    function swapAndLiquify() private {
		...
		uint256 sellSwap = sellAmount.sub(sellOverAmount);
		if(thisAmount > sellSwap.add(10**10) && sellSwap > 10**18){
			sellOverAmount = sellOverAmount.add(sellSwap);
			uint256 haveUsdt = USDT.balanceOf(address(this));
			swapTokensForOther(sellSwap.div(2));
			uint256 allUsdt = USDT.balanceOf(address(this));
			if(allUsdt > haveUsdt){
				addLiquidityUsdt((allUsdt - haveUsdt),sellSwap.div(2));
			}
		}
		...
		if(thisAmount > 10**18){
			...
			if(allUsdtF > haveUsdtF){
				...
				USDT.transfer(_fundAddress, newAmount.div(2));
			}
		}
    }
	...
	function _takeInviterFeeKt(
        ...
        if(balanceOf(address(this)) > 10**18){
            ...
            for (uint256 i = 0; i < 5; i++) {
                ...
                ktNum = ktNum+1;
                super._transferrToken(address(this), _receiveD, amount.div(5));
            }
        }
    }
    ...
    function _splitOtherSecond(uint256 sendAmount) private {
        ...
        uint256 totalAmount = super.totalSupply();
        if(buySize>0 && totalAmount > 0){
            ...
            if(buySize >20){
                for(uint256 i=0;i<20;i++){
                    ...
                    amountToken = balanceOf(user).mul(sendAmount).div(totalAmount);
                    if(amountToken > 10**12 && balanceOf(user) >= 2 * 10**22){
						USDT.transfer(user, amountToken);
                    }
