...
contract ARENAOFFIGHTERS is Context, IERC20, LockToken {
    using SafeMath for uint256;
    using Address for address;
    
    address payable public devWallet = payable(0xE1fecADDE501e359bcfAAF83cB07c56e8E304558); // devWallet Address
    address payable public marketingWallet = payable(0xf795C95a28Ce11B0927ce70A8C804232b7BB283e); // marketing Address
    ...
    string private _name = "ARENA OF FIGHTERS TOKEN";
    ...
    uint256 public _maxTxAmount = 10000000000 * 10**18; //10%
    uint256 private minimumTokensBeforeSwap = 20000000 * 10**18; //0.02%
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap 
    {
        ...
        uint256 ethForDev = newBalance.sub(ethForMarketing).sub(ethForLiquidity);
        transferToAddressETH(devWallet, ethForDev);
    ...
    {
        if(_devFee==0 && _marketingFee==0) {  return(tTransferAmount, rTransferAmount); }
        uint256 tCharity = tAmount.div(100).mul(_devFee.add(_marketingFee));
    ...
    function setTaxFeePercent(uint256 taxFee, uint256 marketingFee, uint256 devFee, uint256 liquidityFee) external onlyOwner() {
        _taxFee = taxFee;
