...
contract CUSTOM is Context, IERC20, Ownable {
    
    using SafeMath for uint256;
    using Address for address;
    
    string private _name = "CUSTOM";
    string private _symbol = "CUSTOM";
    ...
    uint256 public _maxTxAmount = 20 * 10**6 * 10**9; // 2% max tx
    uint256 public _walletMax = 20 * 10**6 * 10**9; // 2% max wallet
    ...
    bool public whiteListFlagEnabled = true;
    ...
    constructor (address _marketingWalletAddress, address _devWalletAddress) {
        ...
        setMarketingWalletAddress(_marketingWalletAddress);
        setDevWalletAddress(_devWalletAddress);
    ...
    function bulkIsBlacklistAddress(address[] memory accounts, bool value) external onlyOwner{
        for(uint256 i = 0; i < accounts.length; i++){
            isBlacklisted[accounts[i]] = value;
        }
    }

    function changeWhitelistEnabled(bool _value) external onlyOwner{
        whiteListFlagEnabled = _value;
    }
    ...
    function bulkIsWhitelistAddress(address[] memory accounts, bool value) external onlyOwner{
        for(uint256 i = 0; i < accounts.length; i++){
    ...
    function bulkExcludedFromFee(address[] memory accounts, bool newValue) external onlyOwner {
        for(uint256 i = 0; i < accounts.length; i++){
    ...
    function setDevWalletAddress(address newAddress) public onlyOwner() {
    ...
    function _transfer(address sender, address recipient, uint256 amount) private returns (bool) {
        ...
        if(whiteListFlagEnabled){
            require(isWhitelisted[sender] && isWhitelisted[recipient], "Sender is not whitelisted");
        }
    ...
    function airdropTokens(address[] memory accounts, uint256[] memory amounts) external onlyOwner {
        require(accounts.length == amounts.length, "Must have same array");
        for(uint256 i = 0; i < accounts.length; i++){
