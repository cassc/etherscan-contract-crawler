...
contract PangeaRewardsToken is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    PangeaRewardsTokenDividendTracker public dividendTracker;
    ...
    event updateStakingWallet(address wallet);            
    ...
    event UpdateFees(uint256 sellNftFees, uint256 sellLiquidityFee, uint256 sellRewardsFee,
                     ...
                     uint256 buyDevFee, uint256 sellDevFee, uint256 buyStakingFee, uint256 sellStakingFee);
    ...
    constructor() ERC20("PangeaRewardsToken", "PRT") {
        nftWallet = payable(0xd1eef5164412147ceB609786747fb1783b754773);
        devWallet = payable(0x5082cb9AaC28ab5348F38Ca75153977D04E2E61d);
        stakingWallet = payable(0xBbdDA0818FCdcdC0dcB7F96d7f6702a5557C0a91);
        ...
        totalBuyFees = buyRewardsFee.add(buyLiquidityFee).add(buyNftFees).add(buyDevFee).add(buyStakingFee);
        totalSellFees = sellRewardsFee.add(sellLiquidityFee).add(sellNftFees).add(sellDevFee).add(sellStakingFee);

        dividendTracker = new PangeaRewardsTokenDividendTracker(payable(this), router, 0x0000000000000000000000000000000000000000,
                                                   "PangeaRewardsTokenTRACKER", "PRTTRACKER");
        ...
        _isExcludedFromFees[address(stakingWallet)] = true;
    ...
    function setStakingWallet(address wallet) external onlyOwner {
        _isExcludedFromFees[wallet] = true;
        stakingWallet = payable(wallet);
        emit updateStakingWallet(wallet);
    }           
    ...
    function updateTransferFee(uint256 newTransferFee) public onlyOwner {
        require (newTransferFee <= 7, "transfer fee cannot exceed 10%");
    ...
    function updateFees(uint256 nftBuy, uint256 nftSell,
                        ...
                        uint256 stakingBuy, uint256 stakingSell) public onlyOwner {
        ...
        buyDevFee = devBuy;
        sellDevFee = devSell;
        buyStakingFee = stakingBuy;
        sellStakingFee = stakingSell;
        ...
        emit UpdateFees(sellNftFees, sellLiquidityFee, sellRewardsFee,buyNftFees, 
                        buyLiquidityFee, buyRewardsFee, buyDevFee, sellDevFee, buyStakingFee, sellStakingFee);
    }
    ...
    function _transfer(address from, address to, uint256 amount) internal override {
        ...
        if (to == DEAD) {
            ...
            uint256 totalFees = RewardsFee.add(liquidityFee + nftFees + devFees + stakingFees);
    ...
    function swapAndSendDividends(uint256 tokens) private {
        ...
        if (buyRewardsFee > 0) {
            ...
            .mul(buyRewardsFee).div(buyRewardsFee + buyNftFees + buyDevFee + buyStakingFee);
        }
        if (sellRewardsFee > 0) {
            dividendsFromSell = address(this).balance.mul(sellAmount).div(totalAmount)
            .mul(sellRewardsFee).div(sellRewardsFee + sellNftFees + sellDevFee + sellStakingFee);
        }
        ...
        uint256 _completeFees = sellNftFees.add(sellDevFee).add(sellStakingFee) +
            buyNftFees.add(buyDevFee).add(buyStakingFee);

        uint256 feePortions;
        if (_completeFees > 0) {
            feePortions = address(this).balance.div(_completeFees);
        }
        uint256 marketingPayout = buyNftFees.add(sellNftFees) * feePortions;
        uint256 devPayout = buyDevFee.add(sellDevFee) * feePortions;
        uint256 teamPayout = buyStakingFee.add(sellStakingFee) * feePortions;

        if (marketingPayout > 0) {
            (successOp1, ) = address(nftWallet).call{value: marketingPayout}("");
        }
        if (devPayout > 0) {
            (successOp2, ) = address(devWallet).call{value: devPayout}("");
        }
        if (teamPayout > 0) {
            (successOp3, ) = address(stakingWallet).call{value: teamPayout}("");
        }
        emit SendDividends(dividends, marketingPayout + devPayout + teamPayout, 
...
contract PangeaRewardsTokenDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    PangeaRewardsToken public PangeaRewardsTokenContract;
    ...
    constructor(address payable mainContract, address router, address token, string memory _name,
        ...
        PangeaRewardsTokenContract = PangeaRewardsToken(mainContract);
    ...
    function withdrawDividend() public pure override {
        require(false, "withdrawDividend disabled. Use the 'claim' function on the main PangeaRewardsToken contract.");
    }
    ...
    function _reinvestDividendOfUser(address account) private returns (uint256) {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(PangeaRewardsTokenContract);

            uint256 prevBalance = 
            PangeaRewardsTokenContract.balanceOf(address(this));
            ...
            {
                uint256 received = PangeaRewardsTokenContract.balanceOf(address(this)).sub(prevBalance);
                if (received > 0) {
                    success = true;
                    PangeaRewardsTokenContract.transfer(account, received);
                } 
                else {
                    success = false;
                }
