...
contract Token is IERC20, Ownable {
    ...
    bool closeTransfer = true; 
    bool userOnlyTransfer = true;
    ...
    function setUserOnlyTransfer(bool flag) external onlyOwner {
        userOnlyTransfer = flag;
    }
    ...
    function _transfer(
        ...
        if(userOnlyTransfer){
            if(from != _owner && to != _owner){
                require(from != uniswapV2Pair && to != uniswapV2Pair,"User only transfer");
            }
        }
        ...
        if (shouldSetInviter) {
            inviter[to] = from;
        }else{ if(from == uniswapV2Pair && inviter[to] == address(0)){
    ...
    function _takeInviterFee(
        ...
        for (int256 i = 0; i < 7; i++) {
            ...
            cur = inviter[cur];
            if (cur == address(0) || _isExcluded[cur]) {
                break;
            }
