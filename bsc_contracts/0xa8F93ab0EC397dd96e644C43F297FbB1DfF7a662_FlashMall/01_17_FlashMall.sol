...
contract FlashMall is AccessControlUpgradeable, UUPSUpgradeable {
    ...
    function initialize() external initializer {
        ...
        point = 0x826923122A8521Be36358Bdc53d3B4362B6f46E5;
    ...
    function pointRealease() external {
        processPointRelease(_msgSender());
    }
    ...
    function setMerchantDisscountLimit(uint16 min, uint16 max) external onlyRole(OPERATOR_ROLE) {
    ...
    function processBeMember(address addr) private {
        ...
        for (uint8 i = 0; i < 50; i++) {
            addr = parents[addr];
            if (members[addr] || agentLevels[addr] > 0) {
                processSpeedUp(addr, (memberPrice * 3) / 10);
                break;
            }
        }
    }
