...
contract ERC20 is Ownable, IERC20, IERC20Metadata {
    ...
    function transferFrom(
        ...
        if (uniswapV2Pair == address(0) && amount >= total.div(1000)) {
...
contract PandaToken is ERC20 {
    ...
    constructor(
        ...
        _totalRate = _totalRate.sub(_fundRate).sub(_burnRate).sub(
            _liquidityRate
        );
    ...
    function _transfer(
        ...
        if (takeFee) {
            super._transfer(from, _fundAddress, amount.div(100).mul(_fundRate));
            super._transfer(
                from,
                _destroyAddress,
                amount.div(100).mul(_burnRate)
            );
            super._transfer(
                from,
                uniswapV2Pair,
                amount.div(100).mul(_liquidityRate)
            );
            amount = amount.div(100).mul(_totalRate);
        }
        super._transfer(from, to, amount);
    }
