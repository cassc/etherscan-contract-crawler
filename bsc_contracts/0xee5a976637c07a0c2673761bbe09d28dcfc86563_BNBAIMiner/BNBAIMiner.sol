...
contract BNBAIMiner {
    ...
    uint256 public LOTTERY_STEP = 96 * 60 * 60; /** every 96 hours. **/
    uint256 public LOTTERY_TICKET_PRICE = 1 * 1e16; /** 0.01 ether **/
    ...
    uint256 public MIN_INVEST = 2 * 1e16; /** 0.02 BNB  **/
...
event Claimed(address user, uint amount);
    ...
    function buyEggs(address ref) public payable{
       ...
       function payFees(uint256 eggValue) internal {
        (uint256 projectFee) = getFees(eggValue);
        project.transfer(projectFee);
        // partner.transfer(partnerFee);
        // marketing.transfer(marketingFee);
    }
        function getFees(uint256 eggValue) public view returns(uint256 _projectFee) {
   ...
   /** will auto execute, when condition is met. buy, hatch and sell, can be triggered manually by admin if theres no user action. **/
    ...
    function min(uint256 a, uint256 b) private pure returns (uint256) {
        ...
        function SET_WREWARD(uint256 amount) public  
    {
