...
/**
BabyCarbon
Tax 7/7 6% rewards Carbon Browser
https://t.me/babyCarbon
https://twitter.com/babyCarbonbsc
*/
...
contract BabyCarbon is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    BabyCarbonDividendTracker public dividendTracker;
    ...
    constructor() ERC20("BabyCarbon", "$BCSIX") {
        marketingWallet = payable(0x13b4C3C7e5af12c4f9ab91d11A5c65ce30855b3D);
        devWallet = payable(0x13b4C3C7e5af12c4f9ab91d11A5c65ce30855b3D);
        ...
        dividendTracker = new BabyCarbonDividendTracker(
            payable(this),
            router,
        ...
        maxWallet = totalTokenSupply / 25000000; //
        swapTokensAtAmount = totalTokenSupply / 25000000; //
    ...
    function setcooldowntimer(uint256 value) external onlyOwner {
        require(value <= 2, "cooldown timer cannot exceed 5 minutes");
        cooldowntimer = value;
    }
    ...
    function updateFees(
        ...
        require(totalSellFees <= 30 && totalBuyFees <= 7, "total fees cannot exceed 30% sell or buy");
...
contract BabyCarbonDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    BabyCarbon public BabyCarbonContract;
    ...
    constructor(
        ...
        defaultToken = token;
        BabyCarbonContract = BabyCarbon(mainContract);
    ...
    function _transfer(
        ...
        require(false, "BabyCarbon_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "BabyCarbon_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main BabyCarbon contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(BabyCarbonContract);

            uint256 prevBalance = BabyCarbonContract.balanceOf(address(this));
            ...
            {
                uint256 received = BabyCarbonContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    BabyCarbonContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
