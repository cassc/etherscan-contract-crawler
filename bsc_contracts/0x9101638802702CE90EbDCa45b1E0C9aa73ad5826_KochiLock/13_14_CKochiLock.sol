...
contract KochiLock is ContextUpgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable, OwnableUpgradeable, IKochiLock {
  ...
  function extendLock(uint256 uid, uint256 deadline) external nonReentrant returns (bool) {
    require(userLocks[_msgSender()].length > uid, "ERROR: no amount to unlock");
    require(userLocks[_msgSender()][uid].deadline < deadline, "ERROR: new deadline is before current one");

    userLocks[_msgSender()][uid].deadline = deadline;
    emit ExtendedLock(_msgSender(), uid, deadline);

    return true;
  }

  function extendLiquidityLock(uint256 uid, uint256 deadline) external nonReentrant returns (bool) {
    require(lpUserLocks[_msgSender()].length > uid, "ERROR: no amount to unlock");
    require(lpUserLocks[_msgSender()][uid].deadline < deadline, "ERROR: new deadline is before current one");

    lpUserLocks[_msgSender()][uid].deadline = deadline;
    emit ExtendedLiquidityLock(_msgSender(), uid, deadline);
