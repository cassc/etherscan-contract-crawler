...
/**
https://twitter.com/elonmusk/status/1649211324344201218?t=Ee7VWFEFxYqfu7GAc0GfLg&s=19
 */
...
contract LeBron is Context, IERC20, Ownable {
    ...
    address payable public marketingWalletAddress= payable (0xB5A7EEA86572AFC0166A39Aea648Cd2641e93D99); // marketing Wallet Address
    address payable public devWalletAddress= payable (0xB5A7EEA86572AFC0166A39Aea648Cd2641e93D99); // marketing Wallet Address
    ...
    //Token Definition
    string private constant _name = "King LeBron";
    string private constant _symbol = "LeBron";
    ...
    uint256 public totalSwapableFee = _liquidityFee + _marketingFee;
    
    //Definition for the swapAndLiquify Trigger.
    uint256 private minimumTokensBeforeSwap = 1_000 * 10**_decimals;
   ...
   //Swap Controls
    ...
    event CaughtEarlyBuyer(address sniper);//sniper code
...
//Trigger Point for SwapandLiquify, Enable Swap and Manual Swap Trigger
    ...
    {
                require(
            _minimumTokensBeforeSwap >= (_tTotal * 1) / 100000,
            "Swap amount cannot be lower than 0.001% total supply."
        );
        require(
            _minimumTokensBeforeSwap <= (_tTotal * 1) / 1000,
...
//Manual Swap in case Automatic Swap is disable or need to trigger individually.
    function manualSwap () external onlyOwner {
    swapAndLiquify();
    emit AuditLog("Owner has trigger a manual contract swap:",msg.sender);
    }
//Update of tax values
function setNewTaxes (uint256 _newMarketingFee,uint256 _newLiquidityFee) external onlyOwner {
...
totalSwapableFee = _liquidityFee + _marketingFee;
...
//Dev Definitions to be used as fallback from contract.
    ...
    function recoverETHfromContract(address _recoveryWallet) public virtual onlyOwner {
...
require(_tokenAddress != address(this), "Owner can't claim contract's balance of its own tokens");	
    ...
    //Trading Controls
        ...
        emit AuditLog(
            "We have enable trading and enable swapAndLiquify.",
            msg.sender
        );
    }
