...
interface IDexAgregator is IDexAgregatorModel {
    ...
    function getAmountsOut(address _tokenIn, address _tokenOut, Split[] memory splits, uint256 amountIn)
    external view returns (uint[] memory amounts);

    function swap(address tokenIn, address tokenOut, Split[] memory splits,
        uint256 amountIn, uint256 amountOutMin, address payable to, uint deadline)
    external payable returns (uint256[] memory amounts);

    function buyback(uint256 tax) external payable;
}
