...
import './interfaces/IGaloPair.sol';
import './libraries/interfaces/IERC20.sol';

import './interfaces/IGaloRouter.sol';
import './libraries/UniswapV2Library.sol';
import './libraries/SafeMath.sol';
import './interfaces/IWETH.sol';

contract GaloRouter is IGaloRouter {
  using SafeMath for uint;
  address public immutable override factory;
  address public immutable override WETH;

  modifier ensure(uint deadline) {
    require(deadline >= block.timestamp, 'galoRouter: EXPIRED');
    _;
  }
  ...
  function _addLiquidity(
    ...
    if (IGaloFactory(factory).getPair(tokenA, tokenB) == address(0)) {
      IGaloFactory(factory).createPair(tokenA, tokenB);
    }
    ...
    if (reserveA == 0 && reserveB == 0) {
      ...
      if (amountBOptimal <= amountBDesired) {
        require(amountBOptimal >= amountBMin, 'galoRouter: INSUFFICIENT_B_AMOUNT');
        (amountA, amountB) = (amountADesired, amountBOptimal);
      } else {
        uint amountAOptimal = UniswapV2Library.quote(amountBDesired, reserveB, reserveA);
        assert(amountAOptimal <= amountADesired);
        require(amountAOptimal >= amountAMin, 'galoRouter: INSUFFICIENT_A_AMOUNT');
  ...
  function addLiquidity(
    ...
    liquidity = IGaloPair(pair).mint(to);
  }
  ...
  function removeLiquidity(
    ...
    address pair = UniswapV2Library.pairFor(factory, tokenA, tokenB);
    IGaloPair(pair).transferFrom(msg.sender, pair, liquidity);
    // send liquidity to pair
    (uint amount0, uint amount1) = IGaloPair(pair).burn(to);
    (address token0,) = UniswapV2Library.sortTokens(tokenA, tokenB);
    (amountA, amountB) = tokenA == token0 ? (amount0, amount1) : (amount1, amount0);
    require(amountA >= amountAMin, 'galoRouter: INSUFFICIENT_A_AMOUNT');
    require(amountB >= amountBMin, 'galoRouter: INSUFFICIENT_B_AMOUNT');
  }
  ...
  function removeLiquidityWithPermit(
    ...
    IGaloPair(pair).permit(msg.sender, address(this), value, deadline, v, r, s);
  ...
  function _swapSupportingFeeOnTransferTokens(address[] memory path, address _to, address referrer) internal {
    for (uint i; i < path.length - 1; i++) {
      ...
      IGaloPair pair = IGaloPair(UniswapV2Library.pairFor(factory, input, output));
      uint amountOutput;
  ...
  function swapExactTokensForTokensSupportingFeeOnTransferTokens(
    ...
    require(
      ...
      'galoRouter: INSUFFICIENT_OUTPUT_AMOUNT'
    );
  }
  ...
  {
    require(path[0] == WETH, 'galoRouter: INVALID_PATH');
  ...
  {
    require(path[path.length - 1] == WETH, 'galoRouter: INVALID_PATH');
    TransferHelper.safeTransferFrom(
      path[0], msg.sender, UniswapV2Library.pairFor(factory, path[0], path[1]), amountIn
    );
    _swapSupportingFeeOnTransferTokens(path, address(this), referrer);
    uint amountOut = IERC20(WETH).balanceOf(address(this));
    require(amountOut >= amountOutMin, 'galoRouter: INSUFFICIENT_OUTPUT_AMOUNT');
