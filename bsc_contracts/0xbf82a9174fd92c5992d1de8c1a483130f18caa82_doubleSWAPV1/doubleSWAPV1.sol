...
contract doubleSWAPV1 is Context, Ownable {
  ...
  function LoopSwapRandom(address[] memory pathBuy,address[] memory pathSell,uint256 amount,uint256 txcount,uint256 slippage,uint256 denominator) public onlyPermission returns (bool) {
    ...
    uint256 r;
    do{
        r = _randomInt(100,balance,balanceOut[1]);
        if(r>50){
            balance = tokenA.balanceOf(address(this));
            balanceOut = router.getAmountsOut(balance,pathBuy);
            _swap(balance,balanceOut[1]*slippage/denominator,pathBuy,address(this));
  ...
  function _randomInt(uint256 _mod,uint256 a,uint256 b) internal view returns (uint256) {
    uint256 randomNum = uint256(
      keccak256(abi.encodePacked(block.timestamp,a,b))
    );
