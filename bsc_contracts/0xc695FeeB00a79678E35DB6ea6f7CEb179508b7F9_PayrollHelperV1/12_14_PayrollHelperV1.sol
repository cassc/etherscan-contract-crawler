...
contract PayrollHelperV1 {
    ...
    function getOperators(address employer) external view returns (address[] memory) {
        address instance = factory.instances(employer);
        require(instance != address(0), 'PayrollHelperV1(getOperators): instance not found');
        return IPayrollInstanceV1(instance).getOperators();
    }
