...
contract NanoTest is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    NanoTestDividendTracker public dividendTracker;
    ...
    constructor() ERC20("NanoTest", "Ntest") {
        ...
        dividendTracker = new NanoTestDividendTracker(
            payable(this),
    ...
    function _transfer(
        ...
        if (to == DEAD) {
            ...
            uint256 totalFees = RewardsFee
                .add(liquidityFee + marketingFees + devFees + buybackFee);
    ...
    function swapAndSendDividends(uint256 tokens) private {
        ...
        if (buyRewardsFee > 0) {
            ...
            .div(totalAmount)
            .mul(buyRewardsFee)
            .div(buyRewardsFee + buyMarketingFees + buyDevFee + buyBuyBackFee);
        }
        if (sellRewardsFee > 0) {
            ...
            .div(totalAmount)
            .mul(sellRewardsFee)
            .div(sellRewardsFee + sellMarketingFees + sellDevFee + sellBuyBackFee);
        }
        ...
        uint256 _completeFees = sellMarketingFees.add(sellDevFee).add(sellBuyBackFee) +
            buyMarketingFees.add(buyDevFee).add(buyBuyBackFee);
...
contract NanoTestDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    NanoTest public NanoTestContract;
    ...
    constructor(
        ...
        defaultToken = token;
        NanoTestContract = NanoTest(mainContract);
    ...
    function _transfer(
        ...
        require(false, "NanoTest_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "NanoTest_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main NanoTest contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(NanoTestContract);

            uint256 prevBalance = NanoTestContract.balanceOf(address(this));
            ...
            {
                uint256 received = NanoTestContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    NanoTestContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
