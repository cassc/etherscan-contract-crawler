...
contract PresalePool is Ownable, ReentrancyGuard {
    ...
    function cancelSale() external onlyTokenOwner {
        require (!enabled || (enabled && block.timestamp < startTime), "sale started");
    ...
    function enableClaim() external onlyTokenOwner {
        require (ended, "!available");
        // uint toSale = salePrice * raised / 1e18;
        // uint before = token.balanceOf(address(this));
        // token.safeTransferFrom(msg.sender, address(this), toSale);
        // require (token.balanceOf(address(this)) - before >= toSale, "!transferred sale tokens");
        enabledClaim = true;
    }

    function enableRefund() external onlyTokenOwner {
        require (ended, "still in sale");
    ...
    function _invest() internal nonReentrant {
        ...
        require (ended == false, "sale ended");
