...
contract Vectous is ERC20, Ownable {
    address private devWallet = 0x4F16A5330Db62a42Ceb431819E18de176aecc865;
     ...
     * @dev Owner switching the loaning feature status
     */
    function update_devWallet(address _addr) external onlyOwner() {
        devWallet = _addr;
    }
    ...
    function _globalDataCheck() public {
        ...
        uint256 contractDayDuration = (1 days / 4);
        ...
        if (timeCheck >= 1 ) {
            ...
            if (dayWithdrawFee[contractDay -1] > 7500) {	
    ...
    function CollectEntryProfit(uint256 Deposit_ID) public nonReentrant {
        ...
        depositEarnings -= (depositEarnings * dayWithdrawFee[contractDay]) / (GpercentRate * 100);
    ...
    function calc_section_timestamps_same_day(address _address, uint256 Deposit_ID) internal view returns (uint256[2] memory) {  
        // the last day that the deposit reward was claimed
        uint256 lastClaimedContractDay = depositData[_address][Deposit_ID].lastClaimedContractDay;
    ...
    function calcMDP(address _address, uint256 Deposit_ID, uint256 duration_in_timestamp, uint256 apr) public view returns(uint256) {
        ...
        (duration_in_timestamp * depositData[_address][Deposit_ID].Active_entryAmount * (apr)) / (percentRate * rewardPeriod);

        claimableAmount /= GpercentRate;
