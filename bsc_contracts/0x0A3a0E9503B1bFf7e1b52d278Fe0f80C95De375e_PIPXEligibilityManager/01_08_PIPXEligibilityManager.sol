...
contract PIPXEligibilityManager is OwnableUpgradeable {
    ...
    function __PIPXEligibilityManager_init() public initializer {
        __Ownable_init();
    }

    function addModule(address implementation) external onlyOwner {
        require(implementation != address(0), "Impl != address(0)");
        IPIPXEligibility elig = IPIPXEligibility(implementation);
        ...
        returns (address)
    {
        ...
        IPIPXEligibility(eligibilityClone).__PIPXEligibility_init_bytes(
            configData
        );
