    ...
    Pausable
{
    ...
    uint256 public maxUpkeepDelta = 1 ether;
        ...
        returns (PEG_STATUS pegStatus_)
    {
        if (block.timestamp < lastUpkeepEpoch + upkeepPeriod) {
            ...
            } else if (usdWad < int256(minPcsDelta)) {
    ...
    function getUsdDeltaWad() public view returns (int256 usdWad_) {
        ...
        if (reservesCzusdWad < _getFeeAdjustedLp(reservesBnbWadUsdVal)) {
            usdWad_ =
                (int256(reservesCzusdWad) -
                    int256(_getFeeAdjustedLp(reservesBnbWadUsdVal))) /
                2;
        } else if (reservesBnbWadUsdVal > _getFeeAdjustedLp(reservesCzusdWad)) {
            usdWad_ = ((int256(reservesBnbWadUsdVal) -
                int256(_getFeeAdjustedLp(reservesCzusdWad))) / 2);
        } else {
            usdWad_ = 0;
        }
    }
