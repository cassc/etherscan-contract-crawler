...
contract SkullzMinter is ERC721Enumerable, Ownable {
  using Strings for uint256;
  ...
  constructor(string memory name, string memory symbol, string memory baseTokenURI) ERC721(name, symbol) {
      ...
      // Save dev wallet
      _wallet = 0xC2733d35f017236559799647306F539Cf819f832;
  }
  ...
  function mint(uint256 _mintAmount) public payable {
    // Checks
    require(!paused, "Skullz: minting has not started yet");
    require(_mintAmount > 0, "Skullz: you have to mint at least one");
    require(_mintAmount <= maxMintAmount,  "Skullz: you can only mint 100 at a time");
    require(lastSupply >= _mintAmount,  "Skullz: the collection is sold out");
    if (useWhitelist && _includeToWhitelist[msg.sender]) {
      require(msg.value == 0, "Skullz: total cost needs to be 0");
      require(_mintAmount <= 1 , "Skullz: whitelisted users only get to mint one free NFT");
      require(balanceOf(msg.sender) == 0, "Skullz: It looks like you already minted. Whitelisted users can only mint one free NFT. Remove from whitelist to purchase more.");
      _randomMint(msg.sender);
      adminSetExcludeFromWhiteList(msg.sender);
      return;
    } else {
      require(msg.value >= cost * _mintAmount, "Skullz: total cost doesn't match");
    }
    // Minting
  ...
  function randomGiveaway(address _winner, uint256 _amount) external onlyOwner {
    // Checks
    require(_winner != address(0), "Skullz: zero address");
    require(_amount > 0, "Skullz: you have to mint at least one");
    require(lastSupply != 0,  "Skullz: the collection is sold out");
  ...
  function tokenURI(uint256 _id) public view virtual override returns (string memory){
    require(_exists(_id),"Skullz: URI query for nonexistent token");
