    ...
    TokensRecoverable
{
        ...
        updateReward(_msgSender())
    {
        ...
        uint256[] memory pointsArray = BLLContract.getPointsForTokenIDs(tokenIds);
        uint points;
        for (uint32 x; x < tokenIds.length; ++x) {
            stakingToken.transferFrom(_msgSender(), address(this), tokenIds[x]);
    ...
    function _withdraw(uint32[] memory tokenIds)
        private
        updateReward(_msgSender())
    {
        if (!stakeRefreshed[_msgSender()]) _refreshStaked(_msgSender());
        ...
        uint points;
        for (uint32 x; x < tokenIds.length; ++x) {
            stakingNFT.transfer(_msgSender(), tokenIds[x]);
    ...
    function withdraw1(uint32[] memory tokenIds)
        public
        nonReentrant
        //updateReward(_msgSender())
    {
    ...
    function withdraw2(uint32[] memory tokenIds)
        public
        nonReentrant
        updateReward(_msgSender())
    {
        ...
        //uint points;
        for (uint32 x; x < tokenIds.length; ++x) {
            stakingToken.transferFrom(address(this), _msgSender(), tokenIds[x]);
            _removeTokenFromStakerEnumeration(_msgSender(), tokenIds[x]);
            //points += pointsArray[x];
        }
        //_balances[_msgSender()] -= points;
    ...
    function withdraw3(uint32[] memory tokenIds)
        public
        nonReentrant
        updateReward(_msgSender())
    {
    ...
    function exit(uint32[] memory tokenIds) external nonReentrant {
        _withdraw(tokenIds);
        getReward();
    }
