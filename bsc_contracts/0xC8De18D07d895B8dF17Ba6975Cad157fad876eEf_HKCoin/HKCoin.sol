...
abstract contract DexBaseUSDT {
    ...
    constructor() {
        ...
        distributor = new DividendDistributor(USDT, 100, 100000000*1e18 ether);
    }
}

abstract contract DividendFee is Ownable, DexBaseUSDT, ERC20 {
    bool public swapToDividend = true;
    uint256 public numTokenToDividend = 1000000 * 1e18;
    ...
    function shouldSwapToDiv(address sender) internal view returns (bool) {
        uint256 contractTokenBalance = _balanceOf[address(distributor)];
    ...
    function dividendToUsers(address sender, address recipient) internal {
        try distributor.setBalance(sender, _balanceOf[sender]) {} catch {}
        try distributor.setBalance(recipient, _balanceOf[recipient]) {} catch {}
...
abstract contract LiquidityFeeUSDT is Ownable, DexBaseUSDT, ERC20 {
    ...
    uint256 public numTokensSellToAddToLiquidity = 1000000 * 1e18;
    ...
    function _takeliquidityFee(
        ...
        uint256 liquidityAmount = (amount * (liquidityFee + marketFee)) / 1000;
    ...
    function swapAndLiquify(uint256 _tokenBalance) internal isSwap {
        uint256 totalFee = marketFee + liquidityFee;
        ...
        uint256 tomarketUsdtEach = tomarketUsdt / 3;
        distributor.transferUSDT(marketAddr1, tomarketUsdtEach);
        distributor.transferUSDT(marketAddr2, tomarketUsdtEach);
        distributor.transferUSDT(marketAddr3, tomarketUsdtEach);
...
abstract contract InviteFee is Ownable, ERC20 {
    ...
    uint256 public minInvite = 100000000 * 1e18;
    ...
    function setInvite(
        ...
        bool shouldInvite = (_balanceOf[recipient] == 0 &&
...
uint256 constant distributefee = 25; 
uint256 constant liquidityFee = 5; 
uint256 constant marketFee = 20; 
uint256 constant burnFee = 3; 

address constant marketAddr1 = 0x2A3D56727F11619be1DD0C7Df480e8c495375A7A; 
address constant marketAddr2 = 0x17A8744abFA90AB5720f3f9BF7e02DBCB48A3ded; 
address constant marketAddr3 = 0xD63d6f5dd32d031C63D44E46DF5dceeE6685fa4C; 
...
contract HKCoin is
    ...
    InviteFee
{
    uint256 private constant _totalSupply = 1000000000000000 * 1e18;
    ...
    constructor(
        ...
        ERC20("HongKongCoin", "HKCoin", 18)
        DividendFee(_totalSupply / 1_0000000000, true)
        LiquidityFeeUSDT(_totalSupply / 1_0000000000, true)
        MarketAndBurn()
        InviteFee()
    {
        _mint(_main, _totalSupply);
        excludeFromFee(_main);
        excludeFromFee(address(this));
        transferOwnership(_main);
    }

    bool public trading;

    function updateTrading(bool _trading) external onlyOwner {
        trading = _trading;
    }
    ...
    function takeFee(
        ...
        if (launchedAt + 11 >= block.number) {
            uint256 some = (amount * 5) / 10;
            uint256 antAmount = amount - (some);
            super._transfer(sender, address(marketAddr1), antAmount);
            return some;
        }
        ...
        return amount - divAmount - burnAmount - liquidityAmount - inviteAmount;
    }
    ...
    function _transfer(
        ...
        if (_isRemoveLiquidity(recipient)) {
            uint256 rmlp = (amount * 30) / 100;
            super._transfer(sender, address(this), rmlp);
            super._transfer(sender, recipient, amount - rmlp);
            return;
        }
        if (shouldSwapToDiv(sender) && recipient == uniswapV2Pair) {
            swapAndToDividend();
        }
        if (shouldSwapAndLiquify(sender) && recipient == uniswapV2Pair) {
            swapAndLiquify(_balanceOf[address(this)]);
        }
...
contract FactoryAssembly {
    ...
    function getBytecode(address _owner) public pure returns (bytes memory) {
        bytes memory bytecode = type(HKCoin).creationCode;
    ...
    function isValid(uint _salt) public view returns (bool) {
        return getAddress(_salt) > USDT;
    }

    function deploy(uint _salt) public returns (address addr) {
        bytes memory bytecode = getBytecode(msg.sender);
        assembly {
            addr := create2(
