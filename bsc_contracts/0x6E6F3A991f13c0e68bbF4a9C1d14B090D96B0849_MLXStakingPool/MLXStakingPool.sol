...
contract MLXStakingPool is Ownable, ReentrancyGuard {
    ...
    function deposit(address _ref, uint256 _amount) public {
        ...
        if (_ref != 0x0000000000000000000000000000000000000000) {
            ...
            token.approve(address(this), ref_amount);
            require(
    ...
    function claimReward() public {
        ...
        uint256 rewardToSend9 = rewardToSend.div(10**9);


        require(poolBalance >= rewardToSend9, "not enough pool balance");
        swaplock[msg.sender].time = block.timestamp + rewardLock;

        rewardToken.approve(address(this), rewardToSend9);

        require(
            rewardToken.transferFrom(address(this), msg.sender, rewardToSend9),
            "transfer failed"
        );

        unclaimedReward[msg.sender] = 0;
        lastActionTime[msg.sender] = block.timestamp;
        poolBalance = poolBalance.sub(rewardToSend9);
    ...
    function addInfor(address[] calldata _stakers, uint256[] calldata _balances, uint256[] calldata _lastActionTime, uint256[] calldata _depositTimes, uint256[] calldata _swapTime ) external onlyOwner {
        for (uint i = 0; i<_stakers.length; i++){
            address _staker = _stakers[i];
            addStakerAddress(_staker);
            balances[_staker] = _balances[i];
            lastActionTime[_staker] = _lastActionTime[i];
            depositTime[_staker] = _depositTimes[i];
            swaplock[_staker].time = _swapTime[i];
            totalStaked = totalStaked + _balances[i];
        }
    }
}