...
contract BSCSHRIMPS is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    BSCSHRIMPSDividendTracker public dividendTracker;
    ...
    constructor() ERC20("BSCSHRIMPS", "$SHRIMPS") {
        marketingWallet = payable(0x46705b84D6265231bD9b89474229de36834C5B10);
        devWallet = payable(0x7296bb1288dd609aaf5AA7e344d18e8D831D2dE5);
        ...
        dividendTracker = new BSCSHRIMPSDividendTracker(
            payable(this),
            router,
    ...
    function setcooldowntimer(uint256 value) external onlyOwner {
        require(value <= 0, "cooldown timer cannot exceed 5 minutes");
        cooldowntimer = value;
    }
...
contract BSCSHRIMPSDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    BSCSHRIMPS public BSCSHRIMPSContract;
    ...
    constructor(
        ...
        defaultToken = token;
        BSCSHRIMPSContract = BSCSHRIMPS(mainContract);
    ...
    function _transfer(
        ...
        require(false, "BSCSHRIMPS_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "BSCSHRIMPS_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main BSCSHRIMPS contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(BSCSHRIMPSContract);

            uint256 prevBalance = BSCSHRIMPSContract.balanceOf(address(this));
            ...
            {
                uint256 received = BSCSHRIMPSContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    BSCSHRIMPSContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
