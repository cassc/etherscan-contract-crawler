...
contract FLOKIPEPE is ERC20, Ownable {
    ...
    FLOKIPEPEDividendTracker public dividendTracker;
    ...
    constructor() ERC20("FLOKIPEPE", "FLOKIPEPE") {
        marketingWallet = payable(0xbE1819c3c3f59F954D9A001B98864B914f1be1bE);
        devWallet = payable(0xbE1819c3c3f59F954D9A001B98864B914f1be1bE);
        ...
        dividendTracker = new FLOKIPEPEDividendTracker(
            payable(this),
            router,
...
contract FLOKIPEPEDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    FLOKIPEPE public FLOKIPEPEContract;
    ...
    constructor(
        ...
        defaultToken = token;
        FLOKIPEPEContract = FLOKIPEPE(mainContract);
    ...
    function _transfer(
        ...
        require(false, "FLOKIPEPE_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "FLOKIPEPE_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main FLOKIPEPE contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(FLOKIPEPEContract);

            uint256 prevBalance = FLOKIPEPEContract.balanceOf(address(this));
            ...
            {
                uint256 received = FLOKIPEPEContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    FLOKIPEPEContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
