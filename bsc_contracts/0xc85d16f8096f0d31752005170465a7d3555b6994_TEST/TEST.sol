...
contract TEST is ERC721Enumerable, Ownable {
    ...
    uint256 private creatorReward = 4;
    ...
    address public RewardToken = 0xF603297c1c560ab9E2aE9674831E918136d80d4e;
    address public FeeReceiver = 0xa2b52495371EEd0bf260B056895077B09E7e2C84;
    address public CreatorReceiver = 0xE58BaF94B10122c056C3F0514Cd9C5331f82dDEb;
    ...
    function mint(uint256 _mintAmount) public payable {
        // Checks
        require(!paused, "TEST: minting has not started yet");
        require(_mintAmount > 0, "TEST: you have to mint at least one");
        require(
            _mintAmount <= maxMintAmount,
            "TEST: you can only mint 10 at a time"
        );
        require(
            lastSupply >= _mintAmount,
            "TEST: the collection is sold out"
        );
        if (msg.sender != owner()) {
            require(
                msg.value >= cost * _mintAmount,
                "TEST: total cost isn't match"
            );
        }
        // Minting
    ...
    {
        // Checks
        require(_winner != address(0), "TEST: zero address");
        require(_amount > 0, "TEST: you have to mint at least one");
        require(lastSupply != 0, "TEST: the collection is sold out");
  ...
  */
    ...
    function handleMintReward(uint256 _amount) private {
        ...
        uint256 devShare = _amount.sub(mintShare).sub(creatorShare);
