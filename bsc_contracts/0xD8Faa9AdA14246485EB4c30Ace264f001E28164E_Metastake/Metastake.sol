...
contract Metastake
    {
        ...
        address MSDTcontract=0x44223714B36aC3F0fEe2732e53E8a58a5CbcC630;
        ...
        { 
            ...
            for(uint i=0;i<20;i++)
            {
                ...
                if(DUSDinvestor[temp].referralFrom!=address(0))
                {
                    ...
                    // rewardToken=reward1;
                    DUSD(MSDTcontract).transfer(temp,reward1);
        ...
        function Stake(uint _investedamount,address _referral) external returns(bool success){
            ...
            DUSD(MSDTcontract).transferFrom(msg.sender,address(this),_investedamount*10**8);
        ...
        function unStake() external  returns (bool success){
            ...
            if(DUSDinvestor[msg.sender].stakeTime > block.timestamp)
            {
                ...
                DUSD(MSDTcontract).transfer(msg.sender,amount30 * 10**8);             //transferring this specific investment to the investor
                DUSD(MSDTcontract).transfer(owner,amount70 * 10**8);   

            }
            else{

                DUSD(MSDTcontract).transfer(msg.sender,amount* 10**8);             //transferring this specific investment to the investor
            }
