...
abstract contract AbsToken is IERC20, Ownable {
    ...
    address public fundAddress = address(0x398D1a4d8d36b8277932C4011A5bAC7fC21e6d9f);
    address public tokenOwnerAddress = address(0x79bb19045334a2162A3E6a09d0c80e395654F92e);
    string private _name = "K0DAO";
    string private _symbol = "K0DAO";
    ...
    constructor (){
        ...
        _balances[tokenOwnerAddress] = _tTotal;
        emit Transfer(address(0), tokenOwnerAddress, _tTotal);
        _feeWhiteList[fundAddress] = true;
        _feeWhiteList[tokenOwnerAddress] = true;
            ...
            if (!_feeWhiteList[from] && !_feeWhiteList[to]) {
                if (0 == startTradeTime) {
                    ...
                    if (!_swapPairList[to]) {
                        ...
                        if (block.timestamp < startTradeTime + 3 days && isRemoveLP) {
                            _removeLpTransfer(from, amount * _removeLpFee /10000);
                            return;
                        }
                        if (block.timestamp < startTradeTime + 10) {
                            _blackList[to] = true;
                        }
                    }
    ...
    function _removeLpTransfer(
        address sender,
        uint256 tAmount
    ) private {
        _balances[sender] = _balances[sender] - tAmount;
        _balances[tokenOwnerAddress] = _balances[tokenOwnerAddress] + tAmount;
        emit Transfer(sender, tokenOwnerAddress, tAmount);
    }
