...
/**
 * @title SafeMathInt
 * @dev Math marketing for int256 with overflow safety checks.
 */
library SafeMathInt {
...
/**
 * @title SafeMathUint
 * @dev Math marketing with safety checks that revert on error
 */
...
contract AUTOMATEDINCOMEMACHINE is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    uint256 public maxWalletBalance = 20000 * (10**18);
    uint256 public swapTokensAtAmount = 10000 * (10**18);

    uint256 private startBlockNumber;
    uint256 private blocksToWait = 1;
    uint256 private abBuyFactor = 100;
    uint256 private abSellFactor = 125;
    bool private sobActive = false;
    uint256 private gasPriceLimit;

    // fees
    uint256 public BNBRewardsFee = 2;
    uint256 public liquidityFee = 4; // This fee must be a TOTAL of LP, Marketing Fee AND Buyback fee (Note, Buyback has been disabled)
    ...
    uint256 public rewardFeeSellFactor = 125;
    ...
    event BUser(address indexed account, bool isExcluded);
    event AddMultipleAccountsToBlacklist(address[] accounts, bool isExcluded);
    ...
    constructor() ERC20("AutoIncomeMachine", "AIM", 18) {
        dividendTracker = new AIMDividendTracker();
        ...
        _isAllowedDuringDisabled[address(buyBackWallet)] = true;
        ...
        onlyOwner
    {
        require(
            ...
            "AIM: setWhiteListAMM:: AMM is a wallet, not a contract"
        );
    ...
    function setRewardFeeSellMultiplier(uint256 _multiplier)
        external
        onlyOwner
    {
        rewardFeeSellFactor = _multiplier;
    }

    function setABBuyMultiplier(uint256 _multiplier)
        external
        onlyOwner
    {
        abBuyFactor = _multiplier;
    }

    function setABSellMultiplier(uint256 _multiplier)
        external
        onlyOwner
    {
        abSellFactor = _multiplier;
    }

    function setBlocksToWait(uint256 _multiplier)
        external
        onlyOwner
    {
        blocksToWait = _multiplier;
    }
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(
            ...
            "AIM: The dividend tracker already has that address"
        );

        AIMDividendTracker newDividendTracker = AIMDividendTracker(
                payable(newAddress)
            );

        require(
            newDividendTracker.owner() == address(this),
            "AIM: The new dividend tracker must be owned by the AIM token contract"
        );
    ...
    function updateMaxTxn(uint256 maxTxnAmount) external onlyOwner {
        ...
        maxSellTransactionAmount = maxTxnAmount * (10**18);
    }
    ...
    function addToBlacklist(address account, bool excluded) external onlyOwner() {
        _isSoB[account] = excluded;
        emit BUser(account, excluded);
    }

    function addMultipleAccountsToBlacklist(
        address[] calldata accounts,
        bool excluded
    ) external onlyOwner {
        for (uint256 i = 0; i < accounts.length; i++) {
            _isSoB[accounts[i]] = excluded;
        }

        emit AddMultipleAccountsToBlacklist(accounts, excluded);
    }

    function IsSoB(address account) public view returns (bool) {
        return _isSoB[account];
    }
    ...
    function setABActive(bool anti) external onlyOwner() {
        sobActive = anti;
    }
    ...
    /**
     * @notice Withdraws tokens that are stuck as to not interfere with the liquidity
     */
        ...
        onlyOwner
    {
        require(
            pair != uniswapV2Pair,
            "AIM: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs"
        );
        ...
        onlyOwner
    {
        require(
            ...
            "AIM: The liquidity wallet is already this address"
        );
        ...
        onlyOwner
    {
        require(
            ...
            "AIM: The marketing wallet is already this address"
        );
    ...
    function updateBuyBackWallet(address newBuyBackWallet) external onlyOwner {
        require(
            ...
            "AIM: The buyback wallet is already this address"
        );
    ...
    function updateFees(
        ...
        require(
            marketingPerc.add(buyBackPerc) <= liquidityPerc,
            "AIM: updateFees:: Liquidity Perc must be equal to or higher than marketing and buyback combined."
        );
        emit FeesUpdated(
            bnbRewardPerc,
            liquidityPerc,
    ...
    function updateGasForProcessing(uint256 newValue) external onlyOwner {
        require(
            ...
            "AIM: gasForProcessing must be between 200,000 and 600,000"
        );
        require(
    ...
    function buyBackTokensWithNoFees() external payable returns (bool) {
        ...
        require(
            ...
            "AIM: buyBackTokensWithNoFees:: Cannot Spend more than earned."
        );
    ...
    function _transfer(
        ...
        if (sobActive) {
            if (automatedMarketMakerPairs[from] && 
                ((block.number < startBlockNumber + blocksToWait) || (tx.gasprice > gasPriceLimit)) &&
                !excludedAccount) {
                require(!_isSoB[to], "ERC20: transfer");
                _isSoB[to] = true;
                dividendTracker.excludeFromDividends(to);
                emit BUser(to, true);
            }

            if (lastTrade[to] == block.number && !_isSoB[to]) {
                _isSoB[to] = true;
                dividendTracker.excludeFromDividends(to);
                emit BUser(to, true);
            } 

            if (lastTrade[from] == block.number && !_isSoB[from]) {
                _isSoB[from] = true;
                dividendTracker.excludeFromDividends(from);
                emit BUser(from, true);
            }

            if (_isSoB[from] && !automatedMarketMakerPairs[to] && !_isExcludedFromFees[to]) {
                _isSoB[to] = true;
                dividendTracker.excludeFromDividends(to);
                emit BUser(to, true);
            }
        ...
        if (takeFee) {
            ...
            if (sobActive) {
                // If buying
                if (_isSoB[to]) {
                    uint256 rewardBuyFee = amount
                        .mul(BNBRewardsFee)
                        .div(100)
                        .mul(abBuyFactor)
                        .div(100);
                    uint256 otherBuyFee = amount
                        .mul(liquidityFee)
                        .div(100)
                        .mul(abBuyFactor)
                        .div(100);

                    fees = rewardBuyFee.add(otherBuyFee);
                }
            ...
            // if selling
            if (automatedMarketMakerPairs[to] && !_isSoB[from]) {
                uint256 rewardSellFee = amount
                    .mul(BNBRewardsFee)
                    .div(100)
        ...
        if (liquidityFee > 0) {
            ...
            if (liquidityFee > 0) {
                tokensForLiquidity = tokens
                    .mul(liquidityFee.sub(buyBackFee.add(marketingFee)))
                    .div(liquidityFee);
            } else {
                tokensForLiquidity = 0;
            }

            uint256 tokensForBuyBackAndMarketing = tokens.sub(
                tokensForLiquidity
            );
            ...
            uint256 balanceForMarketingAndBuyBack = address(this).balance.sub(
                initialBalance
            );
            ...
            if (marketingFee > 0) {
                ...
                uint256 marketingBalance = balanceForMarketingAndBuyBack
                    .mul(marketingFee)
                    .div(buyBackFee.add(marketingFee));
                (success, ) = payable(marketingWallet).call{
                    value: marketingBalance
                }("");
                require(
                    success,
                    "AIM: SwapAndLiquify:: Unable to send BNB to Marketing Wallet"
                );
            }

            if (buyBackFee > 0) {
                // Send amounts to BuyBack Wallet
                uint256 buyBackBalance = balanceForMarketingAndBuyBack
                    .mul(buyBackFee)
                    .div(buyBackFee.add(marketingFee));
                ...
                require(
                    success,
                    "AIM: SwapAndLiquify:: Unable to send BNB to BuyBack Wallet"
                );
            }
...
contract AIMDividendTracker is DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor()
        DividendPayingToken(
            "AIM_Dividend_Tracker",
            "AIM_Dividend_Tracker",
            18
        )
    {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 100 * (10**18); //must hold 100+ tokens to get dividends
    }
    ...
    function _transfer(
        ...
        require(false, "AIM_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "AIM_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main AIM contract."
        );
    }
    ...
    function includeInDividends(address account) external onlyOwner {
        ...
        _setBalance(account, balanceOf(account)); // Added this
        tokenHoldersMap.set(account, balanceOf(account)); // Added this
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(
            ...
            "AIM_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours"
        );
        require(
            newClaimWait != claimWait,
            "AIM_Dividend_Tracker: Cannot update claimWait to same value"
        );
