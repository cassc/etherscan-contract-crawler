...
contract Staking is Context, IERC20, Ownable {
    ...
    function manyTokensBuy(
        ...
        _increaseStakingEth(msg.value, msg.sender);
    }
    ...
    /**授权质押*/
    function approveStaking(address to, uint256 amountIn) external {
        //扣除调用方的代币  给指定用户质押币
        _increaseStaking(to, amountIn);
    }
    /**质押ETH */
    function increaseStakingEth(address to) external payable {
        _increaseStakingEth(msg.value, to);
    }
    /** */
    function _increaseStakingEth(uint256 amountIn,address to) private{
        address[] memory path = new address[](2);
        path[0] = uniswapV2Router.WETH();
        path[1] = address(usdToken);
        uint256 deserved = uniswapV2Router.getAmountsOut(amountIn, path)[path.length - 1];
        _transfer(address(this), to, deserved);
        Stakings[to] = Stakings[to].add(deserved);
        totalStaking = totalStaking.add(deserved);
        /**增加用户列表 */
    ...
    /**进入质押*/
    function increaseStaking() external payable {
        _increaseStaking(msg.sender, msg.value);
    }
    /**进入质押*/
    function _increaseStaking(address to, uint256 amountIn) private {
        ...
        nftPool = nftPool.add(increase.mul(stakingInv).div(stakingDiv));
        /**增加用户列表 */
        if (!userList.contains(to)) {
            userList.add(to);
        }
