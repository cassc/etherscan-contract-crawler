...
contract SUZAKU is IBEP20, Ownable {
    ...
    TaxWallets public _taxWallet = TaxWallets ({
        dev: 0x610f195388F340E945988B0CF6f6134713278BF4,
        shib: 0x610f195388F340E945988B0CF6f6134713278BF4,
        marketing: 0x610f195388F340E945988B0CF6f6134713278BF4
    });
    ...
    uint8 private constant BLACKLIST_BLOCKS = 1; //number of blocks that will be included in auto blacklist
...
/////////////////////////////   EVENTS  /////////////////////////////////////////
    ...
    event UpdateshibWallet(address newshibWallet);          
...
/////////////////////////////   CONSTRUCTOR  /////////////////////////////////////////
    ...
    //allows dev to change token name and symbol. 
...
///// FUNCTIONS CALLABLE BY ANYONE /////
    ...
    //Claims reward chosen by holder. Differentiates between BNB and other BEP20 tokens
...
///// AUTHORIZED FUNCTIONS /////
    ...
    function createLPandBNB(uint16 permilleOfPancake, bool ignoreLimits) external onlyOwner {
    ...
    function dynamicSettings(bool burn, bool limits, bool liquidity, bool sells) external onlyOwner {
        dynamicBurn = burn;
    ...
    //Mainly used for addresses such as CEX, presale, etc    
    function setStakingExclusionStatus(address addr, bool exclude) external onlyOwner {
        if(exclude)
    ...
    function launch() external onlyOwner {
        require(IBEP20(_pancakePairAddress).totalSupply() > 0, "Liquidity has not been provided!");
    ...
    function setPresale(address presaleAddress) external onlyOwner {
    ...
    function setTaxExemptionStatus(address account, bool exempt) external onlyOwner {
        taxExempt[account] = exempt;
    }

    //independtly set whether wallet is exempt from limits
    function setLimitExemptionStatus(address account, bool exempt) external onlyOwner {
    ...
    //Performs a buyback of another BEP20 token and automatically burns tokens
    function triggerExternalBuyback(uint256 amount, address token) external authorized {
        buybackToken(amount, token);
    }

    //Update limit ratios. ofCurrentSupply = true will set max wallet based on current supply. False will use initial supply
    function updateLimits(uint16 newMaxWalletRatio, uint16 newMaxSellRatio, uint16 newMaxBuyRatio, uint16 newDivisor, bool ofCurrentSupply) external onlyOwner {
        ...
        onlyOwner 
    {
        ...
        totalTaxRatio = newBurn + newBuyback + newDev + newLiquidity + newshib + newMarketing + newRewards;
        totalSwapRatio = totalTaxRatio - newBurn;
        distributeRatio = totalSwapRatio - newLiquidity;

        emit RatiosChanged (newBurn, newBuyback, newDev, newLiquidity, newshib, newMarketing, newRewards);
    }

    //update allocation of mainReward and miscReward
    function updateRewardSplit (uint8 mainSplit, uint8 miscSplit) external onlyOwner {
    ...
    function updateTax(uint8 newBuy, uint8 newSell, uint8 newTransfer) external onlyOwner {
    ...
    function withdrawshib() external authorized {
        uint256 amount = shibBalance;
        shibBalance = 0;
        _sendBnb(_taxWallet.shib, amount);
    }
    ...
    function setDevWallet(address payable addr) external authorized {
    ...
    function setshibWallet(address payable addr) external authorized {
        address prevshib = _taxWallet.shib;
        _excluded.remove(prevshib);
        _taxWallet.shib = addr;
        _excluded.add(_taxWallet.shib);
        emit UpdateshibWallet(addr);
    }
...
////// VIEW FUNCTIONS /////
    ...
    function getBlacklistInfo() external view returns (
        ...
        return (launchBlock, BLACKLIST_BLOCKS, snipersRekt, blacklistEnabled, revertSameBlock);
    }
    ...
    function getWithdrawBalances() external view returns (uint256 buyback, uint256 dev, uint256 shib, uint256 marketing) {
        return (buybackBalance, devBalance, shibBalance, marketingBalance);
    }
...
/////////////////////////////   PRIVATE FUNCTIONS  /////////////////////////////////////////
    ...
    function _distributeStake(uint256 bnbAmount, bool newStakingReward) private {
        ...
        uint256 devSplit = (bnbAmount*_taxRatios.dev) / distributeRatio;
        uint256 buybackSplit = (bnbAmount*_taxRatios.buyback) / distributeRatio;  
        uint256 stakingSplit = (bnbAmount*_taxRatios.rewards) / distributeRatio;
        uint256 shibSplit = (bnbAmount*_taxRatios.shib) / distributeRatio;      
    ...
    function _transfer(address sender, address recipient, uint256 amount) private {
        ...
        if (isContractTransfer || isLiquidityTransfer || isExcluded) {
            ...
            require(tradingEnabled || isWhitelisted[recipient], 'Trading has not been enabled yet.'); 
    ...
    function claimToken(address addr, address token, uint256 payableAmount, bool main) private {
        ...
        subtractStake(addr, amount, main);
        totalPayouts += amount;
