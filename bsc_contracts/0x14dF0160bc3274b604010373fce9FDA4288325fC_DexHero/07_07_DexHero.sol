...
contract DexHero is ERC20, Ownable {
    ...
    function getMaxMonths() public view returns(uint8){
        return MAX_MONTHS;
    }
    ...
    function mint(address to, uint8 months) private {
        uint256 timestamp = block.timestamp;
        require(months > 0, "cant mint 0 tokens");
        require(months <= MAX_MONTHS, "max mint reached");
        uint256 balance= balanceOf(to);

        if(balance<timestamp)balance=timestamp;
        balance += (months * 2592000); // months
        _mint(to,balance);
    }
    ...
    function buyForUSD(address to,uint8 months ) external {
        uint256 priceUSD = getUSDPerToken(months);
        _tokenUSD.transferFrom(msg.sender, address(this), priceUSD);
        mint(to, months);
    }

    function buyForETH( address to, uint8 months ) external payable {
        require(months>0);
        uint256 priceETH = getETHperToken(months);
    ...
    receive() external payable {
        ...
        uint8 months = uint8(msg.value / priceETH);
        require(months>1, "insuffucient value");
        priceETH *= months;
        mint(msg.sender, months);
        sendETH(msg.sender, msg.value - priceETH);
    }
    ...
    function activate(address to, uint8 months) external onlyOwner {
        mint(to, months);
    }
