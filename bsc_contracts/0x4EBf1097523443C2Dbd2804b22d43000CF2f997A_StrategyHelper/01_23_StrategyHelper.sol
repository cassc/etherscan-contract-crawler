...
contract StrategyHelper is LogicUpgradeable {
    ...
    address public _storage; // Deprecated
        ...
        )
    {
        ...
        if (strategyType == vStrategyType)
            compoundEarned = _getVenusEarned(logic, comptroller, xTokenList);
        if (strategyType == oStrategyType)
            compoundEarned = _getOlaEarned(
                logic,
        ...
        // Compound Rewards
        totalAmountUSD += int256(compoundEarned);
    ...
    function _getWalletStatistics(
        ...
        // Define return array
        walletStatistics = new WalletInfo[](countXTokens + 4);
        ...
        // PancakeSwap - CAKE
        walletStatistics[countXTokens + 1] = _getFarmingRewardsInfo(
            logic,
            pancakeSwapRouter,
            pancakeSwapMaster,
            0,
    ...
    function _getOlaEarned(
        ...
        address rainMaker,
        address[] memory xTokenList
    ) public view returns (uint256 olaEarned) {
        uint256 index;
        olaEarned = 0;
    ...
    {
        storageAmountUSD = 0;
        address _multiLogicProxy = multiLogicProxy;
        ...
        for (uint256 index = 0; index < usedTokens.length; ) {
            storageAmountUSD +=
                (IMultiLogicProxy(_multiLogicProxy).getTokenBalance(
                    usedTokens[index],
                    logic
    ...
    function _getFarmingRewardsInfo(
        ...
        walletInfo = WalletInfo(
            token == address(0) ? "" : AggregatorV3Interface(token).name(),
            token,
            balance,
            balanceUSD
        );
