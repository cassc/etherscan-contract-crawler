...
contract XeonLabsRecurring_V24Approved is Ownable {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;

    address public XEONLABS_TOKEN_ADDRESS;
    uint256 public MAX_STAKE = 20000000 ether;
    uint256 public MAX_STAKE_PER_USER = 20000000 ether;
    uint256 public MIN_STAKE = 10 ether;

    uint256 public REWARD_PERIOD = 30 days;

    uint256[] public RELEASE_PERIODS = [
        0,0,0,0,0,0,5000,5000,5000,5000,5000,5000,5000,5000,5000,5000,5000,5000,5000,5000,5000,5000,10000,10000
    ];
    uint256 public TOTAL_RELEASE_DAYS = 3 days;

    uint256 public ROI_PERCENTAGE = 30000; // 3%
    uint256 public TOTAL_PERCENTAGE = 300000;
    ...
    uint256 public deposited_XERCReward;
    ...
    constructor(address _xeonLabsToken) {
        require(_xeonLabsToken!=address(0),"Invalid xeonlabs token address");

        XEONLABS_TOKEN_ADDRESS = _xeonLabsToken;
    }
    
    function investAmount(uint256 _amount) external{
        require(_amount>=MIN_STAKE,"Cannot stake less than 10 XERC");
        require(totalInvestments.add(_amount)<=MAX_STAKE,"Cannot stake more than 20000000 XERC");
        require(investors[msg.sender].totalInvestment.add(_amount)<=MAX_STAKE_PER_USER,"Cannot stake more than 20.000,000 XERC");
        ...
        IERC20(XEONLABS_TOKEN_ADDRESS).safeTransferFrom(msg.sender,address(this),_amount);
    ...
    function getXercBalance() public view returns(uint256 balance){
        balance = IERC20(XEONLABS_TOKEN_ADDRESS).balanceOf(address(this));
    }
    ...
    function withdrawReward() external {
        ...
        IERC20(XEONLABS_TOKEN_ADDRESS).safeTransfer(msg.sender,totalRewardToRelease);
    ...
    function depositXERC(uint256 _amount) external onlyOwner{
        deposited_XERCReward = deposited_XERCReward.add(_amount);
        IERC20(XEONLABS_TOKEN_ADDRESS).safeTransferFrom(msg.sender,address(this),_amount);
    }
