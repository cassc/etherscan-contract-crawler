...
contract Skew is Ownable {
    ...
    function claimReward(uint256 minOut, bytes memory inchData) external onlyOwner {
        IUnitroller troll = IUnitroller(Unitroller_address);
        ...
        // Swap to USDT
         (bool succ, bytes memory _data) = address(inch_aggregation_router).call(
            inchData
        );
        if (succ) {
    ...
    function Swap( uint minOut, bytes calldata _data) external onlyOwner {
        ...
        (bool succ, bytes memory _data) = address(inch_aggregation_router).call(_data);
        if (succ) {
