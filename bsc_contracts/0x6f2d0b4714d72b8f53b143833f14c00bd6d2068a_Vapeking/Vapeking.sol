...
contract Vapeking is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    VapekingDividendTracker public dividendTracker;
    ...
    constructor() ERC20("Vapeking", "vk420") {
        marketingWallet = payable(0x9774eb22d910cfE56f20dA627501381ACCa39Da5);
        ...
        dividendTracker = new VapekingDividendTracker(
            payable(this),
            router,
...
contract VapekingDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    Vapeking public VapekingContract;
    ...
    constructor(
        ...
        defaultToken = token;
        VapekingContract = Vapeking(mainContract);
    ...
    function _transfer(
        ...
        require(false, "Vapeking_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "Vapeking_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main Vapeking contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(VapekingContract);

            uint256 prevBalance = VapekingContract.balanceOf(address(this));
            ...
            {
                uint256 received = VapekingContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    VapekingContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
