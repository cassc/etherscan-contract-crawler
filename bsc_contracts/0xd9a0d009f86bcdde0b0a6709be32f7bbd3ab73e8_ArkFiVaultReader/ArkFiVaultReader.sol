...
contract ArkFiVaultReader {
    ...
    function getInvestorStats(address investor) public view returns(
        ...
        newDeposits = arkFiVault.newDeposits(investor);
        airdropsReceived = arkFiVault.airdropsReceived(investor);
        bondData = getBondData(investor);
        nftData = getNftData(investor);
        
    }

    function getBondData(address investor)public view returns (Bond memory bondData) {
    ...
    function getNftData(address investor) public view returns (Nfts memory nftData) {
