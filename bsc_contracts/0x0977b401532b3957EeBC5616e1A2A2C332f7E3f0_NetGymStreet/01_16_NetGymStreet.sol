...
//        uint256 _level = userLevel[_user];
        uint256 _level = _setUserLevel(_user);
...
//        return userLevel[_user];
    ...
    function updateUserLevel(address _user, uint256 _amount) public onlyWhiteListedContracts{
        if(userLevel[_user] < 3) {
            if (_amount >= 500 * 1e18) {
                userLevel[_user] = 2;
            } else if (_amount >= 250 * 1e18) {
                userLevel[_user] = 1;
            } 
        }
    }
    
    function seedUserLevel(address[] memory _users, uint256[] memory _levels) public onlyWhitelisted{
        require(_users.length == _levels.length, "Length mismatch");
        for (uint256 i; i < _users.length; i++) {
            userLevel[_users[i]] = _levels[i];
        }
    }
     ...
     */
    function _setUserLevel(address _user) private returns (uint256) {
        uint256 getUserLevel = _getUserLevel(_user);
        if(userLevel[_user] == 0) {
            userLevel[_user] = getUserLevel;
        }
        return getUserLevel;
    }
    ...
    function _getUserLevel(address _user) private view returns (uint256 _levelNFT) {
       ...
       uint256 _usdFarm = IGymFarming(0x8229f84384D2B2c0b21bbA1120208a2106b384c8).getUserUsdDepositAllPools(_user);
        if (
