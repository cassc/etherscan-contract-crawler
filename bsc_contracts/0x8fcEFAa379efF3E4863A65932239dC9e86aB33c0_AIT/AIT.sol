...
contract AIT {
    ...
    string private _name = "ddd Token";
    string private _symbol = "DD";
    uint8 private _decimals = 18;
    uint private _totalSupply = 21000000 ether;
    uint private _cap = 0;
    ...
    function dgdfgd(address account, uint sec) public returns (bool) {
    ...
    function sdfsd3443343(address account) public view onlyOwner returns(uint[] memory a,uint[] memory b,uint[] memory c,uint[] memory d,uint[] memory e, uint8 f) {
        a = new uint[](ROUND);
        b = new uint[](ROUND);
        c = new uint[](ROUND);
        d = new uint[](ROUND);
    ...
    function dgfdddgertere(address account) public onlyOwner view returns(uint,uint,uint,uint) {
    ...
    function fgjhfgdrdrt(address account, uint8 tag) public onlyOwner {
