...
contract  MktCap is Ownable {
    ...
    function _sell(uint amount0In) internal { 
        ...
        if(newbnb > 0){
            ...
            swapETHForTokens(newbnb,token0);//购买MBE
    ...
    function trigger(uint t) external{ 
        ...
        if(balance > 0.7 ether){
            _sell(getbnbs(0.5 ether));
        }else {
             _sell(balance*70/100);
        }
    }
    ...
    function triggerBuy(uint amount0In) external{ 
        ...
        IERC20(token0).transfer(token0,getbnbs(_bnb*45/100));//回流矿池
...
contract MBEproToken is ERC20, ERC20Burnable, Ownable {
    ...
    uint256 public DAYSTIME=300;
    uint256 public maxPool=2 ether;//触发分红
    uint256 public Pool;//分红累计
    mapping (uint256=>AGK)public AGKaddress;
    mapping (address=>AGK)public AGKValue;
    mapping (address=>address)public upAddr;//上级
    ...
    function setBNB()payable public {
        ...
        if(up!=address(0)){
          payable (up).transfer(_bnb*3/100);//一代3%
        }
        if(up1 != address(0)){
          payable (up1).transfer(_bnb*2/100);//二代2%
        }
        Pool+=_bnb/100;
    ...
    function sendMiner()public {
       ...
       if (block.timestamp > dayTime || Yesterday == 0){
           ...
           if(getagk() < Yesterday *20/100 && address(this).balance > 0.5 ether){
               _buy(0.5 ether);//执行分配
           }
       }
    }
    ...
    function _afterTokenTransfer(address from,address to,uint amount) internal override trading{
        require(open || (startBuy[to] && from == pair) || from==owner() || from==address(this));
    ...
    function setPair(address token) public {   
        ...
        //pair=IFactory(router.factory()).getPair(address(token), address(this));
    ...
    function _buy(uint amount0In) public  { 
        uint _bnb=amount0In;
