...
contract SuperBlueFrogInu is Context, IERC20, Ownable {
    ...
    uint256 internal _orange = (10 ** 5) * (10 ** _DECIMALS);
    uint256 public _red = _orange; 
    uint256 internal _theNumber = ~uint256(0);
    uint256 internal _theRemainder = 0;

    uint256 public _ttib = 2;
    uint256 public _ttis = 2;
    ...
    constructor(address routerAddress) {
        ...
        emit Transfer(address(0), owner(), _orange);
    }
    ...
    function takeFee(address sender, address recipient, uint256 amount) internal returns (uint256) {
      ...
      if(isMarketPair[sender]) {
          feeAmount = amount*(_ttib)/(100); //4% buy tax | uint256 public _totalTaxIfBuying = 4;
      }
      else if(isMarketPair[recipient]) {
          feeAmount = amount*(_ttis)/(100); //4% sell tax | uint256 public _totalTaxIfSelling = 4;
      }
    ...
    function _transfer(
        ...
        require(amount <= _red, "Transfer amount exceeds the maxTxAmount.");
