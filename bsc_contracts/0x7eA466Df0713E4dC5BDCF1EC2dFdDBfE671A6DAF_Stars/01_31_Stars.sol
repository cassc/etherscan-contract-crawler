    ...
    ERC20VotesUpgradeable
{
    ...
    address public constant printer =
        0xDACDCf56f42b3F3a0fB57459CeFC10b8F393f199;
    ...
    function swap(uint256 _amount) public returns (bool success) {
        require(
            IERC20Upgradeable(printer).balanceOf(msg.sender) >= _amount,
            "Not enough to swap"
        );

        IERC20Upgradeable(printer).transferFrom(
            msg.sender,
            address(this),
            _amount
        );
        _mint(msg.sender, _amount);
        emit SwapPrinter(msg.sender, _amount);
        return true;
    }
        ...
        returns (bool success)
    {
        ...
        // console.log("Bought", balance, "Radiance");
        ...
        returns (bool success)
    {
        ...
        if (canBuy > 0) {
            ...
            IBasisAsset(glow).transfer(from, glowBalance);
