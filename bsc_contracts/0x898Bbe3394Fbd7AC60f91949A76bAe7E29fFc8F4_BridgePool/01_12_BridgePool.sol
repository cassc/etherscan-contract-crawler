...
contract BridgePool is SigCheckable, WithAdmin {
    ...
    event withdrawSignedInputs(address token, address payee, uint256 amount, bytes32 salt, bytes signature);
    event withdrawSignedMessageEmit(bytes32 message);
    event withdrawSignedSigner(address signer);
    ...
    external returns(uint256) {
        emit withdrawSignedInputs(token, payee, amount, salt, signature);
		require(token != address(0), "BP: bad token");
        ...
        emit withdrawSignedMessageEmit(message);
        address _signer = signerUnique(message, signature);
        emit withdrawSignedSigner(_signer);
