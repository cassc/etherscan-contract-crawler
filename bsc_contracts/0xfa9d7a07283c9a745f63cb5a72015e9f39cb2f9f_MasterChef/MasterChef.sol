...
contract BEP20DCXa is Context, IBEP20, Ownable {
  using SafeMath for uint256;
  ...
  constructor() public {
    _name = "Diverse Capital of Asiatic Exchanges";
    _symbol = "DCXa";
    _decimals = 18;
...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of DCXas
        // entitled to a user but is pending to be distributed is:
        //
        //   pending reward = (user.amount * pool.accDCXaPerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
        //   1. The pool's `accDCXaPerShare` (and `lastRewardBlock`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint;       // How many allocation points assigned to this pool. DCXa to distribute per block.
        uint256 lastRewardBlock;  // Last block number that DCXa distribution occurs.
        uint256 accDCXaPerShare;   // Accumulated DCXa per share, times 1e12. See below.
        uint16 withdrawFeeBP;      // Withdraw fee in basis points
    }

    // The MRWEB TOKEN!
    BEP20DCXa public dcxa;
    // MRWEB tokens created per block.
    uint256 public dcxaPerBlock;
    // Bonus muliplier for early DCXa makers.
    ...
    // The block number when DCXa mining starts.
    ...
    event EmissionRateUpdated(address indexed user, uint256 dcxaPerBlock);

    constructor(
        BEP20DCXa _dcxa,
        address _feeAddDev,
        uint256 _dcxaPerBlock,
        uint256 _startBlock
    ) public {
        dcxa = _dcxa;
        feeAddDev = _feeAddDev;
        dcxaPerBlock = _dcxaPerBlock;
    ...
    // Update the given pool's DCXa allocation point and deposit fee. Can only be called by the owner.
    ...
    // View function to see pending DCXas on frontend.
    function pendingDCXa(uint256 _pid, address _user) external view returns (uint256) {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];
        uint256 accDCXaPerShare = pool.accDCXaPerShare;
        uint256 lpSupply = pool.lpToken.balanceOf(address(this));
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
            uint256 dcxaReward = multiplier.mul(dcxaPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
            accDCXaPerShare = accDCXaPerShare.add(dcxaReward.mul(1e12).div(lpSupply));
        }
        return user.amount.mul(accDCXaPerShare).div(1e12).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 dcxaReward = multiplier.mul(dcxaPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        dcxa.mint(dcxaReward);
        pool.accDCXaPerShare = pool.accDCXaPerShare.add(dcxaReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }

    // Deposit LP tokens to MasterChef for DCXa allocation.
    function deposit(uint256 _pid, uint256 _amount) public nonReentrant poolExists(_pid) {
        ...
        user.rewardDebt = user.amount.mul(pool.accDCXaPerShare).div(1e12);
    ...
    // Safe DCXa transfer function, just in case if rounding error causes pool to not have enough DCXas.
    function safeDCXaTransfer(address _to, uint256 _amount) internal {
        uint256 dcxaBal = dcxa.balanceOf(address(this));
        bool transferSuccess = false;
        if (_amount > dcxaBal) {
            transferSuccess = dcxa.transfer(_to, dcxaBal);
        } else {
            transferSuccess = dcxa.transfer(_to, _amount);
        }
        require(transferSuccess, "safeDCXaTransfer: transfer failed");
    }
    ...
    function harvest(uint256 _pid) internal {
        ...
        uint256 pending = user.amount.mul(pool.accDCXaPerShare).div(1e12).sub(user.rewardDebt);
        if (pending > 0) {
            // send rewards
            safeDCXaTransfer(msg.sender, pending);
        }
    }

    // Pancake has to add hidden dummy pools in order to alter the emission, here we make it simple and transparent to all.
    function updateEmissionRate(uint256 _dcxaPerBlock) public onlyOwner {
        massUpdatePools();
        emit EmissionRateUpdated(msg.sender, _dcxaPerBlock);
        dcxaPerBlock = _dcxaPerBlock;
    }
