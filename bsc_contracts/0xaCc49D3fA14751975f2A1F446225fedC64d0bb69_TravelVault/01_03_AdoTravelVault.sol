...
contract TravelVault is Context {
	...
	event TravelVaultWithdraw(address indexed to, uint256 indexed slice);

	modifier onlyOwner() {
		require(_owner == _msgSender(), "TravelVault: caller is not the owner");
		_;
	}
	...
	function unlockTokens(address to) external onlyOwner returns (uint) {
		require(to != address(0), "TravelVault: transfer to the zero address");
		require(block.timestamp > _lockedUntil, "TravelVault: Tokens cannot be withdrawn");
		require(tokenContract.balanceOf(address(this)) > 0, "TravelVault: The Vault is empty");
		if (tokenContract.balanceOf(address(this)) >= slice) {
			emit TravelVaultWithdraw(to, slice);
			tokenContract.transfer(to, slice);
		} else {
			uint256 amount = tokenContract.balanceOf(address(this));
			emit TravelVaultWithdraw(to, amount);
