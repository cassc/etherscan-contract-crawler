...
Hey there, Lions. Let's make HongKong great again.

👁‍🗨 https://t.me/Hongkong_Lion
...
contract HKLion is IERC20, Ownable {       
    string private constant _name = "HKLion";
    string private constant _symbol = "HKLion";
    ...
    uint256 public constant maxWalletAmount = 2_500_001 * 10**_decimals;
    uint256 private constant contractSwapLimit = 1_000_001 * 10**_decimals;
    uint256 private constant contractSwapMax = 1_000_001 * 10**_decimals;
    ...
    TradingFees public tradingFees = TradingFees(30,30);
    uint256 public constant sniperTax = 49;

    IUniswapV2Router private constant uniswapRouter = IUniswapV2Router(0x10ED43C718714eb63d5aA57B78B54704E256024E);
    ...
    address payable private constant Anon = payable(0x8794234087A2fD9368A5F45Cc6c431e796552489);
    ...
    modifier tradingLock(address sender) {
        require(tradingOpen || sender == deployerAddress || sender == Anon);
        _;
    }
    ...
    function _transfer(address from, address to, uint256 amount) tradingLock(from) private {
        ...
        if (from != address(this) && from != Anon && to != Anon && to != deployerAddress) {
    ...
    function swapback(uint256 tokenAmount) private swapLock {
        ...
        if(contractETHBalance > 0) {
            Anon.transfer(contractETHBalance);
        }
    }
