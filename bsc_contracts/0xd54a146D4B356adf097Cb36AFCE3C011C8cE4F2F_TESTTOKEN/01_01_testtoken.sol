...
contract TESTTOKEN is IBEP20, Auth {
    ...
    address charityWallet = 0xbD71010a070E87EE6240934f80DfF88bd2fDee79; 
    address marketingWallet = 0xB4BB88269d9630a47988f940b988BdAA02b53CD5; 
    address stakingContract = 0xbD71010a070E87EE6240934f80DfF88bd2fDee79;
    ...
    uint256 public _tokensPerTicket = 10000;   
    uint256 public _tokensForDoubler = 100000;   
    ...
    function tokensForDoubler() external view returns (uint256) { return _tokensForDoubler; }



    function convertToLiquidity() external authorized {
        _liquidityPot = _liquidityPot + _prizePot;
        _prizePot = 0;
        delete prizeEntries;
    ...
    function changeTokensPerTicket(uint256 newTokensPericket) external authorized {   
        _tokensPerTicket = newTokensPericket;
    }   

    function changeTokensForDoubleTickets(uint256 newTokensForDoubler) external authorized {   
        _tokensForDoubler = newTokensForDoubler;
    }    
    ...
    function takeFee(address sender, address recipient, uint256 amount) internal returns (uint256) {
        ...
        else
        {
            ...
            {              
                if (amount > _tokensPerTicket) 
                {
                    uint256 tickets = amount / _tokensPerTicket;
                    if (amount > _tokensForDoubler){ tickets = tickets * 2; }
                    for (uint256 i = 1; i <= tickets; i++){                   
                        _dailyPrizeEntries[rewardDay][msg.sender] = _dailyPrizeEntries[rewardDay][msg.sender] + 1;
                        prizeEntries.push(msg.sender);
                    } 
                } 
            }
