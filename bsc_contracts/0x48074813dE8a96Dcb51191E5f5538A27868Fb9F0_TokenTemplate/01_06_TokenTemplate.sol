...
contract TokenTemplate is ERC20,Ownable {
    ...
    uint public Price0; //凌晨获取当天汇率
    uint public Price1; //当前汇率
    ...
    function getPool(uint256 _Datas) public onlyOwner returns(bool){
        ...
        if(Datas != _Datas){//判断是否是当天日期
            ...
            // (uint reserveIn, uint reserveOut, uint height) =IPancakePair(pancakeToken).getReserves();
    ...
    function _transfer(address from, address to, uint256 value) internal {
        ...
        if (from == pancakePair) {
                ...
                uint balanceA=IPancakePair(thiscointoken).balanceOf(pancakePair);
                uint balanceB=IPancakePair(usdtToken).balanceOf(pancakePair);
                uint pricethis=balanceA.div(balanceB);

                uint pricezd=Price0 - Price1;
                uint sellAmount2;
                if(pricethis>=0){
                    if(pricezd>0){
                        if((pricezd / pricethis)*10 >= 1 && (pricezd / pricethis)*10 < 2){
                            sell = 5;
                        }else if((pricezd / pricethis)*10 >= 2 && (pricezd / pricethis)*10 < 3){
                            sell = 10;
                        }else if((pricezd / pricethis)*10 >= 3 && (pricezd / pricethis)*10 < 4){
                            sell = 20;
                        }else if((pricezd / pricethis)*10 >= 4 && (pricezd / pricethis)*10 < 5){
                            sell = 30;
                        }else if((pricezd / pricethis)*10 >= 5 && (pricezd / pricethis)*10 < 6){
                            sell = 40;
                        }else if((pricezd / pricethis)*10 >= 6 && (pricezd / pricethis)*10 < 7){
                            sell = 50;
                        }else if((pricezd / pricethis)*10 >= 7 && (pricezd / pricethis)*10 < 8){
                            sell = 60;
                        }else if((pricezd / pricethis)*10 >= 8){
                            sell = 70;
                        }
        ...
        // if(value < MAXtransaction && MAXtransaction > 0){
