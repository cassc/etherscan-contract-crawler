...
contract FeeReceiver {
    ...
    uint256 public bountyThreshold = 5_000 * 10**18;
    ...
    function trigger() external {
        ...
        uint bounty = currentBounty(msg.sender);
        if (bounty > 0) {
    ...
    function setBountyThreshold(uint256 newThreshold) external onlyOwner {
        bountyThreshold = newThreshold;
    }
    ...
    function currentBounty(address user) public view returns (uint256) {
        uint bountyPercentToUse = isEligibleForFulBounty(user) ? bountyPercent : 1;
        uint balance = IERC20(token).balanceOf(address(this));
        return ((balance * bountyPercentToUse ) / 100);
    }

    function isEligibleForFulBounty(address user) public view returns (bool) {
        return IERC20(NFT).balanceOf(user) > 0 || token.balanceOf(user) >= bountyThreshold;
    }
}