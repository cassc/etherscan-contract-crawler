...
contract DividendDistributor is IDividendDistributor {
    ...
    IBEP20 RewardToken = IBEP20(0x674aA28Ac436834051fff3fC7b6e59D6f9c57a1c);
...
contract BABYOPT is IBEP20, Auth {
    
    using SafeMath for uint256;

    string public constant _name = "BabyOptimus";
    string public constant _symbol = "BABYOPT";
    ...
    function _basicTransfer(address sender, address recipient, uint256 amount) internal returns (bool) {
      ...
      function extractFee(address sender, address recipient, uint256 amount) internal returns (uint256) {
        ...
        if (sender != pair && sender != recipient){
            if (takeTransferFee){
                feeApplicable = totalFeeIfSelling; 
            }
            else{
