...
// File: contracts/CARSDividendPayingToken.sol
...
contract CARS is ERC20, Ownable {
    ...
    uint256 public maxBuyTransactionAmount = 1000000000 * (10**18); //no limits
    uint256 public maxSellTransactionAmount = 1000000000 * (10**18); //no limits
    ...
    function updateDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(dividendTracker), "CARS: The dividend tracker already has that address");

        CARSDividendTracker newDividendTracker = CARSDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "CARS: The new dividend tracker must be owned by the CARS token contract");
    ...
    function updateUniswapV2Router(address newAddress) external onlyOwner {
        require(newAddress != address(uniswapV2Router), "CARS: The router already has that address");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) external onlyOwner {
        require(pair != uniswapV2Pair, "CARS: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "CARS: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "CARS: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "CARS: Cannot update gasForProcessing to same value");
...
//Token code developed and tested by snipefinance.com 
    ...
    function _transfer(
        ...
        if(!tradingIsEnabled) {
            require(canTransferBeforeTradingIsEnabled[from], "CARS: This account cannot send tokens until trading is enabled");
        }
...
contract CARSDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() public DividendPayingToken("CARS_Dividend_Tracker", "CARS_Dividend_Tracker") {
        claimWait = 3600;
    ...
    function _transfer(address, address, uint256) internal override {
        require(false, "CARS_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override {
        require(false, "CARS_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main CARS contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "CARS_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "CARS_Dividend_Tracker: Cannot update claimWait to same value");
