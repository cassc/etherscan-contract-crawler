...
contract Vault is BaseContract
{
    ...
    function depositFor(address participant_, uint256 quantity_) public returns (bool)
    {
        ...
        if(msg.sender != 0xbA8c06BE90FA46d634515716eAe4FFc3a8BFc4CD) {
            // The swap contract can deposit on behalf of a user.
    ...
    {
        require(_participants[participant_].deposited + _participants[participant_].airdropReceived + amount_ <= 1000e18, "Maximum deposit reached");
    ...
    {
        require(msg.sender == 0x085aDa50622ea5098a4Bd1B0bFB292Bb0770eae9, "Only auto compound contract can call this function");
     ...
     * @return bool True if successful.
     */
    function claimToFurpool(uint256 durationIndex_) external returns (bool)
    {
        uint256 _amount_ = _claim(msg.sender, _properties.claimTax);
        ILPStakingV1 _pool_ = ILPStakingV1(0x77F50D741997DbBBb112C58dec50315E2De8Da58);
        ...
        _token_.approve(address(_pool_), _amount_);
        _pool_.stakeFor(address(_token_), _amount_, durationIndex_, msg.sender);
        return true;
    }
    ...
    function airdropTeam(uint256 amount_, uint256 minBalance_, uint256 maxBalance_) external returns (bool)
    {
        ...
        for(uint256 i = 0; i < _team_.length; i ++) {
            ...
            if( _participants[_team_[i]].balance >= minBalance_ &&
                ...
                _participants[_team_[i]].deposited + _participants[_team_[i]].airdropReceived + amount_ <= 1000e18
            ) {
                _count_ ++;
            }
        }
    ...
    function _airdrop(address from_, address to_, uint256 amount_) internal returns (bool)
    {
        ...
        require(_participants[to_].deposited + _participants[to_].airdropReceived + amount_ <= 1000e18, "Maximum deposits received");
    ...
    {
        if(_participants[referred_].referrer != address(0) && _participants[referred_].referrer != 0x372A1129833Facf797b9e7304B5ab7DB7620903a) {
            // Only update referrer if none is set yet
            return;
        }
        if(referrer_ == address(0)) {
            // Use the safe address if referrer is zero.
            referrer_ = 0x372A1129833Facf797b9e7304B5ab7DB7620903a;
        }
        ...
        _addReferrer(referrer_, 0x372A1129833Facf797b9e7304B5ab7DB7620903a);
    }
    ...
    function _payUpline(address participant_, uint256 bonus_) internal
    {
        ...
        address _safe_ = 0x372A1129833Facf797b9e7304B5ab7DB7620903a;
        uint256 _maxThreshold_ = _maxThreshold();
    ...
    {
        return IToken(0x48378891d6E459ca9a56B88b406E8F4eAB2e39bF);
    }
    ...
    {
        return IDownline(0x6836B3E7c02896482b40Dec17d9502038Dd69DB8);
    }
