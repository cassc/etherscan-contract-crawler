...
contract CounterpartyPool is Initializable, ERC20, Ownable, ReentrancyGuard
{
	...
	mapping(uint64 => FeeInfo) public feeCollectedPerDay;
	...
	function _collectFees() internal returns (uint256 _totalReserve, uint256 _totalSupply)
	{
		...
		if (_amount > 0) {
			...
			feeCollectedPerDay[day].amount += _fee * _totalReserve / _totalSupply;
			feeCollectedPerDay[day].shares += _fee;
		}
	...
	function _updateDay(uint256 _totalReserve) internal
	{
		...
		emit DailyFeeCollected(day, feeCollectedPerDay[day].amount, feeCollectedPerDay[day].shares);
	...
	event DailyFeeCollected(uint64 indexed _day, uint256 _amount, uint256 _shares);
}