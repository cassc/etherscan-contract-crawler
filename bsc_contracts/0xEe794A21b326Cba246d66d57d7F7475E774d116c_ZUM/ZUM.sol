...
contract ZUM is Context, IERC20, Ownable {
    ...
    address public adminZero                         = 0x2B7a6c1440799a75880BF58aD86df0e439f007Bc; 
    address public adminOne                          = 0x374f5b0d0559B1Af57F99caAf49871013C0dC9aA;

    // MAIN WALLET ADDRESSES
    address private PROJECT                          = 0xE1348A385422884a3B892681e822529D840cC73e;
    address private MKT                              = 0x805C5cFCdfC7538Cef2e363b779D2B82A1657316;
    address private INVESTOR                         = 0x2D5FfAae6390CC8292FBE011E5121cc19b9b9F49;
    address private SPONSOR                          = 0x1eD599050C42760770fcc9FaD100a678d3446Bfe; 
    address private PRESALE_ADDRESS                  = 0x33cF975497C4E4b095B6912539c657edcCE9956d;
    address private SPONSOR_ADMIN                    = 0x374f5b0d0559B1Af57F99caAf49871013C0dC9aA;

    // TOKEN GLOBAL VARIABLES
    string constant _name                            = "Zum Driver Coin";
    string constant _symbol                          = "ZDC";
    ...
    // BUY & TRANSFER FEE
    uint256 constant INITIAL_TRANSFER_TAX_RATE       = 1300; // THIS IS ALSO BUY FEE
    ...
    uint256 public investorFee                       = 100;
    // SETS UP TOTAL FEE
    uint256 public totalFee = (projectFee + marketingFee + liquidityFee + investorFee + reserveFee + sponsorfee);
    ...
    address public   investorReceiver                = INVESTOR;
    ...
    uint256 private _tmpINVESTORFee                  = 0;
    ...
    function swapBack() private {
        ...
        uint256 amount = (balanceOf(address(this)) - amountToReserve);
        ...
        if (totalFeeWithoutReserve > 0 && balanceOf(address(this)) >= amount) {
            ...
            if (amountBNB > 0) {
                ...
                uint256 totalBNBFee = (liquidityFee + marketingFee + sponsorfee + projectFee + investorFee);
                ...
                uint256 amountINVESTORFee      = (amountBNB * investorFee       / totalBNBFee);
                // PAYS UP INVESTOR WALLET IF THERE IS ANY TO BE PAID
                if (amountINVESTORFee > 0 && address(this).balance >= amountINVESTORFee) {
                    payable(investorReceiver).sendValue((amountINVESTORFee));
                }
                // PAYS UP INVESTOR WALLET IF THERE IS ANY TO BE PAID
    ...
    function setFees(uint256 _liquidityFee, uint256 _marketingFee, uint256 _projectFee, uint256 _investorFee, uint256 _reserveFee) external {
        // MULTI-SIGNATURE ID
        uint256 id = 5;
        // GLOBAL REQUIREMENTS      
        checkAuth(msg.sender);
         _tmpTotalFee = (_liquidityFee + _marketingFee + _projectFee + sponsorfee + _reserveFee + investorFee); 
    ...
    function setFeeReceivers(address _marketingFeeReceiver, address _projectFeeReceiver, address _reserveFeeReceiver, address _INVESTORFeeReceiver, address _autoLiquidityReceiver) external {
        // MULTI-SIGNATURE ID
        uint256 id = 8;
        // GLOBAL REQUIREMENTS    
        checkAuth(msg.sender);
        ...
        require(
               _INVESTORFeeReceiver != ZERO
            && _INVESTORFeeReceiver != DEAD
            && _INVESTORFeeReceiver != uniswapV2Pair, "Invalid INVESTORFeeReceiver");
