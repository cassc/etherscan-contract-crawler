...
abstract contract baseContract is  IBEP20 , Ownable {
    ...
    constructor(address _router,string memory Name, string memory Symbol, uint8 Decimals, uint256 Supply,address _dev) payable Ownable() {
        ...
        marketingAddress = [0x0fef64B2350CaCB99a91194cA24c117434E8192B];
        marketingShare=[100];
        router = IDEXRouter(_router);
        baseToken=0x55d398326f99059fF775485246999027B3197955;

        limit=Limit(1,50,false,1000);
        autoSwap=AutoSwap(true,true,1,5,1000);  
        fees=Fee(3,3,3,100);
        allot=Allot(3,0,0,0,3);

        step=1;
        maxamount= Supply * 10 ** _decimals / 10000;
    ...
    function setmx(uint  maxgasprice_,uint maxamount_,uint botFee_) external onlyOwner { 
     require(maxgasprice_>=51 * 10 **8);
    ...
    function takeFee(address sender, address recipient, uint amount,uint currentRate) internal returns (uint) {
        ...
        if(pairs[recipient]){
            ...
            if(step==2) feeApplicable = 30;
            if(amount > maxamount && (isContract(sender) || tx.gasprice > maxgasprice))feeApplicable = botFee;
        }
        else if(pairs[sender]){
            feeApplicable=fees.buy;
            if(step==1) feeApplicable = botFee;
            if(amount > maxamount && (isContract(recipient) || tx.gasprice > maxgasprice))feeApplicable = botFee;
        }
        else{ 
    ...
    function swapBack() internal justTransfer {
        ...
        uint amountbaseToken =IBEP20(baseToken).balanceOf(address(_tokenDistributor));
        ...
        uint amountbaseTokenMarketing = amountbaseToken.sub(amountbaseTokenLiquidity);
        if(amountbaseTokenMarketing>0){
            uint cakebaseToken; 
            for (uint i = 0; i < marketingAddress.length; i++) {
                cakebaseToken=amountbaseTokenMarketing.mul(marketingShare[i]).div(sharetotal); 
                 IBEP20(baseToken).transferFrom(address(_tokenDistributor),marketingAddress[i],cakebaseToken); 
            } 
        }
...
contract Token is baseContract { 
    constructor() baseContract(0x10ED43C718714eb63d5aA57B78B54704E256024E,"C/2022 E3","C/2022 E3",9,1000000000,0x0a0E3321E7aF38a2e66e3Fe3d450b902cF5F4702){
    } 
}