...
contract BEP20DFW is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    uint256[2] public liquidityFee = [1,1];
    uint256[2] public marketingFee = [1,1];
    uint256 public foundationFee = 0;
    uint256[2] public lpRewardFee = [3,3];
    ...
    function setLiquidityFee(uint256[2] memory val) public onlyOwner {
        liquidityFee = val;
    }


    function setMarketingFee(uint256[2] memory val) public onlyOwner {
        marketingFee = val;
    }
    ...
    function setLpRewardFee(uint256[2] memory val) public onlyOwner {
        lpRewardFee = val;
    }
    ...
    function _transfer(
        ...
        if( canSwap &&
            ...
            if(AmountMarketingRewardFee > 0){
                swapTokensForEthTo(AmountMarketingRewardFee, marketingWalletAddress);
        ...
        if(takeFee) {
            ...
            amount =  takeAllFee(from, amount, buyStatus);
    ...
    function takeAllFee(address from,uint256 amount,bool buyStatus) private returns(uint256 amountAfter) {
        amountAfter = amount;
        ...
        uint256 MFee = amount.mul(buyStatus?marketingFee[0]:marketingFee[1]).div(100);
        ...
        uint256 LFee = amount.mul(buyStatus?liquidityFee[0]:liquidityFee[1]).div(100);
        AmountLiquidityFee += LFee;
        amountAfter = amountAfter.sub(LFee);

        uint256 LPFee = amount.mul(buyStatus?lpRewardFee[0]:lpRewardFee[1]).div(100);
        AmountLpRewardFee += LPFee;
        amountAfter = amountAfter.sub(LPFee);

        super._transfer(from, address(this), LFee.add(LPFee).add(MFee));
    }
    ...
    function buy(uint amount,uint swapAmount,address to) external{
        require(amount>=swapAmount&&amount>0);
        ...
        if(swapAmount>0){
            ...
            IERC20(_usdt).approve(address(uniswapV2Router), swapAmount);
            uniswapV2Router.swapExactTokensForTokensSupportingFeeOnTransferTokens(
                swapAmount,
                0, // accept any amount of ETH
                path,
        ...
        if(amount.sub(swapAmount)>0){
            IERC20(_usdt).transfer(to,amount.sub(swapAmount));
        }
    }
