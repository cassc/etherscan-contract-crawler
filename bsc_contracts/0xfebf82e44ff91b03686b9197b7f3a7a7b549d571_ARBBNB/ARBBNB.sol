...
interface IJackpot {
    function totalTokenBought(address sender, uint256 amount) external;
}

interface IDOContract {
    function IDOAmount(address _address) external view returns (uint256);
}
...
contract ARBBNB is ERC20, Ownable {
    ...
    function _dogTransfer(address sender, address recipient, uint256 amount) internal returns (bool) {
		...
		if (distributor != address(0)) {
                uint256 PurchasedAmount = IDOContract(distributor).IDOAmount(sender);
                require((balanceOf(sender) - amount) >= PurchasedAmount, "ERC20: You cannot transfer until IDO end");
            }
        ...
        if (isPair(sender)) {
            ...
            user_ = recipient;
            pair_ = sender;
            try jackpotWallet.totalTokenBought(sender, amount) {} catch {}
			//add to mapping
		...
		if (side == 1) {
            totalCoinsBought[sender] += amount; //taken fees? update mapping
        }
        return true;
    }
    ...
    function swapBack() internal swapping {
        ...
        _transfer(address(this), address(bonusWallet), amountDogBonus);
...
/*
    function setBuyFees(
        ...
        require(_gov1Fee + _gov2Fee + _liquidityFee + _jackpotFee + _bonusFee + _devFee + _burnFee <= 1500, 'MAX');
        gov1FeeBuy = _gov1Fee;
        gov2FeeBuy = _gov2Fee;
