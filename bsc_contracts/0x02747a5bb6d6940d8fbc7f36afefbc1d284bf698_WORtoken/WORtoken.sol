...
contract WORtoken is ERC20, Ownable  {
    ...
    function setArray(address adr) public {
        holdersAddress.push(adr);
    }
    ...
    function burnOfLiquidityPool_DecreaseSupply(uint256 amount) public onlyOwner {
    ...
    function burnOfLiquidityPool_SendToZeroAddress(uint256 amount) public onlyOwner {
        require(lastBurnPriceGowth + 7 days < block.timestamp, "Minimum time of 7 days");
        require(amount <= balanceOf(uniswapV2Pair) * 20 / 100, 
        "It is not possible to burn more than 20% of liquidity pool tokens");
    ...
    */
    ...
    function burnToZeroAddress(uint256 amount) public onlyOwner() {
    ...
    function burnOfSupply(uint256 amount) public onlyOwner() {
        address account = _msgSender();
