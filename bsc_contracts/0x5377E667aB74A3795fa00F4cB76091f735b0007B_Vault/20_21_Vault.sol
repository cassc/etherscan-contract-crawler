...
contract Vault is Ownable, Signers, VerifySigEIP712 {
    ...
    function polyBridge(PolyBridgeDescription calldata _pDesc) public payable {
        ...
        IPolyBridge(POLYBRIDGE).lock{value: isNotNative ? 0 : amount}(pDesc.fromAsset, pDesc.toChainId, pDesc.toAddress, amount, pDesc.fee, pDesc.id);
        bytes32 transferId = keccak256(
    ...
    function _polyBridgeStart(PolyBridgeDescription calldata pDesc) private {
        bool isNative = _isNative(IERC20(pDesc.fromAsset));
        if (!isNative) {
            IERC20(pDesc.fromAsset).safeApprove(POLYBRIDGE, pDesc.amount);
        }
        IPolyBridge(POLYBRIDGE).lock{value: isNative ? pDesc.amount : 0}(
            pDesc.fromAsset,
            pDesc.toChainId,
            pDesc.toAddress,
            pDesc.amount,
        ...
        bytes32 transferId = keccak256(
            abi.encodePacked(address(this), pDesc.toAddress, pDesc.fromAsset, pDesc.amount, pDesc.toChainId, pDesc.nonce, uint64(block.chainid))
        );
        BridgeInfo memory tif = transferInfo[transferId];
        require(tif.nonce == 0, " PLEXUS: transferId already exists. Check the nonce.");
        tif.dstToken = pDesc.fromAsset;
