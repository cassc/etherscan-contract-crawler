...
contract Lands is Administered, ReEntrancyGuard {
    ...
    function createCodeLand(uint256 _code) external onlyUser returns (bool) {
        /// @dev check if the land is already created
        require(!LandsWithdrawal[_code].withdrawal, "Land already created");

        /// @dev create a new land
        LandsWithdrawal[_code] = StructLands(address(0), _code, 0, true);
    ...
    function mintLands(
        ...
        IPropertyToken(tokenAddress).mintReserved(_msgSender(), _land.lands);
