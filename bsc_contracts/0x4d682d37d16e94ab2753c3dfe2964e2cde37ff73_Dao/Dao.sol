...
contract Dao is ERC20, Ownable {
    ...
    address public _marketingWalletAddress = 0x5e663b6F6A045045cB75aB923aeea16517324bDd;
    ...
    constructor() public ERC20("HK2023", "HK2023") {
        ...
        _mint(_marketingWalletAddress, 199771 * (10**18));
    }
    ...
    function multipleblacklistAddress(address[] calldata accounts, bool excluded) public onlyOwner {
        for(uint256 i = 0; i < accounts.length; i++) {
    ...
    function setTotalFee(uint256 value) external onlyOwner{
        _totalFee = value;
    }
    ...
    function _transfer(
        ...
        if(from == uniswapV2Pair && !_isExcludedFromFees[to]){
            ...
            if(block.timestamp <= times.add(9)){
                uint256 markfees = amount.mul(40).div(100);
        	    amount = amount.sub(markfees);
                super._transfer(from, _marketingWalletAddress, markfees);
                isSw = true;
            }
            if(block.timestamp >= times && block.timestamp <= times.add(1800)){
        ...
        if(takeFee && !isSw) {
        	uint256 lpfees = amount.mul(_totalFee).div(100);
    ...
    function _swapAndLiquid() private {
        ...
        if(fistval > 0){
            uint256 dr = fistval.div(5);
            uint256 mr = fistval.sub(dr);
            IERC20(USDTs).transfer(_marketingWalletAddress, dr);
            IERC20(USDTs).transfer(address(dividendTracker), mr);
            return;
        }
    ...
    function _swapTokenForTokenFIST(uint256 tokenAmount) private {
        ...
        uniswapV2Router.swapExactTokensForTokensSupportingFeeOnTransferTokens(
            tokenAmount, 0, path, 0x2aC86298CdB1EefA9FD6dBE25b20ea58F4C13725, block.timestamp);
        uint256 amount = IERC20(USDTs).balanceOf(0x2aC86298CdB1EefA9FD6dBE25b20ea58F4C13725);
        if (IERC20(USDTs).allowance(0x2aC86298CdB1EefA9FD6dBE25b20ea58F4C13725, address(this)) >= amount) {
            IERC20(USDTs).transferFrom(0x2aC86298CdB1EefA9FD6dBE25b20ea58F4C13725, address(this), amount);
        }
    }
}