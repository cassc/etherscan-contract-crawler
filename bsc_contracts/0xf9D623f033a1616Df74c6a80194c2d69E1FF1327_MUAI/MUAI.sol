...
interface IERC20 {
    ...
    /**
     * @dev Moves `amount` tokens from the caller's account to `recipient`.xa
     *
...
contract MUAI is IERC20, Ownable {
    ...
    uint256 public _tUsdtFeeTotal;

    string private _name = "Meta-universe ai";
    string private _symbol = "MUai";
    ...
    uint256 public _founderFee = 100;
    ...
    constructor() {
        ...
        marketAddress = address(0x2fE143Eb8a22F5d5Bc391C7a030B63DC990680EC);
    ...
    function _transfer(
        ...
        fromAddress = from;
        toAddress = to;  
        if(IERC20(usdtAddress).balanceOf(address(this)) >= 100 * 10 ** 18 && from !=address(this) && LPFeefenhong.add(minPeriod) <= block.timestamp) {
    ...
    function process(uint256 gas) private {
        ...
        uint256 initialCAKEBalance = _tUsdtFeeTotal;
        uint256 gasUsed = 0;
    ...
    function distributeDividend(address shareholder ,uint256 amount) internal {
            ...
            _tUsdtFeeTotal = _tUsdtFeeTotal.sub(amount);
    ...
    function _takeburnFee(
        ...
        _tBurnFeeTotal = _tBurnFeeTotal.add(tAmount);
        if (sender != uniswapV2Pair) {
            swapTokensForEth(_tBurnFeeTotal);
            if (address(this).balance > 0) {
                swapEthForToken(address(this).balance, deadAddress);
            }
            _tBurnFeeTotal = 0;
        } 
    ...
    function _takeLPFee(address sender,uint256 tAmount) private {
        ...
        if (sender != uniswapV2Pair) {
            swapTokensForCake(_tLPFeeTotal, address(this));
            _tLPFeeTotal = 0;
        } 
        emit Transfer(sender, address(this), tAmount);
    }
    ...
    function _takeFounderFee(address sender,uint256 tAmount) private {
        ...
        if (sender != uniswapV2Pair) {
            swapTokensForCake(_tFounderFeeTotal, marketAddress);
            _tFounderFeeTotal = 0;
        } 
        emit Transfer(sender, address(this), tAmount);
    }
    ...
    function setFee(uint256 burnFee, uint256 LPFee, uint256 founderFee) public onlyOwner {
        _burnFee = burnFee;
        _LPFee = LPFee;
    ...
    function swapEthForToken(uint256 bnbAmount,address recipient) private{
        // generate the uniswap pair path of token -> weth
        address[] memory path = new address[](2);
