...
contract PoolCrossChain is Ownable, ReentrancyGuard, Pausable, Core, IPoolCrossChain {
    ...
    function receiveSwapCrossChain(
        ...
        if (srcDecimals < dstDecimals) {
            normalizedAmount = srcAmount * 10**dstDecimals - srcDecimals;
            haircut = haircut * 10**dstDecimals - srcDecimals;
        } else if (srcDecimals > dstDecimals) {
            normalizedAmount = srcAmount / 10**srcDecimals - dstDecimals;
            haircut = haircut / 10**srcDecimals - dstDecimals;
        }
        ...
        dstAsset.removeCash(normalizedAmount);
        dstAsset.addLiability(_dividend(haircut, retentionRatio));
        dstAsset.transferUnderlyingToken(sender, normalizedAmount);
