...
contract PotLottery is ReentrancyGuard {
    ...
    constructor(address _owner) {
        ...
        minEntranceInUsd = 8000000000; //80 cents ~ 1$
        percentageFee = 3;
        potCount = 0;
        timeBeforeRefund = 1800; //30 minutes
        PotEntryCount = 0;
        entriesCount = 0;

        airdropInterval = 1800 * 30;
        burnInterval = 1800;
        lotteryInterval = 1800 * 7;
        ...
        minimumStakingTime = 1800; // 30 minutes
        ...
        BNBP_Standard = 100;
        hotWalletAddress = 0xDA416dF2538FBCD9445F792D232DFAC1C4E3B1fe;
        hotWalletMinBalance = 5 * 10**18;
        hotWalletMaxBalance = 10 * 10**18;
        
        BNBAddress = 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c ;
        BNBPairAddress = 0x58F876857a02D6762E0101bb5C46A8c1ED44Dc16 ;
        ...
        addToken(
            ...
            0x487BfE79C55AC32785c66774b597699E092d0Cd9,
            true,
            false,
            9
        );
        
        UpdatePrice('Volt Inu');
    }
 ...
 */
        ...
        if (_token.swapToBNB && keccak256(bytes(_token.tokenSymbol)) != keccak256(bytes('WBNB'))) {
            ...
            updateTokenUsdValue(
                ...
                ((price * 10**10) * _BNBPrice) /
                    10**36 //10 raised to the power of BNB decimal(18) and BUSD decimal(18)
            );
        } else {
            updateTokenUsdValue(_tokenName, (price * 10**10) / 10**18);//10**18 is 10 raised to the power of BUSD decimal
        }
    }
    ...
    */
    ...
    function refundToken(string memory _tokenName, address _accountToPay, uint256 _tokenValue) external onlyAdmin {
        _payAccount( _tokenName, _accountToPay, _tokenValue) ;
    }
}