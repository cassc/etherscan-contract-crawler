    ...
    function getNextExpansionRate(uint256 _crystalPrice) public view returns (uint256) {
        ...
        if (epoch < bootstrapEpochs || isBoostEnabled) {
            ...
            if (_crystalPrice > crystalPriceOne) {
                ...
                expansionRate = _crystalPrice.sub(crystalPriceOne);
                uint256 _mse = _calculateMaxSupplyExpansionPercent(crystalSupply).mul(crystalPriceOne).div(10000);
                if (expansionRate > _mse) {
                    expansionRate = _mse;
                }
            }
        }

        return expansionRate.div(crystalPriceOne);
    }
    ...
    function allocateSeigniorage() external onlyOneBlock checkCondition checkEpoch checkOperator {
        ...
        if (epoch < bootstrapEpochs || isBoostEnabled) {
            ...
            if (previousEpochCRSPrice > crystalPriceOne) {
                // Expansion ($CRS Price > 1 $BUSD): there is some seigniorage to be allocated
                uint256 _percentage = previousEpochCRSPrice.sub(crystalPriceOne);
                uint256 _savedForBoardroom;
                maxSupplyExpansionPercent = _calculateMaxSupplyExpansionPercent(crystalSupply);
                uint256 _mse = maxSupplyExpansionPercent.mul(crystalPriceOne).div(10000);
                if (_percentage > _mse) {
                    _percentage = _mse;
                }

                _savedForBoardroom = crystalSupply.mul(_percentage).div(crystalPriceOne);
