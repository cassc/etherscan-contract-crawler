...
contract TradeAI is IBEP20, Ownable
{
    ...
    //Token Info
    string private constant _name = 'TraderAI';
    ...
    uint constant antiBotDuration=2 minutes;
    ...
    function BuyTax() public view returns(uint){
           uint timeSinceLaunch=block.timestamp-LaunchTimestamp;
            if(timeSinceLaunch<=antiBotDuration){
                uint botTax=TAX_DENOMINATOR-buyTax;
                return (buyTax+(botTax-(botTax*timeSinceLaunch/antiBotDuration)));
            }
            return buyTax;
    }
 ...
 tax=BuyTax();
    ...
    function _swapContractToken() private lockTheSwap{
        ...
        isOverLiquified()?0
        :(contractBalance*liquidityTax)/totalTax;

        uint tokenForMarketing= contractBalance-tokenForLiquidity;
