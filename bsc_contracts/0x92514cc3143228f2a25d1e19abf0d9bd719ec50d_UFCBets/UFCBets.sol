...
//official telegram UfcBetsToken 
//official website www.ufcbetstoken.co.uk
...
contract UFCBets is BEP20, LockToken {
    using SafeMath for uint256;
    ...
    UFCBetsDividendTracker public dividendTracker;
    ...
    address payable public _marketingWalletAddress = payable(0x2031e9a8e8171F7a8855BAE36BF1561324F7f75B);
    address payable public _buybackWalletAddress = payable(0xf6D0C1f6ab95De67E5B890E137FA10D48A93Ec88);
    ...
    constructor() BEP20("UFC Bets", "UFCB") 
    {
        dividendTracker = new UFCBetsDividendTracker();
    ...
    {
        require(newAddress != address(dividendTracker), "UFCBets: The dividend tracker already has that address");
        UFCBetsDividendTracker newDividendTracker = UFCBetsDividendTracker(payable(newAddress));
        require(newDividendTracker.owner() == address(this), "UFCBets: The new dividend tracker must be owned by the UFCBets token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "UFCBets: The router already has that address");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "UFCBets: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "UFCBets: The PanBUSDSwap pair cannot be removed from automatedMarketMakerPairs");
        _setAutomatedMarketMakerPair(pair, value);
    }
    

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "UFCBets: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "UFCBets: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "UFCBets: Cannot update gasForProcessing to same value");
...
contract UFCBetsDividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("UFCBets_Dividend_Tracker", "UFCBets_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 2000_000 * (10**18); //must hold 2 Millions+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "UFCBets_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "UFCBets_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main UFCBets contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "UFCBets_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "UFCBets_Dividend_Tracker: Cannot update claimWait to same value");
