...
contract VentionAutoFarmBSC {
    ...
    uint256[] public PLAN_UNLOCK_TIME = [0 days, 0 days];
    uint256[] public PLAN_STAKE_BONUS = [20, 50];

    uint256[] public TOKENS_AIRDROP_PERCENTS = [4, 10];
	
	
	uint256[] public REFERRAL_PERCENTS = [100, 30, 20, 20, 20, 2, 2, 2, 2, 2];
    ...
    uint256 public constant STAKE_MIN_AMOUNT = 1e18; //  1 BUSD
	...
	constructor(address payable wallet, address payable _developer, address payable _marketing, IBEP20 BAF_TOKEN_ADDRESS) public {
        ...
        plans.push(Plan(180, 8)); // 0.8% per day for 180 days = 144% roi
        plans.push(Plan(360, 10)); // 1% per day for 360 days = 360% roi
...
function invest(address payable referrer,uint8 plan) public payable {
	...
	function stakeBafTokens(uint256 bafAmount) public {
		...
		require(BAF_TOKEN.balanceOf(msg.sender) >= bafAmount, "Insuff. VAFBSC balance");
    ...
    function claimBafTokens() public {
        ...
        require(user.bafTokensToClaim > 0, "Insuff. VAFBSC bal.");
	...
	function stake(uint256 busdAmount) public {
	    require(busdAmount >= STAKE_MIN_AMOUNT, "Min. is 1 MATIC");
        require(BUSD.balanceOf(msg.sender) >= busdAmount, "Insuff. MATIC bal.");
