...
contract DividendDistributor is IDividendDistributor {
    ...
    // EARN
    IBEP20 public RWRD = IBEP20(0xECCF35F941Ab67FfcAA9A1265C2fF88865caA005);
    ...
    function setRewardtoken(address _rwrdtoken) external onlyToken returns (string memory) {
        uint256 shareholderCount = shareholders.length;

        uint256 gasLeft = gasleft();
        uint256 index = 0;

        for (index = 0; index < shareholderCount; index++) {
            distributeDividend(shareholders[index]);

            gasLeft = gasleft();
            if (gasLeft == 0) {
                return "Out of gas. Repeat the transaction";
            }
        }

        RWRD = IBEP20(_rwrdtoken);
        return "Reward token has been set";
    }
}

/* Token contract */
contract BullishOnLunaFlare is IBEP20, Auth {
    using SafeMath for uint256;
    ...
    // Name and symbol
    string constant _name = "LunaFlare";
    string constant _symbol = "LUNAF";
    ...
    // Max wallet and TX
    uint256 public _maxBuyTxAmount = (_totalSupply * 50) / 10000; // 2%
    uint256 public _maxSellTxAmount = (_totalSupply * 50) / 10000; // 2%
    uint256 public _maxWalletToken = (_totalSupply * 50) / 10000; // 2%
    ...
    constructor() Auth(msg.sender) {
        ...
        // Set the marketing, community and liq receiver to the owner as default
    ...
    function _transferFrom(
        ...
        uint256 amountReceived = shouldTakeFee(sender)
            ? takeFee(sender, amount, isSell, GREEDMode)
            : amount;
    ...
    function checkMaxWallet(
        ...
        if (
            ...
            recipient != communityFeeReceiver
        ) {
    ...
    function takeFee(
        ...
        if (GREEDMode) {
            if (isSell) {
                ...
                feeAmount = amount.mul(totalFee).mul(3).div(2).div(
                    feeDenominator
                );
            } else {
    ...
    function swapBack() internal swapping {
        ...
        uint256 amountBNBCommunity = amountBNB.mul(communityFee).div(totalBNBFee);
        ...
        (
            bool successCommunity, /* bytes memory data */

        ) = payable(communityFeeReceiver).call{value: amountBNBCommunity, gas: 30000}("");
        require(successMarketing, "marketing receiver rejected ETH transfer");
        require(successCommunity, "community receiver rejected ETH transfer");
    ...
    function setSellFees(
        ...
        communityFeeSell = _communityFeeSell;
        totalFeeSell = _liquidityFeeSell
            .add(_buybackFeeSell)
            .add(_reflectionFeeSell)
            .add(_marketingFeeSell)
            .add(_communityFeeSell);
        feeDenominator = _feeDenominator;
    }

    // Set the reward token address
    function setRewardtoken(address _rwrdtoken) external authorized {
        distributor.setRewardtoken(_rwrdtoken);
    }

    // Set the community receiver
    function setCommunityFeeReceiver(address _communityFeeReceiver) external authorized {
    ...
    function setLPFeeReceiver(address _autoLiquidityReceiver)
        external
        authorized
    {
