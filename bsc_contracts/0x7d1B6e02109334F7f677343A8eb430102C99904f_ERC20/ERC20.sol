...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    ...
    uint256 private _sellBurnFee = 3;
    uint256 private _sellRewardFee = 2;
    uint256 private _transferBurnFee = 5;
    uint256 public largeBuyLimitTime = 5 minutes;
    uint256 public largeBuyAmount = 300000 * 10**18;
    IUniswapV2Router02 public uniswapV2Router;
    address public uniswapV2Pair;
    address private _usdt;
    address private _destroyAddress = address(0x000000000000000000000000000000000000dEaD);
    bool public onAntiPlunge;
    address public antiPlungeOperator;
    address public rewardAddress;
    bool public onSwap;
    uint256 public openSwapTime;
    ...
    constructor(
        ...
        antiPlungeOperator = antiPlungeOperator_;

        isExcludedFromTransferFee[msg.sender] = true;
    }
    ...
    function openSwap() public onlyOwner returns (bool) {
        onSwap = true;
        openSwapTime = block.timestamp;
        return true;
    }
    ...
    function antiPlunge() public returns (bool) {
        require(!onAntiPlunge, "ERC20: Invalid operate");
        require(msg.sender == antiPlungeOperator, "ERC20: Permission denied");
        onAntiPlunge = true;
        _buyBurnFee += 10;
        _sellBurnFee += 10;
        return true;
    ...
    function antiPlungeExist() public returns (bool) {
        require(onAntiPlunge, "ERC20: Invalid operate");
        require(msg.sender == antiPlungeOperator, "ERC20: Permission denied");
        onAntiPlunge = false;
        _buyBurnFee -= 10;
    ...
    function _transfer(
        ...
        if (from == uniswapV2Pair) {
            if (_isRemoveLiquidity()) {
                ...
                //buy
                require(onSwap || inWhitelist[to], "ERC20: can not swap");
                if (inWhitelist[to]) {
                    takeFee = false;
                }
                buy = true;
            ...
            if (_isAddLiquidity()) {
                ...
                //sell
                require(onSwap || inWhitelist[from], "ERC20: can not swap");
                if (inWhitelist[from]) {
                    takeFee = false;
                }
                sell = true;
        ...
        if (takeFee) {
            if (buy) {
                if (onSwap && block.timestamp < openSwapTime + largeBuyLimitTime) {
                    require(amount <= largeBuyAmount, "ERC20: Over the buy limit");
                }
                _tokenBuySell(from, to, amount, true, false);
            }
            if (sell) {
                if (amount == fromBalance) {
                    amount = (amount * 99) / 100;
                }
                _tokenBuySell(from, to, amount, false, true);
            }
            if (transferToken) {
    ...
    function _tokenBuySell(
        ...
        if (buy) {
            ...
            rewardAmount = (amount * _buyRewardFee) / 100;
        }
        if (sell) {
            burnAmount = (amount * _sellBurnFee) / 100;
            rewardAmount = (amount * _sellRewardFee) / 100;
        }
        unchecked {
            _balances[from] = fromBalance - amount;
            // Overflow not possible: the sum of all balances is capped by totalSupply, and the sum is preserved by
            // decrementing then incrementing.
            _balances[_destroyAddress] += burnAmount;
            _balances[rewardAddress] += rewardAmount;
            _balances[to] += amount - burnAmount - rewardAmount;
            _totalSupply -= burnAmount;
        }
        emit Transfer(from, _destroyAddress, burnAmount);
        emit Transfer(from, rewardAddress, rewardAmount);
        emit Transfer(from, to, amount - burnAmount - rewardAmount);
    }
