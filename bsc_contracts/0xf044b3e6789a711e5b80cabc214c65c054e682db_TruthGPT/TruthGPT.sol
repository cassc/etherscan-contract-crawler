...
contract TruthGPT is IBEP20, Auth {
    using SafeMath for uint256;
    ...
    string constant _symbol = "TruthGPT";
    ...
    uint256 public GameDevFee          = 0;
	uint256 public ecoFee          = 0;
    uint256 public totalFee        = marketingFee + reflectionFee + liquidityFee + GameDevFee +ecoFee;
    ...
    constructor () Auth(msg.sender) {
        ...
        autoLiquidityReceiver = 0x59187BF33994F5e5b12905aCb907065C9c6d84D1;
        marketingFeeReceiver = 0x59187BF33994F5e5b12905aCb907065C9c6d84D1;
        GameDevFeeReceiver = 0x59187BF33994F5e5b12905aCb907065C9c6d84D1;
		ecoFeeReceiver = 0x59187BF33994F5e5b12905aCb907065C9c6d84D1;
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if (!authorizations[sender] && recipient != address(this)  && recipient != address(DEAD) && recipient != pair && recipient != marketingFeeReceiver && recipient != GameDevFeeReceiver  && recipient != autoLiquidityReceiver && recipient != ecoFeeReceiver ){
    ...
    function swapBack() internal swapping {
        ...
        uint256 amountBNBDev = amountBNB.mul(GameDevFee).div(totalBNBFee);
		uint256 amountBNBEco = amountBNB.mul(ecoFee).div(totalBNBFee);

        try distributor.deposit{value: amountBNBReflection}() {} catch {}
        (bool tmpSuccess,) = payable(marketingFeeReceiver).call{value: amountBNBMarketing, gas: 30000}("");
        (tmpSuccess,) = payable(GameDevFeeReceiver).call{value: amountBNBDev, gas: 30000}("");
    ...
    function setFees(uint256 _liquidityFee, uint256 _reflectionFee, uint256 _marketingFee, uint256 _GameDevFee,uint256 _ecoFee, uint256 _feeDenominator) external authorized {
        liquidityFee = _liquidityFee;
        reflectionFee = _reflectionFee;
        marketingFee = _marketingFee;
        GameDevFee = _GameDevFee;
		 ecoFee = _ecoFee;
        totalFee = _liquidityFee.add(_reflectionFee).add(_marketingFee).add(_GameDevFee).add(_ecoFee);
        feeDenominator = _feeDenominator;
    }

    
    function setFeeReceivers(address _autoLiquidityReceiver, address _marketingFeeReceiver, address _GameDevFeeReceiver, address _ecoFeeReceiver) external authorized {
