...
contract EternalCakesStaker is Ownable, ReentrancyGuard, IERC721Receiver {
    ...
    address public DISTRIBUTOR = 0x07c569b26A820C99A136ec6f7d684db5815b7f43; 
    ...
    function compound() public {
        ...
        uint forDistribution;
        if (FUND_BOOSTER) {
            uint forBooster = balance.calcPortionFromBasisPoints(BOOSTER_PERCENTAGE);
            forDistribution = balance - (forDev + forRestake + forBooster);
            boost(forBooster);
        } else {
            forDistribution = balance - (forDev + forRestake);
        }
        restake(forRestake);
    ...
    // unlock the cake first and then withdraw
    function unlockCake() public onlyOwner() {
        ICakePool(POOL).unlock(address(this));
    }
