...
// every strand of jembot has a price
// https://t.me/jembotbsc
...
contract Jembot is Ownable, IBEP20 {
    ...
    string constant _name = "JEMBOT";
    string constant _symbol = "JEMBOT";
    uint8 constant _decimals = 9;

    uint256 _totalSupply = 1 * 10**6 * 10**_decimals;

    uint256 public _maxTxAmount = _totalSupply.mul(1).div(100);
    uint256 public _maxBag = _totalSupply.mul(1).div(100);
    ...
    uint256 sellMultiplier = 1200;
    uint256 buyMultiplier = 700;
    ...
    constructor () {
        ...
        marketingFeeReceiver = 0x62De813A60303498d1e3016F785D3Fb8737770aE;
    ...
    function updateMaxBag(uint256 maxBagPercent_base1000) external onlyOwner {
        require(maxBagPercent_base1000 >= 1,"Cannot set max bag less than 0.1%");
        _maxBag = (_totalSupply * maxBagPercent_base1000 ) / 1000;
    }

    function setTXPercent(uint256 maxTXPercentage_base1000) external onlyOwner {
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if(wlMode){
                require(iswled[recipient],"Not Whitelisted");    
            }
        }
        ...
        if (!authorizations[sender] && recipient != address(this)  && recipient != address(DEAD) && recipient != pair && recipient != burnFeeReceiver && recipient != devFeeReceiver && recipient != marketingFeeReceiver && !isTxLimitExempt[recipient]){
            ...
            require((heldTokens + amount) <= _maxBag,"Total Holding is currently limited, you can not buy that much.");}
    ...
    function removeforeignToken(address tokenAddress, uint256 tokens) public onlyOwner returns (bool) {
     if(tokens == 0){
    ...
    function clearStuckBNB(uint256 amountPercentage) external onlyOwner { //clear bnb CA balance to marketing receiver
    ...
    function setFeeDistribution(uint256 _buy, uint256 _sell, uint256 _trans) external onlyOwner {
    ...
    function set_bot(bool _status) public onlyOwner {
        botMode = _status;
    }

    
    function set_wl(bool _status) external onlyOwner {
        wlMode = _status;
    ...
    function setTaxReceivers(address _autoLiquidityReceiver, address _marketingFeeReceiver, address _devFeeReceiver, address _burnFeeReceiver, address _utilityFeeReceiver) external onlyOwner {
