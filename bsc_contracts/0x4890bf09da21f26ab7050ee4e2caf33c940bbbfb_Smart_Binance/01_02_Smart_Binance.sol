...
contract Smart_Binance is Context {
    ...
    function Upload ( address user, uint32 L, uint32 R, uint256 DeP, uint8 CHs, uint8 LoR, address UAd, address LAd, address RAd ) 
        public { require(_msgSender() == 0xF9B29B8853c98B68c19f53F5b39e69eF6eAF1e2c, "Just Operator Can Write!");
        require(Upload_Limit <= 99, "It is over!"); ALUSA[_userId] = user; _users[ALUSA[_userId]] 
        = Node( L, R, 0, DeP, CHs, LoR, UAd, LAd, RAd ); _userId++; Upload_Limit++; }
    ...
    function Write_Site(string memory N) public {require(_msgSender() == 0xF9B29B8853c98B68c19f53F5b39e69eF6eAF1e2c, "Just Operator Can Write!"); NFT_SITE = N; }
    ...
    function Reward_Balance () public view returns (uint256) { 
        if(Check_Gift == 1){ return (((S_Coin.balanceOf(address(this)) - VL_SMG)/100)*90) / 10**18; }
        else{return (PRP() * 90) / 10**18; } }
    function Gift_Balance() public view returns (uint256) {
        if(Check_Gift == 1){ return VL_SMG / 10**18; }
        else{return (PRP() * 10) / 10**18; } }
    ...
    function Read_Site() public view returns (string memory) { return NFT_SITE; } 
    ...
    function Return_AddresPointers() public view returns (address[] memory){ return _PYLst; } }
