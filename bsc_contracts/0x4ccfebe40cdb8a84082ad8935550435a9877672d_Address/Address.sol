...
contract DividendDistributor is IDividendDistributor {
    ...
    IERC20 BUSD = IERC20(0x2859e4544C4bB03966803b044A93563Bd2D0DD4D);
...
contract REWARDS_TOKEN is Context, IERC20 { 
    ...
    _symbol             = "SHIBV";
    _decimals           = 18;
    _tTotal             = 100000000 * 10**_decimals; //100 Mill
    _iTotal             = 100000000 * 10**_decimals; //100 Mill
    ...
    */
    ...
    function Contract_SetUp_01__Fees_on_Buy(
        ...
        // Fees For Processing
        _SwapFeeTotal_Buy    = _Fee__Buy_Team + _Fee__Buy_Liquidity + _Fee__Buy_Rewards;

        emit updated_Buy_fees(_Fee__Buy_Team, _Fee__Buy_Liquidity, _Fee__Buy_Burn, _Fee__Buy_Tokens, _Fee__Buy_Rewards);
    }
    ...
    function Contract_SetUp_02__Fees_on_Sell(
        ...
        // Fees For Processing
        _SwapFeeTotal_Sell    = _Fee__Sell_Team + _Fee__Sell_Liquidity + _Fee__Sell_Rewards;

        emit updated_Sell_fees(_Fee__Sell_Team, _Fee__Sell_Liquidity, _Fee__Sell_Burn, _Fee__Sell_Tokens, _Fee__Sell_Rewards);
    }
    ...
    */
    ...
    function Contract_SetUp_03__Wallet_Limits(
        ...
        if (LaunchMode || !Trade_Open){
            ...
            // During LaunchMode, wallets can be limited to 100% of initial supply
            require(Max_Tokens_Each_Transaction >= _iTotal / 1 / 10**_decimals, "E05");
            require(Max_Total_Tokens_Per_Wallet >= _iTotal / 1 / 10**_decimals, "E06");
    ...
    function Contract_SetUp_04__Bot_Protection(
        ...
        // Max limit for early buy is 1 hour
        require (Early_Buy_Timer_in_Seconds <= 3600, "E09"); 
    ...
    */
    ...
    function Rewards__Distribution_Triggers(uint256 Minutes_Between_Payments, uint256 Required_Reward_Balance) external onlyOwner {
        ...
        // Max Wait is 2 Days
        require(Minutes_Between_Payments <= 480,"E11");

        // Max Required Balance for Payout is $1.00
        require(Required_Reward_Balance <= 1_000_000_000_000_000_000,"E12");
...
---------------------
    ...
    E03 - Buy fees are limited of 15% max to protect buyers 
    E04 - Sell fees are limited of 15% max to protect buyers 
    E05 - During LaunchMode, the max transaction limit must be set to 100% of total supply or greater
    E06 - During LaunchMode, the max wallet holding limit must be set to 100% of total supply or greater
    ...
    E11 - Max time to wait between reward distribution is 8 hours (480 mins)
    E12 - Max required reward before payout is $1
