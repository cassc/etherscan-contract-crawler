    ...
    IStakePool
{
    ...
    function cancel() external { 
        ...
        if(stakeModel.rewardType!=RewardType.NoReward && stakeModel.rewardType!=RewardType.NoRatio && totalSupply()>0){
            distributeRewards();
            require(block.timestamp==lastDistributeTime, "Reward distribution required");
            if(rewardToken!=address(0) && depositAmount>0){
                IERC20Upgradeable(rewardToken).safeTransfer(stakeOwner, depositAmount);
            }
        }
        ...
        external
    {
        ...
        status==StakeStatus.Alive)
            distributeRewards();
        IERC20Upgradeable(stakeToken).safeTransferFrom(_msgSender(), address(this), amount);
        _mint(_msgSender(), amount);
        StakeValidate.validateStake(stakeModel, status);
        ...
        external
    {
        ...
        IERC20Upgradeable(stakeToken).safeTransfer(_msgSender(), amount);
        _burn(_msgSender(), amount);
        StakeValidate.validateUnstake(stakeDateTime[_msgSender()], stakeModel, status);
        if(stakeModel.rewardType!=RewardType.NoReward && 
            (stakeModel.canClaimAnyTime || stakeModel.claimDateTime<=block.timestamp || status!=StakeStatus.Alive))
        {
    ...
    function distributeRewardDividends(uint256 amount, uint256 period) internal {        
        ...
        if (amount > 0) {
            ...
            lastDistributeTime=lastDistributeTime+period;
    ...
    function depositRewards(uint256 amount) external {
        ...
        if(stakeModel.rewardType==RewardType.NoRatio){
            distributeRewardDividends(newDepositAmount, block.timestamp-lastDistributeTime);  
    ...
    function distributeRewards() public returns (uint256){
        ...
        if(period>0){
            ...
            if(amount==0)
                return 0; 
            if(amount>depositAmount){
                period=period*depositAmount/amount;
