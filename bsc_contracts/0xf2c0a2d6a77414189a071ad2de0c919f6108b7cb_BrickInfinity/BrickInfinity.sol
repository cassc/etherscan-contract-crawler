...
contract BrickInfinity is BEP20, Ownable {
    address public constant marketingWallet =
        0xDECC5d248a07547F95b5adB395963bB2dcc84DEa;

    uint256 private immutable _numTokensSellToAddToBNB =
        500000 * (10 ** decimals());
    ...
    uint8 public marketingTaxPercentage = 5;
    ...
    event MarketingTaxPercentageChanged(uint8 oldTax, uint8 newTax);
     ...
     * construction.
     */
    constructor() BEP20("BrickInfinity", "Brick") {
    ...
    /**
     * @dev Changes `marketingTaxPercentage' by keeping threshold to 5%.
     */
    function changeMarketingTax(
        uint8 _marketingTaxPercentage
    ) external onlyOwner returns (bool) {
        require(
            _marketingTaxPercentage <= 5,
            "BEP20: marketing tax must not be greater than 5"
        );

        uint8 oldTax = marketingTaxPercentage;

        marketingTaxPercentage = _marketingTaxPercentage;
        emit MarketingTaxPercentageChanged(oldTax, marketingTaxPercentage);

        return true;
    }

    /**
     * @dev Swaps `tokenAmount' of tokens with BNBs and transfer those BNBs to 'marketingWallet'.
     */
    ...
    function _transfer(
        ...
        if (
            ...
            if (from != pancakeswapV2Pair) {
                ...
                if (contractTokenBalance >= _numTokensSellToAddToBNB) {
            ...
            if (
                !_liquidityAdded && to == pancakeswapV2Pair && from == owner()
            ) {
