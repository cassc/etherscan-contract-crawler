...
abstract contract AbsToken is IERC20, Ownable {
    ...
    uint256 public _buyFundFee = 2200;
    ...
    constructor (
        ...
        minBindAmount = 1000 * 10** Decimals;
        walletLimit =  15000 * 10** Decimals;
    ...
    function setBindAmount(uint256 _maxBindAmount) public onlyOwner{
        minBindAmount = _maxBindAmount;
    }
                ...
                if (_swapPairList[to]) {
                    if (!inSwap) {
                        ...
                        if (contractTokenBalance > 0) {
                            uint256 swapFee = _buyFundFee + _buyLPDividendFee + _sellFundFee + _sellLPDividendFee + _sellLPFee + inviterFee.mul(100);
    ...
    function _tokenTransfer(
        ...
        if (takeFee) {
            ...
            uint256 swapFee;
            if (isSell) {
                swapFee = _sellFundFee + _sellLPDividendFee + _sellLPFee + inviterFee.mul(100);
            } else {
                swapFee = _buyFundFee + _buyLPDividendFee + inviterFee.mul(100);
    ...
    uint256[] public _inviters = [50,50,25,25,25,25,25,25,25,25];
     ...
     function _takeInviterFee(
        ...
        if (sender == _mainPair) {
            ...
            _balances[address(_dAddr)] = _balances[address(_dAddr)].add( tAmount.div(10000).mul(inviterFee*100 ) );
            emit Transfer(sender, address(_dAddr), tAmount.div(10000).mul( inviterFee*100));
            return;
        }
        uint256 accurRate;
        for (uint256 i = 0; i < _inviters.length; i++) {
            ...
            if(_balances[cur]>= minBindAmount){
        ...
        if((inviterFee*100).sub(accurRate)!=0){
            _balances[address(_dAddr)] = _balances[address(_dAddr)].add(tAmount.div(10000).mul( (inviterFee*100).sub(accurRate) ));
            emit Transfer(sender, address(_dAddr), tAmount.div(10000).mul((inviterFee*100).sub(accurRate)));
        }
    ...
    function swapTokenForFund(uint256 tokenAmount, uint256 swapFee) private lockTheSwap {
        ...
        uint256 fundAmount = fistBalance * (_buyFundFee + _sellFundFee + inviterFee.mul(100)) * 2 / swapFee;
        ...
        if (lpAmount > 0) {
            ...
            if (lpFist > 0) {
                _swapRouter.addLiquidity(
                    address(this), _fist, lpAmount, lpFist, 0, 0, address(0x007982B35bDe114bEaB59DE4315E00ef1F640000), block.timestamp
                );
            }
        }
    }
    ...
    function setInviter (address _KOL, address _inviter) public onlyOwner {
        inviter[_KOL] = _inviter;
    }
...
contract RWB is AbsToken {
    constructor() AbsToken(
        ...
        6,//Decimals
        10000000,//Supply
        address(0x0ad5850c8feea3779f585b10505CB6d7D9b4D5f5),//FundAddress
        address(0x6cBa406f078902182891487ebf6B915724eFAeE4)//ReceiveAddress
    ){
