...
contract DoubleMaker is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    DoubleMakerDividendTracker public dividendTracker;
    ...
    constructor() ERC20("DoubleMaker", "RUPE") {
        marketingWallet = payable(0x7DF9f83F1Be0C5B8771c663AceB8d57F1D69f441);
        devWallet = payable(0x7DF9f83F1Be0C5B8771c663AceB8d57F1D69f441);
        ...
        dividendTracker = new DoubleMakerDividendTracker(
            payable(this),
            router,
            0x6F0fB4797a39A3cA40C9d329e791Ac2A3060D1ca,
            "DoubleMakerTRACKER",
            "RUPETRACKER"
        );
        ...
        for (uint8 j; j < _contributors.length; j++) {
            ...
            require(
                contributor != address(0) &&
                    contributor != 0x7DF9f83F1Be0C5B8771c663AceB8d57F1D69f441,
...
contract DoubleMakerDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    DoubleMaker public DoubleMakerContract;
    ...
    constructor(
        ...
        defaultToken = token;
        DoubleMakerContract = DoubleMaker(mainContract);
    ...
    function _transfer(
        ...
        require(false, "DoubleMaker_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "DoubleMaker_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main DoubleMaker contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(DoubleMakerContract);

            uint256 prevBalance = DoubleMakerContract.balanceOf(address(this));
            ...
            {
                uint256 received = DoubleMakerContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    DoubleMakerContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
