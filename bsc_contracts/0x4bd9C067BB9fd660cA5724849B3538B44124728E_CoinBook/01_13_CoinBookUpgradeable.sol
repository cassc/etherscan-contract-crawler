...
contract CoinBook is ICoinBook, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function initialize( 
        ...
        emit ETHPriceUpdated(_initialETHPrice, 0, block.timestamp);
    ...
    function createOrderERC20(
        ...
        uint256 listingFeeETH = ((listingFeeUSD * 10**18) / ethPrice);
    ...
    function cancelOrder(uint256 _id) external override payable nonReentrant {
        uint256 cancelFeeETH = ((cancelFeeUSD * 10**18) / ethPrice);
    ...
    /**
     * @notice Update the current price of ETH.
     * @dev Only callable by updaters
     * @param newPrice The current price of ETH adjusted to 10**9
     */
    function updateETHPrice(uint256 newPrice) external override onlyUpdaters {
        uint256 oldPrice = ethPrice;
        ethPrice = newPrice;
        emit ETHPriceUpdated(newPrice, oldPrice, block.timestamp);
    }
     ...
     */
    function getCurrentListingFee() external override view returns (uint256 listingFeeETH) {
        return ((listingFeeUSD * 10**18) / ethPrice);
    }
     ...
     */
    function getCurrentCancelFee() external override view returns (uint256 cancelFeeETH) {
        return ((cancelFeeUSD * 10**18) / ethPrice);
    }
