...
contract VoltichangeBSC is AccessControlUpgradeable {
    ...
    function swapTokenForToken(
        ...
        if (_tokenIn == VOLT) {
            ...
            _amountOutMin = getAmountOutMin(
                _firstFeeAmount,
                _tokenIn,
                IPancakeRouter02(PANCAKESWAP_V2_ROUTER).WETH()
            );
    ...
    function swapTokenForExactToken(
        ...
        if (_tokenIn == VOLT) {
            ...
            path = createPath(_tokenIn, IPancakeRouter02(PANCAKESWAP_V2_ROUTER).WETH());
            // console.log("starting second swap");
            IPancakeRouter02(PANCAKESWAP_V2_ROUTER)
                .swapExactTokensForETHSupportingFeeOnTransferTokens(
                    _firstFeeAmount,
        ...
        payable
    {
        ...
        if (_tokenOut == IPancakeRouter02(PANCAKESWAP_V2_ROUTER).WETH()) {
            ...
            address[] memory path;
            path = new address[](2);
            path[0] = IPancakeRouter02(PANCAKESWAP_V2_ROUTER).WETH();
            path[1] = _tokenOut;
    ...
    function getPair(address _tokenIn, address _tokenOut)
        ...
        returns (address)
    {
        return IPancakeFactory(PANCAKESWAP_FACTORY).getPair(_tokenIn, _tokenOut);
    }
        ...
        returns (address[] memory)
    {
        ...
        if (
            ...
            path = new address[](3);
            path[0] = _tokenIn;
            path[1] = IPancakeRouter02(PANCAKESWAP_V2_ROUTER).WETH();
            path[2] = _tokenOut;
        }
        return path;
    }
