...
contract vaultBtcbLiq is Ownable{
    ...
    address addrLiqAMT = 0x679Bd76cA0B3f037131AF9170d0462c9FfC9Bc27;
    ...
    constructor(uint256 startSnapshot_){ //to change on prod for static variables
        ...
        amt = AMT(addrAmt);
        liqAmt = LIQUIDITYAMT(addrLiqAMT);
    ...
    function addressAmt(address addr) public view returns (uint256){
        return (shares[addr]*liqAmt.balanceOf(address(this)))/totalShares;
    }
    ...
    function isContractSync() public view returns (bool){
        return (liqAmt.getCurrentSnapshotId()==lastSnapCharged);
    }

    function liqAmtStacked() public view returns(uint256){
        return liqAmt.balanceOf(address(this));
    }
    ...
    function chargeAll() public{
        ...
        uint256 currentSnapshot = liqAmt.getCurrentSnapshotId();
    ...
    function syncContract() public{
        uint256 actualSnapShot = liqAmt.getCurrentSnapshotId();
    ...
    function stake(uint256 amount) public{
        ...
        //Prev status variables
        uint256 totalLiqAmt = liqAmt.balanceOf(address(this));

        //Operations
        liqAmt.transferFrom(msg.sender,address(this),amount);
        ...
        if(totalShares == 0){
            ...
            newShares = (amount * totalShares)/totalLiqAmt; //sharePrice = totalAmt/totalShares
        }
        totalShares = totalShares + newShares;
        shares[msg.sender] = newShares;
        initialSnapShot[msg.sender] = liqAmt.getCurrentSnapshotId();
    ...
    function withdrwal() public {
        ...
        uint256 toSend = (removeShares*totalLiqAmt)/totalShares;
        ...
        liqAmt.transfer(msg.sender,toSend);
        if(amountBtc>0){
