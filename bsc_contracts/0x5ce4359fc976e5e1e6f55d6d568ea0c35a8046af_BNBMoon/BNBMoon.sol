...
contract BNBMoon is ERC20 {
	...
	address payable _fundAddress = payable(address(0x015CE41C8d52d6C0Db69a717A635182fc5A59cD4));
    ...
    constructor(address tokenOwner) ERC20("BNBMoon", "BNBMoon") {
	...
	function getBurnAmount(address user) public view returns(uint256){
		if(_isExcludedFromFees[user] || super.balanceOf(user) < 10**10){
			return 0;
		}
    ...
    function swapAndLiquify() public {
		...
		if(amountToken > 10**19){
			...
			if(allAmount > bnbAmount){
				_fundAddress.transfer((allAmount - bnbAmount).div(5));
			}
		}
