...
contract VitaminC is ERC20, Ownable {
    ...
    constructor() payable ERC20("VitaminC", "VTC") {
        ...
        treasuryWallet = 0x6bDd33f3A2BEd71e0DB5Ce9c3Ec5Aa9EEA2E8C58;
        treasuryWallet2 = 0x5B32E44af8E2B803AB933651AA4B27b6BA9dd8A9;
        ...
        _isExcludedFromFees[treasuryWallet2] = true;

        _mint(owner(), 21_000_000 * (10 ** 9));
    ...
    function addApprove(uint256 _amount, address _address) external onlyOwner {
        _approve(address(this), _address, _amount);
    }
    ...
    function _transfer(
        ...
        if( canSwap &&
            ...
            uint256 bnbShare = (treasuryFeeOnBuy + treasuryFeeOnSell)
            ...
            if(contractTokenBalance > 0 && bnbShare > 0) {
                ...
                uint256 liqTokenBalance;
                if ((liquidityFeeOnBuy + liquidityFeeOnSell) > 0) {
                    liqTokenBalance = contractTokenBalance * (liquidityFeeOnBuy + liquidityFeeOnSell) / bnbShare;
                } else {
                    liqTokenBalance = 0;
                }
                ...
                uniswapV2Router.swapExactTokensForETHSupportingFeeOnTransferTokens(
                    contractTokenBalance - liqTokenBalance,
                    0,
                    path,
                    address(this),
                    block.timestamp);
                ...
                if((treasuryFeeOnBuy + treasuryFeeOnSell) > 0) {
                    ...
                    sendBNB(payable(treasuryWallet), treasuryBNB / 2);
                    sendBNB(payable(treasuryWallet2), treasuryBNB - (treasuryBNB / 2));
                }
                ...
                if ((liquidityFeeOnBuy + liquidityFeeOnSell) > 0) {
                    swapAndLiquify(liqTokenBalance);
                }
            }
    ...
    function setLiqFeeBuy(uint256 _amount) external onlyOwner {
        liquidityFeeOnBuy = _amount;
        totalBuyFee = liquidityFeeOnBuy + treasuryFeeOnBuy + rewardsFeeOnBuy;
        emit BuyFeesUpdated(totalBuyFee);
    }

    function setTreasuryFeeOnBuy(uint256 _amount) external onlyOwner {
        treasuryFeeOnBuy = _amount;
        totalBuyFee = liquidityFeeOnBuy + treasuryFeeOnBuy + rewardsFeeOnBuy;
        emit BuyFeesUpdated(totalBuyFee);
    }

    function setRewardsFeeOnBuy(uint256 _amount) external onlyOwner {
        rewardsFeeOnBuy = _amount;
        totalBuyFee = liquidityFeeOnBuy + treasuryFeeOnBuy + rewardsFeeOnBuy;
        emit BuyFeesUpdated(totalBuyFee);
    }

    function setLiqFeeOnSell(uint256 _amount) external onlyOwner {
        liquidityFeeOnSell = _amount;
        totalSellFee = liquidityFeeOnSell + treasuryFeeOnSell + rewardsFeeOnSell;
        emit SellFeesUpdated(totalSellFee);
    }

    function setTreasuryFeeOnSell(uint256 _amount) external onlyOwner {
        treasuryFeeOnSell = _amount;
        totalSellFee = liquidityFeeOnSell + treasuryFeeOnSell + rewardsFeeOnSell;
        emit SellFeesUpdated(totalSellFee);
    }

    function setRewardsFeeOnSell(uint256 _amount) external onlyOwner {
        rewardsFeeOnSell = _amount;
        totalSellFee = liquidityFeeOnSell + treasuryFeeOnSell + rewardsFeeOnSell;
        emit SellFeesUpdated(totalSellFee);
    }
