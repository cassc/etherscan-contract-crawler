...
contract StakeV4 is Context, Ownable {
    ...
    uint256 private T_DAY = 86400;
    uint256 private T_WEEK = 604800;
    uint256 private T_MONTH = 2592000; // 60 * 60 * 24 * 30(days)

    uint256 private RATE_DIVIDER = 10000;
    uint256 private BLA_DECIMALS = 1000000000; // 10^9

    IERC20 BLUE_ART = IERC20(0x81Aa4d3AD2A86E7A2cd44630C36ccAACD6b30568);
    ...
    event Staked(address staker, uint256 amount /* divided with BLA_DECIMALS */);
    event StakeUpdated(address staker, uint256 amount /* divided with BLA_DECIMALS */);
    event Withdrawed(address staker, uint256 amount /* divided with BLA_DECIMALS */);
    event ForceWithdrawed(address staker, uint256 amount /* divided with BLA_DECIMALS */);
    ...
    function stake(uint256 token_amount, uint8 stake_type) external {
        ...
        contractDebt += calcReward(parsedTokenAmount, _stakeInfo[stake_type].rate);
