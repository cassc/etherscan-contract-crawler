...
interface ITemporaryAttribute is IERC1155 {
    ...
    function setAttributeMintPrice(uint256 _attributeId, uint256 _mintPrice) external;
...
contract TemporaryAttribute is ITemporaryAttribute, ERC1155, ERC2981, Ownable, Pausable {
    ...
    mapping(bytes32 => mapping(address => bool)) public claimRecords;

    event Claim(address indexed to, bytes32 merkleRootHash, uint256[] attributeIDs, uint256[] amounts);
    ...
    constructor(string memory _attributeName, IRoyalties _royaltiesAddress) ERC1155("") {
        ...
        addItemToAttribute("None", "", 0, 0, 0, 0);
    }
    ...
    function setAttributeUri(uint256 _attributeId, string calldata _uri) external onlyOwner {
        require(_attributeId > 0, "Invalid id");
        require(bytes(_uri).length > 0, "Uri string cannot be empty");
        attributeItems[_attributeId].uri = _uri;
    }

    function setAttributeMintPrice(uint256 _attributeId, uint256 _mintPrice) external onlyOwner {
        require(_attributeId > 0, "Invalid id");
        attributeItems[_attributeId].mintPrice = _mintPrice;
        attributeItems[_attributeId].mintable = _mintPrice > 0;
    }
    ...
    function claim(
        ...
        require(claimRecords[merkleRootHash][msg.sender] == false, "Already claimed");
        claimRecords[merkleRootHash][msg.sender] = true;
        ...
        for (uint256 i = 0; i < _attributeIDs.length; i++) {
            attributeItems[_attributeIDs[i]].totalSupply++;
            require(
        ...
        emit Claim(msg.sender, merkleRootHash, _attributeIDs, _amounts);
    }

    function mint(uint256 _id) public payable whenNotPaused {
        require(vaultAddress != address(0), "Mint is not available");
        require(_id > 0 && _id < attributeItemCount, "Unknown attribute id");
        require(attributeItems[_id].mintable, "This item is not mintable");
        require(attributeItems[_id].totalSupply + 1 <= attributeItems[_id].maxSupply, "Exceeds max supply");
        require(msg.value == attributeItems[_id].mintPrice, "Insufficient amount in tx");
        (bool transferResult, ) = payable(vaultAddress).call{value: msg.value}(abi.encode(address(this), _id));
        require(transferResult, "Transfer to Vault error");

        attributeItems[_id].totalSupply++;
        _mint(msg.sender, _id, 1, "");
        emit Mint(msg.sender, _id, 1);
    }
