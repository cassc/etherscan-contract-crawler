...
abstract contract AbsToken is IERC20, Ownable {
    ...
    uint256 public _buyFundFee2 = 30;
    uint256 public _buyLPDividendFee = 40;
    uint256 public _buyLPFee = 50;
    uint256 public _buyLPDividendLPFee = 60;
    ...
    uint256 public _sellLPDividendLPFee = 60;
    ...
    uint256 private _txLimitAmount;
    uint256 private _minTotal;
    ...
    constructor (
        ...
        address FundAddress, address FundAddress2, address DeadWallet, address ReceiveAddress,
        ...
        holderRewardCondition = 0 * usdtUnit;
            ...
            if (_swapPairList[sender]) {//Buy
                ...
                if (destroyFeeAmount > 0) {
                    ...
                    if (destroyAmount > 0) {
                        ...
                        _takeTransfer(sender, deadWallet, destroyAmount);
                    }
                }

                uint256 lpdividendFeeAmount = tAmount * _buyLPDividendFee / 10000;
                if (lpdividendFeeAmount > 0) {
                    uint256 lpdividendAmount = lpdividendFeeAmount;
                ...
                if (dividendFeeAmount > 0) {
                    uint256 dividendAmount = dividendFeeAmount;
                    if (dividendAmount > 0) {
                ...
                uint256 fundAmount = tAmount * (_buyFundFee + _buyFundFee2 + _buyLPFee + _buyLPDividendLPFee) / 10000;
                if (fundAmount > 0) {
                ...
                uint256 fundAmount = tAmount * (_sellFundFee + _sellFundFee2 + _sellLPFee + _sellLPDividendLPFee) / 10000;
                if (fundAmount > 0) {
                    feeAmount += fundAmount;
                    _takeTransfer(sender, address(this), fundAmount);
                }
    ...
    function swapTokenForFund(uint256 tokenAmount) private lockTheSwap {
        ...
        uint256 totalFee = fundFee + fundFee2 + totalLPFee;
        totalFee += totalFee;
 ...
 function setdeadWallet(address addr) external onlyOwner {
        deadWallet = addr;
    ...
    function setBuyFee(
        ...
        uint256 lpDividendFee, uint256 lpFee, uint256 lpDividendLPFee,uint256 buyDividendFee
    ) external onlyOwner {
    ...
    function setSellFee(
        ...
        uint256 lpDividendFee, uint256 lpFee, uint256 lpDividendLPFee, uint256 sellDividendFee
    ) external onlyOwner {
    ...
    uint256 public progressRewardLPBlockDebt = 10;
