...
library poolUtil {
    ...
    function calcUnits(stData storage _self, stHolders storage _holders, address _user, bool _liquidate) internal view returns (uint _units) {        
        ...
        if (!_liquidate) {            
            ...
            for (uint d = 0; d < _holders.transactionLog.length; d++) {     
                transHolders memory _tmp = _holders.transactionLog[d];

                if (_tmp.timestamp == 0) continue;     

                if (_tmp.deposit) {
                    uint _dTime = _tmp.timestamp - _self.lastProcess; //difference between deposit and last harvest user NOT in pool
                    uint _tAmt;
                    uint _cAmt;
                    ...
                    if (_dTime > 0) {
                        ...
                        _tAmt = _tmp.amount * (_tmpTime>0?_tmpTime:1); // amount to deduct from deposit amount 
                        _pt += _tAmt;

                        if (_tmp.account == _user) {
                            _cAmt = (_tmp.amount * _time) - _tAmt; // amount to deduct from total amount
                            if (_amt > 0)
                                _amt -= _cAmt;
                            else
                                _amt += _tAmt; //first deposit
                        }
                    }    
                    else {
                        _pt += _amt;
                    ...
                    uint _wTime = block.timestamp - _tmp.timestamp; //Time the user was not in the pool
                    uint _cTime = _tmp.timestamp - _self.lastProcess; //Time the user was in the pool

                    _pt -= _tmp.amount * _wTime; // remove amount when not in pool.
                    
                    if (_tmp.account == _user) 
                        if (_amt > 0)
                            _amt -= _tmp.amount * _wTime;
                        else
                            _amt += _tmp.amount * (_cTime>0?_cTime:1);
                }
            }
        }
        else {
