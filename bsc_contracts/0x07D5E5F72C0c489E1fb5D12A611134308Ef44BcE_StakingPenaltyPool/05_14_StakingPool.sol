...
contract StakingPool is WhitelistUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function pendingReward(address _user) external view returns (uint256) {
        ...
        uint256 pending = user.amount.mul(tempAccTokensPerShare).div(1e18).sub(user.rewardDebt);
        uint256 total = pending.add(user.rewardHeld);
    ...
    function reinvestTokens() external nonReentrant onlyWhitelisted {
        innerStakeTokens(0, msg.sender, true);
    }
    ...
    function stakeTokens(uint256 _amountToStake) public virtual nonReentrant onlyWhitelisted {
        ...
        innerStakeTokens(_amountToStake.sub(feeAmount), msg.sender, false);
    }

    function stakeFromLocker(uint256 _amountToStake) public virtual nonReentrant onlyWhitelisted {
        innerStakeTokens(_amountToStake, tx.origin, false);
    }
    ...
    function innerStakeTokens(uint256 _amountToStake, address _sender, bool reinvest) private {
        updatePool();
        uint256 pending = 0;
        ...
        if (user.amount > 0) {
            pending = user.amount.mul(accTokensPerShare).div(1e18).sub(user.rewardDebt);

            if (pending > 0) {
                if (!reinvest) {
                    user.rewardHeld = user.rewardHeld.add(pending);
                }
                else {
        ...
        emit TokensStaked(_sender, _amountToStake, pending, reinvest);
    }
    ...
    function withdrawStake(uint256 _amount) public virtual nonReentrant {
        ...
        if (total > 0) {
            if (now > finishTime) {
                ...
                if (_amount == user.amount) {
                    rewardToken.safeTransfer(feeTo, total);
                    user.rewardHeld = 0;
                }
                else {
                    user.rewardHeld = user.rewardHeld.add(pending);
        ...
        emit StakeWithdrawn(msg.sender, _amount, total);
    }
    ...
    function emergencyWithdraw() public virtual nonReentrant {
        ...
        if (user.rewardHeld > 0) {
            rewardToken.safeTransfer(feeTo, user.rewardHeld);
            user.rewardHeld = 0;
        }
