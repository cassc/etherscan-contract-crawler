...
contract PX{
    ...
    uint256 ACTIVATION_TIME = 	1672680625;
    ...
    constructor() public {
        maintenanceAddress = address(0x000000000000000000000000000000000000dEaD);
        erc20 = TOKEN(address(0x2AB0e9e4eE70FFf1fB9D67031E44F6410170d00e));
    }

    function checkAndTransferPX(uint256 _amount) private {
    ...
    function sell(uint256 _amountOfTokens) onlyTokenHolders public {
        ...
        uint256 _taxedPX= SafeMath.sub(_amountOfTokens, _undividedDividends);
        ...
        int256 _updatedPayouts = (int256) (profitPerShare_ * _amountOfTokens + (_taxedPX* magnitude));
        ...
        emit onTokenSell(_customerAddress, _amountOfTokens, _taxedPX, now);
    }
    ...
    function totalPXBalance() public view returns (uint256) {
    ...
    function sellPrice() public view returns (uint256) {
        uint256 _PX= 1e18;
        uint256 _dividends = SafeMath.div(SafeMath.mul(_PX, exitFee_), 100);
        uint256 _taxedPX= SafeMath.sub(_PX, _dividends);
    ...
    function buyPrice() public view returns (uint256) {
        uint256 _PX= 1e18;
        uint256 _dividends = SafeMath.div(SafeMath.mul(_PX, entryFee_), 100);
        uint256 _taxedPX= SafeMath.add(_PX, _dividends);

        return _taxedPX;
    }

    function calculateTokensReceived(uint256 _PXToSpend) public view returns (uint256) {
        uint256 _dividends = SafeMath.div(SafeMath.mul(_PXToSpend, entryFee_), 100);
        uint256 _amountOfTokens = SafeMath.sub(_PXToSpend, _dividends);

        return _amountOfTokens;
    }

    function calculatePXReceived(uint256 _tokensToSell) public view returns (uint256) {
        require(_tokensToSell <= tokenSupply_);
        uint256 _dividends = SafeMath.div(SafeMath.mul(_tokensToSell, exitFee_), 100);
        uint256 _taxedPX= SafeMath.sub(_tokensToSell, _dividends);
    ...
    function purchaseTokens(address _referredBy, address _customerAddress, uint256 _incomingPX) internal isActivated returns (uint256) {
        if (getInvested() == 0) {
          totalHolder++;
        }

        invested_[msg.sender] += _incomingPX;

        uint256 _undividedDividends = SafeMath.div(SafeMath.mul(_incomingPX, entryFee_), 100);
        ...
        uint256 _amountOfTokens = SafeMath.sub(_incomingPX, _undividedDividends);
        ...
        emit onTokenPurchase(_customerAddress, _incomingPX, _amountOfTokens, _referredBy, now);
