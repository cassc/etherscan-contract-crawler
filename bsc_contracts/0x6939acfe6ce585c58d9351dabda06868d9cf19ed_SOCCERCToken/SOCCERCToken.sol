...
contract SOCCERCToken is ERC20, Ownable {
    using SafeMath for uint256;

    address public rewardToken = 0x55d398326f99059fF775485246999027B3197955;
    address public deadWallet = 0x000000000000000000000000000000000000dEaD;
    address public marketingWallet = 0xca3012A020Daf567046200D1cb070F2e77853e41;
    ...
    uint256 public minBalanceSwapToken = 3 * 10**5 * 10**18;

    bool swapIng;

    constructor() ERC20("SOCCER CLUB", "SOCCERC") {
        _excludedFees[_msgSender()] = true;
        _excludedFees[address(this)] = true;
        _excludedFees[marketingWallet] = true;
    ...
    function setSellFees(uint256 fee, uint256 deadfee) external onlyOwner {
        sellFee = fee;
        sellDeadFee = deadfee;
        require(fee + deadfee <= 10, "charge cannot exceed 20%");
    }
        ...
        if (!_excludedFees[from] && !_excludedFees[to]) {
            ...
            if (to == uniswapPair) {
                ...
                if (!swapIng && contractBalance > minBalanceSwapToken) {
                    swapIng = true;
                    swapTokenToUSD(minBalanceSwapToken);
                    swapIng = false;
                }

                if (sellFee > 0) {
                    uint256 _sellFee = amount.mul(sellFee).div(100);
                    super._transfer(from, address(this), _sellFee);
                    fees += _sellFee;
                }

                if (sellDeadFee > 0) {
                    uint256 _selldeadFee = amount.mul(sellDeadFee).div(100);
                    super._transfer(from, deadWallet, _selldeadFee);
                    fees += _selldeadFee;
                }
            }
            if (fees > 0) {
    ...
    function swapTokenToUSD(uint256 tokenAmountIn) private {
        IERC20 token = IERC20(rewardToken);
        _approve(address(this), address(uniswapRouter), tokenAmountIn);
