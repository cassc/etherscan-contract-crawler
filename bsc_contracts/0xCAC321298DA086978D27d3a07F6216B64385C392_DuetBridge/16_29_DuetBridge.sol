...
contract DuetBridge is ReentrancyGuardUpgradeable, PausableUpgradeable, Adminable, IMessageReceiverApp {
    ...
    function _transferXForUser(
        ...
        _transferFrom(user_, originalToken_, amount_);

        bytes memory message = abi.encode(user_, action_);
        require(msg.value >= _calcUserFee(message), "DuetBridge: insufficient fee transferred");
        ...
        emit TransferX(user_, originalToken_, amount_, action_, destChainId_, msg.value, fees);
    }
    ...
    function getTransferXFee() public view returns (uint256) {
        return _calcUserFee(abi.encodePacked(address(this), ACTION_TRANSFER_X));
    }
