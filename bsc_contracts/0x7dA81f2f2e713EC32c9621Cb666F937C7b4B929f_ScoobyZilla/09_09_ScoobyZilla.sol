...
contract ScoobyZilla is Context, IERC20, Ownable {
    ...
    uint256 private _tTotal = 1_000_000_000_000 * 10**18; // 1 trillion
    ...
    uint256 public _maxTxAmount = 30_000_000_000 * 10**18; // 3% max transaction size
    uint256 public _maxWalletSize = 50_000_000_000 * 10**18; // 5% max wallet size
    uint256 private numTokensSellToAddToLiquidity = 1_000_000_000 * 10**18; // 0.1%
    ...
    function _transfer(
        ...
        if (
            from != owner() &&
            to != owner() &&
            !_isExcludedFromMaxTxAmount[from]
        )
            require(
                amount <= _maxTxAmount,
