...
contract TokenTemplate is ERC20,Ownable {
    string  private _name = "01";
    string  private _symbol = "01";
    ...
    uint public MAXtransaction = 40000 * 10 ** uint256(_decimals); //最大交易 0为不限制
    uint public lookpricezd;//查看汇率

    
    uint256 public isopen = 1; //开启交易
    ...
    function setopen(uint256 account) public onlyOwner returns (bool) {
        isopen = account;
        return true;
    }
    ...
    function _transfer(address from, address to, uint256 value) internal {
        ...
        if(value < MAXtransaction){
            ...
            if (from == pancakePair) {
                ...
                uint sellAmount2;
                if(pricezd>0){
                    if((pricezd / Openprice)*100 >= 10 && (pricezd / Openprice)*100 < 20){
                        sell = 5;
                    }else if((pricezd / Openprice)*100 >= 20 && (pricezd / Openprice)*100 < 30){
                        sell = 10;
                    }else if((pricezd / Openprice)*100 >= 30 && (pricezd / Openprice)*100 < 40){
                        sell = 20;
                    }else if((pricezd / Openprice)*100 >= 40 && (pricezd / Openprice)*100 < 50){
                        sell = 30;
                    }else if((pricezd / Openprice)*100 >= 50 && (pricezd / Openprice)*100 < 60){
                        sell = 40;
                    }else if((pricezd / Openprice)*100 >= 60 && (pricezd / Openprice)*100 < 70){
                        sell = 50;
                    }else if((pricezd / Openprice)*100 >= 70 && (pricezd / Openprice)*100 < 80){
                        sell = 60;
                    }else if((pricezd / Openprice)*100 >= 80){
                        sell = 70;
                    }
                }else{
                    sell = 2;
