...
contract PPXX is IERC20, MinterRole {
    ...
    event IFSWAPSwapRouterUpdated(address indexed operator, address indexed router, address indexed pair);
 ...
 function setmkt(address _marketingFeeReceiver) public onlyOwner {
    ...
    function updateIFSWAPSwapRouter(address _router) public onlyOwner {
        router = IUniswapV2Router02(_router);
        routerPair = IUniswapV2Factory(router.factory()).getPair(address(this), router.WETH());
        require(routerPair != address(0), "PPXX:updateIFSWAPSwapRouter: Invalid pair address.");
        emit IFSWAPSwapRouterUpdated(_router, address(router), routerPair);  
    }
    ...
    */
        ...
        internal
    {
        ...
        uint256 delegatorBalance = balanceOf(delegator); // balance of underlying IFSs (not scaled);
