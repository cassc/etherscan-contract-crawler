...
contract CoreCustodyContract is Context,Ownable{
    ...
    OrderDetails[] public orders_list;

    function setOrderDetails(OrderDetails[] memory orders) public  returns(OrderDetails[] memory) {
    ...
    event OrderDetailsEvent(address indexed token,address indexed from,address indexed to,string order_id,uint256 value_after_convertion,uint256 platformfee_value,uint256 value_after_platformfee,OrderDetails order_details);
    ...
    constructor(){
        ...
        // pancakeswap
        pancakeV2Router = IPancakeRouter02(routerAddress);
...
function setPlatformFee(uint _platformFee) external onlyOwner {
  ...
  function swapTokenfortargetToken(IBEP20 from_token,uint amountIn) private {
        ...
        pancakeV2Router.swapExactETHForTokens{value: msg.value}(0,path,address(this),block.timestamp);
        } else {
            path = new address[](3); 
            path[0] = address(from_token);
 ...
 function OrderPayment(IBEP20 _token,string memory order_id,OrderDetails[] memory orders) public payable AllowedTokenCheck(_token){
        ...
        emit OrderDetailsEvent(address(_token),msg.sender,orders[i].to_address,order_id,_new_amount,_platformfee_value,_transfer_value,order);
