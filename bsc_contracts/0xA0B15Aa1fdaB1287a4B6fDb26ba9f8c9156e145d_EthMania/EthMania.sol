...
contract EthMania is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    EthManiaDividendTracker public dividendTracker;

    address constant public deadWallet = 0x000000000000000000000000000000000000dEaD;

    address public immutable ETH = address(0x2170Ed0880ac9A755fd29B2688956BD959F933F8); //ETH
    
    uint256 public swapTokensAtAmount = 50 * (10**6) * (10**18); 
    uint256 public maxTransactionAmount = 2 * (10**9) * (10**18); //2%

    uint256 public _BuyETHRewardsFee = 6;
    uint256 public _BuyLiquidityFee = 1;
    uint256 public _BuyMarketingFee = 2;
    uint256 public _BuyBackEthManiaFee = 1;
  
    uint256 public _BuyTotalFees = _BuyETHRewardsFee.add(_BuyLiquidityFee).add(_BuyMarketingFee).add(_BuyBackEthManiaFee);
    
    uint256 public _SellETHRewardsFee = 10;
    uint256 public _SellLiquidityFee = 1;
    uint256 public _SellMarketingFee = 2;
    uint256 public _SellBackEthManiaFee = 1;
    uint256 public _SellTotalFees = _SellETHRewardsFee.add(_SellLiquidityFee).add(_SellMarketingFee).add(_SellBackEthManiaFee);
    
    uint256 public maxWalletToken = 2 * (10**9) * (10**18); //2%

    address public _marketingWalletAddress = 0xf793029722a49288B10deCF239E1A0C007c4e7A1;
    address public _EthManiaWalletAddress = 0xEBE887508601aCE6f57cfb63e8118e74924a7f1b;
    ...
    constructor() ERC20("EtherMania", "EthMania") {

    	dividendTracker = new EthManiaDividendTracker();
        ...
        dividendTracker.excludeFromDividends(_EthManiaWalletAddress);
        ...
        excludeOrIncludeFromFees(_EthManiaWalletAddress, true);
        ...
        excludeOrIncludeFromMaxTx(_EthManiaWalletAddress, true);
        ...
        excludeOrIncludeFromMaxWallet(_EthManiaWalletAddress, true);
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "EthMania: The dividend tracker already has that address");

        EthManiaDividendTracker newDividendTracker = EthManiaDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "EthMania: The new dividend tracker must be owned by the EthMania token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "EthMania: The router already has that address");
    ...
    function excludeOrIncludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "EthMania: Account is already the value of 'excluded'");
    ...
    {
        require(_isExcludedFromMaxTx[account] != excluded, "EthMania: Account is already the value of 'excluded'");
    ...
    {
        require(_isExcludedFromMaxWallet[account] != excluded, "EthMania: Account is already the value of 'excluded'");
    ...
    function setEthManiaWallet(address payable wallet) external onlyOwner{
         require(wallet != address(0), "Invalid address");
        _EthManiaWalletAddress = wallet;
    }

      function setBuyFees(uint256 buyEthRewardFee, uint256 buyLiquidityFee, uint256 buyMarketingFee, uint256 buyBackEthManiaFee) external onlyOwner
    {
        _BuyETHRewardsFee = buyEthRewardFee;
        _BuyLiquidityFee = buyLiquidityFee;
        _BuyMarketingFee = buyMarketingFee;
        _BuyBackEthManiaFee = buyBackEthManiaFee;
        _BuyTotalFees = _BuyETHRewardsFee.add(_BuyLiquidityFee).add(_BuyMarketingFee).add(_BuyBackEthManiaFee);

    }

    function setSellFees(uint256 sellEthRewardFee, uint256 sellLiquidityFee, uint256 sellMarketingFee, uint256 sellBackEthManiaFee) external onlyOwner
    {
        _SellETHRewardsFee = sellEthRewardFee;
        _SellLiquidityFee = sellLiquidityFee;
        _SellMarketingFee = sellMarketingFee;
        _SellBackEthManiaFee = sellBackEthManiaFee;
        _SellTotalFees = _SellETHRewardsFee.add(_SellLiquidityFee).add(_SellMarketingFee).add(_SellBackEthManiaFee);

    }
      function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "EthMania: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 800000, "EthMania: gasForProcessing must be between 200,000 and 800,000");
        require(newValue != gasForProcessing, "EthMania: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        
        {
            ...
            uint256 walletTokens = contractTokenBalance.mul(_SellMarketingFee.add(_SellBackEthManiaFee)).div(_SellTotalFees);
            uint256 contractBalance = address(this).balance;
            swapTokensForEth(walletTokens);
            uint256 newBalance = address(this).balance.sub(contractBalance);
            uint256 marketingShare = newBalance.mul(_SellMarketingFee).div(_SellBackEthManiaFee.add(_SellMarketingFee));
            uint256 developmentShare = newBalance.sub(marketingShare);
            payable(_marketingWalletAddress).transfer(marketingShare);
            payable(_EthManiaWalletAddress).transfer(developmentShare);
...
contract EthManiaDividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("EthMania_Dividend_Tracker", "EthMania_Dividend_Tracker") {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 10000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "EthMania_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "EthMania_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main EthMania contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "EthMania_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "EthMania_Dividend_Tracker: Cannot update claimWait to same value");
