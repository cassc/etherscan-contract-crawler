...
contract Billions_BNB {
   ...
   { 
       require(block.timestamp>= users[msg.sender].stake[_index].lockDate);
    ...
    function removePoolStake() external
   {
    ...
    function claimPoolReward() public{
        ...
        (uint rewardAmount,) = getUserPoolAmount(msg.sender);
   ...
   function getUserDividend(address userAddress,uint i) public view returns (uint256) {
       ...
       uint256 total_TokenSupply = totalTokenSupply();
            if (user.stake[i].lockDate >=block.timestamp){
                uint day = getCurDay(user.stake[i].startTime);
                dividends = (user.stake[i].amount.mul(user.stake[i].dailyreward).div(1000));
                dividends = dividends.mul(day);    
            }else if(user.stake[i].endTime ==0){
                dividends = user.stake[i].amount.mul(150).div(100);
            }else{
                dividends =0;
            }
