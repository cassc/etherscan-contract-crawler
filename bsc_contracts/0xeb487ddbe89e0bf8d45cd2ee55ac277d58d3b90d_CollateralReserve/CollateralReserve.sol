...
interface ITreasury {
    ...
    function reserveReceiveCollaterals(
        uint256 _mainCollateralAmount,
        uint256 _secondCollateralAmount
    ) external;
...
contract CollateralReserve is OwnableUpgradeable, ICollateralReserve {
    ...
    event MinimumAddedFarmingUpdated(
        uint256 newMinimumAddedFarmingMainCollateral,
        uint256 newMinimumAddedFarmingSecondCollateral
    );
    event AddLiquidityAndFarm(
        uint256 mainCollateralAmount,
        uint256 secondCollateralAmount,
        uint256 wantAmount
    );
    ...
    function fundBalance(
        address _token
    ) external view override returns (uint256 _bal) {
    ...
    function collateralFarmingBalances()
        public
        view
        returns (uint256 _mainCollateralBal, uint256 _secondCollateralBal)
    {
        if (strategy != address(0)) {
            ...
            _mainCollateralBal =
                (IERC20(mainCollateral).balanceOf(_want) * _lpFarmingBal) /
                _lpSupply;
            _secondCollateralBal =
                (IERC20(secondCollateral).balanceOf(_want) * _lpFarmingBal) /
                _lpSupply;
        }
    }
    ...
    function setStrategy(address _strategy) public onlyOwner {
        ...
        if (
            address(_currentStrategy) != address(0) &&
            _currentStrategy.totalBalance() > 0
        ) {
            _currentStrategy.withdrawAll();
        }
        require(
            _strategy == address(0) || IStrategy(_strategy).want() == want,
            "!want"
        );
        strategy = _strategy;
    ...
    function setMinimumAddedFarmingConfig(
        uint256 _minimumAddedFarmingMainCollateral,
        uint256 _minimumAddedFarmingSecondCollateral
    ) public onlyOwner {
        ...
        emit MinimumAddedFarmingUpdated(
            _minimumAddedFarmingMainCollateral,
            _minimumAddedFarmingSecondCollateral
        );
    }

    function requestAddLiquidityAndFarm(
        uint256 _mainCollateralAmount,
        uint256 _secondCollateralAmount
    ) external onlyOwner {
    ...
    function requestWithdrawAndRemoveLiquidity(
        uint256 _wantAmount
    ) external onlyOwner {
    ...
    function transferTo(
        ...
        if (_tokenBal < _amount) {
            uint256 _neededLiqAmount = _calculateNeededLiquidityToRemove(
                _token,
                want,
                _amount + 1 - _tokenBal
            );
    ...
    function receiveCollaterals(
        ...
        if (strategy != address(0)) {
            uint256 _reserve_farming_percent = ITreasury(treasury)
                .reserve_farming_percent();
            if (_reserve_farming_percent > 0) {
                (uint256 _fBalA, uint256 _fBalB) = collateralFarmingBalances();
                uint256 _iBalA = IERC20(mainCollateral).balanceOf(
                    address(this)
                );
                uint256 _iBalB = IERC20(secondCollateral).balanceOf(
                    address(this)
                );
                uint256 _targetBalA = ((_fBalA + _iBalA) *
                    _reserve_farming_percent) / 10000;
                uint256 _targetBalB = ((_fBalB + _iBalB) *
                    _reserve_farming_percent) / 10000;
                if (
                    _fBalA + minimumAddedFarmingMainCollateral < _targetBalA &&
                    _fBalB + minimumAddedFarmingSecondCollateral < _targetBalB
                ) {
                    _addLiquidityAndFarm(
                        _targetBalA - _fBalA,
                        _targetBalB - _fBalB
                    );
                }
    ...
    function _addLiquidityAndFarm(
        ...
        emit AddLiquidityAndFarm(
            _mainCollateralAmount,
            _secondCollateralAmount,
            _idleWantBal
        );
    }
    ...
    function _withdrawAndRemoveLiquidity(uint256 _wantAmount) internal {
        ...
        _strategy.withdraw(
            _wantAmount <= _inFarmBalance ? _wantAmount : _inFarmBalance
        );
    ...
    function _calculateNeededLiquidityToRemove(address _token, address _pair, uint256 _neededTokenAmount) private view returns (uint256 _neededAmount) {
        _neededAmount = (IERC20(_pair).totalSupply() * _neededTokenAmount / IERC20(_token).balanceOf(_token));
        _neededAmount += _neededAmount / 200; // for 0.5% error
    }

    function _approveTokenIfNeeded(address _token, address _spender) private {
        if (
            IERC20(_token).allowance(address(this), _spender) <
            type(uint256).max >> 1
        ) {
