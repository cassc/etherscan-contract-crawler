...
contract ConstructorFacet is Ownable {
    ...
    function constructorHandler(ConstructorParams calldata params, address _factory) external {
        ...
        s.lossless = ILosslessController(IMintFactory(_factory).getLosslessController());
        s._isExcluded[address(s.lossless)] = true;
        s._excluded.push(address(s.lossless));
        emit ExcludedAccount(address(s.lossless));
        // Tax Settings
...
contract TaxFacet is Ownable {
    ...
    function _burn(address account, uint256 amount) internal virtual {
        ...
        if (s.isLosslessOn) {
            s.lossless.beforeBurn(account, amount);
        } 
...
contract TaxToken is Ownable{
    ...
    function transferOutBlacklistedFunds(address[] calldata from) external {
        ...
        require(_msgSender() == address(s.lossless), "LOL");
        for (uint i = 0; i < from.length; i++) {
            _transfer(from[i], address(s.lossless), balanceOf(from[i]));
        }
    }
    ...
    function transfer(address recipient, uint256 amount) public returns (bool) {
        ...
        if (s.isLosslessOn) {
            s.lossless.beforeTransfer(_msgSender(), recipient, amount);
        } 
        _transfer(_msgSender(), recipient, amount);
        s.isTaxed = false;
        if (s.isLosslessOn) {
            s.lossless.afterTransfer(_msgSender(), recipient, amount);
        } 
        return true;
    }
    ...
    function transferFrom(address sender, address recipient, uint256 amount) public returns (bool) {
        ...
        if (s.isLosslessOn) {
            s.lossless.beforeTransferFrom(_msgSender(), sender, recipient, amount);
        }
        _transfer(sender, recipient, amount);
    ...
    function increaseAllowance(address spender, uint256 addedValue) public virtual returns (bool) {
        if (s.isLosslessOn) {
            s.lossless.beforeIncreaseAllowance(_msgSender(), spender, addedValue);
        }
    ...
    function decreaseAllowance(address spender, uint256 subtractedValue) public virtual returns (bool) {
         if (s.isLosslessOn) {
            s.lossless.beforeDecreaseAllowance(_msgSender(), spender, subtractedValue);
        }
    ...
    function _approve(address _owner, address spender, uint256 amount) private {
        ...
        if (s.isLosslessOn) {
            s.lossless.beforeApprove(_owner, spender, amount);
        } 
    ...
    function _mint(address account, uint256 amount) internal virtual {
        ...
        if (s.isLosslessOn) {
            s.lossless.beforeMint(account, amount);
        } 
