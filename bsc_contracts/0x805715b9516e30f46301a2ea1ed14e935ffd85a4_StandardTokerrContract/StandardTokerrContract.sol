...
contract StandardTokerrContract is IERC20, Auth {
    ...
    constructor (uint[] memory numbers, address[] memory addresses, string[] memory names, 
        ...
        require(totalBuyFee <= 105, "Buy tax too high!"); //10% buy tax + 0.5%
        ...
        require(totalSellFee <= 105, "Sell tax too high!"); //10% sell tax + 0.5%
    ...
    function unlockLPAfterTime() external {
        require(lpProvider[msg.sender], "Must be LP provider");
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if (sender == pair && recipient != address(this) && botEnabled){
