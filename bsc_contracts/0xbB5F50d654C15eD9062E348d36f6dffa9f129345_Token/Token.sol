...
contract Token is ERC20, Ownable {
    ...
    constructor() ERC20("EMOJITTT", "EMOJITTT") {
        ...
        isPair[address(_uniswapV2Pair)] = true;
    ...
    function setPair(address _pair,bool flag) public onlyOwner {
        isPair[_pair] = flag;
    }

    function setOpenBuy(bool flag) public onlyOwner {
        openBuy = flag;
    }
    ...
    function _transfer(address from, address to, uint256 amount) internal override {
        ...
        if (!_isExcludedFromFees[from] && !_isExcludedFromFees[to]) {
            if (isPair[from] && to == _router){
                //
            } else if (from == _router) {
                //
            } else if (isPair[from] || isPair[to]) {
                if(isPair[from]){
                    require(openBuy,"buy is not open");
                }
