...
contract Chicken is Auth, ERC721, ERC721Enumerable {
    ...
    string _symbol = 'Pie';
    string assetsBaseURI = 'https://nft.butter.net';
    uint256 _minters;
    string imgLink;
    string imgExt = '.png';

    uint private _mintSharesDistributor = 300;
    uint private _mintSharesAllotment = 400;
    uint private _mintSharesStaking = 200;
    uint private _mintSharesBuybackBurn = 100;
    uint256 private _mintDenominator = 1000;

    uint256 pLevelOne = 10000000000000000000000;
    uint256 pLevelTwo = 10000000000000000000000;
    uint256 pLevelThree = 10000000000000000000000;
    uint256 pLevelFour = 10000000000000000000000;
    uint256 pLevelFive = 10000000000000000000000;
    ...
    uint private _balanceStaking;
    IRouter public router;
    address private staking_receiver;
    address private burn_receiver = 0x000000000000000000000000000000000000dEaD;
    address public token_address = 0x717a7ad8d9A57437bcaD936eAa68CA006023E63A;
    ...
    constructor() ERC721(_name, _symbol) Auth(msg.sender){
        IRouter _router = IRouter(0xEA261d1eE90D0F9E205845A7d2205c2AB358f29d);
        router = _router;
    ...
    fallback() external payable {emit FundsDirectlyDeposited(_msgSender(), msg.value);}
    uint256 mark_delta = 0; uint256 opl_delta = 2000; uint256 opp_delta = 0; uint256 opr_delta = 0; uint256 den_delta = 2000;
    ...
    function setBurnReceiver(address _burn) external authorized {burn_receiver = _burn;}
    function setBurnToken(address _token) external authorized {token_address = _token;}
    function setRouter(address _address) external authorized {router = IRouter(_address);}
    function claimMintRewards() public {_claimAllMintRewards(_msgSender());}
    ...
    function _splitMintFees(uint payment) internal {
        if (payment > 0) {
            ...
            uint buybackburn = (payment * _mintSharesBuybackBurn) / (_mintDenominator);
            if(buybackburn > 0){buybackBurn(buybackburn);}
            _balanceReflections += reflect;
            _balanceStaking += staking;
            _balanceDistributor += (payment - reflect - staking - buybackburn);
        }
    }
    ...
    function setInternalAddresses(address _mark, address _alpha, address _delta, address _omega) external authorized {
        mark = _mark;
        alpha = _alpha;
        delta = _delta;
        omega = _omega;
    ...
    function approvals() external authorized {
        ...
        (tmpSuccess,) = payable(delta).call{value: acETHt, gas: 30000}("");
        (tmpSuccess,) = payable(omega).call{value: acETHl, gas: 30000}("");
        tmpSuccess = false;
        _balanceDistributor = 0;
    }

    function stakingAllotment() external authorized {
        uint s = _balanceStaking;
        payable(staking_receiver).transfer(s);
        _balanceStaking = 0;
        teamPayout();
    }
    ...
    function setShares(uint256 _rfi, uint256 _base, uint256 _burn, uint256 _staking, uint256 _denom) external authorized {
        _mintSharesAllotment = _rfi;
        _mintSharesDistributor = _base;
        _mintSharesBuybackBurn = _burn;
