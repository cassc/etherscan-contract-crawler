...
interface ITreasury {
    ...
    function getMyUSDPrice() external view returns (uint256);
...
contract ShareWrapper {
    ...
    function stake(uint256 amount) public virtual {
        ...
        if(tokenId.length >= amount) {
            for(uint256 i; i < amount; i++) {
                nfts.transferFrom(address(msg.sender), address(this), tokenId[i]);
    ...
    function withdraw(uint256 amount) public virtual {
        ...
        for(uint256 i; i < amount; i++) {
            nfts.transferFrom(address(this), address(msg.sender), tokenIds[tokenIds.length - 1]);
            tokenIds.pop();
        }
...
contract Boardroom is ShareWrapper, ContractGuard {
    ...
    function updateBoardroom(IERC20 _myUSD, IERC721 _nfts, ITreasury _treasury) external onlyOperator {
        myUSD = _myUSD;
        nfts = _nfts;
        treasury = _treasury;
    }
    ...
    function allocateSeigniorage(uint256 amount) external onlyOneBlock onlyOperator {
        ...
        myUSD.safeTransferFrom(msg.sender, address(this), amount);
        emit RewardAdded(msg.sender, amount);
    }

    function governanceRecoverUnsupported(IERC20 _token, uint256 _amount, address _to) external onlyOperator {
        // do not allow to drain core tokens
        require(address(_token) != address(myUSD), "myUSD");
        require(address(_token) != address(nfts), "nfts");
