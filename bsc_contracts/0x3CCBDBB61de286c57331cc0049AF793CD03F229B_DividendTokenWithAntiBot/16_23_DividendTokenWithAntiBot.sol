...
contract DividendTokenWithAntiBot is ERC20Upgradeable, OwnableUpgradeable {
    ...
    function swapAndSendToFee(uint256 tokens) private {
        if(tokenForMarketingFee==rewardToken){
            uint256 initialCAKEBalance = IERC20Upgradeable(rewardToken).balanceOf(
                address(this)
            );
            swapTokensForCake(tokens);
            uint256 newBalance = (IERC20Upgradeable(rewardToken).balanceOf(address(this)))-(
                initialCAKEBalance
            );
            IERC20Upgradeable(rewardToken).transfer(_marketingWalletAddress, newBalance);
            ...
            if(baseTokenForPair==IUniswapV2Router02(uniswapV2Router).WETH()){
                ...
                IERC20Upgradeable(baseTokenForPair).transfer(_marketingWalletAddress, newBalance);
            } 
        }else{
    ...
    function addLiquidity(uint256 tokenAmount, uint256 baseTokenAmount) private {
        ...
        if (baseTokenForPair == IUniswapV2Router02(uniswapV2Router).WETH()) 
            ...
            IERC20Upgradeable(baseTokenForPair).approve(uniswapV2Router, baseTokenAmount);
    ...
    function swapAndSendDividends(uint256 tokens) private {
        swapTokensForCake(tokens);
        uint256 dividends = IERC20Upgradeable(rewardToken).balanceOf(address(this));
        bool success = IERC20Upgradeable(rewardToken).transfer(
            dividendTracker,
            dividends
        );
