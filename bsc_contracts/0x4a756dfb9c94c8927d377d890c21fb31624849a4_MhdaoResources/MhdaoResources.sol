...
// File contracts/interfaces/IMhdaoFarmGettersOnly.sol
pragma solidity ^0.8.9;

interface IMhdaoFarmGettersOnly {
  function balanceOf(address playerAddress) external view returns (uint256);

  function balancesOf(address playerAddress) external view returns (uint256[] memory);

  function getPlayer(address playerAddress) external view returns (IMhdaoPlayer.Player memory);
}

// File contracts/features/MhdaoResources.sol
// SPDX-License-Identifier: MIT
pragma solidity ^0.8.9;

contract MhdaoResources is Initializable, AccessControlUpgradeable, PausableUpgradeable {
  ...
  function initialize(
    ...
    contracts = Contracts({
      ...
      farm: IMhdaoFarmGettersOnly(_farm)
    });
  }
    ...
    returns (uint256 bctTotalSupply, uint256 farm, uint256 pool, uint256[] memory _resources)
  {
    ...
    for (uint256 i = 0; i < totalResources; i++) {
      _resources[i] = MHIERC20(resources[i]).totalSupply();
    }
  }
  ...
  function _inGameBalancesOf(address account) private view returns (InGameBalances memory) {
    IMhdaoPlayer.Player memory player = contracts.farm.getPlayer(account);
  ...
  function _metamaskBalancesOf(address account) private view returns (MetamaskBalances memory) {
    uint256[] memory metamaskResources = new uint256[](totalResources);
    for (uint256 i = 0; i < totalResources; i++) {
      metamaskResources[i] = MHIERC20(resources[i]).balanceOf(account);
    }
    uint256 metamaskBct = contracts.bct.balanceOf(account);
    uint256 metamaskBusd = contracts.busd.balanceOf(account);

    return MetamaskBalances({bct: metamaskBct, busd: metamaskBusd, resources: metamaskResources});
  }
