...
contract TrustLaunchToken is ERC20, Ownable {
    ...
    constructor() ERC20("TrustLaunchToken", "TLT") {
    ...
    function setBuyFee(uint256 value) external onlyOwner {
        //fee is multiplied by 10 >>> 10 = 1%
        require(value >= 0 && value <= 20, "Fee must be greater or equal zero and less than 2%");
        buyFee = value;
    }
            ...
            if(automatedMarketMakerPairs[from] || automatedMarketMakerPairs[to])
            {
                ...
                if(take)  {
                    ...
                    if(automatedMarketMakerPairs[to]) {
                        marketingTokens = originalAmount.mul(sellFee).div(1000);
                    }
                    else{
                        marketingTokens = originalAmount.mul(buyFee).div(1000);
                    }
