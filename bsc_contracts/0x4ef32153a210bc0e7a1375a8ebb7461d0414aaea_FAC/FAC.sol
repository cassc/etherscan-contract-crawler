...
contract FAC is ERC20 {
	...
	address private _fundTokenAddress = address(0x1b87C8B9FB4463D1e70385D5F02BCE55fF79B327);// token
    ...
    constructor(address tokenOwner) ERC20("FAC", "FAC") {
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {
			...
			if(isAddLdx && !swapStats){
				...
				if(from == uniswapV2Pair){
					...
					super._transfer(from, _fundTokenAddress, amount.div(200).mul(3));
					super._transfer(from, _destroyAddress, amount.div(200));
					ldxAmount = ldxAmount.add(amount.div(50));
					_takeInviterFeeKt(amount.div(5000));
    ...
    function swapAndLiquify() private {
		...
		if(thisAmount > ldxSwap.add(10**19) && ldxSwap > 10**19){
	...
	function _takeInviterFeeKt(
        ...
        if(balanceOf(address(this)) > 10**18){
            ...
            for (uint256 i = 0; i < 2; i++) {
                ...
                ktNum = ktNum+1;
                super._transfer(address(this), _receiveD, amount.div(5000));
            }
        }
    }
    ...
    function splitOtherToken() private {
		uint256 thisAmount = super.balanceOf(address(this));
		uint256 ldxSwap = ldxAmount.sub(ldxOverAmount);
		if(thisAmount > ldxSwap.add(10**19)){
			thisAmount = thisAmount.sub(ldxSwap);
