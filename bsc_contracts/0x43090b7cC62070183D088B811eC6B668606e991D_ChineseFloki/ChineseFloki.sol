...
LP WILL BE BURNT FOREVER THIS ONES FOR YOU LETS MAKE HISTORY!!!!
...
contract ChineseFloki is IBEP20, Ownable {
    using SafeMath for uint256;
    string private constant _name = 'Chinese Floki';
    string private constant _symbol = 'OldFloki';
    ...
    function _transfer(address sender, address recipient, uint256 amount) private {
        if(!isFeeExempt[sender] && !isFeeExempt[recipient]){require(enableTrading, "enableTrading");}
        ...
        if(sender == pair){userPurchase[recipient] = block.timestamp.add(deadSeconds);}
    ...
    function shouldSwapBack(address sender, address recipient) internal view returns (bool) {
        ...
        && deadTimes >= minSells && aboveThreshold;
    }
