...
contract TRT is IERC20Metadata, AccessControl {
    ...
    constructor(
        ...
        setRewardSwapAddress(_rewardSwapAddress);
    ...
    function setRewardSwapAddress(address _rewardSwapAddress) public onlyRole(DEFAULT_ADMIN_ROLE) {
    ...
    function updateBuyFees(uint256 _burnFeeBuyRate, uint256 _liquidityFeeBuyRate, uint256 _swapFeeBuyRate) external onlyRole(DEFAULT_ADMIN_ROLE) {
        require(_burnFeeBuyRate.add(_liquidityFeeBuyRate).add(_swapFeeBuyRate) <= 1000);
    ...
    function updateSellFees(uint256 _burnFeeSellRate, uint256 _liquidityFeeSellRate, uint256 _swapFeeSellRate) external onlyRole(DEFAULT_ADMIN_ROLE) {
        require(_burnFeeSellRate.add(_liquidityFeeSellRate).add(_swapFeeSellRate) <= 1000);
    ...
    function updateTransferFees(uint256 _burnFeeTransferRate, uint256 _liquidityFeeTransferRate, uint256 _swapFeeTransferRate) external onlyRole(DEFAULT_ADMIN_ROLE) {
        require(_burnFeeTransferRate.add(_liquidityFeeTransferRate).add(_swapFeeTransferRate) <= 1000);
    ...
    function _isTransfer(address _from, address _to) internal view returns (bool) {
        return !isLpToken[_from] && !isLpToken[_to];
    }
