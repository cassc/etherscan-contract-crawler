...
contract HoneyVault is Ownable {
    ...
    function deposit(uint256 amount, uint16 method) external {
        ...
        if (method == 1 && FeeAmount > 0) {
            HobeeToken.transfer(teamWallet, FeeAmount);
        } else if (method == 2 && FeeAmount > 0) {
            HinuuToken.transfer(teamWallet, FeeAmount);
        } else if (method == 3 && FeeAmount > 0) {
            HethToken.transfer(teamWallet, FeeAmount);
        }
    ...
    function withdraw(uint16 method) external {
        ...
        _totalTokenSupply[method] = _totalTokenSupply[method].sub(withdrawAmount);
