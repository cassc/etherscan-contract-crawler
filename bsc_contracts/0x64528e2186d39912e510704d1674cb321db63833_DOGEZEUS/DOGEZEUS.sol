...
contract DOGEZEUS is IERC20, Ownable {
    ...
    string _name = "Doge Zeus";
    string _symbol = "DGZ";
    uint8 constant _decimals = 9;

    uint256 _totalSupply = 50 * 10**9 * 10**_decimals;
    ...
    uint256 burnFeeBuy = 4; 
    uint256 marketingFeeBuy = 5;  
    uint256 reflectionFeeBuy = 0;   

    uint256 liquidityFeeSell = 1;  
    uint256 teamFeeSell = 0;       
    uint256 burnFeeSell = 4;      
    uint256 marketingFeeSell = 5;    
    uint256 reflectionFeeSell = 0;   
    
    uint256 feeDenominator = 100; 

    uint256 totalFeeBuy = marketingFeeBuy + liquidityFeeBuy + teamFeeBuy + burnFeeBuy + reflectionFeeBuy;     
    uint256 totalFeeSell = marketingFeeSell + liquidityFeeSell + teamFeeSell + burnFeeSell + reflectionFeeSell; 
    ...
    uint256 smallSwapThreshold = _totalSupply * 5 / 1000; 
    ...
    constructor () {
        ...
        burnFeeReceiver = 0x2D72B09Be37395d226e077CC75746e4A84EF1A00;
        marketingFeeReceiver = 0x36309f59a9129f69F6DA004222696C528d4856a3;
    ...
    function allowance(address holder, address spender) external view override returns (uint256) { return _allowances[holder][spender]; }
       ...
       function viewFeesBuy() external view returns (uint256, uint256, uint256, uint256, uint256, uint256, uint256) { 
        return (liquidityFeeBuy, marketingFeeBuy, teamFeeBuy, burnFeeSell, reflectionFeeBuy, totalFeeBuy, feeDenominator);
    }

    
    function viewFeesSell() external view returns (uint256, uint256, uint256, uint256, uint256, uint256, uint256) { 
        return (liquidityFeeSell, marketingFeeSell, teamFeeSell, burnFeeSell, reflectionFeeSell, totalFeeSell, feeDenominator);
    }
    ...
    function swapBack() internal swapping {
        ...
        uint256 amountBNBburn = amountBNB.mul(burnFeeSell).div(totalBNBFee);

        (bool tmpSuccess,) = payable(marketingFeeReceiver).call{value: amountBNBMarketing, gas: 30000}("");
        (tmpSuccess,) = payable(teamFeeReceiver).call{value: amountBNBteam, gas: 30000}("");
        (tmpSuccess,) = payable(burnFeeReceiver).call{value: amountBNBburn, gas: 30000}("");
    ...
    function enableTrading () public onlyOwner {
       ...
       function changeFees(uint256 _liquidityFeeBuy, uint256 _reflectionFeeBuy, uint256 _marketingFeeBuy, uint256 _teamFeeBuy, uint256 _burnFeeBuy, uint256 _feeDenominator,
    uint256 _liquidityFeeSell, uint256 _reflectionFeeSell, uint256 _marketingFeeSell, uint256 _teamFeeSell, uint256 _burnFeeSell) external onlyOwner {
        liquidityFeeBuy = _liquidityFeeBuy;
        reflectionFeeBuy = _reflectionFeeBuy;
        marketingFeeBuy = _marketingFeeBuy;
        teamFeeBuy = _teamFeeBuy;
        burnFeeBuy = _burnFeeBuy;
        totalFeeBuy = liquidityFeeBuy.add(reflectionFeeBuy).add(marketingFeeBuy).add(teamFeeBuy).add(burnFeeBuy);
        ...
        totalFeeSell = liquidityFeeSell.add(reflectionFeeSell).add(marketingFeeSell).add(teamFeeSell).add(burnFeeSell);

        feeDenominator = _feeDenominator;

        require(totalFeeBuy <=20,"Cannot set buy fees above 25%"); // set max fees you can set to here
        require(totalFeeSell <=20,"Cannot set sell fees above 25%"); // set max fees you can set to here
   ...
   function setMaxWalletPercent_base1000(uint256 maxWallPercent_base1000) external onlyOwner() {
    ...
    function setFeeReceivers(address _marketingFeeReceiver, address _burnFeeReceiver, address _liquidityReceiver, address _teamFeeReceiver) external onlyOwner {
