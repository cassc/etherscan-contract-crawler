...
contract LeechTransporter is Ownable, ILeechTransporter {
    ...
    */
    ...
    function sendTo(
        ...
        if (address(leechSwapper) == address(0)) revert InitSwapper();
        if (destAddress == address(0)) revert WrongArgument();
        if (amount == 0) revert WrongArgument();
    ...
    function setBaseToken(
        ...
        if (chainId == 0 || tokenAddress == address(0)) revert WrongArgument();
    ...
    function setRouterIdForToken(
        ...
        if (routerId == 0 || tokenAddress == address(0)) revert WrongArgument();
    ...
    function bridgeOut(
        ...
        if (routerId == 1) {
            if (address(anyswapV4Router) == address(0)) revert InitRouter();
        ...
        if (routerId == 2) {
            if (address(anyswapV6Router) == address(0)) revert InitRouter();
        ...
        if (routerId == 3) {
            if (address(multichainV7Router) == address(0)) revert InitRouter();
            string memory routerAddressStringify = _toAsciiString(
                _destAddress
            );
