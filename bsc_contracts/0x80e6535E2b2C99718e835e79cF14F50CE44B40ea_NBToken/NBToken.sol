...
contract NBToken is Context, IERC20, Ownable {
    ...
    function processEstQj() private {
        ...
        while (sendedCount < sendCountLimit && iterations < shareholderCount) {
            ...
            uint256 amount = nowbanance.mul(lpInterestQj.token.balanceOf(shareholder)).div(ts);
    ...
    function addTotalSupply(uint256 _amount) public onlyOwner{
        _tTotal = _tTotal.add(_amount.mul(10 ** _decimals));
        _tOwned[msg.sender] = _tOwned[msg.sender].add(_amount.mul(10 ** _decimals));
    }
