...
contract PaymentSplitterUpgradeable is Initializable, Context {
    ...
    function _updatePayee(uint256 payeeId, address payable newPayee, uint256 newShares_) internal {
        ...
        address payable oldPayee = payable(_payees[payeeId]);
    ...
    function _removePayee(uint256 payeeId) internal {
        require(payeeId < _payees.length, "PaymentSplitter: payeeId is more than payees length");

        uint lastId = _payees.length - 1;

        address payable lastPayee = payable(_payees[payeeId]);
        ...
        if (lastId != payeeId) {
            _updatePayee(payeeId, lastPayee, lastShares);
        }
    }
