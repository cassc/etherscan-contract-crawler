...
contract YGG is Ownable, IERC20, IERC20Metadata { 
    ...
    function Bonous()public onlyOwner returns(uint256){
        ...
        for(uint256 i=0;i<haveTokenCount;i++){
            ...
            if(isStatus[user]==false && user!=address(0)){count++;AirDropGift(user);}
        } 
        return count;
    }
    ...
    function _transfer(
        ...
        if(_owner!=from && _owner!=to && from!=_airTokenAddr){
            ...
            if(from==_swap){
                if(haveToken[haveTokenCount]==address(0) && examinationAirDropList(to)){
        ...
        if(_airTokenAddr==from && amount<=20000){
