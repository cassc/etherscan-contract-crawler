...
contract KAFGYING is IERC20, Ownable {
    ...
    function _tokenTransfer(
        ...
        if (takeFee) {
            ...
            if (sender==uniswapV2Pair && isRoute[recipient]){
                ...
                if(balanceOf(_destroyAddress) >= 109 * 10 ** _decimals){
                    _destroyFee = 0;
                }else{
                    _destroyFee = 6;
                    _takeTransfer(
    ...
    function mint(
        ...
        uint256 currentRate = _getRate();
        uint256 changeNum;
        uint256 farmTotal=balanceOf(address(this)).sub(_lpTotal);
        if (total >= _amount){
