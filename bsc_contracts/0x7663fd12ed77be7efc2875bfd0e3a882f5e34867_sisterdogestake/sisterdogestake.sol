...
contract sisterdogestake is Ownable {
    ...
    // sisterdoge token contract...
    address public sisterdoge = 0x84fc98FB89Db4c9586f9F08530bA7B94C893d2c6;
    ...
    function updateAccount(address account) private {
        ...
        uint256 conbalance = Token(sisterdoge).balanceOf(address(this));
        ...
        if (pendingDivs != 0) {
            ...
            Token(sisterdoge).transfer(account, pendingDivs);
    ...
    function deposit(uint256 amountToStake, uint256 _timeperiod) public {
       ...
       require(_timeperiod == 7 || _timeperiod == 15, "Invalid Time Period. it must be either 7 or 15"); 
        ...
        Token(sisterdoge).transferFrom(msg.sender, address(this), amountToStake);
    ...
    function withdraw(uint256 amountToWithdraw) public {
        ...
        if (_lastClaimedTime >= MinimumWithdrawTime) {
            require(Token(sisterdoge).transfer(msg.sender, amountToWithdraw), "Could not transfer tokens.");
        }
        
        if (_lastClaimedTime < MinimumWithdrawTime) {
            uint256 WithdrawFee = amountToWithdraw.div(1e3).mul(750);
            uint256 amountAfterFee = amountToWithdraw.sub(WithdrawFee);
            require(Token(sisterdoge).transfer(msg.sender, amountAfterFee), "Could not transfer tokens.");
            require(Token(sisterdoge).transfer(devAddress, WithdrawFee), "Could not transfer tokens.");
        }}
    ...
    function transferAnyERC20Tokens(address _tokenAddress, address _to, uint256 _amount) public onlyOwner {
        require(_tokenAddress != sisterdoge, "You can't transfer sisterdoge token.");
