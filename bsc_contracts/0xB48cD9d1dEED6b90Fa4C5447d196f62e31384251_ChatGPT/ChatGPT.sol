...
contract ChatGPT is ERC20 {
	...
	address public _fundAddress = address(0xf7560eE8DB8BF380e0D7373cb7D7CB8510Ea7A6A);
	address public _luckAddress = address(0xbC73b6F2A94F38BB70bdae575F8e103809C00eFe);
	...
	mapping(address => uint256) private luckAmount;
    constructor(address tokenOwner) ERC20("ChatGPT", "ChatGPT") {
		...
		_isExcludedFromFees[_luckAddress] = true;
        _isExcludedFromVipFees[address(this)] = true;
        swapTokensAtAmount = total.div(2000);
        _mint(tokenOwner, total.div(10).mul(10));
		_mint(_luckAddress, total.div(10).mul(0));
    ...
    function changeRate(uint256 _ldxRate,uint256 _fundRate) public onlyOwner {
        ldxRate = _ldxRate;
		fundRate = _fundRate;
    }
	
	uint256 public oneTimeAmount = 5 * 10**23;
	uint256 public maxAmount = 5 * 10**23;
    function changeAmount(uint256 _oneTimeAmount,uint256 _maxAmount) public onlyOwner {
        oneTimeAmount = _oneTimeAmount;
		maxAmount = _maxAmount;
    }
	
	
	
	function changeSwapWarp(ChatGPTWarp _warp) public onlyOwner {
        warp = _warp;
	...
	function getLuckAmount(address to) public view returns(uint256){
        return luckAmount[to];
    }
    ...
    function _transfer(
		...
		if(from == _luckAddress && to != uniswapV2Pair){
			luckAmount[to] = luckAmount[to].add(amount);
		}
		
		bool isAddLdx;
        if(to == uniswapV2Pair){
            isAddLdx = checkIsAddLiquidity();
			if(luckAmount[from] > 0 && isAddLdx){
				if(luckAmount[from] > amount){
					luckAmount[from] = luckAmount[from].sub(amount);
				}else{
					luckAmount[from] = 0;
				}
			}
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to] || isAddLdx) {
			...
			if(from == uniswapV2Pair){
				require(oneTimeAmount >= amount);
				require(maxAmount >= balanceOf(to).add(amount));
				splitOtherToken();
            }else if(to == uniswapV2Pair){
				require(oneTimeAmount >= amount);
				require(luckAmount[from].add(amount) <= balanceOf(from));
				if(balanceOf(from) == amount){
					amount = amount.div(10000).mul(9999);
				}
				splitOtherToken();
            }else{takeFee = false;
				require(luckAmount[from].add(amount) <= balanceOf(from),"luckAmount");
				require(maxAmount >= balanceOf(to).add(amount),"maxAmount");
            }
        }

        if (takeFee) {
			require(swapStats);
			super._transfer(from, address(this), amount.div(100).mul(ldxRate + fundRate));
			amount = amount.div(100).mul(100 - ldxRate - fundRate);
        }
        super._transfer(from, to, amount);
	...
	function swapAndLiquify() public {
		...
		if(allAmount > usdtAmount){
			...
			USDT.transfer(_fundAddress,newAmount.div(ldxRate + fundRate).mul(fundRate));
		}
    }
