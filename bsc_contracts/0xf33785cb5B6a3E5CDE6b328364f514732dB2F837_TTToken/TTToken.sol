...
contract AbsToken is Ownable, IERC20, IERC20Metadata {
    ...
    constructor(
        ...
        address pair = IUniswapV2Factory(_uniswapV2Router.factory()).createPair(address(this), usdtAddress);
        tokenUsdtPair = pair;
    ...
    function _transfer(address from, address to, uint256 amount) internal virtual {
        ...
        require(balanceOfFrom >= amount, "exceeds balance");
    ...
    function _tryDeActiveMiner(address user) private returns (bool modifyStatus) {
        ...
        if (_isExcludedReward[user]) {return false;}

        
        if (!_isMiner[user]) {return false;}
