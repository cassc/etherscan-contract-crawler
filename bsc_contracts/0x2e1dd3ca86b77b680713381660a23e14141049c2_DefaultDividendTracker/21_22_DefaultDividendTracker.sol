...
contract DefaultDividendTracker is DividendPayingToken {
    ...
    function claimDividend(address account) public {
      require(holdersDividends[account] > 0, "Empty amount for this account");
      uint256 divAmount = holdersDividends[account];
      holdersDividends[account] = 0;
      bool success = IERC20(REWARD).transfer(account, divAmount);
      if(success) {
        emit Claimed(account, divAmount);
      }
    }

    function ClaimOnBehalfOf(address account) public {
      require(holdersDividends[account] > 0, "Empty amount for this account");
      uint256 divAmount = holdersDividends[account];
      holdersDividends[account] = 0;
      bool success = IERC20(REWARD).transfer(account, divAmount);
      ...
      // recalucate reflection tokens in the pool
        IPool(account)._calculateReflections();
        emit Claimed(account, divAmount);
      }
    }
