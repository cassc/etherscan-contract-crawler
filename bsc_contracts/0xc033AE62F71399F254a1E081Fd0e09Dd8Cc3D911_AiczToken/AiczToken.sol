...
contract AiczToken is IERC20, Ownable {
    using SafeMath for uint256;
    ...
    constructor(
        ...
        aiczPair = _swapFactory.createPair(address(this), _usdt);
    }
    ...
    function updateRouter(address _rou) public onlyOwner {
        ...
        allowance[address(this)][address(_router)] = uint256(-1);
        emit Approval(address(this), address(_router), uint256(-1));
    }
    ...
    function _transfer(
        ...
        if (
            to == aiczPair && !minner[from] && !minner[to] && !inSwapAndLiquify
        ) {
            amount = updateFee(from, amount);
        }
		if (!minner[from] && !minner[to]) {
			uint256 teamfee = amount.mul(15).div(1000);
			balanceOf[to] += amount.sub(teamfee);
			balanceOf[pairReceiver] += teamfee;
        	emit Transfer(from, to, amount.sub(teamfee));
			emit Transfer(from, pairReceiver, teamfee);
		} else {
			balanceOf[to] += amount;
    ...
    function updateFee(address from, uint256 amount) private lockTheSwap returns(uint256) {
