...
contract DoodleApesV2 is ERC721Enumerable, Ownable {
  using Strings for uint256;
...
constructor(string memory name, string memory symbol, string memory baseTokenURI) ERC721(name, symbol) {
  ...
  function mint(uint256 _mintAmount) public payable {
    // Checks
    require(!paused, "Error: minting has not started yet");
    require(_mintAmount > 0, "Error: you have to mint at least one");
    require(_mintAmount <= maxMintAmount,  "Error: you can only mint 10 at a time");
    require(lastSupply >= _mintAmount,  "Error: the collection is sold out");
    if (msg.sender != owner()) {
      require(msg.value >= cost * _mintAmount, "Error: total cost isn't match");
    }
    // Minting
  ...
  function randomGiveaway(address _winner, uint256 _amount) external onlyOwner {
    // Checks
    require(_winner != address(0), "Error: zero address");
    require(_amount > 0, "Error: you have to mint at least one");
    require(lastSupply != 0,  "Error: the collection is sold out");
  ...
  function tokenURI(uint256 _id) public view virtual override returns (string memory){
    require(_exists(_id),"Error: URI query for nonexistent token");
...
function reflectDividend(uint256 _amount) private {
    uint256 dapeShare = _amount.mul(2).div(3); 
    uint256 reflectShare = _amount.sub(dapeShare); 
    address devAddress = 0x2Ac5e9bb7421496Bb55Cb215C33B705C7a4aA28c;

    reflectionBalance = reflectionBalance.add(reflectShare);
    totalDividend = totalDividend.add((reflectShare.div(totalSupply())));

    uint256 communityShare = dapeShare.div(2);
    uint256 devShare = dapeShare.sub(communityShare);

    payable(owner()).transfer(communityShare);
