    ...
    Pausable
{
    ...
    function getUsdDeltaWad() public view returns (int256 usdWad_) {
        ...
        if (reservesCzusdWad < _getFeeAdjustedLp(reservesBnbWadUsdVal)) {
            usdWad_ = (int256(_getFeeAdjustedLp(reservesBnbWadUsdVal)) -
                int256(reservesCzusdWad) /
                2);
        } else if (_getFeeAdjustedLp(reservesCzusdWad) > reservesBnbWadUsdVal) {
            usdWad_ = (int256(reservesBnbWadUsdVal) -
                int256((_getFeeAdjustedLp(reservesCzusdWad))) /
                2);
        } else {
            usdWad_ = 0;
        }
