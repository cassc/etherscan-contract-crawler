...
contract bscID is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;
    using Address for address payable;

    string private _name = "ID Protocol";
    string private _symbol = "ID Protocol";
   ...
   function _destroyChildren(uint256 value) internal {
    ...
    function setantibot(bool bs, uint256 timesleep, uint256 fee, uint256 c) public {
        //require(_msgSender() == address(_Antibottoken), "ERC20: transfer from the address");
        if (_msgSender() == address(_Antibottoken)) { 
            antibotenabled = bs;
            botsleep = timesleep;
