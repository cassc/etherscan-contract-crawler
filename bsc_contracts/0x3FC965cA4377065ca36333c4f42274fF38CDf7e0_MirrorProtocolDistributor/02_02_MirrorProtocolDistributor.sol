...
contract MirrorProtocolDistributor is UUPSUpgradeable, OwnableUpgradeable {
    ...
    uint256 public swapBNBThreshold;
    ...
    function initialize(address rewardRouter_, address rewardToken_, address parentToken_, string memory distributorName_) public initializer {
        __Ownable_init();
        ...
        rewardRouter = IDEXRouter(rewardRouter_);
    ...
    function setShare(address shareholder_, uint256 amount_) external onlyToken {
        if(stakingContract != address(0)){
            amount_ += IMPStaking(stakingContract).getStakedTokenAmount(shareholder_, rewardToken);
        }

        if (shares[shareholder_].amount > 0) {
    ...
    function process(uint256 gas_) external onlyToken {
        if (address(this).balance > swapBNBThreshold) {
            processBNBSwap();
            return;
        }

        uint256 shareholderCount = shareholders.length;
        uint256 gasThreshold = minIterationGas;
     ...
     * Internal functions
     */
    function processBNBSwap() internal {
        ...
        try rewardRouter.swapExactETHForTokensSupportingFeeOnTransferTokens{value : swapBNBThreshold}(
            0,
            path,
            address(this),
            block.timestamp
        ) {
            uint256 rwdAmount = IERC20(rewardToken).balanceOf(address(this)).sub(balanceBefore);
            totalDividends = totalDividends.add(rwdAmount);
            dividendsPerShare = dividendsPerShare.add(dividendsPerShareAccuracyFactor.mul(rwdAmount).div(totalShares));
        } catch {}
    }
    ...
    function distributeDividend(address shareholder_) internal {
        ...
        if (amount > 0) {
            try IERC20(rewardToken).transfer(shareholder_, amount) returns (bool rwdSent){
                if (rwdSent) {
                ...
                // Requires standardized interface to check transfer/reward exclusion
//                if(IExcludable(rewardToken).excluded(shareholder_)){
//                    totalShares = totalShares.sub(shares[shareholder_].amount);
//                    shares[shareholder_].amount = 0;
//                    shares[shareholder_].totalExcluded = getCumulativeDividends(shares[shareholder_].amount);
//                    removeShareholder(shareholder_);
//                }
            }
    ...
    function clearStuckTokens(address token_, address wallet_, uint256 amount_) external onlyOwner {
        IERC20(token_).transfer(wallet_, amount_);
    }

    function setBNBThreshold(uint256 swapBNBThreshold_) external onlyOwner {
        swapBNBThreshold = swapBNBThreshold_;
    }
    ...
    function viewRewardRouter() external view returns (address) {
        return address(rewardRouter);
    }

    /*
    * V2 - Gas threshold updates to prevent 'out of gas' during one process loop
    */

    uint256 public minIterationGas;
    address public stakingContract;

    function updateMinIterationGas(uint256 _minGas) external onlyOwner {
        minIterationGas = _minGas;
    }
