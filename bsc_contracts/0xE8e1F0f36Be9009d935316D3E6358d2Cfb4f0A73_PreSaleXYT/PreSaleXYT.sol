...
contract PreSaleXYT is Ownable{
    ...
    function configManager (address _teamAddr) public onlyOwner {
        require(_teamAddr != address(0), "The teamAddr is invalid");
        teamAddr = _teamAddr;
        usdtMap[teamAddr] = usdtMap[teamAddr].add(1);
        xytMap[teamAddr] = xytMap[teamAddr].add(1);
    }

    function preSale(address _invitor,uint256 _usdtAmount) external returns(bool) {
        // check time
    ...
    function _getUserInfo(address _user) internal view returns(
        ...
        return (usdtMap[_user], xytMap[_user], sendXytMap[_user], invitorMap[_user]);
    }
