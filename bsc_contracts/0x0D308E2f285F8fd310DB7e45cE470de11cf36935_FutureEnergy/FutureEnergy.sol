...
Telegram ( https://t.me/FutureEnergyCrypto )

Website ( https://futureenergy.world )

Twitter ( https://twitter.com/FutureEnergyBSC )
...
contract FutureEnergy is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    FutureEnergyDividendTracker public dividendTracker;
    ...
    constructor() ERC20("Future Energy", "FE") {
        MarketingWallet = payable(0xA8Ab1536edbED4957166923e8669e242bffA67a7);
        TeamWallet = payable(0x8C166892fc23cEF92bAC8CeeE6ace66c92D0Be98);
        ...
        dividendTracker = new FutureEnergyDividendTracker(
            payable(this),
            router,
    ...
    function updateFees(
        ...
        require(totalSellFees <= 25 && totalBuyFees <= 25, "total fees cannot exceed 15% sell or buy");
...
contract FutureEnergyDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    FutureEnergy public FutureEnergyContract;
    ...
    constructor(
        ...
        defaultToken = token;
        FutureEnergyContract = FutureEnergy(mainContract);
    ...
    function _transfer(
        ...
        require(false, "FutureEnergy_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "FutureEnergy_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main FutureEnergy contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(FutureEnergyContract);

            uint256 prevBalance = FutureEnergyContract.balanceOf(address(this));
            ...
            {
                uint256 received = FutureEnergyContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    FutureEnergyContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
