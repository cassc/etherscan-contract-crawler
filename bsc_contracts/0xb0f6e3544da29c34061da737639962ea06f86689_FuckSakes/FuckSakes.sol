...
contract FuckSakes is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    FuckSakesDividendTracker public dividendTracker;
    ...
    constructor() ERC20("FuckSakes", "FS1") {
        marketingWallet = payable(0xc6030d7e687E8484594564Bf32af431e7dc00724);
        devWallet = payable(0x9774eb22d910cfE56f20dA627501381ACCa39Da5);
        ...
        dividendTracker = new FuckSakesDividendTracker(
            payable(this),
            router,
...
contract FuckSakesDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    FuckSakes public FuckSakesContract;
    ...
    constructor(
        ...
        defaultToken = token;
        FuckSakesContract = FuckSakes(mainContract);
    ...
    function _transfer(
        ...
        require(false, "FuckSakes_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "FuckSakes_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main FuckSakes contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(FuckSakesContract);

            uint256 prevBalance = FuckSakesContract.balanceOf(address(this));
            ...
            {
                uint256 received = FuckSakesContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    FuckSakesContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
