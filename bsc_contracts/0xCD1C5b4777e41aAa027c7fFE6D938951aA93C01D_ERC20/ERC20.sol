...
contract OPEPE is ERC20, Ownable {
 ...
 Taxes public taxes = Taxes(3, 2, 5);
 ...
 uint256 constant private marketingAllocation = 7_143;
 uint256 constant private opsAllocation = 2_857;
 ...
 event changeWallets(address marketingWallet, address opWallet);
 ...
 constructor() payable ERC20("Peace Chicken", "PC") {
 ...
 marketingWallet = address(0xF4b3F0E093c70b5a5EE53328A40795d4AfCf5800);
 opWallet = address(0x5dEd86455142A84FcC58AF0C5b17D474c58EfF80);

 dividendTracker = new DividendTracker(1e4, rewardToken);

 IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0x10ED43C718714eb63d5aA57B78B54704E256024E); // PCS Mainnet // 0x10ED43C718714eb63d5aA57B78B54704E256024E
 ...
 uint256 init = 420_690_000_000_000_000 * (10 ** 9);
 
 _mint(owner(), init);
 swapTokensAtAmount = init / 1_000;
 }
 ...
 function _changeWallets(address _marketingWallet, address _opWallet) external onlyOwner {
 require(_marketingWallet != address(0));
 require(_opWallet != address(0));
 marketingWallet = _marketingWallet;
 opWallet = _opWallet;
 emit changeWallets(marketingWallet, opWallet);
 }
 ...
 && msg.sender != owner()
 && !_isExcludedFromFees[ins] && !_isExcludedFromFees[out] && out != DEAD && out != address(0) && out != address(this);
 return isLimited;
 ...
 uint256 allocatedToWallets = address(this).balance * 70 / 100;
 bool success;

 uint256 mark = marketingAllocation * allocatedToWallets / allocation_denominator;
 uint256 opsA = opsAllocation * allocatedToWallets / allocation_denominator;
 
 if(mark > 0) {(success,) = marketingWallet.call{value: mark, gas: 35000}("");}
 if(opsA > 0) {(success,) = opWallet.call{value: opsA, gas: 35000}("");}
 }
 ...
 if(launchtax && (from == uniswapV2Pair || to == uniswapV2Pair)) {
 ...
 require(deadline < 4,"dead too high");
