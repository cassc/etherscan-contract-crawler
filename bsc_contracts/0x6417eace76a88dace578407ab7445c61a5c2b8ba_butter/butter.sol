...
contract butter is Auth, stakeIntegration, ReentrancyGuard {
    ...
    function createLiquidity(uint256 tokenAmount) payable public {
        ...
        stakingToken.transferFrom(staker, address(this), tokenAmount);
        addLiquidity(tokenAmount, msg.value);
        uint256 deltaBalance = stakingToken.balanceOf(address(this)).sub(initialBalance);
        internalDeposit(staker, deltaBalance);
    ...
    function addLiquidity(uint256 tokenAmount, uint256 ETHAmount) private {
        stakingToken.approve(address(router), tokenAmount);
