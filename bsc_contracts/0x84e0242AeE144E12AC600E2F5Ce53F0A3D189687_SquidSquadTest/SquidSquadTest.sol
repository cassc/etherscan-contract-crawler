...
contract SquidSquadTest is ERC721Enumerable, Ownable {
  using Strings for uint256;
  ...
  constructor(string memory name, string memory symbol, string memory baseTokenURI) ERC721(name, symbol) {
      ...
      // Save dev wallet
      _wallet = 0x3b42B060466D2bd0Ea14d35f4aaFe68e2a226286;
  }
  ...
  function mint(uint256 _mintAmount) public payable {
    // Checks
    require(!paused, "SquidSquadTest: minting has not started yet");
    require(_mintAmount > 0, "SquidSquadTest: you have to mint at least one");
    require(_mintAmount <= maxMintAmount,  "SquidSquadTest: you can only mint 10 at a time");
    require(lastSupply >= _mintAmount,  "SquidSquadTest: the collection is sold out");
    if (msg.sender != owner()) {
      require(msg.value >= cost * _mintAmount, "SquidSquadTest: total cost isn't match");
    }
    // Minting
  ...
  function randomGiveaway(address _winner, uint256 _amount) external onlyOwner {
    // Checks
    require(_winner != address(0), "SquidSquadTest: zero address");
    require(_amount > 0, "SquidSquadTest: you have to mint at least one");
    require(lastSupply != 0,  "SquidSquadTest: the collection is sold out");
  ...
  function tokenURI(uint256 _id) public view virtual override returns (string memory){
    require(_exists(_id),"SquidSquadTest: URI query for nonexistent token");
