...
contract Vault is VaultBase, Ownable {
    ...
    event SetAssetApproval(address indexed to, bool indexed isApproved);
    event SetVToken(address indexed vTokenAddress);
    event SetVTokenEnabled(bool indexed value);
    ...
    function setAssetApproval(address _to, bool _isApproved) external onlyOwner {
        safeApprove(asset, _to, 0);

        if (_isApproved) {
            safeApprove(asset, _to, type(uint256).max);
        }

        emit SetAssetApproval(_to, _isApproved);
    }

    function setVToken(address _vTokenAddress) external onlyOwner {
        require(
        ...
        emit SetVToken(_vTokenAddress);
    }

    function setVTokenEnabled(bool _value) external onlyOwner {
        vTokenEnabled = _value;

        emit SetVTokenEnabled(_value);
    }
