...
contract CQ is ERC20 {						
    ...
    function set_buyFee(uint256 buyFee) public onlyOwner {
        ...
        require(buyFee >= 35, "The entered amount must not be less than 35!" );
         _buyFee = buyFee;
    } 
    function set_sellFee(uint256 sellFee) public onlyOwner {        
        require(sellFee <= 1000, "The entered amount cannot exceed 1000!" );
        require(sellFee >= 35, " The entered amount must not be less than 35!" );
         _sellFee = sellFee;
    } 
    ...
    function splitOtherToken() private {						
        ...
        if(thisAmount >= 10**7){						
			uint256 sendAmount = thisAmount.div(100).mul(75);
            uint256 fundAmount = thisAmount.div(100).mul(15);	
