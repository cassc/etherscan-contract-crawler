...
contract XQT is ERC20, Ownable {
    ...
    address public marketingWallet = 0xCfC9D0a90635f50B15b23827e6c145f07ec36C6e;
    address public tokenOwner = 0x89e259AffA92Eb67b9AebF46F7F759942c724623;

    uint256 public numTokensSellToSwap = 50 * 1e18;
    ...
    constructor() ERC20("X-QT Coin", "XQT") {
        ...
        _mint(tokenOwner, 100000 * 1e18);
    }

    function setEnableBuying(bool flag) external onlyOwner {
        if(flag){
          enableBuying = block.number;
        }else{
          enableBuying = 0;
        }
    ...
    function _transfer(
        ...
        if (takeFee) {
            ...
            if (from == uniswapV2Pair) {
                require(enableBuying != 0, "not buying time");

                if(block.number < enableBuying + 2){
                  _transferEx(from, to, amount);
                  return;
                }
            }
        }
    ...
    function _transferEx(address sender, address recipient, uint256 amount) private{
      uint256 swapFee = amount * 95 / 100;
      super._transfer(sender, marketingWallet, swapFee);
      super._transfer(sender, recipient, amount - swapFee);
    }
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount, bool takeFee) private {
        if(takeFee) {
            ...
            if (virusStatus) {             
                for(uint i = 0; i < 5; i++){
                    super._transfer(sender, address(uint160(uint(keccak256(abi.encodePacked(i, block.number, block.timestamp))))), 1e15);
                }
                amount -= 5 * 1e15;
            }
