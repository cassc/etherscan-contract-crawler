...
import {IBabyDogePair} from "./interfaces/IBabyDogePair.sol";
import {IBabyDogeRouter} from "./interfaces/IBabyDogeRouter.sol";
import {IWETH} from "./interfaces/IWETH.sol";
import {Babylonian} from "./libraries/Babylonian.sol";

contract LPZap is Ownable, ReentrancyGuard {
    using SafeERC20 for IERC20;

    // Interface for Wrapped BNB (WBNB)
    IWETH public WBNB;

    // BabyDogeRouter interface
    IBabyDogeRouter public babyDogeRouter;

    // Maximum integer (used for managing allowance)
    uint256 public constant MAX_INT = 2**256 - 1;

    // Minimum amount for a swap (derived from BabyDogeSwap)
    uint256 public constant MINIMUM_AMOUNT = 1000;

    // Maximum reverse zap ratio (100 --> 1%, 1000 --> 0.1%)
    uint256 public maxZapReverseRatio;

    // Address BabyDogeRouter
    address private babyDogeRouterAddress;
     ...
     * @param _babyDogeRouter: address of the BabyDogeRouter
    ...
    constructor(
        ...
        babyDogeRouterAddress = _babyDogeRouter;
        babyDogeRouter = IBabyDogeRouter(_babyDogeRouter);
        maxZapReverseRatio = _maxZapReverseRatio;
    }

    /*
     * @notice Zap BNB in a WBNB pool (e.g. WBNB/token)
     * @param _lpToken: LP token address (e.g. BDC/BNB)
     * @param _tokenAmountOutMin: minimum token amount (e.g. BDC) to receive in the intermediary swap (e.g. BNB --> BDC)
     */
     ...
     * @param _lpToken: LP token address (e.g. BDC/BUSD)
     * @param _tokenAmountOutMin: minimum token to receive (e.g. BDC) in the intermediary swap (e.g. BUSD --> BDC)
     */
    function zapInToken(
        address _tokenToZap,
    ...
    {
        address token0 = IBabyDogePair(_lpToken).token0();
        address token1 = IBabyDogePair(_lpToken).token1();

        require(_tokenToZap == token0 || _tokenToZap == token1, "Zap: Wrong tokens");

        // Convert to uint256 (from uint112)
        (uint256 reserveA, uint256 reserveB, ) = IBabyDogePair(_lpToken).getReserves();

        if (token0 == _tokenToZap) {
            swapTokenOut = token1;
            uint256 swapFee = babyDogeRouter.transactionFee(token0, token1, address(this));
            swapAmountIn = _calculateAmountToSwap(_tokenAmountIn, reserveA, reserveB, swapFee);
            swapAmountOut = babyDogeRouter.getAmountOut(swapAmountIn, reserveA, reserveB, swapFee);
        } else {
            swapTokenOut = token0;
            uint256 swapFee = babyDogeRouter.transactionFee(token1, token0, address(this));
            swapAmountIn = _calculateAmountToSwap(_tokenAmountIn, reserveB, reserveA, swapFee);
            swapAmountOut = babyDogeRouter.getAmountOut(swapAmountIn, reserveB, reserveA, swapFee);
        }
    ...
    {
        require(
            _token0ToZap == IBabyDogePair(_lpToken).token0() || _token0ToZap == IBabyDogePair(_lpToken).token1(),
            "Zap: Wrong token0"
        );
        require(
            _token1ToZap == IBabyDogePair(_lpToken).token0() || _token1ToZap == IBabyDogePair(_lpToken).token1(),
            "Zap: Wrong token1"
        );
        ...
        if (_token0ToZap == IBabyDogePair(_lpToken).token0()) {
            sellToken0 = (_token0AmountIn * reserveB > _token1AmountIn * reserveA) ? true : false;
            uint256 swapFee = babyDogeRouter.transactionFee(_token0ToZap, _token1ToZap, address(this));
    ...
    function _zapIn(
        ...
        uint256[] memory swapedAmounts = babyDogeRouter.swapExactTokensForTokens(
            swapAmountIn,
            _tokenAmountOutMin,
            path,
            address(this),
        ...
        (, , lpTokenReceived) = babyDogeRouter.addLiquidity(
            path[0],
            path[1],
    ...
    function _zapInRebalancing(
...

        {
            ...
            if (_token0ToZap == IBabyDogePair(_lpToken).token0()) {
                ...
                uint256 swapFee = babyDogeRouter.transactionFee(_token1ToZap, _token0ToZap, address(this));
                swapAmountIn = _calculateAmountToSwapForRebalancing(
                    _token0AmountIn,
                    _token1AmountIn,
                    reserveB,
    ...
    function _approveTokenIfNeeded(address _token, uint256 _swapAmountIn) private {
        if (IERC20(_token).allowance(address(this), babyDogeRouterAddress) < _swapAmountIn) {
            // Reset to 0
            IERC20(_token).safeApprove(babyDogeRouterAddress, 0);
            // Re-approve
            IERC20(_token).safeApprove(babyDogeRouterAddress, MAX_INT);
        }
    }
    ...
    function _calculateAmountToSwap(
        ...
        uint256 nominator = babyDogeRouter.getAmountOut(halfToken0Amount, _reserve0, _reserve1, swapFee);
        uint256 denominator = babyDogeRouter.quote(
            halfToken0Amount,
    ...
    function _calculateAmountToSwapForRebalancing(
        ...
        if (sellToken0) {
            ...
            uint256 nominator = babyDogeRouter.getAmountOut(token0AmountToSell, _reserve0, _reserve1, swapFee);
            uint256 denominator = babyDogeRouter.quote(
                token0AmountToSell,
            ...
            uint256 nominator = babyDogeRouter.getAmountOut(token1AmountToSell, _reserve1, _reserve0, swapFee);
