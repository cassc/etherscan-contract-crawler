...
contract AIInu is Context, IERC20, ERC20Ownable {
    using SafeMath for uint256;
    using Address for address;

    string private constant tokenName = "AI Inu";
    string private constant tokenSymbol = "AIINU";
    ...
    function GoLive() external onlyOwner returns (bool){
        ...
        IUniswapV2Router02 _pcsRouter = IUniswapV2Router02(0x10ED43C718714eb63d5aA57B78B54704E256024E);
        pcsRouter = _pcsRouter;
        isMaxWalletExcluded[address(pcsRouter)] = true;
        internalApprove(address(this), address(pcsRouter), tokenSupply);
        pcsPair = IUniswapV2Factory(_pcsRouter.factory()).createPair(address(this), _pcsRouter.WETH());
        isMaxWalletExcluded[address(pcsPair)] = true;
        require(address(this).balance > 0, "ERC20: Must have BNB on contract to Go Live!");
    ...
    function internalTransfer(address from, address to, uint256 amount) internal {
        ...
        if(limitsOn){
            if (from != owner() && to != owner() && to != address(0) && to != dead && to != pcsPair) {
                for (uint x = 0; x < 1; x++) {
        ...
        if (!inSwapAndLiquify && swapAndLiquifyStatus && balanceOf(pcsPair) > 0 && totalTokensToSwap > 0 && !isTaxExcluded[to] && !isTaxExcluded[from] && to == pcsPair && overMinimumTokenBalance) {
            taxTokenSwap();
            }
    ...
    function swapTokensForETH(uint256 tokenAmount) internal {
        ...
        internalApprove(address(this), address(pcsRouter), tokenAmount);
