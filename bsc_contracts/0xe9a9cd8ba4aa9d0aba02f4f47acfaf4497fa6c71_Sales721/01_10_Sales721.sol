...
contract Sales721 is Ownable, ReentrancyGuard {
    ...
    function setSaleIDCounter(uint256 _saleIDCounter) external onlyOwner {
    ...
    // whitelist to set sale
    mapping(uint256 => bool) public auctionOnProd;
    ...
    function settleAuction(uint256 _saleID) external nonReentrant onlyWhitelist {
        ...
        if (auction.highestBidPrice != 0) {
            ...
            for (uint256 i = 0; i < auction.base.tokenAddresses.length; i++) {
                IERC721(auction.base.tokenAddresses[i]).safeTransferFrom(
                    auction.base.seller,
                    auction.highestBidder,
    ...
    function addTokenIdToSale(
        ...
        uint256 standardLen = _tokenAddresses.length;
        require(standardLen > 0, "length of tokenAddresses must be > 0");
        require(standardLen == _tokenIDs.length, "length of tokenIDs is wrong");
    ...
    function setAuctionEnv(uint256 _saleID, bool isProd) public nonReentrant onlyOwner {
    ...
    function purchase(
        ...
        uint256 newPurchaseRecord = flashSaleIDToPurchaseRecord[_saleID][msg.sender].add(_amount);
        {
            ...
            if (activityMap[activityId].isEnable) {
                ...
                require(
                    !activityWhitelist[activityId].isWhitelist || activityWhitelist[activityId].whitelists[msg.sender],
        ...
        for (uint256 i = 0; i < _amount; i++) {
            IERC721(flashSale.base.tokenAddresses[targetIndex]).safeTransferFrom(
                flashSale.base.seller,
                msg.sender,
