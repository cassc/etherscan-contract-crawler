...
contract Poop is ERC20, Ownable {
    ...
    PoopDividendTracker public dividendTracker;
    ...
    constructor() ERC20("Poop", "POOP") {
        marketingWallet = payable(0xf496085c142ED3ECd0d36CC2496B819fc417662B);
        devWallet = payable(0xf496085c142ED3ECd0d36CC2496B819fc417662B);
        ...
        dividendTracker = new PoopDividendTracker(
            payable(this),
            router,
        ...
        maxWallet = totalTokenSupply / 5000; // 0.005%
...
contract PoopDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor(
        ...
        defaultToken = token;
        PoopContract = Poop(mainContract);
    ...
    function _transfer(
        ...
        require(false, "Poop_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "Poop_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main Poop contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(PoopContract);

            uint256 prevBalance = PoopContract.balanceOf(address(this));
            ...
            {
                uint256 received = PoopContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    PoopContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
