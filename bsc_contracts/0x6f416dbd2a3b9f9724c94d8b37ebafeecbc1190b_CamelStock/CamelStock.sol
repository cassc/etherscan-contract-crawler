...
// SPDX-License-Identifier: MIT
	...
	contract CamelStock is ERC20, Ownable{
		...
		function withdrawUsdt() external  {
			...
			ERC20(usdt_address).transfer(msg.sender,1);
			
			(uint _year, uint _month, uint _day)=DateTimeLibrary._daysToDate(block.timestamp/DateTimeLibrary.SECONDS_PER_DAY);
        ...
        function withdrawUsdt1() external  {
			address _addr=msg.sender;
        ...
        function withdrawUsdt2() external view returns(uint _year, uint _month, uint _day)  {
			 
			( _year,  _month,  _day)=DateTimeLibrary._daysToDate(block.timestamp/DateTimeLibrary.SECONDS_PER_DAY);
