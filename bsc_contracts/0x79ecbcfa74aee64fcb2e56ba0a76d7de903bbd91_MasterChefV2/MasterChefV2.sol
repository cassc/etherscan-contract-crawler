...
// MasterChef is the master of Stzu. He can make Stzu and he is a fair guy.
//
// Note that it's ownable and the owner wields tremendous power. The ownership
// will be transferred to a governance smart contract once Stzu is sufficiently
...
contract MasterChefV2 is Ownable, ReentrancyGuard {
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint;       // How many allocation points assigned to this pool. Stzus to distribute per block.
        uint256 lastRewardBlock;  // Last block number that Stzus distribution occurs.
        uint256 accTokenPerShare;   // Accumulated Stzus per share, times 1e12. See below.
    ...
    // The STZU TOKEN!
    address public stzuAddress;
    // STZU tokens created per block.
    ...
    // The block number when STZU mining starts.
    ...
    function pendingToken(uint256 _pid, address _user) external view returns (uint256) {
        ...
        uint256 lpSupply = address(pool.lpToken) == address(stzuAddress) ? totalStakedAmount : pool.lpToken.balanceOf(address(this));
    ...
    function safeTokenTransfer(address _to, uint256 _amount) internal {
        uint256 tokenBalance = IBEP20(stzuAddress).balanceOf(address(this));
        bool transferSuccess = false;
        if (_amount > tokenBalance) {
            transferSuccess = IBEP20(stzuAddress).transfer(_to, tokenBalance);
        } else {
            transferSuccess = IBEP20(stzuAddress).transfer(_to, _amount);
        }
    ...
    function SetNewFeeAddress(address newFeeAddress) public onlyOwner {
        require(newFeeAddress != 0x0000000000000000000000000000000000000000, "feeaddress must not be zero.");
        emit SetFeeAddress(feeAddress, newFeeAddress);
    }
}