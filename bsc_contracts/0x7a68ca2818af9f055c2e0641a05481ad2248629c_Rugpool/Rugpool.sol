...
contract Rugpool is Context, Ownable {
    ...
    function startInvestmentRound(uint256 maxInvestment, uint256 hoursToClaim) external onlyOwner {
        ...
        require(investmentRounds[lastInvestmentRoundId + 1].totalInvested <= maxInvestment * 10 ** USD.decimals(), "Predeposits exceed Max Investment!");
    ...
    function getTradingFund(uint256 claimStart) external {
    ...
    function reinvest(address upline) external notBanned(msg.sender) {
        ...
        deposits[lastInvestmentRoundId + 1][msg.sender].amount += amount + deposits[lastInvestmentRoundId][msg.sender].amount;
        deposits[lastInvestmentRoundId][msg.sender].dividendWithdrawnOrReinvested = true;
        players[msg.sender].totalInvested += amount;
        investmentRounds[lastInvestmentRoundId + 1].totalInvested += amount + deposits[lastInvestmentRoundId][msg.sender].amount;
    ...
    function payout() external notBanned(msg.sender) {      
        ...
        if (investmentRounds[lastInvestmentRoundId].dividendPercentage >= 1000) {
            ...
            investmentRounds[lastInvestmentRoundId].totalDividendsEarned += amount;  
        }
