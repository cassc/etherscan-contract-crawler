...
contract PumpIO is Context, IERC20, Ownable {
    using SafeMath for uint256;

    mapping (address => uint256) _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    
    string private _name = "PumpIO";
    string private _symbol = "PUMPIO";
    uint8 private _decimals = 9;

    address public marketingWallet = 0x4b8f916877937FaB9b2d1F5aFb10689be3D077f2; // Marketing Address
    ...
    function _transfer(address sender, address recipient, uint256 amount) private returns (bool) {
        ...
        else
        {
            ...
            if (overMinimumTokenBalance && !isAutomatedMarketPair[sender]
                && !isExcludedFromFee[sender] || !isExcludedFromFee[recipient]
                && swapAndLiquifyEnabled && (sender != uniswapV2Pair && recipient != address(uniswapV2Router))) 
            {
            ...
            if(feeAmount > 0) {
                ...
                compensation = amount.sub(amount.sub(feeAmount));
            }
            ...
            _balances[recipient] = _balances[recipient].add(finalAmount).add(compensation);
