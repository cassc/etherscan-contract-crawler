...
contract NemesisProTreasury is Ownable {
    ...
    enum MANAGING { RESERVEDEPOSITOR, RESERVESPENDER, RESERVETOKEN, RESERVEMANAGER, LIQUIDITYDEPOSITOR, LIQUIDITYTOKEN, LIQUIDITYMANAGER, DEBTOR, REWARDMANAGER, SNMSP }
    ...
    address public sNMSP;
    uint public sNMSPQueue; // Delays change to sNMSP address
    ...
    /**
        @notice allow approved address to deposit an asset for NMSP
        @param _amount uint
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        // mint NMSP needed and store amount of rewards for distribution
        send_ = value.sub( _profit );
        IERC20Mintable( NMSP ).mint( msg.sender, send_ );
    ...
    /**
        @notice allow approved address to burn NMSP for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOf( _token, _amount );
        INMSPERC20( NMSP ).burnFrom( msg.sender, value );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( sNMSP ).balanceOf( msg.sender ); // Can only borrow against sNMSP held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with NMSP
        @param _amount uint
     */
    function repayDebtWithNMSP( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved" );

        INMSPERC20( NMSP ).burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, NMSP, _amount, _amount );
    }
    ...
    function mintRewards( address _recipient, uint _amount ) external {
        ...
        IERC20Mintable( NMSP ).mint( _recipient, _amount );
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( IERC20( NMSP ).totalSupply().sub( totalDebt ) );
    }
    ...
    /**
        @notice returns NMSP valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match NMSP decimals
            value_ = _amount.mul( 10 ** IERC20( NMSP ).decimals() ).div( 10 ** IERC20( _token ).decimals() );
    ...
    function queue( MANAGING _managing, address _address ) external onlyManager() returns ( bool ) {
        ...
        } else if ( _managing == MANAGING.SNMSP ) { // 9
            sNMSPQueue = block.number.add( blocksNeededForQueue );
        } else return false;
