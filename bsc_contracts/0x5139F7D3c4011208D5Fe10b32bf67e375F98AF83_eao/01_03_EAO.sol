...
contract eao {
    ...
    uint256[7] private levelInvite = [0, 5000e18, 10000e18, 20000e18, 30000e18, 40000e18, 50000e18];
    ...
    uint256 private constant lotteryDuration = 3 minutes;
   ...
   IUniswapV2Router02 public immutable uniswapV2Router;
    ...
    function w11111() public {
        ...
        uint256 staticsLS = userRewards.statics;
        if(staticsLS > 0){
            usdt.transfer(defaultRefer, staticsLS.div(6));
        ...
        for(uint256 i = balReached.length; i > 0; i--){
            if(balStatus[balReached[i - 1]]){
                if(_bal < MAXBal.mul(balFreezeStatic[i - 1]).div(100))
                {
                    ...
                    if(_bal < MAXBal.mul(balFreezeDynamic[i - 1]).div(100))
                    {
                        freezeDynamicReward = true;
                        if(_bal < MAXBal.mul(balFreezeAll[i - 1]).div(100))
                        {
                            ...
                            if(RebackEndTime == 0){
                                ...
                                if(block.timestamp.sub(RebackEndTime)>10000){
                ...
                else
                {
                    ...
                    if( _bal >= MAXBal.mul(110).div(100)){
