...
// File contracts/features/MhdaoSwap.sol
// SPDX-License-Identifier: MIT
pragma solidity ^0.8.9;

// Collaborators: Karacurt, Kozika, Mabesi

contract MhdaoSwap is Initializable, AccessControlUpgradeable, PausableUpgradeable {
  using Counters for Counters.Counter;

  bytes32 internal constant OPERATOR_ROLE = keccak256("OPERATOR_ROLE");

  MHIERC20 public paymentToken;
  ...
  event TradeOfferCreated(uint256 id, address alice, uint256 aliceTokenId, address bob, uint256 bobTokenId);
  event TradeOfferCancelled(uint256 id, address alice, uint256 aliceTokenId, address bob, uint256 bobTokenId);
  event TradeOfferExecuted(uint256 id, address alice, uint256 aliceTokenId, address bob, uint256 bobTokenId);
  ...
  function initialize(
    ...
    // Set the contracts
    paymentToken = MHIERC20(_paymentToken);
    trustedNft = IERC721(_trustedNft);
    stasher = IStasher(_stasher);

    // Create the dummy order
    TradeOffer memory tradeOffer = TradeOffer(0, address(0), 0, address(0), 0);
  ...
  function OfferStatusError(Status status) private pure returns (string memory) {
  ...
  function setTrustedNft(address _trustedNft) external onlyRole(OPERATOR_ROLE) {
    trustedNft = IERC721(_trustedNft);
  }
  ...
  function _deleteOffer(uint256 offerId) internal {
    ...
    delete tokenIdToOfferId[offer.aliceTokenId];
  }

  function createTradeOffer(
    uint256 _aliceTokenId,
    address _bob,
    uint256 _bobTokenId
  ) external whenNotPaused returns (uint256) {
    require(_bob != address(0), "INV_ADDRESS");
    require(msg.sender != _bob, "CANNOT_TRADE_WITH_SELF");
    require(_aliceTokenId != _bobTokenId, "INV_TOKEN_ID");
    require(msg.sender == trustedNft.ownerOf(_aliceTokenId), "NOT_OWNER");
    require(_bob == trustedNft.ownerOf(_bobTokenId), "FRIEND_NOT_OWNER");

    if (tokenIdToOfferId[_aliceTokenId] != 0) {
      uint256 offerId = tokenIdToOfferId[_aliceTokenId];
      tradeToStatus[offerId] = Status.CANCELLED;
      TradeOffer memory _offer = offers[offerIdToOfferIndex[offerId]];
      _deleteOffer(tokenIdToOfferId[_aliceTokenId]);

      emit TradeOfferCancelled(_offer.id, _offer.alice, _offer.aliceTokenId, _offer.bob, _offer.bobTokenId);
    }
    ...
    TradeOffer memory offer = TradeOffer({
      ...
      aliceTokenId: _aliceTokenId,
      bob: address(_bob),
      bobTokenId: _bobTokenId
    });

    tokenIdToOfferId[_aliceTokenId] = _offerId;
    tradeToStatus[_offerId] = Status.PENDING;
    _addOffer(offer);
    _offerIdCounter.increment();

    emit TradeOfferCreated(offer.id, offer.alice, offer.aliceTokenId, offer.bob, offer.bobTokenId);
  ...
  function executeTradeOffer(uint256 offerId) external whenNotPaused {
    ...
    uint256 offerIndex = offerIdToOfferIndex[offerId];
    TradeOffer memory offer = offers[offerIndex];
    require(msg.sender == offer.bob, "ACCESS_DENIED");
    _deleteOffer(offerId);

    if (offer.alice != trustedNft.ownerOf(offer.aliceTokenId) || offer.bob != trustedNft.ownerOf(offer.bobTokenId)) {
      tradeToStatus[offer.id] = Status.CANCELLED;

      emit TradeOfferCancelled(offer.id, offer.alice, offer.aliceTokenId, offer.bob, offer.bobTokenId);
    ...
    emit TradeOfferExecuted(offer.id, offer.alice, offer.aliceTokenId, offer.bob, offer.bobTokenId);

    trustedNft.transferFrom(offer.alice, offer.bob, offer.aliceTokenId);
    trustedNft.transferFrom(offer.bob, offer.alice, offer.bobTokenId);
  }
  ...
  function cancelTradeOffer(uint256 offerId) external whenNotPaused {
    ...
    require(msg.sender == offer.alice || msg.sender == offer.bob, "ACCESS_DENIED");
  ...
  function recoverERC20(address _tokenAddress, uint256 _amount, address _recipient) external onlyRole(OPERATOR_ROLE) {
    MHIERC20(_tokenAddress).transfer(_recipient, _amount);
  }
  ...
  function transferNFT(uint256 _tokenId, address _recipient) external {
    trustedNft.safeTransferFrom(msg.sender, _recipient, _tokenId);
  }
}