...
import { ICreditor } from "./ICreditor.sol";
import { IDeltaNeutralVault } from "./IDeltaNeutralVault.sol";
import { LinkList } from "./LinkList.sol";
...
contract AutomatedVaultController is OwnableUpgradeable {
  ...
  // --- Events ---
  event LogAddPrivateVaults(
    address indexed _caller,
    IDeltaNeutralVault[] _vaults
  );
  ...
  function initialize(
    ICreditor[] memory _creditors,
    IDeltaNeutralVault[] memory _privateVaults
  ...
  function usedCredit(address _user) public view returns (uint256) {
    ...
    for (uint8 _i = 0; _i < _length; ) {
      ...
      if (_share != 0)
        _total += IDeltaNeutralVault(_curVault).shareToValue(_share);
  ...
  function addPrivateVaults(IDeltaNeutralVault[] memory _newPrivateVaults)
    external
    onlyOwner
  {
    // sanity check
  ...
  function removePrivateVaults(address[] memory _privateVaultAddresses)
    external
    onlyOwner
  {
    // sanity check
    ...
    for (uint8 _i = 0; _i < _newPrivateVaultLength; ) {
      privateVaults.remove(
        _privateVaultAddresses[_i],
        privateVaults.getPreviousOf(_privateVaultAddresses[_i])
      );
      // uncheck overflow to save gas
      unchecked {
        _i++;
  ...
  function onDeposit(
    ...
    // Check
    if (!privateVaults.has(msg.sender))
      revert AutomatedVaultController_Unauthorized();

    if (totalCredit(_user) < (usedCredit(_user) + _shareValue))
      revert AutomatedVaultController_InsufficientCredit();
  ...
  function getUserVaultShares(address _user, address _vault)
    external
    view
    returns (uint256)
  {
