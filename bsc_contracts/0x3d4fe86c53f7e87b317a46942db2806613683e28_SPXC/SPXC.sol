...
contract SPXC is Context, IERC20, Ownable {
    ...
    string constant private _name = "SpaceXCoin";
    string constant private _symbol = "SPXC";
    ...
    address public _marketingWalletAddress = 0x8516eF338ef4aF43D8CE4eFA2dC0af7a01858D4c;
    address public _SPXWalletAddress = 0xA3d7efbaB4073DbB77eB337Da4b0c71ef14999cF;
    ...
    uint256 public _SPXFee;
    uint256 private _previousSPXFee = _SPXFee;

    uint256 public _BUYtaxFee = 3;
    uint256 public _BUYliquidityFee = 1;
    uint256 public _BUYmarketingFee = 3;
    uint256 public _BUYSPXFee = 3;

    uint256 public _SELLtaxFee = 3;
    uint256 public _SELLliquidityFee = 1;
    uint256 public _SELLmarketingFee = 3;
    uint256 public _SELLSPXFee = 3;
    ...
    function setBUYFeePercent(uint256 taxFee, uint256 liquidityFee, uint256 marketingFee, uint256 SPXFee) external onlyOwner() {
        _BUYtaxFee = taxFee;
        _BUYliquidityFee = liquidityFee;
        _BUYmarketingFee = marketingFee;
        _BUYSPXFee = SPXFee;
        require(_BUYtaxFee + _BUYliquidityFee + _BUYmarketingFee + _BUYSPXFee <= 25, "Buy fee must be 25% or less");
    }
    
    function setSELLFeePercent(uint256 taxFee, uint256 liquidityFee, uint256 marketingFee, uint256 SPXFee) external onlyOwner() {
        _SELLtaxFee = taxFee;
        _SELLliquidityFee = liquidityFee;
        _SELLmarketingFee = marketingFee;
        _SELLSPXFee = SPXFee;
        require(_SELLtaxFee + _SELLliquidityFee + _SELLmarketingFee + _SELLSPXFee <= 25, "Sell fee must be 25% or less");
    }
     
    function setMaxTxPercent(uint256 maxTxPercent) external onlyOwner() {
       require(maxTxPercent >= 1, "Can't set max transaction amount less than 1% of total supply");
    ...
    function setSPXWallet(address wallet) external onlyOwner()
    {
        _SPXWalletAddress = wallet;
    }
    ...
    function calculateSPXFee(uint256 _amount) private view returns (uint256) {
        return _amount.mul(_SPXFee).div(
            10**2
        );
    }
    ...
    function _takeSPX(address sender, uint256 tTransferAmount, uint256 rTransferAmount, uint256 tAmount) private
    returns (uint256, uint256)
    {
        if(_SPXFee==0) {  return(tTransferAmount, rTransferAmount); }
        uint256 tSPX = tAmount.div(100).mul(_SPXFee);
        uint256 rSPX= tSPX.mul(_getRate());
        rTransferAmount = rTransferAmount.sub(rSPX);
        tTransferAmount = tTransferAmount.sub(tSPX);
        _rOwned[address(this)] = _rOwned[address(this)].add(rSPX);
        emit Transfer(sender, address(this), tSPX);
    ...
    function removeAllFee() private {
        if(_taxFee == 0 && _marketingFee == 0 && _SPXFee == 0 && _liquidityFee == 0) return;
        
        _previousTaxFee = _taxFee;
        _previousMarketingFee = _marketingFee;
        _previousLiquidityFee = _liquidityFee;
        _previousSPXFee = _SPXFee;
    ...
    function _transfer(
        ...
        if(to != uniswapV2PairAddr && from != uniswapV2PairAddr)
        {
            takeFee = 0;
        }
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        uint256 tokensForLiquidity = contractTokenBalance.mul(_liquidityFee).div(_liquidityFee.add(_marketingFee).add(_SPXFee));
        uint256 tokensForSPX = contractTokenBalance.mul(_SPXFee).div(_liquidityFee.add(_marketingFee).add(_SPXFee));
        uint256 tokensForMarketing = contractTokenBalance.mul(_marketingFee).div(_liquidityFee.add(_marketingFee).add(_SPXFee));
        ...
        swapTokensForWallets(tokensForMarketing, _marketingWalletAddress);
        swapTokensForWallets(tokensForSPX, _SPXWalletAddress);
    }
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) private {
        ...
        (tTransferAmount, rTransferAmount) = _takeSPX(sender, tTransferAmount, rTransferAmount, tAmount);
        _rOwned[sender] = _rOwned[sender].sub(rAmount);
