...
contract Pool is Initializable, OwnableUpgradeable, ReentrancyGuardUpgradeable, PoolStorage, IPool {
        ...
        returns (uint256 amountInAfterFee, uint256 feeAmount, uint256 lpAmount)
    {
        ...
        uint256 _fee = _calcAddRemoveLPFee(poolValue, _token, tokenPrice, valueChange, true);
        ...
        returns (uint256 outAmount, uint256 outAmountAfterFee, uint256 feeAmount)
    {
        ...
        uint256 _fee = _calcAddRemoveLPFee(poolValue, _tokenOut, tokenPrice, valueChange, false);
        ...
        returns (uint256 amountOutAfterFee, uint256 feeAmount)
    {
        ...
        uint256 feeIn = _calcSwapFee(poolValue, _tokenIn, priceIn, valueChange, true);
        uint256 feeOut = _calcSwapFee(poolValue, _tokenOut, priceOut, valueChange, false);
    ...
    function _calcSwapFee(uint256 _poolValue, address _token, uint256 _tokenPrice, uint256 _valueChange, bool _isSwapIn)
        internal
        view
        returns (uint256)
    {
        ...
        return _calcFeeRate(_poolValue, _token, _tokenPrice, _valueChange, baseSwapFee, taxBasisPoint, _isSwapIn);
    }

    function _calcAddRemoveLPFee(
        uint256 _poolValue,
        address _token,
        uint256 _tokenPrice,
        ...
        return _calcFeeRate(
            _poolValue, _token, _tokenPrice, _valueChange, addRemoveLiquidityFee, addRemoveLiquidityFee, _isAdd
        );
    }
