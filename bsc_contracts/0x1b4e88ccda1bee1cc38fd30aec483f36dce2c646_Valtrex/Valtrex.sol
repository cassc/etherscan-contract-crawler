...
contract Valtrex is Ownable {
    ...
    uint256 public basePercent = 2_50;
    ...
    uint256 public referrerPercent = 5_00;
    ...
    function updateTeam(address _user, uint256 _amount) private {
        ...
        if (amountIndex == countIndex) {
            team.currentPercent = amountIndex * 20;
        } else if (amountIndex < countIndex) {
            team.currentPercent = amountIndex * 20;
        } else {
            team.currentPercent = countIndex * 20;
        }
        if(team.currentPercent > 200){
            team.currentPercent = 200;
        }
    }
