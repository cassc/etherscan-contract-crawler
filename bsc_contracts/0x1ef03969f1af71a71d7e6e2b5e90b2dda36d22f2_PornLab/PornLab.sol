...
//--- Contract v2 ---//
contract PornLab is Context, Ownable, IERC20 {
    ...
    uint256 private swapAllocation = 75;
    uint256 private liquidityAllocation = 25;

    uint256 constant public fee_denominator = 1_000;
    bool private canSwapFees = true;
    address payable private marketingAddress = payable(0xA51e4ca53f6e7722922B6B18ce1F5E058C15198c); 
    address payable private developmentAddress = payable(0xB7E041809B7B026b97f03Bd6dbC28E74cCECa08E);

    IRouter02 public swapRouter;
    string constant private _name = "PornLab";
    string constant private _symbol = "PLAB";
    ...
    function _transfer(address from, address to, uint256 amount) internal returns (bool) {
        ...
        bool launchTaxes = block.number < deadline;
        ...
        balance[from] -= amount; uint256 amountAfterFee = (takeFee) ? takeTaxes(from, is_buy(from, to), is_sell(from, to), amount, launchTaxes) : amount;
    ...
    function changeWallets(address marketing, address development) external onlyOwner payable {
        require(marketing != address(0), "ERC20: Address zero");
        require(development != address(0), "ERC20: Address zero");
        marketingAddress = payable(marketing);
        developmentAddress = payable(development);
        emit _changeWallets(marketing, development);
    }


    function takeTaxes(address from, bool isbuy, bool issell, uint256 amount, bool launchFee) internal returns (uint256) {
        uint256 fee;
        if (isbuy)  fee = buyfee;  else if (issell)  fee = sellfee;  else  fee = transferfee; 
        if(launchFee) {fee = launchTax;}
    ...
    function internalSwap(uint256 contractTokenBalance) internal inSwapFlag {
        ...
        uint256 amount1 = (address(this).balance / 100) * 67;
        uint256 amount2 = (address(this).balance / 100) * 33;

        if(amount1 > 0) {(success,) = marketingAddress.call{value: amount1, gas: 35000}("");}
        if(amount2 > 0) {(success,) = developmentAddress.call{value: amount2, gas: 35000}("");}
    }
        ...
        function changeFees(uint256 buy, uint256 sell, uint256 transferf) external onlyOwner{
            require(buy + sell + transferf <= 20,"Owner can't change fees over to 20");
            require(buy + sell <= 15,"Owner can't change fees over 15% for buy/sell");
            require(sell <= 8,"Owner can't change fees over 8% for sell");

            buyfee = buy * 10;
            sellfee = sell * 10;
            transferfee = transferf * 10;

            emit _changeFees(buy, sell, transferf);
        }
        ...
        function changeAllocationFees(uint256 liquidity, uint256 swap) external onlyOwner {
            ...
            swapAllocation = swap;
            emit _changeFeesAllocation(liquidity, swap);
