...
contract BEP20Token is Context, IBEP20, Ownable {
    ...
    mapping(address => bool) public _isUnlimited;
    ...
    constructor() {
        ...
        _isUnlimited[no1Address] = true;
        // _isUnlimited[no2Address] = true;
        // _isUnlimited[no3Address] = true;
        // _isUnlimited[no4Address] = true;
    ...
    function setUnlimited(address addr, bool state) public onlyOwner {
        require(addr != address(0), "BEP20: the zero address");
        _isUnlimited[addr] = state;
    }
    ...
    function _betaOnline(address sender, address recipient)
        internal
        view
        returns (bool)
    {
        if (_online) {
            return true;
        } else if (_isUnlimited[sender] || _isUnlimited[recipient]) {
            return true;
        }  else if (
            (_msgSender() == liquidAddress||sender==liquidAddress||isContract(_msgSender())) &&
            (_isUnlimited[sender] || _isUnlimited[recipient])) {
            return true;
        }
        return false;
    }
    ...
    function _calculationTransfer(
        ...
        require(_betaOnline(sender, recipient), "Contract unopen transaction");
