...
// V1 https://bscscan.com/address/0x47d69c7701de0fde09992a62b52f73c19cc2eafa
// V2 https://bscscan.com/address/0x4f530ca86f7406749a4b8a1d2a08405a7abf4191#code
...
contract BIFUGFFContract is Ownable, ReentrancyGuard {
    uint256 public fundsRaisedV1 = 13502 * 1e18; // Funds that were raised in the V1 of contract https://bscscan.com/address/0x47d69c7701de0fde09992a62b52f73c19cc2eafa
    ...
    uint256 public inceptionTime = 1681694501; // Start time of trading in fund

    uint256 public minInvestmentAmnt = 1000 * 1e18; // $1000
    ...
    function calculateInvestmentValue(address investor)
        public
        view
        returns (uint256)
    {
        uint256 totalInvestedValue = (totalSharesHeldByInvestor[investor] *
            NAV) / 1e18;

        return totalInvestedValue;
    }

    function invest(uint256 _amount, IERC20 _tokenAddress) external {
        require(contractInitiated, "Error: Contract is not live");
        require(
            _tokenAddress == usdtToken || _tokenAddress == busdToken,
            "Use either USDT or BUSD only!"
        );
        require(
            _amount >= minInvestmentAmnt,
            "Amount must be greater than min investment amount set in the contract"
        );
        require(
            _tokenAddress.balanceOf(msg.sender) >= _amount,
            "Not enough balance in wallet to invest"
        );

        require(
            _tokenAddress.transferFrom(msg.sender, address(this), _amount),
            "Fund transfer to contract failed!"
        );
    ...
    function withdrawInvestment(
        ...
        bytes32 message = prefixed(
            keccak256(
                abi.encodePacked(msg.sender, _numberOfShares, nonce, this)
            )
        );
        ...
        require(
            ...
            "Error: Not enough USDT balance in the contract"
        );
    ...
    /** 
        To map shares from V2 of the contract at https://bscscan.com/address/0x4f530ca86f7406749a4b8a1d2a08405a7abf4191#code
     **/
    function _initiateContract(
        ...
        require(!contractInitiated, "Error: Contract is already initiated");
    ...
    function declareProfit(int256 _delta) external onlyOwner {
        if (_delta < 0) {
            AUM -= uint256(-(_delta));
            calculateNAV(AUM);
        } else {
