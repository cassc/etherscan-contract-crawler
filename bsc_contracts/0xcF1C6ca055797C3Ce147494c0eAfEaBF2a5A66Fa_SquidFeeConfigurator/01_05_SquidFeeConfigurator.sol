...
contract SquidFeeConfigurator is OwnableUpgradeable {

    struct FeeCategory {
        uint256 total;      // total fee charged on each harvest
        uint256 squid;      // split of total fee going to squid fee batcher
    ...
    function getFeeCategory(uint256 _id, bool _adjust) public view returns (FeeCategory memory fees) {
        ...
        if (_adjust) {
            ...
            fees.squid = fees.squid * _totalFee / DIVISOR;
    ...
    function setFeeCategory(
        ...
        uint256 squid = DIVISOR - _call - _strategist;

        FeeCategory memory category = FeeCategory(_total, squid, _call, _strategist, _label, _active);
        feeCategory[_id] = category;
        emit SetFeeCategory(_id, _total, squid, _call, _strategist, _label, _active);
    }
