...
contract Staking is Initializable, OwnableUpgradeable{
    ...
    function removeInvestorInfo(uint8 _level) public onlyOwner {
        for (uint256 s = stakeholders[_level].length ; s >= 0 ; s -= 1){
            for(uint8 _period = 0; _period <= 4; _period += 1 ){
                stake memory tempStakes = stakes[stakeholders[_level][s]][StakingPeriod(_period)][_level];
                totalStake = totalStake - tempStakes.amount;
                tempStakes.amount = 0;
                stakes[stakeholders[_level][s]][StakingPeriod(_period)][_level] = tempStakes;
                stakeholders[_level].pop();
            }
        }
    }
 
    function getInvestorRewards(uint256 _unstakeAmount, StakingPeriod _period, uint _timeStamp) internal view returns (uint256) {
