...
contract ReignToken is ERC20Upgradeable, OwnableUpgradeable, ReentrancyGuardUpgradeable  {
    ...
    function initialize(address _busdToken, address _presale) public initializer {
        ...
        swapEnabled = true;
        rewardYields = [1458000, 1875000, 2291000, 2708000];
    ...
    function _rebase() private {
        if (!inSwap) {
            ...
            if (rewardYields[1] != 1875000) rebaseWithBonus += 1;
    ...
    function fulfillRandomWords(uint256 requestId, uint256[] memory randomness) internal {
        uint256 random1 = (randomness[0] % 1035000) + 3125000;
        uint256 random2 = (randomness[1] % 1035000) + 3125000;
        uint256 random3 = (randomness[2] % 1035000) + 3125000;

        rewardYields = [1458000, random1, random2, random3];
        rebaseWithBonus = 0;
    }
    ...
    function _resetYieldStaking() external onlyOwner {
        resetYieldStaking();
    }
...
if (automatedMarketMakerPairs[sender]) _realFee = totalBuyFee;
    ...
    function setStake(address user, uint256 stakingType, uint256 time) external onlyOwner {
        stacking[user][stakingType].unlockDate = time;
    }
    ...
    function claim(uint256 stakingType) external nonReentrant {
        ...
        if (stacking[msg.sender][stakingType].newStruct == false) {
            amountToClaim = (stacking[msg.sender][stakingType].amount.div(_yields[stakingType + 1]) - stacking[msg.sender][stakingType].amountAtStaking - stacking[msg.sender][stakingType].amountClaim);
        } else {
            amountToClaim = (stacking[msg.sender][stakingType].amountAfterClaim).div(_yields[stakingType + 1]) - stacking[msg.sender][stakingType].amountAtStaking;
        }

        require(amountToClaim > 0, "You have nothing to claim");

        totalStaked[stakingType] -= amountToClaim.mul(_yields[stakingType + 1]);
        _gonBalances[msg.sender] += amountToClaim.mul(getYield());

        if (stacking[msg.sender][stakingType].newStruct == false) {
            stacking[msg.sender][stakingType].amountClaim = stacking[msg.sender][stakingType].amountClaim.mul(_yields[stakingType + 1]);
        }
        
        stacking[msg.sender][stakingType].amountClaim -= (amountToClaim.mul(_yields[stakingType + 1]));
        stacking[msg.sender][stakingType].newStruct = true;
    ...
    function unstake(uint256 stakingType) external nonReentrant {
        ...
        if (stacking[msg.sender][stakingType].newStruct == false) {
            amountToClaim = (stacking[msg.sender][stakingType].amount.div(_yields[stakingType + 1]) - stacking[msg.sender][stakingType].amountClaim);
        } else {
            amountToClaim = (stacking[msg.sender][stakingType].amount - stacking[msg.sender][stakingType].amountClaim).div(_yields[stakingType + 1]);
        }
        
        stacking[msg.sender][stakingType] = StakingUserStruct(0,0,0,0,true,0);
    ...
    function getStakingAmount(address user) external view returns (uint256[] memory _used) {
        ...
        for (uint256 i = 0; i < stakingTypeCount; i++) {
            if (stacking[user][i].newStruct == false) {
                used[i] = (stacking[user][i].amount.div(_yields[i + 1]) - stacking[msg.sender][i].amountClaim);
            } else {
                used[i] = (stacking[user][i].amount - stacking[msg.sender][i].amountClaim).div(_yields[i + 1]);
            }
        }
        return used;
    }
    ...
    function getNewRebaseStakedToken(address user) external view returns (uint256 _nextRebase) {
        ...
        for (uint256 i = 0; i < stakingTypeCount; i++) {
            if (stacking[user][i].newStruct == false) {
                nextRebase += (stacking[user][i].amount.div(_yields[i + 1])) * rewardYields[i + 1] / rewardYieldDenominator;
            } else {
                nextRebase += ((stacking[user][i].amount - stacking[user][i].amountClaim).div(_yields[i + 1])) * rewardYields[i + 1] / rewardYieldDenominator;
            }
        }
        return nextRebase;
    }
    ...
    function getRebaseDailyStakedToken(address user) public view returns (uint256 _nextRebase) {
        uint8[3] memory dailyYield = [9, 11, 13];

        for (uint256 i = 0; i < stakingTypeCount; i++) {
            if (stacking[user][i].newStruct == false) {
                _nextRebase += (stacking[user][i].amount.div(_yields[i + 1]) - stacking[user][i].amountClaim) * dailyYield[i] / 1000;
            } else {
                _nextRebase += (stacking[user][i].amount - stacking[user][i].amountClaim).div(_yields[i + 1]) * dailyYield[i] / 1000;
            }
        }
    }
    ...
    function getMaxSellAmount(address user) public view returns(uint256 maxSell) {
        return (getRebaseDailyStakedToken(user) + (balanceOf(user) * 7 / 1000)) * 2;
    }
