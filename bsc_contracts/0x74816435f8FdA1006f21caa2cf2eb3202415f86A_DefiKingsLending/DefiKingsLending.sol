...
contract DefiKingsLending is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    DefiKingsLendingDividendTracker public dividendTracker;
    ...
    uint256 private gasPriceLimit = 12 * 1 gwei; 
    ...
    event updateTreasuryWallet(address wallet);
    ...
    constructor() ERC20("Defi Kings Lending", "DKL") {
        marketingWallet = payable(0x6042F172bc1c0409c63B17db5a12a662Ab6199F5);
        treasuryWallet = payable(0xCA7B27818984C1208D99A90E2B73A130Aa126D37);
        ...
        dividendTracker = new DefiKingsLendingDividendTracker(
            payable(this),
            router,
            0x2170Ed0880ac9A755fd29B2688956BD959F933F8,
            "DefiKingsLendingTRACKER",
            "DKLTRACKER"
        );
        ...
        maxTxAmount = totalTokenSupply / 100; // 1%
    ...
    function setTreasuryWallet(address wallet) external onlyOwner {
        ...
        emit updateTreasuryWallet(wallet);
    }    
    ...
    // set max tx amount, can not be lower than 0.1% of supply
    ...
    function updateFees(
        ...
        buyTreasuryFee = treasuryBuy;
        sellTreasuryFee = treasurySell;
        ...
        require(totalSellFees <= 15 && totalBuyFees <= 10, "total fees cannot exceed 15% sell or 10% buy");
    ...
    function _transfer(
        ...
        if (to == DEAD) {
            ...
            if (!isBuying && !isSelling) {
                ...
                require(amount <= maxTxAmount, "Exceeds maximum Tx amount." );
                RewardsFee = sellRewardsFee;
                deadFees = sellDeadFees;
                marketingFees = sellMarketingFees;
                liquidityFee = sellLiquidityFee;
                treasuryFees = sellTreasuryFee;
            ...
            uint256 totalFees = RewardsFee
                .add(liquidityFee + marketingFees + treasuryFees);
    ...
    function swapAndSendDividends(uint256 tokens) private {
        ...
        if (buyRewardsFee > 0) {
            ...
            .div(totalAmount)
            .mul(buyRewardsFee)
            .div(buyRewardsFee + buyMarketingFees + buyTreasuryFee);
        }
        if (sellRewardsFee > 0) {
            ...
            .div(totalAmount)
            .mul(sellRewardsFee)
            .div(sellRewardsFee + sellMarketingFees + sellTreasuryFee);
        }
        ...
        uint256 _completeFees = sellMarketingFees.add(sellTreasuryFee) +
            buyMarketingFees.add(buyTreasuryFee);
        ...
        uint256 treasuryPayout = buyTreasuryFee.add(sellTreasuryFee) * feePortions;

        if (marketingPayout > 0) {
            (successOp1, ) = address(marketingWallet).call{value: marketingPayout}("");
        }
        if (treasuryPayout > 0) {
            (successOp2, ) = address(treasuryWallet).call{value: treasuryPayout}("");
        }

        emit SendDividends(
            dividends,
            marketingPayout + treasuryPayout,
...
contract DefiKingsLendingDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    DefiKingsLending public DefiKingsLendingContract;
    ...
    constructor(
        ...
        defaultToken = token;
        DefiKingsLendingContract = DefiKingsLending(mainContract);
    ...
    function _transfer(
        ...
        require(false, "DefiKingsLending_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "DefiKingsLending_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main DefiKingsLending contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(DefiKingsLendingContract);

            uint256 prevBalance = DefiKingsLendingContract.balanceOf(address(this));
            ...
            {
                uint256 received = DefiKingsLendingContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    DefiKingsLendingContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
