...
contract MetaproAirdrop is Ownable, ERC1155Holder, ReentrancyGuard  {
    ...
    function withdrawUnclaimedTokens(uint256 _tokenId, bytes memory _data) public onlyOwner {
        AirdropConfiguration storage configuration = airdrops[_tokenId];

        require(configuration.valid, "MetaproAirdrop: Aidrop invalid");
        require(block.number > configuration.endBlock, "MetaproAirdrop: Airdrop not finished yet");
        ...
        delete createdAirdropTokenIds[_tokenId];
        delete whitelists[_tokenId];
        delete airdrops[_tokenId];
