...
// File: Contracts/KalosZAP/Babylonian.sol
...
// File: Contracts/KalosZAP/IWETH.sol
...
// File: Contracts/KalosZAP/IKalosRouter01.sol


pragma solidity >=0.6.2;

interface IKalosRouter01 {
...
// File: Contracts/KalosZAP/IKalosRouter02.sol


pragma solidity >=0.6.2;


interface IKalosRouter02 is IKalosRouter01 {
...
// File: Contracts/KalosZAP/IKalosPair.sol


pragma solidity >=0.5.0;

interface IKalosPair {
...
// File: Contracts/KalosZAP/ReentrancyGuard.sol
...
// File: Contracts/KalosZAP/Address.sol
...
// File: Contracts/KalosZAP/IERC20.sol
...
// File: Contracts/KalosZAP/SafeERC20.sol
...
// File: Contracts/KalosZAP/Context.sol
...
// File: Contracts/KalosZAP/Ownable.sol
...
// File: Contracts/KalosZAP/KalosZap.sol
 ...
 * Implemented and modified by PancakeSwap - KalosDeFi teams.
 */
contract KalosZap is Ownable, ReentrancyGuard {
    ...
    // Minimum amount for a swap (derived from KalosDeFi)
    ...
    {
        address token0 = IKalosPair(_lpToken).token0();
        address token1 = IKalosPair(_lpToken).token1();

        require(_tokenToZap == token0 || _tokenToZap == token1, "KalosZap: Wrong tokens");

        // Convert to uint256 (from uint112)
        (uint256 reserveA, uint256 reserveB, ) = IKalosPair(_lpToken).getReserves();
    ...
    {
        require(
            _token0ToZap == IKalosPair(_lpToken).token0() || _token0ToZap == IKalosPair(_lpToken).token1(),
            "KalosZap: Wrong token0"
        );
        require(
            _token1ToZap == IKalosPair(_lpToken).token0() || _token1ToZap == IKalosPair(_lpToken).token1(),
            "KalosZap: Wrong token1"
        );

        require(_token0ToZap != _token1ToZap, "KalosZap: Same tokens");

        // Convert to uint256 (from uint112)
        (uint256 reserveA, uint256 reserveB, ) = IKalosPair(_lpToken).getReserves();

        if (_token0ToZap == IKalosPair(_lpToken).token0()) {
        ...
        )
    {
        ...
        require(_tokenToReceive == token0 || _tokenToReceive == token1, "KalosZap: Token not in LP");
    ...
    function _zapIn(
        ...
        require(_tokenAmountIn >= MINIMUM_AMOUNT, "KalosZap: Amount too low");
...

        {
            ...
            require((reserveA >= MINIMUM_AMOUNT) && (reserveB >= MINIMUM_AMOUNT), "KalosZap: Reserves too low");

            if (token0 == _tokenToZap) {
                swapAmountIn = _calculateAmountToSwap(_tokenAmountIn, reserveA, reserveB);
                path[1] = token1;
                require(reserveA / swapAmountIn >= maxZapReverseRatio, "KalosZap: Quantity higher than limit");
            } else {
                swapAmountIn = _calculateAmountToSwap(_tokenAmountIn, reserveB, reserveA);
                path[1] = token0;
                require(reserveB / swapAmountIn >= maxZapReverseRatio, "KalosZap: Quantity higher than limit");
            }
        }
    ...
    function _zapInRebalancing(
        ...
        require(swapAmountIn <= _tokenAmountInMax, "KalosZap: Amount to swap too high");
    ...
    function _zapOut(
        ...
        (uint256 amount0, uint256 amount1) = IKalosPair(_lpToken).burn(address(this));

        require(amount0 >= MINIMUM_AMOUNT, "KalosRouter: INSUFFICIENT_A_AMOUNT");
        require(amount1 >= MINIMUM_AMOUNT, "KalosRouter: INSUFFICIENT_B_AMOUNT");
    ...
    function _approveTokenIfNeeded(address _token, uint256 _swapAmountIn) private {
        if (IERC20(_token).allowance(address(this), kalosRouterAddress) < _swapAmountIn) {
            // Reset to 0
            IERC20(_token).safeApprove(kalosRouterAddress, 0);
            // Re-approve
    ...
    function _calculateAmountToSwapForRebalancing(
        ...
        require(sellToken0 == _isToken0Sold, "KalosZap: Wrong trade direction");
