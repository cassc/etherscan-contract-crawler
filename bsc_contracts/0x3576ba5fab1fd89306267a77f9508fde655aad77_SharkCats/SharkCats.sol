...
contract SharkCats is BEP20, Ownable {
    using SafeMath for uint256;
    ...
    address payable public marketingWallet = payable(address(0x5852b33bd28c0b41F7da224E3f05e021452E92a4)); //marketing wallet
    ...
    event SendDividends(uint256 ethSwapped, uint256 amount);
    ...
    constructor() BEP20("Shark Cats", "SharkCats") {
        ...
        */
        ...
        maxTxAmount = 10000000000 * 10**9; // 1% per per transaction
        maxWallet = 20000000000 * 10**9;// 2% of the supply per wallet limit
    }
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(
            ...
            "SHARK CATS: The dividend tracker already has that address"
        );
        ...
        require(
            ...
            "SHARK CATS: The new dividend tracker must be owned by the TOKEN token contract"
        );
    ...
    require(newAddress != address(uniswapV2Router), "SHARK CATS: The router already has that address");
    ...
    function claimStuckTokens(address _token) external onlyOwner {
        require(_token != address(this),"SHARK CATS: No rug pulls");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(
            ...
            "SHARK CATS: Account is already excluded"
        );
        _isExcludedFromFees[account] = excluded;
    ...
    function setMarketingWallet(address payable _marketing ) external onlyOwner {
        require(_marketing != address(0), "SHARK CATS: marketing Wallet can't be a zero address");
    ...
    function setBuyFees(
        ...
        totalBuyFee = buyFee.reward + buyFee.marketing  + buyFee.autoLP;
        require (totalBuyFee <= 5, "SHARK CATS: Max buy fees limit is 5 percent");
    }
    ...
    function setSellFees(
        ...
        require(totalSellFee <= 5, "SHARK CATS: Max Sell fees limit is 5 percent");
    }
        ...
        onlyOwner
    {
        require(
            pair != uniswapV2Pair,
            "SHARK CATS: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs"
        );
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(
            ...
            "SHARK CATS: Automated market maker pair is already set to that value"
        );
        automatedMarketMakerPairs[pair] = value;
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(
            ...
            "SHARK CATS: gasForProcessing must be between 200,000 and 500,000"
        );
        require(
            newValue != gasForProcessing,
            "SHARK CATS: Cannot update gasForProcessing to same value"
        );
    ...
    function _transfer(
        ...
        if (takeFee) {
            uint256 fees;
            require(amount <= maxTxAmount,"SHARK CATS: Amount exceeds transfer per transaction limit");
            if (!automatedMarketMakerPairs[to]) {
                require(
                    balanceOf(to) + amount <= maxWallet,
                    "SHARK CATS: Balance exceeds Max Wallet limit"
                );
            }
    ...
    function setMaxTx (uint256 amount) external onlyOwner {
        require(amount >= (totalSupply().mul(10).div(1000)),"SHARK CATS: No rug pull");
        maxTxAmount = amount;
    }
    ...
    function swapAndLiquify(uint256 tokens) private {
        ...
        uint256 swapTokens = tokens.mul(buyFee.marketing + sellFee.marketing + (buyFee.autoLP + sellFee.autoLP)/2 + buyFee.reward + sellFee.reward)
                            .div(totalFees);
        ...
        uint256 marketingPart = newBalance.mul(buyFee.marketing + sellFee.marketing)
                                .div(totalFees);
        uint256 rewardPart = newBalance.mul(buyFee.reward + sellFee.reward).div(totalFees);
        (bool ms,) = marketingWallet.call {value: marketingPart}("");
        require (ms, "bnb transfer failed");
        addLiquidity(liqTokens, newBalance - marketingPart - rewardPart);
        sendBUSDdividends (rewardPart);
    }
    ...
    function sendBUSDdividends(uint256 tokens) private {
...
contract TOKENDividendTracker is Ownable, DividendPayingToken {
    ...
    constructor()
        DividendPayingToken("SharkCats_Dividend_Tracker", "SharkCats_Dividend_Tracker")
    {
        claimWait = 3600;
    ...
    function _transfer(
        ...
        require(false, "SHARK CATS_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "SHARK CATS_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main TOKEN contract."
        );
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(
            ...
            "SHARK CATS_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours"
        );
        require(
            newClaimWait != claimWait,
            "SHARK CATS_Dividend_Tracker: Cannot update claimWait to same value"
        );
