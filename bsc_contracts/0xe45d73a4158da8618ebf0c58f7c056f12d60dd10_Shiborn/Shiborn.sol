...
contract Shiborn is ERC20, Ownable {
    ...
    event SendCharityWallet(uint256 charityShiba);
    event SendMarketingWallet(uint256 marketingShiba);
    event SendDevWallet(uint256 devShiba);

    constructor() ERC20("Shiborn", "SHBRN") {
        address newOwner = 0xEe131dBF91D8dBa930b9071Ec15B8E846365cAC1;
        ...
        marketingWallet = 0x66D2860AbAC329842872997Ca4D75C8FB0EC16aE;
        devWallet = 0xE2c92685FAF5c580050FB5989C0fA976b65D394A;
        charityWallet = 0xE284008BAf8E4351F227DAEc19Da6be135C65C1c;
        ShibaAddress = 0x2859e4544C4bB03966803b044A93563Bd2D0DD4D;

        walletToWalletTransferWithoutFee = true;

        maxWalletLimitEnabled = true;
        maxWalletAmount = 3e7 * (10**decimals());

        maxTransactionLimitEnabled = true;
        maxTransactionAmountBuy = 3e6 * (10**decimals());
        maxTransactionAmountSell = 3e6 * (10**decimals());
    ...
    function updateWalletToWalletFee(uint256 _walletToWalletFee)
        ...
        onlyOwner
    {
        require(_walletToWalletFee <= 20, "Fees must be less than 19%");
    ...
    function _transfer(
        ...
        if (
            ...
            uint256 ShibaShare = marketingShare + devShare + charityShare;
            

            if (ShibaShare > 0) {
                uint256 bnbToUsd = (newBalance * ShibaShare) / totalFee;
                path = new address[](2);
                path[0] = uniswapV2Router.WETH();
                path[1] = ShibaAddress;
                
                uint256 initialShibaBalance = IERC20(ShibaAddress).balanceOf(address(this));
                ...
                uint256 currentBalance = IERC20(ShibaAddress).balanceOf(address(this));
                currentBalance -= initialShibaBalance;
                

                if (marketingShare > 0) {
                    uint256 marketingShiba = (currentBalance * marketingShare) / ShibaShare;
                    IERC20(ShibaAddress).transfer(
                        marketingWallet,
                        marketingShiba
                    );
                    emit SendMarketingWallet(marketingShiba);
                }

                if (charityShare > 0) {
                    uint256 charityShiba = (currentBalance * charityShare) / ShibaShare;
                    IERC20(ShibaAddress).transfer(
                        charityWallet,
                        charityShiba);
                    emit SendCharityWallet(charityShiba);
                }
                if (devShare > 0) {
                    uint256 devShiba = (currentBalance * devShare) / ShibaShare;
                    IERC20(ShibaAddress).transfer(
                        devWallet,
                        devShiba);
                    emit SendDevWallet(devShiba);
                }
            }
