...
import { CornCompound } from "./CornCompound.sol";
...
/*
 ...
 1% CORNDAWG to xPERPS stakers
 1% Rehypothication
 Does not receive incentives daily but does receive 1% of all BITCORN deposited into Compound VDC
 ...
 */
contract CornVolatile is Initializable, Ownable, ReentrancyGuard
{
	using SafeERC20 for IERC20;

	struct AccountInfo {
		uint256 amount; // CORNDAWG staked
		uint256 burned; // xPERPS burned
		uint256 reward; // BITCORN reward from Compound VDC accumulated but not claimed
		uint256 drip1; // CORNDAWG from drip pool accumulated but not claimed
		uint256 drip2; // CORNDAWG from compound drip pool accumulated but not claimed
		uint256 boost; // CORNDAWG from boost accumulated but not claimed
		uint256 accRewardDebt; // CORNDAWG reward debt from PCS distribution algorithm
		uint256 accDripDebt1; // CORNDAWG drip pool debt from PCS distribution algorithm
		uint256 accDripDebt2; // CORNDAWG compound drip pool debt from PCS distribution algorithm
		uint256 accBoostDebt; // CORNDAWG reward debt from PCS distribution algorithm
	...
	address public reserveToken; // CORNDAWG
	address public rewardToken; // BITCORN
	address public boostToken; // xPERPS

	address public cornCompound;
	...
	uint256 public accRewardPerShare = 0; // cumulative reward BITCORN per CORNDAWG staked from PCS distribution algorithm
	uint256 public accDripPerShare1 = 0; // cumulative drip pool CORNDAWG per CORNDAWG staked from PCS distribution algorithm
	uint256 public accDripPerShare2 = 0; // cumulative compound drip pool CORNDAWG per CORNDAWG staked from PCS distribution algorithm
	uint256 public accBoostPerShare = 0; // cumulative boost CORNDAWG per xPERPS burned from PCS distribution algorithm
	...
	constructor(address _reserveToken, address _rewardToken, address _boostToken, address _cornCompound)
	{
		initialize(msg.sender, _reserveToken, _rewardToken, _boostToken, _cornCompound);
	}

	function initialize(address _owner, address _reserveToken, address _rewardToken, address _boostToken, address _cornCompound) public initializer
	{
		...
		accRewardPerShare = 0; // cumulative reward BITCORN per CORNDAWG staked from PCS distribution algorithm
		accDripPerShare1 = 0; // cumulative drip pool CORNDAWG per CORNDAWG staked from PCS distribution algorithm
		accDripPerShare2 = 0; // cumulative compound drip pool CORNDAWG per CORNDAWG staked from PCS distribution algorithm
		accBoostPerShare = 0; // cumulative boost CORNDAWG per xPERPS burned from PCS distribution algorithm
	...
	function setCornCompound(address _cornCompound) external onlyOwner
	{
		require(_cornCompound != address(0), "invalid address");
		cornCompound = _cornCompound;
	}
	...
	// stakes CORNDAWG on behalf of another account
	...
	function _deposit(address _sender, uint256 _amount, address _account) internal
	{
		...
		if (_sender == address(this)) {
			...
			_safeTransferFrom(reserveToken, _sender, address(this), _netAmount + _dripAmount + _1percent);
	...
	// claims all (CORNDAWG and BITCORN)
	function claimAll() external nonReentrant returns (uint256 _rewardAmount, uint256 _dripAmount1, uint256 _boostAmount)
	{
		...
		if (_rewardAmount > 0) {
			...
			IERC20(rewardToken).safeApprove(cornCompound, _feeAmount);
			CornCompound(cornCompound).donateDrip2(_feeAmount);
	...
	// compounds all (CORNDAWG and BITCORN)
	function compoundAll() external nonReentrant returns (uint256 _rewardAmount, uint256 _dripAmount1, uint256 _dripAmount2, uint256 _boostAmount)
	{
		...
		if (_rewardAmount > 0) {
			IERC20(rewardToken).safeApprove(cornCompound, _rewardAmount);
			CornCompound(cornCompound).depositOnBehalfOf(_rewardAmount, msg.sender);
		}
	...
	// sends BITCORN to all stakers (avoids reentrancy)
	function __rewardAll(uint256 _amount) external
	{
		require(msg.sender == cornCompound, "access denied");
	...
	// sends CORNDAWG to the drip pool
	...
	// performs the daily distribution from the drip pool (CORNDAWG)
	...
	function _updateDay() internal
	{
		...
		// claims BITCORN rewards from CORNDAWG
		{
