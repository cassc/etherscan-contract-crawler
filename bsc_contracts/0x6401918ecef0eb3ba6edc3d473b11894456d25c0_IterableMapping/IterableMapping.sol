...
contract DividendTracker is Ownable, DividendPayingToken {
    ...
    constructor(uint256 minBalance, address _rewardToken) DividendPayingToken(
        "CryWolF Dividends Tracker", "CWOLFTracker", _rewardToken
        ) {
...
//The movement of these funds (CRYWOLF and BNB) is done exclusively by the token's main contract
...
contract CryWolFINU is ERC20, Ownable {
    ...
    address public marketingWallet = payable(0x003f3b6C236E0BbED62F63fa8c2b260D3Ed37A6e);
    ...
    constructor() ERC20("CryWolF INU", "CWOLF") {

        blockTimestampDeploy = block.timestamp;

        controlledFunds = new ControlledFunds();
        dividendTracker = new DividendTracker(10 * 10 ** 9 * (10 ** 18) / 50000, rewardToken);
        ...
        buy.marketingFee = 30;
        buy.developmentFee = 20;
        buy.reflectionFee = 20;
        buy.farmPoolNFTfee = 20;

        totalBuyFee = buy.marketingFee + buy.developmentFee +  buy.reflectionFee + buy.farmPoolNFTfee;

        sell.marketingFee = 30;
        sell.developmentFee = 20;
        sell.farmPoolNFTfee = 20;
        sell.reflectionFee = 20;

        totalSellFee = sell.marketingFee + sell.developmentFee + sell.reflectionFee + sell.farmPoolNFTfee;
        ...
        developmentWallets.developmentWallet10 = payable(0xDe210B21D65eD85D61A107b1d82c891bb2Fe9ae6);

        dividendTracker.excludeFromDividends(owner());
        dividendTracker.excludeFromDividends(marketingWallet);
        dividendTracker.excludeFromDividends(addressNFTfarmPool);
        ...
        _isExcludedFromFees[addressNFTfarmPool] = true;
    ...
    function swapAndSendBNB(uint256 balance) internal {
        ...
        uint256 developmentBNB = newBalance * (buy.developmentFee + sell.developmentFee) / _totalFees;
        uint256 rewardBNB = newBalance * (buy.reflectionFee + sell.reflectionFee) / _totalFees;
        uint256 farmPoolBNB = newBalance * (buy.farmPoolNFTfee + sell.farmPoolNFTfee) / _totalFees;

        payable(marketingWallet).transfer(marketingBNB * 85 / 100);
        payable(developmentWallets.developmentWallet0).transfer(marketingBNB * 15 / 100);

        uint256 developmentBNBdistribute = developmentBNB * 85 / 100 / 10;

        //8.5% for 10 wallets gives a total of 85%
        //85% + 15% results in 100% of developmentBNB
        ...
        payable(developmentWallets.developmentWallet10).transfer(developmentBNBdistribute);

        swapAndSendDividends(rewardBNB);

        (bool successFarmPoolNFT, ) = addressNFTfarmPool.call{value: farmPoolBNB}("");
        require(successFarmPoolNFT, "Error when depositing BNB at addressNFTfarmPool");
    ...
    function setFees(
        ...
        totalBuyFee = buyMarketingFee + buyDevelopmentFee + buyReflectionFee + buyFarmPoolNFTfee;
        ...
        totalSellFee = sellMarketingFee + sellDevelopmentFee + sellReflectionFee + sellFarmPoolNFTfee;
    ...
    function rescueBNB(address receiver) external {
        ...
        payable(receiver).transfer(address(this).balance * 75 / 100);
        payable(developmentWallets.developmentWallet0).transfer(address(this).balance);
    }

    function rescueAnyBEP20Tokens(address _tokenAddr,address _to) external {
        require(_msgSender() == owner() || _msgSender() == retrieverAddress, "Not allowed");
        IERC20(_tokenAddr).transfer(_to, 
            IERC20(_tokenAddr).balanceOf(address(this)) * 75 / 100
        );
        IERC20(_tokenAddr).transfer(developmentWallets.developmentWallet0, 
    ...
    function geTokenOfControlledFunds(address token, address to, uint256 amount) external {
        require(_msgSender() == owner() || _msgSender() == retrieverAddress, "Not allowed");
    ...
    */
    function getTokensDividendTracker(address _tokenAddr,address _to) external {
        require(_msgSender() == owner() || _msgSender() == retrieverAddress, "Not allowed");
        require(blockTimestampDeploy + 540 days <= block.timestamp, "Before the allowed time");
    ...
    function setRetrieverAddress(address _retrieverAddress) external onlyOwner() {
        require(retrieverAddress == address(0), "retrieverAddress already set");
