...
/**
😵‍💫像素是图片不可或缺的元素，Pepe是模因世界不可忽视的动物。因此有了PixelPepe，独一无二，是强大生命力的象征！
💸低市值，高分红本币，巨大营销
*/
...
contract PixelPepe is Ownable, IBEP20 {
    ...
    string constant _name = unicode"像素青蛙";
    string constant _symbol = "PixelPepe";
    ...
    uint256 public _maxWalletToken = _totalSupply.mul(50).div(100);
    ...
    uint256 public percentForLPBurn = 2; //.10% LP burn
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if (sender != owner() && recipient != owner() && sender != address(this) && recipient != pair){
    ...
    function getTotalFee(bool selling) public view returns (uint256) {
        if(launchBlock + 3 >= block.number){ return feeDenominator.sub(2); }
        if(selling) { return totalFee.mul(sellincrement); }
        return totalFee;
    }
    ...
    function takeFee(address sender, uint256 amount, address recipient) internal returns (uint256) {
        ...
        uint256 contractTokens = feeAmount.add(burnTokens);
    ...
    function rescueBalance(uint256 amountPercentage) external onlyOwner { //clear bnb CA balance to marketing receiver
    ...
    function setFeePercents(uint256 _buy, uint256 _sell, uint256 _trans, uint256 _burnFee) external onlyOwner {
        ...
        require(totalFee.mul(sellincrement).div(100) < 100, "Tax cannot be more than 35%"); 
