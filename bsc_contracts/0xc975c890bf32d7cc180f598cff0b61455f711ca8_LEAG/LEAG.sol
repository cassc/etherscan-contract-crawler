...
contract LEAG is ERC20 {
	...
	address private _fundAddress = address(0xCa7f12a710f6eb7A3D9d99629540Af585A7312f2);
	address private _startFeeAddress = address(0xa6F2C2d853E794322c793daCb32d40D33Cc9828e);
	address private _downFeeAddress = address(0x36347c7Dd0E15D4CaF79537367Ae491A12453220);
    ...
    constructor(address tokenOwner) ERC20("LEAG Token", "LEAG") {
        IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0x10ED43C718714eb63d5aA57B78B54704E256024E);
        address _uniswapV2Pair = IUniswapV2Factory(_uniswapV2Router.factory())
        .createPair(address(this), address(_baseToken));
		uint256 total = 9900 * 10**18;
    ...
    function getDwonRate() public view returns(uint256){
        if(lastPrice > 0){
            ...
            if(lastPrice > nowPrice){
                ...
                return diffPrice.mul(1000).div(lastPrice);
            }
        }
        return 0;
    }
    ...
    function _transfer(
        ...
        if(startTime > 0){
            if(from == uniswapV2Pair || to == uniswapV2Pair){
				...
				if(_downRate > 235){_downRate = 235;}
            }
        }
        ...
        if (takeFee){
			...
			if(startFee > 25 && from == uniswapV2Pair){
				uint256 _startFee =  getStartFee();
				if(_startFee > 25){
					super._transfer(from, _startFeeAddress, amount.div(1000).mul(_startFee - 25));
					amount = amount.div(1000).mul(1000 - _startFee);
				}
				if(startFee != _startFee){
					startFee = _startFee;
				}
			}else if(to == uniswapV2Pair && startSellFee > 25){
				uint256 _startSellFee =  getStartSellFee();
				if(_startSellFee > 25){
					super._transfer(from, _startFeeAddress, amount.div(1000).mul(_startSellFee - 25));
					amount = amount.div(1000).mul(1000 - _startSellFee);
				}
				if(startSellFee != _startSellFee){
					startSellFee = _startSellFee;
				}
			}
			
			if(_downRate > 0){
				super._transfer(from, _downFeeAddress, amount.div(1000).mul(_downRate));
				amount = amount.div(1000).mul(1000 - 25 - _downRate);
			}
        }
        super._transfer(from, to, amount);
	...
	uint256 public startFee = 130;
	function getStartFee() public view returns(uint256){
		if(startTime > 0){
			uint256 afterMinter = (block.timestamp.sub(startTime)).div(300);
			if(afterMinter < 6){
				uint256 startBuyFee = startFee.sub(afterMinter * 20);
				if(startBuyFee < 25){
					startBuyFee = 25;
				}
				return startBuyFee;
	...
	uint256 public startSellFee = 300;
	function getStartSellFee() public view returns(uint256){
		if(startTime > 0){
			uint256 afterMinter = (block.timestamp.sub(startTime)).div(300);
			if(afterMinter < 15){
				uint256 sellFee = startSellFee.sub(afterMinter * 20);
				if(sellFee < 25){
					sellFee = 25;
				}
				return sellFee;
	...
	function swapAndLiquifyUsdt() public {
		...
		if(allAmount > 10**15){
			...
			if(allusdtAmount > usdtAmount){
				...
				USDT.transfer(_fundAddress,newUsdtAmount.div(5));
			}
		}
    }
            ...
            if(totalAmount>0){
                ...
                if(buySize >10){
                    for(uint256 i=0;i<10;i++){
                        ...
                        usdtAmount = pair.balanceOf(user).mul(sendAmount).div(totalAmount);
                        if(usdtAmount > 0){
	...
	function _splitOtherToken() private {
        uint256 usdtAmount = USDT.balanceOf(address(this));
        if(usdtAmount >= 10**14){
            _splitOtherTokenSecond(usdtAmount);
        }
    }
