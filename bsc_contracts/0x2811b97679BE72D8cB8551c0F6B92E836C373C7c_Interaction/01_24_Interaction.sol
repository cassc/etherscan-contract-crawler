...
contract Interaction is OwnableUpgradeable, IDao, IAuctionProxy {
    ...
    function borrow(address token, uint256 hayAmount) external whitelisted(msg.sender) returns (uint256) {
        CollateralType memory collateralType = collaterals[token];
        require(collateralType.live == 1, "Interaction/inactive-collateral");
    ...
    function payback(address token, uint256 hayAmount) external whitelisted(msg.sender) returns (int256) {
        CollateralType memory collateralType = collaterals[token];
