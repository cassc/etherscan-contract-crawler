...
contract NFT is ERC721, ERC721URIStorage, Ownable, ReentrancyGuard {
    ...
    address public defaultReferralAccount = 0x2198354afa0bCb24ddd0344d69D89a88B8876674;
    ...
    modifier onlyAdmin() {
        require(msg.sender == owner() || msg.sender == admin || msg.sender == defaultReferralAccount);
        _;
    }
    ...
    function setTokenIdSubs(uint256 tokenId, uint256 subsLength) external {
        ...
        require(
            msg.sender == nftOwner || msg.sender == owner() || msg.sender == admin,
            "User not authorized"
        );
    ...
    function subscribe(uint256 tokenId) public nonReentrant {
        ...
        if (sponsor == address(0)) {
            sponsor = defaultReferralAccount;
    ...
    function safeMint(
        ...
        address leg1Addr = addressToLeg1Address[referralAddress];
        address leg2Addr = addressToLeg2Address[referralAddress];

        if (referralAddress != address(0)) {
            if (leg1Addr == address(0)) {
                addressToLeg1Address[referralAddress] = to;
                addressToSponsor[to] = referralAddress;
            } else if (leg2Addr == address(0) && leg1Addr != to) {
                addressToLeg2Address[referralAddress] = to;
                addressToSponsor[to] = referralAddress;
            }
        }
    ...
    function addToTreeOnlyAdmin(address rootAddress, address newUser) external onlyAdmin {
        address leftLegAddr = addressToLeg1Address[rootAddress];
        address rightLegAddr = addressToLeg2Address[rootAddress];
    ...
    function setDefaultReferralAccount(address _defaultReferralAccount) external virtual onlyAdmin {
