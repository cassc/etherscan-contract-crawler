    ...
    function withdraw(address payable beneficiary) public payable nonReentrant {
      ...
      for (uint256 i = 0; i < _users.getTotalSubscription(beneficiary); i++) {
        ...
        if (subs.active) {
          ...
          if (poolAmount > 0 || vestingAmount > 0) {
            withdrawAmount += calculateAmountForWithdraw(subs.releasedUsdAmount, subs.takenFromPoolUsd, subs.valueUsd, (vestingAmount+poolAmount));
          }       
        }
      }
      if (withdrawAmount > 0) {
