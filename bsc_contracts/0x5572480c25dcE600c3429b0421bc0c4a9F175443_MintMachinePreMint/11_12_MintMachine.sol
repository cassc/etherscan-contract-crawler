...
contract MintMachine is ReentrancyGuard, Pausable
{
	...
	//function make_reward(uint256 _prob_e6, bool _is_nft, address _address_token_reward, uint256 _amount_or_grade) external onlyOperator whenNotPaused
	function make_reward(uint256 _prob_e6, bool _is_nft, address _address_token_reward, uint256 _amount_or_grade) external
	{
	...
	function gacha(uint256 _amount) external nonReentrant
	{
		...
		for(uint256 i=0; i<gacha_count; i++)
		{
			...
			cur_atari_amount += _deploy_reward(address_user);
		}
	...
	function _deploy_reward(address _address_receiver) internal returns(uint256)
	{
	...
	function set_mint_price(address _address_deposit_token, uint256 _new_mint_price) external
	{
