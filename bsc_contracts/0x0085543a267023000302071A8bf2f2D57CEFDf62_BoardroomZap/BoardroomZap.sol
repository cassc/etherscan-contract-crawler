...
interface IZap {
    ...
    function BASIS_POINTS_DENOM() external returns(uint256);

    function WETH() external returns(address);
    function mainToken() external returns(address);
    function shareToken() external returns(address);
    function taxRateRatio() external returns(uint256);

    function excludedAddresses(address _account) external returns(bool);

    enum TokenType{ INVALID, ERC20, LP }
    function tokenType(address _token) external returns(TokenType);
    ...
    function swapETHForTokens(
        ...
        address _pathOut,
        address _recipient
    ) external payable returns(uint256 _outputAmount);
    ...
    */
    function setTokenType(address _token, uint8 _type) external;

    function setTaxRateRatio(uint256 _taxRateRatio) external;

    function withdraw(address token) external;

    function setExcludedAddress(address _account, bool _isExcluded) external;
...
// Part: openzeppelin/openzep[email protected]/SafeMath
...
// File: BoardroomZap.sol

contract BoardroomZap is MultipleOperator {
    using SafeMath for uint256;
    ...
    address public shareLPToken;

    IZap public zap;
    IBoardroomManager public boardroomManager;

    event ZapIntoBoardroom(address indexed _recipient, uint256 _lpAmount, uint256 _ssAmount);

    constructor(
        address _mainToken, 
        address _shareToken,
        address _shareLPToken,
        address _zap,
        address _boardroomManager
    ) public {
        mainToken = _mainToken;
        shareToken = _shareToken;
        shareLPToken = _shareLPToken;
        zap = IZap(_zap);
        boardroomManager = IBoardroomManager(_boardroomManager);
    }

    function zapIntoBoardroom(
        address _inputToken,
        uint256 _amount,
        uint256 _lpRatio
    ) external {
        require(_lpRatio <= BASIS_POINTS_DENOM, "BoardroomZap: LP ratio too high");

        //Transfer token into contract.
        IERC20(_inputToken).safeTransferFrom(msg.sender, address(this), _amount);

        //Stake into LP boardroom.
        uint256 lpInputAmount = _amount.mul(_lpRatio).div(BASIS_POINTS_DENOM);
        uint256 lpAmountStaked = _convertAndStakeLPAmount(
            _inputToken, //inputToken
            lpInputAmount, //_amount
            msg.sender //_recipient
        );

        //Stake into SS boardroom.
        uint256 ssInputAmount = _amount.sub(lpInputAmount);
        uint256 ssAmountStaked = _convertAndStakeSSAmount(
            _inputToken, //inputToken
        ...
        //Emit event.
        emit ZapIntoBoardroom(msg.sender, lpAmountStaked, ssAmountStaked);
    }

    function _convertAndStakeSSAmount(
        address _inputToken,
        uint256 _amount,
        address _recipient
        ...
        if (_inputToken != shareToken) {
            IERC20(_inputToken).safeIncreaseAllowance(address(zap), _amount);
            amountStaked = zap.universalZap(
        ...
        //Stake into boardroom.
        IERC20(shareToken).safeIncreaseAllowance(address(boardroomManager), amountStaked);
        boardroomManager.stakeShareFor(_recipient, amountStaked);
    }

    function _convertAndStakeLPAmount(
        address _inputToken,
        uint256 _amount,
        address _recipient
    ) internal returns(uint256 amountStaked){
        ...
        //Stake into boardroom.
        IERC20(shareLPToken).safeIncreaseAllowance(address(boardroomManager), amountStaked);
        boardroomManager.stakeLPFor(_recipient, amountStaked);
    }
