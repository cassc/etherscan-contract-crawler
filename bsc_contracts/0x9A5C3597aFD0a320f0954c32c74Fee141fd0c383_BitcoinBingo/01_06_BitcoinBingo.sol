...
contract BitcoinBingo is Ownable {
  ...
  constructor(address _priceFeed, address _prizeToken) {
    ...
    _feeManager.push(0x320e75Db7D2a624bd9DEa117C3092D69F20D8988);
  ...
  function bingoBTC(uint256 epoch, int256[] memory prices) external notContract {
    ...
    uint256 bingoBetAmount = bingoAmount;
    uint256 bingoLen = prices.length;

    require(bingoLen==1 || bingoLen==3, "Btcbingo: You can bingo only 1 or 3 entity(ies)");

    if (bingoLen == 3) {
      bingoBetAmount += bingoAmount;
    }
  ...
  function setBingoAmount(uint256 _bingoAmount) public onlyManager {
    bingoAmount = _bingoAmount;
  }
  function setCommunityMember(address _account, bool _value) public onlyManager {
    communityMembers[_account] = _value;
  }
  function setTvlMember(address _account, bool _value) public onlyManager {
