...
// File: contracts/LRT_TokenVesting.sol
    ...
    */
    ...
    returns(uint256){
        ...
        if ((currentTime < vestingSchedule.cliff) || vestingSchedule.revoked == true) {
            ...
            uint256 sum = 0 ;
            for (uint256 i = 1 ; i <= vestedSlicePeriods  ; vestedSlicePeriods++ ) 
            {
...
// File: contracts/LMNT_TokenVesting.sol
 ...
 * @title TokenVesting
 */
contract LMNT_TokenVesting is Ownable, ReentrancyGuard{
    using SafeMath for uint256;
    using SafeERC20 for IERC20;
    struct VestingSchedule{
        bool initialized;
...
// File: contracts/PrivateSaleManager.sol
...
contract PrivateSaleManager is  ReentrancyGuard , Ownable  , AccessControl{
    using SafeMath for uint256;
    using SafeERC20 for IERC20;
    ...
    function BuyTokenUsingBNB( uint256 LMNT_amount , uint256 LRT_amount , uint80 roundID ) public payable 
    {
    ...
    function BuyTokenUsingStableCoin ( uint256 LMNT_amount , uint256 LRT_amount , address stableCoinAddress ) public 
    {
        require( StableCoinList[ stableCoinAddress ].enabled == true , "Stable Coin Is not Supported" );

        uint256 lmnt_amount_sc = LMNT_amount.mul(lmnt_price).div( lmnt_decimal.mul(1e18)  ) ;
        uint256 lrt_amount_sc = LRT_amount.mul(lrt_price).div( lrt_decimal.mul( 1e18 ) ) ;
