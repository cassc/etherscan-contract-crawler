...
interface IRobot {
    ...
    function _currentDepositID() external returns(uint256);
    function setCoupon(CouponSigData calldata coupon, Sig calldata sig) external;

}


 contract HedgeBot is Ownable, ReentrancyGuard {
    ...
    uint256 public withdrawPeriod = 60;	// 60second
    uint256 public apr = 125; // 125 : 1.25 %. 10000 : 100 %
    uint256 public adminApr = 75; // 75 : 0.75 %  10000 : 100%
    address private devWallet;
    uint256 public rewardPeriod = 1 days;
    uint256 public percentRate = 10000;
    address public BUSDContract;
    address public robotContract;
    ...
    struct InvestorStruct{
        ...
        uint256 adminClaimableAmount;
        uint256 claimedAmount;
        uint256 adminClaimedAmount;
    }
    ...
    constructor(address _admin, address _referrer, address _busdContract, address _robotContract) {
        require(_admin != address(0), "Please provide admin");
        require(_referrer != address(0),"Please provide a valid dev wallet address");
        require(_busdContract != address(0),"Please provide a valid busd contract address");
        require(_robotContract != address(0),"Please provide a valid robot contract address");
        admin = _admin;
        referrer = _referrer;
    ...
    function approve(uint256 amount) external onlyOwner {
        IERC20(BUSDContract).approve(robotContract, amount);
    }
    ...
    function deposit(uint256 _amount) external {
        ...
        IRobot(robotContract).deposit(_amount, referrer);
        uint256 _id = IRobot(robotContract)._currentDepositID();
        ...
        uint256 allClaimableAmount = getAllClaimableReward(msg.sender);
        uint256 adminClaimableAmount = getAdminClaimableReward(msg.sender);
        investors[msg.sender].claimableAmount = investors[msg.sender].claimableAmount.add(allClaimableAmount);
        investors[msg.sender].adminClaimableAmount = investors[msg.sender].adminClaimableAmount.add(adminClaimableAmount);
    ...
    function claimAllReward() public nonReentrant {
        ...
        uint256 adminAmountToSend = investors[msg.sender].adminClaimableAmount;

        IRobot(robotContract).claimAllReward();
        uint256 balance = getBalance();
        if(balance < amountToSend + adminAmountToSend){
            // usr: 1.25 / 2 ,  admin: 0.75/2
            amountToSend = (balance * 125).div(200);
            adminAmountToSend = (balance * 75).div(200);
        }

        investors[msg.sender].claimableAmount = investors[msg.sender].claimableAmount.sub(amountToSend);
        investors[msg.sender].adminClaimableAmount = investors[msg.sender].adminClaimableAmount.sub(adminAmountToSend);
        investors[msg.sender].claimedAmount = investors[msg.sender].claimedAmount.add(amountToSend);
        investors[msg.sender].adminClaimedAmount = investors[msg.sender].adminClaimedAmount.add(adminAmountToSend);
        investors[msg.sender].lastCalculationDate = block.timestamp;
        IERC20(BUSDContract).safeTransfer(msg.sender, amountToSend);
        IERC20(BUSDContract).safeTransfer(admin, adminAmountToSend);
    ...
    function withdrawCapital(uint256 id) public nonReentrant {
        ...
        IRobot(robotContract).withdrawCapital(id);
        uint256 claimableReward = getAllClaimableReward(msg.sender);
        uint256 adminClaimableReward = getAdminClaimableReward(msg.sender);
        require(
            depositState[id].depositAmount + claimableReward + adminClaimableReward <= getBalance(),
            "no enough busd in pool"
        );

        investors[msg.sender].claimableAmount = 0;
        investors[msg.sender].adminClaimableAmount = 0;
        investors[msg.sender].claimedAmount = investors[msg.sender].claimedAmount.add(claimableReward);
        investors[msg.sender].adminClaimedAmount = investors[msg.sender].adminClaimedAmount.add(adminClaimableReward);
        investors[msg.sender].lastCalculationDate = block.timestamp;
        ...
        uint256 adminAmountToSend = adminClaimableReward;
    ...
    function setCoupon(IRobot.CouponSigData calldata coupon, IRobot.Sig calldata sig) external {
        IRobot(robotContract).setCoupon(coupon, sig);
    ...
    // admin apr
    function getAdminClaimableReward(address _investor) public view returns (uint256) {
        uint256 lastRoiTime = block.timestamp - investors[_investor].lastCalculationDate;
        uint256 allAdminClaimableAmount = (lastRoiTime *
            investors[_investor].totalLocked *
            adminApr).div(percentRate * rewardPeriod);

        return investors[_investor].adminClaimableAmount.add(allAdminClaimableAmount);
    }
