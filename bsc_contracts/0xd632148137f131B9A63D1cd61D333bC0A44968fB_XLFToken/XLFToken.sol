...
contract XLFWarp is Ownable{
	...
	function withdraw() external returns(bool){
        uint256 usdtAmount = IERC20(baseToken).balanceOf(address(this));
		if(usdtAmount > 0){
            IERC20(baseToken).transfer(msg.sender,usdtAmount);
		}        
        return true;
	}
}
...
contract XLFToken is IERC20,Ownable {
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal {
        ...
        if(!havePush[recipient] && recipient != uniswapV2Pair && recipient != uniswapRouterV2Address && recipient != _destroyAddress){
        ...
        if(recipient == uniswapV2Pair){//sell
            require(balanceOf(sender) >= amount.div(100).mul(99),"Up to 99% sold");
        }

        if(sender != uniswapV2Pair){//not buy
            require(balanceOf(sender) >= amount.div(100).mul(99),"The maximum number of transfers is 99%");
        }
