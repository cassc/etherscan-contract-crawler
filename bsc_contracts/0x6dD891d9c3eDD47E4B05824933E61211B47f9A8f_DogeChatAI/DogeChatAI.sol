...
//DogechatAI is a cryptocurrency project that combines artificial intelligence 
//with blockchain technology. 4% rewards in USDT for holders.
  
 🟣 Website: http://dogechatai.com
 🟣 Telegram: https://t.me/DogechatAI
...
contract DogeChatAI  is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    DogeChatAIDividendTracker public dividendTracker;
    ...
    uint256 public cooldowntimer = 30; //default cooldown 30 seconds
    ...
    constructor() ERC20("DogeChatAI", "DCTAI") {
        marketingWallet = payable(0x5218780779CAc19e2dC3Da6B592817280c8738E7);
        devWallet = payable(0x3AA7A0290cCDC51824D093590afA571d25695bf5);
        address router = 0x10ED43C718714eb63d5aA57B78B54704E256024E; //0x10ED43C718714eb63d5aA57B78B54704E256024E ,0xD99D1c33F9fC3444f8101754aBC46c52416550D1
        ...
        dividendTracker = new DogeChatAIDividendTracker(
            payable(this),
            router,
        ...
        swapTokensAtAmount = totalTokenSupply / 50; //
    ...
    // set cooldown timer, can only be between 0 and 30 seconds (30 seconds max)
    function setcooldowntimer(uint256 value) external onlyOwner {
        require(value <= 30, "cooldown timer cannot exceed 30 seconds");
        cooldowntimer = value;
    }
    ...
    function setmaxWallet(uint256 value) external onlyOwner {
        value = value * (10**18);
        require(value >= _totalSupply / 1000, "max wallet cannot be set lower");
        maxWallet = value;
    }
    ...
    function updateTransferFee(uint256 newTransferFee) public onlyOwner {
        require (newTransferFee <= 75, "transfer fee cannot exceed 75%");
    ...
    function updateFees(
        ...
        require(totalSellFees <= 20 && totalBuyFees <= 5, "total fees cannot exceed 20% sell or 5% buy");
...
contract DogeChatAIDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    DogeChatAI public DogeChatAIContract;
    ...
    constructor(
        ...
        defaultToken = token;
        DogeChatAIContract = DogeChatAI(mainContract);
        minimumTokenBalanceForAutoDividends = 200000000000_000000000000000000; // 200;000,000,000 tokens
    ...
    function _transfer(
        ...
        require(false, "DogeChatAI_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "DogeChatAI_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main DogeChatAI contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(DogeChatAIContract);

            uint256 prevBalance = DogeChatAIContract.balanceOf(address(this));
            ...
            {
                uint256 received = DogeChatAIContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    DogeChatAIContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
