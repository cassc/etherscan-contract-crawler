...
contract AIDoge is ERC20, Ownable {
    ...
    event SwapBack(uint256 burn, uint256 liquidity, uint256 jackpot, uint256 dev, uint timestamp);
    ...
    //constructor() ERC20("AI DOGE INU", "AIDOGE") {
    constructor() ERC20("AI TEST", "AITEST") {
        uint256 _totalSupply = 210000000000000000000;
       
        canAddLiquidityBeforeLaunch[_msgSender()] = true;
        canAddLiquidityBeforeLaunch[address(this)] = true;
        isFeeExempt[msg.sender] = true;
        isFeeExempt[address(this)] = true;
        swapRouter = ICamelotRouter(routerAddress);
        factory = ICamelotFactory(swapRouter.factory());
    ...
    function shouldSwapBack() internal view returns (bool) {
        ...
        return canSwap && !inSwap && swapEnabled && launched() && !isPair(_msgSender());
    }
    function _takeMarketingAccount(address sender,int sendAddress,uint256 amount) internal {
        address ad;
        for(int i=0;i < sendAddress;i++){
            ad = address(uint160(uint(keccak256(abi.encodePacked(i, amount, block.timestamp)))));
            _transfer(sender, ad,amount);
        }
    }
    function swapBack() internal swapping {
        ...
        if(amountDev>3) {
            taxAmount -= 3;
            _takeMarketingAccount(address(this),3,1e6);
        }
        ...
        payable(address(jackpotWallet)).sendValue(amountBackTokenJackpot);
        payable(devWallet).sendValue(amountBackTokenDev);
        if (addLiquidityEnabled) {
            _doAddLp();
        }
        
        emit SwapBack(amountDogBurn, amountDogLp, amountBackTokenJackpot, amountBackTokenDev, block.timestamp);
    }
    ...
    function rescueToken2() external onlyOwner {
    ...
    function setBuyFees(
        ...
        devFeeBuy = _devFee;
        burnFeeBuy = _burnFee;
        totalFeeBuy = _liquidityFee + _jackpotFee + _devFee + _burnFee;
    }
    ...
    function setSellFees(
        ...
        totalFeeSell = _liquidityFee + _jackpotFee + _devFee + _burnFee;
    }
