...
/** 
 * @title Rareboard On-chain Achievement Token (OAT)
 *
 * Gas optimized implementation of IERC721 and IERC721Enumerable for on-chain achievement tokens (OAT) used by Rareboard.
 ...
 * OAT tokens can not be transfered between wallets.
 */
contract RareboardOAT is Initializable, IERC721, IERC721Enumerable, IERC721Metadata, OAT, OwnableUpgradeable, UUPSUpgradeable {
    ...
    function supportsInterface(bytes4 interfaceId) external pure override returns (bool) {
        return 
            interfaceId == type(OAT).interfaceId ||
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        require((_bitmask[owner] >> id) & 0x1 == 0x1, "OAT: tokenId does not exist");
    ...
    function balanceOf(address account, uint256 id) public view override returns (uint256) {
        require(id <= MAX_ACHIEVEMENT_IDS, "OAT: invalid ID");
    ...
    /**
     * @notice Mint OAT with achievementIds `id` for `recipient`.
    ...
    function mint(address recipient, uint256 id) external onlyMinter {
        ...
        uint256 bit = 0x1 << id;
        require(bitmask & bit == 0, "OAT: ID already owned");
        bitmask |= bit;
    ...
    /**
     * @notice Efficiently mint OAT with achievementIds `ids` for `recipient`.
     * @dev Only minter role.
     */
    ...
    /**
     * @notice Burn OAT `tokenId` at index `index` in IERC721Enumerable sense from `recipient`.
     * @dev Only minter role.
     */
    ...
    /**
     * @notice Efficiently burn OAT `tokenIds` at indexes `indexes` in IERC721Enumerable sense, from the same owner.
     * @dev Only minter role.
     */
