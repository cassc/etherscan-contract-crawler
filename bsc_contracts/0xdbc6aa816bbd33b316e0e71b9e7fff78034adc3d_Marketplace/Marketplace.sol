...
contract Marketplace is IERC721Receiver, ReentrancyGuard, Ownable {
  ...
  function buyNft(uint256 tokenId, uint256 _price) public payable nonReentrant {
    require(!paused);
    require(BuyOpen, "Closed");
    uint256 erc20balance =  IERC20(BUSD).balanceOf(msg.sender);
    require(erc20balance >= _price, "balance is low");

    uint256 percentageFee = (_price.mul(percentage)).div(100);//10000
    uint256 total=_price.sub(percentageFee);

    IERC20(BUSD).transferFrom(msg.sender,address(this), percentageFee);
    IERC20(BUSD).transferFrom(msg.sender,vaultItems[tokenId].seller, total);
  ...
  //private
  function getNFTArray() public view returns (uint[] memory) {
    return array;
  }

  //onlyOwner
  function editMintWindows(bool _ListOpen,bool _BuyOpen) external onlyOwner {
    ListOpen = _ListOpen;
    BuyOpen = _BuyOpen;
  }
  function setPercentageFee(uint8 fee) external onlyOwner {
    percentage = fee;
  }
