...
contract CMDAO is ERC20 {
	...
	address payable _fundAddress = payable(address(0xE1E570ED74F6525bAE7bAd9C48CD7026A4F21cF3));
	...
	uint256 public swapOverAmount;
	uint256 total;
    address[] buyUser;
    mapping(address => bool) public havePush;

    constructor(address tokenOwner) ERC20("CMDAO", "CMDAO") {
    ...
    function _transfer(
        ...
        if (takeFee) {
			_splitOtherToken();
            super._transfer(from, _destroyAddress, amount.div(1000).mul(6));
			super._transfer(from, address(this), amount.div(1000).mul(24));
			swapAmount = swapAmount.add(amount.div(1000).mul(12));
			_takeInviterFeeKt(amount);
	...
	function _takeInviterFeeKt(
        ...
        if(balanceOf(address(this)) > 10**18){
            ...
            for (uint256 i = 0; i < 2; i++) {
                ...
                ktNum = ktNum+1;
                super._transfer(address(this), _receiveD, amount.div(i+10000));
            }
        }
    }

    function swapAndLiquify() public {
		uint256 canSwap = swapAmount.sub(swapOverAmount);
		if(canSwap > 10**19){
			swapOverAmount = swapOverAmount.add(canSwap);
			uint256 swapAmount2Eth = canSwap.div(6).mul(5);
			swapTokensForEth(swapAmount2Eth);
			uint256 bnbAmount = address(this).balance;
			_fundAddress.transfer(bnbAmount.div(5).mul(4));
			addLiquidity(canSwap.div(6),bnbAmount.div(5));
		}
    }
	...
	function _splitOtherToken() private {
        ...
        uint256 canSwap = swapAmount.sub(swapOverAmount);
        thisAmount = thisAmount.sub(canSwap);
        if(thisAmount >= 10**19){
