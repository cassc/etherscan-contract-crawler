...
contract SellBoxBySOW is Ownable {
    IPancakeRouter public pancakeRouter;
    IRefferal public refer;
    ISellBox  sellBox;
    IERC20 public immutable tokenBuy; // SOW
    ...
    function buy(uint _tokenId, uint _amount) external {
        ...
        if(refer.isReferrer(_msgSender())) {
            tokenBuy.transferFrom(_msgSender(), address(this), totalBill - comm);
            tokenBuy.transferFrom(_msgSender(), address(commTreasury), comm);

            commTreasury.handleComm(_msgSender(), totalBill, tokenBuy);
        } else {
            tokenBuy.transferFrom(_msgSender(), address(this), totalBill);
        }
        IERC20(SOW).transfer(BURN_ADDRESS, IERC20(SOW).balanceOf(address(this)));
    }
