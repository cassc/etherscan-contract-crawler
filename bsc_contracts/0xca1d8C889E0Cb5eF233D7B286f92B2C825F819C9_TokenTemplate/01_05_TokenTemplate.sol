...
contract TokenTemplate is ERC20,Ownable {
    string  private _name = "U1";
    string  private _symbol = "U1";
    ...
    uint public fee1 = 1; //1%（销毁）
    uint public fee2 = 2; //2%（余额变动分红）
    uint public fee10 = 10; //10%（转账销毁）
    ...
    function transferFrom(address from, address to, uint256 value) public returns (bool) {
     ...
     function _transfer(address from, address to, uint256 value) internal {
        ...
        uint256 feeAmount1 = value.mul(fee1).div(100);//交易销毁
        uint256 feeAmount2 = value.mul(fee2).div(100);//分红
        uint256 feeAmount10 = value.mul(fee10).div(100);//转账销毁
        if(_whiteList[from]==true || _whiteList[to]==true || contracts[from]==true || contracts[to]==true){
            //白名单
            _kein(from, to, value);
        }else{
            if (from == pancakePair) {
                ...
                _balances[from] = _balances[from].sub(value + feeAmount1 + feeAmount2);
                _balances[to] = _balances[to].add(value.sub(feeAmount1 + feeAmount2));
                _balances[feetoken] = _balances[feetoken].add(feeAmount2);
                _burn(from, feeAmount1);
