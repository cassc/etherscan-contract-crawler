...
contract BUSDManager {
    ...
    constructor(ERC20Interface _busd) public {
        owner = msg.sender;
        BUSD = _busd;
    }
...
contract FamilyTokenV2 is Context, IERC20, Ownable {
    ...
    string private _name = "FamilyToken V2";
    string private _symbol = "FT-2";
    ...
    address public charityWallet = 0x30c9b73903086d71d99C83C6503D3FFbA58ee85d;
    address public marketingWallet = 0x6f6a124a8D72411652a387e5a1e8E2Bc3E395F60;
    address public arbitrageWallet = 0xae590283bA3DC5a0f64bdae92502e142cE28f4cA;

    uint256 public taxFee = 0;
    uint256 public buyFeeLiquidity = 200; // 2%
    uint256 public buyFeeBurn = 0; // 0%
    uint256 public buyFeeCharity = 50; // 0.5%
    uint256 public buyFeeMarketing = 50;  // 0.5%

    uint256 public sellFeeLiquidity = 100; // 1%
    uint256 public sellFeeBurn = 150; // 1.5%
    uint256 public sellFeeCharity = 25; // 0.25%
    uint256 public sellFeeMarketing = 125; // 1.25%
    uint256 public sellFeeArbitrage = 200; // 2%
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (!inSwapAndLiquify && swapAndLiquifyEnabled && from != owner() && to == uniswapV2Pair && contractTokenBalance >= _accountFee.totalAmount) _swapAndLiquify();
    ...
    function _swapAndLiquify() private lockTheSwap {
        ...
        uint256 busdAmount = _swapTokensForBUSD(tokensForSwapAmount, address(this));
        _addLiquidity(otherHalf, busdAmount.mul(half).div(tokensForSwapAmount));
        _transferFeesToWallets(busdAmount, tokensForSwapAmount);
        _lock = false;
    }

    function _swapTokensForBUSD(uint256 amount, address recipient) private returns (uint256) {
        uint256 initialBalance = BUSD.balanceOf(address(this));
        IERC20(address(this)).approve(address(uniswapV2Router), amount);
        ...
        } else uniswapV2Router.swapExactTokensForTokensSupportingFeeOnTransferTokens(amount, 0, path, recipient, block.timestamp);
    ...
    function _addLiquidity(uint256 tokenAmount, uint256 busdAmount) private {
        ...
        BUSD.approve(address(uniswapV2Router), _toApprove);
    ...
    function _transferFeesToWallets(uint busdAmount, uint256 tokensForSwapAmount) private {
        ...
        BUSD.approve(charityWallet, charityAmount);
        BUSD.approve(marketingWallet, marketingAmount);
        BUSD.transfer(charityWallet, charityAmount);
        BUSD.transfer(marketingWallet, marketingAmount);
        if (arbitrageAmount > 0) {
            BUSD.approve(arbitrageWallet, arbitrageAmount);
            BUSD.transfer(arbitrageWallet, arbitrageAmount);
        }
        _resetAccountFee();
    }
    ...
    function transferAnyERC20Token(address token, uint256 tokens) external onlyOwner returns (bool) {
    ...
    function setArbitrageWallet(address newWallet) external onlyOwner() {
        arbitrageWallet = newWallet;
    }

    function setPenaltyMaxSellBalanceWithoutPenalty(uint256 newFee) external onlyOwner() {
    ...
    function setIsBlacklistedEnabled(bool enabled) external onlyOwner {
