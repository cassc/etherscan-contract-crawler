...
contract VotingEscrow is Ownable, ReentrancyGuard {
    ...
    modifier onlyOwnerOrWhitelist() {
        if (msg.sender != owner()) {
            require(contracts_whitelist[msg.sender], "Not whitelisted");
        }
        _;
    }
    ...
    /// @param _depositor Depositor's wallet address
    /// @param _addr User's wallet address
    /// @param _value Amount to deposit
    /// @param unlock_time New time when to unlock the tokens, or 0 if unchanged
    /// @param locked_balance Previous locked amount / timestamp
    /// @param deposit_type The type of deposit
    function _deposit_for(address _depositor, address _addr, uint _value, uint unlock_time, LockedBalance memory locked_balance, DepositType deposit_type) internal {
        ...
        if (_value != 0) {
            IERC20(token).safeTransferFrom(_depositor, address(this), _value);
        }
    ...
    function _create_lock(address _depositor, address _addr, uint _value, uint _unlock_time) internal {
        ...
        _deposit_for(_depositor, _addr, _value, unlock_time, _locked, DepositType.CREATE_LOCK_TYPE);
    }
    ...
    function create_lock(uint _value, uint _unlock_time) external nonReentrant onlyUserOrWhitelist notUnlocked {
        _create_lock(msg.sender, msg.sender, _value, _unlock_time);
    }
    ...
    /// @param _value Amount to deposit
    /// @param _unlock_time Epoch time when tokens unlock, rounded down to whole weeks
    function create_lock_for(address _addr, uint _value, uint _unlock_time) external nonReentrant onlyOwnerOrWhitelist notUnlocked {
        _create_lock(msg.sender, _addr, _value, _unlock_time);
    }
    ...
    function _increase_amount(uint _value) internal {
        ...
        _deposit_for(msg.sender, msg.sender, _value, 0, _locked, DepositType.INCREASE_LOCK_AMOUNT);
    }
    ...
    function _increase_unlock_time(uint _unlock_time) internal {
        ...
        _deposit_for(msg.sender, msg.sender, 0, unlock_time, _locked, DepositType.INCREASE_UNLOCK_TIME);
    }
