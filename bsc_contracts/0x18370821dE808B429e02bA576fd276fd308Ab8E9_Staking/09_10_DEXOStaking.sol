...
contract Staking is Initializable, OwnableUpgradeable{
    ...
    function unStake(uint256 _stake, StakingPeriod _stakePeriod) public {
        ...
        uint256 _investorReward = 0;
        uint256 _unstake = _stake;

        if(validateStakingPeriod(goldTempStake) && goldTempStake.amount > 0){
            if(_stake >= goldTempStake.amount){
                _investorReward += getGoldRewards(_stake, _stakePeriod);
        ...
        if(validateStakingPeriod(silverTempStake) && silverTempStake.amount > 0){
            if(_stake >= silverTempStake.amount){
                _investorReward += getSilverRewards(_stake, _stakePeriod);
        ...
        if(validateStakingPeriod(newTempStake) && newTempStake.amount > 0){
            if(_stake >= newTempStake.amount){
                _investorReward += getNormalRewards(_stake, _stakePeriod);
        ...
        totalStake = totalStake.sub(_unstake);
    ...
    function getDailyRewards(StakingPeriod _stakePeriod) public view returns (uint256) {
        ...
        if(normalTempStake.amount > 0){
            normal_reward = validateStakingPeriod(normalTempStake)?getInvestorRewards(normalTempStake.amount, _stakePeriod, normalTempStake.timestamp).div(365).mul(getStakingPeriodInNumbers(normalTempStake)).div(86400):getInvestorRewards(normalTempStake.amount, _stakePeriod, normalTempStake.timestamp).div(365).mul(block.timestamp - normalTempStake.timestamp).div(86400);
        }
        if(silverTempStake.amount > 0)
            silver_reward = validateStakingPeriod(silverTempStake)?getInvestorRewards(silverTempStake.amount, _stakePeriod, silverTempStake.timestamp).div(365).mul(getStakingPeriodInNumbers(silverTempStake)).div(86400):getInvestorRewards(silverTempStake.amount, _stakePeriod, silverTempStake.timestamp).div(365).mul(block.timestamp - silverTempStake.timestamp).div(86400);
        if(goldTempStake.amount > 0)
            gold_reward = validateStakingPeriod(goldTempStake)?getInvestorRewards(goldTempStake.amount, _stakePeriod, goldTempStake.timestamp).div(365).mul(getStakingPeriodInNumbers(goldTempStake)).div(86400):getInvestorRewards(goldTempStake.amount, _stakePeriod, goldTempStake.timestamp).div(365).mul(block.timestamp - goldTempStake.timestamp).div(86400);
        return (normal_reward + silver_reward + gold_reward);
    }

    function getGoldRewards(uint256 _unstake, StakingPeriod _stakePeriod) public view returns (uint256) {
        uint gold_reward = 0;
        stake memory goldTempStake = stakes[msg.sender][_stakePeriod][0];
        if(goldTempStake.amount > 0)
            gold_reward = validateStakingPeriod(goldTempStake)?getInvestorRewards(goldTempStake.amount, _stakePeriod, goldTempStake.timestamp).div(365).mul(getStakingPeriodInNumbers(goldTempStake)).div(86400):getInvestorRewards(goldTempStake.amount, _stakePeriod, goldTempStake.timestamp).div(365).mul(block.timestamp - goldTempStake.timestamp).div(86400);
    ...
    function getSilverRewards(uint256 _unstake, StakingPeriod _stakePeriod) public view returns (uint256) {
        uint silver_reward = 0;
        stake memory silverTempStake = stakes[msg.sender][_stakePeriod][1];
        if(silverTempStake.amount > 0)
            silver_reward = validateStakingPeriod(silverTempStake)?getInvestorRewards(silverTempStake.amount, _stakePeriod, silverTempStake.timestamp).div(365).mul(getStakingPeriodInNumbers(silverTempStake)).div(86400):getInvestorRewards(silverTempStake.amount, _stakePeriod, silverTempStake.timestamp).div(365).mul(block.timestamp - silverTempStake.timestamp).div(86400);
    ...
    function getNormalRewards(uint256 _unstake, StakingPeriod _stakePeriod) public view returns (uint256) {
        uint normal_reward = 0;
        stake memory normalTempStake = stakes[msg.sender][_stakePeriod][2];
        if(normalTempStake.amount > 0)
            normal_reward = validateStakingPeriod(normalTempStake)?getInvestorRewards(normalTempStake.amount, _stakePeriod, normalTempStake.timestamp).div(365).mul(getStakingPeriodInNumbers(normalTempStake)).div(86400):getInvestorRewards(normalTempStake.amount, _stakePeriod, normalTempStake.timestamp).div(365).mul(block.timestamp - normalTempStake.timestamp).div(86400);
    ...
    function getExpiredDay(StakingPeriod _stakePeriod) public view returns (uint256) {
        ...
        stake memory goldStake = stakes[msg.sender][_stakePeriod][0];
        stake memory silverStake = stakes[msg.sender][_stakePeriod][1];
        stake memory normalStake = stakes[msg.sender][_stakePeriod][2];
        if(goldStake.amount > 0){
            expiredDay = goldStake.timestamp + getStakingPeriodInNumbers(goldStake);
        }else if(silverStake.amount > 0){
            expiredDay = silverStake.timestamp + getStakingPeriodInNumbers(silverStake);
        }else{
            expiredDay = normalStake.timestamp + getStakingPeriodInNumbers(normalStake);
        }
        return expiredDay;
    }
    ...
    function setTotalRewards(uint _totalRewards) public onlyOwner {
