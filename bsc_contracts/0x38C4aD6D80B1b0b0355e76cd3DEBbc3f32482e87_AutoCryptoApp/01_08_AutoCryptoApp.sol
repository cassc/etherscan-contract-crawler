...
contract AutoCryptoApp is Initializable, UUPSUpgradeable {
   ...
   /**
    ...
    function getReduction(address user) public view returns (uint){
        uint taxReduction = taxReduced(user);
        uint tier = getUserTier(user);
        uint feeTier = (10 - (tier - 1)) * 10_000;
        return feeTier - taxReduction;
    }
    ...
    function freeUpdateTier() public {
        ...
        require((_userData[msg.sender].tierOwned > 0 && 
                ...
                block.timestamp - _userData[msg.sender].initialVestingDate > 365 days),
