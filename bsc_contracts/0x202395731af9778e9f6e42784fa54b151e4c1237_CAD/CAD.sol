...
contract CAD is Context, IERC20, Ownable {
    ...
    string private _name = "CryptoAid";
    ...
    address public marketingWallet = 0x16bC6541b743d639cda87ef94d06d31A7C26f1Fb;
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount,bool takeFee) private {
        ...
        if (_isExcluded[sender] && !_isExcluded[recipient]) {
            _transferFromExcluded(sender, recipient, amount.sub(marketingAmount).sub(burnAmount));
        } else if (!_isExcluded[sender] && _isExcluded[recipient]) {
            _transferToExcluded(sender, recipient, amount.sub(marketingAmount).sub(burnAmount));
        } else if (!_isExcluded[sender] && !_isExcluded[recipient]) {
            _transferStandard(sender, recipient, amount.sub(marketingAmount).sub(burnAmount));
        } else if (_isExcluded[sender] && _isExcluded[recipient]) {
            _transferBothExcluded(sender, recipient, amount.sub(marketingAmount).sub(burnAmount));
        } else {
            _transferStandard(sender, recipient, amount.sub(marketingAmount).sub(burnAmount));
        }
        ...
        if(burnAmount > 0) {
            _transferStandard(sender, burnWallet, burnAmount);
        }
