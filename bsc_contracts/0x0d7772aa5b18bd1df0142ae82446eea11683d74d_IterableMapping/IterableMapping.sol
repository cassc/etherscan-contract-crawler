...
contract XXXDollar is BEP20, LockToken {
    using SafeMath for uint256;
    ...
    XXXDollarDividendTracker public dividendTracker;
    ...
    constructor() BEP20("XXXDollar", "XXX") 
    {
        dividendTracker = new XXXDollarDividendTracker();
    ...
    {
        require(newAddress != address(dividendTracker), "XXXDollar: The dividend tracker already has that address");
        XXXDollarDividendTracker newDividendTracker = XXXDollarDividendTracker(payable(newAddress));
        require(newDividendTracker.owner() == address(this), "XXXDollar: The new dividend tracker must be owned by the XXXDollar token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "XXXDollar: The router already has that address");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "XXXDollar: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "XXXDollar: The PanBUSDSwap pair cannot be removed from automatedMarketMakerPairs");
        _setAutomatedMarketMakerPair(pair, value);
    }
    

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "XXXDollar: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "XXXDollar: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "XXXDollar: Cannot update gasForProcessing to same value");
...
contract XXXDollarDividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("XXXDollar_Dividend_Tracker", "XXXDollar_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 2000_000 * (10**18); //must hold 2 Millions+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "XXXDollar_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "XXXDollar_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main XXXDollar contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "XXXDollar_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "XXXDollar_Dividend_Tracker: Cannot update claimWait to same value");
