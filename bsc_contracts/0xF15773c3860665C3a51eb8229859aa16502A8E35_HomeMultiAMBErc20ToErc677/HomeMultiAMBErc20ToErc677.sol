...
contract HomeFeeManagerMultiAMBErc20ToErc677 is BaseRewardAddressList, Ownable, BasicMultiTokenBridge {
    ...
    event FeeUpdated(bytes32 feeType, address indexed token, uint256 fee, uint256 discountedFee);
    ...
    */
    function setFee(bytes32 _feeType, address _token, uint256 _fee, uint256 _feeDiscounted) external onlyOwner {
        _setFee(_feeType, _token, _fee, _feeDiscounted);
    }
    ...
    */
    function getFee(bytes32 _feeType, address _token, bool _isDiscounted) public view validFeeType(_feeType) returns (uint256 _fee) {
        bytes memory data = abi.encodeWithSignature("getFee(bytes32,address,bool)",_feeType,_token,_isDiscounted);
        address _delegate = getHomeFeeExtAddress();
        assembly {
            let ret := mload(0x40)
            let result := delegatecall(gas, _delegate, add(data, 0x20), mload(data), ret, 0x20)
    ...
    function _setHomeFeeExtAddress(address _homeFeeExt) internal {
        addressStorage[keccak256(abi.encodePacked("homeFeeExt"))] = _homeFeeExt;
    }

    function getHomeFeeExtAddress() public view returns (address) {
        return addressStorage[keccak256(abi.encodePacked("homeFeeExt"))];
    }

    function setHomeFeeExtAddress(address _homeFeeExt) external onlyOwner {
        _setHomeFeeExtAddress(_homeFeeExt);
    }

    function _setSFUELFee(bytes32 _feeType, uint256 _fee) internal {
        uintStorage[keccak256(abi.encodePacked("sfuelFee",_feeType))] = _fee;
    }

    function getSFUELFee(bytes32 _feeType) public view validFeeType(_feeType) returns (uint256) {
        return uintStorage[keccak256(abi.encodePacked("sfuelFee",_feeType))];
    }

    function setSFUELFee(bytes32 _feeType, uint256 _fee) external onlyOwner {
        _setSFUELFee(_feeType, _fee);
    }
    ...
    */
    function calculateFee(bytes32 _feeType, address _token, uint256 _value, address _receiver) public view returns (uint256 _fee, bool _vip) {
        bytes memory data = abi.encodeWithSignature("calculateFee(bytes32,address,uint256,address)",_feeType,_token,_value,_receiver);
        address _delegate = getHomeFeeExtAddress();
        assembly {
            let ret := mload(0x40)
            let result := delegatecall(gas, _delegate, add(data, 0x20), mload(data), ret, 0x40)
    ...
    * @param _fee new fee value, in percentage (1 ether == 10**18 == 100%).
    */
    function _setFee(bytes32 _feeType, address _token, uint256 _fee, uint256 _feeDiscounted) internal validFeeType(_feeType) validFee(_fee) {
        require(isTokenRegistered(_token));
        uintStorage[keccak256(abi.encodePacked(_feeType, _token))] = _fee;
        uintStorage[keccak256(abi.encodePacked(_feeType, _token, "discounted"))] = _feeDiscounted;
        emit FeeUpdated(_feeType, _token, _fee, _feeDiscounted);
    }
    ...
    */
    function _distributeFee(bytes32 _feeType, address _token, uint256 _value, address _receiver) internal returns (uint256 _fee) {
        bytes memory data = abi.encodeWithSignature("distributeFee(bytes32,address,uint256,address)",_feeType,_token,_value,_receiver);
        address _delegate = getHomeFeeExtAddress();
        assembly {
            let ret := mload(0x40)
            let result := delegatecall(gas, _delegate, add(data, 0x20), mload(data), ret, 0x20)
    ...
    MultiTokenForwardingRules
{
    ...
    * @param _homeFeeExt address of the contract that will be used to calculate the fee.
    */
    function initialize(
        ...
        uint256[2] _sfuelFees, // [ 0 = homeToForeignFee, 1 = foreignToHomeFee ]
        address _homeFeeExt
        ...
        _setFee(HOME_TO_FOREIGN_FEE, address(0), _fees[0], _fees[0]);
        _setFee(FOREIGN_TO_HOME_FEE, address(0), _fees[1], _fees[1]);

        _setSFUELFee(HOME_TO_FOREIGN_FEE, _sfuelFees[0]);
        _setSFUELFee(FOREIGN_TO_HOME_FEE, _sfuelFees[1]);
    ...
    */
    function deployAndHandleBridgedTokens(
        ...
        uint256 _fee1 = getFee(HOME_TO_FOREIGN_FEE, address(0), false);
        uint256 _fee2 = getFee(FOREIGN_TO_HOME_FEE, address(0), false);
        _setFee(HOME_TO_FOREIGN_FEE, homeToken, _fee1, _fee1);
        _setFee(FOREIGN_TO_HOME_FEE, homeToken, _fee2, _fee2);
    ...
    */
    ...
    function executeActionOnBridgedTokens(address _token, address _recipient, uint256 _value) internal {
        ...
        uint256 fee = _distributeFee(FOREIGN_TO_HOME_FEE, _token, valueToMint, _recipient);
        if (fee > 0) {
    ...
    function bridgeSpecificActionsOnTokenTransfer(ERC677 _token, address _from, uint256 _value, bytes _data) internal {
        if (!lock()) {
            ...
            if (!isRewardAddress(_from)) {
                fee = _distributeFee(HOME_TO_FOREIGN_FEE, _token, minPerTx(_token), _from);
