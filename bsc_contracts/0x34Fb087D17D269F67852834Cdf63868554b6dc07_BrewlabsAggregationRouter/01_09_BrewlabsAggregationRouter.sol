...
contract BrewlabsAggregationRouter is Ownable {
    ...
    function _swapNoSplit(
        ...
        if (BREWS_FEE > 0 || MIN_FEE > 0) {
            // Transfer fees to the claimer account and decrease initial amount
            amounts[0] = _applyFee(_trade.amountIn, BREWS_FEE);
        ...
        require(amounts[amounts.length - 1] >= _trade.amountOut, "BrewlabsAggregationRouter: Insufficient output amount");
        for (uint256 i = 0; i < _trade.adapters.length; i++) {
