...
contract seedStake is Ownable, ReentrancyGuard, Pausable,ERC1155Receiver,ERC1155Holder {
     ...
     */
    // uint256 private rewardsPerHour = 144 * 10 ** 17;
    uint256 private rewardsPerDay = 144 * 10 ** 17;
    ...
    address public weth; 
    address public stakeA;
    address public stakeB;
    ...
    function setStakeAddr(address _stakeA, address _stakeB) public onlyOwner{
        stakeA = _stakeA;
        stakeB = _stakeB;
    }
    function inviteExt(address from,address to) external {
        require(msg.sender == stakeA || msg.sender == stakeB,"address error");
    ...
    function invite(address from, address to) internal {
             if (recommender[to] == address(0) &&  recommender[from] != to && !isRecommender[to]) {
             recommender[to] = from;
             recommenderInfo[from].push(to);
             isRecommender[to] = true;
     ...
     */
    function stake(address from) external payable whenNotPaused {
        ...
        if(recommender[msg.sender] != address(0)){
           if (msg.value == 0) {
            ...
            TransferHelper.safeTransferFrom(weth, msg.sender, recommender[msg.sender], fee * 7 / 10);
        } else {
            ...
            IWETH(weth).transfer(recommender[msg.sender], fee * 7 / 10);
        }
    }else{
        if (msg.value == 0) {
            TransferHelper.safeTransferFrom(weth, msg.sender, walletA, fee / 10);
