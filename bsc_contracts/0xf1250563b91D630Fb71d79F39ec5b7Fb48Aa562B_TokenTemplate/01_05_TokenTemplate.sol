...
contract TokenTemplate is ERC20,Ownable {
    ...
    uint public fee1 = 1; //1%（买入销毁）
    uint public fee11 = 1; //1%（卖出销毁）
    uint public fee2 = 2; //2%（买入余额变动分红）
    uint public fee22 = 2; //2%（卖出余额变动分红）
    ...
    function transferFrom(address from, address to, uint256 value) public returns (bool) {
     ...
     function _transfer(address from, address to, uint256 value) internal {
        ...
        uint256 feeAmount1 = value.mul(fee1).div(100);//买入交易销毁
        uint256 feeAmount11 = value.mul(fee11).div(100);//卖出交易销毁
        uint256 feeAmount2 = value.mul(fee2).div(100);//买入分红
        uint256 feeAmount22 = value.mul(fee22).div(100);//卖出分红
