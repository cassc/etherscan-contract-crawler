...
contract PaydeceEscrow is  ReentrancyGuard , Ownable  {
   ...
   function getState(uint _orderId) public view returns (EscrowStatus){
    ...
    function createEscrow(uint _orderId, address payable _seller, uint256 _value,IERC20 _currency) external virtual   {
        ...
        uint256 _amountFeeBuyer = (_value  * (feeBuyer * 10**_decimals) / (100 * 10**_decimals)); 
    ...
    function _releaseEscrow (uint _orderId) private nonReentrant{
        ...
        //Obtiene el monto a transferir desde el comprador al contrato        
        uint256 _amountFeeBuyer = (escrows[_orderId].value  * (feeBuyer * 10**_decimals) / (100 * 10**_decimals));
        uint256 _amountFeeSeller = (escrows[_orderId].value  * (feeSeller * 10**_decimals) / (100 * 10**_decimals));
