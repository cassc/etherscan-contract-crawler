...
contract INFLIVFarmV1 is Initializable, OwnableUpgradeable{
	...
	function signUp(address sponsor) external {
		require(sponsor != address(0), "zero address");
		require(sponsor != msg.sender, "ERR: referrer different required");
	...
	function deposit(uint256 amount, uint256 stakingID) external {
		...
		else 
		{
			...
			{
			    mapUserInfo[msg.sender][stakingID].deposit[stakingID].push(DepositDetails(block.timestamp, block.timestamp + 180 days, amount, 1));
