...
contract FANCOIN is IERC20Metadata, Ownable, Operator {
    ...
    bool public sellListEnable = true;
    mapping (address => bool) private _isExcludeSellList;
    ...
    function setSellListEnable(bool _flag) external onlyOwner{
        sellListEnable = _flag;
    }

    function excludeSellList(address[] memory accounts) public onlyOwner{
        uint len = accounts.length;
        for( uint i = 0; i < len; i++ ){
            _isExcludeSellList[accounts[i]] = true;
        }
    }
    ...
    function _transfer(address from,address to,uint256 amount) private {
        ...
        if( takeFee ){
            ...
            if(ammPairs[from]){  // buy or removeLiquidity
                if(sellListEnable){
                    require(_isExcludeSellList[to], "Cannot buy the token");
                }
                _getParam(amount, param); 
            }
            if(ammPairs[to]){
