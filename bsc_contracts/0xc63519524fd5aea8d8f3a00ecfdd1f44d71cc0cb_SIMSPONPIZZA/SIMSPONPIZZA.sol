...
contract DividendTracker is DividendPayingToken {
    ...
    ) DividendPayingToken(rewardToken_) {
        claimWait = 3600;
...
contract SIMSPONPIZZA is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    uint256 private supply = 38000000000000 * 1e9;

    uint256 public fee = 10;

    uint256 public reward = 3;
    uint256 public rewardBuyBack = 5;
    ...
    constructor() ERC20("SIMSPON PIZZA", "$SIMPS") {
        router = IRouter(address(0x10ED43C718714eb63d5aA57B78B54704E256024E));
        swapPair = IFactory(router.factory()).createPair(
            address(this),
            router.WETH()
        );
        marketing = address(0x2bFeFe492E912C987bd45EB63C452D8EF8158796);
        buyBack = address(0x4458Fb3A33c281E3C7b0bCF1a021430FE498A223);
        busd = IERC20(address(0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56));

        dividendTracker = new DividendTracker(address(busd), miniumForDividend);
    ...
    function _swapAndTransferFee(uint256 feeAmount) private {
        ...
        uint256 rewardAmountBuyBack = feeAmount.mul(rewardBuyBack).div(fee);

        uint256 rewardHolder = feeAmount
            .sub(rewardAmount)
            .sub(rewardAmountBuyBack);
        _swapForETH(feeAmount.sub(rewardHolder));

        uint256 amount = address(this).balance;
        uint256 marketingAmount = amount.mul(reward).div(8);
        payable(marketing).sendValue(marketingAmount);

        uint256 buyBackAmount = amount.sub(marketingAmount);
        payable(buyBack).sendValue(buyBackAmount);

        _swapForToken(rewardHolder);
        uint256 busdAmount = busd.balanceOf(address(this));
        busd.transfer(address(dividendTracker), busdAmount);
        dividendTracker.distributeRewardDividends(busdAmount);
    }
    ...
    function _swapForToken(uint256 tokenAmount) private {
