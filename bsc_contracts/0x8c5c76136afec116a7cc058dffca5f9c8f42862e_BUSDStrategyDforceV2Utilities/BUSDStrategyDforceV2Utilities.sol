...
// File: contracts/strategies/BUSDStrategyDforceV2Utilities.sol
...
contract BUSDStrategyDforceV2Utilities is Ownable {
    using SafeMath for uint256;
    ...
    function getNormalizedTotalBalance(address _contract) public returns (uint256) {
        ...
        {
            uint256 _amount = busdLendingPoolBalance(_contract); // Check the token balance in contract
            if(_amount > 0){
                _balance = _balance.add(_amount);
            }
        }
    ...
    function busdLendingPoolBalance(address _contract) public returns (uint256) {
