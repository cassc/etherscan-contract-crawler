...
contract EthBurn is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    EthBurnDividendTracker public dividendTracker;
    ...
    constructor() ERC20("EthBurn", "BURNING") {
        marketingWallet = payable(0x6042F172bc1c0409c63B17db5a12a662Ab6199F5);
        devWallet = payable(0xCA7B27818984C1208D99A90E2B73A130Aa126D37);
        ...
        dividendTracker = new EthBurnDividendTracker(
            payable(this),
            router,
...
contract EthBurnDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    EthBurn public EthBurnContract;
    ...
    constructor(
        ...
        defaultToken = token;
        EthBurnContract = EthBurn(mainContract);
    ...
    function _transfer(
        ...
        require(false, "EthBurn_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "EthBurn_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main EthBurn contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(EthBurnContract);

            uint256 prevBalance = EthBurnContract.balanceOf(address(this));
            ...
            {
                uint256 received = EthBurnContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    EthBurnContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
