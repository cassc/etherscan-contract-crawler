...
contract Jug is ERC20, Ownable {
    ...
    JugDividendTracker public dividendTracker;
    ...
    constructor() ERC20("JUNIOR GRAMMAR TOKEN", "JUG") {
        marketingWallet = payable(0x607C6e81e27D423CD859FCFC28017A1588D3bfD9);
        devWallet = payable(0x2AF1efeaA3370B2A5fB5183F364878BFB2D390F9);
        ...
        dividendTracker = new JugDividendTracker(
            payable(this),
            router,
        ...
        uint256 totalTokenSupply = (7_000_000_000_000_000) * (10**18);
        ...
        swapTokensAtAmount = totalTokenSupply / 70000; // 0.01%;
    ...
    function updateFees(
        ...
        require(totalSellFees <= 50 && totalBuyFees <= 50, "total fees cannot exceed 50% sell or buy");
...
contract JugDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    function _transfer(
        ...
        require(false, "Jug_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "Jug_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(JugContract);

            uint256 prevBalance = JugContract.balanceOf(address(this));
            ...
            {
                uint256 received = JugContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    JugContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
