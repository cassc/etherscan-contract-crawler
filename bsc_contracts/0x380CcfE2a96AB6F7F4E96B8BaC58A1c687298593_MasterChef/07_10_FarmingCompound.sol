...
/*
 ...
 1% Burnt
 1% Volatile VDC
 Receives MWT daily from xPERPS staking
 */
contract FarmingCompound is Initializable, Ownable, ReentrancyGuard
{
	...
	struct AccountInfo {
		...
		uint256 reward; // MWT reward accumulated but not claimed
		uint256 drip; // xPERPS from drip pool accumulated but not claimed
		uint256 accRewardDebt; // MWT reward debt from PCS distribution algorithm
	...
	address public rewardToken; // MWT
	...
	function initialize(address _owner, address _reserveToken, address _rewardToken, address _farmingVolatile) public initializer
	{
		...
		accRewardPerShare = 0; // cumulative reward MWT per xPERPS staked from PCS distribution algorithm
	...
	// performs the daily distribution from staking (MWT) and the drip pool (xPERPS)
