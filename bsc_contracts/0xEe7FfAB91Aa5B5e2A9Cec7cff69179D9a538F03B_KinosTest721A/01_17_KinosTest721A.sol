...
contract KinosTest721A is ERC721AQueryable, AccessControlEnumerable {
    ...
    function batchMint(address[] memory accounts, uint256[] memory amounts) external onlyRole(DEFAULT_ADMIN_ROLE) {
        require(accounts.length == amounts.length, "NFT: Incorrect length match for accounts and amounts");
        ...
        require(originalSupply + mintedAmount <= collectionSize, "NFT: Cannot mint over collection size");
    }
