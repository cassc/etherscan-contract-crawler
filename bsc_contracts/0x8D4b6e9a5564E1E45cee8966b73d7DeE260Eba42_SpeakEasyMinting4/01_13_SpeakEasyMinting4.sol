    ...
    OwnableUpgradeable
{
    ...
    // Added in version 4:
    uint256 public MINT_GOLD_MKC_COST;
    uint256 public MINT_GOLD_MKC_LIMIT;
    bool public goldMKCMintingEnabled;

    function initGoldMKC() public onlyTeam {
        MINT_GOLD_MKC_COST = 750000000000000000; // 0.75 BNB
        MINT_GOLD_MKC_LIMIT = 5000;
        goldMKCMintingEnabled = false;
        GOLD_ADDRESS = 0x284744e6D901e5aB25d918dD1dF3Eb0C2f1dF0a4; // mainnet
        _goldMKCNFT = ICocktailNFT(GOLD_ADDRESS);
    }
    /////////////////////
    ...
    function totalGoldMKCMinted() public view returns (uint256) {
        return _goldMKCNFT.minted();
    }
    ...
    function mintGoldMKC() public payable nonReentrant {
        require(goldMKCMintingEnabled == true, "GoldMKC minting disabled");
        require(
            msg.value == MINT_GOLD_MKC_COST,
            "Input value isn't the required mint cost"
        );
        require(
            users[msg.sender].goldMKCMinted == 0,
            "One user can only mint 1 GoldMKC"
        );
        require(
            totalGoldMKCMinted() < MINT_GOLD_MKC_LIMIT,
            "Limit of MKC's minted has been reached"
        );

        try _goldMKCNFT.safeMint(msg.sender) {
            _bank.transfer(msg.value);
        } catch (bytes memory reason) {
            emit LogBytes(reason);
        }

        users[msg.sender].goldMKCMinted = Math.add8(
            users[msg.sender].goldMKCMinted,
            1
        );

        emit EmitMintedNFT(msg.sender, address(0), GOLD_ADDRESS);
    }
    ...
    function enableGoldMKCMinting(bool enable, uint256 limit)
        public
        onlyTeam
        returns (bool enabled, uint256 newLimit)
    {
        goldMKCMintingEnabled = enable;
        if (limit > 0) {
            MINT_GOLD_MKC_LIMIT = limit;
        }
        return (goldMKCMintingEnabled, MINT_GOLD_MKC_LIMIT);
    }
