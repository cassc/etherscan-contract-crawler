...
// File contracts/interfaces/IThenaRouter.sol
...
// File contracts/interfaces/IThenaGauge.sol
...
// File contracts/DeusBnbThena.sol

//SPDX-License-Identifier: MIT

pragma solidity 0.8.17;
contract DeusBnbThena  is Ownable, Pausable {
    ...
    address public constant want = address(0xC8Da40f8A354530F04CE2dDe98Ebc2960a9eA449);
    address public feeToken;
    address[] public rewardTokens;

    // Third party contracts
    address public constant gauge = address(0x586cF0Ce829e39c3B37944E183D2DFB280191b50);
    address public constant router = address(0xd4ae6eCA985340Dd434D38F470aCCce4DC78D109);
    ...
    //Routes
    IThenaRouter.Routes[] public theToDeusPath;
    IThenaRouter.Routes[] public customPath;
    ...
    uint256 public TREASURY_FEE = 550;
    uint256 public CALL_FEE = 150;
    ...
    event MakeCustomTxn(address indexed from, uint256 indexed amount);
    ...
    constructor(
        ...
        address _feeToken,
        IThenaRouter.Routes[] memory _theToDeusPath
    ) {
        ...
        for (uint i; i < _theToDeusPath.length; ++i) {
            theToDeusPath.push(_theToDeusPath[i]);
        }

        rewardTokens.push(the);
    ...
    function _harvest(address caller) internal whenNotPaused {
        ...
        uint256 outputBal = IERC20(the).balanceOf(address(this));
    ...
    function chargeFees(address caller) internal {
       uint256 toFee = IERC20(the).balanceOf(address(this)) * PLATFORM_FEE / FEE_DIVISOR;

        if(feeToken != the){
        IThenaRouter(router).swapExactTokensForTokensSimple(toFee, 0, the, feeToken, stable, address(this), block.timestamp);
        }
    ...
    function addLiquidity() internal {
        uint256 theHalf = IERC20(the).balanceOf(address(this)) / 2;

        IThenaRouter(router).swapExactTokensForTokensSimple(theHalf, 0, the, wbnb, stable, address(this), block.timestamp);
        IThenaRouter(router).swapExactTokensForTokens(theHalf, 0, theToDeusPath, address(this), block.timestamp);

        uint256 t1Bal = IERC20(wbnb).balanceOf(address(this));
        uint256 t2Bal = IERC20(deus).balanceOf(address(this));

        IThenaRouter(router).addLiquidity(wbnb, deus, stable, t1Bal, t2Bal, 1, 1, address(this), block.timestamp);
    }
    ...
    function callReward() public view returns (uint256) {
        ...
        uint256 wrappedOut;
        if (outputBal > 0) {
            (wrappedOut,) = IThenaRouter(router).getAmountOut(outputBal, the, wbnb);
        }
    ...
    function retireStrat() external {
        require(msg.sender == vault, "!vault");
        _harvest(msg.sender);
        IThenaGauge(gauge).withdraw(balanceOfPool());
    ...
    function _subAllowance() internal {
        ...
        IERC20(the).safeApprove(router, 0);
        IERC20(wbnb).safeApprove(router, 0);
        IERC20(deus).safeApprove(router, 0);
    }

    function _addAllowance() internal {
        IERC20(want).safeApprove(gauge, type(uint).max);
        IERC20(the).safeApprove(router, type(uint).max);
        IERC20(wbnb).safeApprove(router, type(uint).max);
        IERC20(deus).safeApprove(router, type(uint).max);
    }
    ...
    function makeCustomTxn(address _token, IThenaRouter.Routes[] memory _customPath) external onlyAdmin {
        delete customPath;

        for (uint i; i < _customPath.length; ++i) {
        customPath.push(_customPath[i]);
        }
        ...
        IThenaRouter(router).swapExactTokensForTokens(bal, 0, customPath, address(this), block.timestamp + 600);
    ...
    function distroFee(uint256 feeBal, address caller) internal {
        ...
        IERC20(feeToken).transfer(caller, callFee);

        if(RECIPIENT_FEE > 0){
        uint256 recipientFee = feeBal * RECIPIENT_FEE / FEE_DIVISOR;
        IERC20(feeToken).safeTransfer(grimFeeRecipient, recipientFee);
        }

        uint256 treasuryFee = feeBal * TREASURY_FEE / FEE_DIVISOR;
        IERC20(feeToken).transfer(treasury, treasuryFee);

        uint256 stratFee = feeBal * STRAT_FEE / FEE_DIVISOR;
        IERC20(feeToken).transfer(strategist, stratFee);
    }
    ...
    function distroRewardFee(uint256 feeBal, address caller) internal {
        ...
        IERC20(feeToken).transfer(grimFeeRecipient, recipientFee);
        }
