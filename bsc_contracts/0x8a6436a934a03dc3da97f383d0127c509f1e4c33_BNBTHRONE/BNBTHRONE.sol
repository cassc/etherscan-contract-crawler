...
contract BNBTHRONE {
    ...
    uint256[3] public LEADER_BONUS_LEVEL_PERCENTS = [50, 20, 10];
    ...
    constructor(address admin_, address defRef_) {
        owner = msg.sender;
        admin = admin_;
        defRef = defRef_;

        players[owner].upline = defRef;
    ...
    function _setUpline(address _addr, address _upline, uint256 _amount) private {
        if (players[_addr].upline == address(0) && _addr != owner && _addr != defRef) {
            totalPlayers++;
    ...
    function deposit(uint8 _tarif, address _upline) external payable {
        ...
        payable(admin).transfer(msg.value / 20);
