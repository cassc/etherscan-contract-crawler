...
// File contracts/features/MhdaoDnaManager_V8.sol
 ...
 * @dev Sells items
 */
contract MhdaoDnaManager_V8 is Initializable, AccessControlUpgradeable, PausableUpgradeable {
  ...
  function traitReroll(uint256 nftId, uint256 chosenTrait) external whenNotPaused {
    ...
    _receivePayment(1 * nft.type_ * nft.rarity * 1e18, priceInResources, false, false, true);
  ...
  function _doReroll(uint256 nftId, uint256 chosenTrait, bool hasThreeTraits) private {
    require(chosenTrait > 0 && chosenTrait <= 50, "e8");
  ...
  function evolve(
    ...
    require(evolutions[nftId] < 11, "max"); // Can't evolve more than 11 times
    ...
    _receivePayment(100 * nft.type_ * nft.rarity * 1e18, priceInResources, false, false, true);
  ...
  function maxOutEvolve(
    uint256 nftId,
    uint256 chosenTrait,
    string memory newName,
    string memory newImageUrl
  ) public whenNotPaused {
    require(evolutions[nftId] <= 10, "max"); // Can't evolve more than 11 times
    uint256 howManyEvos = 11 - evolutions[nftId];

    for (uint256 i = 0; i < howManyEvos; i++) {
  ...
  function improveBaseFarmWithNuts(uint256 nftId, uint256 howManyNuts) public whenNotPaused {
    IMhdaoNft.NftCharacter memory nft = contracts.nft.getNft(nftId);
    require(contracts.nft.ownerOf(nftId) == msg.sender, "e5");
    require(nutsConsumed[nftId] + howManyNuts < 51, "max");
    ...
    _receivePayment(2 * nft.type_ * nft.rarity * 1e18, priceInResources, false, false, true);
  ...
  function maxOutNuts(uint256 nftId, bool sequentially) public whenNotPaused {
    require(nutsConsumed[nftId] < 50, "max");
    uint256 howManyNuts = 50 - nutsConsumed[nftId];
  ...
  function maxOutEvolveAndNuts(
    ...
    maxOutEvolve(nftId, chosenTrait, newName, newImageUrl);
    maxOutNuts(nftId, sequentialNuts);
  }
  ...
  function replaceTrait(
    ...
    _receivePayment(250 * target.type_ * target.rarity * 1e18, priceInResources, false, false, true);
  ...
  function createCollection(
    ...
    collectionsCount++;
    _mintMany(names, type_, chosenTrait, imageUrls, initialTokenIdTracker, collectionsCount);
  ...
  function expandCollection(
    ...
    // create the collection itself:
    uint256 initialTokenIdTracker = contracts.nft.tokenIdTracker();
    _mintMany(names, type_, chosenTrait, imageUrls, initialTokenIdTracker, collectionId);
  ...
  function _mintMany(
    ...
    for (uint i = 0; i < names.length; i++) {
      ...
      traits[1] = (__result.get(msg.sender, 1, 50, collectionId + i, initialTokenIdTracker + i));
  ...
  function setNftEvolutionsAndNutsCosumed(uint256 nftId, uint256 evos, uint256 nuts) external onlyRole(OPERATOR_ROLE) {
    evolutions[nftId] = evos;
    nutsConsumed[nftId] = nuts;
  }
}