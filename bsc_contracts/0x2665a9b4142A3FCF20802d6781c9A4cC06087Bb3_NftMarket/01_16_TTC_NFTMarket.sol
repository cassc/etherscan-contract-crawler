...
contract NftMarket is ReentrancyGuard {
    ...
    event MarketItemCreated (
     ...
     function setCardNftContract(ICardNFT card) public   onlyOwner
    {
        CardNftContract=card;
    }
 
    function setMarketItemPrice(uint256 itemId,uint256 price) public  onlyOwner
    {
        idToMarketItem[itemId] .price=price;
    }

    function setMarketItem(uint256 itemId,uint256 price,address seller,uint256 tokenId) public  onlyOwner
    {
        idToMarketItem[itemId] .price=price;
        idToMarketItem[itemId] .seller= payable(seller);
  ...
  function CreateMarketItemForAdmin(
        address nftContract,
        uint256 tokenId,
        uint256 price,
        address sender
        ...
        require(checkTokenInItems(nftContract,tokenId)==false, "ERC721:  token on sale"); // internal owner
  ...
  function autoCreateMarketItem(
    ...
    function checkTokenInItems(address nftContract,uint256 tokenId) public returns(bool)
    {
        uint itemCount = _itemIds.current();
        console.log('checkTokenInItems');
        console.log(tokenId);
        for (uint i = 0; i < itemCount; i++) {
