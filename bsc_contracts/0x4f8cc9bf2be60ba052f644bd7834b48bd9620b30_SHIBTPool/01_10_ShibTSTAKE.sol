...
contract SHIBTPool is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;

    uint256 amountSHIBT;
    address payTokenSHIBT;
    address vault;
    ...
    function deposit(uint256 _amount) public payable {
        ...
        IBEP20 EGGC = IBEP20(payTokenSHIBT);
        require(EGGC.transferFrom(msg.sender, vault, amountSHIBT));
    ...
    /* SHIBT Admin functions to setup pool */
    ...
    function SHIBTStartIn(uint256 _blocks) public onlyOwner {
        SHIBTSetStart(block.number.add(_blocks));
    }

    function SHIBTSetStart(uint256 _startBlock) public onlyOwner {
    ...
    function SHIBTSetDuration(uint256 _durationBlocks) public onlyOwner {
        bonusEndBlock = startBlock.add(_durationBlocks);
        SHIBTCalcRewardPerBlock();
    }

    function SHIBTCalcRewardPerBlock() public onlyOwner {
        uint256 rewardBal = rewardToken.balanceOf(address(this));
        rewardPerBlock = rewardBal.div(rewardDuration());
    }

    function SHIBTSetRewardMaxTxAmount(uint256 _maxTxAmount) public onlyOwner {
    ...
    function setPaySHIBT(address _pay) public onlyOwner {
        payTokenSHIBT = _pay;
    }

    function setPriceSHIBT(uint256 _price) public onlyOwner {
        amountSHIBT = _price;
    }
