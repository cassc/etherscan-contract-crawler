...
contract Marketplace is ReentrancyGuard {
    ...
    uint256 private listingFee = 0.04 ether;
    ...
    constructor() {
        ...
        _token = IERC20(0x8AC76a51cc950d9822D68b83fE1Ad97B32Cd580d);
    }
    ...
    function changePrice(uint256 marketItemId, uint256 price) public payable nonReentrant {
    ...
    function calculatePercentage(uint256 number, uint256 percentage) private pure returns (uint256) { return number * percentage / 100; }

    function depositTokens(uint _amount) public checkAllowance(_amount) {
        _token.transferFrom(msg.sender, address(this), _amount);
    }

    function pay_marketsale(address nftContractAddress, uint256 marketItemId) external{
        uint256 price = marketItemIdToMarketItem[marketItemId].price;        
        uint256 tokenId = marketItemIdToMarketItem[marketItemId].tokenId;

        depositTokens(price);

        _token.transfer(marketItemIdToMarketItem[marketItemId].seller, calculatePercentage(price, 96));
        _token.transfer(0xBa692424907695d421eFe9986931b4b0E07a6d57, calculatePercentage(price, 4));
