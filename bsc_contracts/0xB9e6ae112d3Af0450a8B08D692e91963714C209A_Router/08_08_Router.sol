...
contract Router is Ownable {
    ...
    */
    ...
    // @notice swap amount in @param _swapCallData must be equal to @param _fromAmount - fee
    */
    ...
    // @param _swapInputAmount swap input amount
    // @param _toToken output token address
    // @param _swapCallData swap callData (intended for one specific protocol)
    ...
    // @param r signature r
    // @param s signature s
    // @notice _permitAmount must be superior or equal to _fromAmount + _operationFee
    */
    function ftatSwapWithPermit(
        ...
        if (msg.value > 0) {
            payable(_onBehalfOf).transfer(msg.value);
        }
