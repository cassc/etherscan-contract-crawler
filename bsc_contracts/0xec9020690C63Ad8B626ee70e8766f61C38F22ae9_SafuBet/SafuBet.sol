...
contract SafuBet {
    ...
    */
        ...
        for(uint i=0; i<event_id.length; i++){
            ...
            if (bets[event_id[i]][_bettor] == true) {
                ...
                eventBetOccurenceAmount[event_id[i]][
                    _occurred[i]
                ] = eventBetOccurenceAmount[event_id[i]][_occurred[i]] + _amount[i]; // increment the amount betted on the occurence
                newEvent.poolSize = newEvent.poolSize + _amount[i]; // update pool amount
                ...
                _totalAmountWagered[_bettor] = _totalAmountWagered[_bettor] + _amount[i];  // increment total amount wagered by user
                ...
                newEvent.poolSize = newEvent.poolSize + _amount[i]; // update pool amount
                newEvent.validatorsNeeded = context_address._calculateValidatorsNeeded(newEvent.poolSize); // update no of validators needed
                newEvent.bettorsCount = newEvent.bettorsCount + 1; // increment users that betted on the event
    ...
    // function gets user event tax
    function getUserEventTax(address _address, uint256 _event_id_)
        external
        view
        returns (uint256)
    {
        return userEventTax[_address][_event_id_];
    }
    ...
    function _incrementTotalAmountBetted(uint256 _amount) internal {
        totalAmountBetted = totalAmountBetted + _amount;
    }
    ...
    */
    function setTotalWinnings(address _address, uint256 _value) internal {
        _totalWinnings[_address] = _totalWinnings[_address] + _value;
    }
    ...
    */
    function setTotalValidatorRewardClaimed(uint256 _value)
        private
    {
        totalValidatorRewardClaimed = totalValidatorRewardClaimed + _value;
    }
    ...
    */
    ...
    /**
     * @dev function gets the odds the user wagered on
    */
    function getOdd(uint256 event_id, address _bettor)
        external view returns (Occurences)
    {
        return userBets[event_id][_bettor].occurence;
    }
    ...
    */
    function setAmountWonByUserOnEvent(uint256 event_id, address _bettor, uint256 _value) internal{
        _amountWonByUserOnEvent[event_id][_bettor] = _amountWonByUserOnEvent[event_id][_bettor] + _value;
    }
    ...
    */
    ...
    function _cummulateEventValidation(uint256 event_id) private {
        ...
        if (
            ...
            amountLostInBet[event_id] = amountLostInBet[event_id] + (
    ...
    function _distributionFormular(uint256 event_id) internal {
        ...
        uint256 div_amount = amountLostInBet[event_id] + winners_reward;
        div_amount = div_amount + eventTax[event_id];
        Distribution storage setDivs = divs[event_id];
        setDivs.bettorsDiv = setDivs.bettorsDiv + ((div_amount * 10) / 100);  // bettors reward
        setDivs.systemDiv = setDivs.systemDiv + ((div_amount * 3) / 100);  // system reward
        setDivs.validatorDiv = setDivs.validatorDiv + ((div_amount * 80 ) / 100); // validator reward
        setDivs.creatorDiv = setDivs.creatorDiv + ((div_amount * 1) / 100); // creator reward
        setDivs.VCDiv = setDivs.VCDiv + ((div_amount * 4) / 100); // VC reward
        setDivs.royalty = setDivs.royalty + ((div_amount * 2) / 100); // Royalty reward
    }
        ...
        hasClaimedReward(event_id)
    {
        ...
        if(bets[event_id][user_address] == true){
            ...
            if(events[event_id].betCreator == user_address){
                ...
                getEventDetails.amountClaimed = getEventDetails.amountClaimed + (
            ...
            if (
                ...
                user_reward = user_reward + (
                    userBets[event_id][user_address].amount + (userEventTax[user_address][event_id])
                ...
                getEventDetails.noOfBettorsRewarded = getEventDetails
                    .noOfBettorsRewarded + 1; // increment no. of event bettors rewarded
                getEventDetails.amountClaimed = getEventDetails.amountClaimed + user_reward; // increment event winnings claimed
                ...
                getEventDetails.noOfBettorsRewarded = getEventDetails
                    .noOfBettorsRewarded + 1; // increment no. of event bettors rewarded
                getEventDetails.amountClaimed = getEventDetails.amountClaimed + event_bonus; // increment event winnings claimed
    ...
    */
    function _reclaimBettingWager(uint256 event_id, address user_address) internal {
        ...
        uint256 amount = userEventTax[user_address][event_id] + userBets[event_id][user_address].amount;
    ...
    */
    function _rewardSystem(uint256 event_id) internal {
        ...
        eventDetails.amountClaimed = eventDetails.amountClaimed + amount; // increment event winnings claimed
    ...
    */
    function _rewardVC(uint256 event_id) internal{
        BetEvent storage eventDetails = events[event_id];
        eventDetails.amountClaimed = eventDetails.amountClaimed + divs[event_id].VCDiv;
    ...
    function _incrementTotalAmountClaimed(uint256 _value) internal {
        totalAmountClaimed = totalAmountClaimed + _value;
    }
