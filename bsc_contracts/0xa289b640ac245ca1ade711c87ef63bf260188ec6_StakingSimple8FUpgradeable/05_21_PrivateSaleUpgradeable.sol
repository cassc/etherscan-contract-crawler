...
contract PrivateSale8FUpgradeableV2 is Initializable, OwnableUpgradeable {
    ...
    function withdrawUsdtTo(address to) public onlyOwner {
    ...
    function withdrawTokens(address to) public onlyOwner {
        uint byBalance = ERC20Interface(tokenAddress).balanceOf(address(this));
        (bool s,) = tokenAddress.call(abi.encodeWithSelector(bytes4(keccak256(bytes('transfer(address,uint256)'))), to, byBalance));
        require(s, 'Transfer error');
    }
