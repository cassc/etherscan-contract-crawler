...
contract MasterContractManager is BoringOwnable, BoringFactory {
    ...
    function _calculateDomainSeparator(uint256 chainId) private view returns (bytes32) {
        return keccak256(abi.encode(DOMAIN_SEPARATOR_SIGNATURE_HASH, keccak256("GniLamp"), chainId, address(this)));
    }
    ...
    /// @notice Other contracts need to register with this master contract so that users can approve them for the GniLamp.
                    ...
                    abi.encodePacked(
                        ...
                        keccak256(
                            abi.encode(
                                ...
                                approved
                                    ? keccak256("Give FULL access to funds in (and approved to) GniLamp?")
                                    : keccak256("Revoke access to GniLamp?"),
                                user,
                                masterContract,
                                approved,
                                nonces[user]++
...
// File: interfaces/IBatchFlashBorrower.sol
...
contract GniLamp is MasterContractManager, BoringBatchable {
    using BoringMath for uint256;
    using BoringMath128 for uint128;
    ...
    /// If 'from' is the GniLmap itself, it's allowed. Any ETH, token balances (above the known balances) or GniLamp balances
    ...
    modifier allowed(address from) {
        if (from != msg.sender && from != address(this)) {
            ...
            require(masterContract != address(0), "GniLamp: no masterContract");
            require(masterContractApproved[masterContract][from], "GniLamp: Transfer not approved");
        }
        _;
    }
    ...
    function deposit(
        ...
        // Checks
        require(to != address(0), "GniLamp: to not set"); // To avoid a bad UI from burning funds
        ...
        require(total.elastic != 0 || token.totalSupply() > 0, "GniLamp: No tokens");
        if (share == 0) {
    ...
    function withdraw(
        ...
        require(total.base >= MINIMUM_SHARE_BALANCE || total.base == 0, "GniLamp: cannot empty");
        totals[token] = total;
        ...
        if (token_ == USE_ETHEREUM) {
            ...
            require(success, "GniLamp: ETH transfer failed");
        } else {
    ...
    function transferMultiple(
        ...
        // Checks
        require(tos[0] != address(0), "GniLamp: to[0] not set"); // To avoid a bad UI from burning funds
    ...
    function flashLoan(
        ...
        require(_tokenBalanceOf(token) >= totals[token].addElastic(fee.to128()), "GniLamp: Wrong amount");
    ...
    function batchFlashLoan(
        ...
        for (uint256 i = 0; i < len; i++) {
            ...
            require(_tokenBalanceOf(token) >= totals[token].addElastic(fees[i].to128()), "GnILamp: Wrong amount");
    ...
    // F5 - Checks-Effects-Interactions pattern followed? (SWC-107)
    // F5: Total amount is updated AFTER interaction. But strategy is under our control.
    // F5: Not followed to prevent reentrancy issues with flashloans and GniLamp skims?
    function harvest(
        IERC20 token,
        bool balance,
        ...
        if (balanceChange > 0) {
            ...
            // But tokens with balances that large are not supported by the GniLamp.
            uint256 sub = uint256(-balanceChange);
