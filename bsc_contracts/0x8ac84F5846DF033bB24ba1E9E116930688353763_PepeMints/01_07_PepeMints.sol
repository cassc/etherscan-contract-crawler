...
contract PepeMints is BoringOwnable, ReentrancyGuard {
    ...
    event Sake(address indexed user, uint stakeAmount, uint stakeTime);
    event EnterAuction(address indexed user, uint rawAmount, uint entryTime);
    event ClaimRewards(address indexed user, uint rewardAmount, uint claimTime);
    ...
    uint public offDays = oneDay; 
    uint public currentDay = 10;
    uint public lastBUYINday;
    uint public buyBackPerecent = 500;
    uint public taxFactor = 10000;
    uint public percentToReceiveOnSell = 2000; // percentage of tokens to be received when buying from PancakeSwap
    ...
    // Set the percentage to be received when buying from PancakeSwap
     function setPercentToReceiveOnSell(uint _percentToReceiveOnSell) external onlyOwner {
        require(2000 <= _percentToReceiveOnSell, "Value to small, use at least 2000!");
    ...
    function addStakesUser(address user, uint _stakeTime, uint _amount, uint _claimed, uint _lastUpdate) external {
      require(settingDone == false, "setStakesUser: dev can not set any more!");
      require(msg.sender == launchpadContract || msg.sender == _dev, "setStakesUser: you are not allowed to set Stakes!");
      stakes[user][stakeNumber[user]].stakeTime = _stakeTime;
      stakes[user][stakeNumber[user]].amount = _amount;
      stakes[user][stakeNumber[user]].claimed = _claimed;
      stakes[user][stakeNumber[user]].lastUpdate = LAUNCH_TIME > _lastUpdate ? LAUNCH_TIME : _lastUpdate;
      stakes[user][stakeNumber[user]].collected = _claimed;
    ...
    function buyAndStake(address _referrer) external nonReentrant payable returns (bool) {
      ...
      sendETH(_dev, devFee); // transfer dev share of ETH to dev
      uint buyBackContractFee = rawAmount * dripBuyAuctionFee / 10000;
      sendETH(buyBackContract, buyBackContractFee); // transfer dev share of ETH to buyBackContract
      ...
      _pancakeRouter.swapExactETHForTokens{value: ethToBuyWith} (
        0,
        path,
        BURN_ADDRESS,
        block.timestamp+1
      ...
      if ((_referrer != address(0) && _referrer != msg.sender) || myRef[msg.sender] != address(0)) {
        ...
        if (block.timestamp >= LAUNCH_TIME + offDays + oneDay) {
          ...
          if (contractETHBalance > 1000) {
              ...
              if (collectedThatDay != 0 && !liqAdded[lastBUYINday] ) {
                  ...
                  if (addedEth > 100000000 && ethBal > 1000) {
    ...
    // function to set the amount of days that need to pass after launch to allow new auction entries
    function setOffDays(uint _offDays) external onlyOwner {
      offDays = _offDays;
    }
    ...
    function claimRewards(uint _stakeIndex) external nonReentrant {
    ...
    function reinvest(uint _stakeIndex, address _referrer) external nonReentrant {
    ...
    function flushLottyPool() external onlyOwner nonReentrant {
      if (lottery_Pool > 0) {
        ...
        emit LottyPoolFlushed(_dev, amount);
      }
    }
