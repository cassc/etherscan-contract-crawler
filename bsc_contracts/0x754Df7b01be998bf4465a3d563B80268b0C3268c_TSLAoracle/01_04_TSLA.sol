...
contract TSLAoracle is Context, Ownable {
    ...
    //address public constant BUSDaddress = 0xe4eD68F6aB0Bba986bf4b039b6F7CdB1fB219B34; //BUSD Oracle Mainnet address
    address public BUSDaddress = 0xAC82EF603D3Faf774be7A648A46529EF9ff4DfBA;
    ...
    constructor(address _teamwallet) {
        require(!isContract(_teamwallet));
    ...
    function ChangeROI() private {
        ...
        pricediff = getLatestPrice() - lastprice;
            ...
            if (currentROI[msg.sender].amount <= 29) {currentROI[msg.sender].amount += 1;}
            else if (currentROI[msg.sender].amount >= 11) {currentROI[msg.sender].amount -= 1;}
            }
        lastprice = getLatestPrice();
    }
    ...
    function Oraclemanagement(address _user, uint256 _amount) internal {
            ...
            if (totinvested > topdeposit && totinvested > 2000 ether){
                TSLAholder =  _user;
                topdeposit = totinvested;
            }
            else if (totinvested > smalltopdeposit && totinvested <= 2000 ether && totinvested >= 50 ether){
    ...
    function deposit(address _ref, uint256 _amount) public  {
        ...
        //totala weekly is the amount that users deposit in a 7 days range, unlike the total user deposit, this refreshes weekly
        ...
        // fees 
        uint256 fee_oracle = total_fee / 7 * 4;  //4% goes into BUSD Oracle
        uint256 fee_gen = (total_fee - fee_oracle); //2% goes to the team
        uint256 total_contract = _amount - total_fee;
        BusdInterface.transferFrom(msg.sender,teamwallet,fee_gen); 
        BusdInterface.transferFrom(msg.sender,BUSDaddress,fee_oracle);
    ...
    currentROI[msg.sender].amount = roi;
    }
    ...
    function claimDailyRewards() public {
        ...
        //the oracle holder receives a bonus on any claim, if they holder holds both Oracles this bonus is doubled and the fees to enter the plaftorm are lowered
        if (nowholder == TSLAholder){
            Kingholder = msg.sender;
        ...
        if (msg.sender != smallTSLAholder){
                uint256 BonusOracle = (rewards * 1 / 100) + approvedWithdrawal[smallTSLAholder].amount;
    ...
    function signal_market() external onlyOwner {
        ...
        resets = block.timestamp + 7 days;
        smallresets = block.timestamp + 3 days;
    }

    //Owner can call Oraclemanagement weekly to update the platform (in case there there are few claims to trigger it in the first few days after 1 week)
    ...
    function changeDev (address _teamwallet, address _neworacle) external onlyOwner {
       teamwallet = _teamwallet;
