    ...
    Ownable
{
    ...
    uint8 public contractPercent = 20; // 25% will roll over to the next lotteryevent
    uint8 public marketingPercent = 5; // 25% will roll over to the next lotteryevent
    ...
    function enterLottery() public payable {
        ...
        if (msg.value < entrance) {
            revert Lottery__notEnoughEthForEntrance();
        }
        if (s_lotteryState != LotteryState.OPEN) {
            revert Lottery__notOpen();
        }
        //require(count[msg.sender] <= 10, "You already have max Tickets");
        require(
            paytoken.transferFrom(msg.sender, address(this), entrance) == true,
            "Could not transfer tokens from your address to this contract"
        );

        s_players.push(payable(msg.sender));
        swapFortuneCashForEth(entrance);
