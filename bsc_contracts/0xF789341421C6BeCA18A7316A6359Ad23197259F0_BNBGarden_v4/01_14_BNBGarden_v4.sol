...
interface IBNBGarden_v2 {
    function stakeFor(address adr, uint256 amtx, address ref) external;
}
...
contract BNBGarden_v4 is Initializable, UUPSUpgradeable, AuthUpgradeable, ReentrancyGuardUpgradeable  {
    ...
    function updatev2() external onlyOwner {
        TOKEN_MAIN.safeApprove(address(BNBGARDEN2), type(uint256).max);
    }
    ...
    function unStakeToToken(uint256 keyy, address targetToken) nonReentrant whenNotPaused public {
        ...
        user.divs += transferAmt; //total dividens collected
    ...
    function migrateV2() nonReentrant whenNotPaused public {
        User memory user = UsersKey[msg.sender];
        uint256 totalWithdrawable;
        for (uint256 i = 0; i < user.depoList.length; i++){
            if (user.depoList[i].initialWithdrawn == false){
                totalWithdrawable +=_migrateKey(i);
            }
        }
        BNBGARDEN2.stakeFor(msg.sender, totalWithdrawable, user.ref);
    }

    function _migrateKey(uint256 keyy) private returns(uint256 transferAmt) {
    ...
    bool public SAFE_UNSTAKE;
    IBNBGarden_v2 constant BNBGARDEN2 = IBNBGarden_v2(0x059f88BEcd200Ac26bB3Cae6CeC0b1D6bDEae50E);
}