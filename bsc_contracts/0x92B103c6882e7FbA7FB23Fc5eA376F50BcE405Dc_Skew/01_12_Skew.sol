...
contract Skew is Ownable {
    ...
    function repayBorrow(
        ...
        return _repayBorrow(token, vToken, amount, minOut, data);
    }
    ...
    function flashLoan(
        ...
        address supplyToken,        // Supply Token address to Venus
        address vSupplyToken,       // vSupply Token address in Venus
        address loanToken,          // Loan Token address from Venus
    ...
    function _action(FlashLoanInfo memory info, uint256 minOut, bytes memory inchData) internal {
        ...
        IvBEP20 vLoanToken = IvBEP20(info.loanToken);
        ...
        if( info.loanToken == info.returnToken ) {
            require(
            ...
            uint256 returnTokenOut = this.Swap(minOut, inchData);
    ...
    function dodoCall(
        ...
        (
            info.supplyToken,            // Supply Token or Borrowed Token address
            info.vSupplyToken,           // Supply vToken or Borrowed vToken address
            info.loanToken,              // Token address to loan from Venus(Don't necessary in repay)
        ...
        if( isRepayBorrow ) {
            ...
            _action(info, info.minOuts[inchIdx], info.inchDatas[inchIdx]);
        }
    }
    ...
    function depositToVenus(address _token, address _vToken, uint256 _amount) external {
        IvBEP20 vToken;
    ...
    function withdrawFromVenus(
        ...
        if( totalBorrow[_token] > 0)
            _repayBorrow(_token, _vToken, totalBorrow[_token], _minOut, _data);
        IvBEP20 vToken;
        vToken = IvBEP20(_vToken);
