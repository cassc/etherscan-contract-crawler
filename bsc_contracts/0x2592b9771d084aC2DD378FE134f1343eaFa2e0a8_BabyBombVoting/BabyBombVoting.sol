...
contract BabyBombVoting is Ownable {
    ...
    mapping(uint256 => uint256) private tierVoteWeights;
    ...
    function setTierVoteWeights(uint256[] calldata _voteWeights) external onlyOwner {
        ...
        for (uint256 i = 1; i <= _voteWeights.length; i++) {
            tierVoteWeights[i] = _voteWeights[i - 1];
        }
    }

    function setOptions(string[] calldata _options) external onlyOwner {
        require(_options.length == 4, "There should be exactly 4 options");
        for (uint256 i = 1; i <= _options.length; i++) {
            options[i] = _options[i - 1];
        }
    }
    ...
    function getVotes() public view returns (uint256[] memory) {
        ...
        for (uint256 i = 1; i <= 4; i++) {
            voteList[i - 1] = votes[i];
        }
        return voteList;
    }

    function getTiers() public view returns (uint256[] memory) {
        uint256[] memory tierList = new uint256[](tiers.length);
    ...
    function getTierVoteWeights() public view returns (uint256[] memory) {
        ...
        for (uint256 i = 1; i <= tiers.length; i++) {
            weightList[i - 1] = tierVoteWeights[i];
        }
        return weightList;
    }
}