...
contract OPTC_Stake is OwnableUpgradeable, ERC721HolderUpgradeable {
    ...
    function stakeCardBatch(uint[] memory tokenIds) external onlyOwner updateDaily {
        uint _debt = countingDebt();
        UserInfo storage user = userInfo[msg.sender];
    ...
    function claimAllReward() external onlyEOA updateDaily checkStart {
        UserInfo storage user = userInfo[msg.sender];
        uint rew;
        uint _debt = countingDebt();
        uint price = OPTC.lastPrice();
    ...
    function claimNode() external onlyEOA checkStart {
    ...
    function claimReward(uint tokenId) external onlyEOA checkStart {
    ...
    function pullOutCard(uint tokenId) external onlyEOA checkStart {
    ...
    function _processCardBuy(uint uAmount, uint optcAmount) internal {
        ...
        nodeShare.syncDebt(optcAmount * reBuyRate[2] / 100);
    }
    ...
    function addLiquidity(uint OptAmount, uint uAmount) internal {
        router.addLiquidity(address(OPTC), address(U), OptAmount, uAmount, 0, 0, address(this), block.timestamp);
    }
    ...
    function getUserPower(address addr) external view returns (uint){
        return userInfo[addr].totalPower;
    }
    ...
    function checkReferInfo(address addr) external view returns (address[] memory referList, uint[] memory power, uint[] memory referAmount, uint[] memory level){
        (referList, referAmount, level) = refer.checkReferList(addr);
        power = new uint[](referList.length);
        for (uint i = 0; i < referList.length; i++) {
            power[i] = userInfo[referList[i]].totalPower;
        }
    }
