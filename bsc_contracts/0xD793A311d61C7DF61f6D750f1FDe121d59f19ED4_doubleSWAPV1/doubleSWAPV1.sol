...
contract doubleSWAPV1 is Context, Ownable {
  ...
  function LoopSwap(address[] memory pathBuy,address[] memory pathSell,uint256 amount,uint256 txcount) public onlyPermission returns (bool) {
    uint256 balanceA;
    uint256 balanceB;
    tokenA = IERC20(pathBuy[0]);
    tokenB = IERC20(pathSell[0]);
    ...
    do{
        ...
        if(s==0){
            _swap(balanceA,pathBuy,address(this));
            s = 1;
        }else{
            _swap(balanceB,pathSell,address(this));
            s = 0;
        }
        i++;
    }while(i<txcount);
