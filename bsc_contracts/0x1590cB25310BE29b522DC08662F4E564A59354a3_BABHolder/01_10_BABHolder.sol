...
error BABHolder__InvalidMintAmount();
error BABHolder__MaxSupplyExceeded();
error BABHolder__InsufficientFunds();
error BABHolder__AllowlistSaleClosed();
error BABHolder__AddressAlreadyClaimed();
error BABHolder__InvalidProof();
error BABHolder__PublicSaleClosed();
error BABHolder__TransferFailed();
error BABHolder__NonexistentToken();
...
contract BABHolder is ERC721AQueryable, Ownable, ReentrancyGuard {
    ...
    modifier mintCompliance(uint256 _mintAmount, uint256 _maxMintAmount) {
        if (_mintAmount < 1 || _mintAmount > _maxMintAmount)
            revert BABHolder__InvalidMintAmount();

        if (totalSupply() + _mintAmount > iMaxSupply)
            revert BABHolder__MaxSupplyExceeded();
    ...
    modifier mintPriceCompliance(uint256 _mintAmount) {
        if (msg.value < sMintPrice * _mintAmount)
            revert BABHolder__InsufficientFunds();
        ...
        mintPriceCompliance(_mintAmount)
    {
        if (sSaleState != SaleState.AllowlistOnly)
            revert BABHolder__AllowlistSaleClosed();

        if (sAllowlistClaimed[_msgSender()])
            revert BABHolder__AddressAlreadyClaimed();

        bytes32 leaf = keccak256(abi.encodePacked(_msgSender()));

        if (!MerkleProof.verify(_merkleProof, sMerkleRoot, leaf))
            revert BABHolder__InvalidProof();
        ...
        mintPriceCompliance(_mintAmount)
    {
        if (sSaleState != SaleState.PublicOpen)
            revert BABHolder__PublicSaleClosed();
    ...
    function withdraw() public onlyOwner nonReentrant {
        ...
        if (!success) revert BABHolder__TransferFailed();
    }
    ...
    {
        if (!_exists(_tokenId)) revert BABHolder__NonexistentToken();
