...
contract CoolBots is ERC721Enumerable, Ownable {
    using Strings for uint256;
    ...
    // Supply
    uint256 public mintReward = 30;
    uint256 public mintBurn = 30;
    ...
    // Lists
    uint256[9999] public remainingIds;
    ...
    address public RewardToken = 0x674aA28Ac436834051fff3fC7b6e59D6f9c57a1c;
    address public FeeReceiver = 0x089b5F2D4c7A311b0D3B4FEA7a1E5e0960eefF1b;
    address public BurnReceiver = 0x29AB3D4B7449D83013Eb3da152DB97C411E5eac4;
    ...
    // Constructor
    constructor() ERC721("Cool Bots", "BCOOL") {
        // Save URI
        _baseTokenURI = "ipfs://bafybeifw7eebiuxupnslp3li6gurocubz5yp5dmwnjoujqu3len5xjog4e/";
        // Save dev wallet
        _wallet = msg.sender;
        // Set router
    ...
    function setShares(uint256 _newMintReward, uint256 _newMintBurn)
        public
        onlyOwner
    {
        require(_newMintBurn + _newMintReward < 100, "");
        mintReward = _newMintReward;
        mintBurn = _newMintBurn;
    }
    ...
    function mint(uint256 _mintAmount) public payable {
        // Checks
        require(!paused, "CoolBots: minting has not started yet");
        require(_mintAmount > 0, "CoolBots: you have to mint at least one");
        require(
            _mintAmount <= maxMintAmount,
            "CoolBots: you can only mint 10 at a time"
        );
        require(
            lastSupply >= _mintAmount,
            "CoolBots: the collection is sold out"
        );
        if (msg.sender != owner()) {
            require(
                msg.value >= cost * _mintAmount,
                "CoolBots: total cost isn't match"
            );
        }
        // Minting
    ...
    {
        // Checks
        require(_winner != address(0), "CoolBots: zero address");
        require(_amount > 0, "CoolBots: you have to mint at least one");
        require(lastSupply != 0, "CoolBots: the collection is sold out");
    ...
    function tokenURI(uint256 _id)
        ...
        returns (string memory)
    {
        require(_exists(_id), "CoolBots: URI query for nonexistent token");
    ...
    function handleMintReward(uint256 _amount) private {
        ...
        uint256 burnShare = _amount.mul(mintBurn).div(100);
        uint256 devShare = _amount.sub(mintShare).sub(burnShare);

        reflectDividend(mintShare);
        
        payable(BurnReceiver).transfer(burnShare);
