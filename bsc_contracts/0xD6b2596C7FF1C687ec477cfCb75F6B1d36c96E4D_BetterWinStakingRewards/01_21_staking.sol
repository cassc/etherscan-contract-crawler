...
contract BetterWinStakingRewards is  ReentrancyGuard, Pausable, Ownable {
    ...
    mapping(address => uint256) public lockupEnds;
    mapping(uint256 => uint256) private lockupPeriod;
    /* ========== CONSTRUCTOR ========== */
    ...
    constructor(
        ...
        durationBonus[0] = 100; // 1 month
        durationBonus[1] = 120; // 3 month
        durationBonus[2] = 130; // 6 month
        durationBonus[3] = 140; // 12 month

        lockupPeriod[0] = 30 days;
        lockupPeriod[1] = 3 * 30 days;
        lockupPeriod[2] = 6 * 30 days;
        lockupPeriod[3] = 12 * 30 days;
    ...
    function stake(uint256 amount, uint256 lockupPeriodId) external nonReentrant whenNotPaused updateReward(msg.sender) {
        require(amount > 0, "Cannot stake 0");
        _totalSupply = _totalSupply.add(amount);
        _balances[msg.sender] = _balances[msg.sender].add(amount);
        lockupEnds[msg.sender] = block.timestamp + lockupPeriod[lockupPeriodId] ;
    ...
    function withdraw(uint256 amount) public nonReentrant updateReward(msg.sender) {
        require(lockupEnds[msg.sender] > block.timestamp, "Cannot withdraw till lockup ends");
