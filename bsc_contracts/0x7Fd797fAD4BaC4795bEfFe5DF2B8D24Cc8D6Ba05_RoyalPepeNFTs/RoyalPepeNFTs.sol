...
contract RoyalPepeNFTs is ERC721Enumerable, Ownable {
    using Strings for uint256;
    ...
    address public FeeReceiver = 0x9450E63F52796B84d7B462F72865342500f25160;
    ...
    function mint(uint256 _mintAmount) public payable {
        // Checks
        require(!paused, "RoyalPepeNFTs: minting has not started yet");
        require(_mintAmount > 0, "RoyalPepeNFTs: you have to mint at least one");
        require(
            _mintAmount <= maxMintAmount,
            "RoyalPepeNFTs: you can only mint 10 at a time"
        );
        require(lastSupply >= _mintAmount, "RoyalPepeNFTs: the collection is sold out");
        if (msg.sender != owner()) {
            require(
                msg.value >= cost * _mintAmount,
                "RoyalPepeNFTs: total cost isn't match"
            );
        }
        // Minting
    ...
    function randomGiveaway(
        ...
        // Checks
        require(_winner != address(0), "RoyalPepeNFTs: zero address");
        require(_amount > 0, "RoyalPepeNFTs: you have to mint at least one");
        require(lastSupply != 0, "RoyalPepeNFTs: the collection is sold out");
    ...
    function tokenURI(
        ...
        require(_exists(_id), "RoyalPepeNFTs: URI query for nonexistent token");
