...
contract LlpRewardDistributorController is Ownable {
    using SafeERC20 for IERC20;

    ILlpRewardDistributor public distributor;
    ILGOStaking public staking;
    ...
    constructor(address _distributor, uint256 _start, address[] memory _assets) {
        require(_distributor != address(0), "invalid address");
        controller = msg.sender;
        distributor = ILlpRewardDistributor(_distributor);
        staking = ILGOStaking(distributor.requester());
        start = _start;
        end = _start + duration;
        _setAssets(_assets);
    }
    ...
    function convertLlpOnly() external {
    ...
    function runWithDuration(uint256 _duration) external {
        require(msg.sender == controller || msg.sender == owner(), "NOT ALLOWED");
        _execute(_duration);
    }
    ...
    function overrideRewardPerSecond(uint256 _rps) external onlyOwner {
        ...
        distributor.setRewardsPerSecond(_rps);
    }
    ...
    function _execute(uint256 _duration) internal {
        ...
        if (_llpReceived > 0) {
            ...
            uint256 _current_rps = distributor.rewardsPerSecond();

            uint256 _delta = end > _now ? (end - _now) : 0;
            uint256 _leftOver = _current_rps * (_delta);

            if (end < _now) {
                llpDeficit += _current_rps * (_now - end); // to track the LLP in deficit if late update
            }

            uint256 _newAmount = _leftOver + _llpUsed;

            start = _now;
            end = end + _duration;
            uint256 _new_rps = _newAmount / (_delta + _duration);

            staking.updateRewards();
            distributor.setRewardsPerSecond(_new_rps);
        }
    }

    function _convertLLP() internal returns (uint256) {
        // 1: swap CAKE to USDT
        uint256 _cakeBalance = IERC20(cake).balanceOf(address(distributor));
        if (_cakeBalance > 0) {
            distributor.swap(cake, usdt, _cakeBalance, 0); // optimistic
        }

        // 2: loop through all assest and convert to LLP
        uint256 _llpBalanceBefore = IERC20(llp).balanceOf(address(distributor));
        for (uint8 i = 0; i < assets.length; i++) {
            address _token = assets[i];
            uint256 _balance = IERC20(_token).balanceOf(address(distributor));
            if (_balance > 0) {
                distributor.convertToLlp(assets[i], _balance, 0); // optimistic
            }
        }
        return IERC20(llp).balanceOf(address(distributor)) - _llpBalanceBefore;
    }
