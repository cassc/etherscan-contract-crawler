...
contract StrategyStatistics is LogicUpgradeable {
     ...
     */
    function setLendBorrowFarmingPair(
        address _lendBorrowFarmingPair
    ) external onlyOwner {
        ...
        )
    {
        ...
        for (uint256 index = 0; index < reserves.length; ) {
            ...
            stakedAmountUSD +=
                (reserved1 *
                    _findPriceUSD(
                        IPancakePair(reserve.lpToken).token1() == reserve.tokenA
                            ? reserve.tokenA
                            : reserve.tokenB,
                        priceUSDList
                    )) /
     ...
     */
    function _getStorageAmount(
        address logic,
        PriceInfo[] memory priceUSDList
    ) public view returns (uint256 storageAmountUSD) {
        storageAmountUSD = 0;
...
contract StrategyStatisticsOld is LogicUpgradeable {
    using SafeERC20Upgradeable for IERC20Upgradeable;
