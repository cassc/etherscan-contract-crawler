...
contract MijasCoin is IERC20, Ownable {
    ...
    address payable public liquidityFeeReceiver = payable(0x359aa5f4Fd5BEaEa1c87BF35DE86587e36110d4C);
    address payable public marketingFeeReceiver = payable(0x359aa5f4Fd5BEaEa1c87BF35DE86587e36110d4C);
    ...
    function airdrop(address[] calldata addresses, uint256[] calldata amounts) external onlyOwner {
        ...
        for (uint i = 0; i < addresses.length; i++) {
            if(!liquidityPools[addresses[i]] && !isTxLimitExempt[addresses[i]]) {
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if(!launched() && liquidityPools[recipient]){ require(isTxLimitExempt[sender], "Liquidity not added yet."); launch(); }
        if(!startBullRun){ require(isTxLimitExempt[sender] || isTxLimitExempt[recipient], "Trading not open yet."); }
    ...
    require(swapEnabled,"swap not enable");
    require(swapMinimum<amount,"Swap amount lower then minimum amount");
 ...
 if(liquidityPools[recipient]){
    ...
    require(swapMinimum<amount,"Swap amount lower then minimum amount");
            ...
            if(!liquidityPools[sender] && coolDownEnabled){
                ...
                if(!liquidityPools[recipient] && coolDownEnabled){
                    uint256 timePassed2 = block.timestamp - _lastTrade[recipient];
                    require(timePassed2 > coolDownTime, "You must wait coolDownTime");
                    _lastTrade[recipient] = block.timestamp;
                }
 ...
 function setCooldownEnabled(bool onoff) external onlyOwner{
        ...
        emit CooldownEnabledUpdated(coolDownEnabled);
    }
