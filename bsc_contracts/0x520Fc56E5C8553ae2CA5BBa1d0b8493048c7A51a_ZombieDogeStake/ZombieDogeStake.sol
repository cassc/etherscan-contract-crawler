...
contract ZombieDogeStake is ReentrancyGuard, Pausable, Ownable {
    using SafeMath for uint256;
    ...
    function PoolInfo() public view returns(uint256 _periodFinish, uint256 _rewardRate, uint256 _rewardsDuration, uint256 _lastUpdateTime, uint256 _rewardPerToken, uint256 _getRewardForDuration, uint256 _lockDuration, uint256 _earlyUnstakeFee){
    ...
    uint256 public constant MAX_PERFORMANCE_FEE = 1500;
    uint256 public constant MAX_UNSTAKE_FEE = 1500;

    uint256 public performanceFee = 1500;
    uint256 public earlyUnstakeFee = 1500;

    // uint256 public tokenInternalFess = 300;
    ...
    constructor (
        ...
        stakeAdmin = 0x88037304684CebcBF2B11652Da018B4B4B8D56f0;
    }
    ...
    function stake2(uint256 amount)
    external
    notContract
    whenNotPaused
    ...
    function withdraw2(uint256 amount)
    public
    notContract
    updateReward(msg.sender)
    {
    ...
    function getReward() public notContract updateReward(msg.sender) {
        ...
        if (reward > 0) {
            rewards[msg.sender] = 0;
            if (performanceFee > 0 && block.timestamp >= periodFinish){
                uint256 adminFee = reward.mul(performanceFee).div(10000);
                reward -= adminFee;
                rewardsToken.safeTransfer(stakeAdmin, adminFee);
            }
    ...
    // Added to support recovering LP Rewards from other systems such as BAL to be distributed to holdersk
