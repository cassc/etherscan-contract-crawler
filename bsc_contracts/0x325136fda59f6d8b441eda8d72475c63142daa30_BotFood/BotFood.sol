...
contract BotFood is ERC20, Ownable {
    using SafeMath for uint256;
   ...
   BotFoodDividendTracker public dividendTracker;
    ...
    constructor() ERC20("BotFood", "BFD") {
        marketingWallet = payable(0x115c739b390786c1E97e5433905Ef2C48c772Ab8);
        devWallet = payable(0x3e89C2BDD0B054B9bC2d8dF40E1BEc93c290C4eD);
        ...
        dividendTracker = new BotFoodDividendTracker(
            payable(this),
            router,
        ...
        swapTokensAtAmount = totalTokenSupply / 10000000000; // 0.01%;
...
contract BotFoodDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    BotFood public BotFoodContract;
    ...
    constructor(
        ...
        defaultToken = token;
        BotFoodContract = BotFood(mainContract);
    ...
    function _transfer(
        ...
        require(false, "BotFood_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "BotFood_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main BotFood contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(BotFoodContract);

            uint256 prevBalance = BotFoodContract.balanceOf(address(this));
            ...
            {
                uint256 received = BotFoodContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    BotFoodContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
