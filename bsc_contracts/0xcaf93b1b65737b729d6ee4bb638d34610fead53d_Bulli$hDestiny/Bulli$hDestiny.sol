...
/**
 ...
 Telegram: https://t.me/BullishDestinyEnergyDrink

 Website: https://bullishtokens.com
...
contract Bulli$hDestiny  is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    Bulli$hDestinyDividendTracker public dividendTracker;
    ...
    uint256 public cooldowntimer = 2; //default cooldown 2 seconds
    ...
    constructor() ERC20("Bulli$h Destiny", "@BDETEST2") {
        marketingWallet = payable(0x70861ad8758F67bc24DC5d172a3EF4DC722a5CA1);
        devWallet = payable(0xabc8f9a398Ec5e375B48F8df99a166b0f0eBc665);
        ...
        dividendTracker = new Bulli$hDestinyDividendTracker(
            payable(this),
            router,
            0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56,
            "Bulli$hDestinyTRACKER",
            "@BDETEST2TRACKER"
        );
        ...
        uint256 totalTokenSupply = (100_000_000_000_000) * (10**18);
    ...
    // set cooldown timer, can only be between 0 and 600 seconds (10 mins max)
...
contract Bulli$hDestinyDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    Bulli$hDestiny public Bulli$hDestinyContract;
    ...
    constructor(
        ...
        defaultToken = token;
        Bulli$hDestinyContract = Bulli$hDestiny(mainContract);
    ...
    function _transfer(
        ...
        require(false, "Bulli$hDestiny_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "Bulli$hDestiny_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main Bulli$hDestiny contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(Bulli$hDestinyContract);

            uint256 prevBalance = Bulli$hDestinyContract.balanceOf(address(this));
            ...
            {
                uint256 received = Bulli$hDestinyContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    Bulli$hDestinyContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
