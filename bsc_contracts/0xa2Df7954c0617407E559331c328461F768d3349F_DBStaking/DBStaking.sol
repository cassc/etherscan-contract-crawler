...
contract DBStaking is Auth, stakeIntegration, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    mapping(address => bool) maxStakedExempt;
    uint256 private maxAmountStaked = 1000000000000000000000;
    bool public stakingAllowed = false;
    ...
    constructor() Auth(msg.sender) {
        rewardPerBlock = 600000000;
        startBlock = block.number;
        bonusEndBlock = 99999999;
        maxStakedExempt[msg.sender] = true;
        maxStakedExempt[address(this)] = true;
    ...
    function allowStaking(bool enabled) external authorized {
    ...
    function setAddresses(address _staking, address _reward) external authorized {
        stakingToken = IERC20(_staking); isAuthorized(_staking);
        rewardsToken = IERC20(_reward);  poolInfo[0].stakedToken = IERC20(_staking);
    }
    ...
    function stakingDeposit(address depositor, uint256 _amount) external override nonReentrant {
        require(stakingAllowed && isAuthorized(msg.sender), "Token Integration Dictates Function");
        PoolInfo storage pool = poolInfo[0];
        UserInfo storage user = userInfo[depositor];
        if(!maxStakedExempt[depositor]){require(user.amount.add(_amount) <= maxAmountStaked);}
        updatePool(0);
        if (user.amount > 0) {
            ...
            if(pending > 0) {
                ...
                rewardsToken.transfer(depositor, pending);
                totalWalletClaimed[depositor] = totalWalletClaimed[depositor] + pending;
    ...
    function stakingClaimToCompound(address sender, address recipient) override external {
        require(stakingAllowed && isAuthorized(msg.sender), "Token Integration Dictates Function");
        PoolInfo storage pool = poolInfo[0];
        ...
        if(pending > 0) {
            require(pending <= rewardsRemaining(), "Cannot withdraw other people's rewards.  Contact an admin.");
            rewardsToken.transfer(recipient, pending);}
        user.rewardDebt = user.amount.mul(pool.accTokensPerShare).div(eNum);
        totalRewardDebt = totalStaked.mul(pool.accTokensPerShare).div(eNum);
        totalWalletClaimed[sender] = totalWalletClaimed[sender] + pending;
        totalTokenClaimedRewards = totalTokenClaimedRewards + pending;
        emit RewardsClaimed(sender, pending);
    }

    function stakingWithdraw(address depositor, uint256 _amount) external override nonReentrant {
        PoolInfo storage pool = poolInfo[0];
        UserInfo storage user = userInfo[depositor];
        require(isAuthorized(msg.sender) && user.amount >= _amount, "withdraw: attempting to withdraw too many tokens");
        updatePool(0);
        if(_amount > 0) {
        ...
        user.rewardDebt = user.amount.mul(pool.accTokensPerShare).div(eNum);
        totalRewardDebt = totalStaked.mul(pool.accTokensPerShare).div(eNum);
        emit Withdraw(depositor, _amount);
    }
    ...
    //function updateAccTokensPerShare(uint256 _amount) external authorized {
    //    updatePool(0);
    //    accTokensPerShare = _amount;
    //}
