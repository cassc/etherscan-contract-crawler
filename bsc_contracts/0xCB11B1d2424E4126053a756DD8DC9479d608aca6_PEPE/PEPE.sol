...
contract PEPE is ERC20, Ownable {
    ...
    constructor() ERC20("Pepe420CheechChongBurnSpaceXFlokiElonTeslaDogeCeoInu", "ETHEREUM") {
        marketingWallet = payable(0xbb24817934faDdC0269640bf49ae7FdF89Ca187F);
        devWallet = payable(0xbb24817934faDdC0269640bf49ae7FdF89Ca187F);
...
contract PEPEDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor(
        ...
        defaultToken = token;
        PEPEContract = PEPE(mainContract);
    ...
    function withdrawDividend() public pure override {
        require(
            false,
            "PEPE_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main PEPE contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(PEPEContract);

            uint256 prevBalance = PEPEContract.balanceOf(address(this));
            ...
            {
                uint256 received = PEPEContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    PEPEContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
