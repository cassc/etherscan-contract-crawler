...
abstract contract DataStorage is EnumerableArrays {
    ...
    function lotteryFractions() public view returns(uint256[] memory fractions) {
        uint256 len = _lotteryFractions.length;
        fractions = new uint256[](len);
        for (uint256 i; i < len; i++) {
            fractions[i] = _lotteryFractions[i];
        }
    }
    ...
    function todayEveryPointValue() public view returns(uint256) {
        uint256 denominator = todayTotalPoint > 0 ? todayTotalPoint : 1;
        return todayAllRewardValue() / denominator;
    }
    ...
    function todayLotteryShare() public view returns(uint256[] memory shares, uint256 remainder) {
        uint256 lotteryValue = todayLotteryValue();
        uint256 len = _lotteryFractions.length;
        shares = new uint256[](len);
        for (uint256 i; i < len; i++) {
            shares[i] = lotteryValue / _lotteryFractions[i];
            lotteryValue = lotteryValue % _lotteryFractions[i];
        }
        remainder = lotteryValue;
    }

    function todayWriterReward() public view returns(uint256 writerReward) {
        (,writerReward) = todayLotteryShare();
    }
