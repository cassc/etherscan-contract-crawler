...
contract PINKY is tokenStaking, IERC20, Ownable {
    ...
    uint256 private boneFee = 100;
    uint256 private tokenStakingFee = 50;
    uint256 private lpStakingFee = 50;
    ...
    address BONE = 0x8AC76a51cc950d9822D68b83fE1Ad97B32Cd580d;
    ...
    address internal bone_receiver = 0x3C9876CF1D3271674f377f593F96d4838b481f35;
    ...
    function compound() override external feelessTransaction {
        ...
        uint256 afterBalance = eBONE.balanceOf(address(this)).sub(initialBalance);
        swapBONEforTokens(afterBalance, msg.sender);
    ...
    function setStructure(uint256 _liquidity, uint256 _marketing, uint256 _rewards, uint256 _bone, uint256 _development, uint256 _total, uint256 _sell, uint256 _trans) external onlyOwner {
        liquidityFee = _liquidity; marketingFee = _marketing; rewardsFee = _rewards; boneFee = _bone;
    ...
    function setInternalAddresses(address _marketing, address _liquidity, address _development, address _bone) external onlyOwner {
        marketing_receiver = _marketing; liquidity_receiver = _liquidity; development_receiver = _development; bone_receiver = _bone;
        isFeeExempt[_marketing] = true; isFeeExempt[_liquidity] = true; isFeeExempt[_development] = true; isFeeExempt[_bone] = true;
    ...
    function swapAndLiquify(uint256 tokens) private lockTheSwap {
        uint256 _denominator = (liquidityFee.add(marketingFee).add(developmentFee).add(rewardsFee).add(boneFee)).mul(2);
        ...
        if(marketingAmount > 0){eBONE.transfer(marketing_receiver, marketingAmount);}
