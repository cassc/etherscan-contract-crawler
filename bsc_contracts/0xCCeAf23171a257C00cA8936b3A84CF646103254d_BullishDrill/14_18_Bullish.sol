...
contract Bullish is ReentrancyGuard, Pausable, ERC1155Holder
{
	...
	function refresh_reward_per_share(uint256 _pool_id) public returns(uint256)
	{
		...
		if(paused() == false)
		{
			...
			return (mint_reward_amount / _pool.total_staked_amount);
		}
	...
	function _distribute_rewards(uint256 _pool_id, address _address_user) public returns(uint256)
	{
		PoolInfo storage _pool = pool_info[_pool_id];
		UserInfo storage _user = user_info[_pool_id][_address_user];
		RewardInfo storage _reward = reward_info[_pool.address_token_reward];
		...
		return final_user_reward_amount;
	}
