...
contract ERC721Staking is Ownable, ReentrancyGuard {
    ...
    function stake(uint256[] calldata _tokenIds) external nonReentrant {
        ...
        for (uint256 i; i < len; ++i) {
            ...
            stakers[msg.sender].tokenStaked.push(_tokenIds[i]);
        }
    ...
    function withdraw(uint256[] calldata _tokenIds) external nonReentrant {
        ...
        stakers[msg.sender].unclaimedRewards += rewards;
        uint256 len = _tokenIds.length;
        uint256 stoken = stakers[msg.sender].tokenStaked.length;
        for (uint256 i; i < len; ++i) {
            require(stakerAddress[_tokenIds[i]] == msg.sender);
            stakerAddress[_tokenIds[i]] = address(0);
            for (uint256 z; z < stoken; z++) {
                if (_tokenIds[i] == stakers[msg.sender].tokenStaked[z]) {
                    removeNft(z);
                }
            }
    ...
    function removeNft(uint256 index) internal {
        require(index < stakers[msg.sender].tokenStaked.length);
        stakers[msg.sender].tokenStaked[index] = stakers[msg.sender]
            .tokenStaked[stakers[msg.sender].tokenStaked.length - 1];
        stakers[msg.sender].tokenStaked.pop();
    }
    ...
    function userStakeInfo(address _user)
        ...
        returns (
            ...
            uint256[] memory _nftokens
        )
    {
        return (
            stakers[_user].amountStaked,
            availableRewards(_user),
            stakers[_user].tokenStaked
        );
    }
