    ...
    IBet2WinUpgradeable
{
    ...
    BitMapsUpgradeable.BitMap private __isResolved;
    ...
    function resolveMatch(
        ...
        uint256 concatId;
        assembly {
            concatId := or(shl(24, gameId_), matchId_)
            mstore(0x00, concatId)
    ...
    //         uint256 remainOdd = odd - HOUSE_EDGE_PERCENT - referralPercent;
    ...
    function settleBet(
        ...
        uint256 concatId = (gameId_ << 24) | matchId_;
        require(__isResolved.get(concatId), "BET2WIN: UNSETTLE_BET");
        uint256 scores = __resolves[uint32(concatId)][status_];
    ...
    function __processBet(
        ...
        require(
            !__isResolved.get((uint256(gameId) << 24) | matchId),
            "BET2WIN: MATCH_RESOLVED"
        );
        ...
        uint256 usdSize = payment_.amount;
        {
            ...
            if (betDetail != 0)
                require(odd * 100 <= betDetail.odd(), "BET2WIN: INVALID_ODD");
