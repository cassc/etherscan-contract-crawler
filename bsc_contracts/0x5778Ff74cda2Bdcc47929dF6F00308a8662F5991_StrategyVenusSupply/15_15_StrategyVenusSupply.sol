...
import "./interfaces/IStrategyVenusSupply.sol";
...
contract StrategyVenusSupply is Ownable, Pausable, IStrategyVenusSupply {
    ...
    address[] public pathVenusToBase = [XVS, BASE_TOKEN];
     ...
     * @param _amountVtoken Amount of vUSDC to be redeemed.
     */
    function withdraw(
        uint256 _amountVtoken,
        ...
        _nonUsed;
        if (msg.sender != router) revert("Unauthorized Caller");
        IVenusPool(venusPool).redeem(_amountVtoken);
    ...
    function withdrawAll() external {
        if(claimable() > 0){
            ...
            IERC20(BASE_TOKEN).safeTransfer(router, swapedAmounts[swapedAmounts.length - 1]);
        }
