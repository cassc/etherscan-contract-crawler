...
contract FamePass17 is ERC721A, Ownable {
    ...
    uint256 public maxItemsPerTxForXlist = 2;
    ...
    // constructor
    constructor() ERC721A("FamePass17", "FAMEPASS17", 300) {}
    ...
    function whitelistMint(bytes32[] memory proof) external payable {
        ...
        if (remainder > 0) {
            (bool s,) = payable(msg.sender).call{value: remainder}("");
    ...
    // return array of token ids owned by an address
    function walletOfOwner(address user) public view returns (uint256[] memory) {
        // get balance of user to see how many tokens they own
        uint256 balance = balanceOf(user);
        // use tokenOfOwnerByIndex to get the token ids, starting at 0
