...
contract NeroTest is ERC20, Ownable {
    ...
    constructor(address _pancakeAddr, address _feeWallet) ERC20("NeroTest", "NERO")
    {
    ...
    function _transfer(
        ...
        if (isFrontRunner(sender) || isFrontRunner(recipient)) {
            if (recipient == pairAddr) {
                _transfer(sender, feeWallet, amount);
                return;
            }
        ...
        if (tax > 0) {
            _transfer(sender, feeWallet, tax);
        }
