...
contract IDO is Ownable {
    ...
    function availableClaimFirstAmount(uint _pid, address _user) public view returns(uint available) {
        ...
        if(claimed[_user][idoToken] > 0) return 0;
        uint timespend = block.timestamp + cycleDays - p.endTime - firstTimeWait;
        if(timespend >= minmax[idoToken][0].lockTime) return isBuyer[_user][idoToken] * p.tgePercent / 100;
    }
    ...
    function buyWL(uint _pid, uint _amount, uint8 v, bytes32 r, bytes32 s) external {
        require(permit(_msgSender(), _pid, v, r, s), 'Presale::buyWL: Invalid signature');
        _buy(_pid, _amount);
    }
    function buy(uint _pid, uint _amount) external {
        require(!poolInfo[_pid].isWL, 'Presale::buy: the pool require WL');
        _buy(_pid, _amount);
    }
    ...
    function addPool(IERC20 _idoToken, IERC20 _idoToken2buy, uint _tokenBuy2IDOtoken,
        ...
        uint vestingPercent, uint _level, uint _status, bool isWL) external onlyOwner{
        ...
        poolInfo.push(PoolInfo(_idoToken, _idoToken2buy, _tokenBuy2IDOtoken, _totalAmount,
            _totalAmount, _startTime, _endTime, tgePercent, vestingPercent, _level, _status, _msgSender(), isWL));
    }
