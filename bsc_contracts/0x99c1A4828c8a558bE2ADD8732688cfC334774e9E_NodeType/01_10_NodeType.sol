...
contract NodeType is INodeType, OwnersUpgradeable, NodeRewards {
    ...
    function applyWaterpackBatch(
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            ...
            Node storage userNode = u.values[tokenIds[i]];
            if (userNode.ammosUsed == 3) continue;
            userNode.ammosUsed += 1;
    ...
    function calculateNodeRewards(address user, uint tokenId) external view returns(uint,uint,uint) {
        User storage u = userOf[user];
        Node storage node = u.values[tokenId];
    ...
    function calculateBaseNodeRewards(address user, uint tokenId) external view returns(uint) {
        User storage u = userOf[user];
        Node storage node = u.values[tokenId];
