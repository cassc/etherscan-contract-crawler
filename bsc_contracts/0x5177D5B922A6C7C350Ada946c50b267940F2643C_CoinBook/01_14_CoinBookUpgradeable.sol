...
contract CoinBook is ICoinBook, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function createOrderERC20(
        ...
        uint256 listingFeeETH = getCurrentListingFee();
    ...
    function cancelOrder(uint256 _id) external override payable nonReentrant {
        uint256 cancelFeeETH = getCurrentCancelFee();
     ...
     */
    function getCurrentListingFee() public override view returns (uint256 listingFeeETH) {
        uint256 currentPrice = uint256(priceFeed.latestAnswer());
        return ((listingFeeUSD * 10**18) / currentPrice);
    }
     ...
     */
    function getCurrentCancelFee() public override view returns (uint256 cancelFeeETH) {
        uint256 currentPrice = uint256(priceFeed.latestAnswer());
        return ((cancelFeeUSD * 10**18) / currentPrice);
    }

    function getCurrentEthPrice() external override view returns (uint256 currentPrice) {
