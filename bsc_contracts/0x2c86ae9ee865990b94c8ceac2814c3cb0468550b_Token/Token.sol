...
contract Token is ERC20 {
    ...
    address payable _fund1Address = payable(address(0xF8cdf806318E1733C37a9F26d025eb94B1a545A9));
	address private _destroyAddress = address(0x000000000000000000000000000000000000dEaD);
    address public _default = address(0xCa7f12a710f6eb7A3D9d99629540Af585A7312f2);
	...
	mapping(address => uint256) public luckAmount;
	mapping(address => uint256) public luckTime;
    ...
    constructor(address tokenOwner) ERC20("TDao", "T1Dao") {
        ...
        total = 1000000 * 10**18;
        swapTokensAtAmount = total.div(200000);
    ...
    function _transfer(
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {
            ...
            if(from == uniswapV2Pair){
              if(startTime.add(10) > block.timestamp){
               ...
               if(startTime.add(600) > block.timestamp){require(amount <= 1 * 10**20,"max amount");}
        ...
        if (takeFee) {
            ...
            _takeInviterFee(from, to, amount);//2
    ...
    function swapAndLiquify(uint256 canSwapAmount) private  {
		if(balanceOf(address(this)) >= canSwapAmount){
            ...
            _fundAddress.transfer(newBalance.div(5).mul(4));
            _fund1Address.transfer(newBalance.div(5).mul(1));
		}
    }
 ...
 function _takeInviterFee(
        ...
        for (int256 i = 0; i < 2; i++) {
            ...
            super._transfer(sender, reciver, tAmount.div(100).mul(rate));
        }
    }
