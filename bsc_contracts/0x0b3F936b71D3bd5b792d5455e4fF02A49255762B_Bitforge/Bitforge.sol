...
contract Bitforge is Ownable, ReentrancyGuard {
    address constant public USDC = 0x8AC76a51cc950d9822D68b83fE1Ad97B32Cd580d;

    uint constant public elecExpendPerDevice_25days = 1016;   // 101.6 USDC 10x magnification first, then reduced
    uint constant lockTime = 25 days;
    uint constant MinutesPerDay = 1 days / 1 minutes;

    // User Invitation Rebate Bonus
    mapping (address => uint) referralBonus;

    // Total deposit amount
    mapping (address => uint) userTotalDepositAmount;

    // Number of orders deposit by users
    mapping (address => uint8) userTotalDepositOrders;

    // User => User's order array
    mapping (address => Order[]) userOrder;

    // Total number of mining machines in the mine
    uint private totalDevices;
    // Total 30-day earnings from the mine
    uint private totalProfit_30days;
    // The accumulated handling fee can only be withdrawn by the administrator
    uint private fees;
    ...
    /**
     * @dev Deposit
     * @param _depositAmount The number of deposit, with a minimum amount of 50
     * @param _invitation Address of superior invitee, fill in blackhole address if none
     */
    function deposit(uint _depositAmount, address _invitation) public {
        ...
        // First get the order array corresponding to the user
        ...
        userTotalDepositOrders[msg.sender]++;                       // Total number of deposit orders + 1
        userTotalDepositAmount[msg.sender] += _depositAmount;      // Total number of deposit + new deposit

        if(_invitation != address(0) && _invitation != msg.sender) referralBonus[_invitation] += _depositAmount * molecular / denominator;   // Inviter Rewards
    ...
    /**
     * @dev Calculate the user's revenue and continue to accrue interest if the principal is not withdraw
     */
    ...
    // Set rebate percentage, numerator - denominatorSet rebate percentage, numerator - denominator
