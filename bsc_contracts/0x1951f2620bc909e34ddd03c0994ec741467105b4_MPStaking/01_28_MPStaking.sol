    ...
    */
    ...
    function distributeRewardsPerToken (address _stakedToken) internal returns (bool complete) {
        ...
        for(uint i = 0; i < tokenCount; i ++) {
            ...
            if(rewardsAmount > 0) {
                ...
                for(uint256 j = 0; j + holderProcessingIndex < stakerCount; j ++) {
                    ...
                    uint256 rewardsPerToken = rewardsAmount.mul(percentOfStaked);
    ...
    function claimRewards (
        ...
        for (uint idx = 0; idx < tokenCount; idx ++) {
            ...
            uint256 amount = staker.rewards[token] / 10 ** 36;
            
            if(prevTotalRewards[token] >= amount)
                prevTotalRewards[token] -= amount;
            else
                prevTotalRewards[token] = 0;
          
            staker.rewards[token] -= amount * 10 ** 36;

            if(userTotalRewardsPerToken[_staker][token] >= amount * 10 ** 36) {
                userTotalRewardsPerToken[_staker][token] -= amount * 10 ** 36;
            }
            else {
    ...
    function withdrawRewards (
        ...
        uint256 rewardsAmount = userTotalRewardsPerToken[stakerAddr][_token] / 10 ** 36;
        ...
        for(uint idx = 0; idx < tokenCount; idx ++) {
            ...
            uint256 amount = staker.rewards[_token] / 10 ** 36;
            if(amount > 0) {
                staker.rewards[_token] -= amount.mul(10 ** 36).mul(_withdrawPercent).div(denominator);
            }
        }
        ...
        userTotalRewardsPerToken[stakerAddr][_token] -= withdrawAmount * 10 ** 36;
        
        if(prevTotalRewards[_token] >= withdrawAmount)
            prevTotalRewards[_token] -= withdrawAmount;
        else
            prevTotalRewards[_token] = 0;
    ...
    function compoundRewards (
        ...
        for(uint idx = 0; idx < tokenCount; idx ++) {
            ...
            uint256 amount = staker.rewards[_token] / 10 ** 36;
            if(amount > 0) {
                staker.rewards[_token] -= amount.mul(10 ** 36).mul(_compoundPercent).div(denominator);
            }
        }
        ...
        userTotalRewardsPerToken[stakerAddr][_token] -= compoundAmount * 10 ** 36;

        if(prevTotalRewards[_token] >= compoundAmount) 
    ...
    function getTokenRewardsOfUser (address _wallet, address _token) public view returns (uint256) {
        return userTotalRewardsPerToken[_wallet][_token] / 10 ** 36;
    }
    ...
    function getTokenStakerCount (address _token) external view returns (uint256) {
        uint256 stakerCountPerToken = tokenStakers[_token].length;
        return stakerCountPerToken;
    }

    // operations for migration
    function unStakeWithRewardsForMigration ( address _token) external whenNotPaused nonReentrant returns (bool) {
        ...
        // Getting rewards
        claimRewardsForMigration(msg.sender, _token);
        ...
        totalStakedAmount[_token] -= unStakingAmount;
        stakeInfos[msg.sender][_token].amount = 0;

        stakingEscrow.transferTokenTo(_token, msg.sender, unStakingAmount);
        return true;
    }

    function claimRewardsForMigration (
        address _staker,
        address _stakedToken
    ) internal {
        ...
        for (uint idx = 0; idx < tokenCount; idx ++) {
            ...
            if(amount > 0) {
                IERC20Upgradeable(token).transfer(_staker, amount);
            }
        }
    }
}