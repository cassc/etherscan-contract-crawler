...
contract HarvestKeeper is Ownable, ReentrancyGuard{
    ...
    struct DepositStruct{
        ...
        uint claimedAmount; // claimed bnb amount
    ...
    function deposit(address _referrer) external payable {
        require(msg.value > 0, "Incorrect ETH");
    ...
    function setCoupon(CouponSigData calldata coupon, Sig calldata sig) external payable {
        ...
        if(coupon.payAmount > 0) {
            ...
            require(msg.value >= coupon.payAmount, "Incorrect ETH");
            uint _id = _getNextDepositID();
            _incrementDepositID();
            depositState[_id].investor = msg.sender;
