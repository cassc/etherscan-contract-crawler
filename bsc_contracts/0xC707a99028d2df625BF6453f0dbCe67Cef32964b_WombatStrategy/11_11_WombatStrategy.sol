...
contract WombatStrategy is IStrategy, Initializable, PausableUpgradeable {
    ...
    function withdraw(uint256 _bnbXToWithdraw)
        external
        override
        whenNotPaused
        returns (uint256)
    {
        // 1 LP = 1 bnbX (before swap fee deduction)
        uint256 amountInBnbX = _withdrawInBnbX(_bnbXToWithdraw);
    ...
    // 2. Send BnbX to user
    function withdrawInBnbX(uint256 _bnbXToWithdraw)
        external
        whenNotPaused
        returns (uint256)
    {
    ...
    function convertBnbToShares(uint256 _amount) public view returns (uint256) {
        ...
        return _convertLPToShares(amountInLP);
    }

    function getPortfolioValueInBnbX(address _user)
        public
        view
        returns (uint256)
    {
        // 1 LP = 1 bnbX (before swap fee deduction)
        return _convertSharesToLP(userShares[_user]);
    }

    function estimateBnbXWithdrawal(uint256 _bnbXToWithdraw)
        public
        view
        returns (uint256 amount, uint256 fee)
    {
        // 1 LP = 1 bnbX (before swap fee deduction)
        return _convertLPToBnbX(_bnbXToWithdraw);
    }
        ...
        )
    {
        ...
        amountInBnbX = _depositToBnbX();
        amountInLP = _depositToWombat(amountInBnbX);
        amountInShares = _convertLPToShares(amountInLP);
    }

    // Deposit bnbX to Wombat Liquidity Pool and receive Wombat Liquidity Pool token
    function _depositToWombat(uint256 _amount) private returns (uint256) {
    ...
    function _depositToBnbX() private returns (uint256) {
    ...
    function _withdrawInBnbX(uint256 _amountInLP) private returns (uint256) {
        uint256 _shares = _convertLPToShares(_amountInLP);
        require(userShares[msg.sender] >= _shares, "Insufficient balance");

        totalShares -= _shares;
        userShares[msg.sender] -= _shares;
        totalInLP -= _amountInLP;

        uint256 pid = wombatMaster.getAssetPid(
            wombatPool.addressOfAsset(address(bnbX))
        );
        wombatMaster.withdraw(pid, _amountInLP);
    ...
    function _convertLPToBnbX(uint256 _amount)
        private
        view
        returns (uint256 amount, uint256 fee)
    {
        return wombatPool.quotePotentialWithdraw(address(bnbX), _amount);
    }
        ...
        returns (uint256)
    {
        ...
        return (_amount * _totalInLP) / _totalShares;
    }

    function _convertLPToShares(uint256 _amount)
        private
        view
        returns (uint256)
    {
        ...
        return (_amount * _totalShares) / _totalInLP;
    }
