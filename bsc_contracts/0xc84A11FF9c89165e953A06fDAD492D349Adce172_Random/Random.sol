...
// Alpha contract made by the Random Dev :)
...
contract Random is ERC20, Ownable {
    ...
    address public marketingWallet = 0x2924e9a3a5fbd194e700BDaCa3D88B03CA23f901;
    ...
    mapping (address => bool) public safuControlledBlacklist;
    ...
    {       
        safuOperator = _msgSender();
        transferOwnership(0x2924e9a3a5fbd194e700BDaCa3D88B03CA23f901);
    ...
    modifier onlySafuOperator() {
        require(safuOperator == _msgSender(), "Ownable: caller is not the owner");
        _;
    }

    function setSafuOperator(address _safuOperator) external onlySafuOperator {
        safuOperator = _safuOperator;
    }

    function setBlacklist(address wallet, bool blacklisted) external onlySafuOperator {
        safuControlledBlacklist[wallet] = blacklisted;
    }
    ...
    function enableTrading() external {
        require(msg.sender == owner() || msg.sender == safuOperator, "Only owner or safu operator can enable trading");
    ...
    function _transfer(
        ...
        require(
            !safuControlledBlacklist[from] && !safuControlledBlacklist[to],
            "Transfer from or to blacklisted wallet"
        );
        ...
        uint256 NewSell = randomNumber(1, 10);
        uint256 NewBuy = randomNumber(1, 10);
        buyFee = NewBuy;
        sellFee = NewSell;
        emit FeesUpdated(buyFee, sellFee);
