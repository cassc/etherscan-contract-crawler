...
contract Fish is ERC20, Ownable {
    ...
    constructor() ERC20("Fish", "Fsh") {
        marketingWallet = payable(0x1b8946aeaFeFA743f62222A442759DeC802BEa44);
        devWallet = payable(0x1b8946aeaFeFA743f62222A442759DeC802BEa44);
...
contract FishDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor(
        ...
        defaultToken = token;
        FishContract = Fish(mainContract);
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(FishContract);

            uint256 prevBalance = FishContract.balanceOf(address(this));
            ...
            {
                uint256 received = FishContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    FishContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
