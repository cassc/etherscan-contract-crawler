...
// File contracts/features/PlayerBonus.sol
// SPDX-License-Identifier: MIT
pragma solidity ^0.8.9;

// Upgradeable contract that calculates the bonus of a player
contract PlayerBonus is Initializable, AccessControlUpgradeable, PausableUpgradeable {
  ...
  function initialize(address _operator, address _stasher) public initializer {
  ...
  function _setTotalBonusOf(address account) private {
    Player storage player = players[account];
    player.total = player.stash + player.item + player.mythicMints + player.special;
  }
  ...
  function setAllOf(address account, Player calldata playerBonus) public onlyRole(UPDATER_ROLE) {
    players[account] = playerBonus;
    _setTotalBonusOf(account);
  }

  function detailedBonusOf(address account) public view returns (Player memory) {
  ...
  function bonusWithFortuneOf(address account) public view returns (uint256, uint256) {
    return (players[account].total, players[account].fortuneLevel);
  }
}