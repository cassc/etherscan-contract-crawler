...
// File contracts/features/MhtMigrator.sol
// SPDX-License-Identifier: MIT
pragma solidity ^0.8.9;

contract MhtMigrator is Initializable, PausableUpgradeable, AccessControlUpgradeable {
  ...
  uint256 public finalExchangeableBlock;

  event Migrated(address account, uint256 amount);

  // TODO deployar novamente esse cara
  function initialize(
    address _operator,
    address _mht,
    address _bct,
    ...
    finalExchangeableBlock = startBlock + 28800 * 180; // 180 days

    bct.approve(_stasher, type(uint256).max);
  }

  function migrate() external whenNotPaused {
    require(block.number <= finalExchangeableBlock, "E170");

    uint256 amount = mht.balanceOf(msg.sender);
    _migrate(amount);
  }

  function _migrate(uint256 amount) internal {
    // Fetch and burn MHT
    mht.burnFrom(msg.sender, amount);

    // Send BCT
    bct.transfer(msg.sender, amount);

    // Update total migrated
    totalMigrated += amount;

    emit Migrated(msg.sender, amount);
  }

  function burnRemaining() external onlyRole(OPERATOR_ROLE) {
    require(block.number > finalExchangeableBlock, "E171");

    uint256 remainingMht = mht.balanceOf(address(this));
    if (remainingMht > 0) {
      mht.burn(remainingMht);
    }

    uint256 remainingBct = bct.balanceOf(address(this));
    if (remainingBct > 0) {
      bct.burn(remainingBct);
    }
  }
