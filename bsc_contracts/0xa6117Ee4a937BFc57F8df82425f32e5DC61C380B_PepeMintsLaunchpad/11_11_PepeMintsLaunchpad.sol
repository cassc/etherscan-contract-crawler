...
contract PepeMintsLaunchpad is BoringOwnable, ReentrancyGuard {
  ...
  function harvestAndStake() public nonReentrant {
    ...
    if (offeringTokenAmount > 0) {
      pepeMints.addStakesUser(address(msg.sender), offeringTokenAmount);
    }
  ...
  function getOfferingAmount(address _user) public view returns(uint256) {
    if (totalAmount > raisingAmount) {
      ...
      return totalAmount > 0 ? userInfo[_user].amount * offeringAmount / totalAmount : 0;
    }
  }
  ...
  function finalWithdraw(uint256 _ethAmount) public onlyOwner {
    ...
    require (totalAdminRetrieved + _ethAmount <= raisingAmount, "can't take out more than the target raise!");
    if(_ethAmount > 0) {
        totalAdminRetrieved+=_ethAmount;
