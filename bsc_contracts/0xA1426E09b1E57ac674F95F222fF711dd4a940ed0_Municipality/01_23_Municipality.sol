...
contract Municipality is OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    uint8 private constant PURCHASE_TYPE_REPAIR = 4;
    ...
    function purchaseParcels(uint256 _parcelsAmount, uint256 _referrerId) external onlySaleActive notZeroAddress {
        require(_parcelsAmount<=10, "Municipality: Parcels amount should be less or equal to 10");
        LastPurchaseData storage lastPurchase = lastPurchaseData[msg.sender];
        INetGymStreet(netGymStreetAddress).addGymMlm(msg.sender, _referrerId);
        (,uint256 priceForSinglePurchase, ) = _getPriceForPurchaseParcels(msg.sender, _parcelsAmount);
        _transferToContract(priceForSinglePurchase);
        userToPurchasedAmountMapping[msg.sender] += priceForSinglePurchase;
        _updateAdditionLevel(msg.sender);
        ...
        usersMintableNFTAmounts[msg.sender].parcels += _parcelsAmount;
        uint256[] memory nftIds = new uint256[](0);
        emit PurchaseMade(msg.sender, PURCHASE_TYPE_PARCELS, nftIds, priceForSinglePurchase);
    }
    ...
    function purchaseMiners(uint256 _minersCount, uint256 _referrerId) external onlySaleActive notZeroAddress {
        require(_minersCount<=40, "Municipality: Parcels amount should be less or equal to 40");
        INetGymStreet(netGymStreetAddress).addGymMlm(msg.sender, _referrerId);
        (,uint256 priceForSinglePurchase,) = _getPriceForPurchaseMiners(msg.sender, _minersCount);
        _transferToContract(priceForSinglePurchase);
        LastPurchaseData storage lastPurchase = lastPurchaseData[msg.sender];
        userToPurchasedAmountMapping[msg.sender] += priceForSinglePurchase;
        _updateAdditionLevel(msg.sender);
        lastPurchase.dollarValue += priceForSinglePurchase;
        _lastPurchaseDateUpdate(msg.sender);
        IAmountsDistributor(amountsDistributorAddress).distributeAmounts(priceForSinglePurchase, OPERATION_TYPE_MINT_MINERS, msg.sender);
        usersMintableNFTAmounts[msg.sender].miners += _minersCount;
        uint256[] memory nftIds = new uint256[](0);
        emit PurchaseMade(msg.sender, PURCHASE_TYPE_MINERS, nftIds, priceForSinglePurchase);
    }
    ...
    function purchaseBasicBundle(uint8 _bundleType, uint256 _referrerId) external onlySaleActive notZeroAddress
    {
        ...
        uint256[] memory nftIds = new uint256[](0);
        emit PurchaseMade(msg.sender, PURCHASE_TYPE_BUNDLE, nftIds, bundlePurchasePrice);
    ...
    function purchaseSuperBundle(uint8 _bundleType, uint256 _referrerId) external onlySaleActive notZeroAddress
    {
        ...
        usersMintableNFTAmounts[msg.sender].miners += bundle.minersAmount;
        uint256[] memory nftIds = new uint256[](0);
        emit PurchaseMade(msg.sender, PURCHASE_TYPE_SUPER_BUNDLE, nftIds, bundlePurchasePrice);
    ...
    function purchaseParcelsBundle(uint8 _bundleType, uint256 _referrerId) external onlySaleActive notZeroAddress
    {
        ...
        usersMintableNFTAmounts[msg.sender].parcels += bundle.parcelsAmount;
        uint256[] memory nftIds = new uint256[](0);
        emit PurchaseMade(msg.sender, PURCHASE_TYPE_PARCELS_BUNDLE, nftIds, bundlePurchasePrice);
        emit BundlePurchased(msg.sender, _bundleType);
    }
    ...
    function purchaseMinersBundle(uint8 _bundleType, uint256 _referrerId) external onlySaleActive notZeroAddress {
        ...
        lastPurchase.dollarValue += bundlePurchasePrice;
        _lastPurchaseDateUpdate(msg.sender);
        IAmountsDistributor(amountsDistributorAddress).distributeAmounts(bundlePurchasePrice, OPERATION_TYPE_MINT_MINERS, msg.sender);
        usersMintableNFTAmounts[msg.sender].miners += bundle.minersAmount;
        uint256[] memory nftIds = new uint256[](0);
        emit PurchaseMade(msg.sender, PURCHASE_TYPE_MINERS_BUNDLE, nftIds, bundlePurchasePrice);
        emit BundlePurchased(msg.sender, _bundleType);
    }
    ...
    function grantParcels(uint256 parcelsAmount, uint256 _referrerId, address _user) external onlyOwner {
        require(parcelsAmount < 25, "Municipality: The amount of miners should be less or equal to 25");
        if(_referrerId != 0) {
    ...
    function grantMiners(uint8 _minersAmount, uint256 _referrerId, address _user) external onlyOwner {
        require(_minersAmount <= 100, "Municipality: The amount of miners should be less than or equal to 100");
        if(_referrerId != 0) {
            INetGymStreet(netGymStreetAddress).addGymMlm(_user, _referrerId);
        }
    ...
    function grantParcelUpgrades(uint8 _upgradesAmount, uint256 _referrerId, address _user) external onlyOwner {
        if(_referrerId != 0) {
            INetGymStreet(netGymStreetAddress).addGymMlm(_user, _referrerId);
        }
    ...
    function repairAllMiners() external onlySaleActive notZeroAddress {
        ...
        uint256[] memory nftIds = new uint256[](0);
        emit PurchaseMade(msg.sender, PURCHASE_TYPE_REPAIR, nftIds, _amount);
    ...
    function upgradeStandardParcelsGroup(uint256[] memory _parcelIds) external onlySaleActive {
        ...
        uint256[] memory nftIds = new uint256[](0);
        emit PurchaseMade(msg.sender, PURCHASE_TYPE_UPGRADE_STANDARD_PARCELS_GROUP, nftIds, _totalUpgradePrice);
    }
    ...
    function _updateAdditionLevel(address _user) private {
        ...
        // if (termTimestamp + 2505600 > block.timestamp){ // 30 days
            if (userToPurchasedAmountMapping[_user] >= 11322 * 1e18) {
                _additionalLevel = 21;
            } else if (userToPurchasedAmountMapping[_user] >= 5687 * 1e18) {
                _additionalLevel = 14;
            } else if (userToPurchasedAmountMapping[_user] >= 2938 * 1e18) {
                _additionalLevel = 9;
            } else if (userToPurchasedAmountMapping[_user] >= 600 * 1e18) {
                _additionalLevel = 5;
            }  
