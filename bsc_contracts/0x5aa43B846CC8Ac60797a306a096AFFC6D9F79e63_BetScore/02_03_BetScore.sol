...
contract BetScore {
    ...
    function bet(uint16 team1Score, uint16 team2Score) payable public {
        if(!canParticipate) {
            revert("The game is closed");
        }
    ...
    function changeBet(uint16 team1Score, uint16 team2Score) public {
        if(!canParticipate) {
            revert("The game is closed");
        }
    ...
    function claim() public {
        if(!isAnswered) revert('The answer is not submitted');
        bool canClaim = false;
    ...
    function setAnswer(uint16 team1Score, uint16 team2Score) onlyOwner public {
        if(msg.sender != i_owner) {
            revert("Only the owner can set the answer");
        }
        isAnswered = true;
