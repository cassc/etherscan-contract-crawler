...
contract SpookyPig is Context, IBEP20, Ownable {
    ...
    string private _name = "Spooky Pig";
    ...
    uint256 public marketFeeOnBuying = 60; // 6% will be added to the market address
    ...
    uint256 public marketFeeOnSelling = 60; // 5% will be added to the market address
    uint256 public devFeeOnSelling = 30; // 2% will be added to the development address
    ...
    constructor(address payable _marketWallet, address payable _devWallet) {
        ...
        _isExcludedFromMaxHolding[dexPair] = true;
    ...
    function includeOrExcludeFromMaxHolding(address account, bool value)
        ...
        onlyOwner
    {
        _isExcludedFromMaxHolding[account] = value;
    }
    ...
    function _transfer(
        ...
        if(!_isExcludedFromMaxHolding[to]){
            require(balanceOf(to).add(amount) <= maxHoldLimit, "BEP20: max hold limit exceeds");
        }
    ...
    function setFeeCountersOnBuying(uint256 amount) private {
        liquidityFeeCounter += amount.mul(liquidityFeeOnBuying).div(percentDivider);
    ...
    function setFeeCountersOnSelling(uint256 amount) private {
        liquidityFeeCounter += amount.mul(liquidityFeeOnSelling).div(percentDivider);
