...
contract BulldogNFT is ERC721Enumerable, Ownable {
    using Strings for uint256;
    ...
    uint256 public totalDividend;
     // Lists
    uint256[55] public remainingIds;
    ...
    address public RewardToken = 0xFc93C1BDD261a377c850a13e797a296380B495b0;
    address public FeeReceiver = 0x0A80c47fe868B04176B9FBe5d0CA75aFfc41Ac07;
    ...
    function updateBaseImage(string memory _newBaseImage)
        public
        onlyOwner
    {
    ...
    function mint(uint256 _mintAmount) public payable {
        // require
        require(!paused, "BulldogNFT: minting has not started yet");
        require(_mintAmount > 0, "BulldogNFT: you have to mint at least one");
        require(
            _mintAmount <= maxMintAmount,
            "BulldogNFT: you can only mint 10 at a time"
        );
        require(
            lastSupply >= _mintAmount,
            "BulldogNFT: the collection is sold out"
        );
        if (msg.sender != owner()) {
            require(
                msg.value >= cost * _mintAmount,
                "BulldogNFT: total cost isn't match"
            );
        }
        // Minting
    ...
    function getAllNFTs() public view returns (BullStruct[] memory) {
        return minted;
    }
    ...
    function toImage(uint256 tokenId) internal view returns (string memory) {
        ...
        return bytes(currentBaseURI).length > 0
            ? string(abi.encodePacked(currentBaseURI,"BD", tokenId.toString(), baseImage))
            : "";
    }
    ...
    {
        // Checks
        require(_winner != address(0), "BulldogNFT: zero address");
        require(_amount > 0, "BulldogNFT: you have to mint at least one");
        require(lastSupply != 0, "BulldogNFT: the collection is sold out");
    ...
    function tokenURI(uint256 _id)
        ...
        returns (string memory)
    {
        require(_exists(_id), "BulldogNFT: URI query for nonexistent token");
