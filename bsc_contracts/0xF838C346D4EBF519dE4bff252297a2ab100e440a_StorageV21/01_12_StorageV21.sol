...
contract StorageV21 is Initializable, OwnableUpgradeable, PausableUpgradeable {
    ...
    mapping(uint256 => EarnBLID) public earnBLID;
    uint256 public countEarns;
    uint256 public countTokens;
    mapping(uint256 => address) public tokens;
    mapping(address => uint256) public tokenBalance;
    mapping(address => address) public oracles;
    mapping(address => bool) public tokensAdd;
    mapping(address => DepositStruct) public deposits;
    mapping(address => uint256) public tokenDeposited;
    mapping(address => int256) public tokenTime;
    uint256 public reserveBLID;
    ...
    function getDepositMappingElement(
        uint256 index,
        address account
    ) public view returns(uint256) {
        DepositStruct storage depositor = deposits[msg.sender];
        uint256 res;
        if (index == 0) {
            res = depositor.amount[account];
        } else if (index == 1) {
            res = (uint256)(depositor.tokenTime[account]);
        } else if (index == 2) {
            res = depositor.depositIterate[account];
        }
        return res;
    }

    function getEarnBLIDMappingElement(
        uint256 index,
        address token
    ) public view returns(uint256) {
        EarnBLID storage thisEarnBLID = earnBLID[index];
        uint256 res;
        res = thisEarnBLID.rates[token];
        return res;
    }    
}