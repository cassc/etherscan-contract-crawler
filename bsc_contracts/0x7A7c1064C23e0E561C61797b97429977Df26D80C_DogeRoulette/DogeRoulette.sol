...
/*  Website:  https://doge-roulette.com
	Telegram: https://t.me/dogeroulette
	Twitter:  https://twitter.com/DogeRouletteBSC
...
contract DividendPayingToken is ERC20, Ownable, DividendPayingTokenInterface, DividendPayingTokenOptionalInterface {
	...
	address private _BNBAddress = 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c;  // Testnet: 0xae13d989daC2f0dEbFf460aC112a837C89BAa7cd  Mainnet:  0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c
...
contract DogeRoulette is Ownable, ERC20 {
	IUniswapV2Router02 public uniswapV2Router;
	address public immutable uniswapV2Pair;

	string private constant _name = "DogeRoulette";
	string private constant _symbol = "DRLT";
	uint8 private constant _decimals = 18;

	DogeRouletteDividendTracker public dividendTracker;
	...
	uint256 public minimumTokensBeforeSwap = initialSupply * 5 / 10000;   //initialsuppy / 1000 = 0.1%
	uint256 public gasForProcessing = 300000;

	// Reflection cycle
	uint256 private _dividendTokenTimestamp;
	uint256 public dividendTokenCycleInHours = 4;
	...
	// Base taxes
	TaxPeriod private _base = TaxPeriod('base',0,0,3,3,2,2,5,5);
	...
	event Burned(address indexed to, uint amount0); 
	...
	constructor() ERC20(_name, _symbol) {
		//DOGE
		dividendTokens.push(address(0xbA2aE424d960c26247Dd6c32edC70B295c744C43)); 
		//MainNet  0xbA2aE424d960c26247Dd6c32edC70B295c744C43
		//TestNet  0x7ef95a0FEE0Dd31b22626fA2e10Ee6A223F8a684

		//Shib
		dividendTokens.push(address(0x2859e4544C4bB03966803b044A93563Bd2D0DD4D)); 
		//MainNet  0x2859e4544C4bB03966803b044A93563Bd2D0DD4D
		//TestNet  0x78867BbEeF44f2326bF8DDd1941a4439382EF2A7

		//BabyDoge
		dividendTokens.push(address(0xc748673057861a797275CD8A068AbB95A902e8de)); 
		//MainNet  0xc748673057861a797275CD8A068AbB95A902e8de
		//TestNet  0x8BaBbB98678facC7342735486C851ABD7A0d17Ca
		
		dividendToken = dividendTokens[dividendTokenIndex];

		dividendTracker = new DogeRouletteDividendTracker();
        ...
        _isExcludedFromMaxWalletLimit[address(deadWallet)] = true;
	...
	function updateDividendTracker(address newAddress) external onlyOwner {
		require(newAddress != address(dividendTracker), "DogeRoulette: The dividend tracker already has that address");
		DogeRouletteDividendTracker newDividendTracker = DogeRouletteDividendTracker(payable(newAddress));
		require(newDividendTracker.owner() == address(this), "DogeRoulette: The new dividend tracker must be owned by the DogeRoulette token contract");
	...
	function _setAutomatedMarketMakerPair(address pair, bool value) private {
		require(automatedMarketMakerPairs[pair] != value, "DogeRoulette: Automated market maker pair is already set to that value");
    ...
    function excludeFromFees(address account, bool excluded) external onlyOwner {
		require(_isExcludedFromFee[account] != excluded, "DogeRoulette: Account is already the value of 'excluded'");
	...
	function excludeFromMaxTransactionLimit(address account, bool excluded) external onlyOwner {
		require(_isExcludedFromMaxTransactionLimit[account] != excluded, "DogeRoulette: Account is already the value of 'excluded'");
		_isExcludedFromMaxTransactionLimit[account] = excluded;
		emit ExcludeFromMaxTransferChange(account, excluded);
	}
	function excludeFromMaxWalletLimit(address account, bool excluded) external onlyOwner {
		require(_isExcludedFromMaxWalletLimit[account] != excluded, "DogeRoulette: Account is already the value of 'excluded'");
	...
	function setWallets(address newLiquidityWallet, address newMarketingWallet) external onlyOwner {
		if(liquidityWallet != newLiquidityWallet) {
			require(newLiquidityWallet != address(0), "DogeRoulette: The liquidityWallet cannot be 0");
			emit WalletChange('liquidityWallet', newLiquidityWallet, liquidityWallet);
			liquidityWallet = newLiquidityWallet;
		}
		if(marketingWallet != newMarketingWallet) {
			require(newMarketingWallet != address(0), "DogeRoulette: The marketingWallet cannot be 0");
	...
	function setFeeOnSelectedWalletTransfers(address account, bool value) external onlyOwner {
		require(_feeOnSelectedWalletTransfers[account] != value, "DogeRoulette: The selected wallet is already set to the value ");
	...
	function setUniswapRouter(address newAddress) external onlyOwner {
		require(newAddress != address(uniswapV2Router), "DogeRoulette: The router already has that address");
	...
	function setGasForProcessing(uint256 newValue) external onlyOwner {
		require(newValue != gasForProcessing, "DogeRoulette: Cannot update gasForProcessing to same value");
		emit GasForProcessingChange(newValue, gasForProcessing);
		gasForProcessing = newValue;
	}
	function setMaxTransactionAmount(uint256 newValue) external onlyOwner {
		require(newValue != maxTxAmount, "DogeRoulette: Cannot update maxTxAmount to same value");
		emit MaxTransactionAmountChange(newValue, maxTxAmount);
		maxTxAmount = newValue;
	}
	function setMaxWalletAmount(uint256 newValue) external onlyOwner {
		require(newValue != maxWalletAmount, "DogeRoulette: Cannot update maxWalletAmount to same value");
		emit MaxWalletAmountChange(newValue, maxWalletAmount);
		maxWalletAmount = newValue;
	}
	function setMinimumTokensBeforeSwap(uint256 newValue) external onlyOwner {
		require(newValue != minimumTokensBeforeSwap, "DogeRoulette: Cannot update minimumTokensBeforeSwap to same value");
	...
	function addDividendToken(address newDividendToken) external onlyOwner {
		require(newDividendToken != address(0), "DogeRoulette: The dividend token cannot be 0");
		require(newDividendToken != address(this), "DogeRoulette: The dividend token cannot be the contract address");
		dividendTokens.push(newDividendToken);
	}
	function removeDividendToken(address removedDividendToken) external onlyOwner {
		require(dividendTokens.length > 1, "DogeRoulette: Must have at least 1x dividend token");
	...
	function claimBNBOverflow(uint256 amount) external onlyOwner {
		require(amount < address(this).balance, "DogeRoulette: Cannot send more than contract balance");
	...
	function _transfer(
		...
		if(!_isAllowedToTradeWhenDisabled[from] && !_isAllowedToTradeWhenDisabled[to]) {
			require(isTradingEnabled, "DogeRoulette: Trading is currently disabled.");
			require(!_isBlocked[to], "DogeRoulette: Account is blocked");
			require(!_isBlocked[from], "DogeRoulette: Account is blocked");
			if (!_isExcludedFromMaxTransactionLimit[to] && !_isExcludedFromMaxTransactionLimit[from]) {
				require(amount <= maxTxAmount, "DogeRoulette: Buy amount exceeds the maxTxBuyAmount.");
			}
			if (!_isExcludedFromMaxWalletLimit[to]) {
				require((balanceOf(to) + amount) <= maxWalletAmount, "DogeRoulette: Expected wallet amount exceeds the maxWalletAmount.");
			}
		}
...
contract DogeRouletteDividendTracker is DividendPayingToken {
	using SafeMath for uint256;
	using SafeMathInt for int256;
	...
	constructor() DividendPayingToken("DogeRoulette_Dividend_Tracker", "DogeRoulette_Dividend_Tracker") {
		claimWait = 3600;
	...
	function _transfer(address, address, uint256) internal pure override {
		require(false, "DogeRoulette_Dividend_Tracker: No transfers allowed");
	}
	function excludeFromDividends(address account) external onlyOwner {
	...
	function setTokenBalanceForDividends(uint256 newValue) external onlyOwner {
		require(minimumTokenBalanceForDividends != newValue, "DogeRoulette_Dividend_Tracker: minimumTokenBalanceForDividends already the value of 'newValue'.");
		minimumTokenBalanceForDividends = newValue;
	}
	function updateClaimWait(uint256 newClaimWait) external onlyOwner {
		require(newClaimWait >= 3600 && newClaimWait <= 86400, "DogeRoulette_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
		require(newClaimWait != claimWait, "DogeRoulette_Dividend_Tracker: Cannot update claimWait to same value");
