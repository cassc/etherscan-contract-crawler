...
Telegram: https://t.me/justfeetitBSC
...
contract FEET is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    FEETDividendTracker public dividendTracker;
    ...
    constructor() ERC20("Just Feet It", "FEET") {
        marketingWallet = payable(0xE69690c63Fe53964643467A87327B0f381BFC530);
        devWallet = payable(0x6dfC0A8B12CBF3fE5251C8c1F64315ACC1256cCD);
        ...
        dividendTracker = new FEETDividendTracker(
            payable(this),
            router,
...
contract FEETDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor(
        ...
        defaultToken = token;
        FEETContract = FEET(mainContract);
        minimumTokenBalanceForAutoDividends = 10000_000000000000000000; // 10K tokens
    ...
    function _transfer(
        ...
        require(false, "FEET_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "FEET_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(FEETContract);

            uint256 prevBalance = FEETContract.balanceOf(address(this));
            ...
            {
                uint256 received = FEETContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    FEETContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
