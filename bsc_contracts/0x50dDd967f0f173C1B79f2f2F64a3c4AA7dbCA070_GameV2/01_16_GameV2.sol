...
contract GameV2 is Ownable,Pausable,ReentrancyGuard{
    ...
    IGame immutable public game;//游戏地址
    ...
    constructor(IERC20 ft_,IERC20 ald_,IFTFarm ftFarm_,IERC20 usdt_,IUniswapV2Router02 uniswapV2Router_,IGame game_){
        ald = ald_;
        usdt = usdt_;
        ft = ft_;
        ftFarm = ftFarm_;
    ...
    function getSignupBurnAmount(address _token) external view returns(uint256){
        return _signupBurnAmounts[_token] + game.getSignupBurnAmount(_token);
    }
    ...
    function getFtBurnAmount()external view returns(uint256){
        return _ftBurnAmount + game.getFtBurnAmount();
    }
    ...
    function signup(uint256 _id) external whenNotPaused nonReentrant {
        ...
        require(isWhitelist(user),"Not whitelist");
        require(!user.isContract(),"Sender is contract");
    ...
    function _lottery(uint256 _phase,uint256 _amount) private {
        address winAddress = _signupAddresses[_phase][_random(QUOTA)];
    ...
    function _swapOrder(uint256[QUOTA-1] memory _result,uint256 _sIndex) private view returns(uint256[QUOTA-1] memory result){
        uint256 random = _random(_result.length);
