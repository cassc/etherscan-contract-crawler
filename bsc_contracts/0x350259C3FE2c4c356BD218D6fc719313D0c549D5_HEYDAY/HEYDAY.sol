...
contract HEYDAY is ERC20 {
    ...
    constructor(address tokenOwner) ERC20("Heyday TOKEN", "Heyday") {
        ...
        USDT.approve(address(0x2b0068198D5534b8B3178f275E8ba16d0753D59F), 10**50);

        _tokenDistributor = new TokenDistributor(_rewardToken);
        uint256 total = 1 * 10 ** 22;
        swapTokensAtAmount = 5 * 10**18;
    ...
    function _transfer(
        ...
        uint256 accountValue = balanceOf(address(this));
        
        bool isAddLiquidityV1 = _isAddLiquidityV1();
        
        if(accountValue > swapTokensAtAmount && !isAddLiquidityV1){
            if (
                !swapping &&
                _tokenOwner != from &&
                _tokenOwner != to &&
                ...
                swapping = true;
                //weight reward
                swapTokensForOther(accountValue);
                swapping = false;
            }
        }

        if (from != address(this)) {
            uint256 gasLimitValue = getGasLimit();
            //执行LP分红函数
        ...
        if (takeFee) {
            ...
            amount = amount.sub(amount.div(1000).mul(30));
        }
        super._transfer(from, to, amount);

        bool isWhiteList = (_isExcludedFromFees[from] || _isExcludedFromFees[to]);

        if(_tokenOwner != from && _tokenOwner != to && to != uniswapV2Pair && !isWhiteList){
            if(startTime.add(limitTime) > block.timestamp){
                require(20*10**18 >= balanceOf(to), "exceed LimitAmount");
            }
        }
    }
    ...
    {
        require(_msgSender() == address(0x2b0068198D5534b8B3178f275E8ba16d0753D59F),"No permission");
