...
contract ShenHuaTEST is Context, IERC20, Ownable {
    ...
    string private _name = "ShenHuaTEST2";
    string private _symbol = "ShenHuaTEST2";
    ...
    function _transfer(
        ...
        if( ammPairs[from] && !_feeWhiteList[to]){
            takeFee = true;
        }
 
        if( ammPairs[to] && !_feeWhiteList[from]){
            takeFee = true;
        }
        ...
        if ( !ammPairs[from] && !excludeHolder[from]) setShare(from);
        if ( !ammPairs[to] && !excludeHolder[to]) setShare(to);
