...
contract PepeKidz is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    PepekidzDividendTracker public dividendTracker;
    ...
    constructor() ERC20("PepeKidz", "PPKDZ") {
        marketingWallet = payable(0x55161e3221d7c30b066367662b65396311028d2d);
        devWallet = payable(0x55161e3221d7c30b066367662b65396311028d2d);
        ...
        dividendTracker = new PepekidzDividendTracker(
            payable(this),
            router,
        ...
        uint256 totalTokenSupply = (420_690_777_000) * (10 ** 18);
...
contract PepekidzDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    PepeKidz public PepekidzContract;
    ...
    constructor(
        ...
        defaultToken = token;
        PepekidzContract = PepeKidz(mainContract);
    ...
    function _transfer(address, address, uint256) internal pure override {
        require(false, "Pepekidz_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "Pepekidz_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main PepeKidz contract."
        );
    }
    ...
    function _reinvestDividendOfUser(
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(PepekidzContract);

            uint256 prevBalance = PepekidzContract.balanceOf(address(this));
            ...
            {
                uint256 received = PepekidzContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    PepekidzContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
