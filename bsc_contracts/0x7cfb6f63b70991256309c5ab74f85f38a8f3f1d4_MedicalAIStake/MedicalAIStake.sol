...
contract MedicalAIStake is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    event Deposit(address indexed user, uint256 amount,uint256 lockDuration);
    event Withdraw(address indexed user, uint256 amount);
    event EmergencyWithdraw(address indexed user, uint256 amount);

    constructor(
    ) {
        stakingToken = IBEP20(0x6A5eF8cCc77DcD5D14F8c6669221aC71b74398Aa);
    ...
    function deposit(uint256 _amount, uint256 _lockDuration) public nonReentrant {
        lockDuration = _lockDuration;

        if(holderUnlockTime[msg.sender] == 0){
            holderUnlockTime[msg.sender] = block.timestamp + _lockDuration;
        }
        PoolInfo storage pool = poolInfo[0];
        ...
        emit Deposit(msg.sender, _amount, _lockDuration);
    }
    ...
    function calculateNewRewards() public view returns (uint256) {
        ...
        return (((block.timestamp - pool.lastRewardTimestamp) * totalStaked) * apy / 100 / 31536000);
    }
