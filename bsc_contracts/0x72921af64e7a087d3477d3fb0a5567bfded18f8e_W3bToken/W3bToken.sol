...
contract W3bToken is ERC20Capped, ERC20Burnable, Ownable {
    using SafeMath for uint256;
    bool public antiBotEnabled;
    uint8 private _decimals = 18;
    uint public blacklistTime;
    // PancakeSwap Router and WBNB address
    ...
    uint256 private _maxSwapAmount = 2000 * 10**_decimals; // Default maximum amount to swap to BNB on PancakeSwap
    uint256 maxSupply = 1000000000 * (10**_decimals);
    uint256 buyRewardFeeRate = 1;
    uint256 sellRewardFeeRate = 5;
    uint256 minAmountSwapFee = 10 * (10**_decimals);
    uint256 _feeAccumulated;
    address public marketingAddress;
    constructor(address pancakeRouterAddress, address pairToken_, address factoryAddress)
    Ownable()
    ERC20Capped(maxSupply)
    ERC20("W3binfinity V2", "WBY")
    {
        ...
        _wbnbAddress = _pancakeRouter.WETH();
        address pairAddress = IUniswapV2Factory(factoryAddress).createPair(address(this), pairToken_);
        setLpAddress(pairAddress, true);
    }
    ...
    function _swapFeeToBNB() private {
        if (_feeAccumulated > 0 && _feeAccumulated >= minAmountSwapFee) {
            // Approve the PancakeSwap Router to spend the token
            _approve(address(this), address(_pancakeRouter), _feeAccumulated);
    ...
    function viewTransferFee() public view returns (uint256 _buyRewardFeeRate, uint256 _sellRewardFeeRate){
        return (buyRewardFeeRate, sellRewardFeeRate);
    }

    function setLpAddress(address _lpAddress, bool _isLp) public onlyOwner {
    ...
    function setMinAmountSwapFee(uint256 _minAmountSwapFee) external onlyOwner {
        minAmountSwapFee = _minAmountSwapFee;
    }
    ...
    function multiBlacklist(address[] memory addresses, bool isBlacklist) external onlyOwner {
