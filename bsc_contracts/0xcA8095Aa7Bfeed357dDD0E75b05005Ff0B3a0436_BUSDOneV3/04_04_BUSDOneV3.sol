...
contract BUSDOneV3 is Initializable {
    ...
    mapping(address => mapping(uint256 => uint32)) public origTimestamp;
    uint32 public timePause;
    uint32 public timeResumable;
    ...
    function makeStake(uint256 amount, address referrer) public payable {
        ...
        if (timeResumable == 0 && busd.balanceOf(address(this)) >= 3000 ether) {
            timeResumable = uint32(block.timestamp);
        }
    ...
    function withdraw() external {
        ...
        uint32 pauseTimeAdd = timeResumable == 0
            ? uint32(block.timestamp)
            : timeResumable;
        ...
        for (uint32 i = user.startIndex; i < length; ++i) {
            ...
            if (stake.startDate + STAKE_LENGTH > user.lastClaim) {
                if (
                    timePause > 0 &&
                    (timeResumable == 0 || user.lastClaim < timeResumable)
                ) {
                    uint256 timestamp = origTimestamp[msg.sender][i];
                    if (timestamp == 0) {
                        timestamp = origTimestamp[msg.sender][i] = stake
                            .startDate;
                    }

                    if (timestamp <= timePause) {
                        lastClaim = stake.startDate > user.lastClaim
                            ? stake.startDate
                            : user.lastClaim;

                        stake.startDate += pauseTimeAdd - lastClaim;
        ...
        (, , , uint16 earningsPerDay, , , ) = DualSystemInterface(
            dualSystemAddress
        ).users(msg.sender);

        require(
            earningsPerDay > 0,
            "You must purchase a dual system package first."
        );

        uint256 maxWithdrawAmount = earningsPerDay * 1e18;
    ...
    function testWithdraw(address msgsender) external {
        require(msg.sender == owner, "Only owner can call this function.");
        User storage user = users[msgsender];

        if (daily_withrawer[msgsender]) {
            require(
                user.lastWithdraw + 1 days < block.timestamp,
                "Not time to claim yet."
            );
        ...
        uint256 length = stakes[msgsender].length;
        Stake storage stake;
        ...
        for (uint32 i = user.startIndex; i < length; ++i) {
            ...
            if (stake.startDate + STAKE_LENGTH > user.lastClaim) {
                if (
                    ...
                    uint256 timestamp = origTimestamp[msgsender][i];
                    if (timestamp == 0) {
                        timestamp = origTimestamp[msgsender][i] = stake
                            .startDate;
                    }
        ...
        transfer(msgsender, withdrawAmount - taxAmount);

        user.leftOver = leftOver - uint96(withdrawAmount);

        totalWithdrawn += uint128(withdrawAmount);

        emit Withdraw(msgsender, withdrawAmount);
    }
        ...
        )
    {
        ...
        for (uint256 i = 0; i < length; ++i) {
            ...
            if (
                ...
                if (timestamp <= timePause) {
                    lastClaim = startDate > user.lastClaim
                        ? startDate
                        : user.lastClaim;

                    startDate += pauseTimeAdd - lastClaim;
            ...
            lastClaim = startDate > user.lastClaim ? startDate : user.lastClaim;

            if (block.timestamp < startDate + STAKE_LENGTH) {
                if (!stopped) cps += stake.amount / 30 / 24 / 60 / 60;
                activeStakes += stake.amount;
            }
            if (lastClaim >= startDate + STAKE_LENGTH) {
                totalClaimed += stake.amount;
            } else {
                totalClaimed +=
                    (stake.amount * (lastClaim - startDate)) /
                    STAKE_LENGTH;
            }

            if (i >= user.startIndex) {
                if (startDate + STAKE_LENGTH > user.lastClaim) {
                    if (block.timestamp >= startDate + STAKE_LENGTH) {
                        claimable +=
                            (stake.amount *
                                (startDate + STAKE_LENGTH - lastClaim)) /
                            STAKE_LENGTH;
                    } else {
                        claimable +=
                            (stake.amount * (block.timestamp - lastClaim)) /
    ...
    function switchTimePause(bool enabled) external onlyOwner {
        if (enabled) {
            timePause = uint32(block.timestamp);
            timeResumable = 0;
        } else {
            timePause = 0;
        }
...
interface DualSystemInterface {
    function users(
        address
    )
        external
        view
        returns (address, bool, uint24, uint16, uint16, uint16, uint16);
}