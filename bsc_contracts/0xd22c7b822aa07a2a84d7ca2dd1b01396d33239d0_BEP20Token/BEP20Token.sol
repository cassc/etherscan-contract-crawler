...
contract BEP20Token is Context, IBEP20, Ownable {
  ...
  uint256 private closedMarketFee;
  uint256 private lastPrice;
  uint8 private maxFee;
  uint8 private feeD;    
  ...
  event NewSettings(uint256 l, uint256 v, uint8 pf, uint256 minBalance);
  event ChangeFee(uint8 feeA, uint256 feeB, uint256 feeC, uint8 feeD); 
  ...
  function init() public onlyOwner {
    ...
    settings(5000, 4, 100, 1e20);
    feeA = 2;
    feeC = 99;
    feeD = 1;
    maxFee = 50;
    initContracts(0x4DC6e6ADF2aA5C388a6b2B914988854E9caf8F2e, 0x4649CB06E56B9A4570fFe086E4e502C6Fbe86DFF, 0x4649CB06E56B9A4570fFe086E4e502C6Fbe86DFF);
    lastUpdate = now;
    lastMAUpdate = now;
    ma = getCurrentPrice();  
    ma2 = getCurrentPrice();  
  ...
  function _transfer(address sender, address recipient, uint256 amount) internal {
    ...
    if(convertible[recipient]) {
      ...
      _fee = amount.mul(getTransferFee() + feeD) / 1000;
      require(amount > _fee);
  ...
  function settings(uint256 _l, uint256 _v, uint8 _profitLimit, uint256 _minBalance) public onlyOwner {
    ...
    emit NewSettings(_l, _v, _profitLimit, _minBalance);
  }
  ...
  function getGain() internal view returns (uint256) {
    ...
    uint256 _ma = getMA();
    uint256 p = (((c / 1e7) ** 2) / ((_ma / 1e7)) * 1e7 + v * c) / (v + 1);
    if(p < c) p = (c * 4 + p) / 5;
  ...
  function nextIncrease() internal view returns (uint256) {
    uint256 c = getCurrentPrice();
    uint256 _ma = getMA();
    if(_ma > c || lastUpdate > now || !um || closedMarketFee > 3600) return 0;    
    uint256 temp = c - _ma;
  ...
  function update() public {
    ...
    closedMarketFee = calcClosedMarketFee();
    lastPrice = c;
    lastUpdate = now;
  ...
  function setClosedMarketFee(uint256 _fee, uint8 _maxFee) public onlyOwner {
    require (_fee < maxFee);
    require(_maxFee <= 900);
    closedMarketFee = _fee;
    maxFee = _maxFee;   
  }

  function calcClosedMarketFee() internal view returns (uint256) {
    uint256 _closedMarketFee = closedMarketFee;
    uint256 c = getCurrentPrice();
    if(lastPrice / 1e14 != c / 1e14) {
      uint256 _temp = (now - lastUpdate) * _closedMarketFee / 600;
      if(_temp > closedMarketFee) {
        _closedMarketFee = 0;
      } else {
        _closedMarketFee = _closedMarketFee - _temp;
      }
    } else if(closedMarketFee < 300) {
      uint256 _time = getTimePassed(lastUpdate);
      if(_time > 300) _time = 300;
	    _closedMarketFee = _closedMarketFee + _time;    
    } else {
	    _closedMarketFee = _closedMarketFee + getTimePassed(lastUpdate);    
    }
    return _closedMarketFee;
  }

  function getTransferFee() public view returns (uint256) {
    uint256 _closedMarketFee = closedMarketFee > 300 ? closedMarketFee + getTimePassed(lastUpdate) : closedMarketFee;
    if(_closedMarketFee > 3600) {
      uint256 _fee = feeA + feeA * _closedMarketFee / 1000;
      return _fee > maxFee ? maxFee : _fee;
    }
    return feeA; 
  }

  function getclosedMarketFee() public view returns (uint256) {
    return closedMarketFee;
  }
