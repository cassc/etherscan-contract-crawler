...
Tg > https://t.me/DogeEmpire 

Twitter > https://twitter.com/DOGE__EMPIRE  

Website > https://www.dogeempire.org
...
contract DOGEEMPIRE is IERC20, Ownable {
    using SafeMath for uint256;
    string private constant _name = 'DOGE EMPIRE';
    ...
    address internal constant development_receiver = 0xAEe8db30914AC694C6cB65C2fA00dE360075eC72; 
    address public constant marketing_receiver = 0xAEe8db30914AC694C6cB65C2fA00dE360075eC72;
    ...
    function AutomaticTaxLoweringSystem() internal {
        if(launchTime.add(10 minutes) <= block.timestamp){totalFee = uint256(4000); sellFee =uint256(3000);}
        if(launchTime.add(30 minutes) <= block.timestamp){totalFee = uint256(3000); sellFee =uint256(4000);}
        if(launchTime.add(45 minutes) <= block.timestamp){totalFee = uint256(1000); sellFee =uint256(1600);}
        if(launchTime.add(60 minutes) <= block.timestamp){totalFee = uint256(800); sellFee =uint256(800);}
    ...
    function AutomaticMaxLiftingSystem() internal {
        if(launchTime.add(10 minutes) <= block.timestamp){
            _maxTxAmount = ( _totalSupply * 400 ) / 10000; _maxWalletToken = ( _totalSupply * 200 ) / 10000;}
