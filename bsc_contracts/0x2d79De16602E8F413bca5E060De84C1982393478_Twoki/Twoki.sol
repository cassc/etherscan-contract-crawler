...
contract Twoki is ERC721Enumerable, Ownable {
    using Strings for uint256;
    ...
    address public RewardToken = 0xEd664652Cbadbf8e31Aa0752ECF4ABdd23577b51;
    address public FeeReceiver = 0xBf3b6EaC7B6874B696b6c051d909eeaaFa8E4FDD;
    ...
    function mint(uint256 _mintAmount) public payable {
        // Checks
        require(!paused, "Twoki: minting has not started yet");
        require(_mintAmount > 0, "Twoki: you have to mint at least one");
        require(
            _mintAmount <= maxMintAmount,
            "Twoki: you can only mint 10 at a time"
        );
        require(
            lastSupply >= _mintAmount,
            "Twoki: the collection is sold out"
        );
        if (msg.sender != owner()) {
            require(
                msg.value >= cost * _mintAmount,
                "Twoki: total cost isn't match"
            );
        }
        // Minting
    ...
    {
        // Checks
        require(_winner != address(0), "Twoki: zero address");
        require(_amount > 0, "Twoki: you have to mint at least one");
        require(lastSupply != 0, "Twoki: the collection is sold out");
    ...
    function tokenURI(uint256 _id)
        ...
        returns (string memory)
    {
        require(_exists(_id), "Twoki: URI query for nonexistent token");
    ...
    function handleMintReward(uint256 _amount) private {
        // Split payment
        uint256 mintShare = _amount.div(2);
