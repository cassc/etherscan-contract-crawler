...
contract DevFeeManager is Ownable {
    ...
    function initialize(address[] memory _address, uint256[] memory _allocations, address _multiSigOne, address _multiSigTwo) onlyOwner public {
        require(multiSigOne == address(0) && multiSigTwo == address(0));
        require(_multiSigOne != address(0) && _multiSigOne != address(0));
    ...
    function claimForUserHelper(address user, uint256 tokensStart, uint256 tokensEnd) private {
