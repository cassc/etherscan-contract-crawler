...
contract SENSES is ERC20, Ownable {
    ...
    constructor() payable ERC20("SenseSeven", "SENSES") {
        ...
        totalBuyFee         = ecoSystemFeeonBuy + treasuryFeeOnBuy + rewardsFeeOnBuy;

        ecoSystemFeeonSell  = 1;
        treasuryFeeOnSell   = 2;
        rewardsFeeOnSell    = 1;

        totalSellFee        = ecoSystemFeeonSell + treasuryFeeOnSell + rewardsFeeOnSell;

        treasuryWallet = 0xcd89c59e835bAF5FAf306a0E5c99860f555763b6;
        ecosystemWallet = 0xebe3F931529F4726c4817c99D17170349346405B;
    ...
    function updateBuyFees(uint256 _ecoSystemFeeonBuy, uint256 _treasuryFeeOnBuy, uint256 _rewardsFeeOnBuy) external onlyOwner {
        ecoSystemFeeonBuy   = _ecoSystemFeeonBuy;
        treasuryFeeOnBuy    = _treasuryFeeOnBuy;
        rewardsFeeOnBuy     = _rewardsFeeOnBuy;

        totalBuyFee = _ecoSystemFeeonBuy + _treasuryFeeOnBuy + _rewardsFeeOnBuy;

        require(totalBuyFee <= 7, "Buy fee cannot be more than 7%");

        emit BuyFeesUpdated(totalBuyFee);
    }

    function updateSellFees(uint256 _ecoSystemFeeonSell, uint256 _treasuryFeeOnSell, uint256 _rewardsFeeOnSell) external onlyOwner {
        ecoSystemFeeonSell   = _ecoSystemFeeonSell;
        treasuryFeeOnSell    = _treasuryFeeOnSell;
        rewardsFeeOnSell     = _rewardsFeeOnSell;

        totalSellFee = _ecoSystemFeeonSell + _treasuryFeeOnSell + _rewardsFeeOnSell;

        require(totalSellFee <= 7, "Sell fee cannot be more than 7%");
    ...
    function changeEcoSystemWallet(address _ecosystemWallet) external onlyOwner {
        ...
        require(!isContract(_ecosystemWallet), "Eco wallet cannot be a contract");
        require(_ecosystemWallet != DEAD, "Eco wallet cannot be the zero address");
    ...
    function _transfer(
        ...
        if( canSwap &&
            ...
            uint256 bnbShare = (treasuryFeeOnBuy + treasuryFeeOnSell) + (rewardsFeeOnBuy + rewardsFeeOnSell) + (ecoSystemFeeonBuy + ecoSystemFeeonSell);
            ...
            if(contractTokenBalance > 0 && bnbShare > 0) {
                ...
                if((ecoSystemFeeonBuy + ecoSystemFeeonSell) > 0) {
                    uint256 ecosystemBNB = newBalance * (ecoSystemFeeonBuy + ecoSystemFeeonSell) / bnbShare;
