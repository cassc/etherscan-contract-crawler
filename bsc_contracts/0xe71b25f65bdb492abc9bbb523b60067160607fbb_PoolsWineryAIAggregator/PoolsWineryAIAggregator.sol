...
contract PoolsWineryAIAggregator is Ownable {
    ...
    function swapExactInput(
        ...
        if (_isFromETH) {
            ...
            IERC20(_steps[0].fromToken).transferFrom(msg.sender, address(this), _amountIn);
        }
    ...
    function _getAmountIns(SwapStep[] memory _steps, uint256 _amountOut) internal view returns (uint256 amountIn) {
        ...
        for (uint256 i = _steps.length; i > 0; ) {
            ...
            if (_step.protocol == BAKERY_SWAP || _step.protocol == UNI_SWAP) {
                ...
                (amountIn, ) = IPool(_step.pair).quoteAmountIn(_step.fromToken, _step.toToken, int256(amountIn));
            }
        }
    }
    ...
    function _swap(SwapStep memory _step, address _receiver) internal {
        ...
        if (_step.protocol == BAKERY_SWAP || _step.protocol == UNI_SWAP) {
            ...
            _step.protocol == CURVE || _step.protocol == CURVE_UNDERLYING || _step.protocol == PANCAKE_STABLE_SWAP
        ) {
            _swapCurve(_step);
        }
    }
    ...
    function checkCallBytes(
        bytes memory _data
    ) external pure returns (SwapStep[] memory, bytes memory, bytes4, uint256, uint256, uint256) {
        (
            SwapStep[] memory _steps,
            bytes memory _signature,
            bytes4 _selector,
