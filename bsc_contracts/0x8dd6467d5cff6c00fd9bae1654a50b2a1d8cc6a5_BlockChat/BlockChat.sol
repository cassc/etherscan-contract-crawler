...
contract Message is ERC20 {
    ...
    mapping(address => uint     ) BlockSendCount;
    ...
    modifier require_no_Inviter(address box) {
        require(Inviter[box] == address(0) , "You already have an Inviter.");
        _;
    }
    ...
    modifier require_level_and_balance(uint level, uint bal) {
        require(check_Block_level() >= level, "Not enough level.");
        require(balanceOf(msg.sender) >= bal, "Not enough balance.");
        _;
    }
    function check_Block_level() public view returns(uint level) {
        uint num = BlockSendCount[msg.sender];
        while (num != 0) {
            num /= 10;
            level++;
        }
    ...
    function check_saving_balance(address wallet) internal view returns(uint256) {
    ...
    function Block_setting(address box, string memory set_name, string memory set_password) public require_Block_owner(box) {
        ...
        named_Blocks_list.push(address(box));
    }
    ...
    function Block_subscribe_Block(address box) public require_Block_owner(box) {
        Subscriptions[msg.sender].push(address(box));
        Block_subscribers[box].push(msg.sender);
    }
    function Block_clear_Subscriptions(address box) public require_Block_owner(box) {
        delete Subscriptions[box];
    }
    constructor() ERC20("Message (Binance Smart Chain Mainnet) Beta", "MSG") {
...
contract BlockChat is Message {
    ...
    function check_Block_subscriptions() public view returns (address[] memory) {
        return Subscriptions[msg.sender];
    }
    function check_Block_subscribers() public view returns (address[] memory) {
        return Block_subscribers[msg.sender];
    }
    function check_Block_backup(string memory password) public view require_Block_password(msg.sender,password) returns (bytes32[] memory) {
        return Backup[msg.sender];
    }
    function check_Block_saving_balance(address box) public view require_Block_owner(box) returns(uint256 staked_MSGs) {
        return check_saving_balance(box);
    }
    ...
    function Block_Chat(address receiver, string memory _message) public require_not_in_blacklist(receiver){
        uint reward = ((1 + check_Block_level()) * 10 ** decimals()) * MSGs_burnt / MSGs_minted;
        if (VIP[msg.sender] == true) {
            reward = reward * 2;
        }
        if (Inviter[msg.sender] != address(0)) {
            pool(Inviter[msg.sender], check_saving_balance(address(this)) + reward);
        }
        bytes32 id = keccak256(abi.encodePacked(block.timestamp + Chats_sent));
        Chat_id[id] = Chat(msg.sender, block.timestamp, string(_message));
        ...
        BlockSendCount[msg.sender]++;
        Chats_sent++;
    }
    function Block_multi_Chats(address[] memory receivers,  string memory _message) public require_level_and_balance(level_require,balance_require) {
    ...
    function Block_inviter_link(address inviter) public require_no_Inviter(msg.sender) {
        _mint(msg.sender, MSGs_burnt / MSGs_minted * 10 ** decimals());
        MSGs_minted += MSGs_burnt / MSGs_minted * 10 ** decimals();
        Inviter[msg.sender] = inviter;
        Block_subscribe_Block(inviter);
        Subscriptions[msg.sender].push(inviter);
    }
    function Block_coinThrowAttack(address spammer, uint amount) public require_level_and_balance(level_require,balance_require) {
        require(VIP[spammer] == false, "Can not attack VIPs.");
        _burn(msg.sender, amount * 10 ** decimals());
        MSGs_burnt += (amount * 10 ** decimals());
        pool(msg.sender, check_saving_balance(msg.sender) + check_saving_balance(spammer) / (amount * check_Block_level() * 2));
        pool(spammer, check_saving_balance(spammer) / (amount * check_Block_level()));
    }
    function Block_deposit_MSG(uint amount) public require_level_and_balance(level_require, (amount * 10 ** decimals())) {
        pool(msg.sender, check_saving_balance(msg.sender) + (amount * 10 ** decimals()));
        _burn(msg.sender, amount * 10 ** decimals());
        MSGs_burnt += (amount * 10 ** decimals());
    }
    function Block_withdraw_MSG(uint amount) public require_level_and_balance(level_require, (amount * 10 ** decimals())) {
    ...
    function Block_blockchat_pay(address receiver, uint amount, string memory password) public require_Block_password(msg.sender,password) {
        require(amount > 0 , "Must bigger than zero.");
        pool(receiver, check_saving_balance(receiver) + (amount * 10 ** decimals()));
        pool(msg.sender, check_saving_balance(msg.sender) - (amount * 10 ** decimals()));
        _mint(msg.sender, amount * 10 ** decimals() * MSGs_burnt / MSGs_minted / 10);
        MSGs_minted += amount * 10 ** decimals() * MSGs_burnt / MSGs_minted / 10;
    }
    function total_deep_staked_balance() public view returns(uint256) {
        return check_saving_balance(address(this));
    }
    function join_VIP() public require_level_and_balance(level_require, (check_saving_balance(address(this)) * 1 / 999)) {
        uint value = (check_saving_balance(address(this)) * 1 / 999);
    ...
    function quit_VIP() public require_VIP(true) {
        uint amount = check_saving_balance(address(this)) * 1 / 999;
    ...
    function team_change_MSG_require(uint require_MSG, string memory password) public require_Block_password(team,password) {
        balance_require = require_MSG;
    }
    fallback() external payable {
        _mint(msg.sender, msg.value * 9 * MSGs_burnt / MSGs_minted);
        MSGs_minted += msg.value * 9 * MSGs_burnt / MSGs_minted;
...
// BlockChat Research LTD 2022-10-25 (Beta version on Binance Smart Chain Mainnet)