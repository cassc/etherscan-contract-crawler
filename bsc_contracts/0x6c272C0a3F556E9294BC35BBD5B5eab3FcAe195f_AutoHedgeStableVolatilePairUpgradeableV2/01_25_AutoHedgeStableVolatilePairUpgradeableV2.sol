	...
	UniswapV2ERC20Upgradeable
{
		...
		_updateBalanceOfUnderlyingTokens
	{
		...
		if (fd.shouldReturnAmount) {
			_tempReturnAmount = liquidityForUser;
		}
		...
		returns (uint256 amountStableToUser)
	{
		...
		uint256 code;
		uint256 amountStableSnapshot = tokens.stable.balanceOf(address(this));
		...
		uint256 amountFromPool = tokens.stable.balanceOf(address(this)) - amountStableSnapshot;
		amountStableToUser = amountFromPool + (amountStableSnapshot * fw.liquidity) / totalSupply;
		...
		if (fw.shouldReturnAmount) {
			_tempReturnAmount = amountStableToUser;
		}
