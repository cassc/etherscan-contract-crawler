...
contract LiquidCryptoBridge_v2 is ERC20, Ownable {
  ...
  function claimReward(address _account, uint256 _amount) public {
    ...
    require(userReward >= _amount, "LC Bridge v2: Exceed reward amount");
    require(_amount <= address(this).balance, "LC Bridge v2: Not enough liquidity");

    uint256 _now = block.timestamp;
    StakeInfo storage tmpInfo = userInfo[_account];
    tmpInfo.debtReward = tmpInfo.debtReward + tmpInfo.amount * (_now - tmpInfo.startedAt);
  ...
  function deposit() public payable onlyManager {
    if (totalSupply() > address(this).balance) {
      ...
      if (msg.value > needAmount) {
        ...
        require(success1, "LC Bridge v2: Failed refund overflow");
      }
    }
  }
