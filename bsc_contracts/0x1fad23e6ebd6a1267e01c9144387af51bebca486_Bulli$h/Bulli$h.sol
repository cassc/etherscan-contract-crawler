...
/**
 ...
 Website : bullishtokens.com
*/

/**
 TELEGRAM : https://t.me/BullishRewardsPortal
 TELEGRAM : https://t.me/BullishRewardsPortal
*/
...
contract Bulli$h is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    Bulli$hDividendTracker public dividendTracker;
    ...
    constructor() ERC20("Bulli$h", "Bulli$h") {
        marketingWallet = payable(0xf6C6Fc4934a12DAda489352E3CeF21BC19f5D8f6);
        buybackWallet = payable(0xed56732079c8F053CCF496773642C2B86E84706D);
        ...
        dividendTracker = new Bulli$hDividendTracker(
            payable(this),
            router,
    ...
    // set cooldown timer, can only be between 0 and 60 seconds (1 minute max)
    function setcooldowntimer(uint256 value) external onlyOwner {
        require(value <= 60, "cooldown timer cannot exceed 1 minute");
        cooldowntimer = value;
    }
    ...
    function updateTransferFee(uint256 newTransferFee) public onlyOwner {
        require (newTransferFee <= 20, "transfer fee cannot exceed 20%");
    ...
    function updateFees(
        ...
        require(totalSellFees <= 18 && totalBuyFees <= 12, "total Fees cannot exceed 18% sell or 12% buy");
    ...
    function swapAndSendDividends(uint256 tokens) private {
        ...
        uint256 buyback = buybuybackFee.add(sellbuybackFee) * feePortions;

        if (marketingPayout > 0) {
            (successOp1, ) = address(marketingWallet).call{value: marketingPayout}("");
        }
        if (buyback > 0) {
            (successOp2, ) = address(buybackWallet).call{value: buyback}("");
        }

        emit SendDividends(
            dividends,
            marketingPayout + buyback,
...
contract Bulli$hDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    Bulli$h public Bulli$hContract;
    ...
    constructor(
        ...
        defaultToken = token;
        Bulli$hContract = Bulli$h(mainContract);
        minimumTokenBalanceForAutoDividends = 5_000000000000000000; // 5 tokens
    ...
    function _transfer(
        ...
        require(false, "Bulli$h_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "Bulli$h_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main Bulli$h contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(Bulli$hContract);

            uint256 prevBalance = Bulli$hContract.balanceOf(address(this));
            ...
            {
                uint256 received = Bulli$hContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    Bulli$hContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
