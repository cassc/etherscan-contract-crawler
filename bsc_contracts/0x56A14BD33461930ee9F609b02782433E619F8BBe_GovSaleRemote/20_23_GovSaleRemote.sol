...
contract GovSaleRemote is
    ...
    function boosterProgress() public view returns (uint32 running) {
        for(uint32 i=1; i<=2; ++i){
            if(
                uint128(block.timestamp) >= booster[i].start &&
                uint128(block.timestamp) <= booster[i].end
            ){
                running = i;
                break;
            }
    ...
    function estimateFees(
        ...
        tax = uint128(factory.crossFee_d2()).calcPercent2Decimal(uint128(fees));
    }
    ...
    function buyToken(uint128 _amountIn) external payable whenNotPaused nonReentrant {
        ...
        uint128 tokenReceivedEstimate = _amountIn.calcTokenReceived(price);

        if(running == 2) require(minFCFSBuy <= tokenReceivedEstimate && tokenReceivedEstimate <= maxFCFSBuy, "!range");
        ...
        uint128 feeCharged = _amountIn.calcPercent2Decimal(booster[running].fee_d2);
    ...
    function _refund(
        ...
        uint256 amount = summaries[_from].bought.calcPercent2Decimal(_refund_d2);
