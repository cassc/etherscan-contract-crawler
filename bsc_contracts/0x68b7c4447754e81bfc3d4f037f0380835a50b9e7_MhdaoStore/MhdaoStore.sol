...
// File contracts/features/MhdaoStore.sol
 ...
 * @dev Sells items
 */
contract MhdaoStore is Initializable, AccessControlUpgradeable, PausableUpgradeable {
  ...
  function getExtraSquadPrices(uint256 currentSquads) public view returns (uint256 bct, uint256 resourceCents) {
    ...
    uint256 priceInCheese = currentSquads <= 5 ? 0 : (currentSquads - 5) * 12;
  ...
  function getMentorLevelPrices(uint256 currentLevel) public pure returns (uint256 bct, uint256 resourceCents) {
    return (currentLevel * 550 * 1E18, 3 * 100);
  }
  ...
  function buyMentorLevel(uint256 levels, bool withBusd) external whenNotPaused {
    ...
    (uint256 priceInBct, uint256 priceInCheese) = getMentorLevelPrices(player.mentorLevel);
