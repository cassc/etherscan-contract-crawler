...
contract ZOMBIEFLOKI is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    ZOKIDividendTracker public dividendTracker;
    ...
    constructor() ERC20("ZOMBIE FLOKI", "ZOKI") {
        marketingWallet = payable(0x382886661090652711501Dfc932Be0D73Bee6E62);
        devWallet = payable(0xf74caf18eC589ce4473a48F9E6C9355Ce8B10525);
        ...
        dividendTracker = new ZOKIDividendTracker(
            payable(this),
            router,
...
contract ZOKIDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    ZOMBIEFLOKI public ZOKIContract;
    ...
    constructor(
        ...
        defaultToken = token;
        ZOKIContract = ZOMBIEFLOKI(mainContract);
    ...
    function _transfer(
        ...
        require(false, "ZOKI_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "ZOKI_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main B contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(ZOKIContract);

            uint256 prevBalance = ZOKIContract.balanceOf(address(this));
            ...
            {
                uint256 received = ZOKIContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    ZOKIContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
