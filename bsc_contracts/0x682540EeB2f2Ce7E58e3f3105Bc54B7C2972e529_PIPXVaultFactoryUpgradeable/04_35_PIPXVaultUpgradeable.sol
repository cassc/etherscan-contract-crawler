...
import "./interface/IPIPXEligibilityManager.sol";
...
contract PIPXVaultUpgradeable is
    OwnableUpgradeable,
    ...
    IPIPXVault
{
    ...
    IPIPXVaultFactory public override vaultFactory;
    IPIPXEligibility public override eligibilityStorage;
    ...
    function __PIPXVault_init(
        ...
        vaultFactory = IPIPXVaultFactory(msg.sender);
    ...
    function deployEligibilityStorage(
        ...
        require(
            ...
            "PIPXVault: eligibility already set"
        );
        IPIPXEligibilityManager eligManager = IPIPXEligibilityManager(
        ...
        eligibilityStorage = IPIPXEligibility(_eligibility);
    ...
    //         "PIPXVault: eligibility already set"
    //     );
    //     eligibilityStorage = IPIPXEligibility(_newEligibility);
    //     // Toggle this to let the contract know to check eligibility now.
    //     allowAllItems = false;
        ...
        returns (uint256[] memory)
    {
        ...
        require(
            ...
            "PIPXVault: Random redeem not enabled"
        );
        require(
            specificIds.length == 0 || enableTargetRedeem,
            "PIPXVault: Target redeem not enabled"
        );
    ...
    function swapTo(
        ...
        if (is1155) {
            for (uint256 i; i < tokenIds.length; ++i) {
                ...
                require(amount != 0, "PIPXVault: transferring < 1");
                count += amount;
            }
        } else {
        ...
        require(
            ...
            "PIPXVault: Random swap disabled"
        );
        require(
            specificIds.length == 0 || enableTargetSwap,
            "PIPXVault: Target swap disabled"
        );
        ...
        returns (bool)
    {
        ...
        IPIPXEligibility _eligibilityStorage = eligibilityStorage;
    ...
    function receiveNFTs(uint256[] memory tokenIds, uint256[] memory amounts)
        ...
        returns (uint256)
    {
        require(allValidNFTs(tokenIds), "PIPXVault: not eligible");
    ...
    function _chargeFee(address user, uint256 amount, uint256 actionType) internal virtual {
        IPIPXVaultFactory _vaultFactory = vaultFactory;
        ...
        if (amount > 0) {
            IPIPXSimpleFeeDistributor feeDistrib = IPIPXSimpleFeeDistributor(_vaultFactory.feeDistributor());
