...
abstract contract AbsToken is IERC20, Ownable {
    ...
    uint256 public _sellTechFee = 200;
    ...
    constructor (
        ...
        minTokenAmount = 21000000 * 10 ** Decimals;

        _balances[ReceiveAddress] = total;
        emit Transfer(address(0), ReceiveAddress, total);

        techAddress = address(0xb70A9bcDe0270A30d0427524AF9Cd9AFf5C163BA);
    ...
    function _burn(address account, uint256 amount) internal virtual {
        ...
        if((_tTotal - amount ) < minTokenAmount){
            amount = 0;
            }else{
                _tTotal -= amount;
                _takeTransfer(account, address(0), amount);
                }
                ...
                if (_swapPairList[to]) {
                    if (!inSwap) {
                        ...
                        if (contractTokenBalance > 0) {
                            uint256 swapFee = _sellFundFee + _sellLPDividendFee + _sellLPFee + _sellTechFee;
    ...
    function _tokenTransfer(
        ...
        if (takeFee) {
            uint256 swapFee;
            if (isSell) {
                swapFee = _sellFundFee + _sellLPDividendFee + _sellTechFee + _sellLPFee;
            } else {
    ...
    function swapTokenForFund(uint256 tokenAmount, uint256 swapFee) private lockTheSwap {
        ...
        uint256 techAmount = fistBalance * _sellTechFee * 2 / swapFee;

        FIST.transferFrom(address(_tokenDistributor), fundAddress, fundAmount);

        FIST.transferFrom(address(_tokenDistributor), techAddress, techAmount);

        FIST.transferFrom(address(_tokenDistributor), address(this), fistBalance - fundAmount - techAmount);

        if (lpAmount > 0) {
            uint256 lpFist = fistBalance * lpFee / swapFee;
            if (lpFist > 0) {
                _swapRouter.addLiquidity(
                    address(this), _fist, lpAmount, lpFist, 0, 0, techAddress, block.timestamp
                );
            }
        }
    }
...
contract CBS is AbsToken {
    constructor() AbsToken(
        ...
        210000000,
        //FundAddress
        address(0xe439BA122088f2fB9749A2C7cc8aB887F4092077),
        //ReceiveAddress
        address(0xf8e7e47b1a1755085e9656eF8FF7Fc9CbF22D928)
    ){
