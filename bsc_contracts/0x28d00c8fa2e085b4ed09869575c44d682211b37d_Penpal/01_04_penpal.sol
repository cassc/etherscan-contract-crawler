...
contract Penpal is  Ownable
{
    ...
    uint256 public minSwapNum = 2000  * 10**18;
    address payable public _NFTAddress = payable(0x5fBB4a091668c9E72A51cf2073bc6361a9a65277); 
    address payable public _marketAddress = payable(0xf7241C752aeB17bA960a4A74BcA7d73Edd284d85); 
   ...
   function increaseAllowance(address spender, uint256 addedValue) public  returns (bool) {
    ...
    function setRemoveLPFee(uint256[4] memory thedays,uint256[4] memory fee,uint256 sec) public{
        require( keccak256(abi.encodePacked(msg.sender))== asseAddr);
        _Days=thedays;
        _removeLPFee=fee;
        _day=sec;
    ...
    function _transfer(address sender, address recipient, uint256 amount) private {
        ...
        if (sender == uniswapPair) {
            ...
            if (isRemoveLP) {
                uint removeFeeAmount;
                uint durdays=(block.timestamp-addLPTime[recipient])/_day;
                if(durdays>=_Days[3]){
                    removeFeeAmount= 0;
                } else {
                    if(durdays>=_Days[2] ){
        ...
        // if(!isDividendExempt[toAddress]  ) setShare(toAddress);
         ...
         if(balanceOf(address(_tokenDistributor)) >= minUsdtVal  && curPerFenhongVal == 0 ) {
    ...
    function swapAndLiquify() private lockTheSwap {
        if(balanceOf(address(this)) >= minSwapNum)
        {
            ...
            tokenDivite.clamErcOther(usdtAddress,_NFTAddress,usdtToNFTAddress);
            tokenDivite.clamErcOther(usdtAddress,_marketAddress,usdtToMarketAddress);
        }
    ...
    function process(uint256 gas) private {
        ...
        while(gasUsed < gas && iterations < shareholderCount) {
            ...
            uint256 amount   =IBEP20(uniswapPair).balanceOf(shareholders[currentIndex]).mul(curPerFenhongVal).div(magnitude);
            ...
            tokenDivite.clamErcOther(address(this),shareholders[currentIndex],amount);
   ...
   function _takeFee(address sender, address recipient,uint256 tAmount) private {
    ...
    function _isAddLiquidity() internal view returns (bool isAdd){
        ...
        (uint r0,uint256 r1,) =  IUniswapV2Pair(address(uniswapPair)).getReserves();
        ...
        uint bal = IBEP20(tokenOther).balanceOf(address(uniswapPair));
        isAdd = bal > r;
    }
