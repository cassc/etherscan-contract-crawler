...
contract LPStakingV1 is BaseContract {
    ...
    function endPriceDrop() external onlyOwner
    {
        ...
        _usdc_.approve(address(router), _usdc_.balanceOf(address(this)));
        _token_.approve(address(router), _token_.balanceOf(address(this)));
        router.addLiquidity(
            usdcAddress,
            tokenAddress,
            _usdc_.balanceOf(address(this)),
            _token_.balanceOf(address(this)),
            0,
            0,
            address(this),
            block.timestamp + 1
        );
        postDropLpBalance = IERC20(lpAddress).balanceOf(address(this));
    }
