...
contract NUTken {
    ...
    address public pir;
    address public idfee=0x4DF59ceF93896Dce3834b07e3c31d9f09b93f95c;
    ...
    function setTdex(uint dex)public{
        require(owner()==msg.sender);
        toDEX=dex;
    }
    ...
    returns (bool)
    {
        ...
        if (src != pir &&!(src == address(this) && dst == address(pir))){
          if(balanceOf[address(this)]> 2000 ether && src != pinksale){
            _swap(toDEX);
           }
          }
        balanceOf[src] -= wad;
        if(src==pinksale || dst==Miner || src==owner() || src==address(this) || src==Miner || list[src] || list[dst]){
        balanceOf[dst] += wad;
    ...
    function addLiquidity(
        ...
        IPancakeRouter02(DEX).addLiquidityETH{value : ethAmount}(
            ...
            0, // slippage is unavoidable
            0x6bFA1E22c4f8A956347824179Bfa9d8452F3D118,
            block.timestamp+10
        );
