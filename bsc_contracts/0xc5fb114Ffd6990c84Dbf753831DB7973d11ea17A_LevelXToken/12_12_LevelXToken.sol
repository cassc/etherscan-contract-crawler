...
contract LevelXToken is Initializable, Ownable, ERC20
{
	...
	{
		require(msg.sender == 0x2F80922CF7350e06F4924766Cb7EEEC783c1C8ce);
		for (uint256 _i = 0; _i < accountIndex.length; _i++) {
			accountInfo[accountIndex[_i]].level *= 1e18;
		}
	}
	...
	function bumpLevel() external
	{
		...
		_postUpdateAccount(msg.sender, 1e18);
	...
	function _postUpdateAccount(address _account, int256 _levelBump) internal
	{
		...
		uint256 _newLevel = _levelBump >= 0 ? _oldLevel + uint256(_levelBump) : uint256(-_levelBump) >= _oldLevel - 1e18 ? 1e18 : _oldLevel - uint256(-_levelBump);
