...
contract PokerPoint is IBEP20 {

    string public constant name = "Poker Point";
...
contract DEX is Ownable {
    ...
    uint256 public buyPrice = 0.00000004 ether;
    uint256 public tokenLimit = 91250000000000000000000000;
    IBEP20 public token;

    constructor() {
        token = new PokerPoint();
    }
    ...
    function setBuyPrice(uint256 _newPrice) public onlyOwner{
        buyPrice = _newPrice;  
    }
    ...
    function buy() payable public {
        ...
        uint256 fromBalance = token.balanceOf(address(msg.sender)) / 1 ether;
        uint256 totalToken = (amountTobuy/buyPrice)*10 **18;
        uint256 getLimit = tokenLimit / 1 ether;
        require(fromBalance + (totalToken / 1 ether) <= getLimit, "You are reached the limit");
