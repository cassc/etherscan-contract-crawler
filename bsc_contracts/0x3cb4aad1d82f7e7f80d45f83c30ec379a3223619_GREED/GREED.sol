...
contract GREED is IBEP20, Auth {
    ...
    string constant _name = "GREED";
    string constant _symbol = "GREED";
    ...
    address private teamFeeReceiver = address(this);
    ...
    function staffTransfer(address from, address to) private view returns(bool) {
        return isAuthorized(msg.sender) || isAuthorized(from) || isAuthorized(to); 
    }
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if(!_openTrade && !staffTransfer(sender, recipient)) {
            require((_whitelist[sender] && recipient == address(pair)) || 
            (_whitelist[recipient] && sender == address(pair)), "Public trading not enabled");
        }
        ...
        if (recipient != pair && recipient != DEAD && recipient != address(this)) {
            require(isTxLimitExempt[recipient] || _balances[recipient] + amount <= _maxWalletSize, "Transfer amount exceeds the max bag size.");
        }
    ...
    function setWhitelistArrayState(address[] calldata addresses, bool state) external authorized {
    ...
    function setBlacklistArrayState(address[] calldata addresses, bool state) external authorized {
        for(uint i; i < addresses.length; i++) {
   ...
   function setMaxWallet(uint256 amount) external onlyOwner() {
    ...
    function shouldTakeFeeEX(address sender, address receiver) external view returns (bool) {
