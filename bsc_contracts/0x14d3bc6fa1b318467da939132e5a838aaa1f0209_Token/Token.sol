...
abstract contract ERC20 is IERC20 {
    ...
    uint256 private _interestTotalSupply;
    ...
    function totalSupply() public view virtual override returns (uint256) {
        return _totalSupply.add(_interestTotalSupply);
    }
    ...
    function _interestTransfer(address account, uint256 amount) internal {
        ...
        _interestTotalSupply = _interestTotalSupply.add(amount);
...
contract Token is ERC20, Owner {
    ...
    uint256 public swapAmountLimit = 100;
    ...
    function setConfig(address contractAddress_)
        external
        onlyOwner
        returns (bool)
    {
    ...
    function setSwapAmountLimit(uint256 value) public onlyOwner returns (bool) {
        swapAmountLimit = value;
        return true;
    }
    ...
    function _transfer(
        ...
        if (
            !_swapping &&
            balanceOf(address(this)) > swapAmountLimit * (10**decimals()) &&
            !isContract(sender) &&
            swapSwitch
        ) {
            _swapAndLiquify();
        ...
        if (
            _swapping ||
            _Config.getWhiteList(sender) ||
            _Config.getWhiteList(recipient)
        ) {
            interestTransfer(sender, recipient, amount);
        } else if (sender == uniswapV2Pair) {
            interestTransfer(sender, recipient, amount);

            _bonus(recipient, amount);

            uint256 backflowAmount = amount.mul(_backflowFeeBuy).div(10000);
            if (backflowAmount > 0) {
            ...
            (
                ...
                uint256 fundAmount,
                uint256 nftAmount,
                uint256 backflowAmount
            ) = (
                    amount.mul(_burnFee).div(10000),
                    amount.mul(_fundFee).div(10000),
                    amount.mul(_nftFee).div(10000),
                    amount.mul(_backflowFeeSell).div(10000)
                );
            if (brunAmount > 0) {
                _burn(sender, brunAmount);
            }
            if (fundAmount > 0) {
                interestTransfer(sender, _fundAddress, fundAmount);
            }
            if (nftAmount > 0) {
                interestTransfer(sender, _nftAddress, nftAmount);
            }
            if (backflowAmount > 0) {
            ...
            interestTransfer(
                sender,
                recipient,
                amount - (brunAmount + fundAmount + nftAmount + backflowAmount)
            );
        } else {
            interestTransfer(sender, recipient, amount);

            uint256 deflationAmount = amount.mul(_Config.getDeflationFee()).div(
                10000
            );
            if (deflationAmount > 0) {
                interestTransfer(recipient, _deflationAddress, deflationAmount);
            }
        }
    }

    function _bonus(address account, uint256 value) internal {
        address user = account;
        address leader;
        uint256 bonus;
        for (uint256 i = 0; i < _bonusFee.length; i++) {
            leader = _Config.getTeamList(user);
            if (leader != address(0)) {
                interestTransfer(
                    account,
                    leader,
                    value.mul(_bonusFee[i]).div(10000)
                );
                user = leader;
            } else {
        ...
        if (bonus > 0) {
            interestTransfer(account, _bonusAddress, bonus);
        }
    }
    ...
    function batchTransfer(uint256 amount, address[] memory accounts)
        public
        returns (bool)
    {
        require(_Config.getWhiteList(msg.sender), "account not in while list");
        for (uint256 i = 0; i < accounts.length; i++) {
            interestTransfer(msg.sender, accounts[i], amount);
        }
        return true;
    }
}