...
contract NFTStaking is IERC721Receiver, Ownable{
    ...
    function decreaseLiquidity(uint _tokenId) external {
        uint128 tax = getLiquidity(_tokenId) - (getLiquidity(_tokenId) * 10 / 100);
    ...
    function Claim() public updateReward(msg.sender) checkStart{
        ...
        if (reward > 0) {
            ...
            emit TressuryPaid(msg.sender, tressuryamount);
            emit VaultFarmHarvestFee(msg.sender, taxamount);
        }
    }
    ...
    function emergencyWithdrawal(uint _tokenId) external onlyOwner(){
        nftToken.safeTransferFrom(address(this), msg.sender, _tokenId);
    }
}