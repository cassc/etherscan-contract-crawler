    ...
    NativePoolStorage
{
    ...
    function addSigner(address _signer) external override onlyOwner whenNotPaused onlyNotPmm {
        require(!isSigner[_signer], "Signer is already added");
        isSigner[_signer] = true;
        emit AddSigner(_signer);
    }

    function removeSigner(address _signer) external override onlyOwner whenNotPaused onlyNotPmm {
    ...
    function executeSwapToTreasury(
        ...
        require(
            buyerTokenAmount <= type(uint256).max / 2,
