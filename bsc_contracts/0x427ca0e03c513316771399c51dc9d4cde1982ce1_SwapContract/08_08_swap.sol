...
contract SwapContract is ERC20Upgradeable, OwnableUpgradeable {
    ...
    function _safeTransfer(address token, address to, uint value) private {
        ...
        require(success && (data.length == 0 || abi.decode(data, (bool))), 'BabelSwap: TRANSFER_FAILED');
    } 
    ...
    function _update(uint balance0, uint balance1, uint112 _reserve0, uint112 _reserve1) private {
        // if (balance0 > uint112(-1) || balance1 > uint112(-1)) revert('OVERFLOW');
        // require(balance0 <= uint112(-1) && balance1 <= uint112(-1), 'BabelSwap: OVERFLOW');
    ...
    function lockLiquidity(uint amount, uint lockedPeriod) public {
    ...
    function unLockLiquidity(uint index) public {
    ...
    function withdrawTokens(address _token, address _to, uint _amount) external onlyOwner {
        if(_token == token1) revert("Invalid token");
    ...
    function swap(uint amount0Out, uint amount1Out, uint256 amount0In, uint256 amount1In, address to) public lock {
        ...
        if(amount0Out > maxToken0 || amount1Out > maxToken1) revert("Too much amount amount");
