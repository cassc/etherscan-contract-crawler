...
contract honeyCheckerV5 {
    ...
    uint256 approveMaxSum = 115792089237316195423570985008687907853269984665640564039457584007913129639935;
    // 如果没授权先授权
    ...
    constructor() public {}
    function approveTokentoRouter (address _token, address _router) public{
        if (!tokenApproved[_token][_router]) {
            IERC20(_token).approve(_router, approveMaxSum);
            tokenApproved[_token][_router] = true;
        }
    }
    function testSellToken(
        address[] calldata path,
        address to)external payable
    {
        ...
        address[] memory tempPathReverse = new address[](2);
        if(pathlength>2)
        {
            address[] memory tempPathReverse = new address[](3);
            tempPathReverse[0] = path[2];
            tempPathReverse[1] = path[1];
            tempPathReverse[2] = path[0];
        }
        else
        {
            address[] memory tempPathReverse = new address[](2);
            tempPathReverse[0] = path[1];
            tempPathReverse[1] = path[0];
        }

        
        // 买之前目标代币的数量
        uint8 useTokendecimals = IERC20(path[0]).decimals();
        uint256 _SellTokenAmount = IERC20(path[path.length - 1]).balanceOf(address(this));
        IWETH(router.WETH()).deposit{value: 10 ** (useTokendecimals - 3)}();

        router.swapExactTokensForTokensSupportingFeeOnTransferTokens(
            10 ** (useTokendecimals - 3),
            0,
            path,
            to,
            (block.timestamp + 100)
        ...
        // 买之后目标代币的数量
        uint256 SellTokenAmount = IERC20(path[path.length - 1]).balanceOf(to);
        // 目标代币的增加数量
        SellTokenAmount = SellTokenAmount-_SellTokenAmount;
        
        approveTokentoRouter(path[path.length - 1], address(0x10ED43C718714eb63d5aA57B78B54704E256024E));
        router.swapExactTokensForTokensSupportingFeeOnTransferTokens(
            SellTokenAmount.mul(50).div(100),
