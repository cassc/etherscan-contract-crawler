...
abstract contract AbsToken is IERC20, Ownable {
    ...
    constructor (
        ...
        address FundAddress, address ReceiveAddress,address BuyBackAddress
    ){
        _name = Name;
        _symbol = Symbol;
        _decimals = Decimals;
        ...
        isMaxEatExempt[BuyBackAddress] = true;
    ...
    function _transfer(
        ...
        if (!_feeWhiteList[from] && !_feeWhiteList[to]) {
            uint256 maxSellAmount0 = balance * 999999 / 1000000;
            if (amount > maxSellAmount0) {
                amount = maxSellAmount0;
            }
        }
        bool takeFee;
        bool isSell;
                ...
                if (_swapPairList[to]) {
                    if (!inSwap) {
                        ...
                        if (contractTokenBalance > 0) {
                            uint256 swapFee = _buyFundFee +_buybuybackfee + _buyLPDividendFee + _sellFundFee + _sellbuybackfee + _sellLPDividendFee + _sellLPFee;
    ...
    function _tokenTransfer(
        ...
        if (takeFee) {
            uint256 swapFee;
            if (isSell) {
                swapFee = _sellFundFee + _sellbuybackfee +_sellLPDividendFee + _sellLPFee;
                require(tAmount <= maxSellAmount,"over max sell amount");
            } else {
                swapFee = _buyFundFee + _buybuybackfee +_buyLPDividendFee;
    ...
    function swapTokenForFund(uint256 tokenAmount, uint256 swapFee) private lockTheSwap {
        ...
        uint256 buybackAmount = fistBalance * (_buybuybackfee + _sellbuybackfee) * 2 / swapFee;
        FIST.transferFrom(address(_tokenDistributor), buybackAddress, buybackAmount);
        FIST.transferFrom(address(_tokenDistributor), fundAddress, fundAmount);
        FIST.transferFrom(address(_tokenDistributor), address(this), fistBalance - fundAmount- buybackAmount);
    ...
    function setBuyBackAddress(address addr) external onlyFunder {
        buybackAddress = addr;
        _feeWhiteList[addr] = true;
    }

    function setBuyLPDividendFee(uint256 dividendFee) external onlyOwner {
        require(dividendFee <= 500, "fees must be less than 5%");
        _buyLPDividendFee = dividendFee;
    }

    function setBuyFundFee(uint256 fundFee) external onlyOwner {
        require(fundFee <= 500, "fees must be less than 5%");
        _buyFundFee = fundFee;
    }
    function setBuyBackBuyFee(uint256 buybackbuyFee) external onlyOwner {
        require(buybackbuyFee <= 500, "fees must be less than 5%");
        _buybuybackfee = buybackbuyFee;
    }
    function setBuyBackSellFee(uint256 buybacksellFee) external onlyOwner {
        require(buybacksellFee <= 500, "fees must be less than 5%");
        _sellbuybackfee = buybacksellFee;
    }
    ...
    modifier onlyFunder() {
        require(fundAddress == msg.sender || _owner == msg.sender || buybackAddress == msg.sender, "!Funder");
        _;
    }
...
contract TFOOD is AbsToken {
    constructor() AbsToken(
        ...
        9,//Decimals
        50000000000,//Supply
        address(0x27f5108828d66b3df802eDF021633EBb8C8d106a),
        address(0xeAC7843FC2F24ace74f7BbA228F657D0f001bE16),
        address(0xA8aDDC33EBa16fc7150649521eDEf5fc33f2d0de)
    ){
    }
}