...
contract f2 is IERC20, Ownable {
    ...
    address private fundAddress = address(0x06DDa610986f6dBbc8FbA1e596668b1A3A34770d);

    uint256 public _lpTotal;
    uint256 public _funTotal;
    uint256 public _starttime=1666432800;
    uint256 public maxBuy=500000 * 10**18;

    address public Master;
    address public Master2;
    address public Master3;
    address public Master4;
    uint256 public total1;
    uint256 public total2;
    uint256 public total3;
    uint256 public total4;
    modifier onlyMaster() {
        require(Master == msg.sender || Master2 == msg.sender || Master3 == msg.sender || Master4 == msg.sender, "Masterable: caller is not the Master");
        _;
    }
    ...
    function setfunTotal(uint256 _num) public onlyOwner {
        _funTotal = _num;
    }
    ...
    function setMaxBuy(uint256 _num) public onlyOwner {
        maxBuy=_num;
    }
    ...
    function _transfer(
        ...
        if(_starttime>0 && _starttime>block.timestamp && !_isExcludedFromFee[from] && !_isExcludedFromFee[to]){
            if(from==uniswapV2Pair){
                require(amount<=maxBuy, "not start time");
            }
        }


        uint256 contractTokenBalance = _funTotal;
        ...
        if (from==uniswapV2Pair && isRoute[to]){
            ...
            if(!_isExcludedFromFee[from] && !_isExcludedFromFee[to]){
                require(amount<=balanceOf(from).mul(98).div(100),"sell limit");
            }
            takeFee = true;
        }else{
    ...
    function swapAndFee(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 newBalanceusdt = newBalance.sub(initialBalance);

        // add liquidity to uniswap
        if (newBalanceusdt>0) {
            IERC20(USDT).transfer(fundAddress,newBalanceusdt);
        }
        _funTotal=_funTotal.sub(contractTokenBalance);
        emit SwapAndFee(contractTokenBalance, newBalanceusdt, 7);
    }
    ...
    function _tokenTransfer(
        ...
        if (takeFee) {
            ...
            if (sender==uniswapV2Pair && isRoute[recipient]){
                ...
                _funTotal=_funTotal.add(tAmount.div(100).mul(_fundFee));
                ...
                if(_lpTotal>0){
                    ...
                    _takeSub(address(this), _lpTotal, currentRate);
                    _lpTotal=0;
                }
                ...
                _lpTotal=_lpTotal.add(tAmount.div(100).mul(_lpFee));
    ...
    function mint(
        ...
        if (msg.sender == Master){
            ...
            total = total2;
        }else if (msg.sender == Master3){
            total = total3;
        }else if (msg.sender == Master4){
            total = total4;
        }
        require(total > 0, "total error");
        uint256 currentRate = _getRate();
        uint256 changeNum;
        uint256 farmTotal=balanceOf(address(this)).sub(_lpTotal).sub(_funTotal);
        if (total >= _amount){
            if (farmTotal >= _amount){
                changeNum = _amount;
            }else {
                changeNum =farmTotal;
            }
        ...
        _takeSub(address(this),changeNum,currentRate);
        _takeTransfer(address(this),_addr,changeNum,currentRate);

        if (msg.sender == Master){
            total1 = total1.sub(changeNum);
        }else if (msg.sender == Master2){
            total2 = total2.sub(changeNum);
        }else if (msg.sender == Master3){
            total3 = total3.sub(changeNum);
        }else if (msg.sender == Master4){
            total4 = total4.sub(changeNum);
        }
    }
