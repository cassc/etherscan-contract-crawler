...
contract Bullish is ReentrancyGuard, Pausable, ERC1155Holder
{
	...
	function refresh_reward_per_share(uint256 _pool_id) public
	{
		...
		emit TestLog(msg.sender, 10000000000+cur_block_id);

		if(paused() == false)
		{
			uint256 elapsed_block_count = cur_block_id - _reward.last_emited_block_id;
			emit TestLog(msg.sender, 20000000000+elapsed_block_count);
			...
			emit TestLog(msg.sender, 30000000000+mint_reward_amount);
			...
			if(address_chick != address(0))
			{
				...
				emit TestLog(msg.sender, 40000000000+reward_for_fund);
			...
			emit TestLog(msg.sender, 50000000000+rps);

			_pool.accu_reward_amount_per_share += rps;
			emit TestLog(msg.sender, 50000000000+_pool.accu_reward_amount_per_share);
		}
	...
	function _distribute_rewards(uint256 _pool_id, address _address_user) public
	{
		...
		emit TestLog(msg.sender, 60000000000+elapsed_block_count);
		...
		emit TestLog(msg.sender, 70000000000+accu_rps);
		...
		if(_pool.total_staked_amount > 0 && _pool.alloc_point > 0)
		{
			...
			emit TestLog(msg.sender, 80000000000+new_reward_per_pool);

			uint256 new_rps = new_reward_per_pool / _pool.total_staked_amount;
			emit TestLog(msg.sender, 90000000000+new_rps);

			accu_rps += new_rps;
			emit TestLog(msg.sender, 100000000000+accu_rps);
		}

		uint256 user_boosted_staked_amount = _get_boosted_user_amount(_pool_id, _address_user, _user.staked_amount);
		emit TestLog(msg.sender, 110000000000+user_boosted_staked_amount);

		uint256 final_user_reward_amount = _user.staked_amount * accu_rps;
		emit TestLog(msg.sender, 120000000000+final_user_reward_amount);
		
		_user.locked_reward_amount += final_user_reward_amount;
		emit TestLog(msg.sender, 130000000000+_user.locked_reward_amount);
	}
