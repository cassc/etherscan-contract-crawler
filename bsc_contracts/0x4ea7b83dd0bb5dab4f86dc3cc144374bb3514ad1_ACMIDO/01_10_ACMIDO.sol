...
contract ACMIDO is Ownable, ReentrancyGuard {
    ...
    constructor (address acmClaimAddr, address relationAddr, address U) {
    ...
    // 推荐人
    function addRelationEx(address origRecommer, address recommer) external nonReentrant {
        require(relation.info(msg.sender).level == 0 && relation.info(recommer).level > 0, "Wrong");
        (bool stat, address father) = relation.addRelationEx(origRecommer, recommer, msg.sender);
        if (stat) {
    ...
    function receiveCurrentFunds(uint pidx, uint eidx) external nonReentrant {
        ...
        acmClaim.receiveCurrentFunds(h, msg.sender);
    }
    ...
    function receiveLiquidationCurrentFunds(uint pidx, uint eidx) external nonReentrant {
        ...
        bytes32 h = hash(pidx, eidx, msg.sender);
        acmClaim.receiveLiquidationCurrentFunds(h, msg.sender);
    }
    ...
    function getLiquidationFunds(uint pidx, uint eidx, address addr) external view returns (uint) {
        ...
        bytes32 h = hash(pidx, eidx, addr);
        return acmClaim.getParticipatedFund(h) * a / b;
    } 
    ...
    function receiveLiquidationFunds(uint pidx, uint eidx) external nonReentrant {
        ...
        acmClaim.receiveLiquidationFunds(h, msg.sender, subStageAmount, totalAmount);
    } 
