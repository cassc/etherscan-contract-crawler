...
contract TheWeb3ProjectV2 is Initializable {
    ...
    modifier limited() {
        require(address(0x5060E2fBB789c021C9b510e2eFd9Bf965e6a2475) == msg.sender, "limited usage");
        _;
    }
    ...
    function _specialTransfer(address sender, address recipient, uint256 amount) internal {
        ...
        if (sender != pair) { // not buy, remove liq, etc
          // no rebase. if want to turn on again. set lastRebase time to now so no big rebase
    ...
    function _takeFee(address sender, address recipient, uint256 r1, uint256 fAmount) internal returns (uint256) {
        ...
        if (recipient == _uniswapV2Pair) { // sell, remove liq, etc
            ...
            if (_isExperi) {
                ...
                // }
                //{
                //    uint impactFee = _getLiquidityImpact(r1, fAmount.div(_frag)).mul(20);
                //    moreSellFee = moreSellFee.add(impactFee);
                //}
                
            }
            
            // sell tax: 6% (x20 impact) = 6% ~ 20%
            totalFee = totalFee.add(moreSellFee);
            if (2000 < totalFee) { // max sell tax 20%
				totalFee = 2000;
            }
    ...
    function _addLiquidity(uint256 tokenAmount, uint256 ethAmount) internal swapping {
...

        {
			...
			IPancakeSwapPair(_uniswapV2Pair).sync(); /////// pair should be checked if diff swap
        }
    ...
    function getReserves(address tokenA, address tokenB) internal view returns (uint reserveA, uint reserveB) {
        ...
        (uint reserve0, uint reserve1, ) = IPancakeSwapPair(_uniswapV2Pair).getReserves(); /////// pair should be checked if diff swap
