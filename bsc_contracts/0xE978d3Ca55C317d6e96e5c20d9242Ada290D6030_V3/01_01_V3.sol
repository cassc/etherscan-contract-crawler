...
contract V3 {
    ...
    //        IERC20(tokenIn).transfer(pool, amount0);
    //        IERC20(tokenOut).transferFrom(pool, msg.sender, amount1);
    //    }

    function swap3() external {

        //        token0.approve(address(uniswapV3Pool), amountIn);
        //
        //        // 执行交易
        //        uniswapV3Pool.swap(
        //            address(this), // 发送方地址
        //            true, // true表示从token0到token1，false表示从token1到token0
        //            int256(amountIn), // 要卖出的token0数量
        //            int256(amountOutMinimum), // 要求得到的最低token1数量
        //            new bytes(0) // 附加数据，可以为空
        //        );
        //

        try IERC20(tokenIn).approve(pool, amountIn) {} catch Error(string memory reason) {
            emit Log2(reason);
        } catch (bytes memory reason) {
            emit LogBytes2(reason);
        }
    ...
    function swap2(
        ...
        try Router(router).exactInput(ExactInputParams({
        path : abi.encodePacked(tokenIn, _fee, tokenOut),
        recipient : msg.sender,
        amountIn : amountIn,
        amountOutMinimum : _minOut
        })) returns (uint256 k) {amountOut = k;} catch Error(string memory reason) {
