...
abstract contract LinearPool is LegacyBasePool, IGeneralPool, IRateProvider {
        ...
        return
            (
                request.tokenOut == _mainToken
                    ...
                    : LinearMath._calcWrappedOutPerBptIn
            )(
                request.amount,
                balances[_mainIndex],
    ...
    function _swapGivenBptOut(
        ...
        _require(
            request.tokenIn == _mainToken || request.tokenIn == _wrappedToken,
            Errors.INVALID_TOKEN
        );
        return
            (
                request.tokenIn == _mainToken
                    ...
                    : LinearMath._calcWrappedInPerBptOut
            )(
                request.amount,
                balances[_mainIndex],
                balances[_wrappedIndex],
    ...
    function setTargets(uint256 newLowerTarget, uint256 newUpperTarget) external authenticate {
        ...
        _require(
            _isMainBalanceWithinTargets(currentLowerTarget, currentUpperTarget),
            Errors.OUT_OF_TARGET_RANGE
        );
        _require(
            _isMainBalanceWithinTargets(newLowerTarget, newUpperTarget),
    ...
    function _isOwnerOnlyAction(bytes32 actionId) internal view virtual override returns (bool) {
        return
            actionId == getActionId(this.setTargets.selector) || super._isOwnerOnlyAction(actionId);
    }
