...
contract AVANCE is ERC20, ERC20Detailed, Pausable {

    using SafeMath for uint256;
    uint256 TB; 
    uint256 DT;
    uint256 MST = 262800*6;

    uint256 public WA; 
    uint256 public MS; 
    uint256 public RR;
    uint256 public Steps = 262800*6 minutes; 
    uint256 ST = 1 minutes;
    ...
    constructor()
    ERC20Detailed("AVANCE", "AVNC",18)
    {
        DT = block.timestamp;
        RR = 3885000*2*(10**18);
        _isExcluded[msg.sender] = true;
        _isExcluded[address(this)] = true;
        _mint(owner(),(3330000*(10**18)));
        _mint(address(this), 3885000*2*(10**18));
        TB = balanceOf(address(this));
        Steps= Steps.div(60);
        MS = 925000*12*(10**18);
    }
    ...
    {
        uint256 totalTime;
        totalTime = (block.timestamp.sub(DT)).div(ST);
        if(totalTime >= MST){
            totalTime = MST;
        }
        return totalTime;
    ...
    returns(uint256)
    {
        ...
        if(_user == owner()){
            ...
            TokenPerMinutes = (TB.div(Steps));
            _reward =  (calcTime.mul(TokenPerMinutes));

            return _reward.sub(WA);  
        }
        else{
            return 0;
        }       
    ...
    onlyOwner
    {
        ...
        WA += transferReward;
        RR -= transferReward;
        require(WA <= balanceOf(address(this)), "Not enough balance!");
