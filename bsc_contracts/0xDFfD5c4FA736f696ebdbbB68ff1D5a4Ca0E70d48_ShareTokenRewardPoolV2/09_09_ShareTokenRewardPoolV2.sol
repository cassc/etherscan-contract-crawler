...
contract ShareTokenRewardPoolV2 is Operator {
    ...
    uint256 constant public firstMonthReward = 2118 ether;
    ...
    uint256 constant rewardDecreaseEachMonthPercent = 1000; // 10%
    ...
    function pendingUser(uint256 _pid, address _user, uint256 _fromTime, uint256 _toTime) internal view returns (uint256) {
        if (_fromTime > _toTime) return 0;
        if (_toTime >= poolEndTime) {
            if (_fromTime >= poolEndTime) _fromTime = poolEndTime;
    ...
    function getUserReward(uint256 _pid, address _user, uint256 _fromTime, uint256 _toTime) internal view returns (uint256) {
        ...
        for (uint256 i = fromMonth; i <= toMonth; ++i) {
            ...
            if (timeTo > timeFrom && tokenSupply > 0) {
