...
contract FlexIRA is ERC20Upgradeable, DividendPayingTokenInterface, DividendPayingTokenOptionalInterface, OwnableUpgradeable {
    ...
    address public assetFund;

    uint256 public minUserDeposit;
    uint256 public maxUserDeposit;
    uint256 public maxTotalSupply;

    function initialize(address token_, address assetFund_) public initializer {
        __ERC20_init("FlexIRA", "FIRA");
        __Ownable_init();

        token = IERC20(token_);
        assetFund = assetFund_;

        depositFeePct100 = 15;
        minUserDeposit = 1 ether;
        maxUserDeposit = 100000 ether;
        maxTotalSupply = 10000 ether;
    }

    function updateAssetFund(address addr) public onlyOwner {
        assetFund = addr;
    }

    function updateMinUserDeposit(uint256 value) public onlyOwner {
        minUserDeposit = value;
    }

    function updateMaxUserDeposit(uint256 value) public onlyOwner {
        maxUserDeposit = value;
    }

    function updateMaxTotalSupply(uint256 value) public onlyOwner {
    ...
    function deposit(uint256 amount) public {
        ...
        _safeTransferTokensFrom(_msgSender(), address(this), amount);
        uint256 balanceAfter = token.balanceOf(address(this));

        uint256 received = balanceAfter.sub(balanceBefore);

        require(received > 0, 'FlexIRA: received zero or negative tokens');

        uint256 depositFee = received * depositFeePct100 / 100;

        _safeTransferTokens(assetFund, depositFee);

        _mint(_msgSender(), received.sub(depositFee));
    }

    function withdrawAllDeposit() public {
        uint256 myBalance = balanceOf(_msgSender());
        require(myBalance > 0, "FlexIRA: balance is empty");

        _burn(msg.sender, myBalance);
        _safeTransferTokens(msg.sender, myBalance);
    }
    ...
    function withdrawDividend() public override {
        ...
        if (_withdrawableDividend > 0) {
            ...
            _safeTransferTokens(msg.sender, _withdrawableDividend);
        }
    }
