...
contract SimpleStaking is Ownable {
    ...
    function getUserClaimableReward(address account) public view returns (uint256) {
        uint256 totalAmount = userAmounts[account] * totalRewards / totalStakedTokens;
        return totalAmount > userClaimed[account] ? totalAmount - userClaimed[account] : 0;
    }
    ...
    function claim() external virtual {
        ...
        address account = msg.sender;
        uint256 amount = getUserClaimableReward(account);
