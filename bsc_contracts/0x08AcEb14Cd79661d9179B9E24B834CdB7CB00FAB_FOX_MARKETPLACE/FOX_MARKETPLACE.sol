...
contract FOX_MARKETPLACE {
    ...
    address private constant CEO = 0x6A4D3Fe038eaB7F3EEf5a3db51A931bcf8aff152;
    ...
    mapping(uint256 => Bids[]) bidsOfIndex;
    mapping(address => EnumerableSet.UintSet) indexesOfBidsPlacedByAddress;
    mapping(address => uint256[]) offersOfAddress;
    ...
    function offerForSale(uint256 id, uint256 price, address contractAddress, address currencyAddress) external {
        ...
        thisNft.index = nftsForSale.length;
        offersOfAddress[msg.sender].push(nftsForSale.length);
    ...
    function buy(uint256 index) external {
        ...
        if(fee>0) require(IBEP20(thisNft.currency).transferFrom(msg.sender, CEO, thisNft.price * fee / feeDenominator),"Token transfer failed");
    ...
    function buyWithBnb(uint256 index) external payable {
        NFT memory thisNft = nftsForSale[index];
        require(indexesForSale.contains(index), "Not for sale anymore");
        require(!thisNft.isAuction, "Can't buy an auction");
        require(thisNft.currency == address(0), "This function only works for BNB");
        require(msg.value == thisNft.price, "Wrong amount of BNB sent");
        if(fee>0) {
            (bool success,) = address(CEO).call{value: thisNft.price * fee / feeDenominator}("");
            require(success, "Transfer failed");
        }
        payable(thisNft.seller).transfer(thisNft.price * (feeDenominator - fee) / feeDenominator);
        indexesForSale.remove(index);
        IERC721(thisNft.nftContract).transferFrom(thisNft.seller, msg.sender, thisNft.id);
        emit NftBought(index, msg.sender, thisNft);
    }
    ...
    function placeBid(uint256 index, uint256 bid) external {
        ...
        thisBid.bidAmount = bid;
        bidsOfIndex[index].push(thisBid);
        indexesOfBidsPlacedByAddress[msg.sender].add(index);
        emit BidPlaced(index, nftsForSale[index]);
    }

    function placeBidWithBnb(uint256 index) external payable {
        NFT memory thisNft = nftsForSale[index];
        require(indexesForSale.contains(index), "Not for sale anymore");
        require(thisNft.isAuction, "Can't bid on a nonAuction");
        require(thisNft.currency == address(0), "This function only works for BNB");
        require(msg.value > thisNft.price, "Bid needs to be higher than last");
        require(thisNft.startingTimeStamp <= block.timestamp, "Auction hasn't started yet");
        require(thisNft.endingTimeStamp >= block.timestamp, "Auction is finished");
        if(thisNft.highestBidder != address(0)) payable(thisNft.highestBidder).transfer(thisNft.price);
        nftsForSale[index].highestBidder = msg.sender;
        nftsForSale[index].price = msg.value;
        Bids memory thisBid;
        thisBid.bidder = msg.sender;
        thisBid.bidAmount = msg.value;
        bidsOfIndex[index].push(thisBid);
        indexesOfBidsPlacedByAddress[msg.sender].add(index);
        emit BidPlaced(index, nftsForSale[index]);
    }
    ...
    function finalizeAuction(uint256 index) external {
        ...
        if(thisNft.currency == address(0)) {
            ...
            if(fee>0) require(IBEP20(thisNft.currency).transfer(CEO, thisNft.price * fee / feeDenominator),"Token transfer failed");
    ...
    function acceptCurrentBid(uint256 index) external {
        NFT memory thisNft = nftsForSale[index];
    ...
    function cancelUnsoldOffer(uint256 index) external {
        ...
        require(thisNft.seller == msg.sender, "Only the seller can do that");
        require(indexesForSale.contains(index), "Not for sale anymore");
        require(!thisNft.isAuction, "That's an auction");
        indexesForSale.remove(index);
    ...
    function getAllNftsForSale() public view returns(NFT[] memory) {
        uint256[] memory allIndexes = indexesForSale.values();
        NFT[] memory allNftsForSale = new NFT[](allIndexes.length);
        for (uint256 i = 0; i < allIndexes.length; i++) allNftsForSale[i] = nftsForSale[allIndexes[i]];
        return allNftsForSale;
    }

    function getSomeNftsForSale(uint256 start, uint256 howMany) public view returns(NFT[] memory) {
        uint256[] memory allIndexes = indexesForSale.values();
        NFT[] memory someNftsForSale = new NFT[](howMany);
        for (uint256 i = start; i < start + howMany; i++) someNftsForSale[i] = nftsForSale[allIndexes[i]];
        return someNftsForSale;
    }

    function getAllBidsOfIndex(uint256 index) public view returns(Bids[] memory) {
        return bidsOfIndex[index];
    }

    function getAllBidsOfAddress(address wallet) public view returns(uint256[] memory) {
        return indexesOfBidsPlacedByAddress[wallet].values();
    }

    function getAllOffersOfAddress(address wallet) public view returns(uint256[] memory) {
        return offersOfAddress[wallet];
    }
