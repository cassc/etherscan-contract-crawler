...
contract DecentralizedFinance is ERC20, Ownable {
    ...
    // BSC USDT  the coin for dividend
    address public dividendCoin = 0x55d398326f99059fF775485246999027B3197955;

    address public marketingWallet = 0x4FfA88D48562B499AF0E776Aa1b5E1B05c32fa32;

    uint DECIMAL_NUM = 10 ** 8;

    uint256 public swapTokensAtAmount = 20 * DECIMAL_NUM;
    ...
    uint256 directFee_buy = 1;
    uint256 indirectFee_buy = 1;

    uint256 directFee_sell = 1;
    ...
    address private multiSigWallet = 0x4FfA88D48562B499AF0E776Aa1b5E1B05c32fa32;
    ...
    constructor() ERC20("Decentralized Finance", "DFI") {
        ...
        */
        _mint(owner(), 30_000 * DECIMAL_NUM);
    ...
    function _transfer(
        ...
        if((from != uniswapV2Pair && to != uniswapV2Pair)&&(!(automatedMarketMakerPairs[from]|| automatedMarketMakerPairs[to]))) {
            ...
            if(recommend[to] == address(0) && amount >= 1_000) {
                recommend[to] = from;
        ...
        if(!_isExcludedFromFees[from] && !_isExcludedFromFees[to]) {
            uint recommendFee = 0;
            uint256 feesAmount = amount.mul(totalFees).div(1000);
    ...
    function sendTokenToRecommend(address trader, uint _buyFeeAmount, uint _recommendFeeFee) private {
        ...
        if (balanceOf(recommend[trader]) < 20 * DECIMAL_NUM) {
            return;
        }
        ...
        if (balanceOf(indirectRecommendation[trader]) < 20 * DECIMAL_NUM) {
            return;
        }
    ...
    function swapAndSendToUSDTForRecommend(address trader, uint256 _amount) private {
        if (balanceOf(address(this)) < _amount || _amount < 1) {
            return;
        }
        ...
        if (balanceOf(recommendRecipient) < 20 * DECIMAL_NUM) {
            return;
        }
        ...
        if (balanceOf(indirectRecommendRecipient) < 20 * DECIMAL_NUM) {
            hasIndeirect = false;
        }
    ...
    function swapAndSendDividends(uint256 tokens) private{
        ...
        uint256 dividends = IERC20(dividendCoin).balanceOf(address(this));
        bool success = IERC20(dividendCoin).transfer(address(dividendTracker), dividends);
...
contract DividendTracker is Ownable, DividendPayingToken {
    ...
    address tokenWallet ;
    address public multiSigWallet = 0x4FfA88D48562B499AF0E776Aa1b5E1B05c32fa32;
    ...
    constructor() DividendPayingToken("Dividen_Tracker", "Dividend_Tracker") {
        claimWait = 60;
        minimumTokenBalanceForDividends = 20 * DECIMAL_NUM; // must hold 20+ tokens
    }
