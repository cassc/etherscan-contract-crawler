...
abstract contract Project {
    ...
    uint256 public swapLp = 4;
    uint256 public swapBoost = 2;
    uint256 public swapReward = 3;
    uint256 public swapMarketing = 2;
    uint256 public swapCoinwars = 1;
    uint256 public swapBurn = 0;
    uint256 public totalSellFee = swapLp + swapBurn + swapMarketing + swapCoinwars + swapReward + swapBoost;

    address public marketingWallet = 0x58c801F210871d78761559025A05fCDdF527f821;
    address public coinwarsWallet = 0x58c801F210871d78761559025A05fCDdF527f821;

    string constant PROJECT_NAME = "Bumble";
    string constant SYMBOL = "Bmbl";
    uint8 constant DECIMALS = 9;

    uint256 constant PROJECT_TOTAL_SUPPLY = 1 * 10**9 * 10**DECIMALS;

    uint256 public maxTxnAmount = (PROJECT_TOTAL_SUPPLY * 20) / 1000; // (PROJECT_TOTAL_SUPPLY * 10) / 1000 [this equals 1%]
    uint256 public maxWalletAmount = maxTxnAmount;//

    uint256 constant BUY_FEE          = 12;
...
contract DividendDistributor is IDividendDistributor  {
    ...
    function setNewRewardForShareholder(address shareholder, address customRwrd) external onlyToken {
        ...
        if(shareholderShare.currentRwrd != customRwrd) {
            ...
            address previousReward = shareholderShare.currentRwrd;
            rewardsInfo[previousReward].totalHolders -= 1;
...
// Stats on the shareholder
    ...
    function getShareholderReward(address account) public view returns (address currentRwrd) {
        if(shares[account].hasCustom) {
            currentRwrd = shares[account].currentRwrd;
        } else {
            currentRwrd = address(defaultRwrd);
 ...
 * MainContract
 */
contract Bumble is Project, IBEP20, Ownable {
    using SafeMath for uint256;
    ...
    // Return the balance INCLUDING all locked tokens
    // this is the actual actual balance for an account balanceOf factors in the locked tokens
...
// ***
    ...
    function swapBack() internal swapping {
        ...
        (bool tmpSuccess,) = payable(marketingWallet).call{value: marketingToPay, gas: 30000}("");
...
// ***
    ...
    function getShareholderCurrentReward(address holder) external view returns (
        address rwrd
    ) {
        return distributor.getShareholderReward(holder);
    }
    ...
    function getLockedTokensForWallet(address wallet) external view returns (uint256) {
        return lockedTokens[wallet];
    }

    // Return balance information for a given wallet
    function getTokenBalanceInfoForWallet(address wallet) external view returns (
        uint256 fullBalance, 
        uint256 lockedBalance, 
        uint256 availableBalance
    ) {
        fullBalance = getActualBalanceOf(wallet);
        lockedBalance = lockedTokens[wallet];
        availableBalance = balanceOf(wallet);
    }
