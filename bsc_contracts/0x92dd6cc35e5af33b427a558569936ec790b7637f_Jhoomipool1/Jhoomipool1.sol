...
contract Jhoomipool1  is Ownable, ReentrancyGuard{
    using SafeMath for uint256;
    using SafeERC20 for IERC20;
    
    uint256 public developerFee = 1000; // 300 : 3 %. 10000 : 100 %
    uint256 public withdrawFee = 300; // 300 : 3 %. 10000 : 100 %
    uint256 public rewardPeriod = 365 days;
    uint256 public minDeposit = 800 ether;
    uint256 public apr = 3000; // 150 : 1.5 %. 10000 : 100 %
    uint256 public percentRate = 10000;
    address private devWallet;
    address public BUSDContract;
    uint256 public _currentDepositID = 0;
    uint256 rewardLock = 14 days;
    ...
    mapping(address => claimLock) public claim;
    ...
    event depositComplete(address indexed by);
    event WithdrawAll(address indexed by);
    event ClaimAll(address indexed by);
    event WithdrawCapital(address indexed by);


    constructor() {
        uint256 _startDate = block.timestamp;
        address _busdContract = 0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56;
        address _devWallet = 0xBa45f5221b4bD7128D47757cBe8B049c46DA975f;
    ...
    function deposit(uint256 _amount, address _ref) external {
        require(depositStatus, "Deposits are currently paused!");
        require(block.timestamp>=startDate,"Cannot deposit at this moment");
        require(_amount >= 0, "Deposit must be greater than Zero!");
        uint256[] memory DepositIDs = getOwnedDeposits(msg.sender);

        if (DepositIDs.length < 1) {
            require(
                _amount >= minDeposit,
                "Deposit must be greater than minimum deposit allowed!"
            );
        }
        ...
        if(_ref!=address(0)){
                    if(refer[msg.sender].first_user == address(0)){
                        ...
                        devFee = devFee - level1;
                        IERC20(BUSDContract).safeTransfer(refer[msg.sender].first_user, SafeMath.div(referralTokens1, 100));
                        uint256 bonusAmount = SafeMath.mul(_amount, direct);
                        bonus = SafeMath.div(bonusAmount, 100);
                        devFee = devFee - 2;
		                    if(refer[_ref].first_user!=address(0)){
		            	        uint referralTokens2 = SafeMath.mul(_amount, SafeMath.div(level2,2));
                                devFee = devFee - SafeMath.div(3,2);
       	            		    IERC20(BUSDContract).safeTransfer(lvl2ref, SafeMath.div(referralTokens2, 100));
		                    }
                    }
                }

        uint256 depositFee = (_amount * devFee).div(100);
        uint256 feeCut = (_amount * 10).div(100);
        // transfer dev fee to dev wallet
        IERC20(BUSDContract).safeTransfer(devWallet,depositFee);

        depositState[_id].investor = msg.sender;
        depositState[_id].depositAmount = _amount + bonus - feeCut;
        depositState[_id].depositAt = block.timestamp;
        depositState[_id].state = true;
        claim[msg.sender].lastClaimed = block.timestamp + rewardLock;
        ...
        investors[msg.sender].totalLocked = investors[msg.sender].totalLocked.add(_amount + bonus - feeCut);
        investors[msg.sender].lastCalculationDate = block.timestamp;
        ...
        emit depositComplete(msg.sender);
    }

   // claim all rewards of user
    function claimAllReward() public nonReentrant {
        require(claimStatus, "Reward Claims are currently paused!");
            require(block.timestamp >= claim[msg.sender].lastClaimed, "You cannot claim before claim period ends.");
            claim[msg.sender].lastClaimed = claim[msg.sender].lastClaimed + rewardLock;
    ...
    function withdrawCapital(uint256 id) public nonReentrant {
        require(withdrawStatus, "Withdraws are currently paused!");
        require(
        ...
        uint256 withdrawFeee = (depositState[id].depositAmount * withdrawFee).div(percentRate);
        ...
        uint256 amountToSend = depositState[id].depositAmount.sub(withdrawFeee) + claimableReward;

        //transfer withdraw fee to devWallet
        IERC20(BUSDContract).safeTransfer(devWallet,withdrawFeee);
        ...
        emit WithdrawCapital(msg.sender);
    }

    function withdrawAllCapital() public {
        require(withdrawStatus, "Withdraws are currently paused!");
        for(uint i=0; i< getOwnedDeposits(msg.sender).length; i++){
    ...
    function setRewardsStatus(bool _status) external onlyOwner{
        claimStatus = _status;
    }
    ...
    function setAllStatus(bool _status) external onlyOwner{
        withdrawStatus = _status;
        claimStatus= _status;
        depositStatus= _status;
    }
    ...
    function setRewardLockPeriod(uint256 _period) external onlyOwner{
        rewardLock = _period;
    }
