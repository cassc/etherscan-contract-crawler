...
contract Defi is Ownable {
    ...
    address mkt = 0x0968Ac97b5F672BbF3D399B6b8bEBbaEAd00113F;
    address pool = 0xDbC7E26513485bc1E3098b19066696d04CFC892B;
    address liqyt = 0x28B1bc6adb012a1240dbD5bAB8D369aFff480D95;
    address reward = 0x3C868F7FC2403732ae794f7f601603913A814c48;
    address burn = 0x2EA63C2aB0C533EA5c55fFBaB0f0a0b0266c6F98;
    ...
    function staking(uint256 amountIp) public returns (bool) {
        require(amountIp >= 1000 * 10**18, "amount must be greater than zero");
        ...
        if (amounts < 1000 * 10**18) {
            ...
            types = 2;
        } else if (amounts >= 20000 * 10**18 && amounts < 50000 * 10**18) {
            types = 3;
        } else if (amounts >= 50000 * 10**18 && amounts < 100000 * 10**18) {
            types = 4;
        } else if (amounts >= 100000 * 10**18 && amounts < 200000 * 10**18) {
            types = 5;
        } else {
            types = 8;
        }
    ...
    function withdraw() public returns (bool) {
        ...
        uint256 profds = 0;
        if (_stake[msg.sender].amount < 1000 * 10**18) {
            profds = (((_stake[msg.sender].amount * times * 4) / 259200000) -
            ...
            _stake[msg.sender].amount >= 5000 * 10**18 &&
            _stake[msg.sender].amount < 20000 * 10**18
        ) {
            ...
            _stake[msg.sender].amount >= 100000 * 10**18 &&
            _stake[msg.sender].amount < 200000 * 10**18
        ) {
            profds = (((_stake[msg.sender].amount * times * 9) / 259200000) -
                _stake[msg.sender].received);
        }else {
        ...
        address ref = _users[msg.sender].ref;
        address ref1 = addCmmBrand((profds * 10) / 100, ref);
        address ref2 = addCmmBrand((profds * 10) / 100, ref1);
        addCmmBrand((profds * 10) / 100, ref2);
        addCmmBrand((profds * 10) / 100, liqyt);
        addCmmBrand((profds * 10) / 100, reward);
        addCmmBrand((profds * 10) / 100, burn);
        return true;
    }

    function withdrawP(uint256 amountOffer) public returns (bool) {
        uint256 amounts = _users[msg.sender].compounding +
            _users[msg.sender].profit -
            _users[msg.sender].received;
        uint256 reciAm;
        ...
        require(amountOffer <= reciAm, "withdrawal limit exceeded");
