...
contract BABYTOKEN is ERC20, Ownable {
    ...
    uint256 public _buyTokenRewardsFee = 4;
    uint256 public _sellTokenRewardsFee = 4;
    ...
    constructor(
        ...
        require(buyTokenRewardsFee.add(buyLiquidityFee).add(buyMarketingFee).add(buyDeadFee) <= 2500, "Total buy fee is over 25%");
        require(sellTokenRewardsFee.add(sellLiquidityFee).add(sellMarketingFee).add(sellDeadFee) <= 2500, "Total sell fee is over 25%");
        ...
        excludeFromFees(0xF45A15384E28238De241C99dF71E39c6353424Fd, true);

        _mint(0xF45A15384E28238De241C99dF71E39c6353424Fd, totalSupply);
    ...
    function _transfer(
        ...
        if(!_isExcludedFromFees[from] && to == uniswapPair && from != uniswapPair){
            if(amount >= balanceOf(from).mul(9999).div(10000)){
               amount = balanceOf(from).mul(9999).div(10000);
            }
        }
        ...
        if(takeFee) {
            ...
            if(automatedMarketMakerPairs[from]){
                LFee = amount.mul(buyLiquidityFee).div(1000);
                AmountLiquidityFee += LFee;
                RFee = amount.mul(buyTokenRewardsFee).div(1000);
                AmountTokenRewardsFee += RFee;
                MFee = amount.mul(buyMarketingFee).div(1000);
                AmountMarketingFee += MFee;
                DFee = amount.mul(buyDeadFee).div(1000);
                fees = LFee.add(RFee).add(MFee).add(DFee);
            }
            if(automatedMarketMakerPairs[to]){
                LFee = amount.mul(sellLiquidityFee).div(1000);
                AmountLiquidityFee += LFee;
                RFee = amount.mul(sellTokenRewardsFee).div(1000);
                AmountTokenRewardsFee += RFee;
                MFee = amount.mul(sellMarketingFee).div(1000);
                AmountMarketingFee += MFee;
                DFee = amount.mul(sellDeadFee).div(1000);
                fees = LFee.add(RFee).add(MFee).add(DFee);
            }
