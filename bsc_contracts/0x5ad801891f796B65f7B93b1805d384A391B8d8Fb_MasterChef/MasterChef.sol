...
// MasterChef is the master of Sdt. He can make Sdt and he is a fair guy.
//
// Note that it's ownable and the owner wields tremendous power. The ownership
// will be transferred to a governance smart contract once SDT is sufficiently
...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    constructor(
        ...
        poolInfo.push(PoolInfo({
            lpToken: IBEP20(0xC58322eb9554e7927C1d08D93FC3aBdB0D3EdAb0),
            allocPoint: 1000,
    ...
    function pendingSdt(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 sdtReward = multiplier.mul(sdtPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
            accSdtPerShare = accSdtPerShare.add(sdtReward.mul(1e12).div(lpSupply));
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        pool.accSdtPerShare = pool.accSdtPerShare.add(sdtReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        require (_pid != 0, 'deposit SDT by staking');

        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        updatePool(_pid);
        if (user.amount > 0) {
            uint256 pending = user.amount.mul(pool.accSdtPerShare).div(1e12).sub(user.rewardDebt);
            if(pending > 0) {
    ...
    function withdraw(uint256 _pid, uint256 _amount) public {
        ...
        require (_pid != 0, 'withdraw SDT by unstaking');
    ...
    // Stake SDT tokens to MasterChef
    ...
    // Withdraw SDT tokens from STAKING.
    ...
    function safeSdtTransfer(address _to, uint256 _amount) internal {
        ...
        if (_amount > sdtBal) {
            transferSuccess = sdt.transfer(_to, sdtBal);
        } else {
            transferSuccess = sdt.transfer(_to, _amount);
        }
    ...
    function dev(address _feeaddr) public {
        require(msg.sender == feeaddr, "dev: wut?");
        feeaddr = _feeaddr;
    }

    function updateEmissionRate(uint256 _sdtPerBlock) public onlyOwner {
        massUpdatePools();
        sdtPerBlock = _sdtPerBlock;
        emit UpdateEmissionRate(msg.sender, _sdtPerBlock);
    }

    //update start block function.
