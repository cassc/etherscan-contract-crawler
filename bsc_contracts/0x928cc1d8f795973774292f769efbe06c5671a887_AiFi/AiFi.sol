...
contract AiFi is ERC20, Ownable {
  ...
  function setPcsAddress(address pcs_address_) external onlyOwner {
    pcs_address = pcs_address_;
  }
  ...
  function _beforeTokenTransfer(address from_, address to_, uint256 amount_)
    ...
    if(to_ == pcs_address) {
      require(!is_paused, "You are not allowed to transfer to PCS");
    }
