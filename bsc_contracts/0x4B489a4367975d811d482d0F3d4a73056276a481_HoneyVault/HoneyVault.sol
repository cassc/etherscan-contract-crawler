...
contract HoneyVault is Ownable {
    ...
    function setOnlyDistributor(address _distributor) external {
        require(
            operator == msg.sender,
            "Operator can call this function"
        );
    ...
    function withdraw(uint16 method) external {
        ...
        if (method == 1 && withdrawAmount > 0) {
            HobeeToken.transfer(msg.sender, withdrawAmount);
        } else if (method == 2 && withdrawAmount > 0) {
            HinuuToken.transfer(msg.sender, withdrawAmount);
        } else if (method == 3 && withdrawAmount > 0) {
            HethToken.transfer(msg.sender, withdrawAmount);
        }
    ...
    function setTeamWallet(address _teamWallet) public onlyOwner returns (address)
    {
        teamWallet = _teamWallet;
        return teamWallet;
    }
