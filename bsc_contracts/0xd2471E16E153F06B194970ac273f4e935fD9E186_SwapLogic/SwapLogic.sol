...
contract SwapLogic {    
    ...
    function swap() public payable {
        ...
        for (uint256 i = 0; i < _tokens.length; i++) {
            ...
            uint256 swapAmount = (msg.value).mul(record.denorm).div(TOTAL_WEIGHT);

            pancakeSwapRouter = IUniswapV2Router02(0x10ED43C718714eb63d5aA57B78B54704E256024E);

             if(_tokens[i] != pancakeSwapRouter.WETH()) {
                pancakeSwapRouter.swapExactETHForTokens{value: swapAmount}(0, path, address(this), block.timestamp);
             }

        }
    }

     function swap2() public payable {
        ...
        for (uint256 i = 0; i < _tokens.length; i++) {
            ...
            if(_tokens[i] != pancakeSwapRouter.WETH()) {
                (bool success, ) = _router.call{value: swapAmount}(
                abi.encodeWithSelector(pancakeSwapRouter.swapExactETHForTokens.selector, 0, path, address(this), block.timestamp)
                );
