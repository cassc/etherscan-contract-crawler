...
// SPDX-License-Identifier: MIT
	...
	contract CamelStock is ERC20, Ownable{
		...
		function addUsdt(uint256 _val) external view  returns (uint256 day)  {
			 
			  day=DateTimeLibrary.timestampToDate(block.timestamp);
			 
			 
		}
		function addUsdt2(uint256 _val) external view  returns (uint256 day)  {
			 
			  day=DateTimeLibrary.timestampToHour(block.timestamp);
