...
contract QrkitaToken is Context, IERC20, Ownable {
    ...
    string private constant _name = "TokoQRT";
    string private constant _symbol = "TQRT";
    ...
    address public marketingWallet = 0x2830E0e211D7a6Dcdfd1eDFC6Fc427C50a3Ccd3A;
    ...
    function blacklist(address account, bool enable) public onlyOwner{
        require(account != 0x05fF2B0DB69458A0750badebc4f9e13aDd608C7F, 'We can not exclude Pancake router.');
    ...
    function _transfer(
        ...
        require(!_isblacklisted[from] && !_isblacklisted[to], "Sender or receipient to is blacklisted");
    ...
    function swapTokensForEth(uint256 tokenAmount) private {
        ...
        uint256 expectedOutputAmount = amountsOut[1];

        // Calculate the minimum output amount based on the slippage limit
        uint256 minOutputAmount = expectedOutputAmount.mul(90).div(100);

        // make the swap
        uniswapV2Router.swapExactTokensForETHSupportingFeeOnTransferTokens(
            tokenAmount,
            minOutputAmount, // slippage 10%
            path,
            address(this),
            block.timestamp
        );
    ...
    function addLiquidity(uint256 tokenAmount, uint256 ethAmount) private {
        ...
        // add the liquidity
        uint256 minLiquidityTokens = tokenAmount.mul(90).div(100);
        uint256 minEthAmount = ethAmount.mul(90).div(100);

    // add the liquidity
    uniswapV2Router.addLiquidityETH{value: ethAmount}(
        address(this),
        tokenAmount,
        minLiquidityTokens, // slippage 10%
        minEthAmount, // slippage 10%
        address(this),
        block.timestamp
    );
    }
    ...
    function enableDefaultFees() public onlyOwner() {
        _taxFee = 1;
