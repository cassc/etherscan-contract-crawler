 ...
 * SPDX-License-Identifier: MIT
    ...
    Website: pneumonoultramicroscopicsilicovolcanoconiosis.app

    Telegram: https://t.me/Pneumonoul
...
contract IrnBruInu is IERC20Metadata, Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    using Address for address;
    
    address internal deadAddress = 0x000000000000000000000000000000000000dEaD;
    
    address public marketingWallet = 0x25cAA5B80bd27cd47524cec0480D6EA6C84CBaD6;
    address public developmentWallet = 0x25cAA5B80bd27cd47524cec0480D6EA6C84CBaD6;
    
    string constant _name = "Irn Bru Inu";
    string constant _symbol = "IRNBRU";
    ...
    uint256 public developmentFee = 30; // 3%
    uint256 public lpFee = 30; // 3%
    uint256 private totalFee;
    
    // Total = 100% (1000)
    uint256 public marketingPortionOfSwap = 250; // 25%
    uint256 public developmentPortionOfSwap = 375; // 37.5%
    uint256 public lpPortionOfSwap = 375; // 37.5%
    ...
    constructor () {
        ...
        IuniswapV2Router _newuniswapRouter = IuniswapV2Router(0x10ED43C718714eb63d5aA57B78B54704E256024E);
        pair = IuniswapV2Factory(_newuniswapRouter.factory()).createPair(address(this), _newuniswapRouter.WETH());
        router = _newuniswapRouter;

        // set fees
        totalFee = marketingFee.add(developmentFee).add(lpFee);
    ...
    function updateFees( uint256 _marketing, uint256 _development, uint256 _lp) external onlyOwner {
       totalFee = _marketing.add(_development).add(_lp); 
    ...
    function updatePortionsOfSwap(uint256 marketingPortion, uint256  developmentPortion, uint256 lpPortion) 
    external onlyOwner {
        
        uint256 totalPortion = marketingPortion.add(developmentPortion).add(lpPortion);
