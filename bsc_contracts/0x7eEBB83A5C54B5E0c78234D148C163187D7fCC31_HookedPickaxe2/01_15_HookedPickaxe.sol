...
contract HookedPickaxe2 is ERC721, ERC721Burnable, AccessControl {
    ...
    constructor() ERC721("Hooked Pickaxe 2", "HPA2") {
    ...
    function mint(address to) external onlyRole(GAMER_ROLE) {
        require(balanceOf(to) == 0, "Wallet HPA2 limit exceeded");
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        ...
        string memory json = Base64.encode(bytes(string(abi.encodePacked('{"name": "Hooked Pickaxe2 #', Strings.toString(tokenId), '", "description": "The fortune explorer that gives access for one to earn and own the first share of crypto in Hooked platform.", "image": "data:image/svg+xml;base64,', Base64.encode(bytes(output)), '"}'))));
