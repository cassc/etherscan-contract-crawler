...
// File: contracts/strategies/BUSDStrategyDforceV1.sol
...
contract BUSDStrategyDforceV2 is Ownable {
    using SafeMath for uint256;
    ...
    uint256 public rebalanceGasStipend = 3000000; // This is the gas units that are covered by executing a rebalance from the WBNB profit  
    ...
    uint256 public minBUSDDeposit = 5000e18; // Minimum amount user can deposit before strat adds it
    ...
    */
    ...
    function expectedProfit(address _executor, uint256 seed) external returns (uint256,uint256,bytes32) {
        ...
        if(ratio > 0 && ratio < minHealthRatio) {
            ...
            wbnbGain = wbnbGain.add(rebalanceGasStipend.mul(gasPrice));
