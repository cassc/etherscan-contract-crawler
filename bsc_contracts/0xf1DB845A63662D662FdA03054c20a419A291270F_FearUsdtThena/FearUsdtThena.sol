...
// File contracts/FearUsdtThena.sol

//SPDX-License-Identifier: MIT

pragma solidity 0.8.17;
contract FearUsdtThena  is Ownable, Pausable {
    using SafeERC20 for IERC20;

    // Tokens
    address public constant wbnb = address(0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c);
    address public constant the = address(0xF4C8E32EaDEC4BFe97E0F595AdD0f4450a863a11);
    address public constant fear = address(0x9Ba6a67a6F3b21705a46B380A1B97373a33da311);
    ...
    //Routes
    IThenaRouter.Routes[] public bnbToFearPath;
    IThenaRouter.Routes[] public bnbToUsdtPath;
    ...
    constructor(
        ...
        IThenaRouter.Routes[] memory _bnbToFearPath,
        IThenaRouter.Routes[] memory _bnbToUsdtPath
    ) {
        ...
        for (uint i; i < _bnbToFearPath.length; ++i) {
            bnbToFearPath.push(_bnbToFearPath[i]);
        }

        for (uint i; i < _bnbToUsdtPath.length; ++i) {
            bnbToUsdtPath.push(_bnbToUsdtPath[i]);
        }
    ...
    function addLiquidity() internal {
        ...
        IThenaRouter(router).swapExactTokensForTokens(wbnbHalf, 0, bnbToFearPath, address(this), block.timestamp);
        IThenaRouter(router).swapExactTokensForTokens(wbnbHalf, 0, bnbToUsdtPath, address(this), block.timestamp);

        uint256 t1Bal = IERC20(fear).balanceOf(address(this));
        uint256 t2Bal = IERC20(usdt).balanceOf(address(this));

        IThenaRouter(router).addLiquidity(fear, usdt, stable, t1Bal, t2Bal, 1, 1, address(this), block.timestamp);
    }
    ...
    function _subAllowance() internal {
        ...
        IERC20(fear).safeApprove(router, 0);
        IERC20(usdt).safeApprove(router, 0);
    }
    ...
    function _addAllowance() internal {
        ...
        IERC20(fear).safeApprove(router, type(uint).max);
