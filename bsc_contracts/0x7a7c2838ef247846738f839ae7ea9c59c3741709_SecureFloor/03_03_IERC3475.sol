...
interface IERC3475 {
     ...
     * @param _principal is the principal amount
     */
    function issue(address _to, uint256 _principal, Transaction calldata _transaction) external;
    ...
    struct BondParameters {
        ...
        //bool isCoin;    // true if pair token is native coin (ETH, BNB, etc)
        uint256 issuanceDate;   // IBO end date. Until this date voting is forbidden.
        uint256 prepaymentPenalty;
        uint256 maturityDate;
        uint256 maturityProfitDate; //maturityProfitDate amount
    }
    ...
    function updateFarming(uint256 classId, address user) external;
    // initialize Derex variables
    function initDerex(address _derexRouter) external;
    // unlock private pool
    function unlockPool(uint256 classId, uint256 nonceId) external;
