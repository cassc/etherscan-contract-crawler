...
contract TITANPROTOCOL is IERC20, Context, Ownable {
    ...
    address private Titan__Marketing_Wallet = 0x7e737Fa38B7b71F1662E1022A0bc23e7b539aF16;
    address private Titan__Team_Wallet = 0x1E58F31Fbc7Dc1E8356A24BA60FB2b63a20f91EC;
    
    string private constant _symbol = "TPC";
    string private constant _name = "Titan Protocol";

    uint256 public Titan__max_BuyLimit = 1000000 * 10**9;
    uint256 public Titan__max_SellLimit = 500000 * 10**9;

    uint256 public Titan__max_WalletLimit = 1000000  * 10**9;
    ...
    Taxes public taxes = Taxes(0, 5, 2, 3);
    ...
    constructor (address routerAddress) {
        ...
        _isExcludedFromFee[Titan__Marketing_Wallet] = true;
        _isExcludedFromFee[Titan__Team_Wallet] = true;
        
        allowedTransfer[address(this)] = true;
        allowedTransfer[owner()] = true;
        allowedTransfer[pair] = true;
        allowedTransfer[Titan__Marketing_Wallet] = true;
        allowedTransfer[Titan__Team_Wallet] = true;
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if(from == pair && !_isExcludedFromFee[to] && !swapping){
            require(amount <= Titan__max_BuyLimit, "You are exceeding Titan__max_BuyLimit");
            require(balanceOf(to) + amount <= Titan__max_WalletLimit, "You are exceeding Titan__max_WalletLimit");
        }
        
        if(from != pair && !_isExcludedFromFee[to] && !_isExcludedFromFee[from] && !swapping){
            require(amount <= Titan__max_SellLimit, "You are exceeding Titan__max_SellLimit");
            if(to != pair){
                require(balanceOf(to) + amount <= Titan__max_WalletLimit, "You are exceeding Titan__max_WalletLimit");
            }
            if(coolDownEnabled){
    ...
    function swapAndLiquify(uint256 contractBalance, Taxes memory temp) private lockTheSwap{
        ...
        if(marketingAmt > 0){
            payable(Titan__Marketing_Wallet).sendValue(marketingAmt);
        }
        uint256 teamAmt = unitBalance * 2 * temp.team;
        if(teamAmt > 0){
            payable(Titan__Team_Wallet).sendValue(teamAmt);
        }
    }
    ...
    function Titan__Update_MarketingWallet(address newWallet) external onlyOwner{
        Titan__Marketing_Wallet = newWallet;
    }
    
    function Titan__Update_TeamWallet(address newWallet) external onlyOwner{
        Titan__Team_Wallet = newWallet;
    }
    ...
    function updateMaxTxLimit(uint256 maxBuy, uint256 maxSell) external onlyOwner{
        Titan__max_BuyLimit = maxBuy * 10**decimals();
        Titan__max_SellLimit = maxSell * 10**decimals();
    }
    
    function Titan__Max_Walletlimit(uint256 amount) external onlyOwner{
        Titan__max_WalletLimit = amount * 10**decimals();
    }
