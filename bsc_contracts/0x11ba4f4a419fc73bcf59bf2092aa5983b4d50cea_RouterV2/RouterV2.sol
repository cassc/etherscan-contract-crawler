...
// File: contracts/libraries/MainLibrary.sol
...
library MainLibrary {
    ...
    function sortTokens(address tokenA, address tokenB) internal pure returns (address token0, address token1) {
        require(tokenA != tokenB, 'MainLibrary: IDENTICAL_ADDRESSES');
        (token0, token1) = tokenA < tokenB ? (tokenA, tokenB) : (tokenB, tokenA);
        require(token0 != address(0), 'MainLibrary: ZERO_ADDRESS');
    }
    ...
    function quote(uint amountA, uint reserveA, uint reserveB) internal pure returns (uint amountB) {
        require(amountA > 0, 'MainLibrary: INSUFFICIENT_AMOUNT');
        require(reserveA > 0 && reserveB > 0, 'MainLibrary: INSUFFICIENT_LIQUIDITY');
...
interface IUniswapV2Router01 {
    ...
    function getAmountOut(address _factory, uint amountIn, uint reserveIn, uint reserveOut) external returns (uint amountOut);
    function getAmountIn(address _factory, uint amountOut, uint reserveIn, uint reserveOut) external returns (uint amountIn);
