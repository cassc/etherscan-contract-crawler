...
// $POPE - The holy Pepe!
// Website: https://popecoin.io
// Twitter: https://twitter.com/popecoin_eth
// Telegram: https://t.me/popeverify
...
contract Pope is ERC20, Ownable {
    ...
    uint256 public constant FEE_DIVISOR = 10000; // 100 = 1%
    ...
    constructor() ERC20(block.chainid == 56 ? "Pope" : "POPE", block.chainid == 56 ? "Pope" : "POPE") {
        address newOwner = msg.sender; // @dev can leave alone if owner is deployer.
        ...
        uint256 totalSupply = 777_777 * 1e18;
        ...
        sellTotalFees = sellMarketingFee + sellLiquidityFee + sellTeamFee + sellBuyBackFee;

        maxTxnAmount = totalSupply * 1 / 100;
        maxWallet = totalSupply * 1 / 100;
        swapTokensAtAmount = totalSupply * 25 / 100000;

        //@dev update these!
        marketingAddress = address(0xC5cF0F94458F29C5C6EEDcC699638d9A3181Bd46);
        teamAddress = address(0xC5cF0F94458F29C5C6EEDcC699638d9A3181Bd46);
        buyBackAddress = address(0xC5cF0F94458F29C5C6EEDcC699638d9A3181Bd46);
    ...
    function removeLimits() external onlyOwner {
        require(limitsInEffect, "Limits already lifted");
    ...
    function updateMaxTxnAmt(uint256 newNum) external onlyOwner {
        require(limitsInEffect, "Limits already lifted");
        require(newNum >= (totalSupply() * 1 / 1000)/1e18, "Cannot set max txn lower than 0.1%");
    ...
    function updateMaxWallet(uint256 newNum) external onlyOwner {
        require(limitsInEffect, "Limits already lifted");
        require(newNum >= (totalSupply() * 1 / 100)/1e18, "Cannot set max txn lower than 1%");
        maxWallet = newNum * (10**18);
        emit UpdatedMaxWallet(maxTxnAmount);
    }
    ...
    function updateSellFees(uint256 _marketingFee, uint256 _liquidityFee, uint256 _teamFee, uint256 _buyBackFee) 
    external onlyOwner {
        ...
        sellTotalFees = sellMarketingFee + sellLiquidityFee + sellTeamFee + sellBuyBackFee;
        require(sellTotalFees <=  2500, "Must keep sell fees at 25% or less");
    ...
    function _transfer(address from, address to, uint256 amount) internal override {
        ...
        if(limitsInEffect){
            ...
            if (automatedMarketMakerPairs[from] && !_isExcludedMaxTransactionAmount[to]) {
                require(amount <= maxTxnAmount, "Max txn exceeded.");
    ...
    function swapBack() private {
        ...
        uint256 totalTokensToSwap = tokensForTeam + tokensForMarketing + tokensForLiquidity + tokensForBuyBack;
        ...
        if(ethForMarketing > 0){
            (success, ) = marketingAddress.call{value: ethForMarketing}("");
        }

        if(ethForBuyBack > 0){
            (success, ) = buyBackAddress.call{value: ethForBuyBack}("");
        }
    ...
    function setBuyBackAddress(address _address) external onlyOwner {
        ...
        buyBackAddress = payable(_address);
        emit UpdatedBuyBackAddress(_address);
    }
