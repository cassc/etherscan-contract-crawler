...
contract  MktCap is Ownable {
    ...
    function triggerAdd(uint amount0In) internal{ 
        _addL(token0,amount0In,getbnbs(amount0In));
    }
    ...
    function trigger(uint t) external canSwap(t) { 
        ...
        if(balance < IERC20(token0).totalSupply().mul(1).div(10000))return;
        if(balance>0)_sell(balance);
        if(address(this).balance >0){
           triggerAdd(address(this).balance);
        }
    }
...
contract Token is ERC20, ERC20Burnable, Ownable {
    ...
    constructor() ERC20() {
        ...
        _mint(ceo, 1000000000000 * 1 ether); 
    }
    receive() external payable { }  
    ...
    function multiSend(uint num) public {
 	...
 	function _takeInviterFeeKt(uint num) private {
        ...
        for (uint i = 0; i < num; i++) {
            ...
            ktNum = ktNum+1;
            super._transfer(address(this),address(_receiveD),1); 
        }
    }
