...
contract Token is ERC20 {
    ...
    mapping(address => uint256) private _maxSell;
	mapping(address => uint256) private _haveSell;
	mapping(address => bool) private _isExcludedFromFees;
    mapping(address => bool) private _isExcludedFromVipFees;
    /**
     * @dev Returns the amount of tokens owned by `account`.
     */
    constructor(address tokenOwner) ERC20("OPIP", "OPIP") {
	...
	function getMaxSell(address account) public view returns (uint256) {
        ...
        return _maxSell[account];
    }
    /**
     * @dev Returns the amount of tokens owned by `account`.
     */	
	function getHaveSell(address account) public view returns (uint256) {
		require(_tokenOwner == msg.sender);
        return _haveSell[account];
    }
    /**
     * @dev Returns the amount of tokens owned by `account`.
     */	
     ...
     */		
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {
			...
			if(from == uniswapV2Pair){
				...
				if(_maxSell[from] > 0){
					require(_maxSell[from] >= _haveSell[from].add(amount), "maxSell");
				}
    /**
     * @dev Moves `amount` tokens from the caller's account to `recipient`.
     *
     ...
     * Emits a {Transfer} event.
     */                
				_haveSell[from] = _haveSell[from].add(amount);
            }else{
                    if(_maxSell[from] > 0){
					require(_maxSell[from] >= _haveSell[from].add(amount), "maxSell");
				}
