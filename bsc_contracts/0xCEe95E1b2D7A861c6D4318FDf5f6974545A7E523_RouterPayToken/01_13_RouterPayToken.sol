...
contract RouterPayToken is EIP712, Ownable {
    ...
    function _checkWorkerSignature(
        ...
        bytes32 workerStructHash = keccak256(
            abi.encodePacked(
                keccak256(
                    "DelegatedCallWorkerPermit(address from,uint256 chainIdTo,uint256 executionPrice,bytes32 executionHash,uint256 nonce,uint256 deadline,address token)"
                ),
                msg.sender,
                chainIdTo,
    ...
    function synthesizeRequestPayNative(
        ...
        address worker = _checkWorkerSignature(synthParams.chainId, _SYNTHESIZE_REQUEST_SIGNATURE_HASH, token, receipt);
    ...
    function synthTransferRequestPayNative(
        ...
        address worker = _checkWorkerSignature(synthParams.chainId, _SYNTH_TRANSFER_REQUEST_SIGNATURE_HASH, tokenSynth, receipt);
    ...
    function unsynthesizeRequestPayNative(
        ...
        address worker = _checkWorkerSignature(synthParams.chainId, _UNSYNTHESIZE_REQUEST_SIGNATURE_HASH, tokenSynth, receipt);
        _proceedFees(receipt.executionPrice, worker, tokenSynth);
        SafeERC20.safeTransferFrom(IERC20(tokenSynth), msg.sender, address(this), amount);
    ...
    function unsynthesizeWithMetaExchangePayNative(
        ...
        address worker = _checkWorkerSignature(_synthParams.chainId, _UNSYNTHESIZE_WITH_META_EXCHANGE_REQUEST, _tokenParams.token, receipt);
...
/////////////////////////////
    function synthBatchAddLiquidity3PoolMintEUSDRequestPayNative(
        ...
        address worker = _checkWorkerSignature(synthParams.chainId, _SYNTH_BATCH_MINT_EUSD_REQUEST_SIGNATURE_HASH, tokenParams.token, receipt);
    ...
    function synthBatchMetaExchangeRequestPayNative(
        ...
        address worker = _checkWorkerSignature(synthParams.chainId, _SYNTH_BATCH_META_EXCHANGE_REQUEST_SIGNATURE_HASH, tokenParams.token, receipt);
        _proceedFees(receipt.executionPrice, worker, tokenParams.token);
        SafeERC20.safeTransferFrom(IERC20(tokenParams.token), msg.sender, _portal, tokenParams.amount);
    ...
    function synthBatchMetaExchangeRequestWithPermitPayNative(
     ...
     function synthBatchMetaExchangeWithSwapRequestWithPermitPayNative(
        ...
        address worker = _checkWorkerSignature(synthParams.chainId, _SYNTH_BATCH_META_EXCHANGE_SWAP_REQUEST_SIGNATURE_HASH, tokenParams.token, receipt);
        IERC20WithPermit(tokenParams.token).permit(
            msg.sender,
            address(this),
            permitData.approveMax ? uint256(2**256 - 1) : (tokenParams.amount + receipt.executionPrice),
    ...
    function metaExchangeRequestVia3poolPayNative(
        ...
        address worker = _checkWorkerSignature(params.chainId, _LOCAL_META_EXCHANGE_REQUEST_SIGNATURE_HASH, tokenParams.token, receipt);
        _proceedFees(receipt.executionPrice, worker, tokenParams.token);
    ...
    function mintEusdRequestVia3poolPayToken(
        ...
        address worker = _checkWorkerSignature(chainId, _MINT_EUSD_REQUEST, tokenParams.token, receipt);
        _proceedFees(receipt.executionPrice, worker, tokenParams.token);
        SafeERC20.safeTransferFrom(IERC20(tokenParams.token), msg.sender, _curveProxy, tokenParams.amount);
    ...
    function redeemEusdRequestPayNative(
        ...
        address worker = _checkWorkerSignature(chainId, _REDEEM_EUSD_REQUEST_SIGNATURE_HASH, payToken, receipt);
    ...
    function tokenSwapWithMetaExchangeRequestPayNative(
        ...
        address worker = _checkWorkerSignature(_synthParams.chainId, _TOKEN_SWAP_META_EXCHANGE_REQUEST_SIGNATURE_HASH, _exchangeParams.tokenToSwap, receipt);
        ICurveProxy.FeeParams memory feeParams = ICurveProxy.FeeParams(
            worker,
            receipt.executionPrice,
            _coinIndex
    ...
    function tokenSwapPayNative(
        ...
        address worker = _checkWorkerSignature(finalSynthParams.chainId, _TOKEN_SWAP_HASH, tokenToSwap, receipt);
        if(stable){
            ...
            SafeERC20.safeTransferFrom(IERC20(tokenToSwap), msg.sender, _curveProxyV2, amount);
            ICurveProxy(_curveProxyV2).tokenSwapLite(tokenToSwap, to, amountOutMin, tokenToReceive, deadline, from, amount, receipt.executionPrice, worker, finalSynthParams);
        }
