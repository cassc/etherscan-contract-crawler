...
contract LPStakingV1 is BaseContract {
    ...
    function startPriceDrop() external onlyOwner
    {
        ...
        preDropLpBalance = _lptoken_.balanceOf(address(this));
        _lptoken_.approve(address(router), preDropLpBalance);
        (preDropUsdcBalance, preDropFurBalance) = router.removeLiquidity(
            usdcAddress,
            tokenAddress,
            preDropLpBalance,
            0,
    ...
    {
        (postDropLpBalance,,) = _buyLPwithUSDC(284581230073014000000000);
    }
     ...
     */
    function needsUpdate(address participant_) public returns (bool)
    {
        return
            postDropLpBalance > 0 &&
