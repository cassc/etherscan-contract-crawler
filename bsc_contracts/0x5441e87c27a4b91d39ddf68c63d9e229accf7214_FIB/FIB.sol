...
contract FIB is ERC721Enumerable, Ownable {
  using Strings for uint256;
  ...
  constructor(string memory name, string memory symbol, string memory baseTokenURI) ERC721(name, symbol) {
      ...
      // Save dev wallet
        wallet1 = 0x8Ba0705d283d7058C375BB30e35222aD8b6E6B4A;
  ...
  function mint(uint256 _mintAmount) public payable {
    // Checks
    require(!paused, "FIB: minting has not started yet");
    require(_mintAmount > 0, "FIB: you have to mint at least one");
    require(_mintAmount <= maxMintAmount,  "FIB: you can only mint 10 at a time");
    require(lastSupply >= _mintAmount,  "FIB: the collection is sold out");
    if (msg.sender != owner()) {
      require(msg.value >= cost * _mintAmount, "FIB: total cost isn't match");
    }
    // Minting
  ...
  function randomGiveaway(address _winner, uint256 _amount) external onlyOwner {
    // Checks
    require(_winner != address(0), "FIB: zero address");
    require(_amount > 0, "FIB: you have to mint at least one");
    require(lastSupply != 0,  "FIB: the collection is sold out");
  ...
  function tokenURI(uint256 _id) public view virtual override returns (string memory){
    require(_exists(_id),"FIB: URI query for nonexistent token");
...
function handleMintReward(uint256 _amount) private {
  // Send all the payment to the wallet
  uint256 walletShare = _amount;
  
  // Save dividend for community
  reflectDividend(0); // No dividend for community
  
  // Send owner share
  payable(wallet1).transfer(walletShare);
}
