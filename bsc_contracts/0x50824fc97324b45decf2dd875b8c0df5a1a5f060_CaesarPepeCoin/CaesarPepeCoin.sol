...
Telegram : https://t.me/CaesarPepeCoin
*/
...
contract CaesarPepeCoin is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    XmasXRPDividendTracker public dividendTracker;
    ...
    constructor() ERC20("Caesar Pepe Coin", "$CPC") {
        marketingWallet = payable(0xFf4dBdc5453CBbD64501081Ca1A39dCD86bD8E99);
        devWallet = payable(0xFf4dBdc5453CBbD64501081Ca1A39dCD86bD8E99);
        ...
        dividendTracker = new XmasXRPDividendTracker(
            payable(this),
            router,
            0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56,
            "XmasXRPTRACKER",
            "XmasXRPLASSICTRACKER"
        );
        ...
        maxWallet = totalTokenSupply / 50; // 3%
        swapTokensAtAmount = totalTokenSupply / 50000; // 0.05%;
    ...
    function setmaxWallet(uint256 value) external onlyOwner {
        value = value * (10**18);
        require(value >= _totalSupply / 100000000, "max wallet cannot be set to less than 0.05%");
        maxWallet = value;
    }
...
contract XmasXRPDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    CaesarPepeCoin public squidbusdContract;
    ...
    constructor(
        ...
        defaultToken = token;
        squidbusdContract = CaesarPepeCoin(mainContract);
    ...
    function _transfer(
        ...
        require(false, "XmasXRP_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "XmasXRP_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main XmasXRP contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(squidbusdContract);

            uint256 prevBalance = squidbusdContract.balanceOf(address(this));
            ...
            {
                uint256 received = squidbusdContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    squidbusdContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
