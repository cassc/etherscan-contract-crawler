...
contract BabyPoolV2 is SafeOwnable {
    ...
    function fetch(address _to, uint _amount) internal returns(uint) {
        if (fetchVaultType == FETCH_VAULT_TYPE.FROM_ALL) {
            return vault.mint(msg.sender, _amount);
    ...
    function poolInfo(uint _pid) external view returns (IERC20 token, uint256 allocPoint, uint256 lastRewardBlock, uint256 accRewardPerShare) {
