...
contract ZUSDTrade is Ownable {
    ...
    uint256 public buyPrice = 120*(10**6);
    uint256 public sellPrice = 108*(10**6);
    bool public enableLP = false;
    uint256 public priceAdj = 10000000000;
    ...
    function buyAmountInMain(uint256 _amount) public view returns (uint){
        uint256 curPrice = buyPrice;
        if(enableLP){
            curPrice = getPriceToken();
        }
    ...
    function sellAmount(uint256 _amount) public view returns (uint){
        ...
        return _amount*curPrice*sellAdj;
    }

    function sellAmountInUSDT(uint256 _amount) public view returns (uint){
        uint256 curPrice = sellPrice;
        if(enableLP){
            curPrice = getPriceToken();
        }
