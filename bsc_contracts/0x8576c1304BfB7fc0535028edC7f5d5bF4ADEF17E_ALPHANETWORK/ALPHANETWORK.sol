...
contract ALPHANETWORK is Context, IBEP20, Auth {
    ...
    string constant _name = "ALPHANETWORK";
    string constant _symbol = "ALPHA";
    uint8 constant _decimals = 9;

    uint256 _totalSupply = 1 * 10**8 * 10**_decimals;

	uint256 public _maxWalletAmount = 1 * 10**8 * 10**_decimals;
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if (!isFeeExempt[sender] && !isFeeExempt[recipient] && recipient != address(this)  && recipient != address(DEAD) && recipient != pair){
	...
	function burnOut(uint256 amount) external onlyOwner {
