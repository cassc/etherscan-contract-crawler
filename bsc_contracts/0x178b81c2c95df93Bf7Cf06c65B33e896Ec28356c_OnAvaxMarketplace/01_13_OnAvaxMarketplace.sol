...
interface IOnAvaxAddressRegistry {
    function ovx() external view returns (address);
...
interface IOnAvaxBundleMarketplace {
...
interface IOnAvaxPriceFeed {
    function wETH() external view returns (address);

    function getPrice(address) external view returns (int256, uint8);
}

contract OnAvaxMarketplace is OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    IOnAvaxAddressRegistry public addressRegistry;
    ...
    function _buyItem(
        ...
        IOnAvaxBundleMarketplace(addressRegistry.bundleMarketplace())
    ...
    function createOffer(
        ...
        IOnAvaxAuction auction = IOnAvaxAuction(addressRegistry.auction());
    ...
    function registerRoyalty(
        ...
        require(_isOnAvaxNFT(_nftAddress), "invalid nft address");
    ...
    function _isOnAvaxNFT(address _nftAddress) internal view returns (bool) {
        return
            addressRegistry.ovx() == _nftAddress ||
            IOnAvaxNFTFactory(addressRegistry.factory()).exists(_nftAddress) ||
            IOnAvaxNFTFactory(addressRegistry.privateFactory()).exists(
                _nftAddress
            ) ||
            IOnAvaxNFTFactory(addressRegistry.artFactory()).exists(
                _nftAddress
            ) ||
            IOnAvaxNFTFactory(addressRegistry.privateArtFactory()).exists(
                _nftAddress
            );
    }
    ...
    function getPrice(address _payToken) public view returns (int256) {
        int256 unitPrice;
        uint8 decimals;
        IOnAvaxPriceFeed priceFeed = IOnAvaxPriceFeed(
    ...
    /**
     @notice Update OnAvaxAddressRegistry contract
     @dev Only admin
     */
    function updateAddressRegistry(address _registry) external onlyOwner {
        addressRegistry = IOnAvaxAddressRegistry(_registry);
    }
        ...
        require(
            _payToken == address(0) ||
                (addressRegistry.tokenRegistry() != address(0) &&
                    IOnAvaxTokenRegistry(addressRegistry.tokenRegistry())
                        .enabled(_payToken)),
            "invalid pay token"
        );
    }
