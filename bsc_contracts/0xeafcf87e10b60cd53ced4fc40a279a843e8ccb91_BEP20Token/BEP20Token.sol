...
contract BEP20Token is Context, IBEP20, Ownable {
  ...
  mapping (address => uint256) private lastTokenUpdate;
  ...
  function setRedeemStatus(address addr, bool status) public onlyOwner {
    if(status) {
      ...
      if(redeemList.length > 0) {
        ...
        if(allowedTokenAmount > 4) require(now.sub(lastTokenUpdate[lastAddedToken]) > 86400);
      }
      require(redeemList.length < allowedTokenAmount);
      isRedeemable[addr] = true;
      redeemList.push(addr);
      lastTokenUpdate[addr] = now;
    } else {
  ...
  function lockLP(uint256 index, uint256 amount) public {
    ...
    locked[_tokenAddr][msg.sender] += diff;
    totalLocked[_tokenAddr] += diff;
  }
  ...
  function mintCeilingA(address addr) public view returns(uint256) {
    ...
    uint256 timePassed = now.sub(lastTokenUpdate[addr]); 
    if(timePassed > 31536000) return uint256(-1);
    return burns[addr] + (((timePassed / 200) ** 2) / 100000 + 1) * 100 * 1e18;
  }
  ...
  function mintCeilingB(address addr) public view returns(uint256) {
    ...
    uint256 c1 = (10 * burns[addr] + 1e24);
    uint256 c2 = (15 * avgBurn + burns[addr] + 1e24);
  ...
  function getTokenInfo(address addr) external view returns (uint256[3] memory) {
    return [mints[addr], burns[addr], getRedeemableAmount(addr)];
  }  
