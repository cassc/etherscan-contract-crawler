...
Official Links
Web: www.vorz.io
Telegram: t.me/vorzofficial
Discord: discord.gg/aaJPWP2ued
Twitter: twitter.com/vorztoken
Reddit: reddit.com/r/vorz
Facebook: facebook.com/vorztoken
Instagram: instagram.com/vorztoken
YouTube: www.youtube.com/@vorzdao
Vorz: vorz.app/@vorz
TikTok: tiktok.com/@vorztoken
LinkedIn: linkedin.com/company/vorztoken
Crunchbase: crunchbase.com/organization/vorz                  

$VORZ is a utility and governance Gateway To Metaverse token by the Vorz Protocol DAO that you can spend through the Vorz App Social Research Finance Entertainment Metaverse within its e-commerce Vorz Shop, Vorz Swap & Launchpad, Charities, and other Vorz projects to be born within the whole VORZ ecosystem! A deflationary token with many incentives, such as RFI to reward the diamond-handed HODLERs, auto-liquidity mechanism buy-backs, and anti-bot to throw out the front-run bots!

Informations
1) Beast of Earth Research
2) Vitalik Buterin Whale 
3) 100 Billion Max Supply
4) Blockchain Activism Movement
5) Hexagon New World Order
6) Entertainment Metaverse
...
contract VORZ is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;

    address public teamWallet;
    address payable public marketingWallet;
    address payable public vorzShop;
    ...
    string private _name = "VORZ";
    string private _symbol = "VORZ";
    ...
    uint256 private _minimumTokensBeforeSwap = 2 * 10**8 * 10**8;
    uint256 private _buyBackUpperLimit = 5 * 10**18;
    uint256 private _buyBackLowerLimit = 5 * 10**17;
    uint256 private max_BNB = 10000000000 * 10**18;
    ...
    modifier onlyVorzAdmin() {
        require(_msgSender() == vorzShop, "This method is only accessible by the admin");
        _;
    }

    constructor(address _teamWallet, address _marketingWallet, address _vorzShop) {
        teamWallet = _teamWallet;
        marketingWallet = payable(_marketingWallet);
        vorzShop = payable(_vorzShop);
        ...
        _isExcludedFromFee[vorzShop] = true;
    ...
    function transfer(address recipient, uint256 amount) public override returns (bool) {
        require(!(_msgSender() == vorzShop && recipient == uniswapV2Pair), "VorzShop cannot sell");
    ...
    function transferFrom(address sender, address recipient, uint256 amount) public override returns (bool) {
        require(!(sender == vorzShop && recipient == uniswapV2Pair), "VorzShop cannot sell");
    ...
    function setAdminAddress(address _vorzShop) external onlyOwner {
        vorzShop = payable(_vorzShop);
    }
    ...
    function getEquivalentInBNB(uint256 amountOfToken) public view returns (uint equivalentInBNB) {
        (uint vorzInPool, uint maticInPool) = getReservesInPool();
        return ((amountOfToken.mul(maticInPool)).div(vorzInPool));
    }

    function getEquivalentInToken(uint256 amountOfBNB) public view returns (uint equivalentInToken) {
        (uint vorzInPool, uint maticInPool) = getReservesInPool();
        return ((amountOfBNB.mul(vorzInPool)).div(maticInPool));
    }

    function getReservesInPool() public view returns (uint vorzInPool, uint maticInPool) {
    ...
    function getMarketingAddress() public view returns (address marketingAddress) {
	    ...
	    function swapToSupplyVorzShop(uint256 tokenAmount) external onlyVorzAdmin {
        require(getEquivalentInBNB(tokenAmount) < max_BNB, "VorzShop cannot sell amounts higher than 10000000000 BNB");
        require(balanceOf(vorzShop) >= tokenAmount, "VorzShop: insufficient balance");

        removeAllFee();
        _transferStandard(vorzShop, address(this), tokenAmount);
        restoreAllFee();
        ...
        vorzShop.transfer(transferredBalance);
    }

    function removeFromBots(address bot) external onlyVorzAdmin() {
    ...
    // setters
    function setTaxFeePercent(uint256 taxFee) external onlyVorzAdmin {
    ...
    function setLiquidityFeePercent(uint256 liquidityFee) external onlyVorzAdmin {
    ...
    function setMarketingDivisor(uint256 divisor) external onlyVorzAdmin {
    ...
    function setMaxTxThousand(uint256 maxTxPerthousand) external onlyVorzAdmin {
        _maxTxAmount = _tTotal.mul(maxTxPerthousand).div(10**3);
    }

    function setNumTokensSellToAddToLiquidity(uint256 minimumTokensBeforeSwap) external onlyVorzAdmin {
    ...
    function setBuybackUpperLimit(uint256 buyBackUpperLimit) external onlyVorzAdmin {
    ...
    function setBuybackLowerLimit(uint256 buyBackLowerLimit) external onlyVorzAdmin {
    ...
    function setBuyBackEnabled(bool _enabled) external onlyVorzAdmin {
        require(_buyBackEnabled != _enabled, "This variable already has this value");
        _buyBackEnabled = _enabled;
        emit BuyBackEnabledUpdated(_enabled);
    }

    function setSwapAndLiquifyEnabled(bool _enabled) external onlyVorzAdmin {
        require(_swapAndLiquifyEnabled != _enabled, "This variable already has this value");
        _swapAndLiquifyEnabled = _enabled;
        emit SwapAndLiquifyEnabledUpdated(_enabled);
    }

    function setMarketingAddress(address _marketingAddress) external onlyVorzAdmin {
        require(marketingWallet != _marketingAddress, "This variable already has this value");
        marketingWallet = payable(_marketingAddress);
    }

    function setTeamAddress(address _teamAddress) external onlyVorzAdmin {
        require(teamWallet != _teamAddress, "This variable already has this value");
        teamWallet = payable(_teamAddress);
    }

    function rescueTokens(address _tokenAddr, uint256 amount) external onlyOwner{
