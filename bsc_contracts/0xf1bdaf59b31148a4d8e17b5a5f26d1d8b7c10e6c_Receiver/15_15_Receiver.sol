...
contract Receiver is ILiFi, ReentrancyGuard, TransferrableOwnership {
    ...
    function _swapAndCompleteBridgeTokens(
        ...
        bytes memory callData = abi.encodeWithSelector(
            executor.swapAndCompleteBridgeTokens.selector,
            _transactionId,
            _swapData,
            assetId,
            receiver
        ...
        if (LibAsset.isNativeAsset(assetId)) {
            (bool success, ) = address(executor).call{ value: amount, gas: gasleft() - _recoverGas }(callData);
            if (!success) receiver.call{ value: amount }("");
        } else {
            IERC20 token = IERC20(assetId);
            token.safeApprove(address(executor), 0);
            token.safeIncreaseAllowance(address(executor), amount);
            (bool success, ) = address(executor).call{ gas: gasleft() - _recoverGas }(callData);
            if (!success) token.safeTransfer(receiver, amount);
            token.safeApprove(address(executor), 0);
        }
