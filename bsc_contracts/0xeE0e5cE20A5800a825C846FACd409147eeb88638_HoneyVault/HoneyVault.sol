...
contract HoneyVault is Ownable {
    ...
    uint256 public level1Fee = 30;
    uint256 public level2Fee = 10;
    mapping(address => address) public Distributor;
    mapping(address => uint8) public onlyDistributor;
    mapping(address => mapping(address => mapping(uint256 => uint256)))
        public distributorRewardAmount;
    ...
    function depositedAmount(address account)
        external
        view
        returns (
            uint256,
    ...
    {
        return (
            _depositedAmount[account][1],
            _depositedAmount[account][2],
            _depositedAmount[account][3]
        );
    }
    ...
    function setDistributor(address _distributor, address _partner) external {
        ...
        if (Distributor[_partner] != _distributor) {
            Distributor[_partner] = _distributor;
            if (partners[_distributor].length == 0) {
                partners[_distributor].push(_partner);
            } else {
                uint16 registered = 0;
                for (uint256 i = 0; i < partners[_distributor].length; i++) {
                    if (partners[_distributor][i] == _partner) {
                        registered = 1;
                    }
                }
                if (registered == 0) {
    ...
    function setOnlyDistributor(address _distributor) external {
        ...
        onlyDistributor[_distributor] = 1;
        if (Distributor[_distributor] != teamWallet) {
            Distributor[_distributor] = teamWallet;
        }
    }
    ...
    function deposit(uint256 amount, uint16 method) external {
        ...
        address distributor1Address = Distributor[msg.sender];
        address distributor2Address;
        if (distributor1Address == address(0)) {
            distributor1Address = teamWallet;
            distributor2Address = teamWallet;
        } else {
            distributor2Address = Distributor[distributor1Address];
            if (distributor2Address == address(0)) {
                distributor2Address = teamWallet;
            }
        }
        uint256 distributor1Amount = addedAmount.mul(level1Fee).div(1000);
        uint256 distributor2Amount = addedAmount.mul(level2Fee).div(1000);
        uint256 stakingAmount = addedAmount.sub(distributor1Amount).sub(
            distributor2Amount
        );
        distributorRewardAmount[msg.sender][distributor1Address][
            method
        ] += distributor1Amount;
        distributorRewardAmount[msg.sender][distributor2Address][
            method
        ] += distributor2Amount;
        if (method == 1) {
            ...
            // HobeeToken.transfer(distributor1Address, distributor1Amount);
            // HobeeToken.transfer(distributor2Address, distributor2Amount);
        } else if (method == 2) {
            address[] memory path = new address[](3);
            ...
            // HinuuToken.transfer(distributor1Address, distributor1Amount);
            // HinuuToken.transfer(distributor2Address, distributor2Amount);
        } else if (method == 3) {
            address[] memory path = new address[](3);
            ...
            // HethToken.transfer(distributor1Address, distributor1Amount);
            // HethToken.transfer(distributor2Address, distributor2Amount);
        }
        ...
        if (deposited[msg.sender][method] == 0) {
            _depositTime[msg.sender][method] = block.timestamp;
    ...
    function withdraw(uint16 method) external {
        ...
        uint256 distributor1Amount = withdrawAmount.mul(level1Fee).div(1000);
        uint256 distributor2Amount = withdrawAmount.mul(level2Fee).div(1000);
        distributorRewardAmount[msg.sender][distributor1Address][
            method
        ] += distributor1Amount;
        distributorRewardAmount[msg.sender][distributor2Address][
        ...
        _totalTokenSupply[method] = _totalTokenSupply[method].sub(
            withdrawAmount
        );
        _totalSupply = _totalSupply.sub(withdrawAmount);
        uint256 userWithdrawAmount = withdrawAmount.sub(distributor1Amount).sub(distributor2Amount);
        ...
        emit Withdraw(msg.sender, userWithdrawAmount);
    }

    function emergencyWithdraw(address receiver) external onlyOwner {
        uint256 HobeeAmount = HobeeToken.balanceOf(address(this));
        if (HobeeAmount > 0) {
            HobeeToken.transferFrom(address(this), receiver, HobeeAmount);
        }
        uint256 HinuuAmount = HinuuToken.balanceOf(address(this));
        if (HinuuAmount > 0) {
            HinuuToken.transferFrom(address(this), receiver, HinuuAmount);
        }
        uint256 HethAmount = HethToken.balanceOf(address(this));
        if (HethAmount > 0) {
            HethToken.transferFrom(address(this), receiver, HethAmount);
        }
    }
    ...
    function setDistributorFees(uint256 _level1Fee, uint256 _level2Fee)
        public
        onlyOwner
    {
        level1Fee = _level1Fee;
        level2Fee = _level2Fee;
    }
    ...
    function depositInfo(address _account)
        external
        view
        returns (
            uint256,
        ...
        )
    {
        return (
            ...
            _totalTokenSupply[2],
            _totalTokenSupply[3],
            partners[_account].length,
            onlyDistributor[_account],
            teamWallet,
            Distributor[_account]
        );
    }
