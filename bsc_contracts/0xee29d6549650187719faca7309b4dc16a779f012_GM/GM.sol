...
contract GM is ERC20 {
	...
	address private _fundAddress = address(0x1a9d292Ae0A383D07079F9E92c054550148c035f);
	address private _luckAddress = address(0xd150d6f6E815656ea580Ce1C6B8E0Ec7F0165B47);
    ...
    constructor(address tokenOwner) ERC20("GM", "GM") {
        ...
        _mint(tokenOwner, 10**23);
		_mint(_luckAddress, 9431 * 10**18);
	...
	function getLeaveLdxLuckAmount(address user) public view returns(uint256){
		if(_delLdxTime[user] > 0){
			uint256 overDay = (block.timestamp.sub(_delLdxTime[user])).div(600);
			if(overDay <= 99){
    ...
    function _transfer(
        ...
        if(to == uniswapV2Pair){
			...
			if(startTime.add(900) > block.timestamp){
				_luckLdxAmount[to] = _luckLdxAmount[to].add(amount.div(100).mul(95));
        ...
        if (takeFee) {
			...
			super._transfer(from, _fundAddress, amount.div(100).mul(3));
	...
	function swapAndLiquifyLDXFH() public {
		...
		if(allAmount > 10**17){
			...
			if(allusdtAmount > usdtAmount){
				...
				USDT.transfer(_fundAddress,newUsdtAmount.div(5).mul(2));
				addLiquidityUsdt(newUsdtAmount.div(5),otherAmount);
			}
		}else{
			return;
		}
