...
contract ApexV3 is BEP20, Auth {
    ...
    uint256 public _maxWalletToken = _totalSupply; // 2%
    ...
    function setMaxTxPercent_base10000(uint256 maxTXPercentage_base10000) external onlyOwner {
        require(maxTXPercentage_base10000 >= 0,"Cannot set max transaction less than 0.1%");
...
*/
    ...
    function update_fees() internal {
        require(totalFee.mul(buyMultiplier).div(100) <= 1000, "Buy tax cannot be more than 15%");
        require(totalFee.mul(sellMultiplier).div(100) <= 1000, "Sell tax cannot be more than 15%");
        require(totalFee.mul(sellMultiplier + buyMultiplier).div(100) <= 2000, "Buy+Sell tax cannot be more than 20%");
