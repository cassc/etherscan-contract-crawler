...
contract LevelXToken is Initializable, Ownable, ERC20
{
	...
	bool public __autoClaimForPair; // whether or not LVLX rewards are auto claimed for the liquidity pool
	...
	function initialize(address _owner, string memory _name, string memory _symbol, address _router) public initializer
	{
		...
		autoClaim[pair] = true;
		autoClaimBurnFactor[pair] = 50e16; // 50%
	...
	function updateAutoClaim(address[] memory _accounts, bool _autoClaim, uint256 _burnFactor) external onlyOwner
	{
		...
		for (uint256 _i = 0; _i < _accounts.length; _i++) {
			...
			emit UpdateAutoClaim(_account, _autoClaim, _burnFactor);
		}
	}
	...
	function claimOnBehalfOf(address _account, uint256 _i, address _receiver) external onlyOwner returns (uint256 _amount)
	{
		...
		_amount = _claim(_account, _i, _receiver, 0e18);
		_postUpdateAccount(_account, 0e18);
	...
	function claim(uint256 _i) external returns (uint256 _amount)
	{
		...
		_updateEpoch();
		_updateAccount(msg.sender);
		_amount = _claim(msg.sender, _i, msg.sender, 0e18);
		_postUpdateAccount(msg.sender, 0e18);
		return _amount;
	}

	function _claim(address _account, uint256 _i, address _receiver, uint256 _burnFactor) internal returns (uint256 _amount)
	{
		AccountInfo storage _accountInfo = accountInfo[_account];
		...
		if (_amount > 0) {
			...
			if (_i == 0) {
				...
				if (_burnFactor > 0) {
					...
					_amount -= _burnAmount;
					_transfer(INTERNAL_ADDRESS, FURNACE, _burnAmount);
				}
				...
				if (_burnFactor > 0) {
					uint256 _burnAmount = _amount * _burnFactor / 100e16;
					_amount -= _burnAmount;
					IERC20(_rewardToken).safeTransfer(FURNACE, _burnAmount);
				}
	...
	function _updateAccount(address _account) internal
	{
		...
		if (_newBalance > 0) {
			...
			if (autoClaim[_account]) {
				_claim(_account, 0, _account, autoClaimBurnFactor[_account]);
			}
			if (_account == pair) {
	...
	event UpdateAutoClaim(address indexed _account, bool _autoClaim, uint256 _burnFactor);
