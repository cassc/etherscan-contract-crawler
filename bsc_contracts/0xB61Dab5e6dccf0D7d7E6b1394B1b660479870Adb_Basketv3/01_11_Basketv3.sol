...
contract Basketv3 is Ownable {
  ...
  function depositFromBridge(address _account, PoolInfo[] calldata _pools, uint256 _itokenAmount, uint256 _fee, uint8 _bridge) public {
  ...
  function withdraw(address _account, PoolInfo[] calldata _pools, StragateSwapInfo calldata _stgSwap, BridgeSwapInfo calldata _lcbrgSwap, uint256 _fee) public {
    uint256 poolLen = _pools.length;
    if (poolLen > 0) {
      for (uint256 i=0; i<poolLen; i++) {
