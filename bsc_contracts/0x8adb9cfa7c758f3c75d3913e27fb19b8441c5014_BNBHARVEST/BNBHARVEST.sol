...
contract BNBHARVEST {
    ...
    uint256 constant public DAILY_LIMIT = 0.5 ether; // only 0.5 BNB per day anti whale
	uint256 constant public MAX_INVESTS = 300; //300 DEPO TRANSACTIONS PER WALLET ADDRESS
	uint256[] public REFERRAL_PERCENTS = [10]; //1%
	uint256 constant public TOTAL_REF = 50; //5%
	uint256 constant public CEO_FEE = 40; //4%
	uint256 constant public DEV_FEE = 10; //1%
	uint256 constant public REINVEST_BONUS = 10; //1%
	uint256 constant public SEED_BONUS = 10; //1%
	...
	constructor(address payable ceoAddr, address payable devAddr, uint256 start) public {
        ...
        plans.push(Plan(1, 1050));  //1 DAY  105%
        plans.push(Plan(2, 550));   //2 DAYS 110%
	}

	function invest(address referrer, uint8 plan) public payable {
		require(block.timestamp > startDate, "contract does not launch yet");
		require(msg.value >= INVEST_MIN_AMOUNT, "min amount is 0.001 BNB");
	...
	function withdraw() public {
        ...
        uint256 seedBonus = getUserSeedBonus(msg.sender);
		if (seedBonus > 0) {
			user.seed = 0;
			totalAmount = totalAmount.add(seedBonus);
		}

        if(dLimit[cDay()][msg.sender] + totalAmount > DAILY_LIMIT){
            user.bonus = dLimit[cDay()][msg.sender].add(totalAmount).sub(DAILY_LIMIT);
		...
		if(user.referrer != address(0)){
			...
			users[upline].seed = users[upline].seed.add(seedAmount);
			users[upline].totalSeed = users[upline].totalSeed.add(seedAmount);
			totalSeed = totalSeed.add(seedAmount);
			emit SeedPaid(upline, msg.sender, seedAmount, block.timestamp);
		}
	}
