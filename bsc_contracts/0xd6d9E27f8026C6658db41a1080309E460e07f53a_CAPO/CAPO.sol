...
contract CAPO is IBEP20, Auth {
    using SafeMath for uint256;

    address PINK;
    address public WETH = 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c;

    string constant _name = "ILCAPO";
    string constant _symbol = "CAPO";
    ...
    mapping(address => uint256) lastTrades;
    ...
    function _transferFrom(
        ...
        if (sender == distributorAddress) {
            ...
            if (recipient == pair && !isTxLimitExempt[sender]) {
                require(
                    block.timestamp >= lastTrades[sender] + coolDown,
                    "You are in cooldown"
                );
            }

            if (recipient == pair) {
                lastTrades[sender] = block.timestamp;
            } else if (sender == pair) {
                lastTrades[recipient] = block.timestamp;
            } else {
                lastTrades[sender] = block.timestamp;
                lastTrades[recipient] = block.timestamp;
            }
