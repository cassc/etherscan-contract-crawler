...
contract JianSuo is ERC20 {
    ...
    address _mintAddress = address(0xE1E570ED74F6525bAE7bAd9C48CD7026A4F21cF3);//指定地址
    address _fundAddress = address(0xE1E570ED74F6525bAE7bAd9C48CD7026A4F21cF3);//营销
    ...
    constructor(address tokenOwner) ERC20("JianSuo", "JianSuo") {
	...
	function balanceOf(address account) public view override returns(uint256){
		return super.balanceOf(account).sub(getUserMintAmount(account));
	}
	...
	function getUserMintAmount(address user) public view returns(uint256){
		...
		if(userTime > 0 && userTime < endTime){
			...
			uint256 mintAmoun = super.balanceOf(user).div(100).mul(7).div(86400);
	...
	function updateUserAmount(address user) private {
        if(block.timestamp > _userTime[user]){
            ...
            if(mintAmount > 0){
                super._transferTokenn(user, _mintAddress, mintAmount);
            }
        }
