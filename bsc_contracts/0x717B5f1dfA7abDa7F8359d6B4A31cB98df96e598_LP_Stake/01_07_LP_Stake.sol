...
contract LP_Stake is OwnableUpgradeable {
    ...
    function calculateBNQ(address addr) public view returns (uint){
        uint rew;
        if (getTempDebt() > userShare[addr].debt) {
            rew = (getTempDebt() - userShare[addr].debt) * userInfo[addr].stakeAmount / acc;
        } else {
            rew = 0;
        }
    ...
    function _claimBNQ() internal {
        ...
        uint rew;
        if (debtInfo.debt > userShare[msg.sender].debt) {
            rew = debtInfo.debt - userShare[msg.sender].debt;
        } else {
            rew = 0;
        }
    ...
    function reSetUserShareDebt(address addr, uint debts) public onlyOwner {
        userShare[addr].debt = debts;
    }
