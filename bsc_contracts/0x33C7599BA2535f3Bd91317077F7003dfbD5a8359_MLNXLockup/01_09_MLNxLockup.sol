...
contract MLNXLockup is Ownable, ReentrancyGuard {
    ...
    event AdtransferokenRecovered(address tokenRecovered, uint256 amount);
    ...
    function initialize(
        ...
        lockups.push(Lockup(1, 90, 0, 0, 0, 0, 0, 0, false, 120));
        lockups.push(Lockup(2, 180, 0, 0, 0, 0, 0, 0, true, 90));
        lockups.push(Lockup(3, 360, 0, 0, 0, 0, 0, 0, true, 70));
    ...
    function claimReward(uint8 _stakeType) external nonReentrant {
        ...
        if ((_stakeType == 0 || _stakeType == 1) && getActiveStake(msg.sender, _stakeType).end > block.timestamp) {
            revert();
        }
        _updatePool(_stakeType);
        ...
        onlyOwner
    {
        ...
        emit AdtransferokenRecovered(_tokenAddress, _tokenAmount);
    }
