...
contract House is VRFConsumerBaseV2, ReentrancyGuard {
    ...
    function getAdjustedBalance() internal view returns(uint) {
    ...
    /* Play functions */
    function makeBet(uint8 _direction) external payable {
        if(msg.value <= 0 || getAdjustedBalance() - msg.value < getMinBalance() || _direction != 1 && _direction != 2) {
            revert InvalidBet();
        }
    ...
    function reBet(uint amount, uint8 _direction) external {
        if(amount <= 0 || getAdjustedBalance() - amount < getMinBalance() || _direction != 1 && _direction != 2 || winnings[msg.sender] < amount) {
            revert InvalidBet();
        }

        requestRandomWords(_direction, msg.sender, amount, true);
    }
