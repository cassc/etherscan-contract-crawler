...
contract StratX_pancakeLP is Ownable, ReentrancyGuard, Pausable {
    ...
    uint256 public controllerFee = 3100;
    ...
    constructor() public {
        ...
        usdtToToken0Path = [usdt, wbnb, token0Address];
        usdtToToken1Path = [usdt, wbnb, token1Address];

        if (token0Address == busd) {
            earnedToToken0Path = [earnedAddress,  usdt, wbnb, busd];
        } else if (token0Address != earnedAddress) {
            earnedToToken0Path = [earnedAddress,  usdt, wbnb,  busd, token0Address];
        }

        if (token1Address == busd) {
            earnedToToken1Path = [earnedAddress,  usdt, wbnb, busd];
        } else if (token1Address != earnedAddress) {
            earnedToToken1Path = [earnedAddress,  usdt, wbnb, busd, token1Address];
        }
...
//            wantLockedTotal = wantLockedTotal.add(pairBal.sub(1));
    ...
    function earn() public whenNotPaused {
        ...
        uint256 buyBackAmt = usdtAmt.mul(buyBackRate).div(buyBackRateMax);
//	IERC20(usdt).safeIncreaseAllowance(uniRouterAddress, buyBackAmt);
	_safeSwap(
            uniRouterAddress,
            buyBackAmt,
            slippageFactor,
