...
contract Rock is ERC20, Ownable {
    ...
    RockDividendTracker public dividendTracker;
    ...
    constructor() ERC20("Rock", "RCK") {
        ...
        dividendTracker = new RockDividendTracker(
            payable(this),
            router,
...
contract RockDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor(
        ...
        defaultToken = token;
        RockContract = Rock(mainContract);
    ...
    function _transfer(
        ...
        require(false, "Rock_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "Rock_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main Rock contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(RockContract);

            uint256 prevBalance = RockContract.balanceOf(address(this));
            ...
            {
                uint256 received = RockContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    RockContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
