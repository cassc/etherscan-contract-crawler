...
contract NFTFractional is Upgradable {
    ...
    */
    ...
    function removeAdmin(address[] memory listAddress) external onlyController {
        for (uint256 i = 0; i < listAddress.length; i++) {
            adminList[listAddress[i]] = 0;
        }   
    }
    ...
    */
    function purchaseFNFT(uint256[] memory _datas, string memory _purchaseId,bytes memory _signature) external {
        require(
        ...
        IERC20(pool.acceptToken).transferFrom(msg.sender, pool.receiveAddress, _datas[1]);
        IERC20(fnftInfo.tokenFNFT).transfer(msg.sender, _datas[3]);

        pool.availableBalance -= _datas[3];
        userInfo.purchased += _datas[1];
        emit PurchaseFNFT(_datas[0], userInfo.purchased, userInfo.alloction - userInfo.purchased, _datas[3], msg.sender, _purchaseId);
    }
    ...
    */
    function unStakeTierPool(uint256[] memory _datas) external {
        ...
        UserInfo storage userInfo = userInfos[_datas[0]][msg.sender];
        require(userInfo.stakeLastTime + pool.lockDuration * AQUATER_MINUTE < block.timestamp, "NFTFractional: User is in lock duration");
    ...
    */
    function withdrawDelayToken(uint256 poolId) external {
        ...
        require(block.timestamp - userInfo.unStakeLastTime >= tierPool.withdrawDelayDuration * AQUATER_MINUTE, "Pool doesnt finish withraw delay duration");
    ...
    * @param _signature signature of user
    */
    function claimReward(uint256[] memory _datas, string memory _claimId,bytes memory _signature) external {
        require(
            ISignatureUtils(signatureUtils).verify(
                _datas[0],
                _datas[1],
        ...
        uint256 balanceSender = IERC20(fnftInfo.tokenFNFT).balanceOf(msg.sender);
        ...
        IERC20(fnftInfo.tokenFNFT).transferFrom(msg.sender, address(this), _datas[1]);
        IERC20(rewardPool.rewardToken).transfer(msg.sender, amountTransfer);
        ERC20Token(fnftInfo.tokenFNFT).burnFrom(address(this), _datas[1]);
        rewardPool.totalRewardAmount -= amountTransfer;
        emit ClaimReward(_datas[0], _datas[1], balanceSender - _datas[1] , amountTransfer, msg.sender, _claimId);
    }
}