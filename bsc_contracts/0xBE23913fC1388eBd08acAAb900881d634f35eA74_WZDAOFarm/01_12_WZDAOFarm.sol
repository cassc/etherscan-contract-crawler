...
contract WZDAOFarm is Ownable, ReentrancyGuard, IFarm {
    ...
    function getStaking(uint256 _pid, address _address)
        external
        view
        ...
        nonReentrant
    {
        ...
        require(
            pool.tokenB.balanceOf(user) >= tokenBAmount &&
                tokenA.balanceOf(user) >= _amount,
            "Insufficient balance"
        );
        require(
            _stakings[_pid][user].tokenAAmount.add(_amount) >=
                minStakingTokenAAmount,
            "The staking amount must be greater than minStakingTokenAAmount"
        );

        tokenA.safeTransferFrom(user, address(this), _amount);
        _stakings[_pid][user].tokenAAmount = _stakings[_pid][user]
            .tokenAAmount
            .add(_amount);
        ...
        if (_isDefiWz(pool.tokenB)) {
            ...
            payBAmount = pool.tokenB.balanceOf(address(this)).sub(
                beforeBBalance
            );
        }
        _stakings[_pid][user].tokenBAmount = _stakings[_pid][user]
            .tokenBAmount
            .add(payBAmount);
    ...
    function updatePool(
        ...
        uint256 _rbase,
        bool _enable
    ) external onlyOwner existsPool(_pid) {
        Pool storage pool = _pools[_pid];
        pool.rate = _rate;
        pool.rbase = _rbase;
        pool.enable = _enable;
