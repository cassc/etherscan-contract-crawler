...
contract LPStakingV1 is BaseContract {
     ...
     * DEBUG
     */
    function getTotalReflection() external view returns (uint256) {
    ...
    function getLPholdersLength() external view returns (uint256) {
        return LPholders.length;
    }

    function updateLastUpdateTime(uint256 newTime_) external onlyOwner {
        _lastUpdateTime = newTime_;
    }
    /**
     * -------------------------------------------------------------------------
     * END DEBUG
    ...
    function claimRewards() external {
        ...
        usdcClaimed[msg.sender] += _usdcAmount_;
        IERC20(usdcAddress).transfer(msg.sender, _usdcAmount_);
    }
