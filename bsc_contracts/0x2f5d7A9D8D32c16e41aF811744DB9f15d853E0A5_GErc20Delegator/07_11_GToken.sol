...
abstract contract GToken is GTokenInterface, ExponentialNoError, TokenErrorReporter {
    ...
    function changeUserBorrowDiscountInternal(address borrower) internal{
        accrueInterest();
        (accountBorrows[borrower].principal,accountBorrows[borrower].interestIndex,totalBorrows,totalReserves) = PlanetDiscount(discountLevel).changeUserBorrowDiscount(borrower);    
    }
    ...
    function repayBorrowFresh(address payer, address borrower, uint repayAmount) internal returns (uint) {
        ...
        /***************************************************************************************************/
        // update the borrow balane of the user in the discount contract to the latest
        changeLastBorrowBalanceAtBorrow(borrower);
     ...
     * @param repayAmount The amount of the underlying borrowed asset to repay
     */
    function liquidateBorrowFresh(address liquidator, address borrower, uint repayAmount, GTokenInterface gTokenCollateral) internal {
    ...
    function getUserAndAverageBoost(address user) external view returns (uint256 userBoostAPR, uint256 averageBoostAPR) {
        
        (uint256 userTokens, uint256 userFactor) = (accountTokens[user],  userFactors[user]);

        if(totalSupply == 0 || userTokens == 0 || totalFactor == 0) return (0, 0);

        uint256 exchangeRate = exchangeRateStoredInternal();
        uint256 blocksPerYear = 10512000;
        uint256 gammaBoostPerYear = gammatroller.getGammaSpeed(address(this))*blocksPerYear*gammatroller.getGammaBoostPercentage(address(this))/10000;

        averageBoostAPR = gammaBoostPerYear*1e18/(totalSupply*exchangeRate);

        userBoostAPR = gammaBoostPerYear*1e18*(userFactor/totalFactor)/(userTokens*exchangeRate);
