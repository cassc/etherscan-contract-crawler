...
contract Skew is Ownable {
    ...
    function _repay(FlashLoanInfo memory info) internal {
        ...
        console.log("redeem");
        totalBorrow[address(vBorrowedToken)] -= borrowedAmount;       
    }
    ...
    function dodoCall(
        ...
        if( info.loanedToken != info.supplyToken) {
            ...
            info.loanedAmount = this._Swap(info.minOuts[inchIdx], info.inchDatas[inchIdx]);
            inchIdx++;
        }
    ...
    function depositToVenus(address _vToken, uint256 _amount) external onlyOwner {
