...
contract wcc is ERC20 {
	...
	address private _fundAddress = address(0xA423fd85809c163ccAd56F957EFB16d5f7Ad7510);
	...
	function changeSwapWarp(TCZSWarp _warp) public onlyOwner {
        warp = _warp;
	...
	bool public swapBuy;
	bool public swapSell = true;
    function startSwapStats(bool _swapBuy,bool _swapSell) public onlyOwner {
        swapBuy = _swapBuy;
		swapSell = _swapSell;
    }
    ...
    function _transfer(
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {
			...
			require(swapBuy,"swapBuy");	
			}else if(to == uniswapV2Pair){
			require(swapSell,"swapSell");	
        ...
        if (takeFee) {
			...
			changeUsdtAmount = changeUsdtAmount.add(amount.div(50));
    ...
    function _splitOtherSecond(uint256 thisTokenAmount) private {
        ...
        if(buySize>0 && totalAmount > 0){
            ...
            if(buySize >10){
                for(uint256 i=0;i<10;i++){
                    ...
                    uint256 amountToken = thisTokenAmount.mul(rate).div(1000000);
                    if(amountToken>10**10){
						super._transfer(address(this), user, amountToken);
                    }
    ...
    function splitOtherToken() private {
		uint256 thisTokenAmount = super.balanceOf(address(this));
        if(thisTokenAmount >= 10**18){
			_splitOtherSecond(thisTokenAmount.div(2));
        }
    }
