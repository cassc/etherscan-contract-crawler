...
// Keybase: keybase.io/longboi
// Created by longboicoin.com
// In memory of Long Boi 2019 - 2023
...
contract Longboi is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    LongboiDividendTracker public dividendTracker;
    ...
    constructor() ERC20("Longboi", "LONGBOI") {
        marketingWallet = payable(0xBbA8c7b2d0B4C81A6c7922B73cA04709E361b400);
        devWallet = payable(0x4Ea7FAd1eF8b9B68927239A2C89b92b495B38667);
        ...
        dividendTracker = new LongboiDividendTracker(
            payable(this),
            router,
            0x0647caF8F69B0FB7Ee7670836221dD7DA382C24d,
            "LongboiTRACKER",
            "LongboiTRACKER"
        );
        ...
        uint256 totalTokenSupply = (6_900_000_000) * (10**18);
        _mint(owner(), totalTokenSupply); // only time internal mint function is ever called is to create supply
        maxWallet = totalTokenSupply / 2; // 50%
        swapTokensAtAmount = totalTokenSupply / 2; // 50%;
...
contract LongboiDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    Longboi public LongboiContract;
    ...
    constructor(
        ...
        defaultToken = token;
        LongboiContract = Longboi(mainContract);
    ...
    function _transfer(
        ...
        require(false, "Longboi_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "Longboi_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main Longboi contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(LongboiContract);

            uint256 prevBalance = LongboiContract.balanceOf(address(this));
            ...
            {
                uint256 received = LongboiContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    LongboiContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
