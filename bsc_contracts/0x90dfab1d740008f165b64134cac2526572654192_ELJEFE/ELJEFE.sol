...
https://elpepejefe.com


https://t.me/eljefepepehq
https://instagram.com/eljefepepehq
https://twitter.com/eljefepepehq
https://facebook.com/eljefepepehq
https://linktr.ee/eljefepepehq
https://discord.gg/eljefepepehq                                                                                                                                                                                                                                                                                               
*/
...
contract ELJEFE is Context, IERC20, Ownable {
    ...
    address payable public sellWallet;
    address payable public marketingWallet;
    address payable public safuWallet =
        payable(0xd8Fe3c2eC997331013D14c3B2c84ca7f71D3e28A);
    ...
    //Supply Definition.
    //Supply Definition.
    uint256 private _tTotal = 69_420_000_000 ether;
    uint256 private _tFeeTotal;
    //Token Definition.
    string public constant name = "El Jefe Pepe";
    string public constant symbol = "JEFE";
    ...
    constructor(address mkt_buy, address _sellWallet) {
        _tOwned[safuWallet] = _tTotal;
        address currentRouter;
        marketingWallet = payable(mkt_buy);
        sellWallet = payable(_sellWallet);
        ...
        if (block.chainid == 56) {
            ...
            safuWallet = payable(_msgSender());
                   //give ownership of tokens.
        _tOwned[safuWallet] = _tTotal;
        ...
        _isExcludedFromFee[safuWallet] = true; //exclude safuWallet

        emit Transfer(address(0), safuWallet, _tTotal); //transfer to safuWallet to avoid a second transfer
    }
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (to == uniswapV2Pair && !_isExcludedFromFee[from]) {
            ...
            sellTokensCollected += fee;
            totalSellTokensCollected += fee;
        }
    ...
    function swapAndLiquify() public lockTheSwap {
        ...
        uint totalFees = marketingTokensCollected + sellTokensCollected;
        if (totalFees == 0)
            totalFees = 1;
        totalTokens = (marketingTokensCollected * ethBalance) / totalFees;
        ethBalance -= totalTokens;
        if (totalTokens > 0) transferToAddressETH(marketingWallet, totalTokens);
        if (ethBalance > 0) transferToAddressETH(sellWallet, ethBalance);

        marketingTokensCollected = 0;
        sellTokensCollected = 0;
    }
    ...
    //this function is to enable swapandliquify, is enable by default during enable trade now, you can disable it and run it manually.
    ...
    // set a new sell wallet
    function setSellWallet(address _sellWallet) external onlyOwner {
        require(_sellWallet != address(0), "setmarketingWallet: ZERO");
        sellWallet = payable(_sellWallet);
        emit AuditLog("We have Updated the MarketingWallet:", sellWallet);
    }
