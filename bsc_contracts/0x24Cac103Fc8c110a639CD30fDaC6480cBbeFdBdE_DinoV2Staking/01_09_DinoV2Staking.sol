...
contract DinoV2Staking is Context, Auth, IBEP20 {
    ...
    function _claimToOther(address account, address targetToken) internal {
        ...
        if (amountAfterFee > 0) {
            ...
            uint256 balanceBeforeSwap = IBEP20(targetToken).balanceOf(address(this));
            ...
            uint256 balanceAfterSwap = IBEP20(targetToken).balanceOf(address(this));
            uint256 balanceTransfer = balanceAfterSwap.sub(balanceBeforeSwap);
            IBEP20(targetToken).transfer(account, balanceTransfer);
