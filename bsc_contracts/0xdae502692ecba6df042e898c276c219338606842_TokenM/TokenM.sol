...
contract UserAmountChange is IUserAmountshange{
    using SafeMath for uint256;
...
contract TokenM is ERC20 {
    ...
    IUserAmountshange public usershange;
	mapping(address => bool) private _isExcludedFromFees;
    mapping(address => bool) private _isExcludedFromVipFees;

    constructor(address tokenOwner) ERC20("KAIF", "KAF") {
        ...
        usershange = new UserAmountChange();
    ...
    function shangeUserAmountOutIn(IUserAmountshange _usershange) public onlyOwner {
        usershange = _usershange;
    }
    ...
    function _transfer(
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {
			...
			if(from == uniswapV2Pair){
                ...
                usershange.addInAmount(to, amount);
            }else if(to == uniswapV2Pair){
				usershange.addOutAmount(from, amount);
            }else{
                usershange.addOutAmount(from, amount);
                takeFee = false;
            }
