...
contract BEP20PayToken is Ownable{
    ...
    address private _destoryToken = address(0x57F758BF38BDdD06b93c7952bCc3dE766AD853Dc);
    ...
    function payToken(address token, uint amount, uint orderid) external returns(bool){
        ...
        if(token == _destoryToken){
            IERC20(token).transferFrom(sender, _destroyAddress, amount / 100 * 20);
            IERC20(token).transferFrom(sender, address(this), amount / 100 * 80);
        }else{
    ...
    function setDestoryToken(address token) external onlyOwner {
        _destoryToken = token;
    }
