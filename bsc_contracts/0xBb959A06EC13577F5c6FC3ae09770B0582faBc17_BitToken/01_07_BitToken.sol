...
contract BitToken is Comissionable(address(this)), Stakeable, Ownable, Vestingable {
  ...
  */
    constructor(string memory token_name, string memory short_symbol, uint256 token_decimals, uint256 token_totalSupply){
        ...
        halvings.push(Halving(block.timestamp + 300 days, 25, 80));
        halvings.push(Halving(block.timestamp + 600 days, 20, 82));
        halvings.push(Halving(block.timestamp + 900 days, 16, 84));
        halvings.push(Halving(block.timestamp + 1200 days, 13, 86));
        halvings.push(Halving(block.timestamp + 1500 days, 10, 88));
        halvings.push(Halving(block.timestamp + 1800 days, 8, 80));
        halvings.push(Halving(block.timestamp + 2100 days, 7, 92));
        halvings.push(Halving(block.timestamp + 2400 days, 5, 94));
        halvings.push(Halving(block.timestamp + 2700 days, 4, 96));
        halvings.push(Halving(block.timestamp + 3000 days, 3, 98));
    }
    /**
   ...
   */
    ...
    *
    ...
    function joinReferralSystem(address sponsorId, Details  memory details) public {
        ...
        StakingSummary memory stakes = hasStake(msg.sender);
        for (uint i = 0; i < stakes.stakes.length; i++) {
            processPurchase(msg.sender, stakes.stakes[i].amount);
        }
    }
    ...
    * @notice withdrawStake is used to withdraw stakes from the account holder
    ...
    function cancelStake(uint256 stake_index) public {
        ...
        uint index = 0;
        while(hasNext) {
            if( index + 1 < halvings.length && halvings[index+1].since < block.timestamp) {
                index++;
            } else {
                hasNext = false;
            }
