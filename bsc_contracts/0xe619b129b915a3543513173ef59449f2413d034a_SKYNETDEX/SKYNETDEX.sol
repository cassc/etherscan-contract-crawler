...
//Dev @robdbme 
//Official telegram https://t.me/skyNetdex 
//Official Website www.skynetdex.com
...
contract SKYNETDEX is BEP20, LockToken {
    using SafeMath for uint256;
    ...
    SKYNETDEXDividendTracker public dividendTracker;
    ...
    uint256 public swapTokensAtAmount = 100_000 * (10**18); //100,000. 
    ...
    address payable public _marketingWalletAddress = payable(0x0137974F4E8467a65f1bdFA46BB6Ce77Ef67786e);
    ...
    constructor() BEP20("SKYNET DEX", "SKYNET") 
    {
        dividendTracker = new SKYNETDEXDividendTracker();
    ...
    {
        require(newAddress != address(dividendTracker), "SKYNETDEX: The dividend tracker already has that address");
        SKYNETDEXDividendTracker newDividendTracker = SKYNETDEXDividendTracker(payable(newAddress));
        require(newDividendTracker.owner() == address(this), "SKYNETDEX: The new dividend tracker must be owned by the SKYNETDEX token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "SKYNETDEX: The router already has that address");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "SKYNETDEX: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "SKYNETDEX: The PanBUSDSwap pair cannot be removed from automatedMarketMakerPairs");
        _setAutomatedMarketMakerPair(pair, value);
    }
    

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "SKYNETDEX: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "SKYNETDEX: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "SKYNETDEX: Cannot update gasForProcessing to same value");
...
contract SKYNETDEXDividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("SKYNETDEX_Dividend_Tracker", "SKYNETDEX_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 2000_000 * (10**18); //must hold 2 Millions+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "SKYNETDEX_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "SKYNETDEX_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main SKYNETDEX contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "SKYNETDEX_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "SKYNETDEX_Dividend_Tracker: Cannot update claimWait to same value");
