...
contract LevelXToken is Initializable, Ownable, ERC20
{
	...
	uint256 constant INITIAL_BASELINE_TOTAL_ACTIVE_SUPPLY = 40_000_000e18; // 40M
	...
	bool public autoClaimForPair; // whether or not LVLX rewards are auto claimed for the liquidity pool
	...
	function updateAutoClaimForPair(bool _autoClaimForPair) external onlyOwner
	{
		autoClaimForPair = _autoClaimForPair;
		emit UpdateAutoClaimForPair(_autoClaimForPair);
	}
	...
	function claimForPair(uint256 _i) external onlyOwner returns (uint256 _amount)
	{
		...
		_amount = _claim(pair, _i, msg.sender);
	...
	function bumpLevel() external
	{
		...
		_postUpdateAccount(msg.sender, 1e18 * int256(totalActiveSupply) / int256(baselineTotalActiveSupply));
	...
	function claim(uint256 _i) external returns (uint256 _amount)
	{
		...
		_updateEpoch();
		_updateAccount(msg.sender);
		_amount = _claim(msg.sender, _i, msg.sender);
		_postUpdateAccount(msg.sender, 0e18);
		return _amount;
	}
	...
	event UpdateAutoClaimForPair(bool _autoClaimForPair);
