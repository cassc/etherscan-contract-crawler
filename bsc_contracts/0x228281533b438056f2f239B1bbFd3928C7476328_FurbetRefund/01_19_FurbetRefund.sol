...
contract FurbetRefund is BaseContract
{
     ...
     * @param expiration_ Message expiration.
     */
    function refund(bytes memory signature_, uint256 amount_, uint256 expiration_) external whenNotPaused
    {
     ...
     * @param expiration_ Message expiration.
     */
    function refundToFurfi(bytes memory signature_, uint256 amount_, uint256 expiration_) external whenNotPaused
    {
        require(expiration_ > block.timestamp, "Expired");
        require(!refunded[msg.sender], "Already refunded");
        require(amount_ > 0, "Invalid amount");
