...
contract Strategy is Ownable, ReentrancyGuard, Pausable {
    ...
    uint256 public controllerFee = 1000; // 10%
    ...
    constructor(
        ...
        wbnbAddress = IXRouter02(_uniRouterAddress).WETH();

        if (isAutoComp) {
            if (!isSingleVault) {
                token0Address = IPair(wantAddress).token0();
                token1Address = IPair(wantAddress).token1();
            }
    ...
    function setIsAutoEarn(bool _isAutoEarn) public onlyAllowGov {
        isAutoEarn = _isAutoEarn;
    }
}