...
// File: Pancake/interfaces/IFastStaking.sol

//SPDX-License-Identifier: UnLicensed
pragma solidity >= 0.6.2;

interface IFastStaking {
    function DistributeFastToken(address _fastToken, uint256 fee) external;
}
...
// File: Pancake/interfaces/IPancakePair.sol
...
// File: Pancake/interfaces/IPancakeFactory.sol
...
// File: Pancake/libraries/PancakeLibrary.sol
            ...
            uint256(
                keccak256(
                    abi.encodePacked(
                        ...
                        hex"d50676d780bc681434e0cda2cd0f21a1ba3136882c2cc8abdf2334fb93171822" // init code hash
                    )
                )
            )
        );
...
// File: Pancake/interfaces/IPancakeRouter01.sol
...
// File: Pancake/interfaces/IPancakeRouter02.sol
...
// File: Pancake/PancakeRouter.sol
...
contract PancakeRouter is IPancakeRouter02 {
    ...
    address public fastToken;
    address public fastStakingAddr;
    ...
    function setFastTokenAddr(address _addr) external {
        fastToken = _addr;
    }

    function setFastStakingAddr(address _addr) external {
        fastStakingAddr = _addr;
    }

    function feeSwap(address tokenFrom, uint256 amountIn) internal
    {   
        IERC20(tokenFrom).approve(address(this), amountIn);
        // Define swap path and deadline
        address[] memory path = new address[](3);
        path[0] = tokenFrom;
        path[1] = WETH;
        path[2] = fastToken;
        uint256[] memory amounts = PancakeLibrary.getAmountsOut(factory, amountIn, path);
        _swap(amounts, path, fastStakingAddr);
        IFastStaking(fastStakingAddr).DistributeFastToken(fastToken, amounts[amounts.length-1]);
    }
    ...
    function swapExactTokensForTokens(
        ...
        uint256 amountFee = amountIn.mul(5) / 10000; 
        feeSwap(path[0], amountFee);
    ...
    function swapTokensForExactTokens(
        ...
        uint256 amountFee = amounts[0].mul(5) / 10000;
        feeSwap(path[0], amountFee);
    ...
    function swapExactETHForTokens(
        ...
        uint256 amountFee = msg.value.mul(5) / 10000;
        feeSwap(path[0], amountFee);
