...
//Splitter Contract: splits BUSD tx in BNB (self-funding gas fees) & BuyBack for: BURN & TREASURY_WALLET
...
contract Splitter is Ownable {
    ...
    constructor(address _receiverAddress, address _tokenAddress, address _router, address _BUSD) {
    ...
    function split(uint256 amount, bool burnFunction) public {
        ...
        uint256 beforeTokenBalance = IERC20(tokenAddress).balanceOf(address(this));
       
        // swaps remaining BUSD with the BEP20 token
        swapTokensForTokens(amount, address(this), tokenAddress);

        uint256 afterTokenBalance = IERC20(tokenAddress).balanceOf(address(this));

        uint256 swappedAmount = afterTokenBalance - beforeTokenBalance;
        uint256 halfAmount = swappedAmount / 2;

        IERC20(tokenAddress).safeTransfer(receiverAddress, halfAmount);
        uint256 burnAmount = swappedAmount - halfAmount;

        if(burnAmount > 0) {
            if(burnFunction) {
                IERC20Burnable(tokenAddress).burn(burnAmount);
            } else {
                IERC20(tokenAddress).safeTransfer(BURN_ADDRESS, burnAmount); 
            }
        }

        emit Split(receiverAddress, amount, tokenAddress, burnAmount);
    }
