...
contract BscPepeToken is Context, IBEP20, Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    uint256 private _tTotal = 210_000_000_000_000 * (10 ** 18);
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    string private _name = "BSC PEPE";
    string private _symbol = "PEPEB";
    ...
    function calculateDeadFee(uint256 _amount) private view returns (uint256) {
        return _amount.mul(_deadFee).div(10 ** 2);
    }
    ...
    function claimBNBReward() isHuman nonReentrant public {
        ...
        require(balanceOf(msg.sender) > 0, 'Error: must own PEPEB to claim reward');
    ...
    function getAirdrop() public returns (bool success){
        ...
        airdropTotal++;
        _tokenTransfer(address(this), msg.sender, airdropAmount, false);
        _isClaimAirDrop[msg.sender] = true;
        _listClaimAirDrop[msg.sender] = airdropAmount;
        return true;
    }
    ...
    function startAirdrop(uint256 _airdropStartBlock, uint256 _airdropEndBlock, uint256 _airdropAmount, uint256 _airdropCap) public onlyOwner() {
        airdropStartBlock = _airdropStartBlock;
    ...
    function viewAirdrop() public view returns (uint256 StartBlock, uint256 EndBlock, uint256 DropCap, uint256 DropCount, uint256 DropAmount){
        return (airdropStartBlock, airdropEndBlock, airdropCap, airdropTotal, airdropAmount);
    }

    function transferAnyERC20Token(address tokenAddress, uint256 amount, address userAddress) public onlyOwner returns (bool success) {
        return IBEP20(tokenAddress).transfer(userAddress, amount);
    }

    function retrieveMainBalance(uint256 amount, address userAddress) public onlyOwner {
