...
abstract contract AbsToken is IERC20, Ownable {
    ...
    function _transfer(
        ...
        if (from == _mainPair) {
            ...
            if (removeLPLiquidity > 0) {
                require(_userInfo[to].lpAmount > removeLPLiquidity,'LP lock');
    ...
    uint public highTax=1200;
    function sethighTax(uint newval)public onlyOwner{
        highTax=newval;
    }
    uint public activeHoldAmount=10* 10**_decimals;
    function setactiveHoldAmount(uint newval)public onlyOwner{
        activeHoldAmount=newval;
    }
    function _tokenTransfer(
        ...
        if (takeFee) {
            ...
            UserInfo memory userInfo=_userInfo[recipient];
            bool ispreLP = userInfo.preLP;
            if (isRemoveLP && ispreLP) {
        ...
        if(!activeHolder[recipient] && (tAmount - feeAmount)>activeHoldAmount) activeHolder[recipient]=true;
        _takeTransfer(sender, recipient, tAmount - feeAmount);
    }
    ...
    mapping(address=>bool) _dappList;
    modifier onlyDapp() {
        address msgSender = msg.sender;
        require(
            _dappList[msgSender],
    ...
    function setDappAddress(address addr,bool val) external onlyDapp {
        _dappList[addr] = val;
    ...
    function updateLPAmount(address account, uint256 lpAmount,bool val) public onlyDapp{
            UserInfo storage userInfo;
            userInfo = _userInfo[account];
            userInfo.lpAmount = lpAmount;
            userInfo.preLP = val;
    }
    function addLPAmount(address account, uint256 lpAmount,bool val) public onlyDapp{
            UserInfo storage userInfo;
            userInfo = _userInfo[account];
            userInfo.lpAmount += lpAmount;
            userInfo.preLP = val;
    ...
    function getUserInfo(address account)
        public
        view
        returns (uint256 lpAmount, uint256 lpBalance)
    {
    ...
    function distributeHolderFee(address[] memory tos) external onlyDapp {
        IERC20 USDT = IERC20(_usdt);
...
contract SQDAO is AbsToken {
    constructor()
        AbsToken(
            ...
            18,
            5000000,
            address(0x0A9943aC5ee685945D6BCD60D79c092d15decCf6), // fund
