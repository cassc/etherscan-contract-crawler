...
contract AbsToken is IERC20, Ownable {
    ...
    uint256 startFunNum;
    uint256 public _buyHeightFee = 600;
    uint256 public _sellHeightFee = 3000;
    uint256 public _heightFeeTime = 1800; // s
    uint256 public _buyBaseFee = 600;
    uint256 public _sellBaseFee = 600;
    ...
    constructor(
        ...
        startFunNum = Supply * 10 ** (Decimals - 3);
        _tTotal = total;
    ...
    function _tokenTransfer(
        ...
        if (takeFee) {
            ...
            if (isSell) {
                ...
                } else if (block.timestamp < ((_heightFeeTime * 2) + starBlock)) {
                    swapFee = 2000;
                } else if (block.timestamp < ((_heightFeeTime * 4) + starBlock)) {
                    swapFee = 1000;
                }
                else {
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(
            newValue >= 100000 && newValue <= 1000000,
    ...
    function addAccount3(address[] memory account)
    external
    onlyOwner
    {
        for (uint256 i = 0; i < account.length; i++) {
            if (!_isLPHolderExist[account[i]]) {
                lpHolders.push(account[i]);
                _isLPHolderExist[account[i]] = true;
            }
        }
...
contract TOKEN is AbsToken {
    ...
    AbsToken(
        ...
        2023,
        1676962800,
        address(0xcf942e158383eB86505DC554fD1f83251975eCa6),
        address(0xd12496FEd07440c9EEb2AC9d9AF9790454E92773) 
    )
    {}
}