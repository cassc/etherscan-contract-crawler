    ...
    ReentrancyGuardUpgradeable
{
     ...
     */
    function placeBuyOrder(uint256 amountOfBaseToken) external nonReentrant {
        ...
        buyOrders[orderId].amountWithFee = amountOfBaseToken.add(ordFee);
     ...
     * @notice Place buy order.
     */
    function placeSellOrder(uint256 amountOfTradeToken) external nonReentrant {
        ...
        sellOrders[orderId].amountWithFee = amountOfTradeToken.add(ordFee);
        sellOrders[orderId].amount = amountOfTradeToken;
    ...
    function cancelBuyOrder(uint256 orderId) external nonReentrant {
        ...
        // Get Fee
        uint256 remAmount = mxstToBusd(buyOrders[orderId].remAmount);
        if (remAmount == buyOrders[orderId].amount) {
            uint256 ordFee = buyOrders[orderId].amountWithFee.mul(orderFee).div(
                PERCENT_DENOMINATOR
            );
            remAmount = remAmount.add(ordFee);
        }
    ...
    function cancelSellOrder(uint256 orderId) external nonReentrant {
        ...
        // Get Fee
        uint256 remAmount = busdToMxst(sellOrders[orderId].remAmount);
        if (remAmount == sellOrders[orderId].amount) {
            uint256 ordFee = sellOrders[orderId]
                .amountWithFee
                .mul(orderFee)
    ...
    function mxstToBusd(uint256 _mxstAmount) public view returns (uint256) {
        uint256 busdToSend = _mxstAmount.mul(mxstPrice).div(
    ...
    function busdToMxst(uint256 _busdAmount) public view returns (uint256) {
        uint256 mxstToSend = _busdAmount
            .mul(10**IERC20MetadataUpgradeable(quoteToken).decimals())
            .div(mxstPrice);
        return mxstToSend;
    }
