...
abstract contract BaseAdapterBsc is Ownable {
     ...
     */
    /// #if_succeeds {:msg "Set path failed"} paths[_inToken][_outToken].length == _paths.length;
    function setPath(
        address _inToken,
        address _outToken,
     ...
     */
    /// #if_succeeds {:msg "Set investor failed"} investor == _investor && investor != old(investor);
     ...
     * @param _account address of user
     * @param _amountIn payable eth from Investor
     * @param _tradeTokenId TradeNFT token id
     */
    function deposit(
        uint256 _tokenId,
        uint256 _amountIn,
     ...
     * @param _userInfo user adapter infor from TradeNFT
     */
    function pendingReward(uint256 _tokenId, UserAdapterInfo memory _userInfo)
        external
        view
        virtual
