...
/**
* Telegram : https://t.me/babysurgexsrg20portal
* Website : https://babysurgex-23.my.canva.site/
* Max Wallet: 30,000
* Max Txn: 15,000
*/
...
contract BabyDSurgeX is IERC20, Context, Ownable, ReentrancyGuard {
    event Bought(
    ...
    // token data
    string private constant _name = "BabySurgeX";
    string private constant _symbol = "BBYSRGX";
    ...
    // Total Supply
    uint256 public constant _totalSupply = 10**6 * _DECMULTIPLIER;
    ...
    uint256 public maxBag = _totalSupply  / 33;
    uint256 public maxTX = _totalSupply/2 / 33;

    //Tax collection
    uint256 public taxBalance = 0;

    //Tax wallets
    address public marketingWallet = 0x1CDD5F2C874D3f0f35dfF468985ba466EB04264F;
    address public charityWallet = 0x11440ce01BB1FbED3996D3e6eBABA1895A6412f4;

    // Tax Split
    uint256 public marketingShare = 300;
    uint256 public charityShare = 200;
    ...
    //trading parameters
    uint256 public liquidity = 5 * 10**4 * 10**_srgDecimals;
    ...
    function changeTaxDistribution(
        ...
        require(
            newmarketingShare + newcharityShare  + newRewardShare ==
                SHAREDIVISOR,
    ...
    function changeFeeReceivers(
        ...
        require(
            newmarketingWallet != address(0) && newcharityWallet != address(0),
    ...
    function withdrawTaxBalance() external nonReentrant onlyOwner {
        if(marketingShare>0){
        bool temp1 = SRGIE.transfer(marketingWallet, (taxBalance * marketingShare) /(marketingShare + charityShare));
        require(temp1,"failed to send funds");
        }

        if(charityShare>0){
        bool temp2 = SRGIE.transfer(charityWallet, (taxBalance * charityShare) /(marketingShare + charityShare));
