...
contract KochiAirdrop is ContextUpgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable, OwnableUpgradeable, IKochiAirdrop {
  ...
  function editAirdropSchedule(uint256 uid, ESchedule schedule, uint256 schedule_duration) external onlyOwner {
    require(uid < airdrops.length, "KochiAirdrop: airdrop does not exist");

    airdrops[uid].schedule = schedule;
    airdrops[uid].schedule_duration = schedule_duration;
  }

  function editAirdropStartline(uint256 uid, uint256 startline) external onlyOwner {
    require(uid < airdrops.length, "KochiAirdrop: airdrop does not exist");

    airdrops[uid].startline = startline;
  }
}