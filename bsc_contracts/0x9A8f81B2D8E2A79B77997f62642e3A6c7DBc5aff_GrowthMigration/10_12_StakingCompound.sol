...
import { StakingVolatile } from "./StakingVolatile.sol";

/*
 Compound VDC fees are 11% in and 11% out, they are distributed in the following way:
 7% to drip pool
 1% Instant dividends to stakers
 1% GRO to PERPS stakers
 1% Burnt
 1% Volatile VDC
 Receives incentives daily from GRO staking
 */
contract StakingCompound is Initializable, Ownable, ReentrancyGuard
{
	...
	struct AccountInfo {
		...
		uint256 reward; // GRO reward accumulated but not claimed
	...
	uint256 constant DEFAULT_LAUNCH_TIME = 1662652800; // 2022-09-08 4PM UTC
	...
	constructor(address _reserveToken, address _boostToken, address _stakingVolatile)
	{
		initialize(msg.sender, _reserveToken, _boostToken, _stakingVolatile);
	}

	function initialize(address _owner, address _reserveToken, address _boostToken, address _stakingVolatile) public initializer
	{
	...
	function _deposit(address _sender, uint256 _amount, address _account) internal
	{
		...
		IERC20(reserveToken).safeApprove(stakingVolatile, _1percent);
		StakingVolatile(stakingVolatile).rewardAll(_1percent);
	}
