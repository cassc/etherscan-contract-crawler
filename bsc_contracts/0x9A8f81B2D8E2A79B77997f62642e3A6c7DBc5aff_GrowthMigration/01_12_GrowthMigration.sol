...
contract GrowthMigration is Initializable, Ownable, ReentrancyGuard
{
	...
	address constant CULTIVATOR_DEPOSIT = 0xD649685E7ce349336F8B04EB033f82A4e98337E6; // emp money cultivator deposit
	address constant CULTIVATOR_APPROVE = 0x121183FB38813D24e2C489FBf224C8a1D7fad58E; // emp money cultivator approve

	uint256 constant DEFAULT_LAUNCH_TIME = 1666720800; // 2022-10-25 6PM UTC
	...
	function cultivate(uint256 _maxAmount) external payable hasLaunched nonReentrant returns (uint256 _amount, uint256 _shares)
	{
		...
		IERC20(cultivatorToken).safeApprove(CULTIVATOR_APPROVE, _shares);
		ICultivator(CULTIVATOR_DEPOSIT).depositFor(msg.sender, _shares);
		IERC20(cultivatorToken).safeApprove(CULTIVATOR_APPROVE, 0);
