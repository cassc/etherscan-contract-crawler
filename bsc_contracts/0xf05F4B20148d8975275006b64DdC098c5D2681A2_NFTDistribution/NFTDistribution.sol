...
// File: contracts/utils/IRateRule.sol
...
// File: contracts/utils/NFTDistribution.sol
...
contract NFTDistribution is ERC721Holder, Ownable {
    ...
    uint256 public configuredTopCommissionNum = 20;
    ...
    function updateTopCommissions(UserInfo memory updatedUserInfo) internal {
        if (topCommissions.length == 0) {
            ...
            userInfo[updatedUserInfo.user] = updatedUserInfo;
            return;
        }
        ...
        if (topCommissions.length == 1) return;
    ...
    function tickerSwap(Ticker[] storage tickers, uint256 i, uint256 j) internal {
        Ticker memory tempTicker = tickers[i];
        tickers[i] = tickers[j];
        tickers[j] = tempTicker;
    }
