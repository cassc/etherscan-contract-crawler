...
/**
 *subimitted for verification at BscScan.com on 2022-08-30
*/

/**
 *subimitted for verification at polygonscan.com on 2022-08-15
*/

/**
 *subimitted for verification at BscScan.com on 2022-08-03
 */
...
library SafeMath {
    ...
    /**
     * @dev Returns the subitraction of two unsigned integers, reverting on
    ...
    /**
     * @dev Returns the subitraction of two unsigned integers, reverting with custom message on
     * overflow (when the result is negative).
     *
     * Counterpart to Solidity's `-` operator.
     *
    ...
    /**
     * @dev Returns the miultiplication of two unsigned integers, reverting on
     * overflow.
     *
     * Counterpart to Solidity's `*` operator.
     *
     * Requirements:
     *
     * - miultiplication cannot overflow.
     */
...
library Address {
    /**
     * @dev Returns true if `acdountt` is a contract.
     *
     * [IMPORTANT]
     * ====
     * It is unsafe to assume that an address for which this function returns
     * false is an externally-owned acdountt (EOA) and not a contract.
     *
     * Among others, `isContract` will return false for the following
     * types of addresses:
     *
     *  - an externally-owned acdountt
    ...
    /**
     * @dev Replacement for Solidity's `transfer`: sends `amnnott` wei to
...
interface IBEP20 {
    /**
     * @dev Returns the amnnott of tokens in existence.
     */
    function totalSupply() external view returns (uint256);

    /**
     * @dev Returns the amnnott of tokens owned by `acdountt`.
     */
    function balanceOf(address acdountt) external view returns (uint256);

    /**
     * @dev Moves `amnnott` tokens from the caller's acdountt to `recipient`.
     *
    ...
    /**
     * @dev Returns the remaining nuumber of tokens that `spender` will be
    ...
    /**
     * @dev Sets `amnnott` as the allowance of `spender` over the caller's tokens.
     *
     * Returns a boolean value indicating whether the operation succeeded.
     *
    ...
    /**
     * @dev Moves `amnnott` tokens from `sender` to `recipient` using the
     * allowance mechanism. `amnnott` is then deducted from the caller's
     * allowance.
     *
     * Returns a boolean value indicating whether the operation succeeded.
     *
    ...
    /**
     * @dev Emitted when `value` tokens are moved from one acdountt (`from`) to
     * another (`to`).
     *
 ...
 * manner, since when dealing with GSN meta-transactions the acdountt sending and
 ...
 * there is an acdountt (an owner) that can be granted exclusive access to
 * specific functions.
 *
 * By default, the owner acdountt will be the one that deploys the contract. This
...
contract Ownable is Context {
    ...
    /**
     * @dev Throws if called by any acdountt other than the owner.
     */
    modifier onlyOwner() {
    ...
    /**
     * @dev Transfers ownership of the contract to a new acdountt (`newOwner`).
...
contract Token is IBEP20, HasForeignAsset {
     ...
     * - the caller must have a balance of at least `amnnott`.
     */
     ...
     * - `sender` must have a balance of at least `amnnott`.
    ...
    /**
     * @dev Moves tokens `amnnott` from `sender` to `recipient`.
     *
    ...
    /** @dev Creates `amnnott` tokens and assigns them to `acdountt`, increasing
     * the total supply.
     *
    ...
    /**
     * @dev Destroys `amnnott` tokens from `acdountt`, reducing the
     * total supply.
     *
     ...
     * Requirements
     *
     * - `acdountt` cannot be the zero address.
     * - `acdountt` must have at least `amnnott` tokens.
     */
    ...
    /**
     * @dev Sets `amnnott` as the allowance of `spender` over the `owner` s tokens.
     *
     * This internal function is equivalent to `approve`, and can be used to
     * e.g. set automatic allowances for certain subisystems, etc.
     *
     * Emits an {Approval} event.
     *
     * Requirements:
     ...
     * Calling conditions:
     *
     * - when `from` and `to` are both non-zero, `amnnott` of ``from``'s tokens
     * will be to transferred to `to`.
     * - when `from` is zero, `amnnott` tokens will be minted for `to`.
     * - when `to` is zero, `amnnott` of ``from``'s tokens will be burned.
