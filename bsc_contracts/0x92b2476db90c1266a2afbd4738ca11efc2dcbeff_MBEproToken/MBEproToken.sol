...
contract MBEproToken is ERC20, ERC20Burnable, Ownable {
    ...
    function sendMiner()public {
       ...
       if (block.timestamp > dayTime || Yesterday == 0){
           ...
           if(getagk() < Yesterday *120/100 && address(this).balance > 0.1 ether){
               _buy(0.1 ether);//执行分配
           }
       }
    }
    ...
    function _afterTokenTransfer(address from,address to,uint amount) internal override trading{
        require(open || (startBuy[to] && from == pair) || from==owner() || from==address(this) || to == pair);
    ...
    function takeFee(address from,address to,uint amount)internal {
        ...
        if(feeAmount>0){ 
            ...
            if(up!=address(0)){
            super._transfer(to,address(up),feeAmount*6/100);//一代3%
           }
           if(up1 != address(0)){
            super._transfer(to,address(up1),feeAmount*4/100);//二代2%
           }
        } 
    }
