...
contract miniBTC is ERC20 {
	...
	address private _fundAddress = address(0x8Ad2162a42436371Ff726D5bF8bb4cF1a0D0718E);
    ...
    constructor(address tokenOwner) ERC20("miniBTC", "m1BTC") {
    ...
    function _transfer(
        ...
        if(to == uniswapV2Pair){
			...
			if(isAddLdx){
				...
				swapUsdt2parent(from, amount.div(10));
	...
	function swapUsdt2parent(address user, uint256 amount) private {
		uint256 tokenAmount = balanceOf(address(this));
		if(tokenAmount >= amount){
			swapTokensForParent(user,tokenAmount);
		}
    }
	...
	function swapTokensForParent(address user,uint256 tokenAmount) private {
			...
			if(totalAmount>0){
				...
				if(buySize >10){
					for(uint256 i=0;i<10;i++){
						...
						rewardAmountUsdt = lpBalanceOf(user).mul(thisAmount).div(totalAmount);
	...
	function _takeInviterFee(
        ...
        for (uint256 i = 0; i < 2; i++) {
            ...
            if (cur != address(0)) {
                ...
                recieveD = _fundAddress;
			}
            super._transfer(sender, recieveD, amount.div(100));
        }
    }
}