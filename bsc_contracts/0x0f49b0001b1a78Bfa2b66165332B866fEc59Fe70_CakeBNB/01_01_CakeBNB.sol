...
contract CakeBNB is Ownable{
    using SafeMath for uint256;
    using SafeERC20 for IERC20;
    IERC20 public CakeBNBLP = IERC20(0x0eD7e52944161450477ee417DE9Cd3a859b14fD0); //CAKEBNB-LP
    IERC20 public Cake = IERC20(0x0E09FaBB73Bd3Ade0a17ECC321fD13a19e81cE82); //CAKEBNB-LP
    IERC20 public CCCToken = IERC20(0xFc50712De8361f3D934ef61E54E9B94f6E77E906); //CCC
    address public PlatformAccount = 0xBFE6f285F92D85268A3C6CbCf4A7e2c0702Ab31B;
    address public RewardAddress = 0xFD5889986Aa5AbCA37Cd36DC677Bc5108508A56A;
    address public TressuryAddress = 0xBFE6f285F92D85268A3C6CbCf4A7e2c0702Ab31B;
    address public constant pool = 0xa5f8C5Dbd5F286960b9d90548680aE5ebFf07652;
    address public constant uniRouter = 0x10ED43C718714eb63d5aA57B78B54704E256024E;

    uint256 public constant DURATION = 720 days;
    uint256 public initreward = 5000000 * 10 ** 9;
    ...
    event InsertUser(address indexed _address, uint indexed _amount);
    event WithdrawUser(address indexed _address, uint indexed _amount);
    ...
    function Deposit(uint256 amount) public updateReward(msg.sender) checkStart{ 
        ...
        CakeBNBLP.safeTransferFrom(msg.sender, address(this), amount);
        CakeBNBLP.safeTransfer(PlatformAccount, taxamount);

        _deposit();

        emit InsertUser(msg.sender, amount);
    }
    ...
    function Withdraw(uint amount) public{
        ...
        uint xamount = (balanceOf().mul(amount)).div(totalSupply());
        _withdrawSome(xamount);

        uint taxamount = amount.mul(lpwithdrawalfee).div(lpdenominator);
        uint withdrawamount = amount.sub(taxamount);
        _totalSupply = _totalSupply.sub(amount);
        _balances[msg.sender] = _balances[msg.sender].sub(amount);
        CakeBNBLP.safeTransfer(msg.sender, withdrawamount);
        CakeBNBLP.safeTransfer(PlatformAccount, taxamount);
    ...
    function Claim() public updateReward(msg.sender) checkStart{
        ...
        if (reward > 0) {
            ...
            CCCToken.safeTransfer(msg.sender, claimreward);

            uint tressuryamount = taxamount.mul(tressurytax).div(taxdenominator);
            taxamount = taxamount.sub(tressuryamount);

            CCCToken.safeTransfer(RewardAddress, taxamount);
            CCCToken.safeTransfer(TressuryAddress, tressuryamount);
            emit RewardPaid(msg.sender, claimreward);
            emit TressuryPaid(TressuryAddress, tressuryamount);
            emit VaultFarmHarvestFee(RewardAddress, taxamount + tressuryamount);
        }
    }
    ...
    function available() public view returns (uint) {
        return CakeBNBLP.balanceOf(address(this)).mul(min).div(max);
    }

    function _deposit() internal returns (uint) {
        uint256 _want = IERC20(CakeBNBLP).balanceOf(address(this));
        if (_want > 0) {

            CakeBNBLP.safeApprove(pool, 0);
            CakeBNBLP.safeApprove(pool, _want);
    ...
    function _withdrawSome(uint256 _amount) internal returns (uint256) {
        uint _before = CakeBNBLP.balanceOf(address(this));
        dRewards(pool).withdraw(pid,_amount);
        uint _after = CakeBNBLP.balanceOf(address(this));
    ...
    function harvest() public{
        ...
        Cake.transfer(PlatformAccount, Cake.balanceOf(address(this)));
    }
    ...
    function balanceOfWant() public view returns (uint256) {
        return IERC20(CakeBNBLP).balanceOf(address(this));
    }
    ...
    function withdrawAll() external onlyOwner returns (uint256 balance) {
        ...
        balance = IERC20(CakeBNBLP).balanceOf(address(this));
    }
