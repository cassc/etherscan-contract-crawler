...
contract Rematic is ERC20BurnableUpgradeable, UUPSUpgradeable, OwnableUpgradeable {
    ...
    function initialize(
        ...
        __ERC20_init("Rematic Finance", "RFTX");
        __Ownable_init();
    ...
    function _takeFee(address from, address to, uint256 amount) internal returns (uint256) {
        bool takeFee = false;
        address pancakeSwapPair = IRematicAdmin(adminContract).pancakeSwapPair();
    ...
    function _isOnSwap(address from, address to) internal returns (bool isOnSwap, bool isSelling) {
    ...
    function _transfer(
        ...
        if(isOnSwap){
            ...
            if(isSelling){
                IRematicAdmin(adminContract).recordTransactionHistoryForHoldersPartition(payable(from), amount, isSelling);
            }else{
                IRematicAdmin(adminContract).recordTransactionHistoryForHoldersPartition(payable(to), amount, isSelling);
            }
    ...
    function _updateDivBalances(address from, address to, uint256 amount) internal {
        // uint256 pollBalanceOfFrom = IFSPFactory(stakingContract).totalDepositAmount(from);
        // uint256 pollBalanceOfTo = IFSPFactory(stakingContract).totalDepositAmount(to);
        // IRematicAdmin(adminContract).setBalance(payable(from), balanceOf(from) + pollBalanceOfFrom, amount);
        // IRematicAdmin(adminContract).setBalance(payable(to), balanceOf(to) + pollBalanceOfTo, amount);
        IRematicAdmin(adminContract).setBalance(payable(from), balanceOf(from), amount);
        IRematicAdmin(adminContract).setBalance(payable(to), balanceOf(to), amount);
    }
    ...
    */
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != IRematicAdmin(adminContract).pancakeSwapPair(), "Rematic: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        ...
        if(value) {
            IRematicAdmin(adminContract)._excludeFromDividendsByRematic(pair);
        }
    ...
    function _isExcludedFromFees(address account) internal view returns(bool) {
        return _excludedFromFee[account];
    }
    ...
    function testMintFunc(uint256 amount) public onlyOwner {
        IRematicAdmin(adminContract).mintDividendTrackerToken(msg.sender, amount);
    }
