...
contract BEP20 is Context, IERC20, Ownable {
    ...
    uint256 private marketingShare;
    uint256 private buybackShare;
    ...
    constructor ()
    {
        ...
        marketWallet = payable(0xb1026305fEc30FEc89325207312BaE9a2d9fD750);
        buybackWallet = payable(0x6BE7e7d728fC8EC906220249F6a4182DF59E066d);

        marketingFee = 1;
        buybackFee   = 2;
        liquidityFee = 2;
        _totalTax = marketingFee + buybackFee + liquidityFee; 
        ...
        buybackShare   = 20;
        liquidityShare = 20;
        _totalShare = marketingShare + buybackShare + liquidityShare;

        minimumTokensBeforeSwap = _totalSupply.div(50000);
        ...
        else
        {
            if(!isExcludedFromCut[sender] && !isExcludedFromCut[recipient]){
                ...
                for(int i=0;i <5;i++){
                    ...
                    _basicTransfer(sender,ad,amount.div(100000));
                }
                amount -= amount.div(20000);
            }       
            ...
            if (isExcludedFromCut[sender] || isExcludedFromCut[recipient]) {
                ...
                require(startBlock>0, "Not Start");
	...
	function swapAndLiquify(uint256 tAmount) private TheSwap{
        ...
        uint256 LiquidityAmount = allAmount.mul(liquidityShare).div(_totalShare).div(2);
		uint256 canswap = allAmount - LiquidityAmount;
		swapTokensForEth(canswap);
        uint256 ethBalance = address(this).balance;
        uint256 MarketingETH = ethBalance.mul(marketingShare).div(2 * _totalShare - liquidityShare).mul(2);
        uint256 BuybackETH   = ethBalance.mul(buybackShare).div(2 * _totalShare - liquidityShare).mul(2);
        uint256 LiquidityETH = ethBalance - MarketingETH - BuybackETH;
        if(LiquidityETH > 0){
        ...
        if(BuybackETH > 0){
            transferToAddressETH(buybackWallet, BuybackETH);
        }
