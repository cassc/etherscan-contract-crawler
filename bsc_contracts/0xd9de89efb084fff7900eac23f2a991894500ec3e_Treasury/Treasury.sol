...
contract Depot is Ownable, IReferralReport {
    ...
    constructor(address prevDepotAddr) Ownable() {

        require(prevDepotAddr != address(0), "non-zero address for prev depot");

         //init reg
        registry = new AddressRegistry();

        prevDepot = Depot(prevDepotAddr);
        migrate();
    ...
    function deposit(uint _amount) isRunning external {
        ...
        users[_user].deposit_base = users[_user].current_balance; ////this ensures equal daily amounts instead of asymptotic decay 
    ...
    function migrate() private {
        ...
        total_users = prevDepot.total_users();
        total_deposited = prevDepot.total_deposited();
        total_claimed = prevDepot.total_claimed();
        total_rewards = prevDepot.total_claimed();
        total_txs = prevDepot.total_txs();
        current_balance = prevDepot.current_balance();
    }

    function migrateUser(address _user) private returns(bool) {

        DepotUser memory prevAccount = prevDepot.getUser(_user);

        if (prevAccount.exists){
            users[_user] = DepotUser(
                prevAccount.exists, 
                prevAccount.deposits, 
                prevAccount.deposit_base, 
                prevAccount.current_balance, 
                prevAccount.payouts, 
                prevAccount.rewards, 
                prevAccount.last_time
            );
