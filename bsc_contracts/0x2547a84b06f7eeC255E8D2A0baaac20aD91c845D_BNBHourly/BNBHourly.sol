...
contract BNBHourly {
	...
	uint256 constant public INVEST_MIN_AMOUNT = 0.01 ether; //0.1
    uint256 constant public INVEST_MAX_AMOUNT = 1000 ether; 
    uint256 constant public WITHDRAW_MIN_AMOUNT = 0.01 ether; //0.1
    uint256 constant public WITHDRAW_MAX_AMOUNT = 100 ether; 
    uint256 constant public MAX_DEPOSIT = 300;
    uint256 constant public INVEST_STEP = 0.01 ether; //0.01
    uint256 constant public MASTER_MIN_DEPOSIT = 5;
	uint256[] public REFERRAL_PERCENTS = [50, 20];
	uint256 constant public CEO_FEE = 20;
	...
	constructor() {
        ceoWallet = payable(0x798383ebe11e5F6eCFa30867BE93a3509034B1bb);
		marketinWallet = payable(0x798383ebe11e5F6eCFa30867BE93a3509034B1bb);
        insuranceWallet = payable(0x798383ebe11e5F6eCFa30867BE93a3509034B1bb);

		startDate = 1672743600;

        plans.push(Plan(25, 60));
        plans.push(Plan(25, 65));
		plans.push(Plan(25, 70));
		plans.push(Plan(25, 75));
		plans.push(Plan(25, 80));
		plans.push(Plan(25, 85));
	}
	...
	function invest(address referrer) public payable {
        ...
        if(depositsLength > 0){
			...
			if(depositsLength >= 2 * MASTER_MIN_DEPOSIT){
                plan = 2;
            }
			if(depositsLength >= 3 * MASTER_MIN_DEPOSIT){
                plan = 3;
            }
			if(depositsLength >= 4 * MASTER_MIN_DEPOSIT){
                plan = 4;
            }
			if(depositsLength >= 5 * MASTER_MIN_DEPOSIT){
                plan = 5;
            }
        }
