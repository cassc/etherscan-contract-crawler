...
contract StakeToMe is Ownable {
    ...
    address public dev2 = 0x3D57A659bBE3db7a3F9f534AFCd9fE73286A9eb8;
  ...
  // -- Ref Approval -- // 
     ...
     // ---- Referral System ----
       ...
       require(ref != address(0) && ref != msg.sender);
        ...
        if(i == 0) {
          ...
          // here to add 1% of user investment.
    ...
    function withdrawROI() public {
      require(!withdrawInit[msg.sender].init,"You have exceed the amount to withdraw");
        ...
        if(ROI_WITHDRAW[msg.sender].totalWithdraw + withdrawn[msg.sender].total_withdrawn >= depositQuery[msg.sender].investment * ProfitX) {
    ...
    function RefDailyROI(address _addr, uint256 _position) public view returns(uint256) {
        ...
        uint256 userCapital = RefRewards[_addr][_position].amount; // fixed amount daily return
        uint256 perBlock = userCapital / dailyBlock;
        uint256 CurrentID = block.number;
        uint256 total = CurrentID - blockID;
        return total * perBlock;
   ...
   function DailyRefROIReward(address _addr) public view returns (uint256) {
    ...
    function withdrawRef() public {
       ...
       if(totalReward >= depositQuery[msg.sender].investment) {
        ...
        if(withdrawn[msg.sender].total_withdrawn + ROI_WITHDRAW[msg.sender].totalWithdraw  >= depositQuery[msg.sender].investment * ProfitX) {
          withdrawInit[msg.sender] = Record5x(msg.sender,true);
        }
