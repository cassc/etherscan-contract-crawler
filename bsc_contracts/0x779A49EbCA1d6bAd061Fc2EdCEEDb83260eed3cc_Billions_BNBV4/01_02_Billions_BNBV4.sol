...
contract Billions_BNBV4 {
   ...
   function createStake(address _user,uint256 _stake) internal{
    ...
    function stake(uint256 _stake) public {
        ...
        require(balance>=_stake,"no funds");
       if(users[msg.sender].stake.length == 0) addStakeholder(msg.sender);
       users[msg.sender].stake.push(Stake(_stake, block.timestamp, 0,block.timestamp.add(150 days) ,roiPercentage));
       logs[msg.sender].push(log(_stake,"Stake",block.timestamp));
    ...
    function removePoolStake() external
   {
    ...
    function getCurentPoolPercent(address _user) public view returns(uint per){
        if(systemPoolAmount>0){
            ...
            per = (value*1e2/totalStake).mul(100);
            per = per/1e8;
            }
