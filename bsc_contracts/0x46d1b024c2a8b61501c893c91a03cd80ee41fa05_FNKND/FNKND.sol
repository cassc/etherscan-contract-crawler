...
contract FNKND is ERC20 {
    ...
    address _fundAddress = address(0x59309DD2fa1D4465EDB8f4f83C9fe4D68BecbC2f);
    ...
    constructor(address tokenOwner) ERC20("FNKND", "FNKND") {
	...
	function changeSwapWarp(FNKNDWarp _warp) public onlyOwner {
        warp = _warp;
    ...
    function getNowPrice() public view returns(uint256){
        ...
        uint256 poolung = balanceOf(uniswapV2Pair);
        if(poolung > 0){
    ...
    function _transfer(
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to] || isAddLdx || balanceOf(_destroyAddress) >= 900 * 10**18) {
            takeFee = false;
        }else{
			if(from == uniswapV2Pair){
                if(startTime.add(1800) > block.timestamp){
                    require(balanceOf(to).add(amount) <= 5 * 10**18,"max amount");
                }
            }else if(to == uniswapV2Pair){
                require(startTime.add(600) < block.timestamp);
            }else{
				if(startTime.add(1800) > block.timestamp){
                    require(balanceOf(to).add(amount) <= 5 * 10**18,"max amount");
                }
        ...
        if (takeFee) {
			...
			if(fundrate > 0){
				super._transfer(from, address(this), amount.div(100).mul(fundrate));
				amount = amount.div(100).mul(93-fundrate);
			}else{
				amount = amount.div(100).mul(93);
			}
        }
        super._transfer(from, to, amount);
        if(to == uniswapV2Pair){
    ...
    function swapAndLiquify(uint256 tokenAmount) private {
        ...
        if(allusdt > preusdt){
            uint256 newUsdt = allusdt.sub(preusdt);
            USDT.transfer(_fundAddress, newUsdt.div(3));
        }
    }
    ...
    function _splitOtherSecondBuy(uint256 sendAmount) private {
        ...
        if(buySize>0 && sendAmount > 0 && totalAmount > 0){
            ...
            if(buySize >10){
                for(uint256 i=0;i<10;i++){
                    if(buyindex >= buySize){buyindex = 0;}
                    user = buyUser[buyindex];
                    rate = balanceOf(user).mul(100000000).div(totalAmount);
                    uint256 amountUsdt = sendAmount.mul(rate).div(100000000);
                    if(amountUsdt>10**10 && balanceOf(user) >= 2 * 10**18){
						USDT.transfer(user,amountUsdt);
                    }
					buyindex = buyindex.add(1);
                }
            }else{
    ...
    function _splitOtherSecondLDX(uint256 sendAmount) private {
        uint256 buySize = sellUser.length;
    ...
    function splitOtherTokenLDX() private {
		uint256 thisAmount = USDT.balanceOf(address(this));
        if(thisAmount >= 10**17){
			uint256 sendAmount = thisAmount.div(5).mul(4);
			_splitOtherSecondLDX(sendAmount);
        }
    }
	
    function getLDXsize() public view returns(uint256){
        return sellUser.length;
    }
}