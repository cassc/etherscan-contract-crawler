...
contract Testor is IBEP20, Auth {
    
    using SafeMath for uint256;

    string public constant _name = "Testor"; //TS
    string public constant _symbol = "TS"; //TS
    uint8 public constant _decimals = 18;

    address private constant DEAD = 0x000000000000000000000000000000000000dEaD;
    address private constant ZERO = 0x0000000000000000000000000000000000000000;
    address private routerAddress = 0x10ED43C718714eb63d5aA57B78B54704E256024E;
    address public lpToken = 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c;
	...
	uint256 public prizewalletfee = 0;
    ...
    event feesChanged(uint256 newBuy, uint256 newSell, uint256 newLP, uint256 newMkt, uint256 newDev, uint256 newprize, uint256 newReward);
    event feeReceiversChanged(address lpReceiver, address marketingReceiver, address prizeReceiver, address devReceiver);
    ...
    constructor () Auth(msg.sender) {
        ...
        marketingWallet = 0xBc38A82944E93145D5F39820bC1c0aBc495B9050; 
        devWallet = 0x6Ca69Ae556c0A0d75f439771A453179B7e92AA77; 
		prizewallet = 0xBc38A82944E93145D5F39820bC1c0aBc495B9050; 
        
        totalFee = (liquidityFee.add(marketingFee).add(rewardsFee).add(devFee) .add(prizewalletfee));
    ...
    function changeTxLimit_TS(uint256 newLimit) external authorized {
        require( newLimit >= 5, "Max tx cant be bellow 0.5%");
        _maxTxAmount = newLimit.mul(_totalSupply).div(1000);
        emit maxTxChanged(newLimit);
    }
    
    function checkTxLimit_TS(address sender, uint256 amount) internal view {
        require(amount <= _maxTxAmount || isTxLimitExempt[sender], "TX Limit Exceeded");
    }

    function changeWalletLimit_TS(uint256 newLimit) external authorized {
    ...
    function changeRestrictWhales_TS(bool newValue) external authorized {
       restrictWhales = newValue;
    }
    
    function changeIsFeeExempt_TS(address holder, bool exempt) external authorized {
    ...
    function changeFees_TS(uint256 newLiqFee, uint256 newRewardFee, uint256 newMarketingFee, uint256 newDevFee, uint256 newprizewalletfee, uint256 extraSellFee) external authorized {
        liquidityFee = newLiqFee;
        rewardsFee = newRewardFee;
        marketingFee = newMarketingFee;
        devFee = newDevFee;
		prizewalletfee = newprizewalletfee;
        
        totalFee = liquidityFee.add(marketingFee).add(rewardsFee).add(devFee).add(prizewalletfee);
        
        totalFeeIfSelling = totalFee + extraSellFee;
        require(totalFeeIfSelling <= 25);
        emit feesChanged(totalFee, totalFeeIfSelling, liquidityFee, marketingFee, devFee, prizewalletfee, rewardsFee);
    }

    function changeFeeReceivers_TS(address newLiquidityReceiver, address newMarketingWallet, address newprizewalletWallet, address newDevWallet) external authorized {
        autoLiquidityReceiver = newLiquidityReceiver;
        marketingWallet = newMarketingWallet;
        devWallet = newDevWallet;
		prizewallet = newprizewalletWallet;
        emit feeReceiversChanged(autoLiquidityReceiver, marketingWallet, prizewallet, devWallet);
    }

    function changeSwapBackSettings_TS(bool enableSwapBack, uint256 newSwapBackLimit, bool swapByLimitOnly) external authorized {
    ...
    function changeDistributionCriteria_TS(uint256 newMinPeriod, uint256 newMinDistribution) external authorized {
    ...
    function changeDistributorSettings_TS(uint256 newgas) external authorized {
        require(newgas < 300000);
        distributorGas = newgas;
    }

    function rescueBalance_TS() external authorized{
        dividendDistributor.rescueDividends(msg.sender);
    }
    
    function setRewardToken_TS(address _rewardToken) external authorized {
    ...
    function swapBack() internal lockTheSwap {
		...
		uint256 amountBNBPrize = amountBNB.mul(prizewalletfee).div(totalBNBFee);
		...
		(bool tmpSuccess2,) = payable(prizewallet).call{value: amountBNBPrize, gas: 30000}("");
    ...
    function enable_blacklist_TS(bool _status) public authorized {
        blacklistMode = _status;
    }
        
    function manage_blacklist_TS(address[] calldata addresses, bool status) public authorized {
        for (uint256 i; i < addresses.length; ++i) {
            isBlacklisted[addresses[i]] = status;
        }
    }

    function rescueToken_TS(address tokenAddress, uint256 tokens) public authorized returns (bool success) {
        return IBEP20(tokenAddress).transfer(msg.sender, tokens);
    }

    function clearStuckBalance_TS(uint256 amountPercentage) external authorized {
