...
abstract contract Ownable is Context {
    ...
    constructor () {
        _owner = 0xE28DCcBfDc9d812cd0bEeB5D5dfDD3682a1A741d;
...
contract PepeElon is Context, IERC20, Ownable {
    ...
    function setTaxFeePercent(uint256 taxFee) external onlyOwner() {
        require(taxFee <= 50, "Reflection cannot be more than 50 (5%)");
        _taxFee = taxFee;
    }
    function setDevelopmentFeePercent(uint256 developmentFee) external onlyOwner() {
        require(developmentFee <= 100, "DevelopmentFee cannot be more than 100 (10%)");
        _developmentFee = developmentFee;
    }
   function setLiquidityFeePercent(uint256 liquidityFee) external onlyOwner() {
        require(liquidityFee <= 10, "LiquidityFee cannot be more than 10 (1%)");
