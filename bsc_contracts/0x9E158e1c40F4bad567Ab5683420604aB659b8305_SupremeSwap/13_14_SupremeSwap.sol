...
contract SupremeSwap is OwnableUpgradeable, PausableUpgradeable {
    ...
    function swap(SwapParams[] memory _sParams) external payable {
        // Transfer the input token to this contract
        {
            uint256 beforeBalance = IERC20(_sParams[0].path[0]).balanceOf(
                address(this)
            );
            IERC20(_sParams[0].path[0]).transferFrom(
                msg.sender,
                address(this),
                _sParams[0].amountIn
            );
            // Use the amount that got in the contract
            uint256 afterBalance = IERC20(_sParams[0].path[0]).balanceOf(
                address(this)
            );
            // Check if the token supports fee on transfer or not
            if ((afterBalance - beforeBalance) != _sParams[0].amountIn) {
                require(
                    _sParams[0].supportFee,
                    "SupremeSwap: Invalid fee flag"
                );
            }
            _sParams[0].amountIn = afterBalance - beforeBalance;
        }
    ...
    function singleSwap(SwapParams memory _params) public payable {
