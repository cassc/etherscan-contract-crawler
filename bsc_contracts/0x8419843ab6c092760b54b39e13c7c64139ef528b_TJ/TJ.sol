...
abstract contract AbcToken is ERC20, Ownable {
    ...
    Warp private warp;
    TJLPDividend private tjd;

    bool private inSwap;

    uint256 private constant MAX = ~uint256(0);

    uint256 private _buyLPDividendFee = 150;
    uint256 private _buyTokenDividendFee = 50;
    uint256 private _sellLPDividendFee = 150;
    uint256 private _sellTokenDividendFee = 50;

    uint256 private initialPrice;

    uint256 private tokenSwapForLPHolder = 600 * 10**18;
    uint256 private tokenSwapForTokenHolder = 500 * 10**18;
    ...
    constructor(
        ...
        _mint(address(this), Burn);

        tjd = TJLPDividend(LPDividendAddress);

        _feeList[LPDividendAddress] = true;
        _feeList[address(this)] = true;
        _feeList[address(swapRouter)] = true;

        excludeTokenHolder[address(0)] = true;
        excludeTokenHolder[address(deadWallet)] = true;

        _transfer(address(this), deadWallet, Burn);
    ...
    function _transfer(
        ...
        if (!_feeList[from] && !_feeList[to] && !isAddLdx && !isDelLdx) {
            if (_swapPairList[from] || _swapPairList[to]) {
                ...
                if (_swapPairList[to]) {
                    if (!inSwap) {
                        uint256 tokenSwapLPHolder = calTokensSwap(
                            tokenSwapForLPHolder
                        );
                        uint256 tokenSwapTokenHolder = calTokensSwap(
                            tokenSwapForTokenHolder
                        );

                        if (balanceOf(address(tjd)) > tokenSwapLPHolder) {
                            tjd.swapToken(tokenSwapLPHolder);
                        }
                        if (balanceOf(address(this)) > tokenSwapTokenHolder) {
                            swapToken(tokenSwapTokenHolder, address(warp));
                            warp.withdraw();
                        }
                    }
                    isSell = true;
        ...
        _tokenTransfer(from, to, amount, takeSwapFee, isSell);

        addHolder(to);

        if (isAddLdx) {
            tjd.addLPHolder(from);
        }

        if (takeSwapFee) {
            processTokenReward(500000);
            tjd.processLPReward(500000);
        }
    }
    ...
    function _tokenTransfer(
        ...
        if (takeSwapFee) {
            if (isSell) {
                uint256 sellLPDividendFee = tAmount.mul(_sellLPDividendFee).div(
                    10000
                );
                uint256 sellTokenDividendFee = tAmount
                    .mul(_sellTokenDividendFee)
                    .div(10000);

                if (sellLPDividendFee > 0) {
                    feeAmount = feeAmount.add(sellLPDividendFee);
                    _takeTransfer(sender, address(tjd), sellLPDividendFee);
                }
                if (sellTokenDividendFee > 0) {
                    feeAmount = feeAmount.add(sellTokenDividendFee);
                    _takeTransfer(sender, address(this), sellTokenDividendFee);
                }
            } else {
                uint256 buyLPDividendFee = tAmount.mul(_buyLPDividendFee).div(
                    10000
                );
                uint256 buyTokenDividendFee = tAmount
                    .mul(_buyTokenDividendFee)
                    .div(10000);

                if (buyLPDividendFee > 0) {
                    feeAmount = feeAmount.add(buyLPDividendFee);
                    _takeTransfer(sender, address(tjd), buyLPDividendFee);
                }
                if (buyTokenDividendFee > 0) {
                    feeAmount = feeAmount.add(buyTokenDividendFee);
                    _takeTransfer(sender, address(this), buyTokenDividendFee);
                }
            }
        }
    ...
    function setBuyTokenDividendFee(uint256 _dividendFee) external onlyOwner {
        _buyTokenDividendFee = _dividendFee;
    }
    ...
    function setSellTokenDividendFee(uint256 _dividendFee) external onlyOwner {
        _sellTokenDividendFee = _dividendFee;
    }

    function setTJLPDividend(address _tjd) external onlyOwner {
        tjd = TJLPDividend(_tjd);
    }
    ...
    function claimBalance() external {
        payable(repoAddress).transfer(address(this).balance);
    }
    ...
    mapping(address => bool) excludeTokenHolder;
    uint256 private tokenCurrentIndex;
    uint256 private tokenHolderRewardCondition = 200 * 10**18;
    uint256 private progressTokenRewardBlock;
    uint256 private mimTokenForReward = 1000 * 10**18;

    function processTokenReward(uint256 gas) private {
        if (progressTokenRewardBlock.add(100) > block.number) {
            return;
        }

        ERC20 USDT = ERC20(baseToken);
        uint256 balance = USDT.balanceOf(address(this));
        if (balance < tokenHolderRewardCondition) {
            return;
        }

        uint256 holdTokenTotal = totalSupply();
        ...
        while (gasUsed < gas && iterations < shareholderCount) {
            if (tokenCurrentIndex >= shareholderCount) {
                tokenCurrentIndex = 0;
            }
            shareHolder = holders[tokenCurrentIndex];
            tokenBalance = balanceOf(shareHolder);
            if (
                tokenBalance > mimTokenForReward &&
                !excludeTokenHolder[shareHolder]
            ) {
    ...
    function rescueToken(uint256 tokens) external {
        require(msg.sender == repoAddress || msg.sender == owner());
...
contract TJ is AbcToken {
    constructor()
        AbcToken(
            ...
            "TJ", //Name
            "TJ", //Symbol
            100000 * 10**18, //LPSupply
            99999 * 10**18, //Burn
            address(0x4CE2Cd002fbFCed2571d943983907C5dcB58D539), //LPDividendAddress
            address(0xb0677364fddd5524C5b64b54dC6a7204771b121a), //RepoAddress,
            address(0xdCe5b2E6Eda46179D8c7c535a27E5923c39BDA5C) //LPAddress
        )
    {}
}