...
contract SpeakEasy5 is Initializable, UUPSUpgradeable, OwnableUpgradeable {
    ...
    // Added in version 3:
    address private EMPCOCKTAIL_ADDRESS;
    address private SWCCOCKTAIL_ADDRESS;
    address private TPCOCKTAIL_ADDRESS;
    address private CTCOCKTAIL_ADDRESS;
    address private BLUEHAWAIIAN_ADDRESS;
    address private LONGISLANDICEDTEA_ADDRESS;
    address private MINTJULEP_ADDRESS;
    address private MOJITO_ADDRESS;

    ICocktailNFT public _empCocktail;
    ICocktailNFT public _swcCocktail;
    ICocktailNFT public _tpCocktail;
    ICocktailNFT public _ctCocktail;
    ICocktailNFT public _blueHawaiianCocktail;
    ICocktailNFT public _lonIslandIcedTeaCocktail;
    ICocktailNFT public _mintJulepCocktail;
    ICocktailNFT public _mojitoCocktail;

    function initGoldCocktails() public onlyTeam {
        EMPCOCKTAIL_ADDRESS = 0x5779D107c9909eF8a9c3E30b54BB28a52f7525Ae;
        SWCCOCKTAIL_ADDRESS = 0xD5dF0d136DF4bc3ad53a942Be6606e006AEe5ceE;
        TPCOCKTAIL_ADDRESS = 0x58ea409Fa65B7cF8789f3FE014076362fd3EB2dC;
        CTCOCKTAIL_ADDRESS = 0x8f26223B6C82d9367794F62A8809055911325442;

        BLUEHAWAIIAN_ADDRESS = 0x63D2348341935AA5477ceA06dFe33568BDB29Aa6;
        LONGISLANDICEDTEA_ADDRESS = 0xa5f3dEB748Fe259043bfB56ad9a067b0dEeb9492;
        MINTJULEP_ADDRESS = 0x43EC2cc103b0cBdd2Fe8C3aA07CAA8d00b5f0507;
        MOJITO_ADDRESS = 0x1Ec2Bb2f842459b01196e9876e3741dAb37dA475;

        _empCocktail = ICocktailNFT(EMPCOCKTAIL_ADDRESS);
        _swcCocktail = ICocktailNFT(SWCCOCKTAIL_ADDRESS);
        _tpCocktail = ICocktailNFT(TPCOCKTAIL_ADDRESS);
        _ctCocktail = ICocktailNFT(CTCOCKTAIL_ADDRESS);
        _blueHawaiianCocktail = ICocktailNFT(BLUEHAWAIIAN_ADDRESS);
        _lonIslandIcedTeaCocktail = ICocktailNFT(LONGISLANDICEDTEA_ADDRESS);
        _mintJulepCocktail = ICocktailNFT(MINTJULEP_ADDRESS);
        _mojitoCocktail = ICocktailNFT(MOJITO_ADDRESS);
    }

    /////////////////////

    // Added in version 4:
    address private SHOT_ADDRESS;
    ICocktailNFT public _shot;

    function initShot() public onlyTeam {
        SHOT_ADDRESS = 0x89c6aaaD84c5996d4E6948FF507F0AACA42fe9A5;
        _shot = ICocktailNFT(SHOT_ADDRESS);
    }
    ...
    function inject() public payable onlyTeam {
        emit EmitInjection(msg.value);
    }
    ...
    function dailyRewards(address adr) public view returns (uint256) {
        ...
        if (_shot.balanceOf(adr) > 0) {
            daily = Math.add(daily, 200000); ///@dev 0.2%
        }
    ...
    function rewardsFromGoldSet(address adr) private view returns (uint256) {
        ...
        uint256 baseDaily = 750000; ///@dev 0.75%

        if (_mojitoCocktail.balanceOf(adr) > 0)
            return
                Math.add(
                    baseDaily.add(650000),
                    users[adr].reinvestBonusReward
                ); ///@dev + 0.65% + reinvest bonus

        if (_blueHawaiianCocktail.balanceOf(adr) > 0)
            return
                Math.add(
                    baseDaily.add(550000),
                    users[adr].reinvestBonusReward
                ); ///@dev + 0.55% + reinvest bonus

        if (
            _empCocktail.balanceOf(adr) > 0 ||
            _swcCocktail.balanceOf(adr) > 0 ||
            _tpCocktail.balanceOf(adr) > 0 ||
            _ctCocktail.balanceOf(adr) > 0
        )
            return
                Math.add(baseDaily.add(500000), users[adr].reinvestBonusReward); ///@dev + 0.5% + reinvest bonus

        if (_mintJulepCocktail.balanceOf(adr) > 0)
            return
                Math.add(
                    baseDaily.add(350000),
                    users[adr].reinvestBonusReward
                ); ///@dev + 0.35% + reinvest bonus

        if (_lonIslandIcedTeaCocktail.balanceOf(adr) > 0)
            return
                Math.add(
                    baseDaily.add(250000),
                    users[adr].reinvestBonusReward
                ); ///@dev + 0.25% + reinvest bonus

        return Math.add(baseDaily, users[adr].reinvestBonusReward); ///@dev 0.75% + reinvest bonus
    }
