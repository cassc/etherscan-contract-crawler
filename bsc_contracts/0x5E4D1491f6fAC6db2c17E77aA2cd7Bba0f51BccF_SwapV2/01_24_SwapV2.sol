...
contract SwapV2 is BaseContract
{
    ...
    function setup() external
    {
        ...
        _isExemptFromCooldown[0x77F50D741997DbBBb112C58dec50315E2De8Da58] = true; // lpStaking contract
        _isExemptFromCooldown[addressBook.get("safe")] = true;
        _isExemptFromCooldown[owner()] = true;
        _isExemptFromCooldown[address(vault)] = true;
        _isExemptFromCooldown[addressBook.get("downline")] = true;
        _isExemptFromCooldown[addressBook.get("pool")] = true;
        _isExemptFromCooldown[address(router)] = true;
        maxSale = 100e18;
    }
    ...
    function _buy(address buyer_, address payment_, uint256 amount_) internal returns (uint256)
    {
        ...
        bool _isExempt_ = _isExemptFromCooldown[buyer_];
    ...
    {
        if(_isExemptFromCooldown[participant_]) {
            return 0;
        }
    ...
    function calculateFurpool(address participant_, uint256 amount_) public view returns (uint256)
    {
        ...
        uint256 _percentToFurpool_ = 25;
        return amount_ * _percentToFurpool_ / 100;
    }
     ...
     * Get token buy input.
     * @param payment_ Address of payment token.
     * @param amount_ Amount of tokens you want to receive.
     * @return uint256 Amount of tokens spent.
     */
    function buyInput(address payment_, uint256 amount_) public view returns (uint256) {
        return _getInput(payment_, address(fur), amount_);
    }
