...
contract Token is Context, ITRC20, Ownable {
    ...
    string private _name = 'MakerDAO';
    string private _symbol = 'MKII';
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) private {
        ...
        (uint256 tTransferAmount,  uint256 tLiquidity, uint256 tBurn) = _getValues(tAmount);
    ...
    function _getValues(uint256 tAmount) private view returns (uint256, uint256, uint256) {
        (uint256 tTransferAmount, uint256 tLiquidity, uint256 tBurn) = _getTValues(tAmount);
