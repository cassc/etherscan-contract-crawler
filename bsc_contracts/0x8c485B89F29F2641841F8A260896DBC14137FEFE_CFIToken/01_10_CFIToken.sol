...
contract Wrap {
    ...
    constructor(IERC20 cfi_, IERC20 usdt_) {
        cfi = cfi_;
        usdt = usdt_;
    }
    ...
    function withdraw() external {
        ...
        if (usdtBalance > 0) {
            usdt.transfer(address(cfi), usdtBalance);
        }
    }
}

contract CFIToken is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    uint256 public immutable lpRewardFee = 4;
    ...
    constructor(
        ...
        swapTokensAtAmount = totalSupply.mul(50).div(10**6); // 0.05%;
        ...
        returns (uint256 amountAfter)
    {
        ...
        //回购
        uint256 BBFee = amount.mul(buybackFee).div(100);
        ...
        super._transfer(
            from,
            address(this),
            LPRFee.add(MFee).add(BBFee).add(LPFee)
        );
    }
