...
// 
// 📱 Telegram: https://t.me/Turtles_ceo
// 🌎 Website: http://www.turtles.bio
// 🌐 Twitter: https://twitter.com/TurtlesCEO
...
contract Turtles is ERC20, Ownable {
    ...
    uint256 public  rewardFeeOnBuy      = 3;
    uint256 public  rewardFeeOnSell     = 3;

    uint256 public  marketingFeeOnBuy   = 7;//It's includes the development tax rate
    uint256 public  marketingFeeOnSell  = 7;

    uint256 private _totalFeesOnBuy     = 10;
    uint256 private _totalFeesOnSell    = 10;

    uint256 private devRate = 25; // 7% * 25%

    address public marketingWallet = 0x9A34E067aBA6d7f931584E08B6065468D17e8883;
    address public devWallet = 0xeDB5636B48e053AEEeC88B688f572B08a9220AC0;
    ...
    event UpdateBuyFees(uint256 marketingFeeOnBuy, uint256 rewardFeeOnBuy);
    event UpdateSellFees(uint256 marketingFeeOnSell, uint256 rewardFeeOnSell);
    ...
    constructor() ERC20("Turtles Inu", "Turtles") 
    {
    ...
    function updateBuyFees(uint256 _marketingFeeOnBuy, uint256 _rewardFeeOnBuy) external onlyOwner {
        require(
            _marketingFeeOnBuy + _rewardFeeOnBuy <= 10,
            "Fees must be less than 10%"
        );
        rewardFeeOnBuy    = _rewardFeeOnBuy;
        marketingFeeOnBuy = _marketingFeeOnBuy;
        _totalFeesOnBuy   = marketingFeeOnBuy + rewardFeeOnBuy;
        emit UpdateBuyFees(_marketingFeeOnBuy, _rewardFeeOnBuy);
    }

    function updateSellFees(uint256 _marketingFeeOnSell, uint256 _rewardFeeOnSell) external onlyOwner {
        require(
            _marketingFeeOnSell + _rewardFeeOnSell <= 10,
            "Fees must be less than 10%"
        );        
    ...
    function changeDevRate(uint256 _devRate) external onlyOwner{
        require(_devRate <= 100,"devRate not in 0~100");
        require(_devRate >= 0 ,"devRate not in 0~100");
        devRate = _devRate;
    }
        ...
        if( canSwap &&
            ...
            if(contractTokenBalance > 0 && bnbShare > 0) {
                ...
                if(marketingShare > 0) {
                    ...
                    uint256 devBNB =(marketingBNB * devRate)/100;
                    payable(devWallet).sendValue(devBNB);
                    payable(marketingWallet).sendValue(marketingBNB-devBNB);
