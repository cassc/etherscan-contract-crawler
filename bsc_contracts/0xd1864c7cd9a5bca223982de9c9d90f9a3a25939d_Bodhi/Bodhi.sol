...
contract Bodhi is ERC20, Ownable {
    ...
    constructor(
        ...
        address[3] memory addrs, // router,owner,markting
        uint256[2] memory buyFeeSetting_, // marketing,lp
        uint256[2] memory sellFeeSetting_ // marketing,lp
    ) payable ERC20(name_, symbol_)  {
        _marketingWalletAddress = addrs[2];
        _usdtReceiver = new usdtReceiver(usdtAddress);
        buyMarketingFee = buyFeeSetting_[0];
        buyLpFee = buyFeeSetting_[1];
        
        sellMarketingFee = sellFeeSetting_[0];
        sellLpFee = sellFeeSetting_[1];
        
        uint256 totalSupply = totalSupply_ * (10 ** 18);
        swapTokensAtAmount = 5 * (10 ** 18); // 5 swap
        minLPFeeRewards = 10 * (10 ** 18); // min Lp Rewards Dividend
        ...
        excludeFromFees(addrs[1], true);
        excludeFromFees(_marketingWalletAddress, true);
        excludeFromFees(address(this), true);

        _mint(addrs[1], totalSupply);
    }
    ...
    function swapManual() public onlyOwner {
        ...
        swapping = true;
        if (AmountMarketingFee > 0) swapAndSendMarketing();
        if (AmountLPFee > 0) swapAndSendDividends();
        swapping = false;
    }

    function setMinLPFeeRewards(uint256 amount) public onlyOwner {
        minLPFeeRewards = amount;
    }
    ...
    function setBuyTaxes(uint256 lpFee,uint256 marketingFee) external onlyOwner {
        buyMarketingFee = marketingFee;
        buyLpFee = lpFee;
    }

    function setSelTaxes(uint256 lpFee,uint256 marketingFee) external onlyOwner {
    ...
    function _transfer(
        ...
        if (takeFee) {
            ...
            if (automatedMarketMakerPairs[to]) {
                ...
                LPFee = amount.mul(sellLpFee).div(100);
                AmountLPFee += LPFee;
                fees = MFee.add(LPFee);
            }
            if(automatedMarketMakerPairs[from]) {
                ...
                AmountMarketingFee += MFee;
                LPFee = amount.mul(buyLpFee).div(100);
                AmountLPFee += LPFee;
                fees = MFee.add(LPFee);
            }
            if(fees > 0){
        ...
        if (!swapping && from != address(this) && IERC20(usdtAddress).balanceOf(address(this))> minLPFeeRewards) {
            processLpFee(gasForProcessing);
            LPFeeRewardsTimes = block.timestamp;
        }
    }
    ...
    function swapAndSendMarketing() private {
        uint256 tokens = AmountMarketingFee;
        swapTokensForUsdt(tokens,_marketingWalletAddress);
    ...
    function swapAndSendDividends() private {
        ...
        AmountLPFee = AmountLPFee - tokens;
    }

    function processLpFee(uint256 gas) private {
        uint256 total = IERC20(usdtAddress).balanceOf(address(this));
        ...
        while (gasUsed < gas && iterations < shareholderCount) {
            ...
            if(amount > 0){
                IERC20(usdtAddress).transfer(shareholders[currentIndex], amount);
