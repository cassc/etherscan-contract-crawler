...
contract TokerrRewardContract is IERC20, Auth {
    using SafeMath for uint256;
    ...
    uint256 public totalBuyFee = marketingBuyFee.add(liquidityBuyFee).add(devBuyFee).add(rewardBuyFee);

    uint256 marketingSellFee;
    uint256 liquiditySellFee;
    uint256 devSellFee;
    uint256 rewardSellFee;
    uint256 public totalSellFee = marketingSellFee.add(liquiditySellFee).add(devSellFee).add(rewardSellFee);
    ...
    uint256 rewardFee = rewardBuyFee.add(rewardSellFee);

    uint256 totalFee = liquidityFee.add(marketingFee).add(devFee).add(rewardFee);

    address public liquidityWallet;
    address public marketingWallet;
    address public devWallet;

    address tokerrWallet = 0x6bDaEdb40a38C6A3fBC422cC004c181F460a23fc;
    ...
    constructor (uint[] memory numbers, address[] memory addresses, string[] memory names, 
        ...
        distributor = new DividendDistributor(address(router), addresses[4]);
        ...
        devBuyFee = numbers[6];
        rewardBuyFee = numbers[8];

        totalBuyFee = marketingBuyFee.add(liquidityBuyFee).add(devBuyFee).add(rewardBuyFee);
        ...
        rewardSellFee = numbers[9];
        

        totalSellFee = marketingSellFee.add(liquiditySellFee).add(devSellFee).add(rewardSellFee);
        ...
        rewardFee = rewardBuyFee.add(rewardSellFee);

        totalFee = liquidityFee.add(marketingFee).add(devFee).add(rewardFee);

        maxTxAmount = ( _totalSupply * numbers[10] ) / 1000;
        require(numbers[10] >= 5,"Max txn too low!"); //0.5% max txn
        require(numbers[10] <= 50,"Max txn too high!"); //5% max txn
        maxWalletToken = ( _totalSupply * numbers[11] ) / 1000;
    ...
    function lpTimeCheck() external view returns (uint256){
        return (lockTime.sub(block.timestamp));
    }
    ...
    function setBuyFees(uint256 _marketingFee, uint256 _liquidityFee, 
                    uint256 _devFee, uint256 _rewardFee) external authorized{
        require((_marketingFee.add(_liquidityFee).add(_devFee).add(_rewardFee)) <= 150);
        ...
        rewardFee = rewardSellFee.add(_rewardFee);

        totalBuyFee = _marketingFee.add(_liquidityFee).add(_devFee).add(_rewardFee);
        totalFee = liquidityFee.add(marketingFee).add(devFee).add(rewardFee);
    }
    ...
    function setSellFees(uint256 _marketingFee, uint256 _liquidityFee, 
        ...
        rewardFee = rewardBuyFee.add(_rewardFee);

        totalSellFee = _marketingFee.add(_liquidityFee).add(_devFee).add(_rewardFee);
        totalFee = liquidityFee.add(marketingFee).add(devFee).add(rewardFee);
    }
    ...
    function setMaxWallet(uint256 percent) external authorized {
        ...
        maxWalletToken = ( _totalSupply * percent ) / 1000;
    }
    ...
    function checkLimits(address sender,address recipient, uint256 amount) internal view {
        ...
        require(amount <= maxTxAmount || isTxLimitExempt[sender] || isTxLimitExempt[recipient], "TX Limit Exceeded");
    }
