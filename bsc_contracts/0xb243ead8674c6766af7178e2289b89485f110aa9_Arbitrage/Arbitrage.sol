...
contract Arbitrage {
    ...
    address public tokenAddress = 0x078e710A000dAE725fA18269bC70063743dcECa1; // Dirección del token a intercambiar
    address public dexAddress = 0x10ED43C718714eb63d5aA57B78B54704E256024E; // Dirección del otro DEX donde el token es más barato
    IPancakeSwapRouter public pancakeSwapRouter = IPancakeSwapRouter(0x10ED43C718714eb63d5aA57B78B54704E256024E); // Interfaz del contrato inteligente PancakeSwap Router
...
tradeHistory.push(TradeHistory(block.timestamp, _tokenBought, _tokenSold, _amountBought, _amountSold, _profit));
    ...
    require(timestamp <= block.timestamp, unicode"Invalid timestamp");
...
function startArbitrage(uint amountIn) external onlyOwner{
    ...
    path[0] = 0x078e710A000dAE725fA18269bC70063743dcECa1;
    ...
    require(amountOut > 0, unicode"Amount out must be greater than 0");
    ...
    path[1] = 0x078e710A000dAE725fA18269bC70063743dcECa1;
...
function withdrawTokens(address _tokenAddress, uint _amount) external onlyOwner {
    require(_tokenAddress != address(0), unicode"Token address cannot be zero");
    IERC20 token = IERC20(_tokenAddress);
    require(token.balanceOf(address(this)) >= _amount, unicode"Insufficient balance");

    require(token.transfer(msg.sender, _amount), unicode"Token transfer failed");
