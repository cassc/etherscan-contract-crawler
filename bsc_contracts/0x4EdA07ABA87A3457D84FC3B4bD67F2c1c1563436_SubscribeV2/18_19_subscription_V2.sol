...
contract SubscribeV2 is Subscribe {
     ...
     * @param _tariffId Identifier of tariff
     */
    function set_userTariff(uint8 _tariffId) external {
        userTariff[msg.sender] = _tariffId;
    }
     ...
     */
    function writeOffTariff(address _addr) internal onlyForUser(_addr) {
    ...
    function writeOffAfterRenewTariff(address _addr) internal {
        User memory _user = users[_addr];
