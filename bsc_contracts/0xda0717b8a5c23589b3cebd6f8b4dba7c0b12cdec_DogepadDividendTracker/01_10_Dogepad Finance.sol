...
contract DogepadFinance is ERC20, Ownable {
    ...
    DogepadDividendTracker public dividendTracker;

    address public constant deadWallet = 0x000000000000000000000000000000000000dEaD;
    address public marketingWallet = 0x77ff0a60715B3976cb0fcdD5fdbC5Ee3F2973C9B;
    ...
    constructor() ERC20("Dogepad Finance", "DPF") {
        dividendTracker = new DogepadDividendTracker();
        ...
        */
        _tokengeneration(owner(), 1e7 * (10**9));
    }

    receive() external payable {}

    function updateDividendTracker(address newAddress) public onlyOwner {
        DogepadDividendTracker newDividendTracker = DogepadDividendTracker(
            payable(newAddress)
        );
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(
            ...
            "Dogepad: Account is already the value of 'excluded'"
        );
        _isExcludedFromFees[account] = excluded;
    ...
    function setSwapTokensAtAmount(uint256 amount) external onlyOwner {
        require(amount < 1e5,"Swap Threshold should be less than 1% of total supply");
    ...
    function setBuyTaxes(uint256 _rewards, uint256 _marketing, uint256 _liquidity) external onlyOwner{
        ...
        require((_rewards + _marketing + _liquidity ) <= 10, "Must keep fees at 10% or less");
    }
    ...
    function _setAutomatedMarketMakerPair(address newPair, bool value) private {
        require(
            ...
            "Dogepad: Automated market maker pair is already set to that value"
        );
        automatedMarketMakerPairs[newPair] = value;
    ...
    function setGasForProcessing(uint256 newValue) external onlyOwner {
        require(
            ...
            "Dogepad: gasForProcessing must be between 200,000 and 500,000"
        );
        require(
            newValue != gasForProcessing,
            "Dogepad: Cannot update gasForProcessing to same value"
        );
...
contract DogepadDividendTracker is Ownable, DividendPayingToken {
    ...
    constructor()
        DividendPayingToken("Dogepad_Dividen_Tracker", "Dogepad_Dividend_Tracker")
    {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 1000 * (10**decimals()); 
    }
    ...
    function _transfer(
        ...
        require(false, "Dogepad_Dividend_Tracker: No transfers allowed");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(
            ...
            "Dogepad_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours"
        );
        require(
            newClaimWait != claimWait,
            "Dogepad_Dividend_Tracker: Cannot update claimWait to same value"
        );
