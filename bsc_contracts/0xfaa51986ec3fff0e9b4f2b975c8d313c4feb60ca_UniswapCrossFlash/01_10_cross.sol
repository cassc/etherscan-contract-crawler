...
contract UniswapCrossFlash {
    ...
    function startArbitrage(address _tokenBorrow,address _swapToken, uint256 _amount) external {
        IERC20(_swapToken).safeApprove(address(PANCAKE_ROUTER), MAX_INT);
        IERC20(_tokenBorrow).safeApprove(address(PANCAKE_ROUTER), MAX_INT);
        IERC20(WBNB).safeApprove(address(PANCAKE_ROUTER), MAX_INT);

        IERC20(WBNB).safeApprove(address(APE_ROUTER), MAX_INT);
        IERC20(_swapToken).safeApprove(address(APE_ROUTER), MAX_INT);
        ...
        bytes memory data = abi.encode(_tokenBorrow,_swapToken, _amount, msg.sender);
    ...
    function pancakeCall(
        ...
        (address tokenBorrow,address swapToken, uint256 amount, address myAddress) = abi.decode(
            _data,
