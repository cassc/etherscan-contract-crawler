...
// File contracts/pandalock/PandaLock.sol
...
contract PandaLock {
    ...
    function lock(
        ...
        emit LockAdded(id, token, owner, amount, unlockDate, title);
        return id;
    }
    ...
    function vestingLock(
        ...
        emit LockAdded(id, token, owner, amount, tgeDate, title);
        return id;
    }
    ...
    function editLockTitle(uint256 lockId, string memory title) external {
        Lock storage userLock = _locks[lockId];
        require(
            userLock.owner == msg.sender,
            "You are not the owner of this lock"
        );
        userLock.title = title;
