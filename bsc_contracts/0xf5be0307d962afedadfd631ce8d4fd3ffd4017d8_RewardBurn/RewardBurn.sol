...
contract RewardBurn is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    RewardBurnDividendTracker public dividendTracker;
    ...
    constructor() ERC20("Reward Burn", "RBC") {
        marketingWallet = payable(0x16901DDed47c9294b5b8DDB7a775D0bc4b9aAa0f);
        devWallet = payable(0x75F808685f2083F5bb6ed194Ae20Fb26dFD460F7);
        ...
        dividendTracker = new RewardBurnDividendTracker(
            payable(this),
            router,
            0x16fC5da7976dEb95188c14757e8504c86DB0359A,
            "RewardBurnTRACKER",
            "RBCTRACKER"
        );
...
contract RewardBurnDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    RewardBurn public RewardBurnContract;
    ...
    constructor(
        ...
        defaultToken = token;
        RewardBurnContract = RewardBurn(mainContract);
    ...
    function _transfer(
        ...
        require(false, "RewardBurn_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "RewardBurn_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main rewardburn contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(RewardBurnContract);

            uint256 prevBalance = RewardBurnContract.balanceOf(address(this));
            ...
            {
                uint256 received = RewardBurnContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    RewardBurnContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
