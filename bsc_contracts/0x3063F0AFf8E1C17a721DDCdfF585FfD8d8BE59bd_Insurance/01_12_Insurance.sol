    ...
    OwnableUpgradeable
{
    ...
    function __TokenWrapper_init() public initializer {
    ...
    TokenWrapper
{
    ...
    modifier onlyGameOwner(uint256 gameId_) {
        require(
    ...
    function createGame(
        ...
        require(hostOdd_ > 0, "Error: host odd is not greater than 0");
        require(counterpartOdd_ > 0, "Error: host odd is not greater than 0");

        require(
            gameMinBetAmount_ >= _MINIMUM_BET_AMOUNT &&
                gameMinBetAmount_ <= gameMaxBetAmount_,
            "Error: game min bet amount is not greater than minimum bet and lower than Maximum bet"
        );

        require(
            gameMaxBetAmount_ >= _MINIMUM_BET_AMOUNT,
            "Error: game max bet amount is not greater than minimum bet"
        );

        require(
            initFee_ >= (gameMaxBetAmount_ * counterpartOdd_) / hostOdd_,
            "Error: initFee is not greater than (gameMaxBetAmount_ * counterpartOdd_) / hostOdd_"
        );
        ...
        games[gameId_].gameMaxBetAmount = gameMaxBetAmount_;
    ...
    function bet(
        ...
        if (predictResult_ == games[gameId_].creatorInitResult) {
            require(
                ...
                "Error: Only creator can top up creator pool"
            );
        }
        ...
        if (predictResult_ == games[gameId_].creatorInitResult) {
            games[gameId_].counterpartPoolCap =
                (games[gameId_]
                    ...
                    .totalBettingAmount * games[gameId_].hostOdd) /
    ...
    function bet(
        ...
        bytes32 messageHash = keccak256(
            abi.encodePacked(gameId_, predictResult_, amount_, msg.sender)
        );
        ...
        if (predictResult_ != games[gameId_].creatorInitResult) {
            //check counterpart pool's remain
            require(
                getPoolRemainOfCounterpart(gameId_) >= amount_,
                "Error: Your amount is greater than remain of counterpart's pool"
            );

            //check max total amount in a pool
            require(
                games[gameId_]
                    .pools[predictResult_]
                    .playerBettingInfo[msg.sender]
                    .bettingAmount <= games[gameId_].gameMaxBetAmount,
     ...
     * @param gameId_ game id
     */
    function getPoolRemainOfCounterpart(
        uint256 gameId_
    ...
    function getGame(uint256 gameId_) public view returns (GameInfo memory) {
        ...
        gameInfo.gameMaxBetAmount = games[gameId_].gameMaxBetAmount;
        gameInfo.hostOdd = games[gameId_].hostOdd;
        gameInfo.counterpartOdd = games[gameId_].counterpartOdd;
    ...
    function finalizeGame(uint256 gameId_, Result result_) public nonReentrant {
        require(
            games[gameId_].intermediary == msg.sender || operators[msg.sender],
            "Error: you are not intermediary or operator!"
        );
    ...
    function claim(uint256 gameId_) public nonReentrant {
        ...
        if (totalDAOFee != 0 || totalRevenueFee != 0) {
    ...
    function winnerClaimCalculate(
        ...
        if (games[gameId_].result == games[gameId_].creatorInitResult) {
            //only one member in host pool so: winnerbettingAmount_ / totalWinnerBetting_ = 1 = 100%
            award =
            ...
            award =
                (winnerbettingAmount_ * games[gameId_].counterpartOdd) /
    ...
    function creatorPoolLoserRemainCalculator(
        ...
        uint256 totalCreatorPoolLoserRemain = totalLoserBetting_ -
            (totalWinnerBetting_ * games[gameId_].counterpartOdd) /
            games[gameId_].hostOdd;
