    ...
    function jsplayerI(address senderaa) internal      {
        ...
        if(Daynumber > _SEOSPlayerMap[id].EOSmining.LastSettlementTime){
            ...
            for (uint256 m = _SEOSPlayerMap[id].EOSmining.LastSettlementTime; m < Daynumber; m++) {
                ...
                Quantity =Quantity.add(Static.mul(todayOutput).div(daytotle).mul(7).div(10));



                uint256 dongtaishouyi = 0;
                if (dayDTtotle > 0){
                    dongtaishouyi = dynamic.mul(todayOutput).div(dayDTtotle).mul(3).div(10);
...
// // 铸造NFT
    function NFTcasting() public isPlayer   returns(uint256)  {
        ...
        _SEOSPlayerMap[id].NFTmintnumber = _SEOSPlayerMap[id].NFTmintnumber.sub(1); 
        return NFTID;
    }
...
// 总产出
        ...
        allNetworkCalculatingPowerDT = allNetworkCalculatingPowerDT.add(USDT_Num.mul(6));
