...
contract BitXStaking is ReentrancyGuardUpgradeable, OwnableUpgradeable {
    ...
    uint256 public miniStakingTime;
    uint256 apy;
    uint256 month;
    uint256 day;
    uint256 reward;
    ...
    event ReferralsAwarded(
        uint256 referralReward1,
        uint256 referralReward2,
        uint256 referralReward3
    );
    ...
    function __BitXStaking_init(IERC20 _bxg, IBitXGoldSwap _pool)
        external
        initializer
    {
        __Ownable_init();
    ...
    function addReferral(address[] memory _referrals, address staker) external {
        require(
            StakedReferrals[staker].firstReferral == address(0),
            "Referrals already added!"
        );
    ...
    function minStakingTime(uint256 _miniStakingTime) external onlyOwner {
        miniStakingTime = _miniStakingTime;
    }
    ...
    function stake(uint256 _amount) external nonReentrant returns (uint256) {
        ...
        require(
            (Staker[msg.sender].amountStaked + _amount) >= 20e18,
            "Amount should be atleast 20 BXG!"
        );
        // Transfer the token from the wallet to the Smart contract
        bxg.transferFrom(msg.sender, address(this), _amount);

        // Create staker if he doesn't exist
        bool referralsAdded = StakedReferrals[msg.sender].firstReferral ==
            address(0)
            ? false
            : true;
        if (Staker[msg.sender].staker != msg.sender) {
            Staker[msg.sender] = StakerStruct(
                msg.sender,
                _amount,
                1,
                referralsAdded,
            ...
            Staker[msg.sender].amountStaked =
                Staker[msg.sender].amountStaked +
                _amount;
    ...
    function unStake(uint256 _amount, uint256 _tokensId) external nonReentrant {
        require(
            block.timestamp - StakedTokens[msg.sender][_tokensId].startingTime <
                miniStakingTime,
            "Your reward is ready, claim it!"
        );
        require(
            StakedTokens[msg.sender][_tokensId].amountStaked >= _amount,
            "Amount being unstaked is too much!"
        );
        ...
        Staker[msg.sender].amountStaked =
            Staker[msg.sender].amountStaked -
            _amount;
        StakedTokens[msg.sender][_tokensId].amountStaked =
            StakedTokens[msg.sender][_tokensId].amountStaked -
            _amount;
        if (StakedTokens[msg.sender][_tokensId].amountStaked == 0)
            delete StakedTokens[msg.sender][_tokensId];

        emit Unstake(msg.sender, _amount);
    }

    // @param _amount The amount of reward to withdraw
    function withdraw(uint256 _amount, uint256 _tokensId)
        external
        nonReentrant
    {
        ...
        require(
            StakedTokens[msg.sender][_tokensId].amountStaked >= _amount,
            "Amount being withdrawen is too much!"
        );

        uint256 timeStaked = block.timestamp -
            StakedTokens[msg.sender][_tokensId].startingTime;
        //Check that it has been atleast three months
        require(
            timeStaked >= miniStakingTime,
            "It hasn't been one month since staking"
        );
    ...
    function calculateRewards(uint256 _amount, uint256 _timeStaked)
        public
        view
        returns (uint256)
    {
    ...
    function awardReferrals(address _staker, uint256 _amount) internal {
        ...
        if (
            ...
            pool.transferReward(
                StakedReferrals[_staker].firstReferral,
                reward1
            );
        }
        // Incase of only two referrals
            ...
            pool.transferReward(
                StakedReferrals[_staker].secondReferral,
                reward2
            );
            ...
            pool.transferReward(
                StakedReferrals[_staker].thirdReferral,
                reward3
            );
        }
