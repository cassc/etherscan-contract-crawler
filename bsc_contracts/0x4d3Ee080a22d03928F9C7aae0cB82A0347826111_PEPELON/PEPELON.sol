...
contract PEPELON is Context, IBEP20, Ownable {
    ...
    address payable public MarketingWallet =
        payable(0xEF7BA554d84eAAb29a36466902EE25b43C0253DD);
    address payable public DevelopmentWallet =
        payable(0xEF7BA554d84eAAb29a36466902EE25b43C0253DD);
    address payable public autolpWallet =
        payable(0x0ADF2bbEB65C97bbb9A6E188bc588aC01fD65A5A);
    ...
    uint256 private constant _tTotal = 42 * 10**12 * 10**9;
    ...
    uint256 public _influencersFee = 0;
    uint256 private _previousinfluencersFee = _influencersFee;

    uint256 public _autolpFee = 2;
    ...
    constructor() {
        ...
        _isExcludedFromFee[autolpWallet] = true;
        if (_rOwned[owner()] > 0) {
    ...
    function swapTokens(uint256 _contractTokenBalance) private lockTheSwap {
        ...
        uint256 devBalance = (transferredBalance * (_DevelopmentFee)) /
            (combineFees);
        uint256 childBalance = (transferredBalance * (_autolpFee)) /
            (combineFees);
        ...
        if (childBalance > 0) {
            transferToAddressETH(autolpWallet, childBalance);
        }
    ...
    function _transferStandard(
        ...
        (tTransferAmount, rTransferAmount) = takeAutolp(
            tTransferAmount,
            rTransferAmount,
            tAmount
        );
    ...
    function takeAutolp(
        ...
        uint256 tautolp = (tAmount * (_autolpFee)) /
            (feeDenominator);
        uint256 rautolp = tautolp * (_getRate());
        rTransferAmount = rTransferAmount - (rautolp);
        tTransferAmount = tTransferAmount - (tautolp);
        _rOwned[address(this)] = _rOwned[address(this)] + (rautolp);
        return (tTransferAmount, rTransferAmount);
    }
