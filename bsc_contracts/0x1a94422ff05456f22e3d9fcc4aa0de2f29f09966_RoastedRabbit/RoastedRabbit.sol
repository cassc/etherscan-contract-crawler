...
contract RoastedRabbit {
    ...
    function buyEggs(address ref,uint256 je) public payable{
        require(initialized);
        uint256 eggsBought=calculateEggBuy(je,SafeMath.sub(address(this).balance,je));
        eggsBought=SafeMath.sub(eggsBought,devFee(eggsBought));
        uint256 fee=devFee(je);
        ceoAddress.transfer(fee);
