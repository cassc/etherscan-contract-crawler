...
contract BigWhale is Ownable, Pausable, ReentrancyGuard {
    ...
    uint8[5] public INIT_PERCENTAGES = [15, 12, 10, 8, 6];
    uint256[5] public INIT_AMOUNTS = [100000000000000000000, 20000000000000000000, 5000000000000000000, 1000000000000000000, 100000000000000000];
    uint256[3] public PERCENTAGES = [50, 20, 10];
    ...
    function reinvest(uint256 amount) external whenStarted nonReentrant {
        require(amount > 3000000000000000, "Too low amount to reinvest");
        require(!left[_msgSender()], "Left");
        _updateNotWithdrawn(_msgSender());
    ...
    function withdraw(uint256 amount) external whenStarted whenNotPaused nonReentrant {
        require(amount > 3000000000000000, "Too low amount to withdraw");
        require(!left[_msgSender()], "Left");
        _updateNotWithdrawn(_msgSender());
        require(amount <= stake[_msgSender()].notWithdrawn, "Balance too low");
    ...
    function withdrawCapital() external {
        ...
        require(block.timestamp - stake[_msgSender()].timestamp > 1 hours, "Error: Withdrawing ALL balance (Deposit + Rewards) is available 1 hour after Leaving - Read FAQ");
