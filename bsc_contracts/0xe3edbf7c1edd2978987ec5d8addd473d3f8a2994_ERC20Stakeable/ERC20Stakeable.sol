...
contract ERC20Stakeable is ReentrancyGuard, Ownable {
    ...
    uint256 public rewardsPerHour = 959; // 0.00959%/h or 84% APR

    // Minimum amount to stake
    uint256 public minStake = 250 * 10**6;

    // Minimum amount to stake
    uint256 public maxStake = 5000 * 10**6;


    // Compounding frequency limit in seconds
    uint256 public lockPeriod = 3153600 ; //1 year
    ...
    function deposit(uint256 _amount) external nonReentrant {
        ...
        require(_amount <= maxStake, "Amount larger than maximum deposit");
        require(
    ...
    function withdraw(uint256 _amount) external nonReentrant {
        require(stakers[msg.sender].timeOfLastUpdate + lockPeriod < block.timestamp,
        "Lock period is active");
        require(
            stakers[msg.sender].deposited >= _amount,
    ...
    // Set the minimum and maximum amount for staking in wei
    function setMinMaxStake(uint256 _minStake, uint256 _maxStake) public onlyOwner {
        minStake = _minStake;
        maxStake = _maxStake;
    }
    ...
    // Set the number of days in seconds before deposit can be withdrawn
