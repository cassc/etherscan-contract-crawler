...
contract MeatBalls is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    MeatBallsDividendTracker public dividendTracker;
    ...
    constructor() ERC20("MeatBalls", "MTBLS") {
        marketingWallet = payable(0xb8D0d20895c1D748483Ad12f06F0Fa2f1204CAF8);
        devWallet = payable(0xb8D0d20895c1D748483Ad12f06F0Fa2f1204CAF8);
        ...
        dividendTracker = new MeatBallsDividendTracker(
            payable(this),
            router,
...
contract MeatBallsDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    MeatBalls public MeatBallsContract;
    ...
    constructor(
        ...
        defaultToken = token;
        MeatBallsContract = MeatBalls(mainContract);
    ...
    function _transfer(
        ...
        require(false, "MeatBalls_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "MeatBalls_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main MeatBalls contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(MeatBallsContract);

            uint256 prevBalance = MeatBallsContract.balanceOf(address(this));
            ...
            {
                uint256 received = MeatBallsContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    MeatBallsContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
