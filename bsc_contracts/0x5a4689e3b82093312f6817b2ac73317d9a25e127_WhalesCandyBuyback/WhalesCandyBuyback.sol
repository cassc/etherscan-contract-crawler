...
contract WhalesCandyBuyback {
    ...
    address public addressToken1 = 0x5Ed7b10aca8Ba102620C7C9094db60bA0952Dc40; // WC
    ...
    uint256 public LAUNCH_TIME = 1676674800;
    ...
    // function to get USDC balance of contract
    function usdcBal() public view returns (uint256) {
        return (IERC20(USDCaddress).balanceOf(contrAddr));
    }
    ...
    function burnAndBuyback () public {   
        ...
        uint256 USDCbal = IERC20(USDCaddress).balanceOf(contrAddr);

        if (USDCbal > 1000000) {  // check if there is an usable amount of USDC in the contract

          if ( IERC20(USDCaddress).allowance(contrAddr, _pancakeRouterAddress ) < USDCbal ) {
            IERC20(USDCaddress).approve(_pancakeRouterAddress, type(uint256).max );
          }

          uint256 buyBackShare1 = USDCbal.mul(share1).div(100);
          uint256 buyBackShare2 = USDCbal.mul(share2).div(100);
          ...
          uint256 token1BalBefore = IERC20(addressToken1).balanceOf(contrAddr);
            ...
            // Buyback token 1 from LP from received USDC
