...
/**
* BonkBSC.com - The first BONK on Binance Smart Chain
*/
...
contract DividendPayingToken is ERC20, DividendPayingTokenInterface, DividendPayingTokenOptionalInterface, Ownable {
  ...
  function updateDividendrouter(address newAddress) external onlyOwner {
        require(newAddress != address(router), "BonkBSC: The router already has that address");
...
contract BonkBSC is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    BonkBSCDividendTracker public dividendTracker;
    ...
    constructor() ERC20("BonkBSC", "BonkBSC", 9) {

        dividendTracker = new BonkBSCDividendTracker();

        liquidityWallet = owner();
        operationsWallet = 0x52DEFCAAeD3997c969c49995f1DcCdD3D704d841;
    ...
    function setWhiteListAMM(address ammAddress, bool isWhiteListed) external onlyOwner {
      require(isContract(ammAddress), "BonkBSC: setWhiteListAMM:: AMM is a wallet, not a contract");
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "BonkBSC: The dividend tracker already has that address");

        BonkBSCDividendTracker newDividendTracker = BonkBSCDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "BonkBSC: The new dividend tracker must be owned by the BonkBSC token contract");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) external onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "BonkBSC: The liquidity wallet is already this address");
    ...
    function updateOperationsWallet(address newOperationsWallet) external onlyOwner {
        require(newOperationsWallet != operationsWallet, "BonkBSC: The operations wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) external onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "BonkBSC: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "BonkBSC: Cannot update gasForProcessing to same value");
    ...
    function setRewardToken(address rewardTokenAddress) public returns (bool) {
        require(isContract(rewardTokenAddress), "BonkBSC: setRewardToken:: Address is a wallet, not a contract.");
        require(rewardTokenAddress != address(this), "BonkBSC: setRewardToken:: Cannot set reward token as this token due to Router limitations.");
        require(!isIgnoredToken(rewardTokenAddress), "BonkBSC: setRewardToken:: Reward Token is ignored from being used as rewards.");
    ...
    function setRewardTokenWithCustomAMM(address rewardTokenAddress, address ammContractAddress) public returns (bool) {
        require(isContract(rewardTokenAddress), "BonkBSC: setRewardToken:: Address is a wallet, not a contract.");
        require(ammContractAddress != address(router), "BonkBSC: setRewardToken:: Use setRewardToken to use default Router");
        require(rewardTokenAddress != address(this), "BonkBSC: setRewardToken:: Cannot set reward token as this token due to Router limitations.");
        require(!isIgnoredToken(rewardTokenAddress), "BonkBSC: setRewardToken:: Reward Token is ignored from being used as rewards.");
        require(isAMMWhitelisted(ammContractAddress) == true, "BonkBSC: setRewardToken:: AMM is not whitelisted!");
    ...
    function buyBackTokensWithNoFees() external payable returns (bool) {
        ...
        require(userRawBNBDividends >= holderBNBUsedForBuyBacks[msg.sender].add(msg.value), "BonkBSC: buyBackTokensWithNoFees:: Cannot Spend more than earned.");
    ...
    function _setAutomatedMarketMakerPair(address _pair, bool value) private {
        require(automatedMarketMakerPairs[_pair] != value, "BonkBSC: Automated market maker pair is already set to that value");
    ...
    function _transfer(
        ...
        require(!_isBot[to] || !_isBot[from], "BonkBSC: To/from address is ignored");
...
contract BonkBSCDividendTracker is DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("BonkBSC_Dividend_Tracker", "BonkBSC_Dividend_Tracker", 9) {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 2_000 * (10**9); //must hold 2,000+ tokens to get rewards
    }

    function _transfer(address, address, uint256) pure internal override {
        require(false, "BonkBSC_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() pure public override {
        require(false, "BonkBSC_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main BonkBSC contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "BonkBSC_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "BonkBSC_Dividend_Tracker: Cannot update claimWait to same value");
