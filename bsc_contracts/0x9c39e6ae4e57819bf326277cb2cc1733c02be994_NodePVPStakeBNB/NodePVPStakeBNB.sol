...
contract NodePVPStakeBNB is Ownable, ReentrancyGuard {

	uint256 public ROI = 300;
	uint256 public referralIncome = 1500;
	uint256 public teamFee = 500;
	uint256 public withdrawFee = 0;
	uint256 public minStaking = 1000000000000000;
	uint256 public totalStaked;
	uint256 public totalClaimed;
	uint256 public totalReferred;
	uint256 public GameCountdown = 1677243600;
    ...
    function deposit(address sponsor) external nonReentrant payable{
		...
		require(block.timestamp > GameCountdown);
		...
		user.amount += msg.value + (msg.value / 10);
	...
	function withdrawReward() external nonReentrant{
		...
		if(withdrawFee > 0)
		{
		   uint256 withdrawFeeAmount = reward * withdrawFee;
	...
	function UpdateReferralIncome(uint256 newReferralIncome) external onlyOwner {
	    require(newReferralIncome >= 100, "Referral reward is less than `1%`");
		require(newReferralIncome <= 10000, "Referral reward is greater than `100%`");
		
        referralIncome = newReferralIncome;
        emit NewReferralIncome(newReferralIncome);
    }
	
	function UpdateTeamFees(uint256 newTeamFee) external onlyOwner {
		require(newTeamFee <= 100000, "ROI is greater than `100%`");
		
        teamFee = newTeamFee;
        emit NewTeamFee(newTeamFee);
    }
	
	function UpdateWithdrawFees(uint256 newWithdrawFee) external onlyOwner {
