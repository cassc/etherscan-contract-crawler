  ...
  AdminProxyManager
{
  ...
  event BuyToken(address sale, uint16 dstId, address dstContract, bytes dstPayload);
  ...
  function buyToken(
    ...
    _lzSend(dstChainId, _payload, payable(address(this)), address(0x0), _adapterParams, feeIn);
  ...
  function _nonblockingLzReceive(
    ...
    (address targetSale, , , , , , , ) = abi.decode(
      _payload,
      (address, uint128, uint128, uint256, address[], uint128[], uint128[], uint128[])
    );

    IGovSaleRemote(targetSale).finalize(_payload);
  }

  function setDstSaleGateway(address _dstSaleGateway) external virtual onlyOwner {
