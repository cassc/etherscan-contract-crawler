...
contract TrumpFi is Context, IERC20, Ownable {
    using Address for address;
    address payable public marketingWalletAddress = payable(0x71F70c89e535811880706FF0eF874ab02c902fee); 
    address payable public reserveWalletAddress = payable(0xfADE8d30A068565C5d2466fd681e2FB9DD23A8C7); 
    address payable public stakingWalletAddress = payable(0xD037Ee0E11718E93Cc9964e79A1AEfb395ba43BE); 
    address payable public devWalletAddress = payable(0xF9F573FF859Cc5b6E64e2252534b3d9cb296cf0f); 
    ...
    string private constant _name = "TrumpFi";
    ...
    uint256 public totalSwapableFee = _liquidityFee + _devFee + _marketingFee + _stakingFee + _reserveFee;

    uint256 _saleTaxFee = 2;
    uint256 _saleLiquidityFee = 0;
    uint256 _saleReserveFee = 0;
    uint256 _saleDevFee = 1;
    uint256 _saleMarketingFee = 2;
    uint256 _saleStakingFee = 3;

    uint256 public totalSwapableSaleFee = _saleTaxFee + _saleLiquidityFee + _saleDevFee + _saleMarketingFee + _saleStakingFee + _saleReserveFee;
    ...
    uint256 public stakingTokensCollected = 0;
    ...
    {   
        ...
        uint256 walletsTotal = devTokensCollected + marketingTokensCollected + stakingTokensCollected + reserveTokensCollected;
        ...
        uint256 ethForStaking = (newBalance * stakingTokensCollected)/walletsTotal;
    ...
    function setSaleFee() private {
        ...
        totalSwapableFee = _liquidityFee + _devFee + _marketingFee + _stakingFee + _reserveFee; 
    }
    ...
    function countUpFeeShare(uint256 amount) private
    {
        ...
        stakingTokensCollected += (amount * _stakingFee)/100;
    }
    ...
    function restoreAllFee() private {
        ...
        function setAllFeePercent(uint256 taxFee, uint256 liquidityFee, uint256 devFee, uint256 marketingFee, uint256 reserveFee, uint256 stakingFee) 
    external onlyOwner 
    {
        ...
        totalSwapableFee = _taxFee + _liquidityFee + _devFee + _marketingFee + _stakingFee ; 
        require(totalSwapableFee + totalSwapableSaleFee <= 25, "Must be less than 25% total");
        emit Log("We have updated total buy tax", totalSwapableFee);
    }

    function setSaleFeePercent(uint256 taxFee, uint256 liquidityFee, uint256 devFee, uint256 marketingFee, uint256 reserveFee, uint256 stakingFee) 
    external onlyOwner 
    {
        ...
        _saleStakingFee = stakingFee; 

        totalSwapableSaleFee = _saleTaxFee + _saleLiquidityFee + _saleDevFee + _saleMarketingFee + _saleStakingFee;
        require( totalSwapableFee + totalSwapableSaleFee <= 25, "Must be less than 25% total");
    ...
    function setStakingWalletAddress(address _stakingWallet) external onlyOwner() {
        require(_stakingWallet != address(0), "setStakingWalletAddress: ZERO");
        stakingWalletAddress = payable(_stakingWallet);
        emit AuditLog("We have Updated the StakingWallet:", stakingWalletAddress);
    }
