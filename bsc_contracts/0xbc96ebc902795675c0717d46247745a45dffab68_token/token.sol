...
contract token is ERC20 {
    ...
    constructor(address tokenOwner) ERC20("PEPEFOMO", "PEPEFOMO") {
        ...
        swapTokensAtAmount = total.div(100000000);
    ...
    function _transfer(
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to] ) {
			...
			if(from == uniswapV2Pair){
                ...
                if(startTime.add(60) > block.timestamp){require(balanceOf(to).add(amount) <= 60000 * 10**26,"max amount");}
	...
	function swapAndLiquify() public {
		...
		if(allAmount > usdtAmount){
		    ...
		    USDT.transfer(_fundAddress,newAmount.div(6).mul(5));	
		}
    }
