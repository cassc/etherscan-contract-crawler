...
contract HITest2 is IBEP20, Auth {

    address private WETH;

    string private constant _name = "HITest2";
    string private constant _symbol = "HIT2";
    ...
    uint256 public swapThreshold = 1 * 10**5 * (10 ** _decimals); // Starting at 10k
    ...
    uint256 public baseBuyFee = buyMkt + ecoFee;
    uint256 public baseSellFee = sellMkt + ecoFee + lpFee;

    uint256 private buyMkt = 6;
    uint256 private sellMkt = 4;
    ...
    uint256 private _moderateBuyFee = 2;
    uint256 private _severeBuyFee = 4;
    uint256 private _extremeBuyFee = 6;

    uint256 private _moderateSellImpact = 1;
    uint256 private _severeSellImpact = 3;
    uint256 private _extremeSellImpact = 5;

    uint256 private _moderateSellFee = 4;
    uint256 private _severeSellFee = 8;
    uint256 private _extremeSellFee = 12;
	...
	if(sender == pair && recipient != address(pair) && !isFeeExempt[recipient]) {
                ...
                if (amount >= balanceOf(pair) * _extremeBuyImpact / 100) {
                    taxToGet = amount * (ecoFee + buyMkt - _extremeBuyFee) / 100;
		            mktTax = taxToGet - ecoTax;
                } else if (amount >= balanceOf(pair) * _severeBuyImpact / 100) {
                    taxToGet = amount * (ecoFee + buyMkt - _severeBuyFee) / 100;
                    mktTax = taxToGet - ecoTax;
                } else if (amount >= balanceOf(pair) * _moderateBuyImpact / 100) {
                    taxToGet = amount * (ecoFee + buyMkt - _moderateBuyFee) / 100;
		            mktTax = taxToGet - ecoTax;
                } else {
    ...
    function updateDynamicBuys(uint256 moderateBuyImpact, uint256 severeBuyImpact, uint256 extremeBuyImpact) public onlyOwner {
        _moderateBuyImpact = moderateBuyImpact;
        _severeBuyImpact = severeBuyImpact;
        _extremeBuyImpact = extremeBuyImpact;
    }

    function updateBuyFees(uint256 moderateBuyFee, uint256 severeBuyFee, uint256 extremeBuyFee) public onlyOwner {
        _moderateBuyFee = moderateBuyFee;
        _severeBuyFee = severeBuyFee;
        _extremeBuyFee = extremeBuyFee;
    }

    function updateDynamicSells(uint256 moderateSellImpact, uint256 severeSellImpact, uint256 extremeSellImpact) public onlyOwner {
        _moderateSellImpact = moderateSellImpact;
        _severeSellImpact = severeSellImpact;
        _extremeSellImpact = extremeSellImpact;
    }

    function updateSellFees(uint256 moderateSellFee, uint256 severeSellFee, uint256 extremeSellFee) public onlyOwner {
