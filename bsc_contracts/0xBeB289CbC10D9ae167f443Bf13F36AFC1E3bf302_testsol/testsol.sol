...
contract testsol is Ownable {
    ...
    uint256 private constant topAcheiverPoolPercents = 30;
    uint256 private constant topinvitorPoolPercents = 20;
    ...
    uint256 public topPool;
     ...
     uint256 public topinviterPool;
     uint256 public topacheiverrPool;
 ...
 function _updatedirectNum(address _user) private {
    ...
    function _calLevelNow(address _user) private view returns(uint256) {
        ...
        if(total >= 500e18){
            ...
            if(total >= 2000e18 && totaldirectnum>=20 && totaldirectdepositnum>=10000e18   && user.teamNum >= 300 && maxTeam >= 100000e18 &&  otherTeam >= 100000e18  ){
                levelNow = 5;
            }else if(total >= 2000e18 && totaldirectnum>=10 && totaldirectdepositnum>=5000e18 && user.teamNum >= 150 && maxTeam >= 50000e18 &&  otherTeam >= 50000e18  ){
                levelNow = 4;
            }else if(total >= 500e18  && user.teamNum >= 30 && maxTeam >= 5000e18 &&  otherTeam >= 5000e18  ){
                levelNow = 3;
            }
            else if(total >= 200e18 && totaldirectnum>=5 )
            {
               levelNow = 2;
            }
  ...
  function getTeamDeposit(address _user) public view returns(uint256, uint256, uint256){
   ...
   function _deposit(address _user, uint256 _amount) private {
     ...
     function _distributeDeposit(uint256 _amount) private {
       ...
       // new add on pool inviter / acheiver  /manager / coordinator
        uint256 topinvite = _amount.mul(topinvitorPoolPercents).div(baseDivider);
        topinviterPool = topinviterPool.add(topinvite); 
        
        uint256 topacheive = _amount.mul(topAcheiverPoolPercents).div(baseDivider);
        topacheiverrPool = topacheiverrPool.add(topacheive); 
        ...
        for(uint256 i = 0; i < referDepth; i++){
            if(upline != address(0)){
                ...
                RewardInfo storage upRewards = rewardInfo[upline];
                  ...
                  if(i > 1 && i < 4 && userInfo[upline].level > 1){
                ...
                if(i > 3 && i < 6 && userInfo[upline].level > 2){
                ...
                if(i >= 6 && userInfo[upline].level > 3){
