...
contract GSwapLiquid is Initializable, OwnableUpgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function buy() external payable whenNotPaused nonReentrant {
        ...
        addLiquidityToPoolETH(amount, ((gomaTokenSwap * 2000000000) / 10**9));
        IERC20Upgradeable(tokenOut).transfer(_msgSender(), gomaTokenSwap);
        emit Buy(_msgSender(), amount, commission, gomaTokenSwap);
    }
    ...
    function addLiquidityToPoolETH(uint256 _amountUSD, uint256 _amountGoma) internal returns (uint256) {
        // approve token transfer to cover all possible scenarios
        IERC20Upgradeable(tokenOut).approve(address(pancakeswapRouterV2), _amountGoma);

        // add the liquidity
        (uint256 amountUSD, uint256 amountGoma, uint256 liquidity) = pancakeswapRouterV2.addLiquidityETH{
            value: _amountUSD
        }(tokenOut, _amountGoma, 0, 0, address(this), block.timestamp);
    ...
    function withdrawBNB(address admin) external onlyOwner {
