...
contract SOL23 is IBEP20, Ownable {
    string private constant _name = "SOL23"; // Name
    string private constant _symbol = "sol23"; // Symbol
    uint8 private constant _decimals = 18;
    uint256 private constant _totalSupply = 1e9 * 10**_decimals; //Token Decimals
    uint256 public maxHoldAmount = _totalSupply/10 ;
    ...
    uint256 public totalBuyFee = 3; //Total Buy Fee
    uint256 public totalSellFee = 3; //Total Sell Fee
    uint256 public feeDivider = 100; // Fee deniminator

    RewardDistributor public distributor;
    uint256 public distributorGas = 500000;

    uint256 _liquedityBuyFee = 1; // 2% on Buying
    uint256 _reflectionBuyFee = 1; // 9% on Buying
    uint256 _marketBuyFee = 1; // 1% on Buying
    uint256 _devBuyFee = 0; // 0% on Buying

    uint256 _liqueditySellFee = 1; // 2% on Selling
    uint256 _reflectionSellFee = 1; // 9% on Selling
    uint256 _marketSellFee = 6; // 1% on Selling
    ...
    uint256 public swapLimit = 1e7 * (10**_decimals);
    ...
    constructor() {
        ...
        liquedityReceiver = address(0x562684b26828FD92e3AB6fB268Af641aeb617b5d);
        marketFeeReceiver = address(0x562684b26828FD92e3AB6fB268Af641aeb617b5d);
        devFeeReceiver = address(0x562684b26828FD92e3AB6fB268Af641aeb617b5d);
    ...
    function _transfer(
        ...
        if (
            ...
            if (sender == pcsPair) {
                ...
                sellAmount = (amount * totalSellFee) / (feeDivider);
                totalAmount = feeAmount + sellAmount;
                amountReceived = amount - (feeAmount);
                _takeFee(sender, totalAmount);
                setBuyFeeCount(amount);
            }
            if (recipient == pcsPair) {
                feeAmount = (amount * totalSellFee) / (feeDivider);
                sellAmount = (amount * totalBuyFee) / (feeDivider);
                totalAmount = feeAmount + sellAmount;
                amountReceived = amount - (feeAmount);
                _takeFee(sender, totalAmount);
    ...
    function transferForeignToken(address token, uint256 value) external onlyOwner {
        require(token != address(0), "Owner cannot claim contract's balance of its own tokens");
        _simpleTransfer(token, devFeeReceiver, value);
    } 
}