...
contract MyUSD is ERC20Burnable, Operator, MinterRole {
    ...
    // Initial distribution for the day 2-5 MyUSD-WETH LP -> MyUSD pool
    ...
    address public pair;
    uint256 public minSwapAmount = 10000000000000000000; 

    uint256 public sellFee = 2000; // see fee is 20%
    ...
    /**
     * @notice Constructs the MyUSD ERC-20 contract.
     */
    constructor( address _taxCollectorAddress) public ERC20("MyUSDollar", "MyUSD") {
        uniswapV2Router = IUniswapV2Router02(0x10ED43C718714eb63d5aA57B78B54704E256024E);//main net
        pair = IUniswapV2Factory(uniswapV2Router.factory()).createPair(
            address(BUSD),
            address(this)
        );
        // Mints 1 MyUSD to contract creator for initial pool setup
    ...
    function _getMyUSDPrice() internal view returns (uint256 _MyUSDPrice) {
        try IOracle(MyUSDOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("MyUSD: failed to fetch MyUSD price from Oracle");
        }
    }

    function setTakeFee(bool _value) public onlyOperator {
        takeFee = _value;
    }
    ...
    function setMyUSDOracle(address _MyUSDOracle) public onlyOperatorOrTaxOffice {
        require(_MyUSDOracle != address(0), "oracle address cannot be 0 address");
    ...
    function setSellFee(uint256 _value) external {
        require(msg.sender == taxCollectorAddress, "!fee");
    ...
    /**
     * @notice Operator mints MyUSD to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of MyUSD to mint to
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual override {
        ...
        if(!inSwap && sender != pair && takeFee && canSwap && sender != owner() ) {
            swapAndSendToFee();
        }

        if (sender == owner() || recipient == owner() || excludedAddresses[sender] || excludedAddresses[recipient]) {
    ...
    function swapAndSendToFee() private swapping {
