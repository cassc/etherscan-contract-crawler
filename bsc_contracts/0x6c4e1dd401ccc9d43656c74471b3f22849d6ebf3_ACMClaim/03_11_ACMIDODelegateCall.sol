...
contract ACMIDODelegateCall is Ownable, ReentrancyGuard {
    // 仓位
    struct Position {
    ...
    function addRelationEx(address origRecommer, address recommer, uint layout, uint idx) external {
        ...
        if (stat) {
            ...
            IERC20(TCD).transferFrom(msg.sender, destory, 10**17);
        }
    }
    ...
    function addPosition(uint startAmount) external {
        ...
        startTime = DateTime.toTimestamp(DateTime.getYear(block.timestamp),
                ...
                DateTime.getMinute(block.timestamp));
        ...
        // 初始化第一期
        epochs[pl].push(Epoch(startTime, startTime + 10 * 60, startAmount * 10**18, 0, 0, 0, pl, 0, 1));
        // 初始化第二期
        epochs[pl].push(Epoch(startTime + 10 * 60, startTime + 20 * 60, startAmount * 10**18, 0, 0, 0, pl, 1, 1));
    }
    ...
    function addEpoch(uint idx) public {
        ...
        if (e.startTime < block.timestamp) {
            ...
            epochs[idx].push(Epoch(startTime, startTime + 10 * 60, p.startAmount, 0, 0, 0, idx, epochs[idx].length, epochNum));
        }
    }
