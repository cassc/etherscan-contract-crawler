...
contract RecordStorage is Ownable, ReentrancyGuardRecord {
    ...
    function getPriceOfLP() public view returns (uint256 price){
        ...
        uint256 balancePath1 = TokenTransfer(coinTypeMaping["USDT"]).balanceOf(uniswapV2Pair);
        uint256 balancePath2 = TokenTransfer(coinTypeMaping["DOTC"]).balanceOf(uniswapV2Pair);
    ...
    function subWitnessAvailable(address _addr) public onlyAuthFromAddr {
        ...
        if (_availableTotal >= subWitFee) {
            _need = subWitFee;
            availableTotal[_addr]["DOTC"] = SafeMath.sub(_availableTotal, _need);
        } else {
