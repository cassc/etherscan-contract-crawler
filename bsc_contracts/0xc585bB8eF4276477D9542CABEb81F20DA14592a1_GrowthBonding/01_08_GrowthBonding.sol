...
contract GrowthBonding is Initializable, Ownable, ReentrancyGuard
{
	...
	function redeem() external nonReentrant
	{
		...
		uint256 _boostAmount = _accountInfo.burned == 0 ? 0 : _accountInfo.burned * _roundInfo.boost / _roundInfo.burned;
		uint256 _payoutAmount = _accountInfo.amount == 0 ? 0 : _accountInfo.amount * _roundInfo.payout / _roundInfo.amount;
		uint256 _reserveAmount = _accountInfo.amount + _boostAmount + _payoutAmount;
		...
		uint256 _divsAmount = _accountInfo.amount == 0 ? 0 : _accountInfo.amount * _roundInfo.reward / _roundInfo.amount;
		uint256 _rewardAmount = _divsAmount + _prizeAmount;
		...
		emit Redeem(msg.sender, reserveToken, _reserveAmount, rewardToken, _rewardAmount, _accountInfo.round);
	}
