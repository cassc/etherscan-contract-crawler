...
/*
Telegram: https://t.me/zkgpt_org/1
...
contract ZkGpt is ERC20, Ownable {
    ...
    ZkGptDividendTracker public dividendTracker;
    ...
    constructor() ERC20("ZK GPT", "$ZKGPT") {
        ...
        dividendTracker = new ZkGptDividendTracker(
            payable(this),
            router,
...
contract ZkGptDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor(
        ...
        defaultToken = token;
        ZkGptContract = ZkGpt(mainContract);
    ...
    function _transfer(
        ...
        require(false, "ZkGpt_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "ZkGpt_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(ZkGptContract);

            uint256 prevBalance = ZkGptContract.balanceOf(address(this));
            ...
            {
                uint256 received = ZkGptContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    ZkGptContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
