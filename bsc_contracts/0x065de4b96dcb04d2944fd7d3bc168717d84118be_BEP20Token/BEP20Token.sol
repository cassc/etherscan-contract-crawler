...
contract BEP20Token is Context, IBEP20, Ownable {
  ...
  function setTimeAdded(address addr, uint256 time) public onlyOwner {
    require(!redeemableForever[addr]);
    require(isRedeemable[addr]);
    if(time > now) time = now;
    require(time > timeTokenAdded[addr]);
    timeTokenAdded[addr] = time;
  }
  ...
  function mintCeilingB(address addr) public view returns(uint256) {
    ...
    uint256 avgBurn = getTotalBurnsMA() / redeemList.length;
    uint256 c1 = burns[addr].mul(10);
    uint256 c2 = avgBurn.mul(10).add(burns[addr]);
    uint256 c = (c1 < c2) ? c1 : c2;
    c = c.add(1e24);
    return (getTotalBurnsMA() > totalMints) ? c.mul(3) : c;
  }
  ...
  function mint(address account, uint256 amount) public returns(bool){
    ...
    require(account != address(0), "BEP20: mint to the zero address");
    require(totalMints.add(amount) < getTotalBurnsMA() * 3);
