...
contract BabyFPSNFTs is ERC721Enumerable, Ownable {
    using Strings for uint256;
    ...
    address public RewardToken = 0xfffe03b9306dF57aafe9d291aC906fBfB0b07Be3;
    address public FeeReceiver = 0x1eDD167D4d2d8C8bA6cC00391d99F61a0c2a42A4;
    ...
    function mint(uint256 _mintAmount) public payable {
        // Checks
        require(!paused, "BabyFPS: minting has not started yet");
        require(_mintAmount > 0, "BabyFPS: you have to mint at least one");
        require(
            _mintAmount <= maxMintAmount,
            "BabyFPS: you can only mint 10 at a time"
        );
        require(
            lastSupply >= _mintAmount,
            "BabyFPS: the collection is sold out"
        );
        if (msg.sender != owner()) {
            require(
                msg.value >= cost * _mintAmount,
                "BabyFPS: total cost isn't match"
            );
        }
        // Minting
    ...
    {
        // Checks
        require(_winner != address(0), "BabyFPS: zero address");
        require(_amount > 0, "BabyFPS: you have to mint at least one");
        require(lastSupply != 0, "BabyFPS: the collection is sold out");
    ...
    function tokenURI(uint256 _id)
        ...
        returns (string memory)
    {
        require(_exists(_id), "BabyFPS: URI query for nonexistent token");
