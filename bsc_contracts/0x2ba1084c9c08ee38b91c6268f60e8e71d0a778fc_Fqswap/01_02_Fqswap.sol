...
contract Fqswap is IFqswap, Initializable, ContextUpgradeable, OwnableUpgradeable {
    ...
    modifier lockUpgrade() {
        require(!_upgraded, "Fqswap: Already upgraded");
        _;
        _upgraded = true;
    }

    modifier checkTierIndex(uint256 _index) {
        require(feeTiers.length > _index, "Fqswap: Invalid tier index");
        _;
    }
    ...
    function initialize(address _router) public initializer {
        ...
        __Fqswap_v2_init_unchained(_router);
    }

    function __Fqswap_v2_init_unchained(address _router) internal initializer {
        _name = "Fqswap V2";
        _symbol = "FQS";
        _decimals = 9;

        _tTotal = 1000000 * 10**9;
    ...
    function __Fqswap_tiers_init() internal initializer {
        _defaultFees = _addTier(0, 800, 200, 0, 200, address(0), address(0));
        _addTier(0, 600, 100, 0, 100, address(0), address(0));
        _addTier(0, 500, 100, 0, 50, address(0), address(0));
        _addTier(0, 300, 100, 0, 50, address(0), address(0));
    }
    ...
    function whitelistAddress(address _account, uint256 _tierIndex)
        ...
        preventBlacklisted(_account, "Fqswap: Selected account is in blacklist")
    {
        require(_account != address(0), "Fqswap: Invalid address");
        _accountsTier[_account] = _tierIndex;
    }

    function excludeWhitelistedAddress(address _account) public onlyOwner {
        require(_account != address(0), "Fqswap: Invalid address");
        require(_accountsTier[_account] > 0, "Fqswap: Account is not in whitelist");
    ...
    function checkFees(FeeTier memory _tier) internal view returns (FeeTier memory) {
        ...
        require(_fees <= _maxFee, "Fqswap: Fees exceeded max limitation");
    ...
    function setEcoSystemFeeAddress(uint256 _tierIndex, address _ecoSystem)
        ...
        checkTierIndex(_tierIndex)
    {
        require(_ecoSystem != address(0), "Fqswap: Address Zero is not allowed");
    ...
    function setOwnerFeeAddress(uint256 _tierIndex, address _owner) external onlyOwner checkTierIndex(_tierIndex) {
        require(_owner != address(0), "Fqswap: Address Zero is not allowed");
    ...
    function _approve(
        ...
        private
        preventBlacklisted(owner, "Fqswap: Owner address is blacklisted")
        preventBlacklisted(spender, "Fqswap: Spender address is blacklisted")
    {
    ...
    function _transfer(
        ...
        private
        preventBlacklisted(_msgSender(), "Fqswap: Address is blacklisted")
        preventBlacklisted(from, "Fqswap: From address is blacklisted")
        preventBlacklisted(to, "Fqswap: To address is blacklisted")
        isRouter(_msgSender())
    {
    ...
    function swapAndEvolve() public onlyOwner lockTheSwap {
        ...
        uint256 initialBalance = IFqswap(address(this)).balanceOf(msg.sender);
        // swap BNB for Tokens
        swapBnbForTokens(half);

        // how much BNB did we just swap into?
        uint256 newBalance = IFqswap(address(this)).balanceOf(msg.sender);
        uint256 swapeedToken = newBalance.sub(initialBalance);

        _approve(msg.sender, address(this), swapeedToken);
        IFqswap(address(this)).transferFrom(msg.sender, address(this), swapeedToken);
    ...
    function migrate(address account, uint256 amount)
        external
        override
        preventBlacklisted(account, "Fqswap: Migrated account is blacklisted")
    {
        require(migration != address(0), "Fqswap: Migration is not started");
        require(_msgSender() == migration, "Fqswap: Not Allowed");
    ...
    function withdrawToken(address _token, uint256 _amount) public onlyOwner {
        IFqswap(_token).transfer(msg.sender, _amount);
    }
