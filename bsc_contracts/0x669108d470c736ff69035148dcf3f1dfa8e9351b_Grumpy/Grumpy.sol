...
contract Grumpy is ERC20, Ownable {
    ...
    GrumpyDividendTracker public dividendTracker;
    ...
    constructor() ERC20("Grumpy", "CAT") {
        marketingWallet = payable(0x0343684f6A132Db2541779a82149fdAD851E3fEa);
        devWallet = payable(0x54D30609CCfA7b71b01cfe13e5D391edE4e69047);
        ...
        dividendTracker = new GrumpyDividendTracker(
            payable(this),
            router,
...
contract GrumpyDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor(
        ...
        defaultToken = token;
        GrumpyContract = Grumpy(mainContract);
    ...
    function _transfer(
        ...
        require(false, "Grumpy_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "Grumpy_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main Grumpy contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(GrumpyContract);

            uint256 prevBalance = GrumpyContract.balanceOf(address(this));
            ...
            {
                uint256 received = GrumpyContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    GrumpyContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
