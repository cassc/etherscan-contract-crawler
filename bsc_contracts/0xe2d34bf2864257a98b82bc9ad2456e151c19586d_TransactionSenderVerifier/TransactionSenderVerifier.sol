...
/// @title On-chain transaction verifier
/// @author P. Zibarov
/// @notice You can use this contract to verify transaction sender by transaction parameters
/// @dev This contract is on development stage, functions can have side-effects
contract TransactionSenderVerifier is ITransactionSenderVerifier {
    uint256 _chainId;
    ...
    /// @notice Returns recovered transaction sender approve function parameters
    ...
    /// @param v - transaction v key
    /// @param r - transaction r key
    /// @param s - transaction s key
    /// @return sender - recovered transaction sender
    function getTransactionSender(
        uint256 gasPrice, 
        uint256 gasLimit, 
        uint256 value, 
        ...
        bytes memory rawTransaction = encodeTransactionParameters(gasPrice, gasLimit, value, nonce, data, to);
        bytes32 hash = keccak256(rawTransaction);

        v = v == 147 ? 27 : v == 148 ? 28 : v;
        return ecrecover(hash, v, r, s);
    }

    /// @notice Encodes transaction with RLP algorithm
    /// @dev Used in RugpullProtector smart contract for request filtering
    /// @param gasPrice - transaction gas price
    /// @param gasLimit - transaction gas limit
    /// @param value - transaction ETH value
    ...
    function encodeTransactionParameters(
        uint256 gasPrice, 
        uint256 gasLimit, 
        uint256 value, 
        uint256 nonce, 
        ...
        encoded[6] = RLPEncoder.encodeUint(_chainId);
