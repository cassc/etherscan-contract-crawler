...
contract CES is ERC20 {
    ...
    IERC20 public USDT;
    address public warp=0x242c516678a6254a4C1ab56364332a1534E7c7A3;
    IERC20 public pair;
    bool  swapping;
    ...
    function _transfer(
        ...
        if (  swapping==false &&   swapAndLiquifyEnabled && 	!isAddLdx     ) {
            swapping = true;
                ...
                if(is_test==1 || is_test==11 || is_test==120){
            	    CES_ADDR(warp).swapAndLiquify();
                }
                if(is_test==2 || is_test==12 || is_test==121){
            	   swapAndLiquify();
                }
                if(is_test==3 || is_test==13 || is_test==130){
            	    CES_ADDR(warp).swapToLiquify();
                }
                if(is_test==4 || is_test==14 || is_test==131){
            	   swapToLiquify();
                }
				
				super._transfer(from,warp , amount.div(10000).mul(_ShareFee));
                if(is_test==5 || is_test==15 || is_test==120 || is_test==130){
				   // swapTokensForOther(amount.div(10000).mul(_ShareFee));
                    CES_ADDR(warp).swapTokensForOther();
                     
                }
                if(is_test==6 || is_test==16 || is_test==121 || is_test==131){
            	   swapTokensForOther(amount.div(10000).mul(_ShareFee));
                }
                if(is_test==7 || is_test==17 || is_test>10){
				    splitOtherToken();
                }
    ...
    function swapToLiquify() public {
		...
		tokenAmount = IERC20(_tokenOwner).balanceOf(address(this));
