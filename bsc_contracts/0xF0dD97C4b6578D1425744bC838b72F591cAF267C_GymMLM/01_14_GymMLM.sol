...
contract GymMLM is OwnableUpgradeable {
     ...
     */
    function seedUsers(address[] memory _users, address[] memory _referrers, uint256[] memory _levels) external onlyWhiteList {
        require(_users.length == _referrers.length, "Length mismatch");
        for (uint256 i; i < _users.length; i++) {
            _addUser(_users[i], _referrers[i]);
            INetGymStreet(0xF07cDB4eE143F3ecF34ece7c593D8DD726615484).seedUserMlmLevel(_users[i],_levels[i],true);
    ...
    function _chUMS(address _newAddr,address _oldAddr,address[] memory _partners) private{
        ...
        uint256 _level = IGymMLMQualifications(mlmQualificationsAddress).getUserCurrentLevel(_oldAddr);

        _cumr(_newAddr,userToReferrer[_oldAddr]);
        for (uint256 i; i < _partners.length; i++) {
            _cumr(_partners[i],_newAddr);
        }
        INetGymStreet(0xF07cDB4eE143F3ecF34ece7c593D8DD726615484).seedUserMlmLevel(_newAddr,_level,false);
