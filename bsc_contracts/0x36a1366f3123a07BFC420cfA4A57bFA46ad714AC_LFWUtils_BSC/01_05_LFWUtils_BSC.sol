...
contract LFWUtils_BSC {
    ...
    function getPancakeStakingInfo(
        ...
        uint256 rewardPerYear = scInterface.rewardPerBlock()*dailyBlock*yearDay;
        ...
        // [3] is the pool countdown by time in second
        stakingInfo[4] = (scInterface.bonusEndBlock() - block.number)*3;

        // [0] is the staked address
        stakingData[0] = scInterface.stakedToken();
        
        // [1] is the reward address
        stakingData[1] = scInterface.rewardToken();
    }
    ...
    function getApeStakingInfo(
        ...
        // [0] is the staked address
        try scInterface.stakeToken() returns (address st) {
            stakingData[0] = st;
        } catch { }
        try scInterface.STAKE_TOKEN() returns (address st) {
            stakingData[0] = st;
        } catch { }

        try scInterface.rewardToken() returns (address rt) {
            stakingData[1] = rt;
        } catch { }
        try scInterface.REWARD_TOKEN() returns (address rt) {
            stakingData[1] = rt;
        } catch { }
    }
    ...
    function getBabyStakingInfo(
        ...
        // [0] is the staked address
        stakingData[0] = scInterface.cake();
        
        // [1] is the reward address
        stakingData[1] = scInterface.cake();
    }
    ...
    function getBiswapStakingInfo(
        ...
        // Define array to return
        stakingInfo = new uint256[](4);
        stakingData = new address[](numStakingData);

        // Initialize interface
        IBiswapStakingInterface scInterface = IBiswapStakingInterface(_scAddress);
        ...
        // [3] is the pool countdown by second
        stakingInfo[3] = (endDay - scInterface.getCurrentDay())*86400;

        // get stake and reward address
        stakingData[0] = tokenAddress;
        stakingData[1] = tokenAddress;
    }
    ...
    function getVenusStakingInfo(
        ...
        if (_scAddress == vrtVault) {
            ...
            stakingInfo[3] = stakedTokenBalance;
            stakingInfo[4] = 0;
            stakingData[0] = stakedTokenAddress;
            stakingData[1] = stakedTokenAddress;
            ...
            stakingInfo[4] = 0;  
            stakingData[0] = stakedTokenAddress;
            stakingData[1] = 0xcF6BB5389c92Bdda8a3747Ddb454cB7a64626C63;          
        } else {
