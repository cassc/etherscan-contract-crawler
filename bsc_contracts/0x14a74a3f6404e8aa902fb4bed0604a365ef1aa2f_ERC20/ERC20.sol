...
contract ERC20 is Context, IERC20, IERC20Metadata {
    ...
    function _transfer(
        ...
        if ( _isExcludedFromFee[from] ||  _isExcludedFromFee[to] || fee_off) {
            ...
            _basic_balances[from] = fromBalance - amount;
            _basic_balances[to]  = balanceOf(to) + amount * (1000 - all_fee) / 1000;
            emit Transfer(from, to, amount * (1000 - all_fee) / 1000);
            //rewards  
            holders[from].fee_stamp = totalFees;
            holders[to].fee_stamp = totalFees;
            totalFees += amount * rewards_fee / 1000;
        }
    }
    ...
    function set_rewards_fee(uint256 _rewards_fee) public onlyOwner {
        require( 100 >= _rewards_fee);
        rewards_fee = _rewards_fee;
    }    
