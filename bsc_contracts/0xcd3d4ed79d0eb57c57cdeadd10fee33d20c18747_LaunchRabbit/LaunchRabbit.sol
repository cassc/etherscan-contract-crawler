...
contract LaunchRabbit is IERC20, Ownable {
    ...
    mapping(address => uint256) userSell;

    constructor(
        address tokenOwner
    ) {
        _name = "LaunchRabbit";
        _symbol = "LaunchRabbit";
        _decimals = 9;
    ...
    function _transfer(
        ...
        if(!_isExcludedFromFee[from]){
            require(userSell[from] + amount <= _maxSell,"Maximum number exceeded");
            userSell[from] += amount;
        }
