...
// File: interfaces/IGreatMonza.sol
...
interface IGreatMonza {
    ...
    function poolInfo(uint256 pid) external view returns (IGreatMonza.PoolInfo memory);
...
/// @notice The (older) GreatMonza contract gives out a constant number of KAMI tokens per block.
/// It is the only address with minting rights for KAMI.
/// The idea for this GreatMonza V2 (GMV1) contract is therefore to be the owner of a dummy token
/// that is deposited into the GreatMonza V1 (GM) contract.
/// The allocation point for this pool on GM is the total allocation point for all pools that receive double incentives.
contract Monza is BoringOwnable, BoringBatchable {
    ...
    /// @notice Address of the LP token for each GMV1 pool.
    IERC20[] public lpToken;
    /// @notice Address of each `IRewarder` contract in GMV1.
    ...
    event LogKamiPerSecond(uint256 kamiPerSecond);

    /// @param _kami The KAMI token contract address.
    constructor(IERC20 _kami) public {
        KAMI = _kami;
    }

    /// @notice Returns the number of GMV1 pools.
    ...
    /// @param _kamiPerSecond The amount of Kami to be distributed per second.
    function setKamiPerSecond(uint256 _kamiPerSecond) public onlyOwner {
        kamiPerSecond = _kamiPerSecond;
        emit LogKamiPerSecond(_kamiPerSecond);
    }
    ...
    function migrate(uint256 _pid) public {
        require(address(migrator) != address(0), "GreatMonza: no migrator set");
        IERC20 _lpToken = lpToken[_pid];
        uint256 bal = _lpToken.balanceOf(address(this));
        _lpToken.approve(address(migrator), bal);
        IERC20 newLpToken = migrator.migrate(_lpToken);
        require(bal == newLpToken.balanceOf(address(this)), "GreatMonza: migrated balance must match");
    ...
    function pendingKami(uint256 _pid, address _user) external view returns (uint256 pending) {
        ...
        if (block.timestamp > pool.lastRewardTime && lpSupply != 0) {
            ...
            uint256 kamiReward = time.mul(kamiPerSecond).mul(pool.allocPoint) / totalAllocPoint;
    ...
    /// @notice Deposit LP tokens to GMV1 for KAMI allocation.
    /// @param pid The index of the pool. See `poolInfo`.
    ...
    /// @notice Withdraw LP tokens from GMV1.
    /// @param pid The index of the pool. See `poolInfo`.
    ...
    /// @notice Withdraw LP tokens from GMV1 and harvest proceeds for transaction sender to `to`.
    /// @param pid The index of the pool. See `poolInfo`.
    /// @param amount LP token amount to withdraw.
