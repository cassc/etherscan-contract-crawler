...
contract WillyCamote is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    WillyCamoteDividendTracker public dividendTracker;
    ...
    constructor() ERC20("WillyCamote", "Willy") {
        marketingWallet = payable(0x823bCFdB72D46D312A7994Ea60A1e5b338317960);
        devWallet = payable(0x9BbbF6186B3A7D88632ca74b5d1FD83125F8ec7d);
        ...
        dividendTracker = new WillyCamoteDividendTracker(
            payable(this),
            router,
        ...
        uint256 totalTokenSupply = (100000000000000) * (10**18);
        _mint(owner(), totalTokenSupply); // only time internal mint function is ever called is to create supply
        maxWallet = totalTokenSupply / 100000000000000; // 0.005%
        swapTokensAtAmount = totalTokenSupply / 100000000000000; // 0.05%;
...
contract WillyCamoteDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    WillyCamote public  WillyCamoteContract;
    ...
    constructor(
        ...
        defaultToken = token;
        WillyCamoteContract = WillyCamote(mainContract);
    ...
    function _transfer(
        ...
        require(false, "WillyCamote_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "WillyCamote_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main Willycamote contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(WillyCamoteContract);

            uint256 prevBalance = WillyCamoteContract.balanceOf(address(this));
            ...
            {
                uint256 received = WillyCamoteContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    WillyCamoteContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
