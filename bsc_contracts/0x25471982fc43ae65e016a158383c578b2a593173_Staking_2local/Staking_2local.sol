...
contract Staking_2local is Ownable {
    using SafeMath for uint256;
   ...
   // Update the given pool's 2LC allocation point and deposit fee. Can only be called by the owner.
    ...
    // View function to see pending 2local on the frontend.
    function pendingLocal( address _user) public view returns (uint256) {
        uint256 _pid = 0;
    ...
    function AuserRewardInfo( address account, uint256 index) public view returns (uint256, uint256, uint256, bool) {
        uint256 _pid = 0;
    ...
    function withdraw() public {
       ...
       require(user.amount > 0, "withdraw: not good");
         if (user.amount > 0) {
            ...
            user.amount = user.amount.sub(sumAmount);
            safeLocalTransfer(msg.sender, sumAmount.add(pending));
        }
    }
    ...
    function setLocal(address _localToken) public {
        require(msg.sender == feeAddress, "setFeeAddress: FORBIDDEN");
        local = LocalToken(_localToken);
    }


    // Shows the count of days since user had staked their Lp tokens
    function userDoneDays( address _user) public view returns (uint256 stakedDays){
    ...
    function userStakedDays( address _user) public view returns (uint256 lockedDays){
    ...
    function transferBEP20(IBEP20 token, address to, uint256 amount) external onlyOwner returns (bool) { 
