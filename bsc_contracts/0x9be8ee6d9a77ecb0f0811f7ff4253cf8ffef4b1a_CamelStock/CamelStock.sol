...
// SPDX-License-Identifier: MIT
	...
	contract CamelStock is ERC20, Ownable{
		...
		function withdrawUsdt() external lock {
			...
			require(!lock_address[_addr], "withdrawUsdt: lock_address");
		...
		function set_uptimes(address _addr,uint256 _val) external onlyOwner{
			uptimes[_addr] = _val;
		}
		
		function set_addUsdt(uint256 id,uint256 _val,uint256 timestamp,uint256 day) external lock onlyOwner {
			ERC20(usdt_address).transferFrom(msg.sender,address(this), _val);
			
			if(id>usdt_amount_length){
				usdt_amount_length=	id;
			} 
			day_lists[id]=day;
			time_lists[id]=timestamp;
			usdt_amount_lists[id]=usdt_amount_lists[id].add(_val);
