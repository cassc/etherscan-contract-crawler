...
contract CicleoSubscriptionFactory is OwnableUpgradeable {
    ...
    event SubscriptionManagerCreated(address creator, address indexed subscriptionAddress);

    function initialize(address _botAddress, uint256 _taxPercent, address _taxAccount) initializer public {
        __Ownable_init();
    ...
    function createSubscriptionManager(string memory name, address token, address treasury) external returns (address) {
        CicleoSubscriptionManager subscription = new CicleoSubscriptionManager(address(this), name, token, treasury);
    ...
    function getSubscription(address user) external view returns (SubscriptionManagerStruct[] memory) {
        SubscriptionManagerStruct[] memory callback = new SubscriptionManagerStruct[](accessAllowed[user].length);
