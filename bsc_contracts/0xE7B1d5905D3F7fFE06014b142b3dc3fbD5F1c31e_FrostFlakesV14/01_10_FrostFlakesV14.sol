...
contract FrostFlakesV14 is Initializable, UUPSUpgradeable, OwnableUpgradeable {
    ...
    event EmitOwnerDeposit(uint256 bnbamount);
        ...
        private
    {
        ...
        for (uint256 i = 0; i < snowBattleParticipants.length; i++) {
            if (
                snowBattleParticipants[i].adr == adr &&
                snowBattleParticipants[i].fighters <= 0
            ) {
    ...
    function removeLastBattle() private {
        for (uint256 i = 0; i < previousSnowBattles.length - 1; i++) {
        ...
        returns (PreviousSnowBattles[] memory snowBattles)
    {
        ...
        PreviousSnowBattles[] memory result = new PreviousSnowBattles[](
            resultCount
        );
    ...
    function buyFrostFlakes(address ref) public payable {
        revert("Contract is closed. Thank you for participating!");
        require(
        ...
        require(maxTvlReached(msg.sender) == false, "Total wallet TVL reached");
        require(
    ...
    function enableAutoCompounding() public payable {
        ...
        emit EmitAutoCompounderStart(msg.sender, msg.value, tvl, fee, true);
    }
    ...
    function maxReferralsReached(address refAddress)
        public
        view
        returns (bool)
    {
    ...
    function maxTvlReached(address adr) public view returns (bool) {
        return
            lockedFrostFlakes[adr] >=
            calcBuyFrostFlakes(getBackwardCompatibleMaxTVLInBNB());
    }
    ...
    function setUserName(string memory name) public returns (string memory) {
        revert("Contract is closed. Thank you for participating!");
