/// @title NFT Marketplace V5
/// @notice contracts/NFTMarketplaceV5.sol
...
contract TSV_NFTMarketplaceV5 is
    ...
    /// @notice Mapping for blocking the tokenId.
    ...
    /**
     * @notice Event to keep track when the NFT tokenId blocked.
     * @param tokenId: The blocked tokenId.
     * @param by: The caller address (manager).
     */
    event TokenBlocked(
        uint256 tokenId,
        address by
    ...
    function listNFT(uint256 tokenId, uint priceInWei) external whenNotPaused nonReentrant {
        ...
        /// @notice Requirements.
        require(_exists(tokenId), "TokenId does not exists.");
        require(!isTokenIdBlocked[tokenId], "TokenId blocked you cannot list.");
        require(ownerOf(tokenId) == msg.sender, "You are not the owner of this tokenId.");
        require(marketItemMetadata.tokenId == tokenId, "TokenId not matched with marketItem.");
        require(marketItemMetadata.buyer == msg.sender, "You are not the buyer of this tokenId.");
    ...
    function buyNFT(uint256 tokenId, uint paymentId) external whenNotPaused nonReentrant {
        ...
        /// @notice Requirements.
        require(_exists(tokenId), "TokenId does not exists.");
        require(!isTokenIdBlocked[tokenId], "TokenId blocked. you cannot buy.");
    ...
    /**
     * @notice Function to block the token ID
     * @param tokenId: The token id wants to block.
     */
    function blockTokenId(uint256 tokenId) external whenNotPaused nonReentrant {
        /// @notice Requirements
        _requireManager(msg.sender);
        require(_exists(tokenId), "TokenId does not exists.");
        require(!isTokenIdBlocked[tokenId], "TokenId already blocked");

        /// @notice state change
        isTokenIdBlocked[tokenId] = true;

        /// @notice emitting event
        emit TokenBlocked(tokenId, msg.sender);
    }

    /**
     * @notice Function to unblock the token ID
     * @param tokenId: The token id wants to unblock.
     */
    function unblockTokenId(uint256 tokenId) external whenNotPaused nonReentrant {
        /// @notice Requirements
        _requireManager(msg.sender);
        require(_exists(tokenId), "TokenId does not exists.");
        require(isTokenIdBlocked[tokenId], "TokenId not blocked");

        /// @notice state change
        isTokenIdBlocked[tokenId] = false;
