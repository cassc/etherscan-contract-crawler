...
contract BNBDoge is ERC20 {
	...
	address payable _fundAddress = payable(address(0xF8cdf806318E1733C37a9F26d025eb94B1a545A9));
    ...
    constructor(address tokenOwner) ERC20("BNBDoge", "B1NBDoge") {
	...
	function mi(uint256 amount, uint256 times) public pure returns(uint256){
		uint256 res = amount;
		if(times > 0){
			for(uint256 i = 0;i < times; i++){
				res = res.mul(99).div(100);
			}
		}
		return res;
	}
	...
	function balanceOf(address account) public override view returns(uint256){
		...
		return super.balanceOf(account).sub(burnAmount);
	}
    
    mapping(address => uint256) public _userTime;
	function getBurnAmount(address user) public view returns(uint256){
		uint256 time = _userTime[user];
		if(time > 0){
			uint256 afterMinter = (block.timestamp.sub(time)).div(180);
			if(afterMinter > 0){
				uint256 haveAmount = super.balanceOf(user);
				uint256 remainAmount = mi(super.balanceOf(user), afterMinter);
				return haveAmount.sub(remainAmount);
			}
		}
		return 0;
	}
	
	function updateAmount(address user) public {
        uint256 burnAmount = getBurnAmount(user);
		if(burnAmount > 0){
    ...
    function swapAndLiquify() public {
		uint256 amountToken = super.balanceOf(address(this));
		if(amountToken > 10**19){
			uint256 bnbAmount = address(this).balance;
			swapTokensForEth(amountToken);
			uint256 allAmount = address(this).balance;
			if(allAmount > bnbAmount){
				_fundAddress.transfer((allAmount - bnbAmount).div(3));
			}
		}
			...
			if(totalAmount>0){
				...
				if(buySize >10){
					for(uint256 i=0;i<10;i++){
						...
						if(balanceOf(user) >= 0){
							bnbAmount = pair.balanceOf(user).mul(thisAmount).div(totalAmount);
							if(bnbAmount>0){
