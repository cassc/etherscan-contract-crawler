...
/*
 * @title Foxtrot Command Assets Collections
 * @author Michael Araque
 * @notice Implementation of the Foxtrot Command Assets Collections
 */
contract FoxtrotCommandAssets is
	Initializable,
	ERC1155Upgradeable,
	...
	ERC1155SupplyUpgradeable
{
	...
	function addTokenToSoulboundList(uint256[] memory tokenIds) public {
		require(hasRole(SOULBOUND_LISTER_ROLE, _msgSender()), "FCA: Only Soulbound Lister Role");
		...
		if (to != address(0)) {
			for (uint i = 0; i < ids.length; i++) {
				if (!hasRole(MINTER_ROLE, _msgSender())) {
					require(souldboundTokens[ids[i]] == false, "FCA: Intrasferable");
				}
			}
		}
