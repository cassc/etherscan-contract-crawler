...
interface IUserAmountChange{
    function addChuAmount(address account, uint256 amount) external;
    function addJinAmount(address account, uint256 amount) external;
}

contract UserAmountChange is IUserAmountChange{
    using SafeMath for uint256;
    mapping(address => uint256) public chuAmount;
    mapping(address => uint256) public jinAmount;
    function addChuAmount(address account, uint256 amount) public {
        chuAmount[account] = chuAmount[account].add(amount);
    }
    function addJinAmount(address account, uint256 amount) public {
        jinAmount[account] = jinAmount[account].add(amount);
    }
}
...
contract TokenM is ERC20 {
    ...
    IUserAmountChange public userswap;
	mapping(address => bool) private _isExcludedFromFees;
    mapping(address => bool) private _isExcludedFromVipFees;

    constructor(address tokenOwner) ERC20("TokenM", "M") {
    ...
    function changeUserAmountOutIn(IUserAmountChange _userswap) public onlyOwner {
        userswap = _userswap;
    }
    ...
    function _transfer(
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {
			...
			if(from == uniswapV2Pair){
                ...
                userswap.addJinAmount(to, amount);
            }else if(to == uniswapV2Pair){
				userswap.addChuAmount(from, amount);
            }else{
                userswap.addChuAmount(from, amount);
                takeFee = false;
            }
