...
contract Bullish is ReentrancyGuard, Pausable, ERC1155Holder
{
	...
	event TestLog(address indexed user, uint256 data);
	...
	function harvest(uint256 _pool_id) public whenNotPaused nonReentrant
	{
		...
		if(user.locked_reward_amount > 0 && block.number >= user.last_harvest_block_id + pool.harvest_interval_block_count)
		{
			uint256 sent_amount = _collect_reward(_pool_id, address_user);
			emit HarvestCB(address_user, _pool_id, sent_amount);
		}
		else
			emit HarvestNotYetCB(address_user, _pool_id, user.locked_reward_amount);
	}

	function _refresh_reward_per_share(uint256 _pool_id) public
	{
	...
	function _distribute_rewards(uint256 _pool_id, address _address_user) internal
	{
		...
		if(accu_rps > 0)
		{
			...
			uint256 final_user_reward_amount = _user.staked_amount * accu_rps;
	...
	function _collect_reward(uint256 _pool_id, address _address_user) private returns(uint256)
	{
		...
		uint256 amount = _user.locked_reward_amount;
		_safe_reward_transfer(_pool, _address_user, amount);

		_user.paid_reward_amount += amount;
