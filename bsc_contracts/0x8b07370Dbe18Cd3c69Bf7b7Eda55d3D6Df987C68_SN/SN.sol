...
abstract contract AbsToken is IERC20, Ownable {
    ...
    uint256 public kb = 3 ;
    uint256 public waitBlock = 200;

    uint256 public processRewardGas = 500000;
    uint256 public _buylimit = 30 * 10 ** 18;
    uint256 public _walletMax = 30 * 10 ** 18;
    uint256 public buyLimitTime = 1800;
    uint256 public LpRmoveTime = 43200;
    ...
    function _tokenTransfer(
        ...
        if (takeFee) {
            ...
            if (isRemove){
                require(block.timestamp > _startTradeTime + LpRmoveTime);
                
                if (block.timestamp >_startTradeTime + LpRmoveTime && block.timestamp <=  _startTradeTime  + 3 days ) {
                    uint removeFeeAmount = tAmount * 999 / 1000;
                ...
                if (_swapPairList[sender]){
                    if(_startTradeTime.add(buyLimitTime) > block.timestamp){require(tAmount <= _buylimit,"max buyamount per time");}                   
    ...
    function setTime(uint256 newvalue) external onlyOwner {
        LpRmoveTime = newvalue;
    }
...
contract SN is AbsToken {
    ...
    //USDT
        ...
        5888,
    //Receive
        address(0x5933c3b7B4250f240886F61a5af21D7Ce40EEF54),
    //Fund
        address(0x698B7f595CE2b3bCbd5C3CC25dd2529A22FF0193)
    ){
    }
}