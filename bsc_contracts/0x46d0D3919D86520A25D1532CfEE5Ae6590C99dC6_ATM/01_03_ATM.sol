...
contract ATMPool is WithdrawAble {
    ...
    ATMPowerInterface public atmPower = ATMPowerInterface(0xFc0e203ED9D288b4e2f70F1aDefcd53265662AeE);

    MerchantInterface public merchant = MerchantInterface(0xD99a05A1C3176675Be1f264250F6E3279b3Ea2b8);
    ...
    function distributeRewardPower(address _user,uint256 _amount) internal {
        ...
        for (uint j = 0; j < _layerHashMax; j++) {
            ...
            if(user.amount>= inviteRewardMinPledgeAmount || isWhiteListed[cur]){
                uint256 pendingReward = 0;
...
contract ATM is ATMPool {
    ...
    uint[] public _userBuyFeeDispathRate = [0,0,0,0,0];
    uint[] public _userSellFeeDispathRate = [1000,1000,2000,3000,3000];
    ...
    uint[] public _merchantBuyFeeDispathRate = [100,100,200,300,300];
    ...
    function _takeBuyFee(address _account,uint256 tAmount) public returns(uint allocatedAmount){
        uint curTAmount = 0;
    ...
    function _takeSellFee(address _account,uint256 tAmount) public returns(uint allocatedAmount){
        uint curTAmount = 0;
    ...
    function _sellFee(address sender, address recipient, uint amount) public {
    ...
    function _buyFee(address sender, address recipient, uint amount) public {
