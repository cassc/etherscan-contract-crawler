...
contract BakedBeansV2 is Initializable, UUPSUpgradeable, OwnableUpgradeable {
    ...
    function buyBeans(address ref) public payable {
        ...
        if (baker.hasReferred && baker.totalDeposit >= MIN_REF_DEPOSIT_FOR_BONUS && refExists(baker.adr, baker.upline) == false) {
    ...
    function refExists(address ref, address upline) private view returns (bool) {
        for (uint i = 0; i < bakers[upline].bonusEligibleReferrals.length; i++) {
    ...
    function eat() public {
        ...
        if (
            ...
            uint256 afterTax = addWithdrawalTaxes(baker.adr, beansInBnbBeforeFee);
