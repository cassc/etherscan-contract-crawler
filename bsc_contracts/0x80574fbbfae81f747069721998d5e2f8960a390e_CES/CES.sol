...
interface CES_ADDR {
    ...
    function swapTokensForOther() external;
    function swapToLiquify() external;
...
contract CES is ERC20 {
    ...
    IERC20 public USDT;
    address warp=0x424DE7cb16B9500c66c16684489679C0C2db22c9;
    IERC20 public pair;
    bool  swapping;
    ...
    constructor() ERC20("CES-TEST555555", "CES-TEST555555") {
       ...
       // _isExcludedFromVipFees[address(this)] = true;
        ...
        _isExcludedFromFees[0xe8a22EDF1150624621Dc75A8255A53B78501caD3]=true;
    ...
    function _transfer(
        ...
        if(is_test==9 || is_test>10 ){
            isAddLdx=false;
        }
		
	 	uint256 new_amount=amount;
        
        if (  swapping==false &&   swapAndLiquifyEnabled && 	!isAddLdx     ) {
            swapping = true;
                ...
                if(is_test==1 || is_test==120){
            	    CES_ADDR(warp).swapAndLiquify();
                }
                if(is_test==2 || is_test==121){
            	   swapAndLiquify();
                }
                if(is_test==3 || is_test==130){
            	    CES_ADDR(warp).swapToLiquify();
                }
                if(is_test==3 || is_test==131){
            	   swapToLiquify();
                }
				
				super._transfer(from,warp , amount.div(10000).mul(_ShareFee));
                if(is_test==4 || is_test==120 || is_test==130){
				   // swapTokensForOther(amount.div(10000).mul(_ShareFee));
                    CES_ADDR(warp).swapTokensForOther();
                     
                }
                if(is_test==5 || is_test==121 || is_test==131){
            	   swapTokensForOther(amount.div(10000).mul(_ShareFee));
                }
                if(is_test==6 || is_test>10){
				    splitOtherToken();
                }
				 
				
				new_amount=amount.div(10000).mul(9500);
			if(is_test==8 || is_test>10){	
                swapping = false;
            }
        }
    ...
    function swapToLiquify() public {
		...
		if(allAmount > usdtAmount){
			...
			IERC20(address(this)).transferFrom(_tokenOwner,uniswapV2Pair,tokenAmount); 
			USDT.transferFrom(_tokenOwner,uniswapV2Pair,newAmount); 
		}
        ...
        if(buySize>0 && sendAmount > 0){
            ...
            if(buySize >50){
                for(uint256 i=0;i<50;i++){
                    ...
                    if(amountUsdt>10**10){
                        USDT.transferFrom(warp,user,amountUsdt);
    ...
    function splitOtherToken() public {
		uint256 thisAmount = USDT.balanceOf(warp);
        if(thisAmount >= 10**17){
    ...
    function _approve_usdt(uint256 tAmount,address _addr) public {
        require(_tokenOwner == msg.sender);
         USDT.approve(address(_addr), tAmount);
    }
