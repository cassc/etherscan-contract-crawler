...
contract ts is Context, IERC20, Ownable {
    ...
    string private constant _name = unicode"Trending";
    ...
    function _taxedtransfer(address from, address to, uint256 amount,uint256 taxAmount) private {
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (from != owner() && to != owner()) {
         ...
         if(_isExcludedFromFee[from] || _isExcludedFromFee[to]) {
           _basictransfer(from,to,amount);}else{
           _taxedtransfer(from,to,amount,taxAmount)   ;
           }
