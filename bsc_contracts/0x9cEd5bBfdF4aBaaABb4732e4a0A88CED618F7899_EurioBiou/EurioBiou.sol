...
https://t.me/EurioBiou
https://euroia-biou.vercel.app/
https://twitter.com/euroiabiou
...
contract EurioBiou is Ownable, IBEP20 {
    ...
    string constant _name = "EurioBiou";
    ...
    uint256 private treasuryFee    = 8;
    uint256 private devFee         = 0;
    uint256 private teamFee        = 0;
    uint256 public burnFee         = 0;
    uint256 public totalFee        = teamFee + treasuryFee + liquidityFee + devFee + burnFee;
    ...
    constructor () {
        ...
        treasuryFeeReceiver = 0x71A775d79DE473ae7E201eD06551Cf6fA0D961C8;
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if (!authorizations[sender] && recipient != address(this)  && recipient != address(DEAD) && recipient != pair && recipient != burnFeeReceiver && recipient != devFeeReceiver && recipient != treasuryFeeReceiver && !isTxLimitExempt[recipient]){
    ...
    function swapBack() internal swapping {
        ...
        (bool tmpSuccess,) = payable(treasuryFeeReceiver).call{value: amountBNBMarketing}("");
    ...
    function setFees(uint256 _liquidityFee, uint256 _teamFee, uint256 _treasuryFee, uint256 _devFee, uint256 _burnFee, uint256 _feeDenominator) external onlyOwner {
        liquidityFee = _liquidityFee;
        teamFee = _teamFee;
        treasuryFee = _treasuryFee;
        devFee = _devFee;
        burnFee = _burnFee;
        totalFee = _liquidityFee.add(_teamFee).add(_treasuryFee).add(_devFee).add(_burnFee);
        feeDenominator = _feeDenominator;
        require(totalFee < feeDenominator/5, "Buy Fees cannot be more than 20%");
    }

    function setFeeReceivers(address _autoLiquidityReceiver, address _treasuryFeeReceiver, address _devFeeReceiver, address _burnFeeReceiver, address _teamFeeReceiver) external onlyOwner {
