...
contract GovernorsFarm is Owned, ReentrancyGuard {
    ...
    uint256 public totalSupplyMultiplier = 1.5e18; //total supply multiplier to adjust for vdush total supply calc on bnb chain
    ...
    function vdUshShare(address user) public view returns (uint256) {
        return vdUSH.balanceOf(user) * 1e18 / vdUSH.totalSupply();
    }
