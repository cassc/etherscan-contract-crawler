...
// @Title JustMoney Bridged Token - Centiiv Token
...
contract CentiivToken is BridgeOracle, IERC20 {
    mapping(address => uint256) private _balances;

    mapping(address => mapping(address => uint256)) private _allowances;

    uint256 private constant _max = 100000000 * (10**18);
    ...
    modifier belowCap(uint256 amount) {
        require(
            amount <= (_max - _totalSupply - _totalBurned),
            Errors.ABOVE_CAP
        );
        _;
    }
    ...
    function _transfer(
        ...
        address to,
        uint256 amount
    ) internal notZeroAddress(from) notZeroAddress(to) {
    ...
    function _approve(
        ...
        address spender,
        uint256 amount
    ) private notZeroAddress(owner) notZeroAddress(spender) {
    ...
    function burnFrom(address account, uint256 amount) public {
        require(
            amount <= _allowances[account][_msgSender()],
            Errors.NOT_APPROVED
        );
    ...
    function withdrawBNB(address payable recipient)
        external
        onlyOwner
    ...
    function withdrawERC20token(address _token, address payable recipient)
        external
        onlyOwner
        notZeroAddress(recipient)
        returns (bool)
