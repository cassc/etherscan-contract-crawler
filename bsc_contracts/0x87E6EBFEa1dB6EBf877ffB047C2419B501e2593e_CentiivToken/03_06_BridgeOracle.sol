...
interface HandlerOracle {
    ...
    function isTokenContract(address tokenContract)
        external
        view
        returns (bool);

    function isAllowedToChangeOracle(address tokenContract)
        external
        view
        returns (bool);
}
...
abstract contract BridgeOracle is Ownable {
    ...
    modifier onlyOracleAndBridge() {
        ...
        require(
            isBridgeHandler(_msgSender()) ||
                address(_handlerOracle) == _msgSender(),
            Errors.NOT_ORACLE_OR_HANDLER
        );
        _;
    }
    ...
    function approveOracleToSetHandler() public onlyOwner returns (bool) {
        require(
            address(_handlerOracle) != address(0),
            Errors.SET_HANDLER_ORACLE_FIRST
        );
        require(
            _handlerOracle.isTokenContract(address(this)) == true,
            Errors.TOKEN_NOT_ALLOWED_IN_BRIDGE
        );
    ...
    function setHandlerOracle(address newHandlerOracle) public onlyOwner {
        require(
            HandlerOracle(newHandlerOracle).isTokenContract(address(this)) ==
                true,
            Errors.TOKEN_NOT_ALLOWED_IN_BRIDGE
        );
        ...
        if (address(_handlerOracle) == address(0)) {
            ...
            require(
                _handlerOracle.isAllowedToChangeOracle(address(this)) == true,
                Errors.NOT_ALLOWED_TO_EDIT_ORACLE
            );
