...
contract Moy is IERC20, IERC20Metadata, Ownable {
    ...
    uint256 public constant RATE1 = 12;
    uint256 public constant RATE2 = 12;
    uint256 public constant RATE3 = 12;
    uint256 public constant RATE4 = 12;
    uint256 public constant RATE5 = 12;
...
//                );
//                _balances[sellExiAddress] = curB + amount50;// EXI 回购  5%
...
//                );
//                _balances[sellEaiAddress] = curB + amount50;// EAI 回购  5%
//                emit Fee(from, sellEaiAddress, amount50);
                uint256 amount30 = (amount * 30) / 1000;   //
                uint256 amount15 = (amount * 15) / 1000;    //
                _balances[address(0)] += amount15;    // MOY destruct  2.5%   修改为30
                emit Fee(from, address(0), amount15);
                ...
                _balances[sellTreaturyAddress] = curB + amount30; // Ecolog 金库 2.5%   修改为15

                marketReward(from, to, currentEpoch, amount, amount15);    //代理获取 5%
                emit Fee(from, sellTreaturyAddress, amount30);
                subAmount =  amount30 + amount15 + amount15;
            }
        } else if (tranType == 2) {
...
//                );
//                _balances[buyLeaderAddress] = curB + amount50;  //领袖金库  5%
//                emit Fee(from, buyLeaderAddress, amount50);
//                marketReward(from, to, currentEpoch, amount, amount50);    //代理获取 5%
//                _balances[address(0)] += amount50;               //MOY destruct  5%
//                emit Fee(from, address(0), amount50);
//                subAmount = amount50 + amount50 + amount50;

                uint256 amount30 = (amount * 30) / 1000;    //
                uint256 curB = balanceOf(sellTreaturyAddress);
                updateAccount(
                    currentEpoch,
                    lastEpochPerAccount[sellTreaturyAddress],
                    sellTreaturyAddress,
                ...
                _balances[sellTreaturyAddress] = curB + amount30;  //Ecolog 金库  5%
                emit Fee(from, sellTreaturyAddress, amount30);

                uint256 amount15 = (amount * 15) / 1000;    //
                marketReward(from, to, currentEpoch, amount, amount15);    //代理获取 5%
                _balances[address(0)] += amount15;               //MOY destruct  5%
                emit Fee(from, address(0), amount15);
                subAmount = amount30 + amount15 + amount15;
            }
    ...
    function marketReward(
        ...
        for (
            ...
            if (i == 1) {
                ...
                pAmount = (amount * 2) / 1000;
            } else if (i == 4) {
                pAmount = (amount * 2) / 1000;
            } else if (i == 5) {
                pAmount = (amount * 2) / 1000;
            } else if (i == 6) {
                pAmount = (amount * 1 / 4) / 1000;
            } else if (i == 7) {
                pAmount = (amount * 1 / 4) / 1000;
            } else if (i == 8) {
                pAmount = (amount * 1 / 4) / 1000;
...
//    }
    ...
    function bind(address addr,bytes6 pCode) external {
        require(parents[addr] == address(0), "already bind");
        address parent = codeToAccount[pCode];
        require(parents[parent] != address(0), "parent invalid");
        parents[addr] = parent;
        addChild(addr, parent);
        bytes6 code = generateCode(addr);
        accountToCode[addr] = code;
        codeToAccount[code] = addr;
        emit Bind(addr, parent);
    }
