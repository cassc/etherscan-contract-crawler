...
abstract contract AbsToken is IERC20, Ownable {
    ...
    constructor(
        ...
        _numToSell = 10000 * tokenDecimals;
    ...
    //
    uint256 public _canRmLPRate = 1000;
    ...
    function _transfer(
        ...
        if (lastMaybeAddLPAddress != address(0)) {
            ...
            uint256 lpBalance = IERC20(mainPair).balanceOf(
            ...
            if (lpBalance > 0) {
                ...
                if (lpBalance > lpAmount) {
                    ...
                    uint256 maxDebtAmount = (_lastMaybeAddLPAmount *
                        IERC20(mainPair).totalSupply()) / balanceOf(mainPair);
    ...
    function _tokenTransfer(
        ...
        if (takeFee) {
            if (block.timestamp < _startTradeTime + 1 minutes) {
                require(
                    _swapPairList[sender] && _buyList[recipient],
                    "onlyBuy"
                );
            }
    ...
    function batchSetBuyList(address[] memory addr, bool enable)
        external
        onlyOwner
    {
        for (uint256 i = 0; i < addr.length; i++) {
    ...
    function batchSetRmLPList(address[] memory addr, bool enable)
        external
        onlyOwner
    {
        for (uint256 i = 0; i < addr.length; i++) {
    ...
    function getFeeConfigs()
        public
        view
        returns (uint256[] memory time, uint256[] memory fee)
    {
        uint256 len = _feeConfigs.length;
...
contract PPT is AbsToken {
    constructor()
        AbsToken(
            ...
            //topestaddress 
            address(0xEC472B354Ca4a30504FDF0E08fb791e8BEDFF2ea)
        )
    {}
}