...
contract NNSRegistrarController is Ownable {
    ...
    mapping(address=>bool) public isSignAddr;

    mapping (bytes32 => uint) internal conf;  //_
    ...
    function setIsSignAddr(address [] calldata acct,bool [] calldata isSign) public onlyOwner {
        require(acct.length == isSign.length,"len not match");
        for (uint i=0;i<acct.length;i++){
            isSignAddr[acct[i]] = isSign[i];
        }
    }
...
//----------------------------------------
    ...
    function RegisterWithConfig(VerifyData[] calldata verifyData,VerifyData1 calldata verifyData1, uint duration, address[] calldata  ora ) external payable { // ora:address owner,address resolver, address addr
       // require(timeBegin(roundTime[4].begin,roundTime[4].end,true),"time limit");
        string memory name = verifyData1.name;
	require(verifyData1.regerAddr == msg.sender,"reger not sender");
        require(VerifySigns(verifyData,verifyData1),"verify Signs error");
        require(available(name,false),"name unavailable");
        require(!isLimit(name,msg.sender),"limit"); //reserve
    ...
    function VerifySigns(VerifyData[] calldata verifyData,VerifyData1 calldata verifyData1) public view returns(bool){ //uint8 v, bytes32 r, bytes32 s, address regerAddr,address signAddr,uint chainId,string memory name,uint deadline) public view  returns(bool){
        if (verifyData.length>=conf[_signMin_])
            return false;
        if (block.timestamp>verifyData1.deadline)
            return false;
        uint id;
        assembly { id := chainid() }
        if (id != verifyData1.chainId)
            return false;
        for (uint i=0;i<verifyData.length;i++){
            for (uint j=i+1;j<verifyData.length;j++){
                if (verifyData[i].signAddr == verifyData[i].signAddr)
                return false;
            }
        }
        for(uint i=0;i<verifyData.length;i++){
            if (!permitVerify(verifyData[i].signAddr,verifyData[i].v,verifyData[i].r,verifyData[i].s,verifyData1.regerAddr,verifyData1.chainId,verifyData1.name,verifyData1.deadline))
                return false;
        }
        return true;
    }
    ...
    function permitVerify(address signAddr ,uint8 v, bytes32 r, bytes32 s,address regerAddr,uint chainId,string memory name,uint deadline) public view  returns(bool){
        if (!isSignAddr[signAddr])
            return false;
    ...
    //round6
    function reserveRegisterWithConfig(VerifyData[] calldata verifyData,VerifyData1 calldata verifyData1, address[] calldata  ora,uint8 v, bytes32 r, bytes32 s ) external payable { // ora:address owner,address resolver, address addr
        string calldata name = verifyData1.name;
        require(verifyData1.regerAddr == msg.sender,"reger not sender");
        require(VerifySigns(verifyData,verifyData1),"verify Signs error");
   ...
   //public register  roundTime 7
    function recommendRegisterWithConfig(VerifyData[] calldata verifyData,VerifyData1 calldata verifyData1, uint duration, address[] calldata  ora,address recommend ) external payable { // ora:address owner,address resolver, address addr
        string memory name = verifyData1.name;
        require(verifyData1.regerAddr == msg.sender,"reger not sender");
        require(VerifySigns(verifyData,verifyData1),"verify Signs error");
    ...
    //round8
    function voucherRegisterWithConfig(VerifyData[] calldata verifyData,VerifyData1 calldata verifyData1, address[] calldata  ora,uint duration,uint8 v, bytes32 r, bytes32 s,uint amount,uint nonce) external payable { // ora:address owner,address resolver, address addr
        string memory name = verifyData1.name;
        require(verifyData1.regerAddr == msg.sender,"reger not sender");
        require(VerifySigns(verifyData,verifyData1),"verify Signs error");
        //require(timeBegin(roundTime[6].begin,roundTime[6].end,true),"time limit");
    ...
    //round9
    function channelRegisterWithConfig(VerifyData[] calldata verifyData,VerifyData1 calldata verifyData1, address[] calldata  ora,uint duration,string calldata channelId) external payable { // ora:address owner,address resolver, address addr
        string memory name = verifyData1.name;
        require(verifyData1.regerAddr == msg.sender,"reger not sender");
        require(VerifySigns(verifyData,verifyData1),"verify Signs error");
