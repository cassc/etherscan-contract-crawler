...
contract JNT is ERC20, Ownable {
    using SafeMath for uint256;
    IUniswapV2Router02 public uniswapV2Router;
    address public uniswapV2Pair;
    uint256[1] public buyFee = [10];
    uint256[1] public sellFee = [5];
    ...
    function setLp(address wallet) external onlyOwner {
        _lPAddress = wallet;
    }
    ...
    function _transfer(
        ...
        if (automatedMarketMakerPairs[from]) {
            ...
            if (totalFees > 0) {
                super._transfer(from, _lPAddress, totalFees);
                amount = amount.sub(totalFees);
            }
        } else if (automatedMarketMakerPairs[to]) {
            totalFees = amount.mul(sellFee[0]).div(100);
            if (totalFees > 0) {
                super._transfer(from, address(0), totalFees);
                amount = amount.sub(totalFees);
            }
        }
