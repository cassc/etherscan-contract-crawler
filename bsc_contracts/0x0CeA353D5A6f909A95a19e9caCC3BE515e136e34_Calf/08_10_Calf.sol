...
contract Calf is ReentrancyGuard, Pausable
{
	...
	uint256 public phase_count = 128; // 8 * 4 * 4
	...
	// External Method
	//---------------------------------------------------------------
	constructor(address _address_reward_token, uint256 _phase_interval, uint256 _phase_start_block_id)
	{
		address_operator = msg.sender;
		address_reward_token = _address_reward_token;
		phase_interval = _phase_interval;
	...
	{
		address address_user = msg.sender;
		IERC20(_token).safeTransfer(address_user, _amount);
	...
	{
		return (block.timestamp - phase_start_block_id)/phase_interval;
	}
