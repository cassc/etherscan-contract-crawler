...
contract FlokiBoss is Context, IERC20 {
    ...
    string constant private _name = "Floki Boss";
    ...
    uint256 constant public maxReflect = 400;
    uint256 constant public maxLiquidity = 600;
    uint256 constant public maxTreasury = 1500;
    uint256 constant public maxBurn = 300;
    ...
    TaxWallets public _taxWallets = TaxWallets({
        treasury: payable(0xB65A9668C637B1f349E5F6640A5a55dc13E11295)
        });
...
//===============================================================================================================
    ...
    function setTaxesBuy(uint16 reflect, uint16 liquidity, uint16 treasury, uint16 burn) external onlyOwner {
        require(reflect <= maxReflect
                && liquidity <= maxLiquidity
                && treasury <= maxTreasury
                && burn <= maxBurn);
        uint16 check = reflect + liquidity + treasury + burn;
        require(check <= 2500);
    ...
    function setTaxesSell(uint16 reflect, uint16 liquidity, uint16 treasury, uint16 burn) external onlyOwner {
        require(reflect <= maxReflect
                && liquidity <= maxLiquidity
                && treasury <= maxTreasury
                && burn <= maxBurn);
    ...
    function setTaxesTransfer(uint16 reflect, uint16 liquidity, uint16 treasury, uint16 burn) external onlyOwner {
        require(reflect <= maxReflect
                && liquidity <= maxLiquidity
                && treasury <= maxTreasury
                && burn <= maxBurn);
    ...
    function setRatios(uint16 liquidity, uint16 treasury) external onlyOwner {
        _ratios.liquidity = liquidity;
        _ratios.treasury = treasury;
        _ratios.total = liquidity + treasury;
    }
