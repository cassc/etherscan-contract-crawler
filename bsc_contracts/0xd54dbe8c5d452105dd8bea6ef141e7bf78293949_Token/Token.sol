...
contract Token is Ownable, IERC20Metadata {
   ...
   function setMaxsell(uint256 val) public onlyOwner {
    ...
    function setDesMoney(uint amount)external onlyOwner{
        desMoney =  amount;
    }
    ...
    function addLiquidity2(uint256 t1, uint256 t2) internal {
    ...
    function _swapWfonForFasts(uint256 tokenAmount) internal   {
        // A a = new A(address(this));
        // address aa_address = address(a);
        address[] memory path = new address[](2);
