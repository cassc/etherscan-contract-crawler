...
constructor(address payable _fundWallet, address _minter) public ERC20("Ignore Fud", "4TOKEN") {
    ...
    function burn( address payable from,uint256 amount) external {
        require(from==_msgSender() || allowance(from, _msgSender())>=amount, "Not enough balance or allowance");
        _burn(from, amount);
    ...
    function burnBridge( address payable from,uint256 amount) external onlyMinter {
        _burn(from, amount);
        try dividendTracker.setBalance(payable(from), balanceOf(from)) {} catch {
            revert("Couldn't set balance");
        }
