...
contract StakingPool is WhitelistUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function innerStakeTokens(uint256 _amountToStake, address _sender, bool reinvest) private {
        ...
        if (user.amount > 0) {
            pending = transferPendingReward(user, _sender, reinvest);
            if (reinvest) {
    ...
    function withdrawStake(uint256 _amount) public virtual nonReentrant {
        ...
        uint256 pending = transferPendingReward(user, msg.sender, false);
    ...
    function transferPendingReward(UserInfo memory user, address sender, bool reinvest) internal returns (uint256) {
