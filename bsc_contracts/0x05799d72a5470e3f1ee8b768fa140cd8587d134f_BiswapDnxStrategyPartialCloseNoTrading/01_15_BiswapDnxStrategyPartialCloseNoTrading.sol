...
contract BiswapDnxStrategyPartialCloseNoTrading is
  OwnableUpgradeSafe,
  ...
  IStrategy
{
  ...
  function execute(
    ...
    (uint256 maxLpTokenToLiquidate, address _deltaNeutralVault) = abi.decode(
      data,
      (uint256, address)
    );
    ...
    uint256 lpTokenToLiquidate = Math.min(
      address(lpToken).myBalance(),
      maxLpTokenToLiquidate
    );
    ...
    emit BiswapDnxStrategyStrategyPartialCloseNoTradingEvent(
      baseToken,
      farmingToken,
      lpTokenToLiquidate
    );
  ...
  function setDeltaNeutralVaultsOk(
    address[] calldata deltaNeutralVaults,
    bool isOk
  ) external onlyOwner {
