...
contract Rugged is ERC20, Ownable, ERC20Burnable {
    ...
    address public LP_recipient          = 0x58acaA266f20D59F423B91282494c4CBA7037a3d;
    address public MARKETING_WALLET      = 0x4c3a786D165dC1336484Ee370c7585D2AFC90D6b;
    address public WBNB;
    ...
    constructor() ERC20("Rugged", "RUGGED") {
    ...
    modifier antiSniper(address from, address to, address callee){
        ...
        if(!_excludedFromAntiSniper[from]
            && !_excludedFromAntiSniper[to] && !_excludedFromAntiSniper[callee]) {
                require(!(size1 > 0 || size2 > 0 || size3 > 0),"Bag Holder Antibot: Sniper Detected");
            }
        _;
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual override antiSniper(sender, recipient, msg.sender) {
        ...
        if (recipient != owner() && sender != owner() && !isAutomatedMarketMakerPair[recipient]) {
            ...
            require(balanceOf(recipient) + adjAmt < totalSupply().mul(150).div(10000), "BEP20: user cannot hold more than 1.5% of the total supply");
        }
    ...
    function swapAndLiquify(uint256 amount) private {
        ...
        // swap tokens for BNB
        swapTokensForEth(half); // <- this breaks the BNB -> Bag Holder swap when swap+liquify is triggered
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != pancakeV2Pair, "RUGGED: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(isAutomatedMarketMakerPair[pair] != value, "RUGGED: Automated market maker pair is already set to that value");
