...
import { LevelXVolatile } from "./LevelXVolatile.sol";

contract LevelXCompoundSilo
{
 ...
 */
contract LevelXCompound is Initializable, Ownable, ReentrancyGuard
{
	...
	struct AccountInfo {
		...
		uint256 reward; // LVLX reward accumulated but not claimed
	...
	constructor(address payable _reserveToken, address _boostToken1, address _boostToken2, address _levelXVolatile, address _stakingCompound, address _partnerVolatile)
	{
		initialize(msg.sender, _reserveToken, _boostToken1, _boostToken2, _levelXVolatile, _stakingCompound, _partnerVolatile);
	}

	function initialize(address _owner, address payable _reserveToken, address _boostToken1, address _boostToken2, address _levelXVolatile, address _stakingCompound, address _partnerVolatile) public initializer
	{
		...
		silo = address(new LevelXCompoundSilo(_reserveToken));
		...
		levelXVolatile = _levelXVolatile;
	...
	function setLevelXVolatile(address _levelXVolatile) external onlyOwner
	{
		require(_levelXVolatile != address(0), "invalid address");
		levelXVolatile = _levelXVolatile;
	}
	...
	function setDripRatePerDay1(uint256 _dripRatePerDay1, uint256 _dripBoostRatePerDay1) external onlyOwner
	{
		require(_dripRatePerDay1 + _dripBoostRatePerDay1 <= 100e16, "invalid rate");
	...
	function setDripRatePerDay2(uint256 _dripRatePerDay2, uint256 _dripBoostRatePerDay2) external onlyOwner
	{
		require(_dripRatePerDay2 + _dripBoostRatePerDay2 <= 100e16, "invalid rate");
	...
	function _deposit(address _sender, uint256 _amount, address _account) internal
	{
		...
		IERC20(reserveToken).safeApprove(levelXVolatile, _1percent);
		LevelXVolatile(levelXVolatile).rewardAll(_1percent);
	}
	...
	function withdraw(uint256 _amount) external nonReentrant
	{
		...
		if (whitelist[msg.sender] || whitelistAll) {
			...
			_ensureBalance(_netAmount + _1percent);
	...
	function _compound(address _account, uint256 _i) internal returns (uint256 _rewardAmount)
	{
		...
		if (_rewardAmount > 0) {
			...
			if (_i == 1) {
				IERC20(_rewardToken).safeApprove(stakingCompound, _rewardAmount);
				LevelXCompound(stakingCompound).depositOnBehalfOf(_rewardAmount, _account);
			}
			else
			if (_i == 2) {
	...
	function _updateDay() internal
	{
		...
		uint256 _ratePerDay1 = dripRatePerDay1 + 2 * dripBoostRatePerDay1;
		if (_ratePerDay1 > 0) {
			// calculates the percentage of the drip pool and distributes
			{
				...
				if (totalStaked > 0) {
					accDripPerShare1 += _amountDrip * 1e36 / totalStaked;
					allocDrip1 += _amountDrip;
				}

				uint256 _amountBoost = (_amount - _amountDrip) / 2;
				if (totalBurned1 > 0) {
					accBoostPerShare1 += _amountBoost * 1e36 / totalBurned1;
					totalDrip1 -= _amountBoost;
					totalBoost1 += _amountBoost;
				}
				if (totalBurned2 > 0) {
					accBoostPerShare2 += _amountBoost * 1e36 / totalBurned2;
					totalDrip1 -= _amountBoost;
					totalBoost2 += _amountBoost;
				}
		...
		uint256 _ratePerDay2 = dripRatePerDay2 + 2 * dripBoostRatePerDay2;
		if (_ratePerDay2 > 0) {
			// calculates the percentage of the compound drip pool and distributes
			{
				...
				if (totalStaked > 0) {
					accDripPerShare2 += _amountDrip * 1e36 / totalStaked;
