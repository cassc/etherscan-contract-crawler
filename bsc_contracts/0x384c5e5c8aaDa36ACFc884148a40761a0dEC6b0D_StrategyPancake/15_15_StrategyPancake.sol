...
contract StrategyPancake is Ownable, Pausable, IStrategyPancake {
    ...
    address constant public BASE_TOKEN = 0x8AC76a51cc950d9822D68b83fE1Ad97B32Cd580d;

    address constant public USDT = 0x55d398326f99059fF775485246999027B3197955;

    ///@notice The strategy's staking token. - LP
    address constant public WANT = 0xEc6557348085Aa57C72514D67070dC863C0a5A8c;

    ///@notice Address of Pancakeswap's Masterchef.
    address constant public MASTERCHEF = 0xa5f8C5Dbd5F286960b9d90548680aE5ebFf07652;
    ...
    address[] public token0ToBase = [USDT, BASE_TOKEN];

    address[] public token1ToBase = [BASE_TOKEN];
    ...
    constructor(
        ...
        IERC20(BASE_TOKEN).safeApprove(address(uniV2Router), type(uint256).max);
    ...
    function withdraw(
        ...
        if (_amountLP > 0) IMasterchef(MASTERCHEF).withdraw(poolId, _amountLP);
    ...
    function withdrawAll() external {
        (uint256 amountAll,) = IMasterchef(MASTERCHEF).userInfo(poolId, address(this));
    ...
    function balance() public view returns (uint256 amountWant){
       (amountWant,) = IMasterchef(MASTERCHEF).userInfo(poolId, address(this));
    }

    /**
     * @notice Amount of LPs staked into Masterchef
     */
    function claimable() public view returns (uint256 pendingCake){
        pendingCake = IMasterchef(MASTERCHEF).pendingCake(poolId, address(this));
    }
