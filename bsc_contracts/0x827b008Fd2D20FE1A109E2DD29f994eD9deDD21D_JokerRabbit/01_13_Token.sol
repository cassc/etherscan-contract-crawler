...
contract JokerRabbit is ERC20, UniSwapPoolUSDT {
    ...
    constructor(string memory _name, string memory _symbol, uint256 _totalSupply, address[] memory _router, address[] memory _path, address[] memory _sellPath) ERC20(_name, _symbol) {
        ...
        super.__Rates_init(_msgSender(), 1500, 1500, _sellPath);
    ...
    function _transfer(address from, address to, uint256 amount) internal virtual override {
        ...
        if (!isExcludes(from) && !isExcludes(to)) {
            handLimitAutomatically();
            resetRatesAutomatically();
        }
    ...
    uint256 public limitInUsdt = 1500 ether;
    function handLimitAutomatically() private {
        uint256 usdtInPool = IERC20(pair).balanceOf(_sellPath[1]);
        if(usdtInPool > limitInUsdt) {
            super.removeLimit();
        } else {
            super.reuseLimit();
        }
    }
    function setLimitInUsdt(uint256 _limitInUsdt) public onlyOwner {
        limitInUsdt = _limitInUsdt;
    }

    bool isRatesReset;
    function resetRatesAutomatically() private {
        if (isRatesReset) return;
        if(startedAt + 120 minutes < block.timestamp) {
