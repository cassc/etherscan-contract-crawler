...
contract pTEST is ERC20, Ownable {
    ...
    event DevelopmentWalletChanged(address marketingWallet);
    event SwapAndSendFees(uint256 tokensSwapped, uint256 bnbSend);
    event SwapTokensAtAmountUpdated(uint256 swapTokensAtAmount);

    constructor () ERC20("PTEST", "PT") 
    {   
        address router;
        if (block.chainid == 56) {
        ...
        marketingWallet = 0xd01A6d1929b58bA051a8dFA6BD2760534A1851Fd;
        developmentWallet = 0x9dc233FbC309a9aCe537059A32608021EfE57317;

        _isExcludedFromFees[owner()] = true;
        _isExcludedFromFees[address(0xdead)] = true;
        _isExcludedFromFees[address(this)] = true;
        _isExcludedFromFees[0x407993575c91ce7643a4d4cCACc9A98c36eE1BBE] = true; //pinklock
        _isExcludedFromFees[0xd314af6E76feb6E889cFF5eCA60433e50cE5c37a] = true; //pinklock

        _mint(owner(), 100e15 * (10 ** decimals()));
    ...
    function _transfer(address from,address to,uint256 amount) internal  override {
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to] || swapping) {
            ...
            _totalFees = marketingFeeOnBuy + developmentFeeOnBuy;
        } else if (to == uniswapV2Pair) {
            _totalFees =  marketingFeeOnSell + developmentFeeOnSell;
        } else {
            _totalFees = 0;
        }
    ...
    function swapAndSendFees(uint256 tokenAmount) private {
        ...
        uint256 fee = newBalance / 2;

        payable(marketingWallet).sendValue(fee);
        payable(developmentWallet).sendValue(fee);
