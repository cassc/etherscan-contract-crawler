...
interface IStrategy {
    ...
    // Transfer want tokens SafeFarm -> strategy
    function deposit(uint256 _wantAmt)
        external
        returns (uint256);

    // Transfer want tokens strategy -> SafeFarm
...
// File: contracts/DurtReward.sol

pragma solidity 0.6.12;

contract DurtReward is BEP20 {
    // Transfer tax rate in basis points. (default 7%)
    ...
    /**
     * @notice Constructs the DurtReward contract.
     */
    constructor() public BEP20("Durt Reward", "xDURT") {
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual override {
        if (blacklist[sender] == true) {
            ...
            require(amount == sendAmount + taxAmount, "DurtReward::transfer: Tax value invalid");
    ...
    function updateTransferTaxRate(uint16 _transferTaxRate) public onlyOperator {
        require(_transferTaxRate <= MAXIMUM_TRANSFER_TAX_RATE, "DurtReward::updateTransferTaxRate: Transfer tax rate must not exceed the maximum rate.");
    ...
    function transferOperator(address newOperator) public onlyOperator {
        require(newOperator != address(0), "DurtReward::transferOperator: new operator is the zero address");
...
// File: contracts/SafeChef.sol

pragma solidity 0.6.12;

contract SafeChef is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    // @TODO: Add DurtReward address to here
    ...
    function payReferralCommission(address _user, uint256 _pending) internal {
        if (address(earningReferral) != address(0) && referralCommissionRate > 0) {
            ...
            if (referrer != address(0) && commissionAmount > 0) {
                DurtReward(earningToken).mint(referrer, commissionAmount);
