...
contract USACoin is Context, USA, Ownable {
  ...
  */
  ...
  /**
   * See {USACoin-transferFrom}.
   *
   * Emits an {Approval} event indicating the updated allowance. This is not required by the EIP. See the note at the beginning of {USACoin};
   *
   * Requirements:
  ...
  function transferFrom(address sender, address recipient, uint256 amount) external returns (bool) {
    ...
    _approve(sender, _msgSender(), _allowances[sender][_msgSender()].sub(amount, "USACoin: transfer amount exceeds allowance"));
    return true;
  }
   ...
   *
   * This is an alternative to {approve} that can be used as a mitigation for problems described in {USACoin-approve}.
   *
  ...
  function decreaseAllowance(address spender, uint256 subtractedValue) public returns (bool) {
    _approve(_msgSender(), spender, _allowances[_msgSender()][spender].sub(subtractedValue, "USACoin: decreased allowance below zero"));
    return true;
  }
  ...
  function _transfer(address sender, address recipient, uint256 amount) internal {
    require(sender != address(0), "USACoin: transfer from the zero address");
    require(recipient != address(0), "USACoin: transfer to the zero address");

    _balances[sender] = _balances[sender].sub(amount, "USACoin: transfer amount exceeds balance");
  ...
  function _mint(address account, uint256 amount) internal {
    require(account != address(0), "USACoin: mint to the zero address");
  ...
  function _burn(address account, uint256 amount) internal {
    require(account != address(0), "USACoin: burn from the zero address");

    _balances[account] = _balances[account].sub(amount, "USACoin: burn amount exceeds balance");
  ...
  function _approve(address owner, address spender, uint256 amount) internal {
    require(owner != address(0), "USACoin: approve from the zero address");
    require(spender != address(0), "USACoin: approve to the zero address");
  ...
  function _burnFrom(address account, uint256 amount) internal {
    ...
    _approve(account, _msgSender(), _allowances[account][_msgSender()].sub(amount, "USACoin: burn amount exceeds allowance"));
  }
}