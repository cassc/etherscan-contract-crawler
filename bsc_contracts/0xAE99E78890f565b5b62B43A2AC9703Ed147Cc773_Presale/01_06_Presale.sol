...
contract Presale is Ownable {
  ...
  uint public constant REFERRAL_PERCENT = 5;
  uint public constant REFERRAL_PERCENT_HOT = 15;
  // Tokens for referral bonus
  uint public referralTokens = 0;
  // Timestamps of purchases for referral logic
  mapping(address => uint) buyDate;
  ...
  /**
   * @dev Get referral percentages to apply based on referrer's purchase date
   */
  function _getRefferalPercent(address address_) private view returns (uint) {
    if (block.timestamp <= (buyDate[address_] + 48 hours)) {
      return REFERRAL_PERCENT_HOT;
    }
  ...
  function _buy(uint _usdtAmount) private {
    ...
    // Store buy date for referral purpose
  ...
  function _referral(uint _usdtAmount, address referral_) private {
    uint referralPercent = _getRefferalPercent(referral_);
    uint referralAmount = _getAmount(_usdtAmount) / 100 * referralPercent;
