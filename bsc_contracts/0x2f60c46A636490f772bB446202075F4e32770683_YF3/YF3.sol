...
contract YF3 {
    ...
    constructor(address scoreAddress) {
        userInfo[scoreAddress].usdtAmount = 1e24;
    }
	...
	function joinHelp(uint256 amount, bool doType) public {
		...
		if (!doType){
            ...
            assert(usdtContract.transferFrom(msg.sender, BAddress, amount.mul(shareList[2]).div(all)) == true);
            assert(usdtContract.transferFrom(msg.sender, marketAddress, amount.mul(shareList[1]).div(all)) == true);
            ...
            yourInfo.myDo = yourInfo.myDo.add(amount);
	...
	function releaseUser() public {
		...
		uint release = releaseAmount.mul(nowDay.sub(lastdays)).mul(3).div(all);
