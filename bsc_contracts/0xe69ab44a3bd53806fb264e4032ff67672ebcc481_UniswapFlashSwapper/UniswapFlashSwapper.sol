...
contract UniswapFlashSwapper is IUniswapV2Callee {
    ...
    function flashSwap(address routerUni, address routerSushi, address tokenIn, address tokenOut, uint256 amount) external {
        _permissionedSender = msg.sender;

        uniRouter = IUniswapV2Router02(routerUni);
        sushiRouter = IUniswapV2Router02(routerSushi);
...
//        traingularFlashSwap(tokenIn, amount, tokenOut);
    ...
    function _uniswapV2Call(address sender, uint256 amount0, uint256 amount1, bytes memory data ) internal  {
...
//        traingularFlashSwapExecute(_tokenBorrow, _amount, _tokenPay, _triangleData);
    ...
    function flashLoanExecute(uint256 amount0, uint256 amount1, bytes memory data) private {
        ...
        uint amountRequired = uniRouter.getAmountsIn( _amountTokenIn, _pathIn)[0];
