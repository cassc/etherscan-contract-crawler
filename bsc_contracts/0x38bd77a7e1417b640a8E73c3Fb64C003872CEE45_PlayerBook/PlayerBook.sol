...
contract PlayerBook is OwnableUpgradeable {
    ...
    function initialize(address new_nft) external initializer {
        __Ownable_init();

        _nft = new_nft;
        startTime = block.timestamp.add(1000 days);
    ...
    function Mint() public isOnSell {
        IERC20(_usdt).safeTransferFrom(msg.sender,teamAddr,1*1e16);
        nftCon(_nft).safeMint(msg.sender);

    }

    function setTime(uint new_startTime) public onlyOwner {
        startTime = new_startTime;

    }

    function setteamAddr(address new_teamAddr) public onlyOwner {
        teamAddr = new_teamAddr;

    }
    

    function govWithdraw(address _erc20,uint amount) public onlyOwner{
        IERC20(_erc20).safeTransfer(msg.sender,amount);
    }
