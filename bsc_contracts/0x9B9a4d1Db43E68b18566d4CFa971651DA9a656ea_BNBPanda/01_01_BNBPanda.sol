...
contract BNBPanda is Context, IERC20, Ownable {
    
    using SafeMath for uint256;
    using Address for address;
    
    string private _name = "BNBPanda";
    string private _symbol = "BNBPanda";
    uint8 private _decimals = 9;

    uint public launchedBlock;
    uint public killblock = 0;
    bool public isLaunch = false;
    address payable public marketingWalletAddress = payable(0xbAF5cFe88Ea6a3C18cdC827Ec3ACcB7df4086cDB); // Marketing Address
    address payable public liquidityAddress = payable(0xBc86721eA48055bf968Fd8769E945060f32f46Cf); // Team Address
    ...
    constructor () {
        ...
        _totalTaxIfBuying = _buyLiquidityFee.add(_buyMarketingFee).add(_buyBurnFee);
        _totalTaxIfSelling = _sellLiquidityFee.add(_sellMarketingFee).add(_sellBurnFee);
        _totalDistributionShares = _liquidityShare.add(_marketingShare).add(_burnShare);
    ...
    function setBuyTaxes(uint256 newLiquidityTax, uint256 newMarketingTax, uint256 newBurnTax) external onlyOwner() {
    ...
    function setSellTaxes(uint256 newLiquidityTax, uint256 newMarketingTax, uint256 newBurnTax) external onlyOwner() {
    ...
    function setLiquidityWalletAddress(address newAddress) external onlyOwner() {
        liquidityAddress = payable(newAddress);
    }
    ...
    function swapAndLiquify(uint256 tAmount) private lockTheSwap {
        ...
        uint256 tokenBurn = tokensForSwap.mul(_burnShare).div(_totalDistributionShares);
        _basicTransfer(address(this), deadAddress, tokenBurn);

        tokensForSwap = tokensForSwap.sub(tokenBurn);
        ...
        uint256 amountBNBTeam = amountReceived.mul(_burnShare).div(totalBNBFee);
