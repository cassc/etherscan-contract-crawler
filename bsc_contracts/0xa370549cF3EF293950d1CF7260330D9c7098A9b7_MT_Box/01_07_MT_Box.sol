...
contract MT_Box is OwnableUpgradeable {
    ...
    function rand(uint256 _length) internal returns (uint256) {
        uint256 random = uint256(keccak256(abi.encodePacked(block.difficulty, block.timestamp, randomSeed, _length)));
    ...
    function setWhiteBoxAmount(address[] memory addrs, uint[] memory amounts) external onlyManager {
        for (uint i = 0; i < addrs.length; i++) {
