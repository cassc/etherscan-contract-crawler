...
contract TayarraCoin is TayarraStorage, Initializable, ERC20Upgradeable, ERC20BurnableUpgradeable, PausableUpgradeable, OwnableUpgradeable {
    ...
    function calculateOffsetForFees(uint256 amount, bool isBuy, uint256 marketingFee, uint256 developmentFee, uint256 liquidityFee) internal pure returns(uint256) {
        if (isBuy) {
            uint256 totalBuy = amount * ((MARKETING_TAX_BUY+DEVELOPMENT_TAX_BUY+LIQUIDITY_TAX_BUY) / 100);
            return totalBuy - marketingFee - developmentFee - liquidityFee;
        } else {
            uint256 totalSell = amount * ((MARKETING_TAX_SELL+DEVELOPMENT_TAX_SELL+LIQUIDITY_TAX_SELL) / 100);
            return totalSell - marketingFee - developmentFee - liquidityFee;
        }
    }
    ...
    function isUserWhitelistedForClaim(address _addressToCheck) public onlyOwner view returns(bool) {
    ...
    function isUserWhitelistedForPause(address _addressToCheck) public onlyOwner view returns(bool) {
    ...
    function isUserWhitelistedForFees(address _addressToCheck) public onlyOwner view returns(bool) {
    ...
    function isUserWhitelistedForLaunch(address _addressToCheck) public onlyOwner view returns(bool) {
        bool userIsWhitelistedForPause = vars().whitelistedAddressesForPause[_addressToCheck];
        bool userIsWhitelistedForFees = vars().whitelistedAddressesForFees[_addressToCheck];
