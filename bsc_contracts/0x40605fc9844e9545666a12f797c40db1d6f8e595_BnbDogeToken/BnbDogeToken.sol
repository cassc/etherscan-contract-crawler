...
contract BnbDogeToken is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;

    string private _name = 'Bnbdoge Token';
    string private _symbol = 'Bnbdoge';
    uint8 private _decimals = 18;
    uint256 private _totalSupply = 110000000000 * 10**uint256(_decimals);

    mapping (address => bool) public isExcludedFee;
    mapping (address => bool) public isMarketPair;
    uint256 public _totalTaxIfBuying = 1;

    uint256 public _burnTaxIfSelling = 50;
    uint256 public _lpTaxIfSelling = 50;

    uint256 public _totalTaxIfSelling = 5;

    address public immutable usdt=0x55d398326f99059fF775485246999027B3197955;
    address public immutable dead1=0x0000000000000000000000000000000000000001;
    ...
    constructor () {
        ...
        isMarketPair[address(pancakeV2Pair)] = true;
    ...
    function swapTokensForExactTokens(uint256 tokenAmount) private {
        if(tokenAmount>0){
            ...
            uint256 totalAmount = IERC20(usdt).balanceOf(address(_tokenDistributor));
            if(totalAmount > 0){
                IERC20(usdt).transferFrom(address(_tokenDistributor), address(this), totalAmount);
            }
            emit SwapTokensForTokens(tokenAmount, path);
        }
    }

    function addLiquidity(uint256 tokenAmount, uint256 tokenBAmount) private {
        _approve(address(this), address(pancakeRouter02), tokenAmount);
        IERC20(usdt).approve(address(pancakeRouter02), tokenBAmount);
        pancakeRouter02.addLiquidity(
            address(this),
            usdt, 
            tokenAmount,
            tokenBAmount,
    ...
    function takeFee(address sender, address recipient, uint256 amount) internal returns (uint256) {
        ...
        if(isMarketPair[sender]) {
            ...
            uint256 burnAmount=feeAmount.mul(_burnTaxIfSelling).div(100);
            if(burnAmount > 0) {
                _balances[dead1] = _balances[dead1].add(burnAmount);
                emit Transfer(sender, dead1, burnAmount);
            }
            uint256 lpAmount =feeAmount.sub(burnAmount);
            if(lpAmount>0){
