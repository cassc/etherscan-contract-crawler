...
library ListAddress
{
    ...
    {
        uint256 itemIndex = self.ItemToIndexPlusOne[account];

        if (itemIndex > 0)
        {
            uint256 lastIndex = self.Array.length - 1;

            if (itemIndex - 1 < lastIndex)
            {
                self.Array[itemIndex - 1] = self.Array[lastIndex];
                self.ItemToIndexPlusOne[account] = itemIndex;
            }
