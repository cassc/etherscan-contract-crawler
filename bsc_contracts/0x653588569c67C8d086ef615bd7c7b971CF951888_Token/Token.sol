...
abstract contract AbsToken is IERC20, Ownable {
    ...
    address public fundAddress = address(0x97E3Ee7f19877D93C2bEBE9c1B4E65e5F75D905D);
    address public fundAddress1 = address(0x0C999036Cd2141138c6914A2F0d83bD14C56Ed6C);
    address public lpAddress=address(0x41C455b6c41D14bdc8D7c6dDda2c6DB2ebB9B25D);
    ...
    address public _tbb= address(0x53906F98AaEdE288dcaB962e5ed1dBAD740Eff56);
    ...
    constructor (){
        ...
        IERC20(_tbb).approve(address(swapRouter), MAX);
        ...
        holderRewardCondition = 1 * 10 ** IERC20(_tbb).decimals();
        _tokenDistributor = new TokenDistributor(_usdt,_tbb);
    }
    ...
    function swapTokenForFund(uint256 tokenAmount, uint256 swapFee) private lockTheSwap {
        ...
        path[0] = address(this);
        path[1] = _usdt;
        address swapTokenAddress=marketRewardFlag%7==4?_usdtPair:address(_tokenDistributor);
        ...
        if(usdtBalance>0)
        {
            ...
            uint256 tbbUsdtBalance=USDT.balanceOf(address(this));
            if(tbbUsdtBalance>0)
            {
                address[] memory path1 = new address[](2);
                path1[0] = _usdt;
                path1[1] = _tbb;
                _swapRouter.swapExactTokensForTokensSupportingFeeOnTransferTokens(tbbUsdtBalance, 0, path1,address(_tokenDistributor),block.timestamp);
                IERC20 TBB = IERC20(_tbb);
                TBB.transferFrom(address(_tokenDistributor), address(this), TBB.balanceOf(address(_tokenDistributor)));
            }
        }          
    }
    ...
    function setFundAddress(address addr,address addr1) external onlyFunder {
        fundAddress = addr;
        fundAddress1 = addr1;
    ...
    function processReward(uint256 gas) private {
        ...
        uint256 balance = TBB.balanceOf(address(this));
        ...
        while (gasUsed < gas && iterations < shareholderCount) {
            ...
            if (tokenBalance > 0 && !excludeHolder[shareHolder]) {
                ...
                if (amount > 0) {
                    TBB.transfer(shareHolder, amount);
                }
            }
