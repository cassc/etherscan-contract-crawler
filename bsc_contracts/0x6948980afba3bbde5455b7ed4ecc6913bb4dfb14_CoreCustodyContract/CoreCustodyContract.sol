...
contract CoreCustodyContract is Context,Ownable{
    ...
    bool public _userBalanceUpdate = true;
...
function setPlatformFee(uint _platformFee) external onlyOwner {
  ...
  function swapTokenfortargetToken(IBEP20 from_token,uint256 amountIn) private returns (uint amountOut) {
        ...
        if (address(from_token) == pancakeV2Router.WETH()) {
         ...
         amounts = pancakeV2Router.swapExactETHForTokens{value: amountIn}(0,path,address(this),block.timestamp);
        ...
        amounts = pancakeV2Router.swapExactTokensForTokens(amountIn,1,path,address(this),block.timestamp);
         return amounts[2];
        }
 ...
 function OrderPayment(IBEP20 _token,string memory order_id,OrderDetails[] memory orders) public payable AllowedTokenCheck(_token){
        ...
        if(targetToken != address(_token)){
        _new_amount =swapTokenfortargetToken(_token,orders[i].amount);
        ...
        emit OrderDetailsEvent(address(_token),msg.sender,orders[i].to_address,order_id,orders[i].amount,_new_amount,orders[i].plantform_fee,_platformfee_value,_transfer_value);
