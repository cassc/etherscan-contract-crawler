...
contract NfterriumNet is Initializable, PausableUpgradeable, AccessControlUpgradeable, UUPSUpgradeable {
    ...
    function checkStatus(uint256 _userId) internal {
        ...
        if (oldStatus < newStatus || (oldStatus == 0 && newStatus == 0)) {
            countToUpline(_userId, oldStatus, newStatus);
            countToRef(_userId, oldStatus, newStatus);
        }
        if (sChanged > 0) {
            for (uint256 i = 1; i <= sChanged; i++) {
                if (uChanged[i].id > 0 && users[uChanged[i].id].status < uChanged[i].status) {
                    uint8 oStatus = users[uChanged[i].id].status;
                    emit StatusUpdate(_userId, oStatus, uChanged[i].status);
                    users[uChanged[i].id].status = uChanged[i].status;
                    delete(uChanged[i]);
                }
            }
            sChanged = 0;
    ...
    function calculateStatus(uint256 _userId) internal returns (uint8, uint8) {
        ...
        if (oldStatus < newStatus) {
            sChanged = sChanged.add(1);
            UChanged memory uch = UChanged({
                id: _userId,
                status: newStatus
            });
            uChanged[sChanged] = uch;
    ...
    function countToRef(uint256 _userId, uint8 _oldStatus, uint8 _newStatus) internal {
        ...
        if (_oldStatus == 0 && _newStatus == 0) {
            refDirCounts[refId][_newStatus] = refDirCounts[refId][_newStatus].add(1);
        } else {
            if (refDirCounts[refId][_oldStatus] > 0) {
                refDirCounts[refId][_oldStatus] = refDirCounts[refId][_oldStatus].sub(1);
            }
            refDirCounts[refId][_newStatus] = refDirCounts[refId][_newStatus].add(1);
        }
