...
contract DistributionPool is Initializable, OwnableUpgradeable {
    ...
    function initialize( address token_, uint256 startTime_) external initializer {
        __Ownable_init();
        token = IERC20(token_);
    ...
    function claim(uint128 nonce, bytes calldata signature, address referrer) public {
        ...
        if (referrer != address(0) && referrer != _msgSender()) {
            if (referReward < MAX_REFER_TOKEN) {
                uint256 num = amount * 50 / 1000;
            ...
            IBNBDoge(address(token)).setReferrer(_msgSender(), referrer);
        }
