...
// File: contracts/ REAUv2DividendPayingToken.sol
...
// File: contracts/ REAUv2.sol

pragma solidity ^0.7.6;

contract REAUv2 is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    REAUv2DividendTracker public dividendTracker;

    address public burnAddress;
    
    uint256 public maxBuyTransactionAmount = 1000000000000 * (10**18); //no limits
    uint256 public maxSellTransactionAmount = 1000000000000 * (10**18); //no limits
    uint256 public swapTokensAtAmount = 2000000 * (10**18);
    uint256 public _maxWalletToken = 20000000 * (10**18); //2% limit
    ...
    function updateDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(dividendTracker), "REAUv2: The dividend tracker already has that address");

        REAUv2DividendTracker newDividendTracker = REAUv2DividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "REAUv2: The new dividend tracker must be owned by the REAUv2 token contract");
    ...
    function updateUniswapV2Router(address newAddress) external onlyOwner {
        require(newAddress != address(uniswapV2Router), "REAUv2: The router already has that address");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) external onlyOwner {
        require(pair != uniswapV2Pair, "REAUv2: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "REAUv2: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "REAUv2: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "REAUv2: Cannot update gasForProcessing to same value");
...
//Token code developed and tested by snipefinance.com 
    ...
    function _transfer(
        ...
        if(!tradingIsEnabled) {
            require(canTransferBeforeTradingIsEnabled[from], "REAUv2: This account cannot send tokens until trading is enabled");
        }
...
contract REAUv2DividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() public DividendPayingToken("REAUv2_Dividend_Tracker", "REAUv2_Dividend_Tracker") {
        claimWait = 3600;
    ...
    function _transfer(address, address, uint256) internal override {
        require(false, "REAUv2_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override {
        require(false, "REAUv2_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main REAUv2 contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "REAUv2_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "REAUv2_Dividend_Tracker: Cannot update claimWait to same value");
