...
contract PancakeFlashSwap {
    ...
    function startArbitrage(
        ...
        if (A != WBNB && B != WBNB) {
            ...
            IERC20(WBNB).safeApprove(address(N_ROUTER), MAX_INT);
        }

        // TO AVOID ERROR:  SafeERC20: approve from non-zero to non-zero allowance
        if (A != CAKE && B != CAKE) {
            IERC20(CAKE).safeApprove(address(M_ROUTER), MAX_INT);
            IERC20(CAKE).safeApprove(address(N_ROUTER), MAX_INT);
        }
    ...
    function Calculate_Arbitrage(
        ...
        address N_FACTORY,
        address N_ROUTER
    ) external view returns (uint256, uint256, address[6] memory) {
        ...
        return (loanAmount, best_ratio, best_order);
    }
