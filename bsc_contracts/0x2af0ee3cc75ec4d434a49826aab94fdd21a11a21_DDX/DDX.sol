...
contract DDX is Context, IERC20, Ownable {
    ...
    string private _name     = "Diamond Dex";
    string private _symbol   = "DDX";
    uint8  private _decimals = 12;
   
    uint256 private constant MAX = type(uint256).max;
    uint256 private _tTotal = 8_000_000_000_000_000 * (10 ** _decimals);
    ...
    { 
        ...
        marketingWallet = 0x4151D3697D9745D75a02dfcefce63CB5232D2AE8;
...
//------------------TaxAndTransfer------------------//
    function _tokenTransfer(address sender, address recipient, uint256 amount) private {
        ...
        if (_isExcludedFromFees[sender] || 
            ...
            burnFeeAmount = (amount * burnFeeonSell / 100);
            setSellFee();
        }else if(sender == uniswapV2Pair){
            burnFeeAmount = (amount * burnFeeonBuy / 100);
            setBuyFee();
        }else{
            setSellFee();
        }
        ...
        if (burnFeeAmount > 0){
            _transferStandard(sender, DEAD, burnFeeAmount);
...
//------------------FeeManagment------------------//
    ...
    function setBuyFeePercentages(uint256 _taxFeeonBuy, uint256 _liquidityFeeonBuy, uint256 _marketingFeeonBuy, uint256 _burnFeeonBuy) external onlyOwner {
        ...
        totalBuyFees = taxFeeonBuy + liquidityFeeonBuy + marketingFeeonBuy + burnFeeonBuy;
        require(totalBuyFees <= 12, "Buy fees cannot be greater than 12%");
    }
    ...
    function setSellFeePercentages(uint256 _taxFeeonSell, uint256 _liquidityFeeonSell, uint256 _marketingFeeonSell, uint256 _burnFeeonSell) external onlyOwner {
        ...
        totalSellFees = taxFeeonSell + liquidityFeeonSell + marketingFeeonSell + burnFeeonSell;
        require(totalSellFees <= 12, "Sell fees cannot be greater than 12%");
    }
}