...
contract WeLandBox3 is ERC721Enumerable, Ownable {
    using Strings for uint256;
    ...
    uint256 public totalDividend;
     // Lists
    uint256[50] public remainingIds;
    ...
    address public RewardToken = 0x1c5A65eDeda96E7dAf0715d978cC643184FBbd6C;
    address public FeeReceiver = 0xff21Cb20E6f82A32873a59f74c9348d19E0239dB;

    //Contracts
    IUniswapV2Router02 public router;
    IERC20 public token;

    struct LandBoxStruct {
        uint256 id;
        address buyer;
        uint256 cost;
        string imageURL;
    ...
    function updateBaseExtension(string memory _newBaseExtension)
        public
        onlyOwner
    {
        baseExtension = _newBaseExtension;
    }
    function updateImageExtension(string memory _newImageExtension)
        public
        onlyOwner
    {
        baseImage = _newImageExtension;
    }
    ...
    function mint(uint256 _mintAmount) public payable {
        // require
        require(!paused, "WeLandBox: minting has not started yet");
        require(_mintAmount > 0, "WeLandBox: you have to mint at least one");
        require(
            _mintAmount <= maxMintAmount,
            "WeLandBox: you can only mint 50 at a time"
        );
        require(
            lastSupply >= _mintAmount,
            "WeLandBox: the collection is sold out"
        );
        if (msg.sender != owner()) {
            require(
                msg.value >= cost * _mintAmount,
                "WeLandBox: total cost isn't match"
            );
        }
    ...
    function getAllNFTs() public view returns (LandBoxStruct[] memory) {
        return minted;
    }
    ...
    {
        // Checks
        require(_winner != address(0), "WeLandBox: zero address");
        require(_amount > 0, "WeLandBox: you have to mint at least one");
        require(lastSupply != 0, "WeLandBox: the collection is sold out");
    ...
    function tokenURI(uint256 _id)
        ...
        returns (string memory)
    {
        require(_exists(_id), "WeLandBox: URI query for nonexistent token");
