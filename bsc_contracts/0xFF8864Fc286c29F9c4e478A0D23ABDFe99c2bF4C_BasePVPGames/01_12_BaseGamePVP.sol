...
contract BasePVPGames {
    ...
    function changeMythAddress(address _token) external {
        require(msg.sender == owner, "only owner");
        mythAddress = payable(_token);
    }
    ...
    function withdrawTokenWinnings(address _token) external {
        ...
        if (_token != mythAddress) {
            ...
            MythToken mythContract = MythToken(mythAddress);
            mythContract.mintTokens(
                rewardPerCent * (1000 - mythFee),
                msg.sender
            );
            emit claimedWinnings(
                msg.sender,
    ...
    function setMythFee(uint256 amount) external {
        require(owner == msg.sender, "Only owner can change myth fee");
        mythFee = amount;
    }
