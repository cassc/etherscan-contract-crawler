...
contract MSSC is MSSCBase, MembraneAuth {
    ...
    function publishSecret(
        ...
        cycleExists(cycleId)
        isHybrid(cycleId)
        // TODO: if the secret publication transaction is mined when the lock period is over,
        // it should still be published, because in the rest of the blockchains it is already published
        isLocked(cycleId)
    {
        if (_cycles[cycleId].cycleType == CycleType.OnePeriodLock) {
            _onePeriodLocks[cycleId].validateSecret(cycleId, secret);
