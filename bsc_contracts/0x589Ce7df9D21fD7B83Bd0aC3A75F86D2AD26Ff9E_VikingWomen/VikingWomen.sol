...
contract VikingWomen is ERC721 , DefaultOperatorFilterer {
    ...
    constructor()
        ERC721("The Wiking Women", "TWW")
    {}

    function mint() external payable {
        
        if (userMints[msg.sender] >= maxMints) revert MintLimit();
        if (totalSupply + 1 > 41) revert SoldOut();
        ...
        returns (string memory)
    {
        return
            string(
                abi.encodePacked("ipfs://bafybeig2rt4zazum7s5xwqli6mdkxfhsteirl2a4ldtmlehb5q6af7vcrq/", Strings.toString(tokenId), ".json")
            );
    }
