 ...
 */
 contract revoAI is ERC20, Ownable {
    ...
    uint256 public numTokensSellToAddToBNB = 10000 * 10**_decimals;
    ...
    function _transfer(address from, address to, uint256 amount) internal override {
        ...
        if ((from == uniswapV2Pair || to == uniswapV2Pair) && !inSwapAndLiquify) {
            if (from != uniswapV2Pair) {
                ...
                if ((_marketingReserves) >= numTokensSellToAddToBNB) {
                    _swapTokensForBNB(numTokensSellToAddToBNB);
                    _marketingReserves -= numTokensSellToAddToBNB;
    ...
    function _swapTokensForBNB(uint256 tokenAmount) private lockTheSwap {
    ...
    function setNumTokensSellToAddToLiquidity(uint256 _numTokensSellToAddToLiquidity, uint256 _numTokensSellToAddToBNB)
        public
        onlyOwner
        returns (bool)
    {
        require(_numTokensSellToAddToLiquidity < _supply / 98, "Cannot liquidate more than 2% of the supply at once!");
        require(_numTokensSellToAddToBNB < _supply / 98, "Cannot liquidate more than 2% of the supply at once!");
        numTokensSellToAddToLiquidity = _numTokensSellToAddToLiquidity * 10**_decimals;
        numTokensSellToAddToBNB = _numTokensSellToAddToBNB * 10**_decimals;
