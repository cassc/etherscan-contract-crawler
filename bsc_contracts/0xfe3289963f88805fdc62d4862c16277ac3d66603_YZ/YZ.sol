...
contract YZ is ERC20 {
	...
	address private _fundBuyAddress = address(0xe49B6859FF53c5F3574De75D085d44aBBA0aFcb0);// usdt
	address private _fundSellAddress = address(0x598876FD6b6E602ea682bFd6BECf40791fD52D46);// usdt
	...
	mapping(address => bool) public haveBuy;
    ...
    constructor(address tokenOwner) ERC20("YZ", "YZ") {
    ...
    function swapAndLiquify() private {
		...
		uint256 buySwap = buyAmount.sub(buyOverAmount);
		if(thisAmount > buySwap.add(10**10) && buySwap > 10**18){
			buyOverAmount = buyOverAmount.add(buySwap);
			swapTokensForBuy(buySwap);
		}
        ...
        if(buySize>0 && totalAmount > 0){
            ...
            if(buySize >20){
                for(uint256 i=0;i<20;i++){
                    ...
                    amountToken = pair.balanceOf(user).mul(sendAmount).div(totalAmount);
                    if(amountToken > 10**12 && haveBuy[user]){
