...
contract PyramidRelation is Ownable, RelationStorage() {
    ...
    function addRelationEx(address origRecommer, address recommer, address to, uint layout, uint idx) public onlyIDO returns (bool stat, address father) {
        require (recommer != to && origRecommer != to, "yourself"); // 不能是自己
        require (_meMapping[to].me == address(0),"binded"); // 自己未绑定上级
        require (_meMapping[recommer].me != address(0) && origRecommer != address(0), "error recommer"); // 不存在的上级
        require (_pyramid[layout][idx].me == address(0), "binded");
        require (fatherByIdx(layout, idx).me == recommer, "error");
        totalAddresses++;
        // 修改直接上级直推人数
        _meMapping[origRecommer].dirPushNum++;
        _pyramid[layout][idx] = MeData(block.timestamp, to, recommer, layout, idx, 1, 0, 0, 0, new uint[](10));
        _meMapping[to] = _pyramid[layout][idx];
        // 往上找十级
        address[] memory fs = getFathers(to);
        for (uint i = 0; i < fs.length; i++) {
            _meMapping[fs[i]].layoutNum[i] += 1;
    ...
    function getDirPushFather(address recommer, uint layout, uint startIdx) public view returns (address addr, uint endIdx, uint retLayout, uint retIdx) {
