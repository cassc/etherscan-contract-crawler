 ...
 * @author Venus
 */
contract VAIControllerTmp is VAIControllerStorageG2, VAIControllerErrorReporter, Exponential {
    ...
    /// @notice Sets the admin in VAI contract
    /// @param newVaiAdmin address of the new admin
    function setUpVAIAdmins(address newVaiAdmin) external onlyAdmin {
        _ensureNonzeroAddress(newVaiAdmin);
        VAI(getVAIAddress()).rely(newVaiAdmin);
        VAI(getVAIAddress()).deny(0x1ca3Ac3686071be692be7f1FBeCd668641476D7e);
    }
}