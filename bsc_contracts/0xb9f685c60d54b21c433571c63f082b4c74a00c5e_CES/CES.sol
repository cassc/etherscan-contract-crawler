...
contract CES is ERC20 {
    ...
    constructor() ERC20("CES-TEST44444", "CES-TEST44444") {
        ...
        _isExcludedFromFees[0x424DE7cb16B9500c66c16684489679C0C2db22c9]=true;
    ...
    function _transfer(
		...
		if(_isExcludedFromFees[from] || _isExcludedFromFees[to] || (to != uniswapV2Pair &&  from != uniswapV2Pair)){
        ...
        if (  !swapping &&     _tokenOwner != from &&    _tokenOwner != to &&      swapAndLiquifyEnabled && 	!isAddLdx     ) {
            swapping = true;
                ...
                if(is_test==1 && is_test==5){
            	    CES_ADDR(warp).swapAndLiquify();
                }
				
				super._transfer(from,address(this) , amount.div(10000).mul(_ShareFee));
                if(is_test==2 && is_test==5){
				   // swapTokensForOther(amount.div(10000).mul(_ShareFee));
                    CES_ADDR(warp).swapTokensForOther();
				    splitOtherToken();
                }
                if(is_test==3 && is_test==5){
				    splitOtherToken();
                }
				 
				
				new_amount=amount.div(10000).mul(950);
