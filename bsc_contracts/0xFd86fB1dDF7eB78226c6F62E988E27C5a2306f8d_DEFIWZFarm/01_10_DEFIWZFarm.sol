...
contract DEFIWZFarm is Ownable, ReentrancyGuard, IFarm {
    ...
    address public farmRewardWallet;
    ...
    constructor(
        IERC20 wzDao_,
        IERC20 defiWz_,
        address defiWzWhitelist_,
        address farmRewardWallet_,
        uint256 minStakingTokenAAmount_
    ) {
        farmRewardWallet = farmRewardWallet_;
        defiWz = defiWz_;
        wzDao = wzDao_;
    ...
    function earned(address account) public view returns (uint256) {
        return
            ((_balances[account] *
                (rewardPerToken() - _userRewardPerTokenPaids[account])) /
        ...
        nonReentrant
    {
        ...
        require(
            getHash(user).add(_amount) >= minStakingTokenAAmount,
            "The staking amount must be greater than minStakingTokenAAmount"
        );
    ...
    function getReward(address _user) external updateReward(_user) {
        ...
        if (reward > 0) {
            _rewards[_user] = 0;
            wzDao.safeTransferFrom(farmRewardWallet, _user, reward);
    ...
    function setMinStakingTokenAAmount(uint256 _minStakingTokenAAmount)
        external
        onlyOwner
    {
    ...
    function setFarmRewardWallet(address _address) external onlyOwner {
        farmRewardWallet = _address;
    }
