...
contract StratManager is Ownable, Pausable {
    ...
    modifier onlySafeFarm() {
        require(msg.sender == safeFarm, "!safeFarm");
        _;
    }
    modifier onlyHarvester() {
        require(msg.sender == sysHarvester, "!harvester");
        _;
    }
...
// RESTRICTED FUNCTIONS
    ...
    function setHarvester(address _newHarvester) external onlyOwner {
        sysHarvester = _newHarvester;
    }
    ...
    function setOutputToNativeRoute(
        ...
        _routeValidate(route, output, native);

        outputToNativeRoute = route;
    }
    ...
    // compounds earnings and charges performance fee by system harvester
    function harvest(uint256 _txFee) external whenNotPaused onlyHarvester {
        _harvest(_txFee);
    }
...
// INTERNAL FUNCTIONS
    function _routeValidate(
        address[] memory route, address _src, address _dest
    ) internal pure {
        require(route.length > 1 && _src != _dest, 'invalid operation');
        require(route[0] == _src, 'invalid src token');
        require(route[route.length - 1] == _dest, 'invalid dest token');
    }
    ...
    function _chargeFees(uint256 feeAdd) internal returns (uint256) {
        uint256 allBal = _outputBalance();
        if (allBal == 0) return 0;

        uint256 toNative = allBal * poolFee / MAX_FEE + feeAdd;
        require(toNative <= allBal, 'high fee');

        if (output != native) {
            _swapToken(toNative, outputToNativeRoute, address(this));
    ...
    function _sendPoolFees(uint256 nativeBal) internal {
        ...
        if (frfiFeeAmount > 0) {
            IERC20(native).safeTransfer(frfiFeeRecipient, frfiFeeAmount);
        }

        uint256 strategistFeeAmount = nativeBal - callFeeAmount - frfiFeeAmount;
        if (strategistFeeAmount > 0) {
            IERC20(native).safeTransfer(strategistFeeRecipient, strategistFeeAmount);
        }
...
// VIRTUAL
    ...
    function _harvest(uint256 _txFee) internal virtual {}
