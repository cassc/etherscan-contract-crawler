...
contract LevelManager is Initializable, AdminableUpgradeable, ILevelManager, WithLevels, WithPools {
    ...
    event Registered(address indexed account, address pool, address sale, uint256 time);
    ...
    function lock(
        ...
        address pool,
        uint256 startTime
    ) external override onlyIDO {
        internalLock(account, pool, msg.sender, startTime);
    }
    ...
    function internalLock(
        ...
        try IStakingLockable(poolAddress).lock(account, registeredAt) {} catch {}

        emit Registered(account, poolAddress, saleAddress, registeredAt);
    }
    ...
    function batchRegister(
        ...
        for (uint256 i = 0; i < addresses.length; i++) {
            internalLock(addresses[i], poolAddresses[i], saleAddresses[i], registeredAt[i]);
        }
    }
}