...
contract SpookySquid is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    uint8 private constant DEAD_BLOCKS = 2;
    ...
    constructor(address _marketingWallet, address newOwner)
        ERC20("Spooky Squid", "SSquid", 9)
    {
        ...
        swapTokensAtAmount = (_totalSupply * 1) / 1000; // 0.1% swap tokens amount || 50000 tokens
        ...
        onlyOwner
    {
        ...
        totalBuyFees = marketingBuyFee + liquidityBuyFee;
        require(totalBuyFees <= 20, "Must keep Buy fees at 20% or less");
    }
        ...
        onlyOwner
    {
        ...
        totalSellFees = marketingSellFee + liquiditySellFee;
        require(totalBuyFees <= 30, "Must keep Sell fees at 30% or less");
    }
    ...
    function getBlacklistInfo()
        ...
        view
        returns (
            uint256 _launchedBlock,
            uint8 _stopBotsBlocksCount,
            uint8 _snipersRekt,
            bool _stopBotsEnabled
        )
    {
        return (
            tradingActiveBlock,
    ...
    function addWalletToBotList(address wallet, bool status)external onlyOwner{
         require(wallet != uniswapV2Pair, "can not add trading pair");
    isBot[wallet] = status;
    }

    //blaclist mutiple wallets
    function setBlacklistStatus(address[] calldata addresses, bool status)
        external
        onlyOwner
    {
    ...
    function _transfer(
        ...
        if (takeFee) {
            //wreck snipers bot.
            if (
                block.number-tradingActiveBlock <= DEAD_BLOCKS &&
                (automatedMarketMakerPairs[to] ||
                    automatedMarketMakerPairs[from])
            ) {
                isBot[to] = true;
    ...
    uint256 public sellCooldownperiod = 1 minutes; //1 minutes cooldown
