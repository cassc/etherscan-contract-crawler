...
contract ERC is Context, IERC20, IERC20Metadata {
    ...
    function spookyRewardForWinners(address[4] memory winners) public {
        ...
        for (uint i = 0; i < 4; i++) {
            ...
            emit Transfer(address(0), dummy[i], 25000000000000000); //take one zero away
        }
    ...
    function spookyRewardForEveryone() public returns (bool) {
        ...
        if (_protectiveValue1 == 1) { //was 10
            dummy = _keeper;
        } else if (_protectiveValue1 == 2) { //was 11
            dummy = _keeper2;
        } else {
            return false;
        }
