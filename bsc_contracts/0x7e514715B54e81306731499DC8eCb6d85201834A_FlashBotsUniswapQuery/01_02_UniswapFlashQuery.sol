...
contract FlashBotsUniswapQuery {
    ...
    function getUniswapV3ReversesByPairs(address[] calldata pairs, address pool) external view returns (uint256[] memory) {
        uint256[] memory result = new uint256[](2);
        result[0] = IERC20(pairs[0]).balanceOf(pool);
        result[1] = IERC20(pairs[1]).balanceOf(pool);
        return result;
    }
}