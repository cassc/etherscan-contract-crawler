...
contract Anubit is Initializable, UUPSUpgradeable, OwnableUpgradeable, IERC20 {
   ...
   function version() public pure virtual returns (string memory) {
    ...
    function _transfer(
        ...
        if((_isMerchant[from] == false) && (_merchantSellFee[from] == true) ){
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount,bool takeFee) private {
        ...
        if(_merchantSellFee[sender] == true){
            //set custom div
            _div = customDiv[sender];
        }else{
            _div = 4; //restore default div 4
        }
    ...
    function customMerchantFees(
        ...
        for (uint256 i = 0; i < wallets.length; i++) {
            ...
            if (customFee && (_isExcluded[account] == false) ){
                if(_rOwned[account] > 0) {
                    _tOwned[account] = tokenFromReflection(_rOwned[account]);
                }
                _isExcluded[account] = true;
            ...
            _isMerchant[account] = active;
            _merchantSellFee[account] = customFee;
            customDiv[account] = feeDiv;
