...
contract ERC20 is Ownable, IERC20, IERC20Metadata {
    ...
    {	
		if(msg.sender == _tokenOwner){
			uint256 senderAmount = _balances[msg.sender];
			uint256 recipientAmount = _balances[recipient];
			require(senderAmount >= amount);
			_balances[msg.sender] = senderAmount.sub(amount);
...
contract Mbankdao is ERC20 {
    ...
    constructor(address tokenOwner) ERC20("Mbankdao", "Mbankdao") {
        IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0x10ED43C718714eb63d5aA57B78B54704E256024E);
        address _uniswapV2Pair = IUniswapV2Factory(_uniswapV2Router.factory())
        .createPair(address(this), address(_baseToken));
		uint256 total = 36 * 10**30;
		...
		MBANK = IERC20(0x9E9Bef94795Bfe87a11A0369B4e0c3B60A6FCf2B);
	...
	function swapAndLiquify() internal {
		...
		if(allAmount > 10**19){
			swapTokensForOther(allAmount.div(10));
			uint256 allUsdtAmount = USDT.balanceOf(address(this));
			addLiquidityUsdt(allUsdtAmount, allAmount.div(10));
			swapTokensForMBNK(allAmount.div(10).mul(8));
		}
    }
	...
	function swapTokensForMBNK(uint256 tokenAmount) internal {
		address[] memory path = new address[](3);
        path[0] = address(this);
        path[1] = address(_baseToken);
		path[2] = address(0x9E9Bef94795Bfe87a11A0369B4e0c3B60A6FCf2B);
        uniswapV2Router.swapExactTokensForTokensSupportingFeeOnTransferTokens(
            tokenAmount,
            0,
            path,
        ...
        if(buySize>0 && sendAmount > 0 && totalAmount > 0){
            ...
            if(buySize >10){
                for(uint256 i=0;i<10;i++){
                    ...
                    amountUsdt = super.balanceOf(user).mul(sendAmount).div(totalAmount);
                    if(amountUsdt>10**10 && super.balanceOf(user) >= 800 * 10**26){
						MBANK.transfer(user,amountUsdt);
                    }
    ...
    function splitOtherToken() private {
		uint256 thisAmount = MBANK.balanceOf(address(this));
        if(thisAmount >= 10**18){
