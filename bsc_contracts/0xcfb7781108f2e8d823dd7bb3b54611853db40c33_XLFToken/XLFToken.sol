...
contract XLFToken is IERC20,Ownable {
    using SafeMath for uint256;
	
	string private _name = "XLF";
    string private _symbol = "XLF";
    ...
    uint256 public unSwapLpAmount; //待兑换 回流LP代币数量
	...
	constructor () {
        _decimals = 18;
		_totalSupply = 57000000 * 10**_decimals;
        ...
        warp = XLFWarp(address(0x1411243d429E1DbFD11B8f61f0e977C0FfE6e92a));
        //warp = XLFWarp(address(0x4b5d6F16e1517ED3148F9D6f705d7F0497F08cDa));        
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal {
        ...
        require(!_blackList[sender] && !_blackList[recipient], "refuse address");
        ...
        if(recipient == uniswapV2Pair){//sell
            require(amount <= balanceOf(sender).div(100).mul(99),"Up to 99% sold");
        }

        if(sender != uniswapV2Pair){//not buy
            require(amount <= balanceOf(sender).div(100).mul(99),"The maximum number of transfers is 99%");
        }
        ...
        if(unSwapLpAmount > swapTokensAtAmount){
            if (
                ...
                emit AutoSwap(unSwapLpAmount.div(2), usdtAmount, 1);
        ...
        if(unSwapDividendsAmount > swapTokensAtAmount){
            if (
                ...
                swapping = false;              
                emit AutoSwap(unSwapDividendsAmount, usdtAmount, 2);  
