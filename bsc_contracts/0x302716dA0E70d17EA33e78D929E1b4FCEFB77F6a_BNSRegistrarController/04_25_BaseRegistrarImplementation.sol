...
contract BaseRegistrarImplementation is Ownable,ERC721, ERC721Enumerable, ERC721URIStorage,ERC721Burnable, BaseRegistrar{
    ...
    mapping (uint =>string) names;  //id=>name
	...
	function __BaseRegistrarImplementation_i(BNS _bns, bytes32 _baseNode) initializer public {
        __ERC721_init("BSC Name Service", "BNS");
        __ERC721Enumerable_init();
        __ERC721URIStorage_init();
        __ERC721Burnable_init();
        __Ownable_init();
        __BaseRegistrarImplementation_init(_bns,_baseNode);
    }

    function __BaseRegistrarImplementation_init(BNS _bns, bytes32 _baseNode) internal onlyInitializing {
        __BaseRegistrarImplementation_init_unchained( _bns, _baseNode);
    }

    function __BaseRegistrarImplementation_init_unchained(BNS _bns, bytes32 _baseNode) internal onlyInitializing {
        bns = _bns;
        baseNode = _baseNode;
		GRACE_PERIOD = 7 days;
    }
    ...
    // Set the baseNode .
    function setBaseNode(bytes32 _baseNode,address resolver) external onlyOwner {
        baseNode = _baseNode;
    ...
    function registerWithName(uint256 id, address owner, uint duration,string calldata name) external  returns(uint) {
      names[id] =  name; 
      return _register(id, owner, duration, true);
    }
