...
contract Zui is Context, IBEP20, Ownable {
    ...
    string private _name = "Zui"; // token name
    string private _symbol = "ZUI"; // token ticker
    ...
    address payable public projectWallet; // project wallet address
    address public burnAddress = 0x0000000000000000000000000000000000000000;

    uint256 public minTokenToSwap = 1000000 * 1e9;
    uint256 public maxHoldingAmount = 2000000000 * 1e9;
    uint256 public minBuyLimit = 1000000000 * 1e9;
    ...
    uint256 public projectWalletFeeOnBuying = 50;
    uint256 public burnFeeOnBuying = 0;

    // sell tax fee
    uint256 public reflectionFeeOnSelling = 0;
    uint256 public liquidityFeeOnSelling = 30;
    uint256 public projectWalletFeeOnSelling = 50;
    uint256 public burnFeeOnSelling = 0;

    // for smart contract use
    uint256 private _currentReflectionFee;
    uint256 private _currentLiquidityFee;
    uint256 private _currentprojectWalletFee;
    uint256 private _currentBurnFee;
    uint256 private _accumulatedProjectWallet;
    ...
    constructor(address payable _projectWallet) {
    ...
    function setBuyFeePercent(
        ...
        projectWalletFeeOnBuying = _projectWalletFee;
    ...
    // owner can change project address
    function setprojectWalletAddress(address payable _newAddress)
        external
        onlyOwner
    {
        projectWallet = _newAddress;
    }
        ...
        uint256 percentage = tAmount
            .mul(
                _currentReflectionFee.add(_currentLiquidityFee).add(
                    _currentprojectWalletFee.add(_currentBurnFee)
                )
            )
            .div(1e3);
        return percentage;
    ...
    function setBuyFee() private {
        ...
        _currentprojectWalletFee = projectWalletFeeOnBuying;
    ...
    function setSellFee() private {
        ...
        _currentprojectWalletFee = projectWalletFeeOnSelling;
    ...
    function _takeAllFee(address sender,uint256 tAmount, uint256 currentRate) internal {
        ...
        if (tFee > 0) {
            ...
            _accumulatedProjectWallet = _accumulatedProjectWallet.add(
                tAmount.mul(_currentprojectWalletFee).div(1e3)
            );
   ...
   function _takeBurnFee(address sender,uint256 tAmount, uint256 currentRate) internal {
    ...
    function swapAndLiquify(address from, address to) private {
        ...
        if (
            ...
            uint256 bnbForprojectWallet = deltaBalance.sub(bnbToBeAddedToLiquidity);  

            // sending bnb to award pool wallet
            if(bnbForprojectWallet > 0)
                projectWallet.transfer(bnbForprojectWallet); 
