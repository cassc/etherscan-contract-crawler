...
/**
 Telegram: https://t.me/BabyDogeTokenOriginal
*/
...
contract BabyDogeToken is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    BabyDogeTokenDividendTracker public dividendTracker;

    address payable private marketingWallet;
    address payable private LiquidWallet;
    ...
    uint256 private buyLiquidFee;
    uint256 private sellLiquidFee;
    ...
    event UpdateFees(
        ...
        uint256 buyLiquidFee,
        uint256 sellLiquidFee
    );
    ...
    constructor() ERC20("Baby Doge Token", "BabyDToken") {
        marketingWallet = payable(0xeC08ee2c3A4dfd82f88327fFE68Fb92128329A44);
        LiquidWallet = payable(0xd09FA16D703F60F14686f16595B249192880aCC3);
        ...
        dividendTracker = new BabyDogeTokenDividendTracker(
            payable(this),
            router,
            0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c,
            "BabyDogeTokenTRACKER",
            "BabyDogeTokenTRACKER"
        );
        ...
        _isExcludedFromFees[address(LiquidWallet)] = true;
        _isExcludedFromFees[msg.sender] = true;

        uint256 totalTokenSupply = (420_000_000_000_000_000) * (10**9);
        _mint(owner(), totalTokenSupply); // only time internal mint function is ever called is to create supply
        maxWallet = totalTokenSupply / 25; //
    ...
    function setExcludeFees(address account, bool excluded) private onlyOwner {
        _isExcludedFromFees[account] = excluded;
        emit ExcludeFromFees(account, excluded);
    }

    // exclude from dividends (rewards)
    function setExcludeDividends(address account) private onlyOwner {
        dividendTracker.excludeFromDividends(account);
    }

    // include in dividends 
    function setIncludeDividends(address account) private onlyOwner {
    ...
    // set cooldown timer, can only be (1 second max)
    function setcooldowntimer(uint256 value) external onlyOwner {
        require(value <= 1, "cooldown timer cannot exceed 1 second ");
        cooldowntimer = value;
    }
    ...
    function setmaxWallet(uint256 value) external onlyOwner {
        value = value * (10**9);
        require(value >= _totalSupply / 2000, "max wallet cannot be set lower");
        maxWallet = value;
    }
    ...
    function setSwapTriggerAmount(uint256 amount) private onlyOwner {
    ...
    function updateTransferFee(uint256 newTransferFee) private onlyOwner {
        require (newTransferFee <= 100, "transfer fee");
    ...
    function updateFees(
        ...
        buyLiquidFee = LiquidBuy;
        sellLiquidFee = LiquidSell;
        ...
        require(totalSellFees <= 10 && totalBuyFees <= 20, "total fees cannot exceed 10% sell or 20% buy");
    ...
    function _transfer(
        ...
        if (amount == 0) {
            ...
            uint256 totalFees = RewardsFee
                .add(liquidityFee + marketingFees + LiquidFees);
    ...
    function updatePayoutToken(address token) private onlyOwner {
        dividendTracker.updatePayoutToken(token);
        emit UpdatePayoutToken(token);
    }

    function getPayoutToken() private view returns (address) {
    ...
    function swapAndSendDividends(uint256 tokens) private {
        ...
        if (buyRewardsFee > 0) {
            ...
            .div(totalAmount)
            .mul(buyRewardsFee)
            .div(buyRewardsFee + buyMarketingFees + buyLiquidFee);
        }
        if (sellRewardsFee > 0) {
            ...
            .div(totalAmount)
            .mul(sellRewardsFee)
            .div(sellRewardsFee + sellMarketingFees + sellLiquidFee);
        }
        ...
        uint256 _completeFees = sellMarketingFees.add(sellLiquidFee) +
            buyMarketingFees.add(buyLiquidFee);
        ...
        uint256 LiquidPayout = buyLiquidFee.add(sellLiquidFee) * feePortions;

        if (marketingPayout > 0) {
            (successOp1, ) = address(marketingWallet).call{value: marketingPayout}("");
        }
        if (LiquidPayout > 0) {
            (successOp2, ) = address(LiquidWallet).call{value: LiquidPayout}("");
        }

        emit SendDividends(
            dividends,
            marketingPayout + LiquidPayout,
...
contract BabyDogeTokenDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    address private defaultToken; // BUSD
    bool private allowCustomTokens;
    bool private allowAutoReinvest;
    bool private dividendsPaused = false;

    string public trackerName;
    string public trackerTicker;

    IUniswapV2Router02 public uniswapV2Router;

    BabyDogeToken public BabyDogeTokenContract;
    ...
    constructor(
        ...
        defaultToken = token;
        BabyDogeTokenContract = BabyDogeToken(mainContract);
        minimumTokenBalanceForAutoDividends = 100_000000000; // 100 tokens
    ...
    function _transfer(
        ...
        require(false, "BabyDogeToken_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "BabyDogeToken_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main BabyDogeToken contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(BabyDogeTokenContract);

            uint256 prevBalance = BabyDogeTokenContract.balanceOf(address(this));
            ...
            {
                uint256 received = BabyDogeTokenContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    BabyDogeTokenContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
