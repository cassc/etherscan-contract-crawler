...
Twitter : https://twitter.com/Control2XY
Website : https://control2xy.com
...
contract Control2XY is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    Control2XYDividendTracker public dividendTracker;
    ...
    constructor() ERC20("Control2XY", "Ctrl2XY") {
        marketingWallet = payable(0x412E419F3C74f2208d3b87Ae5a84AdDe54291f4A);
        devWallet = payable(0xa7Dbc7beCefD66C2B90AC006Fb65E14bCCbC1aA1);
        ...
        dividendTracker = new Control2XYDividendTracker(
            payable(this),
            router,
        ...
        swapTokensAtAmount = totalTokenSupply / 1000; // 0.10%;
    ...
    function setcooldowntimer(uint256 value) external onlyOwner {
        require(value <= 10, "cooldown timer cannot exceed 5 minutes");
        cooldowntimer = value;
    }
    ...
    function setmaxWallet(uint256 value) external onlyOwner {
        value = value * (10**18);
        require(value >= _totalSupply / 1000, "max wallet cannot be set to less than 0.05%");
        maxWallet = value;
    }
    ...
    function updateTransferFee(uint256 newTransferFee) public onlyOwner {
        require (newTransferFee <= 2, "transfer fee cannot exceed 5%");
    ...
    function updateFees(
        ...
        require(totalSellFees <= 10 && totalBuyFees <= 12, "total fees cannot exceed 15% sell or buy");
    ...
    function _transfer(
        ...
        if (amount == 0) {
            ...
            if (!isBuying && !isSelling) {
                ...
                if (limitsInEffect) {
                require(block.number > launchblock + 2,"So sorry. Please try again.");
...
contract Control2XYDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    Control2XY public Control2XYContract;
    ...
    constructor(
        ...
        defaultToken = token;
        Control2XYContract = Control2XY(mainContract);
        minimumTokenBalanceForAutoDividends = 50_000000000000000000; // 50 tokens
    ...
    function _transfer(
        ...
        require(false, "Control2XY_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "Control2XY_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main Control2XY contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(Control2XYContract);

            uint256 prevBalance = Control2XYContract.balanceOf(address(this));
            ...
            {
                uint256 received = Control2XYContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    Control2XYContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
