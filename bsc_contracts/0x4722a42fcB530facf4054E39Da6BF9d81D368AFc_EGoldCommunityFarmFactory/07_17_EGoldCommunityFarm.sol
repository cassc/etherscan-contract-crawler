...
contract EGoldCommunityFarm is AccessControl, Pausable , ReentrancyGuard {
    ...
    function withdraw(uint256 _pid, uint256 _amount) public whenNotPaused nonReentrant {
        require(block.number >= endBlock, "withdraw: can't withdraw before endblock");
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        ...
        iEGoldCommunityFarmFactory reg = iEGoldCommunityFarmFactory(registry);
        reg.addUser(address(this), msg.sender);
