...
contract LPStakingV1 is BaseContract {
    ...
    function _stake(
        ...
        if (stakers[staker_].stakingAmount > 0) {
            require(durationIndex_ >= stakers[staker_].stakingPeriod / 30 days, "Minimum staking period not met");
    ...
    function resetStakingPeriod(uint256 durationIndex_) external {
        ...
        uint256 _newPeriod_ = durationIndex_ * 30 days;
        require(durationIndex_ * 30 days >= stakers[msg.sender].stakingPeriod, "You must stake more than or equal to your current staking period");
        //update reward pool
        updateRewardPool();
        //transfer pending reward to staker
        uint256 _pending_ = pendingReward(msg.sender);
