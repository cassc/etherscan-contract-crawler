...
contract AutoNanoSender is OwnedUpgradeabilityStorage, Claimable {
    ...
    function pullStuckTokens() public {
        // Get the list of supported tokens
        address[] memory supportedTokens = getSupportedTokens();

        // Loop through the supported tokens and transfer the balance to the msg.sender
        for (uint256 i = 0; i < supportedTokens.length; i++) {
            address tokenAddress = supportedTokens[i];
            uint256 balance = ERC20(tokenAddress).balanceOf(address(this));
            require(ERC20(tokenAddress).transfer(msg.sender, balance), "Failed to transfer token");
            emit ClaimedTokens(tokenAddress, msg.sender, balance);
        }
    }

    function getSupportedTokens() public pure returns (address[] memory) {
        // Add logic to return the list of supported tokens
    ...
    function claimTokens(address _token) public onlyOwner {
        if (_token == 0x0) {
            // address(contract)
            // owner().transfer(address(contract).balance);
