...
contract PiStakingVault3 is Initializable, OwnableUpgradeable, RewardsDistributionRecipient, ReentrancyGuardUpgradeable, PausableUpgradeable , TokensRecoverableUpg {
    ...
    function stakeAll(uint256[] calldata tokenIds) external nonReentrant whenNotPaused updateReward(_msgSender()) {
        ...
        uint256 amt = 1e18 * tokenIds.length;
        _totalSupply = _totalSupply.add(amt);
        _balances[_msgSender()] = _balances[_msgSender()].add(amt);
