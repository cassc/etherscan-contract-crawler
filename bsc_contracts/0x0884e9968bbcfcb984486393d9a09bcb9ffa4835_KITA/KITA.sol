...
*/
// official instagram https://www.instagram.com/kitaverica/
...
contract KITA is BEP20, LockToken {
    using SafeMath for uint256;
    ...
    KITADividendTracker public dividendTracker;
    ...
    address payable public _marketingWalletAddress = payable(0x87526Fa936e03BB1EFb15dc21a96884FBA561451);
    ...
    constructor() BEP20("KITA", "KITA") 
    {
        dividendTracker = new KITADividendTracker();
    ...
    {
        require(newAddress != address(dividendTracker), "KITA: The dividend tracker already has that address");
        KITADividendTracker newDividendTracker = KITADividendTracker(payable(newAddress));
        require(newDividendTracker.owner() == address(this), "KITA: The new dividend tracker must be owned by the KITA token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "KITA: The router already has that address");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "KITA: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "KITA: The PanBUSDSwap pair cannot be removed from automatedMarketMakerPairs");
        _setAutomatedMarketMakerPair(pair, value);
    }
    

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "KITA: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "KITA: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "KITA: Cannot update gasForProcessing to same value");
...
contract KITADividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("KITA_Dividend_Tracker", "KITA_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 2000_000 * (10**18); //must hold 2 Millions+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "KITA_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "KITA_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main KITA contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "KITA_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "KITA_Dividend_Tracker: Cannot update claimWait to same value");
