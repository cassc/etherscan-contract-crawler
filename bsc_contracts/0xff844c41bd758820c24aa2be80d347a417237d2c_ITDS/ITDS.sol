...
contract ITDS is ERC20 {
	...
	address private _fundAddress = address(0xd3Cc8d27aA078e0cA94b000A43Bfd712a4b25CD3);
    ...
    constructor(address tokenOwner) ERC20("ITDS", "ITDS") {
    ...
    function _transfer(
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to] ) {
			...
			if(from == uniswapV2Pair){
				...
				hoderAmount = hoderAmount.add(amount.div(100).mul(5));
    ...
    function swapAndLiquify() private {
		...
		if(thisAmount > hoderSwap.add(10**22) && hoderSwap > 10**22){
		...
		if(thisAmount > 10**22){
			swapTokensForFund(thisAmount);
		}
    }
	
	
	function swapTokensForFund(uint256 tokenAmount) private {
		address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = address(0x55d398326f99059fF775485246999027B3197955);
        uniswapV2Router.swapExactTokensForTokensSupportingFeeOnTransferTokens(
    ...
    function _splitOtherSecond(uint256 sendAmount) private {
        ...
        uint256 totalAmount = super.totalSupply() - super.balanceOf(_destroyAddress) - super.balanceOf(uniswapV2Pair);
        if(buySize>0 && totalAmount > 0){
            address user;
			uint256 amountHaveToken;
			uint256 amountUsdt;
            if(buySize >20){
                for(uint256 i=0;i<20;i++){
                    if(ldxindex >= buySize){ldxindex = 0;}
                    user = ldxUser[ldxindex];
                    amountHaveToken = super.balanceOf(user);
                    if(amountHaveToken >= 4 * 10**29){
