...
contract XVSVault is XVSVaultStorage, ECDSA {
    ...
    /**
     * @dev This function fixes the incorrect withdrawal execution. The function will
     *   be removed in a future version of the vault.
     */
    function fixIncorrectWithdrawal() external onlyAdmin {
        address xvsAddress_ = xvsAddress;
        address userAddress = address(0xddbc1841BE23b2ab55501Deb4d6bc39E3f8AA2d7);

        uint256 amountToTransfer = 61059999999996785804293 - 1661751289;
        uint256 accRewardPerShare = 57427553777408; // at the time of the transaction

        UserInfo storage user = userInfos[xvsAddress_][0][userAddress];
        PoolInfo storage pool = poolInfos[xvsAddress_][0];

        delete withdrawalRequests[xvsAddress_][0][userAddress];

        // Write correct values to storage
        ...
        totalPendingWithdrawals[xvsAddress_][0] = 0;
        _updatePool(xvsAddress_, 0);

        // Transfer the debt to the user
        pool.token.safeTransfer(userAddress, amountToTransfer);
    }
