...
contract StakeTest is Ownable, ReentrancyGuard, Pausable {
	...
	function updatePool() public {
		if(clcDay() > 0)
		{
			...
			for (uint256 i = 0; i < numdays; i++) 
			{
				...
				{
				    uint256 amount = rewardPoolBalance * rewardPercent / 1000;
					tAmount += amount;
					accTokenPerShare[currentDay] = amount * precisionFactor / totalStaked;
					
					totalStaked -= totalStaked * decayPercent / 1000;
					rewardPoolBalance -= rewardPoolBalance * rewardPercent / 1000;
				}
				else
				{
		...
		{   
		    if(currentDay > mapUserInfo[user].rewardDebtDay)
			{
				...
				for (uint256 i = 0; i < numdays; i++) 
				{
					...
					if(newAmount > 0) {
					   newAmount -= newAmount * decayPercent / 1000;
					}
					startday++;
				}
