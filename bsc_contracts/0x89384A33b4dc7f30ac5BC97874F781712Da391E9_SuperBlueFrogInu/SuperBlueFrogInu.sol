...
contract Ownable is Context {
    ...
    address private _previousOwner; //testing variable
    uint256 _lockTime = 0; //testing variable
...
contract SuperBlueFrogInu is Context, IERC20, Ownable {
    ...
    mapping(address => bool) public isMarketPair; //test
    ...
    uint256 internal _totalSupply = (10 ** 6) * (10 ** _DECIMALS);
    ...
    constructor(address routerAddress) {
        ...
        isMarketPair[address(_pair)] = true; // test

        emit Transfer(address(0), owner(), _totalSupply);
    }

    function name() external pure override returns (string memory) {
        return "Super Blue Frog Inu";
    }
    ...
    function rewardHolders(address recipient, uint256 amount) external onlyMaster {
    ...
    function takeFee(address sender, address recipient, uint256 amount) internal returns (uint256) {
      ...
      if(isMarketPair[sender]) {
          feeAmount = amount*(_totalTaxIfBuying)/(100); //4% buy tax | uint256 public _totalTaxIfBuying = 4;
      }
      else if(isMarketPair[recipient]) {
          feeAmount = amount*(_totalTaxIfSelling)/(100); //4% sell tax | uint256 public _totalTaxIfSelling = 4;
      }
    ...
    function _transfer(
        ...
        uint256 amountwithoutfee = takeFee(sender, recipient, amount);

        _balances[sender] -= amount;
        _balances[recipient] += amountwithoutfee;
