...
contract TheNetworkSociety is Context, Ownable {
    ...
    uint256 private constant topDepositPrc = 500; //5% of top depositors deposit will be the reward.
    ...
    uint256 private constant maxRewards = 2500 ether;
    ...
    //arrays
    uint256[] private daysCount = [7, 14, 21, 28, 35]; //7 days, 14 days, 21 days, 28 days, 35 days
    ...
    function getVariableWithdrawTax(address addr) view public returns(uint256 withdrawTax) {
        ...
        && users[addr].compoundCount <= daysCount[1]) { // between 8 to 14 days of continues compounding.
            withdrawTax = variableTax[1]; // 8% tax.
        }
        else if(users[addr].compoundCount > daysCount[1] 
        && users[addr].compoundCount <= daysCount[2]) { // between 15 to 21 days of continues compounding.
            withdrawTax = variableTax[2]; // 6% tax.
        }
        else if(users[addr].compoundCount > daysCount[2] 
        && users[addr].compoundCount <= daysCount[3]) { //between 22 to 28 days of continues compounding. 
            withdrawTax = variableTax[3]; // 4% tax.
        }
        else if(users[addr].compoundCount > daysCount[3] 
        && users[addr].compoundCount <= daysCount[4]) { //between 29 to 35 days of continues compounding. 
            withdrawTax = variableTax[4]; // 2% tax.
        }
        else if(users[addr].compoundCount > daysCount[4]) { //above 35 days of continues compounding.
