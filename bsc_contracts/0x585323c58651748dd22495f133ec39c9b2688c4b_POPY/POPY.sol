...
contract POPY is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    POPYDividendTracker public dividendTracker;
    ...
    constructor() ERC20("POPY", "PPY") {
        marketingWallet = payable(0x6D8e45C383cBC40aF3700f315BF115EC5b6f44C9);
        devWallet = payable(0x6D8e45C383cBC40aF3700f315BF115EC5b6f44C9);
        ...
        dividendTracker = new POPYDividendTracker(
            payable(this),
            router,
...
contract POPYDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor(
        ...
        defaultToken = token;
        POPYContract = POPY(mainContract);
    ...
    function _transfer(
        ...
        require(false, "POPY_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "POPY_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main POPY contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(POPYContract);

            uint256 prevBalance = POPYContract.balanceOf(address(this));
            ...
            {
                uint256 received = POPYContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    POPYContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
