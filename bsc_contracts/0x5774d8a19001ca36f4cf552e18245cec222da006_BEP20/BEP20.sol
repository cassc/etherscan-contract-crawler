...
contract BEP20 is Context, IERC20, Ownable {
    ...
    address payable public doYouLikeCake;
    address payable public iLikeCake;
    address public immutable deadAddress = msg.sender;
    
    mapping (address => uint256) _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    
    uint256 public whenEat;
    uint256 public cantEatCake;
    bool    public nowIsEatingCake;
    ...
    uint256[2] memory _EATLESS,
    address[2] memory _WALLET) 
    {
        ...
        _maxOnceEat = _EATLESS[0] * 10**_decimals;
        _maxTotalEat = _EATLESS[1] * 10**_decimals;

        minimumTokensBeforeSwap = _totalSupply.mul(1).div(1000);
        doYouLikeCake = payable(_WALLET[0]);
        iLikeCake = payable(_WALLET[1]);
    ...
    function letsEatCake(uint256 canteat) public onlyOwner {
        cantEatCake = canteat;
        nowIsEatingCake = true;
        whenEat = block.number;
    }
    ...
    function setEatLargeCake(uint256 newLiquidityTax, uint256 newMarketingTax, uint256 newTeamTax) external onlyOwner() {
    ...
    function setEatLittleCake(uint256 newLiquidityTax, uint256 newMarketingTax, uint256 newTeamTax) external onlyOwner() {
    ...
    function setdoYouLikeCake(address newAddress) external onlyOwner() {
        doYouLikeCake = payable(newAddress);
    }

    function setiLikeCake(address newAddress) external onlyOwner() {
        iLikeCake = payable(newAddress);
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) private returns (bool) {
        ...
        /** Check the cake */
        require(whoCantEat[sender] == false, "You Cant Eat!");
        ...
        else
        {
            if(!isOnceEatExempt[sender] && !isOnceEatExempt[recipient]) {
                require(amount <= _maxOnceEat, "Transfer amount exceeds the.");
            }            
            ...
            if (isExcludedFromCut[sender] || isExcludedFromCut[recipient]) {
                ...
                require(/* eatCake */nowIsEatingCake);
                if (block.number <= whenEat + cantEatCake) {youCantEat(recipient);}
                finalAmount = takeFee(sender, recipient, amount);
            }

            if(LookMaxEat && !isMaxEatExempt[recipient])
                require(balanceOf(recipient).add(finalAmount) <= _maxTotalEat);
    ...
    function swapAndLiquify(uint256 tAmount) private lockTheSwap {
        ...
        if(amountBNBMarketing > 0)
            transferToAddressETH(doYouLikeCake, amountBNBMarketing);

        if(amountBNBTeam > 0)
            transferToAddressETH(iLikeCake, amountBNBTeam);
