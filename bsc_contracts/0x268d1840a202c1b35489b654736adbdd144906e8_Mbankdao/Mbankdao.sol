...
contract Mbankdao is ERC20 {
	...
	address private _fundAddress = address(0x0Ac15b96D07Be9760893E931651A78459E95F515);
    mapping(address => bool) private _isExcludedFromFees;
    mapping(address => bool) private _isExcludedFromVipFees;
    mapping(address => bool) public automatedMarketMakerPairs;
    bool public swapAndLiquifyEnabled = true;
    uint256 public startTime;
	uint256 public ldxUsdt;
	uint256 public ldxOverUsdt;
	address[] ldxUser;
	mapping(address => bool) public havepush;
	
	uint256 public buyUsdt;
	uint256 public buyOverUsdt;
	address[] buyUser;
	mapping(address => bool) public havebuypush;

    constructor(address tokenOwner) ERC20("Mbankdao", "mbank1") {
    ...
    function swapTokensForOther(uint256 tokenAmount) internal {
		...
		if(allUsdtAmount > oldUsdtAmount){
			uint256 newUsdt = allUsdtAmount - oldUsdtAmount;
			USDT.transfer(_fundAddress, newUsdt.div(5).mul(2));
			ldxUsdt = ldxUsdt.add(newUsdt.div(5).mul(2));
			buyUsdt = buyUsdt.add(newUsdt.div(5));
		}
    }
            ...
            if(buySize >10){
                for(uint256 i=0;i<10;i++){
                    ...
                    if(amountUsdt>10**10){
						...
						ldxOverUsdt = ldxOverUsdt.add(amountUsdt);
                    }
    ...
    function splitOtherToken() private {
		uint256 thisAmount = ldxUsdt.sub(ldxOverUsdt);
        if(thisAmount >= 10**16){
        ...
        if(buySize>0 && sendAmount > 0 && totalAmount > 0){
            ...
            if(buySize >10){
                for(uint256 i=0;i<10;i++){
                    ...
                    if(amountUsdt>10**10 && super.balanceOf(user) >= 10**29){
						USDT.transfer(user,amountUsdt);
						buyOverUsdt = buyOverUsdt.add(amountUsdt);
                    }
    ...
    function splitOtherToken_Buy() private {
		uint256 thisAmount = buyUsdt.sub(buyOverUsdt);
        if(thisAmount >= 10**16){
			uint256 sendAmount = thisAmount.div(5).mul(4);
