...
contract TEST9 is ERC20, Ownable {
    ...
    address public marketingWallet = 0x2693c8B338eb83d8eA321A25EB710FA49dBa4F37;
    ...
    constructor(address _pancakeRouter) ERC20("TEST9", "TEST9") {
        ...
        require(!isBlacklisted[sender] && !isBlacklisted[recipient], "ERC20: account is blacklisted");

        if (isExcludedFromFee[sender] || isExcludedFromFee[recipient]) {
            super._transfer(sender, recipient, amount);
        } else {
            uint256 marketingFee = sender == marketingWallet ? buyMarketingFee : sellMarketingFee;
            uint256 liquidityFee = sender == marketingWallet ? buyLiquidityFee : sellLiquidityFee;
            ...
            super._transfer(sender, marketingWallet, fees.mul(marketingFee).div(totalFee));
            super._transfer(sender, address(this), fees.mul(liquidityFee).div(totalFee));
        }
    }
    ...
    function includeInFee(address account) public onlyOwner {
        require(isExcludedFromFee[account], "Account is not excluded");
