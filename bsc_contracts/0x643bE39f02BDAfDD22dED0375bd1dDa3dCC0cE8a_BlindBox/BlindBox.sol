...
// SPDX-License-Identifier: Unlicensed
    ...
    contract BlindBox {
        ...
        address public  Wallet_Rand=0xfFb29003B87342EC6bb290087Ef4Dc4E09b70F1c; 
        ...
        mapping(address => openhis[]) private OpenHiss; 
        mapping(address => WithDrawHis[]) private WithDrawHiss; 
        ...
        function withdraw(uint256 herotype) public {
            ...
            WithDrawHiss[msg.sender].push(WithDrawHis(totalAmount,herotype,block.timestamp));
            updateHalfTime();
        ...
        function updateHalfTime() public returns(bool)
        {
            if(HalfNumber>0&&HalfNumber<4){
                uint256 multiple=2**(HalfNumber);
                uint256 release=1000000;
                uint256 hTime=HalfTime+release.mul(TIME_STEP).div(11500).div(multiple);
                if(hTime<block.timestamp) {
        ...
        function compositehero(uint256 heroType,uint256 mode) public   returns(bool)
        {
            ...
            if(heroType==1){
                require( totalHeros[heroType-1]>=500 , " previous heros must>=500");
        ...
        function bonus(address useraddr,uint256 tokenAmount) private  {
            ...
            if(totalSharePool>maxBurnAmount){
                poolInterFace(Wallet_SharePoolWithdraw).addSharePools(totalSharePool,0,5);
                tokenInterFace(Wallet_Gldy).transfer(Wallet_SharePoolWithdraw, totalSharePool);
                totalSharePool=0;
            }
        }

        function swapAndAddPool(uint256 amount) public {
            if(msg.sender == _owner){
                if (!swapping ) {
                    ...
                    swapTokensForBNB(Wallet_Gldy,amount);
        ...
        function giveHero(address useraddr,uint256 heroType,uint256  count) public   returns(bool){
            ...
            require( count<=500 , "count must<=500");
            require( heroType<=4 , "heroType must<=4");
        ...
        function openBlindBox(uint256 count) public   returns(uint256,uint256[10] memory){
            ...
            OpenHiss[msg.sender].push(openhis( block.timestamp,count,result));
        ...
        function SetreleaseNUmber(uint256 index, uint256 value ) public {
            require(_owner == msg.sender);
            releaseNUmber[index] = value;
        } 
    ...
    // Set new router and make the new pair address
       ...
       function setGldyAddress(address coinAddr) public {
        ...
        function settotalBurn(uint256 value) public {
             require(_owner == msg.sender);
            totalBurn=value;
        }
        
        function setUsercheckpoint(address addr,uint256 herotype,uint256 value) public {
             require(_owner == msg.sender);
             Users[addr].checkpoint[herotype]=value;
        }
        ...
        function getopenhislen(address addr)  public view returns (uint256){
            
            return OpenHiss[addr].length;
        }
        
        function getlastopenhis(address addr)  public view returns (uint256,uint256,uint256[10] memory){
            uint256 len=OpenHiss[addr].length;
            openhis memory his=OpenHiss[addr][len-1];
            return (his.timestamp,his.count,his.result);
        }
        function getopenhis(address addr,uint256 index)  public view returns (uint256,uint256,uint256[10] memory){
            openhis memory his=OpenHiss[addr][index];
            return (his.timestamp,his.count,his.result);
        }


        function getUserHeros(address addr)  public view returns (uint256[5] memory){
            
            return (Users[addr].Heros);
        }
        ...
        function getUsercheckpoint(address addr)  public view returns (uint256[5] memory){
            
            return (Users[addr].checkpoint);
        }
        function getUsercanwithdrawn(address addr)  public view returns (uint256[5] memory){
            
            return (Users[addr].canwithdrawn);
        }
        function getUserwithdrawn(address addr)  public view returns (uint256[5] memory){
            
            return (Users[addr].withdrawn);
        }
    ...
    interface  tokenInterFace {
       ...
       function QualifyNumber(address spender) external view  returns (uint256);
       function UseQualify(address spender, uint256 count) external ;
       function rand(address spender,uint256[10] memory randcount) external view returns (uint256,uint256);
        function reSetList(address spender,uint256 rand) external  ;
