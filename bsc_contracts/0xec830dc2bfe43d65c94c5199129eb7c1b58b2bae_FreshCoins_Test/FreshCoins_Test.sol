...
/**
---------------------------------------------------------------------------------------------------
 Contract Developed by FreshCoins.io
 t.me/FreshCoinsTG

 [Visit FreshCoins.io right now]
---------------------------------------------------------------------------------------------------
*/
...
contract FreshCoins_Test is BEP20, Ownable {
    ...
    uint256 public tokenLiquidityThreshold = 499999 * 10**18; 

    uint256 public genesis_block;
    uint256 private deadline = 3;
    uint256 private launchtax = 99;

    address public marketingWallet = 0x3Ea0d8B81650918C6F571002BEf7347826aC2ECa;
    address public developmentWallet = 0xF713AD5800DEAC4A5D2B605B41AC0Cb09c01B5d5;
    ...
    Taxes public taxes = Taxes(1, 1, 0);
    Taxes public sellTaxes = Taxes(1, 3, 0);
    ...
    constructor() BEP20("FreshCoins Test", "FCT") {
        ...
        lockTheSwap
    {
        ...
        if (contractBalance >= tokenLiquidityThreshold) {
            ...
            uint256 marketingAmt = unitBalance * swapTaxes.marketing;
            if (marketingAmt > 0) {
                payable(marketingWallet).sendValue(marketingAmt);
            }

            uint256 developmentAmt = unitBalance * swapTaxes.development;
            if (developmentAmt > 0) {
    ...
    function SetBuyTaxes(uint256 _marketing, uint256 _development, uint256 _liquidity)
        external
        onlyOwner
    {
        taxes = Taxes(_marketing, _development, _liquidity);
        require(
            (_marketing + _development + _liquidity) <= 7,
            "Must keep fees at 7% or less"
        );
    }

    function SetSellTaxes(uint256 _marketing, uint256 _development, uint256 _liquidity)
        external
        onlyOwner
    {
    ...
    function updateDevelopmentWallet(address newWallet) external onlyOwner {
        require(newWallet != address(0), "Development Address cannot be zero address");
