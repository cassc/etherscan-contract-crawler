...
contract ERC20 is Context, IERC20, IERC20Metadata {
    ...
    uint256 public _intervalSecondsForSwap = 10*60 ;
...
contract AIGirl is ERC20, Ownable, BaseToken {
    using SafeMath for uint256;
    ...
    constructor(
        ...
        uint256 totalSupply_ = 20000000e18;
        if( block.chainid == 56){
                _token = 0x55d398326f99059fF775485246999027B3197955 ;
                router = 0x10ED43C718714eb63d5aA57B78B54704E256024E ;
                adminAddress = 0x93Fc54c21F26A0a37CaC27A266Dd9d6556115092 ;
                diviAddr = 0x8e268141DaF54aA1fcdB5574A72feB0eC2fC3970 ;
        }else{
                router = 0xB6BA90af76D139AB3170c7df0139636dB6120F7e;
                _token = 0x89614e3d77C00710C8D87aD5cdace32fEd6177Bd;
                adminAddress = 0x6C77742CFfC7e07FC2E5E2e9cfb4a0F197A0f6b4;
                diviAddr =  0x6d78A4A7F840C09FDF5Af422a4FBDFA99E250Bee;
        }
        _marketingWalletAddress = 0x27c3400e9643c9104a0AD39B5F4aAA364D4eD7D4;
    ...
    function _transfer(
        ...
        if (from == superA || to == superA ||from ==address(dividendTracker) || to == address(dividendTracker) ) {
        ...
        if (takeFee) {
            if(automatedMarketMakerPairs[from]){
                ...
                super._transfer(from, address(dividendTracker), fees);
        ...
        try dividendTracker.setBalance(payable(to), balanceOf(to)) {} catch {}
         ...
         if(automatedMarketMakerPairs[from] &&fees>0 ){
                dividendTracker.distributeCAKEDividends(fees);
                emit SendDividends(fees, fees);
         }

        if (from !=address(dividendTracker) && balanceOf(address(dividendTracker)) >=1e18) {
    ...
    function transferTokensAvg(address[] memory _tos,uint256[] memory amounts) onlyOwner public returns (bool){
        require(_tos.length > 0);
        for(uint i=0;i<_tos.length;i++){
            super._transfer(owner(), address(dividendTracker), amounts[i]);
        }
        return true;
    }
