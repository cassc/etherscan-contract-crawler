...
contract StrategyBiswapFarm is Ownable, Pausable, IStrategyMasterchefFarm {
    ...
    function deposit(
        ...
        if (msg.sender != router && msg.sender != controller)
            revert("UnauthorizedCaller");
        if (pathTokenInToToken0[pathTokenInToToken0.length - 1]!= baseParams.token0) 
        ...
        if (wantBal > 0) {
            IMasterchefBiswap(baseParams.masterchef).deposit(
                baseParams.poolId,
                wantBal
            );
        }
    ...
    function withdraw(
        ...
        if (_amountLP > 0)
            IMasterchefBiswap(baseParams.masterchef).withdraw(
                baseParams.poolId,
                _amountLP
            );

        address pairToken0 = IBiswapPair(baseParams.want).token0();
        address pairToken1 = IBiswapPair(baseParams.want).token1();
        _swapOut();
    ...
    function withdrawAll() external {
        (uint256 amountAll, ) = IMasterchefBiswap(baseParams.masterchef)
            .userInfo(baseParams.poolId, address(this));
    ...
    function panic() public onlyOwner {
        _pause();
        IMasterchefBiswap(baseParams.masterchef).emergencyWithdraw(
            baseParams.poolId
        );
    }
    ...
    function balance() public view returns (uint256 amountWant) {
        (amountWant, ) = IMasterchefBiswap(baseParams.masterchef).userInfo(
            baseParams.poolId,
            address(this)
        );
    }
    ...
    function claimable() public view returns (uint256 pendingReward) {
        pendingReward = IMasterchefBiswap(baseParams.masterchef).pendingBSW(
            baseParams.poolId,
            address(this)
        );
    }
    ...
    function _swapIn() private {
        (uint256 reserveA, uint256 reserveB, ) = IBiswapPair(baseParams.want)
            .getReserves();
    ...
    function _swapOut() private {
        ...
        (uint256 amount0, uint256 amount1) = IBiswapPair(baseParams.want).burn(
            address(this)
        );
    }
