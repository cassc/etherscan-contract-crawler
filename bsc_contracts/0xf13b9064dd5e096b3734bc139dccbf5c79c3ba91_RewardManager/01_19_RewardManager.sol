  ...
  function syncPendingReward(address _user, uint[] calldata _pendingDays, uint[][] calldata _fotaPrices) external onlyMainAdmin {
    ...
    for (uint dayIndex = 0; dayIndex < _pendingDays.length; dayIndex++) {
      ...
      for (uint gameIndex = 0; gameIndex < _fotaPrices[dayIndex].length; gameIndex++) {
        price = _fotaPrices[dayIndex][gameIndex];
        if (price == 0) {
          delete rewards[_user][pendingDay][gameIndex];
        } else {
          rewards[_user][pendingDay][gameIndex].userAmount = rewards[_user][pendingDay][gameIndex].userAmount * 1000 / price;
          rewards[_user][pendingDay][gameIndex].farmShareAmount = rewards[_user][pendingDay][gameIndex].farmShareAmount * 1000 / price;
          rewards[_user][pendingDay][gameIndex].referralShareAmount = rewards[_user][pendingDay][gameIndex].referralShareAmount * 1000 / price;
          rewards[_user][pendingDay][gameIndex].landLordShareAmount = rewards[_user][pendingDay][gameIndex].landLordShareAmount * 1000 / price;
