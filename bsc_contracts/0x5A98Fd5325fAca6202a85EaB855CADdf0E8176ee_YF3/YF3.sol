...
contract YF3 is Context, Ownable{
    ...
    function doThing(uint doType, uint amount) public payable returns (bool){
        ...
        if (doType == 1){
            ...
            assert(uCoin.transferFrom(msg.sender, contractAddress, amount.mul(takeunit).mul(leftP).div(1000)) == true);
        } else {
            // score + coin
    ...
    function setContractAddress (address _setAddress)  public onlyOwner{
        contractAddress = _setAddress;
    }
