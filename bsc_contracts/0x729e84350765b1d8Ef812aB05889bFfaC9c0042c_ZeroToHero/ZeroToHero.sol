...
contract ZeroToHero is ERC721  {
    uint256 public totalSupply;
    uint256 public cost = 0.0010 ether;
    ...
    constructor()
        ERC721("Zero2Hero Bootcamp CIS", "ZTHBC")
    {}

    function mint() external payable {
        
        if (userMints[msg.sender] >= maxMints) revert MintLimit();
        if (totalSupply + 1 > 200) revert SoldOut();
        ...
        returns (string memory)
    {
        return
            string(
                abi.encodePacked("ipfs://bafybeigzuayjjtk5pyreydqbnyyp3upppyrx47aeq3xxdaz2kif3t5afva/", Strings.toString(tokenId), ".json")
            );
    }
