...
contract KOY is ERC20, Ownable {
    ...
    KOYDividendTracker public dividendTracker;
    ...
    constructor() ERC20("KOY", "KOY") {
        ...
        dividendTracker = new KOYDividendTracker(
            payable(this),
            router,
...
contract KOYDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    function _transfer(
        ...
        require(false, "KOY_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "KOY_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main KOY contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(KOYContract);

            uint256 prevBalance = KOYContract.balanceOf(address(this));
            ...
            {
                uint256 received = KOYContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    KOYContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
