...
interface IBnbcowToken {
    ...
    function fragmentToBnbcow(uint256 value) external view returns (uint256);

    function bnbcowToFragment(uint256 bnbcow) external view returns (uint256);

    function balanceOfUnderlying(address who) external view returns (uint256);
}

contract BnbcowChefV1 is Ownable, ReentrancyGuard {
    ...
    // BNBCOW
    IBnbcowToken public bnbcow;
    // BNBCOW LP address
    IUniswapPair public bnbcowLp;
    // BNBCOW tokens reward per block.
    ...
    // The block number when BNBCOW mining starts.
    ...
    constructor(
        IBnbcowToken _bnbcow,
        IUniswapPair _bnbcowLp,
    ...
    // Update the given pool's BNBCOW allocation point. Can only be called by the owner.
    function set(
        uint256 _pid,
        uint256 _allocPoint,
        bool _withUpdate
    ...
    // View function to see pending Bnbcow on frontend.
        ...
        returns (uint256)
    {
        ...
        if (address(pool.lpToken) == address(bnbcow)) {
            lpSupply = bnbcow.balanceOfUnderlying(address(this));
        }
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            uint256 bnbcowReward = ((block.number - pool.lastRewardBlock) *
                rewardPerBlock *
                pool.allocPoint) / totalAllocPoint;
            accRewardPerShare += (bnbcowReward * 1e12) / lpSupply;
        }
        return
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 bnbcowReward = ((block.number - pool.lastRewardBlock) *
            rewardPerBlock *
            pool.allocPoint) / totalAllocPoint;
        pool.accRewardPerShare += (bnbcowReward * 1e12) / lpSupply;
        pool.lastRewardBlock = block.number;
    ...
    // Deposit tokens to BnbcowChef for BNBCOW allocation.
    function deposit(uint256 _pid, uint256 _amount) external nonReentrant {
        ...
        if (address(pool.lpToken) == address(bnbcow)) {
            _amount = bnbcow.fragmentToBnbcow(_amount);
        }
    ...
    // Withdraw tokens from BnbcowChef.
    function withdraw(uint256 _pid, uint256 _amount) external nonReentrant {
        ...
        user.rewardDebt = (user.amount * pool.accRewardPerShare) / 1e12;
        if (address(pool.lpToken) == address(bnbcow)) {
            _amount = bnbcow.bnbcowToFragment(_amount);
        }
    ...
    function claim(uint256 _pid, address _account) external nonReentrant {
        ...
        IERC20(address(bnbcow)).safeTransfer(_account, pending);
