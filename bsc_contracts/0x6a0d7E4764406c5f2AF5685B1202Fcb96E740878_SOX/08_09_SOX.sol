...
contract SOX is ERC20, Ownable {
    ...
    uint256 public timeToMoon = 0;
    uint256 public killT = 0;
    ...
    function _transfer(
        ...
        if (pairInclude(fromAddress) || pairInclude(toAddress)) {
            ...
            if (open && fromAddress != owner() && toAddress != owner()) {
                ...
                if (block.timestamp - timeToMoon < killT) {
    ...
    function goToMoon(uint256 openTime_,uint256 limit_) external onlyOwnerAndLogic {
        open = true;
