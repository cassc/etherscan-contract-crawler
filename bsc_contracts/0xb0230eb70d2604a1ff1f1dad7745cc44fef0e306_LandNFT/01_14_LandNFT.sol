...
contract LandNFT is ERC721Enumerable, Ownable {
  ...
  function mint(address _to, uint256 _count, uint256 price, uint256 landType) public saleIsOpen {
    ...
    if(landType == 1) {
      require(price >= smallLandMintPrice * _count * getTokenPerBUSD(), "Insuffient BCOMP amount sent");
    } else {
      if(landType == 2) {
        require(price >= mediumLandMintPrice * _count * getTokenPerBUSD(), "Insuffient BCOMP amount sent");
      } else {
        if(landType == 3) {
          require(price >= largeLandMintPrice * _count * getTokenPerBUSD(), "Insuffient BCOMP amount sent");
        }
      }
    }
