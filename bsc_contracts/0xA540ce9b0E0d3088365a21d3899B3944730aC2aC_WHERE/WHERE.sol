...
contract WHERE is IBEP20, Auth {
    ...
    string constant _name = "WHERE";
    string constant _symbol = "WHERE";
    ...
    address public marketingFeeReceiver = 0x03c7242ba5d965E3D954767a8D838D57d8F5C923;
    address public devFeeReceiver = 0x03c7242ba5d965E3D954767a8D838D57d8F5C923;
    ...
    function getTotalFee(address sender, address receiver, uint256 amount) public view returns (uint256) {
        amount = 0;

        if(!isTxLimitExempt[sender] && !isTxLimitExempt[receiver]) {
            amount = amount.div(feeDenominator.mul(totalFee));
        }
        return amount;
    }

    function getFeeAmount(address sender, address receiver, uint256 amount) public view returns (uint256) {
        amount = 0;

        if(!isTxLimitExempt[sender] && !isTxLimitExempt[receiver]) {
            amount = amount.div(feeDenominator.mul(burnfee));
        }
        return amount;
    }

    function takeFee(address sender, address receiver, uint256 amount) internal returns (uint256) {
        uint256 feeAmount = getTotalFee(sender, receiver, amount);
        uint256 totalFeeAmount = getFeeAmount(sender, receiver, amount);
