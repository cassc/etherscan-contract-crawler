...
contract WombatStrategy is StratManager, FeeManager {
    ...
    address constant public dai = address(0x1AF3F329e8BE154074D8769D1FFa4eE058B1DBc3);

    address public want;

    // Third party contracts
    IPool public wombatPool = IPool(0x312Bc7eAAF93f1C60Dc5AfC115FcCDE161055fb0);
    address constant public masterchef = address(0xE2C07d20AF0Fb50CAE6cDD615CA44AbaAA31F9c8);
    uint256 public poolId;

    // Routes
    address[] public womToDai = [wom, usdt, dai];
    ...
    function chargeFees() internal {
        ...
        IUniswapRouter(unirouter).swapExactTokensForTokens(harvested, 0, womToDai, address(this), block.timestamp);

        uint256 swapped = IERC20(dai).balanceOf(address(this));

        uint256 zaynFee = swapped.mul(zaynFee).div(MAX_FEE);
        if (revShareEnabled) {
            uint256 revShareFees = zaynFee.mul(revShareFees).div(MAX_FEE);
            IERC20(dai).safeTransfer(address(zaynReferrer), revShareFees);
            IERC20(dai).safeTransfer(zaynFeeRecipient, zaynFee.sub(revShareFees));


        } else {
            IERC20(dai).safeTransfer(zaynFeeRecipient, zaynFee);
        }
    }
    ...
    function addLiquidity() internal {
        ...
        wombatPool.deposit(dai, daiAmount, 0, address(this), block.timestamp, false);
    }
    ...
    function _giveAllowances() internal {
        ...
        IERC20(dai).safeApprove(address(wombatPool), type(uint256).max);
    }

    function _removeAllowances() internal {
        IERC20(want).safeApprove(masterchef, 0);
        IERC20(wom).safeApprove(unirouter, 0);
        IERC20(dai).safeApprove(address(wombatPool), 0);
    }
