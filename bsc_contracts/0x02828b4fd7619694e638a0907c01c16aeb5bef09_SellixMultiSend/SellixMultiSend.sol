...
contract SellixMultiSend {
    ...
    function multiSendBnb(address[] addresses, uint256[] amounts) public payable returns(bool success){
        uint total = 0;
        for(uint8 i = 0; i < amounts.length; i++){
            total = total.add(amounts[i]);
        }
        
        //ensure that the bnb is enough to complete the transaction
        uint requiredAmount = total.add(bnbSendFee * 1 wei); //.add(total.div(100));
    ...
    function withdrawBnb(address addr, uint amount) public onlyOwner returns(bool success){
    ...
    function setBnbFee(uint _bnbSendFee) public onlyOwner returns(bool success){
        bnbSendFee = _bnbSendFee;
        return true;
    }
