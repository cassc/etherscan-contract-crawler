...
// File: pancake/GRVSFeeHandler.sol
...
// GRVSFeeHandler_V3
contract GRVSFeeHandler is UUPSUpgradeable, OwnableUpgradeable {
    ...
    event RmoveLiquidityFailure(IGrovePair pair, uint amount, uint amountAMin, uint amountBMin);
    event NewGroveSwapRouter(address indexed sender, address indexed router);
    event NewOperatorAddress(address indexed sender, address indexed operator);
    event NewGroveBurnAddress(address indexed sender, address indexed burnAddress);
    event NewGroveVaultAddress(address indexed sender, address indexed vaultAddress);
    event NewGroveBurnRate(address indexed sender, uint groveBurnRate);

    address public grove;
    IGroveRouter02 public groveSwapRouter;
    address public operatorAddress; // address of the operator
    address public groveBurnAddress;
    address public groveVaultAddress;
    uint public groveBurnRate; // rate for burn (e.g. 718750 means 71.875%)
    ...
    function initialize(
        address _grove,
        address _groveSwapRouter,
        address _operatorAddress,
        address _groveBurnAddress,
        address _groveVaultAddress,
        uint _groveBurnRate,
    ...
    initializer
    {
        ...
        grove = _grove;
        groveSwapRouter = IGroveRouter02(_groveSwapRouter);
        operatorAddress = _operatorAddress;
        groveBurnAddress = _groveBurnAddress;
        groveVaultAddress = _groveVaultAddress;
        groveBurnRate = _groveBurnRate;
        for (uint256 i = 0; i < destinations.length; ++i)
        {
            validDestination[destinations[i]] = true;
        }
        WETH = IWETH(groveSwapRouter.WETH());
        operatorTopUpLimit = 100 ether;
    }

    /**
     * @notice Sell LP token, buy back $GROVE. The amount can be specified by the caller.
    ...
    internal
    {
        uint allowance = info.pair.allowance(address(this), address(groveSwapRouter));
        if (allowance < info.amount) {
            IERC20Upgradeable(address(info.pair)).safeApprove(address(groveSwapRouter), UNLIMITED_APPROVAL_AMOUNT);
        }
        address token0 = info.pair.token0();
        address token1 = info.pair.token1();
    try groveSwapRouter.removeLiquidity(
    token0,
    token1,
    info.amount,
    info.amountAMin,
    ...
    internal
    {
        ...
        uint allowance = IERC20Upgradeable(token).allowance(address(this), address(groveSwapRouter));
        if (allowance < amountIn) {
            IERC20Upgradeable(token).safeApprove(address(groveSwapRouter), UNLIMITED_APPROVAL_AMOUNT);
        }
    try groveSwapRouter.swapExactTokensForTokensSupportingFeeOnTransferTokens(
    amountIn,
    amountOutMin,
    path,
    address(this),
    ...
    /**
     * @notice Send $GROVE tokens to specified wallets(burn and vault)
     * @dev Callable by owner/operator
     */
    function sendGrove(uint amount)
    external
    onlyOwnerOrOperator
    {
        require (amount > 0, "invalid amount");
        uint burnAmount = amount * groveBurnRate / RATE_DENOMINATOR;
        // The rest goes to the vault wallet.
        uint vaultAmount = amount - burnAmount;
        IERC20Upgradeable(grove).safeTransfer(groveBurnAddress, burnAmount);
        IERC20Upgradeable(grove).safeTransfer(groveVaultAddress, vaultAmount);
    }
    ...
    /**
     * @notice Set GroveSwapRouter
     * @dev Callable by owner
     */
    function setGroveSwapRouter(address _groveSwapRouter) external onlyOwner {
        groveSwapRouter = IGroveRouter02(_groveSwapRouter);
        emit NewGroveSwapRouter(msg.sender, _groveSwapRouter);
    }
    ...
    /**
     * @notice Set address for `grove burn`
     * @dev Callable by owner
     */
    function setGroveBurnAddress(address _groveBurnAddress) external onlyOwner {
        groveBurnAddress = _groveBurnAddress;
        emit NewGroveBurnAddress(msg.sender, _groveBurnAddress);
    }
     ...
     * @dev Callable by owner
     */
    function setGroveVaultAddress(address _groveVaultAddress) external onlyOwner {
        groveVaultAddress = _groveVaultAddress;
        emit NewGroveVaultAddress(msg.sender, _groveVaultAddress);
    }

    /**
     * @notice Set percentage of $GROVE being sent for burn
     * @dev Callable by owner
     */
    function setGroveBurnRate(uint _groveBurnRate) external onlyOwner {
        require(_groveBurnRate < RATE_DENOMINATOR, "invalid rate");
        groveBurnRate = _groveBurnRate;
        emit NewGroveBurnRate(msg.sender, _groveBurnRate);
    }
    ...
    {
        IGroveFactory grvsFactory = IGroveFactory(factory);
        uint256 maxLength = grvsFactory.allPairsLength();
        uint256 length = size;
        if (cursor >= maxLength) {
        ...
        for (uint256 i = 0; i < length; ++i) {
            address tempAddr = address(grvsFactory.allPairs(cursor+i));
            values[i] = tempAddr;
        }
    ...
    )
    {
        ...
        for (uint256 i = 0; i < lps.length; ++i) {
            IGrovePair pair = IGrovePair(lps[i]);
