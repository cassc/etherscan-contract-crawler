    ...
    WhitelistManager
{
    ...
    function depositWithBonus(
        ...
        if (_bonusInitialMargin > 0) {
            ...
            uint256 _newBonusBalance =  busdBonusBalances[_positionManager][_trader] + _bonusInitialMargin;
            if(maximumBUSDBonusAccepted != 0 && _newBonusBalance > maximumBUSDBonusAccepted) {
                // add the difference to the real margin
                _realInitialMargin += _newBonusBalance - maximumBUSDBonusAccepted;
                // only accept the maximum bonus
                _bonusInitialMargin = maximumBUSDBonusAccepted - busdBonusBalances[_positionManager][_trader];
            }
            if(_bonusInitialMargin > 0){
    ...
    function updateMaximumBUSDBonusAcceptedPerPosition(uint256 _newMaximumBUSDBonusAccepted) public onlyOwner {
        emit MaxBUSDBonusAcceptedPerPositionUpdated(maximumBUSDBonusAccepted, _newMaximumBUSDBonusAccepted);
