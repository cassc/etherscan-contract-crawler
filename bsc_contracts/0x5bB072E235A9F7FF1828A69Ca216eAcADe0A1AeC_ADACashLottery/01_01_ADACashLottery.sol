...
contract ADACashLottery is Ownable {
    ...
    uint8 public firstplacePercent = 80; // 60 % of Jackpot will go to first Place
    uint8 public secondplacePercent = 15; // 25 % of Jackpot will go to second Place
    uint8 public thirdplacePercent = 5; // 15 % of Jackpot will go to third Place

    uint256 lastburned = 0;

    IERC20 public paytoken = IERC20(0xF9F6f85E4b2296e93Cc25c949D7f0364654485cC);
    ...
    /* Write functions */
    function enterLottery(uint256 _numberOfEntries) external {
        uint256 entrance = getEntranceFee();
        uint256 endprice = entrance * _numberOfEntries;
        ...
        require(
            (ticketCounter[msg.sender] + _numberOfEntries) <= 10,
            "Per wallet only 10 Tickets allowed"
        );

        require(
            paytoken.transferFrom(msg.sender, address(this), endprice) == true,
    ...
    function swapFortuneCashForEth(uint256 tokenAmount) internal {
        address[] memory path = new address[](2);
        path[0] = address(0x651a89fed302227d41425235F8E934502FB94C48);
    ...
    function withdrawPayTokens(uint256 amountPercentage) external onlyOwner {
    ...
    function payWinners(uint256 first) internal {
        ...
        wintoken.transfer(teamWallet, marketing);
        ...
        uint256 burningz = (potSize * 20) / 100;
        wintoken.transfer(deadwallet, burningz);

        lastburned = burning + burningz;
