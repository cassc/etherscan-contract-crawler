...
contract PresaleBSC is Initializable, ReentrancyGuardUpgradeable, OwnableUpgradeable, PausableUpgradeable {
   ...
   */
  function initialize(address _oracle, address _usdt, uint256 _startTime, uint256 _endTime, uint256[1][2] memory _rounds, 
  uint256[][2] memory _tokenQuantity, uint256[][2] memory _lockup, uint256 _defaultLockup, uint256 _initialClaimPercent,
  address _admin
  ...
  function calculatePrice(uint256 _amount) public view returns (uint256 totalValue) {
    ...
    if (_amount + totalTokensSold > rounds[0][currentStep]) {
      ...
      if (linearStartTime == 0) {
        USDTAmount = _amount * rounds[1][currentStep];
      } else {
        USDTAmount = _amount * (rounds[1][currentStep] + ((rounds[1][currentStep] * (((block.timestamp - linearStartTime) * baseDecimals) / (endTime - linearStartTime))) / baseDecimals));
      }
    }
    return USDTAmount;
  }
  ...
  function updateLinearStartTime(uint256 time) external {
    require(_msgSender() == admin || _msgSender() == owner() ,'caller should be owner or admin');
