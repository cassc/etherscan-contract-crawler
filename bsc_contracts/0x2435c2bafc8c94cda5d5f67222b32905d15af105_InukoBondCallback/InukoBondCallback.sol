...
abstract contract BondBaseCallback is IBondCallback, Ownable, ReentrancyGuard {
    ...
    modifier onlyDepositor() {
        require(depositors[msg.sender] == true || msg.sender == address(Inuko) || msg.sender == owner() , "Depositor must be authorized or the token itself.");
        _;
    }
...
contract InukoBondCallback is BondBaseCallback
{
    using TransferHelper for IBEP20;
    ...
    function deposit(uint256 amount_) external onlyDepositor {
        Inuko.safeTransferFrom(msg.sender, address(this), amount_);
        priorBalances[Inuko] = Inuko.balanceOf(address(this));
    }

    /* ========== CONSTRUCTOR ========== */

    constructor(IBondAggregator aggregator_, address _inuko) BondBaseCallback(aggregator_) {
            Inuko = IBEP20(_inuko);
    }
