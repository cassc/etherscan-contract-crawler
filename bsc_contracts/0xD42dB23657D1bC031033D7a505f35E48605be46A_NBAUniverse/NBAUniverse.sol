...
contract NBAUniverse is ERC20, Ownable {
    ...
    constructor() ERC20("NBA Universe", "$NBAU") {
        ...
        charityFeeOnBuy = 1;
        charityFeeOnSell = 1;
        _totalFeesOnBuy = developmentFeeOnBuy + charityFeeOnBuy;
        _totalFeesOnSell = developmentFeeOnSell + charityFeeOnSell;

        charityWallet = 0x3e31FBe0f2e655F53770516100881Dd13B052a67;
        devWallet = 0x18e2d1a4c279aA7977fB5DaC401170160970c996;
    ...
    function swapAndSendFee(uint256 amount) private {
        ...
        uint charityShare = charityFeeOnBuy + charityFeeOnSell;

        if (developmentShare > 0) {
            uint256 developmentBNB = (newBalance * developmentShare) / totalFee;
            payable(devWallet).sendValue(developmentBNB);
        }

        if (charityShare > 0) {
            uint256 charityBNB = (newBalance * charityShare) / totalFee;
            payable(charityWallet).sendValue(charityBNB);
        }

        emit SwapAndSendFee(amount, newBalance);
    }
}