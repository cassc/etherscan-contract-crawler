...
/** CapsLootChest made by @TheCapSurge */
...
contract CapsLootChest is ReentrancyGuard, Ownable {
    ...
    event TotalDrippedLast24hours(uint256 amount);
    event TotalDrippedSinceLaunch(uint256 amount);
    ...
    constructor() {
        ...
        normalWithdrawalTime = 24 hours;
        silverWithdrawalTime = 12 hours;
        goldWithdrawalTime = 6 hours;
    ...
    function claim(address _user) external nonReentrant {
        ...
        emit TotalDrippedLast24hours(totalDrippedLast24Hours);
        emit TotalDrippedSinceLaunch(totalDrippedSinceLaunch);
        emit Claimed(_user, tokensToClaim);

        if (SURGE.balanceOf(address(this)) >= 1000 * 10**18) {
