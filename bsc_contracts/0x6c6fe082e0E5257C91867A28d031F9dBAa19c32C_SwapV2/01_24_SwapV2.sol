...
contract SwapV2 is BaseContract
{
    ...
    function setup() external
    {
        ...
        _isExemptFromCooldown[addressBook.get("lpStaking")] = true;
    ...
    function sell(uint256 amount_) external whenNotPaused
    {
        ...
        if(_furpoolAmount_ > 0) ILPStakingV1(addressBook.get("lpStaking")).stakeFor(address(usdc), _furpoolAmount_, 1, msg.sender);
    ...
    function calculateTax(address participant_, uint256 amount_) public view returns (uint256)
    {
        ...
        uint256 _tax_ = tax;
        if(vault.participantBalance(participant_) * pumpAndDumpRate / 10000 < amount_) {
            _tax_ = tax * pumpAndDumpMultiplier;
        }
        return amount_ * _tax_ / 10000;
    }
    ...
    function calculateFurpool(address participant_, uint256 amount_) public view returns (uint256)
    {
        ...
        if(vault.rewardRate(participant_) < 51) {
            _percentToFurpool_ = 50;
        }
