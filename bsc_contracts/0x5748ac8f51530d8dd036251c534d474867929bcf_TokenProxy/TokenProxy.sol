...
contract TokenProxy {
    ...
    uint256 public maxSupply = 21000000 * 10 ** 18; // 21 million tokens with 18 decimals
    ...
    function transfer(address recipient, uint256 amount) external returns (bool) {
        ...
        if (burnAmount > 0) {
            bytes memory burnData = abi.encodeWithSignature("transfer(address,uint256)", address(this), burnAmount);
            (bool success, ) = tokenAddress.delegatecall(burnData);
            require(success, "Delegatecall failed");
            // The burned tokens are transferred to this contract address, but they cannot be withdrawn
            amount -= burnAmount;
        }
        bytes memory transferData = abi.encodeWithSignature("transfer(address,uint256)", recipient, amount);
        (bool success, ) = tokenAddress.delegatecall(transferData);
        require(success, "Delegatecall failed");
        return true;
    }
}
