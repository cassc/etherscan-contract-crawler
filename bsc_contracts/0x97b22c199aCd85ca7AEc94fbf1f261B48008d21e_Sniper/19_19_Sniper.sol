...
contract Sniper is AccessControlUpgradeable {
	...
	function snipe(address token, uint pairIndex, bool getTokens) external {
		...
		uint initialAmt = amountTrade / 100;

		// Swap 1% of total Swap to test pool
		uint received = router.swapExactETHForTokens{value: initialAmt}(
			1,
			path,
			address(this),
			block.timestamp
		...
		returns (uint256[] memory amounts) {
			...
			uint buyAmt = amountTrade - initialAmt;
			path[0] = WETH;
			path[1] = token;

			uint received2 = router.swapExactETHForTokens{value: buyAmt}(
				1,
				path,
				address(this),
				block.timestamp
			...
			emit Sniped(token, received2, buyAmt);
		} catch {}
