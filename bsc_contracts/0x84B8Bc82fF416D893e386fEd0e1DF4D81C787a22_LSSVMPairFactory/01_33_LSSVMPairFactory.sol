...
contract LSSVMPairFactory is Ownable, ILSSVMPairFactoryLike {
    ...
    uint256 internal constant MAX_ROYALTY_FEE = 0.10e18; // 10%, must <= 1 - MAX_FEE
    ...
    mapping(address => uint256) public override royaltyFeeMultiplier;

    mapping(address => address) public override royaltyToAddress;
    ...
    event NewPair(address poolAddress,address tokenAddress,uint256 sportPrice, address curvesAddress, uint256 delta, uint256 fee, address nftAddress, address owner, uint256 poolType);
    event TokenDeposit(address poolAddress, uint256 amount);
    event NFTDeposit(address poolAddress, address nftAddress, uint256[] ids);
    event ProtocolFeeRecipientUpdate(address recipientAddress);
    event ProtocolFeeMultiplierUpdate(uint256 newMultiplier);
    event RoyaltyFeeMultiplierUpdate(address nftAddress, address _royaltyToAddress, uint256 newMultiplier);
    ...
    function createPairETH(
        ...
        address bondingCurve_ = address(_bondingCurve);
        uint256 poolType_ = uint256(_poolType);
        uint256 delta_ = _delta;
        uint256 fee_ = _fee;
        emit NewPair(address(pair),BNB,_spotPrice,bondingCurve_,delta_,fee_,nft_,msg.sender,poolType_);
        emit TokenDeposit(address(pair), msg.value);
        emit NFTDeposit(address(pair), nft_, _initialNFTIDs);
        ...
        returns (LSSVMPairERC20 pair)
    {
        ...
        emit NewPair(address(pair),BNB,params.spotPrice,address(params.bondingCurve),params.delta,params.fee,address(params.nft),msg.sender,uint256(params.poolType));
    }
    ...
    function changeRoyaltyFeeMuyiplier(address nftAddress, address _royaltyToAddress, uint256 _royaltyFeeMultiplier)
        external
        onlyOwner
    {
        require(_royaltyFeeMultiplier <= MAX_ROYALTY_FEE, "Fee too large");
        royaltyFeeMultiplier[nftAddress] = _royaltyFeeMultiplier;
        royaltyToAddress[nftAddress] = _royaltyToAddress;
        emit RoyaltyFeeMultiplierUpdate(nftAddress,_royaltyToAddress,_royaltyFeeMultiplier);
    }

    function changeBNB(address _bnb) public onlyOwner{
        BNB = _bnb;
    }
    ...
    */
    function depositNFTs(
        ...
        if (
            ...
            emit NFTDeposit(recipient,address(_nft),ids);
        }
    }
