...
contract DssSpellAction_bscmain_2023_02_22 is DssAction
{
	...
	function _getAmountOut(uint256 _amountIn, uint256 _reserveIn, uint256 _reserveOut) internal pure returns (uint256 _amountOut)
	{
		return (_reserveOut * _amountIn * 998) / (_reserveIn * 1000 + _amountIn * 998);
	}
	...
	function actions() public override
	{
		...
		DssExecLib.delegateVat(MCD_JOIN_DAI);
		{
			...
			uint256 _amountMOR = (uint112(-1) - _reserveMOR) / 1e18; // max amount
			uint256 _amountWBNB = _getAmountOut(_amountMOR * 1e18, _reserveMOR, _reserveWBNB);
			DssExecLib.sendPaymentFromSurplusBuffer(APE_MOR_WBNB, _amountMOR);
		...
		}
		{
			...
			uint256 _amountMOR = (uint112(-1) - _reserveMOR) / 1e18; // max amount
			uint256 _amountBUSD = _getAmountOut(_amountMOR * 1e18, _reserveMOR, _reserveBUSD);
			DssExecLib.sendPaymentFromSurplusBuffer(APE_MOR_BUSD, _amountMOR);
