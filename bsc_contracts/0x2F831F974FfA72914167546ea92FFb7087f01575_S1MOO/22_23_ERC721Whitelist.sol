...
abstract contract ERC721Whitelist is Ownable {
     ...
     */
    function grantWhitelistSpots(
        address to,
        uint256 amount
    ) external onlyOwner {
        require(
            whitelistSpotsOpen + amount <= maxWhitelistSpots,
            "Exceeds maxWhitelistSpots"
        );
     ...
     */
    function grantWhitelistSpotsBatch(
        address[] memory to,
        ...
        require(
            whitelistSpotsTotal <= maxWhitelistSpots,
            "Exceeds maxWhitelistSpots"
        );
    }
