...
contract S1MOO is ERC721Base, ERC721Reservable, ERC721Whitelist, ReentrancyGuard {
    ...
    uint256 public price = 0.9 ether;
    ...
    function mint(uint256 _amount) external payable {
        require(msg.value == price * _amount, "Value sent does not match price");
    ...
    function forwardFunds() external onlyOwner nonReentrant {
        ...
        paymentReceiver.transfer(available);
    }
    ...
    function _mintTo(address _to, uint256 _amount) internal override(ERC721Reservable) nonReentrant {
        require(mintActive, "Mint not started yet");
        require(_amount > 0 && _amount <= maxMintPerTx, "Invalid amount");
        require((totalSupply() + _amount + reservedOpen) <= maxSupply, "Exceeds maxSupply");
