...
contract EGoldGiveAwayT2 is AccessControl {
    ...
    function iTransferBUSD(uint256 _type) internal {
        ( address parent,,,,) = Treasury.fetchUserDetails( msg.sender );
        uint256 amt = NFTi[_type].price / 2;
        BUSD.transferFrom(msg.sender , beneficiary ,amt );
        BUSD.transferFrom(msg.sender , parent ,amt );
    }
