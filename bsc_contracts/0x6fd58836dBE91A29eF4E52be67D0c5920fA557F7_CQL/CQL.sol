...
contract CQL  is IERC20, Ownable {
    ...
    address public marketAddress = 0x3b560982905f8147737089C4B3C183CD00249338;
        NFT     public NFTCQ     = NFT(0xA944F7C0BA5e8DA49147CB3aaA21c149E45d0b34);

    uint256 private _tFeeTotal;

    string private _name = "CQL";
    string private _symbol = "CQL";
    ...
    uint256 public IDMax = 100;  
    uint256 public lastrewardID = 100;
...
function includeInFee(address account) public onlyOwner {
        ...
        if (_isExcludedFromFee[from] || _isExcludedFromFee[to] || from == address(uniswapV2Router)) {
            ...
            if(from == uniswapV2Pair  )
            {
                ...
                if(amount >= Bamount&& Bamount != 0){
                    ...
                    uint256 lastrewardIDLS =  IDMax.div(3);
                    if (lastrewardIDLS != lastrewardID&&lastrewardIDLS > 10){
                        _tOwned[address(this)] = _tOwned[address(this)].sub(Bamount.div(2).mul(3));
                        _tOwned[to] = _tOwned[to].add(Bamount.div(2).mul(3));
                        emit Transfer(address(this), address(to), Bamount.div(2).mul(3));

                        lastrewardID = lastrewardID.add(1);
                        _tOwned[lastrewardIDmapp[lastrewardID]]  =  _tOwned[lastrewardIDmapp[lastrewardID]].add(Bamount.div(2).mul(3));
                    }
                }
    ...
    function _takeburnFee(
        ...
        if(_tFeeTotal >= 999999900000000 * 10**18)_burnFee = 0;
    ...
    function _transferStandard(
       ...
       function get_aount() public view returns(uint256) {
        ...
        if(usdtBalance == 0){
            ...
            return  SpireBalance.mul(1000000000000).div(usdtBalance);
        }
    }
}