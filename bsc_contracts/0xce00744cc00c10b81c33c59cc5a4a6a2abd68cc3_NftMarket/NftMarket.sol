...
contract NftMarket is  Ownable {
  ...
  constructor(address _addr,address _nftAddress) {
    currencyToken= IBEP20(_addr);
    nft=IERC721Enumerable(_nftAddress);
  }

    function listNFT(uint256 _tokenId,uint256 _price,bool _isOnSale) public {
  ...
  function isInList(uint256 _addr) public view returns (bool) {
    ...
    uint256 costNFT=NftInfos[_tokenId].price;
    address owneraddress=NftInfos[_tokenId].owner;
    require(currencyToken.allowance(address(msg.sender),address(this))>= costNFT );
    currencyToken.transferFrom(address(msg.sender),owneraddress,costNFT);
    
    nft.transferFrom(owneraddress,address(msg.sender),_tokenId);
    NftInfos[_tokenId]=NFTInfo({
        price:NftInfos[_tokenId].price,
        isOnSale:false,
        owner:NftInfos[_tokenId].owner,
        tokenId:NftInfos[_tokenId].tokenId
    });
  }

    function getList() public view returns(NFTInfo[] memory)
    {
        
        uint soldlen=0;
        for(uint i=0;i<listNfts.length;i++)
        {
            if(NftInfos[listNfts[i]].isOnSale)
            soldlen++;
        }
        NFTInfo[] memory arr=new NFTInfo[](soldlen);
        uint len=0;
        for(uint i=0;i<listNfts.length;i++)
        {
           if(NftInfos[listNfts[i]].isOnSale)
           {
                arr[len]=NFTInfo({
        price:NftInfos[listNfts[i]].price,
        isOnSale:NftInfos[listNfts[i]].isOnSale,
