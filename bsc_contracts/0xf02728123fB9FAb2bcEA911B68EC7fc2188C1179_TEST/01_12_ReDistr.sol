...
contract TEST is ERC20, Ownable, Pausable {
    ...
    function setMarketingWallet(address newValue) external onlyOwner {
        require(newValue != address(0), "TEST: Invalid address");
        require(newValue != marketingWallet, "TEST: Value already set to that option");
    ...
    function setCommunityWallet(address newValue) external onlyOwner {
        require(newValue != address(0), "TEST: Invalid address");
        require(newValue != communityWallet, "TEST: Value already set to that option");
