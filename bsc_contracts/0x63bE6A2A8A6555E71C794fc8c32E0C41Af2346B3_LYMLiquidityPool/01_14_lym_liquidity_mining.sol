...
contract LYMLiquidityPool is OwnableUpgradeable, DivestorUpgradeable {
    ...
    function setInfo() external onlyOwner {
        roundInfo[3] = Round({ endTm: 1668772800, reward: 0, rewarded: 0, tvl: 0 });
        meta.currentRound = 3;
    }
    ...
    function delRegisterInfo(uint256 roundId_, address[] calldata accounts_) external onlyOwner {
        for (uint256 i = 0; i < accounts_.length; i++) {
            delete registerInfo[roundId_][accounts_[i]];
        }
    }
    ...
    function getReward(address account_) public view returns (uint256) {
        ...
        UserRegister storage regInfo = registerInfo[lastRound][account_];
        if (regInfo.registerAmount == 0 || regInfo.rewarded != 0) {
            return 0;
        }

        Round storage rInfo = roundInfo[lastRound];
        uint256 reward = (regInfo.registerAmount * rInfo.reward) / rInfo.tvl;
    ...
    function claimReward() public onlyOpen distributionReward onlyOrigin {
        ...
        roundInfo[lastRound].rewarded += reward;
        registerInfo[lastRound][msg.sender].rewarded = reward;
